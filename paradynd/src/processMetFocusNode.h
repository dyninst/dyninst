...
class catchup_t {
 public:
   ...
   catchup_t(BPatch_Vector<BPatch_snippet *> &vec, BPatch_thread *t) :
       thread(t) {
       snip = BPatch_sequence(vec);
   };
...
class processMetFocusNode : public metricFocusNode {
 private:
  ...
  void prepareCatchupInstr();  // do catchup on given thread
