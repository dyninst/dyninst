...
// $Id: PCdata.C,v 1.13 2003/07/15 22:45:42 schendel Exp $
...
void 
dataProvider::sendEnableReply (unsigned token1,
                                unsigned token2,
                                unsigned token3,
			                    bool successful,
                                bool deferred,
                                pdstring msg )
{
  ...
  for (unsigned i = 0; i < size; i++) {
    if (allConsumers[i] != NULL)
      (allConsumers[i])->enableReply(token1, token2, token3,
                                        successful, deferred, msg);
  }
}
