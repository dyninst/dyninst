...
class DYNDWARF_EXPORT DwarfFrameParser {
 public:
  ...
  bool getRegRepAtFrame(Address pc, MachRegister reg, VariableLocation &loc,
                        FrameErrors_t &err_result);

  bool getRegValueAtFrame(Address pc, MachRegister reg,
                          MachRegisterVal &reg_result, ProcessReader *reader,
                          FrameErrors_t &err_result);

  bool getRegAtFrame(Address pc, MachRegister reg, DwarfResult &cons,
                     FrameErrors_t &err_result);
  ...
  bool getRegsForFunction(Address entryPC, MachRegister reg,
                          std::vector<VariableLocation> &locs,
                          FrameErrors_t &err_result);

 private:
  bool getRegAtFrame_aux(Address pc, Dwarf_Fde fde, Dwarf_Half dwarf_reg,
                         MachRegister orig_reg, DwarfResult &cons,
                         Address &lowpc, FrameErrors_t &err_result);

  bool getFDE(Address pc, Dwarf_Fde &fde, Address &low, Address &high,
              FrameErrors_t &err_result);

  bool getDwarfReg(MachRegister reg, Dwarf_Fde &fde, Dwarf_Half &dwarf_reg,
                   FrameErrors_t &err_result);

  bool handleExpression(Address pc, Dwarf_Signed registerNum,
                        MachRegister origReg, Architecture arch,
