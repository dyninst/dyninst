...
std::map<DwarfFrameParser::frameParser_key, DwarfFrameParser::Ptr>
    DwarfFrameParser::frameParsers;
...
DwarfFrameParser::DwarfFrameParser(Dwarf_Debug dbg_, Architecture arch_)
    : dbg(dbg_), arch(arch_), fde_dwarf_status(dwarf_status_uninitialized) {}

DwarfFrameParser::~DwarfFrameParser() {
  if (fde_dwarf_status != dwarf_status_ok) return;
  for (unsigned i = 0; i < fde_data.size(); i++) {
    dwarf_fde_cie_list_dealloc(dbg, fde_data[i].cie_data, fde_data[i].cie_count,
...
bool DwarfFrameParser::getRegValueAtFrame(Address pc, Dyninst::MachRegister reg,
...
bool DwarfFrameParser::getRegRepAtFrame(Address pc, Dyninst::MachRegister reg,
...
bool DwarfFrameParser::getRegsForFunction(Address entryPC,
  ...
  while (worker > low) {
    ...
    SymbolicDwarfResult cons(loc, arch);
    Address next;
    if (!getRegAtFrame_aux(worker, fde, dwarf_reg, reg, cons, next,
                           err_result)) {
      assert(err_result != FE_No_Error);
...
bool DwarfFrameParser::getRegAtFrame(Address pc, Dyninst::MachRegister reg,
                                     DwarfResult &cons,
                                     FrameErrors_t &err_result) {
  err_result = FE_No_Error;
...
bool DwarfFrameParser::getRegAtFrame_aux(Address pc, Dwarf_Fde fde,
                                         Dwarf_Half dwarf_reg,
                                         MachRegister orig_reg,
                                         DwarfResult &cons, Address &lowpc,
                                         FrameErrors_t &err_result) {
  int result;
  Dwarf_Error err;
  ...
  if (dwarf_reg != DW_FRAME_CFA_COL3) {
    ...
    result = dwarf_get_fde_info_for_reg3(
        fde, dwarf_reg, pc, &value_type, &offset_relevant, &register_num,
        &offset_or_block_len, &block_ptr, &row_pc, &err);
  } else {
    dwarf_printf("\tcol3 reg, using CFA\n");
    result = dwarf_get_fde_info_for_cfa_reg3(
        fde, pc, &value_type, &offset_relevant, &register_num,
        &offset_or_block_len, &block_ptr, &row_pc, &err);
  }
  ...
  dwarf_printf(
      "\t Got FDE data starting at 0x%lx; value_type %d, offset_relevant %d, "
      "offset/block len %d\n",
  ...
  if (value_type == DW_EXPR_OFFSET || value_type == DW_EXPR_VAL_OFFSET) {
    ...
    if (!handleExpression(pc, register_num, orig_reg, arch, cons, done,
                          err_result)) {
  ...
  switch (value_type) {
    ...
    case DW_EXPR_EXPRESSION: {
      ...
      result = dwarf_loclist_from_expr(dbg, block_ptr, offset_or_block_len,
                                       &llbuf, &listlen, &err);
      if (result != DW_DLV_OK) {
      ...
      if (!decodeDwarfExpression(llbuf, NULL, cons, arch)) {
...
void DwarfFrameParser::setupFdeData() {
  ...
  fde_cie_data fc;
  int result = dwarf_get_fde_list(dbg, &fc.cie_data, &fc.cie_count,
                                  &fc.fde_data, &fc.fde_count, &err);
  if (result == DW_DLV_OK) {
    fde_data.push_back(fc);
  }

  result = dwarf_get_fde_list_eh(dbg, &fc.cie_data, &fc.cie_count, &fc.fde_data,
                                 &fc.fde_count, &err);
  if (result == DW_DLV_OK) {
    fde_data.push_back(fc);
  }
...
bool DwarfFrameParser::getFDE(Address pc, Dwarf_Fde &fde, Address &low,
                              Address &high, FrameErrors_t &err_result) {
  Dwarf_Addr lowpc = 0, hipc = 0;
  dwarf_printf("Getting FDE for 0x%lx\n", pc);
  bool found = false;
  unsigned cur_fde;
  for (cur_fde = 0; cur_fde < fde_data.size(); cur_fde++) {
    int result = dwarf_get_fde_at_pc(fde_data[cur_fde].fde_data, (Dwarf_Addr)pc,
                                     &fde, &lowpc, &hipc, NULL);
    if (result == DW_DLV_ERROR) {
...
bool DwarfFrameParser::getDwarfReg(Dyninst::MachRegister reg, Dwarf_Fde &fde,
                                   Dwarf_Half &dwarf_reg,
                                   FrameErrors_t &err_result) {
  Dwarf_Error err;
...
bool DwarfFrameParser::handleExpression(Address pc, Dwarf_Signed registerNum,
                                        Dyninst::MachRegister origReg,
                                        Dyninst::Architecture arch,
                                        DwarfResult &cons, bool &done,
                                        FrameErrors_t &err_result) {
  ...
  switch (registerNum) {
    case DW_FRAME_CFA_COL3:
      ...
      err_result = FE_No_Error;
      if (!getRegAtFrame(pc, Dyninst::FrameBase, cons, err_result)) {
        assert(err_result != FE_No_Error);
        return false;
      }
      break;
    ...
    default: {
      Dyninst::MachRegister dyn_register =
          MachRegister::DwarfEncToReg(registerNum, arch);
