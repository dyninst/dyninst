...
DwarfFrameParser::Ptr
DwarfFrameParser::create(Dwarf* dbg, Elf* eh_frame, Architecture arch)
{
    if(!dbg && !eh_frame)
        return NULL;
...
bool
DwarfFrameParser::hasFrameDebugInfo()
{
    setupCFIData();
...
bool
DwarfFrameParser::getRegValueAtFrame(Address pc, Dyninst::MachRegister reg,
                                     Dyninst::MachRegisterVal& reg_result,
                                     ProcessReader* reader, FrameErrors_t& err_result)
{
...
bool
DwarfFrameParser::getRegRepAtFrame(Address pc, Dyninst::MachRegister reg,
                                   VariableLocation& loc, FrameErrors_t& err_result)
{
...
bool
DwarfFrameParser::getRegsForFunction(std::pair<Address, Address>    range,
...
bool
DwarfFrameParser::getRegAtFrame(Address pc, Dyninst::MachRegister reg, DwarfResult& cons,
                                FrameErrors_t& err_result)
{
    err_result = FE_No_Error;
    ...
    for(size_t i = 0; i < cfi_data.size(); i++)
    {
        ...
        if(reg != Dyninst::ReturnAddr && reg != Dyninst::FrameBase && reg != Dyninst::CFA)
