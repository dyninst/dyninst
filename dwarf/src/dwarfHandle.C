...
void DwarfHandle::locate_dbg_file() {
  ...
  bool result =
      file->findDebugFile(filename, debug_filename, buffer, buffer_size);
  if (!result) return;
...
bool DwarfHandle::init_dbg() {
  ...
  status = dwarf_elf_init(file->e_elfp(), DW_DLC_READ, err_func, &file_data,
                          &file_data, &err);
  if (status != DW_DLV_OK) {
  ...
  if (dbg_file) {
    status = dwarf_elf_init(dbg_file->e_elfp(), DW_DLC_READ, err_func,
                            &dbg_file_data, &dbg_file_data, &err);
    if (status != DW_DLV_OK) {
      init_dwarf_status = dwarf_status_error;
      return false;
    }
...
bool DwarfHandle::hasFrameData(Elf_X *e) {
  ...
  if (!shstrtab.isValid()) return false;
...
Elf_X *DwarfHandle::origFile() { return file; }

Elf_X *DwarfHandle::debugLinkFile() { return dbg_file; }

Dwarf_Debug *DwarfHandle::line_dbg() {
  if (!init_dbg()) return NULL;
  return line_data;
}
...
DwarfHandle::~DwarfHandle() {
  if (init_dwarf_status != dwarf_status_ok) return;

  Dwarf_Error err;
  if (dbg_file_data) dwarf_finish(dbg_file_data, &err);
  if (file_data) dwarf_finish(file_data, &err);
}
...
DwarfHandle::ptr DwarfHandle::createDwarfHandle(string filename_, Elf_X *file_,
  ...
  DwarfHandle::ptr ret =
      DwarfHandle::ptr(new DwarfHandle(filename_, file_, err_func_));
  all_dwarf_handles.insert(make_pair(filename_, ret));
  return ret;
}

DwarfFrameParserPtr DwarfHandle::frameParser() { return sw; }
