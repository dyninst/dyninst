...
bool decodeDwarfExpression(Dwarf_Locdesc *dwlocs, long int *initialStackValue,
                           VariableLocation &loc, Dyninst::Architecture arch) {
  SymbolicDwarfResult res(loc, arch);
  if (!decodeDwarfExpression(dwlocs, initialStackValue, res, arch))
    return false;
  res.val();
  return true;
}
...
bool decodeDwarfExpression(Dwarf_Locdesc *dwlocs, long int *initialStackValue,
                           ProcessReader *reader, Address pc, Dwarf_Debug dbg,
...
bool decodeDwarfExpression(Dwarf_Locdesc *dwlocs, long int *initialStackValue,
                           DwarfResult &cons, Dyninst::Architecture arch) {
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (DW_OP_lit0 <= locations[i].lr_atom &&
        locations[i].lr_atom <= DW_OP_lit31) {
      dwarf_printf("\t\t Pushing unsigned val 0x%lx\n",
                   locations[i].lr_atom - DW_OP_lit0);
      cons.pushUnsignedVal(
          (Dyninst::MachRegisterVal)(locations[i].lr_atom - DW_OP_lit0));
      continue;
    }

    /* reg0 - reg31: named registers (not their constants) */
    if (DW_OP_reg0 <= locations[i].lr_atom &&
        locations[i].lr_atom <= DW_OP_reg31) {
      dwarf_printf(
          "\t\t Pushing reg %s\n",
          MachRegister::DwarfEncToReg(locations[i].lr_atom - DW_OP_reg0, arch)
              .name()
              .c_str());
    ...
    if (DW_OP_breg0 <= locations[i].lr_atom &&
        locations[i].lr_atom <= DW_OP_breg31) {
      dwarf_printf(
          "\t\t Pushing reg %s + %d\n",
          MachRegister::DwarfEncToReg(locations[i].lr_atom - DW_OP_reg0, arch)
              .name()
      ...
      cons.readReg(MachRegister::DwarfEncToReg(
          locations[i].lr_atom - DW_OP_breg0, arch));
    ...
    switch (locations[i].lr_atom) {
      ...
      case DW_OP_bregx:
        dwarf_printf("\t\t Pushing reg %s + %d\n",
                     MachRegister::DwarfEncToReg(locations[i].lr_number, arch)
                         .name()
                         .c_str(),
      ...
      case DW_OP_call_frame_cfa:
        // This is a reference to the CFA as computed by stack walking
        // information.
      ...
      case DW_OP_skip: {
        ...
        if (bytes < 0) {
          for (j = i - 1; j >= 0; j--) {
            if (locations[j].lr_offset == target) {
              break;
            }
          } /* end search backward */
        } else {
