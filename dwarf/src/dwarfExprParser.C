                      ...
                      VariableLocation& loc, Dyninst::Architecture arch)
{
    ...
    if(!decodeDwarfExpression(expr, listlen, initialStackValue, res, arch))
        return false;
    res.val();
    return true;
}
    ...
    for(unsigned int i = 0; i < count; i++)
    {
        ...
        if(DW_OP_lit0 <= locations[i].atom && locations[i].atom <= DW_OP_lit31)
        {
            ...
            cons.pushUnsignedVal(
                (Dyninst::MachRegisterVal)(locations[i].atom - DW_OP_lit0));
            continue;
        }
        ...
        if(DW_OP_reg0 <= locations[i].atom && locations[i].atom <= DW_OP_reg31)
        {
            dwarf_printf("\t\t Pushing reg %s\n",
                         MachRegister::DwarfEncToReg(locations[i].atom - DW_OP_reg0, arch)
                             .name()
                             .c_str());
        ...
        if(DW_OP_breg0 <= locations[i].atom && locations[i].atom <= DW_OP_breg31)
        {
            dwarf_printf("\t\t Pushing reg %s + %lu\n",
                         ...
                         locations[i].number);
            cons.readReg(
                MachRegister::DwarfEncToReg(locations[i].atom - DW_OP_breg0, arch));
        ...
        switch(locations[i].atom)
        {
            ...
            case DW_OP_bregx:
                dwarf_printf(
                    "\t\t Pushing reg %s + %lu\n",
                    MachRegister::DwarfEncToReg(locations[i].number, arch).name().c_str(),
                    locations[i].number2);
            ...
            case DW_OP_regx:
                dwarf_printf("\t\t Pushing reg %s\n",
                             MachRegister::DwarfEncToReg(locations[i].number, arch)
                                 .name()
                                 .c_str());
            ...
            case DW_OP_call_frame_cfa:
                ...
                // information. The current order is: Variable: reference frame base
                // (fbreg, above) Frame base: reference CFA CFA: offset from stack pointer
