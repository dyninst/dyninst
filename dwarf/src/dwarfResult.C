...
void
SymbolicDwarfResult::readReg(MachRegister reg)
{
    dwarf_printf("\t\t Read %s\n", reg.name().c_str());
...
void
SymbolicDwarfResult::pushUnsignedVal(MachRegisterVal val)
{
...
void
SymbolicDwarfResult::pushSignedVal(MachRegisterVal val)
{
    dwarf_printf("\t\t Push 0x%lx\n", val);
...
void
SymbolicDwarfResult::pushOp(Operator op)
{
...
void
SymbolicDwarfResult::pushOp(Operator op, long long u)
{
...
void
SymbolicDwarfResult::pushFrameBase()
{
...
void
ConcreteDwarfResult::readReg(MachRegister reg)
{
    Dyninst::MachRegisterVal v;
    if(!reader->GetReg(reg, v))
        error = true;
    push(v);
...
void
ConcreteDwarfResult::pushUnsignedVal(MachRegisterVal v)
{
...
void
ConcreteDwarfResult::pushSignedVal(MachRegisterVal v)
{
    // Someday this will matter...
    push(v);
...
void
ConcreteDwarfResult::pushOp(Operator op)
{
    MachRegisterVal v;
...
void
ConcreteDwarfResult::pushOp(Operator op, long long ref)
{
    switch(op)
    {
        case Add:
        ...
        case Deref: {
            ...
            switch(ref)
            {
                case 1: {
                    unsigned char c;
                    if(!reader->ReadMem(peek(0), &c, sizeof(c)))
                        error = true;
                    v = c;
                    break;
                }
                case 2: {
                    unsigned short s;
                    if(!reader->ReadMem(peek(0), &s, sizeof(s)))
                        error = true;
                    v = s;
                    break;
                }
                case 4: {
                    uint32_t u;
                    if(!reader->ReadMem(peek(0), &u, sizeof(u)))
                        error = true;
                    v = u;
                    break;
                }
...
void
ConcreteDwarfResult::pushFrameBase()
{
    error = true;
}

void
ConcreteDwarfResult::pushCFA()
{
...
void
ConcreteDwarfResult::push(MachRegisterVal v)
{
    operands.push_back(v);
}

bool
ConcreteDwarfResult::eval(MachRegisterVal& v)
{
    if(err())
        return false;
    v = val();
