...
enum DynamicNotifyMode {
  DynamicNotifyDLoader = 0,  //!< On job start, interrupt executable at start of
                             //!the dynamic linker.
  DynamicNotifyStart  //!< On job start, interrupt executable at start of the
                      //!application.
};
...
enum DACTrapMode {
  DACTrap_NoChange =
      0,        //!< Do not modify the current DAC Trap Mode preference.
  TrapOnDAC,    //!< Data address compare interrupts will be presented on the
                //!instruction that attempted to access the data.
  TrapAfterDAC  //!< Data address compare interrupts will be presented on the
                //!instruction following the data access.
};

//! Notify message types

enum NotifyMessageType {
  NotifyMessageType_Signal,  //!< Notify message is due to the occurrence of a
                             //!signal.
  NotifyMessageType_Termination,  //!< Notify message is due to process
                                  //!termination.
  NotifyMessageType_Control  //!< Notify message is due to a change or conflict
                             //!in control authority.
};
...
enum NotifySignalReason {
  ...
  NotifySignal_Breakpoint,       //!< Notification due to a breakpoint trap
                                 //!condition.
  NotifySignal_WatchpointRead,   //!< Notification due to a read watchpoint
                                 //!match.
  NotifySignal_WatchpointWrite,  //!< Notification due to a write watchpoint
                                 //!match.
  NotifySignal_StepComplete,     //!< Notification due to a step request
                                 //!completion.
};
...
enum NotifyControlReason {
  NotifyControl_Conflict,  //!< Notification due to another tool requesting
                           //!control authority.
  NotifyControl_Available  //!< Notification due to a tool relinquishing control
                           //!authority.
};
...
enum ReleaseControlNotify {
  ReleaseControlNotify_Inactive,  //!< Do not send an Available notify message
                                  //!when the next tool releases control.
  ReleaseControlNotify_Active  //!< Send Available notify message when next tool
                               //!releases control.
};
...
enum SpecAccess {
  SpecAccess_UseThreadState = 0,  //!< Read and write operations will use the
                                  //!current state of the thread.
  SpecAccess_ForceNonSpeculative  //!< Read and write operations will be forced
...
enum SharedMemoryAccess {
  SharedMemoryAccess_NotAllow =
      0,  //!< Do not allow shared process memory to be operated on.
  SharedMemoryAccess_Allow  //!< Allow shared process memory to be operatated
                            //!on.
};
...
enum FastBreakMode {
  FastBreak_NoChange =
      0,              //!< Do not modify the current FastTrapMode preference.
...
enum CmdReturnCode {
  ...
  CmdTIDinval = 1,  //!< The Thread ID specified no longer corresponds to an
                    //!active thread in this process or the thread is not in the
                    //!expected state.
  CmdInval = 2,     //!< The command is not recognized.
  CmdTimeout = 3,   //!< Timeout waiting for the target thread to respond.
  CmdAllocErr =
      4,  //!< Could not allocate necessary memory in the outbound message area.
  CmdParmErr = 5,  //!< An invalid paramter was supplied with the command.
  CmdBrkptFail =
      6,  //!< A failure occurred while attempting to set or clear a breakpoint.
  ...
  CmdMemUnavail =
      10,  //!< The requested memory allocation could not be satified.
  CmdFileNotFound = 11,  //!< The requested filename was not found.
  CmdConflict =
      12,  //!< The command conflicts with other commands in the command list.
  CmdPendingNotify = 13  //!< The release control command could not complete
                         //!because of a pending signal notify that has not been
                         //!satisfied.
};
...
enum BG_Thread_ToolState {
  ...
  Hold,  //!< The thread is in the Hold state. A ReleaseThread will remove the
         //!hold.
  Suspend,  //!< Signal delivery on a thread in the process has suspended this
            //!thread.
...
enum BG_Thread_SpecState {
  ...
  TransactionalMemory_Invalid,  //!< Thread is running in transactional memory
                                //!mode, but the specid is invalid.
  //!< Read operations return non-speculative storage, write operations are
  //!discarded.
  SpeculativeExecution,  //!< Thread is running in speculative execution mode.
  SpeculativeExecution_Invalid  //!< Thread is running in speculative execution
                                //!mode, but the specid is invalid.
  //!< Read operations return non-speculative storage, write operations are
  //!discarded.
};
...
struct AttachMessage : public ToolMessage {
 public:
  char toolTag[ToolTagSize];  //!< Character string identifier provided by the
                              //!calling tool.
  uint8_t priority;          //!< Priority level for delivery of Notify message.
  ProcessSelect procSelect;  //!< Attach to either all processes in the node or
                             //!one process in the node
};
...
class ControlMessage : public ToolMessage {
 public:
  BG_Sigset_t notifySet;  //!< Set of signals that tool is notified of when
                          //!encountered. Use notifySignalSet(sigset_t*) to
                          //!initialize
  uint32_t sndSignal;     //!< Signal, if any, to be sent to the process leader
                          //!after control authority has been granted (e.g.
                          //!SIGTRAP, SIGSTOP)
  DynamicNotifyMode
      dynamicNotifyMode;    //!< Dynamic application attachment notify mode.
...
struct ControlAckMessage : public ToolMessage {
  ...
  char
      toolTag[ToolTagSize];  //!< ToolTag of the tool that has control authority
...
struct DetachMessage : public ToolMessage {
  ProcessSelect procSelect;  //!< Detach to either all processes in the node or
                             //!one process in the node
};
...
struct QueryMessage : public ToolMessage {
  ...
  struct CommandDescriptor
      cmdList[MaxQueryCommands];  //!< List of command descriptors.
};
...
struct UpdateMessage : public ToolMessage {
  uint16_t numCommands;  //!< Number of commands in message.
  struct CommandDescriptor
      cmdList[MaxUpdateCommands];  //!< List of command descriptors.
};
...
struct SetupJobMessage : public ToolMessage {
  ...
  uint32_t ranks[MaxRanksPerNode];  //!< Rank of each process running on compute
                                    //!node.
  uid_t userId;                     //!< User id.
  gid_t groupId;                    //!< Primary group id.
  uint32_t nodeId;  //!< Node id used to name symbolic link in toolctl_node
                    //!directory.
...
//! Message to notify of an event within the compute node change of a compute
//! node process.

struct NotifyMessage : public ToolMessage {
  NotifyMessageType
      notifyMessageType;  //!< Which type of notify message is this.
  union {
    struct {                   //!< Definition for NotifyMessageType_Signal.
      uint32_t signum;         //!< Signal number.
      BG_ThreadID_t threadID;  //!< Thread id.
      BG_Addr_t instAddress;   //!< Address of interrupted instruction (signal
                               //!notification).
      BG_Addr_t dataAddress;   //!< Address of the data access causing a
                               //!watchpoint event.
      NotifySignalReason reason;  //!< Signal Notification reason.
    } signal;
    struct {           //!< Definition for NotifyMessageType_Termination.
      int exitStatus;  //!< Process exit status. Byte2: exit return code Byte3:
    ...
    struct {                      //!< Definition for NotifyMessageType_Control.
      ...
      NotifyControlReason reason;  //!< Control Authority notification reason:
                                   //!Conflict or Available.
    } control;
  } type;
};
...
struct GetMemoryAckCmd : public ToolCommand {
  ...
  uint32_t length;        //!< Length of memory in bytes.
  unsigned char data[0];  //!< Memory contents. Variable size data area up to
                          //!MaxMemorySize.
};
...
struct GetProcessDataAckCmd : public ToolCommand {
  ...
  BG_Addr_t
      persistMemoryStartAddr;      //!< Persistent memory region start address.
...
//! Command to set memory. To write a memory breakpoint, a separate command is
//! provided.
...
struct SetMemoryCmd : public ToolCommand {
  ...
  SpecAccess specAccess;  //!< Should speculative access be permitted.
  SharedMemoryAccess
      sharedMemoryAccess;  //!< Should Shared process data writes be allowed.
  unsigned char data[0];   //!< Memory contents. Variable size data area up to
                           //!MaxMemorySize
};
...
struct InstallTrapHandlerCmd : public ToolCommand {
  void (*trap_handler)(int, siginfo_t *,
                       void *);  //!< Signal handler function for SIGTRAP
};
...
//! a watch of an 8 byte area must have the address starting on an 8 byte
//! boundary.
//!
...
struct SetPreferencesCmd : public ToolCommand {
  ...
  DACTrapMode dacMode;  //!< Data address compare interrupt delivery on/after
...
struct ReleaseControlCmd : public ToolCommand {
  ReleaseControlNotify notify;  //!< Controls the sending of the next Notify
                                //!Available message to this tool.
};
