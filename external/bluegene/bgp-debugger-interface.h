...
/*
   ...
   3 - Added GET_STACK_TRACE and END_DEBUG messages, Signal for detach from
   nodes
   4-  Added GET_PROCESS_DATA and GET_THREAD_DATA messages
   5 - Added HOLD_THREAD, RELEASE_THREAD, SIGACTION, MAP_MEM and FAST_TRAP
   messages
...
typedef struct { BG_FPR_t fprs[32]; } BG_FPRSet_t;
...
typedef struct {
  ...
  uint32_t
      numStackFrames;  // Number of stack frame addr/saved lr entries returned
...
#define BG_Debugger_Msg_MAX_PAYLOAD_SIZE \
  (BG_Debugger_Msg_MAX_SIZE - BG_Debugger_Msg_HEADER_SIZE)
...
class BG_Debugger_Msg {
 public:
  ...
  typedef union {
    ...
    struct {
      uint32_t auxVecBufferOffset;  // Requested offset within the available
                                    // AuxVec data to begin retrieval
    ...
    struct {
      ...
      uint32_t auxVecBufferOffset;  // Offset within the available AuxVec data
                                    // of retrieved data
      uint32_t auxVecBufferLength;  // Number of bytes of aux vec data stored
                                    // within the auxVecData buffer
      bool endOfVecData;  // Indicator set to true if the end of the AuxVec data
                          // was reached
    } GET_AUX_VECTORS_ACK;
    ...
    struct {
      ...
      uint32_t numStackFrames;  // Number of stack frame addr/saved lr entries
                                // returned
  ...
  BG_Debugger_Msg(BG_MsgType_t type, BG_NodeNum_t node, BG_ThreadID_t thread,
                  uint32_t sequence, uint32_t returnCode) {
  ...
  static BG_Debugger_Msg generateErrorPacket(BG_Debugger_Msg &original,
                                             BG_ErrorCode_t ec) {
    ...
    switch (original.header.messageType) {
      case GET_REG: {
        ...
        errMsg.dataArea.GET_REG_ACK.registerNumber =
            original.dataArea.GET_REG.registerNumber;
      ...
      case SET_REG: {
        ...
        errMsg.dataArea.SET_REG_ACK.registerNumber =
            original.dataArea.SET_REG.registerNumber;
        break;
      }

      case GET_MEM: {
        errMsg.header.messageType = GET_MEM_ACK;
        errMsg.header.dataLength =
            sizeof(errMsg.dataArea.GET_MEM_ACK) - BG_Debugger_Msg_MAX_MEM_SIZE;
      ...
      case GET_FLOAT_REG: {
        ...
        errMsg.dataArea.GET_FLOAT_REG_ACK.registerNumber =
            original.dataArea.GET_FLOAT_REG.registerNumber;
      ...
      case SET_FLOAT_REG: {
        ...
        errMsg.dataArea.SET_FLOAT_REG_ACK.registerNumber =
            original.dataArea.SET_FLOAT_REG.registerNumber;
        break;
      }
  ...
  //! \return True when message was read successfully, false if there was an
  //! error.
  ...
  //! \param  fd Descriptor to read from.
  //! \param  msg Reference to debugger message.
  //! \param  abortPipeIO Pointer to integer that indicates whether to abort
  //! read on error.
  //! \return True when message was read successfully, false if there was an
  //! error.
  ...
  //! \param  msg Reference to debugger message.
  //! \return True when message was written successfully, false if there was an
  //! error.
  ...
  static const char *getMessageName(BG_MsgType_t type) {
    static const char *BG_Packet_Names[] = {"GET_REG",
                                            "GET_ALL_REGS",
                                            "SET_REG",
                                            "GET_MEM",
                                            "SET_MEM",
  ...
  static void dump(BG_Debugger_Msg &msg) { dump(msg, stdout); }
  ...
  static void dump(BG_Debugger_Msg &msg, FILE *outfile) {
    ...
    fprintf(outfile, "Type: %s from node: %d, return code: %d\n",
            getMessageName(msg.header.messageType), msg.header.nodeNumber,
    ...
    switch (msg.header.messageType) {
      ...
      case GET_REG_ACK: {
        fprintf(outfile, "  Register number: %d    Value: %08x\n",
                msg.dataArea.GET_REG.registerNumber,
                msg.dataArea.GET_REG_ACK.value);
        break;
      }
      ...
      case SET_REG: {
        fprintf(outfile, "  Register number: %d    Value: %08x\n",
                msg.dataArea.SET_REG.registerNumber,
                msg.dataArea.SET_REG.value);
        break;
      }
      ...
      case GET_MEM: {
        fprintf(outfile, "  Memory address: %08x    Length: %d\n  Vals: ",
                msg.dataArea.GET_MEM.addr, msg.dataArea.GET_MEM.len);
        break;
      }

      case GET_MEM_ACK: {
        fprintf(outfile, "  Memory address: %08x    Length: %d\n  Vals: ",
                msg.dataArea.GET_MEM_ACK.addr, msg.dataArea.GET_MEM_ACK.len);
        for (unsigned int i = 0; i < msg.dataArea.GET_MEM_ACK.len; i++)
          fprintf(outfile, "%02x ", msg.dataArea.GET_MEM_ACK.data[i]);
        fprintf(outfile, "\n");
        break;
      }
      ...
      case SET_MEM: {
        fprintf(outfile, "  Memory address: %08x    Length: %d\n  Vals: ",
                msg.dataArea.SET_MEM.addr, msg.dataArea.SET_MEM.len);
        for (unsigned int i = 0; i < msg.dataArea.SET_MEM.len; i++)
          fprintf(outfile, "%02x ", msg.dataArea.SET_MEM.data[i]);
        fprintf(outfile, "\n");
        break;
      }
      ...
      case PROGRAM_EXITED: {
        fprintf(outfile, "  Exit=0,Signal=1: %d   Return code: %d\n",
                msg.dataArea.PROGRAM_EXITED.type,
                msg.dataArea.PROGRAM_EXITED.rc);
        break;
      }
      ...
      case VERSION_MSG_ACK: {
        fprintf(outfile,
                "  Protocol version: %u   Physical Processors: %u  Logical "
                "Processors: %u\n",
      ...
      case GET_THREAD_INFO_ACK: {
        fprintf(outfile, "  Number of threads: %u  TIDs:",
                msg.dataArea.GET_THREAD_INFO_ACK.numThreads);
        for (unsigned int i = 0;
             i < msg.dataArea.GET_THREAD_INFO_ACK.numThreads; i++) {
          fprintf(outfile, " %u",
                  msg.dataArea.GET_THREAD_INFO_ACK.threadIDS[i]);
        }
        fprintf(outfile, "\n");
        break;
      }
      ...
      case SET_THREAD_OPS: {
        fprintf(outfile, "  TID: %u  Operation: %d\n",
                msg.dataArea.SET_THREAD_OPS.tid,
                msg.dataArea.SET_THREAD_OPS.operation);
        break;
      }
      ...
      case GET_AUX_VECTORS: {
        fprintf(outfile, "  Offset: %08x    Requested Length: %d\n  Vals: ",
                msg.dataArea.GET_AUX_VECTORS.auxVecBufferOffset,
      ...
      case GET_AUX_VECTORS_ACK: {
        fprintf(outfile, "  Offset: %08x    Length: %d\n  Vals: ",
                msg.dataArea.GET_AUX_VECTORS_ACK.auxVecBufferOffset,
                msg.dataArea.GET_AUX_VECTORS_ACK.auxVecBufferLength);
        for (unsigned int i = 0;
             i < msg.dataArea.GET_AUX_VECTORS_ACK.auxVecBufferLength; i++)
          fprintf(outfile, "%02x ",
                  msg.dataArea.GET_AUX_VECTORS_ACK.auxVecData[i]);
        fprintf(outfile, "\n");
        break;
      }
      ...
      case GET_STACK_TRACE_ACK: {
        fprintf(outfile,
                "  LR: %08x  IAR: %08x  R1: %08x  Number of stack frames: %u\n",
                msg.dataArea.GET_STACK_INFO_ACK.lr,
                msg.dataArea.GET_STACK_INFO_ACK.iar,
                msg.dataArea.GET_STACK_INFO_ACK.stackFrame,
                msg.dataArea.GET_STACK_INFO_ACK.numStackFrames);
        for (unsigned int i = 0;
             i < msg.dataArea.GET_STACK_INFO_ACK.numStackFrames; i++) {
          fprintf(outfile, "  Frame address: %08x  Saved LR: %08x\n",
                  msg.dataArea.GET_STACK_INFO_ACK.stackInfo[i].frameAddr,
                  msg.dataArea.GET_STACK_INFO_ACK.stackInfo[i].savedLR);
        }
        break;
      }
      ...
      case GET_PROCESS_DATA_ACK: {
        fprintf(outfile,
                "  Rank: %u  TGID: %u  xCoord: %u  yCoord: %u  zCoord: %u  "
                "tCoord: %u\n",
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.rank,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.tgid,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.xCoord,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.yCoord,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.zCoord,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.tCoord);
        fprintf(
            outfile,
            "  Shared memory start: %08x  Shared memory end: %08x  Persistent "
            "memory start: %08x  Persistent memory end: %08x\n",
            msg.dataArea.GET_PROCESS_DATA_ACK.processData.sharedMemoryStartAddr,
            msg.dataArea.GET_PROCESS_DATA_ACK.processData.sharedMemoryEndAddr,
            msg.dataArea.GET_PROCESS_DATA_ACK.processData
                .persistMemoryStartAddr,
            msg.dataArea.GET_PROCESS_DATA_ACK.processData.persistMemoryEndAddr);
        fprintf(outfile,
                "  Heap start: %08x  Heap end: %08x  Heap break: %08x  Mmap "
                "start: %08x  Mmap end: %08x\n",
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.heapStartAddr,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.heapEndAddr,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.heapBreakAddr,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.mmapStartAddr,
                msg.dataArea.GET_PROCESS_DATA_ACK.processData.mmapEndAddr);
        fprintf(outfile, "  Number of threads: %u  TIDs:",
                msg.dataArea.GET_PROCESS_DATA_ACK.numThreads);
        for (unsigned int i = 0;
             i < msg.dataArea.GET_PROCESS_DATA_ACK.numThreads; i++) {
          fprintf(outfile, " %u",
                  msg.dataArea.GET_PROCESS_DATA_ACK.threadIDS[i]);
        }
        fprintf(outfile, "\n");
        break;
      }
      ...
      case GET_THREAD_DATA_ACK: {
        fprintf(outfile,
                "  TID: %u  Core: %d  Stack start: %08x  Stack end: %08x  "
                "Guard start: %08x  Guard end: %08x\n",
                msg.dataArea.GET_THREAD_DATA_ACK.threadData.threadID,
                msg.dataArea.GET_THREAD_DATA_ACK.threadData.core,
                msg.dataArea.GET_THREAD_DATA_ACK.threadData.stackStartAddr,
                msg.dataArea.GET_THREAD_DATA_ACK.threadData.stackEndAddr,
                msg.dataArea.GET_THREAD_DATA_ACK.threadData.guardStartAddr,
                msg.dataArea.GET_THREAD_DATA_ACK.threadData.guardEndAddr);
        ...
        dumpDebugSet(
            &msg.dataArea.GET_THREAD_DATA_ACK.threadData.debugRegisters,
            outfile);
        for (unsigned int i = 0;
             i < msg.dataArea.GET_THREAD_DATA_ACK.threadData.numStackFrames;
             i++) {
          fprintf(
              outfile, "  Frame address: %08x  Saved LR: %08x\n",
              msg.dataArea.GET_THREAD_DATA_ACK.threadData.stackInfo[i]
                  .frameAddr,
              msg.dataArea.GET_THREAD_DATA_ACK.threadData.stackInfo[i].savedLR);
        }
        break;
      }
      ...
      case SIGACTION: {
        fprintf(outfile, "Signum: %08x  flags: %08x mask: %08x\n",
                msg.dataArea.SIGACTION.signum, msg.dataArea.SIGACTION.flags,
                msg.dataArea.SIGACTION.mask);
        break;
      }
 ...
 private:
  ...
  //! \param  abortPipeIO Pointer to integer that indicates whether to abort
  //! read on error.
  //! \return True when message was read succesfully, false if there was an
  //! error.

  static bool readFromFd_p(int fd, BG_Debugger_Msg &msg,
                           volatile int *abortPipeIO) {
    // Read header first
    ...
    while (headerBytesRead < headerBytesToRead) {
      ...
      int headerRc = read(fd, ((unsigned char *)&msg.header) + headerBytesRead,
                          headerBytesToRead - headerBytesRead);
    ...
    while (payloadBytesRead < msg.header.dataLength) {
      ...
      int payloadRc =
          read(fd, ((unsigned char *)&msg.dataArea) + payloadBytesRead,
               msg.header.dataLength - payloadBytesRead);
  ...
  static bool writeOnFd_p(int fd, BG_Debugger_Msg &msg,
                          volatile int *abortPipeIO) {
    ...
    while (bytesWritten < bytesToWrite) {
      ...
      int writeRc = write(fd, ((unsigned char *)&msg) + bytesWritten,
  ...
  static void dumpGPRSet(BG_GPRSet_t *gprs, FILE *outfile) {
    ...
    fprintf(outfile,
            "FPSCR: %08x       LR: %08x       CR: %08x      XER: %08x\n",
            gprs->fpscr, gprs->lr, gprs->cr, gprs->xer);
    fprintf(outfile,
            "  CTR: %08x      IAR: %08x      MSR: %08x     DEAR: %08x\n",
  ...
  static void dumpDebugSet(BG_DebugSet_t *debugRegisters, FILE *outfile) {
    fprintf(outfile,
            "  DBCR0: %08x   DBCR1: %08x   DBCR2: %08x    DBSR: %08x\n",
            debugRegisters->DBCR0, debugRegisters->DBCR1, debugRegisters->DBCR2,
            debugRegisters->DBSR);
    fprintf(outfile,
            "   IAC1: %08x    IAC2: %08x    IAC3: %08x    IAC4: %08x\n",
            debugRegisters->IAC1, debugRegisters->IAC2, debugRegisters->IAC3,
            debugRegisters->IAC4);
    fprintf(outfile,
            "   DAC1: %08x    DAC2: %08x    DVC1: %08x    DVC2: %08x\n",
            debugRegisters->DAC1, debugRegisters->DAC2, debugRegisters->DVC1,
            debugRegisters->DVC2);
    return;
  }
};
