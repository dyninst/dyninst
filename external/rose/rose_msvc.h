...
// (it is initial work with no clear idea if MSVC will be supported in the
// future).
...
// Some versions of the Watcom compiler fail to set _WIN32.  Set ROSE_WIN32 when
// running the Watcom compiler on NT.
#define ROSE_WIN32 1
#endif
#endif
...
// DQ (3/22/2009): This is defined in <linux/limits.h>, for MS we pick a value
// consistatn with MKS.
// Expanded API PATH_MAX. The size of PATH_MAX used by the MKS Toolkit UNIX APIs
// has been increased
...
// DQ (11/27/2009): Needed to cope with bug in MS library: it fails to define
// min/max
// This solution is from the web at:
// http://www.codeproject.com/Messages/3178857/Overcoming-problem-with-std-min-std-max-sharpdefin.aspx
// And then I simplified it to just undefine the min and max macros (which seems
...
// DQ (12/28/2009): Moved this from where is was placed by Thomas in the ROSETTA
// grenerated code.
// This simplifies the generated code to support splitting large generated files
// into smaller files.
