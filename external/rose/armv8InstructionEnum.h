...
enum ARMv8RegisterClass {
  ...
  armv8_regclass_pstate,   /** Minor is the only one pstate register with its
                              flags (defined in ARMv8Flag) being the only relevant
                              fields */
...
*                                                 negative (bit 31), zero (bit
  * 30), carry (bit 29) and overflow (bit 28) flags.
*                                                 These are not added are distinct
  * flags because the four bits are almost always accessed together. */
};

/** ARMv8-A instructions for the AArch64 processor mode
 *  The enum entries are generated by the 'aarch64_manual_parser.py' script in
 * the instructionAPI directory under Dyninst's source tree.*/
