...
int main() {
   ...
   * IA-32 emitter
   *
   * The 32-bit emitter is compiled into the x86_64 build, so its codegen can be
   * checked here. Unlike the 64-bit emitter, it operates on *virtual* registers
   * that the register space maps onto the eight real registers on demand, so
   * each case first allocates its operands and pins them to a real register the
   * way a preceding snippet operation would have left them.
   */
  auto *rs32 = registerSpace::getRegisterSpace(4);
  // A register space is normally specialized from an instrumentation point's
  // liveness before use. There is no public entry point for a bare 32-bit space,
  // so mark the virtual registers dead (i.e., allocatable) directly.
  for(auto *slot : rs32->GPRs()) {
    slot->liveState = registerSlot::dead;
  }
  gen.setRegisterSpace(rs32);
  Dyninst::DyninstAPI::EmitterIA32Stat emitter32{};
  ...
  {
    auto const dividend = rs32->allocateRegister(gen);
    auto const divisor = rs32->allocateRegister(gen);
    auto const quotient = rs32->allocateRegister(gen);
    rs32->noteVirtualInReal(dividend, RealRegister(REGNUM_EAX));
    rs32->noteVirtualInReal(divisor, RealRegister(REGNUM_EBX));
    emitter32.emitDiv(quotient, dividend, divisor, gen, true);
    rs32->freeRegister(divisor);
    rs32->freeRegister(quotient);
    failed |= !verify_emitter(gen, emitter_buffer_t<3>{{
      0x99,       // cdq
      0xf7, 0xfb  // idiv ebx
    }});
  ...
  // sign-extended (cdq would make a top-bit-set dividend overflow the quotient).
  {
    ...
    rs32->noteVirtualInReal(dividend, RealRegister(REGNUM_EAX));
    rs32->noteVirtualInReal(divisor, RealRegister(REGNUM_EBX));
    emitter32.emitDiv(quotient, dividend, divisor, gen, false);
    rs32->freeRegister(divisor);
    rs32->freeRegister(quotient);
    failed |= !verify_emitter(gen, emitter_buffer_t<7>{{
  ...
  // Signed divide with the dividend in ecx: it is moved into eax first
  {
    ...
    auto const divisor = rs32->allocateRegister(gen);
    auto const quotient = rs32->allocateRegister(gen);
    rs32->noteVirtualInReal(dividend, RealRegister(REGNUM_ECX));
    rs32->noteVirtualInReal(divisor, RealRegister(REGNUM_EBX));
    emitter32.emitDiv(quotient, dividend, divisor, gen, true);
    rs32->freeRegister(divisor);
    rs32->freeRegister(quotient);
