...
std::vector<call_test> make_tests(Dyninst::Architecture arch) {
  ...
  return {
    { // push eax
      {0x50},
      di::opcode_test(e_push, "push %"+eax.name()),
      di::register_rw_test{
        reg_set {sp, eax},
        reg_set {sp}
      },
    ...
    { // pop eax
      {0x58},
      di::opcode_test(e_pop, "pop %"+eax.name()),
      di::register_rw_test{
        reg_set {sp},
        reg_set {sp, eax}
      },
    ...
    { // pushf
      {0x9c},
      di::opcode_test(is_64?e_pushfq:e_pushfd, "pushf"+std::string{is_64?"q":"d"}),
      di::register_rw_test{
        reg_set {sp, flags},
        reg_set {sp}
      },
    ...
    { // popf
      {0x9d},
      di::opcode_test(is_64?e_popfq:e_popfd, "popf"+std::string{is_64?"q":"d"}),
      di::register_rw_test{
        reg_set {sp},
