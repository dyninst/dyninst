...
std::vector<data_transfer_test> make_tests(Dyninst::Architecture arch) {
  ...
  return {
    { // cmpxchg dword ptr [ebp + 0x12345678], esi
      ...
      di::opcode_test(e_cmpxchg, "cmpxchg %esi,0x12345678(%"+ebp.name()+")"),
      di::register_rw_test {
    ...
    { // cmova rax, rbp
      {0x0f, 0x47, 0xc5},
      di::opcode_test(e_cmova, "cmova %ebp,%eax"),
      di::register_rw_test {
    ...
    { // mov dword ptr [ebp - 4], 1
      ...
      di::opcode_test(e_mov, "mov $0x1,0xfffffffc(%"+ebp.name()+")"),
      di::register_rw_test {
        reg_set{ebp},
        reg_set{}
      },
    ...
    { // mov byte ptr [ebp - 0x34], dl
      {0x88, 0x55, 0xcc},
      di::opcode_test(e_mov, "mov %dl,0xffffffcc(%"+ebp.name()+")"),
      di::register_rw_test {
        reg_set{ebp, dl},
        reg_set{}
      },
    ...
    { // mov ax, gs
      {0x66, 0x8c, 0xe8},
      di::opcode_test(e_mov, "mov %gs,%ax"),
      di::register_rw_test {
        reg_set{gs},
        reg_set{ax16}
      },
    ...
    { // mov eax, ebp
      {0x89, 0xe8},
      di::opcode_test(e_mov, "mov %ebp,%eax"),
      di::register_rw_test {
        reg_set{bp32},
        reg_set{ax32}
      },
    ...
    { // lea eax, [ebx - 0xe8]
      ...
      di::opcode_test(e_lea, "lea 0xffffff18(%"+ebx.name()+"),%eax"),
      di::register_rw_test {
        reg_set{ebx},
        reg_set{ax32}
      },
...
std::vector<data_transfer_test> make_tests64() {
  ...
  return {
    { // mov dword ptr [rbp - 0x3c], r8d
      {0x44, 0x89, 0x45, 0xc4},
      di::opcode_test(e_mov, "mov %r8d,0xffffffc4(%rbp)"),
      di::register_rw_test {
        reg_set{rbp, r8d},
        reg_set{}
      },
    ...
    { // REX.W cmova rax, rbp
      {0x48, 0x0f, 0x47, 0xc5},
      di::opcode_test(e_cmova, "cmova %rbp,%rax"),
      di::register_rw_test {
    ...
    { // REX.W cmpxchg dword ptr [rbp + 0x12345678], rax
      ...
      di::opcode_test(e_cmpxchg, "cmpxchg %rax,0x12345678(%rbp)"),
      di::register_rw_test {
        reg_set{rax, rbp},
