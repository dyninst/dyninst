...
T_dyninstRPC::mdl_expr::mdl_expr( void )
{
    // nothing to do
}
...
bool T_dyninstRPC::mdl_v_expr::apply( void )
{
  switch (type_) 
  {
    case MDL_EXPR_INT: 
    ...
    case MDL_EXPR_BINOP:
    {
      ...
      if (!left_->apply()) return false;
      if (!right_->apply()) return false;
      ok_ =  true;
      return ok_;
    }
    case MDL_EXPR_FUNC:
    ...
    {
      mdl_var lvar;
      if (!mdl_data::cur_mdl_data->env->get(lvar, var_))
      {
        cerr << "variable '" << var_ 
...
T_dyninstRPC::mdl_icode::~mdl_icode( void )
{
    delete if_expr_; delete expr_;
}


bool T_dyninstRPC::mdl_icode::apply( void )
{
  if (!expr_) return false;
  if (!expr_->apply()) return false;
  if (if_expr_)
  {
    if (!if_expr_->apply()) return false;
  }
  return true;
}
...
T_dyninstRPC::mdl_stmt::mdl_stmt( void )
{
    // nothing else to do
}
...
bool T_dyninstRPC::mdl_list_stmt::apply( pdvector<const instrDataNode*>& )
{
  if (!elements_)
    return false;
  ...
  return (mdl_data::cur_mdl_data->env->add(id_, false, list_type));
}



//----------------------------------------------------------------------------
// mdl_for_stmt methods
//----------------------------------------------------------------------------
...
T_dyninstRPC::mdl_for_stmt::~mdl_for_stmt( void )
{
  delete for_body_;
  delete list_expr_;
}
...
bool T_dyninstRPC::mdl_for_stmt::apply(pdvector<const instrDataNode*>& flags)
{
  mdl_data::cur_mdl_data->env->push();

  if (!mdl_data::cur_mdl_data->env->add(index_name_, false)) return false;
  ...
  bool res = for_body_->apply(flags);
...
bool T_dyninstRPC::mdl_if_stmt::apply(pdvector<const instrDataNode*>& flags)
{
  ...
  bool ret = body_->apply(flags);
  return ret;
}

//----------------------------------------------------------------------------
// mdl_seq_stmt methods
//----------------------------------------------------------------------------
...
bool T_dyninstRPC::mdl_seq_stmt::apply(pdvector<const instrDataNode*>& flags)
{
  if (!stmts_)
    return true;
  unsigned size = stmts_->size();
  for (unsigned index=0; index<size; index++)
    if (!(*stmts_)[index]->apply(flags))
      return false;
  return true;
}
...
bool T_dyninstRPC::mdl_instr_stmt::apply( pdvector<const instrDataNode*>& )
{
  mdl_var temp(false);
  if (!icode_reqs_)
    return false;
...
bool T_dyninstRPC::mdl_constraint::apply( void )
{
  mdl_data::cur_mdl_data->env->push();
  ...
  if (!mdl_data::cur_mdl_data->env->add(id_, true, data_type_)) return false;
...
bool T_dyninstRPC::mdl_metric::apply( void )
{
  mdl_data::cur_mdl_data->env->push();
  if (!mdl_data::cur_mdl_data->env->add(id_, true, type_)) return false;
  assert(temp_ctr_);
  unsigned tc_size = temp_ctr_->size();
  for (unsigned tc=0; tc<tc_size; tc++)
    if (!mdl_data::cur_mdl_data->env->add((*temp_ctr_)[tc], true, MDL_T_COUNTER)) return false;
  ...
  for (unsigned u1=0; u1<size; u1++) {
    if ((*constraints_)[u1]->match_path_) {
      ...
      unsigned gl_size = mdl_data::cur_mdl_data->all_constraints.size();
      bool found=false;
      for (unsigned in=0; in<gl_size; in++)
        if (mdl_data::cur_mdl_data->all_constraints[in]->id_ == (*constraints_)[u1]->id_) {
          found = true; break;
        }
      if (!found) {
...
//  self.name = "Procedure"; self.type = MDL_T_STRING; self.end_allowed = true;
  ...
  mdl_data::cur_mdl_data->fields[MDL_T_VARIABLE] = field_list;
  field_list.resize(0);
...
bool mdl_check_node_constraints() {
	...
	if (bad_string) {
	    cout << "exclude syntax error : " 
            << mdl_data::cur_mdl_data->lib_constraints[i].c_str() 
            << " is not of expected form : /Code/module/function, or /Code/module" << endl;
	    return false;
	} 
    ...
    mdl_data::cur_mdl_data->lib_constraints = modified_lib_constraints;
    return true;
}
...
bool mdl_apply( void )
{
  ...
  //
  unsigned size = mdl_data::cur_mdl_data->stmts.size();
  pdvector<const instrDataNode*> flags;
  for (unsigned u=0; u<size; u++)
    if (!mdl_data::cur_mdl_data->stmts[u]->apply(flags))
      return false;
  ...
  size = mdl_data::cur_mdl_data->all_constraints.size();
  for (unsigned u1=0; u1<size; u1++) {
    bool applied = mdl_data::cur_mdl_data->all_constraints[u1]->apply();
    if ( applied ) {
      ok_cons += mdl_data::cur_mdl_data->all_constraints[u1];
      // cout << "constraint defined: " << mdl_data::all_constraints[u1]->id_ << endl;
    } else {
      cerr << "Error in constraint '" 
            << mdl_data::cur_mdl_data->all_constraints[u1]->id_ 
            << "'. Constraint deleted." << endl;
      delete mdl_data::cur_mdl_data->all_constraints[u1];
    }
  }
  mdl_data::cur_mdl_data->all_constraints = ok_cons;
  ...
  for (unsigned u2=0; u2<size; u2++) {
    ...
    if (mdl_data::cur_mdl_data->all_metrics[u2]->apply() == true)
    {
      ok_mets += mdl_data::cur_mdl_data->all_metrics[u2];
    } else {
      cerr << "Error in metric '" 
            << mdl_data::cur_mdl_data->all_metrics[u2]->id_ 
            << "'. Metric deleted." << endl;
      delete mdl_data::cur_mdl_data->all_metrics[u2];
      mdl_data::cur_mdl_data->all_metrics[u2] = NULL;
    }
  }
  mdl_data::cur_mdl_data->all_metrics = ok_mets;
