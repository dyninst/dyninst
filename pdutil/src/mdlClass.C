...
// $Id: mdlClass.C,v 1.6 2005/12/19 19:43:46 pack Exp $
...
bool daemonMet::set_field (field &f)
{
  ...
  case SET_HOST:
    host_ = *f.val;
    break;
  case SET_MRNET_TOPOLOGY:
	mrnet_topology_ = *f.val;
	break;
  case SET_MPITYPE:
    MPItype_ = *f.val;
    break;
...
daemonMet::daemonMet(pdstring &nm, pdstring &cmd, pdstring &remsh,
                     pdstring &exec, pdstring &u, pdstring &h, pdstring& flav,
                     pdstring &topology, pdstring &MPIt)
  : name_(nm), command_(cmd), remoteShell_(remsh), execDir_(exec), user_(u), 
    host_(h), flavor_(flav), mrnet_topology_(topology),MPItype_(MPIt)
{}
...
void daemonMet::dump() const
{
  ...
  cout << "     mrnet_topology: " << mrnet_topology_ << endl;
