...
void mark_heaps_exec() {
  ...
  unsigned long int adjustedSize =
      (unsigned long int)DYNINSTstaticHeap_16M_anyHeap_1 - alignedHeapPointer +
      sizeOfAnyHeap1;
  ...
  if (result != 0) {
    fprintf(stderr,
            "%s[%d]: Couldn't make DYNINSTstaticHeap_512K_lowmemHeap_1 "
            "executable!\n",
            __FILE__, __LINE__);
    perror("mark_heaps_exec");
  }
 ...
 * been run. Since the order of global constructors is undefined,
 * DYNINSTBaseInit
...
static int tkill(pid_t pid, long lwp, int sig) {
...
void uncaught_breakpoint(int sig) { failed_breakpoint = 1; }
...
int dyn_pid_self() { return getpid(); }
 ...
 *      incompatiblities seem to deal with FP and other special purpose
 *registers.
 ...
 *      onto the context's stack.  Instead of transfering control straight to
 *the
 *      instrumentation, we'll actually go back to dyninstTrapHandler, which
 *will
 *      do a popf/ret to restore flags and go to instrumentation.  The
 *'retPoint'
 *      parameter is the address in dyninstTrapHandler the popf/ret can be
 *found.
 **/
void dyninstTrapHandler(int sig, siginfo_t *sg, ucontext_t *context) {
  ...
  // Find the new IP we're going to and substitute. Leave everything else
  // untouched
  if (DYNINSTstaticMode) {
    ...
    trap_to =
        dyninstTrapTranslate(orig_ip, (unsigned long *)&hdr->num_entries, &zero,
                             (volatile trapMapping_t **)&mapping, &one);
  } else {
    trap_to = dyninstTrapTranslate(orig_ip, &dyninstTrapTableUsed,
...
threadSelf->attr.stackaddr_attr + threadSelf->attr.stacksize_attr);
    printf("START: 0x%lx == 0x%lx\n", (unsigned long)start_func, (unsigned
long)threadSelf->start_routine);

    unsigned char *ptr = (unsigned char *)threadSelf;
    unsigned long tidVal = *((unsigned long *)(ptr + offsetof(struct pthread,
tid)));
    unsigned long stackAddrVal = *((unsigned long *)(ptr + offsetof(struct
pthread, attr) + offsetof(struct pthread_attr, stackaddr_attr)));
    unsigned long stackSizeVal = *((unsigned long *)(ptr + offsetof(struct
pthread, attr) + offsetof(struct pthread_attr, stacksize_attr)));
    unsigned long startFuncVal = *((unsigned long *)(ptr + offsetof(struct
pthread, start_routine)));

    printf("TID = %u, offset = %u\n", tidVal, offsetof(struct pthread, tid));
    printf("STACK = 0x%lx, offset = %u\n", stackAddrVal, offsetof(struct
pthread, attr) + offsetof(struct pthread_attr, stackaddr_attr));
    printf("SIZE = 0x%lx, offset = %u\n", stackSizeVal, offsetof(struct pthread,
attr) + offsetof(struct pthread_attr, stacksize_attr));
