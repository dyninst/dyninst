...
void DYNINSTsafeBreakPoint() { DYNINSTbreakPoint(); }
...
int DYNINSTasyncConnect(int mutatorpid) {
  ...
  if (connect(sock_fd, (struct sockaddr *)&sadr, sizeof(sadr)) ==
      SOCKET_ERROR) {
...
int dyn_pid_self() { return _getpid(); }
...
dyntid_t dyn_pthread_self() { return (dyntid_t)dyn_lwp_self(); }

int DYNINSTthreadInfo(BPatch_newThreadEventRecord *ev) { return 1; }
...
static struct trap_mapping_header *getStaticTrapMap(unsigned long addr,
  ...
  *allocBase = (unsigned long)memInfo.AllocationBase;
  ...
  rtdebug_printf(
      "RTLIB: getStaticTrapMap addr=%lx meminfo.BaseAddress=%lx "
  ...
  curSecn = *(PIMAGE_SECTION_HEADER)(
      ((unsigned char *)peHdr) + sizeof(DWORD) + sizeof(IMAGE_FILE_HEADER) +
  ...
  if (0 != strncmp("DYNINST_REWRITE", str, 15)) {
    fprintf(stderr,
            "ERROR IN RTLIB: getStaticTrapMap found bad string [%s] at %p "
            "(%s[%d])\n",
  ...
  header =
      (struct trap_mapping_header *)((unsigned long)memInfo.AllocationBase +
                                     *((unsigned long *)(str -
                                                         sizeof(void *))));
  ...
  if (header) {
    rtdebug_printf("RTLIB: found trap map header at %lx: [%lx %lx]\n",
                   (unsigned long)header, header->low_entry,
                   header->high_entry);
  } else {
...
LONG dyn_trapHandler(PEXCEPTION_POINTERS e) {
  void *trap_to = 0;
  void *trap_addr =
      (void *)((unsigned char *)e->ExceptionRecord->ExceptionAddress);
  ...
  rtdebug_printf(
      "RTLIB: In dyn_trapHandler for exception type 0x%lx at 0x%lx\n",
  ...
  rtdebug_printf(
      "RTLIB: calling dyninstTrapTranslate(\n\t0x%lx, \n\t"
      "0x%lx, \n\t0x%lx, \n\t0x%lx, \n\t0x%lx)\n",
      (unsigned long)trap_addr - loadAddr + 1, hdr->num_entries, zero, mapping,
      one);

  trap_to =
      dyninstTrapTranslate((void *)((unsigned long)trap_addr - loadAddr + 1),
...
PVOID dyn_AddVectoredExceptionHandler(ULONG isFirst,
