...
void fixInstrumentation(char *soName, unsigned long currAddr,
  ...
  printf(
      "\n run correctly if shared libraries are loaded at a different "
      "address\n");
  printf("\n Exiting.....\n");
  fflush(stdout);
  // while(1);
...
/* 	this function checks the shared library (soName) to see if it
        is currently loaded (loadAddr) at the same place it was before
   (address).
...
void dyninst_dl_debug_state() {
  ...
  if (_r_debug.r_state == 1) {
    do {
      ...
      loadAddr =
          checkSOLoadAddr(map->l_name, (unsigned long)map->l_ld); /*l_addr*/
      if (loadAddr) {
        fixInstrumentation(map->l_name, (unsigned long)map->l_ld,
                           loadAddr); /*l_addr*/
      }
    } while (map->l_next);
  }
...
int checkMutatedFile() {
  ...
  Elf_nextscn =
      (Elf_Scn * (*)(Elf *, Elf_Scn *))dlsym(elfHandle, "elf_nextscn");
  Elf_getdata =
      (Elf_Data * (*)(Elf_Scn *, Elf_Data *))dlsym(elfHandle, "elf_getdata");
  Elf32_getehdr = (Elf32_Ehdr * (*)(Elf *))dlsym(elfHandle, "elf32_getehdr");
  Elf32_getshdr =
      (Elf32_Shdr * (*)(Elf_Scn *))dlsym(elfHandle, "elf32_getshdr");
  Elf64_getehdr = (Elf64_Ehdr * (*)(Elf *))dlsym(elfHandle, "elf64_getehdr");
  Elf64_getshdr =
      (Elf64_Shdr * (*)(Elf_Scn *))dlsym(elfHandle, "elf64_getshdr");
  ...
  for (cnt = 0, scn = NULL; !soError && (scn = Elf_nextscn(elf, scn)); cnt++) {
    shdr = ELF_FUNC(getshdr(scn));
    if (!strncmp((char *)strData->d_buf + shdr->sh_name, "dyninstAPItrampgu",
                 17)) {
      ...
      */
    ...
    } else if (!strncmp((char *)strData->d_buf + shdr->sh_name,
                        "dyninstAPI_data", 15)) {
      elfData = Elf_getdata(scn, NULL);
      tmpPtr = elfData->d_buf;
      dataAddress = -1;
    ...
    } else if (!strncmp((char *)strData->d_buf + shdr->sh_name, "dyninstAPI_",
                        11)) {
      ...
      if (*tmpStr >= 0x30 && *tmpStr <= 0x39) {
        ...
        if (*tmpStr >= 0x30 && *tmpStr <= 0x39) {
          ...
          /* 	the new starting address of the heap is immediately AFTER the
             last
                  dyninstAPI_### section, so we can ALWAYS memcpy the data into
             place
    ...
    if (!strcmp((char *)strData->d_buf + shdr->sh_name,
                "dyninstAPI_mutatedSO")) {
    ...
    defined(x86_64_unknown_linux2_4) /* Blind duplication - Ray */
      ...
      for (soNames = (char *)elfData->d_buf; totallen < elfData->d_size;
           soNames = &((char *)elfData
                           ->d_buf)[strlen(soNames) + 1 + sizeof(unsigned int) +
                                    sizeof(unsigned int)]) {
        /* added a +sizeof(unsigned int) above for flag */
        totallen += strlen(soNames) + 1 + sizeof(unsigned int) +
                    sizeof(unsigned int); /*for flag*/
        memcpy(&mutatedFlag,
               &((char *)elfData->d_buf)[totallen - sizeof(unsigned int)],
               sizeof(unsigned int));
        lmap = _r_debug.r_map;
        while (lmap) {
          ...
          if (mutatedFlag && !strcmp(loadedname, dyninstname)) {
            if (!checkSO(lmap->l_name)) {
              printf("ERROR: %s was mutated during saveworld and",
                     lmap->l_name);
              printf(" the currently loaded %s has not been mutated\n",
                     lmap->l_name);
              printf(
                  " check your LD path to be sure the mutated %s is visible\n",
                  soNames);
              soError = 1;
            }
          }
...
/* 	LINUX PROBLEM. in the link_map structure the map->l_addr field is NOT
        the load address of the dynamic object, as the documentation says.  It
   is the
        RELOCATED address of the object. If the object was not relocated then
   the
        value is ZERO.

        So, on LINUX we check the address of the dynamic section, map->l_ld,
   which is
        correct.
*/
...
#if defined(i386_unknown_linux2_0) || \
    defined(x86_64_unknown_linux2_4) /* Blind duplication - Ray */
          /* ld-linux.so will never be hand relocated so l_addr should be
           * correct. right? */
...
*/
    ...
    if (!strncmp((char *)strData->d_buf + shdr->sh_name, "dyninstAPIhighmem_",
                 18)) {
      ...
      */
      ...
      while (numberUpdates == 0) {
        count++;
        numberUpdates = (unsigned int)((
            (unsigned int *)elfData
                ->d_buf)[(elfData->d_size - (sizeof(unsigned int) * count)) /
                         sizeof(unsigned int)]);
      }
      ...
      oldPageDataSize =
          shdr->sh_size - (((2 * numberUpdates) * sizeof(unsigned int)) +
      ...
      /*fprintf(stderr," updateSize : %d-((2 * %d + 1) * 4))",shdr->sh_size,
       * numberUpdates);*/
      ...
      if (!checkAddr) {
        ...
        mmapAddr = (unsigned long)mmap((void *)shdr->sh_addr, oldPageDataSize,
                                       PROT_READ | PROT_WRITE | PROT_EXEC,
                                       MAP_FIXED | MAP_PRIVATE, fd, mmapAddr);
        /*fprintf(stderr,"MMAP %x %d %x size: %x\n",shdr->sh_addr,
         * mmapAddr,shdr->sh_offset,oldPageDataSize);*/

      } else {
        /*we own it, finish the memcpy */
        mmapAddr = (unsigned long)memcpy(
            (void *)oldPageData, (const void *)shdr->sh_addr, oldPageDataSize);
      ...
      for (i = 0; i < numberUpdates; i++) {
         ...
         * oldPageData[updateOffset]), &(((char*)elfData->d_buf)[updateOffset])
         * , updateSize,updateOffset);*/
    ...
    if (!strcmp((char *)strData->d_buf + shdr->sh_name, "dyninstAPI_loadLib")) {
      ...
      while (*tmpPtr) {
        ...
        if (handle) {
          ...
          printf(
              " This is an unrecoverable error, the instrumentation will not");
          printf(
              "\n run correctly if shared libraries are loaded at a different "
              "address\n");
          printf("\n Exiting.....\n");
