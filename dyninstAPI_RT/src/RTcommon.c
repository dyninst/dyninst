...
HEAP_LOCAL HEAP_TYPE
    DYNINSTstaticHeap_512K_lowmemHeap_1[512 * 1024 /
                                        sizeof(HEAP_TYPE)] ALIGN_ATTRIB;
HEAP_LOCAL HEAP_TYPE
    DYNINSTstaticHeap_16M_anyHeap_1[16 * 1024 * 1024 /
                                    sizeof(HEAP_TYPE)] ALIGN_ATTRIB;
/* These are necessary due to silly C-style 'extern'/linking conventions. */
const unsigned long sizeOfLowMemHeap1 =
    sizeof(DYNINSTstaticHeap_512K_lowmemHeap_1);
const unsigned long sizeOfAnyHeap1 = sizeof(DYNINSTstaticHeap_16M_anyHeap_1);
#else
HEAP_LOCAL HEAP_TYPE
    DYNINSTstaticHeap_8K_lowmemHeap_1[8 * 1024 /
                                      sizeof(HEAP_TYPE)] ALIGN_ATTRIB;
HEAP_LOCAL HEAP_TYPE
    DYNINSTstaticHeap_32K_anyHeap_1[32 * 1024 / sizeof(HEAP_TYPE)] ALIGN_ATTRIB;
/* These are necessary due to silly C-style 'extern'/linking conventions. */
const unsigned long sizeOfLowMemHeap1 =
    sizeof(DYNINSTstaticHeap_8K_lowmemHeap_1);
...
DLLEXPORT void *DYNINST_synch_event_arg2 =
    NULL; /* not read in dyninst's decodeRTSignal*/
DLLEXPORT void *DYNINST_synch_event_arg3 =
    NULL; /* not read in dyninst's decodeRTSignal*/
...
DLLEXPORT void DYNINST_unlock_tramp_guard() { DYNINST_tls_tramp_guard = 1; }
 ...
 *           gcc with -Wl,-init -Wl,...
 *    Windows: called from DllMain, which exists in lieu of
 ...
 **/
int DYNINSTreturnZero() { return 0; }
 ...
 **/
//#define STACKDUMP
void DYNINST_stopThread(void *pointAddr, void *callBackID, void *flags,
                        void *calculation) {
  ...
  reentrant = 1;
  tc_lock_lock(&DYNINST_trace_lock);
  rtdebug_printf("RT_st: pt[%lx] flags[%lx] calc[%lx] ", (long)pointAddr,
                 (long)flags, (long)calculation);
...
void DYNINST_stopInterProc(void *pointAddr, void *callBackID, void *flags,
                           void *calculation, void *objStart, void *objEnd) {
...
DLLEXPORT RT_Boolean DYNINST_boundsCheck(void **boundsArray_, void *arrayLen_,
  ...
  // fprintf(stderr,"D_bc@%p: boundsArray=%p target=%lx idx=%d arrayLen=%d
  // [%d]\n", (void*)DYNINST_boundsCheck, boundsArray_, writeTarget_, idx,
  // arrayLen, __LINE__);
  // rtdebug_printf("D_bc@%p: boundsArray=%p target=%lx idx=%d arrayLen=%d
  // [%d]\n", (void*)DYNINST_boundsCheck, boundsArray_, writeTarget_, idx,
  // arrayLen, __LINE__);
...
int dyninst_lock(dyninst_lock_t *lock) { return tc_lock_lock(lock); }

void dyninst_unlock(dyninst_lock_t *lock) { tc_lock_unlock(lock); }
...
void *dyninstTrapTranslate(void *source, volatile unsigned long *table_used,
