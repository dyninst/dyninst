...
void *map_region(void *addr, int len, int fd) {
  ...
  char *lpMessage = NULL;
  result =
      VirtualAlloc(addr, len, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
  if (!result) {
...
int DYNINSTheap_mmapFdOpen(void) { return 0; }

void DYNINSTheap_mmapFdClose(int fd) {}

RT_Boolean DYNINSTheap_useMalloc(void *lo, void *hi) { return RT_FALSE; }
