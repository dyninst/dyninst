...
pthread_sync::pthread_sync() {
    ...
    this->conds = (pthread_cond_t**)malloc(sizeof (pthread_cond_t*) * 
                                           MAX_CONDS);
    this->registered_conds = (int*)malloc(sizeof(int) * MAX_CONDS);    
...
void pthread_sync::register_cond(unsigned cond_num) {
    ...
    conds[cond_num] = (pthread_cond_t*)calloc(1, sizeof(pthread_cond_t));
