    ...
    if (shmid == -1) {
      ...
      if (errno == ENOSPC) {
        ...
        for (key_t k = key; k <= key + 20; k++) {
          if (TryToReleaseShmSegment(k, size) == gcHappened) success = true;
        }
...
static gcResult TryToReleaseShmSegment(key_t keyToTry, unsigned int size) {
