 ...
 */
//----------------------------------------------------------------------------
// $Id: shmSegment-win.C,v 1.1 2006/11/22 21:45:03 bernat Exp $
//----------------------------------------------------------------------------
//
...
ShmSegment::~ShmSegment(void) {
  assert(baseAddrInDaemon != NULL);
  assert(seg_id != NULL);

  // unmap the segment from our address space
  if (!UnmapViewOfFile((void*)baseAddrInDaemon)) {
#if READY
...
//
ShmSegment* ShmSegment::Create(key_t& key, unsigned int size,
  ...
  while (true) {
    ...
    hMapping = CreateFileMapping(INVALID_HANDLE_VALUE,  // shared memory
                                 NULL, PAGE_READWRITE, 0, size, segName);
    if (hMapping == NULL) {
// unable to create segment
#if READY
      indicate error to user ?
#else
      cerr << "Failed to create file mapping object : " << GetLastError()
           << endl;
#endif  // READY
  ...
  void* mapAddr = MapViewOfFile(hMapping, FILE_MAP_ALL_ACCESS, 0, 0, 0);
  if (mapAddr != NULL) {
...
#else
    cerr << "Failed to map shared memory into address space : "
         << GetLastError() << endl;
#endif  // READY
        CloseHandle(hMapping);
  }

  if (!freeWhenDeleted) seg->markAsLeaveSegmentAroundOnExit();
...
//
ShmSegment* ShmSegment::Open(key_t key, unsigned int size, void* addr) {
  HANDLE hMapping = NULL;
  ShmSegment* shm = NULL;
  char segName[32];
  ...
  if (hMapping != NULL) {
    ...
    void* mapAddr =
        MapViewOfFile(hMapping,             // map object
                      FILE_MAP_ALL_ACCESS,  // permissions
                      0, 0,  // high and low offsets of map start address
                      0);    // map entire file
    if (mapAddr != NULL) {
      // build an object to represent the segment
      shm = new ShmSegment(hMapping, key, size, (Address)mapAddr);
    } else {
#if READY
      indicate failure to user ?
#else
...
ShmSegment* ShmSegment::Copy(BPatch_process* child_thr, bool sameAddress) {
  // Not implemented
...
//
static int BuildSegmentName(key_t key, unsigned int size, char* buffer) {
