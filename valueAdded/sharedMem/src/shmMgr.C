...
shmMgr::shmMgr(BPatch_process *proc, key_t shmSegKey, unsigned shmSize_,
               bool freeWhenDel)
    : nextKeyToTry(shmSegKey),
      shmSegSize(shmSize_),
      totalShmSize(0),
      freespace(0),
      app(proc),
      freeWhenDeleted(freeWhenDel) {}

shmMgr *shmMgr::createSharedManager(BPatch_process *proc, key_t shmSegKey,
...
bool shmMgr::initialize() {
  ...
  if (!shm_lib_name) shm_lib_name = SHARED_MUTATEE_LIB;

  // getBPatch().registerDynLibraryCallback((BPatchDynLibraryCallback)
  // (addLibraryCallback));
  ...
  ShmSegment *new_seg =
      ShmSegment::Create(nextKeyToTry, shmSegSize, freeWhenDeleted);
  if (new_seg == NULL) {
    // No luck
...
shmMgr::shmMgr(const shmMgr *par, BPatch_process *child)
    : nextKeyToTry(par->nextKeyToTry),
      shmSegSize(par->shmSegSize),
      totalShmSize(par->totalShmSize),
      freespace(par->freespace),
      app(child),
      freeWhenDeleted(par->freeWhenDeleted) {
...
Dyninst::Address shmMgr::malloc_int(unsigned size, bool /* align = true*/) {
  ...
  if (freespace < size) {
    ...
    if (new_seg == NULL) {
      // No luck
      fprintf(stderr,
              "shmMgr: unable to allocate: unsufficient space, failed to "
...
shMallocHandle *shmMgr::malloc(unsigned size, bool align /* = true*/,
  ...
  BPatch_variableExpr *expr =
      app->createVariable(buffer, daemonToApplic(retAddr), type);
...
// are equivalent
Dyninst::Address shmMgr::translateFromParentDaemon(Dyninst::Address vaddr,
                                                   const shmMgr *parShmMgr) {
...
// Assumption: the vectors of shared segments in each manager
// are equivalent
Dyninst::Address shmMgr::translateFromParentApplic(Dyninst::Address vaddr,
                                                   const shmMgr *parShmMgr) {
  Dyninst::Address addr = (Dyninst::Address)vaddr;
  for (unsigned i = 0; i < parShmMgr->theShms.size(); i++) {
