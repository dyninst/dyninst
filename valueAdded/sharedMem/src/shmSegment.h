...
class ShmSegment {
 ...
 public:
  ...
  static ShmSegment *Create(key_t &key, unsigned int size,
  ...
  Address offsetInDaemon(Address addr) const { return addr - baseAddrInDaemon; }
  Address offsetInApplic(Address addr) const { return addr - baseAddrInApplic; }
