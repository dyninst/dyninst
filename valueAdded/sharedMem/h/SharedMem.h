...
class shMallocHandle {
 ...
 private:
  ...
  shMallocHandle(Dyninst::Address a, BPatch_variableExpr *e)
      : addrInMutator_(a), expr_(e){};
...
class shmMgr {
 private:
  ...
  shmMgr(BPatch_process *proc, key_t shmSegKey, unsigned shmSize_,
         bool freeWhenDel = true);
  // Fork constructor
  ...
  Dyninst::Address translateFromParentDaemon(Dyninst::Address addr,
                                             const shmMgr *parent);
  Dyninst::Address translateFromParentApplic(Dyninst::Address addr,
                                             const shmMgr *parent);
 ...
 public:
  ...
  // defaultSegmentSize: the initial size for segments (multiple segments are
  // supported)
  // automaticFree: delete shared memory when this object is freed?
  static shmMgr *createSharedManager(BPatch_process *proc, key_t shmSegKey,
                                     unsigned defaultSegmentSize,
                                     bool automaticFree);
  ~shmMgr();

  // Malloc and free equivalents
  shMallocHandle *malloc(unsigned size, bool align = true,
