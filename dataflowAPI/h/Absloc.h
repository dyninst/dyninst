...
public:
    ...
    {
        assert(type_ == Register || type_ == PredicatedRegister);
        return reg_;
    }
    ...
    DATAFLOW_EXPORT const MachRegister& predReg() const
    {
        assert(type_ == PredicatedRegister);
        return preg_;
    }
    DATAFLOW_EXPORT bool isTrueCondition() const
    {
        assert(type_ == PredicatedRegister);
        return trueCond_;
    }
    DATAFLOW_EXPORT void flipPredicateCondition()
    {
        assert(type_ == PredicatedRegister);
        trueCond_ = !trueCond_;
    }
    ...
    DATAFLOW_EXPORT bool operator!=(const Absloc& rhs) const { return !(*this == rhs); }
...
public:
    ...
    DATAFLOW_EXPORT void setGenerator(AST::Ptr generator) { generator_ = generator; }

    DATAFLOW_EXPORT void setSize(size_t size) { size_ = size; }

    DATAFLOW_EXPORT static bool equivalent(const AbsRegion& lhs, const AbsRegion& rhs,
                                           Address addr, ParseAPI::Function* caller,
...
public:
    ...
    {
        size_t operator()(const Ptr& ap) const noexcept { return (size_t) ap.get(); }
    };
    ...
    DATAFLOW_EXPORT Assignment(const InstructionAPI::Instruction& i, const Address a,
    ...
    DATAFLOW_EXPORT static Assignment::Ptr makeAssignment(
        const InstructionAPI::Instruction& i, const Address a, ParseAPI::Function* f,
