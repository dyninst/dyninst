...
class DATAFLOW_EXPORT LivenessAnalyzer {
  ...
  void processEdgeLiveness(ParseAPI::Edge *e, livenessData &data,
                           ParseAPI::Block *block,
                           const bitArray &allRegsDefined);

  void summarizeBlockLivenessInfo(ParseAPI::Function *func,
                                  ParseAPI::Block *block,
                                  bitArray &allRegsDefined);
  bool updateBlockLivenessInfo(ParseAPI::Block *block,
                               bitArray &allRegsDefined);

  ReadWriteInfo calcRWSets(Instruction::Ptr curInsn, ParseAPI::Block *blk,
                           Address a);
 ...
 public:
  ...
  bool query(ParseAPI::Location loc, Type type, OutputIterator outIter) {
    ...
    if (query(loc, type, liveRegs)) {
      for (std::map<MachRegister, int>::const_iterator iter =
               abi->getIndexMap()->begin();
  ...
  bool query(ParseAPI::Location loc, Type type, const MachRegister &machReg,
             bool &live);
