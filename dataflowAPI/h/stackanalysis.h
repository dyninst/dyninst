...
public:
    ...
    public:
        ...
        Definition(ParseAPI::Block* b, Address a, Absloc l)
        : addr(a)
        , block(b)
        , origLoc(l)
        , type(DEF)
        {}
        Definition(ParseAPI::Block* b, Absloc l)
        : addr(0)
        , block(b)
        , origLoc(l)
        , type(DEF)
        {}
        Definition(Address a, Absloc l)
        : addr(a)
        , block(NULL)
        , origLoc(l)
        , type(DEF)
        ...
        bool operator<(const Definition& rhs) const
        {
            if(type == TOP)
                return false;
            if(type == BOTTOM && rhs.type == BOTTOM)
                return false;
            if(type == BOTTOM)
                return true;
    ...
    public:
        ...
        Height(const Height_t h, const Type t = HEIGHT)
        : height_(h)
        , type_(t)
        {}
        Height()
        : height_(uninitialized)
        , type_(TOP)
        {}
        ...
        bool operator!=(const Height& rhs) const { return !(*this == rhs); }
        ...
        bool isBottom() const { return type_ == BOTTOM && height_ == notUnique; }

        bool isTop() const { return type_ == TOP && height_ == uninitialized; }
    ...
    {
    public:
        DefHeight(const Definition& d, const Height& h)
        : def(d)
        , height(h)
        {}
        ...
        bool operator<(const DefHeight& other) const { return def < other.def; }
    ...
    public:
        ...
        std::set<DefHeight>::iterator begin() { return defHeights.begin(); }

        // Returns a constant iterator to the set of DefHeights
        std::set<DefHeight>::const_iterator begin() const { return defHeights.begin(); }

        // Returns an iterator to the end of the set of DefHeights
        std::set<DefHeight>::iterator end() { return defHeights.end(); }

        // Returns a constant iterator to the end of the set of DefHeights
        std::set<DefHeight>::const_iterator end() const { return defHeights.end(); }

        // Returns the size of this set
        std::set<DefHeight>::size_type size() const { return defHeights.size(); }

        // Inserts a DefHeight into this set
        void insert(const DefHeight& dh) { defHeights.insert(dh); }
        ...
        void makeNewSet(ParseAPI::Block* b, Address addr, const Absloc& origLoc,
                        const Height& h);
    ...
    public:
        ...
        , type_(TOP)
        {}
        TransferFunc(long a, long d, Absloc f, Absloc t, bool i = false, bool rt = false,
                     Type type = OTHER)
        : from(f)
        , target(t)
        , delta(d)
        ...
        static TransferFunc sibFunc(std::map<Absloc, std::pair<long, bool>> f, long d,
                                    Absloc t);

        static TransferFunc meet(const TransferFunc& lhs, const TransferFunc& rhs);
        ...
        bool isDelta() const;
        bool isSIB() const;
        bool isTopBottom() const { return topBottom; }

        bool operator==(const TransferFunc& rhs) const
        {
            return from == rhs.from && target == rhs.target && delta == rhs.delta &&
                   abs == rhs.abs && retop == rhs.retop && topBottom == rhs.topBottom &&
                   fromRegs == rhs.fromRegs;
        }

        bool operator!=(const TransferFunc& rhs) const { return !(*this == rhs); }
    ...
    public:
        ...
        void apply(ParseAPI::Block* block, const AbslocState& in, AbslocState& out) const;
    ...
    typedef std::map<ParseAPI::Block*, std::map<Offset, TransferFuncs>>
                                                                      InstructionEffects;
    typedef std::map<ParseAPI::Block*, std::map<Offset, TransferSet>> CallEffects;

    DATAFLOW_EXPORT StackAnalysis();
    DATAFLOW_EXPORT StackAnalysis(ParseAPI::Function* f);
    DATAFLOW_EXPORT StackAnalysis(
        ParseAPI::Function* f, const std::map<Address, Address>& crm,
...
private:
    ...
    DefHeightSet meetDefHeights(const DefHeightSet& s1, const DefHeightSet& s2);
    ...
    bool handleJump(InstructionAPI::Instruction insn, ParseAPI::Block* block, Offset off,
    ...
    void handleLeave(ParseAPI::Block* block, const Offset off, TransferFuncs& xferFuncs);
    ...
    void handleSignExtend(InstructionAPI::Instruction insn, ParseAPI::Block* block,
                          const Offset off, TransferFuncs& xferFuncs);
    void handleSpecialSignExtend(InstructionAPI::Instruction insn,
                                 TransferFuncs&              xferFuncs);
    void handleXor(InstructionAPI::Instruction insn, ParseAPI::Block* block,
                   const Offset off, TransferFuncs& xferFuncs);
