...
struct Variable
{
    DATAFLOW_EXPORT Variable()
    : reg()
    , addr(0)
    {}
    DATAFLOW_EXPORT Variable(AbsRegion r)
    : reg(r)
    , addr(0)
    {}
    DATAFLOW_EXPORT Variable(AbsRegion r, Address a)
    : reg(r)
    , addr(a)
    {}
...
struct Constant
{
    DATAFLOW_EXPORT Constant()
    : val(0)
    , size(0)
    {}
    DATAFLOW_EXPORT Constant(uint64_t v)
    : val(v)
    , size(0)
    {}
    DATAFLOW_EXPORT Constant(uint64_t v, size_t s)
    : val(v)
    , size(s)
    {}
...
struct ROSEOperation
{
    ...
    DATAFLOW_EXPORT ROSEOperation(Op o)
    : op(o)
    , size(0)
    {}
    DATAFLOW_EXPORT ROSEOperation(Op o, size_t s)
    : op(o)
    , size(s)
    {}
...
public:
    ...
    DATAFLOW_EXPORT static std::pair<AST::Ptr, bool> expand(
        const Assignment::Ptr& assignment, bool applyVisitors = true);
    ...
    DATAFLOW_EXPORT static Retval_t expand(Dyninst::Graph::Ptr    slice,
                                           DataflowAPI::Result_t& res);

private:
    // Symbolically evaluate an instruction and assign
    // an AST representation to every written absloc
    static bool expandInsn(const InstructionAPI::Instruction& insn, const uint64_t addr,
                           Result_t& res);

    static Retval_t process(SliceNodePtr ptr, Result_t& dbase,
