...
public:
    ...
    static SliceNode::Ptr create(AssignmentPtr ptr, ParseAPI::Block* block,
...
private:
    SliceNode(AssignmentPtr ptr, ParseAPI::Block* block, ParseAPI::Function* func)
    : a_(ptr)
    , b_(block)
    , f_(func)
    {}
...
private:
    SliceEdge(const SliceNode::Ptr source, const SliceNode::Ptr target,
              AbsRegion const& data)
    : Edge(source, target)
    , data_(data)
    {}
...
public:
    ...
    DATAFLOW_EXPORT Slicer(AssignmentPtr a, ParseAPI::Block* block,
                           ParseAPI::Function* func, bool cache = true,
                           bool stackAnalysis = true);

    DATAFLOW_EXPORT Slicer(AssignmentPtr a, ParseAPI::Block* block,
                           ParseAPI::Function* func, AssignmentConverter* ac);
    ...
    struct DATAFLOW_EXPORT Location
    {
        ...
        Address addr() const
        {
            if(fwd)
                ...
                return (*rcurrent).second;
        }

        Location(ParseAPI::Function* f, ParseAPI::Block* b)
        : func(f)
        , block(b)
        , fwd(true)
        {}
    ...
    {
        Element(ParseAPI::Block* b, ParseAPI::Function* f, AbsRegion const& r,
                Assignment::Ptr p)
        : block(b)
        , func(f)
        , reg(r)
        ...
        {
            if(ptr->addr() < el.ptr->addr())
            {
                return true;
            }
            if(el.ptr->addr() < ptr->addr())
            {
                return false;
            }
            if(ptr->out() < el.ptr->out())
            {
                return true;
            }
            return false;
    ...
    {
        SliceFrame(Location const& l, Context const& c)
        : loc(l)
        , con(c)
        , valid(true)
        {}
    ...
    public:
        ...
        DATAFLOW_EXPORT bool performCacheClear()
        {
            if(clearCache)
            {
                clearCache = false;
        ...
        DATAFLOW_EXPORT void setSearchForControlFlowDep(bool cfd)
        {
            controlFlowDep = cfd;
        }
        ...
        DATAFLOW_EXPORT virtual std::vector<ParseAPI::Function*> followCallBackward(
            ParseAPI::Block* /*callerB*/, CallStack_t& /*cs*/, AbsRegion /*argument*/)
        {
            std::vector<ParseAPI::Function*> vec;
            return vec;
        }
        DATAFLOW_EXPORT virtual bool addPredecessor(AbsRegion /*reg*/) { return true; }
        ...
        DATAFLOW_EXPORT virtual bool addNodeCallback(AssignmentPtr,
                                                     std::set<ParseAPI::Edge*>&)
        {
            return true;
        }
        // Callback function after we have added new a node and corresponding new edges to
        // the slice. This function allows users to inspect the current slice graph and
        // determine which abslocs need further slicing and which abslocs are no longer
        // interesting, by modifying the current SliceFrame.
        DATAFLOW_EXPORT virtual bool modifyCurrentFrame(SliceFrame&, GraphPtr, Slicer*)
        {
            return true;
        }
        DATAFLOW_EXPORT virtual bool ignoreEdge(ParseAPI::Edge*) { return false; }
        DATAFLOW_EXPORT              Predicates()
        : clearCache(false)
        , controlFlowDep(false)
        {}
    };
...
private:
    ...
    bool getStackDepth(ParseAPI::Function* func, ParseAPI::Block* block, Address callAddr,
                       long& height);

    // Add the newly called function to the given Context.
    void pushContext(Context& context, ParseAPI::Function* callee,
    ...
    struct CacheEdge
    {
        CacheEdge(Address src, Address trg)
        : s(src)
        , t(trg)
        {}
        Address s;
    ...
    struct Def
    {
        Def(Element const& e, AbsRegion const& r)
        : ele(e)
        , data(r)
        {}
        Element   ele;
    ...
    public:
        ...
        std::set<Def>& get(AbsRegion const& r) { return defmap[r]; }
        bool defines(AbsRegion const& r) const { return defmap.find(r) != defmap.end(); }
    ...
    void shiftAbsRegion(AbsRegion const& callerReg, AbsRegion& calleeReg,
                        long stack_depth, ParseAPI::Function* callee);

    // Shift all of the abstract regions active in the current frame
    void shiftAllAbsRegions(SliceFrame& cur, long stack_depth,
     ...
     */
    GraphPtr sliceInternal(Direction dir, Predicates& predicates);
    void     sliceInternalAux(GraphPtr g, Direction dir, Predicates& p, SliceFrame& cand,
                              bool skip, std::map<CacheEdge, std::set<AbsRegion>>& visited,
                              std::unordered_map<Address, DefCache>& single,
                              std::unordered_map<Address, DefCache>& cache);

    bool updateAndLink(GraphPtr g, Direction dir, SliceFrame& cand, DefCache& cache,
                       Predicates& p);

    void updateAndLinkFromCache(GraphPtr g, Direction dir, SliceFrame& f,
                                DefCache& cache);

    void removeBlocked(SliceFrame& f, std::set<AbsRegion> const& block);

    bool stopSlicing(SliceFrame::ActiveMap& active, GraphPtr g, Address addr,
                     Direction dir);

    void markVisited(std::map<CacheEdge, std::set<AbsRegion>>& visited,
                     CacheEdge const& e, SliceFrame::ActiveMap const& active);

    void cachePotential(Direction dir, Assignment::Ptr assn, DefCache& cache);

    bool findMatch(GraphPtr g, Direction dir, SliceFrame const& cand,
                   AbsRegion const& cur, Assignment::Ptr assn,
                   std::vector<Element>& matches, DefCache& cache);

    bool getNextCandidates(Direction dir, Predicates& p, SliceFrame const& cand,
                           std::vector<SliceFrame>& newCands);

    /* forward slicing */

    bool getSuccessors(Predicates& p, SliceFrame const& cand,
                       std::vector<SliceFrame>& newCands);

    bool handleCall(Predicates& p, SliceFrame& cur, bool& err);

    bool followCall(Predicates& p, ParseAPI::Block* target, SliceFrame& cur);

    bool handleCallDetails(SliceFrame& cur, ParseAPI::Block* caller_block);

    bool handleReturn(Predicates& p, SliceFrame& cur, bool& err);

    void handleReturnDetails(SliceFrame& cur);

    bool handleDefault(Direction dir, Predicates& p, ParseAPI::Edge* e, SliceFrame& cur,
                       bool& err);

    /* backwards slicing */

    bool getPredecessors(Predicates& p, SliceFrame const& cand,
                         std::vector<SliceFrame>& newCands);

    bool handleCallBackward(Predicates& p, SliceFrame const& cand,
                            std::vector<SliceFrame>& newCands, ParseAPI::Edge* e,
                            bool& err);

    std::vector<ParseAPI::Function*> followCallBackward(Predicates&       p,
                                                        SliceFrame const& cand,
                                                        AbsRegion const&  reg,
                                                        ParseAPI::Block*  caller_block);

    bool handleCallDetailsBackward(SliceFrame& cur);

    bool handleReturnBackward(Predicates& p, SliceFrame const& cand, SliceFrame& newCand,
                              ParseAPI::Edge* e, bool& err);

    bool handleReturnDetailsBackward(SliceFrame& cur, ParseAPI::Block* caller_block);

    bool followReturn(Predicates& p, SliceFrame const& cand, ParseAPI::Block* source);
    void handlePredecessorEdge(ParseAPI::Edge* e, Predicates& p, SliceFrame const& cand,
                               std::vector<SliceFrame>& newCands, bool& err,
                               SliceFrame& nf);

    /* general slicing support */

    void constructInitialFrame(Direction dir, SliceFrame& initFrame);
    ...
    void insertPair(GraphPtr graph, Direction dir, Element const& source,
                    Element const& target, AbsRegion const& data);

    void insertPair(GraphPtr graph, Direction dir, SliceNode::Ptr& source,
                    SliceNode::Ptr& target, AbsRegion const& data);

    void convertInstruction(const InstructionAPI::Instruction&, Address,
                            ParseAPI::Function*, ParseAPI::Block*,
...
private:
    ...
    ParseAPI::Block* getBlock(ParseAPI::Edge* e, Direction dir);

    void insertInitialNode(GraphPtr ret, Direction dir, SliceNode::Ptr aP);

    void mergeRecursiveCaches(std::unordered_map<Address, DefCache>& sc,
                              std::unordered_map<Address, DefCache>& c, Address a);
    ...
    std::unordered_map<AssignmentPtr, SliceNode::Ptr, Assignment::AssignmentPtrHasher>
        created_;
