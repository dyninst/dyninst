...
class Slicer {
 public:
  ...
  class Predicates {
   ...
   public:
    ...
    followCallBackward(ParseAPI::Block * /*callerB*/, CallStack_t & /*cs*/,
                       AbsRegion /*argument*/) {
 ...
 private:
  ...
  bool getStackDepth(ParseAPI::Function *func, ParseAPI::Block *block,
  ...
  void sliceInternalAux(GraphPtr g, Direction dir, Predicates &p,
                        SliceFrame &cand, bool skip,
                        std::map<CacheEdge, std::set<AbsRegion> > &visited,
                        std::map<Address, DefCache> &single,
                        std::map<Address, DefCache> &cache);

  bool updateAndLink(GraphPtr g, Direction dir, SliceFrame &cand,
  ...
  bool handleDefault(Direction dir, Predicates &p, ParseAPI::Edge *e,
  ...
  bool handleReturnBackward(Predicates &p, SliceFrame const &cand,
                            SliceFrame &newCand, ParseAPI::Edge *e, bool &err);

  bool handleReturnDetailsBackward(SliceFrame &cur,
                                   ParseAPI::Block *caller_block);

  bool followReturn(Predicates &p, SliceFrame const &cand,
                    ParseAPI::Block *source);
  void handlePredecessorEdge(ParseAPI::Edge *e, Predicates &p,
  ...
  void convertInstruction(InstructionPtr, Address, ParseAPI::Function *,
                          ParseAPI::Block *, std::vector<AssignmentPtr> &);
  ...
  void mergeRecursiveCaches(std::map<Address, DefCache> &sc,
                            std::map<Address, DefCache> &c, Address a);
  ...
  std::unordered_map<AssignmentPtr, SliceNode::Ptr,
                     Assignment::AssignmentPtrHasher>
      created_;
