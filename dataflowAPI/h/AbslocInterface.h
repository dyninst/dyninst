...
public:
    ...
    DATAFLOW_EXPORT void convertAll(InstructionAPI::Expression::Ptr expr, Address addr,
                                    ParseAPI::Function* func, ParseAPI::Block* block,
                                    std::vector<AbsRegion>& regions);

    DATAFLOW_EXPORT void convertAll(const InstructionAPI::Instruction& insn, Address addr,
                                    ParseAPI::Function* func, ParseAPI::Block* block,
    ...
    DATAFLOW_EXPORT AbsRegion convert(InstructionAPI::Expression::Ptr expr, Address addr,
                                      ParseAPI::Function* func, ParseAPI::Block* block);

    DATAFLOW_EXPORT AbsRegion convertPredicatedRegister(
        InstructionAPI::RegisterAST::Ptr r, InstructionAPI::RegisterAST::Ptr p, bool c);

    // Cons up a stack reference at the current addr
    DATAFLOW_EXPORT AbsRegion stack(Address addr, ParseAPI::Function* func,
                                    ParseAPI::Block* block, bool push);

    DATAFLOW_EXPORT AbsRegion frame(Address addr, ParseAPI::Function* func,
                                    ParseAPI::Block* block, bool push);

private:
    // Returns false if the current height is unknown.
    bool getCurrentStackHeight(ParseAPI::Function* func, ParseAPI::Block* block,
                               Address addr, long& height);
    bool getCurrentFrameHeight(ParseAPI::Function* func, ParseAPI::Block* block,
                               Address addr, long& height);
...
public:
    ...
    : cacheEnabled_(cache)
    , aConverter(false, stack)
    {}

    DATAFLOW_EXPORT void convert(const InstructionAPI::Instruction& insn,
                                 const Address& addr, ParseAPI::Function* func,
                                 ParseAPI::Block*              block,
                                 std::vector<Assignment::Ptr>& assignments);

private:
    void handlePushEquivalent(const InstructionAPI::Instruction I, Address addr,
                              ParseAPI::Function* func, ParseAPI::Block* block,
                              std::vector<AbsRegion>&       operands,
                              std::vector<Assignment::Ptr>& assignments);
    void handlePopEquivalent(const InstructionAPI::Instruction I, Address addr,
                             ParseAPI::Function* func, ParseAPI::Block* block,
                             std::vector<AbsRegion>&       operands,
                             std::vector<Assignment::Ptr>& assignments);

    bool cache(ParseAPI::Function* func, Address addr,
               std::vector<Assignment::Ptr>& assignments);
