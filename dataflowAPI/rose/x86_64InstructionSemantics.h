...
#define Word(Len) WordType<(Len)>
  ...
  X86_64InstructionSemantics(Policy& policy) : policy(policy) {}
  ...
  Word(Len) readMemory(X86SegmentRegister segreg, const Word(64) & addr,
                       Word(1) cond) {
  ...
  Word(16) read16(SgAsmExpression* e) {
    switch (e->variantT()) {
      case V_SgAsmx86RegisterReferenceExpression: {
        SgAsmx86RegisterReferenceExpression* rre =
            isSgAsmx86RegisterReferenceExpression(e);
        // Xiaozhu: Not sure why we 16-bit is different from other lengths of
        // read,
  ...
  Word(64) read64(SgAsmExpression* e) {
    switch (e->variantT()) {
      ...
      case V_SgAsmBinaryAdd: {
        return policy.add(read64(isSgAsmBinaryAdd(e)->get_lhs()),
                          read64(isSgAsmBinaryAdd(e)->get_rhs()));
      }
      case V_SgAsmBinaryMultiply: {
        SgAsmByteValueExpression* rhs =
            isSgAsmByteValueExpression(isSgAsmBinaryMultiply(e)->get_rhs());
  ...
  /* Replaces the least significant 32 bits of a general purpose register with a
   * new value. */
  void updateGPRLowDWord(X86GeneralPurposeRegister reg,
                         const Word(32) & value) {
    Word(64) oldValue = policy.readGPR(reg);
  ...
  /* Writes the specified 64-bit value to the location specified by an
   * instruction operand. */
  ...
  virtual void translate(SgAsmx86Instruction* insn) {
    ...
    // PATCH
    policy.writeIP(
        policy.add(policy.readIP(), number<64>(insn->get_raw_bytes().size())));
    ...
    switch (kind) {
      case x86_mov: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 4:
            ...
            break;
          case 8:
            write64(operands[0], read64(operands[1]));
            break;
          default:
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            switch (numBytesInAsmType(operands[1]->get_type())) {
              case 1:
                write64(operands[0],
                        policy.concat(read8(operands[1]), number<56>(0)));
                break;
              case 2:
                write64(operands[0],
                        policy.concat(read16(operands[1]), number<48>(0)));
                break;
              case 4:
                write64(operands[0],
                        policy.concat(read32(operands[1]), number<32>(0)));
                break;
      ...
      /*
                  case x86_cbw: {
                      ROSE_ASSERT(operands.size() == 0);
                      updateGPRLowWord(x86_gpr_ax, signExtend<8, 16>(extract<0,
         8>(policy.readGPR(x86_gpr_ax))));
                      break;
                  }
      */
      case x86_cwde: {
        ...
        policy.writeGPR(
            x86_gpr_ax,
            signExtend<16, 64>(extract<0, 16>(policy.readGPR(x86_gpr_ax))));
        break;
      }
      /*
                  case x86_cwd: {
                      ROSE_ASSERT(operands.size() == 0);
                      updateGPRLowWord(x86_gpr_dx, extract<16,
         32>(signExtend<16, 32>(extract<0, 16>(policy.readGPR(x86_gpr_ax)))));
                      break;
                  }

                  case x86_cdq: {
                      ROSE_ASSERT(operands.size() == 0);
                      policy.writeGPR(x86_gpr_dx, extract<32, 64>(signExtend<32,
         64>(policy.readGPR(x86_gpr_ax))));
                      break;
                  }
      */
      case x86_lea: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          case 4: {
            write32(operands[0],
                    extract<0, 32>(readEffectiveAddress(operands[1])));
            break;
          }
          case 8: {
      ...
      case x86_and: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result =
                policy.and_(read64(operands[0]), read64(operands[1]));
      ...
      case x86_or: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result =
                policy.or_(read64(operands[0]), read64(operands[1]));
            setFlagsForResult<64>(result);
            write64(operands[0], result);
            break;
          }
      ...
      case x86_xor: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result =
                policy.xor_(read64(operands[0]), read64(operands[1]));
            setFlagsForResult<64>(result);
            write64(operands[0], result);
            break;
          }
      ...
      /*
                  case x86_not: {
                      ...
                      switch (numBytesInAsmType(operands[0]->get_type())) {
                          case 1: {
                              Word(8) result =
         policy.invert(read8(operands[0]));
                              write8(operands[0], result);
                              break;
                          }
                          case 2: {
                              Word(16) result =
         policy.invert(read16(operands[0]));
                              write16(operands[0], result);
                              break;
                          }
                          case 4: {
                              Word(32) result =
         policy.invert(read32(operands[0]));
                              write32(operands[0], result);
                              break;
                          }
                          default:
      ...
      */
      case x86_add: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result =
                doAddOperation<64>(read64(operands[0]), read64(operands[1]),
                                   false, policy.false_());
            write64(operands[0], result);
            break;
          }
      ...
      /*
         ...
         doAddOperation<8>(read8(operands[0]), read8(operands[1]), false,
                          ...
                          case 2: {
                              Word(16) result =
         doAddOperation<16>(read16(operands[0]), read16(operands[1]), false,
                                                                   policy.readFlag(x86_flag_cf));
                              write16(operands[0], result);
                              break;
                          }
                          case 4: {
                              Word(32) result =
         doAddOperation<32>(read32(operands[0]), read32(operands[1]), false,
                                                                   policy.readFlag(x86_flag_cf));
                              write32(operands[0], result);
                              break;
                          }
      ...
      */
      case x86_sub: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result = doAddOperation<64>(
                read64(operands[0]), policy.invert(read64(operands[1])), true,
                policy.false_());
            write64(operands[0], result);
            break;
          }
      ...
      case x86_cmp: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            doAddOperation<64>(read64(operands[0]),
                               policy.invert(read64(operands[1])), true,
                               policy.false_());
            break;
          }
      ...
      case x86_neg: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result = doAddOperation<64>(
                number<64>(0), policy.invert(read64(operands[0])), true,
                policy.false_());
            write64(operands[0], result);
            break;
          }
      ...
      case x86_inc: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result =
                doIncOperation<64>(read64(operands[0]), false, false);
            write64(operands[0], result);
            break;
          }
      ...
      case x86_dec: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) result =
                doIncOperation<64>(read64(operands[0]), true, false);
            write64(operands[0], result);
            break;
          }
      ...
      /*
                  case x86_cmpxchg: {
                      ...
                      switch (numBytesInAsmType(operands[0]->get_type())) {
                          case 1: {
                              ...
                              Word(8) oldAx = extract<0,
         8>(policy.readGPR(x86_gpr_ax));
                              doAddOperation<8>(oldAx, policy.invert(op0), true,
         policy.false_());
                              write8(operands[0],
         policy.ite(policy.readFlag(x86_flag_zf), read8(operands[1]), op0));
                              updateGPRLowByte(x86_gpr_ax,
         policy.ite(policy.readFlag(x86_flag_zf), oldAx, op0));
                              break;
                          }
                          case 2: {
                              Word(16) op0 = read16(operands[0]);
                              Word(16) oldAx = extract<0,
         16>(policy.readGPR(x86_gpr_ax));
                              doAddOperation<16>(oldAx, policy.invert(op0),
         true, policy.false_());
                              write16(operands[0],
         policy.ite(policy.readFlag(x86_flag_zf), read16(operands[1]), op0));
         ...
         policy.ite(policy.readFlag(x86_flag_zf), oldAx, op0));
                          ...
                          case 4: {
                              ...
                              doAddOperation<32>(oldAx, policy.invert(op0),
         true, policy.false_());
                              write32(operands[0],
         policy.ite(policy.readFlag(x86_flag_zf), read32(operands[1]), op0));
      ...
      case x86_shl: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            ...
            Word(1) newCf =
                policy.ite(shiftCountZero, policy.readFlag(x86_flag_cf),
                           extract<63, 64>(policy.shiftLeft(
                               op, policy.add(shiftCount, number<5>(63)))));
            policy.writeFlag(x86_flag_cf, newCf);
            policy.writeFlag(
                x86_flag_of,
                policy.ite(shiftCountZero, policy.readFlag(x86_flag_of),
      ...
      /*
                  case x86_rol: {
                      switch (numBytesInAsmType(operands[0]->get_type())) {
                          case 1: {
                              Word(8) op = read8(operands[0]);
                              Word(5) shiftCount = extract<0,
         5>(read8(operands[1]));
                              Word(8) output = policy.rotateLeft(op,
         shiftCount);
                              policy.writeFlag(x86_flag_cf,
         ...
         policy.ite(policy.equalToZero(shiftCount),
                                                                       policy.readFlag(x86_flag_of),
                                                                       policy.xor_(extract<0,
         1>(output), extract<7, 8>(output))));
                              write8(operands[0], output);
                              break;
                          }
                          case 2: {
                              ...
                              Word(5) shiftCount = extract<0,
         5>(read8(operands[1]));
                              Word(16) output = policy.rotateLeft(op,
         shiftCount);
                              policy.writeFlag(x86_flag_cf,
         policy.ite(policy.equalToZero(shiftCount),
                                                                       policy.readFlag(x86_flag_cf),
         ...
         1>(output), extract<15, 16>(output))));
                          ...
                          case 4: {
                              ...
                              Word(5) shiftCount = extract<0,
         5>(read8(operands[1]));
                              Word(32) output = policy.rotateLeft(op,
         shiftCount);
                              policy.writeFlag(x86_flag_cf,
         policy.ite(policy.equalToZero(shiftCount),
                                                                       policy.readFlag(x86_flag_cf),
      ...
      */
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            ...
            policy.writeFlag(x86_flag_of,
                             policy.ite(policy.equalToZero(shiftCount),
                                        policy.readFlag(x86_flag_of),
                                        policy.xor_(extract<62, 63>(output),
                                                    extract<63, 64>(output))));
            write64(operands[0], output);
            break;
          }
      ...
      /*
                  case x86_shld: {
                      ...
                      switch (numBytesInAsmType(operands[0]->get_type())) {
                          case 2: {
                              ...
                              Word(16) output1 = policy.shiftLeft(op1,
         shiftCount);
                              Word(16) output2 =
         policy.ite(policy.equalToZero(shiftCount),
                                                            number<16>(0),
                                                            policy.shiftRight(op2,
         policy.negate(shiftCount)));
                              Word(16) output = policy.or_(output1, output2);
                              Word(1) newCf =
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_cf),
                                                         extract<15,
         16>(policy.shiftLeft(op1, policy.add(shiftCount, number<5>(15)))));
                              policy.writeFlag(x86_flag_cf, newCf);
                              Word(1) newOf =
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_of),
                                                         policy.xor_(extract<15,
         16>(output), newCf));
         ...
         policy.ite(policy.equalToZero(shiftCount),
                          ...
                          case 4: {
                              ...
                              Word(5) shiftCount = extract<0,
         5>(read8(operands[2]));
                              Word(32) output1 = policy.shiftLeft(op1,
         shiftCount);
                              Word(32) output2 =
         policy.ite(policy.equalToZero(shiftCount),
                                                            number<32>(0),
         ...
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_cf),
                                                         extract<31,
         32>(policy.shiftLeft(op1, policy.add(shiftCount, number<5>(31)))));
                              policy.writeFlag(x86_flag_cf, newCf);
                              Word(1) newOf =
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_of),
                                                         policy.xor_(extract<31,
         32>(output), newCf));
                              policy.writeFlag(x86_flag_of, newOf);
                              write32(operands[0], output);
                  ...
                  case x86_shrd: {
                      ...
                      switch (numBytesInAsmType(operands[0]->get_type())) {
                          case 2: {
                              Word(16) op1 = read16(operands[0]);
                              Word(16) op2 = read16(operands[1]);
                              Word(16) output1 = policy.shiftRight(op1,
         shiftCount);
                              Word(16) output2 =
         policy.ite(policy.equalToZero(shiftCount),
                                                            number<16>(0),
                                                            policy.shiftLeft(op2,
         policy.negate(shiftCount)));
                              Word(16) output = policy.or_(output1, output2);
                              Word(1) newCf =
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_cf),
                                                         extract<0,
         1>(policy.shiftRight(op1, policy.add(shiftCount, number<5>(15)))));
                              policy.writeFlag(x86_flag_cf, newCf);
                              Word(1) newOf =
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_of),
         ...
         policy.ite(policy.equalToZero(shiftCount),
                          ...
                          case 4: {
                              Word(32) op1 = read32(operands[0]);
                              Word(32) op2 = read32(operands[1]);
                              Word(32) output1 = policy.shiftRight(op1,
         shiftCount);
                              Word(32) output2 =
         policy.ite(policy.equalToZero(shiftCount),
                                                            number<32>(0),
         ...
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_cf),
                                                         extract<0,
         1>(policy.shiftRight(op1, policy.add(shiftCount, number<5>(31)))));
                              policy.writeFlag(x86_flag_cf, newCf);
                              Word(1) newOf =
         policy.ite(policy.equalToZero(shiftCount),
                                                         policy.readFlag(x86_flag_of),
      ...
      case x86_bsf: {
        ...
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            ...
            policy.writeFlag(x86_flag_zf, policy.equalToZero(op));
            Word(64) result =
                policy.ite(policy.readFlag(x86_flag_zf), read64(operands[0]),
                           policy.leastSignificantSetBit(op));
            write64(operands[0], result);
            break;
          }
      ...
      isSgAsmx86RegisterReferenceExpression(operands[1])) {
              ...
              Word(32) bitnum = numBytes == 2 ? signExtend<16,
      32>(read16(operands[1])) : read32(operands[1]);
              Word(32) adjustedAddr = policy.add(addr, signExtend<29,
      32>(extract<3, 32>(bitnum)));
              Word(8) val =
      readMemory<8>(getSegregFromMemoryReference(isSgAsmMemoryReferenceExpression(operands[0])),
                                          adjustedAddr, policy.true_());
              Word(1) bitval = extract<0, 1>(policy.rotateRight(val, extract<0,
      3>(bitnum)));
              Word(8) result = policy.or_(val, policy.rotateLeft(number<8>(1),
      extract<0, 3>(bitnum)));
              ...
              switch (numBytesInAsmType(operands[0]->get_type())) {
                  case 2: {
                      ...
                      Word(1) bitval = extract<0, 1>(policy.rotateRight(op0,
      bitnum));
                      Word(16) result = policy.or_(op0,
      policy.rotateLeft(number<16>(1), bitnum));
                      policy.writeFlag(x86_flag_cf, bitval);
                      write16(operands[0], result);
                      break;
                  }
                  case 4: {
                      ...
                      Word(1) bitval = extract<0, 1>(policy.rotateRight(op0,
      bitnum));
                      Word(32) result = policy.or_(op0,
      policy.rotateLeft(number<32>(1), bitnum));
                      policy.writeFlag(x86_flag_cf, bitval);
                      write32(operands[0], result);
                      break;
                  }
...
*/
      case x86_imul: {
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            Word(64) op0 = operands.size() == 1
                               ? policy.readGPR(x86_gpr_ax)
                               : read64(operands[operands.size() - 2]);
            ...
            Word(1) carry = policy.invert(policy.or_(
                policy.equalToZero(policy.invert(extract<7, 128>(mulResult))),
      ...
      case x86_mul: {
        switch (numBytesInAsmType(operands[0]->get_type())) {
          ...
          case 8: {
            ...
            policy.writeGPR(x86_gpr_dx, extract<64, 128>(mulResult));
            Word(1) carry =
                policy.invert(policy.equalToZero(extract<64, 128>(mulResult)));
            policy.writeFlag(x86_flag_cf, carry);
            policy.writeFlag(x86_flag_of, carry);
            break;
          }
      ...
      /*
                  case x86_idiv: {
                      switch (numBytesInAsmType(operands[0]->get_type())) {
                          case 1: {
                              Word(16) op0 = extract<0,
         16>(policy.readGPR(x86_gpr_ax));
                              Word(8) op1 = read8(operands[0]);
                              // if op1 == 0, we should trap
                              Word(16) divResult = policy.signedDivide(op0,
         op1);
                              Word(8) modResult = policy.signedModulo(op0, op1);
                              // if result overflows, we should trap
                              updateGPRLowWord(x86_gpr_ax,
         policy.concat(extract<0, 8>(divResult), modResult));
                              break;
                          }
                          case 2: {
                              Word(32) op0 = policy.concat(extract<0,
         16>(policy.readGPR(x86_gpr_ax)),
                                                           extract<0,
         16>(policy.readGPR(x86_gpr_dx)));
                              Word(16) op1 = read16(operands[0]);
                              // if op1 == 0, we should trap
                              Word(32) divResult = policy.signedDivide(op0,
         op1);
                              Word(16) modResult = policy.signedModulo(op0,
         op1);
                              // if result overflows, we should trap
                              updateGPRLowWord(x86_gpr_ax, extract<0,
         16>(divResult));
                          ...
                          case 4: {
                              Word(64) op0 =
         policy.concat(policy.readGPR(x86_gpr_ax), policy.readGPR(x86_gpr_dx));
                              Word(32) op1 = read32(operands[0]);
                              // if op1 == 0, we should trap
                              Word(64) divResult = policy.signedDivide(op0,
         op1);
                              Word(32) modResult = policy.signedModulo(op0,
         op1);
                              // if result overflows, we should trap
                              policy.writeGPR(x86_gpr_ax, extract<0,
         32>(divResult));
         ...
         16>(policy.readGPR(x86_gpr_ax));
                              Word(8) op1 = read8(operands[0]);
                              // if op1 == 0, we should trap
                              Word(16) divResult = policy.unsignedDivide(op0,
         op1);
                              Word(8) modResult = policy.unsignedModulo(op0,
         op1);
                              // if extract<8, 16> of divResult is non-zero
         (overflow), we should trap
                              updateGPRLowWord(x86_gpr_ax,
         policy.concat(extract<0, 8>(divResult), modResult));
                              break;
                          }
         ...
         16>(policy.readGPR(x86_gpr_dx)));
                              Word(16) op1 = read16(operands[0]);
                              // if op1 == 0, we should trap
                              Word(32) divResult = policy.unsignedDivide(op0,
         op1);
                              Word(16) modResult = policy.unsignedModulo(op0,
         op1);
                              // if extract<16, 32> of divResult is non-zero
         (overflow), we should trap
                              updateGPRLowWord(x86_gpr_ax, extract<0,
         16>(divResult));
                              updateGPRLowWord(x86_gpr_dx, modResult);
         ...
         policy.concat(policy.readGPR(x86_gpr_ax), policy.readGPR(x86_gpr_dx));
                              Word(32) op1 = read32(operands[0]);
                              // if op1 == 0, we should trap
                              Word(64) divResult = policy.unsignedDivide(op0,
         op1);
                              Word(32) modResult = policy.unsignedModulo(op0,
         op1);
                              // if extract<32, 64> of divResult is non-zero
         (overflow), we should trap
                              policy.writeGPR(x86_gpr_ax, extract<0,
         32>(divResult));
                              policy.writeGPR(x86_gpr_dx, modResult);
                  ...
                  case x86_aaa: {
                      ...
                      Word(1) incAh = policy.or_(policy.readFlag(x86_flag_af),
                                                 greaterOrEqualToTen<4>(extract<0,
         4>(policy.readGPR(x86_gpr_ax))));
                      updateGPRLowWord(x86_gpr_ax,
                                       policy.concat(policy.add(policy.ite(incAh,
         number<4>(6), number<4>(0)),
                                                                extract<0,
         4>(policy.readGPR(x86_gpr_ax))),
                                                     policy.concat(number<4>(0),
                                                                   policy.add(policy.ite(incAh,
         number<8>(1), number<8>(0)),
                                                                              extract<8,
         16>(policy.readGPR(x86_gpr_ax))))));
                      policy.writeFlag(x86_flag_of, policy.undefined_());
                      policy.writeFlag(x86_flag_sf, policy.undefined_());
         ...
         4>(policy.readGPR(x86_gpr_ax))));
                      updateGPRLowWord(x86_gpr_ax,
                                       policy.concat(policy.add(policy.ite(decAh,
         number<4>(-6), number<4>(0)),
                                                                extract<0,
         4>(policy.readGPR(x86_gpr_ax))),
                                                     policy.concat(number<4>(0),
                                                                   policy.add(policy.ite(decAh,
         number<8>(-1), number<8>(0)),
                                                                              extract<8,
         16>(policy.readGPR(x86_gpr_ax))))));
                      policy.writeFlag(x86_flag_of, policy.undefined_());
                  ...
                  case x86_aad: {
                      ...
                      Word(8) divisor = read8(operands[0]);
                      Word(8) newAl = policy.add(al, extract<0,
         8>(policy.unsignedMultiply(ah, divisor)));
                      updateGPRLowWord(x86_gpr_ax, policy.concat(newAl,
         number<8>(0)));
                      policy.writeFlag(x86_flag_of, policy.undefined_());
                      policy.writeFlag(x86_flag_af, policy.undefined_());
                      policy.writeFlag(x86_flag_cf, policy.undefined_());
                  ...
                  case x86_bswap: {
                      ...
                      Word(32) newVal = policy.concat(extract<24, 32>(oldVal),
                                                      policy.concat(extract<16,
         24>(oldVal),
                                                                    policy.concat(extract<8,
         16>(oldVal),
                                                                                  extract<0, 8>(oldVal))));
      ...
      */
      case x86_push: {
        ...
        policy.writeMemory(x86_segreg_ss, newSp, read64(operands[0]),
                           policy.true_());
        policy.writeGPR(x86_gpr_sp, newSp);
      ...
      /*
         ...
         policy.readGPR(x86_gpr_di), policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(4)), policy.readGPR(x86_gpr_si), policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(8)), policy.readGPR(x86_gpr_bp), policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(12)), oldSp, policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(16)), policy.readGPR(x86_gpr_bx), policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(20)), policy.readGPR(x86_gpr_dx), policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(24)), policy.readGPR(x86_gpr_cx), policy.true_());
                      policy.writeMemory(x86_segreg_ss, policy.add(newSp,
         number<32>(28)), policy.readGPR(x86_gpr_ax), policy.true_());
                      policy.writeGPR(x86_gpr_sp, newSp);
                      break;
                  }
                  ...
                  case x86_pushfd: {
                      ...
                      policy.writeMemory(x86_segreg_ss, newSp, readEflags(),
         policy.true_());
                      policy.writeGPR(x86_gpr_sp, newSp);
                      break;
                  }
      */
      ...
      case x86_pop: {
        ...
        Word(64) newSp = policy.add(oldSp, number<64>(8));
        write64(operands[0],
                readMemory<64>(x86_segreg_ss, oldSp, policy.true_()));
        policy.writeGPR(x86_gpr_sp, newSp);
        break;
      }
      /*
                  case x86_popad: {
                      ...
                      Word(32) oldSp = policy.readGPR(x86_gpr_sp);
                      Word(32) newSp = policy.add(oldSp, number<32>(32));
                      policy.writeGPR(x86_gpr_di, readMemory<32>(x86_segreg_ss,
         oldSp, policy.true_()));
                      policy.writeGPR(x86_gpr_si, readMemory<32>(x86_segreg_ss,
         policy.add(oldSp, number<32>(4)), policy.true_()));
                      policy.writeGPR(x86_gpr_bp, readMemory<32>(x86_segreg_ss,
         policy.add(oldSp, number<32>(8)), policy.true_()));
                      policy.writeGPR(x86_gpr_bx, readMemory<32>(x86_segreg_ss,
         policy.add(oldSp, number<32>(16)), policy.true_()));
                      policy.writeGPR(x86_gpr_dx, readMemory<32>(x86_segreg_ss,
         policy.add(oldSp, number<32>(20)), policy.true_()));
                      policy.writeGPR(x86_gpr_cx, readMemory<32>(x86_segreg_ss,
         policy.add(oldSp, number<32>(24)), policy.true_()));
                      policy.writeGPR(x86_gpr_ax, readMemory<32>(x86_segreg_ss,
         policy.add(oldSp, number<32>(28)), policy.true_()));
                      readMemory<32>(x86_segreg_ss, policy.add(oldSp,
         number<32>(12)), policy.true_());
                      policy.writeGPR(x86_gpr_sp, newSp);
                      break;
                  }
         ...
         policy.true_());
                  ...
                  case x86_ret: {
                      ...
                      ROSE_ASSERT(insn->get_addressSize() == x86_insnsize_32);
                      ROSE_ASSERT(insn->get_operandSize() == x86_insnsize_32);
                                      Word(32) extraBytes = (operands.size() ==
         1 ? read32(operands[0]) : number<32>(0));
                      Word(32) oldSp = policy.readGPR(x86_gpr_sp);
                      Word(32) newSp = policy.add(oldSp,
         policy.add(number<32>(4), extraBytes));
                      policy.writeIP(policy.filterReturnTarget(readMemory<32>(x86_segreg_ss,
         oldSp, policy.true_())));
                      policy.writeGPR(x86_gpr_sp, newSp);
                      break;
                  }
                  ...
                  case x86_loop: {
                      ...
                      policy.writeIP(policy.ite(doLoop, read32(operands[0]),
         policy.readIP()));
                      break;
                  }
                  case x86_loopz: {
...
#define JUMP(tag)                                                           \
  {                                                                         \
    ROSE_ASSERT(operands.size() == 1);                                      \
    policy.writeIP(                                                         \
        policy.ite(FLAGCOMBO_##tag, read64(operands[0]), policy.readIP())); \
  }
      case x86_jne:
        JUMP(ne);
        break;
  ...
  {                                                                           \
    ...
    switch (numBytesInAsmType(operands[0]->get_type())) {                     \
      ...
      case 4:                                                                 \
        ...
        break;                                                                \
      case 8:                                                                 \
        write64(operands[0], policy.ite(FLAGCOMBO_##tag, read64(operands[1]), \
                                        read64(operands[0])));                \
        break;                                                                \
      default:                                                                \
        ROSE_ASSERT(!"Bad size");                                             \
...
#define STRINGOP_LOOP_E                                      \
  policy.writeIP(policy.ite(                                 \
      ...
      number<64>((uint64_t)(insn->get_address())), policy.readIP()))
...
#undef STRINGOP_LOOP_NE
      ...
      */
      default: {
        //                std::cerr <<"Bad instruction [0x" <<std::hex
        //                <<insn->get_address() <<": "//
