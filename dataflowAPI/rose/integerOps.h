...
template <typename T, size_t Count, bool TooBig>
struct SHL1Helper;
 ...
 * provide non-template versions for those cases
 *  when the arguments are not known at compile time.  The non-template versions
 * typically have "2" appended to their names to
 *  indicate that they should be the second choice--used only when the template
 * version cannot be used. */
namespace IntegerOps {

/** Bitmask constant with bit @p n set.  Handles the case where @p n is greater
 * than the width of type @p T. */
template <typename T, size_t n>
struct SHL1 : public IntegerOpsPrivate::SHL1Helper<
                  T, n, (n >= IntegerOpsPrivate::NumBits<T>::value)> {};

/** Bitmask with bit @p n set.  Handles the case where @p n is greater than the
 * width of type @p T. */
template <typename T>
...
/** Generate a bitmask. The return value has bits @p lobit (inclusive) through
 * @p hibit (inclusive) set, and all other bits
 *  are clear. */
template <typename T>
...
/** Sign extend value.  If the bit @p FromBits-1 is set set for @p value, then
* the result will have bits @p FromBits through @p
*  ToBits-1 also set (other bits are unchanged).  If @p ToBits is less than or
* equal to @p FromBits then nothing happens.
* @{ */
...
inline bool isPowerOfTwo(T value) {
  ...
  if (0 != (value & SHL1<T, 8 * sizeof(T) - 1>::value)) value = ~value + 1;
  uint64_t uval = value;
  while (uval) {
    if (uval & 1) return 1 == uval;
    uval >>= 1u;
  }
  return true;  // treat zero as a power of two
}

/** Returns the base-2 logorithm of @p value.  If @p value is not a power of two
 * then the return value is rounded up to the
 *  next integer. The @p value is treated as an unsigned value. Returns zero if
 * @p value is zero. */
template <typename T>
inline T log2max(T value) {
  ...
  T retval = 0;
  while (uval) {
    if (1 == uval) return retval + (low_bits_set ? 1 : 0);
    if (uval & 1) low_bits_set = true;
    ++retval;
    uval >>= 1;
  }
  return retval;
...
/** Create a shifted value. The return value is created by shifting @p value to
 * the specified position in the result. Other
 *  bits of the return value are clear. The @p hibit is specified so that we can
 * check at run-time that a valid value was
...
/** Extract bits from a value.  Bits @p lobit through @p hibit, inclusive, are
 * right shifted into the result and higher-order
 ...
 * set in the first argument form a subset of the
