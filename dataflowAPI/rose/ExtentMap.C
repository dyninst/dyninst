/* The ExtentMap class. This class is similar std::map<rose_addr_t,rose_addr_t> where the
 * two addresses are the starting offset and size.  The main difference is that if two
 * adjacent extents are added to the map they will be condensed into a single extent. This
 * class is used to keep track of what parts of a binary file have been parsed, and is
 * also used to manage string table free lists, among other things. */
...
AddressIntervalSet
toAddressIntervalSet(const ExtentMap& x)
{
...
std::ostream&
operator<<(std::ostream& out, const AddressInterval& x)
{
    if(x.isEmpty())
    {
        out << "empty";
...
std::ostream&
operator<<(std::ostream& out, const AddressIntervalSet& x)
{
    out << "{";
    BOOST_FOREACH(const AddressInterval& interval, x.intervals())
        out << " " << interval;
...
/** Class method comparing two extents. The return value is one of the following letters,
 * depending on how extent A is related to extent B: C (congruent):  A and B are congruent
...
/** Allocate an extent of the specified size (best fit first) from the extent map,
...
/** Allocate an extent of the specified size (first fit) from the extent map, removing the
 * returned extent from the map. */
Extent
...
void
ExtentMap::dump_extents(std::ostream& o, const std::string& prefix,
                        const std::string& label) const
{
    ...
    for(const_iterator i = begin(); i != end(); ++i, ++idx)
    {
        o << prefix << (label.empty() ? std::string("Extent") : label) << "[" << idx
          ...
          << unsignedToHex(i->first.size()) << (1 == i->first.size() ? " byte" : " bytes")
          << " ending at " << unsignedToHex(i->first.first() + i->first.size()) << "\n";
    }
}

/** Print info about an extent map. This is a little different format than the ostream
 * "<<" operator. */
void
ExtentMap::dump_extents(FILE* f, const char* prefix, const char* label, bool pad) const
{
    ...
    for(const_iterator i = begin(); i != end(); ++i, ++idx)
    {
        ...
        fprintf(f,
                "%s%-*s = offset 0x%08" PRIx64 " (%" PRIu64 "),"
