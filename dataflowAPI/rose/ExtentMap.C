/* The ExtentMap class. This class is similar std::map<rose_addr_t,rose_addr_t>
 * where the two addresses are the starting
 * offset and size.  The main difference is that if two adjacent extents are
 * added to the map they will be condensed into a
 * single extent.  This class is used to keep track of what parts of a binary
 * file have been parsed, and is also used to
...
AddressInterval toAddressInterval(const Extent &x) {
  return x.empty() ? AddressInterval()
                   : AddressInterval::hull(x.first(), x.last());
}
...
std::ostream &operator<<(std::ostream &out, const AddressInterval &x) {
  if (x.isEmpty()) {
    ...
    out << "[" << StringUtility::addrToString(x.least()) << ","
        << StringUtility::addrToString(x.greatest()) << "]";
  }
  return out;
}
...
/** Class method comparing two extents. The return value is one of the following
 * letters, depending on how extent A is related
 *  to extent B:
...
char ExtentMap::category(const Extent &a, const Extent &b) {
  if (a.relaxed_first() == b.relaxed_first() && a.size() == b.size())
    return 'C';
  if (a.relaxed_first() + a.size() <= b.relaxed_first()) return 'L';
  if (a.relaxed_first() >= b.relaxed_first() + b.size()) return 'R';
  if (a.relaxed_first() <= b.relaxed_first() &&
      a.relaxed_first() + a.size() >= b.relaxed_first() + b.size())
    return 'O';
  if (a.relaxed_first() >= b.relaxed_first() &&
      a.relaxed_first() + a.size() <= b.relaxed_first() + b.size())
    return 'I';
  if (a.relaxed_first() <=
      b.relaxed_first()) /*already know a.first+a.size > b.first*/
    return 'B';
  return 'E';
}

/** Allocate an extent of the specified size (best fit first) from the extent
 * map, removing the returned extent from the map. */
Extent ExtentMap::allocate_best_fit(rose_addr_t size) {
  iterator found = best_fit(size, begin());
  if (found == end()) throw std::bad_alloc();
  Extent retval(found->first.first(), size);
  erase(retval);
  return retval;
}

/** Allocate an extent of the specified size (first fit) from the extent map,
 * removing the returned extent from the map. */
Extent ExtentMap::allocate_first_fit(rose_addr_t size) {
...
void ExtentMap::allocate_at(const Extent &request) {
  ROSE_ASSERT(subtract_from(request).size() ==
              0); /*entire request should be on free list*/
  erase(request);
}
...
void ExtentMap::dump_extents(std::ostream &o, const std::string &prefix,
  ...
  for (const_iterator i = begin(); i != end(); ++i, ++idx) {
    o << prefix << (label.empty() ? std::string("Extent") : label) << "[" << idx
      << "]"
      << " = offset " << unsignedToHex(i->first.first()) << " for "
      << unsignedToHex(i->first.size())
      << (1 == i->first.size() ? " byte" : " bytes") << " ending at "
      << unsignedToHex(i->first.first() + i->first.size()) << "\n";
  }
}

/** Print info about an extent map. This is a little different format than the
 * ostream "<<" operator. */
void ExtentMap::dump_extents(FILE *f, const char *prefix, const char *label,
                             bool pad) const {
  char p[4096];
  size_t idx = 0;
  for (const_iterator i = begin(); i != end(); ++i, ++idx) {
    ...
    fprintf(f, "%s%-*s = offset 0x%08" PRIx64 " (%" PRIu64
               "),"
               " for 0x%08" PRIx64 " (%" PRIu64
               ") byte%s,"
               " ending at 0x%08" PRIx64 " (%" PRIu64 ")\n",
            p, w, "", i->first.first(), i->first.first(), i->first.size(),
            i->first.size(), 1 == i->first.size() ? "" : "s",
            i->first.first() + i->first.size(),
            i->first.first() + i->first.size());
  }
}
