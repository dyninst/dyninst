...
// Returns the next name counter. If @p useThis is specified then return that value and
// make sure the next value to be returned is larger.
static uint64_t
...
commentEscape(const std::string& s)
{
    ...
    // Escape angle brackets if they're not balanced.  We could be smarter and escape only
    // the unbalanced angle brackets. We'll leave that as an exercise for the reader. ;-)
...
Node::newFlags(unsigned newFlags)
{
    ...
    if(InteriorPtr inode = isInteriorNode())
        return Interior::create(0, inode->getOperator(), inode->children(), comment(),
                                newFlags);
...
struct Hasher : Visitor
{
    ...
    {
        if(!node->isHashed())
        {  // probably true, but some other thread may have beaten us here.
            ...
            if(LeafPtr leaf = node->isLeafNode())
            {
                ...
                else
                {
                    ...
                    // because variables always have a zero domain width and memory has a
                    // non-zero width, which is already incorporated into the hash from
                    // above.
    ...
    // Incorporates data into the existing hash, h, and returns a new hash. This is no
    // particular well-known algorithm, but testing showed that it gives pretty
    // well-distributed results for close values, particularly when called on two or more
    // pieces of data.
...
Interior::Interior(size_t nbits, Operator op, const Ptr& a, const std::string& comment,
                   unsigned flags)
: Node(comment)
, op_(op)
, nnodes_(1)
{
    ...
    {
        throw Exception(
            "operator size mismatch (specified=" + StringUtility::numberToString(nbits) +
...
Interior::Interior(size_t nbits, Operator op, const Ptr& a, const Ptr& b, const Ptr& c,
                   const std::string& comment, unsigned flags)
: Node(comment)
, op_(op)
, nnodes_(1)
    ...
    switch(op_)
    {
        ...
        case OP_EXTRACT: {
            ...
            if(!child(0)->isNumber())
                throw Exception(
                    toStr(op_) +
                    " operator's first argument (begin bit) must be an integer constant");
            if(!child(1)->isNumber())
                throw Exception(
                    toStr(op_) +
                    " operator's second argument (end bit) must be an integer constant");
            if(!child(2)->isScalar())
        ...
        default: {
            ...
            // expression. This is suitable for things like bitwise operators, add, etc.
            if(!child(0)->isScalar())
...
Interior::print(std::ostream& o, Formatter& fmt)
{
    ...
    // The width of an operator is not normally too useful since it can also be inferred
    // from the width of its operands, but we print it anyway for the benefit of mere
    // humans.
    char bracket = '[';
    if(fmt.show_width)
    {
...
Interior::mustEqual(const Ptr& other_, SMTSolver* solver /*NULL*/)
{
    ...
    {
        // This is probably faster than using an SMT solver. It also serves as the naive
        // approach when an SMT solver is not available.
        retval = true;
    }
    else if(solver)
    {
...
Interior::isEquivalentTo(const Ptr& other_)
{
    ...
    else if(hashval_ != 0 && other->hashval_ != 0 && hashval_ != other->hashval_)
    {
        ...
        // necessarily true due to possible collisions.
        retval = false;
    }
    else if(op_ == other->op_ && children_.size() == other->children_.size())
    {
        ...
        // Cache hash values. There's no need to compute a hash value if we've determined
        // that the two expressions are equivalent because it wouldn't save us any
        // work--two equal hash values doesn't necessarily mean that two expressions are
        // equivalent.  However, if we already know one of the hash values then we can
        // cache that hash value in the other expression too.
        if(retval)
        {
...
Interior::additiveNesting()
{
    ...
    if(nested != NULL && nested->getOperator() == getOperator())
    {
        ...
        // The two addends must be the same width, so zero-extend them if necessary (or
        // should we sign extend?) Note that the first argument (new width) of the UEXTEND
        // operator is not actually used.
...
Interior::foldConstants(const Simplifier& simplifier)
{
    ...
    while(ci1 != children_.end())
    {
        ...
        LeafPtr               leaf;
        while(ci2 != children_.end() && (leaf = (*ci2)->isLeafNode()) && leaf->isNumber())
            ++ci2;
        if(ci1 == ci2 || ci1 + 1 == ci2)
        {  // arg is not a constant, or we had only one constant by itself
            newOperands.push_back(*ci1);
            ++ci1;
        }
        else if(Ptr folded = simplifier.fold(ci1, ci2))
        {  // able to fold all these constants into a new node
            newOperands.push_back(folded);
            modified = true;
            ci1      = ci2;
        }
        else
        {  // multiple constants, but unable to fold
...
AddSimplifier::rewrite(Interior* inode) const
{
    ...
    // by decrementing adjustment. The adjustment, whose width is the same as A and B, is
    // allowed to overflow.  For example, consider the expression, where all values are
    // two bits wide:
    //   (add v1 (invert v1) v2 (invert v2) v3 (invert v3))            by substitution for
    //   invert: (add v1 (negate v1) -1 v2 (negate v2) -1 v3 (negate v3) -1)   canceling
    //   duals gives: (add -1 -1 -1) rewriting as 2's complement (2 bits wide): (add 3 3
    //   3)                                                   constant folding modulo 4:
    //   1
...
XorSimplifier::rewrite(Interior* inode) const
{
    ...
    // If any pairs of arguments are equal, then they don't contribute to the final
    // answer.
...
{
    // FIXME[Robb P. Matzke 2014-05-05]: Constant folding is not currently possible when
    // the operands are wider than 64 bits because Sawyer::Container::BitVector does not
    // provide a multiplication method.
    size_t   totalWidth = 0;
    int64_t  product    = 1;
    unsigned flags      = 0;
    for(/*void*/; begin != end; ++begin)
...
{
    // first arg is high-order bits. Although this is nice to look at, it makes the
    // operation a bit more difficult.
    size_t resultSize = 0;
...
ConcatSimplifier::rewrite(Interior* inode) const
{
    ...
    // If all the concatenated expressions are extract expressions, all extracting bits
    // from the same expression and in the correct order, then we can simplify this to
    // that expression.  For instance:
    //   (concat[32]
    ...
    {  // process args in little endian order
...
ExtractSimplifier::rewrite(Interior* inode) const
{
    ...
    // If the operand is a concat operation then take only the parts we need.  Some
    // examples: (extract 0 24 (concat X[24] Y[8]))  ==> (concat (extract 0 16 X) Y)
    Nodes       newChildren;
    InteriorPtr ioperand = operand->isInteriorNode();
    if(from_node && to_node && from_node->isNumber() && to_node->isNumber() && ioperand &&
       OP_CONCAT == ioperand->getOperator())
    {
        size_t partAt = 0;  // starting bit number in child
        BOOST_REVERSE_FOREACH(const Ptr part, ioperand->children())
        {  // concatenated parts
            ...
            else if(partAt < from)
            {
                ...
                newChildren.push_back(makeExtract(makeInteger(32, from - partAt),
                                                  makeInteger(32, part->nBits()), part));
                partAt = partEnd;
                from += need;
            }
            else if(partEnd > to)
    ...
    // If the operand is another extract operation and we know all the limits then they
    // can be replaced with a single extract.
    if(from_node && to_node && from_node->isNumber() && to_node->isNumber() && ioperand &&
       OP_EXTRACT == ioperand->getOperator())
    {
        ...
        if(from2_node && to2_node && from2_node->isNumber() && to2_node->isNumber())
        {
            ...
            return makeExtract(makeInteger(32, from2 + from), makeInteger(32, from2 + to),
                               ioperand->child(2), inode->comment());
        }
    }
...
RolSimplifier::rewrite(Interior* inode) const
{
    ...
    // If the shift amount is known and is a multiple of the operand size, then this is a
    // no-op
...
UextendSimplifier::rewrite(Interior* inode) const
{
    ...
    if(newsize < oldsize)
    {
        return makeExtract(makeInteger(32, 0), makeInteger(32, newsize), inode->child(1),
                           inode->comment());
    }
...
SdivSimplifier::rewrite(Interior* inode) const
{
       ...
       b_leaf->toInt() != 0)
    {
        ...
        {
            // FIXME[Robb P. Matzke 2014-05-05]: not folding constants larger than 64 bits
            // because Sawyer::Container::BitVector does not currently define division.
        }
    }
    return Ptr();
}
...
ShiftSimplifier::combine_strengths(Ptr strength1, Ptr strength2, size_t value_width) const
{
    ...
    // Calculate the width for the sum of the strengths.  If the width of the value being
    // shifted isn't a power of two then we need to avoid overflow in the sum, otherwise
    // overflow doesn't matter.  The sum should be wide enough to hold a shift amount
    // that's the same as the width of the value, otherwise we wouldn't be able to
    // distinguish between the case where modulo addition produced a shift amount that's
    // large enough to decimate the value, as opposed to a shift count of zero which is a
    // no-op.
    ...
    // Zero-extend the strengths if they're not as wide as the sum.  This is because the
    // ADD operator requires that its operands are the same width, and the result will
    // also be that width.
...
ShlSimplifier::rewrite(Interior* inode) const
{
    ...
    // combining the strengths: (shl AMT1 (shl AMT2 X)) ==> (shl (add AMT1 AMT2) X)
    ...
    // If the shift amount is known to be at least as large as the value, then replace the
    ...
    if(sa_leaf && sa_leaf->isNumber())
    {
        ...
        Ptr    hiBits =
            makeExtract(makeInteger(32, 0), makeInteger(32, nHiBits), inode->child(1));
...
Interior::simplifyTop()
{
    ...
    while(InteriorPtr inode = node->isInteriorNode())
    {
        ...
        switch(inode->getOperator())
        {
            ...
            case OP_SMUL:
                newnode =
                    inode->associative()->commutative()->foldConstants(SmulSimplifier());
                break;
            case OP_UDIV:
...
// class method
LeafPtr
Leaf::createExistingVariable(size_t nbits, uint64_t id, const std::string& comment,
                             unsigned flags)
{
    if(0 == nbits)
        throw Exception("variables must have positive width");
...
// class method
LeafPtr
Leaf::createConstant(const Sawyer::Container::BitVector& bits, const std::string& comment,
                     unsigned flags)
{
    Leaf* node      = new Leaf(comment, flags);
...
// class method
LeafPtr
Leaf::createMemory(size_t addressWidth, size_t valueWidth, const std::string& comment,
                   unsigned flags)
{
    if(0 == addressWidth)
...
// class method
LeafPtr
Leaf::createExistingMemory(size_t addressWidth, size_t valueWidth, uint64_t id,
                           const std::string& comment, unsigned flags)
{
    if(0 == addressWidth)
        throw Exception("memory addresses must have positive width");
...
Leaf::printAsSigned(std::ostream& o, Formatter& formatter, bool as_signed)
{
    ...
    if(isNumber())
    {
        ...
        else if(bits_.size() <= 64)
        {
               ...
               0xffff0000 != (ival & 0xffff0000))
            {
                ...
                else
                {
                    ...
                    // is false) was to print only the hexadecimal format and not the
                    // decimal format, so we'll emulate that. [Robb P. Matzke 2013-12-26]
        ...
        else
        {
            ...
            // FIXME[Robb P. Matzke 2014-05-05]: we should change StringUtility functions
            // to handle BitVector arguments also.
            o << "0x" << bits_.toHex();
        }
    }
    ...
    // Bit width of variable.  All variables have this otherwise there's no way for the
    // parser to tell how wide a variable is when reading it back in.
...
Leaf::mayEqual(const Ptr& other_, SMTSolver* solver)
{
    ...
    {
        // We need an SMT solver to figure out things like "x mayEqual (add y 1))", which
        // is true.
        if(solver)
        {
...
Ptr
makeExistingVariable(size_t nbits, uint64_t id, const std::string& comment,
                     unsigned flags)
{
...
Ptr
makeConstant(const Sawyer::Container::BitVector& bits, const std::string& comment,
             unsigned flags)
{
...
Ptr
makeMemory(size_t addressWidth, size_t valueWidth, const std::string& comment,
           unsigned flags)
{
...
Ptr
makeExistingMemory(size_t addressWidth, size_t valueWidth, uint64_t id,
                   const std::string& comment, unsigned flags)
{
...
Ptr
makeExtract(const Ptr& begin, const Ptr& end, const Ptr& a, const std::string& comment,
            unsigned flags)
{
...
Ptr
makeIte(const Ptr& cond, const Ptr& a, const Ptr& b, const std::string& comment,
        unsigned flags)
{
...
Ptr
makeSet(const Ptr& a, const Ptr& b, const Ptr& c, const std::string& comment,
        unsigned flags)
{
...
Ptr
makeSignExtend(const Ptr& newSize, const Ptr& a, const std::string& comment,
               unsigned flags)
{
...
Ptr
makeWrite(const Ptr& mem, const Ptr& addr, const Ptr& a, const std::string& comment,
          unsigned flags)
{
