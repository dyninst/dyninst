...
/*******************************************************************************************************************************
 *                                      Functors that handle individual ARM64
 *instructions kinds
 *******************************************************************************************************************************/
...
struct IP_add_addsub_imm_execute : P {
  void p(D d, Ops ops, I insn, A args, B raw) {
    ...
    result = d->doAddOperation(operand1, operand2, carry_in,
...
struct IP_adc_execute : P {
  void p(D d, Ops ops, I insn, A args, B raw) {
    ...
    result = d->doAddOperation(
        operand1, operand2,
        ops->and_(d->readRegister(d->REG_NZCV), ops->number_(32, 0x2)),
        ops->boolean_(false), nzcv);
...
struct IP_adr_execute : P {
  void p(D d, Ops ops, I insn, A args, B raw) {
    ...
    if ((EXTR(31, 31) == 1)) {
      base = ops->or_(ops->and_(base, ops->number_(12, 0xfff)), d->Zeros(12));
    }
...
struct IP_b_cond_execute : P {  //
  void p(D d, Ops ops, I insn, A args, B raw) {
    d->BranchTo(
        ops->ite(ops->isEqual(d->ConditionHolds(ops->number_(4, EXTR(0, 4))),
                              ops->boolean_(true)),
...
struct IP_cbz_execute : P {  //
  void p(D d, Ops ops, I insn, A args, B raw) {
    ...
    d->BranchTo(ops->ite(
        ops->isEqual(d->isZero(operand1), ops->boolean_(EXTR(24, 24) == 0)),
...
struct IP_tbz_execute : P {  //
  void p(D d, Ops ops, I insn, A args, B raw) {
    ...
    d->BranchTo(ops->ite(
        ops->isEqual(ops->and_(ops->shiftRight(operand, d->read(args[0])),
                               ops->number_(1, 1)),
...
struct IP_ccmn_reg_execute : P {  //
  void p(D d, Ops ops, I insn, A args, B raw) {
    ...
    ops->ite(
        ops->isEqual(d->ConditionHolds(d->read(args[3])), ops->boolean_(true)),
        d->doAddOperation(operand1, operand2, carry_in, ops->boolean_(false),
                          nzcv),
        ops->unspecified_(1));
...
void DispatcherARM64::iproc_init() {
  iproc_set(rose_aarch64_op_add_addsub_imm,
            new ARM64::IP_add_addsub_imm_execute);
  iproc_set(rose_aarch64_op_adds_addsub_imm,
            new ARM64::IP_adds_addsub_imm_execute);
  iproc_set(rose_aarch64_op_sub_addsub_imm,
            new ARM64::IP_sub_addsub_imm_execute);
  iproc_set(rose_aarch64_op_subs_addsub_imm,
            new ARM64::IP_subs_addsub_imm_execute);
  iproc_set(rose_aarch64_op_add_addsub_ext,
            new ARM64::IP_add_addsub_ext_execute);
  iproc_set(rose_aarch64_op_adds_addsub_ext,
            new ARM64::IP_adds_addsub_ext_execute);
  iproc_set(rose_aarch64_op_sub_addsub_ext,
            new ARM64::IP_sub_addsub_ext_execute);
  iproc_set(rose_aarch64_op_subs_addsub_ext,
            new ARM64::IP_subs_addsub_ext_execute);
  iproc_set(rose_aarch64_op_add_addsub_shift,
            new ARM64::IP_add_addsub_shift_execute);
  iproc_set(rose_aarch64_op_adds_addsub_shift,
            new ARM64::IP_adds_addsub_shift_execute);
  iproc_set(rose_aarch64_op_sub_addsub_shift,
            new ARM64::IP_sub_addsub_shift_execute);
  iproc_set(rose_aarch64_op_subs_addsub_shift,
            new ARM64::IP_subs_addsub_shift_execute);
...
RegisterDescriptor DispatcherARM64::instructionPointerRegister() const {
  return REG_PC;
}

RegisterDescriptor DispatcherARM64::stackPointerRegister() const {
  return REG_SP;
}

static bool isStatusRegister(const RegisterDescriptor &reg) {
  return reg.get_major() == armv8_regclass_pstate &&
         reg.get_minor() == armv8_pstatefield_nzcv;
}

RegisterDictionary::RegisterDescriptors DispatcherARM64::get_usual_registers()
    const {
  ...
  BOOST_FOREACH (const RegisterDescriptor &reg,
    ...
    if (isStatusRegister(reg)) registers.push_back(reg);
  }
  return registers;
}

void DispatcherARM64::set_register_dictionary(
...
void DispatcherARM64::setFlagsForResult(const BaseSemantics::SValuePtr &result,
  ...
  BaseSemantics::SValuePtr ofbit =
      operators->extract(carries, nbits - 2, nbits - 1);
  ...
  BaseSemantics::SValuePtr cv =
      operators->or_(operators->shiftLeft(c, operators->number_(1, 1)), v);

  nzcv = operators->extract(operators->or_(nz, cv), 0, 4);
}

BaseSemantics::SValuePtr DispatcherARM64::parity(
    const BaseSemantics::SValuePtr &v) {
  ASSERT_require(v->get_width() == 8);
  BaseSemantics::SValuePtr p1 = operators->extract(v, 1, 2);
  BaseSemantics::SValuePtr p01 =
      operators->xor_(operators->extract(v, 0, 1), p1);
  BaseSemantics::SValuePtr p3 = operators->extract(v, 3, 4);
  BaseSemantics::SValuePtr p23 =
      operators->xor_(operators->extract(v, 2, 3), p3);
  BaseSemantics::SValuePtr p5 = operators->extract(v, 5, 6);
  BaseSemantics::SValuePtr p45 =
      operators->xor_(operators->extract(v, 4, 5), p5);
  BaseSemantics::SValuePtr p7 = operators->extract(v, 7, 8);
  BaseSemantics::SValuePtr p67 =
      operators->xor_(operators->extract(v, 6, 7), p7);
...
BaseSemantics::SValuePtr DispatcherARM64::ConditionHolds(
  ...
  if (operators->isEqual(baseCond, operators->number_(3, 0)))
    result = operators->isEqual(operators->extract(nzcvVal, 2, 3),
                                operators->number_(1, 1));
  if (operators->isEqual(baseCond, operators->number_(3, 1)))
    result = operators->isEqual(operators->extract(nzcvVal, 1, 2),
                                operators->number_(1, 1));
  ...
  if (operators->isEqual(baseCond, operators->number_(3, 3)))
    result = operators->isEqual(operators->extract(nzcvVal, 0, 1),
                                operators->number_(1, 1));
  if (operators->isEqual(baseCond, operators->number_(3, 4)))
    result = operators->ite(
        operators->isEqual(operators->extract(nzcvVal, 1, 2),
                           operators->number_(1, 1)),
  ...
  if (operators->isEqual(baseCond, operators->number_(3, 6)))
    result = operators->ite(
        operators->isEqual(operators->extract(nzcvVal, 3, 4),
                           operators->extract(nzcvVal, 0, 1)),
        operators->ite(operators->isEqual(operators->extract(nzcvVal, 2, 3),
                                          operators->number_(1, 0)),
                       operators->boolean_(true), operators->boolean_(false)),
  ...
  result = operators->ite(
      operators->isEqual(operators->extract(baseCond, 0, 1),
                         operators->number_(1, 1)),
      operators->ite(
          operators->isNotEqual(baseCond, operators->number_(4, 0xF)),
          operators->invert(result), result),
      result);

  return result;
}

BaseSemantics::SValuePtr DispatcherARM64::NOT(
    const BaseSemantics::SValuePtr &expr) {
...
BaseSemantics::SValuePtr DispatcherARM64::doAddOperation(
    BaseSemantics::SValuePtr a, BaseSemantics::SValuePtr b, bool invertCarries,
...
BaseSemantics::SValuePtr DispatcherARM64::readRegister(
...
void DispatcherARM64::writeRegister(const RegisterDescriptor &reg,
                                    const BaseSemantics::SValuePtr &value) {
  if (reg.get_major() == armv8_regclass_gpr ||
      reg.get_major() == armv8_regclass_pc ||
...
void DispatcherARM64::write(SgAsmExpression *e,
                            const BaseSemantics::SValuePtr &value,
                            size_t addr_nbits /*=0*/) {
...
BaseSemantics::SValuePtr DispatcherARM64::fixMemoryAddress(
