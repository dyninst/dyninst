...
{
void
InsnProcessor::process(const BaseSemantics::DispatcherPtr& dispatcher_,
...
/*******************************************************************************************************************************
 *                                      Functors that handle individual ARM64 instructions
 *kinds
 *******************************************************************************************************************************/
...
struct IP_add_addsub_imm_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        result = d->doAddOperation(operand1, operand2, carry_in, ops->boolean_(false), n,
                                   z, c, v);
...
struct IP_adr_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        if((EXTR(31, 31) == 1))
        {
            base = ops->or_(ops->and_(base, ops->number_(64, 0xfffffffffffff000)),
                            d->Zeros(12));
        }
...
struct IP_b_cond_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        d->BranchTo(ops->ite(ops->isEqual(d->ConditionHolds(ops->number_(4, EXTR(0, 4))),
                                          ops->boolean_(true)),
...
{  //
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        d->BranchTo(
            ops->ite(ops->isEqual(d->isZero(operand1), ops->boolean_(EXTR(24, 24) == 0)),
...
{  //
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        d->BranchTo(
            ops->ite(ops->isEqual(ops->and_(ops->shiftRight(operand, d->read(args[1])),
                                            ops->number_(1, 1)),
...
{  //
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        operand2 = ops->ite(
            ops->isEqual(d->ConditionHolds(d->read(args[3])), ops->boolean_(true)),
            (EXTR(30, 30) == 1 ? (carry_in = true, d->NOT(operand2)) : operand2),
            operand2);
        ops->ite(ops->isEqual(d->ConditionHolds(d->read(args[3])), ops->boolean_(true)),
                 d->doAddOperation(operand1, operand2, carry_in, ops->boolean_(false), n,
                                   z, c, v),
                 ops->unspecified_(1));
...
// TODO: For ubfm/sbfm based instructions, update the grammar to declare variables for
// imms/immr and wmask/tmask instead of reading/calculating them twice
struct IP_ubfm_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        BaseSemantics::SValuePtr tmask = d->getBitfieldMask(
            EXTR(16, 21), EXTR(10, 15), EXTR(22, 22), false, (EXTR(31, 31) + 1) * 32);
        BaseSemantics::SValuePtr wmask = d->getBitfieldMask(
            EXTR(16, 21), EXTR(10, 15), EXTR(22, 22), true, (EXTR(31, 31) + 1) * 32);
        ...
        BaseSemantics::SValuePtr bot =
            ops->or_(ops->and_(dst, d->NOT(wmask)),
                     ops->and_(d->ROR(src, d->read(args[2])), wmask));
        BaseSemantics::SValuePtr top;
        if(d->extend(raw))
            top = d->Replicate(
                ops->and_(ops->shiftRight(src, d->read(args[3])), ops->number_(1, 1)));
        else
            top = dst;
...
struct IP_uxtb_ubfm_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        /*BaseSemantics::SValuePtr tmask = d->getBitfieldMask(EXTR(16, 21), EXTR(10, 15),
           EXTR(22, 22), false, (EXTR(31, 31) + 1) * 32); BaseSemantics::SValuePtr wmask =
           d->getBitfieldMask(EXTR(16, 21), EXTR(10, 15), EXTR(22, 22), true, (EXTR(31,
           31) + 1) * 32); BaseSemantics::SValuePtr dst; if (d->inzero(raw)) dst =
           d->Zeros(64); else dst = d->read(args[0]); BaseSemantics::SValuePtr src =
           d->read(args[1]); BaseSemantics::SValuePtr bot = ops->or_(ops->and_(dst,
           d->NOT(wmask)), ops->and_(d->ROR(src, ops->number_(32, EXTR(16, 21))), wmask));
            BaseSemantics::SValuePtr top;
            if (d->extend(raw))
            top = d->Replicate(ops->and_(ops->shiftRight(src, ops->number_(32, EXTR(10,
           15))), ops->number_(1, 1))); else top = dst; d->write(args[0],
           ops->or_(ops->and_(top, d->NOT(tmask)), ops->and_(bot, tmask)));
            */
        BaseSemantics::SValuePtr src      = d->read(args[1]);
...
struct IP_uxth_ubfm_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        BaseSemantics::SValuePtr bot =
            ops->or_(ops->and_(dst, d->NOT(wmask)),
                     ops->and_(d->ROR(src, ops->number_(32, EXTR(16, 21))), wmask));
        BaseSemantics::SValuePtr top;
        if(d->extend(raw))
            top = d->Replicate(
                ops->and_(ops->shiftRight(src, ops->number_(32, EXTR(10, 15))),
                          ops->number_(1, 1)));
        else
            top = dst;
        d->write(args[0], ops->or_(ops->and_(top, d->NOT(tmask)), ops->and_(bot, tmask)));
...
struct IP_movz_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        result = ops->or_(
            ops->extract(result, 0, (EXTR(21, 22) << 4)),
            ops->or_(
                ops->shiftLeft(d->read(args[1]), ops->number_(32, (EXTR(21, 22) << 4))),
                ops->shiftLeft(ops->extract(result, (EXTR(21, 22) << 4) + 15 + 1,
                                            result->get_width()),
...
struct IP_orr_log_shift_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        if(d->setflags(raw))
        {
            d->writeRegister(d->REG_N, ops->extract(result, result->get_width() - 1,
                                                    result->get_width()));
    ...
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        switch((EXTR(22, 22) ^ EXTR(23, 23)))
        {
            case MemOp_STORE: {
                ...
                d->writeMemory(ops->add(address, ops->number_(32, dbytes)), dbytes,
                               data2);
            }
            break;
            case MemOp_LOAD: {
                ...
                data2 =
                    d->readMemory(ops->add(address, ops->number_(32, dbytes)), dbytes);
...
{  //
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        {
            d->writeRegister(RegisterDescriptor(armv8_regclass_gpr, armv8_gpr_zr, 0, 64),
                             result);
        }
    }
};
...
struct IP_sbc_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        result = d->doAddOperation(operand1, operand2, d->readRegister(d->REG_C),
                                   ops->boolean_(false), n, z, c, v);
...
struct IP_csel_execute : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        result = ops->ite(
            ops->isEqual(d->ConditionHolds(ops->number_(32, d->getConditionVal(raw))),
                         ops->boolean_(true)),
            operand1, op2);
        d->write(args[0], result);
    }
    ...
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        if(d->subop(raw))
        {
            result = ops->add(d->Int(operand3, true),
                              ops->negate(ops->unsignedMultiply(d->Int(operand1, true),
                                                                d->Int(operand2, true))));
        }
        else
        {
...
{  //
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        result = ops->ite(ops->isEqual(d->isZero(operand2), ops->boolean_(true)),
                          d->Zeros(64), result);
...
DispatcherARM64::iproc_init()
{
    ...
    iproc_set(rose_aarch64_op_cmp_subs_addsub_imm,
              new ARM64::IP_cmp_subs_addsub_imm_execute);
    iproc_set(rose_aarch64_op_cmp_subs_addsub_ext,
              new ARM64::IP_cmp_subs_addsub_ext_execute);
    iproc_set(rose_aarch64_op_cmp_subs_addsub_shift,
              new ARM64::IP_cmp_subs_addsub_shift_execute);
    iproc_set(rose_aarch64_op_cmn_adds_addsub_imm,
              new ARM64::IP_cmn_adds_addsub_imm_execute);
    iproc_set(rose_aarch64_op_cmn_adds_addsub_ext,
              new ARM64::IP_cmn_adds_addsub_ext_execute);
    iproc_set(rose_aarch64_op_cmn_adds_addsub_shift,
              new ARM64::IP_cmn_adds_addsub_shift_execute);
    ...
    iproc_set(rose_aarch64_op_tst_ands_log_shift,
              new ARM64::IP_tst_ands_log_shift_execute);
    ...
    iproc_set(rose_aarch64_op_neg_sub_addsub_shift,
              new ARM64::IP_neg_sub_addsub_shift_execute);
    iproc_set(rose_aarch64_op_negs_subs_addsub_shift,
              new ARM64::IP_negs_subs_addsub_shift_execute);
    iproc_set(rose_aarch64_op_mvn_orn_log_shift, new ARM64::IP_mvn_orn_log_shift_execute);
    iproc_set(rose_aarch64_op_mov_add_addsub_imm,
              new ARM64::IP_mov_add_addsub_imm_execute);
...
void
DispatcherARM64::setFlagsForResult(
    const BaseSemantics::SValuePtr& result, const BaseSemantics::SValuePtr& carries,
    bool invertCarries, size_t nbits, BaseSemantics::SValuePtr& n,
    BaseSemantics::SValuePtr& z, BaseSemantics::SValuePtr& c, BaseSemantics::SValuePtr& v)
{
...
{
    Dyninst::AST::Ptr condExpr =
        SymEvalSemantics::SValue::promote(cond)->get_expression();
    Dyninst::DataflowAPI::ConstantAST* constAST =
        dynamic_cast<Dyninst::DataflowAPI::ConstantAST*>(condExpr.get());
...
BaseSemantics::SValuePtr
DispatcherARM64::ROR(const BaseSemantics::SValuePtr& expr,
...
BaseSemantics::SValuePtr
DispatcherARM64::getBitfieldMask(int immr, int imms, int N, bool iswmask, int datasize)
{
    int hsbarg = (N << 6) | (~imms);
    int len    = 0;
...
DispatcherARM64::getDatasize(uint32_t raw)
{
    ...
    {
        int v27_29 = IntegerOps::extract2<uint32_t>(27, 29, raw),
            v23_25 = IntegerOps::extract2<uint32_t>(23, 25, raw);
...
DispatcherARM64::getShiftType(uint32_t raw)
{
    ...
    switch(v10_11)
    {
        ...
        default:
            ASSERT_not_reachable("Cannot have a value greater than 3 for a 2-bit field "
                                 "(field: op2, bits: 10..11)!");
    }
}
...
{
    if(IntegerOps::extract2<uint32_t>(24, 31, raw) == 0x54 &&
       IntegerOps::extract2<uint32_t>(4, 4, raw) == 0)
                                ...
                                BaseSemantics::SValuePtr& c, BaseSemantics::SValuePtr& v)
{
    ...
    BaseSemantics::SValuePtr result = operators->addWithCarries(
...
void
DispatcherARM64::writeRegister(const RegisterDescriptor&       reg,
...
void
DispatcherARM64::writeMemory(const BaseSemantics::SValuePtr& addr, size_t writeSize,
...
/*Just returning the expression as-is, since there is no flag or variable in semantics
indicating whether or not a SValue should be treated as signed/unsigned. The signed-ness
should be taken into account when performing an operation on this value instead */
BaseSemantics::SValuePtr
...
BaseSemantics::SValuePtr
DispatcherARM64::ShiftReg(const BaseSemantics::SValuePtr& src, int shiftType,
...
{
    size_t                   len = expr->get_width();
    BaseSemantics::SValuePtr arg = operators->xor_(operators->extract(expr, 1, len),
                                                   operators->extract(expr, 0, len - 1));
...
DispatcherARM64::Int(const BaseSemantics::SValuePtr& expr, bool isUnsigned)
{
    ...
    else
    {
        ...
        for(int idx = 0; idx < len; idx++)
            if(operators->isEqual(operators->extract(expr, idx, idx + 1),
                                  operators->number_(1, 1)))
        ...
        if(operators->isEqual(operators->extract(expr, len, len + 1),
                              operators->number_(1, 1)))
            ret = operators->add(
                ret, operators->negate(operators->number_(len + 1, 2 << len)));

        return ret;
    }
}

/* Return the input expression as-is. At this point, it is assumed that any RiscOperators
 * implementation that performs division and passes the result to this method would have
 * already performed the rounding (most likely by using the / operator) to match how
 * integer division is normally performed in C/C++. Of course, this would be invalid for
 * floating point division but that is not currently supported. */
BaseSemantics::SValuePtr
