...
Sawyer::Optional<BaseSemantics::SValuePtr> SValue::createOptionalMerge(
    const BaseSemantics::SValuePtr &other_, const BaseSemantics::MergerPtr &,
    SMTSolver *) const {
  // There's no official way to represent BOTTOM
  throw BaseSemantics::NotImplemented(
      "SValue merging for ConcreteSemantics is not supported", NULL);
}

void SValue::bits(const Sawyer::Container::BitVector &newBits) {
  ASSERT_require(newBits.size() == bits_.size());
  ASSERT_require(newBits.size() == get_width());
  bits_ = newBits;
}

bool SValue::may_equal(const BaseSemantics::SValuePtr &other,
                       SMTSolver *) const {
  return 0 == bits_.compare(SValue::promote(other)->bits());
}

bool SValue::must_equal(const BaseSemantics::SValuePtr &other,
                        SMTSolver *) const {
  return 0 == bits_.compare(SValue::promote(other)->bits());
}

void SValue::set_width(size_t newWidth) {
...
uint64_t SValue::get_number() const { return bits_.toInteger(); }

void SValue::print(std::ostream &out, BaseSemantics::Formatter &) const {
  if (get_width() <= 64) {
...
void MemoryState::pageSize(rose_addr_t nBytes) {
  ASSERT_require(map_.isEmpty());
  pageSize_ = std::max(nBytes, (rose_addr_t)1);
}

void MemoryState::allocatePage(rose_addr_t va) {
  rose_addr_t pageVa = alignDown(va, pageSize_);
  unsigned acc = MemoryMap::READABLE | MemoryMap::WRITABLE;
  map_.insert(
      AddressInterval::baseSize(pageVa, pageSize_),
      MemoryMap::Segment(MemoryMap::AllocatingBuffer::instance(pageSize_), 0,
                         acc, "ConcreteSemantics demand allocated"));
}

void MemoryState::memoryMap(const MemoryMap &map,
                            Sawyer::Optional<unsigned> padAccess) {
  map_ = map;
  rose_addr_t va = 0;
  while (map_.atOrAfter(va).next().assignTo(va)) {
    ...
    if (pageVa < va) {
      ...
      map_.insert(
          AddressInterval::hull(pageVa, va - 1),
          MemoryMap::Segment(MemoryMap::AllocatingBuffer::instance(va - pageVa),
                             0, acc, "padding"));
    }
    ...
    if (AddressInterval unused = map_.unmapped(va)) {
      ...
      if (nextPageVa > va) {
        unsigned acc =
            padAccess ? *padAccess : map_.get(va - 1).accessibility();
        map_.insert(AddressInterval::hull(va, nextPageVa - 1),
                    MemoryMap::Segment(
                        MemoryMap::AllocatingBuffer::instance(nextPageVa - va),
                        0, acc, "padding"));
        va = nextPageVa - 1;
      }
    } else {
...
BaseSemantics::SValuePtr MemoryState::readMemory(
    const BaseSemantics::SValuePtr &addr_,
    const BaseSemantics::SValuePtr &dflt_,
    BaseSemantics::RiscOperators *addrOps,
    BaseSemantics::RiscOperators *valOps) {
  ASSERT_require2(8 == dflt_->get_width(),
                  "ConcreteSemantics::MemoryState requires memory cells "
                  "contain 8-bit data");
...
void MemoryState::writeMemory(const BaseSemantics::SValuePtr &addr_,
                              const BaseSemantics::SValuePtr &value_,
                              BaseSemantics::RiscOperators *addrOps,
                              BaseSemantics::RiscOperators *valOps) {
  ASSERT_require2(8 == value_->get_width(),
                  "ConcreteSemantics::MemoryState requires memory cells "
                  "contain 8-bit data");
  rose_addr_t addr = addr_->get_number();
  uint8_t value = value_->get_number();
  if (!map_.at(addr).exists()) allocatePage(addr);
  map_.at(addr).limit(1).write(&value);
}

bool MemoryState::merge(const BaseSemantics::MemoryStatePtr &other,
                        BaseSemantics::RiscOperators *addrOps,
                        BaseSemantics::RiscOperators *valOps) {
  throw BaseSemantics::NotImplemented(
      "MemoryState merging for ConcreteSemantics is not supported", NULL);
}

void MemoryState::print(std::ostream &out, Formatter &) const {
  map_.dump(out);
  ...
  while (map_.atOrAfter(pageVa).next().assignTo(pageVa)) {
    ...
    HexdumpFormat fmt;
    SgAsmExecutableFileFormat::hexdump(out, pageVa, (const unsigned char *)page,
                                       pageSize_, fmt);
    out << "\n";
    if (pageVa + (pageSize_ - 1) == map_.hull().greatest()) break;
    pageVa += pageSize_;
  }
  delete[] page;
}
...
SValuePtr RiscOperators::svalue_number(
...
void RiscOperators::interrupt(int majr, int minr) { currentState()->clear(); }

BaseSemantics::SValuePtr RiscOperators::and_(
...
BaseSemantics::SValuePtr RiscOperators::or_(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::xor_(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::invert(
...
BaseSemantics::SValuePtr RiscOperators::extract(
    const BaseSemantics::SValuePtr &a_, size_t begin_bit, size_t end_bit) {
  ASSERT_require(end_bit <= a_->get_width());
  ASSERT_require(begin_bit < end_bit);
  BitVector result(end_bit - begin_bit);
  result.copy(result.hull(), SValue::promote(a_)->bits(),
              BitRange::hull(begin_bit, end_bit - 1));
  return svalue_number(result);
}

BaseSemantics::SValuePtr RiscOperators::concat(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  ...
  result.copy(BitRange::baseSize(a_->get_width(), b_->get_width()),
              SValue::promote(b_)->bits(),
              BitRange::baseSize(0, b_->get_width()));
  return svalue_number(result);
}

BaseSemantics::SValuePtr RiscOperators::leastSignificantSetBit(
    const BaseSemantics::SValuePtr &a_) {
  uint64_t count =
      SValue::promote(a_)->bits().leastSignificantSetBit().orElse(0);
  return svalue_number(a_->get_width(), count);
}

BaseSemantics::SValuePtr RiscOperators::mostSignificantSetBit(
    const BaseSemantics::SValuePtr &a_) {
  uint64_t count =
      SValue::promote(a_)->bits().mostSignificantSetBit().orElse(0);
  return svalue_number(a_->get_width(), count);
}

BaseSemantics::SValuePtr RiscOperators::rotateLeft(
...
BaseSemantics::SValuePtr RiscOperators::rotateRight(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &sa_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::shiftLeft(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &sa_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::shiftRight(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &sa_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::shiftRightArithmetic(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &sa_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::equalToZero(
    const BaseSemantics::SValuePtr &a_) {
  return svalue_boolean(SValue::promote(a_)->bits().isEqualToZero());
}

BaseSemantics::SValuePtr RiscOperators::ite(
    const BaseSemantics::SValuePtr &sel_, const BaseSemantics::SValuePtr &a_,
...
BaseSemantics::SValuePtr RiscOperators::unsignedExtend(
...
BaseSemantics::SValuePtr RiscOperators::signExtend(
    const BaseSemantics::SValuePtr &a_, size_t new_width) {
...
BaseSemantics::SValuePtr RiscOperators::add(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  BitVector result = SValue::promote(a_)->bits();
...
BaseSemantics::SValuePtr RiscOperators::addWithCarries(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_,
  ...
  BitVector ae = SValue::promote(a_)->bits();
  ae.resize(nbits + 1);
  BitVector be = SValue::promote(b_)->bits();
  be.resize(nbits + 1);
  BitVector ce = SValue::promote(c_)->bits();
  ce.resize(nbits + 1);
...
BaseSemantics::SValuePtr RiscOperators::negate(
    const BaseSemantics::SValuePtr &a_) {
  BitVector result = SValue::promote(a_)->bits();
  result.negate();
  return svalue_number(result);
...
BaseSemantics::SValuePtr RiscOperators::signedDivide(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  // FIXME[Robb P. Matzke 2015-03-31]: BitVector doesn't have a divide method
  if (a_->get_width() > 64 || b_->get_width() > 64) {
    throw BaseSemantics::Exception(
        "signedDivide x[" + StringUtility::addrToString(a_->get_width()) +
...
BaseSemantics::SValuePtr RiscOperators::signedModulo(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  // FIXME[Robb P. Matzke 2015-03-31]: BitVector doesn't have a modulo method
  if (a_->get_width() > 64 || b_->get_width() > 64) {
    throw BaseSemantics::Exception(
        "signedModulo x[" + StringUtility::addrToString(a_->get_width()) +
            "] % y[" + StringUtility::addrToString(b_->get_width()) +
            "] is not implemented",
        currentInstruction());
  }
...
BaseSemantics::SValuePtr RiscOperators::signedMultiply(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  ...
  if (a_->get_width() == 64 && b_->get_width() == 64) {
    // Do long multiplication with 32 bits at a time in 64-bit variables (32
    // bits * 32 bits = 64 bits)
    // FIXME[Robb P. Matzke 2015-03-23]: use arbitrary width vector multiply in
    // Sawyer when it's available.
    ...
    throw BaseSemantics::Exception(
        "signedMultiply x[" + StringUtility::addrToString(a_->get_width()) +
            "] * y[" + StringUtility::addrToString(b_->get_width()) +
            "] is not implemented",
        currentInstruction());
  } else {
    ASSERT_require2(a_->get_width() + b_->get_width() <= 64,
                    "not implemented yet");
    int64_t a = IntegerOps::signExtend2(a_->get_number(), a_->get_width(), 64);
    int64_t b = IntegerOps::signExtend2(b_->get_number(), b_->get_width(), 64);
...
BaseSemantics::SValuePtr RiscOperators::unsignedDivide(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  SValuePtr a = SValue::promote(a_);
  SValuePtr b = SValue::promote(b_);

// This is a common case on 64-bit architectures
// FIXME[Robb P. Matzke 2015-06-24]: this will probably only compile with GCC
// >4.x on a 64-bit machine. The real solution
// would be to either use a multi-precision library (sort of overkill) or
// implement division in Sawyer's BitVector class.
#ifdef __x86_64
  ...
  if (a->get_width() > 64 || b->get_width() > 64) {
    throw BaseSemantics::Exception(
        "unsignedDivide x[" + StringUtility::addrToString(a->get_width()) +
...
BaseSemantics::SValuePtr RiscOperators::unsignedModulo(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  SValuePtr a = SValue::promote(a_);
  SValuePtr b = SValue::promote(b_);
...
// implement division in Sawyer's BitVector class.
  ...
  if (a->get_width() > 64 || b->get_width() > 64) {
    throw BaseSemantics::Exception(
        "unsignedModulo x[" + StringUtility::addrToString(a->get_width()) +
            "] % y[" + StringUtility::addrToString(b->get_width()) +
            "] is not implemented",
        currentInstruction());
  }
...
BaseSemantics::SValuePtr RiscOperators::unsignedMultiply(
    const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &b_) {
  // FIXME[Robb P. Matzke 2015-03-31]: BitVector doesn't have a multiply method
  ASSERT_require2(a_->get_width() <= 64, "not implemented");
  ASSERT_require2(b_->get_width() <= 64, "not implemented");
  ...
  if (a_->get_width() == 64 && b_->get_width() == 64) {
    ...
    throw BaseSemantics::Exception(
        "unsignedMultiply x[" + StringUtility::addrToString(a_->get_width()) +
            "] * y[" + StringUtility::addrToString(b_->get_width()) +
            "] is not implemented",
        currentInstruction());
  } else {
...
BaseSemantics::SValuePtr RiscOperators::readMemory(
    const RegisterDescriptor &segreg, const BaseSemantics::SValuePtr &address,
  ...
  if (cond->is_number() && !cond->get_number()) return dflt;
  ...
  for (size_t bytenum = 0; bytenum < nbits / 8; ++bytenum) {
    size_t byteOffset = ByteOrder::ORDER_MSB == mem->get_byteOrder()
                            ? nbytes - (bytenum + 1)
                            : bytenum;
    BaseSemantics::SValuePtr byte_dflt =
        extract(dflt, 8 * byteOffset, 8 * byteOffset + 8);
    BaseSemantics::SValuePtr byte_addr =
        add(address, number_(address->get_width(), bytenum));
    ...
    SValuePtr byte_value = SValue::promote(
    ...
    if (0 == bytenum) {
      ...
      throw BaseSemantics::Exception(
          "multi-byte read with memory having unspecified byte order",
          currentInstruction());
    }
  }
...
void RiscOperators::writeMemory(const RegisterDescriptor &segreg,
                                const BaseSemantics::SValuePtr &address,
                                const BaseSemantics::SValuePtr &value_,
                                const BaseSemantics::SValuePtr &cond) {
  ASSERT_require(1 == cond->get_width());  // FIXME: condition is not used
  if (cond->is_number() && !cond->get_number()) return;
  ...
  for (size_t bytenum = 0; bytenum < nbytes; ++bytenum) {
    ...
    if (1 == nbytes) {
      ...
      // See BaseSemantics::MemoryState::set_byteOrder
      throw BaseSemantics::Exception(
          "multi-byte write with memory having unspecified byte order",
          currentInstruction());
    }

    BaseSemantics::SValuePtr byte_value =
        extract(value, 8 * byteOffset, 8 * byteOffset + 8);
    BaseSemantics::SValuePtr byte_addr =
        add(address, number_(address->get_width(), bytenum));
...
double RiscOperators::exprToDouble(const BaseSemantics::SValuePtr &a,
  ...
  if (aType == SageBuilderAsm::buildIeee754Binary64()) {
    ...
    throw BaseSemantics::NotImplemented("exprToDouble type not supported",
                                        currentInstruction());
  }
}

BaseSemantics::SValuePtr RiscOperators::doubleToExpr(double d,
  ...
  if (retType == SageBuilderAsm::buildIeee754Binary64()) {
    ...
    throw BaseSemantics::NotImplemented("doubleToExpr type not supported",
                                        currentInstruction());
  }
}

BaseSemantics::SValuePtr RiscOperators::fpFromInteger(
    const BaseSemantics::SValuePtr &intValue, SgAsmFloatType *retType) {
  ASSERT_require(intValue->is_number());
  return doubleToExpr((double)intValue->get_number(), retType);
}

BaseSemantics::SValuePtr RiscOperators::fpToInteger(
    const BaseSemantics::SValuePtr &a, SgAsmFloatType *aType,
    const BaseSemantics::SValuePtr &dflt) {
  double ad = exprToDouble(a, aType);
  size_t nBits = dflt->get_width();
  double minInt = -pow(2.0, nBits - 1);
  double maxInt = pow(2.0, nBits - 1);
  if (isnan(ad) || ad < minInt || ad >= maxInt) return dflt;
  int64_t ai = ad;
  return number_(nBits, ai);
}

BaseSemantics::SValuePtr RiscOperators::fpAdd(const BaseSemantics::SValuePtr &a,
...
BaseSemantics::SValuePtr RiscOperators::fpSubtract(
...
BaseSemantics::SValuePtr RiscOperators::fpMultiply(
    const BaseSemantics::SValuePtr &a, const BaseSemantics::SValuePtr &b,
    SgAsmFloatType *fpType) {
  double ad = exprToDouble(a, fpType);
  double bd = exprToDouble(b, fpType);
...
BaseSemantics::SValuePtr RiscOperators::fpRoundTowardZero(
    const BaseSemantics::SValuePtr &a, SgAsmFloatType *fpType) {
  double ad = exprToDouble(a, fpType);
