...
std::ostream &operator<<(std::ostream &o, const SMTSolver::Exception &e) {
...
// class method
SMTSolver::Stats SMTSolver::get_class_stats() {
  boost::lock_guard<boost::mutex> lock(class_stats_mutex);
  return class_stats;
}

// class method
void SMTSolver::reset_class_stats() {
  boost::lock_guard<boost::mutex> lock(class_stats_mutex);
  class_stats = Stats();
}

SymbolicExpr::Ptr SMTSolver::evidence_for_address(uint64_t addr) {
  return evidence_for_name(StringUtility::addrToString(addr));
}

SMTSolver::Satisfiable SMTSolver::trivially_satisfiable(
    const std::vector<SymbolicExpr::Ptr> &exprs_) {
  std::vector<SymbolicExpr::Ptr> exprs(exprs_.begin(), exprs_.end());
  for (size_t i = 0; i < exprs.size(); ++i) {
    if (exprs[i]->isNumber()) {
      ASSERT_require(1 == exprs[i]->nBits());
      if (0 == exprs[i]->toInt()) return SAT_NO;
...
SMTSolver::Satisfiable SMTSolver::satisfiable(
  ...
  if (retval != SAT_UNKNOWN) return retval;
  ...
  /* Run the solver and read its output. The first line should be the word "sat"
   * or "unsat" */
  {
    ...
    if (debug) {
      ...
      fprintf(
          debug, "SMT Solver reported: %s\n",
          (SAT_YES == retval ? "sat" : SAT_NO == retval ? "unsat" : "unknown"));
      fprintf(debug, "SMT Solver output:\n%s",
              StringUtility::prefixLines(output_text, "     ").c_str());
    }
  }

  if (SAT_YES == retval) parse_evidence();
#endif
  return retval;
}

SMTSolver::Satisfiable SMTSolver::satisfiable(const SymbolicExpr::Ptr &tn) {
...
SMTSolver::Satisfiable SMTSolver::satisfiable(
    std::vector<SymbolicExpr::Ptr> exprs, const SymbolicExpr::Ptr &expr) {
  if (expr != NULL) exprs.push_back(expr);
  return satisfiable(exprs);
}
