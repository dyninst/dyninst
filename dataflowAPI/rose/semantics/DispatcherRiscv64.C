 ...
 *******************************************************************************************************************************/
                    ...
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        switch (op) {
                            ...
                            case rose_riscv64_op_bge: {
                                taken = ops->isSignedGreaterThanOrEqual(d->read(rs1, 64, 0), d->read(rs2, 64, 0));
                                break;
                            }
                            ...
                            case rose_riscv64_op_bgeu: {
                                taken = ops->isUnsignedGreaterThanOrEqual(d->read(rs1, 64, 0), d->read(rs2, 64, 0));
                                break;
                            }
                            default: {
                                assert(0);
                    ...
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        switch (op) {
                            ...
                            case rose_riscv64_op_slti: {
                                d->write(rd, d->ZeroExtend(ops->isSignedLessThan(d->read(rs1, 64, 0), immext), 64));
                                break;
                            }
                            case rose_riscv64_op_sltiu: {
                                d->write(rd, d->ZeroExtend(ops->isUnsignedLessThan(d->read(rs1, 64, 0), immext), 64));
                                break;
                            }
                    ...
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        switch (op) {
                            ...
                            case rose_riscv64_op_slt: {
                                d->write(rd, d->ZeroExtend(ops->isSignedLessThan(d->read(rs1, 64, 0), d->read(rs2, 64, 0)), 64));
                                break;
                            }
                            case rose_riscv64_op_sltu: {
                                d->write(rd, d->ZeroExtend(ops->isUnsignedLessThan(d->read(rs1, 64, 0), d->read(rs2, 64, 0)), 64));
                                break;
                            }
                            ...
                            case rose_riscv64_op_sll: {
                                d->write(rd, ops->shiftLeft(d->read(rs1, 64, 0), ops->extract(d->read(rs2, 64, 0), 0, 5)));
                                break;
                            }
                            case rose_riscv64_op_srl: {
                                d->write(rd, ops->shiftRight(d->read(rs1, 64, 0), ops->extract(d->read(rs2, 64, 0), 0, 5)));
                                break;
                            }
                            ...
                            case rose_riscv64_op_sra: {
                                d->write(rd, ops->shiftRightArithmetic(d->read(rs1, 64, 0), ops->extract(d->read(rs2, 64, 0), 0, 5)));
                                break;
                            }
                            default: {
                                assert(0);
                ...
                struct IP_LOAD : P {
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        d->write(rd, ops->ite(ops->number_(64, is_unsigned), d->SignExtend(data, 64), d->ZeroExtend(data, 64)));
                        return;
                    }
                };
                ...
                struct IP_STORE : P {
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        BaseSemantics::SValuePtr tmp;
                        //offset = d->SignExtend(imm, 64);
                        data = ops->extract(d->read(rs2, 64, 0), ops->number_(64, 0), ops->subtract(ops->signedMultiply(ops->number_(64, width), ops->number_(64, 8)), ops->number_(64, 1)));
                        tmp = d->effectiveAddress(rs1);
                ...
                struct IP_DIV : P {
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        quotient = ops->ite(ops->isEqual(rs2_int, ops->number_(64, 0)), ops->number_(64, -1), ops->ite(ops->boolean_(ops->number_(64, is_unsigned)), ops->signedDivide(rs1_int, rs2_int), ops->unsignedDivide(rs2_int, rs1_int)));
                        quotient = ops->ite(ops->and_(ops->boolean_(ops->invert(ops->boolean_(ops->number_(64, is_unsigned)))), ops->boolean_(ops->isSignedGreaterThanOrEqual(quotient, ops->shiftLeft(ops->number_(64, 1), ops->number_(64, 63))))), ops->negate(ops->shiftLeft(ops->number_(64, 1), ops->number_(64, 63))), quotient);
                ...
                struct IP_REM : P {
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        rs1_int = ops->ite(ops->number_(64, is_unsigned), rs1_bits, rs1_bits);
                        rs2_int = ops->ite(ops->number_(64, is_unsigned), rs2_bits, rs2_bits);
                        remainder = ops->ite(ops->isEqual(rs2_int, ops->number_(64, 0)), rs1_int, ops->ite(ops->boolean_(ops->number_(64, is_unsigned)), ops->signedModulo(rs1_int, rs2_int), ops->unsignedModulo(rs2_int, rs1_int)));
                ...
                struct IP_DIVW : P {
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        rs2_int = ops->ite(ops->number_(64, is_unsigned), rs2_bits, rs2_bits);
                        quotient = ops->ite(ops->isEqual(rs2_int, ops->number_(64, 0)), ops->number_(64, -1), ops->ite(ops->boolean_(ops->number_(64, is_unsigned)), ops->signedDivide(rs1_int, rs2_int), ops->unsignedDivide(rs2_int, rs1_int)));
                        quotient = ops->ite(ops->and_(ops->boolean_(ops->invert(ops->boolean_(ops->number_(64, is_unsigned)))), ops->boolean_(ops->isSignedGreaterThanOrEqual(quotient, ops->shiftLeft(ops->number_(64, 1), ops->number_(64, 31))))), ops->negate(ops->shiftLeft(ops->number_(64, 1), ops->number_(64, 31))), quotient);
                ...
                struct IP_AMO : P {
                    void p(D d, Ops ops, I insn, A args, B raw) {
                        ...
                        addr = vaddr;
                        rs2_val = ops->ite(ops->isSignedLessThanOrEqual(ops->number_(64, width), ops->number_(64, 8)), ops->extract(d->read(rs2, 64, 0), ops->number_(64, 0), ops->signedMultiply(ops->number_(64, width), ops->number_(64, 8))), ops->extract(d->read(rs2, 64, 0), ops->number_(64, 0), ops->signedMultiply(ops->number_(64, width), ops->number_(64, 8))));
                        ...
                        switch (op) {
                            ...
                            // Currently AMOCAS is not supported in Capstone
                            //ops->ite(ops->and_(ops->boolean_(ops->isEqual(op, rose_riscv64_op_amocas)), ops->boolean_(ops->isNotEqual(loaded, ops->ite(ops->isSignedLessThanOrEqual(ops->number_(64, width), ops->number_(64, 8)), ops->extract(d->read(rd, 64, 0), ops->number_(64, 0), ops->signedMultiply(ops->number_(64, 8), ops->number_(64, width))), ops->extract(d->read(rd, 64, 0), ops->number_(64, 0), ops->signedMultiply(ops->number_(64, 8), ops->number_(64, width))))))), d->write(rd, ops->ite(ops->isSignedLessThanOrEqual(ops->number_(64, width), ops->number_(64, 8)), d->SignExtend(loaded, 64), d->SignExtend(loaded, 128))), d->writeMemory(addr, width, d->SignExtend(result, ops->signedMultiply(ops->number_(64, 8), ops->number_(64, width)))));
                             d->writeMemory(addr, width, result);
                             //d->write(rd, ops->ite(ops->isSignedLessThanOrEqual(ops->number_(64, width), ops->number_(64, 8)), d->SignExtend(loaded, 64), d->SignExtend(loaded, 128)));
