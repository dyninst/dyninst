 ...
 *
 *  Semantics in a concrete domain, where all values are actual known bits.
 * Since the symbolic domain does constant folding,
 *  it also can be used as a concrete domain, although using ConcreteSemantics
 * is probably somewhat faster. */
...
/** Smart-ownership pointer to a concrete semantic value. See @ref
 * heap_object_shared_ownership. */
typedef Sawyer::SharedPointer<class SValue> SValuePtr;

/** Formatter for symbolic values. */
typedef BaseSemantics::Formatter
    Formatter;  // we might extend this in the future

/** Type of values manipulated by the concrete domain.
 *
 *  Values of type type are used whenever a value needs to be stored, such as
 * memory addresses, the values stored at those
 *  addresses, the values stored in registers, the operands for RISC operations,
 * and the results of those operations.  Each
 *  value has a known size and known bits.  All RISC operations on these values
 * will produce a new known value. This type is
 *  not capable of storing an undefined value, and any attempt to create an
 * undefined value will create a value with all bits
 *  cleared instead. */
class SValue : public BaseSemantics::SValue {
 ...
 public:
  /** Instantiate a new prototypical value. Prototypical values are only used
   * for their virtual constructors. */
  static SValuePtr instance() { return SValuePtr(new SValue(1)); }

  /** Instantiate a new undefined value of specified width.
   *
   *  This semantic domain has no representation for an undefined values--all
   * values are defined. Therefore any attempt to
   *  create an undefined value will result in all bits being set to zero
   * instead. */
 ...
 public:
  /** Promote a base value to a SymbolicSemantics value.  The value @p v must
   * have a SymbolicSemantics::SValue dynamic type. */
...
/** Shared-ownership pointer to a concrete memory state. See @ref
 * heap_object_shared_ownership. */
typedef boost::shared_ptr<class MemoryState> MemoryStatePtr;

/** Byte-addressable memory.
 *
 *  This class represents an entire state of memory via MemoryMap, allocating
 * new memory in units of pages (the size of a page
 *  is configurable. */
class MemoryState : public BaseSemantics::MemoryState {
 ...
 public:
   ...
   *
   *  The @p addrProtoval and @p valProtoval must both be of
   * ConcreteSemantics::SValue type or derived classes. */
   ...
   *
   *  For efficiency purposes, the data buffers are not copied immediately but
   * rather marked as copy-on-write. However, the
   *  newly constructed memory map will have its own segments, which hold the
 ...
 public:
  /** Virtual constructor.
   *
   *  Creates a memory state having specified prototypical value, which should
   * be of type ConcreteSemantics::SValue or
   *  subclasses. */
   ...
   *
   *  Creates a new deep copy of this memory state. For efficiency purposes, the
   * data buffers are not copied immediately but
   *  rather marked as copy-on-write.  However, the newly constructed memory map
   * will have its own segments, which hold the
   *  segment names, access permissions, etc. */
  ...
  // Dynamic pointer casts
 public:
  /** Recasts a base pointer to a concrete memory state. This is a checked cast
   * that will fail if the specified pointer does
   *  not have a run-time type that is a ConcreteSemantics::MemoryState or
   * subclass thereof. */
 ...
 public:
  ...
  /** Set memory map.
   *
   *  If the specified map's areas not not in units of pages then padding
   * segments will be added to this memory state. The
   *  padding segments will either have the accessibility specified by @p
   * padAccess, or will have the same accessibility as
   *  the memory region being padded.  All padding segments will be named
   * "padding". */
   ...
   *
   *  Memory is allocated in units of the page size and aligned on page-size
   * boundaries.  The page size cannot be changed
   ...
   *
   *  The specified address will be contained in the page, which is aligned on a
   * page boundary. Do not call this if the page
   *  is already allocated unless: it will replace the allocated page with a new
...
/** Shared-ownership pointer to concrete RISC operations. See @ref
 * heap_object_shared_ownership. */
typedef boost::shared_ptr<class RiscOperators> RiscOperatorsPtr;

/** Defines RISC operators for the ConcreteSemantics domain.
 *
 *  These RISC operators depend on functionality introduced into the SValue
 * class hierarchy at the ConcreteSemantics::SValue
 *  level. Therefore, the prototypical value supplied to the constructor or
 * present in the supplied state object must have a
 *  dynamic type which is a ConcreteSemantics::SValue (or subclass).
 *
 *  Each RISC operator should return a newly allocated semantic value rather
 * than trying to re-use an existing one. This will
 *  allow the caller to change the value stored there without affecting any of
 * the input arguments. For example, a no-op that
...
class RiscOperators : public BaseSemantics::RiscOperators {
 ...
 protected:
  RiscOperators(const BaseSemantics::SValuePtr &protoval, SMTSolver *solver)
    ...
    name("Concrete");
    (void)SValue::promote(
        protoval);  // make sure its dynamic type is a ConcreteSemantics::SValue
  }

  RiscOperators(const BaseSemantics::StatePtr &state, SMTSolver *solver)
      : BaseSemantics::RiscOperators(state, solver) {
    name("Concrete");
    (void)SValue::promote(state->protoval());  // values must have
                                               // ConcreteSemantics::SValue
                                               // dynamic type
  }
  ...
  // Static allocating constructors
 public:
  /** Instantiates a new RiscOperators object and configures it to use semantic
   * values and states that are defaults for
   * ConcreteSemantics. */
  static RiscOperatorsPtr instance(const RegisterDictionary *regdict,
                                   SMTSolver *solver = NULL) {
    BaseSemantics::SValuePtr protoval = SValue::instance();
    BaseSemantics::RegisterStatePtr registers =
        RegisterState::instance(protoval, regdict);
    BaseSemantics::MemoryStatePtr memory =
        MemoryState::instance(protoval, protoval);
    BaseSemantics::StatePtr state = State::instance(registers, memory);
    return RiscOperatorsPtr(new RiscOperators(state, solver));
  }

  /** Instantiates a new RiscOperators object with specified prototypical
   * values.  An SMT solver may be specified as the
   *  second argument because the base class expects one, but it is not used for
   * concrete semantics. See @ref solver for
   *  details. */
  ...
  /** Instantiates a new RiscOperators object with specified state.  An SMT
   * solver may be specified as the second argument
   *  because the base class expects one, but it is not used for concrete
   * semantics. See @ref solver for details. */
 ...
 public:
   ...
   *  will fail if @p x does not point to a ConcreteSemantics::RiscOperators
   * object. */
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // New methods for constructing values, so we don't have to write so many
  // SValue::promote calls in the RiscOperators
  // implementations.
 protected:
  ...
  SValuePtr svalue_boolean(bool b) { return SValue::promote(boolean_(b)); }
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Override methods from base class.  These are the RISC operators that are
 ...
 public:
  ...
  virtual BaseSemantics::SValuePtr rotateLeft(
      const BaseSemantics::SValuePtr &a_, const BaseSemantics::SValuePtr &sa_);
  ...
  virtual BaseSemantics::SValuePtr unsignedExtend(
      const BaseSemantics::SValuePtr &a_, size_t new_width);
  ...
  virtual BaseSemantics::SValuePtr fpFromInteger(
      const BaseSemantics::SValuePtr &intValue, SgAsmFloatType *);

  virtual BaseSemantics::SValuePtr fpToInteger(
      const BaseSemantics::SValuePtr &fpValue, SgAsmFloatType *fpType,
      const BaseSemantics::SValuePtr &dflt);
  ...
  virtual BaseSemantics::SValuePtr fpRoundTowardZero(
      const BaseSemantics::SValuePtr &a, SgAsmFloatType *);
