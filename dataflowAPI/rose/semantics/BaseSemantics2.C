...
std::ostream&
operator<<(std::ostream& o, const Exception& x)
{
    x.print(o);
    return o;
}
...
SValuePtr
State::readRegister(const RegisterDescriptor& desc, const SValuePtr& dflt,
                    RiscOperators* ops)
{
...
void
State::writeRegister(const RegisterDescriptor& desc, const SValuePtr& value,
                     RiscOperators* ops)
{
    ASSERT_require(desc.is_valid());
...
SValuePtr
State::readMemory(const SValuePtr& address, const SValuePtr& dflt, RiscOperators* addrOps,
...
void
State::writeMemory(const SValuePtr& addr, const SValuePtr& value, RiscOperators* addrOps,
                   RiscOperators* valOps)
{
...
RiscOperators::isSignedLessThan(const SValuePtr& a, const SValuePtr& b)
{
    ...
    SValuePtr sameSigns =
        invert(xor_(aIsNeg, bIsNeg));  // A and B are both negative or both non-negative?
...
SValuePtr
RiscOperators::fpToInteger(const SValuePtr& /*fpValue*/, SgAsmFloatType* fpType,
...
SValuePtr
RiscOperators::fpConvert(const SValuePtr& a, SgAsmFloatType* aType,
...
RiscOperators::fpIsNan(const SValuePtr& a, SgAsmFloatType* aType)
{
    ...
    ASSERT_not_null(aType);
    SValuePtr exponent =
        extract(a, aType->exponentBits().least(), aType->exponentBits().greatest() + 1);
...
RiscOperators::fpEffectiveExponent(const SValuePtr& a, SgAsmFloatType* aType)
{
    ...
    SValuePtr storedExponent =
        extract(a, aType->exponentBits().least(), aType->exponentBits().greatest() + 1);
    SValuePtr significand = extract(a, aType->significandBits().least(),
                                    aType->significandBits().greatest() + 1);
    SValuePtr retval =
        ite(equalToZero(storedExponent),
            ite(equalToZero(significand),
                ...
                // Stored exponent is zero but significand is not, therefore denormalized
                // number. effective exponent is 1 - bias - (significandWidth -
                // mssb(significand))
                add(number_(expWidth,
                            1 - aType->exponentBias() - aType->significandBits().size()),
            ...
            // exponent is the stored exponent minus the bias.
...
SValuePtr
RiscOperators::fpAdd(const SValuePtr& /*a*/, const SValuePtr& /*b*/,
...
SValuePtr
RiscOperators::fpSubtract(const SValuePtr& /*a*/, const SValuePtr& /*b*/,
                          SgAsmFloatType* /*fpType*/)
{
...
SValuePtr
RiscOperators::fpMultiply(const SValuePtr& /*a*/, const SValuePtr& /*b*/,
                          SgAsmFloatType* /*fpType*/)
{
...
SValuePtr
RiscOperators::fpDivide(const SValuePtr& /*a*/, const SValuePtr& /*b*/,
                        SgAsmFloatType* /*fpType*/)
{
...
RiscOperators::readRegister(const RegisterDescriptor& reg, const SValuePtr& dflt_)
{
    ...
    // If there's an lazily-updated initial state, then get its register, updating the
    // initial state as a side effect.
    if(initialState_)
...
Dispatcher::decrementRegisters(SgAsmExpression* /*e*/)
{
    ...
    isSgAsmRegisterReferenceExpression(node)) { const RegisterDescriptor &reg =
    rre->get_descriptor(); if (rre->get_adjustment() < 0) { SValuePtr adj =
    ops->number_(64, (int64_t)rre->get_adjustment()); if (reg.get_nbits() <= 64) { adj =
    ops->unsignedExtend(adj, reg.get_nbits());  // truncate } else { adj =
    ops->signExtend(adj, reg.get_nbits());      // extend
                    }
...
Dispatcher::incrementRegisters(SgAsmExpression* /*e*/)
{
    ...
    isSgAsmRegisterReferenceExpression(node)) { const RegisterDescriptor &reg =
    rre->get_descriptor(); if (rre->get_adjustment() > 0) { SValuePtr adj =
    ops->unsignedExtend(ops->number_(64, (int64_t)rre->get_adjustment()),
    reg.get_nbits()); ops->writeRegister(reg, ops->add(ops->readRegister(reg), adj));
                }
            }
        }
    } t1(operators);
...
Dispatcher::read(SgAsmExpression* e, size_t value_nbits /*=0*/, size_t addr_nbits /*=0*/)
{
    ...
    else if(SgAsmBinaryLsl* lshift = isSgAsmBinaryLsl(e))
    {
        ...
        size_t           nbits = std::max(lhs->get_nBits(), rhs->get_nBits());
        retval                 = operators->shiftLeft(read(lhs, lhs->get_nBits()),
                                      read(rhs, rhs->get_nBits()));
    }
    else if(SgAsmBinaryAsr* asr = isSgAsmBinaryAsr(e))
    {
        ...
        retval = operators->shiftRightArithmetic(read(lhs, lhs->get_nBits()),
                                                 read(rhs, rhs->get_nBits()));
    }
    else if(SgAsmBinaryLsr* lsr = isSgAsmBinaryLsr(e))
    {
        ...
        retval               = operators->shiftRight(read(lhs, lhs->get_nBits()),
                                       read(rhs, rhs->get_nBits()));
    }
    else
    {
    ...
    // Make sure the return value is the requested width. The unsignedExtend() can expand
    // or shrink values.
    ASSERT_not_null(retval);
