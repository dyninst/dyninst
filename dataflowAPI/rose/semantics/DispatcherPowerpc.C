...
/*******************************************************************************************************************************
 *                                      Functors that handle individual PowerPC
 *instructions kinds
 *******************************************************************************************************************************/

namespace Powerpc
{
// An intermediate class that reduces the amount of typing in all that follows.  Its
// process() method does some up-front checking, dynamic casting, and pointer
// dereferencing and then calls the p() method that does the real work.
...
public:
    ...
    virtual void process(const BaseSemantics::DispatcherPtr& dispatcher_,
    ...
    void assert_args(I insn, A args, size_t nargs)
    {
        if(args.size() != nargs)
        {
            std::string mesg = "instruction must have " +
                               StringUtility::numberToString(nargs) + "argument" +
                               (1 == nargs ? "" : "s");
...
struct IP_addc : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(d->read(args[1], 32), d->read(args[2], 32), no, carries);
        ...
        BaseSemantics::SValuePtr v1 =
            ops->ite(carry_out, ops->number_(32, 0x20000000u), zero);
        BaseSemantics::SValuePtr ones = ops->number_(32, 0xdfffffffu);
        ops->writeRegister(d->REG_XER,
                           ops->or_(ops->and_(ops->readRegister(d->REG_XER), ones), v1));
    }
};
...
struct IP_adde : P
{
    ...
    {
        assert_args(insn, args, 3);
        BaseSemantics::SValuePtr carry_in =
            ops->extract(ops->readRegister(d->REG_XER), 29, 30);
        BaseSemantics::SValuePtr carries = ops->number_(32, 0);
        BaseSemantics::SValuePtr arg2    = d->read(args[2], 32);
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(d->read(args[1], 32), arg2, carry_in, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
...
struct IP_addi : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr signExtended_si =
...
struct IP_addic : P
{
    ...
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr v1 =
            ops->signExtend(ops->extract(d->read(args[2], 32), 0, 16), 32);
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(d->read(args[1], 32), v1, no, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
        ...
        // The value 0xdfffffff is the mask for the Carry (CA) flag
        BaseSemantics::SValuePtr zero = ops->number_(32, 0);
        BaseSemantics::SValuePtr v2 =
            ops->ite(carry_out, ops->number_(32, 0x20000000u), zero);
        BaseSemantics::SValuePtr ones = ops->number_(32, 0xdfffffffu);
        ops->writeRegister(d->REG_XER,
                           ops->or_(ops->and_(ops->readRegister(d->REG_XER), ones), v2));
        if(record)
            d->record(result);
    }
};
...
struct IP_addme : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(d->read(args[1], 32), ones, carry_in, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
...
struct IP_addze : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(d->read(args[1], 32), zero, carry_in, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
...
struct IP_bc : P
{
    ...
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        if(!bo_2)
        {
            ...
            ops->writeRegister(d->REG_CTR,
                               ops->add(ops->readRegister(d->REG_CTR), negOne));
        }
        ...
        SgAsmRegisterReferenceExpression* bi =
            isSgAsmRegisterReferenceExpression(args[1]);
        ASSERT_require(bi && bi->get_descriptor().get_major() == powerpc_regclass_cr &&
                       bi->get_descriptor().get_nbits() == 1);
        BaseSemantics::SValuePtr cr_bi = ops->readRegister(bi->get_descriptor());
        BaseSemantics::SValuePtr cond_ok =
            bo_0 ? ops->boolean_(true) : bo_1 ? cr_bi : ops->invert(cr_bi);
        BaseSemantics::SValuePtr v1     = ops->number_(32, insn->get_address());
...
struct IP_bcctr : P
{
    ...
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        ops->writeRegister(
            d->REG_IAR,
            ops->ite(cond_ok, ops->and_(ops->readRegister(d->REG_CTR), mask), iar));
    }
};
...
struct IP_cmp : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        ops->addWithCarries(ops->invert(ops->xor_(ra, ops->number_(32, 0x80000000u))), v1,
                            no, carries);
        ...
        BaseSemantics::SValuePtr c =
            ops->ite(ops->equalToZero(ops->xor_(ra, rb)), one, v2);
        SgAsmRegisterReferenceExpression* bf =
            isSgAsmRegisterReferenceExpression(args[0]);
        ASSERT_require(bf && bf->get_descriptor().get_major() == powerpc_regclass_cr &&
                       bf->get_descriptor().get_nbits() == 4);
...
struct IP_cmpi : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        // For 32-bit case we can ignore value of L
        BaseSemantics::SValuePtr ra = d->read(args[2], 32);
        BaseSemantics::SValuePtr si =
            ops->signExtend(ops->extract(d->read(args[3], 32), 0, 16), 32);
        BaseSemantics::SValuePtr carries = ops->number_(32, 0);
        // Need to check if ops->boolean_(false) or ops->boolean_(true) should be used!
        // Bias both sides and use unsigned compare.
        // ops->invert(ops->xor_(RA,number<32>(0x80000000U))) yields "(RA+bias)-1"
        ...
        BaseSemantics::SValuePtr one = ops->number_(3, 1);
        BaseSemantics::SValuePtr c =
            ops->ite(ops->equalToZero(ops->xor_(ra, si)), one, v2);
        SgAsmRegisterReferenceExpression* bf =
            isSgAsmRegisterReferenceExpression(args[0]);
        ASSERT_require(bf && bf->get_descriptor().get_major() == powerpc_regclass_cr &&
                       bf->get_descriptor().get_nbits() == 4);
...
struct IP_cmpl : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr one = ops->number_(3, 1);
        BaseSemantics::SValuePtr c =
            ops->ite(ops->equalToZero(ops->xor_(ra, ui)), one, v1);
        SgAsmRegisterReferenceExpression* bf =
            isSgAsmRegisterReferenceExpression(args[0]);
        ASSERT_require(bf && bf->get_descriptor().get_major() == powerpc_regclass_cr &&
                       bf->get_descriptor().get_nbits() == 4);
...
struct IP_cntlzw : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr v1 =
            ops->xor_(ops->mostSignificantSetBit(rs), thirtyOne);
...
struct IP_lbzu : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        SgAsmMemoryReferenceExpression* memoryReference =
            isSgAsmMemoryReferenceExpression(args[1]);
...
struct IP_lmw : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        SgAsmRegisterReferenceExpression* rt =
            isSgAsmRegisterReferenceExpression(args[0]);
        ...
        for(unsigned minor = reg.get_minor(); minor < 32; minor += 1, offset += 4)
        {
            ...
            BaseSemantics::SValuePtr value =
                ops->readMemory(RegisterDescriptor(), addr, dflt, ops->boolean_(true));
            reg.set_minor(minor);
...
struct IP_lwzu : P
{
    ...
    {
        assert_args(insn, args, 2);
        // FIXME: shouldn't DispatcherPowerpc::read() handle this? [Robb P. Matzke
        // 2013-05-08]
        SgAsmMemoryReferenceExpression* memoryReference =
            isSgAsmMemoryReferenceExpression(args[1]);
        SgAsmBinaryAdd* binaryAdd = isSgAsmBinaryAdd(memoryReference->get_address());
        ...
        d->write(args[0],
                 ops->readMemory(RegisterDescriptor(), addr, ops->undefined_(32), yes));
        d->write(ra, addr);
    }
};
...
// Copies the value from the second argument to the first argument.  This is used for a
...
struct IP_mulhw : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr arg2 = d->read(args[2], 32);
        d->write(args[0],
                 ops->extract(ops->signedMultiply(d->read(args[1], 32), arg2), 32, 64));
    }
};
...
struct IP_mulhwu : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr arg2 = d->read(args[2], 32);
        d->write(args[0],
                 ops->extract(ops->unsignedMultiply(d->read(args[1], 32), arg2), 32, 64));
    }
};
...
struct IP_mullw : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr arg2 = d->read(args[2], 32);
        d->write(args[0],
                 ops->extract(ops->signedMultiply(d->read(args[1], 32), arg2), 0, 32));
    }
};
...
struct IP_srawi : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr mask =
            ops->invert(ops->shiftLeft(ops->number_(32, -1), sh));
        BaseSemantics::SValuePtr hasValidBits =
            ops->invert(ops->equalToZero(ops->and_(rs, mask)));
        ...
        ops->writeRegister(d->REG_XER,
                           ops->or_(ops->and_(ops->readRegister(d->REG_XER), v2), v1));
    }
};
...
struct IP_stmw : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        assert_args(insn, args, 2);
        BaseSemantics::SValuePtr          base = d->effectiveAddress(args[1], 32);
        SgAsmRegisterReferenceExpression* rs =
            isSgAsmRegisterReferenceExpression(args[0]);
...
struct IP_stwu : P
{
    ...
    {
        assert_args(insn, args, 2);
        // FIXME: shouldn't this be done by DispatcherPowerpc::read()? [Robb P. Matzke
        // 2013-05-08]
        SgAsmMemoryReferenceExpression* memoryReference =
            isSgAsmMemoryReferenceExpression(args[1]);
        SgAsmBinaryAdd* binaryAdd = isSgAsmBinaryAdd(memoryReference->get_address());
...
struct IP_subf : P
{
    ...
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr arg2 = d->read(args[2], 32);
        BaseSemantics::SValuePtr result =
            ops->add(ops->negate(d->read(args[1], 32)), arg2);
        d->write(args[0], result);
        if(record)
            d->record(result);
    }
...
struct IP_subfc : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr arg2    = d->read(args[2], 32);
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(ops->invert(d->read(args[1], 32)), arg2, yes, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
...
struct IP_subfe : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr carries = ops->number_(32, 0);
        BaseSemantics::SValuePtr arg2    = d->read(args[2], 32);
        BaseSemantics::SValuePtr result  = ops->addWithCarries(
            ops->invert(d->read(args[1], 32)), arg2, carry_in, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
...
struct IP_subfic : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        assert_args(insn, args, 3);
        BaseSemantics::SValuePtr carries = ops->number_(32, 0);
        // To do the subtraction we invert the first operand and add.  To add "1" we set
        // the carry in to true.
        BaseSemantics::SValuePtr yes = ops->boolean_(true);
        BaseSemantics::SValuePtr v1 =
            ops->signExtend(ops->extract(d->read(args[2], 32), 0, 16), 32);
        BaseSemantics::SValuePtr result =
            ops->addWithCarries(ops->invert(d->read(args[1], 32)), v1, yes, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
        ...
        ops->writeRegister(d->REG_XER,
                           ops->or_(ops->and_(ops->readRegister(d->REG_XER), v3), v2));
    }
};
...
struct IP_subfze : P
{
    void p(D d, Ops ops, I insn, A args)
    {
        ...
        BaseSemantics::SValuePtr zero    = ops->number_(32, 0);
        BaseSemantics::SValuePtr result  = ops->addWithCarries(
            ops->invert(d->read(args[1], 32)), zero, carry_in, carries);
        BaseSemantics::SValuePtr carry_out = ops->extract(carries, 31, 32);
        d->write(args[0], result);
...
DispatcherPowerpc::regcache_init()
{
    if(regdict)
    {
        REG_IAR = findRegister("iar",
                               32);  // instruction address register (instruction pointer)
        ...
        REG_CR0 = findRegister(
            "cr0", 4);  // CR Field 0, result of fixed-point instruction; set by record()
...
DispatcherPowerpc::record(const BaseSemantics::SValuePtr& result)
{
    ...
    BaseSemantics::SValuePtr v1 =
        operators->ite(operators->extract(result, 31, 32), four, two);
    ...
    BaseSemantics::SValuePtr so =
        operators->extract(operators->readRegister(REG_XER), 31, 32);
