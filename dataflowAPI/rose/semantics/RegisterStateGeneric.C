...
// Define this if you want extra consistency checking before and after each mutator.  This
// slows things down considerably but can be useful for narrowing down logic errors in the
// implementation.
//#define RegisterStateGeneric_ExtraAssertions

// Define this if you want the readRegister behavior as it existed before 2015-09-24. This
// behavior was wrong in certain ways because it didn't always cause registers to spring
// into existence the first time they were read.
...
static bool
sortByOffset(const RegisterStateGeneric::RegPair& a,
...
{
    // We're initializing with a concrete value, so it really doesn't matter whether we
    // initialize large registers or small registers.  Constant folding will adjust things
    // as necessary when we start reading registers.
...
void
RegisterStateGeneric::initialize_nonoverlapping(
...
void
RegisterStateGeneric::assertStorageConditions(const std::string&        when,
...
RegisterStateGeneric::scanAccessedLocations(const RegisterDescriptor& reg,
                                            ...
                                            RegPairs& preservedParts /*out*/)
{
    ...
    BOOST_FOREACH(RegPair& regpair, pairList)
    {
        ...
        {
            RegisterDescriptor subreg(reg.get_major(), reg.get_minor(), overlap.least(),
                                      overlap.size());
            size_t             extractBegin = overlap.least() - storedLocation.least();
            size_t             extractEnd   = extractBegin + overlap.size();
            accessedParts.push_back(
                RegPair(subreg, ops->extract(regpair.value, extractBegin, extractEnd)));
        }
        ...
        // If we're allowed to modify the storage locations so as to match the accessed
        // register then mark this pair with a null value so we can erase it in the next
        // step.
        if(markOverlapping)
...
SValuePtr
RegisterStateGeneric::readRegister(const RegisterDescriptor& reg, const SValuePtr& dflt,
                                   RiscOperators* ops)
{
    ...
    // Fast case: the state does not store this register or any register that might
    // overlap with this register.
    ...
    // Iterate over the storage/value pairs to figure out what parts of the register are
    // already in existing storage locations, and which parts of those overlapping storage
    // locations are not accessed.
    ...
    // Create values for the parts of the accessed register that weren't stored in the
    // state, but don't store them yet.
    RegPairs newParts;
    if(accessCreatesLocations_)
    {
    ...
    else
    {
        ASSERT_require(
            newLocations
                .isEmpty());  // should have thrown a RegisterNotPresent exception already
    }
    ...
    // Update the register state -- write parts that didn't exist and maybe combine or
    // split some locations.
    if(adjustLocations)
    {
        // Combine all the accessed locations into a single location corresponding exactly
        // to the return value, and split those previously existing locations that partly
        // overlap the returned location.
        pairList.erase(std::remove_if(pairList.begin(), pairList.end(), has_null_value),
                       pairList.end());
    ...
    {
        // Since this is a registerRead operation, the only thing we need to write back to
        // the state are those parts of the return value that had no locations allocated
        // originally.
...
void
RegisterStateGeneric::writeRegister(const RegisterDescriptor& reg, const SValuePtr& value,
                                    RiscOperators* ops)
{
    ...
    {
        // Combine all the accessed locations into a single location corresponding exactly
        // to the written value, and split those previously existing locations that partly
        // overlap the written location.
        pairList.erase(std::remove_if(pairList.begin(), pairList.end(), has_null_value),
                       pairList.end());
        pairList.insert(pairList.end(), preservedParts.begin(), preservedParts.end());
...
void
RegisterStateGeneric::updateWriteProperties(const RegisterDescriptor& reg,
...
RegisterStateGeneric::erase_register(const RegisterDescriptor& reg, RiscOperators* ops)
{
    ...
    // Look for existing registers that overlap with this register and remove them.  If
    // the overlap was only partial, then we need to eventually add the non-overlapping
    // part back into the list.
...
bool
RegisterStateGeneric::insertWriters(const RegisterDescriptor& desc,
...
void
RegisterStateGeneric::setWriters(const RegisterDescriptor& desc,
                                 const AddressSet&         writerVas)
{
    if(writerVas.isEmpty())
    {
...
void
RegisterStateGeneric::set_latest_writer(const RegisterDescriptor& desc,
...
bool
RegisterStateGeneric::hasPropertyAny(const RegisterDescriptor& reg,
...
bool
RegisterStateGeneric::hasPropertyAll(const RegisterDescriptor& reg,
                                     InputOutputProperty       prop) const
{
    if(!properties_.exists(reg))
        return false;
...
bool
RegisterStateGeneric::insertProperties(const RegisterDescriptor&     reg,
...
void
RegisterStateGeneric::setProperties(const RegisterDescriptor&     reg,
                                    const InputOutputPropertySet& props)
{
    if(props.isEmpty())
    {
...
RegisterStateGeneric::findProperties(const InputOutputPropertySet& required,
...
bool
RegisterStateGeneric::merge(const BaseSemantics::RegisterStatePtr& other_,
                            RiscOperators*                         ops)
{
    ASSERT_not_null(ops);
    ...
    BOOST_FOREACH(const RegPair& otherRegVal, other->get_stored_registers())
    {
        ...
        if(is_partly_stored(otherReg))
        {
            ...
            if(BaseSemantics::SValuePtr merged =
                   thisValue->createOptionalMerge(otherValue, merger(), ops->solver())
                       .orDefault())
            {
                   ...
                   0 != pair.value->get_comment().compare(regname + "_0"))
                {
                    ...
                    else
                    {
                        ...
                        if(fmt.get_show_latest_writers())
                        {
                            ...
                            // account that different writer sets can exist for different
                            // parts of the register.
                            AddressSet writers = getWritersUnion(pair.desc);
                            if(writers.size() == 1)
                            {
                                stream << " [writer="
                                       << StringUtility::addrToString(
                            ...
                            {
                                stream << " [writers={";
                                for(Sawyer::Container::Set<rose_addr_t>::ConstIterator
                                        wi = writers.values().begin();
                        ...
                        // FIXME[Robb P. Matzke 2015-08-12]: This doesn't take into
                        // account that different property sets can exist for different
                        // parts of the register.  It also doesn't take into account all
                        // combinations f properties -- just a few of the more common
                        // ones.
