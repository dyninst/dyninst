...
/*******************************************************************************************************************************
 *                                      Functors that handle individual AmdgpuVega
 *instructions kinds
 *******************************************************************************************************************************/
...
struct IP_s_add_u32 : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        scc_value =
            ops->ite(ops->isUnsignedGreaterThan(result, ops->number_(64, 0x100000000)),
                     ops->number_(1, 1), ops->number_(1, 0));
...
struct IP_s_addc_u32 : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        result =
            d->doAddOperation(src0, src1, false, d->readRegister(d->REG_SCC), n, z, c, v);
...
struct IP_s_getpc_b64 : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        BaseSemantics::SValuePtr     result =
            ops->number_(ival->get_significantBits(), ival->get_value());
...
struct IP_s_setpc_b64 : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        BaseSemantics::SValuePtr n, z, c, v;
        result = d->doAddOperation(ops->shiftLeft(highPC, ops->number_(32, 32)), lowPC,
                                   false, ops->boolean_(false), n, z, c, v);
...
struct IP_s_swappc_b64 : P
{
    void p(D d, Ops ops, I insn, A args, B raw)
    {
        ...
        BaseSemantics::SValuePtr storehighPC =
            ops->shiftRight(result, ops->number_(32, 32));
...
void
DispatcherAmdgpuVega::setFlagsForResult(
    ...
    switch (((condVal & 0xF) >> 1)) {
        ...
        case 4:
            result = operators->ite(operators->isEqual(cVal, operators->number_(1, 1)),
                                    operators->ite(
                                            operators->isEqual(zVal, operators->number_(1,
    0)), operators->boolean_(true), operators->boolean_(false)),
...
BaseSemantics::SValuePtr
DispatcherAmdgpuVega::ROR(const BaseSemantics::SValuePtr& expr,
...
BaseSemantics::SValuePtr
DispatcherAmdgpuVega::getBitfieldMask(int immr, int imms, int N, bool iswmask,
                                      int datasize)
{
    int hsbarg = (N << 6) | (~imms);
    int len;
...
DispatcherAmdgpuVega::getShiftType(uint32_t raw)
{
    ...
    {
        case 0:
            return static_cast<int>(AmdgpuVega::InsnProcessor::ShiftType_LSL);
        case 1:
            return static_cast<int>(AmdgpuVega::InsnProcessor::ShiftType_LSR);
        case 2:
            return static_cast<int>(AmdgpuVega::InsnProcessor::ShiftType_ASR);
        case 3:
            return static_cast<int>(AmdgpuVega::InsnProcessor::ShiftType_ROR);
        default:
...
BaseSemantics::SValuePtr
DispatcherAmdgpuVega::doAddOperation(
    BaseSemantics::SValuePtr a, BaseSemantics::SValuePtr b, bool invertCarries,
    const BaseSemantics::SValuePtr& carryIn, BaseSemantics::SValuePtr& n,
    BaseSemantics::SValuePtr& z, BaseSemantics::SValuePtr& c, BaseSemantics::SValuePtr& v)
{
...
void
DispatcherAmdgpuVega::writeRegister(const RegisterDescriptor&       reg,
...
void
DispatcherAmdgpuVega::writeMemory(const BaseSemantics::SValuePtr& addr, size_t writeSize,
...
BaseSemantics::SValuePtr
DispatcherAmdgpuVega::ShiftReg(const BaseSemantics::SValuePtr& src, int shiftType,
                               const BaseSemantics::SValuePtr& amount)
{
    ...
    {
        case AmdgpuVega::InsnProcessor::ShiftType_LSL:
            return operators->shiftLeft(src, amount);
        case AmdgpuVega::InsnProcessor::ShiftType_LSR:
            return operators->shiftRight(src, amount);
        case AmdgpuVega::InsnProcessor::ShiftType_ASR:
            return operators->shiftRightArithmetic(src, amount);
        case AmdgpuVega::InsnProcessor::ShiftType_ROR:
