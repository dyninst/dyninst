...
// Multiplexors are never included in baked results (they control baking instead), so
// messages should never be posted directly to multiplexers.
SAWYER_EXPORT void
...
SequenceFilter::shouldForward(const MesgProps&)
{
    SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
    bool                                     retval =
        nPosted_ >= nSkip_ && 0 == (nPosted_ - nSkip_) % std::max(rate_, (size_t) 1) &&
...
// This is a pointer so that it doesn't depend on order of global variable initialization.
// We'll allocate it the first time we need it.
...
{
#ifdef BOOST_WINDOWS
#    if 0  // [Robb Matzke 2014-06-13] temporarily disable for ROSE linking error (needs
           // psapi.lib in Windows)
              ...
              UnformattedSink::maybeFinal(const Mesg& mesg, const MesgProps& props)
{
    ...
    if(!mesg.isEmpty())
    {
        if(mesg.isCanceled())
        {
            retval = props.cancelationStr.orDefault() + props.lineTermination.orDefault();
            gang()->clear();
        }
        else if(mesg.isComplete())
        {
            retval = props.completionStr.orDefault() + props.lineTermination.orDefault();
            gang()->clear();
        }
        else
        {
              ...
              UnformattedSink::render(const Mesg& mesg, const MesgProps& props)
{
    ...
    retval +=
        maybeTerminatePrior(mesg, props);  // force side effects in a particular order
...
FdSink::init()
{
    ...
    else
    {
        ...
        overridePropertiesNS().useColor =
            false;  // force false; user can still set this if they really want color
    }
#endif
...
FdSink::post(const Mesg& mesg, const MesgProps& props)
{
    if(!partialMessagesAllowed_ && !mesg.isComplete())
        return;
#ifdef BOOST_WINDOWS
    // FIXME[Robb Matzke 2014-06-10]: what is the most basic file level on Windows; one
    // which doesn't need construction?
...
FileSink::init()
{
    ...
    defaultPropertiesNS().isBuffered =
        2 != fileno(file_);  // assume stderr is unbuffered and the rest are buffered
#endif
}
...
SyslogSink::post(const Mesg& mesg, const MesgProps& props)
{
    if(mesg.isComplete())
    {
        ...
        // This mutex doesn't really protect syslog completely and I'm not sure whether
        // syslog itself is thread-safe.  But this mutex will work in the case when syslog
        // is not thread-safe but only one Message::Destination is talking to syslog.
        SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
...
// This is the internal part of Stream. Every Stream has exactly one of these, thus the
// mutex is stored in Stream instead of here.
...
StreamBuf::xsputn(const char* s, std::streamsize& n)
{
    ...
    // This is called from std::ostream::operator<< (and possibly others), so we need to
    // acquire a lock. Since this object is owned by exactly one Stream object, the mutex
    // we lock is in the stream object.
    assert(stream_ != NULL);
...
: std::ostream(new StreamBuf(this))
, nrefs_(0)
, streambuf_(NULL)
{
...
, streambuf_(NULL)
{
    ...
    if(!other)
        throw "Sawyer::Message::Stream initializer is not a Sawyer::Message::Stream "
              "(only a std::ostream)";
    SAWYER_THREAD_TRAITS::LockGuard lock(other->mutex_);
    initFromNS(*other);
}
...
Stream::initFromNS(const Stream& other)
{
    ...
    // If this stream has a partial message then it needs to be canceled.  This also
    // resets things associated with the message, such as the baked properties.
...
SAWYER_EXPORT
SProxy::SProxy(const SProxy& other)
              ...
              Facility::get(Importance imp)
{
    ...
    if(!isConstructed())
    {
        ...
        // trying to use a Stream declared at namespace scope from inside a constructor
        // for another namespace scope object.  There may be issues with how your C++
        // runtime initializes such objects; perhaps your object (the one using this
        // Facility) is being initialized before this Facility.  You could try to figure
        // out the correct order to initialize these objects, or you could explicitly call
    ...
    if(streams_.empty())
    {
        ...
        //
        // The typical way to initialize a global Facility is to declare it with a default
        // constructor then then before it's used for the first time, assign a new
        // Facility object to the global variable.  E.g.,
        //
        // |// file example.C
        ...
        //
        // ROSE users: librose does not currently (2014-09-09) initialize libsawyer until
        // the ROSE frontend() is called. If you're calling into librose before calling
        // "frontend" then you probably want to explicitly initialize ROSE by invoking
        // rose::Diagnostics::initialize() early in "main". This will cause all of ROSE's
        // Facility objects to be constructed.
        std::ostringstream ss;
        ss << "stream " << stringifyImportance(imp) << " in facility " << this
              ...
              Facilities::insertNS(Facility& facility, std::string name)
{
    ...
    if(0 != name.compare(parseFacilityName(s)))
        throw std::logic_error("name '" + name +
...
// class method; thread-safe
// Matches the Perl regular expression /^\s*([+!]?)/ and returns $2 on success with str
// pointing to the character after the match.  Returns the empty string on failure with
// str not adjusted.
SAWYER_EXPORT std::string
...
// class method; thread-safe
// Matches the Perl regular expression /^\s*(<=?|>=?)/ and returns $2 on success with str
// pointing to the character after the match. Returns the empty string on failure with str
// not adjusted.
SAWYER_EXPORT std::string
...
// /^\s*(all|none|debug|trace|where|info|warn|error|fatal)\b/ On match, returns $2 and str
// points to the next character after the match On failure, returns "" and str is
// unchanged
SAWYER_EXPORT std::string
...
// class method
// no global state
// Parses a StreamControlList. On success, returns a non-empty vector and adjust 'str' to
// point to the next character after the list.  On failure, throw a ControlError.
                                ...
                                bool isGlobal)
{
    ...
    while(1)
    {
        ...
        {
            if(!enablement.empty())
                throw ControlError("'" + importance + "' cannot be preceded by '" +
                                       enablement + "'",
                                   enablementStart);
            if(!relation.empty())
                throw ControlError("'" + importance + "' cannot be preceded by '" +
        ...
        while(1)
        {
            ...
            if(t2.empty())
            {
                ...
                while(isspace(*s))
                    ++s;
                if('(' != *s)
                    throw ControlError("expected '(' after message facility name '" +
                                           facilityName + "'",
                                       s);
                ++s;
...
Facilities::print(std::ostream& log) const
{
    ...
    else
    {
        BOOST_FOREACH(const FacilityMap::Node& fnode, facilities_.nodes())
        {
            ...
            // enabled; dashes keep them aligned. Sort of like the format 'ls -l' uses to
            // show permissions.
            for(int i = 0; i < N_IMPORTANCE; ++i)
            {
                Importance mi = (Importance) i;
