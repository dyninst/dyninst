...
now() {
#if defined(SAWYER_HAVE_DYNCOMPAT_CHRONO)
    // dyncompat::chrono is thread-safe since we're using only stack allocated objects
    dyncompat::chrono::system_clock::time_point curtime = dyncompat::chrono::system_clock::now();
    dyncompat::chrono::system_clock::time_point epoch;
    dyncompat::chrono::duration<double> diff = curtime - epoch;
    return diff.count();
#elif defined(DYNCOMPAT_WINDOWS)
    FILETIME ft;
...
SAWYER_EXPORT bool
Mesg::hasText() const {
    return dyncompat::find_token(text_, dyncompat::is_graph()) != text_.end();
}
