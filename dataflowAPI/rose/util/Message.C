...
// thread-safe
SAWYER_EXPORT std::string stringifyImportance(Importance importance) {
  switch (importance) {
    case DEBUG:
      return "DEBUG";
    case TRACE:
    ...
    case N_IMPORTANCE:
      ...
      // default: DO NOT USE or else we won't get compiler warnings when new
      // importances are defined
  }
  throw std::runtime_error("invalid message importance");
}

// thread-safe
SAWYER_EXPORT std::string stringifyColor(AnsiColor color) {
  switch (color) {
    case COLOR_BLACK:
      return "black";
    case COLOR_RED:
...
// thread-safe
SAWYER_EXPORT std::string escape(const std::string &s) {
  std::string retval;
...
// no global state
SAWYER_EXPORT ColorSet ColorSet::blackAndWhite() {
  ColorSet cs;
  cs[WARN] = cs[ERROR] = cs[FATAL] =
      ColorSpec(COLOR_DEFAULT, COLOR_DEFAULT, true);
  return cs;
}

// no global state
SAWYER_EXPORT ColorSet ColorSet::fullColor() {
  ColorSet cs;
...
// no global state
SAWYER_EXPORT MesgProps MesgProps::merge(const MesgProps &other) const {
  MesgProps retval = *this;
  if (!facilityName) retval.facilityName = other.facilityName;
  if (!importance) retval.importance = other.importance;
  if (indeterminate(isBuffered)) retval.isBuffered = other.isBuffered;
  if (!completionStr) retval.completionStr = other.completionStr;
  if (!interruptionStr) retval.interruptionStr = other.interruptionStr;
  if (!cancelationStr) retval.cancelationStr = other.cancelationStr;
  if (!lineTermination) retval.lineTermination = other.lineTermination;
  if (indeterminate(useColor)) retval.useColor = other.useColor;
  return retval;
}

// no global state
SAWYER_EXPORT void MesgProps::print(std::ostream &o) const {
  o << "{facilityName=";
  if (facilityName) {
    o << "\"" << *facilityName << "\"";
  } else {
...
// thread-safety: safe to the extent that std::ostream is safe. props.print is
// safe.
SAWYER_EXPORT std::ostream &operator<<(std::ostream &o,
...
SAWYER_EXPORT void Mesg::insert(const std::string &s) {
  if (isComplete())
    throw std::runtime_error("cannot add text to a completed message");
  text_ += s;
}

SAWYER_EXPORT void Mesg::insert(char ch) {
  if (isComplete())
    throw std::runtime_error("cannot add text to a completed message");
  text_ += ch;
}

SAWYER_EXPORT void Mesg::post(const BakedDestinations &baked) const {
  for (BakedDestinations::const_iterator bi = baked.begin(); bi != baked.end();
       ++bi)
    bi->first->post(*this, bi->second);
}

SAWYER_EXPORT bool Mesg::hasText() const {
...
// thread-safe
SAWYER_EXPORT void Destination::bakeDestinations(const MesgProps &props,
...
SAWYER_EXPORT MesgProps Destination::mergePropertiesNS(const MesgProps &props) {
...
// thread-safe
SAWYER_EXPORT void Multiplexer::bakeDestinations(const MesgProps &props,
                                                 BakedDestinations &baked) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  MesgProps downwardProps = mergePropertiesNS(props);
  for (Destinations::const_iterator di = destinations_.begin();
       di != destinations_.end(); ++di)
    (*di)->bakeDestinations(downwardProps, baked);
}

// Multiplexors are never included in baked results (they control baking
// instead), so messages should never be posted directly
// to multiplexers.
SAWYER_EXPORT void Multiplexer::post(const Mesg &mesg, const MesgProps &props) {
...
Multiplexer::addDestination(const DestinationPtr &destination) {
  ...
  while (!work.empty()) {
    ...
    if (Multiplexer *seq = dynamic_cast<Multiplexer *>(getRawPointer(d)))
      work.insert(work.end(), seq->destinations_.begin(),
                  seq->destinations_.end());
  }
...
Multiplexer::removeDestination(const DestinationPtr &destination) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  destinations_.erase(
      std::remove(destinations_.begin(), destinations_.end(), destination),
      destinations_.end());
  return sharedFromThis().dynamicCast<Multiplexer>();
}
...
SAWYER_EXPORT MultiplexerPtr Multiplexer::to(const DestinationPtr &d1,
                                             const DestinationPtr &d2) {
  to(d1);
  return to(d2);
}
...
// thread-safe
SAWYER_EXPORT void Filter::bakeDestinations(const MesgProps &props,
                                            BakedDestinations &baked) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT size_t SequenceFilter::nSkip() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return nSkip_;
}

// thread-safe
SAWYER_EXPORT SequenceFilterPtr SequenceFilter::nSkip(size_t n) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  nSkip_ = n;
...
// thread-safe
SAWYER_EXPORT size_t SequenceFilter::rate() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return rate_;
}

// thread-safe
SAWYER_EXPORT SequenceFilterPtr SequenceFilter::rate(size_t n) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  rate_ = n;
  return sharedFromThis().dynamicCast<SequenceFilter>();
}
...
// thread-safe
SAWYER_EXPORT size_t SequenceFilter::limit() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return limit_;
}

// thread-safe
SAWYER_EXPORT SequenceFilterPtr SequenceFilter::limit(size_t n) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  limit_ = n;
  return sharedFromThis().dynamicCast<SequenceFilter>();
}
...
// thread-safe
SAWYER_EXPORT size_t SequenceFilter::nPosted() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return nPosted_;
}

// thread-safe
SAWYER_EXPORT bool SequenceFilter::shouldForward(const MesgProps &) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  bool retval = nPosted_ >= nSkip_ &&
                0 == (nPosted_ - nSkip_) % std::max(rate_, (size_t)1) &&
                (0 == limit_ ||
                 (nPosted_ - nSkip_) / std::max(rate_, (size_t)1) < limit_);
  ++nPosted_;
  return retval;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// thread-safe
SAWYER_EXPORT double TimeFilter::minInterval() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return minInterval_;
}

// thread-safe
SAWYER_EXPORT TimeFilterPtr TimeFilter::minInterval(double d) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  minInterval_ = d;
...
// thread-safe
SAWYER_EXPORT double TimeFilter::initialDelay() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return initialDelay_;
}

// thread-safe
SAWYER_EXPORT TimeFilterPtr TimeFilter::initialDelay(double delta) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  if (delta > 0.0) {
    initialDelay_ = delta;
    if (0 == nPosted_) lastBakeTime_ = now();
  }
  return sharedFromThis().dynamicCast<TimeFilter>();
}

// thread-safe
SAWYER_EXPORT size_t TimeFilter::nPosted() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return nPosted_;
}

// thread-safe
SAWYER_EXPORT bool TimeFilter::shouldForward(const MesgProps &) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  ++nPosted_;
  lastBakeTime_ = now();
...
// thread-safe
SAWYER_EXPORT void TimeFilter::forwarded(const MesgProps &) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT bool ImportanceFilter::enabled(Importance imp) const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return enabled_[imp];
}

// thread-safe
SAWYER_EXPORT ImportanceFilterPtr ImportanceFilter::enabled(Importance imp,
                                                            bool b) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  enabled_[imp] = b;
...
// thread-safe
SAWYER_EXPORT ImportanceFilterPtr ImportanceFilter::enable(Importance imp) {
  enabled(imp, true);
  return sharedFromThis().dynamicCast<ImportanceFilter>();
}

// thread-safe
SAWYER_EXPORT ImportanceFilterPtr ImportanceFilter::disable(Importance imp) {
  enabled(imp, false);
  return sharedFromThis().dynamicCast<ImportanceFilter>();
}

// thread-safe
SAWYER_EXPORT bool ImportanceFilter::shouldForward(const MesgProps &props) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  assert(!props.importance ||
         (*props.importance >= 0 && *props.importance < N_IMPORTANCE));
...
// thread-safe
unsigned HighWater::id() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return *id_;
}

// thread-safe
MesgProps HighWater::properties() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return props_;
}

// thread-safe
size_t HighWater::ntext() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return ntext_;
}
...
// This is a pointer so that it doesn't depend on order of global variable
// initialization. We'll allocate it the first time we
// need it.
...
GangPtr Gang::instanceForId(int id) {
  ...
  if (!gangs_) gangs_ = new GangMap;
  return gangs_->insertMaybe(id, Gang::instance());
}

// class method; thread-safe
GangPtr Gang::instanceForTty() { return instanceForId(TTY_GANG); }
...
// class method
PrefixPtr Prefix::silentInstance() {
...
// thread-safe (assuming Windows API is thread-safe)
SAWYER_EXPORT void Prefix::setProgramName() {
#ifdef BOOST_WINDOWS
#if 0  // [Robb Matzke 2014-06-13] temporarily disable for ROSE linking error
       // (needs psapi.lib in Windows)
  ...
  if (FILE *f = fopen("/proc/self/cmdline", "r")) {
    std::string name;
    int c;
    while ((c = fgetc(f)) > 0) name += (char)c;
    fclose(f);
    size_t slash_idx = name.rfind('/');
    if (slash_idx != std::string::npos) name = name.substr(slash_idx + 1);
...
SAWYER_EXPORT const Optional<double> Prefix::startTime() const {
  return startTime_;
}

SAWYER_EXPORT PrefixPtr Prefix::startTime(double t) {
  startTime_ = t;
  return sharedFromThis();
}

SAWYER_EXPORT void Prefix::setStartTime() {
...
SAWYER_EXPORT void Prefix::initFromSystem() {
  setProgramName();
  setStartTime();
}

SAWYER_EXPORT std::string Prefix::toString(const Mesg &mesg,
                                           const MesgProps &props) const {
  ...
  if (showFacilityName_ && props.facilityName) {
    if (SOMETIMES != showFacilityName_ ||
        0 != programNameShown.compare(*props.facilityName)) {
  ...
  if (showImportance_ && props.importance) {
    if (facilityNameShown.empty()) retval << separator;
    retval << "[" << std::setw(5) << std::left
           << stringifyImportance(*props.importance) << "]";
    separator = " ";
  }
...
SAWYER_EXPORT void UnformattedSink::init() {
...
// thread-safe
SAWYER_EXPORT bool UnformattedSink::partialMessagesAllowed() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT GangPtr UnformattedSink::gang() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return gang_;
}

// thread-safe
SAWYER_EXPORT UnformattedSinkPtr UnformattedSink::gang(const GangPtr &g) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  gangInternal(g);
  return sharedFromThis().dynamicCast<UnformattedSink>();
}
...
// thread-safe
SAWYER_EXPORT PrefixPtr UnformattedSink::prefix() const {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  return prefix_;
}

// thread-safe
SAWYER_EXPORT UnformattedSinkPtr UnformattedSink::prefix(const PrefixPtr &p) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  prefix_ = p;
  return sharedFromThis().dynamicCast<UnformattedSink>();
}
...
// not thread-safe
SAWYER_EXPORT std::string UnformattedSink::maybeTerminatePrior(
    const Mesg &mesg, const MesgProps &props) {
  std::string retval;
  if (!mesg.isEmpty()) {
...
// not thread-safe
SAWYER_EXPORT std::string UnformattedSink::maybePrefix(const Mesg &mesg,
                                                       const MesgProps &props) {
  std::string retval;
...
// not thread-safe
SAWYER_EXPORT std::string UnformattedSink::maybeBody(const Mesg &mesg,
                                                     const MesgProps &props) {
  std::string retval;
  if (!mesg.isEmpty()) retval = mesg.text().substr(gang()->ntext());
  return retval;
}

// not thread-safe
SAWYER_EXPORT std::string UnformattedSink::maybeFinal(const Mesg &mesg,
                                                      const MesgProps &props) {
  std::string retval;
  if (!mesg.isEmpty()) {
    if (mesg.isCanceled()) {
      retval =
          props.cancelationStr.orDefault() + props.lineTermination.orDefault();
      gang()->clear();
    } else if (mesg.isComplete()) {
      retval =
          props.completionStr.orDefault() + props.lineTermination.orDefault();
      gang()->clear();
    } else {
...
// thread-safe
SAWYER_EXPORT std::string UnformattedSink::render(const Mesg &mesg,
                                                  const MesgProps &props) {
  std::string retval;
  LockGuard2<SAWYER_THREAD_TRAITS::RecursiveMutex> lock(mutex_,
                                                        gang()->mutex());
  retval += maybeTerminatePrior(
      mesg, props);  // force side effects in a particular order
...
// thread-safe
SAWYER_EXPORT void UnformattedSink::gangInternal(const GangPtr &g) {
  SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
  gang_ = g;
}
...
SAWYER_EXPORT void FdSink::init() {
#ifdef BOOST_WINDOWS
  ...
  if (isatty(fd_)) {
    ...
    defaultPropertiesNS().useColor =
        true;  // use color if the user doesn't care
  } else {
    gangInternal(Gang::instanceForId(fd_));
    overridePropertiesNS().useColor = false;  // force false; user can still set
                                              // this if they really want color
  }
#endif
  defaultPropertiesNS().isBuffered =
      2 != fd_;  // assume stderr is unbuffered and the rest are buffered
}

// thread-safe
SAWYER_EXPORT void FdSink::post(const Mesg &mesg, const MesgProps &props) {
  if (!partialMessagesAllowed_ && !mesg.isComplete()) return;
#ifdef BOOST_WINDOWS
  // FIXME[Robb Matzke 2014-06-10]: what is the most basic file level on
  // Windows; one which doesn't need construction?
...
// only called from the c'tor
SAWYER_EXPORT void FileSink::init() {
#ifdef BOOST_WINDOWS
  gangInternal(Gang::instanceForTty());
  overridePropertiesNS().useColor = true;
  defaultPropertiesNS().isBuffered = false;
  ...
  defaultPropertiesNS().isBuffered =
      2 !=
      fileno(file_);  // assume stderr is unbuffered and the rest are buffered
#endif
}

// thread-safe
SAWYER_EXPORT void FileSink::post(const Mesg &mesg, const MesgProps &props) {
  if (!partialMessagesAllowed_ && !mesg.isComplete()) return;
...
// thread-safe
SAWYER_EXPORT void StreamSink::post(const Mesg &mesg, const MesgProps &props) {
  if (!partialMessagesAllowed_ && !mesg.isComplete()) return;
...
// only called from the c'tor
SAWYER_EXPORT void SyslogSink::init() {
  overridePropertiesNS().isBuffered = true;
}

// thread-safe assuming syslog is thread-safe
SAWYER_EXPORT void SyslogSink::post(const Mesg &mesg, const MesgProps &props) {
  if (mesg.isComplete()) {
    ...
    // This mutex doesn't really protect syslog completely and I'm not sure
    // whether syslog itself is thread-safe.  But this
    // mutex will work in the case when syslog is not thread-safe but only one
    // Message::Destination is talking to syslog.
    SAWYER_THREAD_TRAITS::RecursiveLockGuard lock(mutex_);
...
// This is the internal part of Stream. Every Stream has exactly one of these,
// thus the mutex is stored in Stream instead of
// here.
...
// not synchronized
void StreamBuf::post() {
  if (enabled_ && message_.hasText() &&
      (message_.isComplete() || anyUnbuffered_)) {
    assert(isBaked_);
...
void StreamBuf::bake() {
  if (!isBaked_) {
    ...
    anyUnbuffered_ = false;
    for (BakedDestinations::const_iterator bi = baked_.begin();
         bi != baked_.end() && !anyUnbuffered_; ++bi)
...
std::streamsize StreamBuf::xsputn(const char *s, std::streamsize &n) {
  ...
  // This is called from std::ostream::operator<< (and possibly others), so we
  // need to acquire a lock. Since this object is
  // owned by exactly one Stream object, the mutex we lock is in the stream
  // object.
  assert(stream_ != NULL);
...
SAWYER_EXPORT
Stream::Stream(const std::string facilityName, Importance imp,
...
// thread-safe
SAWYER_EXPORT Stream &Stream::operator=(const Stream &other) {
...
// thread-safe
SAWYER_EXPORT Stream &Stream::operator=(const std::ostream &other_) {
  const Stream *other = dynamic_cast<const Stream *>(&other_);
  if (!other)
    throw "Sawyer::Message::Stream initializer is not a Sawyer::Message::Stream (only a std::ostream)";
  LockGuard2<SAWYER_THREAD_TRAITS::Mutex> lock(mutex_, other->mutex_);
...
void Stream::initFromNS(const Stream &other) {
  ...
  // If this stream has a partial message then it needs to be canceled.  This
  // also resets things associated with
  ...
  // Swap message-related stuff in order to move ownership of the message to
  // this.
  streambuf_->message_.properties() = streambuf_->dflt_props_;
...
// thread-safe
SAWYER_EXPORT void Stream::incrementRefCount() {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  ++nrefs_;
}

// thread-safe
SAWYER_EXPORT size_t Stream::decrementRefCount() {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  assert(nrefs_ > 0);
  return --nrefs_;
}
...
// thread-safe
SAWYER_EXPORT SProxy Stream::dup() const { return SProxy(new Stream(*this)); }

// thread-safe
SAWYER_EXPORT bool Stream::enabled() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  return streambuf_->enabled_;
}

// thread-safe
SAWYER_EXPORT void Stream::enable(bool b) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  if (!b) {
...
// thread-safe
SAWYER_EXPORT void Stream::completionString(const std::string &s,
                                            bool asDefault) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT void Stream::interruptionString(const std::string &s,
                                              bool asDefault) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT void Stream::cancelationString(const std::string &s,
                                             bool asDefault) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT void Stream::facilityName(const std::string &s, bool asDefault) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
DestinationPtr Stream::destination() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  return streambuf_->destination_;
}

// thread-safe
Stream &Stream::destination(const DestinationPtr &d) {
  assert(d != NULL);
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
MesgProps Stream::properties() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
SProxy::SProxy(const SProxy &other) : stream_(other.stream_) {
  if (stream_) stream_->incrementRefCount();
}

SAWYER_EXPORT SProxy &SProxy::operator=(const SProxy &other) {
  if (this != &other) {
...
SAWYER_EXPORT void SProxy::reset() {
  if (stream_ != NULL) {
    if (0 == stream_->decrementRefCount()) delete stream_;
    stream_ = NULL;
  }
}

SAWYER_EXPORT
SProxy::operator bool() const { return stream_ != NULL; }
...
// thread-safe
SAWYER_EXPORT Facility &Facility::operator=(const Facility &other) {
  SAWYER_THREAD_TRAITS::LockGuard lock(other.mutex_);
  constructed_ = CONSTRUCTED_MAGIC;
  name_ = other.name_;
  streams_ = other.streams_;
...
// thread-safe
SAWYER_EXPORT Facility &Facility::initStreams(
    const DestinationPtr &destination) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  if (streams_.empty()) {
...
// thread-safe
SAWYER_EXPORT Facility &Facility::renameStreams(const std::string &name) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  for (size_t i = 0; i < streams_.size(); ++i)
...
// thread-safe
SAWYER_EXPORT Stream &Facility::get(Importance imp) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  if (imp < 0 || imp >= N_IMPORTANCE)
    throw std::runtime_error("invalid importance level");
  if (!isConstructed()) {
    // If you're looking at this line in a debugger it's probably because you're
    // trying to use a Stream declared at
    // namespace scope from inside a constructor for another namespace scope
    // object.  There may be issues with how your C++
    // runtime initializes such objects; perhaps your object (the one using this
    // Facility) is being initialized before this
    // Facility.  You could try to figure out the correct order to initialize
    // these objects, or you could explicitly call
    // the Sawyer initializer: Sawyer::initializeLibrary().
    std::ostringstream ss;
    ss << "stream " << stringifyImportance(imp) << " in facility " << this
       << " is not constructed yet";
    throw std::runtime_error(ss.str());
  }
  if (streams_.empty()) {
    // If you're looking at this line in a debugger it's probably because you're
    // trying to use default-constructed Stream.
    //
    // The typical way to initialize a global Facility is to declare it with a
    // default constructor then then before it's
    // used for the first time, assign a new Facility object to the global
    // variable.  E.g.,
    //
    // |// file example.C
    ...
    //
    // ROSE users: librose does not currently (2014-09-09) initialize libsawyer
    // until the ROSE frontend() is called. If
    // you're calling into librose before calling "frontend" then you probably
    // want to explicitly initialize ROSE by
    // invoking rose::Diagnostics::initialize() early in "main". This will cause
    // all of ROSE's Facility objects to be
    // constructed.
    std::ostringstream ss;
    ss << "stream " << stringifyImportance(imp) << " in facility " << this
...
// thread-safe
SAWYER_EXPORT std::string Facility::name() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  return name_;
}
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::operator=(const Facilities &other) {
  LockGuard2<SAWYER_THREAD_TRAITS::Mutex> lock(mutex_, other.mutex_);
  facilities_ = other.facilities_;
  impset_ = other.impset_;
  impsetInitialized_ = other.impsetInitialized_;
...
// thread-safe
SAWYER_EXPORT Facilities::ImportanceSet Facilities::impset() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  return impset_;
}

// thread-safe
SAWYER_EXPORT Facilities &Facilities::impset(Importance imp, bool enabled) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::insert(Facility &facility,
...
// not synchronized
SAWYER_EXPORT Facilities &Facilities::insertNS(Facility &facility,
                                               std::string name) {
  if (name.empty()) name = facility.name();
  if (name.empty())
  ...
  if (!impsetInitialized_) {
    ...
    for (int i = 0; i < N_IMPORTANCE; ++i) {
      ...
      if (facility[mi]) impset_.insert(mi);
    }
    impsetInitialized_ = true;
  }
  return *this;
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::insertAndAdjust(Facility &facility,
                                                      std::string name) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::erase(const std::string &name) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::erase(Facility &facility) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  FacilityMap map = facilities_;
  ;
  BOOST_FOREACH (const FacilityMap::Node &node, map.nodes()) {
    if (node.value() == &facility) facilities_.erase(node.key());
  }
  return *this;
}

// thread-safe, although the facility could be deleted
SAWYER_EXPORT Facility &Facilities::facility(const std::string &name) const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  return *facilities_[name];
}

// thread-safe
SAWYER_EXPORT Facilities &Facilities::reenable() {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::reenableFrom(const Facilities &other) {
  LockGuard2<SAWYER_THREAD_TRAITS::Mutex> lock(mutex_, other.mutex_);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::enable(const std::string &switch_name,
                                             bool b) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::enable(Importance imp, bool b) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  return enableNS(imp, b);
}

// not synchronized
SAWYER_EXPORT Facilities &Facilities::enableNS(Importance imp, bool b) {
  if (b) {
    impset_.insert(imp);
  } else {
    impset_.erase(imp);
...
// thread-safe
SAWYER_EXPORT Facilities &Facilities::enable(bool b) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
std::string Facilities::ControlTerm::toString() const {
  ...
  if (lo == hi) {
    ...
    s += " levels " + stringifyImportance(lo) + " through " +
         stringifyImportance(hi);
  }
  s += " for " +
       (facilityName.empty() ? "all registered facilities" : facilityName);
  return s;
}

// class method; thread-safe
// Matches the Perl regular expression /^\s*([a-zA-Z]\w*((\.|::)[a-zA-Z]\w*)*/
// On match, returns $1 and str points to the next character after the regular
// expression
// When not matched, returns "" and str is unchanged
SAWYER_EXPORT std::string Facilities::parseFacilityName(const char *&str) {
  std::string name;
  const char *s = str;
  while (isspace(*s)) ++s;
  while (isalpha(*s)) {
...
// class method; thread-safe
// Matches the Perl regular expression /^\s*([+!]?)/ and returns $1 on success
// with str pointing to the character after the
// match.  Returns the empty string on failure with str not adjusted.
SAWYER_EXPORT std::string Facilities::parseEnablement(const char *&str) {
  const char *s = str;
  while (isspace(*s)) ++s;
  if ('!' == *s || '+' == *s) {
    str = s + 1;
...
// class method; thread-safe
// Matches the Perl regular expression /^\s*(<=?|>=?)/ and returns $1 on success
// with str pointing to the character after
// the match. Returns the empty string on failure with str not adjusted.
SAWYER_EXPORT std::string Facilities::parseRelation(const char *&str) {
  const char *s = str;
  while (isspace(*s)) ++s;
...
// /^\s*(all|none|debug|trace|where|info|warn|error|fatal)\b/
// On match, returns $1 and str points to the next character after the match
// On failure, returns "" and str is unchanged
SAWYER_EXPORT std::string Facilities::parseImportanceName(const char *&str) {
  static const char *words[] = {"all",   "none", "debug", "trace", "where",
                                "march", "info", "warn",  "error", "fatal"};
  ...
  for (size_t i = 0; i < nwords; ++i) {
    ...
    if (boost::iequals(std::string(s).substr(0, n),
                       std::string(words[i]).substr(0, n)) &&
        '_' != s[n]) {
      str += (s - str) + n;
      return words[i];
    }
...
Facilities::importanceFromString(const std::string &str) {
  if (boost::iequals(str, "debug")) return DEBUG;
  if (boost::iequals(str, "trace")) return TRACE;
  if (boost::iequals(str, "where")) return WHERE;
  if (boost::iequals(str, "march")) return MARCH;
  if (boost::iequals(str, "info")) return INFO;
  if (boost::iequals(str, "warn")) return WARN;
  if (boost::iequals(str, "error")) return ERROR;
  if (boost::iequals(str, "fatal")) return FATAL;
  return N_IMPORTANCE;  // error
}

// class method
// no global state
// Parses a StreamControlList. On success, returns a non-empty vector and adjust
// 'str' to point to the next character after the
// list.  On failure, throw a ControlError.
SAWYER_EXPORT std::list<Facilities::ControlTerm>
Facilities::parseImportanceList(const std::string &facilityName,
                                const char *&str, bool isGlobal) {
  const char *s = str;
  ...
  while (1) {
    ...
    if (importance.empty()) {
      if (!enablement.empty() || !relation.empty() || !isGlobal)
        throw ControlError("message importance level expected",
                           importanceStart);
      s = elmtStart;
      break;
    }
    ...
    if (boost::iequals(importance, "all") ||
        boost::iequals(importance, "none")) {
      if (!enablement.empty())
        throw ControlError(
            "'" + importance + "' cannot be preceded by '" + enablement + "'",
            enablementStart);
      if (!relation.empty())
        throw ControlError(
            "'" + importance + "' cannot be preceded by '" + relation + "'",
            relationStart);
      term.lo = DEBUG;
      term.hi = FATAL;
      ...
      if (N_IMPORTANCE == imp)
        throw ControlError("'" + importance + "' is not a valid importance",
                           relationStart);
      if (relation.empty()) {
        term.lo = term.hi = imp;
      } else if (relation[0] == '<') {
        ...
        term.hi = imp;
        if (1 == relation.size()) {
          if (DEBUG == imp) continue;  // empty set
        ...
        term.hi = FATAL;
        if (1 == relation.size()) {
          if (FATAL == imp) continue;  // empty set
  ...
  if (!retval.empty()) str = s;
  return retval;
}

// thread-safe
SAWYER_EXPORT std::string Facilities::control(const std::string &ss) {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  ...
  try {
    while (1) {
      ...
      if (t2.empty()) {
        ...
        if (facilityName.empty()) break;
        if (!facilities_.exists(facilityName))
          throw ControlError("no such message facility '" + facilityName + "'",
                             facilityNameStart);
        ...
        if ('(' != *s)
          throw ControlError(
              "expected '(' after message facility name '" + facilityName + "'",
              s);
        ++s;
        ...
        if (')' != *s)
          throw ControlError(
              "expected ')' at end of stream control list for '" +
                  facilityName + "'",
              s);
        ++s;
      }
    ...
    while (isspace(*s)) ++s;
    if (*s) {
      if (terms.empty()) throw ControlError("syntax error", s);
      if (terms.back().facilityName.empty())
        throw ControlError("syntax error in global list", s);
      throw ControlError(
          "syntax error after '" + terms.back().facilityName + "' list", s);
    }
  ...
  for (std::list<ControlTerm>::iterator ti = terms.begin(); ti != terms.end();
       ++ti) {
    const ControlTerm &term = *ti;
    if (term.facilityName.empty()) {
      for (Importance imp = term.lo; imp <= term.hi;
           imp = (Importance)(imp + 1))
...
// thread-safe
SAWYER_EXPORT std::string Facilities::configuration() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  std::string retval;
  BOOST_FOREACH (const FacilityMap::Node &facility, facilities_.nodes()) {
    ...
    for (int imp = 0; imp < N_IMPORTANCE; ++imp) {
      retval += (imp == 0 ? "" : ",");
      if (!(*facility.value())[(Importance)imp]) retval += "!";
...
// thread-safe
SAWYER_EXPORT std::vector<std::string> Facilities::facilityNames() const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
...
// thread-safe
SAWYER_EXPORT void Facilities::print(std::ostream &log) const {
  SAWYER_THREAD_TRAITS::LockGuard lock(mutex_);
  if (impsetInitialized_) {
    for (int i = 0; i < N_IMPORTANCE; ++i) {
      Importance mi = (Importance)i;
      log << (impset_.find(mi) == impset_.end()
                  ? '-'
                  : (mi == WHERE ? 'H' : stringifyImportance(mi)[0]));
    }
  ...
  if (facilities_.isEmpty()) {
    ...
    BOOST_FOREACH (const FacilityMap::Node &fnode, facilities_.nodes()) {
      ...
      // A short easy to read format. Letters indicate the importances that are
      // enabled; dashes keep them aligned.
...
void FacilitiesGuard::restore() {
  BOOST_FOREACH (const State::Node &saved, state_.nodes()) {
    try {
      ...
      // name probably doesn't exist in this facility any more, so don't try to
      // enable/disable its streams.
    }
  }
}
