...
Storage::checkBoost() const
{
    ...
    // [https://svn.boost.org/trac/boost/ticket/9215]. It is better if we check for this
    // at runtime rather than compile time because many other features of boost 1.54 work
    // fine. If we allow the user to continue, then boost::any's move constructor will
    // enter infinite recursion eventually ending with a segmentation fault (although
    // probably not occuring as a result of calling just Storage's c'tor).
