...
Storage::checkBoost() const {
    ...
    // fine. If we allow the user to continue, then dyncompat::any's move constructor will enter infinite recursion eventually
    // ending with a segmentation fault (although probably not occuring as a result of calling just Storage's c'tor).
    // boost version check removed — no longer using boost::any
}
