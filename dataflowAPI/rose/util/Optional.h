...
/** Represents no value.
 *
 *  An object of type Nothing can be assigned to any @ref Optional object to
 * make it contain nothing.
 *
 * @code
 *  Optional<int> opt1;
 ...
 *
 *  This class is similar to boost::optional except simpler in order to avoid
 * problems we were seeing with Microsoft
 *  compilers.
 *
...
class Optional {
 ...
 public:
   ...
   *
   *  Constructs an optional value that points to nothing. The default
   * constructor for @ref Value is not called. */
   ...
   *
   *  Constructs an optional object that holds nothing.  The default constructor
   * for @ref Value is not called. */
  Optional(const Nothing &) : isEmpty_(true) {}

  /** Copy constructor.
   *
   *  Copy constructs an optional so that the new object has the same state
   * (nothing or not) as the source object. If the
   *  source contains an object (not nothing) then the object is copy
   * constructed into this optional. */
  ...
  /** Destructor.
   *
   *  The destructor invokes the destructor for the value if a value is stored,
  ...
  /** Value assignment.
   *
   *  Assigns the @p value to this optional. If this optional previously
   * contained a value then the @ref Value assignment
  ...
  /** %Nothing assignment.
   *
   *  Assigns nothing to this optional.  If this optional previously contained a
   * value then the value's destructor is
   *  called. */
  ...
  /** Optional assignment.
   *
   *  Causes this optional to contain nothing or a copy of the @p other value
   * depending on whether @p other contains a value
   *  or not.  If this didn't contain a value but @p other did, then the @ref
   * Value copy constructor is used; if both
   *  contained a value then the @ref Value assignment operator is used; if this
   * contained a value but @p other doesn't, then
   ...
   *
   *  If this optional contains a value then a reference to that value is
   * returned. Otherwise an
   *  <code>std::domain_error</code> is thrown (the value is not in the
   * container's domain).
   *
   *  @{ */
  ...
  const Value &get() const {
    if (isEmpty_) throw std::domain_error("dereferenced nothing");
   ...
   *
   *  If this optional contains a value then a pointer to the value is returned.
   * Otherwise an <code>std::domain_error</code>
   ...
   *
   *  Returns a reference to the contained value if it exists, otherwise returns
  ...
  const Value &orElse(Value &dflt) { return isEmpty_ ? dflt : **this; }
  /** @} */

  /** Obtain a value or a default.
   *
   *  Returns a copy of the contained value if it exists, otherwise returns a
   ...
   * @endcode
   */
  Value orDefault() const { return isEmpty_ ? Value() : **this; }

  /** Conditionally save a value.
   *
   *  If this optional object has a value then its value is written to @p out
   * and this method returns true; otherwise the
   *  value of @p out is not changed and this method returns false.  This method
   * is convenient to use in conditional
  ...
  /** Compare two values.
   *
   *  Compares two optionals and returns true if they are both empty or if
   * neither is empty and their values compare equal.
   *  This method should be used instead of <code>==</code>. The <code>==</code>
   * operator is disabled because it is prone to
  ...
  bool isEqual(const Optional &other) const {
    return (isEmpty_ && other.isEmpty_) ||
           (!isEmpty_ && !other.isEmpty_ && get() == other.get());
  }
  bool isEqual(const Value &other) const { return !isEmpty_ && get() == other; }
  bool isEqual(const Nothing &) const { return isEmpty_; }
  /** @} */
...
// and thus save users from making
// mistakes like this:
//    Optional<int> x = 0;
//    int y = 1;
