...
private:
    ...
    typedef Sawyer::Container::Interval<dyncompat::uint64_t> ChunkAddressInterval;
...
private:
    ...
    public:
        ...
        ChunkAddressInterval extent() const {
            return ChunkAddressInterval::hull(reinterpret_cast<dyncompat::uint64_t>(data_),
                                              reinterpret_cast<dyncompat::uint64_t>(data_+chunkSize-1));
        }
    };
        ...
        ChunkInfoMap chunkInfoNS() const {
            ...
            for (size_t freeListIdx = 0; freeListIdx < N_FREE_LISTS; ++freeListIdx) {
                for (FreeCell *cell=freeLists_[freeListIdx]; cell!=NULL; cell=cell->next) {
                    typename ChunkInfoMap::ValueIterator found = map.find(reinterpret_cast<dyncompat::uint64_t>(cell));
        ...
        void vacuum() {
            ...
            for (size_t oldFreeListIdx=0; oldFreeListIdx<N_FREE_LISTS; ++oldFreeListIdx) {
                ...
                for (FreeCell *cell = freeLists_[oldFreeListIdx]; cell != NULL; cell = next) {
                    next = cell->next;
                    dyncompat::uint64_t cellAddr = reinterpret_cast<dyncompat::uint64_t>(cell);
            ...
            while (iter!=chunks_.end()) {
                Chunk *chunk = *iter;
                dyncompat::uint64_t cellAddr = chunk->extent().least(); // any cell will do
                if (map[cellAddr].nUsed == 0) {
                    delete chunk;
