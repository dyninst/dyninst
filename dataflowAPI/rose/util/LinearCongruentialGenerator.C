...
{
    if(0 == access("/dev/urandom", R_OK))
    {  // try non-blocking version first
    ...
    else if(0 == access("/dev/random", R_OK))
    {  // this one might block for a while
...
LinearCongruentialGenerator::next(size_t nbits, size_t niter)
{
    ...
    for(size_t i = 0; i < niter; ++i)
    {
        ...
        uint64_t v =
            (value_ >> 17) & 0x3fffff;  // 22 bits: 17 (inclusive) through 39 (exclusive)
        value_ = ((a_hi << 24) | a_lo) * value_ + c;
