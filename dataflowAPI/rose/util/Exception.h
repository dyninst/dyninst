...
class SAWYER_EXPORT RuntimeError : public std::runtime_error {
 public:
  ...
  explicit RuntimeError(const std::string &mesg) : std::runtime_error(mesg) {}
};
...
class SAWYER_EXPORT DomainError : public RuntimeError {
 public:
  ...
  /** Constructor taking a description of the error. */
  explicit DomainError(const std::string &mesg) : RuntimeError(mesg) {}
};

/** Error for non-existing values.
 *
 *  This error is thrown when querying a container and no value is stored for
 * the specified key. */
class SAWYER_EXPORT NotFound : public DomainError {
 public:
  ...
  /** Constructor taking a description of the error. */
  explicit NotFound(const std::string &mesg) : DomainError(mesg) {}
};

/** Error for existing values.
 *
 *  This error is thrown when attempting to modify a container by inserting a
...
class SAWYER_EXPORT AlreadyExists : public DomainError {
 public:
  ...
  /** Constructor taking a description of the error. */
  AlreadyExists(const std::string &mesg) : DomainError(mesg) {}
};
