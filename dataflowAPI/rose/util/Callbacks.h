...
class Callbacks {
 ...
 public:
  ...
  Callbacks &append(const Callbacks &other) {
    callbacks_.insert(callbacks_.end(), other.callbacks_.begin(),
                      other.callbacks_.end());
    return *this;
  }
  ...
  Callback &prepend(const Callbacks &other) {
    callbacks_.insert(callbacks_.begin(), other.callbacks_.begin(),
                      other.callbacks_.end());
    return *this;
  }

  Callbacks &eraseFirst(const Callback &callback) {
    for (typename CbList::iterator iter = callbacks_.begin();
  ...
  Callbacks &eraseLast(const Callback &callback) {
    for (typename CbList::reverse_iterator iter = callbacks_.rbegin();
         iter != callbacks_.rend(); ++iter) {
      if (*iter == callback) {
        callbacks_.erase(iter);
        break;
      }
  ...
  Callbacks &eraseMatching(const Callback &callback) {
    ...
    while (iter != callbacks_.end()) {
      if (*iter == callback) {
        ...
        callbacks_.erase(
            toErase);  // std::list iterators are stable over erasure
      } else {
        ++iter;
      }
    }
  ...
  template <class CB, class Args>
  bool applyCallback(const SharedPointer<CB> &callback, bool chained,
  ...
  bool apply(bool chained, const Arguments &arguments) const {
    for (typename CbList::const_iterator iter = callbacks_.begin();
         iter != callbacks_.end(); ++iter)
