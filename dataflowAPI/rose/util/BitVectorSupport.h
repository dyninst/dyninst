...
template<class Word>
void fromInteger(Word *words, const BitRange &range, dyncompat::uint64_t value) {
...
template<class Word>
dyncompat::uint64_t toInteger(const Word *words, const BitRange &range) {
    dyncompat::uint64_t result = 0;
    ...
    for (size_t i=0; i<nTmpWords; ++i)
        result |= (dyncompat::uint64_t)tmp[i] << (i * bitsPerWord<Word>::value);
    return result;
}
...
template<class Word>
dyncompat::uint64_t toInteger(const Word *words, size_t nbits) {
    dyncompat::uint64_t result = 0;
    ASSERT_require(nbits <= 64);
    size_t nTmpWords = numberOfWords<Word>(nbits);
    for (size_t i=0; i<nTmpWords; ++i)
        result |= (dyncompat::uint64_t)words[i] << (i * bitsPerWord<Word>::value);
    if (nbits < 64)
...
void fromDecimal(Word *vec, const BitRange &range, const std::string &input) {
    dyncompat::uint64_t v = 0;
    DYN_FOREACH (char ch, input) {
        if (isdigit(ch)) {
            dyncompat::uint64_t tmp = v * 10 + (ch - '0');
            if (tmp < v)
