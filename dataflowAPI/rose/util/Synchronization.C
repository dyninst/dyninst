...
#else
// Boost 1.45 and 1.46 say that mt11213b is only 44% as fast as rand48. Also, these things
// were not part of the boost::random namespace in those versions.
...
namespace Sawyer
{
// In order to get thread safety, you probably need to add "-pthread" to all GCC compile
// and link lines.  It is not sufficient to only add "-lpthreads" to the link lines.
#if SAWYER_MULTI_THREADED == 0
#    ifdef _MSC_VER
#        pragma message(                                                                 \
            "Sawyer multi-threading is disabled; even documented thread-safe functions are unsafe!")
#    else
#        warning                                                                         \
            "Sawyer multi-threading is disabled; even documented thread-safe functions are unsafe!"
#    endif
...
fastRandomIndex(size_t n)
{
    ...
    // FIXME[Robb Matzke 2015-12-08]: This leaks memory when threads that use this
    // function are destroyed, but the common denominator across compilers is that
    // thread-local storage can only be applied to POD types.
