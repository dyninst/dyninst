...
#else
// Boost 1.45 and 1.46 say that mt11213b is only 44% as fast as rand48. Also,
// these things were not part of the
...
// In order to get thread safety, you probably need to add "-pthread" to all GCC
// compile and link lines.  It is not sufficient
...
SAWYER_EXPORT size_t fastRandomIndex(size_t n) {
  ...
  // function are destroyed, but the common
  // denominator across compilers is that thread-local storage can only be
  // applied to POD types.
  static SAWYER_THREAD_LOCAL SAWYER_PRN_GENERATOR* generator = NULL;
  if (!generator) generator = new SAWYER_PRN_GENERATOR;
