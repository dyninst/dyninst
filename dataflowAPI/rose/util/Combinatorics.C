...
std::vector<uint8_t> sha1_digest(const uint8_t *data, size_t size) {
  ...
  gcry_error_t error __attribute__((unused)) =
...
uint64_t fnv1a64_digest(const uint8_t *data, size_t size) {
  ...
  for (size_t i = 0; i < size; ++i) hash = (hash ^ data[i]) * 0x100000001b3ull;
  return hash;
}
...
std::string digest_to_string(const std::vector<uint8_t> &data) {
