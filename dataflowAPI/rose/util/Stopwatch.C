...
static Stopwatch::TimePoint getCurrentTime() {
  ...
  // t -= 11644473600000000Ui64;                       // convert file time to
  // microseconds since Unix epoch
  return t / 1e6;
#else  // POSIX
  struct timeval t;
  if (-1 == gettimeofday(&t, NULL)) return 0.0;
  return t.tv_sec + 1e-6 * t.tv_usec;
#endif
}

SAWYER_EXPORT double Stopwatch::report(bool clear) const {
  if (running_) {
...
#endif
  if (clear) elapsed_ = Duration();
  return retval;
}

SAWYER_EXPORT double Stopwatch::start() {
...
SAWYER_EXPORT double Stopwatch::start(double value) {
  double retval = report(false);
...
SAWYER_EXPORT double Stopwatch::restart() { return start(0.0); }

SAWYER_EXPORT double Stopwatch::stop(bool clear) {
  double retval = report(clear);
  running_ = false;
  return retval;
}

SAWYER_EXPORT double Stopwatch::clear(double value) {
  double retval = stop();
  elapsed_ = Duration(value);
  return retval;
}

SAWYER_EXPORT std::ostream &operator<<(std::ostream &out, const Stopwatch &x) {
  out << x.report();
  return out;
}
