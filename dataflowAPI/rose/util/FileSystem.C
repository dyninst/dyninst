...
#if BOOST_FILESYSTEM_VERSION == \
    2  // FIXME[Robb P. Matzke 2014-11-18]: Remove version 2 support
...
Path createTemporaryDirectory() {
...
#ifdef _MSC_VER
  Path dirName =
      "/tmp";  // FIXME[Robb P. Matzke 2014-11-18]: is this right for Windows?
#else
  Path dirName = "/tmp";
  ...
  for (size_t i = 0; i < base.size(); ++i) {
    if ('%' == base[i]) base[i] = "0123456789abcdef"[lcg() % 16];
  }
  dirName /= base;
#else
  Path dirName = boost::filesystem::temp_directory_path() /
                 boost::filesystem::unique_path(tempNamePattern);
#endif
...
Path makeNormal(const Path &path) {
  ...
  for (boost::filesystem::path::const_iterator i = path.begin();
       i != path.end(); ++i) {
#if BOOST_FILESYSTEM_VERSION == \
    2  // FIXME[Robb P. Matzke 2014-11-18]: Remove version 2 support
...
Path makeRelative(const Path &path_, const Path &root_) {
  ...
  while (pathIter != path.end()) retval /= *pathIter++;
  return retval;
}
...
// This doesn't make any sense! First, BOOST_COMPILED_WITH_CXX11 is never
// defined in any version of boost. Second, even if it
// were defined, it would come from boost header files which are always compiled
// with the same compile as that which is
// compiling this source file. [Robb Matzke 2016-02-17]
//#if (__cplusplus >= 201103L)
//#if !defined(BOOST_COMPILED_WITH_CXX11)
//   #warning "Compiling ROSE with C++11 mode: BOOST NOT compiled with C++11
//   support."
//#else
//   #warning "Compiling ROSE with C++11 mode: BOOST WAS compiled with C++11
//   support."
//#endif
//#endif

void copyFile(const Path &src, const Path &dst) {
  // Do not use boost::filesystem::copy_file in boost 1.56 and earlier because
  // it is not possible to cross link c++11 rose
  // with c++89 boost when using this symbol.  Boost issue #6124 fixed in boost
  // 1.57 and later. Our solution is to use C++
  // stream I/O instead, which should still work on non-POSIX systems
  // (Microsoft) although the exception situations might not
  // be exactly precise as POSIX. Use path::string rather than path::native in
  // order to support Filesystem version 2.
...
std::vector<Path> findRoseFilesRecursively(const Path &root) {
  return findNamesRecursively(root, baseNameMatches(boost::regex("rose_.*")),
                              isDirectory);
}
