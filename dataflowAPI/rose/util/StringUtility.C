...
// DQ (8/31/2009): This now compiles properly (at least for analysis, it might
// still fail for the code generation).
// #ifndef USE_ROSE
...
/* Added htmlEscape necessary for QROSE work to this utility library - tps
 * (9Oct2008) */
...
#if ROSE_MICROSOFT_OS
  printf(
      "Error: MSVS implementation of StringUtility::findfile required (not "
      "implemented) \n");
...
// and returns the absolute file name  (with path)
// AS suggested we use realpath that comes with stdlib. However, for a
// translator like ours,
// we need to have two versions of the file: silent and non-silent ones,
// depending on whether
// we need to check and print an error or not.
// That is done by the boolean parameter printErrorIfAny that is set to true by
// the caller
// The dafault version -- enforced by setting printErrorIfAny to false -- is the
// silent one.
//
...
std::string StringUtility::getAbsolutePathFromRelativePath(
    const std::string &relativePath,
    bool printErrorIfAny)  //! get the absolute path from the relative path
{
  string returnString;
  ...
  // exist.  This is
  // a problem for include paths that are specified on the commandline and which
  // don't exist;
  ...
  string resultingPath = "";
  if (rp != NULL) resultingPath = string(rp);
#endif
  ...
  if (resultingPath.empty() == true)  //== NULL )
  {
    ...
    // used to compile
    // ROSETTA before the IR nodes are defined!  So we should just comment it
    // out.
    // DQ (9/4/2006): Only output such warnings if verbose is set to some value
    // greater than zero.
    if (printErrorIfAny == true) {
      ...
      printf(
          "Error: StringUtility::getAbsolutePathFromRelativePath incured an "
          "error in use of realpath() and is returning the input relativePath. "
          "\n");
      //}
    }
...
// display each string representing a variable name
    ...
    returnString += *listStringElementIterator + " ";
    if (separateStrings) returnString += "\n";
  }
...
string StringUtility::listToString(const vector<string> &X,
                                   bool separateStrings) {
...
list<string> StringUtility::stringToList(const string &X) {
  // Build a list of strings representing the input string spearated at newline
  // characters ("\n"'s)
  // All CR are removed, the list returned has no CR in the list OR within the
  // list elements
...
// int subStringLength = 0;
  ...
  // If there is no trailing '\n' then at least include the input X string into
  // the list
  if (nextPos == string::npos) returnStringList.push_back(X);
  ...
  while (nextPos != string::npos) {
    ...
    remainingSubstring = remainingSubstring.substr(
        nextPos + 1, (remainingSubstring.length() - nextSubStringLength) - 0);
#if 0
  ...
  // Remove any strings consisting of only CRs and null strings saved within the
  // list
...
string StringUtility::listToString(const list<int> &X, bool separateStrings) {
...
std::list<std::string> StringUtility::tokenize(std::string X, char delim) {
...
string StringUtility::numberToString(long long x) {
  // DQ (8/31/2009): This now compiles properly (at least for analysis, it might
  // still fail for the code generation).
  // #ifndef USE_ROSE
...
string StringUtility::numberToString(unsigned long long x) {
  // DQ (8/31/2009): This now compiles properly (at least for analysis, it might
  // still fail for the code generation).
  // #ifndef USE_ROSE
  return boost::lexical_cast<std::string>(x);
...
string StringUtility::numberToString(long x) {
  // DQ (8/31/2009): This now compiles properly (at least for analysis, it might
  // still fail for the code generation).
  // #ifndef USE_ROSE
  return boost::lexical_cast<std::string>(x);
...
string StringUtility::numberToString(unsigned long x) {
  // DQ (8/31/2009): This now compiles properly (at least for analysis, it might
  // still fail for the code generation).
  // #ifndef USE_ROSE
  return boost::lexical_cast<std::string>(x);
...
string StringUtility::numberToString(int x) {
  // DQ (8/31/2009): This now compiles properly (at least for analysis, it might
  // still fail for the code generation).
  // #ifndef USE_ROSE
  return boost::lexical_cast<std::string>(x);
...
string StringUtility::numberToString(unsigned int x) {
  // DQ (8/31/2009): This now compiles properly (at least for analysis, it might
  // still fail for the code generation).
  // #ifndef USE_ROSE
  return boost::lexical_cast<std::string>(x);
...
string StringUtility::numberToString(const void *x) {
  // Build a string representing the dimensionOfArrayStatement
  char numberString[128];
...
string StringUtility::numberToString(double x) {
  // Build a string representing the dimensionOfArrayStatement
  char numberString[128];
...
string StringUtility::numberToString(__int128 x) {
  // DQ (2/22/2014): I don't think that the boost::lexical_cast can support
  // __int128 yet.
  long long temp_x = (long long)x;
  return boost::lexical_cast<std::string>(temp_x);
}

string StringUtility::numberToString(unsigned __int128 x) {
  // DQ (2/22/2014): I don't think that the boost::lexical_cast can support
  // __int128 yet.
...
std::string StringUtility::cEscape(const std::string &s) {
  std::string result;
...
unsigned StringUtility::hexadecimalToInt(char ch) {
  if (isxdigit(ch)) {
...
// string StringUtility::addrToString(uint64_t value, size_t nbits, bool
// is_signed)
std::string StringUtility::toHex2(uint64_t value, size_t nbits,
                                  bool show_unsigned_decimal,
...
std::string StringUtility::signedToHex2(uint64_t value, size_t nbits) {
  return toHex2(value, nbits, false, true);
}

std::string StringUtility::unsignedToHex2(uint64_t value, size_t nbits) {
  return toHex2(value, nbits, true, false);
}

string StringUtility::addrToString(uint64_t value, size_t nbits) {
  if (0 == nbits) nbits = value > 0xffffffff ? 64 : 32;
  return toHex2(value, nbits, false, false);
}

string StringUtility::removeRedundantSubstrings(const std::string &s) {
...
// Rama: 12/06/06:
// Donot know where this function is used, but we should be using the stdlib
// function isdigit
bool isNumber(char c) { return (isdigit(c)); }

bool isxNumber(char c) { return (isxdigit(c)); }
...
string StringUtility::removePseudoRedundantSubstrings(const std::string &s) {
  // Convert the string into a list of strings and separate out the redundant
  // entries
  list<string> XStringList = StringUtility::stringToList(s);
  ...
  for (i = XStringList.begin(); i != XStringList.end(); i++) {
    ...
    for (list<string>::iterator j = XStringList.begin(); j != XStringList.end();
         j++) {
      // compare *i and *j and check for pseudo-redundence
      // printf ("top of loop through string: compare *i and *j and check for
      // pseudo-redundence \n");

      // build information about *i
      string::const_iterator i_diffpos =
          find_if((*i).begin(), (*i).end(), isNumber);

      // build information about *j
      string::const_iterator j_diffpos =
          find_if((*j).begin(), (*j).end(), isNumber);
#if 0
      ...
      if (((*i).size() == (*j).size()) &&
        ...
        i_modifiedString = *i;
        i_modifiedString[i_subStringLength] = '$';
        string j_modifiedString = *j;
        j_modifiedString[j_subStringLength] = '$';
#if 0
    ...
    if (!listOfDifferences.empty()) {
      ...
      // char* diffpos = find_if ( modifiedString.c_str(),
      // modifiedString.c_str()+modifiedString.length(), isMarker );
      string::iterator diffpos =
          find_if(i_modifiedString.begin(), i_modifiedString.end(), isMarker);
...
// modifiedString = copyEdit(modifiedString,string("$Y"),maxvalue);
  ...
  for (i = listOfStringsToRemove.begin(); i != listOfStringsToRemove.end();
       i++) {
  ...
  XStringList.insert(XStringList.end(), modifiedStringList.begin(),
                     modifiedStringList.end());
#if 0
...
string StringUtility::copyEdit(const string &inputString,
                               const string &oldToken, const string &newToken) {
  // std::cerr << "StringUtility::copyEdit '" << inputString << "' '" <<
  // oldToken << "' '" << newToken << "'" << std::endl;
  string returnString;
...
void StringUtility::writeFile(const string &outputString,
...
// char* filenamePrefix = "";
  ...
  // <filename>.C.roseShow)
  // Macro OUTPUT_SHOWFILE_TO_FILE is defined in the transformation_1.h header
  // file
  // ostream & outputStream = (OUTPUT_TO_FILE ? ((ostream&) outputFile) :
  // ((ostream&) cout));
  ROSE_ASSERT(outputFile.good() == true);
...
StringUtility::FileWithLineNumbers StringUtility::copyEdit(
...
string StringUtility::readFile(const string &fileName) {
...
StringUtility::FileWithLineNumbers StringUtility::readFileWithPos(
  ...
  string fullFileName =
      StringUtility::getAbsolutePathFromRelativePath(fileName);
...
std::string StringUtility::StringWithLineNumber::toString() const {
  std::ostringstream os;
...
std::string StringUtility::toString(
    const StringUtility::FileWithLineNumbers &strings,
  ...
  for (unsigned int i = 0; i < strings.size(); ++i) {
    ...
    if (inPhysicalFile != newInPhysicalFile) needLineDirective = true;
    if (!inPhysicalFile &&
    ...
    if (needLineDirective) {
      ...
      if (strings[i].filename == "") {  // Reset to actual input file
        result +=
            ...
            numberToString(
                physicalLine +
                1 /* Increment because number is the line number of the NEXT line after the #line directive */) +
            " \"" + filename + "\" */\n";
      } else {
...
string StringUtility::indentMultilineString(const string &inputString,
...
void StringUtility::splitStringIntoStrings(const string &inputString,
                                           char separator,
...
// other IR nodes).
// JJW (10/15/2007): Does this compute a ones-complement checksum like used for
// TCP?
unsigned long StringUtility::generate_checksum(string s) {
  string uniqueName = s;
...
string StringUtility::stripPathFromFileName(const string &fileNameWithPath) {
...
#if ROSE_MICROSOFT_OS
  //       printf ("Error: basename() not available in MSVS (work around not
  //       implemented) \n");
  //       ROSE_ASSERT(false);
...
string StringUtility::stripFileSuffixFromFileName(
    const string &fileNameWithSuffix) {
...
// Extra code not used but which was a part of
// StringUtility::stripFileSuffixFromFileName()
#if 0
     const char *startOfString = &(fileNameWithPath[0]);
...
string StringUtility::fileNameSuffix(const string &fileNameWithSuffix) {
  // Make it safe to input a filename without a suffix (return the filename)
  string::size_type positionOfDot = fileNameWithSuffix.rfind('.');
...
// from Tom, thanks Tom!
string StringUtility::getPathFromFileName(const string &fileNameWithPath) {
  char c_version[PATH_MAX];
  ROSE_ASSERT(fileNameWithPath.size() + 1 < PATH_MAX);
  strcpy(c_version, fileNameWithPath.c_str());
  ...
  _splitpath(c_version, drive, dir, fname, ext);
  //       printf ("Error: dirname() not supported in MSVS 9work around not
  //       implemented) \n");
  //       printf ("dirname = %s \n",dir);
  // tps (08/17/2010) - Made this work under Windows.
...
string StringUtility::escapeNewLineCharaters(const string &X) {
  string returnString;
...
string StringUtility::convertToLowerCase(const string &inputString) {
...
std::string StringUtility::prefixLines(const std::string &lines,
  ...
  while (at < lines.size()) {
    ...
    if (lfpos < lines.size()) retval += prefix;
    at = lfpos;
  }

  if (prefixAtBack && isLineTerminated(lines)) retval += prefix;
  return retval;
}

bool StringUtility::isLineTerminated(const std::string &s) {
  return !s.empty() && ('\n' == s[s.size() - 1] || '\r' == s[s.size() - 1]);
}

std::string StringUtility::fixLineTermination(const std::string &input) {
  std::string output;
  size_t nchars = input.size();
  for (size_t i = 0; i < nchars; ++i) {
    if ('\r' == input[i] && i + 1 < nchars && '\n' == input[i + 1]) {
      // CR+LF: Microsoft Windows, DEC TOPS-10, RT-11 and most other early
      // non-Unix and non-IBM OSes, CP/M, MP/M, DOS
      ...
      // CR (only): Commodore 8-bit machines, Acorn BBC, TRS-80, Apple II
...
std::string StringUtility::makeOneLine(const std::string &s,
  ...
  if (!eat_spaces) result += spaces;
  return result;
}

void StringUtility::add_to_reason_string(std::string &result, bool isset,
                                         bool do_pad, const std::string &abbr,
  ...
  if (isset) {
    ...
    for (size_t i = 0; i < abbr.size(); ++i) result += ".";
  }
}
...
std::string StringUtility::encode_base64(const uint8_t *data, size_t nbytes,
                                         bool do_pad) {
...
std::vector<uint8_t> StringUtility::decode_base64(const std::string &s) {
...
std::string StringUtility::join(const std::string &separator, char *strings[],
                                size_t nstrings) {
...
std::vector<std::string> StringUtility::split(char separator,
...
std::vector<std::string> StringUtility::split(const std::string &separator,
                                              const std::string &str,
                                              size_t maxparts,
                                              bool trim_white_space) {
  std::vector<std::string> retval;
  if (0 == maxparts || str.empty()) return retval;
  if (separator.empty()) {
  ...
  if (trim_white_space) {
    for (size_t i = 0; i < retval.size(); ++i) retval[i] = trim(retval[i]);
  }
  return retval;
}

std::string StringUtility::trim(const std::string &str,
                                const std::string &strip, bool at_beginning,
                                bool at_end) {
  if (str.empty()) return str;
  size_t first = 0, last = str.size() - 1;
  if (at_beginning) first = str.find_first_not_of(strip);
  if (at_end && first != std::string::npos) last = str.find_last_not_of(strip);
  if (first == std::string::npos || last == std::string::npos) return "";
  assert(last >= first);
  return str.substr(first, last + 1 - first);
}

std::string StringUtility::untab(const std::string &str, size_t tabstops,
                                 size_t colnum) {
...
std::string StringUtility::appendAsmComment(const std::string &s,
