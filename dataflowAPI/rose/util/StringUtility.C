...
// DQ (8/31/2009): This now compiles properly (at least for analysis, it might still fail
// for the code generation). #ifndef USE_ROSE
...
#if ROSE_MICROSOFT_OS
    printf("Error: MSVS implementation of StringUtility::findfile required (not "
           "implemented) \n");
...
//
// Rama: 12/06/06
// We need a function getAbsolutePathFromRelativePath that takes any filename and returns
// the absolute file name  (with path) AS suggested we use realpath that comes with
// stdlib. However, for a translator like ours, we need to have two versions of the file:
// silent and non-silent ones, depending on whether we need to check and print an error or
// not. That is done by the boolean parameter printErrorIfAny that is set to true by the
// caller The dafault version -- enforced by setting printErrorIfAny to false -- is the
// silent one.
//
    ...
    bool               printErrorIfAny)  //! get the absolute path from the relative path
{
    ...
    // can return an error if it processes a file or directory that does not exist.  This
    // is a problem for include paths that are specified on the commandline and which
    // don't exist; most compilers silently ignore these and we have to at least ignore
    // them.
    //      string resultingPath="";
    // tps (01/08/2010) : This implementation was incorrect as it mixed char* and string.
    // Fixed it.
    ...
    {
        // DQ (9/4/2006): SgProject is not available within this code since it is used to
        // compile ROSETTA before the IR nodes are defined!  So we should just comment it
        // out. DQ (9/4/2006): Only output such warnings if verbose is set to some value
        // greater than zero.
        if(printErrorIfAny == true)
        {
            ...
            printf(
                "Error: StringUtility::getAbsolutePathFromRelativePath incured an error "
                "in use of realpath() and is returning the input relativePath. \n");
            //}
        }
...
StringUtility::listToString(const list<string>& X, bool separateStrings)
{
    ...
    for(listStringElementIterator = X.begin(); listStringElementIterator != X.end();
...
StringUtility::stringToList(const string& X)
{
    ...
    // characters ("\n"'s) All CR are removed, the list returned has no CR in the list OR
    // within the list elements
...
// DQ (8/31/2009): This now compiles properly (at least for analysis, it might still fail
// for the code generation). #ifdef USE_ROSE
#if 0
...
{
    // DQ (8/31/2009): This now compiles properly (at least for analysis, it might still
    // fail for the code generation). #ifndef USE_ROSE
...
{
    // DQ (2/22/2014): I don't think that the boost::lexical_cast can support __int128
    // yet.
...
std::string
StringUtility::toHex2(uint64_t value, size_t nbits, bool show_unsigned_decimal,
                      bool show_signed_decimal, uint64_t decimal_threshold)
{
    ...
    if(show_signed_decimal)
    {
        ...
        {
            // This is a negative value, so show it as negative.  We have to manually sign
            // extend it first.
...
// Rama: 12/06/06:
// Donot know where this function is used, but we should be using the stdlib function
// isdigit
bool
isNumber(char c)
{
...
StringUtility::removePseudoRedundantSubstrings(const std::string& s)
{
    ...
    for(i = XStringList.begin(); i != XStringList.end(); i++)
    {
        ...
        for(list<string>::iterator j = XStringList.begin(); j != XStringList.end(); j++)
        {
            ...
            if(((*i).size() == (*j).size()) && (i_subStringLength == j_subStringLength) &&
               (i_diffpos != (*i).end()))
            {
...
string
StringUtility::copyEdit(const string& inputString, const string& oldToken,
                        const string& newToken)
{
    // std::cerr << "StringUtility::copyEdit '" << inputString << "' '" << oldToken << "'
    // '" << newToken << "'" << std::endl;
    string                 returnString;
...
void
StringUtility::writeFile(const string& outputString, const string& fileNameString,
...
StringUtility::copyEdit(const StringUtility::FileWithLineNumbers& inputString,
                        ...
                        const StringUtility::FileWithLineNumbers& newToken)
{
    ...
    for(unsigned int i = 0; i < result.size(); ++i)
    {
        ...
        if(pos != string::npos)
        {
            ...
            i += newToken.size();  // Rescan the after-substitution part of the old line,
                                   // but not any of the new text
        }
    }
    return result;
}
...
StringUtility::readFile(const string& fileName)
{
    ...
    // sizeof(char) !!) allocate what is required. the code below is a slightly modified
    // version of what I found at: http://www.cplusplus.com/ref/iostream/istream/read.html
...
StringUtility::readFileWithPos(const string& fileName)
{
    ...
    for(std::string::size_type pos = 0; pos != string::npos;)
    {
        ...
        // cerr << "Added line '" << returnString.substr(lastPos, pos - lastPos) << "' at
        // line " << line << endl;
        ++line;
        ++pos;  // Skip newline
...
StringUtility::StringWithLineNumber::toString() const
{
    ...
    // DQ (1/21/2010): Added support for skipping these when in makes it easer to debug
    // ROSETTA generated files.
                        ...
                        int physicalLine /* Line number in output file */)
{
    ...
    for(unsigned int i = 0; i < strings.size(); ++i)
    {
        ...
        if(strings[i].str == "" && i + 1 == strings.size())
        {  // Special case
...
void
StringUtility::splitStringIntoStrings(const string& inputString, char separator,
...
// DQ (2/18/2006): Added general name mangling for all declarations (and some other IR
// nodes). JJW (10/15/2007): Does this compute a ones-complement checksum like used for
// TCP?
unsigned long
...
// DQ (3/15/2005): New, simpler and better implementation suggested function from Tom,
// thanks Tom!
string
...
//       ROSE_ASSERT(false);
    ...
    // I am not sure why this function was written and so, preserve the functionality
    // using empty return string in such cases.
...
std::string
StringUtility::intToHex(uint64_t i)
{
    ostringstream os;
    os << "0x" << std::hex << i;
    return os.str();
...
std::string
StringUtility::prefixLines(const std::string& lines, const std::string& prefix,
...
StringUtility::fixLineTermination(const std::string& input)
{
    ...
    for(size_t i = 0; i < nchars; ++i)
    {
        if('\r' == input[i] && i + 1 < nchars && '\n' == input[i + 1])
        {
            ...
            // and non-IBM OSes, CP/M, MP/M, DOS (MS-DOS, PC-DOS, etc.), Atari TOS, OS/2,
            // Symbian OS, Palm OS.
            output += '\n';
            ++i;
        }
        ...
        {
            // CR (only): Commodore 8-bit machines, Acorn BBC, TRS-80, Apple II family,
...
std::vector<std::string>
StringUtility::split(char separator, const std::string& str, size_t maxparts,
...
std::vector<std::string>
StringUtility::split(const std::string& separator, const std::string& str,
...
std::string
StringUtility::trim(const std::string& str, const std::string& strip, bool at_beginning,
                    bool at_end)
{
    if(str.empty())
        return str;
