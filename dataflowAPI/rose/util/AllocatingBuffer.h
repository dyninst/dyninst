 ...
 *
 *  Allocates as many elements as requested and manages the storage.  There is
 * no requirement that the elements be contiguous
...
class AllocatingBuffer : public Buffer<A, T> {
 ...
 public:
   ...
   *
   *  Allocates a new buffer of the specified size.  The values in the buffer
   * are default-constructed, and deleted when this
   *  buffer is deleted. */
  ...
  void resize(Address newSize) /*override*/ { values_.resize(newSize); }

  Address read(Value *buf, Address address, Address n) const /*override*/ {
    n = std::min(n, available(address));
    if (buf && n > 0) memcpy(buf, &values_[address], n * sizeof(values_[0]));
    return n;
  }

  Address write(const Value *buf, Address address, Address n) /*override*/ {
    n = std::min(n, available(address));
    if (buf && n > 0) memcpy(&values_[address], buf, n * sizeof(values_[0]));
    return n;
  }
