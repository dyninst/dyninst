 ...
 *
 *  This can be useful to reserve areas of of a BufferMap address space without
 * actually storing any data at them.  All
 *  reads return default values and writes using such a buffer will fail (return
 * zero). */
...
class NullBuffer : public Buffer<A, T> {
 ...
 public:
   ...
   *
   *  The new buffer will act as if it contains @p size values, although no
  ...
  const Value *data() const /*override*/ { return NULL; }
};
