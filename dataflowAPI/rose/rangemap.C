...
// const double
double Range<double>::relaxed_first() const {
  return r_first;
}
...
template <>
Range<double>::Pair Range<double>::split_range_at(const double &at) const {
  assert(!empty());
...
// I think this is correct since this is being returned by value.
// const float
float Range<float>::relaxed_first() const {
  return r_first;
}
...
template <>
Range<float>::Pair Range<float>::split_range_at(const float &at) const {
  assert(!empty());
  assert(at >= first() && at <= last());
