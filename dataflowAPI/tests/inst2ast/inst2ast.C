...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    cerr << "Usage: " << argv[0] << "inst2ast <test_program> <func_to_analyze>"
         << endl;
    exit(-1);
  }
  ...
  for (std::set<BPatch_basicBlock *>::iterator b_iter = blocks.begin();
    ...
    for (unsigned i = 0; i < insns.size(); ++i) {
      ...
      converter.convert(insns[i].first, insns[i].second, function[0], assigns);
      ...
      for (Result_t::const_iterator r_iter = res.begin(); r_iter != res.end();
           ++r_iter) {
        cout << "-----------------" << endl;
