...
bool LivenessAnalyzer::queryBlock(Location loc, Type type, vector<Address> & addrs, vector<bitArray> & liveRegs, bitArray & blockOutLiveRegs, vector<bitArray> &defRegs, vector<bitArray> &useRegs) 
{
//TODO: consider the trustness of the location 
   ...
   while(current != blockAddrs.rend() && *current > addr)
   {
      ...
      useRegs.push_back(rwAtCurrent.read);
      defRegs.push_back(rwAtCurrent.written);
...
bool LivenessAnalyzer::queryBlock(ParseAPI::Location loc, Type type, vector<Address> & addrs,vector<bitArray> & liveRegs , bitArray & blockOutLiveRegs){
  vector<bitArray> defRegs;
  vector<bitArray> useRegs;
  return queryBlock(loc,type,addrs,liveRegs,blockOutLiveRegs,defRegs,useRegs);
}
