...
std::string regs2 =
    " rrrrrrrrrrrrrrrrrrrrrrrm1111110000000000ssoscgfedrnoditszapci11111100dsbsbdca";
std::string regs3 =
    " 7654321076543210765432105432109876543210bbrssssssftfffffffffp54321098iippxxxx";
...
int
LivenessAnalyzer::getIndex(MachRegister machReg)
{
    return abi->getIndex(machReg);
}

const bitArray&
LivenessAnalyzer::getLivenessIn(Block* block)
{
    // Calculate if it hasn't been done already
    liveness_cerr << endl << "LivenessAnalyzer::getLivenessIn()" << endl;
    liveness_cerr << "Getting liveness for block " << hex << block->start() << dec
                  << endl;
...
void
LivenessAnalyzer::processEdgeLiveness(Edge* e, livenessData& data, Block* block,
                                      const bitArray& allRegsDefined)
{
    ...
    liveness_cerr << "Accumulating from block " << hex << (e->trg())->start() << dec
                  << endl;
    liveness_cerr << data.out << endl;
}

const bitArray&
LivenessAnalyzer::getLivenessOut(Block* block, bitArray& allRegsDefined)
{
    assert(blockLiveInfo.find(block) != blockLiveInfo.end());
    livenessData& data = blockLiveInfo[block];
    ...
    liveness_cerr << "getLivenessOut for block [" << hex << block->start() << ","
                  << block->end() << "]" << dec << endl;

    std::for_each(
        boost::make_filter_iterator(epred, target_edges.begin(), target_edges.end()),
        boost::make_filter_iterator(epred, target_edges.end(), target_edges.end()),
        boost::bind(&LivenessAnalyzer::processEdgeLiveness, this, boost::placeholders::_1,
                    boost::ref(data), block, boost::ref(allRegsDefined)));
...
void
LivenessAnalyzer::summarizeBlockLivenessInfo(Function* func, Block* block,
                                             bitArray& allRegsDefined)
{
    ...
    InstructionDecoder decoder(reinterpret_cast<const unsigned char*>(
                                   getPtrToInstruction(block, block->start())),
                               block->size(), block->obj()->cs()->getArch());
    ...
    while(curInsn.isValid())
    {
        ...
        liveness_printf("%s[%d] After instruction %s at address 0x%lx:\n", FILE__,
                        __LINE__, curInsn.format().c_str(), current);
...
bool
LivenessAnalyzer::updateBlockLivenessInfo(Block* block, bitArray& allRegsDefined)
{
    bool          change = false;
    livenessData& data   = blockLiveInfo[block];
    ...
    liveness_cerr << "Updating block info for block " << hex << block->start() << dec
                  << endl;
    liveness_cerr << "     " << regs1 << endl;
    liveness_cerr << "     " << regs2 << endl;
    liveness_cerr << "     " << regs3 << endl;
...
void
LivenessAnalyzer::analyze(Function* func)
{
    if(liveFuncCalculated.find(func) != liveFuncCalculated.end())
        return;
    liveness_printf(
        "Caculate basic block level liveness information for function %s (%lx)\n",
...
bool
LivenessAnalyzer::query(Location loc, Type type, bitArray& bitarray)
{
    ...
    const unsigned char* insnBuffer = reinterpret_cast<const unsigned char*>(
        getPtrToInstruction(loc.block, blockBegin));
    assert(insnBuffer);
    ...
    liveness_printf("%s[%d] instPoint calcLiveness: %d, 0x%lx, 0x%lx\n", FILE__, __LINE__,
                    current != blockAddrs.rend(), *current, addr);
...
bool
LivenessAnalyzer::query(Location loc, Type type, const MachRegister& machReg, bool& live)
{
    bitArray liveRegs;
...
ReadWriteInfo
LivenessAnalyzer::calcRWSets(Instruction curInsn, Block* blk, Address a)
{
    liveness_cerr << "calcRWSets for " << curInsn.format() << " @ " << hex << a << dec
                  << endl;
    ReadWriteInfo ret;
    ...
    {
        case c_CallInsn:
            // Call instructions not at the end of a block are thunks, which are not
            // ABI-compliant. So make conservative assumptions about what they may read
            // (ABI) but don't assume they write anything.
        ...
        case c_BranchInsn:
            if(!curInsn.allowsFallThrough() && isExitBlock(blk))
            {
                ...
                ret.read |=
                    ((abi->getCallReadRegisters()) | (abi->getReturnReadRegisters()));
                ret.written |= (abi->getCallWrittenRegisters());
            }
            break;
        default: {
...
void*
LivenessAnalyzer::getPtrToInstruction(Block* block, Address addr) const
{
    if(addr < block->start())
        return NULL;
    if(addr > block->end())
...
bool
LivenessAnalyzer::isExitBlock(Block* block)
{
    boost::lock_guard<Block> g(*block);
    ...
    if(interprocEdge && !intraprocEdge)
        return true;
    else
        return false;
}
...
void
LivenessAnalyzer::clean(Function* func)
{
    if(liveFuncCalculated.find(func) != liveFuncCalculated.end())
    {
    ...
    if(cachedLivenessInfo.getCurFunc() == func)
        cachedLivenessInfo.clean();
}
...
MachRegister
LivenessAnalyzer::changeIfMMX(MachRegister machReg)
{
    if(!isMMX(machReg))
        return machReg;
    if(width == 4)
