...
std::string regs1 =
    " ttttttttddddddddcccccccmxxxxxxxxxxxxxxxxgf                  "
    "rrrrrrrrrrrrrrrrr";
std::string regs2 =
    " rrrrrrrrrrrrrrrrrrrrrrrm1111110000000000ssoscgfedrnoditszapci11111100dsbs"
    "bdca";
std::string regs3 =
    " 7654321076543210765432105432109876543210bbrssssssftfffffffffp54321098iipp"
    "xxxx";
...
void LivenessAnalyzer::processEdgeLiveness(Edge *e, livenessData &data,
                                           Block *block,
  ...
  liveness_cerr << "Accumulating from block " << hex << (e->trg())->start()
                << dec << endl;
...
const bitArray &LivenessAnalyzer::getLivenessOut(Block *block,
  ...
  std::for_each(
      boost::make_filter_iterator(epred, target_edges.begin(),
                                  target_edges.end()),
      boost::make_filter_iterator(epred, target_edges.end(),
                                  target_edges.end()),
      boost::bind(&LivenessAnalyzer::processEdgeLiveness, this, _1,
...
bool LivenessAnalyzer::updateBlockLivenessInfo(Block *block,
  ...
  bool change = false;
  livenessData &data = blockLiveInfo[block];
  liveness_cerr << "Updating block info for block " << hex << block->start()
                << dec << endl;
...
bool LivenessAnalyzer::query(Location loc, Type type, bitArray &bitarray) {
  ...
  liveness_printf("%s[%d] instPoint calcLiveness: %d, 0x%lx, 0x%lx\n", FILE__,
                  __LINE__, current != blockAddrs.rend(), *current, addr);
...
bool LivenessAnalyzer::query(Location loc, Type type,
                             const MachRegister &machReg, bool &live) {
  bitArray liveRegs;
...
ReadWriteInfo LivenessAnalyzer::calcRWSets(Instruction::Ptr curInsn, Block *blk,
                                           Address a) {
  liveness_cerr << "calcRWSets for " << curInsn->format() << " @ " << hex << a
                << dec << endl;
  ReadWriteInfo ret;
  ...
  switch (category) {
    case c_CallInsn:
      ...
      // ABI-compliant.
      // So make conservative assumptions about what they may read (ABI) but
      // don't assume they write anything.
