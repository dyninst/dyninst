...
{
    stackanalysis_exception(std::string what)
    : std::runtime_error(std::string("Stackanalysis failure: ") + what)
    {}
};
...
static void
getInsnInstances(Block* block, InsnVec& insns)
{
...
void
add_target(std::queue<Block*>& worklist, Edge* e)
{
    worklist.push(e->trg());
}

void
add_target_list_exclude(std::queue<Block*>& worklist, std::set<Block*>& excludeSet,
                        Edge* e)
{
    Block* b = e->trg();
...
void
add_target_exclude(std::stack<Block*>& workstack, std::set<Block*>& excludeSet, Edge* e)
{
    Block* b = e->trg();
    if(excludeSet.find(b) == excludeSet.end())
    {
...
void
StackAnalysis::summarizeBlocks(bool verbose)
{
    ...
    while(!workstack.empty())
    {
        ...
        std::for_each(boost::make_filter_iterator(epred, targs.begin(), targs.end()),
...
void
StackAnalysis::fixpoint(bool verbose)
{
    ...
    while(!worklist.empty())
    {
        ...
        if(input == blockInputs[block] && touchedSet.find(block) != touchedSet.end())
        {
            // No new work here
            if(verbose)
            {
...
{
void
getRetAndTailCallBlocks(Function* func, std::set<Block*>& retBlocks)
{
    retBlocks.clear();
    intra_nosink_nocatch epred;
    std::set<Block*>     doneSet;
...
bool
StackAnalysis::canGetFunctionSummary()
{
...
bool
StackAnalysis::getFunctionSummary(TransferSet& summary)
{
    try
    {
        genInsnEffects();
        ...
        for(auto iter = tempSummary.begin(); iter != tempSummary.end(); iter++)
        {
            ...
            if(!tf.isIdentity() &&
               (tf.target.type() != Absloc::Stack || tf.target.off() >= 0))
            {
...
StackAnalysis::summarize()
{
    ...
    for(auto bit = blockInputs.begin(); bit != blockInputs.end(); ++bit)
    {
        ...
        for(iter = (*insnEffects)[block].begin(); iter != (*insnEffects)[block].end();
            ++iter)
        {
               ...
               (*callEffects)[block].find(off) != (*callEffects)[block].end())
            {
                ...
                for(auto summaryIter = summary.begin(); summaryIter != summary.end();
                    summaryIter++)
                {
            ...
            aIter != (*intervals_)[block].end(); aIter++)
        {
            ...
            for(auto tIter = as.begin(); tIter != as.end(); tIter++)
            {
                ...
                for(auto dIter = dhSet.begin(); dIter != dhSet.end(); dIter++)
                {
                    ...
                    if(def.addr == 0 && defAddrs.find(def.block) != defAddrs.end() &&
                       defAddrs[def.block].find(def.origLoc) != defAddrs[def.block].end())
                    {
...
void
StackAnalysis::computeInsnEffects(ParseAPI::Block* block, Instruction insn,
...
void
StackAnalysis::findDefinedHeights(ParseAPI::Block* b, Address addr,
...
void
StackAnalysis::findDefHeightPairs(
    Block* b, Address addr, std::vector<std::pair<Absloc, DefHeightSet>>& defHeights)
{
    if(func == NULL)
        return;
...
StackAnalysis::DefHeightSet
StackAnalysis::findDefHeight(Block* b, Address addr, Absloc loc)
{
    DefHeightSet ret;
    ret.makeTopSet();
...
StackAnalysis::Height
StackAnalysis::find(Block* b, Address addr, Absloc loc)
{
...
StackAnalysis::Height
StackAnalysis::findSP(Block* b, Address addr)
{
    return find(b, addr, Absloc(sp()));
}

StackAnalysis::Height
StackAnalysis::findFP(Block* b, Address addr)
{
    return find(b, addr, Absloc(fp()));
}

std::ostream&
operator<<(std::ostream& os, const Dyninst::StackAnalysis::Height& h)
{
...
public:
    ...
    StateEvalVisitor(Address addr, Instruction insn, StackAnalysis::AbslocState* s)
    : defined(true)
    , state(s)
    {
        rip = addr + insn.size();
    ...
    virtual void visit(RegisterAST* rast)
    {
        ...
        else if(state != NULL)
        {
            ...
            if(regState == state->end() || regState->second.size() != 1 ||
            ...
            {
                results.push_back(
                    make_pair(regState->second.begin()->height.height(), true));
            }
        }
        else
        {
...
void
StackAnalysis::handleXor(Instruction insn, Block* block, const Offset off,
...
void
StackAnalysis::handleDiv(Instruction insn, TransferFuncs& xferFuncs)
{
    std::vector<Operand> operands;
    insn.getOperands(operands);
    ...
    MachRegister quotientReg =
        (boost::dynamic_pointer_cast<InstructionAPI::RegisterAST>(quotient))->getID();
    MachRegister remainderReg =
        (boost::dynamic_pointer_cast<InstructionAPI::RegisterAST>(remainder))->getID();
...
void
StackAnalysis::handleMul(Instruction insn, TransferFuncs& xferFuncs)
{
    ...
    MachRegister targetReg =
        (boost::dynamic_pointer_cast<InstructionAPI::RegisterAST>(target))->getID();
    ...
    else
    {
        ...
        if(dynamic_cast<Immediate*>(multiplier.get()))
        {
            ...
            else if(multiplierVal == 1)
            {
                ...
                {
                    // mul reg1, reg2, 1
                    MachRegister multiplicandReg =
                        boost::dynamic_pointer_cast<RegisterAST>(multiplicand)->getID();
                    Absloc targetLoc(targetReg);
                    Absloc multiplicandLoc(multiplicandReg);
                    xferFuncs.push_back(
                        TransferFunc::copyFunc(multiplicandLoc, targetLoc));
...
void
StackAnalysis::handlePushPop(Instruction insn, Block* block, const Offset off, int sign,
                             TransferFuncs& xferFuncs)
{
    long    delta = 0;
            ...
            if(!spHeight.isTop() && !spHeight.isBottom())
            {
                ...
                if(dynamic_cast<RegisterAST*>(readExpr.get()))
                {
                    ...
                    MachRegister readReg =
                        boost::dynamic_pointer_cast<RegisterAST>(readExpr)->getID();
    ...
    else if(insn.getOperation().getID() == e_pop && !insn.writesMemory())
    {
        ...
        MachRegister targReg =
            boost::dynamic_pointer_cast<RegisterAST>(targExpr)->getID();
...
void
StackAnalysis::handleReturn(Instruction insn, TransferFuncs& xferFuncs)
{
    long                 delta = 0;
    std::vector<Operand> operands;
    insn.getOperands(operands);
...
void
StackAnalysis::handleAddSub(Instruction insn, Block* block, const Offset off, int sign,
                            TransferFuncs& xferFuncs)
{
    ...
    if(insn.writesMemory())
    {
        ...
        else
        {
            ...
            xferFuncs.push_back(TransferFunc::deltaFunc(writtenLoc, sign * immVal));
        }
        return;
    }
...
void
StackAnalysis::handleLEA(Instruction insn, TransferFuncs& xferFuncs)
{
    ...
    STACKANALYSIS_ASSERT(writtenSet.size() == 1);
    STACKANALYSIS_ASSERT(readSet.size() == 0 || readSet.size() == 1 ||
                         readSet.size() == 2);
    MachRegister written = (*writtenSet.begin())->getID();
    ...
    else if(readSet.size() == 1)
    {
        ...
        MachRegister reg =
            (boost::dynamic_pointer_cast<InstructionAPI::RegisterAST>(regExpr))->getID();
    ...
    else if(readSet.size() == 2)
    {
        ...
        MachRegister base =
            (boost::dynamic_pointer_cast<InstructionAPI::RegisterAST>(baseExpr))->getID();
        MachRegister index =
            (boost::dynamic_pointer_cast<InstructionAPI::RegisterAST>(indexExpr))
                ->getID();
...
void
StackAnalysis::handleLeave(Block* block, const Offset off, TransferFuncs& xferFuncs)
{
...
void
StackAnalysis::handlePushPopFlags(int sign, TransferFuncs& xferFuncs)
{
    // Fixed-size push/pop
...
void
StackAnalysis::handlePushPopRegs(int sign, TransferFuncs& xferFuncs)
{
    // Fixed-size push/pop
    // 8 registers
...
void
StackAnalysis::handlePowerAddSub(Instruction insn, Block* block, const Offset off,
...
void
StackAnalysis::handlePowerStoreUpdate(Instruction insn, Block* block, const Offset off,
                                      TransferFuncs& xferFuncs)
{
...
void
StackAnalysis::handleMov(Instruction insn, Block* block, const Offset off,
                         TransferFuncs& xferFuncs)
{
    // Some cases:
    //   1. mov reg, reg
...
void
StackAnalysis::handleZeroExtend(Instruction insn, Block* block, const Offset off,
                                TransferFuncs& xferFuncs)
{
...
void
StackAnalysis::handleSignExtend(Instruction insn, Block* block, const Offset off,
                                TransferFuncs& xferFuncs)
{
...
void
StackAnalysis::handleSpecialSignExtend(Instruction insn, TransferFuncs& xferFuncs)
{
    // This instruction sign extends the read register into the written
    // register. Copying insn't really correct here...sign extension is going
    // to change the value...
    ...
    MachRegister readReg;
    if(writtenReg == x86_64::rax)
        readReg = x86_64::eax;
    else if(writtenReg == x86_64::eax)
        readReg = x86_64::ax;
    else if(writtenReg == x86_64::ax)
        readReg = x86_64::al;
    else if(writtenReg == x86::eax)
        readReg = x86::ax;
    else if(writtenReg == x86::ax)
        readReg = x86::al;
    else
        STACKANALYSIS_ASSERT(false);
...
void
StackAnalysis::handleSyscall(Instruction insn, Block* block, const Offset off,
                             TransferFuncs& xferFuncs)
{
...
void
StackAnalysis::handleDefault(Instruction insn, Block* block, const Offset off,
                             TransferFuncs& xferFuncs)
{
    ...
    for(auto wIter = writtenLocs.begin(); wIter != writtenLocs.end(); wIter++)
    {
        ...
        if(writtenLoc.type() == Absloc::Register && writtenLoc.reg().size() < 4)
        {
...
bool
StackAnalysis::isCall(Instruction insn)
{
    return insn.getCategory() == c_CallInsn;
}

bool
StackAnalysis::isJump(Instruction insn)
{
    return insn.getCategory() == c_BranchInsn;
}

bool
StackAnalysis::handleNormalCall(Instruction insn, Block* block, Offset off,
                                TransferFuncs& xferFuncs, TransferSet& funcSummary)
{
    ...
    if(dynamic_cast<Dereference*>(callAddrExpr.get()))
    {
        ...
        if(children.size() == 1 && dynamic_cast<Immediate*>(children[0].get()) &&
           ...
           functionSummaries.find(calledAddr) != functionSummaries.end())
        {
            ...
            for(auto fsIter = fs.begin(); fsIter != fs.end(); fsIter++)
            {
                ...
                if(tf.target.type() == Absloc::Stack)
                {
                    ...
                    if(spHeight.isBottom() || spHeight.isTop())
                        continue;
    ...
    for(auto eit = outs.begin(); eit != outs.end(); ++eit)
    {
        ...
        Function* target_func =
            target_bbl->obj()->findFuncByEntry(target_bbl->region(), target_bbl->start());
...
bool
StackAnalysis::handleJump(Instruction insn, Block* block, Offset off,
                          TransferFuncs& xferFuncs, TransferSet& funcSummary)
{
    Address                  calledAddr = 0;
    boost::lock_guard<Block> g(*block);
    ...
    {
        Edge* edge = *eit;
        if(!edge->interproc() || edge->type() != DIRECT)
            continue;
...
bool
StackAnalysis::handleThunkCall(Instruction insn, Block* block, const Offset off,
                               TransferFuncs& xferFuncs)
{
    ...
    if(!thunkAddInsn.isValid())
        return true;
    if(thunkAddInsn.getOperation().getID() != e_add)
        return true;
    std::set<RegisterAST::Ptr> writtenRegs;
    thunkAddInsn.getOperand(0).getWriteSet(writtenRegs);
    if(writtenRegs.size() != 1)
        return true;
...
void
StackAnalysis::createEntryInput(AbslocState& input)
{
...
void
StackAnalysis::createSummaryEntryInput(TransferSet& input)
{
    // Get a set of all input locations
    std::set<Absloc> inputLocs;
    for(auto beIter = blockEffects->begin(); beIter != blockEffects->end(); beIter++)
    {
    ...
    for(auto locIter = inputLocs.begin(); locIter != inputLocs.end(); locIter++)
    {
...
StackAnalysis::AbslocState
StackAnalysis::getSrcOutputLocs(Edge* e)
{
    Block* b = e->src();
...
StackAnalysis::TransferSet
StackAnalysis::getSummarySrcOutputLocs(Edge* e)
{
    Block* b = e->src();
...
void
StackAnalysis::meetInputs(Block* block, AbslocState& blockInput, AbslocState& input)
{
    input.clear();
    intra_nosink_nocatch epred2;
    ...
    std::for_each(boost::make_filter_iterator(epred2, inEdges.begin(), inEdges.end()),
                  ...
                  boost::bind(&StackAnalysis::meet, this,
                              boost::bind(&StackAnalysis::getSrcOutputLocs, this,
...
void
StackAnalysis::meetSummaryInputs(Block* block, TransferSet& blockInput,
                                 TransferSet& input)
{
    input.clear();
    intra_nosink_nocatch     epred2;
    ...
    std::for_each(boost::make_filter_iterator(epred2, inEdges.begin(), inEdges.end()),
                  ...
                  boost::bind(&StackAnalysis::meetSummary, this,
                              boost::bind(&StackAnalysis::getSummarySrcOutputLocs, this,
                                          boost::placeholders::_1),
                              boost::ref(input)));
...
StackAnalysis::DefHeightSet
StackAnalysis::meetDefHeights(const DefHeightSet& s1, const DefHeightSet& s2)
{
    DefHeightSet newSet;
    if(s1.size() == 1 && s1.begin()->height.isTop())
        return s2;
    if(s2.size() == 1 && s2.begin()->height.isTop())
        return s1;
    ...
    newSet = s1;
    for(auto iter = s2.begin(); iter != s2.end() && newSet.size() <= DEF_LIMIT; iter++)
    {
        newSet.insert(*iter);
    }
...
void
StackAnalysis::meet(const AbslocState& input, AbslocState& accum)
{
...
void
StackAnalysis::meetSummary(const TransferSet& input, TransferSet& accum)
{
    for(auto iter = input.begin(); iter != input.end(); ++iter)
    {
        const Absloc&       loc       = iter->first;
...
StackAnalysis::Height&
StackAnalysis::Height::operator+=(const Height& other)
{
    if(isBottom())
        return *this;
    if(other.isBottom())
...
StackAnalysis::Height&
StackAnalysis::Height::operator+=(const signed long& rhs)
{
    if(isBottom())
        return *this;
    if(isTop())
...
const StackAnalysis::Height
StackAnalysis::Height::operator+(const Height& rhs) const
{
    if(isBottom())
        return bottom;
    if(rhs.isBottom())
...
const StackAnalysis::Height
StackAnalysis::Height::operator+(const signed long& rhs) const
{
    if(isBottom())
        return bottom;
    if(isTop())
...
StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::identityFunc(Absloc r)
{
    return copyFunc(r, r);
}

StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::deltaFunc(Absloc r, long d)
{
    return TransferFunc(uninitialized, d, r, r);
}

StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::absFunc(Absloc r, long a, bool i)
{
    return TransferFunc(a, 0, Absloc(), r, i);
}

StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::copyFunc(Absloc f, Absloc t, bool i)
{
    return TransferFunc(uninitialized, 0, f, t, i);
}

StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::bottomFunc(Absloc r)
{
    return TransferFunc(notUnique, notUnique, Absloc(), r, false, false, BOTTOM);
}

StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::retopFunc(Absloc r)
{
    return TransferFunc(uninitialized, 0, Absloc(), r, false, true, TOP);
}

StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::sibFunc(std::map<Absloc, std::pair<long, bool>> f, long d,
                                     Absloc t)
{
...
StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::meet(const TransferFunc& lhs, const TransferFunc& rhs)
{
    // Handle default-constructed TransferFuncs
    if(lhs.isTop() && !lhs.isRetop())
        return rhs;
    if(rhs.isTop() && !rhs.isRetop())
        return lhs;
    ...
    if(lhs.isAbs())
    {
        ...
        else if(rhs.isCopy() || rhs.isBottom() || rhs.isRetop() || rhs.isSIB())
        {
            ret = rhs;
        }
        else
    ...
    else if(lhs.isSIB())
    {
        ...
        else if(rhs.isSIB())
        {
            ...
            else
            {
                ...
                // FIXME if SIB function handling changes.
                ret = lhs;
                for(auto iter = rhs.fromRegs.begin(); iter != rhs.fromRegs.end(); iter++)
                {
                    const Absloc&                loc     = iter->first;
                    const std::pair<long, bool>& val     = iter->second;
                    auto                         findLoc = ret.fromRegs.find(loc);
                    if(findLoc == ret.fromRegs.end() ||
                       (val.first == 1 && ret.fromRegs[loc].first != 1))
                    {
...
bool
StackAnalysis::TransferFunc::isBaseRegCopy() const
{
...
bool
StackAnalysis::TransferFunc::isBaseRegSIB() const
{
    return isSIB() && fromRegs.size() == 2 && target.type() == Absloc::Register &&
           target.reg().size() == 4 && target.reg().getBaseRegister().size() == 8 &&
           fromRegs.find(target) != fromRegs.end() &&
           fromRegs.find(Absloc(target.reg().getBaseRegister())) != fromRegs.end();
}

bool
StackAnalysis::TransferFunc::isIdentity() const
{
    return isCopy() && from == target && delta == 0 && !topBottom;
}

bool
StackAnalysis::TransferFunc::isBottom() const
{
    return type_ == BOTTOM;
}

bool
StackAnalysis::TransferFunc::isTop() const
{
    return type_ == TOP;
}

bool
StackAnalysis::TransferFunc::isRetop() const
{
    return isTop() && retop;
}

bool
StackAnalysis::TransferFunc::isCopy() const
{
    return from.isValid();
}

bool
StackAnalysis::TransferFunc::isAbs() const
{
    return !isTop() && !isBottom() && !isCopy() && !isSIB();
}

bool
StackAnalysis::TransferFunc::isDelta() const
{
    return delta != 0;
}

bool
StackAnalysis::TransferFunc::isSIB() const
{
    return (fromRegs.size() > 0);
}

void
StackAnalysis::DefHeightSet::makeTopSet()
{
    defHeights.clear();
...
void
StackAnalysis::DefHeightSet::makeBottomSet()
{
    defHeights.clear();
    Definition d;
...
void
StackAnalysis::DefHeightSet::makeNewSet(Block* b, Address addr, const Absloc& origLoc,
                                        const Height& h)
{
    defHeights.clear();
...
void
StackAnalysis::DefHeightSet::addInitSet(const Height& h)
{
    defHeights.insert(DefHeight(Definition(), h));
}

void
StackAnalysis::DefHeightSet::addDeltaSet(long delta)
{
...
StackAnalysis::Height
StackAnalysis::DefHeightSet::getHeightSet() const
{
    Height h;
...
StackAnalysis::Definition
StackAnalysis::DefHeightSet::getDefSet() const
{
    Definition d;
    if(defHeights.size() == 0)
    {
...
bool
StackAnalysis::DefHeightSet::isTopSet() const
{
    if(defHeights.size() == 0)
        return true;
    return defHeights.size() == 1 && defHeights.begin()->def.type == Definition::TOP &&
           defHeights.begin()->height.isTop();
}

bool
StackAnalysis::DefHeightSet::isBottomSet() const
{
    return defHeights.size() == 1 && defHeights.begin()->def.type == Definition::BOTTOM &&
...
StackAnalysis::DefHeightSet
StackAnalysis::TransferFunc::apply(const AbslocState& inputs) const
{
...
StackAnalysis::TransferFunc
StackAnalysis::TransferFunc::summaryAccumulate(const TransferSet& inputs) const
{
    TransferFunc input;
    ...
    if(input.target.isValid())
        STACKANALYSIS_ASSERT(input.target == target);
    ...
    if(isSIB())
    {
        ...
        for(auto iter = fromRegs.begin(); iter != fromRegs.end(); ++iter)
        {
            ...
            if(findLoc == inputs.end())
            {
                ...
                {
                    newFromRegs.insert(
                        make_pair(fromLocOrig, make_pair(scaleOrig, false)));
                }
            }
            else
            {
                ...
                if(fromRegFunc.isSIB())
                {
                        ...
                        regIter != fromRegFunc.fromRegs.end(); ++regIter)
                    {
                        ...
                        if(found == newFromRegs.end())
                        {
                            newFromRegs.insert(make_pair(
                                replaceLoc, make_pair(newScale, replaceTopBottom)));
                        }
                    }
                }
        ...
        if(anyToppedTerms)
        {
            ...
            for(auto iter = newFromRegs.begin(); iter != newFromRegs.end(); iter++)
            {
...
void
StackAnalysis::TransferFunc::accumulate(TransferSet& inputs)
{
    inputs[target] = summaryAccumulate(inputs);
}

void
StackAnalysis::SummaryFunc::apply(Block* block, const AbslocState& in,
...
void
StackAnalysis::SummaryFunc::accumulate(const TransferSet& in, TransferSet& out) const
{
    // Copy all the elements we don't have xfer funcs for.
    out = in;
...
std::string
StackAnalysis::format(const AbslocState& input) const
{
    std::stringstream ret;
    for(auto iter = input.begin(); iter != input.end(); ++iter)
    {
        ...
        {
            ret << "(" << iter2->def.format() << ", " << iter2->height.format() << "), ";
        }
        ret << "}, ";
    }
    return ret.str();
...
std::string
StackAnalysis::format(const TransferSet& input) const
{
    std::stringstream ret;
    for(auto iter = input.begin(); iter != input.end(); ++iter)
    {
...
long
StackAnalysis::extractDelta(Result deltaRes)
{
    long delta;
...
bool
StackAnalysis::getSubReg(const MachRegister& reg, MachRegister& subreg)
{
    if(reg == x86_64::rax)
        subreg = x86_64::eax;
    else if(reg == x86_64::rbx)
        subreg = x86_64::ebx;
    else if(reg == x86_64::rcx)
        subreg = x86_64::ecx;
    else if(reg == x86_64::rdx)
        subreg = x86_64::edx;
    else if(reg == x86_64::rsp)
        subreg = x86_64::esp;
    else if(reg == x86_64::rbp)
        subreg = x86_64::ebp;
    else if(reg == x86_64::rsi)
        subreg = x86_64::esi;
    else if(reg == x86_64::rdi)
        subreg = x86_64::edi;
    else if(reg == x86_64::r8)
        subreg = x86_64::r8d;
    else if(reg == x86_64::r9)
        subreg = x86_64::r9d;
    else if(reg == x86_64::r10)
        subreg = x86_64::r10d;
    else if(reg == x86_64::r11)
        subreg = x86_64::r11d;
    else if(reg == x86_64::r12)
        subreg = x86_64::r12d;
    else if(reg == x86_64::r13)
        subreg = x86_64::r13d;
    else if(reg == x86_64::r14)
        subreg = x86_64::r14d;
    else if(reg == x86_64::r15)
        subreg = x86_64::r15d;
    else
        return false;
    return true;
...
void
StackAnalysis::retopBaseSubReg(const MachRegister& reg, TransferFuncs& xferFuncs)
{
...
// xferFuncs.
void
StackAnalysis::copyBaseSubReg(const MachRegister& reg, TransferFuncs& xferFuncs)
{
    if(reg.size() == 4 && reg.getBaseRegister().size() == 8)
    {
        const MachRegister& baseReg = reg.getBaseRegister();
        xferFuncs.push_back(TransferFunc::copyFunc(Absloc(reg), Absloc(baseReg), true));
    }
    else if(reg.size() == 8 && reg.getBaseRegister().size() == 8)
    {
        MachRegister subreg;
        ...
        {
            xferFuncs.push_back(
                TransferFunc::copyFunc(Absloc(reg), Absloc(subreg), false));
        }
    }
}
...
void
StackAnalysis::bottomBaseSubReg(const MachRegister& reg, TransferFuncs& xferFuncs)
{
    if(reg.size() == 4 && reg.getBaseRegister().size() == 8)
    {
        const MachRegister& baseReg = reg.getBaseRegister();
