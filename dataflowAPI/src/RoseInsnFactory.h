...
public:
    ...
    DATAFLOW_EXPORT virtual SgAsmInstruction* convert(
        const InstructionAPI::Instruction& insn, uint64_t addr);

protected:
    virtual SgAsmInstruction* createInsn() = 0;

    virtual void setOpcode(SgAsmInstruction* insn, entryID opcode, prefixEntryID prefix,
                           std::string mnem) = 0;
...
{
public:
    DATAFLOW_EXPORT RoseInsnX86Factory(Architecture arch)
    : a(arch){};
...
{
public:
    DATAFLOW_EXPORT RoseInsnArmv8Factory(Architecture arch)
    : a(arch){};
...
{
public:
    DATAFLOW_EXPORT RoseInsnAmdgpuVegaFactory(Architecture arch)
    : a(arch){};
