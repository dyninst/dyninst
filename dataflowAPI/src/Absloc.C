...
Absloc
Absloc::makePC(Architecture arch)
{
    return Absloc(MachRegister::getPC(arch));
}

Absloc
Absloc::makeSP(Architecture arch)
{
    return Absloc(MachRegister::getStackPointer(arch));
}

Absloc
Absloc::makeFP(Architecture arch)
{
...
bool
Absloc::operator<(const Absloc& rhs) const
{
    if(type_ != rhs.type_)
...
Absloc::operator==(const Absloc& rhs) const
{
    ...
    switch(type_)
    {
        ...
        case Heap:
            return addr_ == rhs.addr_;
        case PredicatedRegister:
            return (reg_ == rhs.reg_) && (preg_ == rhs.preg_) &&
                   (trueCond_ == rhs.trueCond_);
        default:
            return true;
    }
}
...
bool
AbsRegion::contains(const Absloc::Type t) const
{
...
bool
AbsRegion::contains(const Absloc& loc) const
{
...
bool
AbsRegion::contains(const AbsRegion& rhs) const
{
    if(type_ != Absloc::Unknown)
    {
    ...
    if(rhs.absloc_.type() == Absloc::PredicatedRegister &&
       rhs.absloc_.reg() == absloc_.reg())
        return true;

    // Stack slots operate kinda... odd...
    if((absloc_.type() == Absloc::Stack) && (rhs.absloc_.type() == Absloc::Stack))
    {
...
bool
AbsRegion::containsOfType(Absloc::Type t) const
{
    if(type_ == t)
        return true;
...
bool
AbsRegion::operator==(const AbsRegion& rhs) const
{
...
Assignment::Ptr
Assignment::makeAssignment(const InstructionAPI::Instruction& i, const Address a,
...
void
Assignment::addInput(const AbsRegion& reg)
{
    inputs_.push_back(reg);
}

void
Assignment::addInputs(const std::vector<AbsRegion>& region)
{
