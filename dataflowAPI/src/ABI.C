...
int
ABI::getIndex(MachRegister machReg)
{
...
const bitArray&
ABI::getCallReadRegisters() const
{
    if(addr_width == 4)
        return *callRead_;
...
const bitArray&
ABI::getCallWrittenRegisters() const
{
    if(addr_width == 4)
        return *callWritten_;
    else if(addr_width == 8)
...
const bitArray&
ABI::getReturnReadRegisters() const
{
    if(addr_width == 4)
        return *returnRead_;
    else if(addr_width == 8)
...
const bitArray&
ABI::getParameterRegisters() const
{
    if(addr_width == 4)
        return *callParam_;
    else if(addr_width == 8)
...
const bitArray&
ABI::getSyscallReadRegisters() const
{
    if(addr_width == 4)
        return *syscallRead_;
    else if(addr_width == 8)
...
const bitArray&
ABI::getSyscallWrittenRegisters() const
{
    if(addr_width == 4)
...
ABI::initialize64()
{
    ...
    (*returnRead64_)[machRegIndex_x86_64()[x86_64::rcx]] = true;  // Not correct,
                                                                  // temporary
