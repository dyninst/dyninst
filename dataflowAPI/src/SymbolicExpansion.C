...
bool SymbolicExpansion::expandPPC32(SgAsmInstruction *rose_insn,
                                    SymEvalPolicy &policy) {
  SgAsmPowerpcInstruction *insn =
      static_cast<SgAsmPowerpcInstruction *>(rose_insn);
...
bool SymbolicExpansion::expandAarch64(SgAsmInstruction *rose_insn,
  ...
  } catch (rose::BinaryAnalysis::InstructionSemantics2::BaseSemantics::Exception
               &e) {
    fprintf(stderr,
            "Instruction processing threw exception for instruction: %s",
            insn_dump.c_str());
  }
