        ...
        for( ; begin != end; ++begin) {
            ...
            if(skip_edges.find(edge) == skip_edges.end()) {
                SliceNode::Ptr parent =
                    dyn_static_pointer_cast<SliceNode>(
                        edge->source());
...
SymEval::Retval_t SymEval::expand(Graph::Ptr slice, Result_t &res) {
   ...
   bool skippedInput = false;
    ...
    for ( ; gbegin != gend; ++gbegin) {
        Node::Ptr ptr = *gbegin;
        SliceNode::Ptr cur = dyn_static_pointer_cast<SliceNode>(ptr);
    ...
    for ( ; vit != sortVector.end(); ++vit) {
        ...
        Node::Ptr cur = dyn_static_pointer_cast<Node>(ptr);
    ...
    for (; gbegin != gend; ++gbegin) {
        ...
        Node::Ptr ptr = *gbegin;
        SliceNode::Ptr sptr = 
            dyn_static_pointer_cast<SliceNode>(ptr);
...
SymEval::Retval_t SymEval::process(SliceNode::Ptr ptr,
   ...
   bool success = false;
    ...
    for (; begin != end; ++begin) {
       SliceEdge::Ptr edge = dyn_static_pointer_cast<SliceEdge>(*begin);
       SliceNode::Ptr source = dyn_static_pointer_cast<SliceNode>(edge->source());
