...
bool SymEval::expand(Result_t &res, std::set<InstructionPtr> &failedInsns,
                     bool applyVisitors) {
  ...
  if (applyVisitors) {
    ...
    for (Result_t::iterator i = res.begin(); i != res.end(); ++i) {
      ...
      AST::Ptr tmp = simplifyStack(i->second, i->first->addr(),
                                   i->first->func(), i->first->block());
      BooleanVisitor b;
...
void dfs(Node::Ptr source, std::map<Node::Ptr, int> &state,
  ...
  for (; eit != edges.end(); ++eit) {
    ...
    if (done && (*sit).second > 0) skipEdges.insert(edge);

    if (!done) dfs(cur, state, skipEdges);
  }
 ...
 public:
  ...
  void insert(SliceNode::Ptr n, bool force_done = false) {
    ...
    for (; begin != end; ++begin) {
      ...
      if (skip_edges.find(edge) == skip_edges.end()) {
        ...
        if (done.find(parent) == done.end()) ++pcnt;
      }
    }
  ...
  void mark_done(SliceNode::Ptr n) {
    ...
    for (; begin != end; ++begin) {
      ...
      if (skip_edges.find(edge) == skip_edges.end()) {
        SliceNode::Ptr child =
            boost::static_pointer_cast<SliceNode>(edge->target());
        if (remove(child)) children.insert(child);
      }
    }
  ...
  bool is_done(SliceNode::Ptr n) const { return done.find(n) == done.end(); }
...
SymEval::Retval_t SymEval::expand(Dyninst::Graph::Ptr slice,
  ...
  for (; gbegin != gend; ++gbegin) {
    Node::Ptr ptr = *gbegin;
    expand_cerr << "pushing " << (*gbegin)->format() << " to sortVector"
                << endl;
  ...
  while (1) {
    ...
    expand_cerr << "Visiting node " << aNode->assign()->format() << " order "
                << order << endl;
    ...
    if (post && !(post->equals(prev))) {
      ...
      for (; oB != oE; ++oB) {
        if (worklist.skipEdges().find(*oB) == worklist.skipEdges().end()) {
          SliceNode::Ptr out =
              boost::static_pointer_cast<SliceNode>((*oB)->target());
          worklist.insert(out);
        }
      }
    }
...
bool SymEval::expandInsn(const InstructionAPI::Instruction::Ptr insn,
                         const uint64_t addr, Result_t &res) {
  ...
  switch (insn->getArch()) {
    case Arch_x86: {
      ...
      if (policy.failedTranslate()) {
        cerr << "Warning: failed semantic translation of instruction "
             << insn->format() << endl;
        return false;
      }
    ...
    case Arch_aarch64: {
      ...
      BaseSemantics::SValuePtr protoval =
          SymEvalSemantics::SValue::instance(1, 0);
      BaseSemantics::RegisterStatePtr registerState =
          SymEvalSemantics::RegisterStateARM64::instance(protoval, reg_dict);
      BaseSemantics::MemoryStatePtr memoryState =
          SymEvalSemantics::MemoryStateARM64::instance(protoval, protoval);
      BaseSemantics::StatePtr state = SymEvalSemantics::StateARM64::instance(
          res, addr, insn->getArch(), insn, registerState, memoryState);
      BaseSemantics::RiscOperatorsPtr ops =
          SymEvalSemantics::RiscOperatorsARM64::instance(state);
...
SymEval::Retval_t SymEval::process(SliceNode::Ptr ptr, Result_t &dbase,
                                   std::set<Edge::Ptr> &skipEdges) {
  ...
  for (; begin != end; ++begin) {
    ...
    if (skipEdges.find(edge) != skipEdges.end()) {
      expand_cerr << "In process, skipping edge from " << source->format()
                  << endl;
      skippedEdge = true;
      continue;
    }
  ...
  for (std::map<const AbsRegion, std::set<Assignment::Ptr> >::iterator iter =
           inputMap.begin();
    ...
    expand_cerr << "Before substitution: "
    ...
    expand_cerr << "\t result is " << (ast ? ast->format() : "<NULL AST>")
                << endl;
  }
  ...
  expand_cerr << "Result of post-substitution simplification: "
              << ptr->assign()->format()
              << " == " << (ast ? ast->format() : "<NULL AST>") << endl;
...
AST::Ptr SymEval::simplifyStack(AST::Ptr ast, Address addr,
  ...
  StackAnalysis::Height fp = sA.find(
