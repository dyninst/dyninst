...
bool SymEval::expand(Result_t &res, std::set<InstructionAPI::Instruction> &failedInsns, bool applyVisitors) {
  ...
  for(Result_t::iterator i = res.begin(); i != res.end(); ++i) {
    ...
    bool success = expandInsn(ptr->insn(), ptr->addr(), res);
    if(!success) {
...
void dfs(Node::Ptr source, std::map<Node::Ptr, int> &state, std::set<Edge::Ptr> &skipEdges) {
  ...
  if(ssit == state.end()) {
    dyncompat::tuples::tie(ssit, dyncompat::tuples::ignore) = state.insert(make_pair(source, 1));
  } else {
    (*ssit).second++;
  }
...
public:
  ...
  void insert(SliceNode::Ptr n, bool force_done = false) {
    ...
    for(; begin != end; ++begin) {
      ...
      if(skip_edges.find(edge) == skip_edges.end()) {
        SliceNode::Ptr parent = dyncompat::static_pointer_cast<SliceNode>(edge->source());
  ...
  void mark_done(SliceNode::Ptr n) {
    ...
    for(; begin != end; ++begin) {
      Edge::Ptr edge = *begin;
      if(skip_edges.find(edge) == skip_edges.end()) {
        SliceNode::Ptr child = dyncompat::static_pointer_cast<SliceNode>(edge->target());
        if(remove(child)) {
...
SymEval::Retval_t SymEval::expand(Dyninst::Graph::Ptr slice, DataflowAPI::Result_t &res) {
  ...
  // First, we'll sort the nodes in some deterministic order so that the loop
  ...
  for(; gbegin != gend; ++gbegin) {
    ...
    Node::Ptr ptr = *gbegin;
    SliceNode::Ptr sptr = dyncompat::static_pointer_cast<SliceNode>(ptr);
  ...
  while(1) {
    ...
    dyncompat::tie(aNode, order) = worklist.pop_next();
    if(order == -1) { // empty
      break;
    }
    ...
    expand_cerr << "Visiting node " << aNode->assign()->format() << " order " << order << endl;
    ...
    if(post && !(post->equals(prev))) {
      ...
      for(; oB != oE; ++oB) {
        if(worklist.skipEdges().find(*oB) == worklist.skipEdges().end()) {
          SliceNode::Ptr out = dyncompat::static_pointer_cast<SliceNode>((*oB)->target());
          worklist.insert(out);
        }
      }
    }
...
static bool expandInsn(const Instruction &insn, const uint64_t addr, Result_t &res) {
  ...
  switch(insn.getArch()) {
    ...
    case Arch_ppc64: {
      ...
      BaseSemantics::SValuePtr protoval = SymEvalSemantics::SValue::instance(1, 0);
      BaseSemantics::RegisterStatePtr registerState =
          SymEvalSemantics::RegisterStateASTPPC64::instance(protoval, reg_dict);
      BaseSemantics::MemoryStatePtr memoryState = SymEvalSemantics::MemoryStateAST::instance(protoval, protoval);
      BaseSemantics::StatePtr state =
          SymEvalSemantics::StateAST::instance(res, addr, insn.getArch(), insn, registerState, memoryState);
      BaseSemantics::RiscOperatorsPtr ops = SymEvalSemantics::RiscOperatorsAST::instance(state);
    ...
    case Arch_aarch64: {
      ...
      BaseSemantics::SValuePtr protoval = SymEvalSemantics::SValue::instance(1, 0);
      BaseSemantics::RegisterStatePtr registerState =
          SymEvalSemantics::RegisterStateASTARM64::instance(protoval, reg_dict);
      BaseSemantics::MemoryStatePtr memoryState = SymEvalSemantics::MemoryStateAST::instance(protoval, protoval);
      BaseSemantics::StatePtr state =
          SymEvalSemantics::StateAST::instance(res, addr, insn.getArch(), insn, registerState, memoryState);
      BaseSemantics::RiscOperatorsPtr ops = SymEvalSemantics::RiscOperatorsAST::instance(state);
    ...
    case Arch_amdgpu_gfx908: {
      ...
      BaseSemantics::SValuePtr protoval = SymEvalSemantics::SValue::instance(1, 0);
      BaseSemantics::RegisterStatePtr registerState =
          SymEvalSemantics::RegisterStateAST_amdgpu_gfx908::instance(protoval, reg_dict);
      BaseSemantics::MemoryStatePtr memoryState = SymEvalSemantics::MemoryStateAST::instance(protoval, protoval);
      BaseSemantics::StatePtr state =
          SymEvalSemantics::StateAST::instance(res, addr, insn.getArch(), insn, registerState, memoryState);
      BaseSemantics::RiscOperatorsPtr ops = SymEvalSemantics::RiscOperatorsAST::instance(state);
    ...
    case Arch_amdgpu_gfx90a: {
      ...
      BaseSemantics::SValuePtr protoval = SymEvalSemantics::SValue::instance(1, 0);
      BaseSemantics::RegisterStatePtr registerState =
          SymEvalSemantics::RegisterStateAST_amdgpu_gfx90a::instance(protoval, reg_dict);
      BaseSemantics::MemoryStatePtr memoryState = SymEvalSemantics::MemoryStateAST::instance(protoval, protoval);
      BaseSemantics::StatePtr state =
          SymEvalSemantics::StateAST::instance(res, addr, insn.getArch(), insn, registerState, memoryState);
      BaseSemantics::RiscOperatorsPtr ops = SymEvalSemantics::RiscOperatorsAST::instance(state);
    ...
    case Arch_amdgpu_gfx940: {
      ...
      BaseSemantics::SValuePtr protoval = SymEvalSemantics::SValue::instance(1, 0);
      BaseSemantics::RegisterStatePtr registerState =
          SymEvalSemantics::RegisterStateAST_amdgpu_gfx940::instance(protoval, reg_dict);
      BaseSemantics::MemoryStatePtr memoryState = SymEvalSemantics::MemoryStateAST::instance(protoval, protoval);
      BaseSemantics::StatePtr state =
          SymEvalSemantics::StateAST::instance(res, addr, insn.getArch(), insn, registerState, memoryState);
      BaseSemantics::RiscOperatorsPtr ops = SymEvalSemantics::RiscOperatorsAST::instance(state);
...
SymEval::Retval_t process(SliceNode::Ptr ptr, Result_t &dbase, std::set<Edge::Ptr> &skipEdges) {
  ...
  for(; begin != end; ++begin) {
    ...
    if(skipEdges.find(edge) != skipEdges.end()) {
      expand_cerr << "In process, skipping edge from " << source->format() << endl;
      skippedEdge = true;
      continue;
    }
    ...
    expand_cerr << "Assigning input " << edge->data().format() << " from assignment " << assign->format() << endl;
  ...
  AST::Ptr ast;
  dyncompat::tie(ast, failedTranslation) = SymEval::expand(ptr->assign());
  // expand_cerr << "\t ... resulting in " << dbase.format() << endl;

  // We have an AST. Now substitute in all of its predecessors.
  for(std::map<const AbsRegion, std::set<Assignment::Ptr>>::iterator iter = inputMap.begin(); iter != inputMap.end();
      ++iter) {
    ...
    for(std::set<Assignment::Ptr>::iterator iter2 = iter->second.begin(); iter2 != iter->second.end(); ++iter2) {
  ...
  expand_cerr << "Result of substitution: " << ptr->assign()->format() << " == " << (ast ? ast->format() : "<NULL AST>")
              << endl;
  ...
  if(failedTranslation) {
    return SymEval::FAILED_TRANSLATION;
  } else if(skippedEdge || skippedInput) {
    return SymEval::SKIPPED_INPUT;
  } else if(success) {
    return SymEval::SUCCESS;
  } else {
    return SymEval::FAILED;
  }
}
