...
std::pair<AST::Ptr, bool>
SymEval::expand(const Assignment::Ptr& assignment, bool applyVisitors)
{
...
bool
SymEval::expand(Result_t& res, std::set<InstructionAPI::Instruction>& failedInsns,
                bool applyVisitors)
{
...
bool
edgeSort(Edge::Ptr ptr1, Edge::Ptr ptr2)
{
...
void
dfs(Node::Ptr source, std::map<Node::Ptr, int>& state, std::set<Edge::Ptr>& skipEdges)
{
...
bool
vectorSort(SliceNode::Ptr ptr1, SliceNode::Ptr ptr2)
{
...
SymEval::Retval_t
SymEval::expand(Dyninst::Graph::Ptr slice, DataflowAPI::Result_t& res)
{
    ...
    while(1)
    {
        ...
        expand_cerr << "Visiting node " << aNode->assign()->format() << " order " << order
                    << endl;
...
bool
SymEval::expandInsn(const Instruction& insn, const uint64_t addr, Result_t& res)
{
    switch(insn.getArch())
    {
        case Arch_x86: {
...
SymEval::Retval_t
SymEval::process(SliceNode::Ptr ptr, Result_t& dbase, std::set<Edge::Ptr>& skipEdges)
{
    ...
    for(; begin != end; ++begin)
    {
        ...
        expand_cerr << "Assigning input " << edge->data().format() << " from assignment "
                    << assign->format() << endl;
        ...
        iter != inputMap.end(); ++iter)
    {
        ...
        expand_cerr << "Before substitution: " << (ast ? ast->format() : "<NULL AST>")
                    << endl;
...
AST::Ptr
SymEval::simplifyStack(AST::Ptr ast, Address addr, ParseAPI::Function* func,
