...
SgAsmInstruction*
RoseInsnFactory::convert(const Instruction& insn, uint64_t addr)
{
    SgAsmInstruction* rinsn = createInsn();
    _addr                   = addr;
    rinsn->set_address(addr);
    rinsn->set_mnemonic(insn.format());
    setOpcode(rinsn, insn.getOperation().getID(), insn.getOperation().getPrefixID(),
              insn.getOperation().format());
    ...
    for(unsigned i = 0; i < insn.size(); ++i)
        rawBytes.push_back(insn.rawByte(i));
    rinsn->set_raw_bytes(rawBytes);

    // operand list
    SgAsmOperandList* roperands = new SgAsmOperandList;

    // std::cerr << "Converting " << insn.format(addr) << " @" << std::hex << addr <<
    // std::dec << std::endl;

    // std::cerr << "checking instruction: " << insn.format(addr) << " for special
    // handling" << std::endl;
    ...
    // std::cerr << "no special handling by opcode, checking if we should mangle
    // operands..." << std::endl;
    ...
    for(std::vector<InstructionAPI::Operand>::iterator opi = operands.begin();
        opi != operands.end(); ++opi, ++i)
    {
        InstructionAPI::Operand& currOperand = *opi;
        // std::cerr << "Converting operand " << currOperand.format(arch(), addr) <<
        // std::endl;
        SgAsmExpression* converted =
            convertOperand(currOperand.getValue(), addr, insn.size());
        if(converted == NULL)
            return NULL;
...
SgAsmExpression*
RoseInsnFactory::convertOperand(const Expression::Ptr expression, int64_t addr,
                                size_t insnSize)
{
    if(!expression)
        return NULL;
...
void
RoseInsnX86Factory::setOpcode(SgAsmInstruction* insn, entryID opcode,
                              prefixEntryID prefix, std::string)
{
...
bool
RoseInsnX86Factory::handleSpecialCases(entryID, SgAsmInstruction*, SgAsmOperandList*)
{
    // Does nothing?
...
void
RoseInsnX86Factory::massageOperands(const Instruction&                    insn,
                                    std::vector<InstructionAPI::Operand>& operands)
{
    switch(insn.getOperation().getID())
    {
        case e_lea: {
            ...
            Dereference::Ptr tmp =
                Dereference::Ptr(new Dereference(operands[1].getValue(), u32));
...
SgAsmInstruction*
RoseInsnPPCFactory::createInsn()
{
    return new SgAsmPowerpcInstruction;
}

void
RoseInsnPPCFactory::setOpcode(SgAsmInstruction* insn, entryID       opcode,
                              prefixEntryID /*prefix*/, std::string mnem)
{
...
void
RoseInsnPPCFactory::setSizes(SgAsmInstruction*)
{}

bool
RoseInsnPPCFactory::handleSpecialCases(entryID iapi_opcode, SgAsmInstruction* insn,
    ...
    switch(iapi_opcode)
    {
        ...
        case power_op_bclr: {
...
#ifdef os_windows
            // Visual Studio doensn't define htobe32, so we assume that Windows is always
            // little endian.
            ...
            else
            {
                ...
                // bi has a 5-bit value. The top 3 bit specifies which condition register
                // to use The bottom 2 bit specifies which bit within the given condition
                // register
                bi = ((raw >> 16) & 0x0000001F);
                rose_operands->append_operand(new SgAsmIntegerValueExpression(
                    bo, new SgAsmIntegerType(ByteOrder::ORDER_LSB, 8, false)));

                SgAsmDirectRegisterExpression* dre = new SgAsmDirectRegisterExpression(
                    RegisterDescriptor(powerpc_regclass_cr, 0, bi, 1));
            ...
            if(branch_target)
            {
                rose_operands->append_operand(
                    new SgAsmDoubleWordValueExpression(branch_target));
            }
            else if(power_op_bcctr == iapi_opcode)
            {
                rose_operands->append_operand(new SgAsmPowerpcRegisterReferenceExpression(
                    powerpc_regclass_spr, powerpc_spr_ctr));
            }
            else
            {
                assert(power_op_bclr == iapi_opcode);
                rose_operands->append_operand(new SgAsmPowerpcRegisterReferenceExpression(
                    powerpc_regclass_spr, powerpc_spr_lr));
            }
            return true;
        }
        break;
        ...
        case power_op_svcs: {
            ...
            rose_operands->append_operand(new SgAsmIntegerValueExpression(
                lev, new SgAsmIntegerType(ByteOrder::ORDER_LSB, 8, false)));
            // cerr << "LEV = " << lev << endl;
            return true;
        }
        default:
...
void
RoseInsnPPCFactory::massageOperands(const Instruction&                    insn,
                                    std::vector<InstructionAPI::Operand>& operands)
{
    /*
    ...
    */
    ...
    // Any store, however, Dyninst expects in RS, RA, RB/displacement form.  Very
    // confusing, but we handle it cleanly here.
    if(opcode != power_op_rldicr && opcode != power_op_rldic &&
       !operands[0].isWritten() && operands.size() >= 2 && operands[1].isWritten() &&
       !operands[1].writesMemory())
    {
        // std::cerr << "swapping RS and RA in " << insn.format() << std::endl;
        std::swap(operands[0], operands[1]);
    }
    if(opcode == power_op_cmp || opcode == power_op_cmpl || opcode == power_op_cmpi ||
       opcode == power_op_cmpli)
    {
        operands.push_back(
            Operand(Immediate::makeImmediate(Result(u8, 1)), false, false));
...
void
RoseInsnArmv8Factory::setSizes(SgAsmInstruction* /*insn*/)
{}

SgAsmInstruction*
RoseInsnArmv8Factory::createInsn()
{
    return new SgAsmArmv8Instruction;
}

void
RoseInsnArmv8Factory::setOpcode(SgAsmInstruction* insn, entryID opcode, prefixEntryID,
                                std::string)
{
...
bool
RoseInsnArmv8Factory::handleSpecialCases(entryID, SgAsmInstruction*, SgAsmOperandList*)
{
    return false;
}
...
SgAsmInstruction*
RoseInsnAmdgpuVegaFactory::createInsn()
{
    return new SgAsmAmdgpuVegaInstruction;
}

void
RoseInsnAmdgpuVegaFactory::setOpcode(SgAsmInstruction* insn, entryID opcode,
                                     prefixEntryID, std::string)
{
