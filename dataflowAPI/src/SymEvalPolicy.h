...
struct Handle
{
    ...
    Handle(const Handle& rhs) { v_ = new AST::Ptr(rhs.var()); }
...
{
public:
    SymEvalPolicy(Result_t& r, Address addr, Dyninst::Architecture a,
    ...
    void writeGPR(X86GeneralPurposeRegister r, Handle<32> value)
    {
        ...
        {
            /*
            std::cerr << "Warning: discarding write to GPR " << convert(r).format() <<
          std::endl; for (std::map<Absloc, Assignment::Ptr>::iterator foozle =
          aaMap.begin(); foozle != aaMap.end(); ++foozle) { std::cerr << "\t" <<
          foozle->first.format() << std::endl;
            }
            */
        }
    }
    ...
    {
        std::map<Absloc, Assignment::Ptr>::iterator i =
            aaMap.find(convert(powerpc_regclass_gpr, r));
        if(i != aaMap.end())
        {
            res[i->second] = value.var();
        }
    ...
    {
        std::map<Absloc, Assignment::Ptr>::iterator i =
            aaMap.find(convert(powerpc_regclass_spr, r));
        if(i != aaMap.end())
        {
            res[i->second] = value.var();
        }
    ...
    {
        std::map<Absloc, Assignment::Ptr>::iterator i =
            aaMap.find(convert(powerpc_regclass_cr, field));
        if(i != aaMap.end())
        {
            res[i->second] = value.var();
        }
    }
    Handle<16> readSegreg(X86SegmentRegister r) { return Handle<16>(wrap(convert(r))); }
    ...
    Handle<1> readFlag(X86Flag f) { return Handle<1>(wrap(convert(f))); }
    ...
    template <size_t Len>
    Handle<Len> readMemory(X86SegmentRegister /*segreg*/, Handle<32> addr, Handle<1> cond)
    {
        if(cond == true_())
        {
            return Handle<Len>(getUnaryAST(ROSEOperation::derefOp, addr.var(), Len));
        }
        else
        {
            return Handle<Len>(
                getBinaryAST(ROSEOperation::derefOp, addr.var(), cond.var(), Len));
        }
    }

    template <size_t Len>
    Handle<Len> readMemory(Handle<32> addr, Handle<1> cond)
    {
        return Handle<Len>(
            getBinaryAST(ROSEOperation::derefOp, addr.var(), cond.var(), Len));
    }
    template <size_t Len>
    void writeMemory(X86SegmentRegister, Handle<32> addr, Handle<Len> data,
                     Handle<32> repeat, Handle<1> cond)
    {
        ...
        if(i != aaMap.end())
        {
            ...
            {
                res[i->second] =
                    getBinaryAST(ROSEOperation::writeRepOp, data.var(), repeat.var());
            }
            else
            {
                res[i->second] = getTernaryAST(ROSEOperation::writeRepOp, data.var(),
                                               cond.var(), repeat.var());
            }
        }
    }
    ...
    template <size_t Len>
    void writeMemory(Handle<32> addr, Handle<Len> data, Handle<1> cond)
    {
        std::map<Absloc, Assignment::Ptr>::iterator i = aaMap.find(Absloc(0));
        if(i != aaMap.end())
        {
            ...
            {
                res[i->second] =
                    getBinaryAST(ROSEOperation::writeOp, data.var(), cond.var());
            }
        }
    }
    ...
    Handle<1> true_() { return Handle<1>(getConstAST(1, 1)); }

    Handle<1> false_() { return Handle<1>(getConstAST(0, 1)); }

    // TODO FIXME - we need a "bottom".
    Handle<1> undefined_() { return Handle<1>(getBottomAST()); }

    // If-then-else

    template <size_t Len>
    Handle<Len> ite(Handle<1> sel, Handle<Len> ifTrue, Handle<Len> ifFalse)
    {
        return Handle<Len>(
            getTernaryAST(ROSEOperation::ifOp, sel.var(), ifTrue.var(), ifFalse.var()));
    }
    ...
    Handle<To - From> extract(Handle<Len> a)
    {
        ...
        return Handle<To - From>(getTernaryAST(ROSEOperation::extractOp, a.var(),
                                               number<Len>(From).var(),
                                               number<Len>(To).var(), To - From));
    }
    ...
    {
        return Handle<To>(
            getBinaryAST(ROSEOperation::signExtendOp, a.var(), number<32>(To).var()));
    }
    ...
    {
        return Handle<Len1 + Len2>(
            getBinaryAST(ROSEOperation::concatOp, a.var(), b.var(), Len1 + Len2));
    }
    ...
    {
        return Handle<Len1 + Len2>(
            getBinaryAST(ROSEOperation::sMultOp, a.var(), b.var()));
    }
    ...
    {
        return Handle<Len1 + Len2>(
            getBinaryAST(ROSEOperation::uMultOp, a.var(), b.var()));
    }
    ...
    {
        return Handle<Len>(
            getBinaryAST(ROSEOperation::addOp, a.var(),
                         getBinaryAST(ROSEOperation::addOp, b.var(), c.var())));
    }
    ...
    {
        return Handle<Len>(
            getBinaryAST(ROSEOperation::xorOp, a.var(),
                         getBinaryAST(ROSEOperation::xorOp, b.var(), c.var())));
    }

    template <size_t Len>
    Handle<Len> addWithCarries(Handle<Len> a, Handle<Len> b, Handle<1> carryIn,
                               Handle<Len>& carries)
    {
    ...
    Handle<64> rdtsc() { return number<64>(0); }
    ...
    Handle<To> extendByMSB(Handle<From> a)
    {
        ...
        return Handle<To>(
            getBinaryAST(ROSEOperation::extendMSBOp, a.var(), number<32>(To).var()));
    }
...
private:
    ...
    AST::Ptr wrap(Absloc r) { return VariableAST::create(Variable(AbsRegion(r), addr)); }
    ...
    AST::Ptr getBottomAST() { return BottomAST::create(false); }

    AST::Ptr getUnaryAST(ROSEOperation::Op op, AST::Ptr a, size_t s = 0)
    {
        return RoseAST::create(ROSEOperation(op, s), a);
    }

    AST::Ptr getBinaryAST(ROSEOperation::Op op, AST::Ptr a, AST::Ptr b, size_t s = 0)
    {
        return RoseAST::create(ROSEOperation(op, s), a, b);
    }

    AST::Ptr getTernaryAST(ROSEOperation::Op op, AST::Ptr a, AST::Ptr b, AST::Ptr c,
                           size_t s = 0)
    {
...
{
public:
    SymEvalPolicy_64(Result_t& r, Address addr, Dyninst::Architecture a,
    ...
    Handle<64> readSegreg(X86SegmentRegister r) { return Handle<64>(wrap(convert(r))); }
    void       systemCall(unsigned char value)
    {
        fprintf(stderr, "WARNING: syscall %d detected; unhandled by semantics!\n",
                (unsigned int) (value));
    ...
    template <size_t Len>
    Handle<Len> readMemory(X86SegmentRegister /*segreg*/, Handle<64> addr, Handle<1> cond)
    {
        if(cond == true_())
        {
            return Handle<Len>(getUnaryAST(ROSEOperation::derefOp, addr.var(), Len));
    ...
    template <size_t Len>
    Handle<Len> readMemory(Handle<64> addr, Handle<1> cond)
    {
        return Handle<Len>(
            getBinaryAST(ROSEOperation::derefOp, addr.var(), cond.var(), Len));
    }
    template <size_t Len>
    void writeMemory(X86SegmentRegister, Handle<64> addr, Handle<Len> data,
                     Handle<64> repeat, Handle<1> cond)
    {
        // We can only write memory once per instruction. Therefore,
        // instead of sweating what we're actually being handed (or
        // what AbsRegion we're actually assigning), we build in
    ...
    template <size_t Len>
    void writeMemory(Handle<64> addr, Handle<Len> data, Handle<1> cond)
    {
        std::map<Absloc, Assignment::Ptr>::iterator i = aaMap.find(Absloc(0));
        if(i != aaMap.end())
        {
    ...
    Handle<To> extendByMSB(Handle<From> a)
    {
        // return Handle<To>(a.var());
        return Handle<To>(
            getBinaryAST(ROSEOperation::extendMSBOp, a.var(), number<64>(To).var()));
    }
