...
bool
containsCall(ParseAPI::Block*);
bool
containsRet(ParseAPI::Block*);
ParseAPI::Function*
getEntryFunc(ParseAPI::Block*);
...
*/
Graph::Ptr
Slicer::sliceInternal(Direction dir, Predicates& p)
{
    Graph::Ptr                     ret;
    SliceNode::Ptr                 aP;
...
void
Slicer::sliceInternalAux(Graph::Ptr g, Direction dir, Predicates& p, SliceFrame& cand,
                         ...
                         unordered_map<Address, DefCache>& cache)
{
    ...
    for(unsigned i = 0; i < nextCands.size(); ++i)
    {
        ...
        if(!f.valid || (p.slicingSizeLimitFactor() > 0 &&
                        visited.size() > p.slicingSizeLimitFactor() * g->size()))
        {
            widenAll(g, dir, cand);
        }
        else
...
void
Slicer::removeBlocked(SliceFrame& f, set<AbsRegion> const& block)
{
...
void
Slicer::markVisited(map<CacheEdge, set<AbsRegion>>& visited, CacheEdge const& e,
...
bool
Slicer::updateAndLink(Graph::Ptr g, Direction dir, SliceFrame& cand, DefCache& cache,
                      Predicates& p)
{
    ...
    if(dir == forward)
        convertInstruction(cand.loc.current->first, cand.addr(), cand.loc.func,
                           cand.loc.block, assns);
    else
        convertInstruction(cand.loc.rcurrent->first, cand.addr(), cand.loc.func,
                           cand.loc.block, assns);
    ...
    for(unsigned i = 0; i < assns.size(); ++i)
    {
        ...
        {
            if(findMatch(g, dir, cand, (*ait).first, assns[i], matches, cache))
            {  // links
    ...
    if(!change && matches.empty())
    {  // no change -- nothing killed, nothing added
        return true;
    }
...
void
Slicer::updateAndLinkFromCache(Graph::Ptr g, Direction dir, SliceFrame& f,
                               DefCache& cache)
{
    SliceFrame::ActiveMap::iterator ait = f.active.begin();
...
void
Slicer::cachePotential(Direction dir, Assignment::Ptr assn, DefCache& cache)
{
    if(dir == forward)
    {
 ...
 */
bool
Slicer::findMatch(Graph::Ptr g, Direction dir, SliceFrame const& cand,
                  AbsRegion const& reg, Assignment::Ptr assn, vector<Element>& matches,
                  DefCache& cache)
{
    bool hadmatch = false;
    if(dir == forward)
        ...
        if(reg.contains(assn->out()) || assn->out().contains(reg))
        {
            ...
            if(cand.loc.block->obj()->cs()->getArch() == Arch_cuda)
            {
                if(reg.contains(assn->out()) && !assn->out().contains(reg))
                {
                    ...
                    slicing_printf(
                        "\t\t\t Handle predicate: search for %s, find %s, generate %s\n",
                        reg.format().c_str(), assn->out().format().c_str(),
                        ne.reg.format().c_str());
                    matches.push_back(ne);
                }
...
bool
Slicer::getNextCandidates(Direction dir, Predicates& p, SliceFrame const& cand,
 ...
 */
bool
Slicer::getSuccessors(Predicates& p, SliceFrame const& cand, vector<SliceFrame>& newCands)
{
    ...
    else
    {
        ...
        for(; eit != targets.end(); ++eit)
        {
            ...
            {
                SliceFrame nf = cand;
                slicing_printf("\t\t Handling default edge type %d... ", (*eit)->type());
...
void
Slicer::handlePredecessorEdge(ParseAPI::Edge* e, Predicates& p, SliceFrame const& cand,
                              vector<SliceFrame>& newCands, bool& err, SliceFrame& nf)
{
 ...
 */
bool
Slicer::getPredecessors(Predicates& p, SliceFrame const& cand,
                        vector<SliceFrame>& newCands)
{
    ...
    {
        sources_edges.insert(
            make_pair(make_pair((*eit)->src()->start(), (int) (*eit)->type()), *eit));
    }
 ...
 */
bool
Slicer::handleCall(Predicates& p, SliceFrame& cur, bool& err)
{
 ...
 */
bool
Slicer::followCall(Predicates& p, ParseAPI::Block* target, SliceFrame& cur)
{
    // FIXME quote:
    ...
    for(Context::reverse_iterator calls = cur.con.rbegin(); calls != cur.con.rend();
        ++calls)
    {
        if(NULL != calls->func)
        {
...
void
Slicer::shiftAllAbsRegions(SliceFrame& cur, long stack_depth, ParseAPI::Function* callee)
{
 ...
 */
bool
Slicer::handleCallDetails(SliceFrame& cur, ParseAPI::Block* caller_block)
{
 ...
 */
bool
Slicer::handleReturn(Predicates& /* p */, SliceFrame& cur, bool& err)
{
 ...
 */
void
Slicer::handleReturnDetails(SliceFrame& cur)
{
...
static bool
EndsWithConditionalJump(ParseAPI::Block* b)
{
    bool cond = false;
    for(auto eit = b->targets().begin(); eit != b->targets().end(); ++eit)
        if((*eit)->type() == COND_TAKEN)
            cond = true;
    return cond;
}

bool
Slicer::handleDefault(Direction dir, Predicates& p, ParseAPI::Edge* e, SliceFrame& cur,
                      bool& /* err */)
{
    if(dir == forward)
    {
    ...
    else
    {
        ...
        if(p.searchForControlFlowDep() && EndsWithConditionalJump(e->src()))
        {
            ...
            cur.active[AbsRegion(Absloc(
                MachRegister::getPC(cur.loc.block->obj()->cs()->getArch())))] = newE;
...
bool
Slicer::handleCallBackward(Predicates& p, SliceFrame const& cand,
...
Slicer::followCallBackward(Predicates& p, SliceFrame const& cand, AbsRegion const& reg,
...
bool
Slicer::handleCallDetailsBackward(SliceFrame& cur)
{
...
bool
Slicer::handleReturnBackward(Predicates& p, SliceFrame const& cand, SliceFrame& newCand,
                             ParseAPI::Edge* e, bool& err)
{
...
bool
Slicer::handleReturnDetailsBackward(SliceFrame& cur, ParseAPI::Block* caller_block)
{
    ParseAPI::Function* caller = cur.con.front().func;
    ParseAPI::Function* callee = cur.loc.func;
...
bool
Slicer::followReturn(Predicates& p, SliceFrame const& cand, ParseAPI::Block* source)
{
...
bool
containsCall(ParseAPI::Block* block)
{
...
bool
containsRet(ParseAPI::Block* block)
{
    // We contain a call if the out-edges include
    // either a CALL or a CALL_FT edge
    const Block::edgelist&          targets = block->targets();
...
static void
getInsnInstances(ParseAPI::Block* block, Slicer::InsnVec& insns)
{
...
ParseAPI::Function*
getEntryFunc(ParseAPI::Block* block)
{
...
Graph::Ptr
Slicer::forwardSlice(Predicates& predicates)
{
    // delete cache state
    unique_edges_.clear();
...
Graph::Ptr
Slicer::backwardSlice(Predicates& predicates)
{
    // delete cache state
    unique_edges_.clear();
...
bool
Slicer::getStackDepth(ParseAPI::Function* func, ParseAPI::Block* block, Address callAddr,
                      long& height)
{
...
void
Slicer::pushContext(Context& context, ParseAPI::Function* callee,
...
void
Slicer::shiftAbsRegion(AbsRegion const& callerReg, AbsRegion& calleeReg, long stack_depth,
...
bool
Slicer::kills(AbsRegion const& reg, Assignment::Ptr& assign)
{
       ...
       reg.absloc().type() == Absloc::Register)
    {
        ...
        if(index >= 0)
            if(abi->getCallWrittenRegisters()[abi->getIndex(r)] && r != x86_64::r11)
                return true;
    }
...
SliceNode::Ptr
Slicer::createNode(Element const& elem)
{
...
void
Slicer::convertInstruction(const Instruction& insn, Address addr,
...
void
Slicer::getInsnsBackward(Location& loc)
{
...
void
Slicer::insertPair(Graph::Ptr ret, Direction dir, Element const& source,
...
void
Slicer::insertPair(Graph::Ptr ret, Direction dir, SliceNode::Ptr& s, SliceNode::Ptr& t,
...
void
Slicer::widenAll(Graph::Ptr g, Direction dir, SliceFrame const& cand)
{
    SliceFrame::ActiveMap::const_iterator ait = cand.active.begin();
    for(; ait != cand.active.end(); ++ait)
    {
...
void
Slicer::widen(Graph::Ptr ret, Direction dir, Element const& e)
{
    if(dir == forward)
    {
...
void
Slicer::markAsEndNode(Graph::Ptr ret, Direction dir, Element& e)
{
    if(dir == forward)
    {
...
void
Slicer::fastForward(Location& loc, Address addr)
{
    ...
    {
        slicing_cerr << "Cannot find addr " << std::hex << addr << "in block ["
                     << loc.block->start() << "," << loc.block->end() << ")"
...
void
Slicer::fastBackward(Location& loc, Address addr)
{
...
// never read.
void
Slicer::cleanGraph(Graph::Ptr ret)
{
    ...
    for(std::list<Node::Ptr>::iterator tmp = toDelete.begin(); tmp != toDelete.end();
        ++tmp)
    {
        ret->deleteNode(*tmp);
    }
...
void
Slicer::promotePlausibleNodes(GraphPtr g, Direction d)
{
...
ParseAPI::Block*
Slicer::getBlock(ParseAPI::Edge* e, Direction dir)
{
    return ((dir == forward) ? e->trg() : e->src());
}

bool
Slicer::isWidenNode(Node::Ptr n)
{
...
void
Slicer::insertInitialNode(GraphPtr ret, Direction dir, SliceNode::Ptr aP)
{
    if(dir == forward)
    {
        // Entry node
...
void
Slicer::constructInitialFrame(Direction dir, SliceFrame& initFrame)
{
...
void
Slicer::mergeRecursiveCaches(std::unordered_map<Address, DefCache>& single,
                             std::unordered_map<Address, DefCache>& unified, Address)
{
    for(auto first = addrStack.rbegin(), last = addrStack.rend(); first != last; ++first)
    {
