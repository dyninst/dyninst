...
void
AbsRegionConverter::convertAll(InstructionAPI::Expression::Ptr expr, Address addr,
                               ...
                               std::vector<AbsRegion>& regions)
{
    ...
    if(boost::dynamic_pointer_cast<Dereference>(expr))
    {
        ...
        for(std::vector<Expression::Ptr>::const_iterator i = tmp.begin(); i != tmp.end();
            ++i)
        {
    ...
    expr->getUses(used);
    for(std::set<InstructionAST::Ptr>::const_iterator j = used.begin(); j != used.end();
        ++j)
    {
...
void
AbsRegionConverter::convertAll(const InstructionAPI::Instruction& insn, Address addr,
                               ParseAPI::Function* func, ParseAPI::Block* block,
    ...
    if(!usedCache(addr, func, used))
    {
            ...
            i != regsRead.end(); ++i)
        {
            if(insn.getArch() == Arch_aarch64)
            {
                ...
                std::vector<MachRegister> flagRegs = { aarch64::n, aarch64::z, aarch64::c,
                                                       aarch64::v };

                if((machReg & 0xFF) == (aarch64::pstate & 0xFF) &&
                   (machReg & 0xFF0000) == (aarch64::SPR))
                {
                    for(std::vector<MachRegister>::iterator itr = flagRegs.begin();
                        itr != flagRegs.end(); itr++)
                    {
                        used.push_back(AbsRegionConverter::convert(
                            RegisterAST::Ptr(new RegisterAST(*itr))));
                    }
                }
                else
                {
    ...
    if(!definedCache(addr, func, defined))
    {
            ...
            i != regsWritten.end(); ++i)
        {
            if(insn.getArch() == Arch_aarch64)
            {
                   ...
                   (machReg & 0xFF0000) == (aarch64::SPR))
                {
                    for(std::vector<MachRegister>::iterator itr = flagRegs.begin();
                        itr != flagRegs.end(); itr++)
                    {
                        defined.push_back(AbsRegionConverter::convert(
                            RegisterAST::Ptr(new RegisterAST(*itr))));
                    }
                }
                else
            ...
            else if(insn.getArch() == Arch_cuda && insn.hasPredicateOperand())
            {
                ...
                defined.push_back(AbsRegionConverter::convertPredicatedRegister(
                    *i, o.getPredicate(), o.isTruePredicate()));
            }
            else
            {
                defined.push_back(AbsRegionConverter::convert(*i));
...
AbsRegion
AbsRegionConverter::convert(RegisterAST::Ptr reg)
{
...
AbsRegion
AbsRegionConverter::convertPredicatedRegister(RegisterAST::Ptr r, RegisterAST::Ptr p,
                                              bool c)
{
...
AbsRegion
AbsRegionConverter::convert(Expression::Ptr exp, Address addr, ParseAPI::Function* func,
                            ParseAPI::Block* block)
{
    ...
    long spHeight     = 0;
    bool stackDefined = getCurrentStackHeight(func, block, addr, spHeight);
    long fpHeight     = 0;
    bool frameDefined = getCurrentFrameHeight(func, block, addr, fpHeight);
    ...
    Address pcVal = addr;
    if(block->obj()->cs()->getArch() == Arch_x86 ||
       block->obj()->cs()->getArch() == Arch_x86_64)
    {
...
AbsRegion
AbsRegionConverter::stack(Address addr, ParseAPI::Function* func, ParseAPI::Block* block,
                          bool push)
{
    if(!stackAnalysisEnabled_)
    {
        //        std::cerr << "Stack analysis disabled, returning Stack absregion" <<
        //        std::endl;
        return AbsRegion(Absloc::Stack);
    }
    long spHeight    = 0;
    bool stackExists = getCurrentStackHeight(func, block, addr, spHeight);
    if(!stackExists)
    {
        return AbsRegion(Absloc::Stack);
    }
    ...
    return AbsRegion(Absloc(spHeight, 0, func));
}

AbsRegion
AbsRegionConverter::frame(Address addr, ParseAPI::Function* func, ParseAPI::Block* block,
                          bool push)
{
    long fpHeight    = 0;
    bool frameExists = getCurrentFrameHeight(func, block, addr, fpHeight);
    ...
    return AbsRegion(Absloc(fpHeight, 0, func));
}

bool
AbsRegionConverter::getCurrentStackHeight(ParseAPI::Function* func,
                                          ParseAPI::Block* block, Address addr,
                                          long& height)
{
    if(!stackAnalysisEnabled_)
        return false;
...
bool
AbsRegionConverter::getCurrentFrameHeight(ParseAPI::Function* func,
                                          ParseAPI::Block* block, Address addr,
                                          long& height)
{
    if(!stackAnalysisEnabled_)
...
bool
AbsRegionConverter::usedCache(Address addr, ParseAPI::Function* func,
                              std::vector<AbsRegion>& used)
{
    if(!cacheEnabled_)
        return false;
    FuncCache::iterator iter = used_cache_.find(func);
    if(iter == used_cache_.end())
        return false;
...
bool
AbsRegionConverter::definedCache(Address addr, ParseAPI::Function* func,
                                 std::vector<AbsRegion>& defined)
{
    if(!cacheEnabled_)
        return false;
    FuncCache::iterator iter = defined_cache_.find(func);
    if(iter == defined_cache_.end())
        return false;
    AddrCache::iterator iter2 = iter->second.find(addr);
    if(iter2 == iter->second.end())
        return false;
...
void
AssignmentConverter::convert(const Instruction& I, const Address& addr,
                             ParseAPI::Function* func, ParseAPI::Block* block,
                             std::vector<Assignment::Ptr>& assignments)
{
    ...
    switch(I.getOperation().getID())
    {
        ...
        case amdgpu_op_s_setpc_b64: {
            ...
            std::vector<Operand> operands;
            I.getOperands(operands);
            // SETPC_B64 should only have one operand, which is the SGPR_PAIR that stores
            // the new PC VALUE Since we don't want to introduce extra Class at the
            // instructionAPI level, we need to break it down ourserlves here So we don't
            // need to deal with reading the value in rose DispatcherAmdgpuVega Depends on
            // the situation, we might also need to differentaite betwwen  vega and rdna,
            // but for now we focus on vega
            assert(operands.size() == 1);
            RegisterAST::Ptr sgpr_pair =
                boost::dynamic_pointer_cast<RegisterAST>(operands[0].getValue());
            unsigned int offset = sgpr_pair->getID() - amdgpu_vega::sgpr_vec2_0;
        ...
        case amdgpu_op_s_swappc_b64: {
            ...
            RegisterAST::Ptr store_sgpr_pair =
                boost::dynamic_pointer_cast<RegisterAST>(operands[1].getValue());
            unsigned int store_offset =
                store_sgpr_pair->getID() - amdgpu_vega::sgpr_vec2_0;
            AbsRegion store_oper0 =
                AbsRegion(MachRegister(amdgpu_vega::sgpr0 + store_offset));
            AbsRegion store_oper1 =
                AbsRegion(MachRegister(amdgpu_vega::sgpr0 + store_offset + 1));

            AbsRegion pc = AbsRegion(Absloc::makePC(func->isrc()->getArch()));

            Assignment::Ptr store_pc_lowA =
                Assignment::makeAssignment(I, addr, func, block, store_oper0);
            store_pc_lowA->addInput(pc);

            Assignment::Ptr store_pc_highA =
                Assignment::makeAssignment(I, addr, func, block, store_oper1);
            store_pc_highA->addInput(pc);

            Assignment::Ptr pcA = Assignment::makeAssignment(I, addr, func, block, pc);

            RegisterAST::Ptr target_sgpr_pair =
                boost::dynamic_pointer_cast<RegisterAST>(operands[0].getValue());
            unsigned int target_offset =
                target_sgpr_pair->getID() - amdgpu_vega::sgpr_vec2_0;
            AbsRegion target_oper0 =
                AbsRegion(MachRegister(amdgpu_vega::sgpr0 + target_offset));
            AbsRegion target_oper1 =
                AbsRegion(MachRegister(amdgpu_vega::sgpr0 + target_offset + 1));

            pcA->addInput(target_oper0);
            pcA->addInput(target_oper1);

            assignments.push_back(pcA);
            // while the below 2 assignments are also there, we comment it out for now
            // TODO:
        ...
        case amdgpu_op_s_add_u32: {
            ...
            Assignment::Ptr scc_assign =
                Assignment::makeAssignment(I, addr, func, block, scc);

            Assignment::Ptr add_assign =
                Assignment::makeAssignment(I, addr, func, block, dst1);
        ...
        case e_push: {
            ...
            // According to the InstructionAPI, the first operand will be the argument,
            // the second will be ESP.
            assert(operands.size() == 2);
            ...
            aConverter.convertAll(operands[0].getValue(), addr, func, block, oper0);
        ...
        case e_call: {
            ...
            Assignment::Ptr a =
                Assignment::makeAssignment(I, addr, func, block, pcRegion[0]);
            if(!used.empty())
            {
                for(std::vector<AbsRegion>::const_iterator u = used.begin();
                    u != used.end(); ++u)
                {
                    if(!(u->contains(pcRegion[0])) && !(u->contains(sp)))
                    {
                        a->addInput(*u);
                    }
                }
        ...
        case e_pop: {
            // <reg> = *SP
            // SP = SP + 4/8
            // Amusingly... this doesn't have an intra-instruction dependence. It should
            // to enforce the order that <reg> = *SP happens before SP = SP - 4, but since
            // the input to both uses of SP in this case are the, well, input values... no
            // "sideways" edges. However, we still special-case it so that SP doesn't
            // depend on the incoming stack value... Also, we use the same logic for
            // return, defining it as PC = *SP SP = SP + 4/8
            ...
            // According to the InstructionAPI, the first operand will be the explicit
            // register, the second will be ESP.
            assert(operands.size() == 2);
        ...
        case e_leave: {
            ...
            // This is going to give the stack analysis fits... for now, I think it just
            // reverts the stack depth to 0.
            ...
            */
            ...
            Assignment::Ptr fpA = Assignment::makeAssignment(I, addr, func, block, fp);
        ...
        case e_xchg: {
            ...
            aConverter.convertAll(operands[1].getValue(), addr, func, block, oper1);
            ...
            Assignment::Ptr a =
                Assignment::makeAssignment(I, addr, func, block, oper0[0]);
            a->addInputs(oper1);

            Assignment::Ptr b =
                Assignment::makeAssignment(I, addr, func, block, oper1[0]);
            b->addInputs(oper0);
        ...
        case power_op_stwu: {
            ...
            AbsRegion              effAddr = aConverter.convert(tmp, addr, func, block);
            std::vector<AbsRegion> regions;
            aConverter.convertAll(operands[0].getValue(), addr, func, block, regions);
            ...
            Assignment::Ptr mem =
                Assignment::makeAssignment(I, addr, func, block, effAddr);
            mem->addInput(RS);
    ...
    // and the like. So we're basically looking for indirect branches or absolute
    // branches. (are there absolutes on IA-32?). Also, conditional branches and the flag
    // registers they use.
...
void
AssignmentConverter::handlePushEquivalent(const Instruction I, Address addr,
                                          ...
                                          std::vector<Assignment::Ptr>& assignments)
{
    ...
    Assignment::Ptr spA = Assignment::makeAssignment(I, addr, func, block, stackTop);
...
void
AssignmentConverter::handlePopEquivalent(const Instruction I, Address addr,
                                         ParseAPI::Function* func, ParseAPI::Block* block,
                                         std::vector<AbsRegion>&       operands,
                                         std::vector<Assignment::Ptr>& assignments)
{
    ...
    Assignment::Ptr spA = Assignment::makeAssignment(I, addr, func, block, operands[0]);
...
bool
AssignmentConverter::cache(ParseAPI::Function* func, Address addr,
                           std::vector<Assignment::Ptr>& assignments)
{
    if(!cacheEnabled_)
    {
