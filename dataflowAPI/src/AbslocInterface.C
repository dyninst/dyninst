...
void AbsRegionConverter::convertAll(InstructionAPI::Expression::Ptr expr,
                                    Address addr, ParseAPI::Function *func,
...
AbsRegion AbsRegionConverter::convert(Expression::Ptr exp, Address addr,
  ...
  static Expression::Ptr thePC64(
      new RegisterAST(MachRegister::getPC(Arch_x86_64)));
  static Expression::Ptr thePCPPC(
      new RegisterAST(MachRegister::getPC(Arch_ppc32)));
  ...
  if (isFrame && stackAnalysisEnabled_) {
    if (res.defined && frameDefined) {
      return AbsRegion(Absloc(res.convert<Address>(), 0, func));
    } else {
...
AbsRegion AbsRegionConverter::stack(Address addr, ParseAPI::Function *func,
                                    ParseAPI::Block *block, bool push) {
  long spHeight = 0;
...
AbsRegion AbsRegionConverter::frame(Address addr, ParseAPI::Function *func,
                                    ParseAPI::Block *block, bool push) {
  long fpHeight = 0;
...
bool AbsRegionConverter::getCurrentFrameHeight(ParseAPI::Function *func,
  ...
  StackAnalysis::Height heightSA = sA.find(
      block, addr, MachRegister::getFramePointer(func->isrc()->getArch()));
  ;
...
bool AbsRegionConverter::usedCache(Address addr, ParseAPI::Function *func,
                                   std::vector<AbsRegion> &used) {
...
bool AbsRegionConverter::definedCache(Address addr, ParseAPI::Function *func,
                                      std::vector<AbsRegion> &defined) {
  if (!cacheEnabled_) return false;
...
void AssignmentConverter::convert(const Instruction::Ptr I, const Address &addr,
                                  ParseAPI::Function *func,
                                  ParseAPI::Block *block,
  ...
  switch (I->getOperation().getID()) {
    case e_push: {
      ...
      // According to the InstructionAPI, the first operand will be the
      // argument, the second will be ESP.
    ...
    case e_pop: {
      // <reg> = *SP
      // SP = SP + 4/8
      // Amusingly... this doesn't have an intra-instruction dependence. It
      // should to enforce
      // the order that <reg> = *SP happens before SP = SP - 4, but since the
      // input to both
      // uses of SP in this case are the, well, input values... no "sideways"
      // edges.
      // However, we still special-case it so that SP doesn't depend on the
      // incoming stack value...
    ...
    case e_leave: {
      ...
      // This is going to give the stack analysis fits... for now, I think it
      // just reverts the
      // stack depth to 0.
      ...
      Assignment::Ptr spA =
          Assignment::makeAssignment(I, addr, func, block, sp);
      spA->addInput(fp);
      ...
      */
      ...
      Assignment::Ptr fpA =
          Assignment::makeAssignment(I, addr, func, block, fp);
    ...
    case e_ret_far: {
      ...
      Assignment::Ptr pcA =
          Assignment::makeAssignment(I, addr, func, block, pc);
    ...
    default:
      ...
      for (std::vector<AbsRegion>::const_iterator i = defined.begin();
           i != defined.end(); ++i) {
        Assignment::Ptr a =
            Assignment::makeAssignment(I, addr, func, block, *i);
        a->addInputs(used);
        assignments.push_back(a);
      }
      break;
  ...
  // PC-handling section
  // Most instructions use the PC to set the PC. This includes calls, relative
  // branches,
...
void AssignmentConverter::handlePushEquivalent(
    const Instruction::Ptr I, Address addr, ParseAPI::Function *func,
    ParseAPI::Block *block, std::vector<AbsRegion> &operands,
    std::vector<Assignment::Ptr> &assignments) {
  ...
  Assignment::Ptr spA =
      Assignment::makeAssignment(I, addr, func, block, stackTop);
...
void AssignmentConverter::handlePopEquivalent(
  ...
  Assignment::Ptr spA =
      Assignment::makeAssignment(I, addr, func, block, operands[0]);
...
bool AssignmentConverter::cache(ParseAPI::Function *func, Address addr,
                                std::vector<Assignment::Ptr> &assignments) {
  if (!cacheEnabled_) {
    return false;
  }
