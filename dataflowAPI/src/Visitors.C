...
AST::Ptr
StackVisitor::visit(AST* a)
{
    return a->ptr();
}

AST::Ptr
StackVisitor::visit(BottomAST* b)
{
    return b->ptr();
}

AST::Ptr
StackVisitor::visit(ConstantAST* c)
{
    return c->ptr();
}

AST::Ptr
StackVisitor::visit(StackAST* s)
{
    return s->ptr();
}
...
AST::Ptr
StackVisitor::visit(VariableAST* v)
{
    ...
    switch(r->val().op)
    {
        ...
        case ROSEOperation::extractOp: {
            if(newKids[0]->getID() == AST::V_ConstantAST)
            {
                ...
                // Let's fix this...
                // newKids[1] is the "from", and newKids[2] is the "to". As in "mask from
                // 0 to 16".
                ...
                case AST::V_ConstantAST:
                    ...
                    switch(newKids[1]->getID())
                    {
                        case AST::V_ConstantAST: {
                            ...
                            return ConstantAST::create(
                                Constant(const0.val + const1.val,
                                         ((const0.size > const1.size) ? const0.size
                                                                      : const1.size)));
                        }
                        case AST::V_StackAST:
...
AST::Ptr
BooleanVisitor::visit(AST* t)
{
    return t->ptr();
}

AST::Ptr
BooleanVisitor::visit(BottomAST* b)
{
    return b->ptr();
}

AST::Ptr
BooleanVisitor::visit(ConstantAST* c)
{
    return c->ptr();
}

AST::Ptr
BooleanVisitor::visit(StackAST* s)
{
    return s->ptr();
}

AST::Ptr
BooleanVisitor::visit(VariableAST* v)
{
    return v->ptr();
}

AST::Ptr
BooleanVisitor::visit(RoseAST* r)
{
