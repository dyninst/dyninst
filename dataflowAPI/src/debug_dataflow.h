...
#define slicing_cerr \
  if (df_debug_slicing) cerr
#define stackanalysis_cerr \
  if (df_debug_stackanalysis) cerr
#define convert_cerr \
  if (df_debug_convert) cerr
#define expand_cerr \
  if (df_debug_expand) cerr
#define liveness_cerr \
  if (df_debug_liveness) cerr
...
#define slicing_printf(format, args...)                       \
  do {                                                        \
    if (df_debug_slicing) slicing_printf_int(format, ##args); \
  } while (0)
#define stackanalysis_printf(format, args...)                             \
  do {                                                                    \
    if (df_debug_stackanalysis) stackanalysis_printf_int(format, ##args); \
  } while (0)
#define convert_printf(format, args...)                       \
  do {                                                        \
    if (df_debug_convert) convert_printf_int(format, ##args); \
  } while (0)
#define expand_printf(format, args...)                      \
  do {                                                      \
    if (df_debug_expand) expand_printf_int(format, ##args); \
  } while (0)
#define liveness_printf(format, args...)                        \
  do {                                                          \
    if (df_debug_liveness) liveness_printf_int(format, ##args); \
  } while (0)
