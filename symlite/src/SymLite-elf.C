...
Symbol_t
SymElf::getSymbolByName(std::string symname)
{
    Symbol_t ret;
    for(unsigned i = 0; i < elf->e_shnum(); i++)
    {
...
Symbol_t
SymElf::getContainingSymbol(Dyninst::Offset offset)
{
#if 1
    if(!cache)
    {
...
bool
SymElf::getSegment(unsigned num, SymSegment& seg)
{
    if(num >= elf->e_phnum())
        return false;
...
unsigned
SymElf::getAddressWidth()
{
    return elf->wordSize();
}

bool
SymElf::getABIVersion(int& major, int& minor) const
{
...
bool
SymElf::isBigEndianDataEncoding() const
{
    return (elf->e_endian() != 0);
}

Architecture
SymElf::getArchitecture() const
{
    return elf->getArch();
}

unsigned long
SymElf::getSymbolSize(const Symbol_t& sym)
{
...
Dyninst::Offset
SymElf::getSymbolOffset(const Symbol_t& sym)
{
    assert(sym.i2 != INVALID_SYM_CODE);
...
Dyninst::Offset
SymElf::getSymbolTOC(const Symbol_t& sym)
{
    GET_SYMBOL(sym, shdr, symbols, name, idx);
    (void) name;  // Silence warnings
...
std::string
SymElf::getSymbolName(const Symbol_t& sym)
{
    GET_SYMBOL(sym, shdr, symbols, name, idx);
    (void) idx;
...
std::string
SymElf::getDemangledName(const Symbol_t& sym)
{
    assert(sym.i2 != INVALID_SYM_CODE);
    int         cache_index = -1;
...
bool
SymElf::isValidSymbol(const Symbol_t& sym)
{
    return (sym.i2 != INVALID_SYM_CODE);
}

static int
symcache_cmp(const void* a, const void* b)
{
...
unsigned long
SymElf::getSymTOC(const Elf_X_Sym& symbol, unsigned idx)
{
    if(need_odp && symbol.ST_TYPE(idx) == STT_FUNC)
    {
        unsigned long odp_addr   = odp_section->sh_addr();
...
Symbol_t
SymElf::lookupCachedSymbol(Dyninst::Offset off)
{
    unsigned min = 0;
...
Section_t
SymElf::getSectionByName(std::string name)
{
    unsigned short stridx  = elf->e_shstrndx();
    Elf_X_Shdr     strshdr = elf->get_shdr(stridx);
    Elf_X_Data     strdata = strshdr.get_data();
...
Section_t
SymElf::getSectionByAddress(Dyninst::Address addr)
{
    Section_t ret;
    ret.i1 = -1;
...
Dyninst::Address
SymElf::getSectionAddress(Section_t sec)
{
...
std::string
SymElf::getSectionName(Section_t sec)
{
    assert(isValidSection(sec));
    Elf_X_Shdr shdr = elf->get_shdr(sec.i1);
...
bool
SymElf::isValidSection(Section_t sec)
{
    return (sec.i1 != -1);
}
...
SymReader*
SymElfFactory::openSymbolReader(std::string pathname)
{
    SymElf*                                  se = NULL;
...
SymReader*
SymElfFactory::openSymbolReader(const char* buffer, unsigned long size)
{
...
bool
SymElfFactory::closeSymbolReader(SymReader* sr)
{
