...
class BPATCH_DLL_EXPORT BPatch {
  ...
  /* this is used to denote the fully qualified name of the prelink command on
   * linux */
 ...
 public:
  ...
  void registerDynamicCallsiteEvent(BPatch_process *process,
                                    Dyninst::Address callTarget,
  ...
  static void reportError(BPatchErrorLevel severity, int number,
                          const char *str);
 ...
 public:
  ...
  static void formatErrorString(char *dst, int size, const char *fmt,
  ...
  BPatch_type *createArray(const char *name, BPatch_type *ptr, unsigned int low,
                           unsigned int hi);
  ...
  bool getPidList(BPatch_remoteHost &remote,
                  BPatch_Vector<unsigned int> &pidlist);

  bool getPidInfo(BPatch_remoteHost &remote, unsigned int pid,
                  std::string &pidStr);
