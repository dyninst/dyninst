...
class BPATCH_DLL_EXPORT BPatch_image : public BPatch_sourceObj {
 ...
 public:
  ...
  BPatch_Vector<BPatch_parRegion *> *getParRegions(
      bool incUninstrumentable = false);
  ...
  BPatch_Vector<BPatch_function *> *findFunction(
      ...
      void *user_data = NULL, int showError = 0,
      bool incUninstrumentable = false);
  ...
  //
  //  Returns a BPatch_type corresponding to <name>, if exists, NULL if not
  //  found
  ...
  typedef std::vector<std::pair<unsigned long, unsigned long> >::iterator
      arange_iter;
