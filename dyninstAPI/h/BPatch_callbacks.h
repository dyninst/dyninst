...
typedef void (*BPatchOneTimeCodeCallback)(BPatch_thread* proc, void* userData,
                                          void* returnValue);

typedef void (*BPatchErrorCallback)(BPatchErrorLevel severity, int number,
...
typedef void (*BPatchDynLibraryCallback)(BPatch_thread* proc, BPatch_object* mod,
                                         bool load);
...
typedef void (*BPatchSignalHandlerCallback)(BPatch_point* at_point, long signum,
                                            BPatch_Vector<Dyninst::Address>* handlers);

typedef void (*InternalSignalHandlerCallback)(BPatch_point* at_point, long signum,
                                              BPatch_Vector<Dyninst::Address>& handlers);

typedef void (*InternalCodeOverwriteCallback)(BPatch_point*    fault_instruc,
                                              Dyninst::Address viol_target);

// This callback is invoked whenever previously un-analyzed code is discovered through
// runtime analysis, and delivers a vector of  functions whose analysis has been modified
// and a vector of functions that are newly discovered.
...
typedef void (*BPatchCodeOverwriteEndCallback)(
    ...
    BPatch_Vector<BPatch_function*>& owFuncs, BPatch_Vector<BPatch_function*>& modFuncs,
