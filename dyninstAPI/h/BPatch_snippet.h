...
class BPATCH_DLL_EXPORT BPatch_snippet {
 ...
 public:
  BPatch_snippet(const BPatch_snippet &src);
 ...
 public:
  ...
  //  BPatch_snippet::~BPatch_snippet
  //  Destructor, decrements reference count to snippet, deleting when none are
  //  left
  ...
  //  Returns an estimated cost of executing the snippet at a specified point,
  //  in seconds.
...
class BPATCH_DLL_EXPORT BPatch_variableExpr : public BPatch_snippet {
  ...
  BPatch_variableExpr(const char *in_name, BPatch_addressSpace *in_addSpace,
                      AddressSpace *as, AstNodePtr ast_wrapper_,
                      BPatch_type *type, void *in_address);
  // Used to get forked copies of variable expressions
  // Used by malloc & malloc_by_type
  BPatch_variableExpr(BPatch_addressSpace *in_addSpace, AddressSpace *as,
 ...
 public:
  ...
  //  returns false if the type info isn't available (i.e. we don't know the
  //  size)
...
// machineConditionExpr, then have this reimplemented as
// ifExpr(machineConditionExpr, ...),
// and have an optimization (fast path) for that case using the specialized
// AST that supports this class. Memory instrumentation has no need for a
// standalone
...
class BPATCH_DLL_EXPORT BPatch_shadowExpr : public BPatch_snippet {
 public:
  ...
  BPatch_shadowExpr(bool entry, const BPatchStopThreadCallback &cb,
...
class BPATCH_DLL_EXPORT BPatch_stopThreadExpr : public BPatch_snippet {
 public:
  ...
  BPatch_stopThreadExpr(const BPatchStopThreadCallback &cb,
                        const BPatch_snippet &calculation,
                        const mapped_object &obj, bool useCache = false,
                        BPatch_stInterpret interp = BPatch_noInterp);
};
