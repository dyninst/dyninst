...
class OneTimeCodeInfo {
 ...
 public:
  OneTimeCodeInfo(bool _synchronous, void *_userData,
                  BPatchOneTimeCodeCallback _cb, unsigned _thrID)
...
class BPATCH_DLL_EXPORT BPatch_process : public BPatch_addressSpace {
 ...
 private:
  ...
  // Used because callbacks go (and can clean up user code) before the low-level
  // process
  // sets flags.
  bool terminated;
  bool reportedExit;
 ...
 protected:
  ...
  BPatch_process(const char *path, const char *argv[], BPatch_hybridMode mode,
                 const char **envp = NULL, int stdin_fd = 0, int stdout_fd = 1,
                 int stderr_fd = 2);
  // for attaching
 ...
 public:
  ...
  bool triggerStopThread(instPoint *intPoint, func_instance *intFunc, int cb_ID,
                         void *retVal);
  bool triggerSignalHandlerCB(instPoint *point, func_instance *func,
                              long signum,
  ...
  // Finalizes all instrumentation logically added since a call to
  // beginInsertionSet.
  //  Returns true if all instrumentation was successfully inserted; otherwise,
  //  none
  //  was. Individual instrumentation can be manipulated via the
  //  BPatchSnippetHandles
  //  returned from individual calls to insertSnippet.
  //
  //  atomic: if true, all instrumentation will be removed if any fails to go
  //  in.
  //  modified: if provided, and set to true by finalizeInsertionSet, additional
  //            steps were taken to make the installation work, such as
  //            modifying
  //            process state.  Note that such steps will be taken whether or
  //            not
  ...
  //
  //  Have the specified code be executed by the mutatee once.  Dont wait until
  //  done.
