...
class BPATCH_DLL_EXPORT BPatch_instruction {
 ...
 protected:
  ...
  int *condition;     // -1 means no condition, all other values are machine
                      // specific
 ...
 public:
  BPatch_instruction(internal_instruction *insn, long unsigned int _addr);
 ...
 public:
  bool equals(const BPatch_instruction *mp) const {
  ...
  bool equals(const BPatch_instruction &rp) const {
    ...
    res = (isLoad == rp.isLoad) && (isStore == rp.isStore) &&
          (preFcn == rp.preFcn) && (condition == rp.condition) &&
  ...
  bool hasALoad() const {
    return nacc == 1 ? isLoad[0] : (isLoad[0] || isLoad[1]);
  }
  bool hasAStore() const {
    return nacc == 1 ? isStore[0] : (isStore[0] || isStore[1]);
  }
...
class BPATCH_DLL_EXPORT BPatch_branchInstruction : public BPatch_instruction {
 ...
 public:
  BPatch_branchInstruction(internal_instruction *insn, long unsigned int _addr,
                           void *target)
      : BPatch_instruction(insn, _addr), target_(target) {}
...
class BPATCH_DLL_EXPORT BPatch_register {
 ...
 private:
  BPatch_register(std::string n, int e) : name_(n), number_(e){};
