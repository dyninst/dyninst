...
bool
PCEventHandler::handle(EventPtr ev)
{
    return handler().handle_internal(ev);
}

bool
PCEventHandler::handle_internal(EventPtr ev)
{
    ...
    if(!ev->getProcess())
    {
        proccontrol_printf("%s[%d]: Event received w/o associated ProcControl process "
                           "object, treating as handled successfully\n",
                           FILE__, __LINE__);
        return true;
    }
    ...
    if(dyn_debug_proccontrol)
    {
        proccontrol_printf(
            ...
            (int) ret, (int) (evProc->getDesiredProcessState() == PCProcess::ps_running),
    ...
    if(ret  // there were no errors
       ...
       && !evProc->hasReportedEvent()  // we aren't in the middle of processing an event
                                       // that we reported to ProcControl
       && !evProc->isTerminated()      // If one of the handling routines has marked the
                                       // process exited
       && !evProc->hasPendingEvents()  // Can't continue the process until all pending
                                       // events handled for all threads
    )
    {
    ...
    {
        proccontrol_printf(
            "%s[%d]: pending exit reported to BPatch-level, marking process exited\n",
            FILE__, __LINE__);
        evProc->markExited();
    }
...
bool
PCEventHandler::handleExit(EventExit::const_ptr ev, PCProcess* evProc) const
{
    evProc->setReportingEvent(false);
    static bool reportPreExit =
        true;  // PCEventMuxer::useBreakpoint(EventType(EventType::Pre, EventType::Exit));
...
bool
PCEventHandler::handleCrash(EventCrash::const_ptr ev, PCProcess* evProc) const
{
    if(ev->getEventType().time() == EventType::Pre)
    {
...
bool
PCEventHandler::handleForceTerminate(EventForceTerminate::const_ptr ev,
...
bool
PCEventHandler::handleFork(EventFork::const_ptr ev, PCProcess* evProc) const
{
    evProc->setReportingEvent(false);
    if(ev->getEventType().time() == EventType::Pre)
    {
    ...
    else
    {
        ...
        if(childProc == NULL)
        {
            proccontrol_printf("%s[%d]: failed to create process representation for "
                               "child %d of process %d\n",
                               FILE__, __LINE__, ev->getChildProcess()->getPid(),
                               evProc->getPid());
            return false;
        }
        ...
        {
            proccontrol_printf(
                "%s[%d]: user wants newly created process running after event handling\n",
                FILE__, __LINE__);
...
bool
PCEventHandler::handleExec(EventExec::const_ptr ev, PCProcess*& evProc) const
{
    evProc->setReportingEvent(false);
    if(ev->getEventType().time() == EventType::Pre)
    {
...
bool
PCEventHandler::handleThreadCreate(EventNewThread::const_ptr ev, PCProcess* evProc) const
{
    ...
    {
        proccontrol_printf(
            "%s[%d]: no user thread info for thread %d/%d, postponing thread create\n",
    ...
    {
        proccontrol_printf("%s[%d]: event corresponds to initial thread, ignoring thread "
                           "create for thread %d/%d\n",
                           FILE__, __LINE__, evProc->getPid(), ev->getLWP());
        return true;
    }
    ...
    {
        proccontrol_printf("%s[%d]: thread already created with TID 0x%lx, ignoring "
                           "thread create on thread %d/%d\n",
                           FILE__, __LINE__, (unsigned long) pcThr->getTID(),
                           evProc->getPid(), ev->getLWP());
        return true;
    }
    ...
    if(newThr == NULL)
    {
        proccontrol_printf("%s[%d]: failed to create internal thread representation for "
                           "new thread %d/%d\n",
                           FILE__, __LINE__, evProc->getPid(), ev->getLWP());
        return false;
    }
...
bool
PCEventHandler::handleThreadDestroy(EventThreadDestroy::const_ptr ev,
                                    PCProcess*                    evProc) const
{
    if(ev->getEventType().time() == EventType::Pre)
    {
        proccontrol_printf("%s[%d]: handling pre thread destroy for %d/%d\n", FILE__,
                           __LINE__, evProc->getPid(), ev->getThread()->getLWP());
        BPatch_process* bpproc = BPatch::bpatch->getProcessByPid(evProc->getPid());
        if(bpproc == NULL)
        {
            proccontrol_printf("%s[%d]: failed to locate BPatch_process for process %d\n",
        ...
        if(exitThread == NULL)
        {
            ...
            // ignore any unknown thread destroy events (they correspond to lwp destroy
            // events)
            proccontrol_printf("%s[%d]: failed to locate internal thread representation "
                               "for thread %d/%d, ignoring event\n",
                               FILE__, __LINE__, evProc->getPid(),
                               ev->getThread()->getLWP());
            return true;
        }
...
bool
PCEventHandler::handleSignal(EventSignal::const_ptr ev, PCProcess* evProc) const
{
    ...
    {
        malware_cerr << "Write to protected address 0x" << std::hex << ev->getAddress()
                     << std::dec << std::endl;
        ...
        {
            std::pair<bool, Address> trans = evProc->getMemEm()->translateBackwards(addr);
            if(trans.first)
            {
                addr = trans.second;
    ...
    // Debugging only
    if((dyn_debug_proccontrol || dyn_debug_crash) && isCrashSignal(ev->getSignal()))
    {
...
bool
PCEventHandler::handleRTBreakpoint(EventBreakpoint::const_ptr ev, PCProcess* evProc) const
{
    ...
    if(rtTrapFuncAddr == 0 || sync_event_id_addr == 0 || sync_event_arg1_addr == 0)
    {
        proccontrol_printf(
            "%s[%d]: signal is not RT library breakpoint. Some address is 0: "
            "rtTrapFuncAddr %lx, sync_even_id_addr %lx, sync_event_arg1_addr %lx\n",
    ...
    {
        proccontrol_printf("%s[%d]: signal is not RT library breakpoint. Breakpoint from "
                           "address %lx, expected %lx\n",
                           FILE__, __LINE__, ev->getAddress(), rtTrapFuncAddr);
        return false;
    }

    // Get the type of the event
    if(!evProc->readDataWord((const void*) sync_event_id_addr, sizeof(int), &status,
                             false))
        return false;
...
bool
PCEventHandler::handleUserMessage(PCProcess* evProc, BPatch_process* bpProc,
...
bool
PCEventHandler::handleDynFuncCall(PCProcess* evProc, BPatch_process* bpProc,
                                  Address rt_arg) const
{
...
bool
PCEventHandler::handleStopThread(PCProcess* evProc, Address rt_arg) const
{
...
bool
PCEventHandler::handleLibrary(EventLibrary::const_ptr ev, PCProcess* evProc) const
{
    ...
    for(set<Library::ptr>::const_iterator i = added.begin(); i != added.end(); ++i)
    {
        ...
        if(evProc->usesDataLoadAddress())
            dataAddress = (*i)->getDataLoadAddress();
        ...
        mapped_object* newObj =
            mapped_object::createMappedObject(tmpDesc, evProc, evProc->getHybridMode());
        if(newObj == NULL)
        {
        ...
        // TODO special handling for libc on Linux (breakpoint at __libc_start_main if
        // cannot find main)
    ...
    for(set<Library::ptr>::const_iterator i = deleted.begin(); i != deleted.end(); ++i)
    {
        ...
        if(evProc->usesDataLoadAddress())
            dataAddress = (*i)->getDataLoadAddress();
        deletedDescriptors.push_back(
            fileDescriptor((*i)->getAbsoluteName(), (*i)->getLoadAddress(), dataAddress));
    }
...
bool
PCEventHandler::handleBreakpoint(EventBreakpoint::const_ptr ev, PCProcess* evProc) const
{
...
bool
PCEventHandler::handleRPC(EventRPC::const_ptr ev, PCProcess* evProc) const
{
    ...
    proccontrol_printf("%s[%d]: handling completion of RPC %lu on thread %d/%d\n", FILE__,
                       __LINE__, ev->getIRPC()->getID(), ev->getProcess()->getPid(),
                       ev->getThread()->getLWP());
    ...
    if(rpcInProg->deliverCallbacks)
    {
        ...
        callbackResult = BPatch_process::oneTimeCodeCallbackDispatch(
            evProc, rpcInProg->rpc->getID(), rpcInProg->userData, rpcInProg->returnValue);
    }
