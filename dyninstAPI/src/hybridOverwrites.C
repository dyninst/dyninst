...
static void
overwriteAnalysis_wrapper(BPatch_point* point, void* calc)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
        ->hybridOW()
        ->overwriteAnalysis(point, calc);
}

static void
overwriteSignalCB_wrapper(BPatch_point* viol_instruc, Address viol_target)
{
    dynamic_cast<BPatch_process*>(viol_instruc->getFunction()->getProc())
        ->getHybridAnalysis()
        ->hybridOW()
        ->overwriteSignalCB((Address) viol_instruc->getAddress(), viol_target);
}
InternalCodeOverwriteCallback
...
HybridAnalysisOW::owLoop::owLoop(HybridAnalysisOW* hybridow, Address writeTarg)
{
    hybridow_                    = hybridow;
...
bool
HybridAnalysisOW::codeChangeCB(std::vector<BPatch_function*>& modfuncs)
{
    for(std::map<int, owLoop*>::iterator lIter = idToLoop.begin();
        lIter != idToLoop.end(); lIter++)
    {
        for(std::set<BPatch_basicBlock*, HybridAnalysis::blockcmp>::iterator biter =
                ((*lIter).second)->blocks.begin();
            biter != ((*lIter).second)->blocks.end(); biter++)
        {
...
void
HybridAnalysisOW::owLoop::setWriteTarget(Address targ)
{
    writeTarget_ = targ;
}

void
HybridAnalysisOW::owLoop::setWritesOwnPage(bool wop)
{
    writesOwnPage_ = wop;
}
void
HybridAnalysisOW::owLoop::setActive(bool act)
{
    activeStatus_ = act;
}

// return value of 0 means not in loop
HybridAnalysisOW::owLoop*
HybridAnalysisOW::findLoop(Address blockStart)
{
...
bool
HybridAnalysisOW::isInLoop(Address blockAddr, bool activeOnly)
{
 ...
 */
bool
HybridAnalysisOW::deleteLoop(owLoop* loop, bool useInsertionSet, BPatch_point* writePoint,
                             bool uninstrument)
{
    mal_printf("deleteLoop: loopID =%d active=%d %s[%d]\n", loop->getID(),
               loop->isActive(), FILE__, __LINE__);
 ...
 */
bool
HybridAnalysisOW::removeLoop(owLoop* loop, bool useInsertionSet, BPatch_point* writePoint,
                             bool uninstrument)
{
    ...
    if(isLoopActive)
    {
        ...
        if(writePoint)
        {
            cerr << "Calling overwriteAnalysis with point @ " << hex
                 << writePoint->getAddress() << dec << endl;
            overwriteAnalysis(writePoint, (void*) (intptr_t) loop->getID());
        }
        else
        {
            std::set<BPatch_function*> funcsToInstrument;
            proc()->overwriteAnalysisUpdate(loop->shadowMap, deadBlocks, modFuncs,
                                            funcsToInstrument, changedPages, changedCode);
            assert(!changedCode &&
                   "bug, overwrite loops should not contain "
...
bool
HybridAnalysisOW::hasLoopInstrumentation(bool activeOnly, BPatch_function& func,
                                         std::set<owLoop*>* loops_)
{
 ...
 */
void
HybridAnalysisOW::owLoop::instrumentOverwriteLoop(Address writeInsn)
{
    assert(blocks.size());
    ...
    while(bIter != blocks.end())
    {
        ...
        {
            if(blocks.end() == blocks.find(outEdges[eIdx]->getTarget()))
            {
                ...
                if(src->lowlevel_block()->isShared())
                {
                    ...
                    {
                        if(funcs[fidx] !=
                           src->getFlowGraph()->getFunction()->lowlevel_func())
                        {
                            BPatch_function* bpf = hybridow_->proc()->findOrCreateBPFunc(
                                funcs[fidx], src->getFlowGraph()->getModule());

                            BPatch_basicBlock* shared =
                                bpf->getCFG()->findBlockByAddr(src->getStartAddress());
    ...
    BPatch_stopThreadExpr stopForAnalysis(
        overwriteAnalysis_wrapper, BPatch_constExpr(getID()), false, BPatch_noInterp);
    // 2a.Instrument at loop exit edges
    ...
    while(eIter != instEdges.end())
    {
        ...
        snippetHandle = hybridow_->proc()->insertSnippet(stopForAnalysis, *edgePoint,
                                                         BPatch_firstSnippet);
    ...
    while(uIter != unresExits_.end())
    {
        ...
        mal_printf(" instr unresolved: 0x%p in func at 0x%p\n", (void*) uAddr,
                   (*uIter)->getFunction()->getBaseAddr());
        if((*uIter)->isDynamic())
        {
            long            st = 0;
            vector<Address> targs;
            if(hybridow_->hybrid_->getCallAndBranchTargets((*uIter)->llpoint()->block(),
                                                           targs))
            {
                st = *targs.begin();
            }
            BPatch_constExpr stSnip(st);
            BPatch_ifExpr    stopIfNewTarg(BPatch_boolExpr(BPatch_ne, dynTarg, stSnip),
                                        stopForAnalysis);
            snippetHandle = hybridow_->proc()->insertSnippet(stopIfNewTarg, *(*uIter),
                                                             BPatch_firstSnippet);
        }
        else
        {
            snippetHandle = hybridow_->proc()->insertSnippet(stopForAnalysis, *(*uIter),
                                                             BPatch_firstSnippet);
        }
        assert(snippetHandle);
        snippets.insert(snippetHandle);
...
void
HybridAnalysisOW::owLoop::instrumentOneWrite(Address                  writeInsnAddr,
                                             vector<BPatch_function*> writeFuncs)
{
    ...
    BPatch_stopThreadExpr stopForAnalysis(
        overwriteAnalysis_wrapper, BPatch_constExpr(loopID_), false, BPatch_noInterp);
    ...
    for(unsigned fidx = 0; fidx < writeFuncs.size(); fidx++)
    {
        ...
        block_instance* block =
            writeFuncs[fidx]->lowlevel_func()->obj()->findOneBlockByAddr(writeInsnAddr);
        if(!block)
            continue;
        instPoint* ip =
            instPoint::postInsn(writeFuncs[fidx]->lowlevel_func(), block, writeInsnAddr);
        BPatch_point* writePoint = hybridow_->proc()->findOrCreateBPPoint(
            writeFuncs[fidx], ip, BPatch_locInstruction);
...
*/
void
HybridAnalysisOW::owLoop::instrumentLoopWritesWithBoundsCheck()
{
    assert(!blocks.empty());
    ...
    while(bIter != blocks.end())
    {
        ...
        for(unsigned widx = 0; widx < (*blockWrites).size(); widx++)
        {
            if(BPatch_locSubroutine == (*blockWrites)[widx]->getPointType() ||
               !hybridow_->isRealStore((Address)(*blockWrites)[widx]->getAddress(),
        ...
        loopWrites.insert(loopWrites.end(), blockWrites->begin(), blockWrites->end());
    ...
    if(boundsIdx > 0)
    {
        mal_printf("BA[%u] = [%lx %lx]\n", boundsIdx - 2, boundsArray[boundsIdx - 2],
                   boundsArray[boundsIdx - 1]);
    }
    ...
    // create and initialize the snippet for the array of bounds
    BPatch_variableExpr* varBA =
        hybridow_->proc()->malloc(array_length * sizeof(Address));
    ...
    hybridow_->hybrid()->getRuntimeLib()->findFunction("DYNINST_boundsCheck", funcs);
    ...
    for(unsigned wIdx = 0; wIdx < loopWrites.size(); wIdx++)
    {
        ...
        BPatch_stopThreadExpr stopForAnalysis(overwriteAnalysis_wrapper,
                                              BPatch_constExpr(-1 * loopID_), false,
                                              BPatch_noInterp);
...
HybridAnalysisOW::getParentLoop(BPatch_function& func, Address writeAddr)
{
        ...
        if(writeHits[writeAddr] >= HIT_THRESHOLD)
        {
            ...
            if(writeStackIdx < stacks.size())
            {
                ...
                for(unsigned fidx = 1; fidx < stacks[writeStackIdx].size(); fidx++)
                {
                    ...
                    BPatch_function* curF = proc()->findOrCreateBPFunc(curIntF, NULL);
                    ...
                    // get loop
                    BPatch_basicBlockLoop* curL = getWriteLoop(*curF, origPC, false);
                    if(curL)
                    {
    ...
    else
    {
        ...
        for(vector<BPatch_point*>::iterator pit = callPs.begin(); pit != callPs.end();
            pit++)
        {
            BPatch_basicBlockLoop* curL =
                getWriteLoop(*callF, (Address)(*pit)->getAddress(), false);
            if(curL)
            {
...
// gets biggest loop without unresolved/multiply resolved indirect ctrl flow that it can
// find
BPatch_basicBlockLoop*
HybridAnalysisOW::getWriteLoop(BPatch_function& func, Address writeAddr,
                               bool allowCallerLoop)
{
    ...
    while(lIter != loops_.end())
    {
        ...
        {
            // set writeLoop if the curloop has no indirect or non-returning control
            // transfers
            bool hasUnresolved = false;
            (*lIter)->getLoopBasicBlocks(loopBlocks);
            for(vector<BPatch_basicBlock*>::iterator bIter = loopBlocks.begin();
                bIter != loopBlocks.end() && !hasUnresolved; bIter++)
            {
                ...
                for(vector<BPatch_point*>::iterator pIter = blockPoints.begin();
                    pIter != blockPoints.end(); pIter++)
                {
                    if((*pIter)->isDynamic())
                    {
                        vector<Address> targs;
                        if(!hybrid_->getCallAndBranchTargets((*pIter)->llpoint()->block(),
                                                             targs))
                        {
                        ...
                        {
                            mal_printf("loop has an indirect transfer resolving to "
                                       "multiple targets at %p\n",
                                       (*pIter)->getAddress());
                            hasUnresolved = true;
                        }
                    }
    ...
    if(writeLoop)
    {
        //        mal_printf("CHOSE nat'l loop with: block[0] at 0x%x, back edge at
        //        0%x\n",
...
static void
addLoopFunc(BPatch_function* func, set<BPatch_function*>& visited,
            set<BPatch_function*>& visitMe)
{
    ...
    else if(!func->getModule()->isSystemLib())
    {
        ...
        fprintf(stderr,
                "ERROR: overwrite loop calls into func at "
                "0x%lx in module %s that is not marked with "
                "malware mode %s[%d]\n",
                (Address) func->getBaseAddr(), modName, FILE__, __LINE__);
    }
}
...
bool
HybridAnalysisOW::addFuncBlocks(owLoop* loop, std::set<BPatch_function*>& addFuncs,
                                ...
                                std::set<int>&              overlappingLoops)
{
    ...
    for(set<BPatch_function*>::iterator fIter = addFuncs.begin(); fIter != addFuncs.end();
        fIter++)
    {
            ...
            bIter != fBlocks.end(); bIter++)
        {
            ...
            {
                mal_printf(
                    "block [%lx %lx] in loop %d overlaps with existing loop %d %s[%d]\n",
                    blockStart, (*bIter)->getEndAddress(), loop->getID(),
                    blockToLoop[blockStart], FILE__, __LINE__);
        ...
        for(vector<BPatch_point*>::iterator pIter = unresolvedCF.begin();
            !hasUnresolved && pIter != unresolvedCF.end(); pIter++)
        {
            vector<Address> targs;
            ...
            if(1 != targs.size())
            {
                ...
                mal_printf("loop %d calls func %p which has an indirect "
                           ...
                           "%s[%d]\n",
                           loop->getID(), (*fIter)->getBaseAddr(), (*pIter)->getAddress(),
                           targs.size(), FILE__, __LINE__);
            }
            else
            {
                // add target function if it's not in seenFuncs
                addLoopFunc(proc()->findFunctionByEntry(targs[0]), seenFuncs,
                            nextAddFuncs);
            }
        }
        ...
        if(ParseAPI::RETURN != (*fIter)->lowlevel_func()->ifunc()->retstatus())
        {
            mal_printf("loop %d calls func %p which could be "
                       "non-returning %s[%d]\n",
                       loop->getID(), (*fIter)->getBaseAddr(), FILE__, __LINE__);
            hasUnresolved = true;
        }
    }
    ...
    {
        if(false == addFuncBlocks(loop, nextAddFuncs, seenFuncs, overlappingLoops))
        {
            hasUnresolved = true;
        }
    }
...
bool
HybridAnalysisOW::setLoopBlocks(owLoop* loop, BPatch_basicBlockLoop* writeLoop,
                                std::set<int>& overlappingLoops)
{
        ...
        bIter != loopBlocks.end(); bIter++)
    {
        ...
        if(blockToLoop.find(blockStart) != blockToLoop.end())
        {
            ...
            // this block corresponds to another instrumented loop!
            fprintf(stderr,
                    "WARNING: block [%lx %lx] in loop %d overlaps with "
                    "existing loop %d %s[%d]\n",
                    blockStart, (*bIter)->getEndAddress(), loop->getID(),
                    blockToLoop[blockStart], FILE__, __LINE__);
            overlappingLoops.insert(blockToLoop[blockStart]);
            ...
            pIter != blockPoints.end(); pIter++)
        {
            if((*pIter)->isDynamic())
            {
                ...
                mal_printf("loop %d has a resolved indirect transfer at %p with "
                           "target %lx\n",
                           loop->getID(), (*pIter)->getAddress(), *targs.begin());

                vector<BPatch_function*> targFuncs;
                proc()->findFunctionsByAddr(*targs.begin(), targFuncs);
                if(!targFuncs.empty() && targFuncs[0]->getModule()->isExploratoryModeOn())
                {
        ...
        if(targFunc && (*bIter)->getFlowGraph()->getModule() == targFunc->getModule())
        {
    ...
    if(!loopFuncs.empty())
    {
        ...
        if(false == addFuncBlocks(loop, loopFuncs, loopFuncCopy, overlappingLoops))
        {
...
void
HybridAnalysisOW::makeShadow_setRights(Address, owLoop*)
{}
void
HybridAnalysisOW::overwriteAnalysis(BPatch_point*, void*)
{}
bool
HybridAnalysisOW::removeOverlappingLoops(owLoop*, std::set<int>&)
{
    return false;
}
...
// overlaps with
bool
HybridAnalysisOW::removeOverlappingLoops(owLoop* loop, std::set<int>& overlappingLoops)
{
    ...
    for(; lIter != overlappingLoops.end(); lIter++)
    {
        ...
        if(otherLoop->blocks.empty())
        {  // if cleanup is only partially done
            deleteLoop(otherLoop, false);
            removeLoops.insert(*lIter);
        }
        else
        {  // iterate through otherLoop's blocks to ensure they're a subset
            std::set<BPatch_basicBlock*, HybridAnalysis::blockcmp>::iterator obIter =
                otherLoop->blocks.begin();
            for(; obIter != otherLoop->blocks.end(); obIter++)
            {
                if(loop->blocks.end() == loop->blocks.find(*obIter))
                {
                    fprintf(stderr,
                            "WARNING: write loop %d is not a subset of "
    ...
    if(isSuperset)
    {
        ...
        for(lIter = overlappingLoops.begin(); lIter != overlappingLoops.end(); lIter++)
        {
            deleteLoop(idToLoop[*lIter],
                       false);  // CAREFUL! removes write perm's from loop pages
        }
        ...
        if(!loop->shadowMap.empty())
        {
            ...
            for(; siter != loop->shadowMap.end(); siter++)
            {
                proc()->setMemoryAccessRights(
                    siter->first, proc()->lowlevel_process()->getMemoryPageSize(),
...
void
HybridAnalysisOW::makeShadow_setRights(
 ...
 */
void
HybridAnalysisOW::overwriteAnalysis(BPatch_point* point, void* loopID_)
{
    ...
    {
        fprintf(stderr,
                "executed instrumentation for old loop %d at point %lx "
                "%s[%d]\n",
    ...
    if(overwroteLoop)
    {
        ...
        for(std::map<Address, unsigned char*>::iterator pIter = loop->shadowMap.begin();
            pIter != loop->shadowMap.end(); pIter++)
        {
    ...
    proc()->overwriteAnalysisUpdate(loop->shadowMap, deadBlocks, owFuncs,
                                    funcsToInstrument, changedPages, changedCode);
    ...
    for(map<Address, int>::iterator lit = blockToLoop.begin(); lit != blockToLoop.end();)
    {
...
bool
HybridAnalysisOW::isRealStore(Address insnAddr, block_instance* block,
                              BPatch_function* func)
{
    using namespace InstructionAPI;
    const unsigned char* buf = reinterpret_cast<const unsigned char*>(
        proc()->lowlevel_process()->getPtrToInstruction(insnAddr));
    InstructionDecoder decoder(buf, InstructionDecoder::maxInstructionLength,
 ...
 */
void
HybridAnalysisOW::overwriteSignalCB(Address faultInsnAddr, Address writeTarget)
{
    using namespace std;
    // debugging output
    ...
    if(loop)
    {
        ...
        // checks
        if(loop->isRealLoop() && pageAddress <= faultInsnAddr &&
           faultInsnAddr < (pageAddress + pageSize) && !loop->writesOwnPage())
        {
    ...
    if(bblLoop)
    {
        ...
        if(1 == faultFuncs.size() && setLoopBlocks(loop, bblLoop, overlappingLoops) &&
            ...
            removeOverlappingLoops(loop, overlappingLoops)))
        {
            ...
            if(pageAddress <= faultInsnAddr && faultInsnAddr < (pageAddress + pageSize))
            {
                loop->instrumentLoopWritesWithBoundsCheck();
                loop->setWritesOwnPage(true);
    ...
    if(!bblLoop)
    {
        mal_printf("Instrumenting after overwrite instruction at %lx in block "
                   "[%lx %lx), loopID=%d %s[%d]\n",
                   faultInsnAddr, faultBlocks[0]->getStartAddress(),
                   faultBlocks[0]->getEndAddress(), loop->getID(), FILE__, __LINE__);
    ...
    else
    {
        ...
        mal_printf("Instrumenting loop for write at %lx, loop has %lx blocks, "
                   "loopID=%d %s[%d]\n",
                   faultInsnAddr, loop->blocks.size(), loop->getID(), FILE__, __LINE__);
    }
...
bool
HybridAnalysisOW::registerCodeOverwriteCallbacks(BPatchCodeOverwriteBeginCallback cbBegin,
...
bool
HybridAnalysisOW::removeCodeOverwriteCallbacks()
{
    bool ret = false;
...
bool
HybridAnalysisOW::getActiveLoops(std::vector<HybridAnalysisOW::owLoop*>& active)
{
...
bool
HybridAnalysisOW::activeOverwritePages(std::set<Address>& pages)
{
