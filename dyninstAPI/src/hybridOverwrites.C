...
static void overwriteSignalCB_wrapper(BPatch_point *viol_instruc,
                                      Address viol_target) {
...
HybridAnalysisOW::owLoop::~owLoop() {
  ...
  std::map<Dyninst::Address, unsigned char *>::iterator sIter =
      shadowMap.begin();
...
bool HybridAnalysisOW::codeChangeCB(std::vector<BPatch_function *> &modfuncs) {
...
void HybridAnalysisOW::owLoop::setActive(bool act) { activeStatus_ = act; }
...
HybridAnalysisOW::owLoop *HybridAnalysisOW::findLoop(Address blockStart) {
  if (blockToLoop.find(blockStart) != blockToLoop.end()) {
    std::map<int, owLoop *>::iterator iter =
        idToLoop.find(blockToLoop[blockStart]);
 ...
 */
bool HybridAnalysisOW::deleteLoop(owLoop *loop, bool useInsertionSet,
                                  BPatch_point *writePoint, bool uninstrument) {
 ...
 * return true if the loop was active
 */
bool HybridAnalysisOW::removeLoop(owLoop *loop, bool useInsertionSet,
                                  BPatch_point *writePoint, bool uninstrument) {
  ...
  if (isLoopActive) {
    ...
    if (writePoint) {
      ...
      proc()->overwriteAnalysisUpdate(loop->shadowMap, deadBlocks, modFuncs,
                                      funcsToInstrument, changedPages,
                                      changedCode);
      assert(!changedCode &&
  ...
  for (; bIter != loop->blocks.end(); bIter++) {
    if (!(*bIter)->lowlevel_block()) {
      mal_printf(
          "WARNING: Can't remove overwritten loop block, the "
...
bool HybridAnalysisOW::hasLoopInstrumentation(bool activeOnly,
...
void HybridAnalysisOW::owLoop::instrumentOneWrite(
    Address writeInsnAddr, vector<BPatch_function *> writeFuncs) {
  realLoop_ = false;
  writeInsns.insert(writeInsnAddr);

  // create the stopthread expression
  BPatch_stopThreadExpr stopForAnalysis(overwriteAnalysis_wrapper,
                                        BPatch_constExpr(loopID_), false,
                                        BPatch_noInterp);
  ...
  for (unsigned fidx = 0; fidx < writeFuncs.size(); fidx++) {
    ...
    block_instance *block =
        writeFuncs[fidx]->lowlevel_func()->obj()->findOneBlockByAddr(
            writeInsnAddr);
    if (!block) continue;
...
void HybridAnalysisOW::owLoop::instrumentLoopWritesWithBoundsCheck() {
  ...
  while (bIter != blocks.end()) {
    ...
    loopWrites.insert(loopWrites.end(), blockWrites->begin(),
                      blockWrites->end());
    // store block bounds or alter previous entry if blocks are contiguous
    if (boundsIdx > 0 &&
        boundsArray[boundsIdx - 1] == (*bIter)->getStartAddress()) {
  ...
  hybridow_->hybrid()->getRuntimeLib()->findFunction("DYNINST_boundsCheck",
                                                     funcs);
...
BPatch_basicBlockLoop *HybridAnalysisOW::getParentLoop(BPatch_function &func,
                                                       Address writeAddr) {
  set<BPatch_function *> callFs;
  vector<BPatch_point *> callPs;
  func.getCallerPoints(callPs);
  for (vector<BPatch_point *>::iterator pit = callPs.begin();
       pit != callPs.end(); pit++) {
...
// that it can find
BPatch_basicBlockLoop *HybridAnalysisOW::getWriteLoop(BPatch_function &func,
                                                      Address writeAddr,
  ...
  while (lIter != loops.end()) {
    //        mal_printf("found nat'l loop w/ block[0] at 0x%x, back edge at
    //        0%x\n",
    ...
    if ((*lIter)->containsAddressInclusive(writeAddr) &&
      ...
      for (vector<BPatch_basicBlock *>::iterator bIter = loopBlocks.begin();
        ...
        if (blockToLoop.find(blockStart) != blockToLoop.end()) {
          ...
          mal_printf(
              "WARNING: block [%lx %lx] in loop overlaps with "
              "existing loop %d %s[%d]\n",
              blockStart, (*bIter)->getEndAddress(), blockToLoop[blockStart],
              FILE__, __LINE__);
        }
        ...
        for (vector<BPatch_point *>::iterator pIter = blockPoints.begin();
          ...
          if ((*pIter)->isDynamic()) {
            ...
            if (!hybrid_->getCallAndBranchTargets((*pIter)->llpoint()->block(),
              ...
              mal_printf(
                  "loop has an indirect transfer resolving to multiple targets "
                  "at %lx\n",
                  (*pIter)->getAddress());
              hasUnresolved = true;
            }
...
static void addLoopFunc(BPatch_function *func, set<BPatch_function *> &visited,
...
bool HybridAnalysisOW::addFuncBlocks(owLoop *loop,
  ...
  for (set<BPatch_function *>::iterator fIter = addFuncs.begin();
    ...
    for (std::set<BPatch_basicBlock *>::iterator bIter = fBlocks.begin();
      ...
      if ((*bIter)->lowlevel_block()->llb()->isCallBlock() &&
        ...
        mal_printf(
            "loop %d calls func %lx which has a non-returning "
            "call at %lx %s[%d]\n",
      ...
      if (blockToLoop.find(blockStart) != blockToLoop.end() &&
        ...
        mal_printf(
            "block [%lx %lx] in loop %d overlaps with existing loop %d "
            "%s[%d]\n",
      ...
      // if the block has a call to an unseen function, add it for next
      // iteration
    ...
    for (vector<BPatch_point *>::iterator pIter = unresolvedCF.begin();
      ...
      if (1 != targs.size()) {
        ...
        hasUnresolved = true;
        mal_printf(
            "loop %d calls func %lx which has an indirect "
    ...
    if (ParseAPI::RETURN != (*fIter)->lowlevel_func()->ifunc()->retstatus()) {
      mal_printf(
          "loop %d calls func %lx which could be "
          "non-returning %s[%d]\n",
  ...
  if (!hasUnresolved && nextAddFuncs.size()) {
    if (false ==
        addFuncBlocks(loop, nextAddFuncs, seenFuncs, overlappingLoops)) {
      hasUnresolved = true;
    }
  }
...
bool HybridAnalysisOW::setLoopBlocks(owLoop *loop,
  ...
  for (vector<BPatch_basicBlock *>::iterator bIter = loopBlocks.begin();
    ...
    for (vector<BPatch_point *>::iterator pIter = blockPoints.begin();
      ...
      if ((*pIter)->isDynamic()) {
        ...
        assert(targs.size() == 1);
        mal_printf(
            "loop %d has a resolved indirect transfer at %lx with "
            "target %lx\n",
  ...
  if (!loopFuncs.empty()) {
    ...
    if (false ==
        addFuncBlocks(loop, loopFuncs, loopFuncCopy, overlappingLoops)) {
...
bool HybridAnalysisOW::removeOverlappingLoops(owLoop *, std::set<int> &) {
  return false;
}
...
bool HybridAnalysisOW::removeOverlappingLoops(owLoop *loop,
  ...
  if (isSuperset) {
    ...
    for (lIter = overlappingLoops.begin(); lIter != overlappingLoops.end();
         lIter++) {
    ...
    if (!loop->shadowMap.empty()) {
      ...
      std::map<Address, unsigned char *>::iterator siter =
...
void HybridAnalysisOW::makeShadow_setRights(
    Address
        pageAddr,  // addr on the page, not necessarily the start of the page
    owLoop *loop) {
...
bool HybridAnalysisOW::isRealStore(Address insnAddr, block_instance *block,
  ...
  for (std::vector<Assignment::Ptr>::const_iterator a_iter =
           assignments.begin();
 ...
 *     an adjacent page or is in a subsequent iteration of the instrumented
loop:
 ...
 * . Restore write permissions to the written page
 */
void HybridAnalysisOW::overwriteSignalCB(Address faultInsnAddr,
                                         Address writeTarget) {
  using namespace std;
  // debugging output
  ...
  for (unsigned fidx = 0; fidx < faultFuncs.size(); fidx++) {
    faultBlocks.push_back(
        faultFuncs[fidx]->getCFG()->findBlockByAddr(faultInsnAddr));
  }
  ...
  if (loop) {
    ...
    if (loop->isRealLoop() && pageAddress <= faultInsnAddr &&
      ...
      mal_printf(
          "discovered that loop %d writes to one of its code pages, "
  ...
  BPatch_basicBlockLoop *bblLoop =
      getWriteLoop(*faultFuncs[0], faultInsnAddr, true);

  if (bblLoop) {
    // set loop blocks, returns false if there's indirect control flow in the
    // loop,
    ...
    if (1 == faultFuncs.size() &&
      ...
      if (pageAddress <= faultInsnAddr &&
        ...
        for (unsigned bidx = 0; bidx < faultBlocks.size(); bidx++) {
          ...
          if (writeTarget >= faultBlocks[bidx]->getStartAddress() &&
            ...
            assert(0 &&
                   "KEVINTEST: test what happens when "
      ...
      mal_printf(
          "Found loop, but it contains unresolved control flow "
  ...
  if (!bblLoop) {
    mal_printf(
        "Instrumenting after overwrite instruction at %lx in block "
        "[%lx %lx), loopID=%d %s[%d]\n",
        faultInsnAddr, faultBlocks[0]->getStartAddress(),
        faultBlocks[0]->getEndAddress(), loop->getID(), FILE__, __LINE__);
    assert(blockToLoop.find(faultBlocks[0]->getStartAddress()) ==
           blockToLoop.end());
    for (unsigned bidx = 0; bidx < faultBlocks.size(); bidx++) {
    ...
    mal_printf(
        "Instrumenting loop for write at %lx, loop has %d blocks, "
        "loopID=%d %s[%d]\n",
...
bool HybridAnalysisOW::activeOverwritePages(std::set<Address> &pages) {
  ...
  for (vector<HybridAnalysisOW::owLoop *>::iterator lit = active.begin();
       lit != active.end(); lit++) {
    for (map<Address, unsigned char *>::iterator pit =
             (*lit)->shadowMap.begin();
