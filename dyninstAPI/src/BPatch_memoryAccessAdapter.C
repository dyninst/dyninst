...
BPatch_memoryAccessAdapter::convert(Instruction insn, Address current, bool is64)
{
            ...
            if(first)
            {
                if(mac.prefetch)
                {
                    if(mac.prefetchlvl > 0)  // Intel
                        bmap = new BPatch_memoryAccess(
                            new internal_instruction(NULL), current, false, false,
                            mac.imm, mac.regs[0], mac.regs[1], mac.scale, 0, -1, -1, 0,
                    ...
                    switch(mac.sizehack)
                    {  // translation to pseudoregisters
                        case 0:
                            bmap = new BPatch_memoryAccess(
                                new internal_instruction(NULL), current, mac.read,
                                mac.write, mac.size, mac.imm, mac.regs[0], mac.regs[1],
                                mac.scale, bmapcond, mac.nt);
                            break;
                        case shREP:  // use ECX register to compute final size as mac.size
                                     // * ECX
                            bmap = new BPatch_memoryAccess(
                                new internal_instruction(NULL), current, mac.read,
                                mac.write, mac.imm, mac.regs[0], mac.regs[1], mac.scale,
                                0, -1, 1, log2[mac.size], bmapcond, false);
                            break;
                        case shREPESCAS:
                            bmap = new BPatch_memoryAccess(
                                new internal_instruction(NULL), current, mac.read,
                                mac.write, mac.imm, mac.regs[0], mac.regs[1], mac.scale,
                                0, -1, IA32_ESCAS, log2[mac.size], bmapcond, false);
                            break;
                        case shREPNESCAS:
                            bmap = new BPatch_memoryAccess(
                                new internal_instruction(NULL), current, mac.read,
                                mac.write, mac.imm, mac.regs[0], mac.regs[1], mac.scale,
                                0, -1, IA32_NESCAS, log2[mac.size], bmapcond, false);
                            break;
                        case shREPECMPS:
                            bmap = new BPatch_memoryAccess(
                                new internal_instruction(NULL), current, mac.read,
                                mac.write, mac.imm, mac.regs[0], mac.regs[1], mac.scale,
                                0, -1, IA32_ECMPS, log2[mac.size], bmapcond, false);
                            break;
                        case shREPNECMPS:
                            bmap = new BPatch_memoryAccess(
                                new internal_instruction(NULL), current, mac.read,
                                mac.write, mac.imm, mac.regs[0], mac.regs[1], mac.scale,
                                0, -1, IA32_NECMPS, log2[mac.size], bmapcond, false);
                            break;
                        default:
                ...
                {  // translation to pseudoregisters
                    case 0:
                        bmap->set2nd(mac.read, mac.write, mac.size, mac.imm, mac.regs[0],
                                     mac.regs[1], mac.scale);
                        break;
                    case shREP:  // use ECX register to compute final size as mac.size *
                                 // ECX
                        bmap->set2nd(mac.read, mac.write, mac.imm, mac.regs[0],
                                     mac.regs[1], mac.scale, 0, -1, 1, log2[mac.size],
                                     bmapcond, false);
                        break;
                    case shREPESCAS:
                    case shREPNESCAS:
                    ...
                    case shREPECMPS:
                        bmap->set2nd(mac.read, mac.write, mac.imm, mac.regs[0],
                                     mac.regs[1], mac.scale, 0, -1, IA32_ECMPS,
                                     log2[mac.size], bmapcond, false);
                        break;
                    case shREPNECMPS:
                        // fprintf(stderr, "In set2nd[shREPNECMPS]!!!\n");
                        bmap->set2nd(mac.read, mac.write, mac.imm, mac.regs[0],
                                     mac.regs[1], mac.scale, 0, -1, IA32_NECMPS,
                                     log2[mac.size], bmapcond, false);
                        break;
                    default:
                        assert(!"Unknown size hack");
    ...
    for(std::vector<Operand>::iterator op = operands.begin(); op != operands.end(); ++op)
    {
        ...
        if(isLoad || isStore)
        {
            ...
            {
                RegisterAST::Ptr byteOverride = boost::dynamic_pointer_cast<RegisterAST>(
                    insn.getOperand(0).getValue());
                assert(byteOverride);
            ...
            {
                return new BPatch_memoryAccess(new internal_instruction(NULL), current,
                                               isLoad, isStore, (long) 0, ra, rb,
                                               (long) 0, 9999, -1);
            }
            return new BPatch_memoryAccess(new internal_instruction(NULL), current,
                                           isLoad, isStore, bytes, imm, ra, rb);
        }
    }
    return NULL;
    ...
    for(std::vector<Operand>::iterator op = operands.begin(); op != operands.end(); ++op)
    {
        ...
        if(isLoad || isStore)
        {
            ...
            // insn.format().c_str(),op->getValue()->format().c_str()); fprintf(stderr,
            // "imm: %d, ra: %d, rb: %d, scale: %d\n", imm, ra, rb, sc);

            return new BPatch_memoryAccess(new internal_instruction(NULL), current,
                                           isLoad, isStore, bytes, imm, ra, rb, sc);
        }
    }
    return NULL;
#else
...
void
BPatch_memoryAccessAdapter::visit(Dereference* d)
{
    bytes = d->size();
}
...
void
BPatch_memoryAccessAdapter::visit(Immediate* i)
{
...
void
BPatch_memoryAccessAdapter::visit(BinaryFunction* b)
{
