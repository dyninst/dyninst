...
BPatch_memoryAccess* BPatch_memoryAccessAdapter::convert(Instruction::Ptr insn,
  ...
  const unsigned char* addr =
      reinterpret_cast<const unsigned char*>(insn->ptr());
    ...
    if (mac.is) {
      ...
      if (first) {
        if (mac.prefetch) {
          if (mac.prefetchlvl > 0)  // Intel
            bmap = new BPatch_memoryAccess(
                new internal_instruction(NULL), current, false, false, mac.imm,
                mac.regs[0], mac.regs[1], mac.scale, 0, -1, -1, 0, bmapcond,
                false, mac.prefetchlvl);
          else  // AMD
            bmap = new BPatch_memoryAccess(
                new internal_instruction(NULL), current, false, false, mac.imm,
                mac.regs[0], mac.regs[1], mac.scale, 0, -1, -1, 0, bmapcond,
                false, mac.prefetchstt + IA32AMDprefetch);
        } else
          switch (mac.sizehack) {  // translation to pseudoregisters
            case 0:
              bmap = new BPatch_memoryAccess(
                  new internal_instruction(NULL), current, mac.read, mac.write,
            ...
            case shREP:  // use ECX register to compute final size as mac.size *
              ...
              bmap = new BPatch_memoryAccess(
                  new internal_instruction(NULL), current, mac.read, mac.write,
                  mac.imm, mac.regs[0], mac.regs[1], mac.scale, 0, -1, 1,
                  log2[mac.size], bmapcond, false);
              break;
            case shREPESCAS:
              bmap = new BPatch_memoryAccess(
                  new internal_instruction(NULL), current, mac.read, mac.write,
                  mac.imm, mac.regs[0], mac.regs[1], mac.scale, 0, -1,
                  IA32_ESCAS, log2[mac.size], bmapcond, false);
              break;
            case shREPNESCAS:
              bmap = new BPatch_memoryAccess(
                  new internal_instruction(NULL), current, mac.read, mac.write,
                  mac.imm, mac.regs[0], mac.regs[1], mac.scale, 0, -1,
                  IA32_NESCAS, log2[mac.size], bmapcond, false);
              break;
            case shREPECMPS:
              bmap = new BPatch_memoryAccess(
                  new internal_instruction(NULL), current, mac.read, mac.write,
                  mac.imm, mac.regs[0], mac.regs[1], mac.scale, 0, -1,
                  IA32_ECMPS, log2[mac.size], bmapcond, false);
              break;
            case shREPNECMPS:
              bmap = new BPatch_memoryAccess(
                  new internal_instruction(NULL), current, mac.read, mac.write,
                  mac.imm, mac.regs[0], mac.regs[1], mac.scale, 0, -1,
                  IA32_NECMPS, log2[mac.size], bmapcond, false);
              break;
            default:
        ...
        switch (mac.sizehack) {  // translation to pseudoregisters
          ...
          case shREP:  // use ECX register to compute final size as mac.size *
                       // ECX
            bmap->set2nd(mac.read, mac.write, mac.imm, mac.regs[0], mac.regs[1],
                         mac.scale, 0, -1, 1, log2[mac.size], bmapcond, false);
            break;
          case shREPESCAS:
          case shREPNESCAS:
          ...
          case shREPECMPS:
            bmap->set2nd(mac.read, mac.write, mac.imm, mac.regs[0], mac.regs[1],
                         mac.scale, 0, -1, IA32_ECMPS, log2[mac.size], bmapcond,
                         false);
            break;
          case shREPNECMPS:
            // fprintf(stderr, "In set2nd[shREPNECMPS]!!!\n");
            bmap->set2nd(mac.read, mac.write, mac.imm, mac.regs[0], mac.regs[1],
                         mac.scale, 0, -1, IA32_NECMPS, log2[mac.size],
                         bmapcond, false);
            break;
          default:
            assert(!"Unknown size hack");
    ...
    if (isLoad || isStore) {
      ...
      if (insn->getOperation().getID() == power_op_lmw ||
        ...
        RegisterAST::Ptr byteOverride =
            boost::dynamic_pointer_cast<RegisterAST>(
      ...
      if (insn->getOperation().getID() == power_op_lswi ||
        ...
        Immediate::Ptr byteOverride = boost::dynamic_pointer_cast<Immediate>(
            insn->getOperand(2).getValue());
        assert(byteOverride);
...
void BPatch_memoryAccessAdapter::visit(Dereference* d) { bytes = d->size(); }
...
void BPatch_memoryAccessAdapter::visit(RegisterAST* r) {
  ...
  if ((ra == -1) && !setImm) {
    ...
    fprintf(stderr,
            "ASSERT: only two registers used in a power load/store calc!\n");
    assert(0);
  }
}
