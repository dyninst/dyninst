...
BPatch_localVarCollection::~BPatch_localVarCollection()
{
    for(auto iter = localVariablesByName.begin(); iter != localVariablesByName.end();
        ++iter)
    {
        delete iter->second;
    }
...
void
BPatch_localVarCollection::addLocalVar(BPatch_localVar* var)
{
 ...
 */
BPatch_localVar*
BPatch_localVarCollection::findLocalVar(const char* name)
{
    auto iter = localVariablesByName.find(name);
    if(iter == localVariablesByName.end())
        return NULL;
    return iter->second;
}
...
BPatch_typeCollection::getGlobalTypeCollection()
{
...
BPatch_typeCollection*
BPatch_typeCollection::getModTypeCollection(BPatch_module* bpmod)
{
    assert(bpmod);
...
void
BPatch_typeCollection::freeTypeCollection(BPatch_typeCollection* tc)
{
    assert(tc);
    tc->refcount--;
    if(tc->refcount == 0)
 ...
 */
BPatch_type*
BPatch_typeCollection::findType(const char* name)
{
...
BPatch_type*
BPatch_typeCollection::findTypeLocal(const char* name)
{
    auto iter = typesByName.find(name);
    if(iter != typesByName.end())
        return iter->second;
...
BPatch_type*
BPatch_typeCollection::findTypeLocal(const int& ID)
{
...
BPatch_type*
BPatch_typeCollection::findOrCreateType(const int& ID)
{
    auto iter = typesByID.find(ID);
    if(iter != typesByID.end())
    {
...
BPatch_type*
BPatch_typeCollection::addOrUpdateType(BPatch_type* type)
{
...
BPatch_type*
BPatch_typeCollection::findType(const int& ID)
{
    auto iter = typesByID.find(ID);
    if(iter != typesByID.end())
        return iter->second;
 ...
 */
BPatch_type*
BPatch_typeCollection::findVariableType(const char* name)
{
 ...
 */
void
BPatch_typeCollection::addType(BPatch_type* type)
{
    if(type->getName() != NULL)
...
void
BPatch_typeCollection::clearNumberedTypes()
{
 ...
 */
BPatch_type*
BPatch_builtInTypeCollection::findBuiltInType(const char* name)
{
    auto iter = builtInTypesByName.find(name);
    if(iter != builtInTypesByName.end())
        return iter->second;
    return NULL;
}

BPatch_type*
BPatch_builtInTypeCollection::findBuiltInType(const int& ID)
{
    auto iter = builtInTypesByID.find(ID);
    if(iter != builtInTypesByID.end())
        return iter->second;
    return NULL;
}

void
BPatch_builtInTypeCollection::addBuiltInType(BPatch_type* type)
{
    if(type->getName() != NULL)
    {  // Type could have no name.
