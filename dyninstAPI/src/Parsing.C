...
public:
    ...
    : Dyninst::SymtabAPI::Function(re.getDynSym())
    , r{ re }
    {}

    std::string getName() const override { return r.name(); }

    Offset getOffset() const override { return r.target_addr(); }

    unsigned int getSize() const override { return 0; }

    SymtabAPI::Module* getModule() const override { return nullptr; }
};

Function*
DynCFGFactory::mkfunc(Address addr, FuncSource src, std::string name, CodeObject* obj,
                      CodeRegion* reg, InstructionSource* isrc)
{
    _mtx.lock();
    parse_func*          ret;
    SymtabAPI::Symtab*   st;
...
void
DynParseCallback::destroy_cb(Block* b)
{
    _img->destroy(b);
}

void
DynParseCallback::destroy_cb(Edge* e)
{
    _img->destroy(e);
}

void
DynParseCallback::destroy_cb(Function* f)
{
    _img->destroy(f);
}

void
DynParseCallback::remove_edge_cb(ParseAPI::Block*, ParseAPI::Edge*, edge_type_t)
{
    // cerr << "Warning: edge removal callback unimplemented" << endl;
}

void
DynParseCallback::remove_block_cb(ParseAPI::Function*, ParseAPI::Block*)
{
...
void
DynParseCallback::interproc_cf(Function* f, Block* b, Address /*addr*/,
...
{  // calls function that updates bytes if needed
    ...
    return codeBytesUpdateCB(_img->cb_arg0(), target_ + _img->desc().code());
}
...
void
DynParseCallback::split_block_cb(ParseAPI::Block* first, ParseAPI::Block* second)
{
