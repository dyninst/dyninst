...
void defaultErrorFunc(BPatchErrorLevel level, int num,
...
BPatch::~BPatch() {
  ...
  if (stdTypes) BPatch_typeCollection::freeTypeCollection(stdTypes);
  if (APITypes) BPatch_typeCollection::freeTypeCollection(APITypes);
  if (builtInTypes) delete builtInTypes;
...
BPatch *BPatch::getBPatch() { return bpatch; }

char *BPatch::getPrelinkCommand() { return systemPrelinkCommand; }
...
bool BPatch::isTypeChecked() { return typeCheckOn; }
void BPatch::setTypeChecking(bool x) { typeCheckOn = x; }
bool BPatch::parseDebugInfo() { return debugParseOn; }
bool BPatch::delayedParsingOn() { return delayedParsing_; }
void BPatch::setDebugParsing(bool x) { debugParseOn = x; }
bool BPatch::baseTrampDeletion() { return baseTrampDeletionOn; }
void BPatch::setBaseTrampDeletion(bool x) { baseTrampDeletionOn = x; }
bool BPatch::isTrampRecursive() { return trampRecursiveOn; }
void BPatch::setTrampRecursive(bool x) { trampRecursiveOn = x; }
void BPatch::setLivenessAnalysis(bool x) { livenessAnalysisOn_ = x; }
bool BPatch::livenessAnalysisOn() { return livenessAnalysisOn_; }

void BPatch::setLivenessAnalysisDepth(int x) { livenessAnalysisDepth_ = x; }
int BPatch::livenessAnalysisDepth() { return livenessAnalysisDepth_; }

bool BPatch::hasForcedRelocation_NP() { return forceRelocation_NP; }
void BPatch::setForcedRelocation_NP(bool x) { forceRelocation_NP = x; }
bool BPatch::autoRelocationOn() { return autoRelocation_NP; }
void BPatch::setAutoRelocation_NP(bool x) { autoRelocation_NP = x; }
void BPatch::setDelayedParsing(bool x) { delayedParsing_ = x; }
bool BPatch::isMergeTramp() { return true; }
void BPatch::setMergeTramp(bool) {}

bool BPatch::isSaveFPROn() { return saveFloatingPointsOn; }
void BPatch::setSaveFPR(bool x) { saveFloatingPointsOn = x; }

bool BPatch::isForceSaveFPROn() { return forceSaveFloatingPointsOn; }
void BPatch::forceSaveFPR(bool x) { forceSaveFloatingPointsOn = x; }
...
BPatchErrorCallback BPatch::registerErrorCallback(
    BPatchErrorCallback function) {
 ...
 * func	The function to be called.
 */
BPatchOneTimeCodeCallback BPatch::registerOneTimeCodeCallback(
 ...
 * function	The function to be called.
 */
BPatchDynLibraryCallback BPatch::registerDynLibraryCallback(
    BPatchDynLibraryCallback function) {
 ...
 */
const char *BPatch::getEnglishErrorString(int /* number */) { return "%s"; }
 ...
 */
void BPatch::reportError(BPatchErrorLevel severity, int number,
                         const char *str) {
  if (bpatch == NULL) {
 ...
 *		function.
 */
void BPatch::formatErrorString(char *dst, int size, const char *fmt,
                               const char *const *params) {
  int cur_param = 0;
...
BPatch_Vector<BPatch_process *> *BPatch::getProcesses() {
  ...
  for (auto iter = info->procsByPid.begin(); iter != info->procsByPid.end();
       ++iter) {
 ...
 *
 */
void BPatch::registerForkedProcess(PCProcess *parentProc,
                                   PCProcess *childProc) {
...
void BPatch::registerExecExit(PCProcess *proc) {
  ...
  if (process->image) process->image->removeAllModules();
...
void BPatch::registerNormalExit(PCProcess *proc, int exitcode) {
  ...
  // We now run the process out; set its state to terminated. Really, the user
  // shouldn't
...
void BPatch::registerDynamicCallsiteEvent(BPatch_process *process,
                                          Address callTarget,
                                          Address callAddr) {
  ...
  if (point == NULL) {
    proccontrol_printf(
        "%s[%d]: failed to find point for dynamic callsite event\n", FILE__,
        __LINE__);
    return;
  }
  ...
  BPatch_function *bpatchTargetFunc =
      process->findOrCreateBPFunc(targetFunc, NULL);
...
static void buildPath(const char *path, const char **argv, char *&pathToUse,
                      char **&argvToUse) {
  ifstream file;
  file.open(path);
...
BPatch_process *BPatch::processCreate(const char *path, const char *argv[],
  ...
  BPatch_process *ret = new BPatch_process(
      (pathToUse ? pathToUse : path),
      (argvToUse ? (const_cast<const char **>(argvToUse)) : argv), mode, envp,
  ...
  if (!ret->llproc || !ret->llproc->isStopped() ||
 ...
 */
BPatch_process *BPatch::processAttach(const char *path, int pid,
                                      BPatch_hybridMode mode) {
  clearError();

  if (!OS_isConnected()) {
    reportError(BPatchFatal, 68,
                "Error: Attempted to attach to process before connected to "
                "target server.");
    return NULL;
  }
...
BPatch_type *BPatch::createStruct(const char *name,
  ...
  for (i = 0; i < fieldNames.size(); i++) {
    ...
    fields.push_back(new pair<string, Type *>(fieldNames[i],
                                              fieldTypes[i]->getSymtabType()));
  }
...
BPatch_binaryEdit *BPatch::openBinary(const char *path,
...
void BPatch::setInstrStackFrames(bool r) { instrFrames = r; }

bool BPatch::getInstrStackFrames() { return instrFrames; }

bool BPatch::isConnected() { return OS_isConnected(); }
...
bool BPatch::getPidList(BPatch_remoteHost &remote,
                        BPatch_Vector<unsigned int> &pidlist) {
  if (remote.type >= BPATCH_REMOTE_DEBUG_END) {
    fprintf(stderr, "Unknown remote debugging protocol %d\n", remote.type);
    return false;
  }
...
bool BPatch_libInfo::registerMonitoredPoint(BPatch_point *point) {
  if (monitoredPoints_.find((Address)point->getAddress()) !=
      monitoredPoints_.end())
    return false;
