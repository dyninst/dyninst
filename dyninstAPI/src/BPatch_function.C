 ...
 */
bool
BPatch_function::getSourceObj(BPatch_Vector<BPatch_sourceObj*>& children)
{
...
bool
BPatch_function::getNames(BPatch_Vector<const char*>& names)
{
    assert(func);
...
bool
BPatch_function::getMangledNames(BPatch_Vector<const char*>& names)
{
    assert(func);
    unsigned pre_size = names.size();
...
BPatch_function::removeInstrumentation(bool useInsertionSet)
{
    ...
    {
        vector<BPatchSnippetHandle*> allSnippets = points[pidx]->getCurrentSnippets();
...
void
BPatch_function::getUnresolvedControlTransfers(
...
void
BPatch_function::getAbruptEndPoints(BPatch_Vector<BPatch_point*>& abruptEnds /*output*/)
{
...
BPatch_function::getCallerPoints(std::vector<BPatch_point*>& callerPoints)
{
        ...
        iter != callerBlocks.end(); ++iter)
    {
            ...
            fit != callerFuncs.end(); fit++)
        {
            ...
            BPatch_point*    curPoint =
                addSpace->findOrCreateBPPoint(callerFunc, point, BPatch_locSubroutine);
...
void
BPatch_function::getCallPoints(BPatch_Vector<BPatch_point*>& callPoints)
{
...
void
BPatch_function::getEntryPoints(BPatch_Vector<BPatch_point*>& entryPoints /*output*/)
{
    BPatch_point* curPoint =
        addSpace->findOrCreateBPPoint(this, instPoint::funcEntry(func), BPatch_locEntry);
    entryPoints.push_back(curPoint);
}

void
BPatch_function::getExitPoints(BPatch_Vector<BPatch_point*>& exitPoints)
{
...
void
BPatch_function::getAllPoints(std::vector<BPatch_point*>& bpPoints)
{
...
BPatch_Vector<BPatch_localVar*>*
BPatch_function::getParams()
{
    if(!mod->isValid())
        return NULL;
    constructVarsAndParams();
    // Do *not* hand back the alphabetical locals collection; hand back the params in the
...
BPatch_function::findPoint(const BPatch_procedureLocation loc)
{
 ...
 */
void
BPatch_function::addParam(Dyninst::SymtabAPI::localVar* lvar)
{
 ...
 */
BPatch_localVar*
BPatch_function::findLocalVar(const char* name)
{
    if(!mod->isValid())
        return NULL;
    constructVarsAndParams();
 ...
 */
BPatch_localVar*
BPatch_function::findLocalParam(const char* name)
{
    if(!mod->isValid())
        return NULL;
    constructVarsAndParams();
...
BPatch_function::getCFG()
{
    ...
    if(!valid)
    {
        ...
        fprintf(stderr, "CFG is NULL for func %s at %lx %s[%d]\n",
                lowlevel_func()->symTabName().c_str(), func->addr(), FILE__, __LINE__);
        return NULL;
    }
    return cfg;
}
...
void
BPatch_function::constructVarsAndParams()
{
...
bool
BPatch_function::findVariable(const char* name, BPatch_Vector<BPatch_variableExpr*>& vars)
{
    if(!mod->isValid())
        return false;
    ...
    if(lv)
    {
        ...
        BPatch_variableExpr* nv = new BPatch_variableExpr(
            imgPtr->getAddressSpace(), func->proc(), lv, lv->getType(), (*points)[0]);
        vars.push_back(nv);
        return true;
    }
...
BPatch_function::findVariable(const char* name)
{
...
bool
BPatch_function::getVariables(BPatch_Vector<BPatch_variableExpr*>& /*vect*/)
{
    return false;
}
...
void
BPatch_function::fixupUnknown(BPatch_module* module)
{
...
const char*
BPatch_function::addName(const char* name, bool isPrimary, /* = true */
                         bool isMangled)
{ /* = false */
...
bool
BPatch_function::findOverlapping(BPatch_Vector<BPatch_function*>& funcs)
{
    assert(func);
    assert(addSpace);
...
Dyninst::ParseAPI::convert(const BPatch_function* f)
{
    return f->func->ifunc();
}

Dyninst::PatchAPI::PatchFunction*
Dyninst::PatchAPI::convert(const BPatch_function* f)
{
    return f->func;
}
...
bool
BPatch_function::getSharedFuncs(set<BPatch_function*>& sharedFuncs)
{
    std::set<func_instance*> ishared;
    if(!func->getSharingFuncs(ishared))
        return false;
    for(std::set<func_instance*>::iterator iter = ishared.begin(); iter != ishared.end();
        ++iter)
    {
...
bool
BPatch_function::addMods(std::set<StackMod*> mods)
{
#if defined(cap_stack_mods)
