...
// constructor
BPatch_sourceBlock::BPatch_sourceBlock() : sourceFile(NULL) {}

// constructor
BPatch_sourceBlock::BPatch_sourceBlock(const char *filePtr,
                                       std::set<unsigned short> &lines)
    : sourceFile(filePtr), sourceLines(lines) {}

const char *BPatch_sourceBlock::getSourceFile() { return sourceFile; }

void BPatch_sourceBlock::getSourceLines(BPatch_Vector<unsigned short> &lines) {
  std::copy(sourceLines.begin(), sourceLines.end(), std::back_inserter(lines));
}

#ifdef IBM_BPATCH_COMPAT
bool BPatch_sourceBlock::getAddressRange(void *&_startAddress,
                                         void *&_endAddress) {
  return false;
}
