...
std::string parse_func::calcParentFunc(const parse_func *imf,
  ...
  if (strSize > 0) {
    ...
    return tempPDS;
  } else /* if it starts with @ just return the full function as its parent,
...
void parse_func::parseOMP(image_parRegion *parReg, parse_func *parentFunc,
  ...
  int lastSecSize =
      0;  // the number of sections in the most recent sections construct
  int totalSectionGroups =
      0;  // the number of sections constructs encountered (in source),
  ...
  for (unsigned a = 0; a < parentFunc->parRegions().size(); a++) {
    ...
    /* For the sections, we can't just count the number of section constructs
       we run into, since multiple outlined functions line up to one section
       construct */
  ...
  // if the currentSectionNum is not zero, it means there are still more
  // outlined functions
  // for a single section construct still out there, so we don't count the
  ...
  // we need to parse the parent function to get all the information about the
  // region, mostly for worksharing constructs
  bool hasLoop = parentFunc->parseOMPParent(
      parReg, desiredNum + totalSectionGroups, currentSectionNum);

  // we parse the outlined function to look for inlined constructs like "Master"
  // and "Ordered"
  parseOMPFunc(hasLoop);
}

void parse_func::parseOMPFunc(bool /*hasLoop*/) {
  if (OMPparsed_) return;
  OMPparsed_ = true;

  /* We parse the parent to get info if we are in an outlined function, but
     there can be some
     inlined functions we might miss out on if we don't check those out too */
  int regValues[10 + 1]; /* Only care about registers 3-10 (params) */
  for (int i = 0; i < 11; i++) regValues[i] = -1;
...
bool BinaryEdit::doStaticBinarySpecialCases() {
  ...
  func_instance *globalCtorHandler =
      mobj->findGlobalConstructorFunc(LIBC_CTOR_HANDLER);
  ...
  func_instance *globalDtorHandler =
      mobj->findGlobalDestructorFunc(LIBC_DTOR_HANDLER);
  ...
  if (!replaceHandler(globalCtorHandler, dyninstCtorHandler, &ctorsListInt,
                      SYMTAB_CTOR_LIST_REL)) {
    ...
    inst_printf("%s[%d]: replaced ctor function %s with %s\n", FILE__, __LINE__,
                LIBC_CTOR_HANDLER.c_str(), DYNINST_CTOR_HANDLER.c_str());
  }

  if (!replaceHandler(globalDtorHandler, dyninstDtorHandler, &dtorsListInt,
                      SYMTAB_DTOR_LIST_REL)) {
    ...
    inst_printf("%s[%d]: replaced dtor function %s with %s\n", FILE__, __LINE__,
                LIBC_DTOR_HANDLER.c_str(), DYNINST_DTOR_HANDLER.c_str());
  }
   ...
   * scenario is not supported with the initial release of the binary rewriter
   * for
   * static binaries.
   *
...
// Search for last of a fixed number of calls
  ...
  const unsigned char *p =
      reinterpret_cast<const unsigned char *>(initRegion->getPtrToRawData());
  ...
  if (!ctorAddress ||
      !parse_img()->codeObject()->cs()->isValidAddress(ctorAddress)) {
  ...
  if ((ret = findFuncByEntry(ctorAddress)) == NULL) {
    logLine(
        "unable to create representation for global constructor function\n");
    return NULL;
  }

  inst_printf("%s[%d]: set global constructor address to 0x%lx\n", FILE__,
              __LINE__, ctorAddress);
...
func_instance *mapped_object::findGlobalDestructorFunc(
  ...
  unsigned bytesSeen = 0;
  const unsigned char *p =
      reinterpret_cast<const unsigned char *>(finiRegion->getPtrToRawData());
  ...
  if (!dtorAddress ||
      !parse_img()->codeObject()->cs()->isValidAddress(dtorAddress)) {
