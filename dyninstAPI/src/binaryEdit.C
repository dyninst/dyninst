...
bool
BinaryEdit::readTextSpace(const void* inOther, u_int size, void* inSelf)
{
...
bool
BinaryEdit::writeTextSpace(void* inOther, u_int size, const void* inSelf)
{
    ...
    while(to_do)
    {
        ...
        void* local_ptr = ((void*) (offset + (Address) range->get_local_ptr()));
        inst_printf("Copying to 0x%p [base=0x%p] from 0x%lx (%u bytes)  target=0x%lx  "
                    "offset=0x%lx\n",
...
bool
BinaryEdit::readDataSpace(const void* inOther, u_int amount, void* inSelf, bool)
{
    return readTextSpace(inOther, amount, inSelf);
}

bool
BinaryEdit::writeDataSpace(void* inOther, u_int amount, const void* inSelf)
{
    return writeTextSpace(inOther, amount, inSelf);
}

bool
BinaryEdit::readTextWord(const void* inOther, u_int size, void* inSelf)
{
    return readTextSpace(inOther, size, inSelf);
}

bool
BinaryEdit::writeTextWord(void* inOther, u_int size, const void* inSelf)
{
    return writeTextSpace(inOther, size, inSelf);
}

bool
BinaryEdit::readDataWord(const void* inOther, u_int amount, void* inSelf, bool)
{
    return readTextSpace(inOther, amount, inSelf);
}

bool
BinaryEdit::writeDataWord(void* inOther, u_int amount, const void* inSelf)
{
    return writeTextSpace(inOther, amount, inSelf);
}
...
Address
BinaryEdit::inferiorMalloc(unsigned size, inferiorHeapType /*ignored*/, Address /*near*/,
                           bool*    err)
{
...
void
BinaryEdit::inferiorFree(Address item)
{
...
bool
BinaryEdit::inferiorRealloc(Address item, unsigned newsize)
{
...
Address
BinaryEdit::offset() const
{
    fprintf(stderr, "error BinaryEdit::offset() unimpl\n");
    return 0;
}
Address
BinaryEdit::length() const
{
...
bool
BinaryEdit::multithread_capable(bool)
{
    return multithread_capable_;
}

bool
BinaryEdit::multithread_ready(bool)
{
...
void
BinaryEdit::makeInitAndFiniIfNeeded()
{}

bool
BinaryEdit::archSpecificMultithreadCapable()
{
    return false;
}
#endif
...
bool
BinaryEdit::getStatFileDescriptor(const std::string& name, fileDescriptor& desc)
{
...
#if !defined(os_linux) && !defined(os_freebsd)
mapped_object*
BinaryEdit::openResolvedLibraryName(std::string                         filename,
                                    std::map<std::string, BinaryEdit*>& allOpened)
{
    /*
     * Note: this does not actually do any library name resolution, as that is
     * OS-dependent If resolution is required, it should be implemented in an OS-dependent
     * file (see linux.C for an example)
     *
     * However, this version allows the RT library to be opened with this function
     * regardless if library name resolution has been implemented on a platform.
     */
...
bool
BinaryEdit::getResolvedLibraryPath(const std::string&, std::vector<std::string>&)
{
    ...
    return false;
                                       ...
                                       defined(arch_power) || defined(arch_aarch64)))
bool
...
bool
BinaryEdit::isMultiThreadCapable()
{
...
bool
BinaryEdit::getAllDependencies(std::map<std::string, BinaryEdit*>& deps)
{
    Symtab*                 symtab = mobj->parse_img()->getObject();
...
bool
BinaryEdit::writeFile(const std::string& newFileName)
{
    // Step 1: changes.
    ...
    // unsigned long flags: these are a SymtabAPI abstraction. We're going with text|data
    // because
    //    we might have both.
    ...
    symObj->addRegion(lowWaterMark_, newSectionPtr, highWaterMark_ - lowWaterMark_,
                      ".dyninstInst", Region::RT_TEXTDATA, true);
    ...
    if(mobj == getAOut())
    {
        ...
        for(unsigned i = 0; i < dependentRelocations.size(); i++)
        {
            ...
            /*
              ...
              !symObj->hasReladyn()) { Address addr = referring->getOffset(); bool result
              = writeDataSpace((void *) to, getAddressWidth(), &addr); assert(result);
              continue;
              }
            */
            ...
            */
            ...
            /*
              ...
              Region::RT_RELA); } else { newSec->addRelocationEntry(to, newSymbol,
              relocationEntry::dynrel);
              }
            */
        }
    }
    ...
    // Symtab::addSection(Offset vaddr, void *data, unsigned int dataSize, std::string
    // name,
...
Address
BinaryEdit::maxAllocedAddr()
{
    inferiorFreeCompact();
    Address hi = lowWaterMark_;

    for(auto iter = heap_.heapActive.begin(); iter != heap_.heapActive.end(); ++iter)
    {
...
bool
BinaryEdit::inferiorMallocStatic(unsigned size)
{
...
bool
BinaryEdit::createMemoryBackingStore(mapped_object* obj)
{
    ...
    for(unsigned i = 0; i < regs.size(); i++)
    {
        ...
        {
            newTracker = new memoryTracker(regs[i]->getMemOffset(), regs[i]->getMemSize(),
...
void
BinaryEdit::addDependentRelocation(Address to, Symbol* referring)
{
...
Address
BinaryEdit::getDependentRelocationAddr(Symbol* referring)
{
    Address retAddr = 0x0;
    for(unsigned i = 0; i < dependentRelocations.size(); i++)
    {
...
void
BinaryEdit::addLibraryPrereq(std::string libname)
{
    Symtab* symObj = mobj->parse_img()->getObject();
...
void
BinaryEdit::buildDyninstSymbols(std::vector<Symbol*>& newSyms, Region* newSec,
                                Module* newMod)
{
...
mapped_object*
BinaryEdit::getMappedObject()
{
    return mobj;
}

void
BinaryEdit::setupRTLibrary(std::vector<BinaryEdit*>& r)
{
    rtlib = r;
...
func_instance*
BinaryEdit::findOnlyOneFunction(const std::string& name, const std::string& libname,
                                bool search_rt_lib)
{
...
void
BinaryEdit::setMultiThreadCapable(bool b)
{
    multithread_capable_ = b;
}

void
BinaryEdit::addSibling(BinaryEdit* be)
{
    if(this != be)
    {
...
std::vector<BinaryEdit*>&
BinaryEdit::getSiblings()
{
    return siblings;
}
...
void
BinaryEdit::addTrap(Address from, Address to, codeGen& gen)
{
    gen.invalidate();
    gen.allocate(4);
