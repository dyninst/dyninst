...
BPatch_flowGraph::~BPatch_flowGraph()
{
    if(loops)
    {
        for(std::set<BPatch_loop*>::iterator iter = loops->begin(); iter != loops->end();
            ++iter)
        {
            delete *iter;
        }
...
BPatch_flowGraph::getExitBasicBlock(BPatch_Vector<BPatch_basicBlock*>& nbb)
{
    /*   for (func_instance::BlockSet::const_iterator iter =
       ll_func()->exitBlocks().begin(); iter != ll_func()->exitBlocks().end(); ++iter) {
     */
...
bool
BPatch_flowGraph::createBasicBlocks()
{
    assert(ll_func());
    ...
    for(ibIter = iblocks.begin(); ibIter != iblocks.end(); ibIter++)
    {
        ...
        */
        ...
        // const block_instance::edgelist &srcs = (*ibIter)->sources();
        for(PatchBlock::edgelist::const_iterator iter = srcs.begin(); iter != srcs.end();
            ++iter)
        {
        ...
        for(PatchBlock::edgelist::const_iterator iter = trgs.begin(); iter != trgs.end();
            ++iter)
        {
            edge_instance* iedge = SCAST_EI(*iter);
            // Skip interprocedural edges
...
BPatch_flowGraph::createSourceBlocks()
{
        ...
        iter != allBlocks.end(); ++iter)
    {
        ...
        for(auto cur = insnsByAddr.begin(); cur != insnsByAddr.end(); ++cur)
        {
...
void
BPatch_flowGraph::fillDominatorInfo()
{
...
BPatch_flowGraph::dump()
{
        ...
        iter != allBlocks.end(); ++iter)
    {
        fprintf(stderr, "[%d 0x%p 0x%p]\n", (*iter)->blockNo(),
                (void*) ((*iter)->getStartAddress()), (void*) ((*iter)->getEndAddress()));
    }
}
...
func_instance*
BPatch_flowGraph::ll_func() const
{
    return func_->lowlevel_func();
}

AddressSpace*
BPatch_flowGraph::getllAddSpace() const
{
...
BPatch_basicBlock*
BPatch_flowGraph::findBlock(block_instance* inst)
{
...
BPatch_edge*
BPatch_flowGraph::findEdge(edge_instance* inst)
{
