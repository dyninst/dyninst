...
BPatch_flowGraph::BPatch_flowGraph(BPatch_function *func, bool &valid)
    : func_(func),
      addSpace(func->getAddSpace()),
      mod(func->getModule()),
      loops(NULL),
      loopRoot(NULL),
...
void BPatch_flowGraph::findLoopExitInstPoints(
    BPatch_loop *loop, BPatch_Vector<BPatch_point *> *points) {
...
BPatch_Vector<BPatch_point *> *BPatch_flowGraph::findLoopInstPoints(
    const BPatch_procedureLocation loc, BPatch_loop *loop) {
  /*
  ...
  switch (loc) {
    case BPatch_locLoopEntry: {
      ...
      if (0 == points->size()) {
        fprintf(stderr,
                "Warning: request to instrument loop entry "
    ...
    case BPatch_locLoopStartIter: {
      ...
      for (auto bit = entries.begin(); bit != entries.end(); ++bit) {
        ...
        BPatch_point *p = getAddSpace()->findOrCreateBPPoint(
            func_, instPoint::blockEntry((*bit)->ifunc(), llHead),
...
bool BPatch_flowGraph::getAllBasicBlocks(std::set<BPatch_basicBlock *> &abb) {
  std::copy(allBlocks.begin(), allBlocks.end(), std::inserter(abb, abb.end()));
  return true;
}

bool BPatch_flowGraph::getAllBasicBlocks(BPatch_Set<BPatch_basicBlock *> &abb) {
  std::set<BPatch_basicBlock *> tmp;
  getAllBasicBlocks(tmp);
  std::copy(tmp.begin(), tmp.end(),
            std::inserter(abb.int_set, abb.int_set.end()));
  return true;
}
...
bool BPatch_flowGraph::getEntryBasicBlock(
    BPatch_Vector<BPatch_basicBlock *> &ebb) {
...
bool BPatch_flowGraph::getExitBasicBlock(
    BPatch_Vector<BPatch_basicBlock *> &nbb) {
  /*   for (func_instance::BlockSet::const_iterator iter =
     ll_func()->exitBlocks().begin();
...
void BPatch_flowGraph::createLoops() {
  ...
  for (auto lit = patch_loops.begin(); lit != patch_loops.end(); ++lit) {
    ...
    // set parent pointer
    if (l->parent != NULL) pl->parent = _loop_map[l->parent];
    // set contained loop vector
    vector<PatchLoop *> containedLoops;
    l->getContainedLoops(containedLoops);
    for (auto lit2 = containedLoops.begin(); lit2 != containedLoops.end();
         ++lit2)
...
void BPatch_flowGraph::getLoopsByNestingLevel(BPatch_Vector<BPatch_loop *> &lbb,
  ...
  for (std::set<BPatch_loop *>::iterator iter = loops->begin();
    ...
    if (outerMostOnly && (*iter)->parent != NULL) continue;
...
bool BPatch_flowGraph::getLoops(BPatch_Vector<BPatch_basicBlockLoop *> &lbb) {
  getLoopsByNestingLevel(lbb, false);
  return true;
}

// get the outermost loops in this flow graph
bool BPatch_flowGraph::getOuterLoops(
...
bool BPatch_flowGraph::createBasicBlocks() {
  ...
  const PatchFunction::Blockset &iblocks = ll_func()->blocks();
  PatchFunction::Blockset::const_iterator ibIter;
  // for( unsigned int i = 0; i < iblocks.size(); i++ )
  for (ibIter = iblocks.begin(); ibIter != iblocks.end(); ibIter++) {
    ...
    for (block_instance::edgelist::const_iterator iter = srcs.begin(); iter !=
    srcs.end(); ++iter) {
    */
    const PatchBlock::edgelist &srcs = iblk->sources();
    // const block_instance::edgelist &srcs = (*ibIter)->sources();
    for (PatchBlock::edgelist::const_iterator iter = srcs.begin();
         iter != srcs.end(); ++iter) {
    ...
    for (PatchBlock::edgelist::const_iterator iter = trgs.begin();
         iter != trgs.end(); ++iter) {
      edge_instance *iedge = SCAST_EI(*iter);
      // Skip interprocedural edges
      if (iedge->interproc()) continue;
...
bool BPatch_flowGraph::createSourceBlocks() {
  ...
  for (std::set<BPatch_basicBlock *>::iterator iter = allBlocks.begin();
    ...
    for (std::vector<std::pair<Instruction::Ptr, Address> >::const_iterator
             cur = insnsByAddr.begin();
         cur != insnsByAddr.end(); ++cur) {
    ...
    if (lines.size() != 0) {
      ...
      for (unsigned int j = 0; j < lines.size(); j++) {
        ...
        // /* DEBUG */ fprintf( stderr, "%s[%d]: Inserting %s:", __FILE__,
        // __LINE__, currentSourceFile );
        // /* DEBUG */ std::set< unsigned short >::iterator iter =
        // lineNos.begin();
        // /* DEBUG */ for( ; iter != lineNos.end(); iter++ ) { fprintf( stderr,
        // " %d", * iter ); }
        // /* DEBUG */ fprintf( stderr, "\n" );
        BPatch_sourceBlock *sourceBlock =
            new BPatch_sourceBlock(currentSourceFile, lineNos);
...
void BPatch_flowGraph::fillDominatorInfo() {
  if (isDominatorInfoReady) return;
...
void BPatch_flowGraph::fillPostDominatorInfo() {
  if (isPostDominatorInfoReady) return;
...
pdpair<u_short, u_short> getLoopMinMaxSourceLines(BPatch_loop *loop) {
  BPatch_Vector<BPatch_basicBlock *> blocks;
  loop->getLoopBasicBlocks(blocks);
...
BPatch_loopTreeNode *BPatch_flowGraph::getLoopTree() {
  if (loopRoot == NULL) {
    ...
    loopRoot =
        new BPatch_loopTreeNode(this, ll_func()->getLoopTree(), _loop_map);
  }
  return loopRoot;
}
...
void BPatch_flowGraph::printLoops() { dfsPrintLoops(getLoopTree()); }
...
BPatch_basicBlock *BPatch_flowGraph::findBlock(block_instance *inst) {
  std::map<const block_instance *, BPatch_basicBlock *>::const_iterator iter =
      blockMap_.find(inst);
...
BPatch_edge *BPatch_flowGraph::findEdge(edge_instance *inst) {
  ...
  std::map<const edge_instance *, BPatch_edge *>::const_iterator iter =
      edgeMap_.find(inst);
