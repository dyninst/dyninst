...
BPatch_instruction::~BPatch_instruction() {
  ...
  if (insn_) delete insn_;
}

BPatch_basicBlock *BPatch_instruction::getParent() { return parent; }

void *BPatch_instruction::getAddress() { return (void *)addr; }
BPatch_point *BPatch_instruction::getInstPoint() {
  ...
  assert(bpproc);
  instPoint *point = instPoint::preInsn(ifunc, parent->block(), addr);

  BPatch_point *ret =
      bpproc->findOrCreateBPPoint(NULL, point, BPatch_locInstruction);
...
std::string BPatch_register::name() const { return name_; }
