...
inferiorHeap::inferiorHeap(const inferiorHeap &src) {
  ...
  for (auto iter = src.heapActive.begin(); iter != src.heapActive.end();
       ++iter) {
...
void inferiorHeap::clear() {
  ...
  for (unsigned i = 0; i < heapFree.size(); i++) delete heapFree[i];
  heapFree.clear();
  ...
  for (unsigned j = 0; j < bufferPool.size(); j++) delete bufferPool[j];
  bufferPool.clear();
}
