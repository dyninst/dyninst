...
extern int bpfatal_lf(const char *__file__, unsigned int __line__,
                      const char *format, ...);
...
// C++ prototypes
#define signal_cerr \
  if (dyn_debug_signal) cerr
#define startup_cerr \
  if (dyn_debug_startup) cerr
#define parsing_cerr \
  if (dyn_debug_parsing) cerr
#define proccontrol_cerr \
  if (dyn_debug_proccontrol) cerr
#define stackwalk_cerr \
  if (dyn_debug_stackwalk) cerr
#define relocation_cerr \
  if (dyn_debug_reloc) cerr
#define springboard_cerr \
  if (dyn_debug_springboard) cerr
#define malware_cerr \
  if (dyn_debug_malware) cerr
#define trap_cerr \
  if (dyn_debug_trap) cerr
#define sensitivity_cerr \
  if (dyn_debug_sensitivity) cerr
#define dyn_unw_cerr \
  if (dyn_debug_dyn_unw) cerr
#define thread_cerr \
  if (dyn_debug_thread) cerr
#define infmalloc_cerr \
  if (dyn_debug_infmalloc) cerr
#define crash_cerr \
  if (dyn_debug_crash) cerr
#define stackmods_cerr \
  if (dyn_debug_stackmods) cerr
#define ast_cerr \
  if (dyn_debug_ast) cerr
...
#define startup_printf(format, args...)                        \
  do {                                                         \
    if (dyn_debug_startup) startup_printf_int(format, ##args); \
  } while (0)
#define parsing_printf(format, args...)                        \
  do {                                                         \
    if (dyn_debug_parsing) parsing_printf_int(format, ##args); \
  } while (0)
#define proccontrol_printf(format, args...)                            \
  do {                                                                 \
    if (dyn_debug_proccontrol) proccontrol_printf_int(format, ##args); \
  } while (0)
#define stackwalk_printf(format, args...)                          \
  do {                                                             \
    if (dyn_debug_stackwalk) stackwalk_printf_int(format, ##args); \
  } while (0)
#define inst_printf(format, args...)                     \
  do {                                                   \
    if (dyn_debug_inst) inst_printf_int(format, ##args); \
  } while (0)
#define reloc_printf(format, args...)                      \
  do {                                                     \
    if (dyn_debug_reloc) reloc_printf_int(format, ##args); \
  } while (0)
#define dyn_unw_printf(format, args...)                        \
  do {                                                         \
    if (dyn_debug_dyn_unw) dyn_unw_printf_int(format, ##args); \
  } while (0)
#define mutex_printf(format, args...)                      \
  do {                                                     \
    if (dyn_debug_mutex) mutex_printf_int(format, ##args); \
  } while (0)
#define thread_printf(format, args...)                       \
  do {                                                       \
    if (dyn_debug_thread) thread_printf_int(format, ##args); \
  } while (0)
#define catchup_printf(format, args...)                        \
  do {                                                         \
    if (dyn_debug_catchup) catchup_printf_int(format, ##args); \
  } while (0)
#define regalloc_printf(format, args...)                         \
  do {                                                           \
    if (dyn_debug_regalloc) regalloc_printf_int(format, ##args); \
  } while (0)
#define ast_printf(format, args...)                    \
  do {                                                 \
    if (dyn_debug_ast) ast_printf_int(format, ##args); \
  } while (0)
#define write_printf(format, args...)                      \
  do {                                                     \
    if (dyn_debug_write) write_printf_int(format, ##args); \
  } while (0)
#define infmalloc_printf(format, args...)                          \
  do {                                                             \
    if (dyn_debug_infmalloc) infmalloc_printf_int(format, ##args); \
  } while (0)
#define crash_printf(format, args...)                      \
  do {                                                     \
    if (dyn_debug_crash) crash_printf_int(format, ##args); \
  } while (0)
#define stackmods_printf(format, args...)                          \
  do {                                                             \
    if (dyn_debug_stackmods) stackmods_printf_int(format, ##args); \
  } while (0)
