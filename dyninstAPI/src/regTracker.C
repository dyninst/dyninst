...
void regTracker_t::addKeptRegister(codeGen &gen, dapi::AstNode *n,
...
void regTracker_t::removeKeptRegister(codeGen &gen, dapi::AstNode *n) {
...
Dyninst::Register regTracker_t::hasKeptRegister(dapi::AstNode *n) {
  auto iter = tracker.find(n);
  if (iter == tracker.end())
...
void regTracker_t::decreaseAndClean(codeGen &) {
  ...
  std::vector<dapi::AstNode *> delete_list;
...
void regTracker_t::debugPrint() {
  ...
  for (auto iter = tracker.begin(); iter != tracker.end(); ++iter) {
    fprintf(stderr, "dapi::AstNode %p: register %u, condition level %d\n",
