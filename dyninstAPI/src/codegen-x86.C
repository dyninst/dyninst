...
unsigned
copy_prefixes_nosize_or_segments(const unsigned char*& origInsn, unsigned char*& newInsn,
                                 unsigned insnType)
{
    unsigned retval    = 0;
    unsigned nPrefixes = count_prefixes(insnType);
...
void
insnCodeGen::generateBranch(codeGen& gen, Address fromAddr, Address toAddr)
{
    GET_PTR(insn, gen);
    long disp;
...
void
insnCodeGen::generateBranch(codeGen& gen, int disp32)
{
    // Branches have sizes...
    if(disp32 >= 0)
...
void
insnCodeGen::generatePush64(codeGen& gen, Address val)
{
    GET_PTR(insn, gen);
#if 0
  for (int i = 3; i >= 0; i--) {
...
void
insnCodeGen::generateBranch64(codeGen& gen, Address to)
{
...
void
insnCodeGen::generateBranch32(codeGen& gen, Address to)
{
...
void
insnCodeGen::generateCall(codeGen& gen, Address from, Address target)
{
    // assert(target);
...
pcRelData::apply(Address addr)
{
    ...
    if(*(origInsn + nPrefixes) == 0x0F)
        nOpcodeBytes = 2;
    if((*(origInsn + nPrefixes) == 0x0F) &&
       (*(origInsn + nPrefixes + 1) == 0x38 || *(origInsn + nPrefixes + 1) == 0x3A))
        nOpcodeBytes = 3;
 ...
 **/
bool
insnCodeGen::generateMem(codeGen& gen, instruction&    insn, Address /*origAddr*/,
                         Address /*newAddr*/, Register loadExpr, Register storeExpr)
{
    /**********
     * Check parameters
     **********/
    ...
    ia32_decode(IA32_DECODE_MEMACCESS | IA32_DECODE_CONDITION, insn_ptr, orig_instr,
                (gen.width() == 8));
    ...
    {
        if((insn_ptr[i] != 0xf0) && (insn_ptr[i] != 0xf2) && (insn_ptr[i] != 0xf3) &&
           (insn_ptr[i] != 0x2e) && (insn_ptr[i] != 0x36) && (insn_ptr[i] != 0x3e) &&
           (insn_ptr[i] != 0x26) && (insn_ptr[i] != 0x64) && (insn_ptr[i] != 0x65) &&
           (insn_ptr[i] != 0x66) && (insn_ptr[i] != 0x67))
        {
            if(new_rex != 0)
            {
                *walker++ = new_rex;
...
bool
insnCodeGen::modifyJump(Address targetAddr, NS_x86::instruction& insn, codeGen& gen)
{
...
bool
insnCodeGen::modifyJcc(Address targetAddr, NS_x86::instruction& insn, codeGen& gen)
{
    const unsigned char* origInsn = insn.ptr();
    unsigned             insnType = insn.type();
    Address              from     = gen.currAddr();
...
bool
insnCodeGen::modifyCall(Address targetAddr, NS_x86::instruction& insn, codeGen& gen)
{
...
bool
insnCodeGen::modifyData(Address targetAddr, instruction& insn, codeGen& gen)
{
    // We may need to change these from 32-bit relative
    // to 64-bit absolute. This happens with the jumps and calls
    // as well, but it's better encapsulated there.
...
bool
insnCodeGen::modifyDisp(signed long newDisp, instruction& insn, codeGen& gen,
                        Architecture arch, Address addr)
{
    ...
    else
    {
        ...
        relocation_cerr << "\t\tOld displacement: " << std::hex << origDisp
                        << " New: " << newDisp << std::dec << std::endl;
    }
    ...
    relocation_cerr << "\t\tModRM: " << std::hex << (unsigned int) modrm
                    << " mod: " << (unsigned int) modrm_mod
                    << " rm: " << (unsigned int) modrm_rm << std::endl;
    ...
    {
        relocation_cerr << "\t\tERROR: Old Size: " << std::dec << insnSz
                        << " New size: " << newInsnSz
                        << " Expected size: " << (insnSz + expectedDifference)
                        << std::endl;
        return false;
    }
    ...
    getMemoryOffset(NULL, NULL, i, addr, MachRegister(), StackAnalysis::Height(0),
                    StackAnalysis::Definition(), newAccess, arch, modifyDefinition);
    if(!newAccess)
    {
        if(newDisp != 0)
        {
