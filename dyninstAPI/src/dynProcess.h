...
class PCProcess : public AddressSpace {
  ...
  friend class PCEventMuxer;  // Needed for some state queries that are
                              // protected
 ...
 public:
  ...
  static PCProcess *createProcess(const std::string file,
                                  pdvector<std::string> &argv,
                                  BPatch_hybridMode analysisMode,
                                  pdvector<std::string> &envp,
                                  const std::string dir, int stdin_fd,
  ...
  static std::string createExecPath(const std::string &file,
  ...
  // Process status
  bool isBootstrapped() const;  // true if Dyninst has finished it's
  ...
  bool isForcedTerminating() const {
    return forcedTerminating_;
  }  // true if we're in the middle of a forced termination
  bool isTerminated() const;  // true if the process is terminated ( either via
                              // normal exit or crash )
  bool hasExitedNormally()
      const;               // true if the process has exited (via a normal exit)
  ...
  bool writeDataWord(void *inTracedProcess, u_int amount, const void *inSelf);
  bool readDataSpace(const void *inTracedProcess, u_int amount, void *inSelf,
                     bool displayErrMsg);
  bool readDataWord(const void *inTracedProcess, u_int amount, void *inSelf,
                    bool displayErrMsg);
  ...
  bool readTextWord(const void *inTracedProcess, u_int amount, void *inSelf);
 ...
 public:
  ...
  void inferiorMallocConstraints(
      Address near, Address &lo, Address &hi,
  ...
  // iRPC interface
  bool postIRPC(AstNodePtr action, void *userData, bool runProcessWhenDone,
                PCThread *thread, bool synchronous, void **result, bool userRPC,
                bool isMemAlloc = false, Address addr = 0);
  bool postIRPC(void *buffer, int size, void *userData, bool runProcessWhenDone,
                PCThread *thread, bool synchronous, void **result, bool userRPC,
                bool isMemAlloc = false, Address addr = 0);

 private:
  bool postIRPC_internal(void *buffer, unsigned size, unsigned breakOffset,
                         Register resultReg, Address addr, void *userData,
                         bool runProcessWhenDone, PCThread *thread,
                         bool synchronous, bool userRPC, bool isMemAlloc,
                         void **result);
...
// TODO FIXME
  ...
  struct ActiveDefensivePad {
    ...
    ActiveDefensivePad(Address a, Address b, block_instance *c,
                       block_instance *d)
  ...
  virtual bool multithread_capable(
      bool ignoreIfMtNotSet = false);  // platform-specific
  ...
  bool bindPLTEntry(const SymtabAPI::relocationEntry &, Address,
 ...
 protected:
  ...
  static PCProcess *setupForkedProcess(PCProcess *parent,
                                       ProcControlAPI::Process::ptr pcProc);
  ...
  // Hybrid Mode
  bool triggerStopThread(Address pointAddress, int callbackID,
                         void *calculation);
...
class inferiorRPCinProgress : public codeRange {
 public:
  ...
  virtual unsigned get_size() const {
    return (rpcCompletionAddr - rpc->getAddress()) + 1;
  }
  virtual void *getPtrToInstruction(Address /*addr*/) const {
    assert(0);
    return NULL;
  }
...
class StackwalkSymLookup : public Dyninst::Stackwalker::SymbolLookup {
 ...
 public:
  ...
  virtual bool lookupAtAddr(Dyninst::Address addr, std::string &out_name,
                            void *&out_value);
  virtual ~StackwalkSymLookup();
};

class StackwalkInstrumentationHelper
    : public Dyninst::Stackwalker::DyninstDynamicHelper {
 private:
  PCProcess *proc_;
...
class DynFrameHelper : public Dyninst::Stackwalker::FrameFuncHelper {
 ...
 public:
  ...
  virtual Dyninst::Stackwalker::FrameFuncHelper::alloc_frame_t allocatesFrame(
      Address addr);
...
class DynWandererHelper : public Dyninst::Stackwalker::WandererHelper {
 ...
 public:
  ...
  virtual Dyninst::Stackwalker::WandererHelper::pc_state isPCInFunc(
      Address func_entry, Address pc);
