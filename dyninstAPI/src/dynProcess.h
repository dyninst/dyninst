...
public:
    ...
    static PCProcess* createProcess(const std::string         file,
                                    ...
                                    std::vector<std::string>& envp, const std::string dir,
    ...
    // Process status
    bool isBootstrapped()
        const;  // true if Dyninst has finished it's initialization for the process
    ...
    bool isTerminated()
        const;  // true if the process is terminated ( either via normal exit or crash )
...
// TODO FIXME
    ...
    bool bindPLTEntry(const SymtabAPI::relocationEntry&, Address, func_instance*,
                      Address);
...
protected:
    ...
    , curThreadIndex_(0)
    ,  // threads are created from ProcControl threads
    reportedEvent_(false)
    , savedPid_(pcProc->getPid())
    , savedArch_(pcProc->getArchitecture())
    ...
    void writeDebugDataSpace(void* inTracedProcess, u_int amount, const void* inSelf);
    bool launchDebugger();
...
public:
    ...
    virtual Dyninst::Stackwalker::WandererHelper::pc_state isPCInFunc(Address func_entry,
                                                                      Address pc);
