...
instPoint*
instPoint::funcEntry(func_instance* f)
{
    return f->funcEntryPoint(true);
}

instPoint*
instPoint::funcExit(func_instance* f, block_instance* b)
{
    return f->funcExitPoint(b, true);
}

instPoint*
instPoint::blockEntry(func_instance* f, block_instance* b)
{
    return f->blockEntryPoint(b, true);
}

instPoint*
instPoint::blockExit(func_instance* f, block_instance* b)
{
    return f->blockExitPoint(b, true);
}

instPoint*
instPoint::preCall(func_instance* f, block_instance* b)
{
    return f->preCallPoint(b, true);
}

instPoint*
instPoint::postCall(func_instance* f, block_instance* b)
{
    return f->postCallPoint(b, true);
}

instPoint*
instPoint::edge(func_instance* f, edge_instance* e)
{
...
instPoint*
instPoint::preInsn(func_instance* f, block_instance* b, Address a, Instruction insn,
                   bool trusted)
{
...
instPoint*
instPoint::postInsn(func_instance* f, block_instance* b, Address a, Instruction insn,
                    bool trusted)
{
...
instPoint::instPoint(Type t, PatchMgrPtr mgr, func_instance* f, block_instance* b)
: Point(t, mgr, f, b)
, baseTramp_(NULL)
{}

instPoint::instPoint(Type t, PatchMgrPtr mgr, block_instance* b, func_instance* f)
: Point(t, mgr, b, f)
, baseTramp_(NULL)
{}

instPoint::instPoint(Type t, PatchMgrPtr mgr, block_instance* b, Address a, Instruction i,
                     func_instance* f)
: Point(t, mgr, b, a, i, f)
, baseTramp_(NULL)
{}

instPoint::instPoint(Type t, PatchMgrPtr mgr, edge_instance* e, func_instance* f)
: Point(t, mgr, e, f)
, baseTramp_(NULL)
{}
...
std::pair<instPoint*, instPoint*>
instPoint::getInstpointPair(instPoint* i)
{
    switch(i->type())
    {
        case None:
        ...
        case PreCall:
            return std::pair<instPoint*, instPoint*>(i, postCall(i->func(), i->block()));
        case PostCall:
            return std::pair<instPoint*, instPoint*>(preCall(i->func(), i->block()), i);
        default:
...
instPoint*
instPoint::fork(instPoint* parent, AddressSpace* child)
{
...
func_instance*
instPoint::func() const
{
    return SCAST_FI(the_func_);
}

block_instance*
instPoint::block() const
{
    return SCAST_BI(the_block_);
}

edge_instance*
instPoint::edge() const
{
    return SCAST_EI(the_edge_);
}

bool
instPoint::checkInsn(block_instance* b, Instruction& insn, Address a)
{
    ...
    switch(type_)
    {
        ...
        default:
            mal_printf("ERROR: returning 0 from instPoint::nextExecutedAddr "
                       ...
                       (unsigned int) type(), block()->start(), block()->end());
            return 0;
    }
}

std::string
instPoint::format() const
{
    stringstream ret;
    ret << "iP(";
    switch(type_)
...
Dyninst::PatchAPI::InstancePtr
getChildInstance(Dyninst::PatchAPI::InstancePtr parentInstance, AddressSpace* childProc)
{
    ...
    for(instPoint::instance_iter iter = pPoint->begin(); iter != pPoint->end(); ++iter)
    {
        if(*iter == parentInstance)
            return *c_iter;
        ++c_iter;
    }
...
InstancePtr
instPoint::pushFront(SnippetPtr snip)
{
...
InstancePtr
instPoint::pushBack(SnippetPtr snip)
{
...
instPoint::liveRegisters()
{
    ...
    {
        case FuncEntry:
            if(!live->query(ParseAPI::Location(EntrySite(func()->function(),
                                                         func()->function()->entry())),
        ...
        case BlockEntry:
            if(!live->query(
                   ParseAPI::Location(BlockSite(func()->function(), block()->block())),
                   LivenessAnalyzer::Before, liveRegs_))
                assert(0);
            break;
        case BlockExit:
        ...
        case FuncExit:
            if(!live->query(
                   ParseAPI::Location(ExitSite(func()->function(), block()->block())),
                   LivenessAnalyzer::After, liveRegs_))
                assert(0);
            break;
        case PostCall:
            if(!live->query(
                   ParseAPI::Location(CallSite(func()->function(), block()->block())),
                   LivenessAnalyzer::After, liveRegs_))
                assert(0);
            break;
        case PreCall:
