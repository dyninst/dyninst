...
Dyninst::Address
BPatch_object::fileOffsetToAddr(const Dyninst::Offset fileOffset)
{
...
struct findFunc
{
    ...
    {
        mod->findFunction(name.c_str(), funcs, notify_on_failure, regex_case_sensitive,
                          incUninstrumentable, dont_use_regex);
    }

    findFunc(std::string n, std::vector<BPatch_function*>& f, bool n_o_f, bool r, bool i,
             bool d)
    : name(n)
    , funcs(f)
...
BPatch_object::findFunction(std::string name, std::vector<BPatch_function*>& funcs,
                            ...
                            bool incUninstrumentable, bool dont_use_regex)
{
    ...
    if(notify_on_failure && (num_funcs_in == funcs.size()))
    {
        // notify once
        char msg[1024];
        sprintf(msg, "%s[%d]:  Object %s: unable to find function %s", __FILE__, __LINE__,
...
Dyninst::ParseAPI::convert(const BPatch_object* o)
{
    if(!o->obj)
        return NULL;
...
Dyninst::PatchAPI::PatchObject*
Dyninst::PatchAPI::convert(const BPatch_object* o)
{
    if(!o)
        return NULL;
    return o->obj;
...
Dyninst::SymtabAPI::convert(const BPatch_object* o)
{
    if(!o)
        return NULL;
...
BPatchSnippetHandle*
BPatch_object::insertInitCallback(BPatch_snippet& callback)
{
...
BPatchSnippetHandle*
BPatch_object::insertFiniCallback(BPatch_snippet& callback)
{
...
{
void
getCalledFuncs(BPatch_function* func, std::set<BPatch_function*>& callSet)
{
    const std::vector<BPatch_point*>* callPoints = func->findPoint(BPatch_subroutine);
    if(callPoints == NULL)
        return;
    for(auto callIter = callPoints->begin(); callIter != callPoints->end(); callIter++)
    {
...
void
resolveCalls(BPatch_function* func, std::map<Address, Address>& callResolutionMap)
{
    const std::vector<BPatch_point*>* callPoints = func->findPoint(BPatch_subroutine);
    if(callPoints == NULL)
        return;
...
void
DLDFS(unsigned depthLimit, std::set<BPatch_function*> allFuncsSet,
...
public:
    ...
    void topoCycleVisit(
        BPatch_function* func, std::set<BPatch_function*>& unvisitedSet,
...
void
topoCycleSort(std::set<BPatch_function*>                               unvisitedSet,
              std::queue<std::pair<std::set<BPatch_function*>, bool>>& sortedSetList)
{
...
void
funcSummaryFixpoint(std::set<BPatch_function*>                     funcSet,
                    ...
                    std::map<Address, StackAnalysis::TransferSet>& functionSummaries)
{
    ...
    for(auto callerIter = funcSet.begin(); callerIter != funcSet.end(); callerIter++)
    {
...
void
addModsFuncSet(const std::set<StackMod*>&                      mods,
...
void
BPatch_object::addModsAllFuncs(const std::set<StackMod*>& mods_, bool interproc,
                               std::vector<std::pair<BPatch_function*, bool>>& modResults,
                               unsigned                                        depthLimit)
{
#if defined(cap_stack_mods)
    ...
    if(interproc)
    {
        ...
        std::queue<std::pair<std::set<BPatch_function*>, bool>> ssl2 = sortedSetList;
        ...
        {
            addModsFuncSet(mods_, allModFuncs, sortedSetList.front(), funcEntryToSummary,
                           callResolutionMap, modResults);
            sortedSetList.pop();
        }
    }
    else
