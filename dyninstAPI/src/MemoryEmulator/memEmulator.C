...
void MemoryEmulator::update() {
  ...
  sensitivity_cerr << "UpdateMemEmulator: writing guard value " << guardValue
                   << endl;
  // 64->32 bit is annoying...
  if (addrWidth() == 4) {
    struct MemoryMapper32 newMapper;

    aS_->readDataSpace((void *)mutateeBase_, sizeof(newMapper), &newMapper,
                       false);
...
void MemoryEmulator::addRegion(Region *reg, Address base, Address, Address) {
  ...
  if (aS_->proc() && BPatch_defensiveMode == aS_->proc()->getHybridMode()) {
    ...
    assert(proc);
    proc->stopProcess();
    proc->changeMemoryProtections(mutateeBase, reg->getMemSize(), memPerm_,
                                  false);
  }
...
void MemoryEmulator::removeRegion(Region *reg, Address base) {
  ...
  //<< codeRegions[i]->getMemOffset() + codeRegions[i]->getMemSize() +
  //obj->codeBase() << endl;
...
void MemoryEmulator::removeRegion(Address addr, unsigned size) {
  ...
  cerr << "MemoryEmulator: removing region " << hex << addr << " : " << size
       << dec << endl;
...
unsigned MemoryEmulator::addrWidth() { return aS_->getAddressWidth(); }
...
void MemoryEmulator::synchShadowOrig(bool toOrig) {
  ...
  for (RegionMap::iterator iter = addedRegions_.begin();
    ...
    // copying to). We then select snippets of target (where springboards
    // reside)
...
void MemoryEmulator::removeSpringboards(func_instance *func) {
  malware_cerr << "untracking springboards from deadfunc " << hex
               << func->addr() << dec << endl;
...
void MemoryEmulator::debug() const {
  ...
  for (std::vector<MemoryMapTree::Entry>::iterator iter = elements.begin();
    ...
    cerr << "\t\t " << hex << "[" << iter->first.first << ","
         << iter->first.second << "]: " << iter->second << dec << endl;
#if 0  // debug output
...
void MemoryEmulator::addPOPAD(Address addr) { emulatedPOPADs_.insert(addr); }
