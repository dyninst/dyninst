...
class MemEmulatorTransformer : public Transformer {
 ...
 public:
  ...
  MemEmulatorTransformer() : aConverter(false){};

  virtual ~MemEmulatorTransformer(){};

 private:
  WidgetPtr createReplacement(InsnWidgetPtr reloc, func_instance *func,
                              block_instance *);

  bool canRewriteMemInsn(InsnWidgetPtr reloc, func_instance *func);

  bool isSensitive(InsnWidgetPtr reloc, func_instance *func,
