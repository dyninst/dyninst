...
Process::cb_ret_t PCEventMuxer::ret_stopped(Process::cbProcStop,
                                            Process::cbProcStop);
Process::cb_ret_t PCEventMuxer::ret_continue(Process::cbProcContinue,
                                             Process::cbProcContinue);
Process::cb_ret_t PCEventMuxer::ret_default(Process::cbDefault,
                                            Process::cbDefault);
...
PCEventMuxer::WaitResult PCEventMuxer::wait_internal(bool block) {
  proccontrol_printf("[%s/%d]: PCEventMuxer waiting for events, %s\n", FILE__,
                     __LINE__, (block ? "blocking" : "non-blocking"));
  if (!block) {
    Process::handleEvents(false);
    proccontrol_printf(
        "[%s:%d] after PC event handling, %d events in mailbox\n", FILE__,
    ...
    proccontrol_printf(
        "[%s:%d] PCEventMuxer::wait_internal, blocking, mailbox size is %d\n",
...
DThread::dthread_ret_t PCEventMuxer::main(void *) {
  ...
  // Since we're callback-based, the decode and handle/enqueue will be taken
  // care of
  ...
  while (1) {
    if (!Process::handleEvents(true)) {
      // Complain
      proccontrol_printf(
          "%s[%d]: error returned by Process::handleEvents: %s\n", FILE__,
          __LINE__, getLastErrorMsg());
      continue;
    }
  }
...
bool PCEventMuxer::registerCallbacks() {
  ...
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::Crash), defaultCallback);
  ret &= Process::registerEventCallback(
  ...
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::ThreadDestroy),
      threadDestroyCallback);
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::Signal), signalCallback);
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::Library), defaultCallback);
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::Breakpoint), breakpointCallback);
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::RPC), RPCCallback);
  ret &= Process::registerEventCallback(
      EventType(EventType::Any, EventType::SingleStep), defaultCallback);

  // Fork/exec/exit
  if (useCallback(EventType(EventType::Pre, EventType::Exit))) {
    ret &= Process::registerEventCallback(
        EventType(EventType::Pre, EventType::Exit), exitCallback);
    assert(ret);
  }
  if (useCallback(EventType(EventType::Post, EventType::Exit))) {
    ret &= Process::registerEventCallback(
        EventType(EventType::Post, EventType::Exit), exitCallback);
    assert(ret);
  }
  if (useCallback(EventType(EventType::Pre, EventType::Fork))) {
    ret &= Process::registerEventCallback(
        EventType(EventType::Pre, EventType::Fork), defaultCallback);
    assert(ret);
  }
  if (useCallback(EventType(EventType::Post, EventType::Fork))) {
    ret &= Process::registerEventCallback(
        EventType(EventType::Post, EventType::Fork), defaultCallback);
    assert(ret);
  }
  if (useCallback(EventType(EventType::Pre, EventType::Exec))) {
    ret &= Process::registerEventCallback(
        EventType(EventType::Pre, EventType::Exec), defaultCallback);
    assert(ret);
  }
  if (useCallback(EventType(EventType::Post, EventType::Exec))) {
    ret &= Process::registerEventCallback(
        EventType(EventType::Post, EventType::Exec), defaultCallback);
    assert(ret);
  }
...
// Apologies for the #define, but I get tired of copying the same text over and
// over, and since this has
...
#define INITIAL_MUXING                                                        \
  ...
  proccontrol_printf(                                                         \
      "%s[%d]: Begin callbackMux, process pointer = %p, event %s\n", FILE__,  \
      __LINE__, process, ev->name().c_str());                                 \
  if (process == NULL) {                                                      \
...
PCEventMuxer::cb_ret_t PCEventMuxer::breakpointCallback(EventPtr ev) {
  ...
  if (hasCtrlTransfer) {
    proccontrol_printf(
        "%s[%d]: received control transfer breakpoint on thread %d/%d (0x%lx "
        "=> 0x%lx)\n",
...
PCEventMuxer::cb_ret_t PCEventMuxer::RPCCallback(EventPtr ev) {
  ...
  if (rpcInProg->resultRegister == REG_NULL) {
    ...
    if (!ev->getThread()->getRegister(resultReg, resultVal)) {
      proccontrol_printf(
          "%s[%d]: failed to retrieve register from thread %d/%d\n", FILE__,
          __LINE__, ev->getProcess()->getPid(), ev->getThread()->getLWP());
      ev = Event::const_ptr(new Event(EventType::Error));
    } else {
...
void PCEventMuxer::enqueue(EventPtr ev) { mailbox_.enqueue(ev); }
...
void PCEventMailbox::enqueue(Event::const_ptr ev) {
  ...
  proccontrol_printf("%s[%d]: Added event %s to mailbox, size now %d\n", FILE__,
                     __LINE__, ev->name().c_str(), eventQueue.size());
...
bool PCEventMailbox::find(PCProcess *proc) { return (procCount[proc] > 0); }
