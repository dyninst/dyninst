...
Process::cb_ret_t PCEventMuxer::ret_continue(Process::cbProcContinue,
...
PCEventMuxer::WaitResult
PCEventMuxer::wait(bool block)
{
    return muxer().wait_internal(block);
}

bool
PCEventMuxer::handle(PCProcess* proc)
{
    return muxer().handle_internal(proc);
}

PCEventMuxer::WaitResult
PCEventMuxer::wait_internal(bool block)
{
    ...
    proccontrol_printf(
        "[%s:%d] PCEventMuxer::wait_internal, blocking, mailbox size is %u\n", FILE__,
        __LINE__, mailbox_.size());
...
bool
PCEventMuxer::handle_internal(PCProcess*)
{
    bool ret = true;
    while(mailbox_.size())
    {
...
bool
PCEventMuxer::hasPendingEvents(PCProcess* proc)
{
    return mailbox_.find(proc);
}

bool
PCEventMuxer::handle(EventPtr ev)
{
    // Find the correct PCEventHandler and dispatch
    return PCEventHandler::handle(ev);
}

DThread::dthread_ret_t
PCEventMuxer::main(void*)
{
...
bool
PCEventMuxer::registerCallbacks()
{
    ...
    ret &= Process::registerEventCallback(EventType(EventType::Any, EventType::Crash),
                                          defaultCallback);
    ret &= Process::registerEventCallback(
        EventType(EventType::Any, EventType::ForceTerminate), defaultCallback);
    ret &= Process::registerEventCallback(
        EventType(EventType::Any, EventType::ThreadCreate), threadCreateCallback);
    ret &= Process::registerEventCallback(
        EventType(EventType::Any, EventType::ThreadDestroy), threadDestroyCallback);
    ret &= Process::registerEventCallback(EventType(EventType::Any, EventType::Signal),
                                          signalCallback);
    ret &= Process::registerEventCallback(EventType(EventType::Any, EventType::Library),
                                          defaultCallback);
    ret &= Process::registerEventCallback(
        EventType(EventType::Any, EventType::Breakpoint), breakpointCallback);
    ret &= Process::registerEventCallback(EventType(EventType::Any, EventType::RPC),
                                          RPCCallback);
    ret &= Process::registerEventCallback(
        EventType(EventType::Any, EventType::SingleStep), SingleStepCallback);

    // Fork/exec/exit
    if(useCallback(EventType(EventType::Pre, EventType::Exit)))
    {
        ret &= Process::registerEventCallback(EventType(EventType::Pre, EventType::Exit),
                                              exitCallback);
        assert(ret);
    }
    if(useCallback(EventType(EventType::Post, EventType::Exit)))
    {
        ret &= Process::registerEventCallback(EventType(EventType::Post, EventType::Exit),
                                              exitCallback);
        assert(ret);
    }
    if(useCallback(EventType(EventType::Pre, EventType::Fork)))
    {
        ret &= Process::registerEventCallback(EventType(EventType::Pre, EventType::Fork),
                                              defaultCallback);
        assert(ret);
    }
    if(useCallback(EventType(EventType::Post, EventType::Fork)))
    {
        ret &= Process::registerEventCallback(EventType(EventType::Post, EventType::Fork),
                                              defaultCallback);
        assert(ret);
    }
    if(useCallback(EventType(EventType::Pre, EventType::Exec)))
    {
        ret &= Process::registerEventCallback(EventType(EventType::Pre, EventType::Exec),
                                              defaultCallback);
        assert(ret);
    }
    if(useCallback(EventType(EventType::Post, EventType::Exec)))
    {
        ret &= Process::registerEventCallback(EventType(EventType::Post, EventType::Exec),
                                              defaultCallback);
        assert(ret);
    }
...
// Apologies for the #define, but I get tired of copying the same text over and over, and
// since this has a shortcut return it can't be a subfunction.
...
#define DEFAULT_RETURN                                                                   \
    ...
    proccontrol_printf("%s[%d]: after muxing event, mailbox size is %u\n", FILE__,       \
                       __LINE__, m.mailbox_.size());                                     \
    return ret;

PCEventMuxer::cb_ret_t
PCEventMuxer::defaultCallback(EventPtr ev)
{
    INITIAL_MUXING;
...
PCEventMuxer::cb_ret_t
PCEventMuxer::SingleStepCallback(EventPtr ev)
{
    INITIAL_MUXING;
    ...
    for(ProcControlAPI::RegisterPool::iterator iter = regs.begin(); iter != regs.end();
        ++iter)
    {
        // cerr << "\t Reg " << (*iter).first.name() << ": " << hex << (*iter).second <<
        // dec << endl;
        if(((*iter).first.isPC()))
        {
            pc = (*iter).second;
...
PCEventMuxer::cb_ret_t
PCEventMuxer::exitCallback(EventPtr ev)
{
    INITIAL_MUXING;
...
PCEventMuxer::cb_ret_t
PCEventMuxer::crashCallback(EventPtr ev)
{
    INITIAL_MUXING;
...
PCEventMuxer::cb_ret_t
PCEventMuxer::signalCallback(EventPtr ev)
{
    INITIAL_MUXING;
    ...
    if(evSignal->getSignal() == 11)
    {
        ...
        for(ProcControlAPI::RegisterPool::iterator iter = regs.begin();
            iter != regs.end(); ++iter)
        {
            cerr << "\t Reg " << (*iter).first.name() << ": " << hex << (*iter).second
                 << dec << endl;
        ...
        {
            unsigned tmp = 0;
            process->readDataSpace((void*) (esp + (i * 4)), 4, &tmp, false);
...
PCEventMuxer::cb_ret_t
PCEventMuxer::breakpointCallback(EventPtr ev)
{
    INITIAL_MUXING;
        ...
        i != breakpoints.end(); ++i)
    {
        ...
        if((*i) == process->getBreakpointAtMain())
        {
            ...
            {
                proccontrol_printf(
                    "%s[%d]: failed to remove main breakpoint in event handling\n",
                    FILE__, __LINE__);
            ...
            // Need to pass the event on the user thread to indicate to that the
    ...
    if(hasCtrlTransfer)
    {
        proccontrol_printf("%s[%d]: received control transfer breakpoint on thread %d/%d "
                           ...
                           ev->getThread()->getLWP(), evBreak->getAddress(),
                           ctrlTransferPt->getToAddress());
        ret = ret_continue;
        return ret;
    }
...
PCEventMuxer::RPCCallback(EventPtr ev)
{
    ...
    else
    {
        ...
        MachRegister resultReg =
            convertRegID(rpcInProg->resultRegister, ev->getProcess()->getArchitecture());
        ...
        else
        {
            ...
            proccontrol_printf("%s[%d]: iRPC %lu return value = 0x%lx\n", FILE__,
                               __LINE__, rpcInProg->rpc->getID(), resultVal);
        }
    }
...
PCEventMuxer::cb_ret_t
PCEventMuxer::threadCreateCallback(EventPtr ev)
{
    INITIAL_MUXING;
...
PCEventMuxer::cb_ret_t
PCEventMuxer::threadDestroyCallback(EventPtr ev)
{
    INITIAL_MUXING;
...
void
PCEventMuxer::enqueue(EventPtr ev)
{
    mailbox_.enqueue(ev);
}

PCEventMuxer::EventPtr
PCEventMuxer::dequeue(bool block)
{
...
void
PCEventMailbox::enqueue(Event::const_ptr ev)
{
    ...
    if(evProc)
    {
        ...
        proccontrol_printf(
            "%s[%d]: Added event %s from process %d to mailbox, size now %lu\n", FILE__,
            __LINE__, ev->name().c_str(), evProc->getPid(), eventQueue.size());
    }
    else
    {
        proccontrol_printf("%s[%d]: Got bad process: event %s not added\n", FILE__,
                           __LINE__, ev->name().c_str());
        assert(false);
    }
...
Event::const_ptr
PCEventMailbox::dequeue(bool block)
{
    /* NB: This procedure assumes the queue is not modified while we are dequeueing an
     * event */
    ...
    proccontrol_printf("%s[%d]: Returning event %s from mailbox for process %d\n", FILE__,
                       __LINE__, event_ptr->name().c_str(), evProc->getPid());
...
bool
PCEventMailbox::find(PCProcess* proc)
{
    std::lock_guard<CondVar<>> l{ queueCond };
