...
bool
fileDescriptor::IsEqual(const fileDescriptor& fd) const
{
...
#endif
    if(file_match_ && (addr_match) && (member_ == fd.member_) && (pid_ == fd.pid_))
        return true;
    else
        return false;
}
...
void*
get_raw_symtab_ptr(Symtab* linkedFile, Address addr)
{
...
Address
deref_opd(Symtab* linkedFile, Address addr)
{
    Region* reg = linkedFile->findEnclosingRegion(addr);
 ...
 */
Address
evaluate_main_address(Symtab* linkedFile, Function* f, Block* b)
{
    using namespace DataflowAPI;
            ...
            if(assignments.size() >= 1)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        mainAddress = 0x0;
                        startup_printf(
                            "%s[%d]:  FindMainVisitor cannot find main address in %s\n",
...
#    endif
        ...
        if(!foundStart)
        {
            Symbol* startSym = new Symbol("_start", Symbol::ST_FUNCTION, Symbol::SL_LOCAL,
                                          Symbol::SV_INTERNAL, eReg->getMemOffset(),
                                          linkedFile->getDefaultModule(), eReg, 0);
        ...
        if(!foundFini)
        {
            ...
            {
                Symbol* finiSym = new Symbol(
                    "_fini", Symbol::ST_FUNCTION, Symbol::SL_LOCAL, Symbol::SV_INTERNAL,
                    finisec->getMemOffset(), linkedFile->getDefaultModule(), finisec, 0);
    ...
    if(linkedFile->findRegion(dynamicsec, ".dynamic") == true)
    {
        ...
        {
            Symbol* newSym = new Symbol("_DYNAMIC", Symbol::ST_OBJECT, Symbol::SL_LOCAL,
    ...
    if(linkedFile->isExec())
    {
        ...
        else
        {
            ...
            linkedFile->addSymbol(new Symbol("main", Symbol::ST_FUNCTION,
                                             Symbol::SL_GLOBAL, Symbol::SV_DEFAULT, eAddr,
                                             linkedFile->getDefaultModule(), eReg));
        }
    }
#endif
...
bool
image::getInferiorHeaps(vector<pair<string, Address>>& codeHeaps,
                        vector<pair<string, Address>>& dataHeaps)
{
    if((codeHeaps_.size() == 0) && (dataHeaps_.size() == 0))
        return false;
...
image::addSymtabVariables()
{
    ...
    for(vector<Variable*>::iterator varIter = allVars.begin(); varIter != allVars.end();
        varIter++)
    {
...
pdmodule*
image::findModule(const string& name, bool wildcard)
{
    pdmodule* found = NULL;
...
const std::vector<image_variable*>&
image::getAllVariables()
{
    analyzeIfNeeded();
...
const std::vector<image_variable*>&
image::getExportedVariables() const
{
    return exportedVariables;
}

const std::vector<image_variable*>&
image::getCreatedVariables()
{
    analyzeIfNeeded();
...
bool
image::getModules(vector<pdmodule*>& mods)
{
    bool                   ret = false;
    std::vector<pdmodule*> toReturn;
    for(map<Module*, pdmodule*>::const_iterator iter = mods_.begin(); iter != mods_.end();
        iter++)
    {
        ret = true;
...
void
image::findModByAddr(const Symbol* lookUp, vector<Symbol*>& mods, string& modName,
...
unsigned int
int_addrHash(const Address& addr)
{
...
image*
image::parseImage(fileDescriptor& desc, BPatch_hybridMode mode, bool parseGaps)
{
    /*
    ...
    unsigned long difftime   = lendtime - lstarttime;
    double        dursecs    = difftime / (1000);
    cout << __FILE__ << ":" << __LINE__ << ": parsing image " << desc.file().c_str()
...
static bool
CheckForPowerPreamble(parse_block* entryBlock, Address& tocBase)
{
...
image::analyzeImage()
{
    ...
    unsigned long difftime   = lendtime - lstarttime;
    double        dursecs    = difftime / (1000);
    cout << __FILE__ << ":" << __LINE__ << ": analyzeImage of " << name_ << " took "
         << dursecs << " msecs" << endl;
#endif
}
...
image::image(fileDescriptor& desc, bool& err, BPatch_hybridMode mode, bool parseGaps)
: desc_(desc)
, imageOffset_(0)
, imageLen_(0)
, dataOffset_(0)
...
, arch(Dyninst::Arch_none)
{
    ...
    if(!imageLen_ && linkedFile->getObjectType() != SymtabAPI::obj_RelocatableFile)
    {
    ...
    determineImageType();
    if(isDyninstRTLib())
    {  // don't parse gaps in the runtime library
        parseGaps_ = false;
    }
...
image::~image()
{
    ...
    for(map<Module*, pdmodule*>::iterator iter = mods_.begin(); iter != mods_.end();
        iter++)
    {
        delete(iter->second);
    }
    ...
    allImages.erase(std::remove_if(allImages.begin(), allImages.end(),
...
bool
pdmodule::findFunction(const std::string& name, std::vector<parse_func*>& found)
{
...
bool
pdmodule::findFunctionByMangled(const std::string& name, std::vector<parse_func*>& found)
{
    ...
    const std::vector<parse_func*>* obj_funcs =
...
bool
pdmodule::findFunctionByPretty(const std::string& name, std::vector<parse_func*>& found)
{
    // For efficiency sake, we grab the image vector and strip out the
    // functions we want.
    // We could also keep them all in modules and ditch the image-wide search;
...
void
pdmodule::dumpMangled(std::string& prefix) const
{
...
parse_func*
image::addFunction(Address functionEntryAddr, const char* fName)
{
    set<CodeRegion*> regions;
    CodeRegion*      region;
    ...
    Symbol* funcSym =
        new Symbol(funcName, Symbol::ST_FUNCTION, Symbol::SL_GLOBAL, Symbol::SV_DEFAULT,
                   functionEntryAddr, mod->mod(), NULL, UINT_MAX);
...
pdmodule*
image::getOrCreateModule(Module* mod)
{
...
parse_func*
image::findFuncByEntry(const Address& entry)
{
    analyzeIfNeeded();
...
const std::vector<parse_func*>*
image::findFuncVectorByPretty(const std::string& name)
{
    // Have to change here
...
const std::vector<parse_func*>*
image::findFuncVectorByMangled(const std::string& name)
{
    std::vector<parse_func*>* res = new std::vector<parse_func*>;
...
const std::vector<image_variable*>*
image::findVarVectorByPretty(const std::string& name)
{
...
const std::vector<image_variable*>*
image::findVarVectorByMangled(const std::string& name)
{
...
bool
pdmodule::getFunctions(std::vector<parse_func*>& funcs)
{
...
bool
pdmodule::getVariables(std::vector<image_variable*>& vars)
{
...
bool
image::getExecCodeRanges(std::vector<std::pair<Address, Address>>& ranges)
{
...
Symbol*
image::symbol_info(const std::string& symbol_name)
{
...
bool
image::findSymByPrefix(const std::string& prefix, std::vector<Symbol*>& ret)
{
    unsigned        start;
    vector<Symbol*> found;
    std::string     reg = prefix + std::string("*");
    if(!linkedFile->findSymbol(found, reg.c_str(), Symbol::ST_UNKNOWN, SymtabAPI::anyName,
                               true))
        return false;
...
std::unordered_map<Address, std::string>*
image::getPltFuncs()
{
    bool result;
    if(pltFuncs)
...
void
image::getPltFuncs(std::map<Address, std::string>& out)
{
    out.clear();
    vector<SymtabAPI::relocationEntry> fbt;
...
image_variable*
image::createImageVariable(Offset offset, std::string name, int size, pdmodule* mod)
{
    // What to do here?
    auto iter = varsByAddr.find(offset);
    if(iter != varsByAddr.end())
        return iter->second;
...
const vector<parse_block*>&
image::getNewBlocks() const
{
    return newBlocks_;
}
void
image::clearNewBlocks()
{
    newBlocks_.clear();
}

void
image::setImageLength(Address newlen)
{
    imageLen_ = newlen;
}

void
image::destroy(ParseAPI::Block*)
{}
void
image::destroy(ParseAPI::Edge*)
{}
void
image::destroy(ParseAPI::Function*)
{}

void
image::insertPLTParseFuncMap(const std::string& name, parse_func* f)
{
