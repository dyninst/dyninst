...
//  we really do not want to have this defined, but I'm defining it for the
//  moment to get thru paradyn seperation
...
class int_symbol {
 public:
  int_symbol(SymtabAPI::Symbol *sym, Address base)
      : addr_(base + sym->getOffset()), sym_(sym) {}
...
class int_variable {
 ...
 public:
  int_variable(image_variable *var, Address base, mapped_module *mod);
...
class mapped_object : public codeRange, public Dyninst::PatchAPI::DynObject {
 ...
 private:
  mapped_object();
  mapped_object(fileDescriptor fileDesc, image *img, AddressSpace *proc,
                BPatch_hybridMode mode = BPatch_normalMode);

 public:
  // We need a way to check for errors; hence a "get" method
  static mapped_object *createMappedObject(
      fileDescriptor &desc, AddressSpace *p,
      BPatch_hybridMode m = BPatch_normalMode, bool parseGaps = true);
  static mapped_object *createMappedObject(
      ProcControlAPI::Library::const_ptr lib, AddressSpace *p,
      BPatch_hybridMode m = BPatch_normalMode, bool parseGaps = true);

  // Copy constructor: for forks
  mapped_object(const mapped_object *par_obj, AddressSpace *child);

  // Will delete all func_instances which were originally part of this object;
  // including
  // any that were relocated (we can always follow the "I was relocated"
  // pointer).
  ~mapped_object();
  ...
  bool findBlocksByRange(Address startAddr, Address endAddr,
                         std::list<block_instance *> &pageBlocks);
  void findFuncsByRange(Address startAddr, Address endAddr,
  ...
  void replacePLTStub(SymtabAPI::Symbol *PLTsym, func_instance *func,
                      Address newAddr);
 ...
 public:
  ...
  const pdvector<func_instance *> *findFuncVectorByPretty(
      const std::string &funcname);
  const pdvector<func_instance *> *findFuncVectorByMangled(
      const std::string &funcname);

  bool findFuncsByAddr(std::vector<func_instance *> &funcs);
  bool findBlocksByAddr(std::vector<block_instance *> &blocks);

  const pdvector<int_variable *> *findVarVectorByPretty(
      const std::string &varname);
  const pdvector<int_variable *> *findVarVectorByMangled(
      const std::string &varname);
  ...
  edge_instance *findEdge(ParseAPI::Edge *, block_instance *src = NULL,
                          block_instance *trg = NULL);
 ...
 private:
  ...
  typedef std::unordered_map<std::string, std::vector<func_instance *> *>
      func_index_t;
  typedef std::unordered_map<std::string, std::vector<int_variable *> *>
      var_index_t;

  std::unordered_map<const image_variable *, int_variable *>
      everyUniqueVariable;
  ...
  typedef enum { mangledName = 1, prettyName = 2, typedName = 4 } nameType_t;
