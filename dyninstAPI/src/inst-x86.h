...
#define LOAD_VIRTUAL32(x, insn) \
  emitMovRMToReg(REGNUM_EAX, REGNUM_EBP, -1 * (x * 4), insn)
#define SAVE_VIRTUAL32(x, insn) \
  emitMovRegToRM(REGNUM_EBP, -1 * (x * 4), REGNUM_EAX, insn)
#define LOAD_VIRTUAL64(x, insn) \
  emitMovRMToReg(REGNUM_RAX, REGNUM_RBP, -1 * (x * 8), insn)
#define SAVE_VIRTUAL64(x, insn) \
  emitMovRegToRM(REGNUM_RBP, -1 * (x * 8), REGNUM_RAX, insn)
...
void emitOpExtRegImm(int opcode, int opcode2, RealRegister dest, int imm,
                     codeGen &gen);
...
void emitOpSegRMReg(unsigned opcode, RealRegister dest, RealRegister src,
                    int disp, codeGen &gen);
...
struct stackItem {
  enum stackItem_t { reg_item, stacktop, framebase } item;
  RealRegister reg;
