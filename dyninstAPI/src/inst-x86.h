...
#define AMD64_STACK_ALIGNMENT                                                            \
    32  // This is extremely conservative.
        // 16 may be enough.
...
void
emitOpRegRM(unsigned opcode, RealRegister dest, RealRegister base, int disp,
            codeGen& gen);
void
emitOpExtRegImm8(int opcode, char ext, RealRegister dest, unsigned char imm,
                 codeGen& gen);
void
emitOpExtRegImm(int opcode, int opcode2, RealRegister dest, int imm, codeGen& gen);
void
...
emitPushImm(unsigned int imm, codeGen& gen);
void
emitAddMemImm32(Address dest, int imm, codeGen& gen);
void
...
struct stackItemLocation
{
    RealRegister reg;
    int          offset;
    stackItemLocation(RealRegister r, int o)
    {
        reg    = r;
        offset = o;
    }
