...
bool Modification::processBlock(BlockList::iterator &iter) {
...
void Modification::replaceCall(BlockPtr block, int_function *target, instPoint *cur) {
  Block::ElementList &elements = block->elements();
  ...
  cerr << "Replacing call @" << hex << cf->addr() << dec
  ...
  if (block->bbl()->func()->obj() == target->obj()) {
    ...
    CFElement::DestinationMap::iterator d_iter = cf->destMap_.find(CFElement::Taken);
    ...
    Element::Ptr callAST = ASTElement::create(AstNode::funcCallNode(target, args), cur);
    elements.back().swap(callAST);
  }
}


void Modification::replaceFunction(BlockPtr block, int_function *to) {
  ...
  // And now to create a BaseTramp. Let's see if we can find one
  // in the function...
...
void Modification::removeCall(BlockPtr block) {
  const Block::ElementList &elements = block->elements();
  ...
  CFElement::DestinationMap::iterator d_iter = cf->destMap_.find(CFElement::Fallthrough);
  TargetInt *ftTarg = d_iter->second;

  for (CFElement::DestinationMap::iterator e_iter = cf->destMap_.begin();
       e_iter != cf->destMap_.end(); ++e_iter) {
    if (e_iter->second != ftTarg)
      delete e_iter->second;
  }
  cf->destMap_.clear();
  cf->destMap_[CFElement::Fallthrough] = ftTarg;
}
