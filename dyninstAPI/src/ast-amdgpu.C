...
AstNodePtr AstNode::stackGenericNode() { return AstNodePtr(new AstStackGenericNode()); }

AstNodePtr AstNode::labelNode(std::string &label) { return AstNodePtr(new AstLabelNode(label)); }
...
AstCallNode::AstCallNode(func_instance *func, std::vector<AstNodePtr> &args)
    : AstNode(), func_addr_(0), func_(func), callReplace_(false), constFunc_(false) {
...
AstCallNode::AstCallNode(func_instance *func)
    : AstNode(), func_addr_(0), func_(func), callReplace_(true), constFunc_(false) {}

AstCallNode::AstCallNode(const std::string &func, std::vector<AstNodePtr> &args)
    : AstNode(), func_name_(func), func_addr_(0), func_(NULL), callReplace_(false),
      constFunc_(false) {
  for (unsigned i = 0; i < args.size(); i++) {
    args[i]->referenceCount++;
    args_.push_back(args[i]);
...
AstCallNode::AstCallNode(Address addr, std::vector<AstNodePtr> &args)
    : AstNode(), func_addr_(addr), func_(NULL), callReplace_(false), constFunc_(false) {
  for (unsigned i = 0; i < args.size(); i++) {
    args[i]->referenceCount++;
    args_.push_back(args[i]);
  }
...
AstSequenceNode::AstSequenceNode(std::vector<AstNodePtr> &sequence) : AstNode() {
...
AstMemoryNode::AstMemoryNode(memoryType mem, unsigned which, int size_)
...
Address AstMiniTrampNode::generateTramp(codeGen &gen, int &trampCost, bool noCost) {
...
bool AstNode::generateCode_phase2(codeGen &, bool, Address &, Dyninst::Register &) {
  ...
  if (dynamic_cast<AstNullNode *>(this))
  ...
  if (dynamic_cast<AstOperatorNode *>(this))
    fprintf(stderr, "operatorNode\n");
  if (dynamic_cast<AstOperandNode *>(this))
    fprintf(stderr, "operandNode\n");
  if (dynamic_cast<AstCallNode *>(this))
    fprintf(stderr, "callNode\n");
  if (dynamic_cast<AstSequenceNode *>(this))
    fprintf(stderr, "seqNode\n");
  if (dynamic_cast<AstVariableNode *>(this))
    fprintf(stderr, "varNode\n");
...
bool AstOperatorNode::generateOptimizedAssignment(codeGen &, int, bool) { return false; }

bool AstOperatorNode::generateCode_phase2(codeGen &gen, bool noCost, Address &retAddr,
                                          Dyninst::Register &retReg) {
  if (!loperand) {
    return false;
  }
  ...
  case ifOp: {
    ...
    if (eoperand) {
      ...
      if (preelse_patches_size != postelse_patches_size) {
        ...
        emitA(branchOp, 0, 0, (Dyninst::Register)codeGen::getDisplacement(elseSkipStart, endIndex),
  ...
  case getAddrOp: {
    ...
    case operandType::variableValue:
      ...
      assert(loperand->getOVar());
      loperand->emitVariableLoad(loadOp, retReg, retReg, gen, noCost, gen.rs(), size, gen.point(),
                                 gen.addrSpace());
      break;
    ...
    default:
      cerr << "Uh oh, unknown loperand type in getAddrOp: "
           << static_cast<uint64_t>(loperand->getoType()) << endl;
  ...
  case storeOp: {
    ...
    case operandType::FrameAddr:
      addr = (Address)loperand->getOValue();
      emitVstore(storeFrameRelativeOp, src1, src2, addr, gen, noCost, gen.rs(), size, gen.point(),
                 gen.addrSpace());
      loperand->decUseCount(gen);
      break;
    case operandType::RegOffset: {
      ...
      emitVload(loadRegRelativeAddr, addr, (long)loperand->getOValue(), src2, gen, noCost, gen.rs(),
    ...
    case operandType::ParamAtEntry: {
      ...
      emitR(getParamOp, (Address)lnode->oValue, src1, src2, gen, noCost, gen.point(),
    ...
    case operandType::AddressAsPlaceholderRegAndOffset: {
      ...
      AstOperandNode *offset =
          dynamic_cast<AstOperandNode *>((AstNode *)loperand.get()->operand().get());
      assert(offset);
  ...
  case divOp:
    assert(
        false &&
        "divOp needs an algorithmic implementation. AMDGPU doesn't have integer division opcode.");
    break;
  case plusOp:
  case minusOp:
  case xorOp:
...
bool AstOperandNode::generateCode_phase2(codeGen &gen, bool noCost, Address &,
  ...
  case operandType::DataReg:
    ...
    break;
  case operandType::origRegister: {
    // For AMDGPU, we will treat origRegister as normal register (DataReg) until we support
    // spilling. gen.rs()->readProgramRegister(gen, (Dyninst::Register)(long)oValue, retReg, size);
    retReg = (Dyninst::Register)(long)oValue;
    break;
  }
  case operandType::variableAddr:
  ...
  case operandType::ParamAtEntry: {
    ...
    src = emitR(paramOp, (Address)oValue, Dyninst::Null_Register, retReg, gen, noCost, gen.point(),
                gen.addrSpace()->multithread_capable());
    REGISTER_CHECK(src);
    if (src != retReg) {
      // Move src to retReg. Can't simply return src, since it was not
...
bool AstVariableNode::generateCode_phase2(codeGen &gen, bool noCost, Address &addr,
                                          Dyninst::Register &retReg) {
  return ast_wrappers_[index]->generateCode_phase2(gen, noCost, addr, retReg);
}

bool AstOriginalAddrNode::generateCode_phase2(codeGen &gen, bool noCost, Address &,
                                              Dyninst::Register &retReg) {
  RETURN_KEPT_REG(retReg);
  if (retReg == Dyninst::Null_Register) {
    retReg = allocateAndKeep(gen, noCost);
  ...
  emitVload(loadConstOp, (Address)gen.point()->addr_compat(), retReg, retReg, gen, noCost);
  return true;
}

bool AstActualAddrNode::generateCode_phase2(codeGen &gen, bool noCost, Address &,
                                            Dyninst::Register &retReg) {
  if (retReg == Dyninst::Null_Register) {
    retReg = allocateAndKeep(gen, noCost);
  }
...
int AstCallNode::costHelper(enum CostStyleType costStyle) const {
  ...
  for (unsigned u = 0; u < args_.size(); u++)
    if (args_[u])
      total += args_[u]->costHelper(costStyle);
  return total;
}
...
BPatch_type *AstNode::checkType(BPatch_function *) { return BPatch::bpatch->type_Untyped; }
...
bool AstNode::accessesParam() { return false; }
...
bool AstVariableNode::accessesParam() { return ast_wrappers_[index]->accessesParam(); }
...
bool AstVariableNode::canBeKept() const { return ast_wrappers_[index]->canBeKept(); }
...
AstNodePtr AstOperandNode::deepCopy() {
  ...
  copy->oVar = oVar;
  if (operand_)
    copy->operand_ = operand_->deepCopy();
...
void AstMiniTrampNode::getChildren(std::vector<AstNodePtr> &children) { children.push_back(ast_); }
...
void AstVariableNode::setVariableAST(codeGen &gen) {
  // fprintf(stderr, "Generating code for variable in function %s with start address 0x%lx at
  ...
  Address addr =
      gen.point()->addr_compat(); // Dyninst::Offset of inst point from function base address
  bool found = false;
...
void AstMiniTrampNode::setVariableAST(codeGen &g) {
  if (ast_)
    ast_->setVariableAST(g);
}

bool AstCallNode::containsFuncCall() const { return true; }

bool AstOperatorNode::containsFuncCall() const {
  if (loperand && loperand->containsFuncCall())
    return true;
  if (roperand && roperand->containsFuncCall())
    return true;
  if (eoperand && eoperand->containsFuncCall())
    return true;
  return false;
}

bool AstOperandNode::containsFuncCall() const {
  if (operand_ && operand_->containsFuncCall())
    return true;
  return false;
}

bool AstMiniTrampNode::containsFuncCall() const {
  if (ast_ && ast_->containsFuncCall())
    return true;
  return false;
}
...
bool AstSequenceNode::containsFuncCall() const {
  for (unsigned i = 0; i < sequence_.size(); i++) {
    if (sequence_[i]->containsFuncCall())
      return true;
  }
  return false;
}

bool AstVariableNode::containsFuncCall() const { return ast_wrappers_[index]->containsFuncCall(); }
...
bool AstCallNode::usesAppRegister() const {
  for (unsigned i = 0; i < args_.size(); i++) {
    if (args_[i] && args_[i]->usesAppRegister())
      return true;
  }
  return false;
}
...
bool AstOperatorNode::usesAppRegister() const {
  if (loperand && loperand->usesAppRegister())
    return true;
  if (roperand && roperand->usesAppRegister())
    return true;
  if (eoperand && eoperand->usesAppRegister())
    return true;
  return false;
}

bool AstOperandNode::usesAppRegister() const {
  if (oType == AstNode::operandType::FrameAddr || oType == AstNode::operandType::RegOffset ||
      oType == AstNode::operandType::origRegister || oType == AstNode::operandType::Param ||
      oType == AstNode::operandType::ParamAtEntry || oType == AstNode::operandType::ParamAtCall ||
...
bool AstSequenceNode::usesAppRegister() const {
  for (unsigned i = 0; i < sequence_.size(); i++) {
    if (sequence_[i]->usesAppRegister())
      return true;
  }
  return false;
}

bool AstVariableNode::usesAppRegister() const { return ast_wrappers_[index]->usesAppRegister(); }
...
void regTracker_t::debugPrint() {
  ...
  for (auto iter = tracker.begin(); iter != tracker.end(); ++iter) {
    fprintf(stderr, "AstNode %p: register %u, condition level %d\n", (void *)iter->first,
            iter->second.keptRegister, iter->second.keptLevel);
  }
...
bool AstSnippetNode::generateCode_phase2(codeGen &gen, bool, Address &, Dyninst::Register &) {
...
std::string AstOperandNode::format(std::string indent) {
  std::stringstream ret;
  ret << indent << "Oper/" << hex << this << dec << "(" << convert(oType) << "/" << oValue << ")"
      << endl;
  if (operand_)
...
std::string AstNode::convert(operandType type) {
  ...
  case operandType::AddressAsPlaceholderRegAndOffset:
    return "AddressAsPlaceholderRegAndOffset";
  case operandType::DataReg:
    return "DataReg";
  case operandType::DataIndir:
    return "DataIndir";
