...
void
insnCodeGen::generateMoveToSPR(codeGen& gen, Register toSPR, unsigned sprReg)
{
...
void
insnCodeGen::generateMoveFromSPR(codeGen& gen, Register toSPR, unsigned sprReg)
{
    // Check that this SPR exists
    if(sprReg != SPR_TAR && sprReg != SPR_LR && sprReg != SPR_CTR)
        assert("SPR Register is not valid" == 0);
...
void
insnCodeGen::generateVectorLoad(codeGen& gen, unsigned vectorReg, Register RegAddress)
{
...
void
insnCodeGen::generateVectorStore(codeGen& gen, unsigned vectorReg, Register RegAddress)
{
...
bool
insnCodeGen::generateBranchTar(codeGen& gen, Register scratch, Address dest, bool isCall)
{
...
bool
insnCodeGen::generateBranchLR(codeGen& gen, Register scratch, Address dest, bool isCall)
{
...
#include "function.h"
instPoint*
GetInstPointPower(codeGen& gen, Address from)
{
...
insnCodeGen::generateLongBranch(codeGen& gen, Address from, Address to, bool isCall)
{
    ...
    if(isCall)
    {
        ...
        // First generate a scratch register by moving something, i choose R11 to send to
        // TAR
    ...
    else
    {
        ...
        if(scratch == REG_NULL)
        {
            ...
            if(scratch == REG_NULL)
            {
                ...
                if(!usingLR && !usingCTR)
                {
                    // fprintf(stderr, "%s\n", "Couldn't grab free register - Using a trap
                    // instruction.....");
                    return generateBranchViaTrap(gen, from, to, isCall);
                }
            }
        ...
        {
            // Now the fun stuff....
            // Loed destination value into r11, copy it to SPR_TAR, restore the original
            // R11 value.
            insnCodeGen::loadImmIntoReg(gen, registerSpace::r10, to);
            insnCodeGen::generateMoveToSPR(gen, registerSpace::r10, SPR_TAR);
            if(usingCTR)
    ...
    //       __LINE__); fprintf(stderr, "This is an instruction which we would trap
    //       on\n"); fprintf(stderr, "[insnCodeGen::generateLongBranch] Building long
    //       branch from %llx to %llx at position %llx and this branch call status is
...
{
    // fprintf(stderr, "[insnCodeGen::generateBranchViaTrap] Generating branch via trap
    // from %p to %p\n", from, to);
    long disp = to - from;
    if(ABS(disp) <= MAX_BRANCH)
    {
...
void
insnCodeGen::generateAddReg(codeGen& gen, int op, Register rt, Register ra, Register rb)
{
    instruction insn;
    insn.clear();
...
void
insnCodeGen::generateLoadReg(codeGen& gen, Register rt, Register ra, Register rb)
{
    instruction insn;
    insn.clear();
...
void
insnCodeGen::generateStoreReg(codeGen& gen, Register rt, Register ra, Register rb)
{
    instruction insn;
    insn.clear();
...
void
insnCodeGen::generateLoadReg64(codeGen& gen, Register rt, Register ra, Register rb)
{
    instruction insn;
    insn.clear();
    XFORM_OP_SET(insn, LXop);
...
void
insnCodeGen::generateStoreReg64(codeGen& gen, Register rs, Register ra, Register rb)
{
    instruction insn;
    insn.clear();
    XFORM_OP_SET(insn, STXop);
...
void
insnCodeGen::generateImm(codeGen& gen, int op, Register rt, Register ra, int immd)
{
...
void
insnCodeGen::generateMemAccess64(codeGen& gen, int op, int xop, Register r1, Register r2,
                                 int immd)
{
...
void
insnCodeGen::generateLShift(codeGen& gen, Register rs, int shift, Register ra)
{
    instruction insn;
...
void
insnCodeGen::generateRShift(codeGen& gen, Register rs, int shift, Register ra, bool s)
{
    instruction insn;
...
// sld ra, rs, rb
void
insnCodeGen::generateLShift64(codeGen& gen, Register rs, int shift, Register ra)
{
    instruction insn;
...
// srd ra, rs, rb
void
insnCodeGen::generateRShift64(codeGen& gen, Register rs, int shift, Register ra, bool)
{
...
void
insnCodeGen::generateSimple(codeGen& gen, int op, Register src1, Register src2,
                            Register dest)
{
    instruction insn;
...
void
insnCodeGen::generateRelOp(codeGen& gen, int cond, int mode, Register rs1, Register rs2,
                           Register rd, bool s)
{
    instruction insn;
...
void
insnCodeGen::loadImmIntoReg(codeGen& gen, Register rt, long value)
{
...
// stores.
void
insnCodeGen::loadPartialImmIntoReg(codeGen& gen, Register rt, long value)
{
    if(MIN_IMM16 <= value && value <= MAX_IMM16)
        return;
...
int
insnCodeGen::createStackFrame(codeGen& gen, int numRegs, std::vector<Register>& freeReg,
...
void
insnCodeGen::generateMoveFromLR(codeGen& gen, Register rt)
{
    instruction insn;
    insn.clear();
...
void
insnCodeGen::generateMoveToLR(codeGen& gen, Register rs)
{
    instruction insn;
    insn.clear();
...
void
insnCodeGen::generateMoveToCR(codeGen& gen, Register rs)
{
    instruction insn;
    insn.clear();
    XFORM_OP_SET(insn, MTSPRop);
...
bool
insnCodeGen::modifyJump(Address target, NS_power::instruction&, codeGen& gen)
{
    ...
    generateBranch(gen, gen.currAddr(), target, false);
...
bool
insnCodeGen::modifyJumpCall(Address target, NS_power::instruction&, codeGen& gen)
{
    failedLongBranchLocal      = false;
    shouldAssertIfInLongBranch = false;
    // fprintf(stderr, "Setting link: %d Target Address: %p\n", IFORM_LK(insn), target);
    // assert(IFORM_LK(insn) == true);
    generateBranch(gen, gen.currAddr(), target, true);
    if(failedLongBranchLocal == true)
    {
        failedLongBranchLocal      = false;
        shouldAssertIfInLongBranch = true;
...
bool
insnCodeGen::modifyJcc(Address target, NS_power::instruction& insn, codeGen& gen)
{
    ...
    if(ABS(disp) >= MAX_CBRANCH)
    {
        ...
        else
        {
            ...
            insnCodeGen::generateBranch(gen, 2 * instruction::size());

            bool link = (BFORM_LK(insn) == 1);
            // Make sure to use the (to, from) version of generateBranch()
            // in case the branch is too far, and trap-based instrumentation
            // is needed.
            insnCodeGen::generateBranch(gen, gen.currAddr(), target, link);
            return true;
        }
    }
    else
