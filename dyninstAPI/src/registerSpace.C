  ...
  for (unsigned i = 0; i < registers.size(); i++) {
    ...
    switch (registers[i]->type) {
      case registerSlot::GPR: {
        ...
        if (rs->addr_width == 4) physical = false;
#endif
...
Register registerSpace::getScratchRegister(codeGen &gen,
  ...
  regalloc_printf(
      "Allocating register: selection is %s\n",
  ...
  for (unsigned i = 0; i < regs.size(); i++) {
    ...
    regalloc_printf(
        "%s[%d]: getting scratch register, examining %d of %d: reg %d (%s), "
...
bool registerSpace::checkVolatileRegisters(
    codeGen & /*gen*/, registerSlot::livenessState_t state) {
  if (addr_width == 8) {
    ...
    for (unsigned i = REGNUM_OF; i <= REGNUM_RF; i++) {
      if (registers_[i]->liveState == state) return true;
    }
    return false;
  }
...
bool registerSpace::saveVolatileRegisters(codeGen &gen) {
  savedFlagSize = 0;
  if (!checkVolatileRegisters(gen, registerSlot::live)) return false;
...
bool registerSpace::restoreVolatileRegisters(codeGen &gen) {
  if (!checkVolatileRegisters(gen, registerSlot::spilled)) return false;
...
bool registerSpace::isFreeRegister(Register num) {
  ...
  if (reg->refCount > 0) return false;
  if (reg->keptValue) return false;
  return true;
}
...
bool registerSpace::readProgramRegister(codeGen &gen, Register source,
                                        Register destination, unsigned
#if !defined(arch_power)
                                                                  size
#endif
                                        ) {
  ...
  switch (src->spilledState) {
    ...
    case registerSlot::framePointer: {
      ...
      gen.codeEmitter()->emitLoadRelative(
          destination, src->saveOffset, framePointer(),
...
bool registerSpace::writeProgramRegister(codeGen &gen, Register destination,
                                         Register source, unsigned) {
  ...
  switch (src->spilledState) {
    ...
    case registerSlot::framePointer: {
      ...
      // pointer.
      // Use that to load it.
      gen.codeEmitter()->emitStoreRelative(
          source, dest->saveOffset, framePointer(),
          gen.addrSpace()->getAddressWidth(), gen);
      return true;
      break;
    }
...
void registerSlot::debugPrint(const char *prefix) {
  ...
  fprintf(stderr,
          "Num: %d, name %s, type %s, refCount %d, liveState %s, beenUsed %d, "
          "initialState %s, offLimits %d, keptValue %d, alloc %d\n",
          number, name.c_str(),
          (type == GPR) ? "GPR" : ((type == FPR) ? "FPR" : "SPR"), refCount,
          (liveState == live) ? "live"
                              : ((liveState == spilled) ? "spilled" : "dead"),
          beenUsed,
...
bool registerSpace::anyLiveGPRsAtEntry() const {
  for (unsigned i = 0; i < GPRs_.size(); i++) {
    if (GPRs_[i]->liveState != registerSlot::dead) return true;
  }
  return false;
}
...
bool registerSpace::anyLiveFPRsAtEntry() const {
  for (unsigned i = 0; i < FPRs_.size(); i++) {
    if (FPRs_[i]->liveState != registerSlot::dead) return true;
  }
  return false;
}
...
bool registerSpace::anyLiveSPRsAtEntry() const {
  for (unsigned i = 0; i < SPRs_.size(); i++) {
    if (SPRs_[i]->liveState != registerSlot::dead) return true;
  }
  return false;
}
...
#if defined(arch_x86) || defined(arch_x86_64)
  if (addr_width == 4) return realRegs();
#endif
  return GPRs_;
}
...
Register registerSpace::getRegByName(const std::string name) {
  ...
  if (cur == registersByName.end()) return REG_NULL;
  return (*cur).second;
}
...
RealRegister registerSpace::findReal(registerSlot *virt_r,
                                     bool &already_setup) {
  assert(virt_r);
  ...
  for (unsigned i = 0; i < regState().size(); i++) {
    ...
    if (!real_reg.is_allocatable) continue;
  ...
  if (used_free_reg != -1) return RealRegister(used_free_reg);
...
void registerSpace::spillReal(RealRegister r, codeGen &gen) {
  if (!regState()[r.reg()].is_allocatable) return;
  if (!regState()[r.reg()].contains) return;
...
void registerSpace::loadReal(RealRegister r, registerSlot *virt_r,
                             codeGen &gen) {
...
RealRegister registerSpace::loadVirtualForWrite(registerSlot *virt_r,
                                                codeGen &gen) {
  assert(virt_r);
  bool done;
...
void registerSpace::loadVirtualToSpecific(registerSlot *virt_r,
                                          RealRegister real_r, codeGen &gen) {
  bool already_in_reg;
...
void registerSpace::loadVirtualToSpecific(Register virt_r, RealRegister real_r,
                                          codeGen &gen) {
...
bool registerSpace::spilledAnything() { return regs_been_spilled.size() != 0; }
...
void registerSpace::unifyTopRegStates(codeGen &gen) {
  ...
  if (regStateStack.size() == 0) return;
  ...
  for (unsigned i = 0; i < dest->registerStates.size(); i++) {
    ...
    if (!rrs.is_allocatable || !rrs.contains) continue;
  ...
  for (unsigned i = 0; i < src->registerStates.size(); i++) {
    ...
    std::map<registerSlot *, unsigned>::iterator j =
        dest_virts.find(rrs.contains);
...
int &registerSpace::pc_rel_offset() {
  if (!regStateStack.size()) initRealRegSpace();
...
void registerSpace::setInstFrameSize(int val) { instFrameSize_ = val; }

int registerSpace::getInstFrameSize() { return instFrameSize_; }
...
int registerSpace::getStackHeight() { return 0; }
#endif
