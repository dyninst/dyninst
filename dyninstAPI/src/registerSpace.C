...
registerSpace*
registerSpace::getRegisterSpace(unsigned width)
{
    if(globalRegSpace_ == NULL)
        initialize();
...
registerSpace*
registerSpace::getRegisterSpace(AddressSpace* as)
{
    return getRegisterSpace(as->getAddressWidth());
}

registerSpace*
registerSpace::conservativeRegSpace(AddressSpace* proc)
{
...
registerSpace*
registerSpace::optimisticRegSpace(AddressSpace* proc)
{
    registerSpace* ret = getRegisterSpace(proc);
...
registerSpace*
registerSpace::irpcRegSpace(AddressSpace* proc)
{
...
registerSpace*
registerSpace::savedRegSpace(AddressSpace* proc)
{
    registerSpace* ret = getRegisterSpace(proc);
...
registerSpace*
registerSpace::actualRegSpace(instPoint* iP)
{
...
void
registerSpace::createRegisterSpace(std::vector<registerSlot*>& registers)
{
...
void
registerSpace::createRegisterSpace64(std::vector<registerSlot*>& registers)
{
    // We need to initialize the following:
    // registers_ (std::unordered_map)
    // GPRs_ (vector of pointers to elements in registers_
...
void
registerSpace::createRegSpaceInt(std::vector<registerSlot*>& registers, registerSpace* rs)
{
...
bool
registerSpace::trySpecificRegister(codeGen& gen, Register num, bool noCost)
{
...
bool
registerSpace::allocateSpecificRegister(codeGen& gen, Register num, bool noCost)
{
...
Register
registerSpace::getScratchRegister(codeGen& gen, bool noCost, bool realReg)
{
...
Register
registerSpace::getScratchRegister(codeGen& gen, std::vector<Register>& excluded,
                                  bool noCost, bool realReg)
{
    ...
    regalloc_printf("Allocating register: selection is %s\n",
                    realReg ? (realRegisters_.empty() ? "GPRS" : "Real registers")
                            : "GPRs");

    std::vector<registerSlot*>& regs =
        (realReg ? (realRegisters_.empty() ? GPRs_ : realRegisters_) : GPRs_);
    ...
    for(unsigned i = 0; i < regs.size(); i++)
    {
        ...
        regalloc_printf(
            "%s[%d]: getting scratch register, examining %u of %lu: reg %u (%s), "
            "offLimits %d, refCount %d, liveState %s, keptValue %d\n",
            FILE__, __LINE__, i, regs.size(), reg->number, reg->name.c_str(),
            reg->offLimits, reg->refCount,
            (reg->liveState == registerSlot::live)
                ? "live"
                : ((reg->liveState == registerSlot::dead) ? "dead" : "spilled"),
            reg->keptValue);
...
bool
registerSpace::spillRegister(Register reg, codeGen&, bool /*noCost*/)
{
...
bool
registerSpace::stealRegister(Register reg, codeGen& gen, bool /*noCost*/)
{
    ...
    if(!gen.tracker()->stealKeptRegister(registers_[reg]->number))
        return false;
...
bool
registerSpace::checkVolatileRegisters(codeGen& /*gen*/,
...
#else
bool
registerSpace::checkVolatileRegisters(codeGen&, registerSlot::livenessState_t)
{
    assert(0);
    return false;
}
...
#if defined(arch_x86_64) || defined(arch_x86)
bool
registerSpace::saveVolatileRegisters(codeGen& gen)
{
    savedFlagSize = 0;
    ...
    if(addr_width == 8)
    {
        ...
        if(registers_[REGNUM_SF]->liveState == registerSlot::live ||
           ...
           registers_[REGNUM_CF]->liveState == registerSlot::live || override)
        {
        ...
        if(registers_[REGNUM_OF]->liveState == registerSlot::live || override)
        {
            emitSaveO(gen);
...
#else
bool
registerSpace::saveVolatileRegisters(codeGen&)
{
    assert(0);
    return true;
}
...
#if defined(arch_x86_64) || defined(arch_x86)
bool
registerSpace::restoreVolatileRegisters(codeGen& gen)
{
...
#else
bool
registerSpace::restoreVolatileRegisters(codeGen&)
{
    assert(0);
    return true;
}
...
void
registerSpace::freeRegister(Register num)
{
...
void
registerSpace::forceFreeRegister(Register num)
{
    registerSlot* reg = findRegister(num);
    if(!reg)
        return;
...
bool
registerSpace::isFreeRegister(Register num)
{
    registerSlot* reg = findRegister(num);
    if(!reg)
        return false;
...
void
registerSpace::incRefCount(Register num)
{
    registerSlot* reg = findRegister(num);
    if(!reg)
        return;
...
bool
registerSpace::restoreAllRegisters(codeGen&, bool)
{
    assert(0);
    return true;
}
...
bool
registerSpace::restoreRegister(Register, codeGen&, bool /*noCost*/)
{
    assert(0);
    return true;
}
...
bool
registerSpace::popRegister(Register, codeGen&, bool)
{
    assert(0);
    return true;
}
...
bool
registerSpace::readProgramRegister(codeGen& gen, Register source, Register destination,
                                   unsigned
#if !defined(arch_power)
                                       size
#endif
...
{
#if !defined(arch_power)
    emitLoadPreviousStackFrameRegister((Address) source, destination, gen, size, true);
    return true;
#elif defined(arch_aarch64)
    ...
    switch(src->spilledState)
    {
        ...
        case registerSlot::framePointer: {
            ...
            gen.codeEmitter()->emitLoadRelative(destination, src->saveOffset,
                                                framePointer(),
...
bool
registerSpace::writeProgramRegister(codeGen& gen, Register destination, Register source,
                                    unsigned)
{
    ...
    switch(src->spilledState)
    {
        ...
        case registerSlot::framePointer: {
            ...
            // Use that to load it.
            gen.codeEmitter()->emitStoreRelative(source, dest->saveOffset, framePointer(),
                                                 gen.addrSpace()->getAddressWidth(), gen);
            return true;
            break;
        }
...
registerSlot*
registerSpace::findRegister(Register source)
{
...
registerSlot*
registerSpace::findRegister(RealRegister source)
{
    return realRegisters_[source.reg()];
}

bool
registerSpace::markSavedRegister(RealRegister num, int offsetFromFP)
{
...
bool
registerSpace::markSavedRegister(Register num, int offsetFromFP)
{
...
bool
registerSpace::markSavedRegister(registerSlot* s, int offsetFromFP)
{
    if(s == NULL)
    {
...
void
registerSlot::debugPrint(const char* prefix)
{
    if(!dyn_debug_regalloc)
        return;

    if(prefix)
        fprintf(stderr, "%s", prefix);
    fprintf(stderr,
            "Num: %u, name %s, type %s, refCount %d, liveState %s, beenUsed %d, "
            "initialState %s, offLimits %d, keptValue %d, alloc %d\n",
            number, name.c_str(), (type == GPR) ? "GPR" : ((type == FPR) ? "FPR" : "SPR"),
            refCount,
            (liveState == live) ? "live" : ((liveState == spilled) ? "spilled" : "dead"),
            beenUsed,
            (initialState == deadAlways)
                ? "always dead"
                : ((initialState == deadABI) ? "ABI dead" : "always live"),
            offLimits, keptValue, alloc_num);
}

void
registerSpace::debugPrint()
{
    if(!dyn_debug_regalloc)
        return;
    ...
    fprintf(stderr, "GPRs: %ld, FPRs: %ld, SPRs: %ld\n", (long) GPRs_.size(),
            (long) FPRs_.size(), (long) SPRs_.size());
...
bool
registerSpace::markKeptRegister(Register reg)
{
...
void
registerSpace::unKeepRegister(Register reg)
{
...
void
registerSpace::specializeSpace(rs_location_t state)
{
    for(auto i = registers_.begin(); i != registers_.end(); i++)
    {
...
bool
registerSpace::anyLiveGPRsAtEntry() const
{
    for(unsigned i = 0; i < GPRs_.size(); i++)
    {
...
bool
registerSpace::anyLiveFPRsAtEntry() const
{
    for(unsigned i = 0; i < FPRs_.size(); i++)
    {
...
bool
registerSpace::anyLiveSPRsAtEntry() const
{
    for(unsigned i = 0; i < SPRs_.size(); i++)
    {
...
registerSlot* registerSpace::operator[](Register reg) { return registers_[reg]; }

// Big honkin' name->register map

void
registerSpace::getAllRegisterNames(std::vector<std::string>& ret)
{
    // Currently GPR only
    for(unsigned i = 0; i < GPRs_.size(); i++)
    {
...
std::string
registerSpace::getRegByNumber(Register reg)
{
...
RealRegister
registerSpace::findReal(registerSlot* virt_r, bool& already_setup)
{
    assert(virt_r);
    ...
    for(unsigned i = 0; i < regState().size(); i++)
    {
        ...
        if(!real_reg.contains && real_reg.been_used)
        {
            used_free_reg = i;
        }
        ...
        if(real_reg.contains &&
           (oldest_reg == -1 || real_reg.last_used < regState()[oldest_reg].last_used))
        {
...
void
registerSpace::spillReal(RealRegister r, codeGen& gen)
{
...
void
registerSpace::loadReal(RealRegister r, registerSlot* virt_r, codeGen& gen)
{
...
void
registerSpace::freeReal(RealRegister r)
{
...
RealRegister
registerSpace::loadVirtual(registerSlot* virt_r, codeGen& gen)
{
    assert(virt_r);
    bool done;
...
RealRegister
registerSpace::loadVirtual(Register virt_r, codeGen& gen)
{
    return loadVirtual((*this)[virt_r], gen);
}

RealRegister
registerSpace::loadVirtualForWrite(registerSlot* virt_r, codeGen& gen)
{
    assert(virt_r);
    bool done;
...
RealRegister
registerSpace::loadVirtualForWrite(Register virt_r, codeGen& gen)
{
    return loadVirtualForWrite((*this)[virt_r], gen);
}

void
registerSpace::makeRegisterAvail(RealRegister r, codeGen& gen)
{
    spillReal(r, gen);
...
void
registerSpace::noteVirtualInReal(registerSlot* v_r, RealRegister r_r)
{
...
void
registerSpace::noteVirtualInReal(Register v_r, RealRegister r_r)
{
    noteVirtualInReal((*this)[v_r], r_r);
}

void
registerSpace::loadVirtualToSpecific(registerSlot* virt_r, RealRegister real_r,
                                     codeGen& gen)
{
    bool         already_in_reg;
...
void
registerSpace::loadVirtualToSpecific(Register virt_r, RealRegister real_r, codeGen& gen)
{
    loadVirtualToSpecific((*this)[virt_r], real_r, gen);
}

void
registerSpace::markVirtualDead(Register num)
{
 ...
 **/
void
registerSpace::unifyTopRegStates(codeGen& gen)
{
    if(addr_width == 8)
    {
...
std::vector<RealRegsState>&
registerSpace::regState()
{
    if(!regStateStack.size())
        initRealRegSpace();
...
void
registerSpace::incStack(int val)
{
    if(!regStateStack.size())
        initRealRegSpace();
...
void
registerSpace::setStackHeight(int val)
{
    if(!regStateStack.size())
        initRealRegSpace();
    regStateStack[regStateStack.size() - 1]->stack_height = val;
...
void
registerSpace::setInstFrameSize(int val)
{
    instFrameSize_ = val;
}

int
registerSpace::getInstFrameSize()
{
...
void
registerSpace::spillToVReg(RealRegister /*reg*/, registerSlot* /*v_reg*/,
                           codeGen& /*gen*/)
{
    assert(0);
}
...
void
registerSpace::movVRegToReal(registerSlot* /*v_reg*/, RealRegister /*r*/,
                             codeGen& /*gen*/)
{
    assert(0);
}
...
regState_t::regState_t() { assert(0); }
...
void
registerSpace::incStack(int)
{}

void
registerSpace::setStackHeight(int)
{}
...
void
registerSpace::specializeSpace(const bitArray& liveRegs)
{
...
bool
registerSpace::checkLive(Register reg, const bitArray& liveRegs)
{
    static LivenessAnalyzer live1(4);
    static LivenessAnalyzer live2(8);
    std::pair<std::multimap<Register, MachRegister>::iterator,
              std::multimap<Register, MachRegister>::iterator>
                      range;
    ...
    if(range.first == range.second)
        assert(0);
    for(std::multimap<Register, MachRegister>::iterator iter = range.first;
