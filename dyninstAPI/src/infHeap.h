 ...
 public:
  heapItem()
      : addr(0),
        ...
        status(HEAPfree),
        buffer(NULL) {}
  heapItem(Address a, int n, inferiorHeapType t, bool d = true,
           heapStatus s = HEAPfree)
      : addr(a), length(n), type(t), dynamic(d), status(s), buffer(NULL) {}
...
class disabledItem {
 public:
  ...
  std::vector<addrVecType>
      pointsToCheck;  // list of addresses to check against PCs

  Address getPointer() const { return block.addr; }
  inferiorHeapType getHeapType() const { return block.type; }
  const std::vector<addrVecType> &getPointsToCheck() const {
...
class heapDescriptor {
 public:
  heapDescriptor(const std::string name, Address addr, unsigned int size,
