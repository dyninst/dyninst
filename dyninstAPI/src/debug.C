...
int
trap_printf(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
int
signal_printf_int(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
int
inferiorrpc_printf_int(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
int
startup_printf_int(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
int
bpatch_printf(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
...
void
BPatch_reportError(int errLevel, int num, const char* str)
{
...
void
showInfoCallback(std::string msg)
{
...
void
showErrorCallback(int num, std::string msg)
{
...
void
statusLine(const char* line, bool /* unused */)
{
...
int
bpfatal_lf(const char* __file__, unsigned int __line__, const char* format, ...)
{
    fprintf(stderr, "%s[%d]\n", __FILE__, __LINE__);
    if(NULL == format)
        return -1;
    ...
    fprintf(stderr, "%s[%d]\n", __FILE__, __LINE__);
    int header_len =
        sprintf(errbuf, "[%lu]%s[%u]: ", getExecThreadID(), __file__, __line__);
...
bool
check_env_value(const char* env_var)
{
...
int
signal_printf_int(const char* format, ...)
{
    if(!dyn_debug_signal)
        return 0;
    if(NULL == format)
...
int
inferiorrpc_printf_int(const char* format, ...)
{
    if(!dyn_debug_infrpc)
        return 0;
    if(NULL == format)
...
int
startup_printf_int(const char* format, ...)
{
...
int
parsing_printf_int(const char* format, ...)
{
...
int
proccontrol_printf_int(const char* format, ...)
{
...
int
stackwalk_printf_int(const char* format, ...)
{
...
int
inst_printf_int(const char* format, ...)
{
    if(!dyn_debug_inst)
        return 0;
    if(NULL == format)
...
int
reloc_printf_int(const char* format, ...)
{
    if(!dyn_debug_reloc)
        return 0;
    if(NULL == format)
...
int
dyn_unw_printf_int(const char* format, ...)
{
    if(!dyn_debug_dyn_unw)
        return 0;
    if(NULL == format)
        return -1;
...
int
mutex_printf_int(const char* format, ...)
{
    if(!dyn_debug_mutex)
        return 0;
    if(NULL == format)
...
int
catchup_printf_int(const char* format, ...)
{
...
int
bpatch_printf(const char* format, ...)
{
    if(!dyn_debug_bpatch)
        return 0;
    if(NULL == format)
...
int
regalloc_printf_int(const char* format, ...)
{
...
int
ast_printf_int(const char* format, ...)
{
    if(!dyn_debug_ast)
        return 0;
    if(NULL == format)
...
int
write_printf_int(const char* format, ...)
{
    if(!dyn_debug_write)
        return 0;
    if(NULL == format)
...
int
infmalloc_printf_int(const char* format, ...)
{
    if(!dyn_debug_infmalloc)
        return 0;
    if(NULL == format)
        return -1;
...
int
stackmods_printf_int(const char* format, ...)
{
    if(!dyn_debug_stackmods)
        return 0;
    if(NULL == format)
        return -1;
...
print_stats()
{
    ...
    if(have_stats)
        fprintf(stderr, "Running time: %f sec (user), %f sec (system), %f sec (wall)\n",
                running_time.usecs(), running_time.ssecs(), running_time.wsecs());
    running_time.start();
    ...
    if(check_env_value("DYNINST_STATS_INST"))
    {
        ...
        fprintf(
            stderr,
            "  Generation: %ld calls, %f sec (user), %f sec (system), %f sec (wall)\n",
        ...
        fprintf(
            stderr,
            "  Installation: %ld calls, %f sec (user), %f sec (system), %f sec (wall)\n",
        ...
        fprintf(stderr,
                "  Linking: %ld calls, %f sec (user), %f sec (system), %f sec (wall)\n",
        ...
        fprintf(stderr,
                "  Removal: %ld calls, %f sec (user), %f sec (system), %f sec (wall)\n",
    ...
    if(check_env_value("DYNINST_STATS_PTRACE"))
    {
        ...
        fprintf(stderr,
                "  Write: %ld calls, %ld bytes, %f sec (user), %f sec (system), %f sec "
                "(wall)\n",
        ...
        fprintf(stderr,
                "  Read: %ld calls, %ld bytes, %f sec (user), %f sec (system), %f sec "
                "(wall)\n",
    ...
    if(check_env_value("DYNINST_STATS_PARSING"))
    {
        ...
        fprintf(stderr,
                "  Symtab parsing:  %f sec (user), %f sec (system), %f sec (wall)\n",
    ...
    if(check_env_value("DYNINST_STATS_CODEGEN"))
    {
        ...
        fprintf(stderr,
                "  AST generation: %ld calls, %f sec (user), %f sec (system), %f sec "
                "(wall)\n",
        ...
        fprintf(stderr,
                "  Register allocation: %f sec (user), %f sec (system), %f sec (wall)\n",
        ...
        fprintf(stderr,
                "  Liveness analysis: %f sec (user), %f sec (system), %f sec (wall)\n",
