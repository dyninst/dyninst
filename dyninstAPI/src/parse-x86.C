...
bool
parse_func::writesFPRs(unsigned level)
{
...
mapped_object::findGlobalConstructorFunc(const std::string& ctorHandler)
{
    ...
    const std::vector<func_instance*>* matching_funcs =
        findFuncVectorByMangled(ctorHandler);
...
mapped_object::findGlobalDestructorFunc(const std::string& dtorHandler)
{
    ...
    const std::vector<func_instance*>* matching_funcs =
        findFuncVectorByMangled(dtorHandler);
    if(matching_funcs != NULL)
    {
        return matching_funcs->at(0);
    }
