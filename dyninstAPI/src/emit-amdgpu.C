...
unsigned EmitterAmdgpuGfx908::emitIf(Register expr_reg, Register target, RegControl /* rc */,
  ...
  assert(uint32_t(target) >= SGPR_0 && uint32_t(target) <= SGPR_101 && "target must be an SGPR");
  assert(uint32_t(target) % 2 == 0 && "target must be even as we will use target, target+1 in pair");
  ...
  emitSop1(S_SETPC_B64, /* dest = */ 0, uint32_t(target), /* hasLiteral = */ false, 0, gen);
...
void EmitterAmdgpuGfx908::emitLoadConst(Register dest, Address imm, codeGen &gen) {
  ...
  assert(uint32_t(dest) >= SGPR_0 && uint32_t(dest) <= SGPR_101 && "reg0 must be an SGPR");
  assert(uint32_t(reg0) % 2 == 0 && "reg0 must be even as we will use reg0, reg1 in pair");
...
void EmitterAmdgpuGfx908::emitLongJump(Register reg, uint64_t fromAddress, uint64_t toAddress,
                                       codeGen &gen) {
  assert(uint32_t(reg) >= SGPR_0 && uint32_t(reg) <= SGPR_101 && "reg must be an SGPR");
  assert(uint32_t(reg) % 2 == 0 && "reg must be even as we will use reg, reg+1 in pair");
