...
unsigned EmitterAmdgpuGfx908::emitIf(Register expr_reg, Register target, RegControl /* rc */,
  ...
  assert(target.getValue() >= SGPR_0 && target.getValue() <= SGPR_101 && "target must be an SGPR");
  assert(target.getValue() % 2 == 0 && "target must be even as we will use target, target+1 in pair");
  ...
  emitSop1(S_SETPC_B64, /* dest = */ 0, target.getValue(), /* hasLiteral = */ false, 0, gen);
...
void EmitterAmdgpuGfx908::emitLoadConst(Register dest, Address imm, codeGen &gen) {
  ...
  assert(dest.getValue() >= SGPR_0 && dest.getValue() <= SGPR_101 && "reg0 must be an SGPR");
  assert(reg0.getValue() % 2 == 0 && "reg0 must be even as we will use reg0, reg1 in pair");
...
void EmitterAmdgpuGfx908::emitLongJump(Register reg, uint64_t fromAddress, uint64_t toAddress,
                                       codeGen &gen) {
  assert(reg.getValue() >= SGPR_0 && reg.getValue() <= SGPR_101 && "reg must be an SGPR");
  assert(reg.getValue() % 2 == 0 && "reg must be even as we will use reg, reg+1 in pair");
