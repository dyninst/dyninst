...
baseTramp*
baseTramp::createForIRPC(AddressSpace* as)
{
...
baseTramp*
baseTramp::fork(baseTramp* parent, AddressSpace* child)
{
    if(parent->point())
    {
...
bool
baseTramp::shouldRegenBaseTramp(registerSpace* rs)
{
    ...
    for(unsigned i = 0; i < regs.size(); i++)
    {
        ...
        regalloc_printf("[%s:%d] - checking reg (index %u, number %u, encoding %u)\n",
                        __FILE__, __LINE__, i, reg->number, reg->encoding());
        ...
        if(reg->liveState != registerSlot::spilled && definedRegs[reg->encoding()])
        {
