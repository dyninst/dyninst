...
bool baseTramp::shouldRegenBaseTramp(registerSpace *rs) {
  ...
  if (spilledRegisters && !createdLocalSpace) return true;

  pdvector<registerSlot *> &regs = rs->trampRegs();
  for (unsigned i = 0; i < regs.size(); i++) {
    registerSlot *reg = regs[i];
    regalloc_printf(
        "[%s:%u] - checking reg (index %d, number %d, encoding %d)\n", __FILE__,
    ...
    if ((reg->spilledState != registerSlot::unspilled) &&
        (!definedRegs[reg->encoding()]) && (!reg->offLimits)) {
      saved_unneeded++;
      regalloc_printf(
          "[%s:%u] - baseTramp saved unneeded register %d, "
          "suggesting regen (%d, %d, %d)\n",
          __FILE__, __LINE__, reg->number, reg->spilledState,
          (definedRegs[reg->encoding()] ? 1 : 0), reg->offLimits);
    }
    if (reg->liveState != registerSlot::spilled &&
        definedRegs[reg->encoding()]) {
      regalloc_printf(
          "[%s:%u] - Decided not to save a defined register %d. "
          "App liveness?\n",
          __FILE__, __LINE__, reg->number);
    }
  }
  regalloc_printf("[%s:%u] - Should regen found %d unneeded saves\n", __FILE__,
                  __LINE__, saved_unneeded);
  ...
  if (proc()->getAddressWidth() == 4) {
    ...
    if (actually_saved == regs.size() && needed_saved > X86_REGS_SAVE_LIMIT) {
      return false;
    }
  }
#endif
...
bool baseTramp::generateCode(codeGen &gen, Address baseInMutatee) {
  ...
  if (point_ && point_->empty()) return true;
...
bool baseTramp::generateCodeInlined(codeGen &gen, Address) {
  ...
  if (point_) {
    for (instPoint::instance_iter iter = point_->begin(); iter != point_->end();
         ++iter) {
  ...
  if (guarded() && minis->containsFuncCall()) {
    baseTrampElements.push_back(
        AstNode::funcCallNode("DYNINST_unlock_tramp_guard", empty_args));
  }
  ...
  if (guarded() && minis->containsFuncCall()) {
    baseTrampAST = AstNode::operatorNode(
        ifOp,
        // trampGuardAddr,
...
AddressSpace *baseTramp::proc() const {
  if (point_) return point_->proc();
  if (as_) return as_;
  return NULL;
}

bool baseTramp::checkForFuncCalls() {
  if (ast_) return ast_->containsFuncCall();
  if (point_) {
    /*
...
bool baseTramp::threaded() const {
  if (!proc()->multithread_ready()) return false;
  return true;
}

int baseTramp::numDefinedRegs() {
  int count = 0;
  if (!optimizationInfo_) return -1;
  registerSpace *rs =
      registerSpace::getRegisterSpace(proc()->getAddressWidth());
  pdvector<registerSlot *> &regs = rs->trampRegs();
  for (unsigned i = 0; i < regs.size(); i++) {
    registerSlot *reg = regs[i];
    if (definedRegs[reg->encoding()]) {
...
int baseTramp::funcJumpSlotSize() { return 0; }
