...
static bool kludge_isKernel32Dll(HANDLE fileHandle, std::string &kernel32Name) {
  ...
  if (firstTime) {
    ...
    if (GetSystemDirectory(sysRootDir, MAX_PATH) == 0) assert(0);
    kernel32Name_ = std::string(sysRootDir) + "\\kernel32.dll";
    kernel32H = CreateFile(kernel32Name_.c_str(), GENERIC_READ, FILE_SHARE_READ,
  ...
  if (!GetFileInformationByHandle(fileHandle, &info)) return false;
...
bool PCProcess::dumpImage(std::string outFile) {
  fprintf(stderr,
          "%s[%d]:  Sorry, dumpImage() not implemented for windows yet\n",
          FILE__, __LINE__);
...
void PCProcess::changeMemoryProtections(Address addr, size_t size,
  ...
  for (Address idx = pageBase, idx_e = pageBase + size; idx < idx_e;
       idx += pageSize) {
    ...
    if (!pcProc_->setMemoryAccessRights(idx, pageSize, rights, oldRights)) {
      mal_printf(
          "ERROR: failed to set access rights "
          "for page %lx, %s[%d]\n",
          addr, FILE__, __LINE__);
      ...
      if (!valid) {
        mal_printf(
            "WARNING: set access rights on page %lx that has "
            "no shadow %s[%d]\n",
            addr, FILE__, __LINE__);
      } else {
        ...
        if (shadowRights != oldRights) {
          mal_printf(
              "WARNING: shadow page[%lx] rights %s did not "
...
void dyninst_yield() { SwitchToThread(); }
...
bool OS::execute_file(char *file) {
  ...
  result =
      CreateProcess(NULL, file, NULL, NULL, FALSE, 0, NULL, NULL, &s, &proc);
  if (!result) {
...
void OS::unlink(char *file) { DeleteFile(file); }
...
void PCProcess::inferiorMallocConstraints(Address near, Address &lo,
                                          Address &hi,
...
/**
 ...
 fastcall:
   ...
   * Args - First two arguments that are less than DWORD size are passed in ECX
 & EDX
   * Cleanup - Callee cleans up the stack before returning
 thiscall:
   * C Naming - NA
...
callType func_instance::getCallingConvention() {
  ...
  if (callingConv != unknown_call) return callingConv;
...
static void emitNeededCallSaves(codeGen &gen, Register reg,
...
int EmitterIA32::emitCallParams(codeGen &gen,
                                ...
                                pdvector<Register> &extra_saves, bool noCost) {
...
mapped_object *PCProcess::createObjectNoFile(Address addr) {
  ...
  for (unsigned i = 0; i < mapped_objects.size(); i++) {
    if (addr >= mapped_objects[i]->codeAbs() &&
        addr <
            (mapped_objects[i]->codeAbs() + mapped_objects[i]->imageSize())) {
      fprintf(stderr,
              "createObjectNoFile called for addr %lx, "
              "matching existing mapped_object %s %s[%d]\n",
              addr, mapped_objects[i]->fullName().c_str(), FILE__, __LINE__);
  ...
  if (readDataSpace((void *)addr, getAddressWidth(), &testRead, false)) {
    ...
    if (!pcProc_->findAllocatedRegionAround(addr, memRegion)) {
      mal_printf(
          "ERROR: failed to find allocated region for page %lx, %s[%d]\n", addr,
          FILE__, __LINE__);
      assert(0);
      return NULL;
    }
    ...
    mal_printf(
        "[%lx %lx] is valid region containing %lx and corresponding "
        "to no object, closest is object ending at %lx %s[%d]\n",
        memRegion.first, memRegion.second, addr, closestObjEnd, FILE__,
        __LINE__);
    ...
    char regname[64];
    snprintf(regname, 63, "mmap_buffer_%lx_%lx", memRegion.first,
             memRegion.second);
    fileDescriptor desc(string(regname), memRegion.first, /*  code  */
                        memRegion.first,                  /*  data  */
                        regionSize,                       /* length */
                        rawRegion,                        /* rawPtr */
                        true);                            /* shared */
    mapped_object *obj =
        mapped_object::createMappedObject(desc, this, getHybridMode(), false);
    if (obj != NULL) {
      obj->setMemoryImg();
      ...
      fprintf(
          stderr,
          "Failed to create object (that was not backed by a file) at %lx\n",
          memRegion.first);
      return NULL;
    }
  }
...
bool PCProcess::hasPassedMain() { return true; }

bool PCProcess::startDebugger() { return false; }
...
bool OS_disconnect(BPatch_remoteHost &remote) { return true; }
...
bool PCProcess::hideDebugger() {
  Dyninst::ProcControlAPI::Thread::const_ptr threadPtr_ =
      pcProc_->threads().getInitialThread();
  if (!threadPtr_) return false;
  ...
  unsigned int pebPtr;
  if (!readDataSpace((void *)(tibPtr + 48), getAddressWidth(), (void *)&pebPtr,
                     false)) {
    fprintf(stderr,
            "%s[%d] Failed to read address of Process Environment "
  ...
  if (flag) {
    flag = 0;
    if (!writeDataSpace((void *)(pebPtr + 2), 1, (void *)&flag)) return false;
  }
...
bool PCEventHandler::isValidRTSignal(
    int signal, PCEventHandler::RTBreakpointVal breakpointVal,
...
bool AddressSpace::usesDataLoadAddress() const { return false; }
...
bool PCProcess::multithread_capable(bool ignoreIfMtNotSet) { return true; }
...
bool PCProcess::getExecFileDescriptor(std::string filename, bool waitForTrap,
                                      fileDescriptor &desc) {
...
bool PCProcess::skipHeap(const heapDescriptor &heap) { return false; }

inferiorHeapType PCProcess::getDynamicHeapType() const { return anyHeap; }
