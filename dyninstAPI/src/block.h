...
: public ParseAPI::iterator_predicate<edge_instance*, edge_instance*>
{
public:
    EdgePredicateAdapter()
    : int_(NULL)
    {}
    EdgePredicateAdapter(ParseAPI::EdgePredicate* intPred)
    : int_(intPred)
    {}
    virtual ~EdgePredicateAdapter() {}
    virtual bool pred_impl(edge_instance* const e) const
    {
        return int_->pred_impl(e->edge());
    }
...
template <class OutputIterator>
void
block_instance::getFuncs(OutputIterator result)
{
...
{
    bool operator()(block_instance* const& b1, block_instance* const& b2) const
    {
