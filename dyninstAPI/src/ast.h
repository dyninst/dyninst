...
public:
    ...
    static AstNodePtr funcCallNode(
        Address addr, std::vector<AstNodePtr>& args);  // For when you absolutely need
    // to jump somewhere.
    ...
    // TODO...
    // Needs some way of marking what to save and restore... should be a registerSpace,
    // really
   ...
   static AstNodePtr trampGuardNode();
    ...
    bool condMatch(AstNode* a, std::vector<dataReqNode*>& data_tuple1,
...
{
public:
    AstStackInsertNode(int s, MSpecialType t)
    : AstNode()
    , size(s)
    , type(t)
    {}
...
class AstSnippetNode : public AstNode
{
    ...
    // in our world.
public:
    AstSnippetNode(Dyninst::PatchAPI::SnippetPtr snip)
    : snip_(snip)
    {}
...
void
emitStorePreviousStackFrameRegister(Address register_num, Register src, codeGen& gen,
                                    int size, bool noCost);
