...
#include "FunctionExpansionRecord-x86.h"
...
// Added For Function Relocation
void FERNode::updateOffset(int update) {
    original_offset += update;
}
...
// Added for function relocation
void FunctionExpansionRecord::UpdateExpansion(int index, int update) {
    assert (expansions[index] != NULL);
    expansions[index]->updateOffset(update);
}
