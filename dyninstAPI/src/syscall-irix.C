...
// $Id: syscall-irix.C,v 1.1 2004/03/02 22:46:21 bernat Exp $
...
bool syscallNotification::removePreFork() {
    ...
    // Get existing flags, remove pre-fork, and set    
    sysset_t entryset;
    if (!proc->get_entry_syscalls(&entryset)) return false;
...
bool syscallNotification::removePostFork() {
    ...
    // Get existing flags, remove post-fork, and set
...
bool syscallNotification::removePreExec() {
    ...
    prdelset(&entryset, SYS_exec);
    prdelset(&entryset, SYS_execve);
