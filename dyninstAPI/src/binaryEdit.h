...
public:
    ...
    bool readDataSpace(const void* inOther, u_int amount, void* inSelf, bool showError);
    bool readTextSpace(const void* inOther, u_int amount, void* inSelf);

    bool writeDataSpace(void* inOther, u_int amount, const void* inSelf);
    bool writeTextSpace(void* inOther, u_int amount, const void* inSelf);

    // "Read"/"Write" to an address space with correct endian swapping.
    bool readDataWord(const void* inOther, u_int amount, void* inSelf, bool showError);
    bool readTextWord(const void* inOther, u_int amount, void* inSelf);

    bool writeDataWord(void* inOther, u_int amount, const void* inSelf);
    bool writeTextWord(void* inOther, u_int amount, const void* inSelf);
    ...
    */
    ...
    static bool  getResolvedLibraryPath(const std::string&        filename,
...
private:
    ...
    static bool getStatFileDescriptor(const std::string& file, fileDescriptor& desc);
...
{
public:
    depRelocation(Address a, SymtabAPI::Symbol* r)
    : to(a)
    , referring(r)
    {}
