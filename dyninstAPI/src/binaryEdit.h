...
class BinaryEdit : public AddressSpace {
 public:
  ...
  bool readDataSpace(const void *inOther, u_int amount, void *inSelf,
                     bool showError);
  ...
  bool readDataWord(const void *inOther, u_int amount, void *inSelf,
                    bool showError);
  ...
  */
  ...
  static BinaryEdit *openFile(
      ...
      Dyninst::PatchAPI::Patcher *patch = NULL, const std::string &member = "");
  ...
  mapped_object *openResolvedLibraryName(
      std::string filename, std::map<std::string, BinaryEdit *> &allOpened);

  bool writing() { return writing_; }

  void addDyninstSymbol(SymtabAPI::Symbol *sym) {
    newDyninstSyms_.push_back(sym);
  }
