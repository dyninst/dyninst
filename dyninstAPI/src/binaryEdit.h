...
class BinaryEdit : public AddressSpace {
 ...
 private:
   ...
   bool doStaticBinarySpecialCases();
    ...
    void buildLineMapReloc(pdvector<std::pair<Address, SymtabAPI::LineNoTuple>> & newLineMap, Address orig_addr, Address reloc_addr, unsigned strand_size, const Relocation::TrackerElement* tracker);
    ...
    void buildLineMapCore(pdvector<std::pair<Address, SymtabAPI::LineNoTuple> > & newLineMap, Address orig_addr, Address reloc_addr, unsigned strand_size, const Relocation::TrackerElement* tracker, bool isInstCode);
