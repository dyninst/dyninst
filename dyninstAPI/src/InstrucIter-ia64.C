...
bool InstrucIter::getMultipleJumpTargets( BPatch_Set<Address> & targetAddresses ) {
	...
	if( (img_ && img_->isAOut()) || (proc_ && proc_->getAOut()->findCodeRangeByAddress( current )) ) {
	...
	if( img_ && img_->isAOut() && foundInstruction2 ) {
		void *ptr = img_->getPtrToData( jumpTableAddressAddress );
		if( ! ptr ) { ptr = img_->getPtrToDataInText( jumpTableAddressAddress ); }
		if( !ptr ) {
	...
	else if( proc_ && proc_->getAOut()->findCodeRangeByAddress( jumpTableAddressAddress ) && foundInstruction2 ) {
	...
	for( unsigned int i = 0; i < maxTableLength; ++i ) {
		...
		if( img_ ) {
			...
			if( !ptr ) { ptr = img_->getPtrToDataInText( jumpTableAddress + (sizeof(uint64_t) * i) ); }
			if( !ptr ) {
