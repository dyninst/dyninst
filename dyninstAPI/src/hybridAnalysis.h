...
public:
    ...
    void getCallBlocks(
        Dyninst::Address retAddr, func_instance* retFunc, block_instance* retBlock,
    ...
    struct blockcmp
    {
        bool operator()(const BPatch_basicBlock* b1, const BPatch_basicBlock* b2) const;
    };
...
private:
    ...
    bool instrumentFunction(BPatch_function* func, bool useInsertionSet,
                            bool instrumentReturns = false, bool syncShadow = false);
    bool parseAfterCallAndInstrument(BPatch_point* callPoint, BPatch_function* calledFunc,
                                     bool foundByRet);
    void removeInstrumentation(BPatch_function* func, bool useInsertionSet,
                               bool handlesWereDeleted = false);
    int  saveInstrumentationHandle(BPatch_point* point, BPatchSnippetHandle* handle);
    ...
    friend void BPatch_process::overwriteAnalysisUpdate(
        ...
        std::vector<BPatch_function*>& owFuncs, std::set<BPatch_function*>& monitorFuncs,
...
public:
    ...
    bool hasLoopInstrumentation(bool activeOnly, BPatch_function& func,
...
private:
    ...
    // gets biggest loop without unresolved/multiply resolved indirect ctrl flow that it
    // can find
    ...
    std::map<Dyninst::Address, int>
        blockToLoop;  // KEVINCOMMENT: makes non-guaranteed assumption that only one loop
                      // per block, would it be better to use the last instruction
                      // address?
