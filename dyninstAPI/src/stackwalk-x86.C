...
bool PCProcess::createStackwalkerSteppers() {
  ...
  stepper =
      new StepperWanderer(stackwalker_, dynWandererHelper, dynFrameHelper);
  if (!stackwalker_->addStepper(stepper)) {
    startup_printf("Error adding Stackwalker stepper %p\n", stepper);
    return false;
  }
...
// OUT:
//   *orig_ra - If in frameless instrumentation, the original address of the
//   instruction
//              otherwise, 0x0
//   *stack_height -
  ...
  if (func) {
    ...
    if (FrameFuncHelper::standard_frame == result.first) {
      ...
      if (!blocks.empty()) {
        aBlock = *(blocks.begin());
        frameChecker fc(
            (const unsigned char *)(proc_->getPtrToInstruction(addr)),
            aBlock->size() - (addr - aBlock->start()), proc_->getArch());
...
bool DynWandererHelper::isPrevInstrACall(Address addr, Address &target) {
  ...
  for (std::set<func_instance *>::iterator iter = funcs.begin();
       iter != funcs.end(); ++iter) {
    for (PatchAPI::PatchFunction::Blockset::const_iterator c_iter =
             (*iter)->callBlocks().begin();
...
WandererHelper::pc_state DynWandererHelper::isPCInFunc(Address func_entry,
  ...
  stackwalk_printf(
      "[%s:%u] - DynWandererHelper called for func entry: %lx, pc: %lx - "
      "found callee func: %p, cur func: %p\n",
      __FILE__, __LINE__, func_entry, pc, callee_func, cur_func);
