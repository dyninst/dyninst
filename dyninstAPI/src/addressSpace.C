...
Address
AddressSpace::getTOCoffsetInfo(func_instance* func)
{
    ...
    toc32 = true;
#endif
    if(getAddressWidth() == 8 && !toc64)
        return 0;
    if(getAddressWidth() == 4 && !toc32)
        return 0;
...
void
AddressSpace::copyAddressSpace(AddressSpace* parent)
{
    deleteAddressSpace();
    ...
    if(parent->trampGuardBase_)
        trampGuardBase_ =
            new int_variable(parent->trampGuardBase_, getAOut()->getDefaultModule());
    else
        trampGuardBase_ = NULL;
    ...
    for(PatchAPI::CallModMap::iterator iter = cmm.begin(); iter != cmm.end(); ++iter)
    {
        ...
        {
            func_instance* context = (SCAST_FI(iter2->first) == NULL)
                                         ? NULL
                                         : findFunction(SCAST_FI(iter2->first)->ifunc());
            func_instance* target = (SCAST_FI(iter2->second) == NULL)
                                        ? NULL
...
void
AddressSpace::deleteAddressSpace()
{
    heapInitialized_ = false;
    heap_.clear();
...
bool
AddressSpace::getSymbolInfo(const std::string& name, int_symbol& ret)
{
...
bool
heapItemLessByAddr(const heapItem* a, const heapItem* b)
{
    if(a->addr < b->addr)
    {
        return true;
...
void
AddressSpace::inferiorFreeCompact()
{
    ...
    for(i = 1; i < freeList.size(); i++)
    {
        ...
        if(h1->addr + h1->length > h2->addr)
        {
            fprintf(
                stderr,
                "Error: heap 1 (%p) (0x%p to 0x%p) overlaps heap 2 (%p) (0x%p to 0x%p)\n",
                (void*) h1, (void*) h1->addr, (void*) (h1->addr + h1->length), (void*) h2,
...
void
AddressSpace::addHeap(heapItem* h)
{
...
*/
bool
AddressSpace::isInferiorAllocated(Address block)
{
    return (heap_.heapActive.find(block) != heap_.heapActive.end());
}

Address
AddressSpace::inferiorMallocInternal(unsigned size, Address lo, Address hi,
                                     inferiorHeapType type)
{
    infmalloc_printf(
        "%s[%d]: inferiorMallocInternal, %u bytes, type %d, between 0x%lx - 0x%lx\n",
        FILE__, __LINE__, size, type, lo, hi);
    int freeIndex = findFreeIndex(size, type, lo, hi);
    if(freeIndex == -1)
        return 0;  // Failure is often an option
...
void
AddressSpace::inferiorFreeInternal(Address block)
{
    ...
    infmalloc_printf("%s[%d]: Freed block from 0x%lx - 0x%lx, %u bytes, type %d\n",
                     FILE__, __LINE__, h->addr, h->addr + h->length, h->length, h->type);
}

void
AddressSpace::inferiorMallocAlign(unsigned& size)
{
...
bool
AddressSpace::inferiorReallocInternal(Address block, unsigned newSize)
{
...
bool
AddressSpace::inferiorShrinkBlock(heapItem* h, Address block, unsigned newSize)
{
    ...
    if(succ != NULL)
    {
        infmalloc_printf("%s[%d]: enlarging existing block; old 0x%lx - 0x%lx (%lu), new "
                         "0x%lx - 0x%lx (%u)\n",
                         FILE__, __LINE__, succ->addr, succ->addr + succ->length,
                         succ->addr, succ->addr - shrink, succ->addr + succ->length,
                         succ->length + shrink);
    ...
    else
    {
        ...
        infmalloc_printf(
            "%s[%d]: inferiorRealloc: creating new block 0x%lx to 0x%lx (%d), type %d\n",
...
bool
AddressSpace::inferiorExpandBlock(heapItem* h, Address, unsigned newSize)
{
...
bool
AddressSpace::findFuncsByAll(const std::string&           funcname,
                             std::vector<func_instance*>& res, const std::string& libname)
{  // = "", btw
    ...
    for(unsigned i = 0; i < mapped_objects.size(); i++)
    {
        ...
        {
            const std::vector<func_instance*>* pretty =
...
bool
AddressSpace::findFuncsByPretty(const std::string&           funcname,
...
bool
AddressSpace::findFuncsByMangled(const std::string&           funcname,
                                 std::vector<func_instance*>& res,
                                 const std::string&           libname)
{                                            // = "", btw
    unsigned starting_entries = res.size();  // We'll return true if we find something
...
func_instance*
AddressSpace::findOnlyOneFunction(const string& name, const string& lib,
                                  bool /*search_rt_lib*/)
{
...
bool
AddressSpace::findVarsByAll(const std::string& varname, std::vector<int_variable*>& res,
                            const std::string& libname)
{                                            // = "", btw
    unsigned starting_entries = res.size();  // We'll return true if we find something
    ...
    for(unsigned i = 0; i < mapped_objects.size(); i++)
    {
        if(libname == "" || mapped_objects[i]->fileName() == libname.c_str() ||
           mapped_objects[i]->fullName() == libname.c_str())
        {
            const std::vector<int_variable*>* pretty =
            ...
            {
                const std::vector<int_variable*>* mangled =
...
void*
AddressSpace::getPtrToInstruction(const Address addr) const
{
...
bool
AddressSpace::isCode(const Address addr) const
{
    mapped_object* obj = findObject(addr);
    if(!obj)
        return false;
...
bool
AddressSpace::isData(const Address addr) const
{
    mapped_object* obj = findObject(addr);
    if(!obj)
        return false;
    ...
    if(addr >= dataStart && addr < (dataStart + obj->dataSize()))
        return true;
    return false;
}

bool
AddressSpace::isReadOnly(const Address) const
{
    return false;
}

bool
AddressSpace::isValidAddress(const Address addr) const
{
    mapped_object* obj = findObject(addr);
    if(!obj)
        return false;

    if(obj->isCode(addr) || obj->isData(addr))
        return true;
    return false;
}

mapped_object*
AddressSpace::findObject(Address addr) const
{
    for(unsigned i = 0; i < mapped_objects.size(); i++)
    {
...
{
    mapped_object* obj = findObject(
        static_cast<ParseAPI::SymtabCodeSource*>(co->cs())->getSymtabObject()->file());
    return obj;
}

func_instance*
AddressSpace::findFunction(parse_func* ifunc)
{
    assert(ifunc);
...
block_instance*
AddressSpace::findBlock(parse_block* iblk)
{
    assert(iblk);
...
edge_instance*
AddressSpace::findEdge(ParseAPI::Edge* iedge)
{
    assert(iedge);
...
mapped_module*
AddressSpace::findModule(const std::string& mod_name, bool wildcard)
{
...
mapped_object*
AddressSpace::findObject(std::string obj_name, bool wildcard) const
{
...
// This just iterates over the mapped object vector
mapped_object*
AddressSpace::findObject(fileDescriptor desc) const
{
    for(u_int j = 0; j < mapped_objects.size(); j++)
    {
...
void
AddressSpace::getAllFunctions(std::vector<func_instance*>& funcs)
{
    for(unsigned i = 0; i < mapped_objects.size(); i++)
    {
...
void
AddressSpace::getAllModules(std::vector<mapped_module*>& mods)
{
    for(unsigned i = 0; i < mapped_objects.size(); i++)
    {
...
AddressSpace::findJumpTargetFuncByAddr(Address addr)
{
    ...
    InstructionDecoder decoder((const unsigned char*) getPtrToInstruction(addr),
                               InstructionDecoder::maxInstructionLength, getArch());
...
void
trampTrapMappings::copyTrapMappings(trampTrapMappings* parent)
{
...
void
trampTrapMappings::clearTrapMappings()
{
    needs_updating     = false;
    trapTableUsed      = NULL;
    trapTableVersion   = NULL;
...
void
trampTrapMappings::addTrapMapping(Address from, Address to, bool write_to_mutatee)
{
...
bool
trampTrapMappings::definesTrapMapping(Address from)
{
    return mapping.count(from) != 0;
}

Address
trampTrapMappings::getTrapMapping(Address from)
{
...
bool
trampTrapMappings::needsUpdating()
{
    return needs_updating;
}

bool
trampTrapMappings::empty()
{
    return mapping.empty();
}

AddressSpace*
trampTrapMappings::proc() const
{
    return as;
}

bool
mapping_sort(const trampTrapMappings::tramp_mapping_t* lhs,
...
void
trampTrapMappings::arrange_mapping(tramp_mapping_t& m, bool should_sort,
...
trampTrapMappings::flush()
{
    ...
    if(mappings_to_add.size())
    {
        ...
        bool    result = proc()->writeDataSpace((void*) write_addr, bytes_to_add, buffer);
        assert(result);
        free(buffer);
        buffer = NULL;
...
void
trampTrapMappings::allocateTable()
{
    unsigned entry_size = proc()->getAddressWidth() * 2;
    ...
    SymtabAPI::Symtab* symtab = binedit->getMappedObject()->parse_img()->getObject();
...
bool
AddressSpace::findFuncsByAddr(Address addr, std::set<func_instance*>& funcs,
                              bool includeReloc)
{
    if(includeReloc)
    {
...
bool
AddressSpace::findBlocksByAddr(Address addr, std::set<block_instance*>& blocks,
                               bool includeReloc)
{
    if(includeReloc)
    {
...
func_instance*
AddressSpace::findFuncByEntry(const block_instance* block)
{
...
block_instance*
AddressSpace::findBlockByEntry(Address a)
{
...
func_instance*
AddressSpace::findOneFuncByAddr(Address addr)
{
    std::set<func_instance*> funcs;
    if(!findFuncsByAddr(addr, funcs))
        return NULL;
    if(funcs.empty())
        return NULL;
    if(funcs.size() == 1)
    ...
    for(std::set<func_instance*>::iterator iter = funcs.begin(); iter != funcs.end();
        ++iter)
    {
...
func_instance*
AddressSpace::findFuncByEntry(Address addr)
{
    std::set<func_instance*> funcs;
    if(!findFuncsByAddr(addr, funcs))
        return NULL;
...
void
AddressSpace::setUseTraps(bool usetraps)
{
    useTraps_ = usetraps;
}

bool
AddressSpace::needsPIC(int_variable* v)
{
    return needsPIC(v->mod()->proc());
}

bool
AddressSpace::needsPIC(func_instance* f)
{
    return needsPIC(f->proc());
}

bool
AddressSpace::needsPIC(AddressSpace* s)
{
    if(proc())
...
bool
AddressSpace::sameRegion(Address addr1, Address addr2)
{
    ...
    if(!reg1 ||
       reg1 != mobj->parse_img()->getObject()->findEnclosingRegion(addr2 - baseAddr))
    {
        return false;
    }
    return true;
...
void
AddressSpace::modifyCall(block_instance* block, func_instance* newFunc,
...
void
AddressSpace::replaceFunction(func_instance* oldfunc, func_instance* newfunc)
{
...
bool
AddressSpace::wrapFunction(func_instance* original, func_instance* wrapper,
...
void
AddressSpace::wrapFunctionPostPatch(func_instance*              func,
...
void
AddressSpace::removeCall(block_instance* block, func_instance* context)
{
...
void
AddressSpace::revertCall(block_instance* block, func_instance* context)
{
    /*
...
void
AddressSpace::revertReplacedFunction(func_instance* oldfunc)
{
...
void
AddressSpace::revertWrapFunction(func_instance* wrappedfunc)
{
...
const func_instance*
AddressSpace::isFunctionReplacement(func_instance* func) const
{
...
bool
AddressSpace::delayRelocation() const
{
    return delayRelocation_;
}

bool
AddressSpace::relocate()
{
    if(delayRelocation())
        return true;
        ...
        iter != modifiedFunctions_.end(); ++iter)
    {
        ...
        do
        {  // add overlapping functions in a fixpoint calculation
            repeat           = false;
        ...
        if(getArch() == Arch_ppc64)
        {
            ...
            for(auto fit = modFuncs.begin(); fit != modFuncs.end(); ++fit)
            {
                ...
                {
                    relocation_cerr << "Ignore function " << funct->name() << " at "
                                    << hex
                                    << funct->entryBlock()->GetBlockStartingAddress()
                                    << " as it has the power preabmle" << endl;
                    continue;
                }
...
bool
AddressSpace::relocateInt(FuncSet::const_iterator begin, FuncSet::const_iterator end,
                          Address nearTo)
{
    if(begin == end)
    {
    ...
    if(!writeTextSpace((void*) baseAddr, cm->size(), cm->ptr()))
        return false;
        ...
        for(titer = threads.begin(); titer != threads.end(); titer++)
        {
            ...
            if(!relocPCs.empty())
            {
                ...
                mal_printf("Pulling active frame PC into newest relocation "
                           ...
                           orig, tframe.getPC(), relocPCs.back() + offset,
                           relocPCs.back(), offset);
                break;
            }
        }
    }
...
bool
AddressSpace::transform(CodeMover::Ptr cm)
{
...
AddressSpace::generateCode(CodeMover::Ptr cm, Address nearTo)
{
    ...
    while(1)
    {
        ...
        relocation_cerr << "   Calling inferiorRealloc to fit new size " << cm->size()
                        << ", current base addr is " << std::hex << baseAddr << std::dec
                        << endl;
...
bool
AddressSpace::patchCode(CodeMover::Ptr cm, SpringboardBuilder::Ptr spb)
{
    ...
    for(std::list<codeGen>::iterator iter = patches.begin(); iter != patches.end();
        ++iter)
    {
        springboard_cerr << "Writing springboard @ " << hex << iter->startAddr() << endl;
        if(!writeTextSpace((void*) iter->startAddr(), iter->used(), iter->start_ptr()))
        {
        ...
        if(obj && runtime_lib.end() == runtime_lib.find(obj))
        {
            ...
            if(memEmulator_)
                memEmulator_->addSpringboard(
                    reg, iter->startAddr() - objBase - reg->getMemOffset(), iter->used());
        }
    }
...
void
AddressSpace::causeTemplateInstantiations()
{}

void
AddressSpace::getRelocAddrs(Address orig, block_instance* block, func_instance* func,
...
bool
AddressSpace::getAddrInfo(Address relocAddr, Address& origAddr,
                          vector<func_instance*>& origFuncs, baseTramp*& baseT)
{
...
// KEVINTODO: not clearing out entries when deleting code, and extremely slow in defensive
// mode, over 300 codetrackers for Yoda
bool
AddressSpace::getRelocInfo(Address relocAddr, RelocInfo& ri)
{
    bool ret = false;
...
bool
AddressSpace::inEmulatedCode(Address addr)
{
    // address is relocated (or bad), check relocation maps
    for(CodeTrackers::const_iterator iter = relocatedCode_.begin();
        iter != relocatedCode_.end(); ++iter)
                       ...
                       const std::list<func_instance*>&  deadFuncs)
{
        ...
        bit != owBlocks.end(); bit++)
    {
            ...
            !foundStub && fit != bFuncs.end(); fit++)
        {
            ...
            std::for_each(
                boost::make_filter_iterator(epred_, sourceEdges.begin(),
                                            sourceEdges.end()),
                boost::make_filter_iterator(epred_, sourceEdges.end(), sourceEdges.end()),
                boost::bind(updateSrcListAndVisited, boost::placeholders::_1,
...
bool
AddressSpace::patch(AddressSpace* as)
{
    return as->patcher()->commit();
}

void
AddressSpace::addMappedObject(mapped_object* obj)
{
...
bool
uninstrument(Dyninst::PatchAPI::Instance::Ptr inst)
{
...
unsigned
AddressSpace::getAddressWidth() const
{
