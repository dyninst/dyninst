...
#define TRAMP_FRAME_SIZE_32                                                    \
  ALIGN_QUADWORD(STACKSKIP + GPRSAVE_32 + FPRSAVE + SPRSAVE_32 + FUNCSAVE_32 + \
                 FUNCARGS_32 + LINKAREA_32)
#define TRAMP_FRAME_SIZE_64                                                    \
  ALIGN_QUADWORD(STACKSKIP + GPRSAVE_64 + FPRSAVE + SPRSAVE_64 + FUNCSAVE_64 + \
...
void saveLR(codeGen &gen, Register scratchReg, int stkOffset);
void restoreLR(codeGen &gen, Register scratchReg, int stkOffset);
void setBRL(codeGen &gen, Register scratchReg, long val,
            unsigned ti);  // We're lazy and hand in the next insn
void saveCR(codeGen &gen, Register scratchReg, int stkOffset);
void restoreCR(codeGen &gen, Register scratchReg, int stkOffset);
void saveFPSCR(codeGen &gen, Register scratchReg, int stkOffset);
void restoreFPSCR(codeGen &gen, Register scratchReg, int stkOffset);
