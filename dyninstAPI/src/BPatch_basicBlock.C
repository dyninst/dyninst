...
void
source_helper(ParseAPI::Edge* e, BPatch_Vector<BPatch_basicBlock*>& srcs,
...
void
BPatch_basicBlock::getSources(BPatch_Vector<BPatch_basicBlock*>& srcs)
{
    //  BPatch_basicBlock *b;
    ...
    std::for_each(boost::make_filter_iterator(epred_, isrcs.begin(), isrcs.end()),
                  ...
                  boost::bind(source_helper, boost::placeholders::_1, boost::ref(srcs),
                              flowGraph, func));
...
void
BPatch_basicBlock::getImmediateDominates(BPatch_Vector<BPatch_basicBlock*>& imds)
{
...
void
BPatch_basicBlock::getImmediatePostDominates(BPatch_Vector<BPatch_basicBlock*>& imds)
{
...
bool
BPatch_basicBlock::dominates(BPatch_basicBlock* bb)
{
    if(!bb)
        return false;
...
bool
BPatch_basicBlock::postdominates(BPatch_basicBlock* bb)
{
    if(!bb)
        return false;
...
bool
BPatch_basicBlock::getAddressRange(void*& _startAddress, void*& _lastInsnAddress)
{
...
ostream&
operator<<(ostream& os, BPatch_basicBlock& bb)
{
    unsigned i;
    os << "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
...
struct findInsns : public insnPredicate
{
    ...
    , findPrefetch(false)
    {
        for(std::set<BPatch_opCode>::iterator iter = ops.begin(); iter != ops.end();
            ++iter)
        {
            switch(*iter)
            {
...
BPatch_point*
BPatch_basicBlock::findPoint(Address addr)
{
    ...
    if(!p)
        return NULL;
    return flowGraph->getAddSpace()->findOrCreateBPPoint(flowGraph->getFunction(), p,
...
BPatch_basicBlock::findPointByPredicate(insnPredicate& f)
{
    ...
    for(block_instance::Insns::iterator iter = insns.begin(); iter != insns.end(); ++iter)
    {
        if(f(iter->second))
        {
            instPoint* p =
            ...
            {
                fprintf(stderr,
                        "WARNING: failed to create instpoint for load/store/prefetch %s "
                        "at 0x%lx\n",
...
BPatch_Vector<BPatch_point*>*
BPatch_basicBlock::findPoint(const std::set<BPatch_opCode>& ops)
{
...
BPatch_Vector<BPatch_point*>*
BPatch_basicBlock::findPoint(const BPatch_Set<BPatch_opCode>& ops)
{
    // function is generally uninstrumentable (with current technology)
    if(!flowGraph->getFunction()->func->isInstrumentable())
        return NULL;
...
BPatch_Vector<BPatch_point*>*
BPatch_basicBlock::findPoint(bool (*filter)(Instruction))
{
...
BPatch_point*
BPatch_basicBlock::convertPoint(instPoint* pt)
{
...
//
void
BPatch_basicBlock::getAllPoints(std::vector<BPatch_point*>& bpPoints)
{
    ...
    if(entry)
        bpPoints.push_back(convertPoint(entry));
    if(preCall)
        bpPoints.push_back(convertPoint(preCall));
    if(exit)
        bpPoints.push_back(convertPoint(exit));
}
...
bool
BPatch_basicBlock::getInstructions(std::vector<InstructionAPI::Instruction>& insns)
{
...
bool
BPatch_basicBlock::getInstructions(
    std::vector<std::pair<InstructionAPI::Instruction, Address>>& insnInstances)
{
    using namespace InstructionAPI;
...
unsigned long
BPatch_basicBlock::getStartAddress() const
{
    return iblock->start();
}
...
unsigned long
BPatch_basicBlock::getEndAddress() const
{
    return iblock->end();
}
...
void
BPatch_basicBlock::getIncomingEdges(BPatch_Vector<BPatch_edge*>& inc)
{
...
void
BPatch_basicBlock::getOutgoingEdges(BPatch_Vector<BPatch_edge*>& out)
{
...
int
BPatch_basicBlock::blockNo() const
{
    return iblock->id();
}
...
Dyninst::ParseAPI::Block*
Dyninst::ParseAPI::convert(const BPatch_basicBlock* b)
{
    return b->iblock->block();
}

Dyninst::PatchAPI::PatchBlock*
Dyninst::PatchAPI::convert(const BPatch_basicBlock* b)
{
    return b->iblock;
}

bool
comparison<BPatch_basicBlock*>::operator()(const BPatch_basicBlock* const& x,
...
bool
std::less<BPatch_basicBlock*>::operator()(const BPatch_basicBlock* const& l,
