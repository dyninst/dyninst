...
BPatch_addrSpec_NP::BPatch_addrSpec_NP() : imm(0), scale(0) {
  regs[0] = 0;
  regs[1] = 0;
}

long BPatch_addrSpec_NP::getImm() const { return imm; }

int BPatch_addrSpec_NP::getScale() const { return scale; }

int BPatch_addrSpec_NP::getReg(unsigned i) const { return regs[i]; }

bool BPatch_addrSpec_NP::equals(const BPatch_addrSpec_NP &ar) const {
  return ((imm == ar.imm) && (scale == ar.scale) && (regs[0] == ar.regs[0]) &&
          (regs[1] == ar.regs[1]));
}

BPatch_Vector<BPatch_point *> *BPatch_memoryAccess::filterPoints(
    const BPatch_Vector<BPatch_point *> &points, unsigned int numMAs) {
  ...
  for (unsigned int i = 0; i < points.size(); ++i) {
    ...
    if (ma)
      if (ma->getNumberOfAccesses() >= numMAs) result->push_back(points[i]);
  }
  return result;
}
...
void BPatch_memoryAccess::set1st(bool _isLoad, bool _isStore, long _imm_s,
                                 int _ra_s, int _rb_s, unsigned int _scale_s,
                                 long _imm_c, int _ra_c, int _rb_c,
                                 unsigned int _scale_c, int _preFcn, int _cond,
                                 bool _nt) {
  nacc = 1;
  isLoad[0] = _isLoad;
  isStore[0] = _isStore;
...
void BPatch_memoryAccess::set1st(bool _isLoad, bool _isStore, long _imm_s,
                                 ...
                                 int _rb_c, unsigned int _scale_s, int _preFcn,
                                 int _cond, bool _nt) {
  nacc = 1;
  isLoad[0] = _isLoad;
  isStore[0] = _isStore;
...
BPatch_memoryAccess *BPatch_memoryAccess::init_tables() { return NULL; }

// initializes only the first access; #bytes is a constant
BPatch_memoryAccess::BPatch_memoryAccess(internal_instruction *insn,
                                         Address _addr, bool _isLoad,
                                         bool _isStore, unsigned int _bytes,
                                         long _imm, int _ra, int _rb,
                                         unsigned int _scale, int _cond,
                                         bool _nt)
...
BPatch_memoryAccess::BPatch_memoryAccess(internal_instruction *insn,
                                         Address _addr, bool _isLoad,
                                         bool _isStore, long _imm_s, int _ra_s,
                                         int _rb_s, unsigned int _scale_s,
                                         long _imm_c, int _ra_c, int _rb_c,
                                         unsigned int _scale_c, int _cond,
                                         bool _nt, int _preFcn)
    : BPatch_instruction(insn, _addr) {
  start = new BPatch_addrSpec_NP[nmaxacc_NP];
  count = new BPatch_countSpec_NP[nmaxacc_NP];
...
BPatch_memoryAccess::BPatch_memoryAccess(internal_instruction *insn,
                                         Address _addr, bool _isLoad,
                                         bool _isStore, bool _isPrefetch,
  ...
  count = new BPatch_countSpec_NP[nmaxacc_NP];
  assert(_isPrefetch);
  set1st(_isLoad, _isStore, _imm_s, _ra_s, _rb_s, _imm_c, _ra_c, _rb_c, 0,
         _preFcn);
}
...
BPatch_memoryAccess::BPatch_memoryAccess(internal_instruction *insn,
                                         Address _addr, bool _isLoad,
                                         bool _isStore, long _imm_s, int _ra_s,
                                         int _rb_s, long _imm_c, int _ra_c,
                                         int _rb_c)
    : BPatch_instruction(insn, _addr) {
  start = new BPatch_addrSpec_NP[nmaxacc_NP];
  count = new BPatch_countSpec_NP[nmaxacc_NP];
...
void BPatch_memoryAccess::set2nd(bool _isLoad, bool _isStore,
                                 unsigned int _bytes, long _imm, int _ra,
                                 int _rb, unsigned int _scale) {
  if (nacc >= 2) return;
  nacc = 2;
  isLoad[1] = _isLoad;
  isStore[1] = _isStore;
...
void BPatch_memoryAccess::set2nd(bool _isLoad, bool _isStore, long _imm_s,
                                 int _ra_s, int _rb_s, unsigned int _scale_s,
                                 long _imm_c, int _ra_c, int _rb_c,
...
BPatch_memoryAccess::BPatch_memoryAccess(
    internal_instruction *insn, Address _addr, bool _isLoad, bool _isStore,
    unsigned int _bytes, long _imm, int _ra, int _rb, unsigned int _scale,
    bool _isLoad2, bool _isStore2, unsigned int _bytes2, long _imm2, int _ra2,
...
BPatch_memoryAccess::BPatch_memoryAccess(
    internal_instruction *insn, Address _addr, bool _isLoad, bool _isStore,
    long _imm_s, int _ra_s, int _rb_s, unsigned int _scale_s, long _imm_c,
    int _ra_c, int _rb_c, unsigned int _scale_c, bool _isLoad2, bool _isStore2,
    long _imm2_s, int _ra2_s, int _rb2_s, unsigned int _scale2_s, long _imm2_c,
    int _ra2_c, int _rb2_c, unsigned int _scale2_c)
    : BPatch_instruction(insn, _addr) {
  start = new BPatch_addrSpec_NP[nmaxacc_NP];
  count = new BPatch_countSpec_NP[nmaxacc_NP];
  set1st(_isLoad, _isStore, _imm_s, _ra_s, _rb_s, _scale_s, _imm_c, _ra_c,
         _rb_c, _scale_c, -1, -1, false);
  set2nd(_isLoad2, _isStore2, _imm2_s, _ra2_s, _rb2_s, _scale2_s, _imm2_c,
         _ra2_c, _rb2_c, _scale2_c, -1, false);
}
...
bool BPatch_memoryAccess::equals(const BPatch_memoryAccess &rp) const {
  ...
  for (unsigned int i = 0; i < nacc; ++i) {
    res = res && (isLoad[i] == rp.isLoad[i]) && (isStore[i] == rp.isStore[i]) &&
          (start[i].equals(rp.start[i])) && (count[i].equals(rp.count[i])) &&
          (preFcn[i] == rp.preFcn[i]) && (condition[i] == rp.condition[i]) &&
