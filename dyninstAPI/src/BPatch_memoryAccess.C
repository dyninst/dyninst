...
long
BPatch_addrSpec_NP::getImm() const
{
    return imm;
}

int
BPatch_addrSpec_NP::getScale() const
{
    return scale;
}

int
BPatch_addrSpec_NP::getReg(unsigned i) const
{
    return regs[i];
}

bool
BPatch_addrSpec_NP::equals(const BPatch_addrSpec_NP& ar) const
{
...
BPatch_memoryAccess::filterPoints(const BPatch_Vector<BPatch_point*>& points,
                                  unsigned int                        numMAs)
{
...
const BPatch_addrSpec_NP*
BPatch_memoryAccess::getStartAddr(int which) const
{
    return &(start[which]);
}

const BPatch_countSpec_NP*
BPatch_memoryAccess::getByteCount(int which) const
{
    return &(count[which]);
}
...
void
BPatch_memoryAccess::set1st(bool _isLoad, bool _isStore, long _imm_s, int _ra_s,
                            int _rb_s, unsigned int _scale_s, long _imm_c, int _ra_c,
                            int _rb_c, unsigned int _scale_c, int _preFcn, int _cond,
                            bool _nt)
{
    nacc           = 1;
    isLoad[0]      = _isLoad;
...
void
BPatch_memoryAccess::set1st(bool _isLoad, bool _isStore, long _imm_s, int _ra_s,
                            int _rb_s, long _imm_c, int _ra_c, int _rb_c,
                            unsigned int _scale_s, int _preFcn, int _cond, bool _nt)
{
    nacc           = 1;
    isLoad[0]      = _isLoad;
    isStore[0]     = _isStore;
...
BPatch_memoryAccess*
BPatch_memoryAccess::init_tables()
{
    return NULL;
}
...
: BPatch_instruction(insn, _addr)
{
    start = new BPatch_addrSpec_NP[nmaxacc_NP];
    count = new BPatch_countSpec_NP[nmaxacc_NP];
    set1st(_isLoad, _isStore, _imm_s, _ra_s, _rb_s, _scale_s, _imm_c, _ra_c, _rb_c,
           _scale_c, _preFcn, _cond, _nt);
}
...
void
BPatch_memoryAccess::set2nd(bool _isLoad, bool _isStore, unsigned int _bytes, long _imm,
                            int _ra, int _rb, unsigned int _scale)
{
    if(nacc >= 2)
        return;
    nacc           = 2;
...
void
BPatch_memoryAccess::set2nd(bool _isLoad, bool _isStore, long _imm_s, int _ra_s,
                            int _rb_s, unsigned int _scale_s, long _imm_c, int _ra_c,
                            int _rb_c, unsigned int _scale_c, int _cond, bool _nt)
{
    if(nacc >= 2)
        return;
    nacc           = 2;
...
: BPatch_instruction(insn, _addr)
{
    ...
    set1st(_isLoad, _isStore, _imm_s, _ra_s, _rb_s, _scale_s, _imm_c, _ra_c, _rb_c,
           _scale_c, -1, -1, false);
    set2nd(_isLoad2, _isStore2, _imm2_s, _ra2_s, _rb2_s, _scale2_s, _imm2_c, _ra2_c,
           _rb2_c, _scale2_c, -1, false);
}

bool
BPatch_memoryAccess::equals(const BPatch_memoryAccess* mp) const
{
    return mp ? equals(*mp) : false;
}

bool
BPatch_memoryAccess::equals(const BPatch_memoryAccess& rp) const
{
...
BPatch_addrSpec_NP
BPatch_memoryAccess::getStartAddr_NP(int which) const
{
    return start[which];
}

BPatch_countSpec_NP
BPatch_memoryAccess::getByteCount_NP(int which) const
{
    return count[which];
}
