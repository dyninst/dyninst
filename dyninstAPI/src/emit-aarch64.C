...
codeBufIndex_t
EmitterAARCH64::emitIf(Register expr_reg, Register target, RegControl /*rc*/,
                       codeGen& gen)
{
    instruction insn;
    insn.clear();
...
void
EmitterAARCH64::emitLoadConst(Register dest, Address imm, codeGen& gen)
{
    insnCodeGen::loadImmIntoReg<Address>(gen, dest, imm);
}

void
EmitterAARCH64::emitLoad(Register dest, Address addr, int size, codeGen& gen)
{
...
void
EmitterAARCH64::emitStore(Address addr, Register src, int size, codeGen& gen)
{
    Register scratch = gen.rs()->getScratchRegister(gen);
...
void
EmitterAARCH64::emitOp(unsigned opcode, Register dest, Register src1, Register src2,
                       codeGen& gen)
{
    // dest = src1 + src2
    if(opcode == plusOp)
        insnCodeGen::generateAddSubShifted(gen, insnCodeGen::Add, 0, 0, src1, src2, dest,
                                           true);

    // dest = src1 - src2
    else if(opcode == minusOp)
        insnCodeGen::generateAddSubShifted(gen, insnCodeGen::Sub, 0, 0, src2, src1, dest,
                                           true);
    ...
    // dest = src1 & src2
        ...
        insnCodeGen::generateBitwiseOpShifted(gen, insnCodeGen::And, 0, src1, 0, src2,
                                              dest, true);

    // dest = src1 | src2
    else if(opcode == orOp)
        insnCodeGen::generateBitwiseOpShifted(gen, insnCodeGen::Or, 0, src1, 0, src2,
                                              dest, true);

    // dest = src1 ^ src2
    else if(opcode == xorOp)
        insnCodeGen::generateBitwiseOpShifted(gen, insnCodeGen::Eor, 0, src1, 0, src2,
                                              dest, true);
...
void
EmitterAARCH64::emitRelOp(unsigned opcode, Register dest, Register src1, Register src2,
                          codeGen& gen, bool s)
{
...
void
EmitterAARCH64::emitGetParam(Register, Register param_num, instPoint::Type, opCode op,
                             bool, codeGen& gen)
{
...
void
EmitterAARCH64::emitRelOpImm(unsigned opcode, Register dest, Register src1,
...
void
EmitterAARCH64::emitLoadIndir(Register dest, Register addr_src, int size, codeGen& gen)
{
...
void
EmitterAARCH64::emitStoreIndir(Register addr_reg, Register src, int size, codeGen& gen)
{
...
void
EmitterAARCH64::emitLoadOrigRegRelative(Register dest, Address offset, Register base,
                                        codeGen& gen, bool deref)
{
    ...
    else
    {
        ...
        insnCodeGen::generateAddSubShifted(gen, insnCodeGen::Add, 0, 0, dest, scratch,
                                           dest, true);
    }
}

void
EmitterAARCH64::emitLoadOrigRegister(Address register_num, Register destination,
                                     codeGen& gen)
{
    ...
    if(src->name == "sp")
    {
        insnCodeGen::generateAddSubImmediate(
            gen, insnCodeGen::Add, 0, TRAMP_FRAME_SIZE_64, REG_SP, destination, true);
