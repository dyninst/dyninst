...
void
newCodeCB(std::vector<BPatch_function*>& newFuncs,
          std::vector<BPatch_function*>& modFuncs)
{
    printf("new functions %d mod functions %d %s[%d]\n", (int) newFuncs.size(),
           (int) modFuncs.size(), FILE__, __LINE__);
}

void
HybridAnalysis::synchShadowOrigCB(BPatch_point* point, bool toOrig)
{
    mal_printf("in synch callback for point 0x%lx toOrig=%d\n",
               (Address) point->getAddress(), (int) (long) toOrig);

    proc()->lowlevel_process()->getMemEm()->synchShadowOrig((bool) toOrig);
    ...
    if(!toOrig)
    {
        ...
        for(set<Address>::iterator pit = pages.begin(); pit != pages.end(); pit++)
        {
...
void
HybridAnalysis::signalHandlerCB(BPatch_point*, long, std::vector<Address>&)
{}

bool HybridAnalysis::registerCodeDiscoveryCallback(BPatchCodeDiscoveryCallback)
{
    return true;
}

bool HybridAnalysis::registerSignalHandlerCallback(BPatchSignalHandlerCallback)
{
    return true;
}

bool
HybridAnalysis::removeCodeDiscoveryCallback()
{
    return false;
}

bool
HybridAnalysis::removeSignalHandlerCallback()
{
    return false;
}

void
HybridAnalysis::signalHandlerEntryCB(BPatch_point*, Address)
{}
void
HybridAnalysis::signalHandlerEntryCB2(BPatch_point*, Address)
{}
void
HybridAnalysis::signalHandlerExitCB(BPatch_point*, void*)
{}

void
HybridAnalysis::virtualFreeAddrCB(BPatch_point*, void*)
{}
void
HybridAnalysis::virtualFreeSizeCB(BPatch_point*, void*)
{}
void
HybridAnalysis::virtualFreeCB(BPatch_point*, void*)
{}

void
HybridAnalysis::abruptEndCB(BPatch_point*, void*)
{}
#else

static void
signalHandlerEntryCB_wrapper(BPatch_point* point, void* excRecAddr)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
        ->signalHandlerEntryCB(point, (Address) excRecAddr);
}

static void
signalHandlerEntryCB2_wrapper(BPatch_point* point, void* excCtxtAddr)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
        ->signalHandlerEntryCB2(point, (Address) excCtxtAddr);
}

bool
HybridAnalysis::registerCodeDiscoveryCallback(BPatchCodeDiscoveryCallback cb)
{
...
bool
HybridAnalysis::registerSignalHandlerCallback(BPatchSignalHandlerCallback cb)
{
 ...
 */
void
HybridAnalysis::signalHandlerCB(BPatch_point* point, long signum,
                                std::vector<Address>& handlers)
{
    mal_printf("signalHandlerCB(%lx , %lx, %d handlers)\n", point->getAddress(), signum,
               handlers.size());
    ...
    while(it < handlers.end())
    {
        ...
        BPatch_stopThreadExpr sThread1(signalHandlerEntryCB_wrapper, excRecAddr, false,
                                       BPatch_noInterp);
        BPatch_stopThreadExpr sThread2(signalHandlerEntryCB2_wrapper, excCtxtAddr, false,
                                       BPatch_noInterp);
 ...
 */
void
HybridAnalysis::signalHandlerEntryCB(BPatch_point* point, Address excRecAddr)
{
    ...
    proc()->lowlevel_process()->readDataSpace((void*) excRecAddr,
                                              sizeof(EXCEPTION_RECORD), &record, true);
    Address pcAddr =
        excRecAddr + (Address) & (record.ExceptionAddress) - (Address) &record;
 ...
 */
void
HybridAnalysis::signalHandlerEntryCB2(BPatch_point* point, Address excCtxtAddr)
{
...
void
HybridAnalysis::virtualFreeAddrCB(BPatch_point*, void* addr)
{
...
void
HybridAnalysis::virtualFreeSizeCB(BPatch_point*, void* size)
{
...
void
HybridAnalysis::virtualFreeCB(BPatch_point*, void* t)
{
    assert(virtualFreeAddr_ != 0);
    unsigned type = (unsigned) t;
    cerr << "virtualFree [" << hex << virtualFreeAddr_ << ","
         << virtualFreeAddr_ + (unsigned) virtualFreeSize_ << "], " << (unsigned) type
         << dec << endl;
    ...
    proc()->lowlevel_process()->getMemEm()->removeRegion(virtualFreeAddr_,
                                                         virtualFreeSize_);
    // And nuke the RT cache

    proc()->lowlevel_process()->proc()->flushAddressCache_RT(virtualFreeAddr_,
                                                             virtualFreeSize_);
 ...
 */
void
HybridAnalysis::signalHandlerExitCB(BPatch_point* point, void*)
{
    BPatch_function*                                       func = point->getFunction();
    std::map<Dyninst::Address, ExceptionDetails>::iterator diter =
        handlerFunctions.find((Address) func->getBaseAddr());
    assert(handlerFunctions.end() != diter && 0 != diter->second.faultPCaddr);
    ...
    proc()->lowlevel_process()->readDataSpace((void*) pcLoc, sizeof(resumePC), &resumePC,
                                              true);
...
void
HybridAnalysis::abruptEndCB(BPatch_point* point, void*)
{
    Address pointAddr = (Address) point->getAddress();
    mal_printf("\nabruptEndCB at %lx in function at %lx\n", pointAddr,
               point->getFunction()->getBaseAddr());
    ...
    unsigned       firstNonzero =
        point->llpoint()->block()->end() - reg->offset() - pfunc->obj()->codeBase();
...
static int
getPreCallPoints(ParseAPI::Block* blk, BPatch_process* proc,
                 vector<BPatch_point*>& points)
{
    ...
    blk->getFuncs(pFuncs);
    for(vector<Function*>::iterator fit = pFuncs.begin(); fit != pFuncs.end(); fit++)
    {
        ...
        if(iPoint)
        {
            ...
            BPatch_point*    bpPoint =
                proc->findOrCreateBPPoint(bpFunc, iPoint, BPatch_subroutine);
...
void
HybridAnalysis::getCallBlocks(Address retAddr, func_instance* retFunc,
                              ...
                              set<ParseAPI::Block*>&           callBlocks)                // output
{
    ...
    if(callObj)
    {
        callObj->parse_img()->codeObject()->cs()->findRegions(
            retAddr - 1 - callObj->codeBase(), callRegs);
    }
    if(!callRegs.empty())
    {
        callObj->parse_img()->codeObject()->findBlocks(
            *callRegs.begin(), retAddr - 1 - callObj->codeBase(), callBlocks);
    }

    // remove blocks at returnAddr -1 that either have no call from callBlocks,
    // or that don't end at the correct address
    for(set<Block*>::iterator bit = callBlocks.begin(); bit != callBlocks.end();)
    {
    ...
    for(set<Block*>::iterator bit = callBlocks.begin();
        returningCallB.first == NULL && bit != callBlocks.end(); bit++)
    {
        ...
        {
            if((*eit)->type() == ParseAPI::CALL && !(*eit)->sinkEdge())
            {
                Block*    calledB = (*eit)->trg();
                Function* calledF =
                    calledB->obj()->findFuncByEntry(calledB->region(), calledB->start());
                if(calledF == retFunc->ifunc() || calledF->contains(retBlock->llb()))
                {
                    returningCallB.first = *bit;
                    returningCallB.second =
                        calledB->start() + llproc->findObject(calledB->obj())->codeBase();
                    ...
                    cit != calls.end(); cit++)
                {
                    if(!(*cit)->sinkEdge())
                    {
                        ...
                        Function* tCalledF    = tailCalledB->obj()->findFuncByEntry(
                            tailCalledB->region(), tailCalledB->start());
                           ...
                           tCalledF->contains(retBlock->llb()))
                        {
                            ...
                            returningCallB.second =
                                tailCalledB->start() +
                                llproc->findObject(tailCalledB->obj())->codeBase();
                            break;
                        }
                    }
                }
...
void
HybridAnalysis::badTransferCB(BPatch_point* point, void* returnValue)
{
    Address pointAddr = (Address) point->getAddress();
    ...
    if(point->getPointType() == BPatch_subroutine)
    {
        ...
        if(ParseAPI::RETURN == targFunc->lowlevel_func()->ifunc()->retstatus())
        {
    ...
    if(point->getPointType() == BPatch_locExit)
    {
        ...
        getCallBlocks(returnAddr, point->llpoint()->func(), point->llpoint()->block(),
                      returningCallB, callBlocks);
        ...
        if(!callBlocks.empty())
        {
            ...
            // of a block, patch post-call areas and return
                        ...
                        returningCallB.first->region(), target))
            {
                vector<BPatch_point*> callPts;
            ...
            {  // parse at the call fallthrough
                ...
                mal_printf("stopThread instrumentation found return at %p, "
                           ...
                           point->getAddress(), target, callPoints[0]->getAddress(),
                           FILE__, __LINE__);
                ...
                if(point->llpoint()->block()->llb()->isShared())
                {
                    ...
                    BPatch_function* calledFunc =
                        proc()->findFunctionByEntry(returningCallB.second);
                    parseAfterCallAndInstrument(callPoints[0], calledFunc, true);
                }
                else
                {
                    parseAfterCallAndInstrument(callPoints[0], point->getFunction(),
                                                true);
                }
            }
        }
    ...
    if(!point->isDynamic())
    {
        ...
        if(instrumentedFuncs->end() != instrumentedFuncs->find(func) &&
           (*instrumentedFuncs)[func]->end() != (*instrumentedFuncs)[func]->find(point))
        {
            proc()->deleteSnippet((*(*instrumentedFuncs)[func])[point]);
