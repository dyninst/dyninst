...
void HybridAnalysis::synchShadowOrigCB(BPatch_point *point, bool toOrig) {
  ...
  if (!toOrig) {
    ...
    for (set<Address>::iterator pit = pages.begin(); pit != pages.end();
         pit++) {
      proc()->setMemoryAccessRights(
          *pit, proc()->lowlevel_process()->getMemoryPageSize(),
...
void HybridAnalysis::signalHandlerCB(BPatch_point *, long,
                                     std::vector<Address> &) {}
...
bool HybridAnalysis::removeCodeDiscoveryCallback() { return false; }

bool HybridAnalysis::removeSignalHandlerCallback() { return false; }
...
static void signalHandlerEntryCB_wrapper(BPatch_point *point,
                                         void *excRecAddr) {
...
static void signalHandlerEntryCB2_wrapper(BPatch_point *point,
                                          void *excCtxtAddr) {
  dynamic_cast<BPatch_process *>(point->getFunction()->getProc())
      ->getHybridAnalysis()
...
void HybridAnalysis::signalHandlerCB(BPatch_point *point, long signum,
  ...
  while (it < handlers.end()) {
    ...
    BPatch_stopThreadExpr sThread1(signalHandlerEntryCB_wrapper, excRecAddr,
                                   false, BPatch_noInterp);
    BPatch_stopThreadExpr sThread2(signalHandlerEntryCB2_wrapper, excCtxtAddr,
                                   false, BPatch_noInterp);
 ...
 */
void HybridAnalysis::signalHandlerEntryCB(BPatch_point *point,
                                          Address excRecAddr) {
 ...
 */
void HybridAnalysis::signalHandlerEntryCB2(BPatch_point *point,
                                           Address excCtxtAddr) {
  ...
  // calculate the offset of the fault address in the EXCEPTION_RECORD
  CONTEXT *cont =
      (CONTEXT *)excCtxtAddr;  // bogus pointer, but I won't write to it
...
void HybridAnalysis::virtualFreeCB(BPatch_point *, void *t) {
  ...
  cerr << "virtualFree [" << hex << virtualFreeAddr_ << ","
       << virtualFreeAddr_ + (unsigned)virtualFreeSize_ << "], "
       << (unsigned)type << dec << endl;
  ...
  if (1 || type == MEM_RELEASE) {
    mapped_object *obj =
        proc()->lowlevel_process()->findObject(virtualFreeAddr_);

    if (!obj) return;
    virtualFreeAddr_ = obj->codeBase();
    virtualFreeSize_ = obj->imageSize();
    // DEBUG!
    cerr << "Removing VirtualAlloc'ed shared object " << obj->fileName()
         << endl;
  ...
  for (Address i = virtualFreeAddr_; i < (virtualFreeAddr_ + virtualFreeSize_);
       ++i) {
...
void HybridAnalysis::signalHandlerExitCB(BPatch_point *point, void *) {
  ...
  proc()->lowlevel_process()->readDataSpace((void *)pcLoc, sizeof(resumePC),
                                            &resumePC, true);
  ...
  if (funcs.empty()) {
    ...
    mal_printf(
        "Program will resume at %lx in %d existing functions, "
  ...
  mal_printf(
      "Exception handler exiting at %lx will resume execution at "
      "%lx %s[%d]\n",
...
void HybridAnalysis::abruptEndCB(BPatch_point *point, void *) {
  ...
  unsigned char *regptr =
      (unsigned char *)reg->getPtrToInstruction(reg->offset());
  ...
  if (firstNonzero == regSize) {
    mal_printf(
        "Not extending parse after abruptEnd; the code byes are "
...
static int getPreCallPoints(ParseAPI::Block *blk, BPatch_process *proc,
  ...
  blk->getFuncs(pFuncs);
  for (vector<Function *>::iterator fit = pFuncs.begin(); fit != pFuncs.end();
       fit++) {
...
void HybridAnalysis::getCallBlocks(
    Address retAddr, func_instance *retFunc, block_instance *retBlock,
  ...
  for (set<Block *>::iterator bit = callBlocks.begin();
    ...
    for (Block::edgelist::const_iterator eit = trgs.begin(); eit != trgs.end();
      ...
      if ((*eit)->type() == ParseAPI::CALL && !(*eit)->sinkEdge()) {
        ...
        Function *calledF = calledB->obj()->findFuncByEntry(calledB->region(),
                                                            calledB->start());
 ...
 * 1.1 if it's a system library don't parse at the target, but if the point was
 * marked
 *     as a possibly non-returning indirect call, parse at its fallthrough addr
 *     and return.
 * 1.2 if the target is in the runtime library, translate to an unrelocated
 * address
 *     and continue, but for now assert, since this translation is happening
 * internally
...
void HybridAnalysis::badTransferCB(BPatch_point *point, void *returnValue) {
  ...
  if (point->getPointType() == BPatch_locExit) {
    ...
    if (!callBlocks.empty()) {
      ...
      // of a block, patch post-call areas and return
        ...
        mal_printf(
            "stopThread instrumentation found return at %lx, "
            "parsing return addr %lx as fallthrough of call "
            "instruction at %lx %s[%d]\n",
            (long)point->getAddress(), target, callPoints[0]->getAddress(),
            FILE__, __LINE__);
    ...
    // 3.2.2 no call blocks, parse the return addr as a new function
      ...
      if (point->getFunction()->getModule()->isExploratoryModeOn()) {
        ...
        mal_printf(
            "hybridCallbacks.C[%d] Observed abuse of normal return "
  ...
  if (0 == targFuncs.size()) {
    newParsing = true;
    mal_printf(
        "stopThread instrumentation found jump "
        "at 0x%lx leading to an unparsed target at 0x%lx\n",
        (long)point->getAddress(), target);
  } else {
    newParsing = false;
    mal_printf(
        "stopThread instrumentation added an edge for jump "
  ...
  if (targMod != point->getFunction()->getModule()) {
    ...
    if (newParsing) analyzeNewFunction(point, target, true, false);
  } else {
