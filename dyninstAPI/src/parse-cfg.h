...
void
checkIfRelocatable(instruction insn, bool& canBeRelocated);
...
public:
    ...
    /*** Function naming ***/
    string symTabName() const { return func_->getFirstSymbol()->getMangledName(); }
...
private:
    void calcUsedRegs(); /* Does one time calculation of registers used in a function, if
                            called again it just refers to the stored values and returns
                            that */
    ...
    // If the function contains the power preamble, this field points the corresponding
    // function that does not contain the preamble
