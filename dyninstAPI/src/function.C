...
func_instance::func_instance(parse_func* f, Address baseAddr, mapped_module* mod)
...
func_instance::func_instance(const func_instance* parFunc, mapped_module* childMod)
...
// default block
block_instance*
func_instance::setNewEntry(block_instance* def, std::set<block_instance*>& deadBlocks)
{
...
void
func_instance::setHandlerFaultAddr(Address fa)
{
...
void
func_instance::setHandlerFaultAddrAddr(Address faa, bool set)
{
    if(set)
    {
    ...
    Address faultAddr = 0;
    if(!proc()->readDataSpace((void*) faa, proc()->getAddressWidth(), (void*) &faultAddr,
                              true))
    {
        assert(0);
    }
    ...
    if(proc()->proc()->isRuntimeHeapAddr(faultAddr))
    {
        ...
        assert(success);
        assert(proc()->writeDataSpace((void*) faa, sizeof(Address), (void*) &origAddr));
    }
}

void
func_instance::getReachableBlocks(const set<block_instance*>&  exceptBlocks,
                                  ...
                                  set<block_instance*>&        reachBlocks)  // output
{
    ...
    for(set<parse_block*>::iterator rit = imgReach.begin(); rit != imgReach.end(); rit++)
    {
...
void
print_func_vector_by_pretty_name(std::string prefix, std::vector<func_instance*>* funcs)
{
    unsigned int   i;
    func_instance* func;
...
mapped_object*
func_instance::obj() const
{
    return mod()->obj();
}
AddressSpace*
func_instance::proc() const
{
    return obj()->proc();
}

const func_instance::BlockSet&
func_instance::unresolvedCF()
{
...
void
func_instance::removeAbruptEnd(const block_instance* block)
{
...
block_instance*
func_instance::entryBlock()
{
    return SCAST_BI(entry());
}

unsigned
func_instance::getNumDynamicCalls()
{
    unsigned count = 0;
...
void
func_instance::debugPrint() const
{
    ...
    fprintf(stderr, "  Object: %s (%p), module: %s (%p)\n", obj()->fileName().c_str(),
            (void*) obj(), mod()->fileName().c_str(), (void*) mod());
    for(Blockset::const_iterator cb = all_blocks_.begin(); cb != all_blocks_.end(); cb++)
    {
...
void
func_instance::addSymTabName(const std::string name, bool isPrimary)
{
...
void
func_instance::addPrettyName(const std::string name, bool isPrimary)
{
...
// to the funcs list.
bool
func_instance::getSharingFuncs(block_instance* b, std::set<func_instance*>& funcs)
{
    bool ret = false;
    ...
    for(; fit != lfuncs.end(); ++fit)
    {
        ...
        if(funcs.find(hl_func) == funcs.end())
            ret = true;
...
bool
func_instance::getSharingFuncs(std::set<func_instance*>& funcs)
{
    bool ret = false;
...
bool
func_instance::getOverlappingFuncs(block_instance* block, std::set<func_instance*>& funcs)
{
...
bool
func_instance::getOverlappingFuncs(std::set<func_instance*>& funcs)
{
    bool ret = false;
...
std::string
func_instance::get_name() const
{
    return symTabName();
}

Offset
func_instance::addrToOffset(const Address a) const
{
    return a - (addr() - ifunc()->getOffset());
}

const std::vector<int_parRegion*>&
func_instance::parRegions()
{
...
bool
func_instance::consistency() const
{
...
void
func_instance::triggerModified()
{
    // KEVINTODO: is there anything to do here?
}

Address
func_instance::get_address() const
{
    assert(0);
    return 0;
}
unsigned
func_instance::get_size() const
{
    assert(0);
    return 0;
}
...
bool
func_instance::isInstrumentable()
{
    ...
    if(inst)
        return inst->isInstrumentable(this);
    cerr << "Error: no instrumenter for " << symTabName() << endl;
    return false;
}

block_instance*
func_instance::getBlockByEntry(const Address addr)
{
...
bool
func_instance::getBlocks(const Address addr, set<block_instance*>& blks)
{
...
block_instance*
func_instance::getBlock(const Address addr)
{
    std::set<block_instance*> blks;
    getBlocks(addr, blks);
    for(std::set<block_instance*>::iterator iter = blks.begin(); iter != blks.end();
        ++iter)
    {
        if((*iter)->getInsn(addr).isValid())
            return *iter;
    }
    return NULL;
}
...
bool
func_instance::addSymbolsForCopy()
{
    ...
    if(obj()->isStaticExec())
    {
        ...
        if(!updateRelocationsToSym(oldsym, wrapperSym))
            return false;
    }
    else
    {
...
bool
func_instance::updateRelocationsToSym(Symbol* oldsym, Symbol* newsym)
{
...
Symbol*
func_instance::getWrapperSymbol()
{
...
void
func_instance::createWrapperSymbol(Address entry, std::string name)
{
    if(wrapperSym_)
    {
    ...
    wrapperSym_ = new Symbol(name, Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
                             ...
                             0,       // size - zero okay?
                             false,   // Definitely not dynamic ("Static binaries don't
                                      // have dynamic symbols - Dan")
...
instPoint*
func_instance::funcEntryPoint(bool create)
{
    // Lookup the cached points
    instPoint* p = IPCONV(
        proc()->mgr()->findPoint(Location::Function(this), Point::FuncEntry, create));
    return p;
}

instPoint*
func_instance::funcExitPoint(block_instance* b, bool create)
{
    instPoint* p = IPCONV(
        proc()->mgr()->findPoint(Location::ExitSite(this, b), Point::FuncExit, create));
    return p;
}

void
func_instance::funcExitPoints(Points* pts)
{
...
instPoint*
func_instance::preCallPoint(block_instance* b, bool create)
{
    instPoint* p = IPCONV(
        proc()->mgr()->findPoint(Location::CallSite(this, b), Point::PreCall, create));
    return p;
}

instPoint*
func_instance::postCallPoint(block_instance* b, bool create)
{
    instPoint* p = IPCONV(
        proc()->mgr()->findPoint(Location::CallSite(this, b), Point::PostCall, create));
    return p;
}

void
func_instance::callPoints(Points* pts)
{
    std::vector<Point*> points;
...
instPoint*
func_instance::blockEntryPoint(block_instance* b, bool create)
{
    instPoint* p = IPCONV(proc()->mgr()->findPoint(Location::BlockInstance(this, b),
                                                   Point::BlockEntry, create));
    return p;
}

instPoint*
func_instance::blockExitPoint(block_instance* b, bool create)
{
    instPoint* p = IPCONV(proc()->mgr()->findPoint(Location::BlockInstance(this, b),
                                                   Point::BlockExit, create));
    return p;
}

instPoint*
func_instance::preInsnPoint(block_instance* b, Address a,
                            InstructionAPI::Instruction insn, bool trusted, bool create)
{
...
instPoint*
func_instance::postInsnPoint(block_instance* b, Address a,
                             InstructionAPI::Instruction insn, bool trusted, bool create)
{
    Location loc = Location::InstructionInstance(this, b, a, insn, trusted);
...
void
func_instance::blockInsnPoints(block_instance* b, Points* pts)
{
    std::vector<Point*> points;
...
instPoint*
func_instance::edgePoint(edge_instance* e, bool create)
{
    instPoint* p = IPCONV(proc()->mgr()->findPoint(Location::EdgeInstance(this, e),
                                                   Point::EdgeDuring, create));
    return p;
}

void
func_instance::edgePoints(Points* pts)
{
    std::vector<Point*> points;
...
void
func_instance::removeBlock(block_instance* block)
{
...
void
func_instance::destroy(func_instance* f)
{
...
void
func_instance::split_block_cb(block_instance* b1, block_instance* b2)
{
...
void
func_instance::add_block_cb(block_instance* /*block*/)
{
...
bool
func_instance::getLiveCallerBlocks(
...
// Stack modifications
void
func_instance::addMod(StackMod* mod, TMap* tMap)
{
...
void
func_instance::removeMod(StackMod* mod)
{
    _modifications->erase(mod);
}

void
func_instance::printMods() const
{
...
Accesses*
func_instance::getAccesses(Address addr)
{
...
bool
func_instance::createOffsetVector()
{
    if(_processedOffsetVector)
    {
    ...
    for(auto defIter = _definitionMap->begin(); defIter != _definitionMap->end();
        defIter++)
    {
        ...
        std::set<Address> tmp;
        if(::getAccesses(func, defBlock, defAddr, defInsn, &tmpDefAccesses, tmp, true))
        {
            assert(tmpDefAccesses.size() == 1);
            std::set<StackAccess*>& defAccessSet = tmpDefAccesses.begin()->second;
    ...
    {
        if(!addToOffsetVector((*iter).offset(), (*iter).size(), (*iter).type(), false,
                              (*iter).valid()))
        {
    ...
    else
    {
        ...
        stackmods_printf(
            "Function %s has INCONSISTENT OFFSET VECTOR (FUNCTION WILL BE SKIPPED)\n",
            prettyName().c_str());
    }
...
static int
randomNumGenerator(int i)
{
    return std::rand() % i;
}

static bool
matchRanges(ValidPCRange* a, ValidPCRange* b)
{
    auto aIter = a->begin();
    auto bIter = b->begin();
...
bool
func_instance::randomize(TMap* tMap, bool seeded, int seed)
{
    if(seeded)
    {
        _seeded = true;
    ...
    for(auto iter = stack.begin(); iter != stack.end();)
    {
        ...
        while(/* Stop at the end */
              ...
              (curLoc->off() < raLoc) && (curLoc->off() + curLoc->size() < raLoc) &&
              /* Skip non-debug */
    ...
    for(auto iter = localsRanges.begin(); iter != localsRanges.end(); ++iter)
    {
        ...
        for(auto viter = vec.begin(); viter != vec.end(); ++viter)
        {
            ...
            Move* tmp = new Move(cur->off().height(), cur->off().height() + cur->size(),
                                 nextLoc.height());
            addMod(tmp, tMap);
            nextLoc += cur->size();
        }
...
bool
func_instance::createOffsetVector_Symbols()
{
    bool ret = true;
    ...
    for(auto vIter = _vars.begin(); vIter != _vars.end(); ++vIter)
    {
        ...
        stackmods_printf("\t\t\t Found %s (type %s) @ %ld, size = %u\n",
                         var->getName().c_str(),
                         var->getType(Type::share)->getName().c_str(), offset,
                         var->getType(Type::share)->getSize());
        for(auto locIter = locs.begin(); locIter != locs.end(); ++locIter)
        {
            VariableLocation curLoc = *locIter;
            if(curLoc.mr_reg == MachRegister::getFramePointer(arch))
        ...
        {
            _tmpObjects->insert(
                tmpObject(offset, var->getType(Type::share)->getSize(), sat, valid));
        }
    }
...
bool
func_instance::createOffsetVector_Analysis(ParseAPI::Function*         func,
                                           ParseAPI::Block*            block,
                                           InstructionAPI::Instruction insn, Address addr)
{
    bool ret = true;
    ...
    if(::getAccesses(func, block, addr, insn, accesses, defAddrsToMod))
    {
        ...
        // all the definitions.
        for(auto addrIter = defAddrsToMod.begin(); addrIter != defAddrsToMod.end();
            addrIter++)
        {
...
bool
func_instance::addToOffsetVector(StackAnalysis::Height off, int size,
                                 StackAccess::StackAccessType type, bool isRegisterHeight,
                                 ValidPCRange* valid, MachRegister reg)
{
    ...
    if(size == 0)
    {
        stackmods_printf("\t\t\t addToOffsetVector: trying to add "
                         "non-StackAccess::REGHEIGHT of size 0. Skipping.\n");
        return ret;
    }
    ...
    {
        stackmods_printf("\t\t\t\t This stack access interferes with the RA. We may be "
                         "confused. Skipping.\n");
        if(isDebugType(type))
        {
            // Silently skip; doesn't hurt--if there's an actual access, the others will
            // catch it. Okay to skip here because getAccesses won't return this one
            // because it came from the DWARF.
    ...
    if(_offVec->find(off, lb, ub, existing))
    {
        ...
        if(lb == off)
        {
            ...
            {
                stackmods_printf("\t\t\t\t\t Existing has smaller size. Checking whether "
                                 "to update.\n");
                bool skip = false;
                ...
                for(int i = 0; i < size; i++)
                {
                    ...
                    {
                        // If we find a different range at the potential update size,
                        // skip!
                        if(existing2 != existing)
                        {
                            stackmods_printf(
                                "\t\t\t\t\t\t\t\t WARNING: updating size would conflict "
                                "with different existing range.\n");
                            skip = true;
                            ...
                            else
                            {
                                ...
                                StackLocation* tmp = new StackLocation(
                                    off, size, StackAccess::MISUNDERSTOOD,
        ...
        else if(lb < off && off < ub)
        {
            ...
            {
                stackmods_printf("\t\t\t\t\t Existing and new upper bounds are the same. "
                                 "Skip adding.\n");
            }
            else if(existingUpperBound > newUpperBound)
            {
                stackmods_printf("\t\t\t\t\t Existing upper bound is greater than what "
                                 "we're adding. Skip adding.\n");
            }
            else
            {
                stackmods_printf("\t\t\t\t\t WARNING: conflict because existing upper "
                                 "bound is less than what we're adding.\n");
                ...
                else
                {
                    for(int i = 0; i < size; i++)
                    {
                        ...
                        if(_offVec->find(off + i, lb2, ub2, existing2))
                        {
                            if(existing2 != existing)
                            {
                                stackmods_printf("\t\t\t\t\t\t Range overlaps with "
                                                 "another existing range %s\n",
...
void
func_instance::createTMap_internal(StackMod* mod, StackLocation* loc, TMap* tMap)
{
...
void
func_instance::createTMap_internal(StackMod* mod, TMap* tMap)
{
    ...
    if(!_offVec->definedRegs().empty())
    {
        ...
        {
            for(auto iter2 = (*offIter).second.begin(); iter2 != (*offIter).second.end();
                ++iter2)
            {
