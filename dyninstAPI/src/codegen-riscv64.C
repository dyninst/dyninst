...
{
    if (gen.usesCompressedInstructionFormat()) {
        instruction insn;
                               ...
                               unsigned size)
{
    ...
    for (int i = 0; i < num; i++) {
        generateNop(gen, gen.usesCompressedInstructionFormat());
    }
}
                                      ...
                                      bool isCall)
{
    ...
    if (isCall) {
        generateJal(gen, GPR_RA, disp, gen.usesCompressedInstructionFormat());
    }
    else {
        generateJ(gen, disp, gen.usesCompressedInstructionFormat());
    }
}
                                     ...
                                     bool isCall)
{
    ...
    if (isCall) {
        ...
        if (disp >= UTYPE_IMM_MIN && disp < UTYPE_IMM_MAX) {
            ...
            generateAuipc(gen, GPR_RA, top, gen.usesCompressedInstructionFormat());
            generateAddi(gen, GPR_RA, GPR_RA, offset, gen.usesCompressedInstructionFormat());
        }
        else {
            ...
            generateAuipc(gen, GPR_RA, 0, gen.usesCompressedInstructionFormat());
            generateCalcImm(gen, scratch, disp, true, gen.usesCompressedInstructionFormat());
            generateAdd(gen, GPR_RA, GPR_RA, scratch, gen.usesCompressedInstructionFormat());
        }
        // generate jalr
        generateJalr(gen, GPR_RA, GPR_RA, 0, gen.usesCompressedInstructionFormat());
    }
    else {
        ...
        */
        ...
        if (disp >= UTYPE_IMM_MIN && disp < UTYPE_IMM_MAX) {
            ...
            generateAuipc(gen, scratch, top, gen.usesCompressedInstructionFormat());
            generateAddi(gen, scratch, scratch, offset, gen.usesCompressedInstructionFormat());
        }
        else {
            ...
            generateAuipc(gen, GPR_RA, 0, gen.usesCompressedInstructionFormat());
            generateCalcImm(gen, scratch2, disp, true, gen.usesCompressedInstructionFormat());
            generateAdd(gen, scratch, scratch, scratch2, gen.usesCompressedInstructionFormat());
        }

        generateJr(gen, scratch, 0, gen.usesCompressedInstructionFormat());
    }
}
                                            ...
                                            Dyninst::Address to)
{
    ...
    if (disp >= UTYPE_IMM_MIN && disp < UTYPE_IMM_MAX) {
        ...
        generateAuipc(gen, GPR_T0, top, gen.usesCompressedInstructionFormat());
        generateAddi(gen, GPR_T0, GPR_T0, offset, gen.usesCompressedInstructionFormat());
    }
    else {
        generateAuipc(gen, GPR_T0, 0, gen.usesCompressedInstructionFormat());
        generateCalcImm(gen, GPR_T1, disp, true, gen.usesCompressedInstructionFormat());
        generateAdd(gen, GPR_T0, GPR_T0, GPR_T1, gen.usesCompressedInstructionFormat());
    }

    generateJr(gen, GPR_T0, 0, gen.usesCompressedInstructionFormat());
}
                                     ...
                                     Dyninst::Address to)
{
    ...
    if (disp < BTYPE_IMM_MIN || disp >= BTYPE_IMM_MAX) {
        ...
        */
        ...
        generateCmpBranch(gen, bCondOp, rs1, rs2, disp, gen.usesCompressedInstructionFormat());
    }
}
                                              ...
                                              std::vector<Dyninst::Register> *exclude)
{
    ...
    //generateLi(gen, scratch, static_cast<Dyninst::RegValue>(val), true, gen.usesCompressedInstructionFormat());
    bool useRVC = gen.usesCompressedInstructionFormat();
    if (useRVC) {
        // c.li
                             ...
                             codeGen &gen)
{
    ...
    generateLoadImm(gen, rd, disp, true, true, gen.usesCompressedInstructionFormat());
    return true;
}
                              ...
                              bool useRVC)
{
    // If both rs1 and rs2 are x0, it is equivalent to unconditional branch
    if (rs1 == GPR_ZERO && rs2 == GPR_ZERO) {
        return generateJal(gen, GPR_ZERO, imm, gen.usesCompressedInstructionFormat());
    }
    if (useRVC) {
        if ((rs1 == GPR_ZERO && rs2 >= GPR_X8 && rs2 < GPR_X16) &&
