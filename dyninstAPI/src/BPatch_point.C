...
void
BPatch_point::setLoop(BPatch_basicBlockLoop* l)
{
    ...
    assert(pointType == BPatch_locLoopEntry || pointType == BPatch_locLoopExit ||
           pointType == BPatch_locLoopStartIter || pointType == BPatch_locLoopEndIter);
...
BPatch_point::getCalledFunction()
{
    ...
    if(point->type() != instPoint::PreCall && point->type() != instPoint::PostCall)
    {
...
void
BPatch_point::attachMemAcc(BPatch_memoryAccess* newMemAcc)
{
    if(memacc)
    {
...
BPatch_point::getMemoryAccess()
{
    ...
    attachMemAcc(
        converter.convert(i, point->insnAddr(), point->proc()->getAddressWidth() == 8));
    return memacc;
}
 ...
 */
void*
BPatch_point::monitorCalls(BPatch_function* user_cb)
{
...
BPatch_procedureLocation
BPatch_point::convertInstPointType_t(int intType)
{
...
void
BPatch_point::recordSnippet(BPatch_callWhen when, BPatch_snippetOrder order,
...
bool
BPatch_point::removeSnippet(BPatchSnippetHandle* handle)
{
...
instPoint*
BPatch_point::getPoint(BPatch_callWhen when) const
{
    switch(when)
    {
...
Dyninst::PatchAPI::convert(const BPatch_point* p, BPatch_callWhen when)
{
