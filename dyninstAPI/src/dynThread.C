...
void PCThread::setTid(dynthread_t tid) { manuallySetTid_ = tid; }

int PCThread::getIndex() const { return index_; }
...
void PCThread::findSingleThreadInfo() {
  ...
  proccontrol_printf(
      "%s[%d]: searching stackwalk for initial func and stack top\n", FILE__,
      __LINE__);
...
PCProcess *PCThread::getProc() const { return proc_; }
...
void PCThread::clearStackwalk() { cached_stackwalk_.clear(); }
...
void PCThread::markExited() { pcThr_ = Thread::ptr(); }
