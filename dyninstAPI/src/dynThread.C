...
PCThread::PCThread(PCProcess* parent, int ind, Thread::ptr thr)
: proc_(parent)
, pcThr_(thr)
, index_(ind)
...
PCThread*
PCThread::createPCThread(PCProcess* parent, Thread::ptr thr)
{
...
PCThread::getTid() const
{
    ...
    {
        if(manuallySetTid_ == DYNINST_SINGLETHREADED)
            return 0;
...
void
PCThread::setTid(dynthread_t tid)
{
    manuallySetTid_ = tid;
}

int
PCThread::getIndex() const
{
    return index_;
}

Dyninst::LWP
PCThread::getLWP() const
{
    if(pcThr_ == Thread::ptr())
        return savedLWP_;
...
void
PCThread::findSingleThreadInfo()
{
    assert(startFuncAddr_ == 0 || stackAddr_ == 0);

    std::vector<Frame> stackWalk;
    if(!walkStack(stackWalk))
        return;
    ...
    while((stackAddr_ == 0 || startFuncAddr_ == 0) && pos >= 0)
    {
        ...
        if(stackAddr_ == 0)
            stackAddr_ = (Address) stackWalk[pos].getSP();
        if(startFuncAddr_ == 0)
        {
...
PCProcess*
PCThread::getProc() const
{
    return proc_;
}

bool
PCThread::walkStack(std::vector<Frame>& stackWalk)
{
...
void
PCThread::clearStackwalk()
{
    cached_stackwalk_.clear();
}

bool
PCThread::getRegisters(RegisterPool& regs, bool /* includeFP */)
{
    if(pcThr_ == Thread::ptr())
        return false;
...
bool
PCThread::changePC(Address newPC)
{
    if(pcThr_ == Thread::ptr())
        return false;
...
bool
PCThread::postIRPC(inferiorRPCinProgress* newRPC)
{
    if(pcThr_ == Thread::ptr())
        return false;
