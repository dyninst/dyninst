...
class instMapping {
 public:
  instMapping(const std::string f, const std::string i, const int w,
              callWhen wn, callOrder o, AstNodePtr a = AstNodePtr(),
              std::string l = "")
      : func(f),
        inst(i),
        lib(l),
...
typedef enum gnenum { rc_before_jump, rc_after_jump, rc_no_control } RegControl;
...
void emitV(opCode op, Register src1, Register src2, Register dst, codeGen &gen,
...
void emitVload(opCode op, Address src1, Register src2, Register dst,
               codeGen &gen, bool noCost, registerSpace *rs = NULL,
               int size = 4, const instPoint *location = NULL,
...
// for loadOp and loadConstOp (reading from an Address)
void emitVload(opCode op, const image_variable *src1, Register src2,
               Register dst, codeGen &gen, bool noCost,
               registerSpace *rs = NULL, int size = 4,
               const instPoint *location = NULL, AddressSpace *proc = NULL);

// for storeOp (writing to an Address)
void emitVstore(opCode op, Register src1, Register src2,
                const image_variable *dst, codeGen &gen, bool noCost,
                registerSpace *rs = NULL, int size = 4,
                const instPoint *location = NULL, AddressSpace *proc = NULL);
...
Register emitFuncCall(opCode op, codeGen &gen, pdvector<AstNodePtr> &operands,
                      bool noCost, func_instance *func);

// Obsolete version that uses an address. DON'T USE THIS or expect it to
// survive.
Register emitFuncCall(opCode op, codeGen &gen, pdvector<AstNodePtr> &operands,
