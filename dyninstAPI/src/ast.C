...
static bool
IsSignedOperation(BPatch_type* l, BPatch_type* r)
{
    if(l == NULL || r == NULL)
        return true;
...
char*
AstNode::getSnippetName()
{
    return snippetName;
}

void
AstNode::setSnippetName(char* n)
{
    if(n != NULL)
    {
        //      printf(":::%s\n", n);
...
AstNodePtr
AstNode::stackInsertNode(int size, MSpecialType type)
{
    return AstNodePtr(new AstStackInsertNode(size, type));
}

AstNodePtr
AstNode::stackRemoveNode(int size, MSpecialType type)
{
    return AstNodePtr(new AstStackRemoveNode(size, type));
}

AstNodePtr
AstNode::stackRemoveNode(int size, MSpecialType type, func_instance* func,
                         bool canaryAfterPrologue, long canaryHeight)
{
    return AstNodePtr(
        new AstStackRemoveNode(size, type, func, canaryAfterPrologue, canaryHeight));
}

AstNodePtr
AstNode::stackGenericNode()
{
    return AstNodePtr(new AstStackGenericNode());
}

AstNodePtr
AstNode::labelNode(std::string& label)
{
    return AstNodePtr(new AstLabelNode(label));
}

AstNodePtr
AstNode::operandNode(operandType ot, void* arg)
{
...
AstNodePtr
AstNode::operandNode(operandType ot, AstNodePtr ast)
{
    return AstNodePtr(new AstOperandNode(ot, ast));
}

AstNodePtr
AstNode::operandNode(operandType ot, const image_variable* iv)
{
    return AstNodePtr(new AstOperandNode(ot, iv));
}

AstNodePtr
AstNode::sequenceNode(std::vector<AstNodePtr>& sequence)
{
...
AstNodePtr
AstNode::variableNode(vector<AstNodePtr>&           ast_wrappers,
...
AstNodePtr
AstNode::operatorNode(opCode ot, AstNodePtr l, AstNodePtr r, AstNodePtr e)
{
    return AstNodePtr(new AstOperatorNode(ot, l, r, e));
}

AstNodePtr
AstNode::funcCallNode(const std::string& func, std::vector<AstNodePtr>& args,
...
AstNodePtr
AstNode::funcCallNode(func_instance* func, std::vector<AstNodePtr>& args)
{
    if(func == NULL)
        return AstNodePtr();
    return AstNodePtr(new AstCallNode(func, args));
...
AstNodePtr
AstNode::funcCallNode(func_instance* func)
{
    if(func == NULL)
        return AstNodePtr();
...
AstNodePtr
AstNode::funcCallNode(Address addr, std::vector<AstNodePtr>& args)
{
    return AstNodePtr(new AstCallNode(addr, args));
}

AstNodePtr
AstNode::memoryNode(memoryType ma, int which, int size)
{
    return AstNodePtr(new AstMemoryNode(ma, which, size));
}

AstNodePtr
AstNode::miniTrampNode(AstNodePtr tramp)
{
    if(tramp == NULL)
        return AstNodePtr();
...
AstNodePtr
AstNode::snippetNode(Dyninst::PatchAPI::SnippetPtr snip)
{
    return AstNodePtr(new AstSnippetNode(snip));
}

AstNodePtr
AstNode::scrambleRegistersNode()
{
...
Address
AstMiniTrampNode::generateTramp(codeGen& gen, int& trampCost, bool noCost)
{
...
// node is shared
Register
AstNode::allocateAndKeep(codeGen& gen, bool noCost)
{
...
//
bool
AstNode::generateCode(codeGen& gen, bool noCost, Address& retAddr, Register& retReg)
{
...
bool
AstNode::generateCode(codeGen& gen, bool noCost)
{
...
bool
AstNode::previousComputationValid(Register& reg, codeGen& gen)
{
...
bool
AstNode::initRegisters(codeGen& g)
{
    bool                    ret = true;
...
bool
AstNode::generateCode_phase2(codeGen&, bool, Address&, Register&)
{
    fprintf(
        stderr,
        "ERROR: call to AstNode generateCode_phase2; should be handled by subclass\n");
...
bool
AstNullNode::generateCode_phase2(codeGen& gen, bool, Address& retAddr, Register& retReg)
{
    retAddr = ADDR_NULL;
    retReg  = REG_NULL;
...
bool
AstNode::allocateCanaryRegister(codeGen& gen, bool noCost, Register& reg,
...
bool
AstStackInsertNode::generateCode_phase2(codeGen& gen, bool noCost, Address&, Register&)
{
    ...
    else if(type == CANARY_AST)
    {
        ...
        // 2. Ensures that the canary will get overwritten (rather than empty alignment
        // space) in case of an overflow
        ...
        //  #define TARGET_THREAD_SSP_OFFSET (TARGET_64BIT ? (TARGET_X32 ? 0x18 : 0x28) :
        //  0x14))) */
...
bool
AstStackRemoveNode::generateCode_phase2(codeGen& gen, bool noCost, Address&, Register&)
{
    // Turn off default basetramp instrumentation saves & restores
    gen.setInsertNaked(true);
    gen.setModifiedStackFrame(true);
    ...
    else if(type == CANARY_AST)
    {
        ...
        // NOTE: Skipping saving live registers that will be clobbered by the call because
        // this will be non-returning
...
bool
AstStackGenericNode::generateCode_phase2(codeGen& gen, bool, Address&, Register&)
{
    gen.setInsertNaked(true);
    gen.setModifiedStackFrame(true);
...
#else
bool
AstStackInsertNode::generateCode_phase2(codeGen&, bool, Address&, Register&)
{
    return false;
}

bool
AstStackRemoveNode::generateCode_phase2(codeGen&, bool, Address&, Register&)
{
    return false;
}

bool
AstStackGenericNode::generateCode_phase2(codeGen&, bool, Address&, Register&)
{
    return false;
}
#endif
...
bool
AstLabelNode::generateCode_phase2(codeGen& gen, bool, Address& retAddr, Register& retReg)
{
...
bool
AstOperatorNode::initRegisters(codeGen& g)
{
    bool                    ret = true;
    std::vector<AstNodePtr> kids;
    getChildren(kids);
...
bool
AstOperatorNode::generateOptimizedAssignment(codeGen& gen, int size_, bool noCost)
{
...
#else
bool
AstOperatorNode::generateOptimizedAssignment(codeGen&, int, bool)
{
    return false;
}
#endif
...
bool
AstOperatorNode::generateCode_phase2(codeGen& gen, bool noCost, Address& retAddr,
                                     Register& retReg)
{
    ...
    switch(op)
    {
        ...
        case ifOp: {
            ...
            // We can reuse src1 for the body of the conditional; however, keep the value
            // here so that we can use it for the branch fix below.
        ...
        case storeOp: {
            ...
            switch(loperand->getoType())
            {
                ...
                case RegOffset: {
                    ...
                    // and it only allows for 3.  Prepare the dest address in scratch
                    // register src2.
                ...
                case origRegister:
                    ...
                    // loperand->getOValue(), src1, gen, getSize(), noCost);
                    loperand->decUseCount(gen);
                    break;
                case Param:
                case ParamAtCall:
                ...
                default: {
                    ...
                    // arithmetic expression Generate the left hand side, store the right
                    // to that address
        ...
        default: {
               ...
               doNotOverflow((int64_t) roperand->getOValue()))
            {
                ...
                emitImm(op, src1, (RegValue) roperand->getOValue(), retReg, gen, noCost,
                        gen.rs(), signedOp);
            ...
            else
            {
                ...
                emitV(op, src1, right_dest, retReg, gen, noCost, gen.rs(), size,
                      gen.point(), gen.addrSpace(), signedOp);
                if(src1 != Null_Register && loperand->decRefCount())
                {
...
bool
AstOperandNode::generateCode_phase2(codeGen& gen, bool noCost, Address&, Register& retReg)
{
    RETURN_KEPT_REG(retReg);
    ...
    switch(oType)
    {
        ...
        case ConstantString:
            ...
            {
                emitVload(loadConstOp, addr, retReg, retReg, gen, noCost, gen.rs(), size,
                          gen.point(), gen.addrSpace());
            }
            else
            {
        ...
        default:
            fprintf(
                stderr,
                "[%s:%d] ERROR: Unknown operand type %d in AstOperandNode generation\n",
...
bool
AstMemoryNode::generateCode_phase2(codeGen& gen, bool noCost, Address&, Register& retReg)
{
    RETURN_KEPT_REG(retReg);
...
bool
AstCallNode::initRegisters(codeGen& gen)
{
...
bool
AstCallNode::generateCode_phase2(codeGen& gen, bool noCost, Address&, Register& retReg)
{
...
bool
AstSequenceNode::generateCode_phase2(codeGen& gen, bool noCost, Address&,
                                     Register& retReg)
{
    RETURN_KEPT_REG(retReg);
...
bool
AstVariableNode::generateCode_phase2(codeGen& gen, bool noCost, Address& addr,
                                     Register& retReg)
{
...
bool
AstOriginalAddrNode::generateCode_phase2(codeGen& gen, bool noCost, Address&,
                                         Register& retReg)
{
    RETURN_KEPT_REG(retReg);
    if(retReg == REG_NULL)
    ...
    emitVload(loadConstOp, (Address) gen.point()->addr_compat(), retReg, retReg, gen,
              noCost);
    return true;
}

bool
AstActualAddrNode::generateCode_phase2(codeGen& gen, bool noCost, Address&,
                                       Register& retReg)
{
    if(retReg == REG_NULL)
    {
...
bool
AstDynamicTargetNode::generateCode_phase2(codeGen& gen, bool noCost, Address& retAddr,
                                          Register& retReg)
{
    ...
    {  // this is a dynamic ctrl flow instruction, have
        // getDynamicCallSiteArgs generate the necessary AST
        std::vector<AstNodePtr> args;
        if(!gen.addrSpace()->getDynamicCallSiteArgs(insn, gen.point()->block()->last(),
                                                    args))
        {
            return false;
        }
...
bool
AstScrambleRegistersNode::generateCode_phase2(codeGen& gen, bool, Address&, Register&)
{
    (void) gen;  // unused
#if defined(arch_x86_64)
...
int
AstOperatorNode::costHelper(enum CostStyleType costStyle) const
{
    int total = 0;
    ...
    if(op == ifOp)
    {
        ...
        {  // ie. there's an else block also, for the statements
            if(costStyle == Min)
...
int
AstOperandNode::costHelper(enum CostStyleType costStyle) const
{
    int total = 0;
    if(oType == Constant)
    {
...
int
AstCallNode::costHelper(enum CostStyleType costStyle) const
{
    int total = 0;
    if(func_)
...
int
AstSequenceNode::costHelper(enum CostStyleType costStyle) const
{
    int total = 0;
...
BPatch_type*
AstNode::checkType(BPatch_function*)
{
    return BPatch::bpatch->type_Untyped;
}

BPatch_type*
AstOperatorNode::checkType(BPatch_function* func)
{
...
BPatch_type*
AstOperandNode::checkType(BPatch_function* func)
{
    BPatch_type* ret       = NULL;
    ...
    if(oType == DataIndir)
    {
        ...
        ret = BPatch::bpatch->type_Untyped;
            ...
            (oType == ReturnVal) || (oType == ReturnAddr))
    {
        if(func)
        {
            switch(oType)
...
BPatch_type*
AstCallNode::checkType(BPatch_function* func)
{
    BPatch_type* ret       = NULL;
    bool         errorFlag = false;
...
BPatch_type*
AstSequenceNode::checkType(BPatch_function* func)
{
    BPatch_type* ret       = NULL;
...
bool
AstOperatorNode::canBeKept() const
{
    switch(op)
    {
        case plusOp:
...
bool
AstOperandNode::canBeKept() const
{
    switch(oType)
    {
        case DataReg:
...
AstCallNode::canBeKept() const
{
    if(constFunc_)
    {
        for(unsigned i = 0; i < args_.size(); i++)
        {
            ...
            {
                fprintf(stderr,
                        "AST %p: labelled const func but argument %u cannot be kept!\n",
                        (const void*) this, i);
                return false;
            }
        }
...
bool
AstSequenceNode::canBeKept() const
{
...
bool
AstVariableNode::canBeKept() const
{
    return ast_wrappers_[index]->canBeKept();
}

bool
AstMiniTrampNode::canBeKept() const
{
...
void
AstNode::decUseCount(codeGen& gen)
{
    if(useCount == 0)
        return;
...
void
AstNode::getChildren(std::vector<AstNodePtr>&)
{
#if 0
    fprintf(stderr, "Undefined call to getChildren for type: ");
    if (dynamic_cast<AstNullNode *>(this)) fprintf(stderr, "nullNode\n");
...
void
AstNode::setChildren(std::vector<AstNodePtr>&)
{
#if 0
...
void
AstOperatorNode::getChildren(std::vector<AstNodePtr>& children)
{
    if(loperand)
...
void
AstOperatorNode::setChildren(std::vector<AstNodePtr>& children)
{
...
void
AstOperandNode::getChildren(std::vector<AstNodePtr>& children)
{
    if(operand_)
...
void
AstOperandNode::setChildren(std::vector<AstNodePtr>& children)
{
    if(children.size() == 1)
    {
        // memory management?
...
void
AstCallNode::getChildren(std::vector<AstNodePtr>& children)
{
    for(unsigned i = 0; i < args_.size(); i++)
...
void
AstCallNode::setChildren(std::vector<AstNodePtr>& children)
{
...
void
AstSequenceNode::getChildren(std::vector<AstNodePtr>& children)
{
    for(unsigned i = 0; i < sequence_.size(); i++)
...
void
AstSequenceNode::setChildren(std::vector<AstNodePtr>& children)
{
...
void
AstVariableNode::getChildren(std::vector<AstNodePtr>& children)
{
    ast_wrappers_[index]->getChildren(children);
}

void
AstVariableNode::setChildren(std::vector<AstNodePtr>& children)
{
    ast_wrappers_[index]->setChildren(children);
}

AstNodePtr
AstVariableNode::deepCopy()
{
...
void
AstMiniTrampNode::getChildren(std::vector<AstNodePtr>& children)
{
    children.push_back(ast_);
}

void
AstMiniTrampNode::setChildren(std::vector<AstNodePtr>& children)
{
    if(children.size() == 1)
    {
        // memory management?
...
void
AstOperatorNode::setVariableAST(codeGen& g)
{
    if(loperand)
...
void
AstOperandNode::setVariableAST(codeGen& g)
{
    if(operand_)
...
void
AstCallNode::setVariableAST(codeGen& g)
{
    for(unsigned i = 0; i < args_.size(); i++)
...
void
AstSequenceNode::setVariableAST(codeGen& g)
{
    for(unsigned i = 0; i < sequence_.size(); i++)
...
void
AstVariableNode::setVariableAST(codeGen& gen)
{
    // fprintf(stderr, "Generating code for variable in function %s with start address
    // 0x%lx at address 0x%lx\n",gen.func()->prettyName().c_str(),
    ...
    Address addr =
        gen.point()->addr_compat();  // Offset of inst point from function base address
    bool found = false;
...
void
AstMiniTrampNode::setVariableAST(codeGen& g)
{
    if(ast_)
...
bool
AstCallNode::containsFuncCall() const
{
    return true;
}

bool
AstOperatorNode::containsFuncCall() const
{
...
bool
AstOperandNode::containsFuncCall() const
{
...
bool
AstMiniTrampNode::containsFuncCall() const
{
...
bool
AstSequenceNode::containsFuncCall() const
{
    for(unsigned i = 0; i < sequence_.size(); i++)
    {
...
bool
AstVariableNode::containsFuncCall() const
{
    return ast_wrappers_[index]->containsFuncCall();
}

bool
AstNullNode::containsFuncCall() const
{
    return false;
}

bool
AstStackInsertNode::containsFuncCall() const
{
    return false;
}

bool
AstStackRemoveNode::containsFuncCall() const
{
    return false;
}

bool
AstStackGenericNode::containsFuncCall() const
{
    return false;
}

bool
AstLabelNode::containsFuncCall() const
{
    return false;
}

bool
AstMemoryNode::containsFuncCall() const
{
    return false;
}

bool
AstOriginalAddrNode::containsFuncCall() const
{
    return false;
}

bool
AstActualAddrNode::containsFuncCall() const
{
    return false;
}

bool
AstDynamicTargetNode::containsFuncCall() const
{
    return false;
}
bool
AstScrambleRegistersNode::containsFuncCall() const
{
    return false;
}

bool
AstCallNode::usesAppRegister() const
{
    for(unsigned i = 0; i < args_.size(); i++)
    {
...
bool
AstOperatorNode::usesAppRegister() const
{
...
bool
AstOperandNode::usesAppRegister() const
{
...
bool
AstMiniTrampNode::usesAppRegister() const
{
...
bool
AstSequenceNode::usesAppRegister() const
{
    for(unsigned i = 0; i < sequence_.size(); i++)
    {
...
bool
AstVariableNode::usesAppRegister() const
{
    return ast_wrappers_[index]->usesAppRegister();
}

bool
AstNullNode::usesAppRegister() const
{
    return false;
}

bool
AstStackInsertNode::usesAppRegister() const
{
    return false;
}

bool
AstStackRemoveNode::usesAppRegister() const
{
    return false;
}

bool
AstStackGenericNode::usesAppRegister() const
{
    return false;
}

bool
AstLabelNode::usesAppRegister() const
{
    return false;
}

bool
AstDynamicTargetNode::usesAppRegister() const
{
    return false;
}

bool
AstActualAddrNode::usesAppRegister() const
{
    return false;
}

bool
AstOriginalAddrNode::usesAppRegister() const
{
    return false;
}

bool
AstMemoryNode::usesAppRegister() const
{
    return true;
}

bool
AstScrambleRegistersNode::usesAppRegister() const
{
    return true;
}

void
regTracker_t::addKeptRegister(codeGen& gen, AstNode* n, Register reg)
{
    assert(n);
...
void
regTracker_t::removeKeptRegister(codeGen& gen, AstNode* n)
{
...
Register
regTracker_t::hasKeptRegister(AstNode* n)
{
    auto iter = tracker.find(n);
    if(iter == tracker.end())
        return REG_NULL;
...
bool
regTracker_t::stealKeptRegister(Register r)
{
...
void
regTracker_t::increaseConditionalLevel()
{
    condLevel++;
...
void
regTracker_t::decreaseAndClean(codeGen&)
{
    assert(condLevel > 0);
...
unsigned
regTracker_t::astHash(AstNode* const& ast)
{
    return addrHash4((Address) ast);
}

void
regTracker_t::debugPrint()
{
    if(!dyn_debug_ast)
        return;
...
int_variable*
AstOperandNode::lookUpVar(AddressSpace* as)
{
...
void
AstOperandNode::emitVariableLoad(opCode op, Register src2, Register dest, codeGen& gen,
                                 bool noCost, registerSpace* rs, int size_,
...
void
AstOperandNode::emitVariableStore(opCode op, Register src1, Register src2, codeGen& gen,
                                  bool noCost, registerSpace* rs, int size_,
                                  const instPoint* point, AddressSpace* as)
{
    int_variable* var = lookUpVar(as);
...
bool
AstNode::generate(Point* point, Buffer& buffer)
{
...
bool
AstSnippetNode::generateCode_phase2(codeGen& gen, bool, Address&, Register&)
{
...
std::string
AstNode::format(std::string indent)
{
...
std::string
AstNullNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstStackInsertNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstStackRemoveNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstStackGenericNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstOperatorNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstOperandNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstCallNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstSequenceNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstVariableNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstMemoryNode::format(std::string indent)
{
    std::stringstream ret;
...
std::string
AstNode::convert(operandType type)
{
    switch(type)
    {
        case Constant:
...
bool
AstOperandNode::initRegisters(codeGen& g)
{
    bool                    ret = true;
    std::vector<AstNodePtr> kids;
    getChildren(kids);
