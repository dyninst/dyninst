...
bool dynamic_linking::installTracing() { return true; }
...
bool dynamic_linking::getChangedObjects(EventRecord &,
...
bool dynamic_linking::handleIfDueToSharedObjectMapping(
  ...
  if (ev.type == evtLoadLibrary) {
    ...
    parsing_printf(
        "%s[%d]: load dll %s: hFile=%x, base=%x, debugOff=%x, debugSz=%d "
        "lpname=%x, %d\n",
    ...
    if (!imageName.length()) return true;
    DWORD64 iresult = SymLoadModule64(
        procHandle, ev.info.u.LoadDll.hFile, (PSTR)imageName.c_str(), NULL,
