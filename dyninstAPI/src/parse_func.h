...
#ifndef DYNINST_DYNINSTAPI_PARSE_FUNC_H
...
public:
  ...
  static parse_func *plt_func(Dyninst::SymtabAPI::Function *func, pdmodule *m, image *i,
                              Dyninst::ParseAPI::CodeObject *obj,
                              Dyninst::ParseAPI::CodeRegion *reg, Dyninst::InstructionSource *isrc,
  ...
  Dyninst::ParseAPI::FuncReturnStatus init_retstatus() const;

  // also sets retstatus
  void setinit_retstatus(Dyninst::ParseAPI::FuncReturnStatus rs);

  // true if we stopped the parse at a weird instruction (e.g., arpl)
  bool hasWeirdInsns() {
    return hasWeirdInsns_;
  }
  ...
  size_t getPrevBlocksUnresolvedCF() const {
    return prevBlocksUnresolvedCF_;
  }

  bool savesReturnAddr() const {
    return saves_return_addr_;
  }
  ...
  const Dyninst::SymtabAPI::Function *func() const {
    return func_;
  }

  bool containsPowerPreamble() {
    return containsPowerPreamble_;
  }

  void setContainsPowerPreamble(bool c) {
    containsPowerPreamble_ = c;
  }

  parse_func *getNoPowerPreambleFunc() {
    return noPowerPreambleFunc_;
  }

  void setNoPowerPreambleFunc(parse_func *f) {
    noPowerPreambleFunc_ = f;
  }

  Dyninst::Address getPowerTOCBaseAddress() {
    return baseTOC_;
  }

  void setPowerTOCBaseAddress(Dyninst::Address addr) {
    baseTOC_ = addr;
  }

private:
  // pointer to the underlying symtab Function
  Dyninst::SymtabAPI::Function *func_{nullptr};
