...
bool codeGen::operator==(void *p) const { return (p == (void *)buffer_); }
...
bool codeGen::verify() { return true; }
...
unsigned codeGen::used() const { return offset_ * CODE_GEN_OFFSET_SIZE; }

void *codeGen::start_ptr() const { return (void *)buffer_; }
...
void codeGen::setIndex(codeBufIndex_t index) {
  ...
  // Keep the pad
  if (used() >= size_) {
    // fprintf(stderr, "WARNING: overflow of codeGen structure (%d requested, %d
    // actual), trying to enlarge\n", used(), size_);
...
codeBufIndex_t codeGen::getIndex() const { return offset_; }
...
void codeGen::moveIndex(int disp) {
  ...
  if (cur % CODE_GEN_OFFSET_SIZE) {
    fprintf(stderr,
            "Error in codeGen: current index %d/%d, moving by %d, mod %d\n",
  ...
  switch (fillType) {
    ...
    case cgIllegal: {
      ...
      if ((used() - curUsed) != fillSize) {
        cerr << "ABORTING: " << used() << " - " << curUsed << " != " << fillSize
             << endl;
      }
      assert((used() - curUsed) == (unsigned)fillSize);
      break;
...
void codeGen::fillRemaining(int fillType) {
  if (fillType == cgNOP) {
    insnCodeGen::generateNOOP(*this, size_ - used());
  } else {
    assert(0 && "unimplemented");
  }
}
...
bool codeGen::hasPCRels() const { return (pcrels_.size() != 0); }

pcRelRegion::pcRelRegion(const instruction &i)
    : gen(NULL), orig_instruc(i), cur_offset(0), cur_size(0) {}

pcRelRegion::~pcRelRegion() {}

std::vector<relocPatch> &codeGen::allPatches() { return patches_; }

void codeGen::addPatch(codeBufIndex_t index, patchTarget *source, unsigned size,
                       relocPatch::patch_type_t ptype, Dyninst::Offset off) {
  relocPatch p(index, source, ptype, this, off, size);
  patches_.push_back(p);
}

void codeGen::addPatch(const relocPatch &p) { patches_.push_back(p); }

void codeGen::applyPatches() {
  std::vector<relocPatch>::iterator i;
  for (i = patches_.begin(); i != patches_.end(); i++) (*i).applyPatch();
}
...
bool relocPatch::isApplied() { return applied_; }

bool pcRelRegion::canPreApply() { return false; }

std::string patchTarget::get_name() const { return std::string("UNNAMED"); }

patchTarget::~patchTarget() {}

toAddressPatch::~toAddressPatch() {}

Address toAddressPatch::get_address() const { return addr; }

unsigned toAddressPatch::get_size() const { return 0; }

void toAddressPatch::set_address(Address a) { addr = a; }

codeGen codeGen::baseTemplate;

PCThread *codeGen::thread() { return thr_; }

unsigned codeGen::width() const { return addrSpace()->getAddressWidth(); }
...
instPoint *codeGen::point() const { return ip_; }
...
registerSpace *codeGen::rs() const { return rs_; }

regTracker_t *codeGen::tracker() const { return t_; }

Emitter *codeGen::codeEmitter() const { return emitter_; }
...
void codeGen::endTrackRegDefs() { trackRegDefs_ = false; }

const bitArray &codeGen::getRegsDefined() { return regsDefined_; }

void codeGen::markRegDefined(Register r) {
  if (!trackRegDefs_) return;
  regsDefined_[r] = true;
}
...
void codeGen::registerDefensivePad(block_instance *callBlock, Address padStart,
                                   unsigned padSize) {
