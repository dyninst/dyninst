...
codeGen&
codeGen::operator=(const codeGen& g)
{
...
void
codeGen::allocate(unsigned size)
{
...
void
codeGen::copy(const void* b, const unsigned size, const codeBufIndex_t index)
{
    if(size == 0)
        return;
...
void
codeGen::copy(const void* b, const unsigned size)
{
    if(size == 0)
        return;
...
void
codeGen::copy(const std::vector<unsigned char>& buf)
{
    if(buf.empty())
        return;
...
void
codeGen::insert(const void* b, const unsigned size, const codeBufIndex_t index)
{
    if(size == 0)
        return;
    assert(buffer_);
...
void
codeGen::copyAligned(const void* b, const unsigned size)
{
    if(size == 0)
        return;
...
void*
codeGen::get_ptr(unsigned offset) const
{
    assert(buffer_);
    assert(offset < size_);
    assert(sizeof(codeBuf_t) == CODE_GEN_OFFSET_SIZE);
...
void
codeGen::setIndex(codeBufIndex_t index)
{
    offset_ = index;
...
codeBufIndex_t
codeGen::getIndex() const
{
    return offset_;
}

void
codeGen::moveIndex(int disp)
{
...
long
codeGen::getDisplacement(codeBufIndex_t from, codeBufIndex_t to)
{
...
void
codeGen::fill(unsigned fillSize, int fillType)
{
    switch(fillType)
    {
        case cgNOP:
...
void
codeGen::fillRemaining(int fillType)
{
    if(fillType == cgNOP)
    {
...
void
codeGen::applyTemplate(const codeGen& c)
{
...
void
codeGen::setAddrSpace(AddressSpace* a)
{
    aSpace_ = a;
...
void
codeGen::realloc(unsigned newSize)
{
    if(newSize <= size_)
        return;

    unsigned increment = newSize - size_;
    if(increment < codeGenMinAlloc)
        increment = codeGenMinAlloc;
...
void
codeGen::addPCRelRegion(pcRelRegion* reg)
{
...
void
codeGen::addPatch(codeBufIndex_t index, patchTarget* source, unsigned size,
...
void
codeGen::addPatch(const relocPatch& p)
{
    patches_.push_back(p);
}

void
codeGen::applyPatches()
{
...
bool
pcRelRegion::canPreApply()
{
    return false;
}

std::string
patchTarget::get_name() const
{
...
Address
toAddressPatch::get_address() const
{
    return addr;
}

unsigned
toAddressPatch::get_size() const
{
    return 0;
}
...
regTracker_t*
codeGen::tracker() const
{
    return t_;
}

Emitter*
codeGen::codeEmitter() const
{
    return emitter_;
}

void
codeGen::beginTrackRegDefs()
{
    trackRegDefs_ = true;
#if defined(arch_x86) || defined(arch_x86_64)
