...
public:
    ...
    // also use NULL
    // typedef std::map<block_instance *, std::map<func_instance *, func_instance *> >
    // CallModMap; typedef std::map<func_instance *, func_instance *> FuncModMap;
    ...
    virtual bool readDataWord(const void* inOther, u_int amount, void* inSelf,
                              bool showError)                                   = 0;
    virtual bool readDataSpace(const void* inOther, u_int amount, void* inSelf,
                               bool showError)                                  = 0;
    virtual bool readTextWord(const void* inOther, u_int amount, void* inSelf)  = 0;
    virtual bool readTextSpace(const void* inOther, u_int amount, void* inSelf) = 0;

    virtual bool writeDataWord(void* inOther, u_int amount, const void* inSelf)  = 0;
    virtual bool writeDataSpace(void* inOther, u_int amount, const void* inSelf) = 0;
    virtual bool writeTextWord(void* inOther, u_int amount, const void* inSelf)  = 0;
    virtual bool writeTextSpace(void* inOther, u_int amount, const void* inSelf) = 0;
    ...
    bool findFuncsByAll(const std::string& funcname, std::vector<func_instance*>& res,
                        const std::string& libname = "");

    // Specific versions...
    bool findFuncsByPretty(const std::string& funcname, std::vector<func_instance*>& res,
                           const std::string& libname = "");
    bool findFuncsByMangled(const std::string& funcname, std::vector<func_instance*>& res,
                            const std::string& libname = "");

    bool findVarsByAll(const std::string& varname, std::vector<int_variable*>& res,
                       const std::string& libname = "");
    ...
    bool findFuncsByAddr(Address addr, std::set<func_instance*>& funcs,
                         bool includeReloc = false);
    ...
    void modifyCall(block_instance* callBlock, func_instance* newCallee,
                    func_instance* context = NULL);
    ...
    bool wrapFunction(func_instance* original, func_instance* wrapper,
    ...
    bool getDynamicCallSiteArgs(InstructionAPI::Instruction insn, Address addr,
                                std::vector<AstNodePtr>& args);

    // Default to "nope"
    virtual bool hasBeenBound(const SymtabAPI::relocationEntry&, func_instance*&, Address)
    {
        return false;
    }
    virtual bool bindPLTEntry(const SymtabAPI::relocationEntry& /*entry*/,
                              Address /*base_addr*/, func_instance* /*target_func*/,
                              Address /*target_addr*/)
    {
        return false;
    }
...
public:
    ...
    {
        assert(new_func_cb);
        return new_func_cb(this, f);
    }
    BPatch_point* newInstPointCB(Dyninst::PatchAPI::PatchFunction* f,
                                 Dyninst::PatchAPI::Point* pt, int type)
    {
        assert(new_instp_cb);
        return new_instp_cb(this, f, pt, type);
    }

    // Register callbacks from the higher level
    void registerFunctionCallback(
        BPatch_function* (*f)(AddressSpace* p, Dyninst::PatchAPI::PatchFunction* f))
    {
        new_func_cb = f;
    }
    ...
    void getRelocAddrs(Address orig, block_instance* block, func_instance* func,
                       std::list<Address>& relocs, bool getInstrumentationAddrs) const;

    bool                                       getAddrInfo(Address  relocAddr,  // input
                                                           Address& origAddr, std::vector<func_instance*>& origFuncs,
                                                           baseTramp*& baseTramp);
    ...
    std::map<func_instance*, std::vector<edgeStub>> getStubs(
        const std::list<block_instance*>& owBBIs,
...
protected:
    ...
    bool relocateInt(FuncSet::const_iterator begin, FuncSet::const_iterator end,
                     Address near);
...
protected:
    ...
    std::map<Address, std::map<func_instance*, std::set<DefensivePad>>>
                                                              forwardDefensiveMap_;
    ...
    // PatchAPI stuffs
public:
    Dyninst::PatchAPI::PatchMgrPtr mgr() const
    {
        assert(mgr_);
        return mgr_;
    }
...
extern int
heapItemCmpByAddr(const heapItem** A, const heapItem** B);
