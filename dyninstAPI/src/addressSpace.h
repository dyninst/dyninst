...
class AddressSpace : public InstructionSource {
 public:
  ...
  virtual bool readDataSpace(const void *inOther, u_int amount, void *inSelf,
                             bool showError) = 0;
  virtual bool readTextWord(const void *inOther, u_int amount,
                            void *inSelf) = 0;
  virtual bool readTextSpace(const void *inOther, u_int amount,
                             void *inSelf) = 0;

  virtual bool writeDataWord(void *inOther, u_int amount,
                             const void *inSelf) = 0;
  virtual bool writeDataSpace(void *inOther, u_int amount,
                              const void *inSelf) = 0;
  virtual bool writeTextWord(void *inOther, u_int amount,
                             const void *inSelf) = 0;
  virtual bool writeTextSpace(void *inOther, u_int amount,
                              const void *inSelf) = 0;
  ...
  virtual void *getPtrToData(const Address a) const {
    return getPtrToInstruction(a);
  }
  ...
  bool findVarsByAll(const std::string &varname, pdvector<int_variable *> &res,
                     const std::string &libname = "");
  ...
  void wrapFunctionPostPatch(func_instance *wrapped,
  ...
  // And this....
  typedef boost::shared_ptr<Dyninst::InstructionAPI::Instruction>
      InstructionPtr;
  bool getDynamicCallSiteArgs(InstructionPtr insn, Address addr,
 ...
 private:
  BPatch_function *(*new_func_cb)(AddressSpace *a,
                                  Dyninst::PatchAPI::PatchFunction *f);
  BPatch_point *(*new_instp_cb)(AddressSpace *a,
                                Dyninst::PatchAPI::PatchFunction *f,
 ...
 public:
  ...
  void registerFunctionCallback(BPatch_function *(*f)(
      AddressSpace *p, Dyninst::PatchAPI::PatchFunction *f)) {
    new_func_cb = f;
  };
  void registerInstPointCallback(
      BPatch_point *(*f)(AddressSpace *p, Dyninst::PatchAPI::PatchFunction *f,
                         Dyninst::PatchAPI::Point *ip, int type)) {
    new_instp_cb = f;
  }
 ...
 protected:
  ...
  IntervalTree<Address, std::pair<func_instance *, Address> >
      reverseDefensiveMap_;
  ...
  std::map<func_instance *, Dyninst::SymtabAPI::Symbol *>
      wrappedFunctionWorklist_;
