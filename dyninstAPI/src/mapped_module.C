...
void
mapped_module::addFunction(func_instance* func)
{
    // Just the everything vector... the by-name lists are
    // kept in the mapped_object and filtered if we do a lookup.
    if(std::find(everyUniqueFunction.begin(), everyUniqueFunction.end(), func) !=
       everyUniqueFunction.end())
        return;
    everyUniqueFunction.push_back(func);
}

void
mapped_module::addVariable(int_variable* var)
{
...
// We rely on the mapped_object for pretty much everything...
void
mapped_module::remove(func_instance* func)
{
...
mapped_object*
mapped_module::obj() const
{
    return obj_;
}

SymtabAPI::supportedLanguages
mapped_module::language() const
{
    return pmod()->language();
}

bool
mapped_module::findFuncVectorByMangled(const std::string&           funcname,
                                       std::vector<func_instance*>& funcs)
{
    ...
    const std::vector<func_instance*>* obj_funcs =
        obj()->findFuncVectorByMangled(funcname);
    if(!obj_funcs)
    {
        return false;
    }
...
bool
mapped_module::findFuncVectorByPretty(const std::string&           funcname,
                                      std::vector<func_instance*>& funcs)
{
    // For efficiency sake, we grab the image vector and strip out the
    // functions we want.
    ...
    const std::vector<func_instance*>* obj_funcs =
        obj()->findFuncVectorByPretty(funcname);
    if(!obj_funcs)
        return false;
...
pdmodule*
mapped_module::pmod() const
{
    return internal_mod_;
}

void
mapped_module::dumpMangled(std::string prefix) const
{
...
mapped_module::mapped_module(mapped_object* obj, pdmodule* pdmod)
: internal_mod_(pdmod)
, obj_(obj)
{}

mapped_module*
mapped_module::createMappedModule(mapped_object* obj, pdmodule* pdmod)
{
    assert(obj);
    assert(pdmod);
...
std::string
mapped_module::processDirectories(const std::string& fn) const
{
    ...
    if(!strstr(fn.c_str(), "/./") && !strstr(fn.c_str(), "/../"))
        return fn;
...
AddressSpace*
mapped_module::proc() const
{
    return obj()->proc();
}

int_variable*
mapped_module::createVariable(std::string name, Address addr, int size)
{
    Address base   = obj()->dataBase();
    Offset  offset = addr - base;

    image_variable* img_var =
        pmod()->imExec()->createImageVariable(offset, name, size, pmod());
...
bool
mapped_module::findFuncsByAddr(const Address addr, std::set<func_instance*>& funcs)
{
    std::set<func_instance*> allFuncs;
    unsigned                 size = funcs.size();
    if(!obj()->findFuncsByAddr(addr, allFuncs))
        return false;
...
void
mapped_module::getAnalyzedCodePages(std::set<Address>& pages)
{
    assert(proc()->proc());
    ...
    for(unsigned fidx = 0; fidx < funcs.size(); fidx++)
    {
        ...
        const PatchFunction::Blockset&          blocks = funcs[fidx]->blocks();
        ...
        for(bIter = blocks.begin(); bIter != blocks.end(); bIter++)
        {
            ...
            while(page < bEnd)
            {  // account for blocks spanning multiple pages
                pages.insert(page);
                page += pageSize;
            }
        }
