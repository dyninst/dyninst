...
// $Id: mapped_module.C,v 1.26 2008/01/23 20:24:01 giri Exp $
...
//  Symtab *xcoffObject = fileOnDisk->getObject();
  ...
  void *syms_void = NULL;
  xcoffObject->get_stab_info( stabstr, nstabs, syms_void, stringpool );
...
BOOL CALLBACK add_line_info(SRCCODEINFO *srcinfo, void *param)
{
  ...
  if (last_srcinfo && srcinfo) {
    ...
    li->addLine(last_srcinfo->FileName, last_srcinfo->LineNumber, 0 /* no col info for windows*/,
...
LineInformation *mapped_module::getLineInformation() {
  ...
  LineInformation *lineInfo = pmod()->getLineInformation();
  if(!lineInfo)
  {
      lineInfoValid_ = true;
      return &lineInfo_;
  }    
  LineInformation::const_iterator iter = lineInfo->begin();
  for(;iter!=lineInfo->end();iter++)
  {
      const std::pair<Offset, Offset> range = iter->first;
      LineNoTuple line = iter->second;
      lineInfo_.addLine(line.first, line.second, line.column, obj()->codeBase()+range.first, obj()->codeBase()+range.second);
  }
  lineInfoValid_ = true;
  return &lineInfo_;
}
