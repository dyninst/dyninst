...
const pdvector<func_instance *> &mapped_module::getAllFunctions() {
  ...
  if (everyUniqueFunction.size() == pdfuncs.size()) return everyUniqueFunction;
  ...
  if (everyUniqueFunction.size() < pdfuncs.size() ||
    ...
    fprintf(stderr, "[%s:%u] EUF.size %u != pdfuncs.size %u\n", FILE__,
            __LINE__, (unsigned)everyUniqueFunction.size(),
...
const pdvector<int_variable *> &mapped_module::getAllVariables() {
  ...
  if (everyUniqueVariable.size() == img_vars.size()) return everyUniqueVariable;
...
const string &mapped_module::fileName() const { return pmod()->fileName(); }

const string &mapped_module::fullName() const { return pmod()->fullName(); }

mapped_object *mapped_module::obj() const { return obj_; }
...
bool mapped_module::findFuncVectorByMangled(const std::string &funcname,
  ...
  const pdvector<func_instance *> *obj_funcs =
  ...
  for (unsigned i = 0; i < obj_funcs->size(); i++) {
    if ((*obj_funcs)[i]->mod() == this) funcs.push_back((*obj_funcs)[i]);
  }
...
std::string mapped_module::processDirectories(const std::string &fn) const {
  ...
  if (!strstr(fn.c_str(), "/./") && !strstr(fn.c_str(), "/../")) return fn;
...
int_variable *mapped_module::createVariable(std::string name, Address addr,
                                            int size) {
