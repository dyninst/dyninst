...
bool
BPatch_module::getSourceObj(BPatch_Vector<BPatch_sourceObj*>& vect)
{
    if(!mod)
        return false;
...
bool
BPatch_module::parseTypesIfNecessary()
{
    if(moduleTypes != NULL)
        return false;
...
bool
BPatch_module::getProcedures(BPatch_Vector<BPatch_function*>& funcs,
                            ...
                            bool incUninstrumentable, bool dont_use_regex)
{
                ...
                piter != func->pretty_names_end(); ++piter)
            {
                ...
                if(0 == (err_code = regexec(&comp_pat, pName.c_str(), 1, NULL, 0)))
                {
                    if(func->isInstrumentable() || incUninstrumentable)
                    {
                        ...
                        //	   BPatch_function *foo = proc->findOrCreateBPFunc(func,
                        // NULL);
                        funcs.push_back(foo);
                    }
                    found_match = true;
    ...
    {
        char msg[1024];
        sprintf(msg, "%s[%d]:  Module %s: unable to find function %s", __FILE__, __LINE__,
                mod->fileName().c_str(), name);
        BPatch_reportError(BPatchSerious, 100, msg);
    }
    return &funcs;
}
...
BPatch_module::findFunctionByAddress(void* addr, BPatch_Vector<BPatch_function*>& funcs,
                                     bool notify_on_failure, bool incUninstrumentable)
{
    if(!isValid())
    {
        if(notify_on_failure)
...
BPatch_function*
BPatch_module::findFunctionByMangled(const char* mangled_name, bool incUninstrumentable)
{
    if(!isValid())
        return NULL;
    ...
    if(!mod->findFuncVectorByMangled(mangled_str, int_funcs))
        return NULL;

    if(int_funcs.size() > 1)
    {
        fprintf(stderr,
                "%s[%d]: Warning: found multiple name matches for %s, returning first\n",
...
bool
BPatch_module::remove(BPatch_function* bpfunc)
{
...
bool
BPatch_module::remove(instPoint* point)
{
...
BPatch_variableExpr*
BPatch_module::findVariable(const char* name)
{
    parseTypesIfNecessary();
...
bool
BPatch_module::getVariables(BPatch_Vector<BPatch_variableExpr*>& vars)
{
    if(!isValid())
        return false;
    if(!full_var_parse)
...
bool
BPatch_module::getSourceLines(unsigned long addr, BPatch_Vector<BPatch_statement>& lines)
{
    if(!isValid())
    {
...
bool
BPatch_module::getStatements(BPatch_Vector<BPatch_statement>& statements)
{
...
bool
BPatch_module::getAddressRanges(const char* fileName, unsigned int lineNo,
...
Dyninst::SymtabAPI::convert(const BPatch_module* m)
{
    if(!m->mod)
        return NULL;
...
void
BPatch_module::setDefaultNamespacePrefix(char* /*name*/)
{}

bool
BPatch_module::isSystemLib()
{
    if(!mod)
        return false;
...
void
BPatch_module::enableDefensiveMode(bool on)
{
