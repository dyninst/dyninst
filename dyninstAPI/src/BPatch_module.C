...
BPatch_module::getProceduresInt(bool incUninstrumentable)
{
   if (!isValid())
      return NULL;
   // return cached version if it exists and isn't missing new function entries
   if (retfuncs) {
       if (retfuncs->size() == (unsigned)mod->getFuncVectorSize()) {
           return retfuncs;
       } else {
           retfuncs->clear();
       }
...
void BPatch_module::parseTypes() 
{
   ...
   pair.module = this;
   pair.base_addr = mod->obj()->codeAbs();
   pair.mmod = mod;
