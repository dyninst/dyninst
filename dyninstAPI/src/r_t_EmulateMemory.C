...
bool MemEmulatorTransformer::processBlock(BlockList::iterator &iter) {
  ...
  for (ElementList::iterator e_iter = elements.begin();
    ...
    */
    ...
    Element::Ptr replacement = createReplacement(reloc, func);
...
Element::Ptr MemEmulatorTransformer::createReplacement(CopyInsn::Ptr reloc,
						       int_function *func) {
  ...
  if (!point) return Element::Ptr();
  
  // Replace this instruction with a MemEmulator
  Element::Ptr memE = MemEmulator::create(reloc->insn(),
					  reloc->addr(),
					  point);
  return memE;
}
