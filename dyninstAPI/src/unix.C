...
#endif
{
...
#ifdef BPATCH_LIBRARY
    string relinked_progname = file + ".dyninst";
#else
    string relinked_progname = file + ".paradyn";
#endif
	...
	// First, we only care if the (possibly) relinked
	// program is newer than the original. stat()
	struct stat orig_file_stat;
	struct stat link_file_stat;
	if (stat(file.string_of(), &orig_file_stat))
	  {
	    perror ("Problem opening program");
	    status = 1;
	  }
	else 
	  if (stat(relinked_progname.string_of(), &link_file_stat))
	    {
	      cerr << "Creating relinked program " << relinked_progname << endl;	      status = 2;
	    }
	  else
	    if (orig_file_stat.st_mtime > link_file_stat.st_mtime)
	      {
		cerr << "Original program modified. Relinking now." << endl;
		status = 3;
	      }
	    else
	      if (seeIfRTLinked(relinked_progname))
		{
	...
	switch (status)
	  ...
	  case 1:     // Couldn't open original program
	    break;
	  case 2:     // Relink on general principles
	  case 3:
	    ...
	    // First string: relinked_progname
		...
		return false;
	      ...
	      char command_line[2*strlen(relinked_progname.string_of())
			       +strlen(link_string)+strlen(dyninstName)];
	      sprintf(command_line, 
		      link_string, 
		      relinked_progname.string_of(), 
		      file.string_of(), 
		      dyninstName);
	      // Relink the program
	      system(command_line);
	    }	
	  case 4:     // Don't need to relink
	    file = relinked_progname;
	    break;
	  default:
	    break;
	  }
