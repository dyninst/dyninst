...
// $Id: unix.C,v 1.42 2000/08/18 17:55:07 bernat Exp $
...
#endif
{
    ...
    // Since we don't dynamically load the RT library, we call ld to relink it in for
    // us. Do this before the fork, so if it fails we can still proceed (in some
    // fashion). 
    
    // First question: does the specified object file exist? If not,
    // then we're getting into a lot of trouble over nothing.
    
    int file_desc;
    string old_file = file;
    file_desc = open(file.string_of(), O_RDONLY);
    if (file_desc != -1)
      {
	...
	Object *exec_file = new Object(file.string_of(), pd_log_perror);
	Symbol placeholder;
	cerr << "Done" << endl;
	if (exec_file->get_symbol(string("DYNINSTversion"), placeholder))
	  cerr << "Library found. Continuing." << endl;
	else
	  {
	    cerr << "Runtime library not found. Linking now." << endl;
	    // Put the relinked binary in /tmp under a unique name (generated by mktemp)
	    // Command line: ld -o <unique name> programname runtime
	    //    -bpT:0x10000000 -bpD:0x20000000 -bnso -btextro -bexpall -lc
	    //    -bI:/lib/syscalls.exp
	    // How's that for a link line?
	    char *unique_progname = NULL;
	    char unique_progname_str[strlen(file.string_of())+6+1];
	    unique_progname = (char *)unique_progname_str;
	    sprintf(unique_progname,
		    "%sXXXXXX",
		    file.string_of());
	    unique_progname = mktemp(unique_progname);
	    // Paranoia... try to open the new pathname to be sure we
	    // have permissions (and if not... what do we do?)
	    file_desc = open(unique_progname, O_RDONLY | O_CREAT);
	    if (file_desc == -1)
	      perror("Error creating relinked copy of program");
	    else
	      {
		fprintf(stderr, "New file pathname: %s\n", unique_progname);
		
		// First string: unique_progname
		...
		// Mighty big chunk 'o space
		char command_line[2*strlen(unique_progname)+strlen(link_string)+strlen(dyninstName)];
		sprintf(command_line, link_string, unique_progname, file.string_of(), dyninstName);
		// Relink the program
		system(command_line);
		// Check to see if we can open the result
		int uniq_file_desc;
		uniq_file_desc = open(unique_progname, O_RDONLY);
		if (uniq_file_desc == -1)
		  {
		    string msg = string("Problem creating relinked version of program " + file
					+ " as temporary file " + string(unique_progname));
		    showErrorCallback(101, msg);
		    return false;
		  }
		close(uniq_file_desc);
		
		// Overwrite file with the new string value, since that's what
		// we want to exec, parse, etc.
		file = string(unique_progname);
	      }
	  }
	delete(exec_file);
      }
