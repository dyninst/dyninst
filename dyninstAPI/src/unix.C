...
bool
PCProcess::hideDebugger()
{
    return false;
}

bool
OS::executableExists(const std::string& file)
{
...
void
OS::get_sigaction_names(std::vector<std::string>& names)
{
...
std::string
PCProcess::createExecPath(const std::string& file, const std::string& dir)
{
    std::string ret = file;
    if(dir.length() > 0)
    {
...
bool
PCProcess::multithread_capable(bool ignoreIfMtNotSet)
{
    if(mt_cache_result_ != not_cached)
    {
        if(mt_cache_result_ == cached_mt_true)
            return true;
        else
            return false;
    }
 ...
 **/
static void
findThreadFuncs(PCProcess* p, std::string func, std::vector<func_instance*>& result)
{
...
bool
PCEventHandler::shouldStopForSignal(int signal)
{
    if(signal == SIGSTOP)
        return true;
    return false;
...
bool
PCEventHandler::isCrashSignal(int signal)
{
    switch(signal)
    {
        case SIGBUS:
...
bool
PCEventHandler::isKillSignal(int signal)
{
    switch(signal)
    {
        case SIGKILL:
...
bool
PCEventHandler::isValidRTSignal(int signal, RTBreakpointVal breakpointVal, Address arg1,
                                int status)
{
    ...
    else if(signal == DYNINST_BREAKPOINT_SIGNUM)
    {
        ...
        {
            if((status != DSE_forkExit) || (arg1 != 0))
                return true;

            proccontrol_printf("%s[%d]: child received signal %d\n", FILE__, __LINE__,
                               DYNINST_BREAKPOINT_SIGNUM);
        }
        else
        {
            proccontrol_printf("%s[%d]: mismatch in signal for breakpoint type\n", FILE__,
...
void
PCProcess::changeMemoryProtections(Address addr, size_t size, PCMemPerm rights,
                                   bool setShadow)
{
    (void) addr;
    (void) size;
...
bool
PCProcess::setMemoryAccessRights(Address start, size_t size, PCMemPerm rights)
{
...
bool
PCProcess::getMemoryAccessRights(Address start, PCMemPerm& rights)
{
...
void
PCProcess::redirectFds(int stdin_fd, int stdout_fd, int stderr_fd,
                       std::map<int, int>& fds)
{
    if(stdin_fd != 0)
        fds.insert(std::make_pair(stdin_fd, 0));
    if(stdout_fd != 1)
        fds.insert(std::make_pair(stdout_fd, 1));
    if(stderr_fd != 2)
        fds.insert(std::make_pair(stderr_fd, 2));
}

bool
PCProcess::getDyninstRTLibName()
{
    ...
    for(auto i = rt_paths.begin(); i != rt_paths.end(); ++i)
    {
...
bool
PCProcess::setEnvPreload(std::vector<std::string>& envp, std::string fileName)
{
    ...
    if(envp.size())
    {
        ...
        for(std::vector<std::string>::iterator i = envp.begin(); i != envp.end(); ++i)
        {
            if((*i) == var_name)
            {
                ldPreloadVal = i;
        ...
        if(ldPreloadVal == envp.end())
        {
            ...
            std::string ld_preload =
                std::string(var_name) + std::string("=") + rt_lib_name;
        ...
        else
        {
            ...
            std::string ld_preload = *ldPreloadVal + std::string(":") + rt_lib_name;
            startup_printf("LD_PRELOAD=%s\n", ld_preload.c_str());
    ...
    else
    {
        ...
        if(ld_preload_orig)
        {
            ...
            ld_preload = std::string(var_name) + std::string("=") +
                         std::string(ld_preload_orig) + std::string(":") + rt_lib_name;
        }
        else
        {
            // Define a new var
            ld_preload = std::string(var_name) + std::string("=") + rt_lib_name;
        }
        envp.push_back(ld_preload);
    }
...
bool
PCProcess::getExecFileDescriptor(string filename, bool, fileDescriptor& desc)
{
    Address base = 0;
...
PCProcess::hasPassedMain()
{
    ...
    {
        startup_printf("{%s:%d] - there is a .opd section. The entry offset points to "
                       "the pointer to the real entry\n",
                       FILE__, __LINE__);
    ...
    {
        startup_printf(
            "{%s:%d] - there is no .opd section. The entry offset is the entry\n", FILE__,
            __LINE__);
    }
    ...
    startup_printf("[%s:%d] - hasPassedMain returning %d (%lx %lx)\n", FILE__, __LINE__,
                   (int) result, entry_addr, current_pc);

    return result;
}

bool
PCProcess::startDebugger()
{
                                    ...
                                    std::map<std::string, BinaryEdit*>& retMap)
{
    ...
    auto is_compatible = [this](std::string const& path, std::string const& member) {
        auto temp = std::unique_ptr<BinaryEdit>{ BinaryEdit::openFile(
            path, mgr(), patcher(), member) };
        ...
        if(Archive::openArchive(library, path))
        {
            ...
            if(lib->getAllMembers(members))
            {
                ...
                if(retMap.size() > 0)
                {
                    ...
                    // So we tried loading "libc.a", and got back a swarm of individual
                    // members. Just return the first thing...
                    return retMap.begin()->second->getMappedObject();
                }
            }
            startup_printf("[%s:%d] - Failed to find archive members in '%s' for static "
                           "executable '%s'\n",
        ...
        else
        {
            ...
            if(Symtab::openFile(singleObject, path))
            {
                ...
                if(auto temp = is_compatible(path, {}))
                {
                    ...
                    {
                        startup_printf("%s[%d]: cannot load dynamic object(%s) when "
                                       "rewriting a static binary\n",
                                       FILE__, __LINE__, path.c_str());
                        std::string msg{
                            "Cannot load a dynamic object when rewriting a static binary"
                        };
...
func_instance*
block_instance::callee(std::string const& target_name)
{
...
BinaryEdit::makeInitAndFiniIfNeeded()
{
    ...
    if(linkedFile->isStaticBinary() || linkedFile->getObjectType() == obj_RelocatableFile)
    {
        return;
    }
...
#    endif  // defined(arch_x86) || defined(arch_x86_64)
            linkedFile->addRegion(highWaterMark_, (void*) (emptyFunction), emptyFuncSize,
                                  ".init.dyninst", Dyninst::SymtabAPI::Region::RT_TEXT,
                                  true);
        ...
        Symbol* initSym = new Symbol("_init", Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
                                     Symbol::SV_DEFAULT, initsec->getMemOffset(),
                                     linkedFile->getDefaultModule(), initsec, UINT_MAX);
...
#    elif defined(arch_aarch64)
            static unsigned char empty[] = { 0xfd, 0x7b, 0xbf, 0xa9, 0xfd, 0x03,
                                             0x00, 0x91, 0xfd, 0x7b, 0xc1, 0xa8,
                                             0xc0, 0x03, 0x5f, 0xd6 };
            emptyFunction                = empty;
            emptyFuncSize                = 16;
#    endif  // defined(arch_x86) || defined(arch_x86_64)
            ...
            linkedFile->addRegion(highWaterMark_, (void*) (emptyFunction), emptyFuncSize,
                                  ".fini.dyninst", Dyninst::SymtabAPI::Region::RT_TEXT,
                                  true);
            highWaterMark_ += emptyFuncSize;
            lowWaterMark_ += emptyFuncSize;
        ...
        Symbol* finiSym = new Symbol("_fini", Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
                                     Symbol::SV_DEFAULT, finisec->getMemOffset(),
                                     linkedFile->getDefaultModule(), finisec, UINT_MAX);
