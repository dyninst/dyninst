...
bool PCProcess::hideDebugger() { return false; }
...
std::string PCProcess::createExecPath(const std::string &file,
...
static void findThreadFuncs(PCProcess *p, std::string func,
  ...
  if (lpthread) found = lpthread->findFuncVectorByMangled(func);
  if (found) {
    result = *found;
    return;
  }
  mapped_object *lc = p->findObject("libc.so*", true);
  if (lc) found = lc->findFuncVectorByMangled(func);
  if (found) {
    result = *found;
    return;
  }
...
bool PCProcess::instrumentMTFuncs() {
  ...
  if (!res) {
    fprintf(stderr,
            "[%s:%d] - Couldn't find any dyn_pthread_self, expected 1\n",
            __FILE__, __LINE__);
  }
...
bool PCProcess::setEnvPreload(std::vector<std::string> &envp,
                              std::string fileName) {
  ...
  if (rt_lib_name == NULL) {
    showErrorCallback(
        ERROR_CODE,
        std::string("setEnvPreload: DYNINSTAPI_RT_LIB is undefined"));
  ...
  if (envp.size()) {
    ...
    for (std::vector<std::string>::iterator i = envp.begin(); i != envp.end();
         ++i) {
      if ((*i) == var_name) {
        ldPreloadVal = i;
        break;
    ...
    if (ldPreloadVal == envp.end()) {
      ...
      std::string ld_preload =
          std::string(var_name) + std::string("=") + std::string(rt_lib_name);
      startup_printf("LD_PRELOAD=%s\n", ld_preload.c_str());
      envp.push_back(ld_preload);
    } else {
      // Found, modify envs in-place
      std::string ld_preload =
          *ldPreloadVal + std::string(":") + std::string(rt_lib_name);
      startup_printf("LD_PRELOAD=%s\n", ld_preload.c_str());
...
bool PCProcess::getExecFileDescriptor(string filename, bool,
                                      fileDescriptor &desc) {
  Address base = 0;
...
bool PCProcess::hasPassedMain() {
  ...
  startup_printf("[%s:%u] - hasPassedMain returning %d (%lx %lx)\n", FILE__,
                 __LINE__, (int)result, entry_addr, current_pc);
...
mapped_object *BinaryEdit::openResolvedLibraryName(
    std::string filename, std::map<std::string, BinaryEdit *> &retMap) {
        ...
        if (Archive::openArchive(library, *pathIter)) {
          ...
          if (library->getAllMembers(members)) {
            ...
            for (member_it = members.begin(); member_it != members.end();
                 ++member_it) {
              BinaryEdit *temp = BinaryEdit::openFile(
                  *pathIter, mgr(), patcher(), (*member_it)->memberName());

              if (temp && temp->getAddressWidth() == getAddressWidth()) {
                std::string mapName =
                    *pathIter + string(":") + (*member_it)->memberName();
            ...
            if (retMap.size() > 0) {
              ...
              // So we tried loading "libc.a", and got back a swarm of
              // individual members.
              // Lovely.
          ...
          if (temp && temp->getAddressWidth() == getAddressWidth()) {
            if (singleObject->getObjectType() == obj_SharedLib ||
              ...
              startup_printf(
                  "%s[%d]: cannot load dynamic object(%s) when rewriting a "
                  "static binary\n",
                  FILE__, __LINE__, pathIter->c_str());
              std::string msg = std::string(
                  "Cannot load a dynamic object when rewriting a static "
                  "binary");
  ...
  startup_printf("[%s:%u] - Creation error opening %s\n", FILE__, __LINE__,
                 filename.c_str());
  // If the only thing we could find was a dynamic lib for a static executable,
  // we can reach here; caller should handle this.
  return NULL;
}
...
func_instance *block_instance::callee() {
  ...
  if (pltFuncs.find(target_addr) != pltFuncs.end()) {
    ...
    if (dproc) {
      ...
      for (i = bedit->getSiblings().begin(); i != bedit->getSiblings().end();
           i++) {
...
void BinaryEdit::makeInitAndFiniIfNeeded() {
  ...
  if (!foundInit) {
    ...
    if (!initOffset || !initsec) {
      ...
      linkedFile->addRegion(highWaterMark_, (void *)(emptyFunction),
      ...
      startup_printf(
          "%s[%d]: creating .init.dyninst region, region addr 0x%lx\n", FILE__,
          __LINE__, initsec->getMemOffset());
    }
    startup_printf("%s[%d]: ADDING _init at 0x%lx\n", FILE__, __LINE__,
                   initsec->getMemOffset());
    Symbol *initSym =
        new Symbol("_init", Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
  ...
  if (!foundFini) {
    ...
    if (!finiOffset || !finisec) {
      ...
      startup_printf(
          "%s[%d]: creating .fini.dyninst region, region addr 0x%lx\n", FILE__,
          __LINE__, finisec->getMemOffset());
    }
    startup_printf("%s[%d]: ADDING _fini at 0x%lx\n", FILE__, __LINE__,
                   finisec->getMemOffset());
    Symbol *finiSym =
        new Symbol("_fini", Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
...
Address PCProcess::setAOutLoadAddress(fileDescriptor &desc) {
  ...
  result = stat(pcProc_->libraries().getExecutable()->getAbsoluteName().c_str(),
                &aout);
  if (result == -1) {
