...
#include <stdlib.h>
#include "dyninstAPI/h/codegen-amdgpu.h"
...
void *amdgpuCodeGen::insnPtr(CodeGen &gen) {
	return gen.cur_ptr();
}

#if 0
// Same as above, but increment offset to point at the next insn.
codeBuf_t *amdgpuCodeGen::ptrAndInc(CodeGen &gen) {
...
void amdgpuCodeGen::generate(CodeGen &gen, instruction &insn) {
	gen.copy(insn.raw_, insn.size());
}

void amdgpuCodeGen::generate(CodeGen &gen, instruction &insn, unsigned position) {
	gen.insert(insn.raw_, insn.size() , position);
}

void amdgpuCodeGen::generateIllegal(CodeGen &gen) { // instP.h
	instruction insn;
	generate(gen,insn);
}

void amdgpuCodeGen::generateTrap(CodeGen &) {
...
void amdgpuCodeGen::generate_SOPC( CodeGen & gen,  uint32_t OP, uint32_t SSRC1, uint32_t SSRC0  , bool useImm ){
	...
	if(useImm){
		cmd  = ( cmd | (OP<< 16) | (SSRC1 << 8) | 0xff);
		memcpy( insn.raw_  ,&cmd,  4 );
		memcpy(insn.raw_+4, &SSRC0, 4);
	}else{
...
void amdgpuCodeGen::generate_SOP2( CodeGen & gen,  uint32_t OP, uint32_t SDST, uint32_t SSRC1, uint32_t SSRC0  , bool useImm ){
	uint32_t insn_size = useImm ? 8 : 4;
	instruction insn(insn_size);
	...
	if(useImm){
		...
		memcpy(insn.raw_ , &cmd, 4);
		printf("SOP2 instruction generated, value = %lx\n",cmd);
	}
	generate(gen,insn);
}
...
/*
   void amdgpuCodeGen::generateBranch(CodeGen &gen, long disp, bool link) {
...
amdgpuCodeGen::generate(gen, insn);
}

void amdgpuCodeGen::generateBranch(CodeGen &gen, Address from, Address to, bool link) {
long disp = (to - from);
...
void amdgpuCodeGen::generateCall(CodeGen &gen, Address from, Address to) {
generateBranch(gen, from, to, true);
}

void amdgpuCodeGen::generateLongBranch(CodeGen &gen,
Address from,
Address to,
bool isCall) 
{
...
amdgpuCodeGen::generate(gen, branchInsn);
};
...
void amdgpuCodeGen::generateBranchViaTrap(CodeGen &gen, Address from, Address to, bool isCall) {
	long disp = to - from;
	if (labs(disp) <= MAX_BRANCH_OFFSET) {
	...
	if (gen.addrSpace()) {
		...
		amdgpuCodeGen::generateTrap(gen);
	} else {
...
void amdgpuCodeGen::generateConditionalBranch(CodeGen& gen, Address to, unsigned opcode, bool s)
{
	instruction insn;
	insn.clear();
...
void amdgpuCodeGen::generateAddSubShifted(
		CodeGen &gen, amdgpuCodeGen::ArithOp op, int shift, int imm6, Register rm, 
...
void amdgpuCodeGen::generateAddSubImmediate(
		CodeGen &gen, amdgpuCodeGen::ArithOp op, int shift, int imm12, Register rn, Register rd, bool is64bit) 
{
	instruction insn;
	insn.clear();
...
void amdgpuCodeGen::generateMul(CodeGen &gen, Register rm, Register rn, Register rd, bool is64bit) {
	instruction insn;
	insn.clear();
...
void amdgpuCodeGen::generateDiv(
...
void amdgpuCodeGen::generateBitwiseOpShifted(
		CodeGen &gen, amdgpuCodeGen::BitwiseOp op, int shift, Register rm, int imm6, 
		Register rn, Register rd, bool is64bit) 
{
	instruction insn;
	insn.clear();
	...
	int opcode;
	switch(op) {
		case amdgpuCodeGen::And: opcode = ANDShiftOp;
								 break;
		case amdgpuCodeGen::Or: opcode = ORRShiftOp;
								break;
		case amdgpuCodeGen::Eor: opcode = EORShiftOp;
								 break;
		default:
								 assert(!"amdgpuCodeGen::generateBitwiseOpShifted op is not And, Or or Eor");
	}
...
void amdgpuCodeGen::generateLoadReg(CodeGen &, Register,
		Register, Register)
{
	assert(0);
...
void amdgpuCodeGen::generateStoreReg(CodeGen &, Register,
		Register, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
...
void amdgpuCodeGen::generateLoadReg64(CodeGen &, Register,
		Register, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
...
void amdgpuCodeGen::generateStoreReg64(CodeGen &, Register,
		Register, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
...
void amdgpuCodeGen::generateMove(CodeGen &gen, int imm16, int shift, Register rd, MoveOp movOp)
{
	instruction insn;
	insn.clear();
...
void amdgpuCodeGen::generateMove(
		CodeGen &gen, Register rd, Register rm, bool is64bit)
{
	amdgpuCodeGen::generateBitwiseOpShifted(gen, amdgpuCodeGen::Or, 0, rm, 0, 0x1f, rd, is64bit);  
}

void amdgpuCodeGen::generateMoveSP(CodeGen &gen, Register rn, Register rd, bool is64bit) {
	instruction insn;
	insn.clear();
...
Register amdgpuCodeGen::moveValueToReg(CodeGen &gen, long int val, std::vector<Register> *exclude) {
	Register scratchReg;
	if(exclude)
	...
	template <typename T>
void amdgpuCodeGen::loadImmIntoReg(CodeGen &gen, Register rt, T value)
{
	assert(value >= 0);

	amdgpuCodeGen::generateMove(gen, (value & 0xFFFF), 0, rt, MovOp_MOVZ);
	if(value > 0xFFFF)
		amdgpuCodeGen::generateMove(gen, ((value >> 16) & 0xFFFF), 0x1, rt, MovOp_MOVK);
	if(value > 0xFFFFFFFF)
		amdgpuCodeGen::generateMove(gen, ((value >> 32) & 0xFFFF), 0x2, rt, MovOp_MOVK);
	if(value > 0xFFFFFFFFFFFF)
		amdgpuCodeGen::generateMove(gen, ((value >> 48) & 0xFFFF), 0x3, rt, MovOp_MOVK);
}
...
void amdgpuCodeGen::generateMemAccess(CodeGen &gen, LoadStore accType,
...
void amdgpuCodeGen::generateMemAccessFP(CodeGen &gen, LoadStore accType,
...
void amdgpuCodeGen::generateLShift(CodeGen &, Register, int, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
}
...
void amdgpuCodeGen::generateRShift(CodeGen &, Register, int, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
}
...
// sld ra, rs, rb
void amdgpuCodeGen::generateLShift64(CodeGen &, Register, int, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
}
...
// srd ra, rs, rb
void amdgpuCodeGen::generateRShift64(CodeGen &, Register, int, Register)
{
	assert(0);
	//not implemented
}
...
//
void amdgpuCodeGen::generateNOOP(CodeGen &gen, unsigned size) {
...
void amdgpuCodeGen::generateRelOp(CodeGen &, int, int, Register,
		Register, Register)
{
	assert(0);
	//#warning "This function is not implemented yet!"
...
void amdgpuCodeGen::saveRegister(CodeGen &gen, Register r, int sp_offset, IndexMode im)
{
	generateMemAccess(gen, Store, r, REG_SP, sp_offset, 8, im);
}


void amdgpuCodeGen::restoreRegister(CodeGen &gen, Register r, int sp_offset, IndexMode im)
{
...
// stores.
void amdgpuCodeGen::loadPartialImmIntoReg(CodeGen &, Register, long)
{
	assert(0);
	//#warning "This function is not implemented yet!"
}

int amdgpuCodeGen::createStackFrame(CodeGen &, int, std::vector<Register>& freeReg, std::vector<Register>&){
	assert(0);
	//#warning "This function is not implemented yet!"
	return freeReg.size();
}

void amdgpuCodeGen::removeStackFrame(CodeGen &) {
	assert(0);
	//#warning "This function is not implemented yet!"
}

bool amdgpuCodeGen::generateMem(CodeGen &,
		instruction&,
		Address,
		Address,
		Register,
	...
	void amdgpuCodeGen::generateMoveFromLR(CodeGen &, Register) {
		assert(0);
		//#warning "This function is not implemented yet!"
	}

void amdgpuCodeGen::generateMoveToLR(CodeGen &, Register) {
	assert(0);
	//#warning "This function is not implemented yet!"
}
void amdgpuCodeGen::generateMoveToCR(CodeGen &, Register) {
	assert(0);
	//#warning "This function is not implemented yet!"
}

bool amdgpuCodeGen::modifyJump(Address target,
 ...
 */
#if 0
bool amdgpuCodeGen::modifyJcc(Address target,
		NS_aarch64::instruction &insn,
		CodeGen &gen) {
	long disp = target - gen.currAddr();
			...
			(isTB && labs(disp) > MAX_TBRANCH_OFFSET))
	{
		...
		amdgpuCodeGen::generateBranch(gen, newFrom, target);
	} 
	else
	{
...
bool amdgpuCodeGen::modifyCall(Address target,
		NS_aarch64::instruction &insn,
		CodeGen &gen) {
...
bool amdgpuCodeGen::modifyData(Address target,
		NS_aarch64::instruction &insn,
		CodeGen &gen) 
{
	int raw = insn.asInt();
