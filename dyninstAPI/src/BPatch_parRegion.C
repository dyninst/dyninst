...
void
BPatch_parRegion::printDetails()
{
    parReg->printDetails();
}
BPatch_Vector<BPatch_instruction*>*
BPatch_parRegion::getInstructions(void)
{
    return NULL;
}

bool
BPatch_parRegion::getInstructions(std::vector<InstructionAPI::Instruction>& insns)
{
    using namespace InstructionAPI;
    const unsigned char* buffer =
        (const unsigned char*) (lowlevel_region()->intFunc()->proc()->getPtrToInstruction(
            getStartAddress()));

    InstructionDecoder d(buffer, size(), lowlevel_region()->intFunc()->proc()->getArch());
...
int
BPatch_parRegion::getClause(const char* key)
{
    return parReg->getClause(key);
}

int
BPatch_parRegion::replaceOMPParameter(const char* key, int value)
{
