...
BPatch_parRegion::BPatch_parRegion(int_parRegion* _parReg,
...
void BPatch_parRegion::printDetails() { parReg->printDetails(); }
BPatch_Vector<BPatch_instruction*>* BPatch_parRegion::getInstructions(void) {
  return NULL;
}

bool BPatch_parRegion::getInstructions(
    std::vector<InstructionAPI::Instruction::Ptr>& insns) {
  using namespace InstructionAPI;
  const unsigned char* buffer =
      (const unsigned char*)(lowlevel_region()
                                 ->intFunc()
                                 ->proc()
                                 ->getPtrToInstruction(getStartAddress()));
