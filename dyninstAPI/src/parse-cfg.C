...
parse_func::parse_func(SymtabAPI::Function* func, pdmodule* m, image* i, CodeObject* obj,
                       CodeRegion* reg, InstructionSource* isrc, FuncSource src)
...
bool
parse_func::addSymTabName(std::string name, bool isPrimary)
{
...
bool
parse_func::addPrettyName(std::string name, bool isPrimary)
{
...
bool
parse_func::addTypedName(std::string name, bool isPrimary)
{
...
void
parse_func::changeModule(pdmodule* mod)
{
...
bool
parse_func::isInstrumentableByFunctionName()
{
...
const std::vector<image_parRegion*>&
parse_func::parRegions()
{
    if(!parsed())
        image_->analyzeIfNeeded();
 ...
 */
parse_block::parse_block(CodeObject* obj, CodeRegion* reg, Address addr)
: Block(obj, reg, addr)
...
parse_block::parse_block(parse_func* func, CodeRegion* reg, Address firstOffset)
...
void*
parse_block::getPtrToInstruction(Address addr) const
{
    if(addr < start())
        return NULL;
    if(addr >= end())
...
*/
void*
parse_func::getPtrToInstruction(Address addr) const
{
...
bool
parse_block::isEntryBlock(parse_func* f) const
{
...
bool
parse_block::isIndirectTailCallBlock()
{
    const Block::edgelist& trgs = targets();
    if(!trgs.empty())
    {
...
parse_func*
parse_block::getEntryFunc() const
{
...
void
parse_func::addParRegion(Address begin, Address end, parRegType t)
{
...
void
parse_block::getInsns(Insns& insns, Address base)
{
 ...
 */
void
parse_func::getReachableBlocks(const std::set<parse_block*>&  exceptBlocks,
...
void
parse_func::setinit_retstatus(ParseAPI::FuncReturnStatus rs)
{
    init_retstatus_ = rs;
    if(rs > retstatus())
    {
...
ParseAPI::FuncReturnStatus
parse_func::init_retstatus() const
{
...
void
parse_func::setHasWeirdInsns(bool wi)
{
    hasWeirdInsns_ = wi;
}

void
parse_block::setUnresolvedCF(bool newVal)
{
    unresolvedCF_ = newVal;
}

parse_func*
parse_block::getCallee()
{
