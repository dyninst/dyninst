...
parse_func::parse_func(SymtabAPI::Function *func, pdmodule *m, image *i,
...
void parse_func::addParRegion(Address begin, Address end, parRegType t) {
  ...
  iPar->setParentFunc(
      this);  // when not outlined, parent func will be same as regular
...
ParseAPI::FuncReturnStatus parse_func::init_retstatus() const {
  if (UNSET == init_retstatus_) {
    assert(
        !obj()
             ->defensiveMode());  // should have been set for defensive binaries
    return retstatus();
  }
...
bool parse_func::hasUnresolvedCF() {
  if (unresolvedCF_ == UNSET_CF) {
    for (blocklist::iterator iter = blocks().begin(); iter != blocks().end();
         ++iter) {
    ...
    if (unresolvedCF_ == UNSET_CF) unresolvedCF_ = NO_UNRESOLVED_CF;
  }
