 ...
 */
bool
BPatch_image::getSourceObj(BPatch_Vector<BPatch_sourceObj*>& vect)
{
...
bool
BPatch_image::getProcedures(BPatch_Vector<BPatch_function*>& procs,
...
bool
BPatch_image::getVariables(BPatch_Vector<BPatch_variableExpr*>& vars)
{
    bool some_succeeded = false;
...
bool
BPatch_image::setFuncModulesCallback(BPatch_function* bpf, void* data)
{
...
bool
BPatch_image::getModules(BPatch_Vector<BPatch_module*>& mods)
{
 ...
 */
BPatch_module*
BPatch_image::findModule(const char* name, bool substring_match)
{
    char buf[512];
...
BPatch_Vector<BPatch_function*>*
BPatch_image::findFunction(const char* name, BPatch_Vector<BPatch_function*>& funcs,
                           ...
                           bool incUninstrumentable)
{
    ...
    for(unsigned ai = 0; ai < all_funcs.size(); ai++)
    {
        ...
        for(auto piter = func->pretty_names_begin(); piter != func->pretty_names_end();
            ++piter)
        {
        ...
        if(found_match)
            continue;  // Don't check mangled names

        for(auto miter = func->symtab_names_begin(); miter != func->symtab_names_end();
            ++miter)
        {
...
BPatch_image::findFunction(BPatch_Vector<BPatch_function*>& funcs,
                           ...
                           int showError, bool incUninstrumentable)
{
    std::vector<func_instance*> all_funcs;
    std::vector<AddressSpace*>  as;
    addSpace->getAS(as);
...
bool
BPatch_image::findFunction(Dyninst::Address addr, BPatch_Vector<BPatch_function*>& funcs)
{
    std::vector<AddressSpace*> as;
    ...
    assert(ifuncs.size());
    for(std::set<func_instance*>::iterator iter = ifuncs.begin(); iter != ifuncs.end();
        ++iter)
    {
 ...
 */
BPatch_variableExpr*
BPatch_image::findVariable(const char* name, bool showError)
{
    ...
    if(!var)
    {
        for(unsigned i = 0; i < as.size(); i++)
        {
            string defaultNamespacePref =
                as[i]->getAOut()->parse_img()->getObject()->getDefaultNamespacePrefix();
...
BPatch_variableExpr*
BPatch_image::findVariable(BPatch_point& scp, const char* name, bool showError)
{
...
BPatch_type*
BPatch_image::findType(const char* name)
{
    BPatch_type* type;
...
bool
BPatch_image::getSourceLines(unsigned long addr, BPatch_Vector<BPatch_statement>& lines)
{
...
bool
BPatch_image::getAddressRanges(const char* lineSource, unsigned int lineNo,
...
BPatch_module*
BPatch_image::findModule(mapped_module* base)
{
...
BPatch_object*
BPatch_image::findObject(mapped_object* base)
{
...
BPatch_module*
BPatch_image::findOrCreateModule(mapped_module* base)
{
...
BPatch_object*
BPatch_image::findOrCreateObject(mapped_object* base)
{
 ...
 */
bool
BPatch_image::parseNewFunctions(BPatch_Vector<BPatch_module*>&         affectedModules,
...
void
BPatch_image::removeModule(BPatch_module* mod)
{
...
void
BPatch_image::removeObject(BPatch_object* obj)
{
...
bool
BPatch_image::readString(BPatch_variableExpr* expr, std::string& str, unsigned size_limit)
{
...
bool
BPatch_image::readString(Address addr, std::string& str, unsigned size_limit)
{
    char*    buffer        = NULL;
    ...
    for(;;)
    {
        ...
        if(!result)
        {
            proccontrol_printf("[%s:%d] - ERROR reading address %lx for string\n", FILE__,
                               __LINE__, start_word + buffer_offset);
        ...
        if(size_limit && size_limit < buffer_offset - start_offset)
        {
            ...
            proccontrol_printf(
                "[%s:%d] - WARN string read at %lx exceeded size limit of %u", FILE__,
...
void
BPatch_image::getNewCodeRegions(std::vector<BPatch_function*>& newFuncs,
                                std::vector<BPatch_function*>& modFuncs)
{
    ...
    const std::vector<parse_block*> blocks = as->getAOut()->parse_img()->getNewBlocks();
    ...
    {
        newFuncs.push_back(addSpace->findOrCreateBPFunc(as->findFunction(*fIter), NULL));
    }
...
Dyninst::PatchAPI::PatchMgrPtr
Dyninst::PatchAPI::convert(const BPatch_image* i)
{
