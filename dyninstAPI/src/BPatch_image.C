...
// $Id: BPatch_image.C,v 1.114 2008/04/15 21:39:06 roundy Exp $
                   ...
                   mapped_object *obj, vector<Address> &funcEntryAddrs)
{
    ...
    while (curEntry != funcEntryAddrs.end()) {
        reg = obj->parse_img()->getObject()->findEnclosingRegion(reg, (*curEntry)-baseAddress);
      ...
      const BPatch_Vector<Dyninst::Address> &funcEntryAddrs)
{
    ...
    if (funcEntryAddrs_.size()) {
        ...
        vector<string*> regionNames;
        findEnclosingRegions(funcEntryAddrs_, 
                             regionStartVec, 
                             regionEndVec, 
                             regionEntries, 
                             regionNames,
