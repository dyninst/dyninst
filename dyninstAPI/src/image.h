...
public:
    ...
    {
        if((file_ == fd.file_) && (member_ == fd.member_))
            return true;
        ;
        return false;
    }
...
std::string
getModuleName(std::string constraint);
std::string
getFunctionName(std::string constraint);

int
rawfuncscmp(parse_func*& pdf1, parse_func*& pdf2);
...
public:
    ...
    image(fileDescriptor& desc, bool& err, BPatch_hybridMode mode, bool parseGaps);
...
private:
    void findModByAddr(const SymtabAPI::Symbol* lookUp, vector<SymtabAPI::Symbol*>& mods,
                       string& modName, Address& modAddr, const string& defName);
...
public:
    ...
    bool findFunctionByMangled(const std::string& name, std::vector<parse_func*>& found);
    bool findFunctionByPretty(const std::string& name, std::vector<parse_func*>& found);
...
int
instPointCompare(instPoint*& ip1, instPoint*& ip2);
int
basicBlockCompare(BPatch_basicBlock*& bb1, BPatch_basicBlock*& bb2);
