...
bool BPatch_thread::getCallStack(BPatch_Vector<BPatch_frame> &stack) {
  ...
  for (unsigned int i = 0; i < stackWalk.size(); i++) {
    ...
    stack.push_back(BPatch_frame(this, (void *)frame.getPC(),
                                 (void *)frame.getFP(), isSignalFrame,
                                 isInstrumentation, point));
  }
  return true;
}
...
unsigned BPatch_thread::getBPatchID() { return (unsigned)llthread->getIndex(); }

BPatch_process *BPatch_thread::getProcess() { return proc; }

dynthread_t BPatch_thread::getTid() { return llthread->getTid(); }

Dyninst::LWP BPatch_thread::getLWP() { return llthread->getLWP(); }
 ...
 * supported interface.
 **/
unsigned long BPatch_thread::os_handle() { return (unsigned long)-1; }
 ...
 *
 */
bool BPatch_thread::oneTimeCodeAsync(const BPatch_snippet &expr, void *userData,
...
bool BPatch_thread::isDeadOnArrival() { return false; }
