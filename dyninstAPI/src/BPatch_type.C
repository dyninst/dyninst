...
BPatch_type*
BPatch_type::createFake(const char* _name)
{
...
BPatch_type::BPatch_type(boost::shared_ptr<Type> typ_)
: ID(typ_->getID())
, typ(typ_)
, refCount(1)
{
    ...
    {
        auto base =
            typ_->asDerivedType().getConstituentType(Dyninst::SymtabAPI::Type::share);
...
BPatch_type*
BPatch_type::findOrCreateType(boost::shared_ptr<Dyninst::SymtabAPI::Type> type)
{
    std::map<Dyninst::SymtabAPI::Type*, BPatch_type*>::iterator elem =
        type_map.find(type.get());
...
bool
BPatch_type::operator==(const BPatch_type& otype) const
{
...
const char*
BPatch_type::getName() const
{
    return typ->getName().c_str();
}

boost::shared_ptr<Type> BPatch_type::getSymtabType(Type::do_share_t) const { return typ; }
...
BPatch_type::getConstituentType() const
{
    ...
    if(typ->isDerivedType())
        ctype = typ->asDerivedType().getConstituentType(Type::share);
    else if(typ->isArrayType())
        ctype = typ->asArrayType().getBaseType(Type::share);
    else
        return NULL;
...
BPatch_Vector<BPatch_field*>*
BPatch_type::getComponents() const
{
...
BPatch_dataClass
BPatch_type::convertToBPatchdataClass(dataClass type)
{
    switch(type)
    {
        case dataEnum:
...
Dyninst::SymtabAPI::dataClass
BPatch_type::convertToSymtabType(BPatch_dataClass type)
{
    switch(type)
    {
...
BPatch_field::BPatch_field(Dyninst::SymtabAPI::Field* fld_,
                           BPatch_dataClass typeDescriptor, int value_, int size_)
: typeDes(typeDescriptor)
, value(value_)
, size(size_)
, fld(fld_)
{
...
void
BPatch_field::copy(BPatch_field& oField)
{
    fld     = oField.fld;
...
BPatch_field&
BPatch_field::operator=(BPatch_field& oField)
{
    copy(oField);
    return *this;
}
...
void
BPatch_field::fixupUnknown(BPatch_module* module)
{
    fld->fixupUnknown(module->lowlevel_mod()->pmod()->mod());
}

BPatch_localVar::BPatch_localVar(localVar* lVar_)
: lVar(lVar_)
{
    assert(lVar);
    ...
    if((stClass == storageAddr) && (refClass == storageNoRef))
        ...
        return BPatch_storageRegRef;
            ...
            ((loc->mr_reg == Dyninst::InvalidReg) ||
             (loc->mr_reg == Dyninst::FrameBase) || (loc->mr_reg == Dyninst::CFA)))
...
localVar*
BPatch_localVar::getSymtabVar()
{
    return lVar;
}

const char*
BPatch_localVar::getName()
{
    return lVar->getName().c_str();
}

BPatch_type*
BPatch_localVar::getType()
{
    return type;
}

int
BPatch_localVar::getLineNum()
{
...
long
BPatch_localVar::getFrameOffset()
{
...
BPatch_storageClass
BPatch_localVar::getStorageClass()
{
    return storageClass;
}
...
void
BPatch_localVar::fixupUnknown(BPatch_module* module)
{
...
BPatch_cblock::BPatch_cblock(CBlock* cBlk_)
: cBlk(cBlk_)
{
...
void
BPatch_cblock::fixupUnknowns(BPatch_module* module)
{
