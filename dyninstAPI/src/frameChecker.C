...
: arch(a)
{
    assert((arch == Arch_x86) || (arch == Arch_x86_64));
...
bool
frameChecker::isStackPreamble() const
{
...
bool
frameChecker::isMovStackToBase(unsigned index_to_check) const
{
    if(m_Insns.size() <= index_to_check)
        return false;
    if(m_Insns[index_to_check].getOperation().getID() == e_mov)
    {
        RegisterAST::Ptr stack_ptr(
            new RegisterAST(MachRegister::getStackPointer(Arch_x86)));
        RegisterAST::Ptr base_ptr(
            new RegisterAST(MachRegister::getFramePointer(Arch_x86)));
        std::string debugMe = m_Insns[index_to_check].format();

        if(m_Insns[index_to_check].isWritten(base_ptr) &&
           m_Insns[index_to_check].isRead(stack_ptr))
        {
            return true;
        }
        stack_ptr =
            RegisterAST::Ptr(new RegisterAST(MachRegister::getStackPointer(Arch_x86_64)));
        base_ptr =
            RegisterAST::Ptr(new RegisterAST(MachRegister::getFramePointer(Arch_x86_64)));
        if(m_Insns[index_to_check].isWritten(base_ptr) &&
           m_Insns[index_to_check].isRead(stack_ptr))
        {
            return true;
...
bool
frameChecker::isStackFrameSetup() const
{
