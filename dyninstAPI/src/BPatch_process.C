...
int BPatch_process::getAddressWidth() { return llproc->getAddressWidth(); }
 ...
 */
int BPatch_process::getPid() { return llproc ? llproc->getPid() : -1; }
...
BPatch_process::BPatch_process(const char *path, const char *argv[],
  ...
  if (argv) {
    for (int i = 0; argv[i] != NULL; i++) argv_vec.push_back(argv[i]);
  }

  // Construct a vector out of the contents of envp
  if (envp) {
    for (int i = 0; envp[i] != NULL; ++i) envp_vec.push_back(envp[i]);
  }
  ...
  llproc =
      PCProcess::createProcess(spath, argv_vec, mode, envp_vec, directoryName,
...
bool LinuxConsideredHarmful(pid_t pid)  // PUSH
{
  ...
  if (major == 2 && minor == 6 && (sub < 11 || (sub == 11 && subsub <= 11))) {
    ...
    if (my_ppid == mutatee_ppid || my_pid == mutatee_ppid) return true;
  }
 ...
 *
 */
int BPatch_process::getExitCode() { return exitCode; }
 ...
 *
 */
int BPatch_process::getExitSignal() { return lastSignal; }
...
bool BPatch_process::detach(bool cont) {
  if (image) image->removeAllModules();
 ...
 *
 */
bool BPatch_process::isDetached() { return detached; }
...
BPatch_variableExpr *BPatch_process::getInheritedVariable(
  ...
  return new BPatch_variableExpr(
      this, llproc, parentVar.getBaseAddr(), Null_Register,
...
BPatchSnippetHandle *BPatch_process::getInheritedSnippet(
  ...
  for (unsigned i = 0; i < instances.size(); i++) {
    Dyninst::PatchAPI::Instance::Ptr child =
        getChildInstance(instances[0], llproc);
...
bool BPatch_process::finalizeInsertionSet(bool, bool *) {
  ...
  if (shouldContinue) continueExecution();
...
bool BPatch_process::finalizeInsertionSetWithCatchup(
    bool, bool *, BPatch_Vector<BPatch_catchupInfo> &) {
  return false;
}
...
void *BPatch_process::oneTimeCodeInternal(const BPatch_snippet &expr,
                                          BPatch_thread *thread, void *userData,
                                          BPatchOneTimeCodeCallback cb,
                                          bool synchronous, bool *err,
                                          bool userRPC) {
  ...
  OneTimeCodeInfo *info = new OneTimeCodeInfo(
      synchronous, userData, cb, (thread) ? thread->getBPatchID() : 0);
  ...
  proccontrol_printf("%s[%d]: RPC completed, process status %s\n", FILE__,
                     __LINE__, isStopped() ? "stopped" : "running");
  ...
  do {
    ...
    if (bpfv.size() > 1) {
      std::string msg =
          ...
          std::string(
              "functions called DYNINSTloadLibrary -- not fatal but weird");
      BPatch_reportError(BPatchSerious, 100, msg.c_str());
    }
    BPatch_function *dlopen_func = bpfv[0];
    if (dlopen_func == NULL) break;
...
void BPatch_process::setExitedNormally() { exitedNormally = true; }

void BPatch_process::getThreads(BPatch_Vector<BPatch_thread *> &thrds) {
  for (unsigned i = 0; i < threads.size(); i++) thrds.push_back(threads[i]);
}

bool BPatch_process::isMultithreaded() { return (threads.size() > 1); }
...
BPatch_thread *BPatch_process::getThread(dynthread_t tid) {
  for (unsigned i = 0; i < threads.size(); i++)
    if (threads[i]->getTid() == tid) return threads[i];
  return NULL;
}

BPatch_thread *BPatch_process::getThreadByIndex(unsigned index) {
  for (unsigned i = 0; i < threads.size(); i++)
    if (threads[i]->getBPatchID() == index) return threads[i];
  return NULL;
}

processType BPatch_process::getType() { return TRADITIONAL_PROCESS; }
...
/**
 * This function continues a stopped process, letting it execute in single step
 *mode,
 * and printing the current instruction as it executes.
 **/

void BPatch_process::debugSuicide() { llproc->debugSuicide(); }
...
bool BPatch_process::triggerStopThread(instPoint *intPoint,
                                       func_instance *intFunc, int cb_ID,
                                       void *retVal) {
  ...
  for (unsigned int i = 0; i < BPatch::bpatch->stopThreadCallbacks.size();
       ++i) {
    BPatchStopThreadCallback curCallback =
        BPatch::bpatch->stopThreadCallbacks[i];
    if (cb_ID ==
        BPatch::bpatch->info->getStopThreadCallbackID((Address)curCallback)) {
...
bool BPatch_process::triggerSignalHandlerCB(instPoint *intPoint,
                                            func_instance *intFunc, long signum,
...
bool BPatch_process::hideDebugger() {
  ...
  if (kern) {
    ...
    BPatch_module *rtlib = this->image->findOrCreateModule(
        (*llproc->runtime_lib.begin())->getModules().front());
    vector<BPatch_function *> repfuncs;
    rtlib->findFunction("DYNINST_FakeCheckRemoteDebuggerPresent", repfuncs,
                        false);
    assert(!repfuncs.empty());
    replaceFunction(*funcs[0], *repfuncs[0]);
    disabledFuncs.push_back(
    ...
    bool &changedCode)  // output
{
  ...
  beginInsertionSet();
  for (std::map<func_instance *, set<block_instance *> >::iterator fIter =
           elimMap.begin();
  ...
  for (std::list<func_instance *>::iterator fit = deadFuncs.begin();
    ...
    if ((*fit)->getLiveCallerBlocks(delBlocks, deadFuncs, deadFuncCallers) &&
      ...
      vector<block_instance *>::iterator sit =
          deadFuncCallers[funcAddr].begin();
  ...
  for (map<func_instance *, block_instance *>::iterator nit =
           newFuncEntries.begin();
  ...
  for (std::list<func_instance *>::iterator fit = deadFuncs.begin();
    ...
    for (; bIter != deadBs.end(); bIter++) {
      deadBlocks.push_back(
          pair<Address, int>((*bIter)->start(), (*bIter)->size()));
    }
  }
  ...
  for (map<Address, vector<block_instance *> >::iterator sit =
           deadFuncCallers.begin();
    ...
    for (vector<block_instance *>::iterator bit = sit->second.begin();
      ...
      dfstubs[(*bit)->obj()].push_back(
          edgeStub(*bit, sit->first, ParseAPI::CALL));
    }
  }
    ...
    if (mit->first->parseNewEdges(mit->second)) {
      ...
      for (unsigned fidx = 0; fidx < mit->second.size(); fidx++) {
        ...
        if (bpfunc) {
          ...
          // couldn't reparse
          mal_printf(
              "WARNING: Couldn't re-parse an overwritten "
              "function at %lx %s[%d]\n",
      ...
      mal_printf(
          "ERROR: Couldn't re-parse overwritten "
          "functions %s[%d]\n",
          FILE__, __LINE__);
    }
  }
  ...
  for (std::map<func_instance *, set<block_instance *> >::iterator fit =
           elimMap.begin();
       fit != elimMap.end(); fit++) {
    ...
    if (!stubs[fit->first].empty()) {
      ...
      mal_printf(
          "WARNING: didn't have any stub edges for overwritten "
          "func %lx\n",
          fit->first->addr());
      // KEVINTEST: we used to wind up here with deleted functions, hopefully we
      // do not anymore
    }
