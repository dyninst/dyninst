...
template<class T>
int BPatch_annotatable<T>::createAnnotationType(char* name) {
  pdstring n(name);
...
template<class T>
int BPatch_annotatable<T>::getAnnotationType(char* name) {
  pdstring str(name);
...
template<class T>
int BPatch_annotatable<T>::getMetadata(char* name) {
  pdstring str(name);
...
template<class T>
void BPatch_annotatable<T>::setAnnotation(int type, BPatch_annotation* annotation) {
  if(type<number) {
...
template<class T>
BPatch_annotation* BPatch_annotatable<T>::getAnnotation(int type, int index) {
  if((unsigned)type<annotations.size()) {
    BPatch_Vector<BPatch_annotation*>* v = annotations[type];
...
template<class T>
void* BPatch_annotatable<T>::getAnnotation(char* name, int index) {
  int annotType = getAnnotationType(name);
  if(annotType == -1) {
    annotType = createAnnotationType(name);
    return NULL;
  }
  BPatch_annotation* annot = getAnnotation(annotType, index);
  if(annot == NULL)
    return NULL;
