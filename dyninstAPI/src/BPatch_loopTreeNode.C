...
BPatch_loopTreeNode::BPatch_loopTreeNode(
    BPatch_flowGraph *fg, PatchLoopTreeNode *tree,
...
std::string BPatch_loopTreeNode::getCalleeName(unsigned int i) {
  assert(i < callees.size());
  assert(callees[i] != NULL);
  return callees[i]->prettyName();
}

const char *BPatch_loopTreeNode::name() {
  assert(loop != NULL);
  return hierarchicalName;
}

unsigned int BPatch_loopTreeNode::numCallees() { return callees.size(); }

BPatch_loopTreeNode::~BPatch_loopTreeNode() {
  // Loops are deleted by BPatch_flowGraph...
  for (unsigned i = 0; i < children.size(); i++) delete children[i];
  if (hierarchicalName) free(hierarchicalName);
  // don't delete callees!
}

BPatch_basicBlockLoop *BPatch_loopTreeNode::findLoop(const char *name) {
  if (loop) {
    if (0 == strcmp(name, hierarchicalName)) return loop;
  }
