...
string edge_type_string(BPatch_edgeType t) {
...
BPatch_edge::BPatch_edge(edge_instance *e, BPatch_flowGraph *FG) {
  assert(e);
  edge = e;
  fg = FG;
...
BPatch_flowGraph *BPatch_edge::getFlowGraph() { return fg; }

BPatch_point *BPatch_edge::getPoint() {
  if (!point) {
    BPatch_flowGraph *cfg = getFlowGraph();
    instPoint *ip = instPoint::edge(cfg->ll_func(), edge);
    AddressSpace *as = cfg->getllAddSpace();
    assert(as);

    BPatch_point *newPoint =
        new BPatch_point(cfg->getAddSpace(), cfg->getFunction(), this, ip, as);
    if (newPoint) {
      point = newPoint;
    }
  }
...
PatchAPI::PatchEdge *PatchAPI::convert(const BPatch_edge *e) { return e->edge; }
