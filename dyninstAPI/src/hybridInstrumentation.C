...
static void
virtualFreeAddrCB_wrapper(BPatch_point* point, void* calc)
{
...
static void
virtualFreeSizeCB_wrapper(BPatch_point* point, void* calc)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
...
static void
virtualFreeCB_wrapper(BPatch_point* point, void* calc)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
...
#endif
static void
badTransferCB_wrapper(BPatch_point* point, void* calc)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
...
static void
abruptEndCB_wrapper(BPatch_point* point, void* calc)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
...
static void
signalHandlerCB_wrapper(BPatch_point* point, long snum,
...
static void
signalHandlerExitCB_wrapper(BPatch_point* point, void* dontcare)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
...
static void
synchShadowOrigCB_wrapper(BPatch_point* point, void* toOrig)
{
    dynamic_cast<BPatch_process*>(point->getFunction()->getProc())
        ->getHybridAnalysis()
...
HybridAnalysis::init()
{
    ...
    instrumentedFuncs =
        new map<BPatch_function*, map<BPatch_point*, BPatchSnippetHandle*>*>();
...
bool
HybridAnalysis::setMode(BPatch_hybridMode mode)
{
...
int
HybridAnalysis::saveInstrumentationHandle(BPatch_point*        point,
...
bool
HybridAnalysis::canUseCache(BPatch_point* pt)
{
    bool ret = true;
...
bool
HybridAnalysis::instrumentFunction(BPatch_function* func, bool useInsertionSet,
                                   bool instrumentReturns, bool addShadowSync)
{
    if(!instrumentReturns)
    {
            ...
            iter != inEdges.end(); ++iter)
        {
            ...
            {
                malware_cerr << "Setting instrumentation for function " << func->getName()
                             << " to instrument returns, detected "
    ...
    if(handlerFunctions.end() != handlerFunctions.find((Address) func->getBaseAddr()))
    {
        isHandler = true;
    }
    ...
    if(instrumentedFuncs->end() == instrumentedFuncs->find(func))
    {
        (*instrumentedFuncs)[func] = new std::map<BPatch_point*, BPatchSnippetHandle*>();
    }
    ...
    for(unsigned pidx = 0; pidx < points.size(); pidx++)
    {
        ...
        if(curPoint->isDynamic())
        {
            ...
            mal_printf("hybridInstrumentation[%d] monitoring unresolved at 0x%lx: "
                       "indirect, useCache=%d\n",
                       __LINE__, (unsigned long) curPoint->llpoint()->block()->last(),
                       (int) useCache);
            if(useCache)
            {
                dynamicTransferSnippet = new BPatch_stopThreadExpr(
                    badTransferCB_wrapper, dynTarget, *curPoint->llpoint()->func()->obj(),
            ...
            {  // e.g., libc's initterm function KEVINTODO: make this more robust, add
               // other callbacks
                ...
                handle =
                    proc()->insertSnippet(ifSmallThenStop, *curPoint, BPatch_lastSnippet);
            }
        }
        else
        {  // static ctrl flow
            ...
            if(curPoint->getPointType() == BPatch_locSubroutine)
            {
                mal_printf("hybridInstrumentation[%d] monitoring at 0x%lx: call 0x%lx\n",
                           __LINE__, curPoint->llpoint()->block()->last(), target);
            }
            else
            {
    ...
    if(addShadowSync)
    {
           ...
           (instShadowFuncs_.find(func) == instShadowFuncs_.end()))
        {
            ...
            {
                pointCount++;
                proc()->insertSnippet(BPatch_shadowExpr(false, synchShadowOrigCB_wrapper,
                                                        BPatch_constExpr(0)),
...
void
HybridAnalysis::removeInstrumentation(BPatch_function* func, bool useInsertionSet,
                                      bool handlesWereDeleted)
{
    ...
    malware_cerr << hex << "removing instrumentation from func at " << func->getBaseAddr()
                 << dec << endl;
    // const PatchAPI::PatchFunction::Blockset & blocks =
    // func->lowlevel_func()->getAllBlocks(); for
    // (PatchAPI::PatchFunction::Blockset::const_iterator bit = blocks.begin();
    //     bit != blocks.end();
    //     bit++)
    //{
    //   malware_cerr << hex << "  block [" << (*bit)->start() << " " << (*bit)->end() <<
    //   ")" << dec << endl;
    //}
        ...
        if(proc()->lowlevel_process()->isMemoryEmulated())
        {
            ...
            for(; pit != (*instrumentedFuncs)[func]->end(); pit++)
            {
                if(synchMap_pre_.end() != synchMap_pre_.find(pit->first))
                {
                    ...
                    // thus should not be dereferenced; use them only as key values in
                    // maps.
...
bool
HybridAnalysis::instrumentModule(BPatch_module* mod, bool useInsertionSet)
{
    ...
    for(; fIter != modFuncs->end(); fIter++)
    {
        ...
        {
            if(instrumentFunction(*fIter, false))
                didInstrument = true;
        }
    }
...
// Protects the code in the module
bool
HybridAnalysis::instrumentModules(bool useInsertionSet)
{
    bool didInstrument = false;
    if(useInsertionSet)
    {
 ...
 */
bool
HybridAnalysis::parseAfterCallAndInstrument(BPatch_point*    callPoint,
                                            BPatch_function* calledFunc, bool foundByRet)
{
    assert(callPoint);
    ...
    if(!parsedAfterCallPoint &&
       !hasEdge(callPoint->getFunction(),
                (Address) callPoint->llpoint()->block()->start(), fallThroughAddr))
    {
       ...
       cachePoints_.end() == cachePoints_.find(callPoint))
    {
        for(unsigned ftidx = 0; ftidx < fallThroughFuncs.size(); ftidx++)
        {
            ...
            if(hybridOW() && !hybridOW()->hasLoopInstrumentation(true, *fallThroughFunc))
            {
    ...
    // (even if we didn't parse, we have to do this to get rid of the illegal instructions
    // in the pad)
    proc()->finalizeInsertionSet(false);
...
bool
HybridAnalysis::addIndirectEdgeIfNeeded(BPatch_point* sourcePt, Address target)
{
    ...
    if(targObj && eit == edges.end())
    {
        ...
        worklist.push_back(CodeObject::NewEdgeToParse(
            sourcePt->llpoint()->block()->llb(), target - targObj->codeBase(), etype));
...
bool
HybridAnalysis::analyzeNewFunction(BPatch_point* source, Address target,
...
bool
HybridAnalysis::hasEdge(BPatch_function* func, Address source, Address target)
{
...
void
HybridAnalysis::parseNewEdgeInFunction(BPatch_point* sourcePoint, Address target,
                                       bool useInsertionSet)
{
    ...
    std::set<HybridAnalysisOW::owLoop*> loops;
    if(hybridOW() && hybridOW()->hasLoopInstrumentation(false, *sourceFunc, &loops))
    {
        std::set<HybridAnalysisOW::owLoop*>::iterator lIter = loops.begin();
...
bool
HybridAnalysis::processInterModuleEdge(BPatch_point* point, Address target,
                                       BPatch_module* targMod)
{
    ...
    if(targMod->isSystemLib() && BPatch_defensiveMode != targMod->getHybridMode())
    {
        ...
        if(point->getPointType() == BPatch_subroutine)
        {
            mal_printf("stopThread instrumentation found call %lx=>%lx, "
                       ...
                       point->llpoint()->block()->last(), target, modName, FILE__,
                       __LINE__);
        ...
        else
        {
            ...
            mal_printf("WARNING: transfer into non-instrumented system module "
                       "%s at: %lx=>%lx %s[%d]\n",
                       modName, point->llpoint()->block()->last(), target, FILE__,
                       __LINE__);
            instReturns = true;
        }
    ...
    else if(targMod->isExploratoryModeOn())
    {
        mal_printf("WARNING: Transfer into instrumented module %s "
                   "func %s at: %lx=>%lx %s[%d]\n",
                   modName, funcName, point->llpoint()->block()->last(), target, FILE__,
                   __LINE__);
    }
    else
    {  // jumped or called into module that's not recognized as a
...
bool
HybridAnalysis::blockcmp::operator()(const BPatch_basicBlock* b1,
...
void
HybridAnalysis::deleteSynchSnippet(SynchHandle* handle)
{
...
#else
Dyninst::ProcControlAPI::Process::mem_perm HybridAnalysis::getOrigPageRights(Address)
{
    assert(0);
...
void
HybridAnalysis::addReplacedFuncs(
...
bool
HybridAnalysis::getCallAndBranchTargets(block_instance*       block,
 ...
 */
bool
HybridAnalysis::getCFTargets(BPatch_point* point, vector<Address>& targets)
{
    bool       ret    = true;
                ...
                else
                {
                    ...
                    switch((*iter)->type())
                    {
                        ...
                        case FALLTHROUGH:
                            ...
                            break;
                        default: {  // this is a cond'l taken or jump target, crack
                                    // instruction to get target
                            using namespace InstructionAPI;
...
const HybridAnalysis::AnalysisStats&
HybridAnalysis::getStats()
{
    PCProcess*                   llproc = proc_->lowlevel_process();
    const vector<mapped_object*> objs   = llproc->mappedObjects();
    for(vector<mapped_object*>::const_iterator oit = objs.begin(); oit != objs.end();
        oit++)
    {
