...
PCProcess *PCProcess::createProcess(const string file, pdvector<string> &argv,
  ...
  startup_cerr << "Creating process " << file << " in directory " << dir
               << endl;
  ...
  startup_printf("%s[%d]: stdin: %d, stdout: %d, stderr: %d\n", FILE__,
                 __LINE__, stdin_fd, stdout_fd, stderr_fd);
  ...
  if (!setEnvPreload(envp, path)) {
    startup_cerr << "Failed to set environment var to preload RT library"
                 << endl;
    return NULL;
  }
  ...
  if (!tmpPcProc) {
    ...
    string msg = string("Failed to create process for ") + file + string(": ") +
                 lastErrMsg;
...
PCProcess *PCProcess::setupForkedProcess(PCProcess *parent,
                                         Process::ptr pcProc) {
  ...
  if (parent->tracedSyscalls_)
    ret->tracedSyscalls_ =
        new syscallNotification(parent->tracedSyscalls_, ret);
  else
...
PCProcess *PCProcess::setupExecedProcess(PCProcess *oldProc,
                                         std::string execPath) {
 ...
 * Create:
 *     Fork/Exec
 *                     <-- Trap              Halted in exec (handled by
 * ProcControlAPI)
...
bool PCProcess::bootstrapProcess() {
  ...
  // Insert a breakpoint at the entry point of main (and possibly
  // __libc_start_main)
  if (!hasPassedMain()) {
    ...
    while (!hasReachedBootstrapState(bs_readyToLoadRTLib)) {
      ...
      if (isTerminated()) {
        bperr(
            "The process exited during startup.  This is likely due to one "
            "of two reasons:\n"
        ...
        startup_printf(
            "%s[%d]: program exited early, never reached "
            "initialized state\n",
            FILE__, __LINE__);
        startup_printf(
            "Error is likely due to the application or RT "
  ...
  if (!loadRTLib()) {
    bperr(
        "Dyninst was unable to load the dyninst runtime library "
  ...
  if (!wasCreatedViaFork()) {
    ...
    //
    // This will require checking whether BPatch holds a defined callback and
    // also
...
bool PCProcess::createStackwalker() {
  ...
  if (NULL ==
      (stackwalker_ = Walker::newWalker(procDebug, NULL, symLookup, false))) {
...
void PCProcess::addASharedObject(mapped_object *newObj) {
  ...
  startup_printf("%s[%d]: adding shared object %s, addr range 0x%lx to 0x%lx\n",
                 FILE__, __LINE__, newObj->fileName().c_str(),
...
void PCProcess::removeASharedObject(mapped_object *obj) {
  ...
  proccontrol_printf("Removing shared object %s, addr range 0x%x to 0x%x\n",
                     obj->fileName().c_str(), obj->getBaseAddress(),
                     obj->get_size());
...
bool PCProcess::setAOut(fileDescriptor &desc) {
  ...
  mapped_object *aout =
      mapped_object::createMappedObject(desc, this, getHybridMode());
  if (!aout) {
...
void PCProcess::findSignalHandler(mapped_object *obj) {
  ...
  if (obj->getSymbolInfo(signame, sigSym)) {
    ...
    startup_printf(
        "%s[%d]: findSignalhandler(%p): addingSignalHandler(%p, %d)\n", FILE__,
        __LINE__, obj, (void *)sigSym.getAddr(), size_to_use);
...
void PCProcess::addInferiorHeap(mapped_object *obj) {
  ...
  if (obj->getInfHeapList(infHeaps)) {
    ...
    for (u_int j = 0; j < infHeaps.size(); j++) {
      ...
      infmalloc_printf(
          "%s[%d]: Adding heap from 0x%lx - 0x%lx (%d bytes, type %d) from "
          "mapped object %s\n",
          FILE__, __LINE__, infHeaps[j].addr(),
...
static const unsigned MAX_THREADS =
    32;  // Should match MAX_THREADS in RTcommon.c
...
bool PCProcess::loadRTLib() {
  ...
  if (!readDataWord((void *)vars[0]->getAddress(), sizeof(int),
                    (void *)&loaded_ok, false)) {
...
bool PCProcess::insertBreakpointAtMain() {
  if (main_function_ == NULL) {
    startup_printf(
        "%s[%d]: main function not yet found, cannot insert breakpoint\n",
        FILE__, __LINE__);
    return false;
  }
  ...
  if (!pcProc_->addBreakpoint(addr, mainBrkPt_)) {
    startup_printf(
        "%s[%d]: failed to insert a breakpoint at main entry: 0x%x\n", FILE__,
        __LINE__, addr);
    return false;
  }
...
Breakpoint::ptr PCProcess::getBreakpointAtMain() const { return mainBrkPt_; }
...
bool PCProcess::stopProcess() {
  ...
  if (isInEventHandling()) {
    proccontrol_printf(
        "%s[%d]: process currently in event handling, not stopping\n", FILE__,
        __LINE__);
    return true;
  }
...
bool PCProcess::isAttached() const { return attached_; }
...
bool PCProcess::isExecing() const { return execing_; }

void PCProcess::setExecing(bool b) { execing_ = b; }

bool PCProcess::isExiting() const { return exiting_; }

void PCProcess::setExiting(bool b) { exiting_ = b; }

bool PCProcess::isInEventHandling() const { return inEventHandling_; }

void PCProcess::setInEventHandling(bool b) { inEventHandling_ = b; }

bool PCProcess::hasReportedEvent() const { return reportedEvent_; }

void PCProcess::setReportingEvent(bool b) { reportedEvent_ = b; }
...
bool PCProcess::writeDataWord(void *inTracedProcess, u_int amount,
                              const void *inSelf) {
...
bool PCProcess::readDataSpace(const void *inTracedProcess, u_int amount,
  ...
  if (!result && displayErrMsg) {
    ...
    msg << "System error: unable to read " << amount << "@"
        ...
        << " from process data space: " << getLastErrorMsg()
        << "(pid = " << getPid() << ")";
...
bool PCProcess::writeTextSpace(void *inTracedProcess, u_int amount,
                               const void *inSelf) {
  if (isTerminated()) return false;
  bool result = pcProc_->writeMemory((Address)inTracedProcess, inSelf, amount);
...
bool PCProcess::writeTextWord(void *inTracedProcess, u_int amount,
                              const void *inSelf) {
  if (isTerminated()) return false;
...
PCThread *PCProcess::getInitialThread() const { return initialThread_; }
...
bool PCProcess::removeThread(dynthread_t tid) {
  ...
  proccontrol_printf("%s[%d]: removed thread %lu from process %d\n", FILE__,
                     __LINE__, toDelete->getLWP(), getPid());
  return true;
}
...
bool PCProcess::wasRunningWhenAttached() const { return runningWhenAttached_; }

bool PCProcess::wasCreatedViaAttach() const { return createdViaAttach_; }

bool PCProcess::wasCreatedViaFork() const { return parent_ != NULL; }
...
int PCProcess::getPid() const { return savedPid_; }
...
PCEventHandler *PCProcess::getPCEventHandler() const { return eventHandler_; }
  ...
  for (map<dynthread_t, PCThread *>::iterator i = threadsByTid_.begin();
    ...
    if (!curThr->walkStack(stackWalk)) {
      ...
      proccontrol_printf("%s[%d]: failed to walk stack for thread 0x%lx(%d)\n",
                         FILE__, __LINE__, curThr->getTid(), curThr->getLWP());
    } else {
...
Address PCProcess::inferiorMalloc(unsigned size, inferiorHeapType type,
  ...
  enum MallocAttempt {
    ...
    NewSegmentSizedConstrained =
        3,  // allocate new segment (sized, constrained)
  ...
  infmalloc_printf(
      "%s[%d]: inferiorMalloc entered; size %d, type %d, near 0x%lx (0x%lx to "
      "0x%lx)\n",
  ...
  for (ntry = 0; freeIndex == -1; ntry++) {
    switch (ntry) {
      ...
      case NewSegment1MBConstrained:
        infmalloc_printf(
            "%s[%d]: inferiorMallocDynamic for %d (0x%x) bytes between 0x%lx - "
            "0x%lx\n",
...
static void alignUp(int &val, int align) {
  assert(val >= 0);
  assert(align >= 0);
  ...
  for (unsigned lcv = 0; lcv < requests.size(); lcv++) {
    ...
    for (unsigned funcIter = 0; funcIter < matchingFuncs.size(); funcIter++) {
      ...
      inst_printf("%s[%d]: Instrumenting %s at 0x%lx, offset 0x%lx in %s\n",
                  FILE__, __LINE__, func->symTabName().c_str(), func->addr(),
      ...
      for (std::vector<Point *>::iterator iter = points.begin();
        ...
        Dyninst::PatchAPI::Instance::Ptr inst =
            (req->order == orderFirstAtPoint) ? (*iter)->pushFront(ast)
                                              : (*iter)->pushBack(ast);
        if (inst) {
...
bool PCProcess::postIRPC(void *buffer, int size, void *userData,
                         bool runProcessWhenDone, PCThread *thread,
                         bool synchronous, void **result, bool userRPC,
                         bool isMemAlloc, Address addr) {
  return postIRPC_internal(buffer, size, size, REG_NULL, addr, userData,
                           runProcessWhenDone, thread, synchronous, userRPC,
                           isMemAlloc, result);
}
...
bool PCProcess::postIRPC(AstNodePtr action, void *userData,
  ...
  return postIRPC_internal(irpcBuf.start_ptr(), irpcBuf.used(), breakOffset,
                           resultReg, addr, userData, runProcessWhenDone,
                           thread, synchronous, userRPC, isMemAlloc, result);
}

// DEBUG
#include "instructionAPI/h/InstructionDecoder.h"

bool PCProcess::postIRPC_internal(void *buf, unsigned size,
                                  unsigned breakOffset, Register resultReg,
                                  Address addr, void *userData,
                                  bool runProcessWhenDone, PCThread *thread,
                                  bool synchronous, bool userRPC,
                                  bool isMemAlloc, void **result) {
  if (isTerminated()) {
    proccontrol_printf(
        "%s[%d]: cannot post RPC to exited or terminated process %d\n", FILE__,
        __LINE__, getpid());
    return false;
  }
  ...
  if (synchronous) {
    // We have an interesting problem here. ProcControl allows callbacks to
    // specify whether the
    // process should stop or run; however, that allows us to stop a process in
    // the middle of an
    // inferior RPC. If that happens, manually execute a continue and wait for
    ...
    if (!res) {
      ...
      while (!done) {
        ...
        if (ProcControlAPI::getLastError() != ProcControlAPI::err_notrunning) {
          ...
          proccontrol_printf(
              "%s[%d]: ProcControl reported IRPC thread stopped, continuing "
  ...
  if (!res) {
    proccontrol_printf("%s[%d]: failed to post %s RPC to %s\n", FILE__,
                       __LINE__, (synchronous ? "sync" : "async"),
                       ((thread == NULL) ? "thread" : "process"));
    delete newRPC;
    return false;
  }
...
BPatch_hybridMode PCProcess::getHybridMode() { return analysisMode_; }
    ...
    std::list<block_instance *> &writtenBBIs)                    // output
{
  ...
  list<pair<Address, Address> >::const_iterator rIter =
      overwrittenRanges.begin();
    ...
    const list<pair<Address, Address> > &owRanges)  // input
{
  ...
  for (; rIter != owRanges.end(); rIter++) {
    ...
    objRanges[obj]->push_back(
        pair<Address, Address>(rIter->first, rIter->second));
  }
 ...
 * ex: the set of blocks that are executing on the call stack that were not
 * overwritten
 *
 * primitives
 ...
 *          forall f in F(b): B(F) - R( B(f) - ow , New(f) U (EP(f) \ ow(f)) U
 * (ex(f) intersect Elim(f)) ),
...
bool PCProcess::getDeadCode(
    ...
    std::map<func_instance *, set<block_instance *> >
        & /*elimMap*/,                           // output: elimF
    std::list<func_instance *> & /*deadFuncs*/,  // output: DeadF
    std::map<func_instance *, block_instance *>
        & /*newFuncEntries*/)  // output: newF
{
  assert(0 && "TODO");
  return false;
#if 0
  ...
  for (unsigned int i = 0; !foundFrame && i < stacks.size(); ++i) {
    ...
    for (unsigned int j = 0; !foundFrame && j < stack.size(); ++j) {
      ...
      if (getRelocInfo(framePC, ri) && ri.func) {
        ...
        for (std::set<block_instance *>::iterator cb_iter =
                 callerBlocks.begin();
...
bool PCProcess::generateRequiredPatches(instPoint *callPoint,
  ...
  for (vector<ParseAPI::Function *>::iterator fit = callFuncs.begin();
    ...
    if (rit == relocatedCode_.rend()) {
      mal_printf(
          "WARNING: no relocs of call-fallthrough at %lx "
...
Architecture PCProcess::getArch() const { return savedArch_; }
...
bool PCProcess::needsPIC() { return false; }

bool PCProcess::isInDebugSuicide() const { return isInDebugSuicide_; }
...
bool PCProcess::walkStack(pdvector<Frame> &stackWalk, PCThread *thread) {
  if (stackwalker_ == NULL) return false;
 ...
 */
bool PCProcess::triggerStopThread(Address pointAddress, int callbackID,
                                  void *calculation) {
  ...
  if (callbackID < 0) {
    callbackID *= -1;
    calculation =
        (void *)stopThreadCtrlTransfer(intPoint, (Address)calculation);
  }
...
Address PCProcess::stopThreadCtrlTransfer(instPoint *intPoint, Address target) {
  ...
  if (intPoint->type() == instPoint::FuncExit &&
      intPoint->block()->isFuncExit() && !intPoint->func()->isSignalHandler()) {
  ...
  if (isRuntimeHeapAddr(target)) {
    ...
    malware_cerr << "Looking for matches to incoming address " << hex << target
                 << dec << endl;
    std::pair<func_instance *, Address> tmp;
...
void PCProcess::removeSyncRPCThread(Thread::ptr thr) {
  proccontrol_printf("%s[%d]: removed sync rpc thread %d/%d\n", FILE__,
                     __LINE__, getPid(), thr ? thr->getLWP() : 0);
...
bool PCProcess::continueSyncRPCThreads() {
  for (set<Thread::ptr>::iterator i = syncRPCThreads_.begin();
    ...
    if (!(*i)) {
      ...
      proccontrol_printf(
          "%s[%d]: failed to continue thread %d/%d for sync RPC\n", FILE__,
          __LINE__, getPid(), (*i)->getLWP());
      return false;
    }
  }
...
void PCProcess::invalidateMTCache() { mt_cache_result_ = not_cached; }

StackwalkSymLookup::StackwalkSymLookup(PCProcess *p) : proc_(p) {}

StackwalkSymLookup::~StackwalkSymLookup() {}

bool StackwalkSymLookup::lookupAtAddr(Dyninst::Address addr,
                                      std::string &out_name, void *&out_value) {
