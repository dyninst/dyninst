...
PCProcess*
PCProcess::createProcess(const string file, std::vector<string>& argv,
                         BPatch_hybridMode analysisMode, std::vector<string>& envp,
                         const string dir, int stdin_fd, int stdout_fd, int stderr_fd)
{
    ...
    if(!tmpPcProc)
    {
        ...
        string msg =
            string("Failed to create process for ") + file + string(": ") + lastErrMsg;
...
PCProcess*
PCProcess::attachProcess(const string& progpath, int pid, BPatch_hybridMode analysisMode)
{
    initSymtabReader();
    ...
    if(!ret->bootstrapProcess())
    {
        startup_cerr << "Failed to bootstrap process " << pid << ": terminating..."
                     << endl;
        ret->terminateProcess();
...
PCProcess*
PCProcess::setupForkedProcess(PCProcess* parent, Process::ptr pcProc)
{
    ...
    if(ret->runtime_lib.size() == 0)
    {
        ...
        for(unsigned i = 0; i < ret->mapped_objects.size(); ++i)
        {
            ...
            fileDescriptor        tmpDesc(ret->dyninstRT_name, desc.code(), desc.data());
            if(desc == tmpDesc)
            {
    ...
    {
        signal_handler_location* oldSig =
            dynamic_cast<signal_handler_location*>(sigHandlers[i]);
        assert(oldSig);
    ...
    {
        startup_printf(
            "%s[%d]: failed to copy dangling memory from parent %d to child %d\n", FILE__,
            __LINE__, parent->getPid(), ret->getPid());
        ret->terminateProcess();
...
PCProcess*
PCProcess::setupExecedProcess(PCProcess* oldProc, std::string execPath)
{
    BPatch::bpatch->registerExecCleanup(oldProc, NULL);

    PCProcess* newProc =
        new PCProcess(oldProc->pcProc_, execPath, oldProc->analysisMode_);
...
PCProcess::~PCProcess()
{
    ...
    if(pcProc_ && pcProc_->getData() == this)
        pcProc_->setData(NULL);
}

void
PCProcess::initSymtabReader()
{
...
bool
PCProcess::hasReachedBootstrapState(bootstrapState_t state) const
{
    return state <= bootstrapState_;
}

void
PCProcess::setBootstrapState(bootstrapState_t newState)
{
...
PCProcess::bootstrapProcess()
{
    ...
    if(!hasPassedMain())
    {
        ...
        {
            startup_printf(
                "%s[%d]: bootstrap failed while setting a breakpoint at main\n", FILE__,
                __LINE__);
            return false;
        }
        ...
        while(!hasReachedBootstrapState(bs_readyToLoadRTLib))
        {
            ...
            if(isStopped())
            {
                ...
                {
                    startup_printf(
                        "%s[%d]: bootstrap failed while continuing the process\n", FILE__,
                        __LINE__);
                    return false;
                }
            }
    ...
    if(!wasCreatedViaFork())
    {
        ...
        startup_printf(
            "%s[%d]: installing default Dyninst instrumentation into process %d\n",
            FILE__, __LINE__, getPid());
...
PCProcess::createStackwalker()
{
    ...
    if(NULL == (stackwalker_ = Walker::newWalker(procDebug, NULL, symLookup, false)))
    {
...
void
PCProcess::createInitialThreads()
{
...
bool
PCProcess::createInitialMappedObjects()
{
    if(file_.empty())
    {
    ...
    mapped_object* aout = mapped_object::createMappedObject(
        pcProc_->libraries().getExecutable(), this, analysisMode_);
    ...
    for(LibraryPool::const_iterator i = libraries.begin(); i != libraries.end(); ++i)
    {
        ...
        if((*i) == libraries.getExecutable())
            continue;

        startup_cerr << "Library: " << (*i)->getAbsoluteName() << hex << " / "
                     << (*i)->getLoadAddress() << ", "
                     << ((*i)->isSharedLib() ? "<lib>" : "<aout>") << dec << endl;

        mapped_object* newObj =
            mapped_object::createMappedObject(*i, this, analysisMode_);
        if(newObj == NULL)
        {
        ...
        {
            startup_printf(
                "%s[%d]: RT library already loaded, manual loading not necessary\n",
                FILE__, __LINE__);
        ...
        if(analysisMode_ == BPatch_defensiveMode)
        {
            ...
            if(lib_name == "dyninstAPI_RT.dll" || lib_name == "ntdll.dll" ||
               lib_name == "kernel32.dll" || lib_name == "user32.dll" ||
               lib_name == "KERNELBASE.dll" || lib_name == "msvcrt.dll" ||
               lib_name == "msvcr80.dll" || lib_name == "msvcr100d.dll" ||
               lib_name == "msvcp100d.dll" || lib_name == "MSVCR100.dll")
            {
...
void
PCProcess::addASharedObject(mapped_object* newObj)
{
    assert(newObj);
    ...
    {
        startup_printf(
            "%s[%d]: skipping check for new inferior heaps, heap uninitialized\n", FILE__,
            __LINE__);
    }
}

void
PCProcess::removeASharedObject(mapped_object* obj)
{
    ...
    proccontrol_printf("Removing shared object %s, addr range 0x%lx to 0x%x\n",
                       obj->fileName().c_str(), obj->getBaseAddress(), obj->get_size());

    // TODO Signal handler...
}

bool
PCProcess::setAOut(fileDescriptor& desc)
{
    startup_printf("%s[%d]:  enter setAOut\n", FILE__, __LINE__);

    assert(mapped_objects.size() == 0);

    mapped_object* aout = mapped_object::createMappedObject(desc, this, getHybridMode());
    if(!aout)
    {
...
void
PCProcess::findSignalHandler(mapped_object* obj)
{
    ...
    if(obj->getSymbolInfo(signame, sigSym))
    {
        ...
        startup_printf("%s[%d]: findSignalhandler(%p): addingSignalHandler(%p, %u)\n",
                       FILE__, __LINE__, (void*) obj, (void*) sigSym.getAddr(),
                       size_to_use);
 ...
 */
void
PCProcess::addInferiorHeap(mapped_object* obj)
{
    ...
    if(obj->getInfHeapList(infHeaps))
    {
        ...
        for(u_int j = 0; j < infHeaps.size(); j++)
        {
            infmalloc_printf("%s[%d]: adding heap at 0x%lx to 0x%lx, name %s\n", FILE__,
                             __LINE__, infHeaps[j].addr(),
                             infHeaps[j].addr() + infHeaps[j].size(),
                             infHeaps[j].name().c_str());

            // platform-specific check to ignore this heap
            if(skipHeap(infHeaps[j]))
                continue;

            heapItem* h = new heapItem(infHeaps[j].addr(), infHeaps[j].size(),
                                       infHeaps[j].type(), false);

            infmalloc_printf("%s[%d]: Adding heap from 0x%lx - 0x%lx (%u bytes, type %d) "
                             "from mapped object %s\n",
                             FILE__, __LINE__, infHeaps[j].addr(),
                             infHeaps[j].addr() + infHeaps[j].size(), infHeaps[j].size(),
                             infHeaps[j].type(), obj->fileName().c_str());
...
PCProcess::loadRTLib()
{
    ...
    if(!readDataWord((void*) vars[0]->getAddress(), sizeof(int), (void*) &loaded_ok,
                     false))
    {
...
PCProcess::setRTLibInitParams()
{
    ...
    assert(vars.size() >= 1);
    if(!writeDataWord((void*) vars[0]->getAddress(), sizeof(int),
                      (void*) &dyn_debug_rtlib))
    {
        startup_printf("%s[%d]: writeDataWord failed\n", FILE__, __LINE__);
        return false;
    }
...
bool
PCProcess::insertBreakpointAtMain()
{
...
bool
PCProcess::removeBreakpointAtMain()
{
...
Breakpoint::ptr
PCProcess::getBreakpointAtMain() const
{
    return mainBrkPt_;
}
...
PCProcess::continueProcess()
{
    ...
    {
        proccontrol_printf(
            "%s[%d]: process currently in event handling, not continuing\n", FILE__,
            __LINE__);
        return true;
    }
...
bool
PCProcess::detachProcess(bool cont = true)
{
    if(isTerminated())
        return true;
    ...
    if(tracedSyscalls_)
    {
        ...
        if(!isStopped())
        {
            ...
            {
                proccontrol_printf(
                    "%s[%d]: failed to stop process for removing syscalls\n", FILE__,
                    __LINE__);
                return false;
            }
        }
        ...
        if(cont)
        {
            if(needToContinue)
            {
                ...
                {
                    proccontrol_printf(
                        "%s[%d]: failed to continue process after removing syscalls\n",
                        FILE__, __LINE__);
                }
            }
        }
...
bool
PCProcess::isBootstrapped() const
{
    return bootstrapState_ == bs_initialized;
}

bool
PCProcess::isAttached() const
{
    return attached_;
}

bool
PCProcess::isStopped() const
{
    if(pcProc_ == Process::ptr())
        return true;
    return pcProc_->allThreadsStopped();
}

bool
PCProcess::isTerminated() const
{
    if(pcProc_ == Process::ptr())
        return true;
...
bool
PCProcess::hasExitedNormally() const
{
    if(pcProc_ == Process::ptr())
        return true;
...
bool
PCProcess::isExecing() const
{
    return execing_;
}

void
PCProcess::setExecing(bool b)
{
    execing_ = b;
}

bool
PCProcess::isExiting() const
{
    return exiting_;
}

void
PCProcess::setExiting(bool b)
{
    exiting_ = b;
}

bool
PCProcess::isInEventHandling() const
{
    return inEventHandling_;
}

void
PCProcess::setInEventHandling(bool b)
{
    inEventHandling_ = b;
}

bool
PCProcess::hasReportedEvent() const
{
    return reportedEvent_;
}

void
PCProcess::setReportingEvent(bool b)
{
    reportedEvent_ = b;
}

void
PCProcess::markExited()
{
    pcProc_.reset();
}

void
PCProcess::writeDebugDataSpace(void* inTracedProcess, u_int amount, const void* inSelf)
{
...
bool
PCProcess::writeDataSpace(void* inTracedProcess, u_int amount, const void* inSelf)
{
    if(isTerminated())
    {
...
bool
PCProcess::writeDataWord(void* inTracedProcess, u_int amount, const void* inSelf)
{
    if(isTerminated())
        return false;
...
bool
PCProcess::readDataSpace(const void* inTracedProcess, u_int amount, void* inSelf,
                         bool displayErrMsg)
{
    if(isTerminated())
        return false;
    ...
    if(!result && displayErrMsg)
    {
        ...
        msg << "System error: unable to read " << amount << "@"
            << Address_str((Address) inTracedProcess)
            << " from process data space: " << getLastErrorMsg() << "(pid = " << getPid()
            << ")";
...
bool
PCProcess::readDataWord(const void* inTracedProcess, u_int amount, void* inSelf,
                        bool displayErrMsg)
{
    if(isTerminated())
        return false;
...
bool
PCProcess::writeTextSpace(void* inTracedProcess, u_int amount, const void* inSelf)
{
    if(isTerminated())
        return false;
    bool result = pcProc_->writeMemory((Address) inTracedProcess, inSelf, amount);
...
bool
PCProcess::writeTextWord(void* inTracedProcess, u_int amount, const void* inSelf)
{
    if(isTerminated())
        return false;
...
bool
PCProcess::readTextSpace(const void* inTracedProcess, u_int amount, void* inSelf)
{
    if(isTerminated())
        return false;
...
bool
PCProcess::readTextWord(const void* inTracedProcess, u_int amount, void* inSelf)
{
    if(isTerminated())
        return false;
...
PCThread*
PCProcess::getInitialThread() const
{
    return initialThread_;
}

PCThread*
PCProcess::getThread(dynthread_t tid) const
{
...
PCProcess::addThread(PCThread* thread)
{
    ...
    proccontrol_printf("%s[%d]: added thread %d to process %d\n", FILE__, __LINE__,
                       thread->getLWP(), getPid());
}

void
PCProcess::getThreads(vector<PCThread*>& threads) const
{
...
bool
PCProcess::wasRunningWhenAttached() const
{
    return runningWhenAttached_;
}

bool
PCProcess::wasCreatedViaAttach() const
{
    return createdViaAttach_;
}

bool
PCProcess::wasCreatedViaFork() const
{
    return parent_ != NULL;
}

unsigned
PCProcess::getMemoryPageSize() const
{
    assert(pcProc_);
...
int
PCProcess::getPid() const
{
    return savedPid_;
}

int
PCProcess::incrementThreadIndex()
{
...
PCEventHandler*
PCProcess::getPCEventHandler() const
{
    return eventHandler_;
}

bool
PCProcess::walkStacks(std::vector<std::vector<Frame>>& stackWalks)
{
    bool needToContinue = false;
    bool retval         = true;

    // sanity check
    if(stackwalker_ == NULL)
        return false;
        ...
        i != threadsByTid_.end(); ++i)
    {
        ...
        if(!curThr->walkStack(stackWalk))
        {
            ...
            proccontrol_printf("%s[%d]: failed to walk stack for thread 0x%lx(%d)\n",
                               FILE__, __LINE__, (unsigned long) curThr->getTid(),
                               curThr->getLWP());
        }
        else
        {
    ...
    if(needToContinue)
    {
        ...
        {
            proccontrol_printf(
                "%s[%d]: failed to continue process after performing stackwalking\n",
                FILE__, __LINE__);
            return false;
        }
    }
...
bool
PCProcess::getAllActiveFrames(std::vector<Frame>& activeFrames)
{
    if(threadsByTid_.size() == 0)
        return false;
...
Address
PCProcess::inferiorMalloc(unsigned size, inferiorHeapType type, Address near_, bool* err)
{
    ...
    infmalloc_printf(
        "%s[%d]: inferiorMalloc entered; size %u, type %d, near 0x%lx (0x%lx to 0x%lx)\n",
        ...
        switch(ntry)
        {
            ...
            case NewSegment1MBConstrained:
                infmalloc_printf("%s[%d]:  (3) inferiorMallocDynamic "
                                 ...
                                 FILE__, __LINE__, HEAP_DYN_BUF_SIZE,
                                 (unsigned int) HEAP_DYN_BUF_SIZE, lo, hi);
            ...
            case RemoveRangeConstraints:
                infmalloc_printf(
                    "%s[%d]:  (5) inferiorMalloc: removing range constraints\n", FILE__,
                    __LINE__);
                lo = ADDRESS_LO;
                hi = ADDRESS_HI;
                if(err)
            ...
            case NewSegment1MBUnconstrained:
                infmalloc_printf("%s[%d]:  (6) inferiorMallocDynamic for %d (0x%x) bytes "
                                 "between 0x%lx - 0x%lx\n",
                                 FILE__, __LINE__, HEAP_DYN_BUF_SIZE,
                                 (unsigned int) HEAP_DYN_BUF_SIZE, lo, hi);
                inferiorMallocDynamic(HEAP_DYN_BUF_SIZE, lo, hi);
            ...
            case NewSegmentSizedUnconstrained:
                infmalloc_printf("%s[%d]:  (7) inferiorMallocDynamic for %u (0x%x) bytes "
                                 "between 0x%lx - 0x%lx\n",
                                 FILE__, __LINE__, size, (unsigned int) size, lo, hi);
                inferiorMallocDynamic(size, lo, hi);
                break;
...
void
PCProcess::inferiorFree(Dyninst::Address item)
{
    inferiorFreeInternal(item);
}

bool
PCProcess::inferiorRealloc(Dyninst::Address item, unsigned int newSize)
{
    if(bootstrapState_ <= bs_readyToLoadRTLib)
    {
        return true;
...
bool
PCProcess::inferiorMallocDynamic(int size, Address lo, Address hi)
{
    ...
    switch((int) result)
    {
        ...
        default:
            ...
            heapItem* h =
                new heapItem(result, size, getDynamicHeapType(), true, HEAPfree);
            addHeap(h);
            break;
    }
...
void
PCProcess::installInstrRequests(const std::vector<instMapping*>& requests)
{
    if(requests.size() == 0)
    {
        return;
    ...
    for(unsigned lcv = 0; lcv < requests.size(); lcv++)
    {
        ...
        {
            inst_printf("%s[%d]: failed to find any functions matching %s (lib %s), "
                        "returning failure from installInstrRequests\n",
        ...
        {
            inst_printf(
                "%s[%d]: found %lu functions matching %s (lib %s), instrumenting...\n",
...
bool
PCProcess::postIRPC(void* buffer, int size, void* userData, bool runProcessWhenDone,
...
bool
PCProcess::postIRPC(AstNodePtr action, void* userData, bool runProcessWhenDone,
                    PCThread* thread, bool synchronous, void** result, bool userRPC,
                    bool isMemAlloc, Address addr)
{
    ...
    return postIRPC_internal(irpcBuf.start_ptr(), irpcBuf.used(), breakOffset, resultReg,
                             addr, userData, runProcessWhenDone, thread, synchronous,
                             userRPC, isMemAlloc, result);
}
...
bool
PCProcess::postIRPC_internal(void* buf, unsigned size, unsigned breakOffset,
                             Register resultReg, Address addr, void* userData,
                             bool runProcessWhenDone, PCThread* thread, bool synchronous,
                             bool userRPC, bool isMemAlloc, void** result)
{
    if(isTerminated())
    {
   ...
   while(insn) {
    ...
    if(synchronous)
    {
        // We have an interesting problem here. ProcControl allows callbacks to specify
        // whether the process should stop or run; however, that allows us to stop a
        // process in the middle of an inferior RPC. If that happens, manually execute a
        // continue and wait for completion ourselves.
        if(t)
        ...
        if(!res)
        {
            ...
            {
                proccontrol_printf(
                    "%s[%d]: Iterating in loop waiting for IRPC to complete\n", FILE__,
                    __LINE__);
                if(isTerminated())
                {
                ...
                {
                    proccontrol_printf("%s[%d]: ProcControl reported IRPC thread "
                                       "stopped, continuing and consuming events\n",
                                       FILE__, __LINE__);
...
BPatch_hybridMode
PCProcess::getHybridMode()
{
    return analysisMode_;
}

bool
PCProcess::isExploratoryModeOn() const
{
...
bool
PCProcess::isRuntimeHeapAddr(Address addr) const
{
 ...
 */
bool
PCProcess::getOverwrittenBlocks(
    ...
    std::list<block_instance*>&             writtenBBIs)                    // output
{
    ...
    for(; pIter != overwrittenPages.end(); pIter++)
    {
        ...
        for(unsigned mIdx = 0; mIdx < MEM_PAGE_SIZE; mIdx++)
        {
            ...
            {
                foundStart = false;
                cerr << "\t\t Adding overwritten range " << hex << regionStart << " -> "
...
void
PCProcess::updateCodeBytes(const list<pair<Address, Address>>& owRanges)  // input
{
...
void
PCProcess::flushAddressCache_RT(Address start, unsigned size)
{
    if(start != 0)
    {
    ...
    if(0 == RT_address_cache_addr_)
    {
        ...
        if(vars.size() != 1)
        {
            fprintf(stderr, "%s[%d]:  ERROR:  %d vars matching %s, not 1\n", FILE__,
                    __LINE__, (int) vars.size(), arg_str.c_str());
            assert(0);
        }
    ...
    if(!readDataSpace((void*) RT_address_cache_addr_,
                      sizeof(Address) * TARGET_CACHE_WIDTH, (void*) cacheCopy, false))
    {
        assert(0);
    }
    ...
    {            // and once to flush the flush range
        ...
        for(int idx = 0; idx < TARGET_CACHE_WIDTH; idx++)
        {
            ...
            if(flushStart <= cacheCopy[idx] && flushEnd > cacheCopy[idx])
            {
                cacheCopy[idx] = 0;
            }
        }
 ...
 */
func_instance*
PCProcess::findActiveFuncByAddr(Address addr)
{
    ...
    for(unsigned int i = 0; !foundFrame && i < stacks.size(); ++i)
    {
        ...
        for(unsigned int j = 0; !foundFrame && j < stack.size(); ++j)
        {
            ...
            if(getRelocInfo(framePC, ri) && ri.func)
            {
                frameFunc = ri.func;
            }
            else if(j < (stack.size() - 1))
            {
                ...
                {
                    if(!(*cb_iter)->containsCall())
                        continue;
                    // We have a call point; now see if it called the entry of any
                    // function that maps to a curFunc.
...
bool
PCProcess::patchPostCallArea(instPoint* callPt)
{
...
bool
PCProcess::generateRequiredPatches(instPoint* callPoint, AddrPairSet& patchAreas)
{
        ...
        fit != callFuncs.end(); fit++)
    {
        ...
        {
            // There could be a lot of instrumentation at this point. Bias towards the
            // lowest, non-edge instrumentation
            for(std::map<instPoint*, Address>::iterator inst_iter =
                    reloc.instrumentation.begin();
                inst_iter != reloc.instrumentation.end(); ++inst_iter)
            {
                if(inst_iter->first->type() == PatchAPI::Point::EdgeDuring)
                    continue;
        ...
        {
            map<func_instance*, set<DefensivePad>>::iterator mit =
                forwardDefensiveMap_[callInsnAddr].begin();
            for(; mit != forwardDefensiveMap_[callInsnAddr].end(); ++mit)
            {
                if(callF == mit->first)
                {
                    ...
                    for(; dit != mit->second.end(); ++dit)
                    {
                        ...
                        mal_printf(
                            "patching post-call pad for %lx[%lx] with %lx %s[%d]\n",
...
void
PCProcess::generatePatchBranches(AddrPairSet& branchesNeeded)
{
    for(AddrPairSet::iterator iter = branchesNeeded.begin(); iter != branchesNeeded.end();
        ++iter)
    {
        ...
        if(!writeTextSpace((void*) from, gen.used(), gen.start_ptr()))
        {
            assert(0);
        }
    }
...
std::vector<func_instance*>
PCProcess::pcsToFuncs(std::vector<Frame> stackWalk)
{
...
bool
PCProcess::isInSignalHandler(Address addr)
{
    codeRange* range;
...
void
PCProcess::addSignalHandler(Address addr, unsigned size)
{
...
bool
PCProcess::mappedObjIsDeleted(mapped_object* obj)
{
    for(unsigned i = 0; i < deletedObjects_.size(); ++i)
    {
        if(obj == deletedObjects_[i])
            return true;
    }
...
Architecture
PCProcess::getArch() const
{
    return savedArch_;
}

bool
PCProcess::multithread_ready(bool ignoreIfMtNotSet)
{
    ...
    if(!hasReachedBootstrapState(bs_initialized))
        return false;
    if(!multithread_capable(ignoreIfMtNotSet))
        return false;
...
bool
PCProcess::isInDebugSuicide() const
{
    return isInDebugSuicide_;
}

PCProcess::processState_t
PCProcess::getDesiredProcessState() const
{
    return processState_;
}

void
PCProcess::setDesiredProcessState(PCProcess::processState_t pc)
{
    processState_ = pc;
}

bool
PCProcess::walkStack(std::vector<Frame>& stackWalk, PCThread* thread)
{
    if(stackwalker_ == NULL)
        return false;
    ...
    for(vector<Dyninst::Stackwalker::Frame>::iterator SWB = swWalk.begin(), SWI = SWB,
                                                      SWE = swWalk.end();
        SWI != SWE; ++SWI)
    {
...
bool
PCProcess::getActiveFrame(Frame& frame, PCThread* thread)
{
 ...
 */
bool
PCProcess::triggerStopThread(Address pointAddress, int callbackID, void* calculation)
{
    ...
    mal_printf("handling stopThread %lx[%lx]=>%p %s[%d]\n", ri.reloc, pointAddress,
               calculation, FILE__, __LINE__);
    ...
    {
        callbackID *= -1;
        calculation = (void*) stopThreadCtrlTransfer(intPoint, (Address) calculation);
    }

    /* 3. Trigger the callback for the stopThread
      using the correct snippet instance ID & event type */
    ((BPatch_process*) up_ptr())
        ->triggerStopThread(intPoint, pointfunc, callbackID, (void*) calculation);
 ...
 */
Address
PCProcess::stopThreadCtrlTransfer(instPoint* intPoint, Address target)
{
    ...
    if(intPoint->type() == instPoint::FuncExit && intPoint->block()->isFuncExit() &&
    ...
    if(isRuntimeHeapAddr(target))
    {
        ...
        malware_cerr << "Looking for matches to incoming address " << hex << target << dec
                     << endl;
        std::pair<func_instance*, Address> tmp;
        ...
        mal_printf("translated target %lx to %lx %s[%d]\n", target, unrelocTarget, FILE__,
                   __LINE__);
    }
    else
    {  // target is not relocated, nothing to do but find the
       ...
       // into memory regions that are allocated at runtime
        ...
        if(!obj)
        {
           ...
           for (unsigned i = 0; i < 0x100; ++i) {
            ...
            {
                fprintf(stderr,
                        "ERROR, point %lx has target %lx that responds "
                        "to no object %s[%d]\n",
...
void
PCProcess::triggerNormalExit(int exitcode)
{
...
// Debugging only
bool
PCProcess::setBreakpoint(Address addr)
{
...
Address
PCProcess::getRTEventBreakpointAddr()
{
...
bool
PCProcess::hasRunningSyncRPC() const
{
    return (syncRPCThreads_.size() > 0);
}

void
PCProcess::addSyncRPCThread(Thread::ptr thr)
{
    proccontrol_printf("%s[%d]: added sync rpc thread %d/%d\n", FILE__, __LINE__,
                       getPid(), thr ? thr->getLWP() : 0);
    syncRPCThreads_.insert(thr);
}

void
PCProcess::removeSyncRPCThread(Thread::ptr thr)
{
...
bool
PCProcess::continueSyncRPCThreads()
{
        ...
        i != syncRPCThreads_.end(); ++i)
    {
        if(!(*i))
        {
            ...
            {
                proccontrol_printf(
                    "%s[%d]: failed to continue entire process %d for sync RPC\n", FILE__,
                    __LINE__, getPid());
                return false;
            }
        }
...
void
PCProcess::addTrap(Address from, Address to, codeGen& gen)
{
    gen.invalidate();
    gen.allocate(4);
    ...
    springboard_cerr << "Generated springboard trap " << hex << from << "->" << to << dec
                     << endl;
}
...
{
    map<Address, Breakpoint::ptr>::iterator breakIter = installedCtrlBrkpts.find(from);
    ...
    {
        proccontrol_printf(
            "%s[%d]: failed to remove ctrl transfer breakpoint from 0x%lx\n", FILE__,
            __LINE__, from);
    }
...
void
PCProcess::invalidateMTCache()
{
    mt_cache_result_ = not_cached;
}

bool
PCProcess::supportsUserThreadEvents()
{
    if(!pcProc_)
        return false;
...
bool
StackwalkSymLookup::lookupAtAddr(Dyninst::Address addr, std::string& out_name,
                                 void*& out_value)
{
