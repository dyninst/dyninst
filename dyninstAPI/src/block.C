...
AddressSpace *block_instance::addrSpace() const { return obj()->proc(); }
...
int block_instance::id() const { return llb()->id(); }
...
void block_instance::updateCallTarget(func_instance *func) {
  ...
  if (e && e->sinkEdge()) {
    ...
    mal_printf(
        "WARNING: tried to update the call target of a block "
