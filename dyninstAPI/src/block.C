...
block_instance::block_instance(ParseAPI::Block* ib, mapped_object* obj)
: PatchBlock(ib, obj)
...
AddressSpace*
block_instance::addrSpace() const
{
    return obj()->proc();
}
...
block_instance::getFallthrough()
{
    for(edgelist::const_iterator iter = targets().begin(); iter != targets().end();
        ++iter)
    {
        if((*iter)->type() == FALLTHROUGH || (*iter)->type() == CALL_FT ||
...
block_instance::getTarget()
{
    for(edgelist::const_iterator iter = targets().begin(); iter != targets().end();
        ++iter)
    {
        if((*iter)->type() == CALL || (*iter)->type() == DIRECT ||
...
int
block_instance::id() const
{
    return llb()->id();
}

Address
block_instance::GetBlockStartingAddress()
{
...
void
block_instance::triggerModified()
{
...
void
block_instance::setNotAbruptEnd()
{
    llb()->setAbruptEnd(false);
    vector<func_instance*> funcs;
    getFuncs(std::back_inserter(funcs));
    for(vector<func_instance*>::iterator fit = funcs.begin(); fit != funcs.end(); fit++)
    {
...
void
block_instance::updateCallTarget(func_instance* func)
{
...
func_instance*
block_instance::entryOfFunc() const
{
...
bool
block_instance::isFuncExit() const
{
    parse_block* b = SCAST_PB(llb());
...
func_instance*
block_instance::findFunction(ParseAPI::Function* p)
{
    return obj()->findFunction(p);
}

void*
block_instance::getPtrToInstruction(Address addr) const
{
    if(addr < start())
        return NULL;
    if(addr > end())
