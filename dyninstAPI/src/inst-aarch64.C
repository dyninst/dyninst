...
registerSpace::initialize64()
{
    ...
    for(unsigned idx = r0; idx <= r28; idx++)
    {
        ...
        registers.push_back(new registerSlot(idx, name, false, registerSlot::liveAlways,
                                             registerSlot::GPR));
    }
    // Mark r29 (frame pointer) and r30 (link register) as off-limits
    registers.push_back(
        new registerSlot(r29, "r29", true, registerSlot::liveAlways, registerSlot::GPR));
    registers.push_back(
        new registerSlot(r30, "r30", true, registerSlot::liveAlways, registerSlot::GPR));

    // SPRs
    registers.push_back(
        new registerSlot(lr, "lr", true, registerSlot::liveAlways, registerSlot::SPR));
    registers.push_back(
        new registerSlot(sp, "sp", true, registerSlot::liveAlways, registerSlot::SPR));
    registers.push_back(new registerSlot(pstate, "nzcv", true, registerSlot::liveAlways,
                                         registerSlot::SPR));
    registers.push_back(new registerSlot(fpcr, "fpcr", true, registerSlot::liveAlways,
                                         registerSlot::SPR));
    registers.push_back(new registerSlot(fpsr, "fpsr", true, registerSlot::liveAlways,
                                         registerSlot::SPR));
                                ...
                                int stkOffset)
{
    ...
    map<int, int> sysRegCodeMap =
        map_list_of(SPR_NZCV, 0x5A10)(SPR_FPCR, 0x5A20)(SPR_FPSR, 0x5A21);
    ...
    insnCodeGen::generateMemAccess(gen, insnCodeGen::Store, scratchReg, REG_SP, stkOffset,
                                   4, insnCodeGen::Pre);
}

void
EmitterAARCH64SaveRegs::saveFPRegister(codeGen& gen, Register reg, int save_off)
{
    // Always performing save of the full FP register
    insnCodeGen::generateMemAccessFP(gen, insnCodeGen::Store, reg, REG_SP, save_off, 0,
                                     true);
}
...
unsigned
EmitterAARCH64SaveRegs::saveGPRegisters(codeGen& gen, registerSpace* theRegSpace,
...
unsigned
EmitterAARCH64SaveRegs::saveFPRegisters(codeGen& gen, registerSpace* theRegSpace,
                                        int offset)
{
    unsigned ret = 0;
...
unsigned
EmitterAARCH64SaveRegs::saveSPRegisters(codeGen& gen, registerSpace* theRegSpace,
                                        int offset, bool force_save)
{
        ...
        itr++)
    {
        ...
        saveSPR(gen, theRegSpace->getScratchRegister(gen, true), regMap[cur],
                -4 * GPRSIZE_32);
...
void
EmitterAARCH64SaveRegs::createFrame(codeGen& gen)
{
    // Save link register
...
unsigned
EmitterAARCH64RestoreRegs::restoreGPRegisters(codeGen& gen, registerSpace* theRegSpace,
                                              int offset)
{
    unsigned ret = 0;
...
unsigned
EmitterAARCH64RestoreRegs::restoreFPRegisters(codeGen& gen, registerSpace* theRegSpace,
                                              int offset)
{
    unsigned ret = 0;
...
unsigned
EmitterAARCH64RestoreRegs::restoreSPRegisters(codeGen& gen, registerSpace* theRegSpace,
                                              int, int                     force_save)
{
    int ret = 0;
    ...
    {
        registerSlot* cur = *itr;
        restoreSPR(gen, theRegSpace->getScratchRegister(gen, true), regMap[cur],
                   4 * GPRSIZE_32);
        ret++;
    }
...
void
EmitterAARCH64RestoreRegs::tearFrame(codeGen& gen)
{
...
void
EmitterAARCH64RestoreRegs::restoreSPR(codeGen& gen, Register scratchReg, int sprnum,
                                      int stkOffset)
{
    insnCodeGen::generateMemAccess(gen, insnCodeGen::Load, scratchReg, REG_SP, stkOffset,
                                   4);
...
void
EmitterAARCH64RestoreRegs::restoreFPRegister(codeGen& gen, Register reg, int save_off)
{
    insnCodeGen::generateMemAccessFP(gen, insnCodeGen::Load, reg, REG_SP, save_off, 0,
                                     true);
}
    ...
    switch(op)
    {
        ...
        case minusOp: {
            ...
            insnCodeGen::generateAddSubShifted(
                gen, op == plusOp ? insnCodeGen::Add : insnCodeGen::Sub, 0, 0, rm, src1,
                dest, true);
        }
        break;
        case timesOp: {
        ...
        case xorOp: {
            Register rm = insnCodeGen::moveValueToReg(gen, src2imm);
            insnCodeGen::generateBitwiseOpShifted(gen, insnCodeGen::Eor, 0, rm, 0, src1,
                                                  dest, true);
        }
        break;
        case orOp: {
            Register rm = insnCodeGen::moveValueToReg(gen, src2imm);
            insnCodeGen::generateBitwiseOpShifted(gen, insnCodeGen::Or, 0, rm, 0, src1,
                                                  dest, true);
        }
        break;
        case andOp: {
            Register rm = insnCodeGen::moveValueToReg(gen, src2imm);
            insnCodeGen::generateBitwiseOpShifted(gen, insnCodeGen::And, 0, rm, 0, src1,
                                                  dest, true);
        }
        break;
        case eqOp: {
...
bool
EmitterAARCH64::clobberAllFuncCall(registerSpace* rs, func_instance* callee)
{
    if(!callee)
        return true;
    ...
    if(callee->ifunc()->isLeafFunc())
    {
        ...
        for(std::set<Register>::iterator itr = gpRegs->begin(); itr != gpRegs->end();
            itr++)
...
Register
EmitterAARCH64::emitCallReplacement(opCode, codeGen&, bool, func_instance*)
{
    assert(0);  // Not implemented
    return 0;
}
...
Register
EmitterAARCH64::emitCall(opCode op, codeGen& gen, const std::vector<AstNodePtr>& operands,
                         bool, func_instance* callee)
{
...
codeBufIndex_t
emitA(opCode op, Register src1, Register, long dest, codeGen& gen, RegControl rc, bool)
{
...
emitR(opCode op, Register src1, Register src2, Register dest, codeGen& gen,
      bool /*noCost*/, const instPoint*, bool /*for_MT*/)
{
...
restoreGPRtoGPR(codeGen& gen, Register reg, Register dest)
{
    ...
    if(reg == 31)
    {
        insnCodeGen::generateAddSubImmediate(gen, insnCodeGen::Add, 0, frame_size, REG_SP,
                                             dest, true);
    }
    else
    {
...
static inline void
restoreXERtoGPR(codeGen&, Register)
{
    assert(0);  // Not implemented
}
...
static inline void
moveGPR2531toGPR(codeGen&, Register, Register)
{
    assert(0);  // Not implemented
}
...
void
MovePCToReg(Register dest, codeGen& gen)
{
    instruction insn;
    insn.clear();
           ...
           bool)
{
    ...
    if(rb > -1)
    {
        ...
        insnCodeGen::generateAddSubShifted(gen, insnCodeGen::Add, 0, sc, scratch, dest,
                                           dest, true);
    }
    ...
    if(imm)
        insnCodeGen::generateAddSubImmediate(gen, insnCodeGen::Add, 0, imm, dest, dest,
                                             true);
}

void
emitCSload(const BPatch_addrSpec_NP*, Register, codeGen&, bool)
{
    assert(0);  // Not implemented
}
...
void
emitVstore(opCode op, Register src1, Register /*src2*/, Address dest, codeGen& gen, bool,
           registerSpace* /* rs */, int size, const instPoint* /* location */,
           AddressSpace*)
{
    if(op == storeOp)
    {
...
bool
PCProcess::bindPLTEntry(const SymtabAPI::relocationEntry&, Address, func_instance*,
                        Address)
{
    assert(0);  // Not implemented
    assert(0 && "TODO!");
...
bool
EmitterAARCH64::emitCallRelative(Register, Address, Register, codeGen&)
{
    assert(0);  // Not implemented
    return true;
}
...
bool
EmitterAARCH64::emitLoadRelative(Register dest, Address offset, Register baseReg,
                                 int size, codeGen& gen)
{
    signed long long sOffset = (signed long long) offset;
    if(sOffset >= -256 && sOffset <= 255)
        insnCodeGen::generateMemAccess(gen, insnCodeGen::Load, dest, baseReg, sOffset,
                                       size, insnCodeGen::Pre);
    else
    {
        std::vector<Register> exclude;
        ...
        insnCodeGen::generateAddSubShifted(
            gen, sOffset > 0 ? insnCodeGen::Add : insnCodeGen::Sub, 0, 0, addReg, baseReg,
            baseReg, true);
...
void
EmitterAARCH64::emitStoreRelative(Register source, Address offset, Register base,
                                  int size, codeGen& gen)
{
    if((signed long long) offset <= 255 && (signed long long) offset >= -256)
        insnCodeGen::generateMemAccess(gen, insnCodeGen::Store, source, base, offset,
                                       size, insnCodeGen::Pre);
    else
    {
...
bool
EmitterAARCH64::emitMoveRegToReg(registerSlot*, registerSlot*, codeGen&)
{
    assert(0);  // Not implemented
    return true;
}
...
bool
EmitterAARCH64Stat::emitPLTCommon(func_instance*, bool, codeGen&)
{
    assert(0);  // Not implemented
    return true;
}
...
bool
EmitterAARCH64Dyn::emitTOCCommon(block_instance*, bool, codeGen&)
{
    assert(0);  // Not implemented
    return true;
}
...
bool
EmitterAARCH64Stat::emitPLTCall(func_instance* callee, codeGen& gen)
{
    Address dest      = getInterModuleFuncAddr(callee, gen);
...
bool
EmitterAARCH64Stat::emitPLTJump(func_instance* callee, codeGen& gen)
{
    Address dest      = getInterModuleFuncAddr(callee, gen);
    long    varOffset = dest - gen.currAddr();
...
bool
EmitterAARCH64Stat::emitTOCCall(block_instance* block, codeGen& gen)
{
    assert(0);  // Not implemented
...
bool
EmitterAARCH64Stat::emitTOCJump(block_instance* block, codeGen& gen)
{
    assert(0);  // Not implemented
...
bool
EmitterAARCH64Stat::emitTOCCommon(block_instance*, bool, codeGen&)
{
    assert(0);  // Not implemented
    return false;
}
...
bool
EmitterAARCH64Stat::emitCallInstruction(codeGen&, func_instance*, bool, Address)
{
    assert(0);  // Not implemented
    return true;
}
...
bool
EmitterAARCH64::emitCallInstruction(codeGen&, func_instance*, bool, Address)
{
    assert(0);  // Not implemented
    return true;
}
...
void
EmitterAARCH64::emitLoadShared(opCode op, Register dest, const image_variable* var,
                               bool is_local, int size, codeGen& gen, Address offset)
{
    ...
    {  // loadConstop
        ...
        else
        {
            ...
            insnCodeGen::generateAddSubShifted(
                gen,
                (signed long long) varOffset > 0 ? insnCodeGen::Add : insnCodeGen::Sub, 0,
                0, addReg, baseReg, baseReg, true);
...
void
EmitterAARCH64::emitStoreShared(Register source, const image_variable* var, bool is_local,
                                int size, codeGen& gen)
{
    // create or retrieve jump slot
    Address addr;
    ...
    else
    {
        ...
        // add/sub offset to baseReg
        insnCodeGen::generateAddSubShifted(
            gen, (signed long long) varOffset > 0 ? insnCodeGen::Add : insnCodeGen::Sub,
...
Address
EmitterAARCH64::emitMovePCToReg(Register dest, codeGen& gen)
{
    instruction insn;
    insn.clear();
