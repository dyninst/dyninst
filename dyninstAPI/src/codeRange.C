...
block_instance*
codeRange::is_basicBlockInstance()
{
    return dynamic_cast<block_instance*>(this);
}

func_instance*
codeRange::is_function()
{
    return NULL;
}

parse_func*
codeRange::is_parse_func()
{
    return dynamic_cast<parse_func*>(this);
}

parse_block*
codeRange::is_parse_block()
{
...
mapped_object*
codeRange::is_mapped_object()
{
    return dynamic_cast<mapped_object*>(this);
}

void
codeRangeTree::leftRotate(entry* pivot)
{
    if(!pivot || (pivot == nil))
        return;
    entry* y = pivot->right;
...
void
codeRangeTree::rightRotate(entry* pivot)
{
    if(!pivot || (pivot == nil))
        return;
    entry* x = pivot->left;
...
void
codeRangeTree::deleteFixup(entry* x)
{
...
codeRangeTree::entry*
codeRangeTree::treeSuccessor(entry* x) const
{
    if(!x || (x == nil))
        return NULL;
    if(x->right != nil)
...
void
codeRangeTree::traverse(codeRange** all, entry* node, int& n) const
{
    if(node == nil)
        return;
    if(node->left != nil)
...
void
codeRangeTree::traverse(std::vector<codeRange*>& all, entry* node) const
{
    if(node == nil)
        return;
    if(node->left != nil)
...
void
codeRangeTree::insert(codeRange* value)
{
...
void
codeRangeTree::remove(Address key)
{
...
codeRange**
codeRangeTree::elements(codeRange** buffer) const
{
    if(setData == nil)
        return NULL;
    if(!buffer)
...
bool
codeRangeTree::elements(std::vector<codeRange*>& buffer) const
{
    if(setData == nil)
        return false;
