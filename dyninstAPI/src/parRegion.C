...
const parse_func *image_parRegion::getAssociatedFunc() const {
  return regionIf_;
}
...
void image_parRegion::decodeClauses(int bitmap) {
  /* Par do always evaluates to '0' for power, which could be quite misleading
     to someone asking for the value, so
     for this purpose we just have it always return '1', as in, the thing is
     executed.  In the cases that
     someone actually does put something like if(0) in the code, the par do
     section isn't even generated,
     so this becomes moot */
  if (regionType == OMP_PAR_DO) clauses["IF"] = 1;
  ...
  if (regionType == OMP_DO_FOR || regionType == OMP_SECTIONS ||
      regionType == OMP_SINGLE)
...
int_parRegion::int_parRegion(image_parRegion *ip, Address baseAddr,
                             func_instance *iFunc) {
  ip_ = ip;
...
int int_parRegion::getClause(const char *key) { return ip_->getClause(key); }
...
int int_parRegion::replaceOMPParameter(const char *key, int value) {
  ...
  if (value > 0) writeValue += (unsigned)value;
