...
const parse_func*
image_parRegion::getAssociatedFunc() const
{
    return regionIf_;
}

void
image_parRegion::printDetails()
{
    printf("\n********************************************************\n");
...
void
image_parRegion::decodeClauses(int bitmap)
{
    /* Par do always evaluates to '0' for power, which could be quite misleading to
       someone asking for the value, so for this purpose we just have it always return
       '1', as in, the thing is executed.  In the cases that someone actually does put
       something like if(0) in the code, the par do section isn't even generated,
...
void
image_parRegion::setRegionType(parRegType rt)
{
    regionType = rt;
...
void
image_parRegion::setClause(const char* key, int value)
{
    clauses[key] = value;
}

void
image_parRegion::setClauseLoc(const char* key, Address value)
{
    clauses[key] = value;
}
...
int
int_parRegion::getClause(const char* key)
{
    return ip_->getClause(key);
}

int
image_parRegion::getClause(const char* key)
{
...
Address
int_parRegion::getClauseLoc(const char* key)
{
    return ip_->getClauseLoc(key);
}

Address
image_parRegion::getClauseLoc(const char* key)
{
    if(clauses.find(key) != clauses.end())
        return clauses[key];
    else
...
int
int_parRegion::replaceOMPParameter(const char* key, int value)
{
