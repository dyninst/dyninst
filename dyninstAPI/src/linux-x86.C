...
Address
PCProcess::getLibcStartMainParam(PCThread* trappingThread)
{
    Address  mainaddr  = 0;
...
Address
PCProcess::getTOCoffsetInfo(func_instance*)
{
    assert(!"This function is unimplemented");
    return 0;
}
...
PCProcess::createUnprotectStackAST()
{
    ...
    startup_printf("%s[%d]: creating AST to call mprotect to unprotect libc stack "
                   "protection variable\n",
                   FILE__, __LINE__);
