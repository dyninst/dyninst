...
Address PCProcess::getLibcStartMainParam(PCThread *trappingThread) {
  ...
  if (!readDataSpace((const void *)targetAddr, addrWidth, (void *)&mainaddr,
                     false)) {
...
bool PCProcess::getOPDFunctionAddr(Address &) { return true; }
...
AstNodePtr PCProcess::createUnprotectStackAST() {
  ...
  startup_printf(
      "%s[%d]: creating AST to call mprotect to unprotect libc stack "
      "protection variable\n",
      FILE__, __LINE__);
  ...
  if (!ret || vars.size() == 0) {
    ...
    startup_printf(
        "%s[%d]: Warning: found more than one __stack_prot variable\n", FILE__,
        __LINE__);
  }
  ...
  args.push_back(
      AstNode::operandNode(AstNode::Constant, (void *)(intptr_t)size));
...
bool PCProcess::bindPLTEntry(const SymtabAPI::relocationEntry &entry,
                             Address base_addr, func_instance *,
                             Address target_addr) {
...
bool AddressSpace::getDyninstRTLibName() {
  ...
  dyninstRT_name = std::string(name, split - name) + std::string(modifier) +
                   std::string(suffix);
