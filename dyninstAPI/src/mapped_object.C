...
mapped_object::mapped_object(fileDescriptor fileDesc, image *img,
                             AddressSpace *proc, BPatch_hybridMode mode)
  ...
  startup_printf("[%s:%d] \tcodeBase 0x%lx, codeOffset 0x%lx, size %d\n",
                 FILE__, __LINE__, codeBase_, image_->imageOffset(),
                 image_->imageLength());
  startup_printf("[%s:%d] \tdataBase 0x%lx, dataOffset 0x%lx, size %d\n",
                 FILE__, __LINE__, dataBase_, image_->dataOffset(),
                 image_->dataLength());
...
mapped_object *mapped_object::createMappedObject(fileDescriptor &desc,
  ...
  startup_printf("%s[%d]: name %s, codeBase 0x%lx, dataBase 0x%lx\n", FILE__,
                 __LINE__, desc.file().c_str(), desc.code(), desc.data());
  ...
  // and then correct.
  if (desc.dynamic() && p->getAddressWidth() == 4 &&
    ...
    startup_printf(
        "[%s:%u] - Incorrect binary load address %lx, changing "
        "to %lx\n",
  ...
  if (!desc.isSharedObject()) {
    ...
    // when libc.so is loaded -- this is unnecessary now that we use
    // ProcControlAPI
    //
    // This is now done on-demand when libc is loaded and main has yet to be
    // found
  }
...
mapped_object::mapped_object(const mapped_object *s, AddressSpace *child)
  ...
  for (unsigned k = 0; k < s->everyModule.size(); k++) {
    ...
    mapped_module *mod =
        mapped_module::createMappedModule(this, parMod->pmod());
    assert(mod);
...
mapped_object::~mapped_object() {
  ...
  for (unsigned i = 0; i < everyModule.size(); i++) delete everyModule[i];
  everyModule.clear();
...
Address mapped_object::codeAbs() const { return codeBase_ + imageOffset(); }

Address mapped_object::dataAbs() const { return dataBase_ + dataOffset(); }
...
bool mapped_object::analyze() {
  ...
  for (; fit != allFuncs.end(); ++fit) {
    ...
    if ((*fit)->src() != HINT) findFunction((parse_func *)*fit);
  }
...
mapped_module *mapped_object::findModule(pdmodule *pdmod) {
  if (!pdmod) {
    fprintf(stderr,
            "%s[%d]:  please call this findModule with nonNULL parameter\n",
            FILE__, __LINE__);
    return NULL;
  }
...
const pdvector<func_instance *> *mapped_object::findFuncVectorByPretty(
    const std::string &funcname) {
  if (funcname.c_str() == 0) return NULL;
  // First, check the underlying image.
  const pdvector<parse_func *> *img_funcs =
  ...
  // Slow path: check each img_func, add those we don't already have, and
  // return.
...
const pdvector<func_instance *> *mapped_object::findFuncVectorByMangled(
    const std::string &funcname) {
  if (funcname.c_str() == 0) return NULL;
...
const pdvector<int_variable *> *mapped_object::findVarVectorByPretty(
    const std::string &varname) {
  if (varname.c_str() == 0) return NULL;

  // First, check the underlying image.
  const pdvector<image_variable *> *img_vars =
  ...
  if (iter != allVarsByPrettyName.end()) {
    // Okay, we've pulled in some of the variabletions before (this can happen
    // as a
  ...
  // Slow path: check each img_variable, add those we don't already have, and
  // return.
...
const pdvector<int_variable *> *mapped_object::findVarVectorByMangled(
    const std::string &varname) {
  if (varname.c_str() == 0) return NULL;
...
bool mapped_object::findBlocksByAddr(const Address addr,
  ...
  for (set<ParseAPI::Block *>::iterator llb_iter = stab.begin();
    ...
    for (std::vector<ParseAPI::Function *>::iterator llf_iter =
             ll_funcs.begin();
...
bool mapped_object::findFuncsByAddr(const Address addr,
...
const pdvector<mapped_module *> &mapped_object::getModules() {
  ...
  if (everyModule.size() == pdmods.size()) return everyModule;
...
void mapped_object::addFunctionName(func_instance *func,
  ...
  if (std::find(funcsByName->begin(), funcsByName->end(), func) ==
      funcsByName->end()) {
...
const std::string mapped_object::debugString() const {
  std::string debug;
  debug = std::string(fileName_.c_str()) + ":" + utos(codeBase_) + "/" +
          utos(imageSize());
  return debug;
}
...
bool mapped_object::getInfHeapList(pdvector<heapDescriptor> &infHeaps) {
  ...
  for (u_int j = 0; j < foundHeaps.size(); j++) {
    ...
    if (!strcmp(heap_type_str, "anyHeap"))
      ...
      cerr << "Unknown heap string " << heap_type_str << " read from file!"
           << endl;
      free(temp_str);
      continue;
    }
    infHeaps.push_back(heapDescriptor(foundHeaps[j].first.c_str(),
                                      foundHeaps[j].second, heap_size,
                                      heap_type));
    free(temp_str);
  }
...
void mapped_object::getInferiorHeaps(
    vector<pair<string, Address> > &foundHeaps) {
  ...
  for (unsigned i = 0; i < code_heaps.size(); i++) {
    foundHeaps.push_back(pair<string, Address>(
        code_heaps[i].first, code_heaps[i].second + codeBase()));
  }
  for (unsigned i = 0; i < data_heaps.size(); i++) {
    foundHeaps.push_back(pair<string, Address>(
        data_heaps[i].first, data_heaps[i].second + dataBase()));
  }
}
...
bool mapped_object::findBlocksByRange(
...
void mapped_object::findFuncsByRange(Address startAddr, Address endAddr,
...
bool mapped_object::parseNewEdges(const std::vector<edgeStub> &stubs) {
  ...
  for (unsigned idx = 0; idx < stubs.size(); idx++) {
    ...
    if (targ_obj->findBlockByEntry(stubs[idx].trg)) {
      cerr << "KEVINTEST: VERIFY THAT I WORK: parsing edge for target that "
              "already exists"
           << endl;
  ...
  for (unsigned fidx = 0; fidx < modIFuncs.size(); fidx++) {
    ...
    // func->ifunc()->invalidateCache();//KEVINTEST: used to call this, which
...
void mapped_object::expandCodeBytes(SymtabAPI::Region *reg) {
  ...
  if (codeBase() + reg->getMemOffset() + reg->getMemSize() >
      codeAbs() + get_size()) {
    parse_img()->setImageLength(codeBase() + reg->getMemOffset() +
                                reg->getMemSize() - codeAbs());
  }
}
...
void mapped_object::updateCodeBytes(
    const list<pair<Address, Address> > &owRanges) {
  ...
  for (; rIter != owRanges.end(); rIter++) {
    ...
    Region *curReg =
        parse_img()->getObject()->findEnclosingRegion(lastChangeOffset);
  ...
  for (unsigned int ridx = 0; ridx < allregions.size(); ridx++) {
    ...
    if (expandRegs.end() == expandRegs.find(curreg)) {
      updateCodeBytes(curreg);  // KEVINOPTIMIZE: major overkill here, only
                                // update regions that had unprotected pages
    }
  }
...
bool mapped_object::isUpdateNeeded(Address entry) {
  ...
  void *mappedPtr =
      (void *)((Address)creg->symRegion()->getPtrToRawData() +
               (entry - base - creg->symRegion()->getMemOffset()));
  // compare
...
bool mapped_object::isExpansionNeeded(Address entry) {
  ...
  if (proc()->isMemoryEmulated()) {
    bool valid = false;
    boost::tie(valid, compareStart) =
        proc()->getMemEm()->translate(compareStart);
    assert(valid);
  }
  unsigned compareSize =
...
bool mapped_object::updateCodeBytesIfNeeded(Address entry) {
  ...
  if (pagesUpdated_) {
    // cerr << "\t No pages have been updated in mapped_object, ret false" <<
    // endl;
    return false;
  }
  ...
  mal_printf("%s[%d] updating region [%lx %lx] for entry point %lx\n", FILE__,
             __LINE__, reg->getMemOffset(),
             reg->getMemOffset() + reg->getDiskSize(), entry);
...
// does not delete
void mapped_object::destroy(PatchAPI::PatchFunction *f) { remove(SCAST_FI(f)); }
...
bool mapped_object::isSystemLib(const std::string &objname) {
  ...
  if (std::string::npos != lowname.find("libdyninstapi_rt")) return true;

#if defined(os_linux)
  if (std::string::npos != lowname.find("libc.so")) return true;
  if (std::string::npos != lowname.find("libpthread")) return true;
#endif

#if defined(os_freebsd)
  if (std::string::npos != lowname.find("libc.so")) return true;
  if (std::string::npos != lowname.find("libthr")) return true;
#endif
  ...
  if (std::string::npos != lowname.find("windows\\system32\\") &&
    ...
    return true;
  if (std::string::npos != lowname.find("kernel32.dll")) return true;
  if (std::string::npos != lowname.find("user32.dll")) return true;
  if (std::string::npos != lowname.find("advapi32.dll")) return true;
  if (std::string::npos != lowname.find("ntdll.dll")) return true;
...
void mapped_object::setEmulInsnVal(Address insnAddr, void *val) {
  ...
  emulInsns_[insnAddr] =
      pair<Register, void *>(emulInsns_[insnAddr].first, val);
}
...
#if defined(os_windows)
  string calleeName;
  if (parse_img()->codeObject()->isIATcall(b->last() - codeBase(),
                                           calleeName)) {
...
edge_instance *mapped_object::findEdge(ParseAPI::Edge *e, block_instance *src,
                                       block_instance *trg) {
...
void mapped_object::splitBlock(block_instance *b1, block_instance *b2) {
...
void mapped_object::replacePLTStub(SymtabAPI::Symbol *sym, func_instance *orig,
                                   Address newAddr) {
