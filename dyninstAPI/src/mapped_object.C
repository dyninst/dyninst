...
bool
codeBytesUpdateCB(void* objCB, Address targ)
{
    mapped_object* obj = (mapped_object*) objCB;
    return obj->updateCodeBytesIfNeeded(targ);
}

mapped_object::mapped_object(fileDescriptor fileDesc, image* img, AddressSpace* proc,
...
, memoryImg_(false)
{
    ...
    startup_printf("[%s:%d] \tcodeBase 0x%lx, codeOffset 0x%lx, size %lu\n", FILE__,
                   __LINE__, codeBase_, image_->imageOffset(), image_->imageLength());
    startup_printf("[%s:%d] \tdataBase 0x%lx, dataOffset 0x%lx, size %lu\n", FILE__,
                   __LINE__, dataBase_, image_->dataOffset(), image_->dataLength());
    startup_printf("[%s:%d] \tfileDescriptor: code at 0x%lx, data 0x%lx\n", FILE__,
                   __LINE__, fileDesc.code(), fileDesc.data());
    startup_printf("[%s:%d] \tCode: 0x%lx to 0x%lx\n", FILE__, __LINE__, codeAbs(),
                   codeAbs() + imageSize());
    startup_printf("[%s:%d] \tData: 0x%lx to 0x%lx\n", FILE__, __LINE__, dataAbs(),
                   dataAbs() + dataSize());
...
mapped_object*
mapped_object::createMappedObject(Library::const_ptr lib, AddressSpace* p,
                                  BPatch_hybridMode analysisMode, bool parseGaps)
{
    fileDescriptor desc(lib->getAbsoluteName(), lib->getLoadAddress(),
                        p->usesDataLoadAddress() ? lib->getDataLoadAddress()
                                                 : lib->getLoadAddress());
    return createMappedObject(desc, p, analysisMode, parseGaps);
}

mapped_object*
mapped_object::createMappedObject(fileDescriptor& desc, AddressSpace* p,
                                  BPatch_hybridMode analysisMode, bool parseGaps)
{
    if(!p)
        return NULL;
    ...
    startup_printf("%s[%d]: name %s, codeBase 0x%lx, dataBase 0x%lx\n", FILE__, __LINE__,
                   desc.file().c_str(), desc.code(), desc.data());
    ...
    if(img->codeObject()->cs()->getArch() == Arch_ppc64)
    {
        ...
        for(; fit != allFuncs.end(); ++fit)
        {
            ...
            if(f->getNoPowerPreambleFunc() != NULL)
            {
                ...
                func_instance* noPreambleFunc =
                    obj->findFunction(f->getNoPowerPreambleFunc());
...
, memoryImg_(s->memoryImg_)
{
    ...
    for(auto iter = s->everyUniqueVariable.begin(); iter != s->everyUniqueVariable.end();
        ++iter)
    {
...
mapped_object::~mapped_object()
{
    ...
    for(auto iter = everyUniqueVariable.begin(); iter != everyUniqueVariable.end();
        ++iter)
    {
        delete iter->second;
    }
    ...
    for(auto vp_iter = allVarsByPrettyName.begin(); vp_iter != allVarsByPrettyName.end();
        ++vp_iter)
    {
...
bool
mapped_object::isCode(Address addr) const
{
    Address offset;
...
bool
mapped_object::isData(Address addr) const
{
    Address offset;
    offset = addr - codeBase();
...
mapped_module*
mapped_object::findModule(string m_name, bool wildcard)
{
    parsing_printf("findModule for %s (substr match %d)\n", m_name.c_str(), wildcard);
    std::string tmp = m_name.c_str();
    for(unsigned i = 0; i < everyModule.size(); i++)
    {
        if(everyModule[i]->fileName() == m_name || everyModule[i]->fullName() == m_name ||
           (wildcard && (wildcardEquiv(tmp, everyModule[i]->fileName()) ||
    ...
    if(pdmod)
    {
        mapped_module* mod = mapped_module::createMappedModule(this, pdmod);
        everyModule.push_back(mod);
...
mapped_module*
mapped_object::findModule(pdmodule* pdmod)
{
    if(!pdmod)
    {
    ...
    {
        fprintf(stderr,
                "%s[%d]: WARNING: lookup for module in wrong mapped object! %p != %p\n",
                FILE__, __LINE__, (void*) pdmod->imExec(), (void*) parse_img());
        fprintf(stderr, "%s[%d]:  \t\t %s \n", FILE__, __LINE__,
                parse_img()->name().c_str());
        fprintf(stderr, "%s[%d]:  \t %s != \n", FILE__, __LINE__,
                pdmod->imExec()->name().c_str());
        return NULL;
    }
...
mapped_object::findFuncVectorByMangled(const std::string& funcname)
{
    ...
    // First, check the underlying image.
    const std::vector<parse_func*>* img_funcs =
        parse_img()->findFuncVectorByMangled(funcname);
    if(img_funcs == NULL)
    {
        return NULL;
    }
...
mapped_object::findVarVectorByPretty(const std::string& varname)
{
    ...
    const std::vector<image_variable*>* img_vars =
        parse_img()->findVarVectorByPretty(varname);
    if(img_vars == NULL)
        return NULL;
...
mapped_object::findVarVectorByMangled(const std::string& varname)
{
    ...
    // First, check the underlying image.
    const std::vector<image_variable*>* img_vars =
        parse_img()->findVarVectorByMangled(varname);
    if(img_vars == NULL)
        return NULL;
...
const int_variable*
mapped_object::getVariable(const std::string& varname)
{
    const std::vector<int_variable*>* vars = NULL;
    vars                                   = findVarVectorByPretty(varname);
    if(!vars)
        vars = findVarVectorByMangled(varname);
    if(vars)
    {
        assert(vars->size() > 0);
        return (*vars)[0];
...
block_instance*
mapped_object::findBlockByEntry(Address addr)
{
    std::set<block_instance*> allBlocks;
    if(!findBlocksByAddr(addr, allBlocks))
        return NULL;
...
bool
mapped_object::findBlocksByAddr(const Address              addr,
                                std::set<block_instance*>& return_blocks)
{
    // Quick bounds check...
    if(addr < codeAbs())
    {
    ...
    for(set<ParseAPI::Block*>::iterator llb_iter = stab.begin(); llb_iter != stab.end();
        ++llb_iter)
    {
...
bool
mapped_object::findFuncsByAddr(const Address addr, std::set<func_instance*>& return_funcs)
{
    bool ret = false;
    // Quick and dirty implementation
    std::set<block_instance*> found_blocks;
    if(!findBlocksByAddr(addr, found_blocks))
        return false;
...
func_instance*
mapped_object::findFuncByEntry(const Address addr)
{
    std::set<func_instance*> found_funcs;
    if(!findFuncsByAddr(addr, found_funcs))
        return NULL;
...
const std::vector<mapped_module*>&
mapped_object::getModules()
{
...
bool
mapped_object::getAllFunctions(std::vector<func_instance*>& return_funcs)
{
...
bool
mapped_object::getAllVariables(std::vector<int_variable*>& vars)
{
...
func_instance*
mapped_object::findFunction(ParseAPI::Function* papi_func)
{
    return SCAST_FI(getFunc(papi_func));
}

void
mapped_object::addFunctionName(func_instance* func, const std::string newName,
                               func_index_t& index)
{
    ...
    if(std::find(funcsByName->begin(), funcsByName->end(), func) == funcsByName->end())
    {
...
void
mapped_object::addFunction(func_instance* func)
{
...
int_variable*
mapped_object::findVariable(image_variable* img_var)
{
    if(!img_var)
        return NULL;
...
void
mapped_object::addVariable(int_variable* var)
{
    // Possibly multiple demangled (pretty) names...
    // And multiple functions (different addr) with the same pretty
    // name. So we have a many::many mapping...
    ...
    for(auto pretty_iter = var->pretty_names_begin();
        pretty_iter != var->pretty_names_end(); pretty_iter++)
    {
        ...
        if(std::find(varsByPrettyEntry->begin(), varsByPrettyEntry->end(), var) ==
    ...
    for(auto symtab_iter = var->symtab_names_begin();
        symtab_iter != var->symtab_names_end(); symtab_iter++)
    {
        ...
        if(std::find(varsBySymTabEntry->begin(), varsBySymTabEntry->end(), var) ==
...
AddressSpace*
mapped_object::proc() const
{
    return proc_;
}

bool
mapped_object::isSharedLib() const
{
    if(isMemoryImg())
        return false;
...
bool
mapped_object::isStaticExec() const
{
    return parse_img()->getObject()->isStaticBinary();
}

const std::string
mapped_object::debugString() const
{
    std::string debug;
    debug =
        std::string(fileName_.c_str()) + ":" + utos(codeBase_) + "/" + utos(imageSize());
    return debug;
}

// Search an object for heapage
bool
mapped_object::getInfHeapList(std::vector<heapDescriptor>& infHeaps)
{
    ...
    for(u_int j = 0; j < foundHeaps.size(); j++)
    {
        ...
        {
            cerr << "Skipping heap " << foundHeaps[j].first.c_str() << "with address 0"
                 << endl;
            continue;
        }
        ...
        infHeaps.push_back(heapDescriptor(foundHeaps[j].first.c_str(),
                                          foundHeaps[j].second, heap_size, heap_type));
        free(temp_str);
    }
...
void
mapped_object::getInferiorHeaps(vector<pair<string, Address>>& foundHeaps)
{
...
void*
mapped_object::getPtrToInstruction(Address addr) const
{
    if(!isCode(addr))
        return NULL;
...
void*
mapped_object::getPtrToData(Address addr) const
{
    assert(addr >= dataAbs());
...
void*
mapped_object::get_local_ptr() const
{
...
bool
mapped_object::getSymbolInfo(const std::string& n, int_symbol& info)
{
...
mapped_module*
mapped_object::getOrCreateForkedModule(mapped_module* parMod)
{
...
bool
mapped_object::findBlocksByRange(Address startAddr, Address endAddr,
...
void
mapped_object::findFuncsByRange(Address startAddr, Address endAddr,
                                std::set<func_instance*>& pageFuncs)
{
    std::list<block_instance*> bbls;
    findBlocksByRange(startAddr, endAddr, bbls);
    for(std::list<block_instance*>::iterator iter = bbls.begin(); iter != bbls.end();
        ++iter)
    {
 ...
 */
bool
mapped_object::parseNewFunctions(vector<Address>& funcEntryAddrs)
{
    ...
    while(curEntry != funcEntryAddrs.end())
    {
        ...
        if(reg != NULL)
        {
            ...
            {
                fprintf(stderr,
                        "WARNING: tried to parse at %lx, where a "
        ...
        {
            fprintf(stderr,
                    "ERROR: passed invalid address %lx to "
    ...
    while(curEntry != funcEntryAddrs.end())
    {
        ...
        if(!parse_img()->findFuncs(entryOffset, tmpfuncs))
        {
            ...
            fprintf(stderr,
                    "WARNING, failed to parse function at %lx, "
                    "%s[%d]\n",
                    *curEntry, FILE__, __LINE__);
        }
        else
 ...
 */
bool
mapped_object::parseNewEdges(const std::vector<edgeStub>& stubs)
{
    ...
    for(unsigned idx = 0; idx < stubs.size(); idx++)
    {
        ...
        {
            cerr << "KEVINTEST: VERIFY THAT I WORK: parsing edge for target that already "
                    "exists"
                 << endl;
    ...
    for(unsigned fidx = 0; fidx < modIFuncs.size(); fidx++)
    {
        ...
        // func->ifunc()->invalidateCache();//KEVINTEST: used to call this, which might
        // have been important
 ...
 */
void
mapped_object::expandCodeBytes(SymtabAPI::Region* reg)
{
    assert(reg);
    ...
    if(!proc()->isMemoryEmulated())
    {
        ...
        while(cur != NULL && cur->start() < initializedEnd)
        {
    ...
    static_cast<SymtabCodeSource*>(cObj->cs())->resizeRegion(reg, reg->getMemSize());
    reg->setPtrToRawData(regBuf, copySize);

    // expand this mapped_object's codeRange
    if(codeBase() + reg->getMemOffset() + reg->getMemSize() > codeAbs() + get_size())
    {
        parse_img()->setImageLength(codeBase() + reg->getMemOffset() + reg->getMemSize() -
                                    codeAbs());
    }
}
...
mapped_object::updateCodeBytes(const list<pair<Address, Address>>& owRanges)
{
    ...
    for(; rIter != owRanges.end(); rIter++)
    {
        ...
        Region* curReg = parse_img()->getObject()->findEnclosingRegion(lastChangeOffset);
    ...
    for(set<Region*>::iterator regIter = expandRegs.begin(); regIter != expandRegs.end();
        regIter++)
    {
    ...
    for(unsigned int ridx = 0; ridx < allregions.size(); ridx++)
    {
        ...
        {
            updateCodeBytes(curreg);  // KEVINOPTIMIZE: major overkill here, only update
                                      // regions that had unprotected pages
        }
    }
    ...
    for(rIter = owRanges.begin(); rIter != owRanges.end(); rIter++)
    {
        ...
        unsigned char* regPtr = (unsigned char*) reg->getPtrToRawData() + (*rIter).first -
                                baseAddress - reg->getMemOffset();

        if(!proc()->readDataSpace((void*) readAddr, (*rIter).second - (*rIter).first,
                                  regPtr, true))
        {
            assert(0);
        }
...
void
mapped_object::updateCodeBytes(SymtabAPI::Region* symReg)
{
    assert(NULL != symReg);
    ...
    while(curB != NULL && curB->start() < regStart + symReg->getDiskSize())
    {
    ...
    for(map<Address, WriteableStatus>::iterator pit = protPages_.find(curPage);
        pit != protPages_.end() && pit->first < regEnd; pit++)
    {
...
bool
mapped_object::isUpdateNeeded(Address entry)
{
    using namespace ParseAPI;
    ...
    void* mappedPtr = (void*) ((Address) creg->symRegion()->getPtrToRawData() +
                               (entry - base - creg->symRegion()->getMemOffset()));
    // compare
...
bool
mapped_object::isExpansionNeeded(Address entry)
{
    using namespace SymtabAPI;
    Address base = codeBase();
    ...
    if(expansionCheckedRegions_.end() != expansionCheckedRegions_.find(reg))
    {
        return false;
    }
    ...
    {
        fprintf(stderr, "%s[%d] Failed to read from region [%lX %lX]\n", __FILE__,
                __LINE__, compareStart, compareStart + compareSize);
        assert(0);
    }
...
bool
mapped_object::updateCodeBytesIfNeeded(Address entry)
{
    ...
    Address pageAddr = entry - (entry % proc()->proc()->getMemoryPageSize());
    ...
    if(protPages_.end() != protPages_.find(pageAddr) && PROTECTED == protPages_[pageAddr])
    {
    ...
    SymtabAPI::Region* reg =
        parse_img()->getObject()->findEnclosingRegion(entry - codeBase());
    mal_printf("%s[%d] updating region [%lx %lx] for entry point %lx\n", FILE__, __LINE__,
               reg->getMemOffset(), reg->getMemOffset() + reg->getDiskSize(), entry);
...
mapped_object::remove(func_instance* func)
{
    ...
    for(auto name_iter = func->symtab_names_begin();
        name_iter != func->symtab_names_end(); ++name_iter)
    {
        auto map_iter = allFunctionsByMangledName.find(*name_iter);
        if(map_iter == allFunctionsByMangledName.end())
            continue;
    ...
    for(auto name_iter = func->pretty_names_begin();
        name_iter != func->pretty_names_end(); ++name_iter)
    {
        auto map_iter = allFunctionsByPrettyName.find(*name_iter);
        if(map_iter == allFunctionsByPrettyName.end())
            continue;
...
void
mapped_object::remove(instPoint* point)
{
    BPatch_addressSpace* bpAS  = (BPatch_addressSpace*) proc()->up_ptr();
...
// does not delete
void
mapped_object::destroy(PatchAPI::PatchFunction* f)
{
    remove(SCAST_FI(f));
}

void
mapped_object::removeEmptyPages()
{
...
bool
mapped_object::isSystemLib(const std::string& objname)
{
    ...
    if(std::string::npos != lowname.find(".dll"))
        return true;  // KEVINTODO: find a reliable way of detecting windows system
                      // libraries
#endif
...
bool
mapped_object::isExploratoryModeOn()
{
...
void
mapped_object::addProtectedPage(Address pageAddr)
{
...
void
mapped_object::removeProtectedPage(Address pageAddr)
{
    map<Address, WriteableStatus>::iterator iter = protPages_.find(pageAddr);
    if(iter == protPages_.end())
    {
        ...
        SymtabAPI::Region* reg =
            parse_img()->getObject()->findEnclosingRegion(pageOffset);
        assert(reg);
        set<CodeRegion*> cregs;
        parse_img()->codeObject()->cs()->findRegions(reg->getMemOffset(), cregs);
        if(!cregs.empty())
        {  // (if empty, pageAddr is in uninitialized memory)
            ParseAPI::Block* blk =
                parse_img()->codeObject()->findNextBlock(*cregs.begin(), pageOffset);
...
void
mapped_object::setCodeBytesUpdated(bool newval)
{
    if(BPatch_defensiveMode == analysisMode_)
    {
    ...
    else
    {
        cerr << "WARNING: requesting update of code bytes from memory "
             << "on non-defensive mapped object, ignoring request " << fileName().c_str()
...
func_instance*
mapped_object::findGlobalConstructorFunc(const std::string&)
{
...
func_instance*
mapped_object::findGlobalDestructorFunc(const std::string&)
{
    assert(!"Not implemented");
    return NULL;
}
...
bool
mapped_object::isEmulInsn(Address insnAddr)
{
    return (emulInsns_.end() != emulInsns_.find(insnAddr));
}

void
mapped_object::setEmulInsnVal(Address insnAddr, void* val)
{
...
Register
mapped_object::getEmulInsnReg(Address insnAddr)
{
    assert(emulInsns_.end() != emulInsns_.find(insnAddr));
...
void
mapped_object::addEmulInsn(Address insnAddr, Register effectiveAddrReg)
{
    emulInsns_[insnAddr] = pair<Register, void*>(effectiveAddrReg, (void*) 0);
}

std::string
mapped_object::getCalleeName(block_instance* b)
{
    std::map<block_instance*, std::string>::iterator iter = calleeNames_.find(b);
    if(iter != calleeNames_.end())
        return iter->second;
...
void
mapped_object::setCalleeName(block_instance* b, std::string s)
{
    calleeNames_[b] = s;
}
...
edge_instance*
mapped_object::findEdge(ParseAPI::Edge* e, block_instance* src, block_instance* trg)
{
...
block_instance*
mapped_object::findBlock(ParseAPI::Block* b)
{
    return SCAST_BI(getBlock(b));
}

block_instance*
mapped_object::findOneBlockByAddr(const Address addr)
{
...
void
mapped_object::splitBlock(block_instance* b1, block_instance* b2)
{
...
func_instance*
mapped_object::findFuncByEntry(const block_instance* blk)
{
...
func_instance*
mapped_object::getCallee(const block_instance* b) const
{
    std::map<const block_instance*, func_instance*>::const_iterator iter =
        callees_.find(b);
    if(iter == callees_.end())
        return NULL;
    return iter->second;
}

void
mapped_object::setCallee(const block_instance* b, func_instance* f)
{
    callees_[b] = f;
}
...
void
mapped_object::replacePLTStub(SymtabAPI::Symbol* sym, func_instance* orig,
                              Address newAddr)
{
