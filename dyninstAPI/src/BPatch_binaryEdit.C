...
BPatch_binaryEdit::BPatch_binaryEdit(const char *path, bool openDependencies)
    : BPatch_addressSpace(), creation_error(false) {
  ...
  // Load the RT library and create the collection of BinaryEdits that represent
  // it
  ...
  for (rtlibs_it = rtlibs.begin(); rtlibs_it != rtlibs.end(); ++rtlibs_it) {
    if (!rtlibs_it->second) {
      std::string msg(
          "Failed to load Dyninst runtime library, check the environment "
          "variable DYNINSTAPI_RT_LIB");
...
BPatch_image *BPatch_binaryEdit::getImage() { return image; }
...
bool BPatch_binaryEdit::writeFile(const char *outFile) {
  ...
  for (curBinEdit = llBinEdits.begin(); curBinEdit != llBinEdits.end();
       curBinEdit++) {
    BinaryEdit *bin = (*curBinEdit).second;
    if (bin == origBinEdit) continue;
    if (!bin->isDirty()) continue;
...
processType BPatch_binaryEdit::getType() { return STATIC_EDITOR; }

void BPatch_binaryEdit::getAS(std::vector<AddressSpace *> &as) {
  std::map<std::string, BinaryEdit *>::iterator i = llBinEdits.begin();
  as.push_back(origBinEdit);
  for (; i != llBinEdits.end(); i++) {
    if ((*i).second == origBinEdit) continue;
...
void BPatch_binaryEdit::beginInsertionSet() { return; }

bool BPatch_binaryEdit::finalizeInsertionSet(bool /*atomic*/,
                                             bool * /*modified*/) {
  return true;
}
