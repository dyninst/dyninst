...
, creation_error(false)
{
    ...
    for(rtlibs_it = rtlibs.begin(); rtlibs_it != rtlibs.end(); ++rtlibs_it)
    {
        if(!rtlibs_it->second)
        {
            std::string msg("Failed to load Dyninst runtime library, check the "
                            "environment variable DYNINSTAPI_RT_LIB");
...
bool
BPatch_binaryEdit::isMultiThreadCapable() const
{
...
bool
BPatch_binaryEdit::writeFile(const char* outFile)
{
    ...
    if(!origBinEdit->writeFile(outFile))
        return false;
...
processType
BPatch_binaryEdit::getType()
{
    return STATIC_EDITOR;
}

void
BPatch_binaryEdit::getAS(std::vector<AddressSpace*>& as)
{
    std::map<std::string, BinaryEdit*>::iterator i = llBinEdits.begin();
...
bool
BPatch_binaryEdit::finalizeInsertionSet(bool /*atomic*/, bool* /*modified*/)
{
    return true;
}

BPatch_object*
BPatch_binaryEdit::loadLibrary(const char* libname, bool deps)
{
    ...
    for(lib_it = libs.begin(); lib_it != libs.end(); ++lib_it)
    {
        ...
        if(deps)
            if(!lib.second->getAllDependencies(llBinEdits))
                return NULL;
    }
...
bool
BPatch_binaryEdit::replaceTrapHandler()
{
    // Did we use a trap?
