...
bool PCSensitiveTransformer::processBlock(BlockList::iterator &b_iter) {
  ...
  for (Block::ElementList::iterator iter = elements.begin();
    ...
    // an original instruction (RelocInsn) or the CF wrapper (CFElement)
...
void PCSensitiveTransformer::handleThunkCall(BlockList::iterator &b_iter, 
...
void PCSensitiveTransformer::emulateInsn(BlockList::iterator &b_iter,
  ...
  // by a CFElement, so we just need a "push" (and then to create everything else).  

  assert(insn->getOperation().getID() == e_call);

  // Construct a new Element that will emulate the original instruction here. 
  static Absloc stack_loc(0, 0, "dummy_func");
  Element::Ptr replacement = GetPC::create(insn, addr, stack_loc);
  ...
  if ((*iter) != (*b_iter)->elements().back()) {
    ...
    CFElement::DestinationMap::iterator dest = cf->destMap_.find(CFElement::Taken);
    if (dest != cf->destMap_.end()) {
      // Explicitly do _NOT_ reuse old information - this is just a branch
      
      newCF->destMap_[CFElement::Taken] = dest->second;
	...
	for (CFElement::DestinationMap::iterator foo = cf->destMap_.begin();
