...
void
OffsetVector::insert(OffsetVector::K lb, OffsetVector::K ub, OffsetVector::V v,
                     bool isRegHeight)
{
    if(isRegHeight)
    {
...
void
OffsetVector::update(OffsetVector::K lb, OffsetVector::K newUB)
{
    _stack.update(lb, newUB);
}

void
OffsetVector::update(OffsetVector::K lb, OffsetVector::V newVal)
{
    _stack.updateValue(lb, newVal);
}

bool
OffsetVector::erase(OffsetVector::K key, bool isStack)
{
    if(isStack)
    {
        _stack.erase(key);
...
bool
OffsetVector::erase(MachRegister r, OffsetVector::V val)
{
...
bool
OffsetVector::find(OffsetVector::K key, OffsetVector::V& val) const
{
    return _stack.find(key, val);
}

bool
OffsetVector::find(OffsetVector::K key, MachRegister reg, OffsetVector::V& val) const
{
...
bool
OffsetVector::find(OffsetVector::K key, OffsetVector::K& lb, OffsetVector::K& ub,
...
void
OffsetVector::addSkip(MachRegister reg, StackAnalysis::Height height, Address blockAddr,
                      Address addr)
{
    auto found = _skipRegPCs.find(reg);
    if(found == _skipRegPCs.end())
    {
        std::map<StackAnalysis::Height, std::set<std::pair<Address, Address>>*>* newmap =
            new std::map<StackAnalysis::Height, std::set<std::pair<Address, Address>>*>();
    ...
    {
        std::map<StackAnalysis::Height, std::set<std::pair<Address, Address>>*>* m =
            found->second;
...
bool
OffsetVector::isSkip(MachRegister reg, StackAnalysis::Height height, Address blockAddr,
                     Address addr) const
{
    auto found = _skipRegPCs.find(reg);
    if(found != _skipRegPCs.end())
