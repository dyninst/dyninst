...
BPatch_stackInsertExpr::BPatch_stackInsertExpr(int size) {
  ast_wrapper =
      AstNodePtr(AstNode::stackInsertNode(size, AstNode::GENERIC_AST));
  assert(BPatch::bpatch != NULL);
}

BPatch_stackRemoveExpr::BPatch_stackRemoveExpr(int size) {
  ast_wrapper =
      AstNodePtr(AstNode::stackRemoveNode(size, AstNode::GENERIC_AST));
  assert(BPatch::bpatch != NULL);
}
...
BPatch_canaryCheckExpr::BPatch_canaryCheckExpr(BPatch_function* failureFunc,
                                               bool canaryAfterPrologue,
                                               long canaryHeight) {
  ast_wrapper = AstNodePtr(AstNode::stackRemoveNode(
      0, AstNode::CANARY_AST, failureFunc->lowlevel_func(), canaryAfterPrologue,
      canaryHeight));
