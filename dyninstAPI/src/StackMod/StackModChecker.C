...
StackModChecker::~StackModChecker()
{
    ...
    for(auto bhIter = blockHeights.begin(); bhIter != blockHeights.end(); bhIter++)
    {
...
static BPatch_function*
findCanaryFailureFunction(BPatch_image* image)
{
...
bool
StackModChecker::addModsInternal(std::set<StackMod*> mods)
{
    // Check architecture
    ...
    {
        stackmods_printf("RANDOMIZE has already been applied to this function; "
                         "additional modifications not allowed\n");
        return false;
    }
    ...
    for(auto iter = mods.begin(); iter != mods.end(); ++iter)
    {
        ...
        switch(type)
        {
            ...
            default: {
                stackmods_printf("addMods called with unknown stack modification type, "
                                 "returning false\n");
                func->freeStackMod();
                return false;
            }
    ...
    // The canary has to be the "middle" modification because the snippets are inserted
    // together
    ...
    // Other safety checks:
    //  - Does the stack grow/shrink more than once? For canaries, randomize this might be
    //  fine;
    //    for the others, this could lead to ambiguity about where the modifications
    //    should happen
    if(requiresCheckStackGrowth)
    {
        if(_unsafeStackGrowth || !checkStackGrowth(sa))
        {
            ...
            stackmods_printf("Found that the stack grows and shrinks multiple times in "
                             "this function; INSERT,REMOVE,MOVE disallowed\n");
            func->freeStackMod();
            return false;
        }
    }
    ...
    // We may add an insert for the canary check; we need to then add this to allMods
    // outside the loop
        ...
        switch(type)
        {
            ...
            case StackMod::CANARY: {
                ...
                for(auto point : checkPoints)
                {
                    ...
                    {
                        // This is the old handling code for non-entry canaries; needs to
                        // be updated
#if 0
    ...
    {
        // This will be out-of-order in allMods, but the snippet insertion happened in the
        // right order, so it's OK
    ...
    // addMods should be atomic, so if there was a failure, we need to revert all changes
    // performed by these modifications
    if(cleanupAndReturn)
    {
        stackmods_printf("One or more modifications were not successfully added. All "
    ...
    // Free old TMap
    for(auto tmapIter = oldTMap->begin(); tmapIter != oldTMap->end(); tmapIter++)
    {
 ...
 */
bool
StackModChecker::accumulateStackRanges(StackMod* m)
{
...
bool
StackModChecker::alignStackRanges(int alignment, StackMod::MOrder order,
                                  std::vector<StackMod*>& mods)
{
    ...
    for(rangeIterator rIter = begin; rIter != end;)
    {
        ...
        if(d.rem)
        {
            ...
            if(order == StackMod::NEW)
            {
                mods.push_back(new Insert(StackMod::NEW, (*endXXX).second.first - fixup,
                                          (*endXXX).second.first));
            }
            else if(order == StackMod::CLEANUP)
            {
                mods.insert(mods.begin(),
                            new Remove(StackMod::CLEANUP, (*endXXX).second.first - fixup,
                                       (*endXXX).second.first));
            }
        }
...
void
StackModChecker::findContiguousRange(rangeIterator iter, rangeIterator& end)
{
 ...
 */
bool
StackModChecker::checkStackGrowth(StackAnalysis& sa)
{
    bool ret = true;
...
void
StackModChecker::processBlock(StackAnalysis& sa, ParseAPI::Block* block)
{
...
bool
StackModChecker::checkAllPaths(StackAnalysis& sa)
{
 ...
 */
bool
StackModChecker::checkAllPathsInternal(ParseAPI::Block*               block,
                                       std::set<ParseAPI::Block*>&    state,
                                       std::vector<ParseAPI::Block*>& path,
                                       std::set<ParseAPI::Block*>&    exitBlocks,
                                       StackAnalysis&                 sa)
{
    ...
    else
    {
        ...
        for(auto iter = block->targets().begin(); iter != block->targets().end(); ++iter)
        {
            ...
            if(edge->sinkEdge() || edge->interproc())
                continue;
...
bool
StackModChecker::checkPath(std::vector<ParseAPI::Block*>& path)
{
...
bool
StackModChecker::findInsertOrRemovePoints(StackAnalysis& sa, StackMod* m,
                                          std::vector<BPatch_point*>*& points,
                                          long&                        dispFromRSP)
{
    // Insert and Remove need to be applied when the stack height is at the location of
    // stack memory to be modified; for Insert and StackMod::CLEANUP operations, this is
    // directly above the region in qusetion; otherwise, directly below
    int loc = 0;
...
bool
StackModChecker::checkInsn(ParseAPI::Block* block, Offset off, int loc, StackAnalysis& sa,
                           BPatch_point*& point, long& dispFromRSP)
{
...
bool
StackModChecker::findCanaryPoints(std::vector<BPatch_point*>* insertPoints,
                                  std::vector<BPatch_point*>* checkPoints)
{
    ...
    for(auto bIter = blocks.begin(); bIter != blocks.end(); ++bIter)
    {
        ...
        for(auto iIter = insns.begin(); iIter != insns.end(); ++iIter)
        {
            ...
            for(auto aIter = accesses->begin(); aIter != accesses->end(); ++aIter)
            {
                ...
                for(auto setIter = accessSet.begin(); setIter != accessSet.end();
                    setIter++)
                {
...
/* Check if a particular stack access in instruction insn is safe, given the current set
 * of stack modifications. Accesses are unsafe if a stack modification perturbs the access
 * range, which can happen by:
 ...
 */
bool
StackModChecker::isAccessSafe(InstructionAPI::Instruction insn, StackAccess* access)
{
    ...
    {
        stackmods_printf("\t could not find entry in offVec for this access to %ld, "
                         "defaulting to getAccessSize\n",
                         accessLow.height());
    ...
    // For now, we re-iterate through the modifications; in the future, it would be better
    // to use the tMap somehow
