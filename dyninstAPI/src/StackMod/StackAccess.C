...
StackAccess::format()
{
    ...
    ret << "Access to " << _readHeight.height() << " from " << _reg.name() << " (at "
        << _regHeight.height() << ", defined: " << _regDef.format() << ")"
...
int
getAccessSize(InstructionAPI::Instruction insn)
{
...
{
private:
    typedef std::vector<std::pair<Absloc, StackAnalysis::DefHeightSet>> DefHeights;
    bool                                                                defined;
...
public:
    detectToppedLoc(DefHeights& h)
    : defined(true)
    , containsToppedReg(false)
    , defHeights(h)
    {}
...
bool
defsSameHeights(const StackAnalysis::DefHeightSet& dhSet)
{
    if(dhSet.size() == 0)
        return true;
...
bool
getAccesses(ParseAPI::Function* func, ParseAPI::Block* block, Address addr,
            InstructionAPI::Instruction insn, Accesses* accesses,
            std::set<Address>& defPointsToMod, bool analyzeDefinition)
{
    stackmods_printf("\t\t getAccesses %s, 0x%lx @ 0x%lx: %s\n", func->name().c_str(),
                     block->start(), addr, insn.format().c_str());
    ...
    if(insn.getCategory() == InstructionAPI::c_CallInsn)
    {
        ...
        for(auto iter = abi->getIndexMap()->begin(); iter != abi->getIndexMap()->end();
            iter++)
        {
    ...
    for(auto iter = defHeights.begin(); iter != defHeights.end(); ++iter)
    {
        ...
        if(iter->first.type() != Absloc::Register)
            continue;
        ...
        for(auto dhIter = dhSet.begin(); dhIter != dhSet.end(); dhIter++)
        {
            ...
            if(getMemoryOffset(func, block, insn, addr, curReg, curHeight, curDef, access,
                               arch, analyzeDefinition))
            {
        ...
        // to defsToMod.
        if(accesses->find(curReg) != accesses->end() && (*accesses)[curReg].size() > 1)
        {
            ...
            if(!defsSameHeights(dhSet))
            {
                for(auto dhIter = dhSet.begin(); dhIter != dhSet.end(); dhIter++)
                {
                    ...
                    if(d.type != StackAnalysis::Definition::DEF || d.addr == 0)
                    {
                        stackmods_printf(
                            "\t\t\t\t INVALID: Multiple accesses "
    ...
    if(insn.writesMemory())
    {
        ...
        if(dtl.isTopped())
        {
            ...
            // stack heights.
            for(auto regIter = readRegs.begin(); regIter != readRegs.end(); regIter++)
            {
                Absloc regLoc((*regIter)->getID());

                for(auto hIter = defHeights.begin(); hIter != defHeights.end(); hIter++)
                {
                    if(hIter->first == regLoc && !hIter->second.isTopSet())
                    {
    ...
    else if(insn.readsMemory())
    {
        ...
        for(auto rIter = readOperands.begin(); rIter != readOperands.end(); rIter++)
        {
            detectToppedLoc dtl(defHeights);
            (*rIter)->apply(&dtl);
            if(dtl.isBottomed())
    ...
    if(analyzeDefinition &&
       (accesses->size() != 1 || accesses->begin()->second.size() != 1))
    {
...
public:
    zeroAllGPRegisters(Address ip, ParseAPI::Function* f, ParseAPI::Block* b,
                       InstructionAPI::Instruction i, bool z = false)
    : defined(true)
    , m_ip(ip)
    , func(f)
    , block(b)
...
bool
getMemoryOffset(ParseAPI::Function* func, ParseAPI::Block* block, Instruction insn,
                Address addr, const MachRegister& reg,
                const StackAnalysis::Height& height, const StackAnalysis::Definition& def,
                StackAccess*& ret, Architecture arch, bool analyzeDefinition)
{
