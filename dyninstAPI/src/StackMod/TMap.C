...
void
TMap::update(StackLocation* loc, int delta)
{
    ...
    {
        tmp = new StackLocation(loc->off() + delta, loc->size(), loc->type(),
                                loc->isRegisterHeight(), loc->valid());
    }
    assert(tmp);
...
void
TMap::update(StackLocation* loc, MachRegister reg, int size)
{
...
StackLocation*
TMap::findInMap(StackLocation* src)
{
