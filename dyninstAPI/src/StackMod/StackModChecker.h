...
class StackModChecker {
 public:
  StackModChecker(BPatch_function* b, func_instance* f)
      : bfunc(b), func(f), _unsafeStackGrowth(false) {}
  ~StackModChecker();
 ...
 private:
  ...
  bool alignStackRanges(int alignment, StackMod::MOrder order,
  ...
  void processBlock(Dyninst::StackAnalysis& sa,
  ...
  bool findInsertOrRemovePoints(Dyninst::StackAnalysis& sa, StackMod* m,
                                std::vector<BPatch_point*>*& points,
                                long& dispFromRSP);
  bool checkInsn(Dyninst::ParseAPI::Block* block, Dyninst::Offset off, int loc,
                 Dyninst::StackAnalysis& sa, BPatch_point*& point,
                 long& dispFromRSP);
  bool findCanaryPoints(std::vector<BPatch_point*>* insertPoints,
                        std::vector<BPatch_point*>* checkPoints);

  bool areModificationsSafe();
  bool isAccessSafe(Dyninst::InstructionAPI::Instruction::Ptr insn,
                    StackAccess* access);

  bool _unsafeStackGrowth;

  std::map<Dyninst::ParseAPI::Block*,
           std::vector<Dyninst::StackAnalysis::Height>*>
      blockHeights;
