 ...
 */
BPatch_snippet::BPatch_snippet() { ast_wrapper = AstNodePtr(AstNode::nullNode()); }
...
AstNodePtr
generateVariableBase(const BPatch_snippet& lOperand)
{
    AstNodePtr variableBase;
    if(lOperand.ast_wrapper->getoType() == AstNode::variableValue)
    {
        variableBase =
            AstNode::operandNode(AstNode::variableAddr, lOperand.ast_wrapper->getOVar());
    }
...
//
AstNodePtr
generateArrayRef(const BPatch_snippet& lOperand, const BPatch_snippet& rOperand)
{
    if(lOperand.ast_wrapper == AstNodePtr())
        return AstNodePtr();
    if(rOperand.ast_wrapper == AstNodePtr())
        return AstNodePtr();

    if(lOperand.ast_wrapper->getType() == NULL)
    {
        BPatch_reportError(BPatchSerious, 109, "array reference has no type information");
    }

    //  We have to be a little forgiving of the

    typeArray* arrayType =
        lOperand.ast_wrapper->getType()->getSymtabType(Type::share)->getArrayType();
    if(!arrayType)
    {
        if(lOperand.ast_wrapper->getType() == NULL)
        {
    ...
    AstNodePtr ast       = AstNode::operandNode(
        ...
        AstNode::operatorNode(
            plusOp, arrayBase,
            AstNode::operatorNode(
                timesOp, AstNode::operandNode(AstNode::Constant, (void*) elementSize),
...
//
AstNodePtr
generateFieldRef(const BPatch_snippet& lOperand, const BPatch_snippet& rOperand)
{
    if(lOperand.ast_wrapper == AstNodePtr())
        return AstNodePtr();
    if(rOperand.ast_wrapper == AstNodePtr())
    ...
    typeStruct* structType =
        lOperand.ast_wrapper->getType()->getSymtabType(Type::share)->getStructType();

    if(!structType)
    {
        BPatch_reportError(BPatchSerious, 109,
                           "structure reference has no type information, or structure "
    ...
    if(rOperand.ast_wrapper->getoType() != AstNode::ConstantString || !fieldType ||
    ...
    AstNodePtr ast = AstNode::operandNode(
        AstNode::DataIndir,
        AstNode::operatorNode(plusOp, structBase,
 ...
 */
BPatch_arithExpr::BPatch_arithExpr(BPatch_binOp op, const BPatch_snippet& lOperand,
 ...
 * lOperand     The left operand for the operation.
 */
BPatch_arithExpr::BPatch_arithExpr(BPatch_unOp op, const BPatch_snippet& lOperand)
{
    assert(BPatch::bpatch != NULL);
    ...
    switch(op)
    {
        case BPatch_negate: {
            ...
            negOne->setType(type);
            ast_wrapper =
                AstNodePtr(AstNode::operatorNode(timesOp, negOne, lOperand.ast_wrapper));
            break;
        }
        case BPatch_addr: {
 ...
 * rOperand     The right operand.
 */
BPatch_boolExpr::BPatch_boolExpr(BPatch_relOp op, const BPatch_snippet& lOperand,
                                 const BPatch_snippet& rOperand)
{
...
BPatch_constExpr::BPatch_constExpr(signed int value)
{
    ...
    ast_wrapper =
        AstNodePtr(AstNode::operandNode(AstNode::Constant, (void*) (uintptr_t) value));
    ast_wrapper->setTypeChecking(BPatch::bpatch->isTypeChecked());
...
{
    ast_wrapper = AstNodePtr(
        AstNode::operandNode(AstNode::ConstantString, (void*) const_cast<char*>(value)));
...
{
    ast_wrapper = AstNodePtr(
        AstNode::operandNode(AstNode::Constant, (void*) const_cast<void*>(value)));
...
{
    ast_wrapper = AstNodePtr(
        AstNode::operatorNode(whileOp, conditional.ast_wrapper, body.ast_wrapper));
    assert(BPatch::bpatch != NULL);
    ast_wrapper->setTypeChecking(BPatch::bpatch->isTypeChecked());
}
 ...
 */
BPatch_funcCallExpr::BPatch_funcCallExpr(const BPatch_function&                func,
...
{
    ast_wrapper = AstNodePtr(
        AstNode::operatorNode(ifOp, conditional.ast_wrapper, tClause.ast_wrapper));
 ...
 */
BPatch_ifExpr::BPatch_ifExpr(const BPatch_boolExpr& conditional,
                             const BPatch_snippet& tClause, const BPatch_snippet& fClause)
{
...
BPatch_registerExpr::BPatch_registerExpr(Dyninst::MachRegister mach)
{
    ...
    ast_wrapper =
        AstNodePtr(AstNode::operandNode(AstNode::origRegister, (void*) (intptr_t) reg));
    assert(BPatch::bpatch != NULL);
...
BPatch_variableExpr::BPatch_variableExpr(const char*          in_name,
                                         ...
                                         AstNodePtr ast_wrapper_, BPatch_type* typ,
                                         void* in_address)
: name(in_name)
...
BPatch_variableExpr*
BPatch_variableExpr::makeVariableExpr(BPatch_addressSpace* in_addSpace, int_variable* v,
                                      BPatch_type* type)
{
...
BPatch_variableExpr*
BPatch_variableExpr::makeVariableExpr(BPatch_addressSpace* in_addSpace,
                                      AddressSpace* in_llAddSpace, std::string name,
                                      void* offset, BPatch_type* type)
{
    int_variable* v = in_llAddSpace->getAOut()->getDefaultModule()->createVariable(
 ...
 */
bool
BPatch_variableExpr::setType(BPatch_type* newType)
{
 ...
 */
BPatch_variableExpr::BPatch_variableExpr(BPatch_addressSpace* in_addSpace,
                                         AddressSpace* in_lladdrSpace, void* in_address,
                                         int in_register, BPatch_type* typ,
...
, intvar(NULL)
{
    ...
    switch(in_storage)
    {
        ...
        case BPatch_storageReg:
            variableAst = AstNodePtr(
                AstNode::operandNode(AstNode::origRegister, (void*) (long) in_register));
            isLocal = true;
            break;
        ...
        case BPatch_storageRegOffset:
            variableAst = AstNodePtr(AstNode::operandNode(
                AstNode::RegOffset, AstNode::operandNode(AstNode::DataAddr, address)));
 ...
 */
BPatch_variableExpr::BPatch_variableExpr(BPatch_addressSpace* in_addSpace,
                                         AddressSpace* in_lladdSpace, BPatch_localVar* lv,
                                         BPatch_type* typ, BPatch_point* scp)
: name()
, appAddSpace(in_addSpace)
, lladdrSpace(in_lladdSpace)
, address(NULL)
...
BPatch_variableExpr::readValue(void* dst)
{
    ...
    if(size == 2 || size == 4 || size == 8)
    {
        ...
        if(!lladdrSpace->readDataWord(address, size, dst, true))
            return false;
        return true;
    }
    else if(size)
    {
        if(!lladdrSpace->readDataSpace(address, size, dst, true))
            return false;
        return true;
    }
    else
...
bool
BPatch_variableExpr::writeValue(const void* src, bool /* saveWorld */)
{
    if(isLocal)
    {
        char msg[2048];
 ...
 */
bool
BPatch_variableExpr::writeValue(const void* src, int len, bool /*saveWorld*/)
{
    if(isLocal)
    {
        char msg[2048];
 ...
 */
BPatch_Vector<BPatch_variableExpr*>*
    ...
    for(unsigned int i = 0; i < fields->size(); i++)
    {
        ...
        if(field->getType() != NULL)
        {
            ...
            newVar = new BPatch_variableExpr(const_cast<char*>(field->getName()),
                                             appAddSpace, lladdrSpace, newAst,
                                             field->getType(), (char*) address + offset);
...
BPatch_tidExpr::BPatch_tidExpr(BPatch_process* proc)
{
    ...
    {
        fprintf(stderr,
                "[%s:%d] - Internal Dyninst error.  Found %lu copies of "
...
static void
constructorHelper(const BPatchStopThreadCallback& bp_cb, bool useCache,
                  BPatch_stInterpret interp, AstNodePtr& idNode, AstNodePtr& icNode)
{
    ...
    std::set<BPatchStopThreadCallback>::iterator cbIter = stopThread_cbs->find(bp_cb);
 ...
 */
BPatch_stopThreadExpr::BPatch_stopThreadExpr(const BPatchStopThreadCallback& bp_cb,
...
BPatch_stopThreadExpr::BPatch_stopThreadExpr(const BPatchStopThreadCallback& bp_cb,
                                             const BPatch_snippet&           calculation,
                                             const mapped_object& obj, bool useCache,
                                             BPatch_stInterpret interp)
{
    ...
    AstNodePtr   objStartNode = AstNode::operandNode(AstNode::Constant, (void*) objStart);
    AstNodePtr   objEndNode   = AstNode::operandNode(AstNode::Constant, (void*) objEnd);
...
BPatch_shadowExpr::BPatch_shadowExpr(bool entry, const BPatchStopThreadCallback& bp_cb,
                                     const BPatch_snippet& calculation, bool useCache,
                                     BPatch_stInterpret interp)
{
    AstNodePtr idNode;
    AstNodePtr icNode;
...
// Conversions
Dyninst::PatchAPI::Snippet::Ptr
Dyninst::PatchAPI::convert(const BPatch_snippet* snip)
{
...
bool
BPatch_snippet::checkTypesAtPoint(BPatch_point* p) const
{
    if(!p)
        return true;
