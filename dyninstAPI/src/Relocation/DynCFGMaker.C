...
PatchFunction* DynCFGMaker::makeFunction(Function* f, PatchObject* obj) {
  ...
  if (!mod) {
    ...
    fprintf(stderr, "%s[%d]:  ERROR:  Cannot find module %s\n", FILE__,
            __LINE__, img_func->pdmod()->fileName().c_str());
  }
...
PatchEdge* DynCFGMaker::makeEdge(ParseAPI::Edge* e, PatchBlock* s,
                                 PatchBlock* t, PatchObject* o) {
  ...
  edge_instance* inst =
      new edge_instance(e, s ? SCAST_BI(s) : moS->findBlock(e->src()),
                        t ? SCAST_BI(t) : moT->findBlock(e->trg()));

#else  // doesn't allow inter-object edges
  mapped_object* mo = t ? t->obj() : NULL;
  edge_instance* inst =
      new edge_instance(e, s ? SCAST_BI(s) : mo->findBlock(e->src()),
