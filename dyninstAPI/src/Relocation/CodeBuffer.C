...
void
CodeBuffer::BufferElement::addPIC(const unsigned char* input, unsigned size,
...
void
CodeBuffer::BufferElement::addPIC(const Buffer& buf, TrackerElement* tracker)
{
    addTracker(tracker);
...
void
CodeBuffer::BufferElement::setPatch(Patch* patch, TrackerElement* tracker)
{
    addTracker(tracker);
    assert(patch_ == NULL);
...
void
CodeBuffer::BufferElement::setLabelID(unsigned id)
{
...
void
CodeBuffer::BufferElement::addTracker(TrackerElement* tracker)
{
    trackers_[buffer_.size()] = tracker;
}

bool
CodeBuffer::BufferElement::empty()
{
    // We're empty if:
    // No patch;
    // No label;
...
bool
CodeBuffer::BufferElement::generate(CodeBuffer* buf, codeGen& gen, int& shift,
                                    bool& regenerate)
{
...
bool
CodeBuffer::BufferElement::extractTrackers(CodeTracker* t)
{
    ...
    for(Trackers::iterator iter = trackers_.begin(); iter != trackers_.end(); ++iter)
    {
        TrackerElement* e = iter->second;
        if(!e)
            continue;  // 0-length "mark me" Widgets may not have trackers
        ...
        {
            // relocation_cerr << "\t\t\t Size calc: " << size_ << " - " << iter->first <<
            // endl;
...
void
CodeBuffer::initialize(const codeGen& templ, unsigned numBlocks)
{
...
unsigned
CodeBuffer::defineLabel(Address addr)
{
...
void
CodeBuffer::addPIC(const unsigned char* input, unsigned size, TrackerElement* tracker)
{
...
void
CodeBuffer::addPIC(const void* input, unsigned size, TrackerElement* tracker)
{
    addPIC((const unsigned char*) input, size, tracker);
}

void
CodeBuffer::addPIC(const codeGen& input, TrackerElement* tracker)
{
    addPIC(input.start_ptr(), input.used(), tracker);
}

void
CodeBuffer::addPIC(Buffer buf, TrackerElement* tracker)
{
...
void
CodeBuffer::addPatch(Patch* patch, TrackerElement* tracker)
{
    current().setPatch(patch, tracker);
    size_ += patch->estimate(gen_);
}

CodeBuffer::BufferElement&
CodeBuffer::current()
{
...
bool
CodeBuffer::extractTrackers(CodeTracker* t)
{
    for(Buffers::iterator iter = buffers_.begin(); iter != buffers_.end(); ++iter)
    {
        if(!iter->extractTrackers(t))
            return false;
    }
    return true;
}
...
bool
CodeBuffer::generate(Address baseAddr)
{
    generated_ = false;
...
void
CodeBuffer::updateLabel(unsigned id, Address offset, bool& regenerate)
{
    if(id == (unsigned) -1)
        return;
...
unsigned
CodeBuffer::size() const
{
    return size_;
}

void*
CodeBuffer::ptr() const
{
