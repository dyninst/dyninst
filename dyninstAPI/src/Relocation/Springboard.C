...
SpringboardBuilder::Ptr
SpringboardBuilder::createFunc(FuncSet::const_iterator begin, FuncSet::const_iterator end,
                               AddressSpace* as)
{
...
bool
SpringboardBuilder::generateInt(std::list<codeGen>& springboards, SpringboardMap& input,
                                Priority p)
{
    ...
    for(SpringboardMap::reverse_iterator iter = input.rbegin(p); iter != input.rend(p);
        ++iter)
    {
...
bool
SpringboardBuilder::generate(std::list<codeGen>& springboards, SpringboardMap& input)
{
...
bool
InstalledSpringboards::addFunc(func_instance* func)
{
    if(!addBlocks(func, func->blocks().begin(), func->blocks().end()))
        return false;
    return true;
}

bool
isNoneContained(std::set<ParseAPI::Block*>& blocks)
{
    bool noneContained = true;
    for(auto block = blocks.begin(); block != blocks.end(); block++)
    {
        springboard_cerr << hex << " Checking block " << (*block)->start() << "-"
                         << (*block)->end() << dec << ": ";
        if((*block)->containingFuncs() > 0)
        {
            std::vector<ParseAPI::Function*> funcs;
            springboard_cerr << "failure - contained by " << (*block)->containingFuncs()
                             << " function(s): ";
            ...
            for(auto func = funcs.begin(); func != funcs.end(); func++)
            {
                ...
                springboard_cerr
                    << ((*func)->contains(*block) ? "(correct)" : "(incorrect)");
            }
...
bool
InstalledSpringboards::addBlocks(func_instance* func, BlockIter blocks_begin,
                                 BlockIter blocks_end)
{
    ...
    if ((f->blocks().size() == 1) &&
        ...
        // Extend the block to include any subsequent no-ops that are not part of other
        // blocks
        ...
        { /* there may be more than one range that overlaps with bbl,
           ...
           */
            if(validRanges_.find(lookup, LB, UB, id))
            {
                 ...
                 * below:
                 *
                 * [LB UB)                     already in validRanges_, remove if it gets
                 * split [LB bbl->start()) [bbl->start() LB) [start LB)         when bbl
                 * overlaps multiple ranges and LB is for an N>1 range [bbl->start() UB)
                 * possible if LB < bbl->start() [start UB)         possible if LB <
                 * bbl->start() and bbl overlaps multiple ranges [LB end)             if
                 * last existing range includes end [end UB)             if last existing
                 * range includes end [UB end)             don't add until after loop
                 * exits as there might be more overlapping ranges
                 */
                if(LB < start)
                {
                    ...
                    {  // [start end) or [start end) and [end UB)
                                        ...
                                        const SpringboardReq& r)
{
    ...
    // Arbitrarily select the first function containing this springboard, since only one
    // can win.
    ...
    if(r.useTrap ||
       conflict(r.from, r.from + tmpGen.used(), r.fromRelocatedCode, r.func, r.priority))
    {
        // Generate the trap
        generateTrap(r.from, r.destinations.begin()->second, gen);
        // This check must be left in place.
        // Reason: Suggested springboards use generateSpringboard to create their
        // springboards.
        //         If a suggested springboard's block entry is used by a required
        //         springboard, we could potentially overwrite an instruction from a
        //         required springboard.
        if(conflict(r.from, r.from + gen.used(), r.fromRelocatedCode, r.func, r.priority))
        {
            return Failed;
        }
        if(!addrSpace_->canUseTraps())
        {
            return Failed;
        }

        size = gen.used();
        springboard_cerr << "\t Using a springboard trap for springboard at addr: 0x"
                         << std::hex << r.from << std::endl;
    }
    else
    {
        // regenerate the branch into gen
        // ideally we would like to do gen = tmpGen but there is a problem with codeGen's
        // copy constructor
        generateBranch(r.from, r.destinations.begin()->second, gen);
        springboard_cerr << "\t Using a branch for springboard at addr: 0x" << std::hex
                         << r.from << " with byte size = " << std::dec << gen.used()
                         << std::endl;
    }

    registerBranch(r.from, r.from + size, r.destinations, r.fromRelocatedCode, r.func,
                   r.priority);
    if(gen.valid())
    {
...
bool
SpringboardBuilder::generateMultiSpringboard(std::list<codeGen>&, const SpringboardReq&)
{
    // debugRanges();
    // if (false) cerr << "Request to generate multi-branch springboard skipped @ " << hex
    // << r.from << dec << endl;
...
bool
InstalledSpringboards::conflict(Address start, Address end, bool inRelocated,
                                func_instance* func, Priority p)
{
    if(inRelocated)
    ...
    // The above comment is wholly wrong, but leaving it in and responding to it for the
    // moment. Springboards *should* stay within a particular block, but we attempt to
    // address this by ensuring that higher priority springboards are installed before
    // lower priority ones (which is to say mandatory before optional), and by processing
    // them from high addresses to low ones in each priority group. We glue arbitrary
    // blocks together at each priority level.

    // NOTE: this approach may be responsible for our reinstrumentation bug, where we have
    // the same problem with "installed 2 byte branch, think we now have space for 5 byte
    // branch" that we do with "installed trap, think we now have space for a branch".
    // Need to determine how to properly avoid expanding springboards when reinstrumenting
    // in a general way, not just in the trap/branch case.

    // And now that we're working with a per-address-space set of data structures, we have
    // to keep track of multiple relocation invocations' worth of requests. However, we
    // know that in a given batch of springboard requests, we can't have two springboards
    // starting at the same address. SO: If we find that we're overwriting at the same
    // address as an existing springboard and we're not running into another interval,
    // there's no conflict (replacing a springboard). If we find *any* other case where
    // we're hitting allocated, conflict.
    Address          working   = start;
    Address          LB        = 0;
    Address          UB        = 0;
    SpringboardInfo* state     = NULL;
    SpringboardInfo* lastState = state;
    springboard_cerr << "Conflict called for " << hex << start << "->" << end << dec
                     << endl;
    ...
    while(end > working)
    {
        ...
        {
            springboard_cerr << "\t Conflict: unable to find entry for " << hex << working
                             << dec << endl;
            return true;
        }
        springboard_cerr << "\t\t Found " << hex << LB << " -> " << UB << " /w/ state "
                         << state->val << ", " << state->func->name() << ", priority "
        ...
        if(state->val == Allocated)
        {
            if(LB == start && UB >= end)
            {
                 ...
                 *           If same target, then OK (simplify to conflict for ease of
                 * use) If not same target, then conflict Otherwise, return OK
                 */

                if(state->priority > p)
                {
                    springboard_cerr
                        << "\t Starting range matches already allocated springboard, "
                           "prior springboard had higher priority, ret conflict"
                        << endl;
                    return true;
                }
                if((state->priority == p) && (state->func != func))
                {
                    springboard_cerr
                        << "\t Starting range matches already allocated springboard, "
                           "equivalent priorities and different functions, ret conflict"
                        << endl;
                    return true;
                }
                // In dynamic instrumentation, springboards are installed immediately
                // after users wants to insert a snippet. The user can continue to insert
                // more snippets to the same function, which will trigger Dyninst to
                // perform the relocation one more time. So, we need to overwrite existing
                // springboard to update new instrumentations.
                //
                // However, this will lead to dead code in .dyninstInst. The ideal
                // solution for dynamic instrumentation is to only perform relocation
                // before the user wants to continue to attached/created process.
                springboard_cerr << "\t Starting range matches already allocated "
                                    "springboard, assuming overwrite, ret OK"
                                 << endl;
                return false;
            }

            springboard_cerr << "\t Starting range already allocated, ret conflict"
                             << endl;
            return true;
        }
        if(lastState != NULL && state->func != lastState->func)
        {
            springboard_cerr << "\t Crossed into a different function, ret conflict"
                             << endl;
            return true;
        }
        // BLOCK PRIORITY CHECK:
        //    Check to see if the block we are writing to contains a required (or greater)
        //    springboard What this check prevents is a required springboard in a block at
        //    a lower address from overwriting a block at a higher address (that has
        //    already been written). This check assumes that we are always generating
        //    springboards starting at the highest address and working backwards (i.e.
        //    0xfff... -> 0x000...)
        if(state->priority >= FuncEntry)
        {
            springboard_cerr << "\t Trying to write a springboard that crosses into "
                                "another required block, ret conflict"
                             << std::endl;
            return true;
        }
...
bool
InstalledSpringboards::conflictInRelocated(Address start, Address end)
{
...
void
InstalledSpringboards::registerBranch(Address start, Address end,
                                      const SpringboardReq::Destinations& dest,
                                      bool inRelocated, func_instance* func, Priority p)
{
    ...
    while(end > working)
    {
        ...
        else
        {
            ...
            {
                cerr << "Error: idToUse " << idToUse << " and state->val " << state->val
                     << endl;
            }
    ...
    if(LB < start)
    {
        springboard_cerr << "\tInserting prior space " << hex << LB << " -> " << start
                         << " /w/ range " << idToUse << dec << endl;
        validRanges_.insert(LB, start, info);
    }
    springboard_cerr << "\t Inserting taken space " << hex << start << " -> " << end
                     << " /w/ range " << Allocated << dec << endl;
    validRanges_.insert(start, end, new SpringboardInfo(Allocated, func, p));
    if(UB > end)
    {
        springboard_cerr << "\tInserting post space " << hex << end << " -> " << UB
                         << " /w/ range " << idToUse << dec << endl;
        validRanges_.insert(end, UB, info);
    }
}
...
void
InstalledSpringboards::registerBranchInRelocated(Address start, Address end,
                                                 func_instance* func, Priority p)
{
...
void
InstalledSpringboards::debugRanges()
{
    ...
    for(unsigned i = 0; i < elements.size(); ++i)
    {
        if(false)
            cerr << "\t" << hex << elements[i].first.first << ".."
                 << elements[i].first.second << dec << " -> " << elements[i].second->val
                 << endl;
    }
    if(false)
        cerr << "-------------" << endl;
...
void
SpringboardBuilder::generateBranch(Address from, Address to, codeGen& gen)
{
    gen.invalidate();
    ...
    springboard_cerr << "Generated springboard branch " << hex << from << "->" << to
                     << dec << endl;
...
void
SpringboardBuilder::generateTrap(Address from, Address to, codeGen& gen)
{
...
bool
SpringboardBuilder::createRelocSpringboards(const SpringboardReq& req, bool useTrap,
                                            SpringboardMap& input)
{
        ...
        b_iter != req.destinations.end(); ++b_iter)
    {
        ...
        springboard_cerr << "Looking for addr " << hex << addr << " in function "
                         << func->name() << dec << endl;
            ...
            a_iter != relocAddrs.rend(); ++a_iter)
        {
            ...
            {
                springboard_cerr << "Springboard conflict for " << hex << req.from << "["
                                 << (*a_iter)
                                 << "] our previous springboard here needed a trap, "
                                 << "but due to overwrites we may (erroneously) think "
                                 << "a branch can fit" << dec << endl;
            ...
            springboard_cerr << "Adding springboard from " << hex << *a_iter << " to "
                             << addr << dec << endl;
            input.addRaw(*a_iter, addr, newPriority, func, bbl, req.checkConflicts, false,
                         true, curUseTrap);
        }
    }
    return true;
