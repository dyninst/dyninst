...
SpringboardBuilder::SpringboardBuilder(AddressSpace *a)
    : addrSpace_(a), installed_springboards_(a->getInstalledSpringboards()) {}

SpringboardBuilder::Ptr SpringboardBuilder::createFunc(
    FuncSet::const_iterator begin, FuncSet::const_iterator end,
    AddressSpace *as) {
...
bool SpringboardBuilder::generateInt(std::list<codeGen> &springboards,
                                     SpringboardMap &input, Priority p) {
...
bool SpringboardBuilder::generate(std::list<codeGen> &springboards,
  ...
  // Currently we use a greedy algorithm rather than some sort of scheduling
  // thing.
  // It's a heck of a lot easier that way.

  if (!generateInt(springboards, input, Required)) return false;

  if (!generateInt(springboards, input, Suggested)) return false;

  // Catch up with instrumentation
  if (!generateInt(springboards, input, RelocRequired)) return false;
  if (!generateInt(springboards, input, RelocSuggested)) return false;
...
bool isNoneContained(std::set<ParseAPI::Block *> &blocks) {
  ...
  for (auto block = blocks.begin(); block != blocks.end(); block++) {
    ...
    if ((*block)->containingFuncs() > 0) {
      ...
      springboard_cerr << "failure - contained by "
                       << (*block)->containingFuncs() << " function(s): ";
      (*block)->getFuncs(funcs);
      for (auto func = funcs.begin(); func != funcs.end(); func++) {
        springboard_cerr << " " << (*func)->name() << " ";
        springboard_cerr << ((*func)->contains(*block) ? "(correct)"
                                                       : "(incorrect)");
      }
...
bool InstalledSpringboards::addBlocks(func_instance *func, BlockIter begin,
                                      BlockIter end) {
  ...
  for (; begin != end; ++begin) {
    ...
    // Extend the block to include any subsequent no-ops that are not part of
    // other blocks
    int size = bbl->size();
    ...
    for (Address lookup = start;
         lookup <
         end;) { /* there may be more than one range that overlaps with bbl,
      ...
      if (validRanges_.find(lookup, LB, UB, id)) {
         ...
         * below:
         *
         * [LB UB)                     already in validRanges_, remove if it
         * gets split
         * [LB bbl->start())
         * [bbl->start() LB)
         * [start LB)         when bbl overlaps multiple ranges and LB is for an
         * N>1 range
         * [bbl->start() UB)           possible if LB < bbl->start()
         * [start UB)         possible if LB < bbl->start() and bbl overlaps
         * multiple ranges
         * [LB end)             if last existing range includes end
         * [end UB)             if last existing range includes end
         * [UB end)             don't add until after loop exits as there might
         * be more overlapping ranges
         */
        if (LB < start) {
...
SpringboardBuilder::generateResult_t SpringboardBuilder::generateSpringboard(
    std::list<codeGen> &springboards, const SpringboardReq &r,
    SpringboardMap &input) {
  codeGen gen;
  bool usedTrap = false;
  // Arbitrarily select the first function containing this springboard, since
  // only one can win.
  ...
  if (r.useTrap || conflict(r.from, r.from + gen.used(), r.fromRelocatedCode,
    ...
    usedTrap = true;
    if (conflict(r.from, r.from + 1, r.fromRelocatedCode, r.func, r.priority)) {
      return Failed;
    }
...
bool SpringboardBuilder::generateMultiSpringboard(std::list<codeGen> &,
  ...
  // debugRanges();
  // if (false) cerr << "Request to generate multi-branch springboard skipped @
  // " << hex << r.from << dec << endl;
  // For now we give up and hope it all works out for the best.
  return true;
}

bool InstalledSpringboards::conflict(Address start, Address end,
                                     bool inRelocated, func_instance *func,
                                     Priority p) {
  ...
  // The above comment is wholly wrong, but leaving it in and responding to it
  // for the moment.
  // Springboards *should* stay within a particular block, but we attempt to
  // address this by
  // ensuring that higher priority springboards are installed before lower
  // priority ones
  // (which is to say mandatory before optional), and by processing them from
  // high addresses
  // to low ones in each priority group. We glue arbitrary blocks together at
  // each priority level.

  // NOTE: this approach may be responsible for our reinstrumentation bug, where
  // we have the same
  // problem with "installed 2 byte branch, think we now have space for 5 byte
  // branch"
  // that we do with "installed trap, think we now have space for a branch".
  // Need to determine how to properly
  // avoid expanding springboards when reinstrumenting in a general way, not
  // just in the trap/branch case.

  // And now that we're working with a per-address-space set of data structures,
  // we have to keep track of
  // multiple relocation invocations' worth of requests. However, we know that
  // in a given batch of springboard
  // requests, we can't have two springboards starting at the same address. SO:
  // If we find that we're overwriting at the same address as an existing
  // springboard
  // and we're not running into another interval, there's no conflict (replacing
  // a springboard).
  ...
  springboard_cerr << "Conflict called for " << hex << start << "->" << end
                   << dec << endl;

  while (end > working) {
    springboard_cerr << "\t looking for " << hex << working << dec << endl;
    if (!validRanges_.find(working, LB, UB, state)) {
      springboard_cerr << "\t Conflict: unable to find entry for " << hex
                       << working << dec << endl;
      return true;
    }
    springboard_cerr << "\t\t Found " << hex << LB << " -> " << UB
                     << " /w/ state " << state->val << ", "
                     << state->func->name() << ", priority " << state->priority
                     << dec << endl;
      ...
      if (LB == start && UB >= end) {
        ...
        if (state->priority > p) {
          springboard_cerr << "\t Starting range matches already allocated "
                              "springboard, prior springboard had higher "
                              "priority, ret conflict"
                           << endl;
          return true;
        }
        if ((state->priority == p) && (state->func != func)) {
          springboard_cerr << "\t Starting range matches already allocated "
                              "springboard, equivalent priorities and "
                              "different functions, ret conflict"
                           << endl;
          return true;
        }
...
void InstalledSpringboards::registerBranch(
    Address start, Address end, const SpringboardReq::Destinations &dest,
    bool inRelocated, func_instance *func, Priority p) {
  // Remove the valid ranges for everything between start and end, using much
  // the
  // same logic as above.
  ...
  Address LB = 0, UB = 0;
  Address lb = 0, ub = 0;
  springboard_cerr << "Adding branch: " << hex << start << " -> " << end << dec
                   << endl;
  int idToUse = -1;
  while (end > working) {
    SpringboardInfo *state = NULL;
  ...
  if (LB < start) {
    springboard_cerr << "\tInserting prior space " << hex << LB << " -> "
                     << start << " /w/ range " << idToUse << dec << endl;
    validRanges_.insert(LB, start, info);
  }
  springboard_cerr << "\t Inserting taken space " << hex << start << " -> "
                   << end << " /w/ range " << Allocated << dec << endl;
...
void InstalledSpringboards::registerBranchInRelocated(Address start,
                                                      Address end,
                                                      func_instance *func,
                                                      Priority p) {
...
void InstalledSpringboards::debugRanges() {
  std::vector<std::pair<std::pair<Address, Address>, SpringboardInfo *> >
      elements;
...
void SpringboardBuilder::generateBranch(Address from, Address to,
                                        codeGen &gen) {
  gen.invalidate();
  gen.allocate(16);
  ...
  springboard_cerr << "Generated springboard branch " << hex << from << "->"
                   << to << dec << endl;
}
...
bool SpringboardBuilder::createRelocSpringboards(const SpringboardReq &req,
  ...
  springboard_cerr << "createRelocSpringboards for " << hex << req.from << dec
                   << endl;
  std::list<Address> relocAddrs;
  block_instance *bbl = req.block;
  for (SpringboardReq::Destinations::const_iterator b_iter =
           req.destinations.begin();
    ...
    for (std::list<Address>::const_reverse_iterator a_iter =
             relocAddrs.rbegin();
         a_iter != relocAddrs.rend(); ++a_iter) {
      // springboard_cerr << "\t Mapped address " << hex << *a_iter << dec <<
      // endl;
      ...
      springboard_cerr << "Adding springboard from " << hex << *a_iter << " to "
                       << addr << dec << endl;
      input.addRaw(*a_iter, addr, newPriority, func, bbl, req.checkConflicts,
