...
RelocGraph::~RelocGraph()
{
    ...
    for(auto f = funcs_to_clean.begin(); f != funcs_to_clean.end(); ++f)
    {
        if(*f)
            (*f)->freeStackMod();
    }
...
void
RelocGraph::addRelocBlock(RelocBlock* t)
{
...
void
RelocGraph::addRelocBlockBefore(RelocBlock* cur, RelocBlock* t)
{
    if(t->type() == RelocBlock::Relocated)
    {
        springboards[std::make_pair(t->block(), t->func())] = t;
...
void
RelocGraph::addRelocBlockAfter(RelocBlock* cur, RelocBlock* t)
{
    if(t->type() == RelocBlock::Relocated)
    {
        springboards[std::make_pair(t->block(), t->func())] = t;
...
RelocBlock*
RelocGraph::find(block_instance* b, func_instance* f) const
{
...
RelocBlock*
RelocGraph::findSpringboard(block_instance* b, func_instance* f) const
{
    Map::const_iterator iter = springboards.find(std::make_pair(b, f));
    if(iter == springboards.end())
        return NULL;
    return iter->second;
}

bool
RelocGraph::setSpringboard(block_instance* from, func_instance* func, RelocBlock* to)
{
    if(springboards.find(std::make_pair(from, func)) == springboards.end())
        return false;
...
RelocEdge*
RelocGraph::makeEdge(TargetInt* s, TargetInt* t, edge_instance* e,
...
bool
RelocGraph::removeEdge(RelocEdge* e)
{
...
void
RelocGraph::removeSource(RelocEdge* e)
{
...
void
RelocGraph::removeTarget(RelocEdge* e)
{
...
void
RelocGraph::link(RelocBlock* s, RelocBlock* t)
{
    if(s)
        s->setNext(t);
    if(t)
...
bool
RelocGraph::interpose(RelocEdge* e, RelocBlock* trace)
{
    // Take s ->(e) t to
    // s ->(e) n, n ->(FT) t
...
bool
RelocGraph::changeTarget(RelocEdge* e, TargetInt* n)
{
    removeTarget(e);
    delete e->trg;
    e->trg = n;
...
bool
RelocGraph::changeSource(RelocEdge* e, TargetInt* n)
{
    removeSource(e);
    delete e->src;
    e->src = n;
...
bool
RelocGraph::changeType(RelocEdge* e, ParseAPI::EdgeTypeEnum t)
{
    e->type = t;
    return true;
}
...
bool
Predicates::Interprocedural::operator()(RelocEdge* e)
{
    // Calls and returns are always interprocedural
    if(e->type == ParseAPI::CALL || e->type == ParseAPI::RET)
        return true;

    // If there is an underlying edge_instance, use its interproc() method
    if(e->edge != NULL)
        return e->edge->interproc();
...
bool
Predicates::Intraprocedural::operator()(RelocEdge* e)
{
    // Calls and returns are always interprocedural
    if(e->type == ParseAPI::CALL || e->type == ParseAPI::RET)
        return false;
...
bool
Predicates::Fallthrough::operator()(RelocEdge* e)
{
    return (e->type == ParseAPI::FALLTHROUGH);
}

bool
Predicates::Call::operator()(RelocEdge* e)
{
    return (e->type == ParseAPI::CALL);
}

bool
Predicates::NonCall::operator()(RelocEdge* e)
{
    return (e->type != ParseAPI::CALL);
}

bool
Predicates::CallFallthrough::operator()(RelocEdge* e)
{
    return (e->type == ParseAPI::CALL_FT);
}

bool
Predicates::Edge::operator()(RelocEdge* e)
{
    if(e->type != e_->type())
        return false;
    // Match up source and target blocks
    if(e->src->block() == e_->src() && e->trg->block() == e_->trg())
        return true;
    return false;
}

bool
Predicates::Type::operator()(RelocEdge* e)
{
    return e->type == t_;
}
