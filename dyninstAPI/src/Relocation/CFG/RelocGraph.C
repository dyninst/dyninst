...
RelocBlock *RelocGraph::findSpringboard(block_instance *b,
                                        func_instance *f) const {
  Map::const_iterator iter = springboards.find(std::make_pair(b, f));
  if (iter == springboards.end()) return NULL;
  return iter->second;
}

bool RelocGraph::setSpringboard(block_instance *from, func_instance *func,
                                RelocBlock *to) {
...
RelocEdge *RelocGraph::makeEdge(TargetInt *s, TargetInt *t, edge_instance *e,
...
bool Predicates::Interprocedural::operator()(RelocEdge *e) {
  return (e->type == ParseAPI::CALL || e->type == ParseAPI::RET);
}
...
bool Predicates::Edge::operator()(RelocEdge *e) {
  ...
  if (e->src->block() == e_->src() && e->trg->block() == e_->trg()) return true;
  return false;
}

bool Predicates::Type::operator()(RelocEdge *e) { return e->type == t_; }
