...
class RelocBlock {
 ...
 public:
  ...
  typedef enum { Relocated, Instrumentation, Stub } Type;
  ...
  // Code generation
  bool applyPatches(codeGen &gen, bool &regenerate, unsigned &totalSize,
                    int &shift);
 ...
 private:
  ...
  typedef enum { InEdge, OutEdge } EdgeDirection;
  ...
  bool isNecessary(TargetInt *target, ParseAPI::EdgeTypeEnum edgeType);
  ...
  // list overlaid on a graph. Use the list to traverse in layout order; use the
  // graph
