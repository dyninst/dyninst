...
public:
    ...
    typedef std::map<std::pair<block_instance*, func_instance*>, RelocBlock*> Map;
    ...
    RelocEdge* makeEdge(TargetInt*, TargetInt*, edge_instance* e,
...
template <class Predicate>
void
RelocGraph::applyPredicate(Predicate& p, RelocEdges* e, RelocEdges& results)
{
    for(RelocEdges::iterator iter = e->begin(); iter != e->end(); ++iter)
    {
        if(p(*iter))
...
template <class Predicate, class Dest>
bool
RelocGraph::changeTargets(Predicate& p, RelocEdges* e, Dest n)
{
    RelocEdges tmp;
    applyPredicate(p, e, tmp);
    for(RelocEdges::iterator iter = tmp.begin(); iter != tmp.end(); ++iter)
    {
        if(!changeTarget(*iter, new Target<Dest>(n)))
            return false;
    }
    return true;
}
...
template <class Predicate, class Source>
bool
RelocGraph::changeSources(Predicate& p, RelocEdges* e, Source n)
{
    RelocEdges tmp;
    applyPredicate(p, e, tmp);
    for(RelocEdges::iterator iter = tmp.begin(); iter != tmp.end(); ++iter)
    {
        if(!changeSource(*iter, new Target<Source>(n)))
            return false;
    }
    return true;
}
...
template <class Predicate>
bool
RelocGraph::removeEdge(Predicate& p, RelocEdges* e)
{
    RelocEdges tmp;
    applyPredicate(p, e, tmp);
    for(RelocEdges::iterator iter = tmp.begin(); iter != tmp.end(); ++iter)
...
template <class Predicate>
bool
RelocGraph::interpose(Predicate& p, RelocEdges* e, RelocBlock* t)
{
    RelocEdges tmp;
    applyPredicate(p, e, tmp);
    for(RelocEdges::iterator iter = tmp.begin(); iter != tmp.end(); ++iter)
...
template <class Predicate>
bool
RelocGraph::changeType(Predicate& p, RelocEdges* e, ParseAPI::EdgeTypeEnum t)
{
    RelocEdges tmp;
    applyPredicate(p, e, tmp);
    for(RelocEdges::iterator iter = tmp.begin(); iter != tmp.end(); ++iter)
    {
        if(!changeType(*iter, t))
            return false;
    }
    return true;
}
