...
RelocBlock*
RelocBlock::createReloc(block_instance* block, func_instance* func)
{
    if(!block)
        return NULL;
    ...
    int cnt = 0;
    for(block_instance::Insns::iterator iter = insns.begin(); iter != insns.end();
        ++iter, ++cnt)
    {
        if(block->_ignorePowerPreamble && cnt < 2)
            continue;
        relocation_cerr << "  Adding instruction @" << std::hex << iter->first << std::dec
...
RelocBlock*
RelocBlock::createInst(instPoint* p, Address a, block_instance* block, func_instance* f)
{
    if(!p)
        return NULL;
    if(p->empty())
...
RelocBlock*
RelocBlock::createStub(block_instance* block, func_instance* f)
{
...
bool
RelocBlock::linkRelocBlocks(RelocGraph* cfg)
{
...
void
RelocBlock::getSuccessors(RelocGraph* cfg)
{
    // We're constructing a copy of a subgraph of the CFG. Initially we just copy the
    // nodes (aka RelocBlocks), but we also need to copy edges. There are three types of
    // edges we care care about: Edges to a block that corresponds to a RelocBlock
    // (RelocBlockEdges) Edges to a block that does not correspond to a RelocBlock
    // (BlockEdges) Edges to a raw address, caused by representing inter-CodeObject edges
    //   -- this last is a Defensive mode special.
    /*   const block_instance::edgelist &targets = block_->targets();
         for (block_instance::edgelist::const_iterator iter = targets.begin(); iter !=
       targets.end(); ++iter) {*/
...
void
RelocBlock::getPredecessors(RelocGraph* cfg)
{
    const PatchBlock::edgelist& edges = block_->sources();
    for(PatchBlock::edgelist::const_iterator iter = edges.begin(); iter != edges.end();
        ++iter)
    {
...
void
RelocBlock::processEdge(EdgeDirection e, edge_instance* edge, RelocGraph* cfg)
{
    ParseAPI::EdgeTypeEnum type = edge->type();
    // Maybe we want exception edges too?
    if(type == ParseAPI::RET || type == ParseAPI::NOEDGE)
        return;
    ...
    else
    {
        ...
        if(t)
        {
            if(e == OutEdge)
            {
                cfg->makeEdge(new Target<RelocBlock*>(this), new Target<RelocBlock*>(t),
                              edge, type);
                return;
            }
            else
...
bool
RelocBlock::determineSpringboards(PriorityMap& p)
{
    ...
    if(func_ && func_->entryBlock() == block_)
    {
        relocation_cerr << "determineSpringboards (entry block): " << func_->symTabName()
                        << " / " << hex << block_->start() << " is required" << dec
                        << endl;
    ...
    // Slow crawl
    for(RelocEdges::const_iterator iter = inEdges_.begin(); iter != inEdges_.end();
        ++iter)
    {
        ...
        if((*iter)->src->type() != TargetInt::RelocBlockTarget)
        {
            relocation_cerr << "determineSpringboards (non-relocated source): "
                            << func_->symTabName() << " / " << hex << block_->start()
                            << " is required (type " << (*iter)->src->type() << ")" << dec
                            << endl;
...
RelocBlock::createCFWidget()
{
    ...
    if(insn.isValid())
    {
        if(insn.getCategory() == c_CallInsn || insn.getCategory() == c_ReturnInsn ||
...
void
RelocBlock::preserveBlockGap()
{
    /*   const block_instance::edgelist &targets = block_->targets();
         for (block_instance::edgelist::const_iterator iter = targets.begin(); iter !=
       targets.end(); ++iter) {*/
    if(block_->wasUserAdded())
        return;
    const PatchBlock::edgelist& targets = block_->targets();
    bool                        hasCall = false;
    bool                        hasFT   = false;
...
void
RelocBlock::determineNecessaryBranches(RelocBlock* successor)
{
    for(CFWidget::DestinationMap::const_iterator d_iter =
            cfWidget_->destinations().begin();
        d_iter != cfWidget_->destinations().end(); ++d_iter)
    {
        if(d_iter->first != CFWidget::Taken && d_iter->first != CFWidget::Fallthrough)
            continue;
...
bool
RelocBlock::generate(const codeGen& templ, CodeBuffer& buffer)
{
    relocation_cerr << "Generating block " << id() << " orig @ " << hex << origAddr()
                    << dec << endl;
...
std::string
RelocBlock::format() const
{
    stringstream ret;
    ret << "RelocBlock(" << func()->obj()->fullName() << ": " << func()->name() << " "
        << std::hex << origAddr() << std::dec << "/" << id() << "/" << label_ << ") {"
        << endl;
    for(WidgetList::const_iterator iter = elements_.begin(); iter != elements_.end();
        ++iter)
    {
    ...
    ret << endl;
    ret << "Out edges:";
    for(RelocEdges::const_iterator iter = outEdges_.begin(); iter != outEdges_.end();
        ++iter)
    {
    ...
    for(RelocEdges::iterator iter = outEdges_.begin(); iter != outEdges_.end(); ++iter)
    {
           ...
           (*iter)->type == ParseAPI::CALL_FT)
        {
            index = CFWidget::Fallthrough;
                ...
                (*iter)->type == ParseAPI::COND_TAKEN || (*iter)->type == ParseAPI::CALL)
        {
...
bool
RelocBlock::isNecessary(TargetInt* target, ParseAPI::EdgeTypeEnum edgeType)
{
    if(!next_)
        return true;
...
void
RelocBlock::setCF(CFWidgetPtr cf)
{
    elements_.pop_back();
