...
struct SpringboardReq {
  ...
  bool useTrap;
  SpringboardReq(const Address from_, const Address to_,
                 const Priority priority_, func_instance *func_,
                 block_instance *block_, bool checkConflicts_,
                 bool includeRelocCopies_, bool fromRelocCode_, bool useTrap_)
      : from(from_),
        priority(priority_),
        func(func_),
        block(block_),
  ...
  SpringboardReq()
      : from(0),
        ...
        useTrap(false){};
  void addReq(const Address from_, const Address to_, const Priority priority_,
              func_instance *func_, block_instance *block_,
              bool checkConflicts_, bool includeRelocCopies_,
              bool fromRelocCode_, bool useTrap_) {
...
class SpringboardMap {
 ...
 public:
  ...
  bool empty() const { return sBoardMap_.empty(); }

  void addFromOrigCode(Address from, Address to, Priority p,
                       func_instance *func, block_instance *bbl) {
    // This uses the default constructor if it isn't already there.
    sBoardMap_[p][from].addReq(from, to, p, func, bbl, true, true, false,
                               false);
  }

  void addFromRelocatedCode(Address from, Address to, Priority p) {
    assert(p < RELOC_MAX_PRIORITY);
    sBoardMap_[p][from] =
        SpringboardReq(from, to, p, NULL, NULL, true, false, true, false);
  };

  void addRaw(Address from, Address to, Priority p, func_instance *func,
              block_instance *bbl, bool checkConflicts,
              bool includeRelocatedCopies, bool fromRelocatedCode,
              bool useTrap) {
    sBoardMap_[p][from] =
        SpringboardReq(from, to, p, func, bbl, checkConflicts,
                       includeRelocatedCopies, fromRelocatedCode, useTrap);
  }
...
// Persistent tracking of things that have already gotten springboards across
// multiple
...
struct SpringboardInfo {
  ...
  SpringboardInfo(int v, func_instance *f)
      : val(v), func(f), priority(MIN_PRIORITY) {}
  SpringboardInfo(int v, func_instance *f, Priority p)
      : val(v), func(f), priority(p) {}
};
...
class InstalledSpringboards {
 public:
  ...
  void registerBranch(Address start, Address end,
                      ...
                      bool inRelocatedCode, func_instance *func, Priority p);
  void registerBranchInRelocated(Address start, Address end,
                                 func_instance *func, Priority p);
  bool forceTrap(Address a) { return relocTraps_.find(a) != relocTraps_.end(); }
...
class SpringboardBuilder {
  typedef enum { Failed, MultiNeeded, Succeeded } generateResult_t;

 public:
  typedef boost::shared_ptr<SpringboardBuilder> Ptr;
  typedef std::set<func_instance *> FuncSet;

  static Ptr createFunc(FuncSet::const_iterator begin,
                        FuncSet::const_iterator end, AddressSpace *addrSpace);

  bool generate(std::list<codeGen> &springboards, SpringboardMap &input);

 private:
  SpringboardBuilder(AddressSpace *a);

  bool generateInt(std::list<codeGen> &springboards, SpringboardMap &input,
                   Priority p);
  ...
  // them.
  bool createRelocSpringboards(const SpringboardReq &r, bool useTrap,
                               SpringboardMap &input);

  bool generateReplacements(std::list<codeGen> &input, const SpringboardReq &p,
                            bool useTrap);
  ...
  bool conflict(Address start, Address end, bool inRelocatedCode,
                func_instance *func, Priority p) {
    return installed_springboards_->conflict(start, end, inRelocatedCode, func,
                                             p);
  }
  ...
  void registerBranch(Address start, Address end,
                      const SpringboardReq::Destinations &dest,
                      bool inRelocatedCode, func_instance *func, Priority p) {
    return installed_springboards_->registerBranch(start, end, dest,
                                                   inRelocatedCode, func, p);
  }
