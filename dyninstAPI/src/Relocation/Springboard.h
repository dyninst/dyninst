...
struct SpringboardReq
{
    ...
    bool            useTrap;
    SpringboardReq(const Address from_, const Address to_, const Priority priority_,
                   func_instance* func_, block_instance* block_, bool checkConflicts_,
...
public:
    ...
    void addFromOrigCode(Address from, Address to, Priority p, func_instance* func,
                         block_instance* bbl)
    {
    ...
    void addRaw(Address from, Address to, Priority p, func_instance* func,
                block_instance* bbl, bool checkConflicts, bool includeRelocatedCopies,
                bool fromRelocatedCode, bool useTrap)
    {
        sBoardMap_[p][from] =
            SpringboardReq(from, to, p, func, bbl, checkConflicts, includeRelocatedCopies,
...
public:
    ...
    bool conflict(Address start, Address end, bool inRelocatedCode, func_instance* func,
                  Priority p);
    bool conflictInRelocated(Address start, Address end);

    void registerBranch(Address start, Address end,
                        const SpringboardReq::Destinations& dest, bool inRelocatedCode,
                        func_instance* func, Priority p);
    void registerBranchInRelocated(Address start, Address end, func_instance* func,
                                   Priority p);
...
private:
    ...
    bool generateInt(std::list<codeGen>& springboards, SpringboardMap& input, Priority p);

    generateResult_t generateSpringboard(std::list<codeGen>&   gens,
                                         const SpringboardReq& p);

    bool generateMultiSpringboard(std::list<codeGen>& input, const SpringboardReq& p);
    ...
    bool conflict(Address start, Address end, bool inRelocatedCode, func_instance* func,
                  Priority p)
    {
        return installed_springboards_->conflict(start, end, inRelocatedCode, func, p);
    }
