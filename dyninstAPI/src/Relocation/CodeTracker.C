...
CodeTracker::~CodeTracker() {
  ...
  for (TrackerList::iterator iter = trackers_.begin(); iter != trackers_.end();
       ++iter) {
    delete (*iter);
  }
  trackers_.clear();
...
CodeTracker *CodeTracker::fork(CodeTracker *parent, AddressSpace *child) {
...
bool CodeTracker::origToReloc(Address origAddr, block_instance *block,
                              func_instance *func,
  ...
  FwdMapMiddle::const_iterator iter2 =
      iter->second.find((func ? func->addr() : 0));
...
bool CodeTracker::relocToOrig(Address relocAddr, RelocInfo &ri) const {
  TrackerElement *e = NULL;
  if (!relocToOrig_.find(relocAddr, e)) return false;
...
TrackerElement *CodeTracker::findByReloc(Address addr) const {
  TrackerElement *e = NULL;
  if (!relocToOrig_.find(addr, e)) return NULL;
  return e;
}
...
void CodeTracker::addTracker(TrackerElement *e) {
  ...
  if (!trackers_.empty()) {
    ...
    if (previous->mergeable() && e->mergeable() &&
...
std::ostream &operator<<(std::ostream &os,
  ...
  os << "Tracker(" << hex << e.orig() << "," << e.reloc() << "," << dec
     << e.size();
  switch (e.type()) {
    case TrackerElement::original:
      os << ",o";
