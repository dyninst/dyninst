...
CodeTracker::~CodeTracker()
{
    ...
    for(TrackerList::iterator iter = trackers_.begin(); iter != trackers_.end(); ++iter)
    {
        delete(*iter);
    }
    trackers_.clear();
...
CodeTracker*
CodeTracker::fork(CodeTracker* parent, AddressSpace* child)
{
        ...
        iter != parent->trackers_.end(); ++iter)
    {
        ...
        func_instance*  cF =
            (pE->func() ? child->findFunction(pE->func()->ifunc()) : NULL);
        switch(pE->type())
        {
...
bool
CodeTracker::origToReloc(Address origAddr, block_instance* block, func_instance* func,
                         RelocatedElements& reloc) const
{
    ForwardMap::const_iterator iter = origToReloc_.find(block->start());
    if(iter == origToReloc_.end())
        return false;
...
bool
CodeTracker::relocToOrig(Address relocAddr, RelocInfo& ri) const
{
...
TrackerElement*
CodeTracker::findByReloc(Address addr) const
{
    TrackerElement* e = NULL;
...
void
CodeTracker::addTracker(TrackerElement* e)
{
    ...
    if(!trackers_.empty())
    {
        ...
        if(previous->mergeable() && e->mergeable() && previous->type() == e->type() &&
           e->orig() == (previous->orig() + previous->size()) &&
           e->block() == previous->block() && e->func() == previous->func())
        {
            // Merge!
...
CodeTracker::createIndices()
{
    ...
    for(TrackerList::iterator iter = trackers_.begin(); iter != trackers_.end(); ++iter)
    {
        ...
        if(e->type() == TrackerElement::instrumentation)
        {
            ...
            origToReloc_[e->block()->start()][e->func() ? e->func()->addr() : 0]
                        [e->orig()]
                            .instrumentation[inst->baseT()->point()] = e->reloc();
        }
...
CodeTracker::debug()
{
        ...
        iter != origToReloc_.end(); ++iter)
    {
            ...
            iter2 != iter->second.end(); ++iter2)
        {
            ...
            {
                cerr << "\t\t" << hex << iter3->first << " " << iter3->second.instruction
                     << "(insn)" << iter3->second.instrumentation.size() << " (bts)"
                     << ", block @" << iter->first << ", func @" << iter2->first << dec
                     << endl;
            }
        }
    }
    ...
    for(unsigned i = 0; i < reverseEntries.size(); ++i)
    {
        cerr << "\t" << hex << reverseEntries[i].first.first << "-"
             << reverseEntries[i].first.second << ": " << *(reverseEntries[i].second)
             << dec << endl;
    }
...
{
    os << "Tracker(" << hex << e.orig() << "," << e.reloc() << "," << dec << e.size();
    switch(e.type())
    {
        case TrackerElement::original:
            os << ",o";
