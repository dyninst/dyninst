...
////////////////////////
PCWidget::Ptr PCWidget::create(Instruction::Ptr insn, Address addr, Absloc a,
                               Address thunk) {
...
bool PCWidget::generate(const codeGen &templ, const RelocBlock *trace,
                        CodeBuffer &buffer) {
...
bool PCWidget::PCtoReturnAddr(const codeGen &templ, const RelocBlock *t,
  ...
  if (templ.addrSpace()->proc()) {
    ...
    if (!point ||
        (point->type() != instPoint::PreInsn && point->insnAddr() != addr())) {
...
string PCWidget::format() const {
  stringstream ret;
  ret << "PCWidget(" << std::hex << addr_ << std::dec;
  ret << "" << a_.format();
  return ret.str();
}
...
// TODO get rid of the #ifdef here...
  ...
  *temp = offset;
  ...
  // relocaAddr may have moved if we added instructions to setup a new stack
  // frame
