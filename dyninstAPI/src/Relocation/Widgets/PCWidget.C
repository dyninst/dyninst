...
////////////////////////
PCWidget::Ptr
PCWidget::create(Instruction insn, Address addr, Absloc a, Address thunk)
{
    return Ptr(new PCWidget(insn, addr, a, thunk));
}

TrackerElement*
PCWidget::tracker(const RelocBlock* t) const
{
    assert(addr_ != 1);
...
bool
PCWidget::generate(const codeGen& templ, const RelocBlock* trace, CodeBuffer& buffer)
{
