...
class PCWidget : public Widget {
 ...
 private:
  PCWidget(InstructionAPI::Instruction::Ptr insn, Address addr, Absloc &a,
           Address thunkAddr = 0)
      : insn_(insn), addr_(addr), a_(a), thunkAddr_(thunkAddr){};
...
struct IPPatch : public Patch {
  ...
  IPPatch(Type a, Address b, Register c, Address d,
          InstructionAPI::Instruction::Ptr e, block_instance *f,
          func_instance *g)
      : type(a), addr(b), reg(c), thunk(d), insn(e), block(f), func(g){};
