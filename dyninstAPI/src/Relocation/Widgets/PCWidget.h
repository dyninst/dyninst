...
public:
    ...
    static Ptr   create(InstructionAPI::Instruction insn, Address addr, Absloc a,
                        Address thunk = 0);
...
private:
    PCWidget(InstructionAPI::Instruction insn, Address addr, Absloc& a,
             Address thunkAddr = 0)
    : insn_(insn)
    , addr_(addr)
    , a_(a)
...
struct IPPatch : public Patch
{
    ...
    IPPatch(Type a, Address b, InstructionAPI::Instruction c, block_instance* d,
            func_instance* e)
    : type(a)
    , addr(b)
    , reg((Register) -1)
    ...
    , func(e)
    {}
    IPPatch(Type a, Address b, Register c, Address d, InstructionAPI::Instruction e,
            block_instance* f, func_instance* g)
    : type(a)
    , addr(b)
    , reg(c)
    , thunk(d)
