...
StackModWidget::Ptr
StackModWidget::create(Instruction insn, Address addr, signed long newDisp,
                       Architecture arch)
{
    assert(addr);
...
TrackerElement*
StackModWidget::tracker(const RelocBlock* t) const
{
...
bool
StackModWidget::generate(const codeGen&, const RelocBlock* t, CodeBuffer& buffer)
{
    relocation_cerr << "  Generating a stackframe-sensitive memory access ("
                    << insn_.format() << "," << std::hex << addr_ << ", newDisp "
                    << newDisp_ << std::dec << ")" << endl;
...
bool
StackModPatch::apply(codeGen& gen, CodeBuffer*)
{
