...
bool
CFWidget::generateIndirect(CodeBuffer& buffer, Register reg, const RelocBlock* trace,
                           Instruction insn)
{
    ...
    std::vector<unsigned char> raw(ptr, ptr + insn.size());
    ...
    for(unsigned i = loc.num_prefixes; i < loc.num_prefixes + (unsigned) loc.opcode_size;
        ++i)
    {
        switch(raw[i])
        {
    ...
    for(int i = loc.num_prefixes + (int) loc.opcode_size; i < (int) insn.size(); ++i)
    {
        if((i == loc.modrm_position) && fiddle_mod_rm)
        {
            raw[i] |= 0x20;
            raw[i] &= ~0x10;
        }
...
bool
CFWidget::generateIndirectCall(CodeBuffer& buffer, Register reg, Instruction insn,
                               const RelocBlock* trace, Address /*origAddr*/)
{
    ...
    if(ugly_insn.type() & REL_D_DATA)
    {
        ...
        CFPatch* newPatch = new CFPatch(
            CFPatch::Data, insn, new Target<Address>(origTarget_), trace->func(), addr_);
...
bool
CFPatch::apply(codeGen& gen, CodeBuffer* buf)
{
    ...
    relocation_cerr << "\t\t CFPatch::apply, type " << type << ", origAddr " << hex
                    << origAddr_ << ", and label " << dec << targetLabel << endl;
    if(orig_insn.isValid())
    {
        relocation_cerr << "\t\t\t Currently at " << hex << gen.currAddr()
                        << " and targeting predicted " << buf->predictedAddr(targetLabel)
                        << dec << endl;
        switch(type)
        {
            case CFPatch::Jump: {
                relocation_cerr << "\t\t\t Generating CFPatch::Jump from " << hex
                                << gen.currAddr() << " to "
                                << buf->predictedAddr(targetLabel) << dec << endl;
                if(!insnCodeGen::modifyJump(buf->predictedAddr(targetLabel), *ugly_insn,
                                            gen))
                {
            ...
            case CFPatch::JCC: {
                relocation_cerr << "\t\t\t Generating CFPatch::JCC from " << hex
                                << gen.currAddr() << " to "
                                << buf->predictedAddr(targetLabel) << dec << endl;
                if(!insnCodeGen::modifyJcc(buf->predictedAddr(targetLabel), *ugly_insn,
                                           gen))
                {
            ...
            case CFPatch::Call: {
                if(!insnCodeGen::modifyCall(buf->predictedAddr(targetLabel), *ugly_insn,
                                            gen))
                {
            ...
            case CFPatch::Data: {
                if(!insnCodeGen::modifyData(buf->predictedAddr(targetLabel), *ugly_insn,
                                            gen))
                {
    ...
    else
    {
        ...
        {
            case CFPatch::Jump:
                insnCodeGen::generateBranch(gen, gen.currAddr(),
                                            buf->predictedAddr(targetLabel));
                break;
            case CFPatch::Call:
                insnCodeGen::generateCall(gen, gen.currAddr(),
                                          buf->predictedAddr(targetLabel));
                break;
            default:
                assert(0);
...
bool
CFPatch::isPLT(codeGen& gen)
{
    if(!gen.addrSpace()->edit())
        return false;
...
bool
CFPatch::applyPLT(codeGen& gen, CodeBuffer*)
{
    ...
    {
        cerr << "Attempt to make PLT of type " << type
             << " and can only handle calls or jumps" << endl;
        return false;
    }
...
#if !defined(cap_mem_emulation)
bool
CFWidget::generateAddressTranslator(CodeBuffer&, const codeGen&, Register&,
                                    const RelocBlock*)
{
    return true;
}
                                    ...
                                    Register& reg, const RelocBlock* trace)
{
    ...
    if(start->getReg(0) == REGNUM_ESP || start->getReg(1) == REGNUM_ESP)
    {
    ...
    if(!isCall_ && insn_->getCategory() == c_CallInsn)
        stackShift -= 4;
    ...
    // Now we start stealing from memEmulatorWidget. We need to call our translation
    // function, which means a non-PIC patch to the CodeBuffer. I don't feel like
    // rewriting everything, so there we go.
    ...
    // Dereference
    ::emitMovRMToReg(RealRegister(REGNUM_ECX), RealRegister(REGNUM_ECX), 0, patch);

    // ECX now holds the _actual_ destination, so move it on to the stack.
    // We've got ECX saved
    ::emitMovRegToRM(RealRegister(REGNUM_ESP), 1 * 4, RealRegister(REGNUM_ECX), patch);
