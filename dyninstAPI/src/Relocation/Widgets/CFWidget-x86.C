...
bool CFWidget::generateIndirect(CodeBuffer &buffer, Register reg,
  ...
  ia32_decode(IA32_FULL_DECODER, (const unsigned char *)insn->ptr(),
              orig_instr);
  ...
  for (unsigned i = loc.num_prefixes;
       i < loc.num_prefixes + (unsigned)loc.opcode_size; ++i) {
    switch (raw[i]) {
      case 0xE8:
        raw[i] = 0xE9;
        break;
  ...
  for (int i = loc.num_prefixes + (int)loc.opcode_size; i < (int)insn->size();
       ++i) {
...
bool CFWidget::generateIndirectCall(CodeBuffer &buffer, Register reg,
  ...
  if (ugly_insn.type() & REL_D_DATA) {
    ...
    CFPatch *newPatch =
        new CFPatch(CFPatch::Data, insn, new Target<Address>(origTarget_),
                    trace->func(), addr_);
...
bool CFPatch::apply(codeGen &gen, CodeBuffer *buf) {
  ...
  relocation_cerr << "\t\t CFPatch::apply, type " << type << ", origAddr "
                  << hex << origAddr_ << ", and label " << dec << targetLabel
                  << endl;
  if (orig_insn) {
    relocation_cerr << "\t\t\t Currently at " << hex << gen.currAddr()
                    << " and targeting predicted "
    ...
    switch (type) {
      case CFPatch::Jump: {
        relocation_cerr << "\t\t\t Generating CFPatch::Jump from " << hex
                        << gen.currAddr() << " to "
                        << buf->predictedAddr(targetLabel) << dec << endl;
        if (!insnCodeGen::modifyJump(buf->predictedAddr(targetLabel),
                                     *ugly_insn, gen)) {
      ...
      case CFPatch::Call: {
        if (!insnCodeGen::modifyCall(buf->predictedAddr(targetLabel),
                                     *ugly_insn, gen)) {
      ...
      case CFPatch::Data: {
        if (!insnCodeGen::modifyData(buf->predictedAddr(targetLabel),
                                     *ugly_insn, gen)) {
...
bool CFPatch::applyPLT(codeGen &gen, CodeBuffer *) {
  ...
  relocation_cerr << "Emitting a PLT jump/call, targeting " << callee->name()
                  << endl;
...
#if !defined(cap_mem_emulation)
bool CFWidget::generateAddressTranslator(CodeBuffer &, const codeGen &,
                                         Register &, const RelocBlock *) {
  return true;
}
#else
bool CFWidget::generateAddressTranslator(CodeBuffer &buffer,
  ...
  if (start->getReg(0) == REGNUM_ESP || start->getReg(1) == REGNUM_ESP) {
    cerr << "ERROR: CF insn that uses the stack pointer! " << insn_->format()
         << endl;
  }

  int stackShift = -16;
  // If we are a call _instruction_ but isCall is false, then we've got an extra
  // word
  ...
  // Where are we going?
  func_instance *func =
      templ.addrSpace()->findOnlyOneFunction("RTtranslateMemory");
  // FIXME for static rewriting; this is a dynamic-only hack for proof of
  // concept.
  assert(func);

  // Now we start stealing from memEmulatorWidget. We need to call our
  // translation function,
  // which means a non-PIC patch to the CodeBuffer. I don't feel like rewriting
  // everything,
  // so there we go.
  buffer.addPatch(
      new MemEmulatorPatch(REGNUM_ECX, REGNUM_ECX, addr_, func->addr()),
      tracker(trace));
  patch.setIndex(0);
  ...
  // Dereference
  ::emitMovRMToReg(RealRegister(REGNUM_ECX), RealRegister(REGNUM_ECX), 0,
                   patch);

  // ECX now holds the _actual_ destination, so move it on to the stack.
  // We've got ECX saved
  ::emitMovRegToRM(RealRegister(REGNUM_ESP), 1 * 4, RealRegister(REGNUM_ECX),
                   patch);
