...
bool ASTWidget::generate(const codeGen &, const RelocBlock *,
                         CodeBuffer &buffer) {
...
TrackerElement *ASTWidget::tracker() const {
  OriginalTracker *e = new OriginalTracker(point_->addr_compat(),
                                           point_->block(), point_->func());
  return e;
}

std::string ASTWidget::format() const { return "AST(*)"; }
