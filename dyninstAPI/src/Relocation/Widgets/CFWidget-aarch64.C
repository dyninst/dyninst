...
bool
CFWidget::generateIndirect(CodeBuffer& buffer, Register, const RelocBlock* trace,
                           Instruction insn)
{
    NS_aarch64::instruction mod_insn(insn.ptr());
    // set bit 21 to 0 (bit 21 in the unconditional branch (register) category of
    // instructions indicates whether or not the branch is a call)
...
{
    NS_aarch64::instruction mod_insn(insn.ptr());
    // set bit 21 to 1 (bit 21 in the unconditional branch (register) category of
    // instructions indicates whether or not the branch is a call)
...
CFPatch::apply(codeGen& gen, CodeBuffer* buf)
{
    ...
    if(orig_insn.isValid())
    {
        ...
        switch(type)
        {
            ...
            case CFPatch::Data: {
                /*This patch type is for PC-relative calls and is hence applicable only to
                  x86. The very fact that we're reaching here on ARM means something is
                  wrong.*/
                assert(!"Trying to apply patch for a PC-relative call...should not be "
                        "hitting this on ARM64!");
            }
        }
    }
    else
