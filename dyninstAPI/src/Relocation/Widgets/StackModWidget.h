...
class StackModWidget : public Widget {
 public:
  ...
  static Ptr create(InstructionAPI::Instruction::Ptr insn, Address addr,
                    signed long newDisp, Architecture arch);
 ...
 private:
  StackModWidget(InstructionAPI::Instruction::Ptr insn, Address addr,
                 signed long newDisp, Architecture arch)
      : insn_(insn), addr_(addr), newDisp_(newDisp), arch_(arch){};
...
struct StackModPatch : public Patch {
  StackModPatch(InstructionAPI::Instruction::Ptr a, signed long d,
                Architecture ar, Address ad)
      : orig_insn(a), newDisp(d), arch(ar), addr(ad){};
