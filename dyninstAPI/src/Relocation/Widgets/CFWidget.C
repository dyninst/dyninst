...
CFWidget::Ptr
CFWidget::create(Widget::Ptr atom)
{
    ...
    switch(opt)
    {
        ...
        case Taken_FT: {
            ...
            if(isCall_)
            {
                ...
                if(!generateCall(buffer, destMap_[Taken], trace, insn_))
                    return false;
            }
            else
            {
        ...
        case Indirect: {
            ...
            if(isCall_ || trace->block()->llb()->isIndirectTailCallBlock())
            {
                ...
                if(destMap_.find(Fallthrough) != destMap_.end())
                {
                    if(!generateBranch(buffer, destMap_[Fallthrough], Instruction(),
                                       trace, true))
                        return false;
                }
            }
...
TrackerElement*
CFWidget::tracker(const RelocBlock* trace) const
{
    assert(addr_ != 1);
...
TrackerElement*
CFWidget::destTracker(TargetInt* dest, const RelocBlock* trace) const
{
...
TrackerElement*
CFWidget::addrTracker(Address addr, const RelocBlock* trace) const
{
...
TrackerElement*
CFWidget::padTracker(Address addr, unsigned size, const RelocBlock* trace) const
{
...
void
CFWidget::addDestination(Address index, TargetInt* dest)
{
    assert(dest);
    relocation_cerr << "CFWidget @ " << std::hex << addr() << ", adding destination "
                    << dest->format() << " / " << index << std::dec << endl;

    destMap_[index] = dest;
}

TargetInt*
CFWidget::getDestination(Address dest) const
{
...
bool
CFWidget::generateBranch(CodeBuffer& buffer, TargetInt* to, Instruction insn,
...
bool
CFWidget::generateCall(CodeBuffer& buffer, TargetInt* to, const RelocBlock* trace,
                       Instruction insn)
{
    if(!to)
    {
...
bool
CFWidget::generateConditionalBranch(CodeBuffer& buffer, TargetInt* to,
                                    const RelocBlock* trace, Instruction insn)
{
    assert(to);
...
CFWidget::format() const
{
    ...
    for(DestinationMap::const_iterator iter = destMap_.begin(); iter != destMap_.end();
        ++iter)
    {
        switch(iter->first)
        {
...
CFPatch::CFPatch(Type a, Instruction b, TargetInt* c, const func_instance* d, Address e)
: type(a)
, orig_insn(b)
, target(c)
, func(d)
...
, block_(b)
{
    ...
    // << noop << ", [" << b->start() << " " << b->end() << ") )" << dec <<  endl;
}

bool
PaddingPatch::apply(codeGen& gen, CodeBuffer*)
{
