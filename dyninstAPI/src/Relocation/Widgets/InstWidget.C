...
InstWidget::Ptr
InstWidget::create(instPoint* i)
{
    return Ptr(new InstWidget(i));
}

TrackerElement*
InstWidget::tracker() const
{
    ...
    e = new InstTracker(point_->addr_compat(), point_->tramp(), point_->block_compat(),
                        point_->func());
...
bool
InstWidget::generate(const codeGen&, const RelocBlock*, CodeBuffer& buffer)
{
...
unsigned
InstWidgetPatch::estimate(codeGen&)
{
    return 0;
}
...
bool
RemovedInstWidgetPatch::apply(codeGen& gen, CodeBuffer*)
{
...
unsigned
RemovedInstWidgetPatch::estimate(codeGen&)
{
    return 0;
}
