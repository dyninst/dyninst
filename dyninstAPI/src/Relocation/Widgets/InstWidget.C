...
TrackerElement *InstWidget::tracker() const {
  ...
  e = new InstTracker(point_->addr_compat(), point_->tramp(),
                      point_->block_compat(), point_->func());
...
bool InstWidget::generate(const codeGen &, const RelocBlock *,
                          CodeBuffer &buffer) {
...
std::string InstWidget::format() const { return "InstWidget()"; }

// Could be a lot smarter here...
bool InstWidgetPatch::apply(codeGen &gen, CodeBuffer *) {
  relocation_cerr << "\t\t InstWidgetPatch::apply " << this << " /w/ tramp "
                  << tramp << endl;
...
unsigned InstWidgetPatch::estimate(codeGen &) { return 0; }
...
unsigned RemovedInstWidgetPatch::estimate(codeGen &) { return 0; }
