...
bool
InsnWidget::generate(const codeGen&, const RelocBlock* t, CodeBuffer& buffer)
{
...
TrackerElement*
InsnWidget::tracker(const RelocBlock* t) const
{
...
InsnWidget::Ptr
InsnWidget::create(Instruction insn, Address addr)
{
...
string
InsnWidget::format() const
{
    stringstream ret;
    // ret << "Insn(" << insn_->format() << ")";
    ret << "Insn(" << hex << addr_ << ": " << insn_.format(addr_) << dec << ")";
    return ret.str();
}

unsigned
InsnWidget::size() const
{
    return insn_.size();
}
