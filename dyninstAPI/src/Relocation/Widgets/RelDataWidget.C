...
RelDataWidget::Ptr
RelDataWidget::create(Instruction insn, Address addr, Address target)
{
    assert(addr);
...
TrackerElement*
RelDataWidget::tracker(const RelocBlock* t) const
{
...
bool
RelDataWidget::generate(const codeGen&, const RelocBlock* t, CodeBuffer& buffer)
{
    ...
    relocation_cerr << "  Generating a PC-relative data access (" << insn_.format() << ","
                    << std::hex << addr_ << "," << target_ << std::dec << ")" << endl;
...
bool
RelDataPatch::apply(codeGen& gen, CodeBuffer*)
{
    ...
    {
        relocation_cerr
            << "RelDataPatch returned false from modifyData (original address: "
            << std::hex << orig << ")" << endl;
        return false;
    }
    return true;
