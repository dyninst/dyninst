...
bool
adhocMovementTransformer::process(RelocBlock* cur, RelocGraph* cfg)
{
    ...
    relocation_cerr << "PCRelTrans: processing block (ID= " << cur->id() << ") " << cur
                    << " with " << elements.size() << " elements." << endl;
        ...
        ++iter)
    {
        ...
        {
            relocation_cerr << "  ... isPCRelData at " << std::hex << (*iter)->addr()
                            << std::dec << endl;
            ...
            Widget::Ptr replacement =
                RelDataWidget::create(insn, (*iter)->addr(), target);
            (*iter).swap(replacement);
        }
        else if(isGetPC(*iter, insn, aloc, target))
        {
            Widget::Ptr replacement =
                PCWidget::create(insn, (*iter)->addr(), aloc, target);
                ...
                try
                {
                                             ...
                                             cur->block()->llb(), (*iter)->addr()))
                    {
                        ...
                        stackmods_printf(" ... is Stack Frame SENSITIVE at 0x%lx\n",
                                         (*iter)->addr());
                        stackmods_printf(
                            "\t\t origDisp = %lu, delta = %ld, newDisp = %ld\n", origDisp,
                            delta, newDisp);

                        relocation_cerr
                            << " ... is Stack Frame Sensitive at " << std::hex
                            << (*iter)->addr() << std::dec << ", origDisp = " << origDisp
                            << ", delta = " << delta << ", newDisp = " << newDisp << endl;

                        Widget::Ptr replacement =
                            StackModWidget::create(insn, (*iter)->addr(), newDisp, arch);
                        (*iter).swap(replacement);
                    }
                } catch(stackmod_exception& e)
                {
...
static Address
PCValue(Address addr, Instruction insn)
{
...
bool
adhocMovementTransformer::isPCDerefCF(Widget::Ptr ptr, Instruction insn, Address& target)
{
...
bool
adhocMovementTransformer::isGetPC(Widget::Ptr ptr, Instruction insn, Absloc& aloc,
                                  Address& thunk)
{
    // TODO:
    // Check for call + size;
    ...
    if(addrSpace->isValidAddress(target))
    {
        ...
        relocation_cerr << "      ... decoded target insns " << firstInsn.format() << ", "
           ...
           secondInsn.getCategory() == c_ReturnInsn)
        {
            ...
            relocation_cerr << "Checking operand "
                            << firstInsn.getOperand(1).format(firstInsn.getArch())
                            << endl;
    ...
    {
        relocation_cerr << "\t Call to " << hex << target
                        << " is not valid address, concluding not thunk" << dec << endl;
    }
    return false;
}
...
bool
adhocMovementTransformer::isStackFrameSensitive(Offset& origDisp, signed long& delta,
                                                const Accesses* accesses,
                                                OffsetVector*& offVec, TMap*& tMap,
                                                ParseAPI::Block* block, Address addr)
{
    ...
    if(accesses->size() == 1)
    {
        ...
        StackAnalysis::Height* regDeltas = new StackAnalysis::Height[accessSetSize];
        // Change in access height for each possible location accessed
        StackAnalysis::Height* readDeltas = new StackAnalysis::Height[accessSetSize];
        // Change in definition height for each possible location accessed
        StackAnalysis::Height* defDeltas = new StackAnalysis::Height[accessSetSize];
        ...
        for(auto iter = accessSet.begin(); iter != accessSet.end(); iter++)
        {
            ...
            for(auto tIter = tMap->begin();
                (!foundReg || !foundRead) && tIter != tMap->end(); ++tIter)
            {
                ...
                if(src->isStackMemory() && dest->isStackMemory())
                {
                    ...
                    else
                    {
                           ...
                           access->readHeight() < src->off() + src->size())
                        {
                            ...
                            foundRead = true;
                            readHeightPrime =
                                dest->off() + (access->readHeight() - src->off());
                            stackmods_printf("\t\t\t\t readHeight' = %ld\n",
                                             readHeightPrime.height());
                        }
                    }
        ...
        delta = readDeltas[0].height() - regDeltas[0].height() - defDeltas[0].height();
        for(unsigned j = 1; j < accessSetSize; j++)
        {
            if(delta !=
               readDeltas[j].height() - regDeltas[j].height() - defDeltas[j].height())
            {
