...
bool adhocMovementTransformer::process(RelocBlock* cur, RelocGraph* cfg) {
  ...
  relocation_cerr << "PCRelTrans: processing block (ID= " << cur->id() << ") "
                  << cur << " with " << elements.size() << " elements." << endl;
  ...
  for (RelocBlock::WidgetList::iterator iter = elements.begin();
    ...
    if (isPCRelData(*iter, insn, target)) {
      relocation_cerr << "  ... isPCRelData at " << std::hex << (*iter)->addr()
                      << std::dec << endl;
      // Two options: a memory reference or a indirect call. The indirect call
      // we
      // just want to set target in the CFWidget, as it has the hardware to
      // handle
      ...
      if (cur->func()->hasStackMod() && cur->func()->getMods()->size()) {
        ...
        if (isStackFrameSensitive(origDisp, delta, accesses, offVec, tMap,
          ...
          relocation_cerr << " ... is Stack Frame Sensitive at " << std::hex
                          << (*iter)->addr() << std::dec
                          << ", origDisp = " << origDisp
                          << ", delta = " << delta << ", newDisp = " << newDisp
                          << endl;
...
bool adhocMovementTransformer::isPCDerefCF(Widget::Ptr ptr, InsnPtr insn,
                                           Address& target) {
  ...
  for (set<Expression::Ptr>::const_iterator iter = mems.begin();
    ...
    if (exp->bind(thePC.get(), Result(u64, ptr->addr() + insn->size()))) {
      ...
      Result res = exp->eval();
      if (!res.defined) {
        cerr << "ERROR: failed bind/eval at " << std::hex << ptr->addr()
             << endl;
...
bool adhocMovementTransformer::isPCRelData(Widget::Ptr ptr, InsnPtr insn,
                                           Address& target) {
  target = 0;
  if (insn->getControlFlowTarget()) return false;
...
bool adhocMovementTransformer::isGetPC(Widget::Ptr ptr, InsnPtr insn,
                                       Absloc& aloc, Address& thunk) {
  // TODO:
  // Check for call + size;
  ...
  if (addrSpace->isValidAddress(target)) {
    ...
    relocation_cerr << "\t Call to " << hex << target
                    << " is not valid address, concluding not thunk" << dec
                    << endl;
  }
  return false;
}
...
bool adhocMovementTransformer::isStackFrameSensitive(
    Offset& origDisp, signed long& delta, const Accesses* accesses,
    OffsetVector*& offVec, TMap*& tMap, ParseAPI::Block* block, Address addr) {
  ...
  for (auto iter = accesses->begin(); iter != accesses->end(); ++iter) {
    ...
    stackmods_printf("\t\t regHeight = %ld, readHeight = %ld\n",
                     access->regHeight().height(),
                     access->readHeight().height());
