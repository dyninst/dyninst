...
public:
    ...
    adhocMovementTransformer(AddressSpace* as)
    : addrSpace(as)
    {}
...
private:
    bool isPCDerefCF(WidgetPtr ptr, InstructionAPI::Instruction insn, Address& destPtr);
    bool isPCRelData(WidgetPtr ptr, InstructionAPI::Instruction insn, Address& target);
    // Records where PC was stored
    bool isGetPC(WidgetPtr ptr, InstructionAPI::Instruction insn, Absloc& aloc,
                 Address& thunkAddr);
#    if defined(cap_stack_mods)
    bool isStackFrameSensitive(Offset& origOffset, signed long& delta,
