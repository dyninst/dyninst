...
class adhocMovementTransformer : public Transformer {
 ...
 private:
  bool isPCDerefCF(WidgetPtr ptr, InsnPtr insn, Address &destPtr);
  bool isPCRelData(WidgetPtr ptr, InsnPtr insn, Address &target);
  // Records where PC was stored
  bool isGetPC(WidgetPtr ptr, InsnPtr insn, Absloc &aloc, Address &thunkAddr);
