...
Modification::Modification(const CallModMap &callMod,
    ...
    : callMods_(callMod), funcReps_(funcRepl), funcWraps_(funcWraps){};
...
bool Modification::replaceCall(RelocBlock *trace, RelocGraph *cfg) {
  ...
  relocation_cerr << "Replacing call in trace " << trace->id()
                  << " with call to " << (repl ? repl->name() : "<NULL>")
                  << ", " << hex << (repl ? repl->addr() : 0) << dec << endl;
...
bool Modification::replaceFunction(RelocBlock *trace, RelocGraph *cfg) {
  ...
  if (target) {
    cfg->makeEdge(new Target<RelocBlock *>(stub),
                  new Target<RelocBlock *>(target), NULL, ParseAPI::DIRECT);
  } else {
    cfg->makeEdge(new Target<RelocBlock *>(stub),
...
bool Modification::wrapFunction(RelocBlock *trace, RelocGraph *cfg) {
  ...
  // This is a special case of replaceFunction; the predicate is "all calls
  // except from the
  // wrapper are redirected".
  RelocBlock *stub = makeRelocBlock(newfun->entryBlock(), newfun, trace, cfg);
  RelocBlock *target = cfg->find(newfun->entryBlock(), newfun);
  if (target) {
    relocation_cerr << "\t Also relocated new function, using target "
                    << target->id() << endl;
    cfg->makeEdge(new Target<RelocBlock *>(stub),
                  new Target<RelocBlock *>(target), NULL, ParseAPI::DIRECT);
  } else {
    relocation_cerr << "\t New function " << newfun->name()
                    ...
                    << newfun->entryBlock()->start() << dec << " directly"
                    << endl;
    cfg->makeEdge(new Target<RelocBlock *>(stub),
                  new Target<block_instance *>(newfun->entryBlock()), NULL,
                  ParseAPI::DIRECT);
  ...
  // We also need to track the "new" entry block so we can build a new symbol
  // for it.
...
RelocBlock *Modification::makeRelocBlock(block_instance *block,
                                         func_instance *func, RelocBlock *trace,
                                         RelocGraph *cfg) {
...
Modification::WrapperPredicate::WrapperPredicate(func_instance *f) : f_(f){};
