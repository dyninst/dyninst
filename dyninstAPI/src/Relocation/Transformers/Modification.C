...
Modification::Modification(const CallModMap& callMod, const FuncModMap& funcRepl,
...
bool
Modification::process(RelocBlock* cur, RelocGraph* cfg)
{
    ...
    if(!replaceCall(cur, cfg))
        return false;
    if(!replaceFunction(cur, cfg))
        return false;
    if(!wrapFunction(cur, cfg))
        return false;
...
bool
Modification::replaceCall(RelocBlock* trace, RelocGraph* cfg)
{
    // See if we have a modification for this point
    CallModMap::const_iterator iter = callMods_.find(trace->block());
    if(iter == callMods_.end())
        return true;
    std::map<PatchFunction*, PatchFunction*>::const_iterator iter2 =
        iter->second.find(trace->func());
    if(iter2 == iter->second.end())
        return true;

    func_instance* repl = SCAST_FI(iter2->second);

    relocation_cerr << "Replacing call in trace " << trace->id() << " with call to "
                    << (repl ? repl->name() : "<NULL>") << ", " << hex
                    << (repl ? repl->addr() : 0) << dec << endl;
    ...
    if(!repl)
    {
        if(!cfg->removeEdge(pred, trace->outs()))
            return false;
        return true;
    }
    ...
    if(target)
    {
        if(!cfg->changeTargets(pred, trace->outs(), target))
            return false;
    }
    else
    {
        if(!cfg->changeTargets(pred, trace->outs(), repl->entryBlock()))
            return false;
    }
...
bool
Modification::replaceFunction(RelocBlock* trace, RelocGraph* cfg)
{
    // See if we were something created later
    if(trace->elements().empty())
        return true;

    // See if we're the entry block
    if(trace->block() != trace->func()->entryBlock())
        return true;

    FuncModMap::const_iterator iter = funcReps_.find(trace->func());
    if(iter == funcReps_.end())
        return true;
    ...
    relocation_cerr << "Performing function replacement in trace " << trace->id()
                    << " going to function " << newfun->name() << " /w/ entry block "
                    << (newfun->entryBlock() ? newfun->entryBlock()->start() : -1)
                    << endl;
    // Stub a jump to the replacement function
    RelocBlock* stub   = makeRelocBlock(oldfun->entryBlock(), oldfun, trace, cfg);
    RelocBlock* target = cfg->find(newfun->entryBlock(), newfun);
    if(target)
    {
        cfg->makeEdge(new Target<RelocBlock*>(stub), new Target<RelocBlock*>(target),
                      NULL, ParseAPI::DIRECT);
    }
    else
    {
        cfg->makeEdge(new Target<RelocBlock*>(stub),
                      new Target<block_instance*>(newfun->entryBlock()), NULL,
                      ParseAPI::DIRECT);
    }
    ...
    if(target)
    {
        if(!cfg->changeTargets(pred, trace->ins(), target))
            return false;
    }
    else
    {
        if(!cfg->changeTargets(pred, trace->ins(), newfun->entryBlock()))
            return false;
    }
    return true;
}
...
bool
Modification::wrapFunction(RelocBlock* trace, RelocGraph* cfg)
{
    // See if we're the entry block
    if(trace->block() != trace->func()->entryBlock())
        return true;
    ...
    // This is a special case of replaceFunction; the predicate is "all calls except from
    // the wrapper are redirected".
    RelocBlock* stub   = makeRelocBlock(newfun->entryBlock(), newfun, trace, cfg);
    RelocBlock* target = cfg->find(newfun->entryBlock(), newfun);
    if(target)
    {
        relocation_cerr << "\t Also relocated new function, using target " << target->id()
                        << endl;
        cfg->makeEdge(new Target<RelocBlock*>(stub), new Target<RelocBlock*>(target),
                      NULL, ParseAPI::DIRECT);
    }
    ...
    {
        relocation_cerr << "\t New function " << newfun->name()
                        << " not relocated targeting entry block " << hex
                        << newfun->entryBlock()->start() << dec << " directly" << endl;
        cfg->makeEdge(new Target<RelocBlock*>(stub),
                      new Target<block_instance*>(newfun->entryBlock()), NULL,
                      ParseAPI::DIRECT);
    }
    ...
    CallbackWidget::Ptr c =
        CallbackWidget::create(new WrapperPatch(trace->func(), newname));
...
RelocBlock*
Modification::makeRelocBlock(block_instance* block, func_instance* func,
                             RelocBlock* trace, RelocGraph* cfg)
{
...
bool
Modification::WrapperPredicate::operator()(RelocEdge* e)
{
...
bool
Modification::WrapperPatch::apply(codeGen& gen, CodeBuffer*)
{
