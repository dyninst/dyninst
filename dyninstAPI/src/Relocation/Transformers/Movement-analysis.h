...
private:
    ...
    bool       isPCSensitive(InstructionAPI::Instruction insn, Address addr,
                             const func_instance* func, const block_instance* block,
                             AssignList& sensitiveAssignment);
    Graph::Ptr forwardSlice(Assignment::Ptr ptr, parse_block* block, parse_func* func);
    bool       determineSensitivity(Graph::Ptr slice, bool& intSens, bool& extSens);

    bool insnIsThunkCall(InstructionAPI::Instruction insn, Address addr,
                         Absloc& destination);
    void handleThunkCall(RelocBlock* b_iter, RelocGraph* cfg, WidgetList::iterator& iter,
                         Absloc& destination);
    void emulateInsn(RelocBlock* b_iter, RelocGraph* cfg, WidgetList::iterator& iter,
    ...
    static void cacheAnalysis(const block_instance* bbl, Address addr, bool intSens,
                              bool extSens);
    static bool queryCache(const block_instance* bbl, Address addr, bool& intSens,
                           bool& extSens);
