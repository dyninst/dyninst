...
class PCSensitiveTransformer : public Transformer {
 ...
 private:
  ...
  bool isPCSensitive(InstructionAPI::Instruction::Ptr insn, Address addr,
                     const func_instance *func, const block_instance *block,
                     AssignList &sensitiveAssignment);
  Graph::Ptr forwardSlice(Assignment::Ptr ptr, parse_block *block,
                          parse_func *func);
  bool determineSensitivity(Graph::Ptr slice, bool &intSens, bool &extSens);

  bool insnIsThunkCall(InstructionAPI::Instruction::Ptr insn, Address addr,
                       Absloc &destination);
  void handleThunkCall(RelocBlock *b_iter, RelocGraph *cfg,
                       WidgetList::iterator &iter, Absloc &destination);
  void recordIntSensitive(Address addr);
  void emulateInsn(RelocBlock *b_iter, RelocGraph *cfg,
  ...
  static void cacheAnalysis(const block_instance *bbl, Address addr,
