...
bool PCSensitiveTransformer::process(RelocBlock *reloc, RelocGraph *g) {
  ...
  for (RelocBlock::WidgetList::iterator iter = elements.begin();
    ...
    if (!isPCSensitive(insn, addr, func, block, sensitiveAssignments)) {
      // cerr << "Instruction " << insn->format() << " not PC sensitive,
      // skipping" << endl;
      continue;
    }
    ...
    sensitivity_cerr << "Instruction is sensitive @ " << hex << addr << dec
                     << endl;

    // Optimization: before we do some heavyweight analysis, see if we can
    // shortcut
    bool intSens = false;
    bool extSens = false;
    bool approx = false;
    ...
    if (exceptionSensitive(addr + insn->size(), block)) {
      extSens = true;
      sensitivity_cerr << "Sensitive by exception @ " << hex << addr << dec
                       << endl;
    }
    ...
    if (!queryCache(block, addr, intSens, extSens)) {
      for (AssignList::iterator a_iter = sensitiveAssignments.begin();
        ...
        // cerr << "Forward slice from " << (*a_iter)->format() << hex << " @ "
        // << addr << " (parse of " << (*a_iter)->addr() << dec << ") in func "
        // << block->func()->prettyName() << endl;

        Graph::Ptr slice = forwardSlice(*a_iter, block->llb(), func->ifunc());
        ...
        if (!slice) {
          ...
          if (slice->size() > 10) {
            ...
            sensitivity_cerr << "\t sens analysis returned "
                             << (intSens ? "intSens" : "") << " / "
    ...
    if (extSens) {
      ...
      // Okay, someone wants the original version. That means, for now, we're
      // emulating.
      if (intSens) {
        // Fun for the whole family! We have one instruction that wants the
        // changed
        // version (likely a load or equivalent) and one instruction that wants
        // the
        // original value (that would be a return). Let's see if we can match a
        // thunk call...
        Absloc destination;
        if (insnIsThunkCall(insn, addr, destination)) {
          // A first example of a group transformation. The "internal" piece
          // comes from
          // calling a 2-instruction function that copies the return address
          // elsewhere
          // and returns. So we can remove the internal sensitivity by inlining
          // the
          // call.
          handleThunkCall(reloc, g, iter, destination);
          continue;
        } else {
          // Not a thunk call, and both externally and internally sensitive.
          // Ugh.
          // Well, because of the external sensitivity we're going to emulate
          // the
          // original instruction, which means the internally sensitive target
          // will
          // be transferring back to the original instruction address. Go ahead
          // and
          // record this...
          recordIntSensitive(addr + insn->size());
        }
      }
      // And now to the real work. Replace this instruction with an emulation
      // sequence
      emulateInsn(reloc, g, iter, insn, addr);
    }
...
bool PCSensitiveTransformer::isPCSensitive(Instruction::Ptr insn, Address addr,
  ...
  aConverter.convert(insn, func->addrToOffset(addr), func->ifunc(),
                     block->llb(), assignments);
  for (std::vector<Assignment::Ptr>::iterator a_iter = assignments.begin();
    ...
    if ((*a_iter)->out().contains(pc)) continue;

    // Do you use PC?
    const std::vector<AbsRegion> &ins = (*a_iter)->inputs();
    for (std::vector<AbsRegion>::const_iterator i = ins.begin(); i != ins.end();
         ++i) {
      if (i->contains(pc)) {
        // relocation_cerr << insn->format() << " @" << hex << addr << dec << "
        // is PCsens" << endl;
...
class M_A_Predicates : public Slicer::Predicates {
 public:
  ...
  virtual bool widenAtAssignment(const AbsRegion &search,
                                 const AbsRegion &found) {
    if (search != found) {
      haveWidened = true;
      return true;
    }
...
bool PCSensitiveTransformer::determineSensitivity(Graph::Ptr slice,
  ...
  for (; exitBegin != exitEnd; ++exitBegin) {
    ...
    // Now for the real work - determine the difference in values produced by
    // this
...
// address gets put...
bool PCSensitiveTransformer::insnIsThunkCall(
    InstructionAPI::Instruction::Ptr insn, Address addr, Absloc &destination) {
  ...
  if (!res.defined) {
    // relocation_cerr << "      ... CFT not evallable, ret false from isGetPC"
    // << endl;
    return false;
  }
  ...
  if (addrSpace->isValidAddress(target)) {
    ...
    if (firstInsn && firstInsn->getOperation().getID() == e_mov &&
        firstInsn->readsMemory() && !firstInsn->writesMemory() && secondInsn &&
        secondInsn->getCategory() == c_ReturnInsn) {
...
void PCSensitiveTransformer::handleThunkCall(
    RelocBlock *reloc, RelocGraph *cfg, RelocBlock::WidgetList::iterator &iter,
    Absloc &destination) {
  Widget::Ptr replacement =
      PCWidget::create((*iter)->insn(), (*iter)->addr(), destination);
...
void PCSensitiveTransformer::emulateInsn(RelocBlock *reloc, RelocGraph *cfg,
                                         RelocBlock::WidgetList::iterator &iter,
                                         InstructionAPI::Instruction::Ptr insn,
                                         Address addr) {
  // cerr << "Emulating @" << std::hex << addr << std::dec  << endl;
  // We emulate calls by replacing them with push/jump combinations. The jump
  // will be handled
  // by a CFWidget, so we just need a "push" (and then to create everything
  // else).
...
// TODO: fix this
bool PCSensitiveTransformer::exceptionSensitive(Address a,
                                                const block_instance *bbl) {
...
void PCSensitiveTransformer::cacheAnalysis(const block_instance *bbl,
                                           Address addr, bool intSens,
                                           bool extSens) {
  analysisCache_[bbl][addr] = std::make_pair(intSens, extSens);
}

bool PCSensitiveTransformer::queryCache(const block_instance *bbl, Address addr,
                                        bool &intSens, bool &extSens) {
  // intSens = true;
  // extSens = true;
  // return true;
...
ExtPCSensVisitor::ExtPCSensVisitor(const AbsRegion &a) : isExtSens_(false) {
  if (a.absloc().isPC()) {
    assignPC_ = true;
  } else {
    assignPC_ = false;
...
bool PCSensitiveTransformer::isSyscall(InstructionAPI::Instruction::Ptr insn,
                                       Address) {
  // call *%gs:0x10
  // Build a GS
  ...
  if (insn->isRead(x86_gs)) {
    // relocation_cerr << "Skipping syscall " << insn->format() << hex << "@ "
    // << addr << dec << endl;
    return true;
  }
  return false;
