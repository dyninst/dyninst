...
bool
PCSensitiveTransformer::analysisRequired(RelocBlock* blk)
{
...
bool
PCSensitiveTransformer::process(RelocBlock* reloc, RelocGraph* g)
{
    ...
    if(!block)
    {
        if(adhoc_required)
        {
            // sensitivity_cerr << "Warning: No block, running adhoc: " << adhoc_result <<
            // endl;
            return adhoc_result;
        }
        else
    ...
    for(RelocBlock::WidgetList::iterator iter = elements.begin(); iter != elements.end();
        ++iter)
    {
        ...
        else if(insn.getCategory() == c_CallInsn &&
        ...
        else if(isPCSensitive(insn, addr, func, block, sensitiveAssignments))
        {
            Sens_++;
            ...
            if(!queryCache(block, addr, intSens, extSens))
            {
                    ...
                    a_iter != sensitiveAssignments.end(); ++a_iter)
                {
                    ...
                    // " << addr << " (parse of " << (*a_iter)->addr() << dec << ") in
                    // func " << block->func()->prettyName() << endl;
        ...
        if(extSens)
        {
            ...
            if(intSens)
            {
                ...
                // changed version (likely a load or equivalent) and one instruction that
                // wants the original value (that would be a return). Let's see if we can
                // match a thunk call...
                Absloc destination;
                if(insnIsThunkCall(insn, addr, destination))
                {
                    // A first example of a group transformation. The "internal" piece
                    // comes from calling a 2-instruction function that copies the return
                    // address elsewhere and returns. So we can remove the internal
                    // sensitivity by inlining the call.
    ...
    if(adhoc_required)
    {
        ...
        // sensitivity_cerr << "Completing analysis with adhoc process: " << adhoc_result
        // << endl;
        return adhoc_result;
    }
...
bool
PCSensitiveTransformer::isPCSensitive(Instruction insn, Address addr,
                                      ...
                                      AssignList&           sensitiveAssignments)
{
    if(!(insn.getOperation().getID() == e_call))
        return false;
    if(func->obj()->hybridMode() == BPatch_normalMode)
        return false;
    ...
    aConverter.convert(insn, func->addrToOffset(addr), func->ifunc(), block->llb(),
                       assignments);
        ...
        a_iter != assignments.end(); ++a_iter)
    {
        ...
        for(std::vector<AbsRegion>::const_iterator i = ins.begin(); i != ins.end(); ++i)
        {
            if(i->contains(pc))
            {
                // relocation_cerr << insn->format() << " @" << hex << addr << dec << " is
                // PCsens" << endl;
...
public:
    ...
    virtual bool widenAtPoint(Assignment::Ptr p)
    {
        ...
        if(p->out().containsOfType(Absloc::Register) && !p->out().absloc().isPC())
        {
            haveWidened = true;
            return true;
        }
...
Graph::Ptr
PCSensitiveTransformer::forwardSlice(Assignment::Ptr ptr, parse_block* block,
                                     parse_func* func)
{
    M_A_Predicates pred;
...
bool
PCSensitiveTransformer::determineSensitivity(Graph::Ptr slice, bool& internal,
                                             bool& external)
{
...
// address gets put...
bool
PCSensitiveTransformer::insnIsThunkCall(Instruction insn, Address addr,
                                        Absloc& destination)
{
    ...
    if(addrSpace->isValidAddress(target))
    {
        ...
        if(firstInsn.isValid() && firstInsn.getOperation().getID() == e_mov &&
           firstInsn.readsMemory() && !firstInsn.writesMemory() && secondInsn.isValid() &&
...
void
PCSensitiveTransformer::handleThunkCall(RelocBlock* reloc, RelocGraph* cfg,
...
void
PCSensitiveTransformer::emulateInsn(RelocBlock* reloc, RelocGraph* cfg,
                                    RelocBlock::WidgetList::iterator& iter,
                                    Instruction insn, Address addr)
{
    // cerr << "Emulating @" << std::hex << addr << std::dec  << endl;
    // We emulate calls by replacing them with push/jump combinations. The jump will be
    // handled by a CFWidget, so we just need a "push" (and then to create everything
    // else).
 ...
 */
bool
PCSensitiveTransformer::exceptionSensitive(Address a, const block_instance* bbl)
{
    return false;
    ...
    sensitivity_cerr << "Address: 0x" << hex << a << " Offset: 0x" << hex << a << dec
                     << endl;
    ...
    {
        sensitivity_cerr << "\tWarning: There aren't any exceptions in this symtab!"
                         << endl;
        return false;
    }
    ...
    for(auto e_iter = exceptions.begin(); e_iter != exceptions.end(); e_iter++)
    {
        ...
        for(auto r_iter = ranges.begin(); r_iter != ranges.end(); r_iter++)
        {
            ...
            sensitivity_cerr << "\t\tstart: 0x" << hex << r.low() << " end: 0x"
                             << r.high() << dec << endl;

            if((r.low() <= ts && r.high() >= ts) || (r.low() < te && r.high() >= te))
            {
...
void
PCSensitiveTransformer::cacheAnalysis(const block_instance* bbl, Address addr,
