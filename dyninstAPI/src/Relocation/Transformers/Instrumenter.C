...
bool
Instrumenter::process(RelocBlock* trace, RelocGraph* cfg)
{
    assert(trace);
    ...
    if(!insnInstrumentation(trace))
        return false;
    if(!preCallInstrumentation(trace))
        return false;
    if(!blockEntryInstrumentation(trace))
        return false;
    if(!blockExitInstrumentation(trace))
        return false;

    // And on to the graph modification shtuff.
    if(!funcExitInstrumentation(trace, cfg))
        return false;
    if(!postCallInstrumentation(trace, cfg))
        return false;
    if(!edgeInstrumentation(trace, cfg))
        return false;
    if(!funcEntryInstrumentation(trace, cfg))
        return false;
    return true;
}

bool
Instrumenter::insnInstrumentation(RelocBlock* trace)
{
    ...
    if(trace->func())
    {
        instPre  = trace->func()->findInsnPoints(instPoint::PreInsn, trace->block(), pre,
                                                preEnd);
    ...
    while((instPre && (pre != preEnd)) || (instPost && (post != postEnd)))
    {
        ...
        if(postAddr == (*elem)->addr())
        {
            if(!post->second->empty())
            {
                ...
                while((tmp != trace->elements().end()) && ((*tmp)->addr() == postAddr) &&
                      ((*tmp)->insn().isValid()))
                    ++tmp;
...
bool
Instrumenter::preCallInstrumentation(RelocBlock* trace)
{
    instPoint* call = NULL;
    if(trace->func())
    {
    ...
    // instruction in the list of elements.
    ...
    {
        while(riter != elements.rend() && (*riter)->insn() == call_insn)
            ++riter;
    }
...
bool
Instrumenter::funcExitInstrumentation(RelocBlock* trace, RelocGraph* cfg)
{
    ...
    relocation_cerr << "Checking return statement "
                    << (retcc->insn().isValid() ? retcc->insn().format() : "<no insn>")
...
bool
Instrumenter::handleUnconditionalExitInstrumentation(RelocBlock* trace, RelocGraph*,
                                                     instPoint*  exit)
{
    // Easy
    RelocBlock::WidgetList& elements = trace->elements();
...
bool
Instrumenter::handleCondIndExits(RelocBlock* trace, RelocGraph* cfg, instPoint* exit)
{
    // Let's see if this is easy or complicated. Woo complicated!
    CFWidget::Ptr retcc = trace->cfWidget();
    assert(retcc);
    ...
    RelocBlock* instRelocBlock =
        RelocBlock::createInst(exit, retcc->addr(), trace->block(), trace->func());
    ...
    // Lessee...
    // trace should have a batch of <INDIRECT> edges and a <COND_NOT_TAKEN> edge. The
    // COND_NOT_TAKEN is fine, but the INDIRECTs need to move to instRelocBlock. So...
    // move
    Predicates::Type ind(ParseAPI::INDIRECT);
    if(!cfg->changeSources(ind, trace->outs(), instRelocBlock))
    {
        relocation_cerr
            << "Failed to change sources of indirect edges from old block to new block!"
            << endl;
        return false;
    }

    // Also, instRelocBlock needs a <COND_NOT_TAKEN> to wherever trace's COND_NOT_TAKEN
    // went.
    RelocEdge* ft = trace->outs()->find(ParseAPI::FALLTHROUGH);
    if(!ft)
    {
        relocation_cerr
            << "Error: did not find a FALLTHROUGH out-edge from current block!" << endl;
        return false;
    }
    ...
    cfg->makeEdge(new Target<RelocBlock*>(trace), new Target<RelocBlock*>(instRelocBlock),
                  NULL, ParseAPI::COND_TAKEN);

    return true;
}

bool
Instrumenter::handleCondDirExits(RelocBlock* trace, RelocGraph* cfg, instPoint* exit)
{
    // Let's see if this is easy or complicated. Woo complicated!
    CFWidget::Ptr retcc = trace->cfWidget();
    assert(retcc);
    ...
    // And wire edges correctly.
    // Lessee...
    // The trace has a COND_TAKEN, which we want to redirect to the instrumentation block,
    // and a COND_NOT_TAKEN, which we leave alone.
...
bool
Instrumenter::blockEntryInstrumentation(RelocBlock* trace)
{
...
bool
Instrumenter::blockExitInstrumentation(RelocBlock* trace)
{
    instPoint* exit = NULL;
    if(trace->func())
    {
...
bool
Instrumenter::postCallInstrumentation(RelocBlock* trace, RelocGraph* cfg)
{
    ...
    if(!cfg->interpose(pred, trace->outs(), instRelocBlock))
        return false;
...
bool
Instrumenter::funcEntryInstrumentation(RelocBlock* trace, RelocGraph* cfg)
{
    instPoint* entry = NULL;
    ...
    RelocBlock* instRelocBlock =
        RelocBlock::createInst(entry, trace->origAddr(), trace->block(), trace->func());
    cfg->addRelocBlockBefore(trace, instRelocBlock);

    if(!cfg->makeEdge(new Target<RelocBlock*>(instRelocBlock),
                      new Target<RelocBlock*>(trace), NULL, ParseAPI::FALLTHROUGH))
        return false;

    if(!cfg->setSpringboard(trace->block(), trace->func(), instRelocBlock))
        return false;
    Predicates::Interprocedural pred;
    if(!cfg->changeTargets(pred, trace->ins(), instRelocBlock))
        return false;
...
bool
Instrumenter::edgeInstrumentation(RelocBlock* trace, RelocGraph* cfg)
{
        ...
        iter != targets.end(); ++iter)
    {
        ...
        RelocBlock* instRelocBlock = RelocBlock::createInst(point, iedge->trg()->start(),
                                                            iedge->trg(), trace->func());
...
Widget::Ptr
Instrumenter::makeInstrumentation(PatchAPI::Point* p)
{
