...
public:
    ...
    Modification(const CallModMap& callRepl, const FuncModMap& funcRepl,
...
private:
    ...
    RelocBlock* makeRelocBlock(block_instance* block, func_instance* func,
                               RelocBlock* cur, RelocGraph* cfg);
    ...
    {
        WrapperPatch(func_instance* func, std::string name)
        : func_(func)
        , name_(name)
        {}
