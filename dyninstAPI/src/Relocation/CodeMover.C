...
CodeMover::CodeMover(CodeTracker *t)
    : cfg_(new RelocGraph()), addr_(0), tracker_(t), finalized_(false){};
  ...
  for (; begin != end; ++begin) {
    ...
    if (!func->isInstrumentable()) {
      relocation_cerr << "\tFunction " << func->symTabName()
                      << " is non-instrumentable, skipping" << endl;
      continue;
    }
    ...
    relocation_cerr << "\t Added required entry for " << func->symTabName()
                    << " / " << hex << entry->start() << dec << endl;
  }
...
bool CodeMover::addRelocBlocks(RelocBlockIter begin, RelocBlockIter end,
                               func_instance *f) {
  for (; begin != end; ++begin) {
...
bool CodeMover::addRelocBlock(block_instance *bbl, func_instance *f) {
  ...
  if (!bbl->wasUserAdded()) {
    relocation_cerr << "\t Added suggested entry for " << f->symTabName()
                    << " / " << hex << bbl->start() << dec << endl;
...
void CodeMover::finalizeRelocBlocks() {
  ...
  for (RelocBlock *iter = cfg_->begin(); iter != cfg_->end();
       iter = iter->next()) {
...
bool CodeMover::transform(Transformer &t) {
  if (!finalized_) finalizeRelocBlocks();
...
//
// We wish to minimize the space required by the relocated code. Since some
// platforms
// may have varying space requirements for certain instructions (e.g., branches)
// this
// requires a fixpoint calculation. We start with the original size and increase
// from
// there.
//
// Reasons for size increase:
//   1) Instrumentation. It tends to use room. Odd.
//   2) Transformed instructions. We may need to replace a single instruction
//   with a
//      sequence to emulate its original behavior
//   3) Variable-sized instructions. If we increase branch displacements we may
//   need
...
void CodeMover::disassemble() const { buffer_.disassemble(); }

unsigned CodeMover::size() const { return buffer_.size(); }

void *CodeMover::ptr() const { return buffer_.ptr(); }

codeGen &CodeMover::gen() { return buffer_.gen(); }

///////////////////////

PriorityMap &CodeMover::priorityMap() { return priorityMap_; }
...
SpringboardMap &CodeMover::sBoardMap(AddressSpace *) {
  ...
  if (sboardMap_.empty()) {
    for (PriorityMap::const_iterator iter = priorityMap_.begin();
      ...
      relocation_cerr << "Func " << func->symTabName() << " / block " << hex
                      << bbl->start() << " /w/ priority " << p << dec << endl;
...
void CodeMover::extractDefensivePads(AddressSpace *AS) {
  // For now, we're doing an annoying iteration over all CodeTracker elements
  // looking
  ...
  for (CodeTracker::TrackerList::const_iterator iter = trackers.begin();
