...
CodeMover::Ptr
CodeMover::create(CodeTracker* t)
{
    // Make a CodeMover
...
bool
CodeMover::addFunctions(FuncSet::const_iterator begin, FuncSet::const_iterator end)
{
    ...
    for(; begin != end; ++begin)
    {
        ...
        relocation_cerr << "\t Added FuncEntry for " << func->symTabName() << " / " << hex
                        << entry->start() << dec << endl;
    }
...
bool
CodeMover::addRelocBlocks(RelocBlockIter begin, RelocBlockIter end, func_instance* f)
{
    for(; begin != end; ++begin)
    {
...
bool
CodeMover::addRelocBlock(block_instance* bbl, func_instance* f)
{
...
bool
CodeMover::transform(Transformer& t)
{
    if(!finalized_)
        finalizeRelocBlocks();
...
bool
CodeMover::initialize(const codeGen& templ)
{
...
bool
CodeMover::relocate(Address addr)
{
    addr_ = addr;
...
void
CodeMover::disassemble() const
{
    buffer_.disassemble();
}

unsigned
CodeMover::size() const
{
    return buffer_.size();
}
...
SpringboardMap&
CodeMover::sBoardMap(AddressSpace*)
{
    ...
    if(sboardMap_.empty())
    {
            ...
            iter != priorityMap_.end(); ++iter)
        {
            ...
            {
                relocation_cerr << "\t" << hex << "springboard target "
                                << bbl->start() + 0x8 << endl;
                sboardMap_.addFromOrigCode(bbl->start() + 0x8, to, p, func, bbl);
            }
            else
            {
                relocation_cerr << "\t" << hex << "springboard target " << bbl->start()
                                << endl;
...
void
CodeMover::extractDefensivePads(AddressSpace* AS)
{
