...
void EmitterIA32::emitLEA(Register base, Register index, unsigned int scale,
                          int disp, Register dest, codeGen &gen) {
...
codeBufIndex_t EmitterIA32::emitIf(Register expr_reg, Register target,
  ...
  int disp = 0;
  if (target) disp = target - 6;

  if (rc == rc_before_jump) gen.rs()->pushNewRegState();
  GET_PTR(insn, gen);
  // je dest
  *insn++ = 0x0F;
  *insn++ = 0x84;
...
void EmitterIA32::emitOp(unsigned opcode, Register dest, Register src1,
...
void EmitterIA32::emitRelOp(unsigned op, Register dest, Register src1,
                            Register src2, codeGen &gen) {
  RealRegister src1_r = gen.rs()->loadVirtual(src1, gen);
  RealRegister src2_r = gen.rs()->loadVirtual(src2, gen);
  RealRegister dest_r = gen.rs()->loadVirtualForWrite(dest, gen);
...
void EmitterIA32::emitDiv(Register dest, Register src1, Register src2,
                          codeGen &gen) {
  Register scratch = gen.rs()->allocateRegister(gen, true);
...
void EmitterIA32::emitOpImm(unsigned opcode1, unsigned opcode2, Register dest,
...
void EmitterIA32::emitRelOpImm(unsigned op, Register dest, Register src1,
...
void EmitterIA32::emitTimesImm(Register dest, Register src1, RegValue src2imm,
                               codeGen &gen) {
  int result;
  ...
  if (isPowerOf2(src2imm, result) && result <= MAX_IMM8) {
    // sal dest, result
    if (src1 != dest) emitMovRegToReg(dest_r, src1_r, gen);
...
void EmitterIA32::emitDivImm(Register dest, Register src1, RegValue src2imm,
                             codeGen &gen) {
  int result;
  if (src2imm == 1) return;
...
void EmitterIA32::emitLoad(Register dest, Address addr, int size,
                           codeGen &gen) {
...
void EmitterIA32::emitLoadIndir(Register dest, Register addr_reg, int /*size*/,
                                codeGen &gen) {
...
void EmitterIA32::emitLoadOrigFrameRelative(Register dest, Address offset,
                                            codeGen &gen) {
...
bool EmitterIA32::emitLoadRelative(Register /*dest*/, Address /*offset*/,
...
bool EmitterIA32::emitLoadRelativeSegReg(Register /*dest*/, Address offset,
                                         Register base, int /*size*/,
                                         codeGen &gen) {
...
void EmitterIA32::emitLoadFrameAddr(Register dest, Address offset,
                                    codeGen &gen) {
  RealRegister dest_r = gen.rs()->loadVirtualForWrite(dest, gen);
...
void EmitterIA32::emitLoadOrigRegister(Address register_num, Register dest,
                                       codeGen &gen) {
  RealRegister dest_r = gen.rs()->loadVirtualForWrite(dest, gen);
  restoreGPRtoGPR(RealRegister(register_num), dest_r, gen);
}

void EmitterIA32::emitStoreOrigRegister(Address register_num, Register src,
                                        codeGen &gen) {
  assert(0);  // MATT TODO
  ...
  emitMovRegToRM(RealRegister(REGNUM_EBP), offset, RealRegister(REGNUM_EAX),
                 gen);
}

void EmitterIA32::emitStore(Address addr, Register src, int size,
                            codeGen &gen) {
...
void EmitterIA32::emitStoreIndir(Register addr_reg, Register src, int /*size*/,
                                 codeGen &gen) {
  RealRegister src_r = gen.rs()->loadVirtual(src, gen);
...
void EmitterIA32::emitStoreFrameRelative(Address offset, Register src,
...
bool EmitterIA32::emitBTSaves(baseTramp *bt, codeGen &gen) {
  ...
  if (numRegsUsed == -1 || numRegsUsed > X86_REGS_SAVE_LIMIT) {
    ...
    gen.rs()->markSavedRegister(RealRegister(REGNUM_EAX),
                                7 + flags_saved_i + base_i);
    if (flags_saved) {
    ...
    for (unsigned i = 0; i < regs.size(); i++) {
      ...
      if (bt->definedRegs[reg->encoding()]) {
        ...
        gen.rs()->markSavedRegister(
            RealRegister(reg->encoding()),
  ...
  if (useFPRs) {
    if (gen.rs()->hasXMM) {
      ...
      *insn++ = 0x24;
      ...
      emitOpRegRM(FSAVE, RealRegister(FSAVE_OP), RealRegister(REGNUM_ESP), 0,
                  gen);
    }
  }
...
static void emitRex(bool is_64, Register *r, Register *x, Register *b,
                    codeGen &gen) {
  ...
  if (rex & 0x0f) emitSimpleInsn(rex, gen);
}
...
bool isImm64bit(Address imm) { return (imm >> 32); }
...
void emitMovPCRMToReg64(Register dest, int offset, int size, codeGen &gen,
  ...
  if (size == 8)
    *insn++ =
        static_cast<unsigned char>((dest & 0x8) >> 1 | 0x48);  // REX prefix
  else {
    *insn++ =
        static_cast<unsigned char>((dest & 0x8) >> 1 | 0x40);  // REX prefix
  }
  if (deref_result)
  ...
  *insn++ =
      static_cast<unsigned char>(((dest & 0x7) << 3) | 0x5);  // ModRM byte
...
static void emitMovRMToReg64(Register dest, Register base, int disp, int size,
                             codeGen &gen) {
  Register tmp_dest = dest;
  Register tmp_base = base;
...
static void emitMovSegRMToReg64(Register dest, Register base, int disp,
                                codeGen &gen) {
  Register tmp_dest = dest;
  Register tmp_base = base;
  ...
  emitSegPrefix(base, gen);
  emitRex(true, &tmp_dest, NULL, &tmp_base, gen);
  emitOpSegRMReg(MOV_RM32_TO_R32, RealRegister(tmp_dest),
                 RealRegister(tmp_base), disp, gen);
}

static void emitMovRegToRM64(Register base, int disp, Register src, int size,
                             codeGen &gen) {
  Register tmp_base = base;
  Register tmp_src = src;
...
static void emitOpRegReg64(unsigned opcode, Register dest, Register src,
...
static void emitOpRegRM64(unsigned opcode, Register dest, Register base,
                          int disp, bool is_64, codeGen &gen) {
  Register tmp_dest = dest;
  Register tmp_base = base;
  emitRex(is_64, &tmp_dest, NULL, &tmp_base, gen);
  emitOpRegRM(opcode, RealRegister(tmp_dest), RealRegister(tmp_base), disp,
              gen);
  gen.markRegDefined(dest);
}
...
static void emitOpRegRegImm64(unsigned opcode, Register dest, Register src1,
                              int imm, bool is_64, codeGen &gen) {
...
static void emitOpRegImm8_64(unsigned opcode, unsigned opcode_ext,
                             Register dest, char imm, bool is_64,
                             codeGen &gen) {
  Register tmp_dest = dest;
  emitRex(is_64, NULL, NULL, &tmp_dest, gen);
  GET_PTR(insn, gen);
...
bool EmitterAMD64::emitMoveRegToReg(registerSlot *source, registerSlot *dest,
                                    codeGen &gen) {
...
void EmitterAMD64::emitLEA(Register base, Register index, unsigned int scale,
                           int disp, Register dest, codeGen &gen) {
  Register tmp_base = base;
  Register tmp_index = index;
  Register tmp_dest = dest;
...
codeBufIndex_t EmitterAMD64::emitIf(Register expr_reg, Register target,
                                    RegControl, codeGen &gen) {
  // test %expr, %expr
...
void EmitterAMD64::emitOp(unsigned opcode, Register dest, Register src1,
                          Register src2, codeGen &gen) {
  // TODO: optimize this further for ops where order doesn't matter
  if (src1 != dest) emitMovRegToReg64(dest, src1, true, gen);
  emitOpRegReg64(opcode, dest, src2, true, gen);
  gen.markRegDefined(dest);
}

void EmitterAMD64::emitOpImm(unsigned opcode1, unsigned opcode2, Register dest,
                             Register src1, RegValue src2imm, codeGen &gen) {
  if (src1 != dest) {
...
void EmitterAMD64::emitRelOp(unsigned op, Register dest, Register src1,
                             Register src2, codeGen &gen) {
  // cmp %src2, %src1
...
void EmitterAMD64::emitRelOpImm(unsigned op, Register dest, Register src1,
                                RegValue src2imm, codeGen &gen) {
  /* disabling hack
     // HACKITY - remove before doing anything else
     //
     // If the input is a character, then mask off the value in the register so
    that we're
...
void EmitterAMD64::emitDiv(Register dest, Register src1, Register src2,
                           codeGen &gen) {
  ...
  // If src2 is RDX we need to move it into a scratch register, as the sign
  // extend
  // will overwrite RDX.
  ...
  if (save_rdx) emitPopReg64(REGNUM_RDX, gen);
}

void EmitterAMD64::emitTimesImm(Register dest, Register src1, RegValue src2imm,
                                codeGen &gen) {
  int result = -1;
...
void EmitterAMD64::emitDivImm(Register dest, Register src1, RegValue src2imm,
                              codeGen &gen) {
  int result = -1;
  gen.markRegDefined(dest);
  if (isPowerOf2(src2imm, result) && result <= MAX_IMM8) {
...
void EmitterAMD64::emitLoad(Register dest, Address addr, int size,
                            codeGen &gen) {
...
void EmitterAMD64::emitLoadIndir(Register dest, Register addr_src, int size,
                                 codeGen &gen) {
  emitMovRMToReg64(dest, addr_src, 0, size, gen);
  gen.markRegDefined(dest);
}

void EmitterAMD64::emitLoadOrigFrameRelative(Register dest, Address offset,
                                             codeGen &gen) {
  if (gen.bt()->createdFrame) {
    Register scratch = gen.rs()->getScratchRegister(gen);
    // mov (%rbp), %rax
...
bool EmitterAMD64::emitLoadRelative(Register dest, Address offset,
                                    Register base, int /* size */,
                                    codeGen &gen) {
...
bool EmitterAMD64::emitLoadRelativeSegReg(Register dest, Address offset,
                                          Register base, int /* size */,
                                          codeGen &gen) {
...
void EmitterAMD64::emitLoadFrameAddr(Register dest, Address offset,
                                     codeGen &gen) {
  // mov (%rbp), %dest
  if (gen.bt()->createdFrame) {
...
void EmitterAMD64::emitLoadOrigRegister(Address register_num,
  ...
  stackItemLocation loc =
      getHeightOf(stackItem(RealRegister(register_num)), gen);
...
void EmitterAMD64::emitStoreOrigRegister(Address register_num, Register src,
                                         codeGen &gen) {
...
void EmitterAMD64::emitStore(Address addr, Register src, int size,
                             codeGen &gen) {
  Register scratch = gen.rs()->getScratchRegister(gen);
  gen.markRegDefined(scratch);
...
void EmitterAMD64::emitStoreIndir(Register addr_reg, Register src, int size,
                                  codeGen &gen) {
  emitMovRegToRM64(addr_reg, 0, src, size, gen);
}

void EmitterAMD64::emitStoreFrameRelative(Address offset, Register src,
                                          Register /*scratch*/, int size,
                                          codeGen &gen) {
  if (gen.bt()->createdFrame) {
    Register scratch = gen.rs()->getScratchRegister(gen);
...
void EmitterAMD64::emitStoreRelative(Register src, Address offset,
                                     Register base, int /* size */,
                                     codeGen &gen) {
...
/* Recursive function that goes to where our instrumentation is calling
   ...
   store and reexamine every call out instead of doing it on the fly like
   before*/
...
Register EmitterAMD64::emitCall(opCode op, codeGen &gen,
  ...
  if (inInstrumentation) {
    ...
    for (int i = 0; i < gen.rs()->numGPRs(); i++) {
      ...
      if (reg->refCount > 0 ||  // Currently active
        ...
        if (reg->keptValue) regToSave.second *= -1;
  ...
  if (stack_operands < 0) stack_operands = 0;
  ...
  // RAX = number of FP regs used by varargs on AMD64 (also specified as
  // caller-saved).
  // Clobber it to 0.
  ...
  for (unsigned i = 0; i < operands.size(); i++) {
    ...
    if (operands[i]->decRefCount()) gen.rs()->freeRegister(amd64_arg_regs[i]);
  }
  if (frame_size) {
...
bool EmitterAMD64Dyn::emitCallInstruction(codeGen &gen, func_instance *callee,
                                          Register) {
...
bool EmitterAMD64Stat::emitCallInstruction(codeGen &gen, func_instance *callee,
                                           Register) {
...
void EmitterAMD64::emitGetParam(Register dest, Register param_num,
...
void EmitterAMD64::emitASload(int ra, int rb, int sc, long imm, Register dest,
...
void EmitterAMD64::emitCSload(int ra, int rb, int sc, long imm, Register dest,
                              codeGen &gen) {
  ...
  if (rb >= IA32_EMULATE) {
    ...
    if (rcx_wasUsed) emitPopReg64(REGNUM_RCX, gen);
    if (rdi_wasUsed) emitPopReg64(REGNUM_RDI, gen);
    if (rsi_wasUsed) emitPopReg64(REGNUM_RSI, gen);
    if (rax_wasUsed) emitPopReg64(REGNUM_RAX, gen);
  } else if (rb > -1) {
    ...
    if (sc > 0) emitTimesImm(dest, scratch, 1 << sc, gen);
  } else
...
// (1 qword for our false return address, 16 for the saved registers, 1 more for
// the flags)
...
bool shouldSaveReg(registerSlot *reg, baseTramp *inst, bool saveFlags) {
  ...
  if (saveFlags) {
    ...
    if (reg->number == REGNUM_EAX || reg->number == REGNUM_RAX) return true;
  }
  if (inst && inst->validOptimizationInfo() &&
      !inst->definedRegs[reg->encoding()]) {
    regalloc_printf(
        "\t Base tramp instance doesn't have reg %d (num %d) defined; "
...
void EmitterAMD64::emitStackAlign(int offset, codeGen &gen) {
  ...
  emitOpRegImm8_64(0x83, EXTENDED_0x83_AND, REGNUM_RSP, -AMD64_STACK_ALIGNMENT,
                   true, gen);
...
bool EmitterAMD64::emitBTSaves(baseTramp *bt, codeGen &gen) {
  ...
  for (int i = 0; i < gen.rs()->numGPRs(); i++) {
    registerSlot *reg = gen.rs()->GPRs()[i];
    if (!shouldSaveReg(reg, bt, saveFlags)) continue;
    if (createFrame && reg->encoding() == REGNUM_RBP) continue;
    num_to_save++;
  }
  if (createFrame) {
  ...
  bool skipRedZone =
      (num_to_save > 0) || alignStack || saveOrigAddr || createFrame;
...
bool EmitterAMD64::emitBTRestores(baseTramp *bt, codeGen &gen) {
  ...
  if (saveOrigAddr) emitPopReg64(REGNUM_RAX, gen);
...
bool EmitterIA32Dyn::emitCallInstruction(codeGen &gen, func_instance *callee,
                                         Register ret) {
  // make the call
  ...
  gen.rs()->makeRegisterAvail(RealRegister(REGNUM_EAX),
                              gen);  // caller saved regs
...
bool EmitterIA32Stat::emitCallInstruction(codeGen &gen, func_instance *callee,
                                          Register ret) {
  AddressSpace *addrSpace = gen.addrSpace();
  Address dest;
...
void EmitterIA32::emitLoadShared(opCode op, Register dest,
  ...
  emitMovPCRMToReg(dest_r, addr - gen.currAddr(), gen,
                   (!is_local && var != NULL));
  if (op == loadOp) {
...
void EmitterIA32::emitStoreShared(Register source, const image_variable *var,
...
bool EmitterIA32::emitXorRegRM(Register dest, Register base, int disp,
                               codeGen &gen) {
  RealRegister dest_r = gen.rs()->loadVirtualForWrite(dest, gen);
...
bool EmitterIA32::emitXorRegSegReg(Register /*dest*/, Register base, int disp,
                                   codeGen &gen) {
...
#if defined(arch_x86_64)
void EmitterAMD64::emitLoadShared(opCode op, Register dest,
                                  const image_variable *var, bool is_local,
                                  int size, codeGen &gen, Address offset) {
  Address addr;
  gen.markRegDefined(dest);
  ...
  if (op == loadConstOp) {
    ...
    // Brutal hack for IP-relative: displacement operand on 32-bit = IP-relative
    // on 64-bit.
    if (is_local || !var)
...
void EmitterAMD64::emitStoreShared(Register source, const image_variable *var,
                                   bool is_local, int size, codeGen &gen) {
  Address addr;
  ...
  // load register with address from jump slot
  emitLEA(Null_Register, Null_Register, 0, addr - gen.currAddr() - 7, dest,
          gen);
  // emitMovPCRMToReg64(dest, addr-gen.currAddr(), gen, true);
  if (!is_local) emitLoadIndir(dest, dest, 8, gen);
...
bool EmitterAMD64::emitXorRegRM(Register dest, Register base, int disp,
                                codeGen &gen) {
...
bool EmitterAMD64::emitXorRegSegReg(Register dest, Register base, int disp,
                                    codeGen &gen) {
  Register tmp_dest = dest;
  Register tmp_base = base;
  ...
  emitSegPrefix(base, gen);
  emitRex(true, &tmp_dest, NULL, &tmp_base, gen);
  emitOpSegRMReg(XOR_R32_RM32, RealRegister(tmp_dest), RealRegister(tmp_base),
                 disp, gen);
  gen.markRegDefined(dest);
  return true;
}
