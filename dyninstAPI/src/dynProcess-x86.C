...
void
print_read_error_info(const relocationEntry entry, func_instance*& target_pdf,
                      Address base_addr)
{
...
bool
PCProcess::hasBeenBound(const relocationEntry& entry, func_instance*& target_pdf,
                        Address base_addr)
{
    if(isTerminated())
        return false;
    ...
    {
        sprintf(errorLine,
                "read error in process::hasBeenBound addr 0x%x, pid=%d\n (readDataSpace "
                "returns 0)",
