...
compiler_tuple(Name, Executable, DefString, Platforms, OptStrings,
               ParmStrings, Languages, StdFlags) :-
...
mutatee_tuple(Name, PreprocessedSources, RawSources, Libraries, Platform,
              Compiler, Optimization_level, Groupable) :-
...
write_tuples(Filename, Platform) :-
    ...
    write(Stream, '\n'),
    findall([N, PS, RS, L, Platform, C, O, G],
            mutatee_tuple(N, PS, RS, L, Platform, C, O, G), Mutatees),
    ...
    write(Stream, '\n'),
    findall([M, C, O, R, S, G, T],
            rungroup_tuple(M, C, O, R, S, G, T, Platform),
            RunGroups),
