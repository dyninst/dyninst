...
// $Id: test_fork_8.C,v 1.1 2007/09/24 16:40:45 cooksey Exp $
/*
 * #Name: test7_4
...
class test_fork_8_Mutator : public TestMutator {
private:
  BPatch *bpatch;
...
extern "C" TEST_DLL_EXPORT TestMutator *test_fork_8_factory() {
...
static void checkTestCase4(procType proc_type, BPatch_thread *thread) {
   if(proc_type == Parent_p) {
      if(! verifyProcMemory(thread, "test_fork_8_global1", 789, proc_type)) {
	 passedTest = false;
      }
   } else if(proc_type == Child_p) {
      if(! verifyProcMemory(thread, "test_fork_8_global1", 1000, proc_type)) {
	 passedTest = false;
      }
   }
}
...
test_results_t test_fork_8_Mutator::execute() {
