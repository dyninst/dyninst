...
class SnippetGenerator {
 public:
  ...
  enum SGContext { SG_FunctionName, SG_ModuleName, SG_TID };
 ...
 public:
  ...
  BPatch_snippet *findOrCreateVariable(const char *name, const char *type,
                                       const void *initialValue = NULL);
  BPatch_snippet *findOrCreateArray(const char *name, const char *elementType,
                                    long size);
  BPatch_snippet *findRegister(const char *name);
  BPatch_snippet *findAppVariable(const char *name, bool global = false,
                                  bool local = false);
  ...
  BPatch_function *findFunction(const char *name,
                                std::vector<BPatch_snippet *> params);
