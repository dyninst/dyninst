...
BPatch_snippet*
SnippetGenerator::findOrCreateVariable(const char* name, const char* type,
                                       const void* initialValue)
{
    ...
    varExpr =
        addSpace->createVariable(std::string(name), (Dyninst::Address) NULL, bptype);
...
BPatch_snippet*
SnippetGenerator::findOrCreateArray(const char* name, const char* elementType, long size)
{
    lastError.str()              = "";
    BPatch_variableExpr* varExpr = image->findVariable(name, false);
    if(varExpr != NULL)
    ...
    BPatch_type* array =
        BPatch::bpatch->createArray(arrayTypeName.str().c_str(), type, 0, size - 1);
    ...
    varExpr = addSpace->createVariable(std::string(name),
                                       (Dyninst::Address) varExpr->getBaseAddr(), array);
...
BPatch_snippet*
SnippetGenerator::findRegister(const char* name)
{
    lastError.str() = "";

    if(!addSpace->getRegisters(registers))
    {
        lastError << "Could not retrive registers. Register access may not be available "
                     "on this platform.";
        return NULL;
    }
...
BPatch_snippet*
SnippetGenerator::findAppVariable(const char* name, bool global, bool local)
{
    lastError.str() = "";
    if(global && local)
    {
    ...
    if(!global && point != NULL)
    {
        ...
        if(NULL == varExprGbl || (local && NULL != varExprGbl && varExprGbl != varExpr))
        {  // if var is local
    ...
    if(point != NULL && !local && !global &&
       (NULL != varExprGbl && varExprGbl != varExpr))
    {  // if BPatch_AppVar && both local and global
...
BPatch_snippet*
SnippetGenerator::findParameter(const char* name)
{
    lastError.str() = "";
    if(point == NULL)
    {
...
BPatch_snippet*
SnippetGenerator::findInstVariable(const char* mangledStub, const char* name)
{
    lastError.str()                         = "";
...
BPatch_function*
SnippetGenerator::findFunction(const char* name, std::vector<BPatch_snippet*> params)
{
    lastError.str() = "";
...
BPatch_snippet*
SnippetGenerator::getContextInfo(SGContext context)
{
    char name[512];
    lastError.str() = "";
    if(point == NULL)
