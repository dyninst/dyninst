...
std::map<BPatch_point*, BPatch_snippet*>*
createSnippet(const char* s, std::vector<BPatch_point*> points)
{
    //     dynCdebug = 1;
    char* mutS = strdup(s);
...
BPatch_snippet*
createSnippet(const char* s, BPatch_point& point)
{
...
BPatch_snippet*
createSnippet(const char* s, BPatch_addressSpace& addSpace)
{
    char* mutS = strdup(s);
    set_lex_input(mutS);
...
BPatch_snippet*
createSnippet(FILE* f, BPatch_point& point)
{
...
std::map<BPatch_point*, BPatch_snippet*>*
createSnippet(FILE* f, std::vector<BPatch_point*> points)
{
    std::string fileString;
    if(f == NULL)
    {
...
BPatch_snippet*
createSnippet(FILE* f, BPatch_addressSpace& addSpace)
{
    std::string fileString;
    if(f == NULL)
    {
...
BPatch_snippet*
createSnippet(std::string str, BPatch_point& point)
{
...
std::map<BPatch_point*, BPatch_snippet*>*
createSnippet(std::string str, std::vector<BPatch_point*> points)
{
    char*                                     cstr   = strdup(str.c_str());
    std::map<BPatch_point*, BPatch_snippet*>* retMap = createSnippet(cstr, points);
    free(cstr);
...
BPatch_snippet*
createSnippet(std::string str, BPatch_addressSpace& addSpace)
{
    char*           cstr  = strdup(str.c_str());
    BPatch_snippet* retSn = createSnippet(cstr, addSpace);
    free(cstr);
...
std::string
mangle(const char* varName, const char* snippetName, const char* typeName)
{
...
std::string
getMangledStub(const char* varName, const char* snippetName)
{
    std::stringstream namestr;
    namestr << varNameBase << varName << "_" << snippetName;
    std::string retName = namestr.str();
