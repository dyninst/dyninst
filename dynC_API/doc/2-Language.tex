...
initialization is performed as if in a first-only block (see Section~\ref{sec:firstOnly}). Thus, a variable is initialized only the first time that
...
\begin{lstlisting}
   ...
   BPatch_snippet* snippet1
         = dynC::createSnippet(file1, &entryPoint, "mySnippet1");
   BPatch_snippet* snippet2
         = dynC::createSnippet(file2, &exitPoint, "mySnippet2");
