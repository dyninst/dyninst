...
.. cpp:function:: long int P_tkill(pid_t PID, int SIGNUM) 
...
.. cpp:function:: long int P_ptrace(int req, pid_t pid, Dyninst::Address addr, Dyninst::Address data, int = -1)
.. cpp:function:: int P_select(int wid, fd_set *rd, fd_set *wr, fd_set *ex, struct timeval *tm) 
.. cpp:function:: int P_rexec(char **ahost, u_short inport, char *user, char *passwd, char *cmd, int *fd2p) 
