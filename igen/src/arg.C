...
bool arg::tag_bundle_send_mrnet(ofstream &out_stream, const pdstring &return_value,
				const pdstring &req_tag,const pdstring &fnc_name,const bool srvr) const
{
    return (tag_bundle_send_many_mrnet(out_stream, return_value, req_tag, fnc_name,srvr));
}

bool arg::tag_bundle_send_many_mrnet(ofstream &out_stream, const pdstring &return_value,
				     const pdstring &rq_tag,const pdstring &fnc_name,const bool srvr) const
{

  pdstring argument_list;
  pdstring format_str;
  pdstring base_send = "\n//arg 1\n\tint sret = stream->send("+rq_tag+",";
  pdstring vec_calc = "\n";
  ...
  if(type_ == "void")
      ...
      if(data_type.find("<") < data_type.length())
	{
	  ...
	  Index = "XX";
	  vec_calc +="\n\tfor(unsigned int "+Index+" = 0 ; "+Index+" < "+data_name+".size(); "+Index+"++){\n\n//Arg pdvector\n\n";
