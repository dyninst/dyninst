...
bool interface_spec::gen_ctor_hdr(ofstream &out_stream, bool server) const {
  ...
  if (server)
	...
	//{ Leave out verify for mrnet for now
	    // out_stream << "  virtual bool verifyProtocolAndVersion(MRN::Stream * stream);\n";
	//}
	out_stream << "public:\n";
  ...
  if (Options::ml->address_space() == message_layer::AS_many) 
	...
	{
	  out_stream << " waitLoop(MRN::Network * ntwrk);\n";

	  out_stream << "    "<<Options::type_prefix()<<"message_tags waitLoop(MRN::Network * ntwrk,bool * processed_data);\n";
	}
    }
  else
    {
  ...
  if (Options::ml->serial()) {
	...
	out_stream << "  bool wait_for(" << Options::type_prefix() << "message_tags m);\n";
      ...
      {
	out_stream << "  bool wait_for( MRN::Network * ntwrk, "  << Options::type_prefix() << "message_tags m);\n";
	out_stream << "  bool wait_for( MRN::Stream * stream, "  << Options::type_prefix() << "message_tags m);\n";
      }
	...
	out_stream << "  bool wait_for_and_read(" << Options::type_prefix() << "message_tags m, void** buffer);\n";
       ...
       {
	out_stream << "  bool wait_for_and_read(MRN::Stream * stream, " << Options::type_prefix() << "message_tags m, void** buffer);\n";
       }
	 ...
	 out_stream << "  "
		    << Options::type_prefix() << "message_tags process_buffered(MRN::Stream * stream);\n";
      }
     out_stream << "  // Handles any buffered request\n";
     out_stream << "  "
	...
	out_stream << "  " 
		   << Options::type_prefix()
		   << "message_tags process_buffered(MRN::Stream * stream, unsigned dex);\n";
      }
     out_stream << "  " 
		<< Options::type_prefix()
     ...
     if (Options::ml->name() == "mrnet")
      {
	out_stream << " char** recv_buf, MRN::Stream * stream,";
      }
     out_stream << " bool wait=true, bool *response=NULL);\n";
     if (Options::ml->name() == "mrnet")
      {
	out_stream << "   " << Options::type_prefix()
		   << "message_tags delete_buffer(MRN::Stream * stream, unsigned dex);\n";
      }
     out_stream << "   " << Options::type_prefix()
...
bool interface_spec::gen_ctor_body(ofstream &out_stream, bool server) const 
{
  if (Options::ml->address_space() == message_layer::AS_many) 
    {
      if ((Options::ml->name() == "mrnet")&&(!server))
	out_stream << "#include \"../src/DMthread/DMdaemon.h\"\n";
...
bool interface_spec::gen_server_verify(ofstream &out_stream) const {
  ...
  if (Options::ml->name() != "mrnet") 
  ...
  else
    {
      ...
      out_stream << "\tif( MRN::Network::recv( &tag,(MRN::Packet **)&recv_buffer,&stream,true) < 0)\n\t"<<endl;
    }
...
bool interface_spec::gen_client_verify(ofstream &out_stream) const {
  ...
  if (Options::ml->address_space() == message_layer::AS_many) {
	...
	out_stream << "bool  " << gen_class_prefix(false)
      ...
      {
	out_stream << "bool  " << gen_class_prefix(false)
		   << "verifyProtocolAndVersion(MRN::Stream * stream) {\n";
      }
   ...
   if (Options::ml->name() != "mrnet") 
   ...
   else
     {
       ...
       //out_stream << "  if (!awaitResponse(" << Options::type_prefix() << "verify,buf)) ";

       //out_stream << "\tif( MRN::Network::recv( &tag,(MRN::Packet **)&buf,&stream,true) !=1)\n\t"<<endl;
       //$$$ debug
...
bool interface_spec::gen_process_buffered(ofstream &out_stream, bool srvr) const {
  ...
  if((srvr)&&(Options::ml->name() == "mrnet"))
    {
      out_stream << Options::type_prefix() << "message_tags " << gen_class_prefix(srvr)
		 << "process_buffered(MRN::Stream * stream, unsigned index) {\n";
    }
  else
    {
      out_stream << Options::type_prefix() << "message_tags " << gen_class_prefix(srvr)
  ...
  out_stream << Options::type_prefix() << "message_tags ";
   ...
   if((srvr)&&(Options::ml->name() == "mrnet"))
    {
      out_stream << gen_class_prefix(srvr) << "delete_buffer(MRN::Stream * stream, unsigned index) {\n";
    }
   else
    {
      out_stream << gen_class_prefix(srvr) << "delete_buffer(unsigned index) {\n";
    }
  ...
  out_stream << "  assert((!async_buffer.size() && !head) || (elems_left == (async_buffer.size()-head)));" << endl;
   ...
   if(!srvr)
     {
       if(Options::ml->name() == "mrnet")
	 out_stream << "\tMRN::Stream * stream = NULL;\n\n";
     }
   out_stream << endl;
...
bool interface_spec::gen_await_response(ofstream &out_stream, bool srvr) const {
  ...
  if (Options::ml->name() == "mrnet")
    {
      out_stream << " char** recv_buffer, MRN::Stream *stream,";
    }
...
bool interface_spec::gen_wait_loop(ofstream &out_stream, bool srvr) const {
  ...
  if (Options::ml->name() == "mrnet") 
    {
      ...
      out_stream << Options::type_prefix() << "message_tags ";
      out_stream << gen_class_prefix(srvr) << "waitLoop(MRN::Network * ntwrk, bool * processed_data)\n {\n";
      out_stream << "\t"<<Options::type_prefix() << "message_tags tag;\n";
      out_stream << "\t*processed_data = false;\n";
      ...
      out_stream << "\t	if( sret < 0)\n\t{\n"
		 ...
		 <<"\thandle_error();\n"
		 <<"\treturn " << Options::type_prefix() << "error;\n\t}\n";
      out_stream <<"\tif(sret == 0)\n\t   return "<<Options::type_prefix()<< "last;\n";
      out_stream << "\t*processed_data = true;\n";
      out_stream << "\treturn switch_on(stream, tag, recv_buffer );\n}\n";
}
  ...
  if (Options::ml->address_space() != message_layer::AS_one) 
	...
	{
	  out_stream << "bool " << gen_class_prefix(srvr)
		     << "wait_for(MRN::Network * ntwrk, " << Options::type_prefix() << "message_tags tag) {\n";
	  out_stream << "\tMRN::Stream * stream;\n";
	  out_stream << "\tchar * recv_buffer;\n";
	  ...
	  out_stream << "bool " << gen_class_prefix(srvr)
		     << "wait_for(MRN::Stream * stream, " << Options::type_prefix() << "message_tags tag) {\n";
	  out_stream << "  unsigned size = async_buffer.size();\n";
	  out_stream << "  for (unsigned u=head; u<size; u++) \n";
	  out_stream << "    if (async_buffer[u]->data_type == tag) {\n";
	...
	out_stream << "  bool response;\n";
      ...
      {
 	out_stream << "bool " << gen_class_prefix(srvr)
		   << "wait_for_and_read(MRN::Stream * stream, " << Options::type_prefix() 
		   << "message_tags tag, void** buffer) {\n";
	out_stream << "  bool response;\n";
     }
	...
	//out_stream << "\tif( stream->recv( (int*)&tag,(MRN::Packet **)&recv_buffer,true) ==1)\n\t{\n\t"<<endl;
