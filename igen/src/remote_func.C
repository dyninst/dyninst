...
//	   out_stream << spacesp2 << "if (!execute) {" << endl;
	 ...
	 if (Options::ml->name() != "mrnet")
	   ...
	   {
	     out_stream << spacesp4 << call_sig_.type() << " * buffer = new "+call_sig_.type()+";\n";
	     vrbleToReadInto = "(*buffer)"; //parentheses ARE needed!!
	   }
	 if (Options::ml->name() != "mrnet")
	   { 
   ...
   if(Options::ml->name() == "mrnet")
     {
       if(!srvr)
	 {
	   out_stream << "dynRPCUser * pd = (dynRPCUser*)paradynDaemon::allDaemons[ dm_id ];\n";
	 }
     }
   ...
   if (Options::ml->address_space() == message_layer::AS_many)
       ...
       if (!is_async_call())
	 {
	   ...
	   if (Options::ml->name() != "mrnet")
	     ...
	     {
	       return_arg_.tag_bundle_send_mrnet(out_stream, "ret", "ret_tag",(Options::type_prefix() + "error"),srvr);
	     }
   ...
   if (Options::ml->name() == "mrnet")
    {
      if(!srvr)
	{
      out_stream << spaces<<"dynRPCUser * pd = (dynRPCUser*)paradynDaemon::allDaemons[ item->Daemon_Id ];\n";
	}
    }
   out_stream << spaces << "   ";
...
bool remote_func::save_async_request(const pdstring &spaces,
   ...
   if ((Options::ml->name() == "mrnet")&&(!srvr))
      {
	out_stream << spaces << "   buffer->Daemon_Id = dm_id;\n";
      }
   if (call_sig_.type() != "void")
				       ...
				       const bool srvr) const
{
   ...
   if ((!is_async_call()) && (Options::ml->name() == "mrnet"))
     {
       ...
       out_str << "\tMRN::Communicator * comm = stream->get_Communicator();\n";
   ...
   if (Options::ml->name() != "mrnet") 
       ...
       if (!is_async_call())
	 {
	   ...
	   call_sig_.tag_bundle_send_mrnet(out_str,pass_return ,request_tag(),name(),srvr);
	 }
       else
	 {
	   call_sig_.tag_bundle_send_mrnet(out_str, return_value(),request_tag(),name(),srvr);
	 }
   ...
   if (!is_async_call())
       ...
       if (Options::ml->name() == "mrnet")
	 {
	   out_str << "\twhile(true)\n\t{\n";
	   if (!is_async_call() && !is_void()) 
	     out_str << return_arg_.type() << "* ret_arg = new "<< return_arg_.type() <<";\n";
	   out_str << "\tchar* recv_buffer = NULL;\n";
	   //out_str << "\tif( stream->recv( (int*)&tag,(MRN::Packet **)&recv_buffer,true) !=1)\n\t"<<endl;
