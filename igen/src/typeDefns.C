...
void generate_ptr_type (char *type, char *ptr_type);
void generate_ptr_type(char *baset, char *type)
{
  userDefn *old_base;

  old_base = userPool.find(baset);
  if (!old_base) {
...
char *findAndAddArrayType(char *name) 
{
	...
	exit(0);
      ...
      cout << "For array " << temp << " element type " << name <<
	" is illegal, exiting\n";
      exit(0);
    }
}

userDefn::userDefn(char *n, int userD, List<field*> &f)
{
  userDefn *fd;
...
userDefn::userDefn(char *n, int userD)
{
  userDefn *fd;
  if(fd = userPool.find(n)) {
    printf("Name %s for class definition is already used\n", n);
...
void typeDefn::genHeader()
{
    ...
    if (generateXDR) {
	if (userDefined) {
	  dot_h << "extern xdr_" << name << "(XDR*, " << name << "*);\n";
	  if (do_ptr)
        ...
        if (userDefined) {
	  dot_h << "extern IGEN_pvm_" << name <<
	    "(IGEN_PVM_FILTER, " << name << "*);\n";
	  if (do_ptr)
...
void typeDefn::genBundlerPVM()
{
  ...
  dot_c << "bool_t IGEN_pvm_" << name <<
    "(IGEN_PVM_FILTER __dir__, " << name << " *__ptr__) {\n";
...
void typeDefn::genBundlerXDR()
{
  ...
  dot_c << "bool_t xdr_" << name << "(XDR *__xdrs__, " <<
    name << " *__ptr__) {\n";
  ...
  if (arrayType) {
    ...
    dot_c << type << "), (xdrproc_t) xdr_" << type << "))\n";
    dot_c << "      return FALSE;\n";
  } else {
    for (fp = fields; *fp; fp++)
      (*fp)->genBundler(dot_c);
...
void typeDefn::genPtrBundlerXDR()
{
  ...
  dot_c << "          return(xdr_" << name << "(__xdrs__, *__ptr__));\n";
  dot_c << "          break;\n";
...
typeDefn::typeDefn(char *i, List<field *> &f) : userDefn(i, TRUE, f) {
  arrayType = FALSE;
  userPool.add(this, name);
  type = 0;
}

typeDefn::typeDefn(char *i) : userDefn(i, FALSE) {
  arrayType = FALSE;
  userPool.add(this, name);
  type = 0;
}
...
// for arrays types.
typeDefn::typeDefn(char *i, char *t) : userDefn(i, TRUE) {
  arrayType = TRUE;
