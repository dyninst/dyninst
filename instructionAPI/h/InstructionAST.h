...
/// The %InstructionAST class is the base class for all nodes in the ASTs used
/// by the %Operand class.
/// It defines the necessary interfaces for traversing and searching
/// an abstract syntax tree representing an operand.
/// For the purposes of searching an %InstructionAST, we provide two related
/// interfaces.  The first,
/// \c getUses, will return the registers that appear in a given tree.  The
/// second, \c isUsed, will
/// take as input another tree and return true if that tree is a (not
/// necessarily proper) subtree of this one.
/// \c isUsed requires us to define an equality relation on these abstract
/// syntax trees, and the equality operator is provided by the %InstructionAST,
/// with the details
/// implemented by the classes derived from %InstructionAST.  Two AST nodes are
...
class INSTRUCTION_EXPORT InstructionAST
 ...
 public:
  ...
  /// are equal if they represent the same register.  %Immediates are equal if
  /// they
