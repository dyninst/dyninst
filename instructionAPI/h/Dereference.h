...
///   - If necessary, fill in the %Dereference node with the contents of that addresss,
///   using \c setValue
///
/// The type associated with a %Dereference node will be the type of the value \em read
/// \em from \em memory, not the type used for the address computation.  Two %Dereferences
/// that access the same address but interpret the contents of that memory as different
/// types will produce different values.  The children of a %Dereference at a given
/// address are identical, regardless of the type of dereference being performed at that
/// address. For example, the %Expression shown in Figure 6 could have its root
/// %Dereference, which interprets the memory being dereferenced as a unsigned 16-bit
/// integer, replaced with a %Dereference that interprets the memory being dereferenced as
/// any other type.  The remainder of the %Expression tree would, however, remain
/// unchanged.
...
public:
    ...
    /// representing the address to be dereferenced and a type indicating how the memory
    /// at the address in question is to be interpreted.
    Dereference(Expression::Ptr addr, Result_Type result_type)
    : Expression(result_type)
    , addressToDereference(addr)
    {}
    virtual ~Dereference() {}
