...
/// As a %RegisterAST is a %Expression, it may contain the physical register's
/// contents if
/// they are known.
///
...
class INSTRUCTION_EXPORT RegisterAST : public Expression {
 public:
  /// \brief A type definition for a reference-counted pointer to a
  /// %RegisterAST.
  ...
  /// \param children Since a %RegisterAST has no children, the \c children
  /// parameter is unchanged by this method.
  ...
  /// The \c format method on a %RegisterAST object returns the name associated
  /// with its ID.
  virtual std::string format(formatStyle how = defaultStyle) const;

  /// Utility function to get a Register object that represents the program
  /// counter.
  ///
  /// \c makePC is provided to support platform-independent control flow
  /// analysis.
  static RegisterAST makePC(Dyninst::Architecture arch);

  /// We define a partial ordering on registers by their register number so that
  /// they may be placed into sets
 ...
 protected:
  ...
  virtual bool checkRegID(MachRegister id, unsigned int low,
