...
struct Result_type2type
{
    typedef void* type;
};
template <>
struct Result_type2type<s8>
{
    typedef char type;
};
template <>
struct Result_type2type<u8>
{
    typedef unsigned char type;
};
template <>
struct Result_type2type<s16>
{
    typedef int16_t type;
};
template <>
struct Result_type2type<u16>
{
    typedef uint16_t type;
};
template <>
struct Result_type2type<u24>
{
    typedef uint32_t type;
};
template <>
struct Result_type2type<s32>
{
    typedef int32_t type;
};
template <>
struct Result_type2type<u32>
{
    typedef uint32_t type;
};
template <>
struct Result_type2type<s48>
{
    typedef int64_t type;
};
template <>
struct Result_type2type<u48>
{
    typedef uint64_t type;
};
template <>
struct Result_type2type<s64>
{
    typedef int64_t type;
};
template <>
struct Result_type2type<u64>
{
    typedef uint64_t type;
};
template <>
struct Result_type2type<sp_float>
{
    typedef float type;
};
template <>
struct Result_type2type<dp_float>
{
    typedef double type;
};
template <>
struct Result_type2type<bit_flag>
{
    typedef unsigned char type;
};
...
/// (signed and unsigned), bit values, and single and double precision floating point
/// values. For each of these types, the value of a %Result may be undefined, or it may be
/// a value within the range of the type.
///
...
///
// The %Instruction API's model of %Results is a simple one, and may seem overly
// aggressive about making an %Expression's %Result undefined.  It follows the same basic
// rule as the rest of the API: a decoded %Instruction object represents only the
// information that may be obtained from the machine instruction that was decoded.  As
// discussed in the Expression section, the \c setValue and \c eval interface allows you
// to determine the possible %Results of an %Expression when evaluated over various
// machine states.  From this, you may construct abstractions to represent the set of
// possible results. Alternately, you may use instrumentation to determine the exact
// machine state at the time an instruction executes, which will allow you to evaluate the
// %Result of an %Expression in its actual context.
...
public:
    ...
    /// A %Result may be constructed from a type and any value convertible to the type
    /// that the tag represents. This constructor creates a %Result of type \c t and
    /// contents \c v for any \c v that is implicitly convertible to type \c t. Attempting
    /// to construct a %Result with a value that is incompatible with its type will result
    ...
    ///
    /// Otherwise, they are unequal (due to having different types, an undefined %Result
    /// compared to a defined %Result, or different values).

    bool operator==(const Result& o) const { return !((*this < o) || (o < *this)); }
    /// %Results are formatted as strings containing their contents, represented as
    /// hexadecimal. The type of the %Result is not included in the output.
