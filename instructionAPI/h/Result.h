...
/// (signed and unsigned), bit values, and single and double precision floating
/// point values.
/// For each of these types, the value of a %Result may be undefined, or it may
/// be a value within
...
/// - \c dbl128: a 128-bit integer, which often contains packed floating point
/// values
/// - \c m14: a 14 byte memory value
///
// The %Instruction API's model of %Results is a simple one, and may seem overly
// aggressive about
// making an %Expression's %Result undefined.  It follows the same basic rule as
// the rest of the API:
// a decoded %Instruction object represents only the information that may be
// obtained from the machine
// instruction that was decoded.  As discussed in the Expression section, the \c
// setValue
// and \c eval interface allows you to determine the possible %Results of an
// %Expression when evaluated over various
// machine states.  From this, you may construct abstractions to represent the
// set of possible results.
// Alternately, you may use instrumentation to determine the exact machine state
// at the time an
// instruction executes, which will allow you to evaluate the %Result of an
// %Expression in its actual context.
class INSTRUCTION_EXPORT Result {
 public:
  ...
  Result() : type(u32), defined(false) { val.u32val = 0; }
  Result(const Result& o) : val(o.val), type(o.type), defined(o.defined) {}
  ...
  Result(Result_Type t) : type(t), defined(false) { val.u32val = 0; }

  /// A %Result may be constructed from a type and any value convertible to the
  /// type that the
  /// tag represents.
  /// This constructor creates a %Result of type \c t and contents \c v for any
  /// \c v that is implicitly
  /// convertible to type \c t.  Attempting to construct a %Result with a value
  /// that is incompatible with
  /// its type will result in a compile-time error.
  template <typename T>
  Result(Result_Type t, T v) : type(t), defined(true) {
    switch (type) {
      case u8:
  ...
  Result(Result_Type t, float v) : type(t), defined(true) {
    assert(t == sp_float || t == dp_float);
    val.dblval = v;
  }
  Result(Result_Type t, double v) : type(t), defined(true) {
    assert(t == sp_float || t == dp_float);
    val.dblval = v;
  }
  ~Result() {}
  ...
  ///
  /// Otherwise, they are unequal (due to having different types, an undefined
  /// %Result compared to a defined %Result,
  /// or different values).

  bool operator==(const Result& o) const {
    return !((*this < o) || (o < *this));
  }
  /// %Results are formatted as strings containing their contents, represented
  /// as hexadecimal.
  ...
  std::string format() const {
    if (!defined) {
      ...
      switch (type) {
        case u8:
          // Type promote the characters so that they're treated as integral,
          // not as strings
