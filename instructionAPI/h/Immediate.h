...
public:
    ...
    /// \c isUsed, when called on an %Immediate, will return true if \c findMe represents
    /// an %Immediate with the same value. While this convention may seem arbitrary, it
    /// allows \c isUsed to follow a natural rule: an %InstructionAST is used by another
    /// %InstructionAST if and only if the first %InstructionAST is a subtree of the
    /// second one.
