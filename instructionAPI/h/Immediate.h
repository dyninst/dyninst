...
class INSTRUCTION_EXPORT Immediate : public Expression {
 public:
  ...
  /// \c isUsed, when called on an %Immediate, will return true if \c findMe
  /// represents an %Immediate with the same value.
  /// While this convention may seem arbitrary, it allows \c isUsed to follow a
  /// natural rule: an %InstructionAST is used
  /// by another %InstructionAST if and only if the first %InstructionAST is a
...
class INSTRUCTION_EXPORT ArmConditionImmediate : public Immediate {
 public:
  ...
  static ArmConditionImmediate::Ptr makeArmConditionImmediate(
      const Result& val);
  virtual std::string format(formatStyle) const;
