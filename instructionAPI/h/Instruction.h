...
///
/// Each %Operand is an AST built from %RegisterAST and %Immediate leaves.  For each
/// %Operand, you may determine:
///   - Registers read
///   - Registers written
///   - Whether memory is read or written
///   - Which memory addresses are read or written, given the state of all relevant
///   registers
///
/// Instructions should be constructed from an \c unsigned \c char* pointing to machine
/// language, using the %InstructionDecoder class.  See InstructionDecoder for more
/// details.
///
class Instruction
{
    /// Component version information corresponding to \c
    /// libInstructionAPI.so.(major).(minor).(maintenance) Note that \c maintenance may be
    /// absent from the binary (in which case, it will be zero in the interface).
...
public:
    ...
    /// \param size Contains the number of bytes occupied by the corresponding machine
    /// instruction \param raw Contains a pointer to the buffer from which this
    /// instruction object was decoded.
    ///
    /// Construct an %Instruction from an %Operation and a collection of %Expressions.
    /// This method is not intended to be used except by the %InstructionDecoder class,
    /// which serves as a factory class for producing %Instruction objects.  While an
    /// %Instruction object may be built "by hand" if desired, using the decoding
    /// interface ensures that the operation and operands are a sensible combination, and
    /// that the size reported is based on the actual size of a legal encoding of the
    /// machine instruction represented. In the course of constructing an %Instruction,
    /// the %Expressions in \c operandSource will be transformed to %Operand objects. This
    /// transformation will map the semantic information about which operands are read and
    /// written in the %Operation object \c what to the value computations in \c
    /// operandSource.
    ...
    /// \param regsWritten Insert the set of registers written by the instruction into \c
    /// regsWritten.
    ///
    /// The list of registers returned in \c regsWritten includes registers that are
    /// explicitly written as destination operands (like the destination of a move).  It
    /// also includes registers that are implicitly written (like the stack pointer in a
    /// push or pop instruction).  It does not include any registers used only in
    /// computing the effective address of a write. \c pop \c *eax, for example, writes to
    /// \c esp, reads \c esp, and reads \c eax, but despite the fact that \c *eax is the
    /// destination operand, \c eax is not itself written.
    ///
    ...
    /// or read; also, any registers used in any address calculations are explicitly read.
    /// Any element of the write set or read set that is not explicitly written or read is
    /// implicitly written or read.

    INSTRUCTION_EXPORT void getWriteSet(std::set<RegisterAST::Ptr>& regsWritten) const;

    /// \param regsRead Insert the set of registers read by the instruction into \c
    /// regsRead.
    ///
    ...
    /// \param candidate Subexpression to search for among the values written by this
    /// %Instruction object.
    ///
    ...
    /// \param memAccessors Addresses read by this instruction are inserted into \c
    /// memAccessors
    ///
    /// The addresses read are in the form of %Expressions, which may be evaluated once
    /// all of the registers that they use have had their values set. Note that this
    /// method returns ASTs representing address computations, and not address accesses.
    /// For instance, an instruction accessing memory through a register dereference would
    /// return a %Expression tree containing just the register that determines the address
    /// being accessed, not a tree representing a dereference of that register.
    INSTRUCTION_EXPORT void getMemoryReadOperands(
        std::set<Expression::Ptr>& memAccessors) const;

    /// \param memAccessors Addresses written by this instruction are inserted into \c
    /// memAccessors
    ///
    ...
    /// \return An expression evaluating to the non-fallthrough control flow targets, if
    /// any, of this instruction.
    ///
    ...
    /// returns \c NULL.
    ///
    /// For direct absolute branch instructions, \c getControlFlowTarget will return an
    /// immediate value. For direct relative branch instructions, \c getControlFlowTarget
    /// will return the expression \c PC + offset. In the case of indirect branches and
    /// calls, it returns a dereference of a register (or possibly a dereference of a more
    /// complicated expression).  In this case, data flow analysis will often allow the
    /// determination of the possible targets of the instruction.  We do not do analysis
    /// beyond the single-instruction level in the %Instruction API; if other code
    /// performs this type of analysis, it may update the information in the %Dereference
    /// object using the setValue method in the %Expression interface. More details about
    /// this may be found in Expression and Dereference.
    ...
    ///
    /// \c format is principally a helper function; %Instructions are meant to be written
    /// to output streams via \c operator<<.  \c format is included in the public
    /// interface for diagnostic purposes.
    INSTRUCTION_EXPORT std::string format(Address addr = 0) const;

    /// Returns true if this %Instruction object is valid.  Invalid instructions indicate
    /// that an %InstructionDecoder has reached the end of its assigned range, and that
    /// decoding should terminate.
    INSTRUCTION_EXPORT bool isValid() const;

    /// Returns true if this %Instruction object represents a legal instruction, as
    /// specified by the architecture used to decode this instruction.
    INSTRUCTION_EXPORT bool isLegalInsn() const;

    INSTRUCTION_EXPORT Architecture getArch() const;

    /// Currently, the valid categories are c_CallInsn, c_ReturnInsn, c_BranchInsn,
    /// c_CompareInsn, and c_NoCategory, as defined in %InstructionCategories.h.
    INSTRUCTION_EXPORT InsnCategory getCategory() const;

    typedef std::list<CFT>::const_iterator cftConstIter;
    INSTRUCTION_EXPORT cftConstIter cft_begin() const { return m_Successors.begin(); }
...
public:
    // Should be private, but we're working around some compilers mis-using the 'friend'
    // declaration.
    INSTRUCTION_EXPORT void appendOperand(Expression::Ptr e, bool isRead,
                                          bool isWritten) const;
    INSTRUCTION_EXPORT void appendOperand(Expression::Ptr e, bool isRead, bool isWritten,
                                          bool isImplicit) const;
    INSTRUCTION_EXPORT void appendOperand(Expression::Ptr e, bool isRead, bool isWritten,
                                          bool isImplicit, bool trueP, bool falseP) const;

private:
    void updateSize(const unsigned int new_size) { m_size = new_size; }
    void decodeOperands() const;
    void addSuccessor(Expression::Ptr e, bool isCall, bool isIndirect, bool isConditional,
