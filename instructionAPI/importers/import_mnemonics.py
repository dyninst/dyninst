...
parser.add_argument(
    "--arch", type=str, choices=["x86", "aarch64", "ppc", "riscv64"], default="x86"
)
...
with open("mnemonics.{0:s}".format(args.arch), "w") as f:
    for p in mnemonics.pseudo:
        f.write(
            "{0:s}_{1:s}, /* pseudo mnemonic */\n".format(
                mnemonics.dyninst_prefix, p
            )
        )
...
if mnemonics.aliases is not None:
    ...
    if len(unseen_aliases) > 0:
        for a in sorted(unseen_aliases):
            print(
                "    Didn't find '{0:s}', aliased with ".format(a),
                mnemonics.aliases[a]["values"],
            )
    else:
        print("None")
