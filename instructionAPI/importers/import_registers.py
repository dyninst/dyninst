...
parser.add_argument(
    "--arch", type=str, choices=["x86", "aarch64", "ppc"], default="x86"
)
...
if args.arch == "x86":
    registers = x86.registers.registers(
        args.capstone_dir, args.dyninst_dir, args.spec_dir
    )
elif args.arch == "aarch64":
    registers = aarch64.registers.registers(
        args.capstone_dir, args.dyninst_dir, args.spec_dir
    )
elif args.arch == "ppc":
    registers = ppc.registers.registers(
        args.capstone_dir, args.dyninst_dir, args.spec_dir
    )
...
with open("{0:s}.registers".format(args.arch), "w") as file:
    ...
    for r in registers.all:
        ...
        fmt = "DEF_REGISTER(%{0:d}s, %3d | %{1:d}s | %{2:d}s | Arch_{3:s});\n".format(
            max_name_len + 2, max_size_len, max_cat_len, registers.dyninst_suffix
        )
        file.write(
            fmt
            % (
                r["name"],
                unique_ids_by_category[base_category],
                r["size"],
                r["categories"],
            )
        )
        ...
        if unique_ids_by_category[base_category] > 255:
            raise Exception(
                "There are more than 255 '{0:s}' registers.".format(base_category)
            )
...
with open("{0:s}.aliases".format(args.arch), "w") as file:
    ...
    fmt = 'DEF_REGISTER_ALIAS(%{0:d}s, %{1:d}s, "{2:s}");\n'.format(
        max_alias_len, max_primary_len, registers.dyninst_suffix
    )
