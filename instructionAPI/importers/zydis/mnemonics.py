...
def _read_zydis_mnemonics(file: str) -> [str]:
    # Format is 'ZYDIS_MNEMONIC_NAME'
    reg = re.compile(r"\s+ZYDIS_MNEMONIC_(\w+),")
    mnemonics = []
    with open(file, "r") as f:
        for line in f:
            matches = reg.search(line)
            if matches:
                mnemonics.append(matches.group(1).lower())
    return sorted(mnemonics)


def _read_dyninst_mnemonics(file: str):
    # Format is 'e_name[ = alias],[comment]'
    mnemonics = []
    with open(file, "r") as f:
        for line in f:
...
class mnemonics:
    def __init__(self, zydis_dir: str, dyn_dir: str):
        ...
        # Zydis mnemonics
        m = _read_zydis_mnemonics(zydis_dir + "/include/Zydis/Generated/EnumMnemonic.h")
        for i in _ignored:
            m.remove(i)
        self.external = sorted(m)

        # Dyninst mnemonics
        m = _read_dyninst_mnemonics(dyn_dir + "/common/h/mnemonics/x86_entryIDs.h")
        m = list(set(m) - set(self.pseudo))  # remove pseudo registers
        self.dyninst = sorted(m)
        ...
        # Those are left to Zydis to figure out. We just represent the single mnemonic
        ...
        #    Aliases for the FCMOVcc or LOOPcc mnemonics do not exist.
