...
def parse_xml_specs(spec_dir):
    ...
    for f in glob.glob(spec_dir + "/*.xml"):
        ...
        for reg in content.getElementsByTagName("registers"):
            ...
            if size is None:
                raise Exception(
                    "{0:s}: Unable to find size for '{1:s}'".format(f, str(name))
                )
            ...
            if len(array_tag) > 0:
                start = (
                    array_tag[0]
                    .getElementsByTagName("reg_array_start")[0]
                    .childNodes[0]
                    .data
                )
                end = (
                    array_tag[0]
                    .getElementsByTagName("reg_array_end")[0]
                    .childNodes[0]
                    .data
                )
                ...
                print(
                    "{0:s}: possibly bad register or namespaced dummy '{1:s}'. Skipping.".format(
                        f, name
                    )
                )
                continue
...
def parse_capstone(file: str):
    ...
    def _find_next_category(file):
        ...
        line_fmt = re.compile(
            r"AArch64GenCSSystemOperandsEnum.inc\:GET_ENUM_VALUES_(.+)?\>\s*(.+)$"
        )
        for line in file:
...
def _get_categories(content):
    long_name = (
        content.getElementsByTagName("reg_long_name")[0].childNodes[0].data.lower()
    )

    def _unknown(group, long_name):
        raise Exception(
            "Unknown group '{0:s}' (long name '{1:s}'). Create a new category for it.".format(
                group, long_name
            )
        )
    ...
    _merge_categories = ["SYSMEMORY", "IMPLDEF", "VIRT"]
