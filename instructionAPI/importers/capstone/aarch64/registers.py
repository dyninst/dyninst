...
class registers:
    ...
    def __init__(self, cap_dir: str, dyn_dir: str, spec_dir: str):
        ...
        self.capstone = _read_capstone_registers(
            cap_dir + "/include/capstone/aarch64.h"
        )
        self.capstone_sysregs = aarch64.sysregs.parse_capstone(
            cap_dir + "/include/capstone/aarch64.h"
        )
        self.dyninst = _read_dyninst_registers(
            dyn_dir + "/common/h/registers/aarch64_regs.h"
        )
        ...
        self.all = _process_regs(
            self.capstone,
            self.capstone_sysregs,
            spec_sysregs,
            _aliases | _capstone_aliases,
        )
...
def _read_capstone_registers(file: str):
    ...
    with open(file, "r") as f:
        if not _find_reg_defs(f):
            raise Exception(
                "Unable to find register definitions in '{0:s}'".format(file)
            )
...
def _process_regs(capstone, capstone_sysregs, spec_sysregs, aliases):
    ...
    regs.append({"name": "pc", "size": "FULL", "categories": "SPR", "is_sysreg": False})
    ...
    for reg_name in capstone_sysregs:
        ...
        if size not in _lengths:
            ...
            if size is None:
                raise Exception(
                    "'{0:s}': unknown sysreg size '{1:s}'".format(
                        reg_name, details["size"]
                    )
                )
...
def _encode_sysreg_size(size):
    _sizes = {"32": "D_REG", "64": "FULL", "128": "Q_REG"}
    if size in _sizes:
        return _sizes[size]
...
def _sort_reg_key(reg):
    ...
    basic_fmt = re.compile(
        r"([{0:s}]+?)(\d+)".format("".join(_capstone_by_prefix.keys()))
    )
...
_lengths = {
    ...
    "SVES": {
        "desc": "2048-bit Scalable Vector Extension (SVE) vector length",
        "sortid": 5,
    },
    "PREDS": {"desc": "SVE predicate register", "sortid": 6},
    "SVE2S": {"desc": "512-bit Scalable Vector Extension", "sortid": 7},
    "SVLS": {
        "desc": "2048-bit SME Effective Streaming SVE vector length (SVL)",
        "sortid": 8,
    },
...
_capstone_by_prefix = {
    ...
    "za": {
        "size": "SMEZAS",
        "categories": ["SME"],
    },  # Scalable Matrix Extension ZA array
    "zt": {
        "size": "SVE2S",
        "categories": ["SVE2"],
...
_capstone_by_name = {
    "ffr": _capstone_by_prefix[
        "p"
    ],  # First Fault Register, same size/category as predicate registers
    "fpcr": {"size": "D_REG", "categories": ["SPR"]},  # Floating-Point Control Register
    "nzcv": {"size": "BIT", "categories": ["SPR"]},  # Condition flag bits
    "vg": {
        "size": "FULL",
        "categories": ["SVE"],
    },  # 64-bit SVE vector granule pseudo-register (needed for DWARF mappings)
