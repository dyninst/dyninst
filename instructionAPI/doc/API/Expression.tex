...
Equality of two expressions, $E_1$ and $E_2$, is defined by
\begin{itemize}
\item $E_1$ and $E_2$ are the same type
...
not be determined, a \code{Result} with an undefined value will be returned. See
Figure~\ref{fig:deref-eval} for an illustration of this concept; the operand represented is
...
\apidesc{
    ...
    tree. We present a thorough discussion with examples in Section~\ref{sec:visitor}.
}
