...
\end{apient}
\apidesc{
    \code{what} is the opcode of the instruction; \code{operandSource}
    contains the \code{Expressions} to be transformed into Operands.
    \code{size} contains the number of bytes occupied by the corresponding
    machine instruction; \code{raw} contains a pointer to the buffer from
    which this \code{Instruction} object was decoded.
    ...
    While an Instruction object may be built \char`\"{}by hand\char`\"{} if
    ...
    In the course of constructing an Instruction, the Expressions in
    \code{operandSource} will be transformed to Operand objects. This
    transformation will map the semantic information about which operands are
    read and written in the Operation object \code{what} to the value
    computations in \code{operandSource}. 
}
...
The vector \code{operands} has the instruction's operands appended to it in the same order that they were decoded. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!getOperand@{getOperand}}
...
The \code{getOperand} method returns the operand at position \code{index}, or an empty operand if \code{index} does not correspond to a valid operand in this instruction. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!rawByte@{rawByte}}
...
\item[{\em regsWritten}]Insert the set of registers written by the instruction into \code{regsWritten}.\end{DoxyParams}
The list of registers returned in \code{regsWritten} includes registers that are explicitly written as destination operands (like the destination of a move). It also includes registers that are implicitly written (like the stack pointer in a push or pop instruction). It does not include any registers used only in computing the effective address of a write. \code{pop} \code{$\ast$eax}, for example, writes to \code{esp}, reads \code{esp}, and reads \code{eax}, but despite the fact that \code{$\ast$eax} is the destination operand, \code{eax} is not itself written.
...
\begin{DoxyParams}{Parameters}
\item[{\em regsRead}]Insert the set of registers read by the instruction into \code{regsRead}.\end{DoxyParams}
...
Returns true if \code{candidate} is read by this Instruction. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!isWritten@{isWritten}}
...
Returns true if \code{candidate} is written by this Instruction. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!readsMemory@{readsMemory}}
...
\end{DoxyReturn}
If any operand containing a Dereference object is read, the instruction reads the memory at that address. Also, on platforms where a stack pop is guaranteed to read memory, \code{readsMemory} will return true for a pop operation. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!writesMemory@{writesMemory}}
...
\end{DoxyReturn}
If any operand containing a Dereference object is written, the instruction writes the memory at that address. Also, on platforms where a stack push is guaranteed to write memory, \code{writesMemory} will return true for a push operation. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!getMemoryReadOperands@{getMemoryReadOperands}}
...
\begin{DoxyParams}{Parameters}
\item[{\em memAccessors}]Addresses read by this instruction are inserted into \code{memAccessors} \end{DoxyParams}
...
\begin{DoxyParams}{Parameters}
\item[{\em memAccessors}]Addresses written by this instruction are inserted into \code{memAccessors} \end{DoxyParams}
The addresses written are in the same form as those returned by \code{getMemoryReadOperands} above. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!getControlFlowTarget@{getControlFlowTarget}}
...
\begin{DoxyReturn}{Returns}
False if control flow will unconditionally go to the result of \code{getControlFlowTarget} after executing this instruction. 
\end{DoxyReturn}
...
\end{DoxyReturn}
\code{format} is principally a helper function; Instructions are meant to be written to output streams via \code{operator$<$$<$}. \code{format} is included in the public interface for diagnostic purposes. \index{Dyninst::InstructionAPI::Instruction@{Dyninst::InstructionAPI::Instruction}!isValid@{isValid}}
