...
\end{apient}
\apidesc{
  Inserts into \code{regsWritten} all registers written by the instruction.
  
  A register is considered written if it 1) is an explicit destination operand
  or 2) is implicitly written as a side effect of executing the instruction.
  
  For example on x86, \code{pop eax} explicitly writes to \code{eax} but also
  implicitly writes to the stack pointer (\code{esp}). The set of written
  registers is then \code{eax} and \code{esp}.
}
...
\end{apient}
\apidesc{
  Inserts into \code{regsRead} all registers read by the instruction.
  
  A register is considered read if it 1) is an explicit source operand
  or 2) is implicitly read as a side effect of executing the instruction.
  
  For example on x86, a conditional jump such as \code{jc 0x1234} explicitly
  reads the instruction pointer (\code{ip}) to form the relative target and
  also implicitly reads \code{flags} to check the carry bit (subregister
  \code{cf}). The set of read registers is then \code{ip}, \code{flags}, and
  \code{cf}.

  NOTE: Registers used in an address calculation are considered explicitly read.
  For example, \code{jmp [eax]} explicitly reads \code{eax}.
}
