...
\end{apient}
\apidesc{
Returns a pointer to the buffer from which this instruction was decoded.
}
...
\end{apient}
\apidesc{
    \code{format} is principally a helper function. Instructions are meant to be
    written to output streams via \code{operator<<}. \code{format} is included
    in the public interface for diagnostic purposes.
