...
\apidesc{
  ...
  For example on x86, \code{jc [eax]} explicitly reads \code{eax} but also
  implicitly reads \code{flags} to check the carry bit (subregister cf).
  The set of read registers is then \code{eax}, \code{flags}, and \code{cf}.
