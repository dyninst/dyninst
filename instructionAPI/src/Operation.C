...
{
RegisterAST::Ptr
makeRegFromID(MachRegister regID, unsigned int low, unsigned int high)
{
    return make_shared(singleton_object_pool<RegisterAST>::construct(regID, low, high));
}
RegisterAST::Ptr
makeRegFromID(MachRegister regID)
{
    return make_shared(
        singleton_object_pool<RegisterAST>::construct(regID, 0, regID.size() * 8));
}
...
static bool
getVectorizationInfo(ia32_entry* e)
{
    for(int i = 0; i < 3; i++)
    {
...
Operation_impl::Operation_impl(ia32_entry* e, ia32_prefixes* p, ia32_locations* l,
                               Architecture arch)
: archDecodedFrom(arch)
, prefixID(prefix_none)
{
...
const Operation_impl&
Operation_impl::operator=(const Operation_impl& o)
{
    operationID     = o.operationID;
    archDecodedFrom = o.archDecodedFrom;
    prefixID        = o.prefixID;
...
const Operation_impl::registerSet&
Operation_impl::implicitReads()
{
    SetUpNonOperandData();
...
bool
Operation_impl::isRead(Expression::Ptr candidate)
{
    SetUpNonOperandData();
    ...
    for(VCSet::const_iterator e = otherEffAddrsRead.begin(); e != otherEffAddrsRead.end();
        ++e)
    {
        if(*candidate == *(*e))
        {
...
const Operation_impl::VCSet&
Operation_impl::getImplicitMemReads()
{
    SetUpNonOperandData();
    return otherEffAddrsRead;
}
const Operation_impl::VCSet&
Operation_impl::getImplicitMemWrites()
{
    SetUpNonOperandData();
...
bool
Operation_impl::isWritten(Expression::Ptr candidate)
{
    SetUpNonOperandData();

    for(registerSet::const_iterator r = otherWritten.begin(); r != otherWritten.end();
        ++r)
    {
        if(*candidate == *(*r))
        {
...
entryID
Operation_impl::getID() const
{
    return operationID;
}

prefixEntryID
Operation_impl::getPrefixID() const
{
    return prefixID;
}
...
public:
    OperationMaps(Architecture arch)
    {
        ...
        si.insert(RegisterAST::Ptr(
            new RegisterAST(arch == Arch_x86_64 ? x86_64::rsi : x86::esi)));
        di.insert(RegisterAST::Ptr(
            new RegisterAST(arch == Arch_x86_64 ? x86_64::rdi : x86::edi)));
...
void
Operation_impl::SetUpNonOperandData()
{
    ...
    std::call_once(data_initialized, [&]() {
        if(prefixID == prefix_rep || prefixID == prefix_repnz)
        {
            ...
            otherRead.insert(
                makeRegFromID((archDecodedFrom == Arch_x86) ? x86::ecx : x86_64::rcx));
            otherWritten.insert(
                makeRegFromID((archDecodedFrom == Arch_x86) ? x86::ecx : x86_64::rcx));
            if(prefixID == prefix_repnz)
            {
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::zf : x86_64::zf));
            }
        }
        switch(segPrefix)
        {
            case PREFIX_SEGCS:
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::cs : x86_64::cs));
                break;
            case PREFIX_SEGDS:
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::ds : x86_64::ds));
                break;
            case PREFIX_SEGES:
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::es : x86_64::es));
                break;
            case PREFIX_SEGFS:
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::fs : x86_64::fs));
                break;
            case PREFIX_SEGGS:
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::gs : x86_64::gs));
                break;
            case PREFIX_SEGSS:
                otherRead.insert(
                    makeRegFromID((archDecodedFrom == Arch_x86) ? x86::ss : x86_64::ss));
                break;
        }
