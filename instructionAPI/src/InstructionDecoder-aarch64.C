...
std::map<entryID, std::string>
    InstructionDecoder_aarch64::bitfieldInsnAliasMap =
        boost::assign::map_list_of(aarch64_op_bfi_bfm, "bfi")(
            aarch64_op_bfxil_bfm, "bfxil")(aarch64_op_sbfiz_sbfm, "sbfiz")(
            aarch64_op_sbfx_sbfm, "sbfx")(aarch64_op_ubfiz_ubfm, "ubfiz")(
            aarch64_op_ubfx_ubfm, "ubfx")(aarch64_op_sxtb_sbfm, "sxtb")(
            aarch64_op_sxth_sbfm, "sxth")(aarch64_op_sxtw_sbfm, "sxtw")(
            aarch64_op_uxtb_ubfm, "uxtb")(aarch64_op_uxth_ubfm, "uxth")(
            aarch64_op_lsl_ubfm, "lsl")(aarch64_op_lsr_ubfm, "lsr");
std::map<entryID, std::string> InstructionDecoder_aarch64::condInsnAliasMap =
    boost::assign::map_list_of(aarch64_op_csinc, "csinc")(
        aarch64_op_csinv, "csinv")(aarch64_op_csneg, "csneg")(
...
struct aarch64_insn_entry {
  ...
  aarch64_insn_entry(entryID o, const char *m, operandSpec ops, Bits_t enb,
                     Bits_t mb)
  ...
  aarch64_insn_entry(const aarch64_insn_entry &o)
      : op(o.op),
        mnemonic(o.mnemonic),
        operands(o.operands),
        _encodingBits(o._encodingBits),
        _maskBits(o._maskBits) {}
...
struct aarch64_mask_entry {
  ...
  aarch64_mask_entry(const aarch64_mask_entry &e)
      : mask(e.mask),
        ...
        insnTableIndices(e.insnTableIndices),
...
InstructionDecoder_aarch64::InstructionDecoder_aarch64(Architecture a)
  ...
  std::string condArray[16] = {"eq", "ne", "cs", "cc", "mi", "pl", "vs", "vc",
                               "hi", "ls", "ge", "lt", "gt", "le", "al", "nv"};
  InstructionDecoder_aarch64::condStringMap.assign(&condArray[0],
                                                   &condArray[0] + 16);
  // InstructionDecoder_aarch64::bitfieldInsnAliasMap =
  // boost::assign::map_list_of(aarch64_op_bfi_bfm, "bfi")(aarch64_op_bfxil_bfm,
  // "bfxil")(aarch64_op_sbfiz_sbfm, "sbfiz")(aarch64_op_sbfx_sbfm,
  // "sbfx")(aarch64_op_ubfiz_ubfm, "ubfiz")(aarch64_op_ubfx_ubfm, "ubfx");
}
...
Instruction::Ptr InstructionDecoder_aarch64::decode(
...
void InstructionDecoder_aarch64::processShiftFieldShiftedInsn(int len,
                                                              int val) {
  Result_Type rT;
  ...
  {
    case 0:
      insn_in_progress->appendOperand(makeLeftShiftExpression(lhs, rhs, rT),
                                      true, false);
      break;
    case 1:
      insn_in_progress->appendOperand(
          makeRightLogicalShiftExpression(lhs, rhs, rT), true, false);
      break;
    case 2:
      insn_in_progress->appendOperand(
      ...
      break;
    case 3:
      if (IS_INSN_LOGICAL_SHIFT(insn))  // logical (shifted) -- not applicable
                                        // to add-sub (shifted)
...
Expression::Ptr InstructionDecoder_aarch64::makeOptionExpression(int len,
                                                                 int val) {
  MachRegister reg;
  int encoding = field<16, 20>(insn);

  reg = ((optionField & 0x3) == 0x3)
            ? ((encoding == 31) ? aarch64::zr : aarch64::x0)
            : ((encoding == 31) ? aarch64::wzr : aarch64::w0);
  if (encoding != 31) reg = makeAarch64RegID(reg, encoding);
  ...
  return makeLeftShiftExpression(
      lhs, Immediate::makeImmediate(Result(u32, unsign_extend32(len, val))),
      rT);
}
...
void InstructionDecoder_aarch64::processOptionFieldLSRegOffsetInsn() {
  if (optionField == 0x3)  // option = LSL
  {
    ...
    if (field<23, 23>(insn) == 1) sizeVal = 4;
...
void InstructionDecoder_aarch64::processSystemInsn() {
  ...
  {
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(Result(u8, unsign_extend32(3, op1Field))),
        true, false);
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(Result(u8, unsign_extend32(4, crnField))),
        true, false);
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(Result(u8, unsign_extend32(4, crmField))),
        true, false);
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(Result(u8, unsign_extend32(3, op2Field))),
        true, false);
  ...
  } else  // mrs (register), msr
  {
    ...
    MachRegister reg;
    if ((op0Field & 0x3) == 0x3 && (crnField & 0x3) == 0x3 &&
        (crnField & 0x8) == 0x8)
      reg = aarch64::IMPLEMENTATION_DEFINED_SYSREG;
    else
      reg = InstructionDecoder_aarch64::sysRegMap[systemRegEncoding];
    insn_in_progress->appendOperand(makeRegisterExpression(reg), !isRtRead,
                                    isRtRead);
    insn_in_progress->appendOperand(makeRtExpr(), isRtRead, !isRtRead);
    if (!isRtRead) insn_in_progress->m_Operands.reverse();
  }
}
...
MachRegister InstructionDecoder_aarch64::makeAarch64RegID(
    MachRegister base, unsigned int encoding) {
...
Expression::Ptr InstructionDecoder_aarch64::makeRdExpr() {
  ...
  if (isSIMDInsn) {
    if (IS_INSN_SIMD_ACROSS(insn)) {
      ...
      // some instructions in this set rely on sz for choosing the register and
      // some on size
      ...
      if (size != -1) {
        ...
        isValid = false;
    } else if (IS_INSN_SIMD_MOD_IMM(insn) && _Q == 0 && op == 1 &&
               cmode == 0xE) {
      reg = aarch64::d0;
    ...
    reg = makeAarch64RegID(reg, encoding);
             ...
             !((IS_INSN_FP_CONV_FIX(insn) || (IS_INSN_FP_CONV_INT(insn))) &&
               !IS_SOURCE_GP(insn))) {
    ...
    if (encoding == 31)
      reg = ((IS_INSN_ADDSUB_IMM(insn) || IS_INSN_ADDSUB_EXT(insn) ||
             ...
             !isPstateWritten)
                ...
                : (is64Bit ? aarch64::zr : aarch64::wzr);
    else
      reg = is64Bit ? aarch64::x0 : aarch64::w0;

    if (isValid && encoding != 31) reg = makeAarch64RegID(reg, encoding);
  }
  ...
  if (((IS_INSN_SIMD_VEC_INDEX(insn) || IS_INSN_SCALAR_INDEX(insn)) &&
      ...
      (IS_INSN_SIMD_MOD_IMM(insn) &&
       (((cmode & 0x8) && !(cmode & 0x4) && (cmode & 0x1)) ||
        (!(cmode & 0x8) && (cmode & 0x1)))))
    isRdRead = true;
  // for SIMD/Scalar vector indexed set, some instructions read Rd and some
  // don't. This can be determined from the highest bit of the opcode field (bit
  // 15)
...
void InstructionDecoder_aarch64::OPRop() { op = field<29, 29>(insn); }
...
Expression::Ptr InstructionDecoder_aarch64::makeRnExpr() {
  ...
  if (isSIMDInsn && !IS_INSN_LDST(insn)) {
    if (IS_INSN_SIMD_COPY(insn)) {
      ...
      if ((opcode & 0x1C) == 0x10) isRnVa = true;
...
void InstructionDecoder_aarch64::getMemRefIndexLiteral_OffsetLen(int &immVal,
                                                                 int &immLen) {
...
Expression::Ptr InstructionDecoder_aarch64::makeMemRefIndexLiteral() {
  ...
  return makeDereferenceExpression(makeAddExpression(makePCExpr(), label, u64),
                                   rt);
}
...
void InstructionDecoder_aarch64::getMemRefIndex_SizeSizelen(
    unsigned int &size, unsigned int &sizeLen) {
  size = field<30, 31>(insn);
  if (size == 0x0 && field<23, 23>(insn) == 0x1) size = 4;
  sizeLen = 31 - 30 + 1 + (size / 4);
  return;
}

void InstructionDecoder_aarch64::getMemRefIndexPrePost_ImmImmlen(
    unsigned int &immVal, unsigned int &immLen) {
  immVal = field<12, 20>(insn);
  immLen = 20 - 12 + 1;
  return;
}

void InstructionDecoder_aarch64::getMemRefPair_ImmImmlen(unsigned int &immVal,
                                                         unsigned int &immLen) {
...
Expression::Ptr InstructionDecoder_aarch64::makeMemRefIndexUImm() {
  ...
  return makeDereferenceExpression(makeAddExpression(makeRnExpr(), offset, u64),
                                   rt);
}
...
scaleVal += 2;
Expression::Ptr scale = Immediate::makeImmediate(Result(u32,
unsign_extend32(scaleLen, 1<<scaleVal)));
*/
...
void InstructionDecoder_aarch64::OPRQ() { _Q = field<30, 30>(insn); }

void InstructionDecoder_aarch64::OPRL() { _L = field<30, 30>(insn); }
...
unsigned int InstructionDecoder_aarch64::getSIMD_SING_selem() {
  return (((field<13, 13>(insn) << 1) & 0x2) | (field<21, 21>(insn) & 0x1)) +
         0x1;
}
...
Expression::Ptr InstructionDecoder_aarch64::makeMemRefExPair2(){
    ...
    Expression::Ptr offset = Immediate::makeImmediate(Result(u32,
unsign_extend32(immLen, immVal)));
    return makeDereferenceExpression(makeAddExpression(makeRnExpr(), offset,
u64) , u64);
}
*/
...
Expression::Ptr InstructionDecoder_aarch64::makeMemRefReg_amount() {
  ...
  return Immediate::makeImmediate(
      Result(u32, unsign_extend32(amountLen, amountVal)));
}
...
void InstructionDecoder_aarch64::LIndex() {
  ...
  } else if (IS_INSN_LDST_PAIR_POST(insn) || IS_INSN_LDST_PAIR_OFFSET(insn)) {
...
void InstructionDecoder_aarch64::OPRRnS() { STIndex(); }
...
void InstructionDecoder_aarch64::OPRRnLU() {
  if (IS_INSN_LDST_PRE(insn) || IS_INSN_LDST_POST(insn) ||
      IS_INSN_LDST_PAIR_PRE(insn) || IS_INSN_LDST_PAIR_POST(insn))
    LIndex();
}
...
Expression::Ptr InstructionDecoder_aarch64::makeRmExpr() {
  ...
  if (isSIMDInsn) {
    ...
    reg = is64Bit ? ((encoding == 31) ? aarch64::zr : aarch64::x0)
                  : ((encoding == 31) ? aarch64::wzr : aarch64::w0);
    if (encoding != 31) reg = makeAarch64RegID(reg, encoding);
  }
  ...
  if (IS_INSN_FP_COMPARE(insn) && field<3, 3>(insn) == 1)
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(
            Result(isSinglePrec() ? sp_float : dp_float, 0.0)),
        true, false);
  else
...
void InstructionDecoder_aarch64::OPRsf() {
  if (field<31, 31>(insn) == 0) is64Bit = false;
}
...
void InstructionDecoder_aarch64::setRegWidth() {
  ...
  if (IS_INSN_LDST(insn)) {
    if (IS_INSN_LDST_UIMM(insn) || IS_INSN_LDST_UNSCALED(insn) ||
      ...
      if (opc1 == 0) {
        if (field<30, 31>(insn) != 3) is64Bit = false;
        return;
      } else {
        if (sz == 3) {
          if (opc0 == 1) isValid = false;
        } else {
          if (sz == 2 && opc0 == 1) isValid = false;
          if (opc0 == 1) is64Bit = false;
        }
        return;
      }
    } else if (IS_INSN_LDST_EX(insn)) {
...
MachRegister InstructionDecoder_aarch64::getLoadStoreSimdRegister(
    int encoding) {
  MachRegister reg;
...
Expression::Ptr InstructionDecoder_aarch64::makeRtExpr() {
  ...
  if (isFPInsn) {
    reg =
        makeAarch64RegID(isSinglePrec() ? aarch64::s0 : aarch64::d0, encoding);
  } else if (isSIMDInsn) {
...
void InstructionDecoder_aarch64::getSIMD_MULT_RptSelem(unsigned int &rpt,
                                                       unsigned int &selem) {
  ...
  if (rpt == 0 || selem == 0) isValid = false;
  return;
}
...
void InstructionDecoder_aarch64::OPRRt() {
  ...
  if (IS_INSN_BRANCHING(insn)) {
    if (encoding == 31)
      insn_in_progress->appendOperand(
          makeRegisterExpression(is64Bit ? aarch64::zr : aarch64::wzr), true,
          false);
    else
      insn_in_progress->appendOperand(
          ...
          true, false);
  } else if (op == aarch64_op_prfm_imm || op == aarch64_op_prfm_lit ||
             op == aarch64_op_prfm_reg) {
...
void InstructionDecoder_aarch64::OPRop1() { op1Field = field<16, 18>(insn); }

void InstructionDecoder_aarch64::OPRop2() { op2Field = field<5, 7>(insn); }

void InstructionDecoder_aarch64::OPRCRm() { crmField = field<8, 11>(insn); }
...
void InstructionDecoder_aarch64::OPRscale() {
  ...
  Expression::Ptr scale =
      Immediate::makeImmediate(Result(u32, unsign_extend32(6, scaleVal)));
...
Expression::Ptr InstructionDecoder_aarch64::makeRaExpr() {
  ...
  if (isFPInsn) {
    ...
    reg = is64Bit ? ((encoding == 31) ? aarch64::sp : aarch64::x0)
                  : ((encoding == 31) ? aarch64::wsp : aarch64::w0);
    if (encoding != 31) reg = makeAarch64RegID(reg, encoding);
  }
...
void InstructionDecoder_aarch64::makeBranchTarget(bool branchIsCall,
                                                  bool bIsConditional,
  ...
  insn_in_progress->addSuccessor(makeAddExpression(lhs, rhs, s64), branchIsCall,
                                 false, bIsConditional, false);
  if (branchIsCall) {
    insn_in_progress->addSuccessor(makeFallThroughExpr(), false, false, false,
                                   true);
  }
...
template <typename T>
Expression::Ptr InstructionDecoder_aarch64::makeLogicalImm(int immr, int imms,
                                                           int immsLen,
                                                           Result_Type rT) {
  int len =
      highest_set_bit((nField << immsLen) | (~imms & ((1 << immsLen) - 1))) - 1;
  ...
  switch (field<27, 30>(insn)) {
    ...
    case 0x2:
      if (immr == 0 && (imms == 7 || imms == 15 || imms == 31)) {
        ...
        modifiedID =
            (imms < immr) ? aarch64_op_sbfiz_sbfm : aarch64_op_sbfx_sbfm;
      break;
    case 0xA:
      if (immr == 0 && (imms == 7 || imms == 15)) {
        ...
        modifiedID =
            (imms < immr) ? aarch64_op_ubfiz_ubfm : aarch64_op_ubfx_ubfm;
      break;
    default:
      isValid = false;
  }
...
void InstructionDecoder_aarch64::fix_condinsn_alias_and_cond(int &cond) {
  entryID modifiedID = insn_in_progress->getOperation().operationID;
  if (modifiedID == aarch64_op_csel) return;
  ...
  if (skipRm) cond = ((cond % 2) == 0) ? (cond + 1) : (cond - 1);
}
...
void InstructionDecoder_aarch64::OPRimm() {
  ...
  } else if (hasN)  // logical (immediate), bitfield, extract
  {
    if (IS_FIELD_IMMR(startBit, endBit)) {
      ...
      if (IS_INSN_LOGICAL_IMM(insn)) {
        ...
        if (IS_INSN_BITFIELD(insn)) {
          if (!fix_bitfieldinsn_alias(immr, immVal)) return;
          ...
          if (curID != aarch64_op_lsr_ubfm && immVal < immr) {
            ...
            while ((divisor * factor) < immr) factor += 1;
      ...
      if (IS_INSN_BITFIELD(insn)) {
        imm = Immediate::makeImmediate(
            Result(u32, unsign_extend32(immrLen, immr)));
        insn_in_progress->appendOperand(imm, true, false);
        if (!isLsrLsl) oprRotateAmt--;
      }
    } else
      isValid = false;
  } else if (hasShift) {
  ...
  } else if (IS_INSN_BRANCHING(insn) &&
             !IS_INSN_B_UNCOND_REG(
                 insn)) {  // unconditional branch (immediate), test and branch,
                           // compare and branch, conditional branch
    bool bIsConditional = false;
    if (!(IS_INSN_B_UNCOND(insn))) bIsConditional = true;
    ...
    if (hasb5) insn_in_progress->appendOperand(makeb40Expr(), true, false);
  ...
  } else if (IS_INSN_PCREL_ADDR(insn))  // pc-relative addressing
  {
    if (IS_FIELD_IMMLO(startBit, endBit)) {
      ...
      Expression::Ptr imm = Immediate::makeImmediate(
          Result(s64, (offset << (64 - size)) >> (64 - size)));
    ...
    if (isSinglePrec())
      insn_in_progress->appendOperand(fpExpand<int32_t, s32>(immVal), true,
                                      false);
    else
      insn_in_progress->appendOperand(fpExpand<int64_t, s64>(immVal), true,
                                      false);
    ...
    if (IS_INSN_SIMD_EXTR(insn)) {
      ...
      // immb
        ...
        // check if shift is left; if it is, the immediate has to be processed
        // in a different manner.
        // unfortunately, determining whether the instruction will do a left or
        // right shift cannot be determined in any way other than checking the
        ...
        switch (highest_set_bit(immlo)) {
          ...
          case 0x4:
            shift =
                isRightShift * (128 - elemWidth) + (isRightShift > 0 ? 0 : 64);
            break;
          default:
            isValid = false;
        }
...
void InstructionDecoder_aarch64::reorderOperands() {
  if (oprRotateAmt) {
    ...
    if (curOperands.size() == 3) {
      curOperands.insert(curOperands.begin() + 1, curOperands.back());
      curOperands.pop_back();
      insn_in_progress->m_Operands.assign(curOperands.begin(),
                                          curOperands.end());
    } else
...
void InstructionDecoder_aarch64::processAlphabetImm() {
  if (op == 1 && cmode == 0xE) {
    ...
    for (int imm_index = 0; imm_index < 8; imm_index++)
      imm |=
          (simdAlphabetImm & (1 << imm_index)) ? (0xFF << (imm_index * 8)) : 0;

    insn_in_progress->appendOperand(Immediate::makeImmediate(Result(u64, imm)),
                                    true, false);
  } else if (cmode == 0xF) {
    ...
    if ((cmode & 0xC) == 0x8) shiftAmt = ((cmode & 0x2) >> 1) * 8;
    ...
    Expression::Ptr lhs = Immediate::makeImmediate(
        Result(u32, unsign_extend32(8, simdAlphabetImm)));
    Expression::Ptr rhs =
...
void InstructionDecoder_aarch64::doDelayedDecode(
...
bool InstructionDecoder_aarch64::decodeOperands(
  ...
  aarch64_insn_entry *insn_table_entry =
  ...
  if (IS_INSN_LDST_REG(insn) || IS_INSN_ADDSUB_EXT(insn) ||
      IS_INSN_ADDSUB_SHIFT(insn) || IS_INSN_LOGICAL_SHIFT(insn))
    skipRm = true;
  ...
  if (insn_table_index == 0) isValid = false;
...
int InstructionDecoder_aarch64::findInsnTableIndex(
    unsigned int decoder_table_index) {
  aarch64_mask_entry *cur_entry =
      &aarch64_mask_entry::main_decoder_table[decoder_table_index];
  ...
  if (cur_branches.count(branch_map_key) <= 0) return 0;
  //	branch_map_key = 0;
...
void InstructionDecoder_aarch64::mainDecode() {
  ...
  insn_in_progress =
      makeInstruction(insn_table_entry->op, insn_table_entry->mnemonic, 4,
