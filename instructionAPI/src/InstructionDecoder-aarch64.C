...
const char*
InstructionDecoder_aarch64::bitfieldInsnAliasMap(entryID e)
{
    switch(e)
    {
        case aarch64_op_bfi_bfm:
            return "bfi";
        case aarch64_op_bfxil_bfm:
            return "bfxil";
        case aarch64_op_sbfiz_sbfm:
            return "sbfiz";
        case aarch64_op_sbfx_sbfm:
            return "sbfx";
        case aarch64_op_ubfiz_ubfm:
            return "ubfiz";
        case aarch64_op_ubfx_ubfm:
            return "ubfx";
        case aarch64_op_sxtb_sbfm:
            return "sxtb";
        case aarch64_op_sxth_sbfm:
            return "sxth";
        case aarch64_op_sxtw_sbfm:
            return "sxtw";
        case aarch64_op_uxtb_ubfm:
            return "uxtb";
        case aarch64_op_uxth_ubfm:
            return "uxth";
        case aarch64_op_lsl_ubfm:
            return "lsl";
        case aarch64_op_lsr_ubfm:
            return "lsr";
        default:
...
const char*
InstructionDecoder_aarch64::condInsnAliasMap(entryID e)
{
    switch(e)
    {
        ...
        case aarch64_op_csneg:
            return "csneg";
        case aarch64_op_cinc_csinc:
            return "cinc";
        case aarch64_op_cset_csinc:
            return "cset";
        case aarch64_op_cinv_csinv:
            return "cinv";
        case aarch64_op_csetm_csinv:
            return "csetm";
        case aarch64_op_cneg_csneg:
            return "cneg";
        default:
            assert(!"no alias for entryID");
    };
...
void
InstructionDecoder_aarch64::decodeOpcode(InstructionDecoder::buffer& b)
{
    b.start += 4;
}
...
Instruction
InstructionDecoder_aarch64::decode(InstructionDecoder::buffer& b)
{
    if(b.start > b.end)
        return Instruction();
...
*/
void
InstructionDecoder_aarch64::NOTHING()
{}

void
InstructionDecoder_aarch64::setFPMode()
{
...
void
InstructionDecoder_aarch64::setSIMDMode()
{
...
void
InstructionDecoder_aarch64::OPRtype()
{
    _typeField = field<startBit, endBit>(insn);
}

void
InstructionDecoder_aarch64::processHwFieldInsn(int len, int val)
{
    ...
    Expression::Ptr rhs =
        Immediate::makeImmediate(Result(u32, unsign_extend32(6, shiftAmount)));

    insn_in_progress->appendOperand(makeLeftShiftExpression(lhs, rhs, rT), true, false);
}

void
InstructionDecoder_aarch64::processShiftFieldShiftedInsn(int len, int val)
{
    Result_Type     rT;
    ...
    {
        case 0:
            insn_in_progress->appendOperand(makeLeftShiftExpression(lhs, rhs, rT), true,
                                            false);
            break;
        case 1:
            insn_in_progress->appendOperand(makeRightLogicalShiftExpression(lhs, rhs, rT),
                                            true, false);
            break;
        case 2:
            insn_in_progress->appendOperand(
                makeRightArithmeticShiftExpression(lhs, rhs, rT), true, false);
            break;
        case 3:
            if(IS_INSN_LOGICAL_SHIFT(
                   insn))  // logical (shifted) -- not applicable to add-sub (shifted)
                insn_in_progress->appendOperand(makeRightRotateExpression(lhs, rhs, rT),
                                                true, false);
            else
                isValid = false;
            break;
...
void
InstructionDecoder_aarch64::processShiftFieldImmInsn(int len, int val)
{
    if(shiftField == 0 || shiftField == 1)  // add-sub (immediate)
    {
        ...
        Expression::Ptr lhs = Immediate::makeImmediate(Result(
            rT, rT == u32 ? unsign_extend32(len, val) : unsign_extend64(len, val)));
        Expression::Ptr rhs =
            Immediate::makeImmediate(Result(u32, unsign_extend32(4, shiftAmount)));
...
Expression::Ptr
InstructionDecoder_aarch64::makeOptionExpression(int len, int val)
{
    MachRegister reg;
    int          encoding = field<16, 20>(insn);

    reg = ((optionField & 0x3) == 0x3) ? ((encoding == 31) ? aarch64::xzr : aarch64::x0)
                                       : ((encoding == 31) ? aarch64::wzr : aarch64::w0);
    if(encoding != 31)
    ...
    Result_Type rT =
        is64Bit ? (optionField < 4 ? u64 : s64) : (optionField < 4 ? u32 : s32);

    return makeLeftShiftExpression(
        lhs, Immediate::makeImmediate(Result(u32, unsign_extend32(len, val))), rT);
}

void
InstructionDecoder_aarch64::processOptionFieldLSRegOffsetInsn()
{
...
void
InstructionDecoder_aarch64::processSystemInsn()
{
    ...
    if(op0Field == 0)
    {
        ...
        {
            Expression::Ptr CRm =
                Immediate::makeImmediate(Result(u8, unsign_extend32(4, crmField)));
        ...
        else if(crnField == 2)  // hint
        {
            ...
            Expression::Ptr imm =
                Immediate::makeImmediate(Result(u8, unsign_extend32(7, immVal)));
        ...
        else if(crnField == 4)  // msr (immediate)
        {
            ...
            insn_in_progress->appendOperand(
                Immediate::makeImmediate(Result(u8, unsign_extend32(6, pstatefield))),
                true, false);

            insn_in_progress->appendOperand(
                Immediate::makeImmediate(Result(u8, unsign_extend32(4, crmField))), true,
                false);
    ...
    {
        insn_in_progress->appendOperand(
            Immediate::makeImmediate(Result(u8, unsign_extend32(3, op1Field))), true,
            false);
        insn_in_progress->appendOperand(
            Immediate::makeImmediate(Result(u8, unsign_extend32(4, crnField))), true,
            false);
        insn_in_progress->appendOperand(
            Immediate::makeImmediate(Result(u8, unsign_extend32(4, crmField))), true,
            false);
        insn_in_progress->appendOperand(
            Immediate::makeImmediate(Result(u8, unsign_extend32(3, op2Field))), true,
            false);
    ...
    else  // mrs (register), msr
    {
        ...
        unsigned int systemRegEncoding = (op0Field << 14) | (op1Field << 11) |
                                         (crnField << 7) | (crmField << 3) | op2Field;
...
Result_Type
InstructionDecoder_aarch64::makeSizeType(unsigned int)
{
...
MachRegister
InstructionDecoder_aarch64::makeAarch64RegID(MachRegister base, unsigned int encoding)
{
...
template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRsize()
{
    ...
    if((insnID == aarch64_op_pmull_advsimd && (size == 0x1 || size == 0x2)) ||
       ...
       ((insnID == aarch64_op_sqdmull_advsimd_vec ||
         insnID == aarch64_op_sqdmlal_advsimd_vec ||
         insnID == aarch64_op_sqdmlsl_advsimd_vec) &&
        size == 0))
    {
        isValid = false;
    }
...
Expression::Ptr
InstructionDecoder_aarch64::makeRdExpr()
{
    ...
    if(isSIMDInsn)
    {
        if(IS_INSN_SIMD_ACROSS(insn))
        {
            ...
            else
            {
                ...
                // saddlv and uaddlv with opcode field 0x03 use different sets of
                // registers
                switch(size)
                {
                    case 0x0:
        ...
        {
            // some instructions in this set rely on sz for choosing the register and some
            // on size only one of them is set for an instruction, however
            if(_szField == -1)
            {
                switch(size)
                {
            ...
            else
            {
                ...
                switch(_szField)
                {
                    case 0x0:
                        ...
                        break;
                    case 0x1: {
                        reg = (op_ == aarch64_op_fcvtxn_advsimd) ? aarch64::s0
                                                                 : aarch64::d0;
                    }
                    break;
                    default:
    ...
    {
        if(encoding == 31)
            reg = ((IS_INSN_ADDSUB_IMM(insn) || IS_INSN_ADDSUB_EXT(insn) ||
                    IS_INSN_LOGICAL_IMM(insn)) &&
                   !isPstateWritten)
                      ? (is64Bit ? aarch64::sp : aarch64::wsp)
                      : (is64Bit ? aarch64::xzr : aarch64::wzr);
        else
...
Expression::Ptr
InstructionDecoder_aarch64::makeRnExpr()
{
    ...
    if(isSIMDInsn && !IS_INSN_LDST(insn))
    {
        ...
        else if(IS_INSN_SCALAR_2REG_MISC(insn) || IS_INSN_SCALAR_3SAME(insn))
        {
            ...
            bool isRnVa = false;
            int  opcode = field<12, 16>(insn);
            if(!IS_INSN_SCALAR_3SAME(insn) && (opcode & 0x18) == 0x10 &&
               (opcode & 0x1) == 0x0)
                isRnVa = true;
        ...
        {
            // the below two conditions can easily be combined into one, but would be
        ...
        else if(IS_INSN_SIMD_TAB_LOOKUP(insn))
        {
            ...
            {
                insn_in_progress->appendOperand(makeRegisterExpression(makeAarch64RegID(
                                                    reg, (encoding + reg_index) % 32)),
                                                true, false);
            }
        }
        else if(IS_INSN_SIMD_3DIFF(insn))
    ...
    else
    {
        ...
        if(field<24, 28>(insn) == 0x1B &&
           (field<21, 23>(insn) == 0x1 || field<21, 23>(insn) == 0x5))
            use32for64bit = true;

        if(encoding == 31)
            reg = (IS_INSN_ADDSUB_IMM(insn) || IS_INSN_ADDSUB_EXT(insn))
                      ? (is64Bit ? aarch64::sp : aarch64::wsp)
                      : ((is64Bit && !use32for64bit) ? aarch64::xzr : aarch64::wzr);
        else
...
Expression::Ptr
InstructionDecoder_aarch64::makePCExpr()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makePstateExpr()
{
...
void
InstructionDecoder_aarch64::getMemRefIndexLiteral_OffsetLen(int& immVal, int& immLen)
{
...
void
InstructionDecoder_aarch64::getMemRefIndexLiteral_RT(Result_Type& rt)
{
...
// ****************************************
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefIndexLiteral()
{
    int immVal, immLen;
    getMemRefIndexLiteral_OffsetLen(immVal, immLen);

    Expression::Ptr label =
        Immediate::makeImmediate(Result(s64, sign_extend64(immLen, immVal)));
...
void
InstructionDecoder_aarch64::getMemRefIndex_RT(Result_Type& rt)
{
...
void
InstructionDecoder_aarch64::getMemRefPair_RT(Result_Type& rt)
{
...
void
InstructionDecoder_aarch64::getMemRefIndex_SizeSizelen(unsigned int& size_,
...
void
InstructionDecoder_aarch64::getMemRefIndexPrePost_ImmImmlen(unsigned int& immVal,
                                                            unsigned int& immLen)
{
...
void
InstructionDecoder_aarch64::getMemRefPair_ImmImmlen(unsigned int& immVal,
                                                    unsigned int& immLen)
{
...
// ****************************************
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefIndexUImm()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefIndex_offset9()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefPair_offset7()
{
    /*
unsigned int scaleVal = field<31, 31>(insn);
unsigned int scaleLen = 8;
scaleVal += 2;
Expression::Ptr scale = Immediate::makeImmediate(Result(u32, unsign_extend32(scaleLen,
1<<scaleVal)));
*/
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefIndex_addOffset9()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefPair_addOffset7()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefIndexPre()
{
    Result_Type rt;
    getMemRefIndex_RT(rt);
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefIndexPost()
{
    Result_Type rt;
    getMemRefIndex_RT(rt);
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefPairPre()
{
    Result_Type rt{};
    getMemRefPair_RT(rt);
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefPairPost()
{
    Result_Type rt{};
    getMemRefPair_RT(rt);
...
void
InstructionDecoder_aarch64::getMemRefEx_RT(Result_Type& rt)
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefEx()
{
    Result_Type rt;
    getMemRefEx_RT(rt);
    return makeDereferenceExpression(makeRnExpr(), rt);
...
void
InstructionDecoder_aarch64::OPRQ()
{
    _Q = field<30, 30>(insn);
}

void
InstructionDecoder_aarch64::OPRL()
{
    _L = field<30, 30>(insn);
}

unsigned int
InstructionDecoder_aarch64::getMemRefSIMD_MULT_T()
{
...
void
InstructionDecoder_aarch64::getMemRefSIMD_MULT_RT(Result_Type& rt)
{
...
unsigned int
InstructionDecoder_aarch64::getSIMD_SING_selem()
{
    return (((field<13, 13>(insn) << 1) & 0x2) | (field<21, 21>(insn) & 0x1)) + 0x1;
}

void
InstructionDecoder_aarch64::getMemRefSIMD_SING_RT(Result_Type& rt)
{
...
unsigned int
InstructionDecoder_aarch64::getMemRefSIMD_SING_T()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefSIMD_MULT()
{
    Result_Type rt = invalid_type;
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefSIMD_SING()
{
    Result_Type rt = invalid_type;
...
void
InstructionDecoder_aarch64::getMemRefExPair_RT(Result_Type& rt)
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefExPair()
{
    Result_Type rt;
...
Expression::Ptr InstructionDecoder_aarch64::makeMemRefExPair2(){
    ...
    Expression::Ptr offset = Immediate::makeImmediate(Result(u32, unsign_extend32(immLen,
immVal))); return makeDereferenceExpression(makeAddExpression(makeRnExpr(), offset, u64) ,
u64);
}
*/

Expression::Ptr
InstructionDecoder_aarch64::makeMemRefReg_Rm()
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefReg_amount()
{
    unsigned int S         = field<12, 12>(insn);
...
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefReg_ext()
{
    int immLen = 2;
...
**********************/
Expression::Ptr
InstructionDecoder_aarch64::makeMemRefReg()
{
...
void
InstructionDecoder_aarch64::LIndex()
{
    // never be called
    ...
    // ******************
    else if(IS_INSN_LDST_PRE(insn) || IS_INSN_LDST_UNPRIV(insn) ||
    ...
    {
        if(!IS_INSN_LDST_EX_PAIR(insn))
        {  // Rt2 field == 31, non-pair op
...
void
InstructionDecoder_aarch64::STIndex()
{
    if(IS_INSN_LD_LITERAL(insn))
...
void
InstructionDecoder_aarch64::OPRRn()
{
    if(skipRn)
        return;
    ...
    if(IS_INSN_B_UNCOND_REG(insn))  // unconditional branch (register)
    {
        ...
        insn_in_progress->addSuccessor(makeRnExpr(), field<21, 21>(insn) == 1, true,
                                       false, false);
...
void
InstructionDecoder_aarch64::OPRRnL()
{
    LIndex();
}

void
InstructionDecoder_aarch64::OPRRnS()
{
    STIndex();
}

void
InstructionDecoder_aarch64::OPRRnU()
{
    assert(0);
...
void
InstructionDecoder_aarch64::OPRRnLU()
{
...
void
InstructionDecoder_aarch64::OPRRnSU()
{
    if(IS_INSN_LDST_PRE(insn) || IS_INSN_LDST_POST(insn) || IS_INSN_LDST_PAIR_PRE(insn) ||
       IS_INSN_LDST_PAIR_POST(insn))
        STIndex();
...
unsigned int
InstructionDecoder_aarch64::get_SIMD_MULT_POST_imm()
{
    unsigned int rpt = 0, selem = 0;
    getSIMD_MULT_RptSelem(rpt, selem);
...
unsigned int
InstructionDecoder_aarch64::get_SIMD_SING_POST_imm()
{
    return (getMemRefSIMD_SING_T() >> 3) * getSIMD_SING_selem();
}

Expression::Ptr
InstructionDecoder_aarch64::makeRmExpr()
{
    int          encoding = field<16, 20>(insn);
    MachRegister reg;
        ...
        if(IS_INSN_LDST_SIMD_MULT_POST(insn))
        {
            if(encoding == 31)
            {
                ...
                return Immediate::makeImmediate(
                    Result(u32, unsign_extend32(immLen, immVal)));
            }
            else
                reg = aarch64::x0;
        }
    ...
    else
    {
        ...
        reg = (is64Bit && !use32for64bit)
                  ? ((encoding == 31) ? aarch64::xzr : aarch64::x0)
                  : ((encoding == 31) ? aarch64::wzr : aarch64::w0);
        if(encoding != 31)
            reg = makeAarch64RegID(reg, encoding);
    }
...
void
InstructionDecoder_aarch64::OPRRm()
{
    if(skipRm)
        return;
    ...
    if(IS_INSN_FP_COMPARE(insn) && field<3, 3>(insn) == 1)
        insn_in_progress->appendOperand(
            Immediate::makeImmediate(Result(isSinglePrec() ? sp_float : dp_float, 0.0)),
            true, false);
    else
...
void
InstructionDecoder_aarch64::OPRsf()
{
...
template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRoption()
{
    hasOption   = true;
...
void
InstructionDecoder_aarch64::OPRshift()
{
    hasShift   = true;
...
void
InstructionDecoder_aarch64::OPRhw()
{
    hasHw   = true;
...
template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRN()
{
    hasN   = true;
...
void
InstructionDecoder_aarch64::setRegWidth()
{
...
MachRegister
InstructionDecoder_aarch64::getLoadStoreSimdRegister(int encoding)
{
    MachRegister reg;
...
Expression::Ptr
InstructionDecoder_aarch64::makeRtExpr()
{
    int          encoding = field<0, 4>(insn);
    MachRegister reg;
    ...
    {
        reg = is64Bit ? ((encoding == 31) ? aarch64::xzr : aarch64::x0)
                      : ((encoding == 31) ? aarch64::wzr : aarch64::w0);
        if(encoding != 31)
            reg = makeAarch64RegID(reg, encoding);
    }
...
void
InstructionDecoder_aarch64::getSIMD_MULT_RptSelem(unsigned int& rpt, unsigned int& selem)
{
...
void
InstructionDecoder_aarch64::OPRRt()
{
    int     encoding = field<0, 4>(insn);
    entryID op_      = insn_in_progress->getOperation().operationID;
    ...
    if(IS_INSN_BRANCHING(insn))
    {
        if(encoding == 31)
            insn_in_progress->appendOperand(
                makeRegisterExpression(is64Bit ? aarch64::xzr : aarch64::wzr), true,
                false);
        else
            insn_in_progress->appendOperand(
                makeRegisterExpression(
                    makeAarch64RegID(is64Bit ? aarch64::x0 : aarch64::w0, encoding)),
                true, false);
    }
    else if(op_ == aarch64_op_prfm_imm || op_ == aarch64_op_prfm_lit ||
            op_ == aarch64_op_prfm_reg || op_ == aarch64_op_prfum)
    {
        Expression::Ptr prfop;
        Result          arg = Result(u32, unsign_extend32(5, encoding));

        if((encoding & 0x1E) == 0x6 || (encoding & 0x1E) == 0xE ||
           (encoding & 0x1E) == 0x16 || (encoding & 0x18) == 0x18)
...
void
InstructionDecoder_aarch64::OPRRtL()
{
    int encoding = field<0, 4>(insn);
    ...
    if(IS_INSN_LDST_SIMD_MULT(insn) || IS_INSN_LDST_SIMD_MULT_POST(insn))
    {
        ...
        {
            insn_in_progress->appendOperand(
                makeRegisterExpression(makeAarch64RegID(reg, (encoding + it_rpt) % 32)),
                false, true);
        }
    }
    else if(IS_INSN_LDST_SIMD_SING(insn) || IS_INSN_LDST_SIMD_SING_POST(insn))
    {
        ...
        {
            insn_in_progress->appendOperand(
                makeRegisterExpression(makeAarch64RegID(reg, (encoding + it_selem) % 32)),
                false, true);
        }
    }
    else
...
void
InstructionDecoder_aarch64::OPRRtS()
{
    int encoding = field<0, 4>(insn);
...
Expression::Ptr
InstructionDecoder_aarch64::makeRt2Expr()
{
    ...
    {
        baseReg = is64Bit ? ((encoding == 31) ? aarch64::xzr : aarch64::x0)
                          : ((encoding == 31) ? aarch64::wzr : aarch64::w0);
        if(encoding != 31)
...
void
InstructionDecoder_aarch64::OPRRt2()
{
    assert(0);
}

void
InstructionDecoder_aarch64::OPRRt2L()
{
    insn_in_progress->appendOperand(makeRt2Expr(), false, true);
}

void
InstructionDecoder_aarch64::OPRRt2S()
{
...
template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRcond()
{
    ...
    else
    {
        ...
        Expression::Ptr cond =
            ArmConditionImmediate::makeArmConditionImmediate(Result(u8, condVal));
...
void
InstructionDecoder_aarch64::OPRnzcv()
{
...
void
InstructionDecoder_aarch64::OPRop1()
{
    op1Field = field<16, 18>(insn);
}

void
InstructionDecoder_aarch64::OPRop2()
{
    op2Field = field<5, 7>(insn);
}

void
InstructionDecoder_aarch64::OPRCRm()
{
    crmField = field<8, 11>(insn);
}

void
InstructionDecoder_aarch64::OPRCRn()
{}

template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRS()
{
    sField = field<startBit, endBit>(insn);
}

void
InstructionDecoder_aarch64::OPRopc()
{
...
void
InstructionDecoder_aarch64::OPRscale()
{
    ...
    {
        Expression::Ptr scale = Immediate::makeImmediate(
            Result(u32, unsign_extend32(6 + is64Bit, 64 - scaleVal)));
        insn_in_progress->appendOperand(scale, true, false);
    }
}

Expression::Ptr
InstructionDecoder_aarch64::makeRaExpr()
{
    int          encoding = field<10, 14>(insn);
    MachRegister reg;
...
void
InstructionDecoder_aarch64::OPRRa()
{
...
void
InstructionDecoder_aarch64::OPRo0()
{}

void
InstructionDecoder_aarch64::OPRb5()
{
    OPRsf();
    hasb5 = true;
}
...
void
InstructionDecoder_aarch64::OPRb40()
{}

Expression::Ptr
InstructionDecoder_aarch64::makeb40Expr()
{
...
template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRsz()
{
...
bool
InstructionDecoder_aarch64::isSinglePrec()
{
    if(isFPInsn && !isSIMDInsn)
    {
        ...
        // return false even if type has an invalid value, it won't matter because isValid
        // is set to false and thus the instruction will be marked invalid
...
Expression::Ptr
InstructionDecoder_aarch64::makeRsExpr()
{
    MachRegister baseReg = isFPInsn ? (isSinglePrec() ? aarch64::s0 : aarch64::d0)
...
void
InstructionDecoder_aarch64::OPRRs()
{
    insn_in_progress->appendOperand(makeRsExpr(), false, true);
}

void
InstructionDecoder_aarch64::makeBranchTarget(bool branchIsCall, bool bIsConditional,
                                             int immVal, int immLen)
{
...
Expression::Ptr
InstructionDecoder_aarch64::makeFallThroughExpr()
{
    return makeAddExpression(
        makePCExpr(), Immediate::makeImmediate(Result(u64, unsign_extend64(3, 4))), u64);
}

template <typename T, Result_Type rT>
Expression::Ptr
InstructionDecoder_aarch64::fpExpand(int val)
{
    int N, E, F;
...
template <typename T>
Expression::Ptr
InstructionDecoder_aarch64::makeLogicalImm(int immr_, int imms, int immsLen,
                                           Result_Type rT)
{
...
bool
InstructionDecoder_aarch64::fix_bitfieldinsn_alias(int immr_, int imms)
{
...
void
InstructionDecoder_aarch64::fix_condinsn_alias_and_cond(int& cond)
{
    entryID modifiedID = insn_in_progress->getOperation().operationID;
...
void
InstructionDecoder_aarch64::modify_mnemonic_simd_upperhalf_insns()
{
    ...
    else if(IS_INSN_SIMD_VEC_INDEX(insn) || IS_INSN_SCALAR_INDEX(insn))
    {
        ...
        if((checkval >> 1) == 0x1 ||
           (IS_INSN_SCALAR_INDEX(insn) && (checkval & 0x1) == 0x1))
            add2 = true;
    }
...
template <unsigned int endBit, unsigned int startBit>
void
InstructionDecoder_aarch64::OPRimm()
{
    ...
    else if(hasN)  // logical (immediate), bitfield, extract
    {
        ...
        else if(IS_FIELD_IMMS(startBit, endBit))
        {
            ...
            else
            {
                ...
                if(IS_INSN_BITFIELD(insn))
                {
                    if((is64Bit && nField != 0x1) ||
                       (!is64Bit &&
                        (nField != 0 || (immr & 0x20) != 0 || (immVal & 0x20) != 0)))
                    {
                        isValid = false;
                        return;
                    }
            ...
            {
                imm =
                    Immediate::makeImmediate(Result(u32, unsign_extend32(immrLen, immr)));
                insn_in_progress->appendOperand(imm, true, false);
                if(!isLsrLsl)
                    oprRotateAmt--;
            }
    ...
    {
        if(IS_INSN_ADDSUB_SHIFT(insn) ||
           IS_INSN_LOGICAL_SHIFT(insn))  // add-sub shifted | logical shifted
        {
            if(IS_INSN_LOGICAL_SHIFT(insn) && shiftField == 0 &&
               field<5, 9>(insn) == 0x1F && immVal == 0)
            {
            ...
            else
            {
                ...
                if((IS_INSN_ADDSUB_SHIFT(insn) && shiftField == 0x3) ||
                   (!is64Bit && ((immVal >> 5) & 0x1) == 0x1))
                    isValid = false;
            }
        }
    ...
    {  // unconditional branch (immediate), test and branch, compare and branch,
       // conditional branch
        ...
        if(bIsConditional)
            insn_in_progress->addSuccessor(makeFallThroughExpr(), false, false, true,
                                           true);
    }
    else if(IS_INSN_PCREL_ADDR(insn))  // pc-relative addressing
    {
        ...
        else if(IS_FIELD_IMMHI(startBit, endBit))
        {
            ...
            Expression::Ptr imm = Immediate::makeImmediate(
                Result(s64, (offset << (64 - size_)) >> (64 - size_)));

            insn_in_progress->appendOperand(makeAddExpression(makePCExpr(), imm, u64),
                                            true, false);
        }
        else
            isValid = false;
    ...
    else if(isSIMDInsn)
    {
        ...
        else if(IS_INSN_SIMD_SHIFT_IMM(insn) || IS_INSN_SCALAR_SHIFT_IMM(insn))
        {
            ...
            else if(startBit == 16 && endBit == 18)
            {
                ...
                // check if shift is left; if it is, the immediate has to be processed in
                // a different manner. unfortunately, determining whether the instruction
                // will do a left or right shift cannot be determined in any way other
                // than checking the instruction's opcode
                if(insnID == aarch64_op_shl_advsimd ||
                   insnID == aarch64_op_sqshl_advsimd_imm ||
                   insnID == aarch64_op_sshll_advsimd ||
                   insnID == aarch64_op_sli_advsimd ||
                   insnID == aarch64_op_sqshlu_advsimd ||
                   insnID == aarch64_op_uqshl_advsimd_imm ||
                   insnID == aarch64_op_ushll_advsimd)
                    isRightShift = -1;
                ...
                switch(highest_set_bit(immlo))
                {
                    case 0x1:
                        (!isScalar || (opcode & 0x1C) == 0x0C || (opcode & 0x1C) == 0x10)
                            ? (shift = isRightShift * (16 - elemWidth) +
                                       (isRightShift > 0 ? 0 : 8))
                            : (isValid = false);
                        break;
                    case 0x2:
                        (!isScalar || (opcode & 0x1C) == 0x0C || (opcode & 0x1C) == 0x10)
                            ? (shift = isRightShift * (32 - elemWidth) +
                                       (isRightShift > 0 ? 0 : 16))
                            : (isValid = false);
                        break;
                    case 0x3:
                        (!isScalar || opcode > 0x0A)
                            ? (shift = isRightShift * (64 - elemWidth) +
                                       (isRightShift > 0 ? 0 : 32))
                            : (isValid = false);
                        break;
                    case 0x4:
                        shift = isRightShift * (128 - elemWidth) +
                                (isRightShift > 0 ? 0 : 64);
                        break;
                    default:
                        isValid = false;
    ...
    else  // conditional compare (immediate)
    {
        ...
        Expression::Ptr imm = Immediate::makeImmediate(
            Result(rT, rT == u32 ? unsign_extend32(immLen, immVal)
                                 : unsign_extend64(immLen, immVal)));
        insn_in_progress->appendOperand(imm, true, false);
    }
}

void
InstructionDecoder_aarch64::reorderOperands()
{
    if(oprRotateAmt)
    {
        ...
        while(oprRotateAmt--)
            std::rotate(curOperands.begin(), curOperands.begin() + 1,
                        curOperands.begin() + 3);
...
void
InstructionDecoder_aarch64::processAlphabetImm()
{
    if(op == 1 && cmode == 0xE)
    {
        uint64_t imm = 0;
        ...
        insn_in_progress->appendOperand(Immediate::makeImmediate(Result(u64, imm)), true,
                                        false);
    }
    else if(cmode == 0xF)
    {
        ...
        insn_in_progress->appendOperand(
            Immediate::makeImmediate(Result(u8, simdAlphabetImm)), true, false);
    }
    else
    {
        int shiftAmt = 0;
        ...
        Expression::Ptr lhs =
            Immediate::makeImmediate(Result(u32, unsign_extend32(8, simdAlphabetImm)));
        Expression::Ptr rhs =
            Immediate::makeImmediate(Result(u32, unsign_extend32(5, shiftAmt)));
...
void
InstructionDecoder_aarch64::doDelayedDecode(const Instruction* insn_to_complete)
{
...
bool
InstructionDecoder_aarch64::pre_process_checks(const aarch64_insn_entry& entry)
{
    bool          ret      = false;
    entryID       insnID   = entry.op;
    const string& mnemonic = entry.mnemonic;

    vector<entryID> simdCompareRegInsns  = { aarch64_op_cmeq_advsimd_reg,
                    ...
                    simdCompareZeroInsns = { aarch64_op_cmeq_advsimd_zero,
                                             aarch64_op_cmge_advsimd_zero,
                                             aarch64_op_cmgt_advsimd_zero,
                                             aarch64_op_cmle_advsimd,
                                             aarch64_op_cmlt_advsimd };
    vector<entryID> fpCompareRegInsns    = { aarch64_op_fcmeq_advsimd_reg,
                                          aarch64_op_fcmge_advsimd_reg,
                                          aarch64_op_fcmgt_advsimd_reg },
                    fpCompareZeroInsns   = { aarch64_op_fcmeq_advsimd_zero,
    ...
    else if((find(simdCompareRegInsns.begin(), simdCompareRegInsns.end(), insnID) !=
                 simdCompareRegInsns.end() ||
             (find(fpCompareRegInsns.begin(), fpCompareRegInsns.end(), insnID) !=
              fpCompareRegInsns.end())) &&
            !(IS_INSN_SCALAR_3SAME(insn) || IS_INSN_SIMD_3SAME(insn)))
    {
        ret = true;
    }
    else if((find(simdCompareZeroInsns.begin(), simdCompareZeroInsns.end(), insnID) !=
                 simdCompareZeroInsns.end() ||
             find(fpCompareZeroInsns.begin(), fpCompareZeroInsns.end(), insnID) !=
                 fpCompareZeroInsns.end() ||
             insnID == aarch64_op_rev64_advsimd) &&
            !(IS_INSN_SIMD_2REG_MISC(insn) || IS_INSN_SCALAR_2REG_MISC(insn)))
    {
        ret = true;
    }
    else if((mnemonic.find("sha1") != string::npos ||
             mnemonic.find("sha2") != string::npos) &&
...
bool
InstructionDecoder_aarch64::decodeOperands(const Instruction* insn_to_complete)
{
    int insn_table_index = findInsnTableIndex(0);
    isValid = !pre_process_checks(aarch64_insn_entry::main_insn_table[insn_table_index]);
    const auto& insn_table_entry =
        isValid ? aarch64_insn_entry::main_insn_table[insn_table_index]
    ...
    else
    {
        ...
        entryID         insnID      = insn_in_progress->getOperation().operationID;
        vector<entryID> zeroInsnIDs = {
            aarch64_op_cmeq_advsimd_zero,  aarch64_op_cmge_advsimd_zero,
        ...
        if(find(zeroInsnIDs.begin(), zeroInsnIDs.end(), insnID) != zeroInsnIDs.end())
            insn_in_progress->appendOperand(Immediate::makeImmediate(Result(u32, 0)),
                                            true, false);
        ...
        if(isPstateWritten || isPstateRead)
            insn_in_progress->appendOperand(makePstateExpr(), isPstateRead,
                                            isPstateWritten, true);
    }
...
int
InstructionDecoder_aarch64::findInsnTableIndex(unsigned int decoder_table_index)
{
    ...
    while(insn_iter_index < AARCH64_INSN_LENGTH)
    {
        ...
        {
            branch_map_key =
                branch_map_key | (((insn >> insn_iter_index) & 1) << map_key_index);
            map_key_index++;
        }
        insn_iter_index++;
    }
...
void
InstructionDecoder_aarch64::setFlags()
{
    isPstateWritten = true;
}

void
InstructionDecoder_aarch64::mainDecode()
{
    int         insn_table_index = findInsnTableIndex(0);
