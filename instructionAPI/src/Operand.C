...
INSTRUCTION_EXPORT void Operand::getReadSet(
  ...
  for (curUse = useSet.begin(); curUse != useSet.end(); ++curUse) {
    ...
    if (tmp) {
      if (m_isRead || !(*tmp == *op_value)) regsRead.insert(tmp);
    }
  }
}
INSTRUCTION_EXPORT void Operand::getWriteSet(
    std::set<RegisterAST::Ptr>& regsWritten) const {
  RegisterAST::Ptr op_as_reg =
      boost::dynamic_pointer_cast<RegisterAST>(op_value);
...
INSTRUCTION_EXPORT bool Operand::isRead(Expression::Ptr candidate) const {
  ...
  return op_value->isUsed(candidate) &&
         (m_isRead || !(*candidate == *op_value));
}
...
INSTRUCTION_EXPORT void Operand::addEffectiveReadAddresses(
...
INSTRUCTION_EXPORT void Operand::addEffectiveWriteAddresses(
    std::set<Expression::Ptr>& memAccessors) const {
...
INSTRUCTION_EXPORT std::string Operand::format(Architecture arch,
                                               Address addr) const {
