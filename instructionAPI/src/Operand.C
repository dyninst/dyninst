...
namespace Dyninst
{
    namespace InstructionAPI
    {
        INSTRUCTION_EXPORT void Operand::getReadSet(std::set<RegisterAST::Ptr>& regsRead) const
        {
            ...
            {
                RegisterAST::Ptr tmp = dyncompat::dynamic_pointer_cast<RegisterAST>(*curUse);
                if(tmp) 
                {
        ...
        {
            if(m_isRead && dyncompat::dynamic_pointer_cast<Dereference>(op_value))
            {
        ...
        {
            if(m_isWritten && dyncompat::dynamic_pointer_cast<Dereference>(op_value))
            {
                std::vector<Expression::Ptr> tmp;
                op_value->getChildren(tmp);
                for(std::vector<Expression::Ptr>::const_iterator curKid = tmp.begin();
