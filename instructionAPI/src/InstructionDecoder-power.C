...
void
InstructionDecoder_power::decodeOpcode(InstructionDecoder::buffer& b)
{
    b.start += 4;
}

void
InstructionDecoder_power::FRTP()
{
    FRT();
    FRTS();
}
void
InstructionDecoder_power::FRTS()
{
    isFPInsn           = true;
...
void
InstructionDecoder_power::QFRTP()
{
    QRT();
    QFRTS();
}
void
InstructionDecoder_power::QFRTS()
{
    isFPInsn           = true;
...
void
InstructionDecoder_power::FRSP()
{
    FRS();
    FRSS();
}
void
InstructionDecoder_power::FRSS()
{
    isFPInsn           = true;
    MachRegister regID = makePowerRegID(ppc32::fsr0, field<6, 10>(insn));
...
void
InstructionDecoder_power::QFRSP()
{
    QFRS();
    QFRSS();
}
void
InstructionDecoder_power::QFRSS()
{
    isFPInsn           = true;
    MachRegister regID = makePowerRegID(ppc64::fsr0, field<6, 10>(insn));
    insn_in_progress->appendOperand(makeRegisterExpression(regID), true, false);
...
void
InstructionDecoder_power::FRAP()
{
    FRA();
    FRAS();
}
void
InstructionDecoder_power::FRAS()
{
    isFPInsn           = true;
...
void
InstructionDecoder_power::QFRAP()
{
    QFRA();
    QFRAS();
}
void
InstructionDecoder_power::QFRAS()
{
    isFPInsn           = true;
...
void
InstructionDecoder_power::FRBP()
{
    FRB();
    FRBS();
}
void
InstructionDecoder_power::QRB()
{
    insn_in_progress->appendOperand(makeQRBExpr(), true, false);
    QRBS();
}
void
InstructionDecoder_power::QRBS()
{
    isFPInsn           = true;
...
void
InstructionDecoder_power::FRCP()
{
    FRC();
    FRCS();
}
void
InstructionDecoder_power::FRCS()
{
    isFPInsn           = true;
...
void
InstructionDecoder_power::QFRCP()
{
    QFRC();
    QFRCS();
}
void
InstructionDecoder_power::QFRCS()
{
    isFPInsn           = true;
...
Expression::Ptr
InstructionDecoder_power::makeFallThroughExpr()
{
    return makeAddExpression(makeRegisterExpression(ppc32::pc),
                             Immediate::makeImmediate(Result(u32, 4)), u32);
}
void
InstructionDecoder_power::LI()
{
    insn_in_progress->addSuccessor(makeIFormBranchTarget(), field<31, 31>(insn) == 1,
                                   false, false, false);
}
void
InstructionDecoder_power::BD()
{
    insn_in_progress->addSuccessor(makeBFormBranchTarget(), field<31, 31>(insn) == 1,
                                   false, bcIsConditional, false);
    if(bcIsConditional)
    {
...
using namespace std;
Instruction
InstructionDecoder_power::decode(InstructionDecoder::buffer& b)
{
    if(b.start > b.end)
        return Instruction();
    isRAWritten     = false;
...
bool
InstructionDecoder_power::decodeOperands(const Instruction* insn_to_complete)
{
  ...
  I. Decision Tree:
    ...
    (2) Two special XX3 cases: 0..00 010.. & 0..01 010.., started from 21th bit. (manual
  1208) (3) Other instruction are all implemented normally, the "." bits are treated as 0
  and 1 respectively in the opcode table II. The Rc bit for opcode 60 is the 21st bit

   **	There are 2 XX1 instructions, the last bit of the extended opcodes are ignored
  (30th bit, which are both 0).
    */
...
InstructionDecoder_power::XT()
{
    ...
    insn_in_progress->appendOperand(
        makeRegisterExpression(makePowerRegID(ppc64::vsr0, id)), false, true);
}
void
InstructionDecoder_power::XS()
{
    // TODO: Format DQ has a different encoding.
    //       The single T bit is at bit 28, instaed of bit 31.
    unsigned id = field<6, 10>(insn) + 32 * field<31, 31>(insn);
    insn_in_progress->appendOperand(
        makeRegisterExpression(makePowerRegID(ppc64::vsr0, id)), true, false);
}
void
InstructionDecoder_power::XA()
{
...
void
InstructionDecoder_power::BHRBE()
{
...
Result_Type
InstructionDecoder_power::makeSizeType(unsigned int)
{
    assert(!"not implemented");
    return u32;
}
Expression::Ptr
InstructionDecoder_power::makeSHExpr()
{
...
void
InstructionDecoder_power::setFPMode()
{
    isFPInsn = true;
}
void
InstructionDecoder_power::doDelayedDecode(const Instruction* insn_to_complete)
{
...
MachRegister
InstructionDecoder_power::makePowerRegID(MachRegister base, unsigned int encoding,
                                         int field_)
{
    if(field_ != -1)
    {
...
const power_entry&
InstructionDecoder_power::extended_op_0()
{
...
const power_entry&
InstructionDecoder_power::extended_op_4()
{
    // Extended OpCode 4:
...
const power_entry&
InstructionDecoder_power::extended_op_4_1409()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_4_1538()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_4_1921()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_19()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_30()
{
    power_table::const_iterator entry_it;
...
const power_entry&
InstructionDecoder_power::extended_op_57()
{
    return power_entry::extended_op_57[field<30, 31>(insn)];
}
const power_entry&
InstructionDecoder_power::extended_op_58()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_59()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_60_specials_check()
{
    // If the power decoder is ever redone. Use masking to determine the instructions
    // for 60. Otherwise we are forced to do this fun hack....
...
const power_entry&
InstructionDecoder_power::extended_op_60()
{
...
const power_entry&
InstructionDecoder_power::extended_op_60_347()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_60_475()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_61()
{
    unsigned int xo = field<26, 30>(insn);
    if(xo <= 31)
    {
    ...
    power_table::const_iterator found =
        power_entry::extended_op_61.find(field<21, 30>(insn));
    if(found != power_entry::extended_op_61.end())
        return found->second;
    return invalid_entry;
}
...
const power_entry&
InstructionDecoder_power::extended_op_63()
{
    ...
    {
        power_table::const_iterator found =
            power_entry::extended_op_63.find(field<26, 30>(insn));
...
const power_entry&
InstructionDecoder_power::extended_op_63_583()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_63_804()
{
    const power_table::const_iterator entry_it =
...
const power_entry&
InstructionDecoder_power::extended_op_63_836()
{
    const power_table::const_iterator entry_it =
...
void
InstructionDecoder_power::syscall()
{
...
void
InstructionDecoder_power::LL()
{
    LI();
    return;
}
void
InstructionDecoder_power::SH()
{
...
InstructionDecoder_power::Rc()
{
    if(field<31, 31>(insn))
    {
        if(isFPInsn)
        {
            insn_in_progress->appendOperand(makeRegisterExpression(ppc32::fpscw), false,
                                            true, true);
        }
        else
        {
...
void
InstructionDecoder_power::FXM()
{
    (translateBitFieldToCR<12, 19, ppc32::icr0, 7>(*this))();
    return;
}
void
InstructionDecoder_power::spr()
{
    insn_in_progress->appendOperand(makeSPRExpr(), !isRAWritten, isRAWritten);
    return;
}
void
InstructionDecoder_power::SR()
{
    insn_in_progress->appendOperand(
        makeRegisterExpression(makePowerRegID(ppc32::seg0, field<11, 15>(insn) >> 2)),
        true, true);
    return;
}
void
InstructionDecoder_power::NB()
{
    insn_in_progress->appendOperand(
...
void
InstructionDecoder_power::U()
{
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(Result(u8, field<16, 20>(insn) >> 1)), true, false);
    return;
}
void
InstructionDecoder_power::FLM()
{
    isRAWritten = true;
...
void
InstructionDecoder_power::WC()
{
    insn_in_progress->appendOperand(
        Immediate::makeImmediate(Result(u8, field<9, 10>(insn))), true, false);
    return;
}

bool
InstructionDecoder_power::findRAAndRS(const power_entry* cur)
{
    bool findRA = false;
    bool findRS = false;
...
void
InstructionDecoder_power::mainDecode()
{
    const power_entry* current = &power_entry::main_opcode_table[field<0, 5>(insn)];
    while(current->next_table)
    {
