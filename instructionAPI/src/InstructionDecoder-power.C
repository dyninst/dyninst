...
which are both 0).
    */
    ...
    if(current->op == power_op_bclr) {
      ...
      // The counter register is _always_ implicitly read
      {
        auto ctr = makeRegisterExpression(ppc32::ctr);
        constexpr bool is_read = true;
        constexpr bool is_implicit = true;
        bool const BO = field<6,10>(insn);
        bool const is_written = ((0x2 & BO) == 0);
        insn_in_progress->appendOperand(std::move(ctr), is_read, is_written, is_implicit);
      }
...
#include "power_opcode_tables.C"
  ...
  void InstructionDecoder_power::SR() {
    insn_in_progress->appendOperand(
        makeRegisterExpression(makePowerRegID(ppc32::seg0, field<11, 15>(insn) >> 2)), true, true);
    return;
  }
