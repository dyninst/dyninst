...
#if defined(__GNUC__)
#define insn_printf(format, ...) do{\
    printf("[%s:%u]insn_debug " format, FILE__, __LINE__, ## __VA_ARGS__); \
    }while(0)
#endif

#define AARCH32_INSN_LENGTH 32

class InstructionDecoder_aarch32 : public InstructionDecoderImpl {
  public:
    InstructionDecoder_aarch32(Architecture a);
    virtual ~InstructionDecoder_aarch32();
    ...
    virtual bool decodeOperands(const Instruction* insnToComplete);
    virtual void doDelayedDecode(const Instruction* insnToComplete);
...
#include "aarch32_decoder_autogen.h"
};
