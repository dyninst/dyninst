...
#define IS_INSN_ADDSUB_EXT(I) \
  (field<24, 28>(I) == 0x0B && field<21, 21>(I) == 1)
#define IS_INSN_ADDSUB_SHIFT(I) \
  (field<24, 28>(I) == 0x0B && field<21, 21>(I) == 0)
...
#define IS_INSN_LDST_EX_PAIR(I) \
  (field<24, 29>(I) == 0x08 && field<21, 21>(I) == 0x01)

#define IS_INSN_LD_LITERAL(I) \
  (field<27, 29>(I) == 0x03 && field<24, 25>(I) == 0)

#define IS_INSN_LDST_PAIR(I) (field<27, 29>(I) == 0x05 && field<25, 25>(I) == 0)
#define IS_INSN_LDST_PAIR_PRE(I) \
  (field<27, 29>(I) == 0x05 && field<23, 25>(I) == 0x03)
#define IS_INSN_LDST_PAIR_OFFSET(I) \
  (field<27, 29>(I) == 0x05 && field<23, 25>(I) == 0x02)
#define IS_INSN_LDST_PAIR_POST(I) \
  (field<27, 29>(I) == 0x05 && field<23, 25>(I) == 0x01)
#define IS_INSN_LDST_PAIR_NOALLOC(I) \
  (field<27, 29>(I) == 0x05 && field<23, 25>(I) == 0x00)

#define IS_INSN_LDST_UNSCALED(I)                        \
  (field<27, 29>(I) == 0x07 && field<24, 25>(I) == 0 && \
   field<21, 21>(I) == 0 && field<10, 11>(I) == 0x00)
#define IS_INSN_LDST_POST(I)                            \
  (field<27, 29>(I) == 0x07 && field<24, 25>(I) == 0 && \
   field<21, 21>(I) == 0 && field<10, 11>(I) == 0x01)
#define IS_INSN_LDST_UNPRIV(I)                          \
  (field<27, 29>(I) == 0x07 && field<24, 25>(I) == 0 && \
   field<21, 21>(I) == 0 && field<10, 11>(I) == 0x02)
#define IS_INSN_LDST_PRE(I)                             \
  (field<27, 29>(I) == 0x07 && field<24, 25>(I) == 0 && \
   field<21, 21>(I) == 0 && field<10, 11>(I) == 0x03)
#define IS_INSN_LDST_REG(I)                             \
  (field<27, 29>(I) == 0x07 && field<24, 25>(I) == 0 && \
   field<21, 21>(I) == 1 && field<10, 11>(I) == 0x02)
#define IS_INSN_LDST_UIMM(I) (field<27, 29>(I) == 0x07 && field<24, 25>(I) == 1)

#define IS_INSN_LDST_SIMD_MULT(I)                          \
  (field<31, 31>(I) == 0x00 && field<23, 29>(I) == 0x18 && \
   field<16, 21>(I) == 0)
#define IS_INSN_LDST_SIMD_MULT_POST(I)                     \
  (field<31, 31>(I) == 0x00 && field<23, 29>(I) == 0x19 && \
   field<21, 21>(I) == 0)
#define IS_INSN_LDST_SIMD_SING(I)                          \
  (field<31, 31>(I) == 0x00 && field<23, 29>(I) == 0x1a && \
   field<16, 20>(I) == 0)
#define IS_INSN_LDST_SIMD_SING_POST(I) \
  (field<31, 31>(I) == 0x00 && field<23, 29>(I) == 0x1b)
...
#define IS_INSN_FP_COMPARE(I)                             \
  (field<24, 28>(I) == 0x1E && field<21, 21>(I) == 0x1 && \
   field<10, 13>(I) == 0x8)
#define IS_INSN_FP_CONV_FIX(I) \
  (field<24, 28>(I) == 0x1E && field<21, 21>(I) == 0x0)
#define IS_INSN_FP_CONV_INT(I)                            \
  (field<24, 28>(I) == 0x1E && field<21, 21>(I) == 0x1 && \
   field<10, 15>(I) == 0x0)
#define IS_SOURCE_GP(I)                                  \
  (field<16, 18>(I) == 0x2 || field<16, 18>(I) == 0x3 || \
   field<16, 18>(I) == 0x7)
...
#define IS_INSN_BRANCHING(I)                                              \
  (IS_INSN_B_COND(I) || IS_INSN_B_UNCOND(I) || IS_INSN_B_UNCOND_REG(I) || \
   IS_INSN_B_TEST(I) || IS_INSN_B_COMPARE(I))

#define IS_INSN_SIMD_3DIFF(I)                            \
  (field<31, 31>(I) == 0x0 && field<24, 28>(I) == 0xe && \
   field<21, 21>(I) == 0x1 && field<10, 11>(I) == 0x0)
#define IS_INSN_SIMD_ACROSS(I)                           \
  (field<31, 31>(I) == 0x0 && field<24, 28>(I) == 0xe && \
   field<17, 21>(I) == 0x18 && field<10, 11>(I) == 0x2)
#define IS_INSN_SIMD_COPY(I)                              \
  (field<31, 31>(I) == 0x0 && field<21, 28>(I) == 0x70 && \
   field<15, 15>(I) == 0x0 && field<10, 10>(I) == 0x1)
#define IS_INSN_SIMD_VEC_INDEX(I)                        \
  (field<31, 31>(I) == 0x0 && field<24, 28>(I) == 0xf && \
   field<10, 10>(I) == 0x0)
#define IS_INSN_SIMD_MOD_IMM(I)                            \
  (field<31, 31>(I) == 0x0 && field<19, 28>(I) == 0x1e0 && \
   field<10, 10>(I) == 0x1)
#define IS_INSN_SIMD_SHIFT_IMM(I)                         \
  (field<31, 31>(I) == 0x0 && field<23, 28>(I) == 0x1e && \
   field<19, 22>(I) != 0x0)
#define IS_INSN_SIMD_TAB_LOOKUP(I)                       \
  (field<31, 31>(I) == 0x0 && field<24, 29>(I) == 0xe && \
   field<21, 21>(I) == 0x0 && field<15, 15>(I) == 0x0 && \
   field<10, 11>(I) == 0x0)
#define IS_INSN_SIMD_EXTR(I)                              \
  (field<31, 31>(I) == 0x0 && field<24, 29>(I) == 0x2e && \
   field<21, 21>(I) == 0x0 && field<15, 15>(I) == 0x0 &&  \
   field<10, 10>(I) == 0x0)
#define IS_INSN_SCALAR_3SAME(I)                           \
  (field<30, 31>(I) == 0x1 && field<24, 28>(I) == 0x1e && \
   field<21, 21>(I) == 0x1 && field<10, 10>(I) == 0x1)
#define IS_INSN_SCALAR_3DIFF(I)                           \
  (field<30, 31>(I) == 0x1 && field<24, 28>(I) == 0x1e && \
   field<21, 21>(I) == 0x1 && field<10, 11>(I) == 0x0)
#define IS_INSN_SCALAR_2REG_MISC(I)                       \
  (field<30, 31>(I) == 0x1 && field<24, 28>(I) == 0x1e && \
   field<17, 21>(I) == 0x10 && field<10, 11>(I) == 0x2)
#define IS_INSN_SCALAR_PAIR(I)                            \
  (field<30, 31>(I) == 0x1 && field<24, 28>(I) == 0x1e && \
   field<17, 21>(I) == 0x18 && field<10, 11>(I) == 0x2)
#define IS_INSN_SCALAR_COPY(I)                            \
  (field<30, 31>(I) == 0x1 && field<21, 28>(I) == 0xf0 && \
   field<15, 15>(I) == 0x0 && field<10, 10>(I) == 0x1)
#define IS_INSN_SCALAR_INDEX(I)                           \
  (field<30, 31>(I) == 0x1 && field<24, 28>(I) == 0x1f && \
   field<10, 10>(I) == 0x0)
#define IS_INSN_SCALAR_SHIFT_IMM(I)                       \
  (field<30, 31>(I) == 0x1 && field<23, 28>(I) == 0x3e && \
   field<10, 10>(I) == 0x1)
...
#define IS_FIELD_IMMHI(S, E) (S == 5 && E == 23)
 ...
 private:
  ...
  /*members for handling operand re-ordering, will be removed later once a
   * generic operand ordering method is incorporated*/
  int oprRotateAmt;
  bool hasb5;
  ...
  int highest_set_bit(int32_t val) {
    for (int bit_index = 31; bit_index >= 0; bit_index--)
      if (((val >> bit_index) & 0x1) == 0x1) return bit_index + 1;
  ...
  template <typename T>
  Expression::Ptr makeLogicalImm(int immr, int imms, int immsLen,
                                 Result_Type rT);
