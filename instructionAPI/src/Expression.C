...
void
Expression::setValue(const Result& knownValue)
{
...
bool
Expression::bind(Expression* expr, const Result& value)
{
    // bool retVal = false;
    if(*expr == *this)
    {
...
bool
Expression::isFlag() const
{
    return false;
}
bool
DummyExpr::isStrictEqual(const InstructionAST&) const
{
    return true;
}
bool
DummyExpr::checkRegID(MachRegister, unsigned int, unsigned int) const
{
    return true;
}
