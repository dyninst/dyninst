...
#include "amdgpu_cdna2_opcode_tables.C"
		...
		MachRegister InstructionDecoder_amdgpu_cdna2::makeAmdgpuRegID(MachRegister base, unsigned int encoding , unsigned int len) {
			...
			}else if (base == amdgpu_cdna2::vgpr0){
				switch(len){
					case 2:
		...
		Expression::Ptr InstructionDecoder_amdgpu_cdna2::makePCExpr() {
			MachRegister baseReg = amdgpu_cdna2::pc;
		...
		Expression::Ptr InstructionDecoder_amdgpu_cdna2::decodeSGPRorM0(unsigned int offset){
			if( offset <= 104)
				return makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::sgpr0,offset));
			if (offset == 124)
		...
		void InstructionDecoder_amdgpu_cdna2::finalizeENC_FLATOperands(){
			...
			Expression::Ptr addr_ast = 
				makeTernaryExpression(
						makeRegisterExpression(amdgpu_cdna2::address_mode_32), // TODO: type needs to be fixed
						makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.ADDR)),
						makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.ADDR,2)),
						u64
						);
			switch(layout.SEG){
				case 1:
		...
		void InstructionDecoder_amdgpu_cdna2::finalizeENC_MUBUFOperands(){
			...
			MachRegister vsharp = makeAmdgpuRegID(amdgpu_cdna2::sgpr0,layout.SRSRC<<2,4);
			...
			if(layout.IDXEN){
				if(layout.OFFEN){
					index_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.VADDR));
					offset_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.VADDR+1));
				}else{
					index_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.VADDR));
				}
				...
				if(layout.OFFEN){
					offset_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.VADDR));
				}else{
					// do nothing
				}
			}
			...
			Expression::Ptr index_ast = 
				makeAddExpression(
						...
						makeTernaryExpression(
							add_tid_enable,
							makeRegisterExpression(amdgpu_cdna2::tid),
			...
			Expression::Ptr vdata_ast = makeRegisterExpression(
					makeAmdgpuRegID(amdgpu_cdna2::vgpr0,layout.VDATA,
			...
			if(layout.LDS){
				Expression::Ptr lds_offset_ast = makeRegisterExpression(amdgpu_cdna2::m0,0,15);
				Expression::Ptr mem_offset_ast = makeRegisterExpression(
						makeAmdgpuRegID(amdgpu_cdna2::sgpr0,layout.SOFFSET));
				Expression::Ptr inst_offset_ast = Immediate::makeImmediate(Result(u32,layout.OFFSET));
				Expression::Ptr lds_addr_ast =
					makeAddExpression(
							makeAddExpression(
								makeAddExpression(
									makeRegisterExpression(amdgpu_cdna2::lds_base),
									lds_offset_ast,
									u32
									),
								inst_offset_ast,
			...
			if(isBranch){
				if(isModifyPC){
					...
					new_pc_ast = makeRegisterExpression(
							makeAmdgpuRegID(amdgpu_cdna2::sgpr0,layout.SSRC0,2)
							);
					...
					if(insn_in_progress->getOperation().operationID == amdgpu_op_s_setpc_b64){
    					...
    					store_pc_ast = makeRegisterExpression(
							makeAmdgpuRegID(amdgpu_cdna2::sgpr0,layout.SDST,2)
							);
		...
		void InstructionDecoder_amdgpu_cdna2::finalizeENC_SMEMOperands(){
			...
			if(IS_LD_ST()){
				...
				//MachRegister mr = makeAmdgpuRegID(amdgpu_cdna2::sgpr0,4,2);
				//                cout << " shouldn't it be " << amdgpu_cdna2::sgpr_vec2_0 << " " << mr << endl; 
				Expression::Ptr sbase_expr = makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::sgpr0,layout.SBASE << 1,2));
				if(isScratch)
				...
				Expression::Ptr sdata_expr = makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna2::sgpr0,layout.SDATA,num_elements));
		...
		void InstructionDecoder_amdgpu_cdna2::finalizeENC_VINTRPOperands() {
		}
		...
		bool InstructionDecoder_amdgpu_cdna2::decodeOperands(const amdgpu_cdna2_insn_entry & insn_entry) {
		...
		void InstructionDecoder_amdgpu_cdna2::insnSize(unsigned int insn_size_) {
			this->insn_size = insn_size_;
		}


		Expression::Ptr InstructionDecoder_amdgpu_cdna2::decodeVDST(unsigned int index){
			index += 255;
            return decodeSSRC(index);
		} 


		Expression::Ptr InstructionDecoder_amdgpu_cdna2::decodeVSRC(unsigned int index){
			if (index > 255)
				index = 0;
			MachRegister mr = makeAmdgpuRegID(amdgpu_cdna2::vgpr0,index & 0xff);
			return makeRegisterExpression(mr);
		} 

		Expression::Ptr InstructionDecoder_amdgpu_cdna2::decodeSSRC(unsigned int index){
			if (index < 102){
				MachRegister mr = makeAmdgpuRegID(amdgpu_cdna2::sgpr0,index);
				return makeRegisterExpression(mr);
			}else if(index ==102){
				return makeRegisterExpression(amdgpu_cdna2::flat_scratch_lo);
			}else if(index ==103){
				return makeRegisterExpression(amdgpu_cdna2::flat_scratch_hi);
			}else if(index ==104){
				return makeRegisterExpression(amdgpu_cdna2::xnack_mask_lo);
			}else if(index ==105){
				return makeRegisterExpression(amdgpu_cdna2::xnack_mask_hi);
			}else if ( 106 == index ){
				return makeRegisterExpression(amdgpu_cdna2::vcc_lo);
			}else if ( 107 == index ){
				return makeRegisterExpression(amdgpu_cdna2::vcc_hi);
			}else if (107 < index && index < 124){
				MachRegister mr = makeAmdgpuRegID(amdgpu_cdna2::ttmp0,index-108);
				return makeRegisterExpression(mr);
			}else if (124 == index ){
				return makeRegisterExpression(amdgpu_cdna2::m0);
			}else if ( 126 == index ){
				return makeRegisterExpression(amdgpu_cdna2::exec_lo);
			}else if ( 127 == index ){
				return makeRegisterExpression(amdgpu_cdna2::exec_hi);
				...
				return makeRegisterExpression(amdgpu_cdna2::shared_base);
			}else if( 236 == index){
				return makeRegisterExpression(amdgpu_cdna2::shared_limit);
			}else if( 237 == index){
				return makeRegisterExpression(amdgpu_cdna2::private_base);
			}else if( 238 == index){
				return makeRegisterExpression(amdgpu_cdna2::private_limit);
			}else if( 239 == index){
				return makeRegisterExpression(amdgpu_cdna2::pops_exiting_wave_id);
			}else if (240 == index){
				...
				return makeRegisterExpression(amdgpu_cdna2::vccz);
			}else if ( 252 == index ){
				return makeRegisterExpression(amdgpu_cdna2::execz);
			}else if ( 253 == index ){
				return makeRegisterExpression(amdgpu_cdna2::scc);
			}else if ( 254 == index ){
				assert ( 0 && "reserved index " ) ;
			}
			else if(index == 0xff){
				...
				MachRegister mr = makeAmdgpuRegID(amdgpu_cdna2::vgpr0,index >>8);
				return makeRegisterExpression(mr);
			} 

			cerr << "WARNING UNKNOWN REGISTER : "  << std::dec << index << " " <<(108 <= index && index <= 123 )<<endl;
			//return makeRegisterExpression(amdgpu_cdna2::sgpr0);
