...
namespace Dyninst {
    namespace InstructionAPI {
        ...
        const char* InstructionDecoder_amdgpu_gfx940::bitfieldInsnAliasMap(entryID) {
            assert(!"no alias for entryID");
            return nullptr;
        }
        const char* InstructionDecoder_amdgpu_gfx940::condInsnAliasMap(entryID) {
            assert(!"no alias for entryID");
            return nullptr;
        }
...
#include "amdgpu_gfx940_opcode_tables.C"
        ...
        MachRegister InstructionDecoder_amdgpu_gfx940::makeAmdgpuRegID(MachRegister base, unsigned int encoding , unsigned int) {
            return MachRegister(base.val() + encoding);

        }

        Expression::Ptr InstructionDecoder_amdgpu_gfx940::makePCExpr() {
            MachRegister baseReg = amdgpu_gfx940::pc_all;
        ...
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::decodeSGPRorM0(unsigned int offset){
            ...
            if (offset == 124)
                return makeRegisterExpression(amdgpu_gfx940::m0);
        ...
        void InstructionDecoder_amdgpu_gfx940::processOPR_SMEM_OFFSET(layout_ENC_SMEM & layout){
            if (layout.IMM ==0 ){
        ...
        uint32_t InstructionDecoder_amdgpu_gfx940::decodeOPR_LITERAL(){
            useImm = true;
            immLen = 4;
            if(insn_size == 4)
        ...
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::decodeOPR_SIMM4(uint64_t input){
            return Immediate::makeImmediate(Result(s8, input));
        }
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::decodeOPR_SIMM8(uint64_t input){
            return Immediate::makeImmediate(Result(s8, input));
        }
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::decodeOPR_SIMM16(uint64_t input){
            return Immediate::makeImmediate(Result(s16, input));
        }
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::decodeOPR_SIMM32(uint64_t input){
            return Immediate::makeImmediate(Result(s32, input));
        }
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::decodeOPR_WAITCNT(uint64_t input){
            return Immediate::makeImmediate(Result(s16, input));
        }
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::makeRegisterExpression(MachRegister registerID, uint32_t num_elements){
            if(registerID == amdgpu_gfx940::src_literal){
        ...
        Expression::Ptr InstructionDecoder_amdgpu_gfx940::makeRegisterExpression(MachRegister registerID, uint32_t low, uint32_t high ){
            if(registerID == amdgpu_gfx940::src_literal){
                return Immediate::makeImmediate(Result(u32,decodeOPR_LITERAL()));
            }
...
#include "finalizeOperands.C"
        ...
        void InstructionDecoder_amdgpu_gfx940::reset(){
            immLen = 0;
            insn_size = 0;
            isBranch = false;
            isConditional = false;
        ...
        void InstructionDecoder_amdgpu_gfx940::setupInsnWord(InstructionDecoder::buffer &b) {
            reset();
            if (b.start > b.end)
                return;
            insn = get32bit(b,0);
