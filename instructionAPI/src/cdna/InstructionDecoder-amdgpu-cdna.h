...
#define insn_printf(format, ...) \
        ...
        class InstructionDecoder_amdgpu_cdna : public InstructionDecoderImpl {
            friend struct amdgpu_cdna_insn_entry;
            friend struct amdgpu_mask_entry;
            enum DecodeFamily {sopp};

            public:
            InstructionDecoder_amdgpu_cdna(Architecture a);

            virtual ~InstructionDecoder_amdgpu_cdna();
            ...
            bool decodeOperands(const amdgpu_cdna_insn_entry & insn_entry);
...
#define IS_LD_ST() (isLoad || isStore )
            ...
            void debug_instr();
            #include "amdgpu_cdna_decoder_impl.h"    
        };
    }
}
