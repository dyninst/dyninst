...
#include "Ternary.h"
#include "InstructionDecoder-amdgpu-cdna.h"

namespace Dyninst {
	namespace InstructionAPI {
		typedef void (InstructionDecoder_amdgpu_cdna::*operandFactory)();

		typedef amdgpu_cdna_insn_entry amdgpu_cdna_insn_table[];
		typedef amdgpu_mask_entry amdgpu_decoder_table[];

		const std::array<std::string, 16> InstructionDecoder_amdgpu_cdna::condNames = { {
			"eq", "ne", "cs", "cc", "mi", "pl", "vs", "vc", "hi", "ls", "ge",
			"lt", "gt", "le", "al", "nv",
		} };

		const char* InstructionDecoder_amdgpu_cdna::bitfieldInsnAliasMap(entryID e) {
			switch(e) {
				default: assert(!"no alias for entryID");
			};
		}
		const char* InstructionDecoder_amdgpu_cdna::condInsnAliasMap(entryID e) {
			switch(e) {
				default: assert(!"no alias for entryID");
			};
		}

#include "amdgpu_cdna_insn_entry.h"
...
#include "amdgpu_cdna_opcode_tables.C"

		InstructionDecoder_amdgpu_cdna::InstructionDecoder_amdgpu_cdna(Architecture a)
		...
		InstructionDecoder_amdgpu_cdna::~InstructionDecoder_amdgpu_cdna() {
		}


		using namespace std;
		void InstructionDecoder_amdgpu_cdna::NOTHING() {
		}

		Result_Type InstructionDecoder_amdgpu_cdna::makeSizeType(unsigned int) {
		...
		MachRegister InstructionDecoder_amdgpu_cdna::makeAmdgpuRegID(MachRegister base, unsigned int encoding , unsigned int len) {
			MachRegister realBase = base;
			if (base == amdgpu_cdna::sgpr0){
				switch(len){
					case 2:
						realBase = amdgpu_cdna::sgpr_vec2_0;
						break;
					case 4:
						realBase = amdgpu_cdna::sgpr_vec4_0;
						break;
					case 8:
						realBase = amdgpu_cdna::sgpr_vec8_0;
						break;
					case 16:
						realBase = amdgpu_cdna::sgpr_vec16_0;
						break;
				}
			}else if (base == amdgpu_cdna::vgpr0){
				switch(len){
					case 2:
						realBase = amdgpu_cdna::vgpr_vec2_0;
						break;
					case 4:
						realBase = amdgpu_cdna::vgpr_vec4_0;
						break;
					case 8:
						realBase = amdgpu_cdna::vgpr_vec8_0;
						break;
					case 16:
						realBase = amdgpu_cdna::vgpr_vec16_0;
						break;
				}
		...
		Expression::Ptr InstructionDecoder_amdgpu_cdna::makePCExpr() {
			MachRegister baseReg = amdgpu_cdna::pc;

			return makeRegisterExpression(baseReg);
		}

		void InstructionDecoder_amdgpu_cdna::makeBranchTarget(bool branchIsCall, bool bIsConditional, int immVal,
				int immLen_ = 16) {
		...
		Expression::Ptr InstructionDecoder_amdgpu_cdna::makeFallThroughExpr() {
		...
		bool InstructionDecoder_amdgpu_cdna::decodeOperands(const Instruction *) {
			assert(0 && "decodeOperands deprecated for amdgpu");
			return true;
		}

		Expression::Ptr InstructionDecoder_amdgpu_cdna::decodeSGPRorM0(unsigned int offset){
			if( offset <= 104)
				return makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::sgpr0,offset));
			if (offset == 124)
				return makeRegisterExpression(amdgpu_cdna::m0);
            cerr << " unknown offset in sgpr or m0 " << offset << endl; 
			assert(0 && "shouldn't reach here");
		}


		void InstructionDecoder_amdgpu_cdna::finalizeENC_FLATOperands(){
			layout_ENC_FLAT & layout = insn_layout.ENC_FLAT;

			Expression::Ptr addr_ast = 
				makeTernaryExpression(
						makeRegisterExpression(amdgpu_cdna::address_mode_32), // TODO: type needs to be fixed
						makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.ADDR)),
						makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.ADDR,2)),
						u64
						);
			switch(layout.SEG){
				case 1:
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_MUBUFOperands(){
			layout_ENC_MUBUF & layout = insn_layout.ENC_MUBUF;
			MachRegister vsharp = makeAmdgpuRegID(amdgpu_cdna::sgpr0,layout.SRSRC<<2,4);
			...
			if(layout.IDXEN){
				if(layout.OFFEN){
					index_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.VADDR));
					offset_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.VADDR+1));
				}else{
					index_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.VADDR));
				}
				...
				if(layout.OFFEN){
					offset_expr =  makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.VADDR));
				}else{
					// do nothing
				}
			}
			...
			Expression::Ptr index_ast = 
				makeAddExpression(
						...
						makeTernaryExpression(
							add_tid_enable,
							makeRegisterExpression(amdgpu_cdna::tid),
			...
			Expression::Ptr vdata_ast = makeRegisterExpression(
					makeAmdgpuRegID(amdgpu_cdna::vgpr0,layout.VDATA,
			...
			if(layout.LDS){
				Expression::Ptr lds_offset_ast = makeRegisterExpression(amdgpu_cdna::m0,0,15);
				Expression::Ptr mem_offset_ast = makeRegisterExpression(
						makeAmdgpuRegID(amdgpu_cdna::sgpr0,layout.SOFFSET));
				Expression::Ptr inst_offset_ast = Immediate::makeImmediate(Result(u32,layout.OFFSET));
				Expression::Ptr lds_addr_ast =
					makeAddExpression(
							makeAddExpression(
								makeAddExpression(
									makeRegisterExpression(amdgpu_cdna::lds_base),
									lds_offset_ast,
									u32
									),
								inst_offset_ast,
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_MTBUFOperands(){
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_VOP1Operands(){
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_SOP1Operands(){
			...
			if(isBranch){
				if(isModifyPC){
					...
					new_pc_ast = makeRegisterExpression(
							makeAmdgpuRegID(amdgpu_cdna::sgpr0,layout.SSRC0,2)
							);
					...
					if(insn_in_progress->getOperation().operationID == amdgpu_op_s_setpc_b64){
    					...
    					store_pc_ast = makeRegisterExpression(
							makeAmdgpuRegID(amdgpu_cdna::sgpr0,layout.SDST,2)
							);
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_SOPPOperands(){
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_SOPKOperands(){
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_SOP2Operands(){
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_SMEMOperands(){
			...
			if(IS_LD_ST()){
				...
				if(layout.IMM==0){
					unsigned int indexing_offset = layout.SOFFSET_EN ? layout.SOFFSET : layout.OFFSET;
					...
					offset_expr = layout.SOFFSET_EN ? 
						decodeSGPRorM0(layout.SOFFSET) : Immediate::makeImmediate(Result(u64,0));
				}
				//                cout << "layout.SBASE = " << std::hex << layout.SBASE << endl;
				//MachRegister mr = makeAmdgpuRegID(amdgpu_cdna::sgpr0,4,2);
				//                cout << " shouldn't it be " << amdgpu_cdna::sgpr_vec2_0 << " " << mr << endl; 
				Expression::Ptr sbase_expr = makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::sgpr0,layout.SBASE << 1,2));
				if(isScratch)
				...
				Expression::Ptr sdata_expr = makeRegisterExpression(makeAmdgpuRegID(amdgpu_cdna::sgpr0,layout.SDATA,num_elements));
		...
		void InstructionDecoder_amdgpu_cdna::finalizeENC_SOPCOperands() {
		}
		void InstructionDecoder_amdgpu_cdna::finalizeENC_VOPCOperands() {
		}
		void InstructionDecoder_amdgpu_cdna::finalizeENC_VOP2Operands() {
		}
		void InstructionDecoder_amdgpu_cdna::finalizeENC_VINTRPOperands() {
		}
		void InstructionDecoder_amdgpu_cdna::finalizeENC_DSOperands() {
		}
	    void InstructionDecoder_amdgpu_cdna::finalizeVOP3_0_SDST_ENC_VOP3_1_Operands() {
		}
    	void InstructionDecoder_amdgpu_cdna::finalizeENC_VOP3Operands() {
		}

	    void InstructionDecoder_amdgpu_cdna::finalizeENC_VOP3POperands() {
		}
	    void InstructionDecoder_amdgpu_cdna::finalizeENC_MIMGOperands() {
		}
		...
		bool InstructionDecoder_amdgpu_cdna::decodeOperands(const amdgpu_cdna_insn_entry & insn_entry) {
		...
		void InstructionDecoder_amdgpu_cdna::insnSize(unsigned int insn_size_) {
			this->insn_size = insn_size_;
		}


		Expression::Ptr InstructionDecoder_amdgpu_cdna::decodeVDST(unsigned int index){
			index += 255;
            return decodeSSRC(index);
		} 


		Expression::Ptr InstructionDecoder_amdgpu_cdna::decodeVSRC(unsigned int index){
			if (index > 255)
				index = 0;
			MachRegister mr = makeAmdgpuRegID(amdgpu_cdna::vgpr0,index & 0xff);
			return makeRegisterExpression(mr);
		} 

		Expression::Ptr InstructionDecoder_amdgpu_cdna::decodeSSRC(unsigned int index){
			if (index < 102){
				MachRegister mr = makeAmdgpuRegID(amdgpu_cdna::sgpr0,index);
				return makeRegisterExpression(mr);
			}else if(index ==102){
				return makeRegisterExpression(amdgpu_cdna::flat_scratch_lo);
			}else if(index ==103){
				return makeRegisterExpression(amdgpu_cdna::flat_scratch_hi);
			}else if(index ==104){
				return makeRegisterExpression(amdgpu_cdna::xnack_mask_lo);
			}else if(index ==105){
				return makeRegisterExpression(amdgpu_cdna::xnack_mask_hi);
			}else if ( 106 == index ){
				return makeRegisterExpression(amdgpu_cdna::vcc_lo);
			}else if ( 107 == index ){
				return makeRegisterExpression(amdgpu_cdna::vcc_hi);
			}else if (107 < index && index < 124){
				MachRegister mr = makeAmdgpuRegID(amdgpu_cdna::ttmp0,index-108);
				return makeRegisterExpression(mr);
			}else if (124 == index ){
				return makeRegisterExpression(amdgpu_cdna::m0);
			}else if ( 126 == index ){
				return makeRegisterExpression(amdgpu_cdna::exec_lo);
			}else if ( 127 == index ){
				return makeRegisterExpression(amdgpu_cdna::exec_hi);
				...
				return makeRegisterExpression(amdgpu_cdna::shared_base);
			}else if( 236 == index){
				return makeRegisterExpression(amdgpu_cdna::shared_limit);
			}else if( 237 == index){
				return makeRegisterExpression(amdgpu_cdna::private_base);
			}else if( 238 == index){
				return makeRegisterExpression(amdgpu_cdna::private_limit);
			}else if( 239 == index){
				return makeRegisterExpression(amdgpu_cdna::pops_exiting_wave_id);
			}else if (240 == index){
				...
				return makeRegisterExpression(amdgpu_cdna::vccz);
			}else if ( 252 == index ){
				return makeRegisterExpression(amdgpu_cdna::execz);
			}else if ( 253 == index ){
				return makeRegisterExpression(amdgpu_cdna::scc);
			}else if ( 254 == index ){
				assert ( 0 && "reserved index " ) ;
			}
			else if(index == 0xff){
				...
				MachRegister mr = makeAmdgpuRegID(amdgpu_cdna::vgpr0,index >>8);
				return makeRegisterExpression(mr);
			} 

			cerr << "WARNING UNKNOWN REGISTER : "  << std::dec << index << " " <<(108 <= index && index <= 123 )<<endl;
			//return makeRegisterExpression(amdgpu_cdna::sgpr0);

			assert(0 && "unknown register value ");
		}
#include "amdgpu_cdna_decoder_impl.C"

		void InstructionDecoder_amdgpu_cdna::reset(){
			immLen = 0;
			insn_size = 0;
			num_elements =1;
			isBranch = false;
		...
		void InstructionDecoder_amdgpu_cdna::setupInsnWord(InstructionDecoder::buffer &b) {
			reset();
			if (b.start > b.end)
				return;
			insn = get32bit(b,0);
		...
		void InstructionDecoder_amdgpu_cdna::decodeOpcode(InstructionDecoder::buffer &b) {
			setupInsnWord(b);
			mainDecodeOpcode();
			b.start += insn_in_progress->size();
		}
		
		void InstructionDecoder_amdgpu_cdna::debug_instr(){
		...
		Instruction InstructionDecoder_amdgpu_cdna::decode(InstructionDecoder::buffer &b) {
			setupInsnWord(b);
			mainDecodeOpcode();
		...
		void InstructionDecoder_amdgpu_cdna::doDelayedDecode(const Instruction *insn_to_complete) {
