...
void
Instruction::version(int& major, int& minor, int& maintenance)
{
...
void
Instruction::copyRaw(size_t size, const unsigned char* raw)
{
    if(raw)
    {
        m_size               = size;
...
void
Instruction::decodeOperands() const
{
...
: m_Valid(false)
, m_size(0)
, arch_decoded_from(Arch_none)
, formatter(ArchSpecificFormatter::getFormatter(Arch_x86_64))
{
#if defined(DEBUG_INSN_ALLOCATIONS)
    numInsnsAllocated++;
    if((numInsnsAllocated % 1000) == 0)
...
INSTRUCTION_EXPORT const Instruction&
...
INSTRUCTION_EXPORT bool
Instruction::isValid() const
{
    return m_Valid;
}
...
INSTRUCTION_EXPORT const Operation&
                         Instruction::getOperation() const
{
    return m_InsnOp;
}

INSTRUCTION_EXPORT void
Instruction::getOperands(std::vector<Operand>& operands) const
{
...
INSTRUCTION_EXPORT Operand
Instruction::getOperand(int index) const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT const void*
Instruction::ptr() const
{
    if(m_size > sizeof(m_RawInsn.small_insn))
    {
...
INSTRUCTION_EXPORT unsigned char
Instruction::rawByte(unsigned int index) const
{
    if(index >= m_size)
        return 0;
    if(m_size > sizeof(m_RawInsn.small_insn))
...
INSTRUCTION_EXPORT void
Instruction::getReadSet(std::set<RegisterAST::Ptr>& regsRead) const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT void
Instruction::getWriteSet(std::set<RegisterAST::Ptr>& regsWritten) const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT bool
Instruction::isRead(Expression::Ptr candidate) const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT bool
Instruction::isWritten(Expression::Ptr candidate) const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT Operand
Instruction::getPredicateOperand() const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT bool
Instruction::hasPredicateOperand() const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
...
INSTRUCTION_EXPORT Expression::Ptr
                   Instruction::getControlFlowTarget() const
{
...
INSTRUCTION_EXPORT ArchSpecificFormatter&
                   Instruction::getFormatter() const
{
    return formatter;
}

INSTRUCTION_EXPORT std::string
                   Instruction::format(Address addr) const
{
    if(arch_decoded_from != Arch_cuda && m_Operands.empty())
    {
        decodeOperands();
    ...
    int                                op = 0;
    for(currOperand = m_Operands.begin(); currOperand != m_Operands.end();
        op++, ++currOperand)
    {
...
INSTRUCTION_EXPORT bool
Instruction::allowsFallThrough() const
{
...
INSTRUCTION_EXPORT bool
Instruction::isLegalInsn() const
{
    return (m_InsnOp.getID() != e_No_Entry);
}

INSTRUCTION_EXPORT Architecture
...
Expression::Ptr
Instruction::makeReturnExpression() const
{
...
void
Instruction::addSuccessor(Expression::Ptr e, bool isCall, bool isIndirect,
...
void
Instruction::appendOperand(Expression::Ptr e, bool isRead, bool isWritten,
