...
int findInsnTableIndex(unsigned int insn, unsigned int decoder_table_index) {
  ...
  if (cur_mask == 0) return cur_entry->insnTableIndex;
...
int main() {
  ...
  while (insn != -1) {
    // std::cout<<std::hex<<insn<<":"<<std::dec<<findInsnTableIndex(insn,
    // 0)<<std::endl;
    const char *mnemonic =
        aarch64_insn_entry::main_insn_table[findInsnTableIndex(insn, 0)]
            .mnemonic;
