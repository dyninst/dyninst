  ...
  private:
    ...
    struct translateBitFieldToCR {
      ...
      void operator()() {
        ...
        if(m_dec.field<high, high>(m_dec.insn)) {
          m_dec.insn_in_progress->appendOperand(
              m_dec.makeRegisterExpression(m_dec.makePowerRegID(MachRegister(base), curCR)),
              !m_dec.isRAWritten, m_dec.isRAWritten);
        }
      }
    ...
    struct translateBitFieldToCR<low, high, base, 0> {
      ...
      void operator()() {
        if(m_dec.field<high, high>(m_dec.insn)) {
          m_dec.insn_in_progress->appendOperand(
              m_dec.makeRegisterExpression(m_dec.makePowerRegID(MachRegister(base), 0)),
              !m_dec.isRAWritten, m_dec.isRAWritten);
        }
      }
    ...
    template <int lowBit, int highBit> Expression::Ptr makeBranchTarget() {
      ...
      if(field<30, 30>(insn) == 1) {
        ...
        return Immediate::makeImmediate(
            Result(u32, sign_extend<(highBit - lowBit + 1)>(field<lowBit, highBit>(insn)) << 2));
      } else {
    ...
    MachRegister makePowerRegID(MachRegister base, unsigned int encoding, unsigned int lowBit,
