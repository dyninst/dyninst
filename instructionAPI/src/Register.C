...
RegisterAST::RegisterAST(MachRegister r, unsigned int lowbit,
                         unsigned int highbit)
    : Expression(r), m_Reg(r), m_Low(lowbit), m_High(highbit) {}
RegisterAST::RegisterAST(MachRegister r) : Expression(r), m_Reg(r), m_Low(0) {
  m_High = r.size() * 8;
}

RegisterAST::RegisterAST(MachRegister r, unsigned int lowbit,
                         unsigned int highbit, Result_Type regType)
...
RegisterAST RegisterAST::makePC(Dyninst::Architecture arch) {
  return RegisterAST(MachRegister::getPC(arch), 0,
                     MachRegister::getPC(arch).size());
}
...
bool RegisterAST::checkRegID(MachRegister r, unsigned int low,
                             unsigned int high) const {
#if defined(DEBUG)
  fprintf(stderr, "%s (%d-%d/%x) compared with %s (%d-%d/%x)", format().c_str(),
          m_Low, m_High, m_Reg.getBaseRegister().val(), r.name().c_str(), low,
          high, r.getBaseRegister().val());
#endif
