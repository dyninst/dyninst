...
void
RegisterAST::getChildren(vector<InstructionAST::Ptr>& /*children*/) const
{
    return;
}
void
RegisterAST::getChildren(vector<Expression::Ptr>& /*children*/) const
{
    return;
}
void
RegisterAST::getUses(set<InstructionAST::Ptr>& uses)
{
    uses.insert(shared_from_this());
    return;
}
bool
RegisterAST::isUsed(InstructionAST::Ptr findMe) const
{
    return findMe->checkRegID(m_Reg, m_Low, m_High);
}
MachRegister
RegisterAST::getID() const
{
    return m_Reg;
}

std::string
RegisterAST::format(Architecture arch, formatStyle f) const
{
...
std::string
MaskRegisterAST::format(Architecture, formatStyle f) const
{
    return format(f);
}
...
RegisterAST
RegisterAST::makePC(Dyninst::Architecture arch)
{
    return RegisterAST(MachRegister::getPC(arch), 0, MachRegister::getPC(arch).size());
}

bool
RegisterAST::operator<(const RegisterAST& rhs) const
{
    if(m_Reg < rhs.m_Reg)
        return true;
    if(rhs.m_Reg < m_Reg)
...
bool
RegisterAST::isStrictEqual(const InstructionAST& rhs) const
{
...
RegisterAST::Ptr
RegisterAST::promote(const InstructionAST::Ptr regPtr)
{
...
RegisterAST::Ptr
RegisterAST::promote(const RegisterAST* regPtr)
{
    if(!regPtr)
    ...
    return make_shared(singleton_object_pool<RegisterAST>::construct(
        regPtr->getPromotedReg(), 0, regPtr->getPromotedReg().size()));
}
bool
RegisterAST::isFlag() const
{
    return m_Reg.getBaseRegister() == x86::flags;
}
bool
RegisterAST::checkRegID(MachRegister r, unsigned int low, unsigned int high) const
{
#if defined(DEBUG)
    fprintf(stderr, "%s (%d-%d/%x) compared with %s (%d-%d/%x)", format().c_str(), m_Low,
            m_High, m_Reg.getBaseRegister().val(), r.name().c_str(), low, high,
            r.getBaseRegister().val());
#endif
    return (r.getBaseRegister() == m_Reg.getBaseRegister()) && (low <= m_High) &&
           (high >= m_Low);
}
void
RegisterAST::apply(Visitor* v)
{
    v->visit(this);
}
bool
RegisterAST::bind(Expression* e, const Result& val)
{
    if(Expression::bind(e, val))
    {
        return true;
    }
    //        fprintf(stderr, "checking %s against %s with checkRegID in
    //        RegisterAST::bind... %p", e->format().c_str(),
    // format().c_str(), this);
