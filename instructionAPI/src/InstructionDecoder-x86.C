...
{
bool
readsOperand(unsigned int opsema, unsigned int i)
{
    switch(opsema)
    {
        case s1R2R:
...
bool
writesOperand(unsigned int opsema, unsigned int i)
{
    switch(opsema)
    {
        case s1R2R:
...
bool
implicitOperand(unsigned int implicit_operands, unsigned int i)
{
    return sGetImplicitOP(implicit_operands, i) != 0x0;
}

INSTRUCTION_EXPORT
InstructionDecoder_x86::InstructionDecoder_x86(Architecture a)
...
InstructionDecoder_x86::setMode(bool is64)
{
    InstructionDecoder_x86::is64BitMode = is64;
}

Expression::Ptr
InstructionDecoder_x86::makeSIBExpression(const InstructionDecoder::buffer& b)
{
    unsigned    scale;
    Register    index;
    Register    base;
    ...
    Expression::Ptr indexAST(make_shared(singleton_object_pool<RegisterAST>::construct(
    ...
    return makeAddExpression(
        baseAST, makeMultiplyExpression(indexAST, scaleAST, registerType), registerType);
}

Expression::Ptr
InstructionDecoder_x86::makeModRMExpression(const InstructionDecoder::buffer& b,
                                            unsigned int                      opType)
{
...
Expression::Ptr
InstructionDecoder_x86::decodeImmediate(unsigned int         opType,
                                        const unsigned char* immStart, bool isSigned)
{
    ...
    switch(opType)
    {
        ...
        case op_c:
            assert(
                !"Can't happen: opType unexpected for valid ways to decode an immediate");
            return Expression::Ptr();
        default:
...
static MachRegister IntelRegTable32[][8] = {
    ...
    { x86_64::rax, x86_64::rcx, x86_64::rdx, x86_64::rbx, x86_64::rsp, x86_64::rbp,
      x86_64::rsi, x86_64::rdi }, /* b_64bit */
...
static MachRegister IntelRegTable64[][8] = {
    ...
    { x86_64::ax, x86_64::cx, x86_64::dx, x86_64::bx, x86_64::sp, x86_64::bp, x86_64::si,
      x86_64::di },
    { x86_64::eax, x86_64::ecx, x86_64::edx, x86_64::ebx, x86_64::esp, x86_64::ebp,
      x86_64::esi, x86_64::edi },
    { x86_64::es, x86_64::cs, x86_64::ss, x86_64::ds, x86_64::fs, x86_64::gs, InvalidReg,
      InvalidReg },
    { x86_64::rax, x86_64::rcx, x86_64::rdx, x86_64::rbx, x86_64::rsp, x86_64::rbp,
      x86_64::rsi, x86_64::rdi },
    ...
    { x86_64::k0, x86_64::k1, x86_64::k2, x86_64::k3, x86_64::k4, x86_64::k5, x86_64::k6,
      x86_64::k7 },
    { x86_64::mm0, x86_64::mm1, x86_64::mm2, x86_64::mm3, x86_64::mm4, x86_64::mm5,
      x86_64::mm6, x86_64::mm7 },
    { x86_64::cr0, x86_64::cr1, x86_64::cr2, x86_64::cr3, x86_64::cr4, x86_64::cr5,
      x86_64::cr6, x86_64::cr7 },
    { x86_64::dr0, x86_64::dr1, x86_64::dr2, x86_64::dr3, x86_64::dr4, x86_64::dr5,
      x86_64::dr6, x86_64::dr7 },
    { x86_64::tr0, x86_64::tr1, x86_64::tr2, x86_64::tr3, x86_64::tr4, x86_64::tr5,
      x86_64::tr6, x86_64::tr7 },
    { x86_64::r8, x86_64::r9, x86_64::r10, x86_64::r11, x86_64::r12, x86_64::r13,
      x86_64::r14, x86_64::r15 },
    { x86_64::al, x86_64::cl, x86_64::dl, x86_64::bl, x86_64::spl, x86_64::bpl,
      x86_64::sil, x86_64::dil },
    { x86_64::st0, x86_64::st1, x86_64::st2, x86_64::st3, x86_64::st4, x86_64::st5,
      x86_64::st6, x86_64::st7 },
...
bool
InstructionDecoder_x86::isDefault64Insn()
{
...
MachRegister
InstructionDecoder_x86::makeRegisterID(unsigned int intelReg, unsigned int opType,
                                       bool isExtendedReg)
{
    MachRegister retVal;
...
Result_Type
InstructionDecoder_x86::makeSizeType(unsigned int opType)
{
    switch(opType)
    {
        case op_b:
 ...
 */
bool
decodeAVX(intelRegBanks& bank, int* bank_index, int regnum, AVX_Regtype type,
          ia32_prefixes& pref, unsigned int admet)
{
...
bool
InstructionDecoder_x86::decodeOneOperand(const InstructionDecoder::buffer& b,
    ...
    switch(operand.admet)
    {
        case 0:
            // No operand
            {
                ...
                // bytes: "); for(unsigned int i = 0; i < decodedInstruction->getSize();
                // i++) {
            ...
            return false;
        case am_A: {
            // am_A only shows up as a far call/jump.  Position 1 should be universally
            // safe.
        ...
        case am_S:
            ...
            insn_to_complete->appendOperand(
                makeRegisterExpression(IntelRegTable(m_Arch, b_segment, locs->modrm_reg)),
                isRead, isWritten, isImplicit);
            break;
        case am_T:
            // test register in modrm reg; should only be tr6/tr7, but we'll decode any of
            // them NOTE: this only appears in deprecated opcodes
            insn_to_complete->appendOperand(
        ...
        case am_X: {
            ...
            insn_to_complete->appendOperand(
                makeDereferenceExpression(ds_si, makeSizeType(optype)), isRead, isWritten,
                isImplicit);
        }
        break;
        case am_Y: {
            ...
            insn_to_complete->appendOperand(
                makeDereferenceExpression(es_di, makeSizeType(optype)), isRead, isWritten,
                isImplicit);
        }
        break;
        case am_tworeghack:
            ...
            {
                Expression::Ptr edx(
                    makeRegisterExpression(m_Arch == Arch_x86 ? x86::edx : x86_64::edx));
                Expression::Ptr eax(
                    makeRegisterExpression(m_Arch == Arch_x86 ? x86::eax : x86_64::eax));
            ...
            {
                Expression::Ptr ecx(
                    makeRegisterExpression(m_Arch == Arch_x86 ? x86::ecx : x86_64::ecx));
                Expression::Ptr ebx(
                    makeRegisterExpression(m_Arch == Arch_x86 ? x86::ebx : x86_64::ebx));
                Expression::Ptr highAddr = makeMultiplyExpression(
        ...
        case am_allgprs:
            if(m_Arch == Arch_x86)
            {
                insn_to_complete->appendOperand(makeRegisterExpression(x86::eax), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::ecx), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::edx), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::ebx), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::esp), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::ebp), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::esi), isRead,
                                                isWritten, isImplicit);
                insn_to_complete->appendOperand(makeRegisterExpression(x86::edi), isRead,
                                                isWritten, isImplicit);
            }
            else
            {
...
void
InstructionDecoder_x86::doIA32Decode(InstructionDecoder::buffer& b)
{
    ...
    static ia32_entry invalid = {
        e_No_Entry, 0, 0, false, { { 0, 0 }, { 0, 0 }, { 0, 0 } }, 0, 0, 0
    };
        ...
        if(decodedInstruction->getPrefix()->getPrefix(0) == PREFIX_LOCK)
        {
            switch(decodedInstruction->getEntry()->id)
            {
                ...
                case e_xchg:
                    break;
                default:
                    m_Operation = Operation(&invalid, decodedInstruction->getPrefix(),
                                            locs, m_Arch);
                    return;
            }
        }
    ...
    {
        // Gap parsing can trigger this case; in particular, when it encounters prefixes
        // in an invalid order. Notably, if a REX prefix (0x40-0x48) appears followed by
        // another prefix (0x66, 0x67, etc) we'll reject the instruction as invalid and
        // send it back with no entry.  Since this is a common byte sequence to see in,
        // for example, ASCII strings, we want to simply accept this and move on, not yell
        // at the user.
...
void
InstructionDecoder_x86::decodeOpcode(InstructionDecoder::buffer& b)
{
    doIA32Decode(b);
...
bool
InstructionDecoder_x86::decodeOperands(const Instruction* insn_to_complete)
{
       ...
       decodedInstruction->getEntry()->getID() == e_ret_far)
    {
        Expression::Ptr ret_addr = makeDereferenceExpression(
            makeRegisterExpression(is64BitMode ? x86_64::rsp : x86::esp),
            is64BitMode ? u64 : u32);
    ...
    if(semantics >= s4OP)
    {
           ...
           decodedInstruction->getEntry()->operands[3].optype != 0)
        {
            ...
            if(!decodeOneOperand(b, decodedInstruction->getEntry()->operands[3],
    ...
    {
        insn_to_complete->appendOperand(
            makeMaskRegisterExpression(IntelRegTable(m_Arch, b_kmask, pref.vex_aaa)),
            true, false);
    }
...
InstructionDecoder_x86::decode(InstructionDecoder::buffer& b)
{
    return InstructionDecoderImpl::decode(b);
}
void
InstructionDecoder_x86::doDelayedDecode(const Instruction* insn_to_complete)
{
    InstructionDecoder::buffer b(insn_to_complete->ptr(), insn_to_complete->size());
