...
namespace Dyninst { namespace InstructionAPI {
  ...
  Expression::Ptr InstructionDecoder_x86::makeSIBExpression(const InstructionDecoder::buffer& b) {
    ...
    Expression::Ptr scaleAST(
        make_shared(singleton_object_pool<Immediate>::construct(Result(u8, dword_t(scale)))));
    Expression::Ptr indexAST(make_shared(
        singleton_object_pool<RegisterAST>::construct(makeRegisterID(index, op_type, locs->rex_x))));
    ...
    if(base == 0x05) {
      switch(locs->modrm_mod) {
        ...
        case 0x02:
          baseAST = make_shared(
              singleton_object_pool<RegisterAST>::construct(makeRegisterID(base, op_type, locs->rex_b)));
          break;
        case 0x03:
  ...
  bool InstructionDecoder_x86::decodeOneOperand(const InstructionDecoder::buffer& b,
    ...
    switch(operand.admet) {
      ...
      case am_J: {
        ...
        Expression::Ptr InsnSize(make_shared(
            singleton_object_pool<Immediate>::construct(Result(u8, decodedInstruction->getSize()))));
      ...
      case am_X: {
        ...
        Expression::Ptr segmentOffset(
            make_shared(singleton_object_pool<Immediate>::construct(Result(u32, 0x10))));
