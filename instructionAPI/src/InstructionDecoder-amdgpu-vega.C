...
const char*
InstructionDecoder_amdgpu_vega::bitfieldInsnAliasMap(entryID e)
{
    switch(e)
    {
        default:
...
const char*
InstructionDecoder_amdgpu_vega::condInsnAliasMap(entryID e)
{
    switch(e)
    {
        default:
...
using namespace std;
void
InstructionDecoder_amdgpu_vega::NOTHING()
{}

Result_Type
InstructionDecoder_amdgpu_vega::makeSizeType(unsigned int)
{
...
MachRegister
InstructionDecoder_amdgpu_vega::makeAmdgpuRegID(MachRegister base, unsigned int encoding,
                                                unsigned int len)
{
...
Expression::Ptr
InstructionDecoder_amdgpu_vega::makePCExpr()
{
...
void
InstructionDecoder_amdgpu_vega::makeBranchTarget(bool branchIsCall, bool bIsConditional,
                                                 int immVal, int immLen_ = 16)
{
    Expression::Ptr lhs =
        makeAddExpression(makePCExpr(), Immediate::makeImmediate(Result(s48, 4)), s48);
...
Expression::Ptr
InstructionDecoder_amdgpu_vega::makeFallThroughExpr()
{
    // TODO: while s_call_B64 is always 4 bytes, it is not clear whether all instructions
    // that has a fall through branch are 4 bytes long
    return makeAddExpression(
        makePCExpr(), Immediate::makeImmediate(Result(u64, unsign_extend64(3, 4))), u64);
}

bool
InstructionDecoder_amdgpu_vega::decodeOperands(const Instruction*)
{
...
Expression::Ptr
InstructionDecoder_amdgpu_vega::decodeSGPRorM0(unsigned int offset)
{
    if(offset <= 104)
...
void
InstructionDecoder_amdgpu_vega::finalizeFLATOperands()
{
    layout_flat& layout = insn_layout.flat;

    Expression::Ptr addr_ast = makeTernaryExpression(
        makeRegisterExpression(
            amdgpu_vega::address_mode_32),  // TODO: type needs to be fixed
        makeRegisterExpression(makeAmdgpuRegID(amdgpu_vega::vgpr0, layout.addr)),
        makeRegisterExpression(makeAmdgpuRegID(amdgpu_vega::vgpr0, layout.addr, 2)), u64);
    switch(layout.seg)
    {
        case 1:
...
void
InstructionDecoder_amdgpu_vega::finalizeMUBUFOperands()
{
    ...
    if(layout.idxen)
    {
        ...
        {
            index_expr =
                makeRegisterExpression(makeAmdgpuRegID(amdgpu_vega::vgpr0, layout.vaddr));
            offset_expr = makeRegisterExpression(
                makeAmdgpuRegID(amdgpu_vega::vgpr0, layout.vaddr + 1));
        }
        else
        {
            index_expr =
    ...
    Expression::Ptr index_ast = makeAddExpression(
        index_expr,
        makeTernaryExpression(add_tid_enable, makeRegisterExpression(amdgpu_vega::tid),
                              Immediate::makeImmediate(Result(u32, 0)), u32),
        u32);

    Expression::Ptr offset_ast = makeAddExpression(
        offset_expr, Immediate::makeImmediate(Result(u32, layout.offset)), u32);

    Expression::Ptr buffer_offset = makeAddExpression(
        offset_ast, makeMultiplyExpression(const_stride_ast, index_ast, u64), u64);

    Expression::Ptr sgpr_offset_ast = decodeSSRC(layout.soffset);
    Expression::Ptr addr_ast        = makeAddExpression(
        makeAddExpression(const_base_ast, sgpr_offset_ast, u64), buffer_offset, u64);

    Expression::Ptr vdata_ast = makeRegisterExpression(
        makeAmdgpuRegID(amdgpu_vega::vgpr0, layout.vdata,
                        num_elements  // TODO: This depends on number of elements, which
                                      // is available from opcode
                        ));
    ...
    if(layout.lds)
    {
        ...
        Expression::Ptr mem_offset_ast =
            makeRegisterExpression(makeAmdgpuRegID(amdgpu_vega::sgpr0, layout.soffset));
        Expression::Ptr inst_offset_ast =
            Immediate::makeImmediate(Result(u32, layout.offset));
        Expression::Ptr lds_addr_ast = makeAddExpression(
            makeAddExpression(
                makeAddExpression(makeRegisterExpression(amdgpu_vega::lds_base),
                                  lds_offset_ast, u32),
                inst_offset_ast, u32),
            makeMultiplyExpression(makeRegisterExpression(amdgpu_vega::tid),
                                   Immediate::makeImmediate(Result(u16, 4)), u32),
            u32);
...
void
InstructionDecoder_amdgpu_vega::finalizeMTBUFOperands()
{
...
void
InstructionDecoder_amdgpu_vega::finalizeVOP1Operands()
{
    layout_vop1& layout = insn_layout.vop1;
    // cout << " finalizing vop1 operands , vdst = " << std::dec << layout.vdst << " src0
    // = " << layout.src0 <<endl;
...
void
InstructionDecoder_amdgpu_vega::finalizeSOP1Operands()
{
    ...
    if(isBranch)
    {
        if(isModifyPC)
        {
            ...
            {
                // RegisterAST::Ptr tmpqq =
                // boost::dynamic_pointer_cast<RegisterAST>(new_pc_ast); std::cout <<
                // "setting pc to offset " << std::hex <<tmpqq->getID()<< std::endl;
            ...
            else if(insn_in_progress->getOperation().operationID ==
...
void
InstructionDecoder_amdgpu_vega::finalizeSOPPOperands()
{
...
void
InstructionDecoder_amdgpu_vega::finalizeSOPKOperands()
{
    layout_sopp& layout = insn_layout.sopp;
...
void
InstructionDecoder_amdgpu_vega::finalizeSOP2Operands()
{
...
void
InstructionDecoder_amdgpu_vega::finalizeSMEMOperands()
{
    ...
    if(IS_LD_ST())
    {
        ...
        else
        {
            ...
            offset_expr      = layout.soe ? decodeSGPRorM0(layout.soffset)
                                     : Immediate::makeImmediate(Result(u64, 0));
        }
        //                cout << "layout.sbase = " << std::hex << layout.sbase << endl;
        // MachRegister mr = makeAmdgpuRegID(amdgpu_vega::sgpr0,4,2);
        //                cout << " shouldn't it be " << amdgpu_vega::sgpr_vec2_0 << " "
        //                << mr << endl;
        Expression::Ptr sbase_expr = makeRegisterExpression(
            makeAmdgpuRegID(amdgpu_vega::sgpr0, layout.sbase << 1, 2));
        if(isScratch)
            offset_expr = makeMultiplyExpression(
                offset_expr, Immediate::makeImmediate(Result(u64, 64)), u64);

        Expression::Ptr remain_expr =
            makeAddExpression(inst_offset_expr, offset_expr, u64);
        Expression::Ptr addr_expr = makeDereferenceExpression(
            makeAddExpression(sbase_expr, remain_expr, u64), u64);

        Expression::Ptr sdata_expr = makeRegisterExpression(
            makeAmdgpuRegID(amdgpu_vega::sgpr0, layout.sdata, num_elements));
...
void
InstructionDecoder_amdgpu_vega::finalizeSOPCOperands()
{}
void
InstructionDecoder_amdgpu_vega::finalizeVOPCOperands()
{}
void
InstructionDecoder_amdgpu_vega::finalizeVOP2Operands()
{}
void
InstructionDecoder_amdgpu_vega::finalizeVINTRPOperands()
{}
void
InstructionDecoder_amdgpu_vega::finalizeDSOperands()
{}
void
InstructionDecoder_amdgpu_vega::finalizeVOP3ABOperands()
{}
void
InstructionDecoder_amdgpu_vega::finalizeVOP3POperands()
{}

bool
InstructionDecoder_amdgpu_vega::decodeOperands(const amdgpu_insn_entry& insn_entry)
{
...
inline unsigned int
get32bit(InstructionDecoder::buffer& b, unsigned int offset)
{
    assert(offset % 4 == 0);
...
void
InstructionDecoder_amdgpu_vega::insnSize(unsigned int insn_size_)
{
    this->insn_size = insn_size_;
}

Expression::Ptr
InstructionDecoder_amdgpu_vega::decodeVDST(unsigned int index)
{
    index += 255;
...
Expression::Ptr
InstructionDecoder_amdgpu_vega::decodeVSRC(unsigned int index)
{
    if(index > 255)
        index = 0;
...
Expression::Ptr
InstructionDecoder_amdgpu_vega::decodeSSRC(unsigned int index)
{
    if(index < 102)
    {
    ...
    cerr << "WARNING UNKNOWN REGISTER : " << std::dec << index << " "
         << (108 <= index && index <= 123) << endl;
...
void
InstructionDecoder_amdgpu_vega::reset()
{
    immLen        = 0;
    insn_size     = 0;
    num_elements  = 1;
...
void
InstructionDecoder_amdgpu_vega::setupInsnWord(InstructionDecoder::buffer& b)
{
    reset();
    if(b.start > b.end)
        return;
...
void
InstructionDecoder_amdgpu_vega::decodeOpcode(InstructionDecoder::buffer& b)
{
    setupInsnWord(b);
    mainDecodeOpcode(b);
...
void
InstructionDecoder_amdgpu_vega::debug_instr()
{
...
Instruction
InstructionDecoder_amdgpu_vega::decode(InstructionDecoder::buffer& b)
{
    setupInsnWord(b);
    mainDecodeOpcode(b);
...
void
InstructionDecoder_amdgpu_vega::doDelayedDecode(const Instruction* insn_to_complete)
{
