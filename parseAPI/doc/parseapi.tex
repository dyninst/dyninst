...
instruction at the provided target address. If \texttt{recursive} is 
{\scshape true}, recursive traversal parsing is used as in the default 
\texttt{parse()} method; otherwise only instructions reachable through 
...
\end{apient}
\apidesc{Find the next reachable basic block starting at address \texttt{entry}. Returns {\scshape null} if no such block exists.}
...
\end{apient}
\apidesc{Returns a boolean specifying if the address at \texttt{addr} is located at the call named in \texttt{calleeName}. }
...
\end{apient}
\apidesc{Registers a callback and adds it to the callback batch. Callback will be executed when startCallbackBatch() is called.}

\begin{apient}
void unregisterCallback(ParseCallback * cb)
\end{apient}
\apidesc{Removes the callback \texttt{cb} from the callback batch. This callback will no longer be exectued when 
startCallbackBatch() is exectued.}
...
\end{apient}
\apidesc{Destroy the function lister.}

\begin{apient}
Address getFreeAddr()
\end{apient}
\apidesc{Return a unused address that can be used for instruction insertion.}
...
\end{apient}
\apidesc{Returns a vector of the currently defined hints.}
...
\end{apient}
\apidesc{Creates a function at \texttt{addr} in the code region specified. 
Insructions for this function are given in \texttt{isource}.}
...
Address addr() 
\end{apient}
\apidesc{Returns the first address of the function.}
...
\end{apient}
\apidesc{Returns a list of all blocks ending in a \texttt{call} instruction.}
...
\end{apient}
\apidesc{Set the entry block for this function to \texttt{new_entry}.}

\begin{apient}
void set_retstatus(FuncReturnStatus rs)
\end{apient}
\apidesc{Set the return status for the function to \texttt{rs}}
...
Function * func() 
\end{apient}
\apidesc{Return the function linked to FuncExtent.}
