...
%    };
    ...
    \node [anchor=west,font=\sffamily] (rel1) at ($(origin)+(0.75in,-5.0in)$)
        {\fontsize{24}{32}\selectfont 8.0 Release};
    \node [anchor=west,font=\sffamily] (rel2) at ($(rel1.west)+(0in,-32pt)$)
        {\fontsize{24}{32}\selectfont Nov 2012};
...
\end{apient}
\apidesc{Removes the callback \code{cb} from the callback batch. This callback will no longer be exectued when 
...
\end{apient}
\apidesc{Returns a list of all blocks ending in a \code{return} instruction.}

\begin{apient}
blocklist & exitBlocks()
\end{apient}
\apidesc{Returns a list of all blocks ending in a \code{call} instruction.}
...
Address size()
\end{apient}
\apidesc{Returns $\code{end()} - \code{start()}$.}
