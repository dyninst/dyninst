...
\begin{apient}
bool isValidAddress(const Address);
\end{apient}
\apidesc{Returns \code{true} if the address is a valid code location.}

\begin{apient}
void* getPtrToInstruction(const Address);
\end{apient}
\apidesc{Returns pointer to raw memory in the binary at the provided address.}

\begin{apient}
void* getPtrToData(const Address);
\end{apient}
...
\begin{apient}
bool isAligned(const Address) const;
\end{apient}
\apidesc{For fixed-width instruction architectures, must return \code{true} if the address is a valid instruction boundary and \code{false} otherwise; otherwise returns \code{true}. This method has a default implementation that should be sufficient.}
