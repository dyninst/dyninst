...
\begin{apient}
bool findCatchBlock(Address addr,
                            Address& catchStart);
\end{apient}
...
\end{apient}
\apidesc{Returns \code{true} if $\mbox{\code{addr}} \in [\mbox{\code{low()}},\mbox{\code{high()}})$, \code{false} otherwise.}

\begin{apient}
bool wasUserAdded() const;
\end{apient}
\apidesc{Return \code{true} if this region was added by the user, \code{false} otherwise.}
