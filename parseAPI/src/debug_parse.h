...
namespace ParseAPI
{
extern int PARSER_EXPORT
parsing_printf_int(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
extern int PARSER_EXPORT
malware_printf_int(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
extern int PARSER_EXPORT
           indirect_collect_printf_int(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
...
#define dyn_debug_printf(debug_sys, ...)                                                 \
    do                                                                                   \
    {                                                                                    \
        if(!dyn_debug_initialized || dyn_debug_##debug_sys)                              \
            debug_sys##_printf_int(__VA_ARGS__);                                         \
    } while(0)
