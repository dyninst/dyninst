...
#define parsing_cerr \
  if (dyn_debug_parsing) cerr
#define malware_cerr \
  if (dyn_debug_malware) cerr

#if defined(__GNUC__)
#define parsing_printf(format, args...)              \
  do {                                               \
    if (!dyn_debug_initialized || dyn_debug_parsing) \
      parsing_printf_int(format, ##args);            \
  } while (0)
#define mal_printf(format, args...)                  \
  do {                                               \
    if (!dyn_debug_initialized || dyn_debug_malware) \
      malware_printf_int(format, ##args);            \
  } while (0)
#define indirect_collect_printf(format, args...)              \
  do {                                                        \
    if (!dyn_debug_initialized || dyn_debug_indirect_collect) \
      indirect_collect_printf_int(format, ##args);            \
  } while (0)
