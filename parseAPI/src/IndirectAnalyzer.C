...
static bool
IsIndexing(AST::Ptr node, AbsRegion& index)
{
    RoseAST::Ptr n = boost::static_pointer_cast<RoseAST>(node);
    if(n->val().op != ROSEOperation::sMultOp && n->val().op != ROSEOperation::uMultOp &&
       n->val().op != ROSEOperation::shiftLOp && n->val().op != ROSEOperation::rotateLOp)
        return false;
    if(n->child(0)->getID() != AST::V_VariableAST)
        return false;
    if(n->child(1)->getID() != AST::V_ConstantAST)
        return false;
...
static bool
IsVariableArgumentFormat(AST::Ptr t, AbsRegion& index)
{
    ...
    if(rt->child(0)->getID() != AST::V_ConstantAST ||
       rt->child(1)->getID() != AST::V_RoseAST)
    {
        return false;
    }
    ...
    {
        if(c1->child(0)->getID() == AST::V_RoseAST &&
           c1->child(1)->getID() == AST::V_ConstantAST)
        {
    ...
    std::vector<std::pair<Address, Dyninst::ParseAPI::EdgeTypeEnum>>& outEdges)
{
    ...
    const unsigned char* buf =
        (const unsigned char*) block->region()->getPtrToInstruction(block->last());
    InstructionDecoder dec(buf, InstructionDecoder::maxInstructionLength,
    ...
    if(se.cs->getArch() == Arch_amdgpu_vega &&
       insn.getOperation().getID() == amdgpu_op_s_swappc_b64)
    {
        Result_t symRet;
    ...
    parsing_printf("In function %s, Address %lx, jump target format %s, index loc %s, "
                   "index variable %s, memory read loc %s, isJumpTableFormat %d\n",
                   func->name().c_str(), block->last(), jtfp.format().c_str(),
    ...
    {
        if(jtfp.jumpTargetExpr && func->entry() == block &&
           IsVariableArgumentFormat(jtfp.jumpTargetExpr, jtfp.index))
        {
    ...
    if(!variableArguFormat)
    {
        ...
        {
            parsing_printf(" Cannot find bound, assume there are at most 256 entries and "
                           "scan the table for indirect jump at %lx\n",
                           block->last());
    ...
    inst.indexStride                = 0;
    inst.block                      = block;
    ReadTable(jtfp.jumpTargetExpr, jtfp.index, b, inst.memoryReadSize, inst.isZeroExtend,
              jtfp.constAddr, jumpTableOutEdges, inst.tableStart, inst.tableEnd,
              inst.indexStride, inst.tableEntryMap);
...
// Find all blocks that reach the block containing the indirect jump
void
IndirectControlFlowAnalyzer::GetAllReachableBlock()
{
    reachable.clear();
    queue<Block*> q;
    q.push(block);
...
static Address
ThunkAdjustment(Address afterThunk, MachRegister reg, ParseAPI::Block* b)
{
...
void
IndirectControlFlowAnalyzer::FindAllThunks()
{
    ...
    for(auto bit = reachable.begin(); bit != reachable.end(); ++bit)
    {
        ...
        InstructionDecoder    dec(buf, b->end() - b->start(), b->obj()->cs()->getArch());
        InsnAdapter::IA_IAPI* insnBlock = InsnAdapter::IA_IAPI::makePlatformIA_IAPI(
            b->obj()->cs()->getArch(), dec, b->start(), b->obj(), b->region(),
            b->obj()->cs(), b);
        Address cur = b->start();
        while(cur < b->end())
        {
            if(insnBlock->getInstruction().getCategory() == c_CallInsn &&
               insnBlock->isThunk())
            {
                bool    valid;
                Address addr;
                ...
                const unsigned char* target =
                    (const unsigned char*) b->region()->getPtrToInstruction(addr);
                ...
                for(auto curReg = thunkTargetRegs.begin();
                    curReg != thunkTargetRegs.end(); ++curReg)
                {
                    ThunkInfo t;
                    t.reg   = (*curReg)->getID();
...
void
IndirectControlFlowAnalyzer::ReadTable(
    AST::Ptr jumpTargetExpr, AbsRegion index, StridedInterval& indexBound,
    int memoryReadSize, bool isZeroExtend, set<Address>& constAddr,
    std::vector<std::pair<Address, Dyninst::ParseAPI::EdgeTypeEnum>>& targetEdges,
    Address& minReadAddress, Address& maxReadAddress, int& indexStride,
    std::map<Address, Address>& entries)
{
    ...
    {
        JumpTableReadVisitor jtrv(index, v, cs, block->region(), isZeroExtend,
                                  memoryReadSize);
        ...
        if(jtrv.valid && cs->isCode(jtrv.targetAddress))
        {
            ...
            {
                parsing_printf(
                    "WARNING: resolving jump tables leads to junk instruction from %lx\n",
                    jtrv.targetAddress);
                break;
            }
            if(jtrv.readAddress < minReadAddress || minReadAddress == 0)
                minReadAddress = jtrv.readAddress;
            if(jtrv.readAddress > maxReadAddress)
                maxReadAddress = jtrv.readAddress;
            if(prevReadAddress > 0)
                indexStride = jtrv.readAddress - prevReadAddress;
            prevReadAddress = jtrv.readAddress;
            parsing_printf("\t index %d, table address %lx, target address %lx\n", v,
                           jtrv.readAddress, jtrv.targetAddress);
...
int
IndirectControlFlowAnalyzer::GetMemoryReadSize(Assignment::Ptr memLoc)
{
    if(!memLoc)
    {
...
bool
IndirectControlFlowAnalyzer::IsZeroExtend(Assignment::Ptr memLoc)
{
    if(!memLoc)
    {
        parsing_printf("\tmemLoc is null\n");
    ...
    if(i.format().find("movz") != string::npos)
        return true;
    return false;
}

bool
IndirectControlFlowAnalyzer::FindJunkInstruction(Address addr)
{
    unsigned size = block->region()->offset() + block->region()->length() - addr;
    const unsigned char* buffer =
        (const unsigned char*) (func->isrc()->getPtrToInstruction(addr));
    InstructionDecoder             dec(buffer, size, block->region()->getArch());
    Dyninst::InsnAdapter::IA_IAPI* ahPtr =
        Dyninst::InsnAdapter::IA_IAPI::makePlatformIA_IAPI(
            func->obj()->cs()->getArch(), dec, addr, func->obj(), block->region(),
            func->isrc(), NULL);
