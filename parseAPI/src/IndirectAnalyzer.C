...
bool IndirectControlFlowAnalyzer::NewJumpTableAnalysis(
    std::vector<std::pair<Address, Dyninst::ParseAPI::EdgeTypeEnum> >
        &outEdges) {
  //    if (block->last() == 0xacef04) dyn_debug_parsing = 1; else
  //    dyn_debug_parsing=0;
  ...
  const unsigned char *buf =
      (const unsigned char *)block->obj()->cs()->getPtrToInstruction(
          block->last());
  ...
  if (jumpTableOutEdges.empty() && jtp.jumpTableFormat) {
    ...
    BoundFactsCalculator bfc(func, g, func->entry() == block, rf, thunks, true,
                             jtp.expandCache);
  ...
  outEdges.insert(outEdges.end(), jumpTableOutEdges.begin(),
                  jumpTableOutEdges.end());
...
void IndirectControlFlowAnalyzer::GetAllReachableBlock() {
  ...
  while (!q.empty()) {
    ...
    for (auto eit = cur->sources().begin(); eit != cur->sources().end(); ++eit)
      if ((*eit)->intraproc()) q.push((*eit)->src());
  }
}

static Address ThunkAdjustment(Address afterThunk, MachRegister reg,
                               ParseAPI::Block *b) {
  ...
  const unsigned char *buf =
      (const unsigned char *)(b->obj()->cs()->getPtrToInstruction(afterThunk));
...
void IndirectControlFlowAnalyzer::FindAllThunks() {
  ...
  for (auto bit = reachable.begin(); bit != reachable.end(); ++bit) {
    ...
    // end a basic block. So, we need to check every instruction to find all
    // thunks
    ParseAPI::Block *b = *bit;
    const unsigned char *buf =
        (const unsigned char *)(b->obj()->cs()->getPtrToInstruction(
            b->start()));
    if (buf == NULL) {
    ...
    parsing_printf("Looking for thunk in block [%lx,%lx).", b->start(),
                   b->end());
    InstructionDecoder dec(buf, b->end() - b->start(),
                           b->obj()->cs()->getArch());
    InsnAdapter::IA_IAPI block(dec, b->start(), b->obj(), b->region(),
                               b->obj()->cs(), b);
    while (block.getAddr() < b->end()) {
      if (block.getInstruction()->getCategory() == c_CallInsn &&
        ...
        const unsigned char *target =
            (const unsigned char *)b->obj()->cs()->getPtrToInstruction(addr);
        InstructionDecoder targetChecker(
            target, InstructionDecoder::maxInstructionLength,
            b->obj()->cs()->getArch());
