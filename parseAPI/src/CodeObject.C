...
{
// initialization help
static inline CFGFactory*
__fact_init(CFGFactory* fact)
{
    if(fact)
        return fact;
...
void
CodeObject::version(int& major, int& minor, int& maintenance)
{
...
CodeObject::CodeObject(CodeSource* cs, CFGFactory* fact, ParseCallback* cb, bool defMode,
                       bool ignoreParse)
: _cs(cs)
...
#pragma omp parallel for schedule(auto)
    for(int i = 0; i < size; ++i)
    {
        ...
        if(!cs()->regionsOverlap())
            f = parser->factory()._mkfunc(hints[i]._addr, HINT, hints[i]._name, this, cr,
                                          cs());
        else
            f = parser->factory()._mkfunc(hints[i]._addr, HINT, hints[i]._name, this, cr,
                                          cr);
...
int
CodeObject::findCurrentBlocks(CodeRegion* cr, Address addr, set<Block*>& blocks)
{
    assert(parser);
...
int
CodeObject::findCurrentFuncs(CodeRegion* cr, Address addr, set<Function*>& funcs)
{
...
CodeObject::parseNewEdges(vector<NewEdgeToParse>& worklist)
{
    ...
    for(unsigned idx = 0; idx < worklist.size(); idx++)
    {
        ...
        if(trgB)
        {
            ...
            {
                if((*tit)->trg() == trgB && (*tit)->type() == worklist[idx].edge_type)
                {
                    edgeExists = true;
                }
            }
        ...
        else
        {
            ...
            if(worklist[idx].checked && worklist[idx].edge_type == CALL_FT)
            {
                elem = bundle->add(new ParseWorkElem(
                    bundle, ParseWorkElem::checked_call_ft,
                    parser->link_tempsink(worklist[idx].source, worklist[idx].edge_type),
                    worklist[idx].source->last(), worklist[idx].target, true, false));
            }
            else
            {
                elem = bundle->add(new ParseWorkElem(
    ...
    if(defensiveMode())
    {
        ...
        for(unsigned idx = 0; idx < parsedTargs.size(); idx++)
        {
            ...
            for(set<Function*>::iterator fit = tfuncs.begin(); fit != tfuncs.end(); fit++)
            {
...
bool
CodeObject::isIATcall(Address insnAddr, std::string& calleeName)
{
    // find region
...
void
CodeObject::registerCallback(ParseCallback* cb)
{
    assert(_pcb);
...
void
CodeObject::unregisterCallback(ParseCallback* cb)
{
