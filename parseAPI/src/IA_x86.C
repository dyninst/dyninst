...
bool IA_IAPI::isFrameSetupInsn(Instruction::Ptr i) const {
  if (i->getOperation().getID() == e_mov) {
    ...
    if (i->isRead(stackPtr[_isrc->getArch()]) &&
      ...
      if ((unsigned)i->getOperand(0).getValue()->size() ==
          _isrc->getAddressWidth()) {
        return true;
      } else {
        parsing_printf(
            "%s[%d]: discarding insn %s as stack frame preamble, size mismatch "
            "for %d-byte addr width\n",
...
class nopVisitor : public InstructionAPI::Visitor {
 public:
  nopVisitor()
      : foundReg(false), foundImm(false), foundBin(false), isNop(true) {}
  virtual ~nopVisitor() {}
...
bool isNopInsn(Instruction::Ptr insn) {
  // TODO: add LEA no-ops
  if (insn->getOperation().getID() == e_nop) return true;
...
bool IA_IAPI::isThunk() const {
  ...
  InstructionDecoder targetChecker(
      target, 2 * InstructionDecoder::maxInstructionLength, _isrc->getArch());
...
bool IA_IAPI::isTailCall(Function *context, EdgeTypeEnum type, unsigned int,
  ...
  if (target == NULL) {
    ...
    if (blocks.size() == 1) {
      ...
      // If this case happens, it means the jump goes into overlapping
      // instruction streams,
  ...
  if (curInsn()->getCategory() == c_BranchInsn && valid && callee &&
      callee != context && target && !context->contains(target)) {
  ...
  if (curInsn()->getCategory() == c_BranchInsn && valid && !callee) {
    if (target) {
      parsing_printf(
          "\tjump to 0x%lx is known block, but not func entry, NOT TAIL CALL\n",
          addr);
      tailCalls[type] = false;
      return false;
  ...
  if ((curInsn()->getCategory() == c_BranchInsn)) {
    ...
    if (prevInsn->getOperation().getID() == e_leave) {
      ...
      if (prevInsn->isWritten(stackPtr[_isrc->getArch()])) {
        ...
        if (call_fallthrough) {
          parsing_printf(
              "\tprev insn was %s, but it is the next instruction of a "
              "function call, not a tail call %x %x\n",
              prevInsn->format().c_str());
        } else {
          parsing_printf("\tprev insn was %s, TAIL CALL\n",
                         prevInsn->format().c_str());
...
bool IA_IAPI::isReturn(Dyninst::ParseAPI::Function * /*context*/,
  ...
  // an indirect jump used for jump tables etc. Hence, we need to function and
  // block
  // to determine if an instruction is a return. But these parameters are unused
  // for x86.
...
bool IA_IAPI::isFakeCall() const {
  ...
  if (!_isrc->isCode(entry)) {
    mal_printf(
        "WARNING: found function call at %lx "
  ...
  while (insn->getCategory() == c_CallInsn ||
    ...
    if (!valid || !_cr->contains(entry) || !_isrc->isCode(entry)) {
      mal_printf(
          "WARNING: found call to function at %lx that "
    ...
    delete (ah);
    newdec =
        InstructionDecoder(bufPtr, _cr->length() - entryOff, _cr->getArch());
    ...
    if (insn->isWritten(theStackPtr)) {
      ...
      switch (what) {
        ...
        case e_popad:
          if (1 == sign) {
            mal_printf(
                ...
                "by %d. %s[%d]\n",
                ah->getAddr(), entry, 8 * sign * addrWidth, FILE__, __LINE__);
          }
        ...
        case e_popfd:
          ...
          if (1 == sign) {
            mal_printf(
                "popf ins'n at %lx in func at %lx changes sp "
                "by %d. %s[%d]\n",
        ...
        case e_and:
          ...
          mal_printf(
              "WARNING: saw and instruction at %lx that is not handled by "
              "isFakeCall %s[%d]\n",
              curAddr, FILE__, __LINE__);
          delete ah;
          return false;
