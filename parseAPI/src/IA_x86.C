...
IA_x86::IA_x86(Dyninst::InstructionAPI::InstructionDecoder dec_, Address start_,
...
IA_x86::IA_x86(const IA_x86& rhs)
: IA_IAPI(rhs)
{}
...
bool
IA_x86::isFrameSetupInsn(Instruction i) const
{
    if(i.getOperation().getID() == e_mov)
    {
           ...
           i.isWritten(framePtr[_isrc->getArch()]))
        {
            ...
            {
                parsing_printf("%s[%d]: discarding insn %s as stack frame preamble, size "
                               "mismatch for %u-byte addr width\n",
                               FILE__, __LINE__, i.format().c_str(),
                               _isrc->getAddressWidth());
            }
        }
    }
    return false;
...
public:
    ...
    : offset_(0)
    {}
    virtual void visit(BinaryFunction*) { return; }
    virtual void visit(Immediate* i) { offset_ = i->eval().convert<Address>(); }
    virtual void visit(RegisterAST*) { return; }
    virtual void visit(Dereference*) { return; }
...
IA_x86::isThunk() const
{
    ...
    const unsigned char* target = (const unsigned char*) _isrc->getPtrToInstruction(addr);
...
bool
IA_x86::isTailCall(const Function* context, EdgeTypeEnum type, unsigned int,
    ...
    if((curInsn().getCategory() == c_BranchInsn))
    {
        ...
        else if(prevInsn.getOperation().getID() == e_add)
        {
            if(prevInsn.isWritten(stackPtr[_isrc->getArch()]))
            {
                ...
                {
                    for(auto eit = _curBlk->sources().begin();
                        eit != _curBlk->sources().end(); ++eit)
                    {
                        if((*eit)->type() == CALL_FT)
                        {
                ...
                if(call_fallthrough)
                {
                    parsing_printf("\tprev insn was %s, but it is the next instruction "
                                   "of a function call, not a tail call\n",
                                   prevInsn.format().c_str());
                }
                else
                {
...
bool
IA_x86::isStackFramePreamble() const
{
#if defined(os_windows)
...
bool
IA_x86::cleansStack() const
{
    Instruction ci = curInsn();
    if(ci.getCategory() != c_ReturnInsn)
        return false;
...
bool
IA_x86::isReturn(Dyninst::ParseAPI::Function* /*context*/,
                 Dyninst::ParseAPI::Block* /*currBlk*/) const
{
    ...
    // to determine if an instruction is a return. But these parameters are unused for
    // x86.
    return curInsn().getCategory() == c_ReturnInsn;
}

bool
IA_x86::isReturnAddrSave(Dyninst::Address&) const
{
...
bool
IA_x86::sliceReturn(ParseAPI::Block* /*bit*/, Address /*ret_addr*/,
...
IA_x86::isFakeCall() const
{
    ...
    InstructionDecoder newdec(bufPtr, _cr->length() - entryOff, _cr->getArch());
    ...
    while(insn.getCategory() == c_CallInsn || insn.getCategory() == c_BranchInsn)
    {
        ...
        delete(ah);
        newdec = InstructionDecoder(bufPtr, _cr->length() - entryOff, _cr->getArch());
        ...
        if(insn.isWritten(theStackPtr))
        {
            ...
            switch(what)
            {
                ...
                case e_popfd:
                    ...
                    if(1 == sign)
                    {
                        mal_printf("popf ins'n at %lx in func at %lx changes sp "
                                   "by %d. %s[%d]\n",
                                   ah->getAddr(), entry, sign * 4, FILE__, __LINE__);
                    }
                    break;
                case e_enter:
                ...
                case e_and:
                    ...
                    mal_printf("WARNING: saw and instruction at %lx that is not handled "
                               "by isFakeCall %s[%d]\n",
                               curAddr, FILE__, __LINE__);
                    delete ah;
                    return false;
                ...
                default: {
                    fprintf(stderr,
                            "WARNING: in isFakeCall non-push/pop "
...
bool
IA_x86::isIATcall(std::string& calleeName) const
{
    if(!isDynamicCall())
    {
        return false;
    ...
    do
    {
        ...
        count++;
    } while(count < 100 && _obj->cs()->isValidAddress(funcAsciiAddr + count) &&
