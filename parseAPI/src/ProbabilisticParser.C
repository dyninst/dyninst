...
struct ProbAndAddr
{
    ...
    ProbAndAddr(Address a, double p)
    : addr(a)
    , prob(p)
    {}
    bool operator<(const ProbAndAddr& p) const
    {
        if(double_cmp(prob, p.prob) == 0)
            return addr > p.addr;
        return prob < p.prob;
    }
};
static bool
MatchArgs(unsigned short arg1, unsigned short arg2)
{
    return arg1 == arg2;
}

bool
IdiomTerm::match(const IdiomTerm& it) const
{
...
bool
IdiomTerm::matchOpcode(unsigned short eid) const
{
...
static string
HandleAnOperand(unsigned short arg, int style)
{
    if(arg != NOARG)
    {
        if(arg == MEMARG)
...
IdiomTerm::human_format() const
{
    ...
    else
    {
        ...
        if(arg1 != NOARG)
            entryname += " " + HandleAnOperand(arg1, 1);
        if(arg2 != NOARG)
            entryname += "," + HandleAnOperand(arg2, 1);
    }
...
bool
Idiom::operator<(const Idiom& i) const
{
    if(terms.size() != i.terms.size())
        return terms.size() < i.terms.size();
...
static void
split(const char* str, vector<uint64_t>& terms)
{
...
void
IdiomPrefixTree::addIdiom(const Idiom& idiom)
{
    addIdiom(0, idiom);
}

void
IdiomPrefixTree::addIdiom(int cur, const Idiom& idiom)
{
...
bool
IdiomPrefixTree::findChildrenWithOpcode(unsigned short entry_id, ChildrenType& ret)
{
...
bool
IdiomPrefixTree::findChildrenWithArgs(unsigned short arg1, unsigned short arg2,
                                      const ChildrenType& candidate, ChildrenType& ret)
{
...
const IdiomPrefixTree::ChildrenType*
IdiomPrefixTree::getChildrenByEntryID(unsigned short entry_id)
{
...
const IdiomPrefixTree::ChildrenType*
IdiomPrefixTree::getWildCardChildren()
{
    return getChildrenByEntryID(WILDCARD_ENTRY_ID);
}
ProbabilityCalculator::ProbabilityCalculator(CodeRegion* reg, CodeSource* source,
                                             Parser* p, string model_spec)
: model(model_spec)
, cr(reg)
, cs(source)
, parser(p)
...
static bool
PassPreCheck(unsigned char* buf)
{
    if(buf == NULL)
        return false;
    if(*buf == 0 || *buf == 0x90)
...
double
ProbabilityCalculator::calcProbByMatchingIdioms(Address addr)
{
...
void
ProbabilityCalculator::calcProbByEnforcingConstraints()
{
    // Initialize our knowledge about non-gap functions
    finalized.clear();
    for(auto fit = parser->obj().funcs().begin(); fit != parser->obj().funcs().end();
        ++fit)
    {
    ...
    // Use the prob from matching idioms and push all the FEP candidates into the priority
    // queue
    ...
    while(!q.empty())
    {
        ...
        // constraints.
        if(double_cmp(FEPProb[pa.addr], pa.prob) != 0)
            continue;
        ...
        if(enforceOverlappingConstraints(f, pa.addr, pa.prob, newFEPProb, newReachingProb,
                                         newDiscoveredFuncs))
        {
...
double
ProbabilityCalculator::getFEPProb(Address addr)
{
    if(FEPProb.find(addr) != FEPProb.end())
        return FEPProb[addr];
    return 0;
...
double
ProbabilityCalculator::getReachingProb(Address addr)
{
    if(reachingProb.find(addr) != reachingProb.end())
        return reachingProb[addr];
    return 0;
}

bool
ProbabilityCalculator::isFEP(Address addr)
{
...
double
ProbabilityCalculator::calcForwardWeights(int cur, Address addr, IdiomPrefixTree* tree,
                                          bool& valid)
{
    if(addr >= cr->high())
        return 0;
...
double
ProbabilityCalculator::calcBackwardWeights(int cur, Address addr, IdiomPrefixTree* tree,
                                           set<IdiomPrefixTree*>& matched)
{
    double w = 0;
    if(tree->isFeature())
    ...
    for(Address prevAddr = addr - 1; prevAddr >= cr->low() && addr - prevAddr <= 15;
        --prevAddr)
    {
        DecodeData data;
        if(!decodeInstruction(data, prevAddr))
            continue;
        if(prevAddr + data.len != addr)
            continue;
...
bool
ProbabilityCalculator::decodeInstruction(DecodeData& data, Address addr)
{
...
bool
ProbabilityCalculator::enforceOverlappingConstraints(
    ...
    dyn_hash_set<Function*>&       newDiscoveredFuncs)
{
    ...
    if(finalized.find(f) != finalized.end())
        return true;
    if(newDiscoveredFuncs.find(f) != newDiscoveredFuncs.end())
        return true;
    ...
    for(auto eit = call_edges.begin(); eit != call_edges.end(); ++eit)
    {
        ...
        if(reachingProb.find(target) == reachingProb.end())
            continue;
...
void
ProbabilityCalculator::Finalize(dyn_hash_map<Address, double>& newFEPProb,
                                dyn_hash_map<Address, double>& newReachingProb,
                                dyn_hash_set<Function*>&       newDiscoveredFuncs)
{
...
void
ProbabilityCalculator::Remove(dyn_hash_set<Function*>& newDiscoveredFuncs)
{
...
void
ProbabilityCalculator::prioritizedGapParsing()
{
    priority_queue<ProbAndAddr> q;
    double                      threshold = model.getProbThreshold();
