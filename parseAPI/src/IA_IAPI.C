...
IA_IAPI&
IA_IAPI::operator=(const IA_IAPI& rhs)
{
    dec      = rhs.dec;
    allInsns = rhs.allInsns;
    // curInsnIter = allInsns.find(rhs.curInsnIter->first);
...
IA_IAPI*
IA_IAPI::makePlatformIA_IAPI(Architecture arch, InstructionDecoder dec_, Address where_,
                             CodeObject* o, CodeRegion* r, InstructionSource* isrc,
                             Block* curBlk_)
{
    switch(arch)
    {
...
IA_IAPI::IA_IAPI(InstructionDecoder dec_, Address where_, CodeObject* o, CodeRegion* r,
                 InstructionSource* isrc, Block* curBlk_)
...
, cachedLinkerStubState(false)
{
    ...
    // boost::tuples::tie(curInsnIter, boost::tuples::ignore) =
...
void
IA_IAPI::reset(InstructionDecoder dec_, Address start, CodeObject* o, CodeRegion* r,
               InstructionSource* isrc, Block* curBlk_)
{
    // reset the base
...
{
    //    if(!curInsn()) {
    //        parsing_printf("..... WARNING: failed to advance InstructionAdapter at
    //        0x%lx, allInsns.size() = %d\n", current,
    //                       allInsns.size());
    //        return;
    //    }
...
{
    //    if(!curInsn()) {
    //        parsing_printf("..... WARNING: failed to retreat InstructionAdapter at
    //        0x%lx, allInsns.size() = %d\n", current,
    //                       allInsns.size());
    //        return false;
    //    }
    ...
    {
        parsing_printf("..... WARNING: cowardly refusal to retreat past first "
...
IA_IAPI::hasCFT() const
{
    ...
    if(c == c_BranchInsn || c == c_ReturnInsn)
    {
...
IA_IAPI::isGarbageInsn() const
{
    ...
    if(unlikely(_obj->defensiveMode()))
    {
        ...
        {
            case e_arpl:
                cerr << "REACHED AN ARPL AT " << std::hex << current << std::dec
                     << " COUNTING AS INVALID" << endl;
                ret = true;
                break;
            case e_fisub:
                cerr << "REACHED A FISUB AT " << std::hex << current << std::dec
                     << " COUNTING AS INVALID" << endl;
                ret = true;
                break;
            case e_into:
                cerr << "REACHED AN INTO AT " << std::hex << current << std::dec
                     << " COUNTING AS INVALID" << endl;
                ret = true;
                break;
            case e_mov: {
                ...
                for(set<RegisterAST::Ptr>::iterator rit = regs.begin(); rit != regs.end();
                    rit++)
                {
                    if(Dyninst::isSegmentRegister((*rit)->getID().regClass()))
                    {
                        cerr << "REACHED A MOV SEGMENT INSN AT " << std::hex << current
            ...
            case e_add:
                if(2 == curInsn().size() && 0 == curInsn().rawByte(0) &&
                   0 == curInsn().rawByte(1))
                {
                ...
                break;
            case e_push:  // pushes of segment registers do not occur frequently in real
                          // code (and crash Rose)
                ...
                switch(curInsn().rawByte(0))
                {
                    ...
                    case 0x1e:
                        ret = true;
                        cerr << "REACHED A PUSH OF A SEGMENT REGISTER " << std::hex
                             << current << std::dec << " COUNTING AS INVALID" << endl;
                        break;
                    case 0x0f:
                        if(2 == curInsn().size() && ((0xa0 == curInsn().rawByte(1)) ||
                                                     (0xa8 == curInsn().rawByte(1))))
                        {
                            ret = true;
                            cerr << "REACHED A 2-BYTE PUSH OF A SEGMENT REGISTER "
                                 << std::hex << current << std::dec
                                 << " COUNTING AS INVALID" << endl;
                        }
                        break;
                    default:
...
bool
IA_IAPI::isFrameSetupInsn() const
{
    return isFrameSetupInsn(curInsn());
}

bool
IA_IAPI::isDynamicCall() const
{
...
bool
IA_IAPI::isBranch() const
{
    return curInsn().getCategory() == c_BranchInsn;
}
bool
IA_IAPI::isCall() const
{
...
void
IA_IAPI::parseSyscall(std::vector<std::pair<Address, EdgeTypeEnum>>& outEdges) const
{
    parsing_printf("[%s:%d] Treating syscall as call to sink w/ possible FT edge to next "
                   "insn at 0x%lx\n",
...
void
IA_IAPI::parseSysEnter(std::vector<std::pair<Address, EdgeTypeEnum>>& outEdges) const
{
    ...
    {
        parsing_printf("[%s:%d] Treating sysenter as call to kernel w/normal return to "
                       "next insn at 0x%lx\n",
                       FILE__, __LINE__, getAddr());
        outEdges.push_back(std::make_pair(getNextAddr(), CALL_FT));
    }
...
void
IA_IAPI::getNewEdges(std::vector<std::pair<Address, EdgeTypeEnum>>& outEdges,
                     Function* context, Block* currBlk, unsigned int num_insns,
                     ...
                     const set<Address>&                 knownTargets) const
{
    ...
    if(ci.getCategory() == c_CallInsn)
    {
        ...
        if(callEdge)
        {
            ...
            else
            {
                ...
                outEdges.push_back(
                    std::make_pair(std::numeric_limits<Address>::max(), CALL));
            }
        }
    ...
    else if(ci.getCategory() == c_BranchInsn)
    {
            ...
            if(!isTailCall(context, DIRECT, num_insns, knownTargets))
            {
                ...
                else
                {
                    parsing_printf("%s[%d]: PLT tail call to %lx (%s)\n", FILE__,
                                   __LINE__, target, (*plt_entries)[target].c_str());
                    outEdges.push_back(std::make_pair(target, DIRECT));
        ...
        else
        {
            ...
            if(num_insns == 2)
            {
                ...
                {
                    parsing_printf("\tIndirect branch as 2nd insn of entry block, "
                                   "treating as tail call\n");
                    parsing_printf("%s[%d]: indirect tail call %s at 0x%lx\n", FILE__,
                                   __LINE__, ci.format().c_str(), current);
            ...
            {
                outEdges.push_back(std::make_pair((Address) -1, INDIRECT));
                parsing_printf("%s[%d]: unparsed jump table %s at 0x%lx in function %s "
                               "UNINSTRUMENTABLE\n",
    ...
    else if(ci.getCategory() == c_ReturnInsn)
    {
        ...
        else if(!isReturn(context, currBlk))
        {
            ...
            if(!successfullyParsedJumpTable || outEdges.empty())
            {
                parsing_printf("%s[%d]: BLR unparsed jump table %s at 0x%lx in function "
                               "%s UNINSTRUMENTABLE\n",
                               FILE__, __LINE__, ci.format().c_str(), current,
                               context->name().c_str());
                outEdges.push_back(std::make_pair((Address) -1, INDIRECT));
...
bool
IA_IAPI::isIPRelativeBranch() const
{
    ...
    if(ci.getCategory() == c_BranchInsn && !valid)
    {
        Expression::Ptr cft = ci.getControlFlowTarget();
...
const Instruction&
IA_IAPI::curInsn() const
{
    return curInsnIter->second;
}

bool
IA_IAPI::isLeave() const
{
    Instruction ci = curInsn();
...
bool
IA_IAPI::isDelaySlot() const
{
    return false;
}

const Instruction&
IA_IAPI::getInstruction() const
{
    return curInsn();
}

bool
IA_IAPI::isRealCall() const
{
...
bool
IA_IAPI::isConditional() const
{
    return curInsn().allowsFallThrough();
}

bool
IA_IAPI::simulateJump() const
{
...
IA_IAPI::getCFT() const
{
    ...
    parsing_printf("%s[%d]: binding PC %s in %s to 0x%lx...", FILE__, __LINE__,
                   thePC[_isrc->getArch()]->format(curInsn().getArch()).c_str(),
...
bool
IA_IAPI::isRelocatable(InstrumentableLevel lvl) const
{
    Instruction ci = curInsn();
    if(ci.isValid() && (ci.getCategory() == c_CallInsn))
    {
...
InstrumentableLevel
IA_IAPI::getInstLevel(Function* context, unsigned int num_insns) const
{
