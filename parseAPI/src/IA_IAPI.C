...
IA_IAPI::IA_IAPI(InstructionDecoder dec_, Address where_, CodeObject *o,
  ...
  curInsnIter =
      allInsns.insert(allInsns.end(), std::make_pair(current, dec.decode()));

  initASTs();
}

void IA_IAPI::reset(InstructionDecoder dec_, Address start, CodeObject *o,
                    CodeRegion *r, InstructionSource *isrc, Block *curBlk_) {
  // reset the base
...
void IA_IAPI::advance() {
  if (!curInsn()) {
    parsing_printf(
        "..... WARNING: failed to advance InstructionAdapter at 0x%lx, "
        "allInsns.size() = %d\n",
        current, allInsns.size());
    return;
  }
...
bool IA_IAPI::retreat() {
  if (!curInsn()) {
    parsing_printf(
        "..... WARNING: failed to retreat InstructionAdapter at 0x%lx, "
        "allInsns.size() = %d\n",
        current, allInsns.size());
    return false;
  }
  ...
  if (curInsnIter != allInsns.begin()) {
    ...
    parsing_printf(
        "..... WARNING: cowardly refusal to retreat past first instruction at "
        "0x%lx\n",
        current);
    return false;
  }
...
bool IA_IAPI::hasCFT() const {
  ...
  if (hascftstatus.first) {
    parsing_cerr << "\t Returning cached entry: " << hascftstatus.second
                 << endl;
  ...
  if (unlikely(_obj->defensiveMode())) {
    ...
    switch (e) {
      ...
      case e_add:
        ...
        break;
      case e_push:  // pushes of segment registers do not occur frequently in
                    // real code (and crash Rose)
        ...
        switch (curInsn()->rawByte(0)) {
          ...
          case 0x0f:
            if (2 == curInsn()->size() && ((0xa0 == curInsn()->rawByte(1)) ||
              ...
              ret = true;
              cerr << "REACHED A 2-BYTE PUSH OF A SEGMENT REGISTER " << std::hex
                   << current << std::dec << " COUNTING AS INVALID" << endl;
            }
            break;
          default:
...
bool IA_IAPI::isFrameSetupInsn() const { return isFrameSetupInsn(curInsn()); }
...
bool IA_IAPI::isCall() const { return curInsn()->getCategory() == c_CallInsn; }
...
void IA_IAPI::parseSyscall(
    std::vector<std::pair<Address, EdgeTypeEnum> > &outEdges) const {
  parsing_printf(
      "[%s:%d] Treating syscall as call to sink w/ possible FT edge to next "
      "insn at 0x%lx\n",
...
void IA_IAPI::parseSysEnter(
  ...
  if (scratch.curInsn()->getCategory() == c_BranchInsn) {
    ...
    parsing_printf(
        "[%s:%d] Treating sysenter as call to kernel w/normal return to next "
        "insn at 0x%lx\n",
        FILE__, __LINE__, getAddr());
    outEdges.push_back(std::make_pair(getNextAddr(), CALL_FT));
  }
...
void IA_IAPI::getNewEdges(
  ...
  if (ci->getCategory() == c_CallInsn) {
    ...
    if (unlikely(_obj->defensiveMode())) {
      if (!success || isDynamicCall()) {
        std::string empty;
        if (!isIATcall(empty)) ftEdge = false;
      } else if (!_isrc->isValidAddress(target)) {
        ftEdge = false;
      }
    }

    if (callEdge) outEdges.push_back(std::make_pair(target, CALL));
    if (ftEdge)
    ...
    // Therefore, we need to check for every nop and first non-nop instruction
    // after the jump for catch blocks
    IA_IAPI tmp_ah(*this);
    tmp_ah.advance();
    Address catchStart;
    bool found = false;
    ...
    if (found || (tmp_ah.curInsn() &&
                  _cr->findCatchBlock(tmp_ah.getAddr(), catchStart))) {
    ...
    if (valid) {
      if (!isTailCall(context, DIRECT, num_insns, knownTargets)) {
        if (plt_entries->find(target) == plt_entries->end()) {
          ...
          parsing_printf("%s[%d]: PLT tail call to %x (%s)\n", FILE__, __LINE__,
                         target, (*plt_entries)[target].c_str());
          outEdges.push_back(std::make_pair(target, DIRECT));
      ...
      if (num_insns == 2) {
        ...
        // Anything that tries to go enum->jump table *should* need more than
        // two
        // instructions and has not been seen in the wild....
        if (currBlk == context->entry()) {
          parsing_printf(
              "\tIndirect branch as 2nd insn of entry block, treating as tail "
              "call\n");
      ...
      if (!successfullyParsedJumpTable || outEdges.empty()) {
        outEdges.push_back(std::make_pair((Address)-1, INDIRECT));
        parsing_printf(
            "%s[%d]: unparsed jump table %s at 0x%lx in function %s "
            "UNINSTRUMENTABLE\n",
    ...
    if (ci->allowsFallThrough()) {
      ...
      if (!successfullyParsedJumpTable || outEdges.empty()) {
        parsing_printf(
            "%s[%d]: BLR unparsed jump table %s at 0x%lx in function %s "
            "UNINSTRUMENTABLE\n",
            FILE__, __LINE__, ci->format().c_str(), current,
            context->name().c_str());
        outEdges.push_back(std::make_pair((Address)-1, INDIRECT));
    ...
    // there is possibility that the exception table entry points a nop.
    // Therefore, we need to check for every nop and first non-nop instruction
    // after the return for catch blocks
...
Instruction::Ptr IA_IAPI::curInsn() const { return curInsnIter->second; }
...
bool IA_IAPI::isDelaySlot() const { return false; }

Instruction::Ptr IA_IAPI::getInstruction() const { return curInsn(); }
...
bool IA_IAPI::isConditional() const { return curInsn()->allowsFallThrough(); }
...
bool IA_IAPI::simulateJump() const {
  ...
  // TODO: we don't simulate jumps on x86 architectures; add logic as we need
  // it.
  return false;
}
...
std::pair<bool, Address> IA_IAPI::getCFT() const {
  ...
  parsing_printf("%s[%d]: binding PC %s in %s to 0x%x...", FILE__, __LINE__,
                 thePC[_isrc->getArch()]->format().c_str(),
...
bool IA_IAPI::parseJumpTable(
  ...
  IA_platformDetails *jumpTableParser =
      makePlatformDetails(_isrc->getArch(), this);
...
InstrumentableLevel IA_IAPI::getInstLevel(Function *context,
                                          unsigned int num_insns) const {
  InstrumentableLevel ret =
      InstructionAdapter::getInstLevel(context, num_insns);
