...
LoopAnalyzer::LoopAnalyzer(const Function* f) : func(f) {
...
void LoopAnalyzer::insertCalleeIntoLoopHierarchy(Function* callee,
  ...
  bool success =
      dfsInsertCalleeIntoLoopHierarchy(func->_loop_root, callee, addr);
...
bool LoopAnalyzer::analyzeLoops() {
  ...
  for (auto bit = func->blocks().begin(); bit != func->blocks().end(); ++bit) {
    Block* b = *bit;
    if (header[b] == NULL) {
      // if header[b] == NULL, b is either the header of a outermost loop, or
      // not in any loop
      createLoops(b);
    }
  }
  ...
  for (auto bit = func->blocks().begin(); bit != func->blocks().end(); ++bit) {
    Block* b = *bit;
    if (loops[b] != NULL) func->_loops.insert(loops[b]);
  }
...
Block* LoopAnalyzer::WMZC_DFS(Block* b0, int pos) {
  ...
  edge_sort es;
  visitOrder.insert(visitOrder.end(), b0->targets().begin(),
                    b0->targets().end());
  ...
  for (auto eit = b0->targets().begin(); eit != b0->targets().end(); ++eit) {
    ...
    if (visited.find(b) == visited.end()) {
      ...
      if (DFSP_pos[b] > 0) {
        // case B
        if (loops[b] == NULL) loops[b] = new Loop(func);
        WMZC_TagHead(b0, b);
...
void LoopAnalyzer::FillMoreBackEdges(Loop* loop) {
  ...
  for (auto bit = loop->exclusiveBlocks.begin();
       bit != loop->exclusiveBlocks.end(); ++bit) {
    Block* b = *bit;
    for (auto eit = b->targets().begin(); eit != b->targets().end(); ++eit) {
      Edge* e = *eit;
  ...
  for (auto bit = loop->childBlocks.begin(); bit != loop->childBlocks.end();
       ++bit) {
    Block* b = *bit;
    for (auto eit = b->targets().begin(); eit != b->targets().end(); ++eit) {
      Edge* e = *eit;
