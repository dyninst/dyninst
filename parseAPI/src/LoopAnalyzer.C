...
void
LoopAnalyzer::dfsCreateLoopHierarchy(LoopTreeNode* parent, vector<Loop*>& loops_,
                                     std::string level_)
{
    for(unsigned int i = 0; i < loops_.size(); i++)
    {
        // loop name is hierarchical level
        std::string clevel = (level_ != "") ? level_ + "." + utos(i + 1) : utos(i + 1);
...
void
LoopAnalyzer::createLoopHierarchy()
{
    ...
    for(auto bit = const_cast<const Function*>(func)->blocks().begin();
        bit != const_cast<const Function*>(func)->blocks().end(); ++bit)
    {
        Block* b = *bit;
        for(auto eit = b->targets().begin(); eit != b->targets().end(); ++eit)
        {
            ...
            if((*eit)->type() == CALL)
            {
                ...
                Function* callee =
                    target->obj()->findFuncByEntry(target->region(), target->start());
                if(callee)
                    insertCalleeIntoLoopHierarchy(callee, b->last());
                if(b->obj()->cs()->getArch() == Arch_ppc64)
                {
                    // Since Power 8, the new ABI will typically create two entries for a
                    // function
                    callee = target->obj()->findFuncByEntry(target->region(),
                                                            target->start() - 8);
                    if(callee == NULL)
                        continue;
                    // Make sure that the function entry block indeed aligns with the call
                    // target
                    if(callee->entry()->end() != target->start())
                        continue;
                    // Make sure that the function entry block will fall through to the
                    // call target
                    bool findFT = false;
...
bool
LoopAnalyzer::dfsInsertCalleeIntoLoopHierarchy(LoopTreeNode* node, Function* callee,
                                               unsigned long addr)
{
...
void
LoopAnalyzer::insertCalleeIntoLoopHierarchy(Function* callee, unsigned long addr)
{
...
LoopAnalyzer::analyzeLoops()
{
    ...
    for(auto bit = func->blocks().begin(); bit != func->blocks().end(); ++bit)
    {
        ...
        if(header[b] == NULL)
        {
            // if header[b] == NULL, b is either the header of a outermost loop, or not in
            // any loop
            createLoops(b);
        }
    }
...
Block*
LoopAnalyzer::WMZC_DFS(Block* b0, int pos)
{
    visited.insert(b0);
    DFSP_pos[b0] = pos;
    ...
    {
        if((*eit)->interproc() || (*eit)->sinkEdge() || (*eit)->type() == CATCH)
            continue;
        Block* b = (*eit)->trg();
...
void
LoopAnalyzer::WMZC_TagHead(Block* b, Block* h)
{
    if(b == h || h == NULL)
        return;
    Block *cur1, *cur2;
...
void
LoopAnalyzer::createLoops(Block* cur)
{
