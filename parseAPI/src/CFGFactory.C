...
Edge::Edge(Block *source, Block *target, EdgeTypeEnum type)
    : _source(source), _target(target), _type(type, false) {}

CFGFactory::~CFGFactory() { destroy_all(); }

// ParseAPI call...
Function *CFGFactory::_mkfunc(Address addr, FuncSource src, string name,
...
Function *CFGFactory::mkfunc(Address addr, FuncSource, string name,
                             CodeObject *obj, CodeRegion *reg,
                             Dyninst::InstructionSource *isrc) {
...
Block *CFGFactory::_mkblock(Function *f, CodeRegion *r, Address addr) {
...
Block *CFGFactory::mkblock(Function *f, CodeRegion *r, Address addr) {
  Block *ret = new Block(f->obj(), r, addr);
  return ret;
}

Block *CFGFactory::_mksink(CodeObject *obj, CodeRegion *r) {
  Block *ret = mksink(obj, r);
  blocks_.add(*ret);
  return ret;
}

Block *CFGFactory::mksink(CodeObject *obj, CodeRegion *r) {
  Block *ret = new Block(obj, r, numeric_limits<Address>::max());
  return ret;
}

Edge *CFGFactory::_mkedge(Block *src, Block *trg, EdgeTypeEnum type) {
  Edge *ret = mkedge(src, trg, type);
  edges_.add(*ret);
  return ret;
}

Edge *CFGFactory::mkedge(Block *src, Block *trg, EdgeTypeEnum type) {
...
void CFGFactory::free_func(Function *f) { delete f; }

void CFGFactory::destroy_block(Block *b) {
  b->remove();
  free_block(b);
}

void CFGFactory::free_block(Block *b) { delete b; }

void CFGFactory::destroy_edge(Edge *e) {
  e->remove();
  free_edge(e);
}

void CFGFactory::free_edge(Edge *e) { delete e; }

void CFGFactory::destroy_all() {
