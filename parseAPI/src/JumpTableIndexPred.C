...
static void
BuildEdgesAux(SliceNode::Ptr srcNode, ParseAPI::Block* curBlock,
              ...
              set<ParseAPI::Edge*> allowedEdges)
{
    ...
    for(auto eit = targets.begin(); eit != targets.end(); ++eit)
    {
        ...
        if((*eit)->type() != CALL && (*eit)->type() != RET && (*eit)->type() != CATCH &&
           !(*eit)->interproc() && allowedEdges.find(*eit) != allowedEdges.end())
        {
            EdgeTypeEnum newT = t;
            if(t == _edgetype_end_)
            {
...
static void
BuildEdges(SliceNode::Ptr                                             curNode,
           map<ParseAPI::Block*, map<AssignmentPtr, SliceNode::Ptr>>& targetMap,
...
static bool
IsPushAndChangeSP(Assignment::Ptr a)
{
...
GraphPtr
JumpTableIndexPred::BuildAnalysisGraph(set<ParseAPI::Edge*>& visitedEdges)
{
    ...
    for(auto ait = currentAssigns.begin(); ait != currentAssigns.end(); ++ait)
    {
        ...
        {
            if(xchgCount.find((*ait)->addr()) != xchgCount.end())
                continue;
    ...
    for(auto ait = currentAssigns.begin(); ait != currentAssigns.end(); ++ait)
    {
        ...
        if((AssignIsZF(a) || shouldSkip.find(a->addr()) == shouldSkip.end()) &&
           !IsPushAndChangeSP(a) && (!a->insn().writesMemory() || MatchReadAST(a)))
        {
    ...
    /*
        ...
        if (!cur->hasOutEdges() && cur != virtualExit) {
            newG->insertPair(cur, virtualExit, TypedSliceEdge::create(cur, virtualExit,
       FALLTHROUGH));
        }
        }
...
bool
JumpTableIndexPred::addNodeCallback(AssignmentPtr ap, set<ParseAPI::Edge*>& visitedEdges)
{
    if(unknownInstruction)
        return false;
    if(currentAssigns.find(ap) != currentAssigns.end())
        return true;
    if(currentAssigns.size() > 50)
        return false;
    ...
    */
    ...
    if(!expandRet.second || expandRet.first == NULL)
    {
        ...
        parsing_printf("\t add unknown instruction, opcode %u, %d\n",
                       ap->insn().getOperation().getID(),
                       ap->insn().getOperation().getID() == aarch64_op_b_cond);
        return true;
    }
    ...
    if(ijt)
    {
        // Now we have stopped slicing, so the cache contents may not be complete any
        // more.
        setClearCache(true);
        return false;
    }
...
bool
JumpTableIndexPred::IsIndexBounded(GraphPtr slice, BoundFactsCalculator& bfc,
                                   StridedInterval& target)
{
    findBound = false;
    ...
    parsing_printf("\t checking index bound for %s, %s", index.format().c_str(),
                   tarBoundValue ? "found bound" : "no bound");
...
bool
JumpTableIndexPred::MatchReadAST(Assignment::Ptr a)
{
    pair<AST::Ptr, bool> expandRet = se.ExpandAssignment(a);
    if(!expandRet.second || expandRet.first == NULL)
        return false;
    if(a->out().generator() == NULL)
        return false;
    AST::Ptr write = se.SimplifyAnAST(
        RoseAST::create(ROSEOperation(ROSEOperation::derefOp, a->out().size()),
                        a->out().generator()),
        SymbolicExpression::PCValue(a->addr(), a->insn().size(),
...
bool
JumpTableIndexPred::modifyCurrentFrame(Slicer::SliceFrame& frame, Graph::Ptr g, Slicer*)
{
...
bool
JumpTableIndexPred::ignoreEdge(ParseAPI::Edge* e)
{
