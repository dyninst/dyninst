...
bool
SymbolicExpression::ReadMemory(Address addr, uint64_t& v, int)
{
...
/*AST::Ptr SymbolicExpression::SimplifyRecursive(AST::Ptr ast, Address addr, bool
  keepMultiOne) {

  }*/
AST::Ptr
SymbolicExpression::SimplifyRoot(AST::Ptr ast, Address addr, bool keepMultiOne)
{
        ...
        switch(roseAST->val().op)
        {
            ...
            case ROSEOperation::concatOp: {
                   ...
                   roseAST->child(1)->getID() == AST::V_ConstantAST)
                {
                    ...
                    ConstantAST::Ptr child1 =
                        boost::static_pointer_cast<ConstantAST>(roseAST->child(1));
                    uint64_t val =
                        (child1->val().val << child0->val().size) + child0->val().val;
                ...
                if(roseAST->child(1)->getID() == AST::V_RoseAST)
                {
                    ...
                    if(child1->val().op == ROSEOperation::ifOp)
                        break;
                }
                return roseAST->child(0);
            }
                       ...
                       rOp->val().op == ROSEOperation::sMultOp)
                    {
                           ...
                           rOp->child(1)->getID() == AST::V_ConstantAST)
                        {
                            VariableAST::Ptr varAST1 =
                                boost::static_pointer_cast<VariableAST>(
                                    roseAST->child(0));
                            ...
                            if(varAST1->val().reg == varAST2->val().reg)
                            {
                                ...
                                RoseAST::Ptr newRoot = RoseAST::create(
                                    ROSEOperation(rOp->val()), varAST1, newC);
                                return newRoot;
                            }
                        }
                    }
            ...
            case ROSEOperation::uMultOp:
                ...
                if(roseAST->child(0)->getID() == AST::V_ConstantAST)
                {
                    ConstantAST::Ptr child0 =
                        boost::static_pointer_cast<ConstantAST>(roseAST->child(0));
                    if(child0->val().val == 1 && !keepMultiOne)
                        return roseAST->child(1);
                }
                ...
                if(roseAST->child(1)->getID() == AST::V_ConstantAST)
                {
                    ConstantAST::Ptr child1 =
                        boost::static_pointer_cast<ConstantAST>(roseAST->child(1));
                    if(child1->val().val == 1 && !keepMultiOne)
                        return roseAST->child(0);
                }
                break;
            ...
            case ROSEOperation::rotateLOp:
                ...
                if(roseAST->child(1)->getID() == AST::V_ConstantAST)
                {
                    ...
                    ConstantAST::Ptr child1 =
                        boost::static_pointer_cast<ConstantAST>(roseAST->child(1));
                    if(child1->val().val == 0 && !keepMultiOne)
                        return roseAST->child(0);
                }
                break;
            ...
            default:
                // std::cerr << " TODO: unkown case of rose operation " <<
                // roseAST->val().op;
                break;
        }
    }
    else if(ast->getID() == AST::V_VariableAST)
    {
        ...
        if(varAST->val().reg.absloc().isPC())
        {
            ...
            return ConstantAST::create(
                Constant(addr, getArchAddressWidth(pc.getArchitecture()) * 8));
        }
...
AST::Ptr
SymbolicExpression::SimplifyAnAST(AST::Ptr ast, Address addr, bool keepMultiOne)
{
...
bool
SymbolicExpression::ContainAnAST(AST::Ptr root, AST::Ptr check)
{
    if(*root == *check)
        return true;
    bool     ret           = false;
...
AST::Ptr
SymbolicExpression::DeepCopyAnAST(AST::Ptr ast)
{
    if(ast->getID() == AST::V_RoseAST)
    {
        RoseAST::Ptr  roseAST = boost::static_pointer_cast<RoseAST>(ast);
...
pair<AST::Ptr, bool>
SymbolicExpression::ExpandAssignment(Assignment::Ptr assign, bool keepMultiOne)
{
    ...
    else
    {
        ...
        if(expandRet.second && expandRet.first)
        {
            ...
            AST::Ptr calculation =
                SimplifyAnAST(expandRet.first,
                              PCValue(assign->addr(), assign->insn().size(),
...
AST::Ptr
SymbolicExpression::SubstituteAnAST(AST::Ptr ast, const map<AST::Ptr, AST::Ptr>& aliasMap)
{
...
Address
SymbolicExpression::PCValue(Address cur, size_t insnSize, Architecture a)
{
    switch(a)
    {
        case Arch_x86:
