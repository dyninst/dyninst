...
void
ReachFact::ReverseDFS(ParseAPI::Block* cur, set<ParseAPI::Block*>& visited)
{
    ...
    for(auto eit = src.begin(); eit != src.end(); ++eit)
        if((*eit)->type() != CALL && (*eit)->type() != RET)
            ReverseDFS((*eit)->src(), visited);
}

void
ReachFact::NaturalDFS(ParseAPI::Block* cur, set<ParseAPI::Block*>& visited)
{
    if(visited.find(cur) != visited.end())
        return;
    visited.insert(cur);
    ...
    for(auto eit = trg.begin(); eit != trg.end(); ++eit)
        if((*eit)->type() != CALL && (*eit)->type() != RET)
            NaturalDFS((*eit)->trg(), visited);
}

void
ReachFact::ReachBlocks()
{
...
ReachFact::ReachFact(ThunkData& t)
: thunks(t)
{
    ReachBlocks();
}
