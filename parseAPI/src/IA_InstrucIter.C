...
IA_InstrucIter::IA_InstrucIter(InstrucIter from, CodeObject* o, CodeRegion* r,
                               InstructionSource* s)
...
size_t IA_InstrucIter::getSize() const { return ii.peekNext() - *ii; }

bool IA_InstrucIter::hasCFT() const {
  if (ii.isACallInstruction() || ii.isAJumpInstruction() ||
      ii.isAReturnInstruction() || ii.isACondBranchInstruction() ||
      ii.isACondReturnInstruction() || ii.isADynamicCallInstruction() ||
...
bool IA_InstrucIter::isAbort() const { return ii.isAnAbortInstruction(); }

bool IA_InstrucIter::isInvalidInsn() const { return false; }

bool IA_InstrucIter::isFrameSetupInsn() const { return ii.isFrameSetup(); }

bool IA_InstrucIter::isNop() const { return ii.isANopInstruction(); }
...
bool IA_InstrucIter::isConditional() const {
  return ii.isACondBranchInstruction() || ii.isACondReturnInstruction();
}

Address IA_InstrucIter::getCFT() const {
  if (ii.isAIndirectJumpInstruction() || ii.isADynamicCallInstruction()) {
    return 0;
  }
  return ii.getBranchTargetAddress();
}

bool IA_InstrucIter::isCall() const { return ii.isACallInstruction(); }

void IA_InstrucIter::getNewEdges(
    std::vector<std::pair<Address, EdgeTypeEnum> >& outEdges, Function* context,
    Block* currBlk, unsigned int num_insns,
    dyn_hash_map<Address, std::string>* plt_entries) const {
  if (!hasCFT())
    return;
  else if (ii.isACondBranchInstruction()) {
    outEdges.push_back(
        std::make_pair(ii.getBranchTargetAddress(NULL), COND_TAKEN));
    if (ii.isDelaySlot()) {
      outEdges.push_back(
          std::make_pair(current + 2 * getSize(), COND_NOT_TAKEN));
    } else {
    ...
    if (ii.isRealCall(_isrc, validTarget, simulateJump)) {
      ...
      if (validTarget) {
        if (simulateJump) {
          parsing_printf(
              "[%s:%u] call at 0x%lx simulated as "
              "jump to 0x%lx\n",
    ...
    if (ii.isDelaySlot()) {
      outEdges.push_back(std::make_pair(getAddr() + 2 * getSize(), CALL_FT));
...
bool IA_InstrucIter::isLeave() const { return ii.isALeaveInstruction(); }

bool IA_InstrucIter::isDelaySlot() const { return ii.isDelaySlot(); }

instruction IA_InstrucIter::getInstruction() { return ii.getInstruction(); }
...
bool IA_InstrucIter::isTailCall(Function* context, EdgeTypeEnum /*ignored*/,
...
bool IA_InstrucIter::savesFP() const { return ii.isFramePush(); }

bool IA_InstrucIter::cleansStack() const {
  return ii.getInstruction().isCleaningRet();
}

bool IA_InstrucIter::isReturnAddrSave() const { return ii.isReturnValueSave(); }

bool IA_InstrucIter::isNopJump() const { return false; }
