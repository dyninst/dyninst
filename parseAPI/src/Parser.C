...
Parser::parse()
{
    parsing_printf("[%s:%d] parse() called on Parser %p with state %d\n", FILE__,
                   __LINE__, (void*) this, _parse_state);
...
void
Parser::parse_at(CodeRegion* region, Address target, bool recursive, FuncSource src)
{
    Function*                  f;
    ParseFrame*                pf;
...
#pragma omp parallel for schedule(dynamic)
    ...
    for(size_t i = 1; i < svec.size(); ++i)
        size_vec.push_back(
            make_pair(svec[i].first - svec[i - 1].first, svec[i - 1].second));
    if(svec.size() > 0)
    ...
    for(unsigned idx = 0; idx < work_elems.size(); idx++)
    {
        ...
        if(elem->order() == ParseWorkElem::call_fallthrough)
        {
            ...
            if(callEdge)
            {
                ...
                elem->bundle()->add(new ParseWorkElem(elem->bundle(), callEdge,
                                                      callEdge->src()->last(), callTarget,
                                                      isResolvable, false));
            }
        }
...
LockFreeQueueItem<ParseFrame*>*
Parser::postProcessFrame(ParseFrame* pf, bool recursive)
{
    LockFreeQueue<ParseFrame*> work;
    switch(pf->status())
    {
        ...
        case ParseFrame::RETURN_SET: {
            parsing_printf("[%s] frame %lx's function's return status set to RETURN. "
                           "Chance to resume functions",
                           FILE__, pf->func->addr());
            work.insert(pf);
        ...
        case ParseFrame::PARSED: {
            parsing_printf("[%s] frame %lx complete, return status: %d\n", FILE__,
                           pf->func->addr(), pf->func->retstatus());

            if(unlikely(_obj.defensiveMode() && TAMPER_NONE != pf->func->tampersStack() &&
        ...
        case ParseFrame::FRAME_DELAYED: {
             ...
             * has already been parsed. Here we check if any callee has been parsed, and
             * if it is the case, we can immedately resume the current frame.
             *
             ...
             * And A calls B. Thread 1 is first delayed by B and puts A into the delayed
             * frames. Then thread 2 finishes B and resumes A, and thread 1 is still
             * working on other parts of A. If thread 3 picks up the resumed work item to
             * parse A, thread 3 will find that thread 1 is still working on A, and gives
             * this work item up. However, thread 1 still believes that itself is delayed
             * by B, causing delayed edges never parsed.
             *
            ...
            if(pf->delayedWork.size())
            {
                ...
                for(auto iter = pf->delayedWork.begin(); iter != pf->delayedWork.end();
                    ++iter)
                {
                    Function* ct = iter->second;
                    parsing_printf("[%s] waiting on %s\n", __FILE__, ct->name().c_str());
                    {
                        dyn_c_hash_map<Function*, std::set<ParseFrame*>>::accessor a;
                        set<ParseFrame*> waiters;
                        // This acquires the lock for ct, regardless we get a new set of
                        // not
                        bool new_set =
                            delayed_frames.insert(a, std::make_pair(ct, waiters));
                        ...
                        a->second.insert(pf);
                        delayed_frames_changed.exchange(true,
        ...
        case ParseFrame::PROGRESS:
            // another thread is working on this already; possibly something else
            // unblocked.
            break;
...
void
Parser::LaunchWork(LockFreeQueueItem<ParseFrame*>* frame_list, bool recursive)
{
...
void
Parser::SpawnProcessFrame(ParseFrame* pf, bool recursive)
{
...
void
print_work_queue(LockFreeQueue<ParseFrame*>* work_queue)
{
    ...
    while(current)
    {
        std::cout << "  parse frame " << std::hex << current->value() << std::dec
                  << std::endl;
...
void
Parser::ProcessFrames(LockFreeQueue<ParseFrame*>* work_queue, bool recursive)
{
...
Parser::parse_frames(LockFreeQueue<ParseFrame*>& work, bool recursive)
{
    ...
    bool done = false, cycle = false;
    {
        ...
        if(updated.size())
        {
            for(auto uIter = updated.begin(); uIter != updated.end(); ++uIter)
            {
        ...
        {
            parsing_printf(
                "[%s] Fixed point reached (0 funcs with unknown return status)\n)",
                __FILE__);
            done = true;
        }
...
void
Parser::processFixedPoint(LockFreeQueue<ParseFrame*>& work, bool recursive)
{
    // We haven't yet reached a fixedpoint; let's recurse
    parsing_printf(
        "[%s] Fixed point not yet reached (%d funcs with unknown return status)\n",
...
void
Parser::processCycle(LockFreeQueue<ParseFrame*>& work, bool recursive)
{  // If we've reached a fixedpoint and have remaining frames, we must
    ...
    else
    {
        // We shouldn't get here
        parsing_printf(
            "[%s] No more work can be done (%d funcs with unknown return status)\n",
            __FILE__, delayed_frames.size());
        assert(0);
    }
}
...
Parser::finalize(Function* f)
{
    ...
    for(auto b : blocks)
    {
        ...
        for(auto e : targets)
        {
            // Tail call corretions are only relevant to non-sink COND_TAKEN or non-sink
            // DIRECT edges
            if(e->sinkEdge())
                continue;
            if(e->type() != COND_TAKEN && e->type() != DIRECT)
                continue;
            ...
            // incoming edge, the current edge should be marked as a tail call
            if(trg_has_call_edge && !e->interproc())
            {
                e->_type._interproc = true;
                parsing_printf("from %lx to %lx, marked as tail call (target has a CALL "
                               "incoming edge), re-finalize\n",
                               b->last(), e->trg()->start());
                return false;
            }
            ...
            // If the tail call target is created from a symbol, then keep it as a tail
            // call
            ...
            if(visited.find(e->trg()) != visited.end() && e->trg() != f->entry())
            {
                ...
                parsing_printf("from %lx to %lx, marked as not tail call (part of the "
                               "function), re-finalize\n",
                               b->last(), e->trg()->start());
                return false;
            }
    ...
    if(unlikely(f->obj()->defensiveMode()))
    {
        ...
        for(Function::edgelist::iterator eit = edges.begin(); eit != edges.end(); eit++)
        {
            boost::lock_guard<Block> blockGuard(*(*eit)->src());
            if(2 > (*eit)->src()->targets().size())
            {
                Block* ft =
                    _parse_data->findBlock((*eit)->src()->region(), (*eit)->src()->end());
...
#pragma omp parallel for schedule(dynamic)
    for(size_t i = 0; i < jumpTableVector.size(); ++i)
    {
        ...
        if(start_it == jumpTableStart.end())
            continue;
        if(*start_it < jti->tableEnd)
        {
            ...
            for(Address addr = jti->tableStart; addr < *start_it;
                addr += jti->indexStride)
            {
            ...
            for(auto eit = targets.begin(); eit != targets.end(); ++eit)
            {
                if((*eit)->type() != INDIRECT || (*eit)->sinkEdge())
                    continue;
            ...
            {
                if(validTargets.find(jti->tableEntryMap[addr]) != validTargets.end())
                    continue;
                if(edgeMap.find(jti->tableEntryMap[addr]) == edgeMap.end())
                    continue;
...
Parser::delete_bogus_blocks(Edge* e)
{
    ...
    parsing_printf("Remove an edge from %p[%lx, %lx) to %p[%lx, %lx), type %d\n",
                   (void*) e->src(), e->src()->start(), e->src()->end(), (void*) e->trg(),
                   e->trg()->start(), e->trg()->end(), e->type());
...
Parser::clean_bogus_funcs(dyn_c_vector<Function*>& funcs)
{
    for(auto fit = funcs.begin(); fit != funcs.end(); fit++)
    {
        ...
        if(!interprocEdge)
        {
            ...
            // We have already fixed such bogos tail calls in the previous step of
            // finalizing, so now we should remove such bogus function
            remove_func(f);
...
Parser::init_frame(ParseFrame& frame)
{
    ...
    unsigned size     = frame.codereg->offset() + frame.codereg->length() - ia_start;
    ...
    InstructionAdapter_t* ah = InstructionAdapter_t::makePlatformIA_IAPI(
        obj().cs()->getArch(), dec, ia_start, frame.func->obj(), frame.codereg,
        frame.func->isrc(), b);
...
namespace
{
inline ParseAPI::Edge*
bundle_call_edge(ParseWorkBundle* b)
{
    if(!b)
        return NULL;
...
inline ParseWorkElem*
bundle_call_elem(ParseWorkBundle* b)
{
    if(!b)
        return NULL;
...
Parser::parse_frame(ParseFrame& frame, bool recursive)
{
    ...
    else
    {
        ...
        for(iter = frame.delayedWork.begin(); iter != frame.delayedWork.end(); ++iter)
        {
    ...
    do
    {
        ...
        parsing_printf("Iteration %d for function %s at %lx\n", count,
                       frame.func->name().c_str(), frame.func->addr());
...
Parser::parse_frame_one_iteration(ParseFrame& frame, bool recursive)
{
    ...
    while(!worklist.empty())
    {
        ...
        if(work->order() == ParseWorkElem::call)
        {
            ...
            region_data::edge_data_map*          edm =
                _parse_data->get_edge_data_map(func->region());
            assert(edm->find(a, work->source()));
            parsing_printf("Handling call work element\n");
            cur          = a->second.b;
            Function* ct = NULL;

            // If we're not doing recursive traversal, skip *all* of the call edge
            // processing. We don't want to create a stub. We'll handle the assumption of
            // a fallthrough target for the fallthrough work element, not the call work
            // element. Catch blocks will get excluded but that's okay; our heuristic is
            // not reliable enough that I'm willing to deploy it when we'd only be
            // detecting a non-returning callee by name anyway.
            // --BW 12/2012
            if(!recursive)
            {
            ...
            {
                parsing_printf(
                    "[%s] binding call (call target should have been created) %lx->%lx\n",
                    FILE__, cur->lastInsnAddr(), work->target());
        ...
        else if(work->order() == ParseWorkElem::call_fallthrough)
        {
            ...
            else
            {
                ...
                if(prevInsn.getOperation().getID() == e_syscall)
                {
                    ...
                    if(!getSyscallNumber(func, edge->src(), src, frame.codereg->getArch(),
                                         syscallNumber))
                    {
                        // If we cannot retrieve a syscall number, assume the syscall
                        // returns
                        parsing_printf(
                            "[%s] could not retrieve syscall edge, assuming returns\n",
                            FILE__);
                    }
                    else
                    {
                    ...
                    {
                        parsing_printf(
                            "[%s] edge %lx, no fallthrough for non-returning syscall\n",
                ...
                else if(func->obj()->cs()->isCode(call_elem->target()))
                {
                    ...
                    if(ct && (ct->retstatus() == UNSET))
                    {
                        ...
                        parsing_printf("[%s] Parsing FT edge %lx, corresponding callee "
                                       "(%s) return status unknown; delaying work\n",
                                       __FILE__, work->edge()->src()->lastInsnAddr(),
                                       ct->name().c_str());
                    ...
                    {
                        parsing_printf(
                            "\t Disallowing FT edge: CodeSource reports nonreturning\n");
                    }
                    if(!is_nonret && is_plt)
                    {
                        ...
                        if(is_nonret)
                        {
                            parsing_printf("\t Disallowing FT edge: CodeSource reports "
                                           "PLT nonreturning\n");
                        }
                    }
                    ...
                    if(!is_nonret && !is_plt && ct)
                    {
                        ...
                        {
                            parsing_printf(
                                "\t Disallowing FT edge: function is non-returning\n");
                        }
                    }
        ...
        else if(work->order() == ParseWorkElem::resolve_jump_table)
        {
            ...
            if(!work_ah->isTailCall(frame.func, INDIRECT, frame.num_insns,
                                    frame.knownTargets))
        ...
        if(NULL == cur)
        {
            pair<Block*, ParseAPI::Edge*> newedge =
                add_edge(frame, frame.func, work->edge()->src(), work->source(),
                         work->target(), work->edge()->type(), work->edge());
            cur = newedge.first;
        }
        ...
        else if(cur->createdByFunc() != frame.func)
        {
            ...
            if(func->retstatus() < UNKNOWN)
            {
                ...
                update_function_ret_status(frame, other_func,
                                           frame.mkWork(NULL, other_func));
            }
        ...
        unsigned size = cur->region()->offset() + cur->region()->length() - curAddr;
        const unsigned char* bufferBegin =
        ...
        if(bufferBegin == nullptr)
        {
            ...
            // will be removed.
            parsing_printf("\taddress %lx in a different region from the funcion entry "
                           "at %lx, skip parsing\n",
                           curAddr, func->addr());
            continue;
        }
        ...
        if(!ahPtr)
            ahPtr = InstructionAdapter_t::makePlatformIA_IAPI(
                func->obj()->cs()->getArch(), dec, curAddr, func->obj(), cur->region(),
                func->isrc(), cur);
        else
            ahPtr->reset(dec, curAddr, func->obj(), cur->region(), func->isrc(), cur);
        ...
        while(true)
        {
            ...
            parsing_printf("[%s:%d] curAddr 0x%lx: %s \n", FILE__, __LINE__, curAddr,
                           insn_det.insn->getInstruction().format().c_str());
            ...
            if(func->_is_leaf_function)
            {
                ...
                parsing_printf("[%s:%d] leaf %d funcname %s \n", FILE__, __LINE__,
                               func->_is_leaf_function, func->name().c_str());
                if(!func->_is_leaf_function)
                    func->_ret_addr = ret_addr;
            }
            ...
            if(isNopBlock && !ah->isNop())
            {
                ...
                parsing_printf("[%s:%d] pushing %lx onto worklist\n", FILE__, __LINE__,
                               curAddr);
                frame.pushWork(
                    frame.mkWork(NULL, newedge, ah->getAddr(), curAddr, true, false));
                break;
            }
            ...
            if(ah->hasCFT())
            {
                ...
                else
                {
                    ...
                    {
                        // This is the first time to set the current function to be
                        // RETURN. We may be able to resume some delayed functions
            ...
            else if(func->_saves_fp && func->_no_stack_frame && ah->isFrameSetupInsn())
            {  // isframeSetup is expensive
            ...
            else if(unlikely(func->obj()->defensiveMode()))
            {
                ...
                else if(ah->isNopJump())
                {
                    ...
                    unsigned bufsize = func->region()->offset() +
                                       func->region()->length() - ah->getAddr();
                    const unsigned char* bufBegin =
                        (const unsigned char*) (func->region()->getPtrToInstruction(
                            ah->getAddr()));
                    dec = InstructionDecoder(bufBegin, bufsize, frame.codereg->getArch());
                    ah->reset(dec, curAddr, func->obj(), func->region(), func->isrc(),
                              cur);
                }
                else
                {
            ...
            else if(!cur->region()->contains(ah->getNextAddr()))
            {
                parsing_printf("[%s] next address %lx is outside [%lx,%lx)\n", FILE__,
                               ah->getNextAddr(), cur->region()->offset(),
...
Block*
Parser::block_at(ParseFrame& frame, Function* owner, Address addr, Block*& split)
{
    Block* ret = NULL;
...
Parser::add_edge(ParseFrame& frame, Function* owner, Block* src, Address src_addr,
                 Address dst, EdgeTypeEnum et, ParseAPI::Edge* exist)
{
    Block*                        split   = NULL;
    Block*                        ret     = NULL;
    ...
    src = a->second.b;
    parsing_printf("\t add_edge called, src_addr %lx, dst %lx, edge type %d, looked up "
                   "block [%lx, %lx)\n",
    ...
    if(et != CALL &&
       frame.leadersToBlock.find(ret->start()) == frame.leadersToBlock.end())
    {
    ...
    {
        assert(src->last() == src_addr);
        if(exist->type() == FALLTHROUGH || exist->type() == COND_NOT_TAKEN ||
           exist->type() == CALL_FT)
        {
            if(src->end() != ret->start())
            {
                fprintf(stderr,
                        "src_addr %lx, src: [%lx, %lx), dst [%lx, %lx), target %lx, edge "
                        "type %d\n",
                        src_addr, src->start(), src->end(), ret->start(), src->end(), dst,
                        et);
            }
        }
...
int
Parser::findCurrentBlocks(CodeRegion* cr, Address addr, std::set<Block*>& blocks)
{
    return _parse_data->findBlocks(cr, addr, blocks);
}

int
Parser::findCurrentFuncs(CodeRegion* cr, Address addr, std::set<Function*>& funcs)
{
...
ParseAPI::Edge*
Parser::link_addr(Address src_addr, Block* dst, EdgeTypeEnum et, bool sink,
                  Function* func)
{
    region_data::edge_data_map::accessor a;
...
void
Parser::move_func(Function* func, Address new_entry, CodeRegion* new_reg)
{
...
void
Parser::invalidateContainingFuncs(Function* owner, Block* b)
{
    CodeRegion* cr;
    ...
    for(std::set<Function*>::iterator oit = prev_owners.begin(); oit != prev_owners.end();
        ++oit)
    {
        Function* po     = *oit;
...
void
Parser::resumeFrames(Function* func, LockFreeQueue<ParseFrame*>& work)
{
    // If we do not know the function's return status, don't put its waiters back on the
    // worklist
    if(func->retstatus() == UNSET)
    {
...
bool
Parser::getSyscallNumber(Function* /*func*/, Block* block, Address /*addr*/,
                         Architecture arch, long int& val)
{
    val = -1;
    ...
    InstructionAPI::RegisterAST* regAST =
        new InstructionAPI::RegisterAST(syscallNumberReg);
...
bool
Parser::set_edge_parsing_status(ParseFrame& frame, Address addr, Block* b)
{
    ...
    else
    {
        parsing_printf("[%s:%d] parsing edge at %lx has started by another thread, "
                       "function %s at %lx\n",
                       FILE__, __LINE__, addr, a1->second.f->name().c_str(),
                       a1->second.f->addr());
...
bool
Parser::inspect_value_driven_jump_tables(ParseFrame& frame)
{
    bool ret = false;
    ...
    for(auto bit = frame.value_driven_jump_tables.begin();
        bit != frame.value_driven_jump_tables.end(); ++bit)
    {
        Address                              addr = *bit;
        region_data::edge_data_map::accessor a;
        region_data::edge_data_map*          edm =
            _parse_data->get_edge_data_map(frame.func->region());
        ...
        {
            if(existing.find(oit->first) != existing.end())
                continue;
            ...
            frame.pushWork(
                frame.mkWork(NULL, newedge, block->last(), oit->first, true, false));
        }
    }
    return ret;
}
...
void
Parser::update_function_ret_status(ParseFrame& frame, Function* other_func,
                                   ParseWorkElem* work)
{
     ...
     *       ....
     *    }
     * In such code structure, at line 1), the other_func can be in UNSET, so the check
     * fails. Concurrently, the other_func can be immediately set to RETURN, making the
     * check at line 2) failing. So, the frame.func is neither delayed, nor updates its
     * return status to RETURN, which can lead to wrong NORETURN status.
     */
    ...
    {
        parsing_printf("\t other_func is NORETURN, this path does not impact the return "
