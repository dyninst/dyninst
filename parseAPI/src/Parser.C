...
Parser::Parser(CodeObject &obj, CFGFactory &fact, ParseCallbackManager &pcb)
  ...
  if (obj.cs()->regions().empty()) {
    parsing_printf(
        "[%s:%d] CodeSource provides no CodeRegions"
        " -- unparesable\n",
        FILE__, __LINE__);
...
Parser::~Parser() {
  if (_parse_data) delete _parse_data;

  vector<ParseFrame *>::iterator fit = frames.begin();
  for (; fit != frames.end(); ++fit) delete *fit;
  frames.clear();
}

void Parser::add_hint(Function *f) {
  if (!_parse_data->findFunc(f->region(), f->addr())) record_func(f);
}

void Parser::parse() {
  parsing_printf("[%s:%d] parse() called on Parser %p with state %d\n", FILE__,
                 __LINE__, this, _parse_state);

  // For modification: once we've full-parsed once, don't do it again
  if (_parse_state >= COMPLETE) return;

  if (_parse_state == UNPARSEABLE) return;
  ...
  if (_parse_state < COMPLETE) _parse_state = COMPLETE;
...
void Parser::parse_at(CodeRegion *region, Address target, bool recursive,
                      FuncSource src) {
  Function *f;
  ParseFrame *pf;
  ...
  if (_parse_state < PARTIAL) _parse_state = PARTIAL;
...
void Parser::parse_at(Address target, bool recursive, FuncSource src) {
...
void Parser::parse_vanilla() {
  ...
  for (fit = hint_funcs.begin(); fit != hint_funcs.end(); ++fit) {
    ...
    if (test != ParseFrame::BAD_LOOKUP && test != ParseFrame::UNPARSED) {
...
void Parser::parse_edges(vector<ParseWorkElem *> &work_elems) {
  if (_parse_state == UNPARSEABLE) return;
  ...
  for (unsigned idx = 0; idx < work_elems.size(); idx++) {
    ...
    if (elem->order() == ParseWorkElem::call_fallthrough) {
      ...
      for (Block::edgelist::iterator eit = trgs.begin(); eit != trgs.end();
           eit++) {
        if ((*eit)->type() == CALL) {
          callEdge = *eit;
          if (!(*eit)
                   ->sinkEdge())  // if it's a sink edge, look for nonsink CALL
            break;
        }
      }
      ...
      // Also, in the case I saw, the callee was _not_ returning. Not directly.
      // It
      ...
      if (callEdge) {
        ...
        elem->bundle()->add(new ParseWorkElem(elem->bundle(), callEdge,
                                              callTarget, isResolvable, false));
      }
    }
    ParseFrame *frame =
        _parse_data->findFrame(src->region(), src->lastInsnAddr());
...
void Parser::parse_frames(vector<ParseFrame *> &work, bool recursive) {
  ParseFrame *pf;
    ...
    switch (pf->status()) {
      ...
      case ParseFrame::PARSED:
        parsing_printf("[%s] frame %lx complete, return status: %d\n", FILE__,
                       pf->func->addr(), pf->func->_rs);

        if (unlikely(_obj.defensiveMode() &&
                     TAMPER_NONE != pf->func->tampersStack() &&
                     TAMPER_NONZERO !=
                         pf->func->tampersStack())) {  // may adjust CALL_FT
                                                       // targets, add a frame
                                                       // to the worklist,
      ...
      case ParseFrame::FRAME_DELAYED: {
        ...
        if (pf->delayedWork.size()) {
          ...
          for (iter = pf->delayedWork.begin(); iter != pf->delayedWork.end();
               ++iter) {
  ...
  if (delayedFrames.size() == 0) {
    ...
    parsing_printf(
        "[%s] Fixed point reached (%d funcs with unknown return status)\n",
        __FILE__, delayedFrames.size());
    ...
    if (updated.size()) {
      for (vector<Function *>::iterator uIter = updated.begin();
           uIter != updated.end(); ++uIter) {
        resumeFrames((*uIter), work);
      }

      if (work.size()) {
        parsing_printf(
            "[%s] Updated retstatus of delayed frames, trying again; "
            "work.size() = %d\n",
            __FILE__, work.size());
        parse_frames(work, recursive);
      }
      ...
      // We shouldn't get here
      parsing_printf(
          "[%s] No more work can be done (%d funcs with unknown return "
          "status)\n",
          __FILE__, delayedFrames.size());
      assert(0);
    }
    ...
    parsing_printf(
        "[%s] Fixed point not yet reached (%d funcs with unknown return "
        "status)\n",
        __FILE__, delayedFrames.size());
    ...
    vector<Function *> updated;
    for (map<Function *, set<ParseFrame *> >::iterator iter =
             delayedFrames.begin();
         iter != delayedFrames.end(); ++iter) {
...
void Parser::finalize(Function *f) {
  if (f->_cache_valid) {
    return;
  }
  ...
  if (unlikely(f->obj()->defensiveMode())) {
    ...
    for (Function::edgelist::iterator eit = edges.begin(); eit != edges.end();
         eit++) {
...
void Parser::finalize_funcs(vector<Function *> &funcs) {
...
void Parser::record_func(Function *f) {
  if (!f) return;
...
void Parser::init_frame(ParseFrame &frame) {
  Block *b = NULL;
  Block *split = NULL;
  ...
  if (!frame.func->_entry) {
    ...
    if (b) {
      ...
      frame.func->_entry = b;
      frame.seed =
          new ParseWorkElem(NULL, NULL, frame.func->addr(), true, false);
  ...
  const unsigned char *bufferBegin =
      (const unsigned char *)(frame.func->isrc()->getPtrToInstruction(
          ia_start));
  InstructionDecoder dec(bufferBegin, size, frame.codereg->getArch());
  InstructionAdapter_t ah(dec, ia_start, frame.func->obj(), frame.codereg,
                          frame.func->isrc(), b);
...
void ParseFrame::cleanup() {
  for (unsigned i = 0; i < work_bundles.size(); ++i) delete work_bundles[i];
  work_bundles.clear();
  if (seed) delete seed;
  seed = NULL;
}
...
inline Edge *bundle_call_edge(ParseWorkBundle *b) {
  ...
  for (; it != elems.end(); ++it) {
    if ((*it)->edge()->type() == CALL) return (*it)->edge();
  }
  return NULL;
}
...
void Parser::parse_frame(ParseFrame &frame, bool recursive) {
  ...
  if (frame.status() == ParseFrame::UNPARSED) {
    ...
    for (iter = frame.delayedWork.begin(); iter != frame.delayedWork.end();
         ++iter) {
  ...
  while (!worklist.empty()) {
    ...
    if (work->order() == ParseWorkElem::call) {
      ...
      if (!work->callproc()) {
        ...
        // processing.
        // We don't want to create a stub. We'll handle the assumption of a
        // fallthrough
        // target for the fallthrough work element, not the call work element.
        // Catch blocks
        // will get excluded but that's okay; our heuristic is not reliable
        // enough that I'm willing to deploy
        ...
        parsing_printf("[%s] binding call %lx->%lx\n", FILE__,
                       work->edge()->src()->lastInsnAddr(), work->target());

        pair<Function *, Edge *> ctp =
            bind_call(frame, work->target(), work->edge()->src(), work->edge());
        ct = ctp.first;
      ...
      if (!ce) {
        ...
        if (prevInsn->getOperation().getID() == e_syscall) {
          ...
          if (!getSyscallNumber(func, edge->src(), src,
                                frame.codereg->getArch(), syscallNumber)) {
          ...
          if (ct && (ct->_rs == UNSET)) {
            ...
            parsing_printf(
                "[%s] Parsing FT edge %lx, corresponding callee (%s) return "
                "status unknown; delaying work\n",
          ...
          if (!is_nonret && is_plt) {
            ...
            if (is_nonret) {
              parsing_printf(
                  "\t Disallowing FT edge: CodeSource reports PLT "
                  "nonreturning\n");
            }
          }
          ...
          if (unlikely(!is_nonret && frame.func->obj()->defensiveMode() &&
                       ct)) {
            is_nonret |= (ct->retstatus() == UNKNOWN);
            if (is_nonret) {
              parsing_printf(
                  "\t Disallowing FT edge: function in "
                  "defensive binary may not return\n");
              mal_printf(
                  "Disallowing FT edge: function %lx in "
                  "defensive binary may not return\n",
                  ct->addr());
            } else {
              ...
              if (is_nonret) {
                mal_printf(
                    "Disallowing FT edge: function at %lx "
                    "tampers with its stack\n",
                    ct->addr());
                parsing_printf(
                    "\t Disallowing FT edge: function "
                    "tampers with its stack\n");
              }
            }
          }
          if (is_nonret) {
            parsing_printf(
                "[%s] no fallthrough for non-returning call "
                "to %lx at %lx\n",
                FILE__, target, work->edge()->src()->lastInsnAddr());
        ...
        if (is_nonret) {
          Address catchStart;
          Block *caller = ce->src();
          // There may be nops between this non-returning call and the catch
          // block and
          // there is possibility that the exception table entry points a nop.
          // Therefore, we need to check for every nop and first non-nop
          // instruction after the call for catch blocks

          unsigned size = caller->region()->offset() +
                          caller->region()->length() - caller->end();
          const unsigned char *bufferBegin =
              (const unsigned char *)(func->isrc()->getPtrToInstruction(
                  caller->end()));
          InstructionDecoder dec(bufferBegin, size, frame.codereg->getArch());
          if (!ahPtr)
            ahPtr.reset(new InstructionAdapter_t(dec, caller->end(),
                                                 func->obj(), caller->region(),
                                                 func->isrc(), NULL));
          else
            ahPtr->reset(dec, caller->end(), func->obj(), caller->region(),
                         func->isrc(), NULL);
          ...
          if (found ||
              (ah.getInstruction() &&
               frame.codereg->findCatchBlock(ah.getAddr(), catchStart))) {
            ...
            // push on worklist
            frame.pushWork(frame.mkWork(work->bundle(), catch_edge, catchStart,
                                        true, false));
          }
          continue;
        }
    ...
    if (NULL == cur) {
      pair<Block *, Edge *> newedge =
          add_edge(frame, frame.func, work->edge()->src(), work->target(),
                   work->edge()->type(), work->edge());
      cur = newedge.first;
    }
    ...
    if (!cur->_parsed) {
      ...
      if (func->_rs < UNKNOWN) {
        ...
        Function *other_func =
            _parse_data->findFunc(func->region(), cur->start());
    ...
    while (true) {
      ...
      if (curAddr == nextBlockAddr) {
        ...
        end_block(cur, ah);
        pair<Block *, Edge *> newedge =
            add_edge(frame, frame.func, cur, nextBlockAddr, FALLTHROUGH, NULL);

        if (!HASHDEF(visited, nextBlockAddr) &&
            !HASHDEF(leadersToBlock, nextBlockAddr)) {
          parsing_printf("[%s:%d] pushing %lx onto worklist\n", FILE__,
                         __LINE__, nextBlockAddr);

          frame.pushWork(
              frame.mkWork(NULL, newedge.second, nextBlockAddr, true, false));
        ...
        parsing_printf(
            "[%s:%d] inconsistent instruction stream: "
            "%lx is within [%lx,%lx)\n",
            FILE__, __LINE__, curAddr, nextBlock->start(), nextBlock->end());
      ...
      if (isNopBlock && !ah.isNop()) {
        ...
        if (targ && !HASHDEF(visited, targ->start())) {
          ...
          frame.pushWork(
              frame.mkWork(NULL, newedge.second, targ->start(), true, false));
      ...
      if (ah.hasCFT()) {
        ...
        break;
      } else if (func->_saves_fp && func->_no_stack_frame &&
        ...
        pair<Block *, Edge *> newedge = add_edge(
            frame, frame.func, cur, ah.getNextAddr(), FALLTHROUGH, NULL);
        Block *targ = newedge.first;
        ...
        if (!_pcb.hasWeirdInsns(func) && ah.isGarbageInsn()) {
          ...
          const unsigned char *bufferBegin =
              (const unsigned char *)(func->isrc()->getPtrToInstruction(
                  ah.getAddr()));
          dec = InstructionDecoder(bufferBegin, bufsize,
                                   frame.codereg->getArch());
          ah = InstructionAdapter_t(dec, curAddr, func->obj(), func->region(),
                                    func->isrc(), cur);
        } else {
...
void Parser::end_block(Block *b, InstructionAdapter_t &ah) {
...
void Parser::record_block(Block *b) {
  parsing_printf("[%s:%d] recording block [%lx,%lx)\n", FILE__, __LINE__,
                 b->start(), b->end());
  _parse_data->record_block(b->region(), b);
}

Block *Parser::block_at(Function *owner, Address addr, Block *&split) {
  set<Block *> overlap;
  Block *exist = NULL;
  Block *ret = NULL;
  ...
  if (NULL == (exist = _parse_data->findBlock(cr, addr))) {
    ...
    for (set<Block *>::iterator sit = overlap.begin(); sit != overlap.end();
         ++sit) {
      Block *b = *sit;
...
pair<Block *, Edge *> Parser::add_edge(ParseFrame &frame, Function *owner,
                                       Block *src, Address dst, EdgeTypeEnum et,
                                       Edge *exist) {
  Block *split = NULL;
  Block *ret = NULL;
  Edge *newedge = NULL;
...
Block *Parser::split_block(Function *owner, Block *b, Address addr,
                           Address previnsn) {
  Block *ret;
  CodeRegion *cr;
  ...
  for (std::set<Function *>::iterator oit = prev_owners.begin();
    ...
    if (po->_cache_valid) {
      ...
      parsing_printf(
          "[%s:%d] split of [%lx,%lx) invalidates cache of "
          "func at %lx\n",
          FILE__, __LINE__, b->start(), b->end(), po->addr());
    }
    if (isRetBlock) {
      po->_retBL.clear();  // could remove b from the vector instead of clearing
                           // it, not sure what's cheaper
    }
  }
...
pair<Function *, Edge *> Parser::bind_call(ParseFrame &frame, Address target,
                                           Block *cur, Edge *exist) {
  Function *tfunc = NULL;
  Block *tblock = NULL;
  FuncSource how = RT;
  if (frame.func->_src == GAP || frame.func->_src == GAPRT) how = GAPRT;
...
Function *Parser::findFuncByEntry(CodeRegion *r, Address entry) {
  if (_parse_state < PARTIAL) {
    parsing_printf(
        "[%s:%d] Parser::findFuncByEntry([%lx,%lx),%lx) "
        "forced parsing\n",
...
int Parser::findFuncs(CodeRegion *r, Address addr, set<Function *> &funcs) {
  if (_parse_state < COMPLETE) {
    parsing_printf(
        "[%s:%d] Parser::findFuncs([%lx,%lx),%lx,...) "
        "forced parsing\n",
...
int Parser::findFuncs(CodeRegion *r, Address start, Address end,
                      set<Function *> &funcs) {
  if (_parse_state < COMPLETE) {
    parsing_printf(
        "[%s:%d] Parser::findFuncs([%lx,%lx),%lx,%lx) "
        "forced parsing\n",
...
Block *Parser::findBlockByEntry(CodeRegion *r, Address entry) {
  if (_parse_state < PARTIAL) {
    parsing_printf(
        "[%s:%d] Parser::findBlockByEntry([%lx,%lx),%lx) "
        "forced parsing\n",
        FILE__, __LINE__, r->low(), r->high(), entry);
    parse();
  }
...
Block *Parser::findNextBlock(CodeRegion *r, Address addr) {
  if (_parse_state < PARTIAL) {
    parsing_printf(
        "[%s:%d] Parser::findBlockByEntry([%lx,%lx),%lx) "
        "forced parsing\n",
...
int Parser::findBlocks(CodeRegion *r, Address addr, set<Block *> &blocks) {
  if (_parse_state < COMPLETE) {
    parsing_printf(
        "[%s:%d] Parser::findBlocks([%lx,%lx),%lx,...) "
        "forced parsing\n",
        FILE__, __LINE__, r->low(), r->high(), addr);
    parse();
  }
...
Edge *Parser::link(Block *src, Block *dst, EdgeTypeEnum et, bool sink) {
  assert(et != NOEDGE);
 ...
 */
Edge *Parser::link_tempsink(Block *src, EdgeTypeEnum et) {
...
void Parser::relink(Edge *e, Block *src, Block *dst) {
...
ParseFrame::Status Parser::frame_status(CodeRegion *cr, Address addr) {
  // XXX parsing frames may have been cleaned up, but status
  //     is always cached
  return _parse_data->frameStatus(cr, addr);
}

void Parser::remove_func(Function *func) {
...
void Parser::remove_block(Dyninst::ParseAPI::Block *block) {
...
void Parser::resumeFrames(Function *func, vector<ParseFrame *> &work) {
  // If we do not know the function's return status, don't put its waiters back
  // on the worklist
  if (func->_rs == UNSET) {
    parsing_printf("[%s] %s return status unknown, cannot resume waiters\n",
                   __FILE__, func->name().c_str());
    return;
  }

  // When a function's return status is set, all waiting frames back into the
  // worklist
  ...
  if (iter == delayedFrames.end()) {
    ...
    parsing_printf("[%s] %s return status %d, no waiters\n", __FILE__,
                   func->name().c_str(), func->_rs);
    return;
  } else {
    parsing_printf("[%s] %s return status %d, undelaying waiting functions\n",
                   __FILE__, func->name().c_str(), func->_rs);
    // Add each waiting frame back to the worklist
    set<ParseFrame *> vec = iter->second;
    for (set<ParseFrame *>::iterator fIter = vec.begin(); fIter != vec.end();
         ++fIter) {
...
bool Parser::getSyscallNumber(Function * /*func*/, Block *block,
                              Address /*addr*/, Architecture arch,
                              long int &val) {
  val = -1;
