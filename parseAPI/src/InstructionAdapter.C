...
InstructionAdapter::InstructionAdapter(Address start, CodeObject *o,
                                       CodeRegion *r, InstructionSource *isrc,
                                       Block *curBlk)
    : current(start),
      previous((Address)-1),
...
void InstructionAdapter::reset(Address start, CodeObject *o, CodeRegion *r,
                               InstructionSource *isrc, Block *curBlk) {
  current = start;
  previous = (Address)-1;
  parsedJumpTable = false;
  successfullyParsedJumpTable = false;
...
Address InstructionAdapter::getAddr() const { return current; }

Address InstructionAdapter::getPrevAddr() const { return previous; }

Address InstructionAdapter::getNextAddr() const { return current + getSize(); }

FuncReturnStatus InstructionAdapter::getReturnStatus(
    Function *context, unsigned int num_insns) const {
 ...
 */
bool InstructionAdapter::hasUnresolvedControlFlow(
    Function *context, unsigned int num_insns) const {
  if (isDynamicCall()) {
    return true;
  }
  if (getReturnStatus(context, num_insns) ==
      UNKNOWN) {  // true for indirect branches
    return true;
  }
  return false;
}

InstrumentableLevel InstructionAdapter::getInstLevel(
    Function *context, unsigned int num_insns) const {
  bool valid;
  Address target;
