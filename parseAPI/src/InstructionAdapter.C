...
InstructionAdapter::InstructionAdapter(Address start, CodeObject* o, CodeRegion* r,
...
void
InstructionAdapter::reset(Address start, CodeObject* o, CodeRegion* r,
                          InstructionSource* isrc, Block* curBlk)
{
    current                     = start;
    previous                    = (Address) -1;
...
Address
InstructionAdapter::getAddr() const
{
    return current;
}

Address
InstructionAdapter::getPrevAddr() const
{
    return previous;
}

Address
InstructionAdapter::getNextAddr() const
{
    return current + getSize();
}

FuncReturnStatus
InstructionAdapter::getReturnStatus(Function* context, unsigned int num_insns) const
{
 ...
 */
bool
InstructionAdapter::hasUnresolvedControlFlow(Function*    context,
...
InstrumentableLevel
InstructionAdapter::getInstLevel(Function* context, unsigned int num_insns) const
{
    bool    valid;
    Address target;
