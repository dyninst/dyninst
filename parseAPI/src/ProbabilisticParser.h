...
class IdiomPrefixTree {
 ...
 public:
  ...
  bool findChildrenWithArgs(unsigned short arg1, unsigned short arg2,
...
class ProbabilityCalculator {
  struct DecodeData {
    ...
    unsigned short arg2;
    unsigned short len;
    DecodeData(unsigned short e, unsigned short a1, unsigned short a2,
               unsigned short l)
  ...
  // The highest probability reaching to this address in enforcing overlapping
  // constraints
  ...
  double calcForwardWeights(int cur, Address addr, IdiomPrefixTree *tree,
                            bool &valid);
  // Recursively mathcing prefix idioms and calculate weights
  double calcBackwardWeights(int cur, Address addr, IdiomPrefixTree *tree,
                             std::set<IdiomPrefixTree *> &matched);
  // Enforce the overlapping constraints and
  // return true if the cur_addr doesn't conflict with other identified
  // functions,
  // otherwise return false
  bool enforceOverlappingConstraints(
 ...
 public:
  ProbabilityCalculator(CodeRegion *reg, CodeSource *source, Parser *parser,
