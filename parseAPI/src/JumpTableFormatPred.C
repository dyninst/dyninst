...
JumpTableFormatPred::JumpTableFormatPred(ParseAPI::Function* f, ParseAPI::Block* b,
                                         ReachFact& r, ThunkData& t,
...
static bool
ComputeTOC(Address& ret, Address entry, Block* b)
{
    if(!b->region()->isCode(entry))
        return false;
...
void
JumpTableFormatPred::FindTOC()
{
    // Little endian powerpc changes its ABI, which does not have .opd section, but often
    // load R2 at function entry
...
bool
JumpTableFormatPred::modifyCurrentFrame(Slicer::SliceFrame& frame, Graph::Ptr g,
                                        Slicer* s)
{
    if(!jumpTableFormat)
        return false;
    ...
    for(auto rit = frame.active.begin(); rit != frame.active.end(); ++rit)
    {
        if(rit->first.type() == Absloc::Heap || rit->first.type() == Absloc::Stack)
        {
            ...
            else
            {
                ...
                // jump, it is likely to be a jump table. There are two cases to be
                // handled: 1) On ppc, this could be a read from TOC (read from a constant
                // address) 2) This memory read is assumed and likely to be a spill for a
                // certain register. We syntactically find the location where the memory
                // is written and keep slicing on the source register
                SliceNode::Ptr readNode;
                parsing_printf("\t\tfind another memory read %s %s\n",
                               rit->first.format().c_str(),
                               rit->second[0].ptr->format().c_str());
                if(!findRead(g, readNode))
                {
                    parsing_printf(
                        "\tWARNING: a potential memory spill cannot be handled.\n");
                ...
                // 1. delete all absregions introduced by this read node from the active
                // map
                // 2. search for the closest instruction that writes the same memory
                // location,
    ...
    // This map trakcs the expanded and substituted ASTs for each assignment in the
    // current slice
    ...
    while(!working_list.empty())
    {
        ...
        {
            parsing_printf("\tWARNING: Jump table format slice contains unknown "
                           "instructions: %s. Stop slicing along the path\n",
                           n->assign()->insn().format().c_str());
            if(setFirstMemoryRead)
                firstMemoryRead = true;
            g->deleteNode(n);
            // assert( 0 && "AA");
            return false;
        ...
        {
            parsing_printf(
                "\tWARNING: Jump table format slice contains writes to memory\n");
            jumpTableFormat = false;
            return false;
        }
        ...
        {
            inputs.insert(
                make_pair(VariableAST::create(Variable(AbsRegion(Absloc(ppc64::r2)))),
        ...
        if(aliases.find(n->assign()) != aliases.end())
        {
            ...
            parsing_printf("\t Replacing %s with %s\n",
                           aliases[n->assign()].first->format().c_str(),
                           aliases[n->assign()].second->format().c_str());
        ...
        for(; nbegin != nend; ++nbegin)
        {
            ...
            {
                parsing_printf(
                    "\tWARNING: For %s, its predecessor %s does not have an expression\n",
            ...
            parsing_printf("\t\t pred %s at %lx, lhs %s, rhs %s\n",
                           p->assign()->format().c_str(), p->assign()->addr(),
                           lhs->format().c_str(), rhs->format().c_str());
        }
        parsing_printf("Input map:\n");
        for(auto iit = inputs.begin(); iit != inputs.end(); ++iit)
        {
            parsing_printf("\t %s %s\n", iit->first->format().c_str(),
                           iit->second->format().c_str());
        }
        if(g->isExitNode(n))
        {
            ...
            {
                parsing_printf("Only constant target values found so far, no need to "
                               "check jump target format\n");
                return true;
            }
            break;
        }
    ...
    if(!findIndex && jtfv.findIndex)
    {
        ...
        {
            parsing_printf("\tWARNING: found index variable %s, but it is not in the "
                           "active map of the slice frame!\n",
                           index.format().c_str());
            jumpTableFormat = false;
            return false;
        }
        ...
        {
            parsing_printf(
                "\tWARNING: index variable has more than one slicing element!\n");
        }
...
string
JumpTableFormatPred::format()
{
    if(jumpTargetExpr)
        return jumpTargetExpr->format();
    return string("");
}

bool
JumpTableFormatPred::findRead(Graph::Ptr g, SliceNode::Ptr& readNode)
{
    ...
    for(; gbegin != gend; ++gbegin)
    {
        ...
        if(n->assign() && n->assign()->insn().isValid() &&
           n->assign()->insn().readsMemory())
        {
            readNode = n;
            return true;
        }
...
static Assignment::Ptr
SearchForWrite(SliceNode::Ptr n, AbsRegion& src, Slicer::Location& loc, Slicer* s)
{
    ...
    {
        parsing_printf(
            "\tThe instruction has %lu memory read operands, Should have only one\n",
            memReads.size());
    ...
    while(!workingList.empty() && targetBlock == NULL)
    {
        ...
        // we need to make sure we only inspect instructions before the starting
        // instruction
        Address addr = 0;
        if(curBlock == n->block())
        {
        ...
        for(auto iit = insns.rbegin(); iit != insns.rend(); ++iit)
        {
            ...
            // Ideally, we need an architecture independent way to check whether this is a
            // move instruction. Category c_NoCategory excludes lots of non-move
            // instructions
        ...
        for(auto eit = sources.begin(); eit != sources.end(); ++eit)
        {
            ...
            if(e->type() == CATCH)
                continue;
            if(inQueue.find(e->src()) != inQueue.end())
                continue;
...
bool
JumpTableFormatPred::adjustSliceFrame(Slicer::SliceFrame& frame, SliceNode::Ptr n,
                                      Slicer* s)
{
    ...
    for(; nbegin != nend; ++nbegin)
    {
        ...
        frame.active[src].push_back(
            Slicer::Element(next->block(), next->func(), src, next->assign()));
        if(n->assign()->out() != src)
        {
            aliases[next->assign()] =
                make_pair(VariableAST::create(Variable(n->assign()->out())),
                          VariableAST::create(Variable(src)));
        }
    }
    return true;
}
...
bool
JumpTableFormatPred::isTOCRead(Slicer::SliceFrame& frame, SliceNode::Ptr n)
{
    // Delete all active regions introduce by this memory read,
...
bool
JumpTableFormatPred::ignoreEdge(ParseAPI::Edge* e)
{
