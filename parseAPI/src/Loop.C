...
bool
Loop::containsAddress(Address addr)
{
...
bool
Loop::containsAddressInclusive(Address addr)
{
    vector<Block*> blks;
    ...
    {
        if(addr >= blks[i]->start() && addr < blks[i]->end())
            return true;
    }
...
int
Loop::getBackEdges(vector<Edge*>& edges)
{
...
Loop::getLoops(vector<Loop*>& nls, bool outerMostOnly) const
{
    for(auto iter = containedLoops.begin(); iter != containedLoops.end(); ++iter)
    {
        ...
        if(outerMostOnly && (this != (*iter)->parent))
            continue;
        nls.push_back(*iter);
    }
...
bool
Loop::getLoopBasicBlocks(vector<Block*>& bbs)
{
...
void
Loop::insertBlock(Block* b)
{
    if(childBlocks.find(b) == childBlocks.end())
        exclusiveBlocks.insert(b);
}
void
Loop::insertChildBlock(Block* b)
{
...
bool
Loop::getLoopBasicBlocksExclusive(vector<Block*>& bbs)
{
...
bool
Loop::hasBlockExclusive(Block* block)
{
    vector<Block*> blks;
...
int
Loop::getLoopEntries(vector<Block*>& e)
{
...
void
Loop::insertLoop(Loop* childLoop)
{
    containedLoops.insert(childLoop);
    childLoop->parent = this;
    for(auto L = childLoop->containedLoops.begin(); L != childLoop->containedLoops.end();
        ++L)
    {
        containedLoops.insert(*L);
    }
    for(auto b = childLoop->exclusiveBlocks.begin();
        b != childLoop->exclusiveBlocks.end(); ++b)
    {
        insertChildBlock(*b);
    }
    for(auto b = childLoop->childBlocks.begin(); b != childLoop->childBlocks.end(); ++b)
    {
        insertChildBlock(*b);
    }
}
...
Loop::format() const
{
    ...
    for(std::set<Block*>::iterator iter = childBlocks.begin(); iter != childBlocks.end();
        ++iter)
    {
        ret << (*iter)->start() << ", ";
    }
