...
Loop::Loop(const Function* f) : func(f), parent(NULL) {}

Loop::Loop(Edge* be, const Function* f) : func(f), parent(NULL) {
  backEdges.insert(be);
}

bool Loop::containsAddress(Address addr) {
  for (auto i = exclusiveBlocks.begin(); i != exclusiveBlocks.end(); i++) {
    if ((*i)->containsAddr(addr)) return true;
  }
...
bool Loop::containsAddressInclusive(Address addr) {
  ...
  for (unsigned i = 0; i < blks.size(); i++) {
    if (addr >= blks[i]->start() && addr < blks[i]->end()) return true;
  }
...
bool Loop::hasAncestor(Loop* l) {
  return (l->containedLoops.find(this) != l->containedLoops.end());
}

bool Loop::getLoops(vector<Loop*>& nls, bool outerMostOnly) const {
  for (auto iter = containedLoops.begin(); iter != containedLoops.end();
       ++iter) {
...
bool Loop::getContainedLoops(vector<Loop*>& nls) {
  return getLoops(nls, false);
}

// get the outermost loops nested under this loop
bool Loop::getOuterLoops(vector<Loop*>& nls) { return getLoops(nls, true); }
...
bool Loop::getLoopBasicBlocksExclusive(vector<Block*>& bbs) {
  std::copy(exclusiveBlocks.begin(), exclusiveBlocks.end(),
...
bool Loop::hasBlock(Block* block) {
  ...
  for (unsigned i = 0; i < blks.size(); i++)
    if (blks[i]->start() == block->start()) return true;
  return false;
}
...
const Function* Loop::getFunction() { return func; }
...
void Loop::insertLoop(Loop* childLoop) {
  ...
  for (auto L = childLoop->containedLoops.begin();
       L != childLoop->containedLoops.end(); ++L) {
