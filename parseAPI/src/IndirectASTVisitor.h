...
// bool PerformTableRead(StridedInterval &target, set<int64_t> & jumpTargets,
// CodeSource*);
...
public:
    ...
    SimplifyVisitor(Address a, bool k, SymbolicExpression& sym)
    : addr(a)
    , keepMultiOne(k)
    , se(sym)
    {}
...
public:
    ...
    bool IsResultBounded(AST::Ptr ast) { return bound.find(ast.get()) != bound.end(); }
...
public:
    ...
    JumpTableReadVisitor(AbsRegion i, int v, CodeSource* c, CodeRegion* r, bool ze,
                         int m);
    virtual ASTPtr visit(DataflowAPI::RoseAST* ast);
    virtual ASTPtr visit(DataflowAPI::ConstantAST* ast);
    virtual ASTPtr visit(DataflowAPI::VariableAST* ast);
