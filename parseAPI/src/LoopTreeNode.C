...
const char *LoopTreeNode::getCalleeName(unsigned int i) {
  assert(i < callees.size());
  assert(callees[i] != NULL);
  return callees[i]->name().c_str();
}

const char *LoopTreeNode::name() {
  assert(loop != NULL);
  return hierarchicalName;
}

unsigned int LoopTreeNode::numCallees() { return callees.size(); }
...
Loop *LoopTreeNode::findLoop(const char *name) {
  if (loop) {
