...
Block::consistent(Address addr, Address& prev_insn)
{
    ...
    InstructionAdapter_t* ah = InstructionAdapter_t::makePlatformIA_IAPI(
        _obj->cs()->getArch(), dec, _start, _obj, region(), isrc, this);
...
bool
Interproc::pred_impl(Edge* e) const
{
    bool base = EdgePredicate::pred_impl(e);
...
{
    bool base = EdgePredicate::pred_impl(e);
    return base && !e->interproc() && (!_forward || _context->contains(e->trg())) &&
...
Edge::uninstall()
{
    ...
    if(CALL == type())
    {
        ...
        src()->getFuncs(srcFs);
        for(vector<Function*>::iterator fit = srcFs.begin(); fit != srcFs.end(); fit++)
        {
            if(!(*fit)->_cache_valid)
            {
                continue;
...
void
Edge::destroy(Edge* e, CodeObject* o)
{
    o->destroy(e);
}
...
bool
ParseAPI::Block::wasUserAdded() const
{
...
{
    Offset               off = start();
    const unsigned char* ptr = (const unsigned char*) region()->getPtrToInstruction(off);
    if(ptr == NULL)
        return;
...
Block::operator==(const Block& rhs) const
{
    ...
    return _obj == rhs._obj && _region == rhs._region && _start == rhs._start &&
           _end == rhs._end && _lastInsn == rhs._lastInsn && _srclist == rhs._srclist &&
           _trglist == rhs._trglist && _func_cnt == rhs._func_cnt &&
           _parsed == rhs._parsed;
}
