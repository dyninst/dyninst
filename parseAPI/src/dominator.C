...
dominatorCFG::calcDominators()
{
    ...
    for(auto bit = func->blocks().begin(); bit != func->blocks().end(); ++bit)
    {
        ...
        for(auto eit = srcBlock->targets().begin(); eit != srcBlock->targets().end();
            ++eit)
        {
            if((*eit)->interproc() || (*eit)->sinkEdge())
                continue;
...
void
dominatorCFG::calcPostDominators()
{
    set<Block*> exits;
...
void
dominatorCFG::performComputation()
{
    unsigned i, j;
...
void
dominatorCFG::link(dominatorBB* parent, dominatorBB* block)
{
    dominatorBB* s = block;
...
dominatorBB*
dominatorCFG::parseToDomBB(Block* bb)
{
...
void
dominatorCFG::depthFirstSearch(dominatorBB* v)
{
