...
dominatorBB::dominatorBB(Block *bb, dominatorCFG *dc)
  ...
  semiDom = this;
  label = this;
  if (bb) dc->map_[bb->start()] = this;
}

dominatorBB::~dominatorBB() {}

dominatorBB *dominatorBB::eval() {
  if (ancestor == dom_cfg->nullNode) return label;
  compress();
...
void dominatorBB::compress() {
  if (ancestor->ancestor == dom_cfg->nullNode) return;
  ancestor->compress();
  if (ancestor->label->sdno() < label->sdno()) label = ancestor->label;
  ancestor = ancestor->ancestor;
}

int dominatorBB::sdno() { return semiDom->dfs_no; }

dominatorCFG::dominatorCFG(const Function *f) : func(f), currentDepthNo(0) {
...
dominatorCFG::~dominatorCFG() {
  for (unsigned i = 0; i < all_blocks.size(); i++) delete all_blocks[i];
  delete nullNode;
}
...
void dominatorCFG::calcDominators() {
  ...
  for (auto bit = func->blocks().begin(); bit != func->blocks().end(); ++bit) {
    ...
    for (auto eit = srcBlock->targets().begin();
         eit != srcBlock->targets().end(); ++eit) {
  ...
  for (size_t i = 0; i < all_blocks.size(); i++) {
    ...
    if (!bb || !bb->parseBlock || !bb->immDom || !bb->immDom->parseBlock)
      continue;
...
void dominatorCFG::calcPostDominators() {
  set<Block *> exits;
  for (auto bit = func->exitBlocks().begin(); bit != func->exitBlocks().end();
       ++bit)
    exits.insert(*bit);
  // fill in predecessor and successors
  for (auto bit = func->blocks().begin(); bit != func->blocks().end(); ++bit) {
...
void dominatorCFG::performComputation() {
  ...
  for (i = sorted_blocks.size() - 1; i > 0; i--) {
    ...
    for (j = 0; j < block->pred.size(); j++) {
      ...
      if (pred->sdno() < block->sdno()) block->semiDom = pred->semiDom;
    }
...
// parent->bucket.extract(v);
  ...
  for (i = 1; i < sorted_blocks.size(); i++) {
    ...
    if (block->immDom != block->semiDom) {
      if (block->immDom) block->immDom = block->immDom->immDom;
    }
  }
}
...
void dominatorCFG::depthFirstSearch(dominatorBB *v) {
  ...
  for (unsigned i = 0; i < v->succ.size(); i++) {
    ...
    if (successor->dfs_no != -1) continue;
    successor->parent = v;
