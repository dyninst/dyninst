...
void
BoundFactsCalculator::NaturalDFS(Node::Ptr cur)
{
    nodeColor[cur] = 1;
...
void
BoundFactsCalculator::ReverseDFS(Node::Ptr cur)
{
    nodeColor[cur]     = 1;
...
static void
BuildEdgeFromVirtualEntry(SliceNode::Ptr virtualEntry, ParseAPI::Block* curBlock,
                          ...
                          set<ParseAPI::Block*>& visit, GraphPtr slice)
{
    if(targetMap.find(curBlock) != targetMap.end())
    {
        ...
        for(auto nit = targets.begin(); nit != targets.end(); ++nit)
        {
            ...
            slice->insertPair(virtualEntry, trgNode,
                              TypedSliceEdge::create(virtualEntry, trgNode, FALLTHROUGH));
        }
        return;
    }
    if(visit.find(curBlock) != visit.end())
        return;
...
void
BoundFactsCalculator::DetermineAnalysisOrder()
{
    NodeIterator nbegin, nend;
    ...
    for(int curOrder = 1; curOrder <= orderStamp; ++curOrder)
    {
        ...
        if(!HasIncomingEdgesFromLowerLevel(curOrder, curNodes))
        {
            ...
            else
            {
                ...
                BuildEdgeFromVirtualEntry(virtualEntry, virtualEntry->block(), targetMap,
                                          visit, slice);
            }
        }
    }
...
bool
BoundFactsCalculator::HasIncomingEdgesFromLowerLevel(int                curOrder,
                                                     vector<Node::Ptr>& curNodes)
{
    for(auto nit = curNodes.begin(); nit != curNodes.end(); ++nit)
    {
...
bool
BoundFactsCalculator::CalculateBoundedFacts()
{
    ...
    for(int curOrder = 0; curOrder <= orderStamp; ++curOrder)
    {
        ...
        while(!workingList.empty())
        {
            ...
            if(inQueueLimit[curNode] > IN_QUEUE_LIMIT)
                continue;
...
BoundFact*
BoundFactsCalculator::Meet(Node::Ptr curNode)
{
    SliceNode::Ptr node = boost::static_pointer_cast<SliceNode>(curNode);
...
void
BoundFactsCalculator::CalcTransferFunction(Node::Ptr curNode, BoundFact* newFact)
{
    SliceNode::Ptr node = boost::static_pointer_cast<SliceNode>(curNode);
    if(!node->assign())
        return;
    ...
    {
        parsing_printf(
            "\t\t\t No semantic support for this instruction. Assume it does not affect "
            "jump target calculation. Ignore it (Treat as identity function) except for "
            "ptest. ptest should kill the current predicate\n");
        if(id == e_ptest)
        {
    ...
    int      derefSize   = 0;
    if(node->assign() && node->assign()->insn().isValid() &&
    ...
    if(ar.generator() != NULL)
        outAST = se.SimplifyAnAST(
            RoseAST::create(ROSEOperation(ROSEOperation::derefOp, ar.size()),
                            ar.generator()),
            SymbolicExpression::PCValue(node->assign()->addr(), insn.size(),
                                        node->assign()->block()->obj()->cs()->getArch()));

    else
        outAST = VariableAST::create(Variable(ar));
    /*
     * Naively, bsf and bsr produces a bound from 0 to the number of bits of the source
     * operands. In pratice, especially in libc, the real bound is usually smaller than
     * the size of the source operand. Ex 1: shl    %cl,%edx bsf    %rdx,%rcx Here rcx is
     * in range [0,31] rather than [0,63] even though rdx has 64 bits.
     *
     ...
     * It is important to further anaylze the operand in bsf rather than directly conclude
     * the bound
     */
    if(id == e_bsf || id == e_bsr)
    {
        int size = node->assign()->insn().getOperand(0).getValue()->size();
        newFact->GenFact(outAST, new StridedInterval(StridedInterval(1, 0, size * 8 - 1)),
                         false);
    ...
    if(bcv.IsResultBounded(calculation))
    {
        ...
        newFact->GenFact(outAST, new StridedInterval(*bcv.GetResultBound(calculation)),
                         false);
    }
    else
    {
    ...
    newFact->TrackAlias(SymbolicExpression::DeepCopyAnAST(calculation), outAST,
                        findBound);
...
BoundFact*
BoundFactsCalculator::GetBoundFactIn(Node::Ptr node)
{
...
BoundFact*
BoundFactsCalculator::GetBoundFactOut(Node::Ptr node)
{
