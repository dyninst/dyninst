...
static int64_t
ExtendEuclidean(int64_t a, int64_t b, int64_t& x, int64_t& y)
{
...
void
StridedInterval::Join(const StridedInterval& rhs)
{
    ...
    else
    {
        ...
        if(values.size() != 1)
        {
            ...
            for(cur = values.begin(), next = values.begin(), ++next; next != values.end();
                ++cur, ++next)
...
void
StridedInterval::Add(const StridedInterval& rhs)
{
...
void
StridedInterval::Sub(const StridedInterval& minuend)
{
...
void
StridedInterval::And(const StridedInterval& rhs)
{
...
void
StridedInterval::Or(const StridedInterval& rhs)
{
    // consider
...
void
StridedInterval::Xor(const StridedInterval&)
{
...
void
StridedInterval::Mul(const StridedInterval& rhs)
{
...
void
StridedInterval::Div(const StridedInterval& rhs)
{
    if(rhs.stride == 0)
    {
        low /= rhs.low;
...
void
StridedInterval::ShiftLeft(const StridedInterval& rhs)
{
    if(rhs.stride == 0)
    {
...
void
StridedInterval::ShiftRight(const StridedInterval& rhs)
{
    if(rhs.stride == 0)
    {
...
bool
StridedInterval::operator==(const StridedInterval& rhs) const
{
...
void
StridedInterval::Intersect(StridedInterval& rhs)
{
    if(stride == 0)
    {
        if(rhs.stride == 0)
...
void
StridedInterval::DeleteElement(int64_t val)
{
...
bool
StridedInterval::IsConst(int64_t v) const
{
    return (stride == 0) && (low == v) && (high == v);
}

bool
StridedInterval::IsConst() const
{
    return stride == 0;
}

void
StridedInterval::Print()
{
    parsing_printf("%s\n", format().c_str());
}

void
BoundFact::Meet(BoundFact& bf)
{
    ...
    for(auto rit = relation.begin(); rit != relation.end();)
    {
        ...
        {
            if(*((*rit)->left) == *((*it)->left) && *((*rit)->right) == *((*it)->right) &&
               (*rit)->type == (*it)->type)
            {
                find = true;
                break;
...
BoundFact::Print()
{
    ...
    for(auto rit = relation.begin(); rit != relation.end(); ++rit)
    {
        parsing_printf("\t\t\t\t%s and %s, relation: %d\n",
                       (*rit)->left->format().c_str(), (*rit)->right->format().c_str(),
                       (*rit)->type);
    }
...
void
BoundFact::GenFact(const AST::Ptr ast, StridedInterval* bv, bool isConditionalJump)
{
    bv->Print();
...
void
BoundFact::KillFact(const AST::Ptr ast, bool isConditionalJump)
{
    for(auto fit = fact.begin(); fit != fact.end();)
...
bool
BoundFact::operator!=(const BoundFact& bf) const
{
    if(pred != bf.pred)
        return true;
    if(stackTop != bf.stackTop)
        return true;
    if(fact.size() != bf.fact.size())
        return true;
    if(relation.size() != bf.relation.size())
        return true;
    for(size_t i = 0; i < relation.size(); ++i)
        if(*relation[i] != *bf.relation[i])
            return true;
    if(aliasMap.size() != bf.aliasMap.size())
        return true;
    for(auto ait = aliasMap.begin(); ait != aliasMap.end(); ++ait)
    {
        auto bit = bf.aliasMap.find(ait->first);
        if(bit == bf.aliasMap.end())
            return true;
...
bool
BoundFact::ConditionalJumpBound(Instruction insn, EdgeTypeEnum type)
{
    if(!pred.valid)
    {
        parsing_printf("WARNING: We reach a conditional jump, but have not tracked the "
                       "flag! Do nothing and return\n");
        return true;
    }
...
void
BoundFact::SetPredicate(Assignment::Ptr assign, std::pair<AST::Ptr, bool> expandRet)
{
    ...
    if(expandRet.first == NULL)
    {
        ...
        parsing_printf("\t\tNo semantic support for this instruction. Invalidate current "
                       "predicate\n");
        pred.valid = false;
        // SetToBottom();
        return;
            ...
            if(simplifiedAST->getID() == AST::V_RoseAST)
            {
                   ...
                   rootRoseAST->child(0)->getID() == AST::V_RoseAST)
                {
                    ...
                    if(childAST->val().op == ROSEOperation::andOp)
                    {
                        ...
                        {
                            parsing_printf("\t\t For test instruction, now only handle "
                                           "the case where two operands are the same\n");
                        }
                    }
                }
            }
            ...
            if(simplifiedAST->getID() == AST::V_RoseAST)
            {
                   ...
                   rootRoseAST->child(0)->getID() == AST::V_RoseAST)
                {
                    ...
                    if(childAST->val().op == ROSEOperation::andOp)
                    {
                        ...
                        {
                            ConstantAST::Ptr constAST =
                                boost::static_pointer_cast<ConstantAST>(
                                    childAST->child(1));
                        ...
                        {
                            parsing_printf("\t\t None of the operands is constant, do "
...
StridedInterval*
BoundFact::GetBound(const AST::Ptr ast)
{
    return GetBound(ast.get());
}

StridedInterval*
BoundFact::GetBound(const AST* ast)
{
...
void
BoundFact::IntersectInterval(const AST::Ptr ast, StridedInterval si)
{
...
void
BoundFact::DeleteElementFromInterval(const AST::Ptr ast, int64_t val)
{
    StridedInterval* bv = GetBound(ast);
    if(bv != NULL)
    {
...
void
BoundFact::InsertRelation(AST::Ptr left, AST::Ptr right, RelationType r)
{
    ...
    else if(r == NotEqual)
    {
        ...
        // UnsignedLessThanOrEqual can be combined to a more strict relation like
        // UnsignedLessThan
        for(auto rit = relation.begin(); rit != relation.end(); ++rit)
        {
            Relation* re = *rit;
...
void
BoundFact::AdjustPredicate(AST::Ptr out, AST::Ptr in)
{
    if(!pred.valid)
        return;
    if(*out == *pred.e1 && pred.e2->getID() == AST::V_ConstantAST)
    {
        ...
        if(in->getID() == AST::V_RoseAST)
        {
            ...
            if(root->val().op == ROSEOperation::addOp && *pred.e1 == *(root->child(0)) &&
    ...
    else if(*out == *pred.e2 && pred.e1->getID() == AST::V_ConstantAST)
    {
        ...
        {
            RoseAST::Ptr root = boost::static_pointer_cast<RoseAST>(in);
            if(root->val().op == ROSEOperation::addOp && *pred.e2 == *(root->child(0)) &&
               in->child(1)->getID() == AST::V_ConstantAST)
            {
...
AST::Ptr
BoundFact::GetAlias(const AST::Ptr ast)
{
    for(auto ait = aliasMap.begin(); ait != aliasMap.end(); ++ait)
    {
        if(*(ait->first) == *ast)
...
void
BoundFact::TrackAlias(AST::Ptr expr, AST::Ptr outAST, bool findBound)
{
...
void
BoundFact::PushAConst(int64_t value)
{
    stackTop.valid = true;
...
bool
BoundFact::PopAConst(AST::Ptr ast)
{
    if(!stackTop.valid)
        return false;
...
StridedInterval*
BoundFact::ApplyRelations(AST::Ptr outAST)
{
    AST::Ptr cal;
    for(auto ait = aliasMap.begin(); ait != aliasMap.end(); ++ait)
        if(*(ait->first) == *outAST)
    ...
    if(leftChild->child(1)->getID() != AST::V_ConstantAST)
        return NULL;
    ...
    if(invertAST->val().op != ROSEOperation::invertOp)
        return NULL;
    ...
    if(matched)
        return new StridedInterval(StridedInterval(1, 0, baseValue - 1));
    return NULL;
}

StridedInterval*
BoundFact::ApplyRelations2(AST::Ptr outAST)
{
    AST::Ptr cal;
    for(auto ait = aliasMap.begin(); ait != aliasMap.end(); ++ait)
        if(*(ait->first) == *outAST)
    ...
    RoseAST::Ptr     rightChild = boost::static_pointer_cast<RoseAST>(root->child(1));
    if(rightChild->val().op != ROSEOperation::addOp)
        return NULL;
    if(rightChild->child(0)->getID() != AST::V_RoseAST ||
       rightChild->child(1)->getID() != AST::V_ConstantAST)
        return NULL;
