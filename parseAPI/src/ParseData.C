...
ParseWorkElem *ParseFrame::mkWork(ParseWorkBundle *b, Edge *e, Address target,
...
ParseWorkElem *ParseFrame::mkWork(ParseWorkBundle *b, Block *block,
...
StandardParseData::StandardParseData(Parser *p) : ParseData(p) {}

StandardParseData::~StandardParseData() {}

Function *StandardParseData::findFunc(CodeRegion * /* cr */, Address entry) {
  return _rdata.findFunc(entry);
}

Block *StandardParseData::findBlock(CodeRegion * /* cr */, Address entry) {
  return _rdata.findBlock(entry);
}

int StandardParseData::findFuncs(CodeRegion * /* cr */, Address addr,
                                 set<Function *> &funcs) {
  return _rdata.findFuncs(addr, funcs);
}

int StandardParseData::findFuncs(CodeRegion * /* cr */, Address start,
...
Function *StandardParseData::get_func(CodeRegion *cr, Address entry,
                                      FuncSource src) {
  CodeRegion *reg = NULL;
  Function *ret = NULL;
  char name[32];
  ...
  if (!(ret = findFunc(cr, entry))) {
    ...
    if (reg && reg->isCode(entry)) {
      ...
      ret = _parser->factory()._mkfunc(entry, src, name, &_parser->obj(), reg,
                                       _parser->obj().cs());
...
void StandardParseData::record_frame(ParseFrame *pf) {
  _rdata.frame_map[pf->func->addr()] = pf;
}
void StandardParseData::remove_frame(ParseFrame *pf) {
  _rdata.frame_map.erase(pf->func->addr());
}

ParseFrame *StandardParseData::findFrame(CodeRegion * /* cr */, Address addr) {
  if (HASHDEF(_rdata.frame_map, addr))
    return _rdata.frame_map[addr];
  else
    return NULL;
}
ParseFrame::Status StandardParseData::frameStatus(CodeRegion * /* cr */,
                                                  Address addr) {
...
void StandardParseData::setFrameStatus(CodeRegion * /* cr */, Address addr,
                                       ParseFrame::Status status) {
  _rdata.frame_status[addr] = status;
}

CodeRegion *StandardParseData::reglookup(CodeRegion * /* cr */, Address addr) {
...
void StandardParseData::remove_func(Function *f) {
  remove_extents(f->extents());
  _rdata.frame_status.erase(f->addr());
  _rdata.funcsByAddr.erase(f->addr());
}
void StandardParseData::remove_block(Block *b) {
  _rdata.blocksByAddr.erase(b->start());
  _rdata.blocksByRange.remove(b);
}
void StandardParseData::remove_extents(
    const std::vector<FuncExtent *> &extents) {
...
OverlappingParseData::OverlappingParseData(Parser *p,
...
OverlappingParseData::~OverlappingParseData() {
  ...
  for (; it != rmap.end(); ++it) delete it->second;
}

Function *OverlappingParseData::findFunc(CodeRegion *cr, Address entry) {
  if (!HASHDEF(rmap, cr)) return NULL;
  region_data *rd = rmap[cr];
  return rd->findFunc(entry);
}
Block *OverlappingParseData::findBlock(CodeRegion *cr, Address entry) {
  if (!HASHDEF(rmap, cr)) return NULL;
  region_data *rd = rmap[cr];
  return rd->findBlock(entry);
}
int OverlappingParseData::findFuncs(CodeRegion *cr, Address addr,
                                    set<Function *> &funcs) {
  if (!HASHDEF(rmap, cr)) return 0;
  region_data *rd = rmap[cr];
  return rd->findFuncs(addr, funcs);
}
int OverlappingParseData::findFuncs(CodeRegion *cr, Address start, Address end,
                                    set<Function *> &funcs) {
  if (!HASHDEF(rmap, cr)) return 0;
  region_data *rd = rmap[cr];
  return rd->findFuncs(start, end, funcs);
}
int OverlappingParseData::findBlocks(CodeRegion *cr, Address addr,
                                     set<Block *> &blocks) {
  if (!HASHDEF(rmap, cr)) return 0;
  region_data *rd = rmap[cr];
  return rd->findBlocks(addr, blocks);
}
ParseFrame *OverlappingParseData::findFrame(CodeRegion *cr, Address addr) {
  if (!HASHDEF(rmap, cr)) return NULL;
  region_data *rd = rmap[cr];
...
ParseFrame::Status OverlappingParseData::frameStatus(CodeRegion *cr,
                                                     Address addr) {
...
void OverlappingParseData::setFrameStatus(CodeRegion *cr, Address addr,
                                          ParseFrame::Status status) {
  if (!HASHDEF(rmap, cr)) return;
  region_data *rd = rmap[cr];
  rd->frame_status[addr] = status;
}
Function *OverlappingParseData::get_func(CodeRegion *cr, Address addr,
                                         FuncSource src) {
  Function *ret = NULL;
  char name[32];
  ...
  if (!(ret = findFunc(cr, addr))) {
    ...
    if (cr && cr->isCode(addr)) {
      ...
      parsing_printf("[%s] new function for target %lx\n", FILE__, addr);
      ret =
          _parser->factory()._mkfunc(addr, src, name, &_parser->obj(), cr, cr);
      _parser->record_func(ret);
    }
  }
  return ret;
}
region_data *OverlappingParseData::findRegion(CodeRegion *cr) {
  if (!HASHDEF(rmap, cr)) return NULL;
  return rmap[cr];
}
void OverlappingParseData::record_func(Function *f) {
...
void OverlappingParseData::record_block(CodeRegion *cr, Block *b) {
  if (!HASHDEF(rmap, cr)) {
...
void OverlappingParseData::remove_func(Function *f) {
  remove_extents(f->extents());
...
void OverlappingParseData::remove_block(Block *b) {
...
void OverlappingParseData::record_frame(ParseFrame *pf) {
...
void OverlappingParseData::remove_frame(ParseFrame *pf) {
  CodeRegion *cr = pf->codereg;
  if (!HASHDEF(rmap, cr)) {
...
CodeRegion *OverlappingParseData::reglookup(CodeRegion *cr,
                                            Address /* addr */) {
  return cr;
}
