...
void
ParseFrame::set_status(Status s)
{
    _status.store(s);
...
ParseWorkElem*
ParseFrame::mkWork(ParseWorkBundle* b, Edge* e, Address source, Address target,
...
ParseWorkElem*
ParseFrame::mkWork(ParseWorkBundle* b, Block* block, const InsnAdapter::IA_IAPI* ah)
{
    if(!b)
    {
        b = new ParseWorkBundle();
...
ParseWorkElem*
ParseFrame::mkWork(ParseWorkBundle* b, Function* shared_func)
{
    if(!b)
    {
        b = new ParseWorkBundle();
...
int
StandardParseData::findFuncs(CodeRegion* /* cr */, Address addr, set<Function*>& funcs)
{
    return _rdata.findFuncs(addr, funcs);
}

int
StandardParseData::findFuncs(CodeRegion* /* cr */, Address start, Address end,
                             set<Function*>& funcs)
{
    return _rdata.findFuncs(start, end, funcs);
}
int
StandardParseData::findBlocks(CodeRegion* /* cr */, Address addr, set<Block*>& blocks)
{
...
OverlappingParseData::OverlappingParseData(Parser* p, vector<CodeRegion*>& regions)
: ParseData(p)
{
...
int
OverlappingParseData::findFuncs(CodeRegion* cr, Address addr, set<Function*>& funcs)
{
    region_data* rd = findRegion(cr);
    if(rd == NULL)
        return 0;
...
int
OverlappingParseData::findFuncs(CodeRegion* cr, Address start, Address end,
                                set<Function*>& funcs)
{
    region_data* rd = findRegion(cr);
    if(rd == NULL)
...
int
OverlappingParseData::findBlocks(CodeRegion* cr, Address addr, set<Block*>& blocks)
{
    region_data* rd = findRegion(cr);
    if(rd == NULL)
        return 0;
...
OverlappingParseData::createAndRecordFunc(CodeRegion* cr, Address addr, FuncSource src)
{
    ...
    if(cr && cr->isCode(addr))
    {
        ...
        parsing_printf("[%s] new function for target %lx\n", FILE__, addr);
        ret = _parser->factory()._mkfunc(addr, src, name, &_parser->obj(), cr,
                                         _parser->obj().cs());
        if(record_func(ret) == NULL)
        {
            // No need to delete this function that is not registered because
