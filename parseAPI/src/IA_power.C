...
IA_power::IA_power(Dyninst::InstructionAPI::InstructionDecoder dec_, Address start_,
...
{}
IA_power::IA_power(const IA_power& rhs)
: IA_IAPI(rhs)
{}

IA_power*
IA_power::clone() const
{
    return new IA_power(*this);
}

bool IA_power::isFrameSetupInsn(Instruction) const { return false; }

bool
IA_power::isNop() const
{
    return false;
}

bool
IA_power::isThunk() const
{
    return false;
}

bool
IA_power::isTailCall(const Function* context, EdgeTypeEnum type, unsigned int,
                     const set<Address>& knownTargets) const
{
    ...
    if(callee == NULL ||
       (preambleCallee != NULL && callee->src() != HINT && preambleCallee->src() == HINT))
    {
...
bool
IA_power::savesFP() const
{
    return false;
}

bool
IA_power::isStackFramePreamble() const
{
    return false;
}

bool
IA_power::cleansStack() const
{
    return false;
}
...
bool
IA_power::sliceReturn(ParseAPI::Block* bit, Address ret_addr,
                      ParseAPI::Function* func) const
{
    parsing_printf(" sliceReturn ret 0x%lx address 0x%lx func %s addr 0x%lx \n", ret_addr,
                   bit->lastInsnAddr(), func->name().c_str(), func->addr());
    AST::Ptr                          pcDef;
    ...
    InstructionDecoder retdec(_isrc->getPtrToInstruction(retnAddr),
                              InstructionDecoder::maxInstructionLength, _cr->getArch());
...
bool
IA_power::isReturnAddrSave(Address& retAddr) const
{
...
bool
IA_power::isReturn(Dyninst::ParseAPI::Function* context,
    ...
    if(!func->_is_leaf_function)
    {
       ...
       */
        ...
        const unsigned char*         b =
            (const unsigned char*) (this->_isrc->getPtrToInstruction(blockStart));
        ...
        for(iter = copy.allInsns.rbegin(); iter != copy.allInsns.rend(); iter++)
        {
               ...
               iter->second.isWritten(regLR))
            {
                ...
                foundMTLR = true;
                    ...
                    (iter->second.isRead(regSP) || (iter->second.isRead(reg11))) &&
        ...
        parsing_printf(" Slicing for Addr 0x%lx startAddr 0x%lx ret addr 0x%lx func %s\n",
                       getAddr(), currBlk->start(), func->_ret_addr,
                       func->name().c_str());
...
bool
IA_power::isFakeCall() const
{
    return false;
}

bool
IA_power::isIATcall(std::string&) const
{
    return false;
}
...
linker_stub_t
checkLinkerStub(void* insn_buf, Offset& off)
{
    ...
    if((insn[0].asInt() & 0xffff0000) == ADDIS_R12_R2 && insn[1].asInt() == STD_R2_40R1 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[2].asInt() & 0xffff0000) == ADDI_R12_R12 && insn[4].asInt() == MTCTR_R11 &&
       (insn[3].asInt() & 0xffff0000) == LD_R2_0R12 &&
       (insn[5].asInt() & 0xffff0000) == LD_R11_0R12 && insn[6].asInt() == BCTR)
    {
    ...
    if((insn[0].asInt() & 0xffff0000) == ADDIS_R12_R2 && insn[1].asInt() == STD_R2_40R1 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 && insn[4].asInt() == MTCTR_R11 &&
       (insn[3].asInt() & 0xffff0000) == LD_R2_0R12 &&
       (insn[5].asInt() & 0xffff0000) == LD_R11_0R12 && insn[6].asInt() == BCTR)
    {
        off = (DFORM_SI(insn[0]) << 16) + DFORM_SI(insn[2]);
    ...
    if(insn[1].asInt() == STD_R2_40R1 && (insn[2].asInt() & 0xffff0000) == LD_R11_0R2 &&
       (insn[2].asInt() & 0xffff0000) == ADDI_R2_R2 && insn[4].asInt() == MTCTR_R11 &&
       (insn[3].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[5].asInt() & 0xffff0000) == LD_R2_0R12 && insn[6].asInt() == BCTR)
    {
        off = DFORM_SI(insn[1]);
        return STUB_PLT_CALL;
    }
    ...
    if((insn[0].asInt() & 0xffff0000) == ADDIS_R12_R2 && insn[1].asInt() == STD_R2_40R1 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[3].asInt() & 0xffff0000) == LD_R2_0R12 && insn[4].asInt() == MTCTR_R11 &&
       (insn[5].asInt() & 0xffff0000) == LD_R11_0R12 && insn[6].asInt() == BCTR)
    {
        off = (DFORM_SI(insn[0]) << 16) + DFORM_SI(insn[2]);
        return STUB_PLT_CALL;
    ...
    if((insn[0].asInt() & 0xffff0000) == ADDIS_R12_R2 && insn[1].asInt() == STD_R2_40R1 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[3].asInt() & 0xffff0000) == LD_R2_0R12 &&
       insn[4].asInt() == (ADDIS_R12_R12 | 1) && insn[5].asInt() == MTCTR_R11 &&
       (insn[6].asInt() & 0xffff0000) == LD_R11_0R12 && insn[7].asInt() == BCTR)
    {
        off = (DFORM_SI(insn[0]) << 16) + DFORM_SI(insn[2]);
        return STUB_PLT_CALL;
    }
    ...
    if((insn[0].asInt() & 0xffff0000) == ADDIS_R12_R2 && insn[1].asInt() == STD_R2_40R1 &&
       ...
       insn[5].asInt() == (ADDIS_R12_R12 | 1) && insn[6].asInt() == MTCTR_R11 &&
       (insn[7].asInt() & 0xffff0000) == LD_R11_0R12 && insn[8].asInt() == BCTR)
    {
        off = (DFORM_SI(insn[0]) << 16) + DFORM_SI(insn[2]);
        return STUB_PLT_CALL;
    }
    ...
    */
    ...
    if(insn[0].asInt() == LD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == ADDIS_R12_R2 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[3].asInt() & 0xffff0000) == LD_R2_0R12 && insn[4].asInt() == MTCTR_R11 &&
       (insn[5].asInt() & 0xffff0000) == LD_R11_0R12 && insn[6].asInt() == BCTR)
    {
        fprintf(stderr, "WARNING: Pre-binutils 1.15 linker detected. PLT call stubs may "
    ...
    if(insn[0].asInt() == STD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == ADDIS_R2_R2 &&
    ...
    if(insn[0].asInt() == STD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == ADDI_R2_R2 &&
    ...
    if((insn[0].asInt() & 0xffff0000) == ADDIS_R12_R2 &&
       (insn[1].asInt() & 0xffff0000) == LD_R11_0R12 && insn[2].asInt() == MTCTR_R11 &&
       insn[3].asInt() == BCTR)
    {
    ...
    if((insn[0].asInt() & 0xffff0000) == LD_R11_0R2 && insn[1].asInt() == MTCTR_R11 &&
       insn[2].asInt() == BCTR)
    {
        off = DFORM_SI(insn[0]);
        return STUB_TOC_BRANCH;
    ...
    if(insn[0].asInt() == STD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == ADDIS_R12_R2 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[3].asInt() & 0xffff0000) == ADDIS_R2_R2 &&
       (insn[4].asInt() & 0xffff0000) == ADDI_R2_R2 && insn[5].asInt() == MTCTR_R11 &&
       insn[6].asInt() == BCTR)
    {
    ...
    if(insn[0].asInt() == STD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == ADDIS_R12_R2 &&
       (insn[2].asInt() & 0xffff0000) == LD_R11_0R12 &&
       (insn[3].asInt() & 0xffff0000) == ADDI_R2_R2 && insn[4].asInt() == MTCTR_R11 &&
       insn[5].asInt() == BCTR)
    {
        off = (DFORM_SI(insn[1]) << 16) + DFORM_SI(insn[2]);
        return STUB_TOC_BRANCH;
    ...
    // With offset <= 16 bits && r2offset > 16 bits
    if(insn[0].asInt() == STD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == LD_R11_0R2 &&
    ...
    // With offset <= 16 bits && r2offset <= 16 bits
    if(insn[0].asInt() == STD_R2_40R1 && (insn[1].asInt() & 0xffff0000) == LD_R11_0R2 &&
       (insn[2].asInt() & 0xffff0000) == ADDI_R2_R2 && insn[3].asInt() == MTCTR_R11 &&
       insn[4].asInt() == BCTR)
    {
        off = DFORM_SI(insn[1]);
        return STUB_TOC_BRANCH;
...
AST::Ptr
PPC_BLR_Visitor::visit(AST* a)
{
    return a->ptr();
}

AST::Ptr
PPC_BLR_Visitor::visit(DataflowAPI::BottomAST* b)
{
...
AST::Ptr
PPC_BLR_Visitor::visit(DataflowAPI::ConstantAST* c)
{
...
AST::Ptr
PPC_BLR_Visitor::visit(DataflowAPI::VariableAST* v)
{
...
*/
AST::Ptr
PPC_BLR_Visitor::visit(DataflowAPI::RoseAST* r)
{
    ...
    switch(r->val().op)
    {
        case DataflowAPI::ROSEOperation::andOp: {
            ...
            DataflowAPI::ConstantAST::Ptr mask =
                DataflowAPI::ConstantAST::convert(r->child(1));
