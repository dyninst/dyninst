...
Function::~Function() {
  ...
  for (auto lit = _loops.begin(); lit != _loops.end(); ++lit) delete *lit;
}

Function::blocklist Function::blocks() {
...
Function::const_blocklist Function::blocks() const {
...
const Function::edgelist &Function::callEdges() {
  if (!_cache_valid) finalize();
  return _call_edge_list;
}

Function::const_blocklist Function::returnBlocks() {
  if (!_cache_valid) finalize();
  return const_blocklist(ret_begin(), ret_end());
}

Function::const_blocklist Function::exitBlocks() {
  if (!_cache_valid) finalize();
  return const_blocklist(exit_begin(), exit_end());
}

Function::const_blocklist Function::exitBlocks() const {
  assert(_cache_valid);
  return const_blocklist(exit_begin(), exit_end());
}

vector<FuncExtent *> const &Function::extents() {
  if (!_cache_valid) finalize();
  return _extents;
}
...
Function::blocklist Function::blocks_int() {
  if (_cache_valid || !_entry) return blocklist(blocks_begin(), blocks_end());
  ...
  for (auto bit = blocks_begin(); bit != blocks_end(); ++bit) {
    Block *b = *bit;
    visited[b->start()] = 1;
  ...
  // We need to revalidate that the exit blocks we found before are still exit
  // blocks
  ...
  for (curIt = _exitBL.begin(); curIt != _exitBL.end();) {
    ...
    for (auto eit = cur->targets().begin(); eit != cur->targets().end();
         ++eit) {
      Edge *e = *eit;
      Block *t = e->trg();
  ...
  for (auto bit = exit_begin(); bit != exit_end(); ++bit) {
    Block *b = *bit;
    visited[b->start()] = 2;
  }
  ...
  while (!worklist.empty()) {
    ...
    for (Block::edgelist::const_iterator tit = trgs.begin(); tit != trgs.end();
         ++tit) {
      Edge *e = *tit;
      Block *t = e->trg();
      ...
      if (!HASHDEF(visited, t->start())) {
        parsing_printf(
            "\t Adding target block [%lx,%lx) to worklist according to edge "
            "from %lx, type %d\n",
    ...
    if (exits_func) {
      if (link_return) delayed_link_return(_obj, cur);
 ...
 */
void Function::delayed_link_return(CodeObject *o, Block *retblk) {
  ...
  for (; eit != _entry->sources().end(); ++eit) {
    ...
    if (e->type() == CALL) {
      parsing_printf("[%s:%d] linking return edge %lx -> %lx\n", FILE__,
                     __LINE__, retblk->lastInsnAddr(), e->src()->end());
  ...
  if (link_entry) o->add_edge(retblk, _entry, RET);
}
...
const string &Function::name() const { return _name; }

bool Function::contains(Block *b) {
...
void Function::removeBlock(Block *dead) {
  _cache_valid = false;
  // specify replacement entry prior to deleting entry block, unless
  // deleting all blocks
  if (dead == _entry) {
    mal_printf(
        "Warning: removing entry block [%lx %lx) for function at "
        "%lx\n",
  ...
  for (Block::edgelist::const_iterator oit = outs.begin(); outs.end() != oit;
       oit++) {
    switch ((*oit)->type()) {
      case CALL: {
        bool foundEdge = false;
        for (set<Edge *>::iterator cit = _call_edge_list.begin();
             _call_edge_list.end() != cit; cit++) {
          if (*oit == *cit) {
            foundEdge = true;
...
StackTamper Function::tampersStack(bool recalculate) {
  ...
  for (auto bit = retblks.begin(); retblks.end() != bit; ++bit) {
      ...
      if (outReg.absloc().isPC()) {
        ...
        for (in_iter = (*ait)->inputs().begin();
          ...
          if (in_iter->type() != Absloc::Unknown) {
            ...
            mal_printf(
                "Stack tamper analysis for ret block at "
    ...
    mal_printf(
        "StackTamperVisitor for func at 0x%lx block[%lx %lx) w/ "
    ...
    if (TAMPER_UNSET == _tamper || TAMPER_NONE == _tamper ||
        (TAMPER_NONZERO == _tamper && TAMPER_NONE != curtamper)) {
      _tamper = curtamper;
  ...
  if (TAMPER_UNSET == _tamper) {
    mal_printf(
        "WARNING: we found no valid slices for function at %lx "
        "%s[%d]\n",
...
void Function::destroy(Function *f) { f->obj()->destroy(f); }
...
void Function::getLoopsByNestingLevel(vector<Loop *> &lbb,
  ...
  for (std::set<Loop *>::iterator iter = _loops.begin(); iter != _loops.end();
       ++iter) {
    // if we are only getting the outermost loops
    if (outerMostOnly && (*iter)->parentLoop() != NULL) continue;
...
bool Function::getLoops(vector<Loop *> &lbb) const {
  getLoopsByNestingLevel(lbb, false);
  return true;
}

// get the outermost loops in this flow graph
bool Function::getOuterLoops(vector<Loop *> &lbb) const {
...
bool Function::dominates(Block *A, Block *B) const {
  ...
  for (auto bit = immediateDominates[A]->begin();
       bit != immediateDominates[A]->end(); ++bit)
...
bool Function::postDominates(Block *A, Block *B) const {
  ...
  for (auto bit = immediatePostDominates[A]->begin();
       bit != immediatePostDominates[A]->end(); ++bit)
...
void Function::getImmediatePostDominates(Block *A, set<Block *> &imd) const {
  ...
  if (immediatePostDominates[A] != NULL)
    imd.insert(immediatePostDominates[A]->begin(),
               immediatePostDominates[A]->end());
}
