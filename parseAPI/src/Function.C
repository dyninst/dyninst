...
Function::Function(Address addr, string name, CodeObject* obj, CodeRegion* region,
    ...
    while(!worklist.empty())
    {
        ...
        for(auto e : trgs)
        {
            ...
            if(!HASHDEF(visited, e->trg_addr()))
            {
                if(t)
                {
                    parsing_printf("\t Adding target block [%lx,%lx) to worklist "
                                   "according to edge from %lx, type %d\n",
...
void
Function::set_retstatus(FuncReturnStatus rs)
{
    boost::lock_guard<Function> g(*this);
    ...
    if(obj()->cs()->nonReturning(_name) && rs != NORETURN)
        return;
    ...
    {
        parsing_printf(
            "\tERROR: ret status is already set to RETURN, now setting to NORETURN\n");
    }
    if(_rs == NORETURN && rs == RETURN)
    {
        parsing_printf(
            "\tERROR: ret status is already set to NORETURN, now setting to RETURN\n");
    }
...
Function::removeBlock(Block* dead)
{
    ...
    for(Block::edgelist::const_iterator oit = outs.begin(); outs.end() != oit; oit++)
    {
        switch((*oit)->type())
        {
            case CALL: {
    ...
    for(auto bit = retblks.begin(); retblks.end() != bit; ++bit)
    {
        ...
        for(ait = assgns.begin(); assgns.end() != ait; ait++)
        {
            ...
            if(outReg.absloc().isPC())
            {
                ...
                for(in_iter = (*ait)->inputs().begin(); in_iter != (*ait)->inputs().end();
                    ++in_iter)
                {
...
void
Function::destroy(Function* f)
{
    f->obj()->destroy(f);
}

LoopTreeNode*
Function::getLoopTree() const
{
    boost::lock_guard<const Function> g(*this);
    if(_loop_root == NULL)
    {
...
// set. not NULL.
void
Function::getLoopsByNestingLevel(vector<Loop*>& lbb, bool outerMostOnly) const
{
    boost::lock_guard<const Function> g(*this);
    ...
    for(std::set<Loop*>::iterator iter = _loops.begin(); iter != _loops.end(); ++iter)
    {
        // if we are only getting the outermost loops
        if(outerMostOnly && (*iter)->parentLoop() != NULL)
            continue;
...
Loop*
Function::findLoop(const char* name) const
{
    boost::lock_guard<const Function> g(*this);
...
void
Function::fillDominatorInfo() const
{
    boost::lock_guard<const Function> g(*this);
...
void
Function::fillPostDominatorInfo() const
{
    boost::lock_guard<const Function> g(*this);
...
bool
Function::dominates(Block* A, Block* B) const
{
    boost::lock_guard<const Function> g(*this);
    if(A == NULL || B == NULL)
        return false;
    ...
    if(!immediateDominates[A])
        return false;

    for(auto bit = immediateDominates[A]->begin(); bit != immediateDominates[A]->end();
        ++bit)
        if(dominates(*bit, B))
            return true;
    return false;
...
Block*
Function::getImmediateDominator(Block* A) const
{
    boost::lock_guard<const Function> g(*this);
    fillDominatorInfo();
...
void
Function::getImmediateDominates(Block* A, set<Block*>& imd) const
{
    boost::lock_guard<const Function> g(*this);
    fillDominatorInfo();
...
void
Function::getAllDominates(Block* A, set<Block*>& d) const
{
    boost::lock_guard<const Function> g(*this);
    fillDominatorInfo();
    d.insert(A);
...
bool
Function::postDominates(Block* A, Block* B) const
{
    boost::lock_guard<const Function> g(*this);
    if(A == NULL || B == NULL)
        return false;
    ...
    if(!immediatePostDominates[A])
        return false;

    for(auto bit = immediatePostDominates[A]->begin();
        bit != immediatePostDominates[A]->end(); ++bit)
        if(postDominates(*bit, B))
            return true;
    return false;
}

Block*
Function::getImmediatePostDominator(Block* A) const
{
    boost::lock_guard<const Function> g(*this);
    fillPostDominatorInfo();
...
void
Function::getImmediatePostDominates(Block* A, set<Block*>& imd) const
{
    boost::lock_guard<const Function> g(*this);
    fillPostDominatorInfo();
...
void
Function::getAllPostDominates(Block* A, set<Block*>& d) const
{
    boost::lock_guard<const Function> g(*this);
    fillPostDominatorInfo();
    d.insert(A);
