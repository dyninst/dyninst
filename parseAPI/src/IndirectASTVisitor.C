...
AST::Ptr
SimplifyVisitor::visit(DataflowAPI::RoseAST* ast)
{
...
AST::Ptr
BoundCalcVisitor::visit(DataflowAPI::RoseAST* ast)
{
    ...
    switch(ast->val().op)
    {
        case ROSEOperation::addOp:
            ...
            {
                StridedInterval* val =
                    new StridedInterval(*GetResultBound(ast->child(0)));
        ...
        case ROSEOperation::ifOp:
            ...
            {
                StridedInterval* val =
                    new StridedInterval(*GetResultBound(ast->child(1)));
...
AST::Ptr
BoundCalcVisitor::visit(DataflowAPI::ConstantAST* ast)
{
...
AST::Ptr
BoundCalcVisitor::visit(DataflowAPI::VariableAST* ast)
{
    StridedInterval* astBound = boundFact.GetBound(ast);
    if(astBound != NULL)
        bound.insert(make_pair(ast, new StridedInterval(*astBound)));
...
StridedInterval*
BoundCalcVisitor::GetResultBound(AST::Ptr ast)
{
...
AST::Ptr
JumpCondVisitor::visit(DataflowAPI::RoseAST* ast)
{
...
AST::Ptr
ComparisonVisitor::visit(DataflowAPI::RoseAST* ast)
{
            ...
            if(childRose->val().op == ROSEOperation::addOp)
            {
                ...
                if(minuend->getID() == AST::V_ConstantAST)
                {
                    ConstantAST::Ptr constAST =
                        boost::static_pointer_cast<ConstantAST>(minuend);
                ...
                else if(minuend->getID() == AST::V_RoseAST)
                {
                    ...
                    minuend          = AST::Ptr();
                    if(sub->val().op == ROSEOperation::addOp &&
                       sub->child(0)->getID() == AST::V_RoseAST)
                    {
                        sub = boost::static_pointer_cast<RoseAST>(sub->child(0));
                        if(sub->val().op == ROSEOperation::invertOp)
                        {
                            // Otherwise, the minuend ast is in the form of
                            // add(invert(minuend), 1) Need to extract the real minuend
                            minuend = sub->child(0);
                        }
                    }
                }
...
AST::Ptr
JumpTableFormatVisitor::visit(DataflowAPI::RoseAST* ast)
{
    if(ast->val().op == ROSEOperation::derefOp)
    {
        ...
        {
            parsing_printf(
                "More than one layer of memory accesses, not jump table format\n");
    ...
    if(ast->val().op == ROSEOperation::addOp && memoryReadLayer > 0 && firstAdd)
    {
        firstAdd          = false;
        Address tableBase = 0;
        if(ast->child(0)->getID() == AST::V_ConstantAST &&
           PotentialIndexing(ast->child(1)))
        {
            ConstantAST::Ptr constAST =
                boost::static_pointer_cast<ConstantAST>(ast->child(0));
            tableBase = (Address) constAST->val().val;
        }
        if(ast->child(1)->getID() == AST::V_ConstantAST &&
           PotentialIndexing(ast->child(0)))
        {
            ConstantAST::Ptr constAST =
                boost::static_pointer_cast<ConstantAST>(ast->child(1));
            tableBase = (Address) constAST->val().val;
        }
        if(tableBase)
        {
            ...
            /*
                    if (!b->obj()->cs()->isReadOnly(tableBase)) {
                        parsing_printf("\ttableBase 0x%lx not read only, not jump table
               format\n", tableBase); findIncorrectFormat = true; return AST::Ptr();
                    }
            */
            findTableBase = true;
        }
    ...
    if((ast->val().op == ROSEOperation::uMultOp ||
        ...
        ast->val().op == ROSEOperation::rotateLOp) &&
       memoryReadLayer > 0)
    {
        if(ast->child(0)->getID() == AST::V_ConstantAST &&
           ast->child(1)->getID() == AST::V_VariableAST)
        {
            ConstantAST::Ptr constAst =
                boost::static_pointer_cast<ConstantAST>(ast->child(0));
            if(!((ast->val().op == ROSEOperation::uMultOp ||
                  ast->val().op == ROSEOperation::sMultOp) &&
                 !findTableBase && constAst->val().val == 1))
            {
                findIndex = true;
                numOfVar++;
                VariableAST::Ptr varAst =
                    boost::static_pointer_cast<VariableAST>(ast->child(1));
                index = varAst->val().reg;
                return AST::Ptr();
            }
        }
        if(ast->child(1)->getID() == AST::V_ConstantAST &&
           ast->child(0)->getID() == AST::V_VariableAST)
        {
            ConstantAST::Ptr constAst =
                boost::static_pointer_cast<ConstantAST>(ast->child(1));
            if(!((ast->val().op == ROSEOperation::uMultOp ||
                 ...
                 !findTableBase && constAst->val().val == 1))
            {
                ...
                numOfVar++;
                VariableAST::Ptr varAst =
                    boost::static_pointer_cast<VariableAST>(ast->child(0));
                index = varAst->val().reg;
                return AST::Ptr();
            }
        }
...
AST::Ptr
JumpTableFormatVisitor::visit(DataflowAPI::VariableAST*)
{
    numOfVar++;
    return AST::Ptr();
}
...
bool
JumpTableFormatVisitor::PotentialIndexing(AST::Ptr ast)
{
    ...
    if(ast->getID() == AST::V_RoseAST)
    {
        ...
        if(r->val().op == ROSEOperation::addOp)
        {
            ...
            if(r->child(0)->getID() == AST::V_RoseAST &&
               r->child(1)->getID() == AST::V_ConstantAST)
            {
                RoseAST::Ptr     lc = boost::static_pointer_cast<RoseAST>(r->child(0));
                ConstantAST::Ptr rc =
                    boost::static_pointer_cast<ConstantAST>(r->child(1));
...
JumpTableReadVisitor::JumpTableReadVisitor(AbsRegion i, int v, CodeSource* c,
                                           CodeRegion* r, bool ze, int m)
{
    index          = i;
    indexValue     = v;
    cs             = c;
...
AST::Ptr
JumpTableReadVisitor::visit(DataflowAPI::RoseAST* ast)
{
    unsigned totalChildren = ast->numChildren();
    for(unsigned i = 0; i < totalChildren; ++i)
    {
    ...
    switch(ast->val().op)
    {
        ...
        default:
            parsing_printf(
                "WARNING: unhandled operation in the jump table format AST!\n");
            valid = false;
            break;
    }
...
AST::Ptr
JumpTableReadVisitor::visit(DataflowAPI::ConstantAST* ast)
{
    const Constant& v     = ast->val();
    int64_t         value = v.val;
    if(v.size != 1 && v.size != 64 && (value & (1ULL << (v.size - 1))))
...
AST::Ptr
JumpTableReadVisitor::visit(DataflowAPI::VariableAST* var)
{
    if(var->val().reg != index)
    {
        ...
        parsing_printf("WARNING: the jump table format AST contains a variable that is "
                       "not the index\n");
        valid = false;
    }
...
bool
JumpTableReadVisitor::PerformMemoryRead(Address addr, int64_t& v)
{
    readAddress      = addr;
    ...
    addr -= cs->loadAddress();
#endif
    if(!cs->isCode(addr) && !cs->isData(addr))
        return false;
