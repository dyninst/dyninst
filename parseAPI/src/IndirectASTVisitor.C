  ...
  if (ast->getID() == AST::V_RoseAST) {
    ...
    switch (roseAST->val().op) {
      case ROSEOperation::invertOp:
        if (roseAST->child(0)->getID() == AST::V_RoseAST) {
          RoseAST::Ptr child =
              boost::static_pointer_cast<RoseAST>(roseAST->child(0));
          if (child->val().op == ROSEOperation::invertOp)
            return child->child(0);
        } else if (roseAST->child(0)->getID() == AST::V_ConstantAST) {
          ConstantAST::Ptr child =
      ...
      case ROSEOperation::addOp:
        ...
        // Case 3: if v + v * c = v * (c+1), where v is a variable and c is a
        // constant
        if (roseAST->child(0)->getID() == AST::V_VariableAST &&
            roseAST->child(1)->getID() == AST::V_RoseAST) {
          RoseAST::Ptr rOp =
              boost::static_pointer_cast<RoseAST>(roseAST->child(1));
          if (rOp->val().op == ROSEOperation::uMultOp ||
              rOp->val().op == ROSEOperation::sMultOp) {
            if (rOp->child(0)->getID() == AST::V_VariableAST &&
                rOp->child(1)->getID() == AST::V_ConstantAST) {
              VariableAST::Ptr varAST1 =
                  boost::static_pointer_cast<VariableAST>(roseAST->child(0));
              VariableAST::Ptr varAST2 =
                  boost::static_pointer_cast<VariableAST>(rOp->child(0));
              if (varAST1->val().reg == varAST2->val().reg) {
                ConstantAST::Ptr oldC =
                    boost::static_pointer_cast<ConstantAST>(rOp->child(1));
                ConstantAST::Ptr newC = ConstantAST::create(
                    Constant(oldC->val().val + 1, oldC->val().size));
                RoseAST::Ptr newRoot =
                    RoseAST::create(ROSEOperation(rOp->val()), varAST1, newC);
                return newRoot;
              }
            }
          }
      ...
      case ROSEOperation::xorOp:
        if (roseAST->child(0)->getID() == AST::V_VariableAST &&
            roseAST->child(1)->getID() == AST::V_VariableAST) {
          VariableAST::Ptr child0 =
              boost::static_pointer_cast<VariableAST>(roseAST->child(0));
          VariableAST::Ptr child1 =
              boost::static_pointer_cast<VariableAST>(roseAST->child(1));
      ...
      case ROSEOperation::derefOp:
        ...
        if (roseAST->val().size == 8)
          ...
          return RoseAST::create(ROSEOperation(ROSEOperation::derefOp),
                                 ast->child(0));
        break;
      default:
        break;
    ...
    if (varAST->val().reg.absloc().isPC()) {
      ...
      return ConstantAST::create(
          Constant(varAST->val().addr + insnSize,
                   getArchAddressWidth(pc.getArchitecture()) * 8));
    }
  ...
  switch (ast->val().op) {
    case ROSEOperation::addOp:
      if (IsResultBounded(ast->child(0)) && IsResultBounded(ast->child(1))) {
        ...
        if (*val != BoundValue::top) bound.insert(make_pair(ast, val));
      }
      break;
    case ROSEOperation::invertOp:
    ...
    case ROSEOperation::derefOp:
      if (IsResultBounded(ast->child(0))) {
        ...
        bound.insert(
            make_pair(ast, new BoundValue(StridedInterval(1, 0, 255))));
      }
      break;
...
AST::Ptr BoundCalcVisitor::visit(DataflowAPI::VariableAST *ast) {
  BoundValue *astBound = boundFact.GetBound(ast);
  if (astBound != NULL) bound.insert(make_pair(ast, new BoundValue(*astBound)));
  return AST::Ptr();
}
...
BoundCalcVisitor::~BoundCalcVisitor() {
  for (auto bit = bound.begin(); bit != bound.end(); ++bit)
    if (bit->second != NULL) delete bit->second;
  bound.clear();
}
    ...
    if (child->getID() == AST::V_RoseAST) {
      ...
      if (childRose->val().op == ROSEOperation::addOp) {
        ...
        if (minuend->getID() == AST::V_ConstantAST) {
          ...
          // Otherwise, the minuend ast is in the form of add(invert(minuend),
          // 1)
  ...
  if (ast->val().op == ROSEOperation::derefOp) {
    ...
    if (ast->child(0)->getID() == AST::V_RoseAST) {
      ...
      if (roseAST->val().op == ROSEOperation::derefOp) {
        ...
        parsing_printf(
            "Two directly nested memory access, not jump table format\n");
        ...
        if (roseAST->child(1)->getID() == AST::V_ConstantAST &&
            roseAST->child(0)->getID() == AST::V_VariableAST) {
          ConstantAST::Ptr constAST =
              boost::static_pointer_cast<ConstantAST>(roseAST->child(1));
          tableBase = (Address)constAST->val().val;
        }
        if (tableBase) {
          ...
          if (!b->obj()->cs()->isReadOnly(tableBase)) {
            parsing_printf(
                "\ttableBase 0x%lx not read only, not jump table format\n",
                tableBase);
            findIncorrectFormat = true;
          }
        }
