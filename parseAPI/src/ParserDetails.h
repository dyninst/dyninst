...
static inline bool is_code(Function *f, Address addr) {
  return f->region()->isCode(addr) || f->isrc()->isCode(addr);
}
}
...
class ParseWorkElem {
 public:
   ...
   *
   * 2. We have a tail call heuristics that a jump to its own block is not a
   * tail call.
   ...
   * over potential tail call edges such as cond_taken, br_direct, and
   * br_indirect.
   *
   * 3. Jump table analysis would like to have as much intraprocedural control
   * flow
  ...
  enum parse_work_order {
    ...
    catch_block,
    checked_call_ft,
    resolve_jump_table,  // We want to finish all possible parsing work before
                         // parsing jump tables
    __parse_work_end__
  };

  // allow direct access to setting order/frame type..
  ParseWorkElem(ParseWorkBundle *b, parse_work_order o, Edge *e, Address target,
  ...
  ParseWorkElem(ParseWorkBundle *b, Edge *e, Address target, bool resolvable,
                bool tailcall)
      : _bundle(b),
        _edge(e),
        _targ(target),
...

  {
    if (e) {
      switch (e->type()) {
        ...
        case FALLTHROUGH:
          _order = ret_fallthrough;
          break;
        case CATCH:
          _order = catch_block;
          break;
        case CALL_FT:
          _order = call_fallthrough;
          break;
        default:
  ...
  // parsing jump tables
  ParseWorkElem(ParseWorkBundle *bundle, Block *b,
...
class ParseWorkBundle {
 public:
  ParseWorkBundle() {}
  ~ParseWorkBundle() {
    for (unsigned i = 0; i < _elems.size(); ++i) delete _elems[i];
  }
