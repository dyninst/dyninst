 ...
 */
static inline bool
is_code(Function* f, Address addr)
{
...
public:
    ...
    enum parse_work_order
    {
        ...
        catch_block,
        checked_call_ft,
        resolve_jump_table,  // We want to finish all possible parsing work before parsing
                             // jump tables For shared code, we only parse once. The
                             // return statuses of the functions that share code depend on
                             // the function that performs the real parsing
        func_shared_code,
        __parse_work_end__
    };

    // allow direct access to setting order/frame type..
    ParseWorkElem(ParseWorkBundle* b, parse_work_order o, Edge* e, Address source,
                  Address target, bool resolvable, bool tailcall)
    : _bundle(b)
    , _edge(e)
    , _src(source)
    , _targ(target)
    ...
    ParseWorkElem(ParseWorkBundle* b, Edge* e,
                  /* We also the source address of the edge because the source block
                   * may be split */
                  Address source, Address target, bool resolvable, bool tailcall)
    : _bundle(b)
    , _edge(e)
    , _src(source)
    , _targ(target)
