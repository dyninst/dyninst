    ...
    switch(curEdge->second)
    {
        ...
        default:
            parsing_printf("%s[%d]: adding unknown edge type %d edge %lx->%lx\n", FILE__,
                           __LINE__, curEdge->second, currAddr, curEdge->first);
            break;
    }
...
getBlockInsns(Block& blk, std::set<Address>& addrs)
{
    ...
    const unsigned char* bufferBegin =
        (const unsigned char*) (blk.region()->getPtrToInstruction(blk.start()));
    ...
    InstructionAdapter_t* ah = InstructionAdapter_t::makePlatformIA_IAPI(
        blk.obj()->cs()->getArch(), dec, blk.start(), blk.obj(), blk.region(),
        blk.obj()->cs(), &blk);
...
Parser::getTamperAbsFrame(Function* tamperFunc)
{
    ...
    targFunc =
        _parse_data->createAndRecordFunc(*(targRegs.begin()), target, tamperFunc->src());
...
Parser::tamper_post_processing(LockFreeQueue<ParseFrame*>& work_queue, ParseFrame* pf)
{
    ...
    for(auto widx = work.begin();
        pf->func->tampersStack() == TAMPER_REL && widx != work.end(); widx++)
    {
        if((*widx)->status() == ParseFrame::CALL_BLOCKED &&
           pf->func == (*widx)->call_target)
        {
            for(unsigned bidx = 0; bidx < (*widx)->work_bundles.size(); bidx++)
            {
 ...
 */
inline void
Parser::ProcessUnresBranchEdge(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah,
                               Address target)
{
    ParseCallback::interproc_details det;
    det.ibuf  = (unsigned char*) frame.func->isrc()->getPtrToInstruction(ah->getAddr());
 ...
 */
inline void
Parser::ProcessReturnInsn(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah)
{
 ...
 */
inline void
Parser::ProcessCallInsn(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah,
                        bool isDynamic, bool isAbsolute, bool isResolved, Address target)
{
    ParseCallback::interproc_details det;
    det.ibuf  = (unsigned char*) frame.func->isrc()->getPtrToInstruction(ah->getAddr());
    det.isize = ah->getSize();
...
bool
Parser::ProcessCFInsn(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah)
{
            ...
            !hasUnalignedEdge && curEdge != edges_out.end(); ++curEdge)
        {
            ...
            if(!tregs.empty())
            {
                    ...
                    !hasUnalignedEdge && bit != tblocks.end(); bit++)
                {
                    ...
                    if(hasUnalignedEdge)
                    {
                        mal_printf("Found unaligned blocks [%lx %lx) [%lx %lx), adding "
                                   "abruptEnd "
                                   "point and killing out edges\n",
                                   cur->start(), cur->end(), (*bit)->start(),
                                   (*bit)->end());
                    }
                }
            }
    ...
    if(unlikely(insn_ret != UNSET && frame.func->_rs < RETURN) &&
       !HASHDEF(plt_entries, frame.func->addr()))
    {
    ...
    for(Edges_t::iterator curEdge = edges_out.begin(); curEdge != edges_out.end();
        ++curEdge)
    {
        Edge* newedge         = NULL;
        bool  resolvable_edge = true;
        bool  tailcall        = false;
        parsing_printf("\t\t\tedge target %lx, is_code %d, is_plt %d, edge type %d, "
                       "dynamic_call %d\n",
                       curEdge->first, is_code(frame.func, curEdge->first),
                       HASHDEF(plt_entries, curEdge->first), curEdge->second,
                       dynamic_call);
        if(!is_code(frame.func, curEdge->first) && !HASHDEF(plt_entries, curEdge->first))
        {
        ...
        if(curEdge->second == CALL)
        {
            ...
            ProcessCallInsn(frame, cur, ah, dynamic_call, absolute_call, resolvable_edge,
                            curEdge->first);
            ...
            if(!ah->isCall())
            {
                parsing_printf("Setting edge 0x%p (0x%lx/0x%lx) to interproc\n",
                               (void*) newedge, newedge->src()->start(),
                               newedge->trg_addr());
                          ...
                          frame.knownTargets))
        {
            ...
            parsing_printf("Setting edge 0x%p (0x%lx/0x%lx) to interproc (tail call)\n",
                           (void*) newedge, newedge->src()->start(), newedge->trg_addr());
            newedge->_type._interproc = true;
        }
        ...
        ParseWorkElem* we = bundle->add(new ParseWorkElem(
            bundle, newedge, ah->getAddr(), curEdge->first, resolvable_edge, tailcall));
