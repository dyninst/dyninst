...
static const vector<std::string> skipped_symbols = { "_non_rtti_object::`vftable'",
                                                     "bad_cast::`vftable'",
                                                     "exception::`vftable'",
                                                     "bad_typeid::`vftable'",
                                                     "sys_errlist",
...
SymtabCodeRegion::SymtabCodeRegion(SymtabAPI::Symtab* st, SymtabAPI::Region* reg)
: _symtab(st)
, _region(reg)
{
    ...
    for(auto sit = symbols.begin(); sit != symbols.end(); ++sit)
        if((*sit)->getRegion() == reg &&
           ...
           (*sit)->getType() != SymtabAPI::Symbol::ST_INDIRECT)
        {
            if((*sit)->getRegion()->isText())
                continue;
            knownData[(*sit)->getOffset()] = (*sit)->getOffset() + (*sit)->getSize();
            parsing_printf("Add known data range [%lx, %lx) from symbol %s\n",
                           (*sit)->getOffset(), (*sit)->getOffset() + (*sit)->getSize(),
                           (*sit)->getMangledName().c_str());
        }
}
...
SymtabCodeRegion::isReadOnly(const Address addr) const
{
    if(!contains(addr))
        return false;
    if(_region->getRegionName() == ".data.rel.ro")
        return true;
...
#pragma omp parallel for shared(regs, dregs, symbols, reg_lock) schedule(auto)
    for(unsigned int i = 0; i < regs.size(); i++)
    {
        ...
        parsing_printf("   %lx %s", r->getMemOffset(), r->getRegionName().c_str());
        ...
        if(false == allLoadedRegions && rt != SymtabAPI::Region::RT_TEXT &&
           rt != SymtabAPI::Region::RT_DATA && rt != SymtabAPI::Region::RT_TEXTDATA)
        {
...
#pragma omp parallel for schedule(auto)
    for(unsigned int i = 0; i < fsyms.size(); i++)
    {
        ...
        if(std::find(skipped_symbols.begin(), skipped_symbols.end(), fname_s) !=
           skipped_symbols.end())
        {
            continue;
        }
        ...
        if(!present)
        {
            parsing_printf("[%s:%d] unrecognized Region %lx in function %lx\n", FILE__,
                           __LINE__, sr->getMemOffset(), f->getOffset());
            continue;
        }
...
SymtabCodeSource::init_linkage()
{
    ...
    if(_symtab->findRegion(plt_sec, ".plt.sec"))
    {
        ...
        if(!_symtab->findRegion(rela_plt, ".rela.plt"))
            return;
        ...
        for(size_t off = 0; off < plt_sec->getMemSize(); off += plt_entry_size)
        {
            ...
            if(rel_addr_to_name.find(rel_addr) != rel_addr_to_name.end())
            {
                ...
                if(rel_addr_to_name[rel_addr] == "")
                {
                    ...
                    // One such case is to perform a call to a ifunc to use CPU dispatch
                    // feature
                    char ifunc_name[128];
    ...
    if(!_symtab->findRegion(plt_got, ".plt.got"))
        return;
    if(!_symtab->findRegion(rela_dyn, ".rela.dyn"))
        return;
    if(plt_got->getMemSize() <= 16)
        return;
...
bool
SymtabCodeSource::findCatchBlockByTryRange(Address            addr,
                                           std::set<Address>& catchStarts) const
{
    try_block target(addr, 0, 0);
    catchStarts.clear();
    std::vector<try_block>::const_iterator bit =
        upper_bound(try_blocks.begin(), try_blocks.end(), target);
    if(bit == try_blocks.begin())
        return false;
    --bit;
