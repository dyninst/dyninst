...
public:
    ...
    ParseWorkElem* mkWork(ParseWorkBundle* b, Edge* e, Address source, Address target,
                          bool resolvable, bool tailcall);
    ParseWorkElem* mkWork(ParseWorkBundle* b, Block* block,
                          const InsnAdapter::IA_IAPI* ah);
    ParseWorkElem* mkWork(ParseWorkBundle* b, Function* shared_func);
    ...
    worklist_t worklist;
    std::set<Address>
        knownTargets;  // This set contains known potential targets in this function
...
public:
    ...
    inline std::pair<Address, Block*> get_next_block(Address addr)
    {
        ...
        if((nextBlock = blocksByRange.successor(addr)) && nextBlock->start() > addr)
        {
    ...
    Block* record_block(Block* b)
    {
        Block* ret = NULL;
        {
            ...
            // Inserting failed when another thread has inserted a block with the same
            // starting address
            if(!inserted)
            {
                ret = a->second;
    ...
    void insertBlockByRange(Block* b) { blocksByRange.insert(b); }
...
public:
    ...
    virtual edge_parsing_data           setEdgeParsingStatus(CodeRegion* cr, Address addr,
                                                             Function* f, Block* b)  = 0;
...
public:
    ...
    edge_parsing_data setEdgeParsingStatus(CodeRegion* cr, Address addr, Function* f,
                                           Block* b);
...
inline region_data*
StandardParseData::findRegion(CodeRegion* /* cr */)
{
    return &_rdata;
}
inline Function*
StandardParseData::record_func(Function* f)
{
    return _rdata.record_func(f);
}
inline Block*
StandardParseData::record_block(CodeRegion* /* cr */, Block* b)
{
    return _rdata.record_block(b);
}

inline edge_parsing_data
StandardParseData::setEdgeParsingStatus(CodeRegion*, Address addr, Function* f, Block* b)
{
    return _rdata.set_edge_parsed(addr, f, b);
}

inline void
StandardParseData::getAllRegionData(std::vector<region_data*>& rds)
{
    rds.push_back(&_rdata);
}
inline region_data::edge_data_map*
StandardParseData::get_edge_data_map(CodeRegion*)
{
