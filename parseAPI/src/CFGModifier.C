 ...
 * same type already) */
bool
CFGModifier::redirect(Edge* edge, Block* target)
{
    ...
    if(!target)
    {
        target     = new Block(edge->src()->obj(), edge->src()->region(),
    ...
    if(hasSink)
    {  // sink & src are in same object, so remove edge from src->obj()
        Block*      src = edge->src();
...
Block*
CFGModifier::split(Block* b, Address a, bool trust, Address newlast)
{
    if(!trust)
    {
    ...
    // 2b)
    for(Block::edgelist::iterator iter = b->_trglist.begin(); iter != b->_trglist.end();
        ++iter)
    {
    ...
    // 4)
    for(std::vector<Function*>::iterator iter = funcs.begin(); iter != funcs.end();
        ++iter)
    {
...
bool
CFGModifier::remove(vector<Block*>& blks, bool force)
{
    ...
    for(vector<Block*>::iterator bit = blks.begin(); bit != blks.end(); bit++)
    {
            ...
            iter != blocksToFuncs[b].end(); ++iter)
        {
            ...
            func->removeBlock(b);
            if(b->_func_cnt > 0)
            {  // remove func if count is not 0, in which case
        ...
        if(!b->_srclist.empty())
        {
                ...
                iter != b->_srclist.end(); ++iter)
            {
                ...
                edge->src()->obj()->_pcb->removeEdge(edge->src(), edge,
                                                     ParseCallback::target);
            ...
            iter != b->_trglist.end(); ++iter)
        {
            ...
            //       } else {
            //          // we don't actually wire up edges to the sink block in the
            //          PatchAPI,
        ...
        for(vector<Edge*>::iterator eit = deadEdges.begin(); eit != deadEdges.end();
            eit++)
        {
    ...
    // 6)
    for(std::set<Function*>::iterator iter = allFuncs.begin(); iter != allFuncs.end();
        ++iter)
    {
        Function* func     = *iter;
...
CFGModifier::remove(Function* f)
{
    ...
    for(Function::blocklist::iterator iter = blocks.begin(); iter != blocks.end(); ++iter)
    {
        (*iter)->removeFunc(f);
        if((*iter)->_func_cnt == 0)
        {
...
InsertedRegion*
CFGModifier::insert(CodeObject* obj, Address base, void* data, unsigned size)
{
...
InsertedRegion::~InsertedRegion() { free(buf_); }
