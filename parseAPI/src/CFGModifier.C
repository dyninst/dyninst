...
bool CFGModifier::redirect(Edge *edge, Block *target) {
  ...
  if (hasSink) {  // sink & src are in same object, so remove edge from
                  // src->obj()
    Block *src = edge->src();
    ...
    if (!edge->sinkEdge()) {
      ...
      oldTarget->obj()->_pcb->removeEdge(oldTarget, edge,
  ...
  // If this is an intraprocedural edge it may have changed function boundaries,
  // so we
  ...
  // And if this was an interprocedural edge, mark the target as a function
  // entry
...
bool CFGModifier::remove(vector<Block *> &blks, bool force) {
  ...
  for (vector<Block *>::iterator bit = blks.begin(); bit != blks.end(); bit++) {
    ...
    for (vector<Edge *>::iterator eit = deadEdges.begin();
         eit != deadEdges.end(); eit++) {
...
InsertedRegion *CFGModifier::insert(CodeObject *obj, Address base, void *data,
                                    unsigned size) {
  parsing_cerr << "Inserting new code: " << hex
               << (unsigned)(*((unsigned *)data)) << dec << endl;
...
InsertedRegion::InsertedRegion(Address b, void *d, unsigned s,
                               Architecture arch)
