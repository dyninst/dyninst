...
void ParseCallbackManager::removeEdge(Block *b, Edge *e,
                                      ParseCallback::edge_type_t t) {
  if (inBatch_)
...
void ParseCallbackManager::addEdge(Block *b, Edge *e,
                                   ParseCallback::edge_type_t t) {
  if (inBatch_)
...
void ParseCallbackManager::modifyEdge(Edge *e, Block *b,
                                      ParseCallback::edge_type_t t) {
  if (inBatch_)
...
void ParseCallbackManager::interproc_cf(Function *f, Block *b, Address a,
                                        ParseCallback::interproc_details *d) {
  for (iterator iter = begin(); iter != end(); ++iter)
    (*iter)->interproc_cf(f, b, a, d);
};

void ParseCallbackManager::instruction_cb(Function *f, Block *b, Address a,
...
void ParseCallbackManager::abruptEnd_cf(Address a, Block *b,
                                        ParseCallback::default_details *d) {
  for (iterator iter = begin(); iter != end(); ++iter)
    (*iter)->abruptEnd_cf(a, b, d);
};

bool ParseCallbackManager::absAddr(Address abs, Address &load,
                                   CodeObject *&obj) {
  bool ret = true;
  for (iterator iter = begin(); iter != end(); ++iter)
...
void ParseCallbackManager::destroy_cb(Block *b) {
  for (iterator iter = begin(); iter != end(); ++iter) (*iter)->destroy_cb(b);
};

void ParseCallbackManager::destroy_cb(Edge *e) {
  for (iterator iter = begin(); iter != end(); ++iter) (*iter)->destroy_cb(e);
};

void ParseCallbackManager::destroy_cb(Function *f) {
  for (iterator iter = begin(); iter != end(); ++iter) (*iter)->destroy_cb(f);
};

void ParseCallbackManager::remove_edge_cb(Block *b, Edge *e,
                                          ParseCallback::edge_type_t t) {
  for (iterator iter = begin(); iter != end(); ++iter)
    (*iter)->remove_edge_cb(b, e, t);
};

void ParseCallbackManager::add_edge_cb(Block *b, Edge *e,
                                       ParseCallback::edge_type_t t) {
  for (iterator iter = begin(); iter != end(); ++iter)
    (*iter)->add_edge_cb(b, e, t);
};

void ParseCallbackManager::modify_edge_cb(Edge *e, Block *b,
                                          ParseCallback::edge_type_t t) {
  for (iterator iter = begin(); iter != end(); ++iter)
