...
bool IA_x86Details::isMovAPSTable(
  ...
  * AMD-64 gcc emits a re-accuring idiom of:
  ...
  unsigned int size =
      (currentBlock->_cr->offset() + currentBlock->_cr->length()) -
      currentBlock->current;
...
bool IA_x86Details::isTableInsn(Instruction::Ptr i) {
  ...
  if (i->getOperation().getID() == e_mov && i->readsMemory() &&
      i->isWritten(jumpExpr)) {
    return true;
  }
...
namespace {
IA_IAPI::allInsns_t::const_iterator search_insn_vec(
    Address a, IA_IAPI::allInsns_t const& v) {
...
bool IA_x86Details::parseJumpTable(
  ...
  boost::tie(maxSwitchInsn, branchInsn, foundJCCAlongTaken) =
      findMaxSwitchInsn(currBlk);
  ...
  if (thunkInsn.addrOfInsn != 0) {
    ...
    if (thunkLoc != currentBlock->allInsns.end()) {
      if (thunkLoc->second &&
          thunkLoc->second->getOperation().getID() == e_lea) {
        tableLoc = thunkLoc;
  ...
  bool ok = computeTableBounds(maxSwitchInsn, branchInsn, tableInsn.insn,
                               foundJCCAlongTaken, tableSize, tableStride);
  if (!ok) {
    return false;
  }
    ...
    if (tableLoc->second->getOperation().getID() == e_lea) {
      ...
      if (tableLoc->second->getOperation().getID() == e_movsxd ||
        ...
        if (movsxReadAddr.empty()) {
          ...
          Expression::Ptr op1 = tableLoc->second->getOperand(0).getValue();
          Expression::Ptr op2 = tableLoc->second->getOperand(1).getValue();

          if (op1 && op2) {
            int sz1 = op1->eval().size();
            int sz2 = op2->eval().size();
            parsing_printf(
                "\t\tfound %d-byte to %d-byte move, revised stride to %d\n",
        ...
        static BinaryFunction::funcT::Ptr multiplier(
            new BinaryFunction::multResult());
        static BinaryFunction::Ptr scaleCheck4(new BinaryFunction(
            four, dummy4, (tableStride == 8) ? u64 : u32, multiplier));
        static BinaryFunction::Ptr scaleCheck2(new BinaryFunction(
            two, dummy2, (tableStride == 8) ? u64 : u32, multiplier));
        for (std::set<Expression::Ptr>::const_iterator readExp =
                 movsxReadAddr.begin();
             readExp != movsxReadAddr.end(); ++readExp) {
          if ((*readExp)->isUsed(scaleCheck4)) {
            parsing_printf(
                "\tFound sign-extending mov, revising table stride to scale "
                "(4)\n");
            tableStride = 4;
            ...
            parsing_printf("\tFound sign-extending mov insn %s, readExp %s\n",
                           ...
                           (*readExp)->format().c_str());
            parsing_printf(
                "\tcouldn't match stride expression %s or %s--HELP!\n",
  ...
  // This first compute() should be unnecessary, as we'll already have done the
  // work above.
  // However, if there turn out to be bugs later (PIC tables where we're
  // revising the table insn
  ...
  while (findSubtract->first < currentBlock->current) {
    if (findSubtract->second &&
        findSubtract->second->getOperation().getID() == e_sub) {
  ...
  if (!block.isRealCall()) {
    ...
    if (addInsn) {
      ...
      if (addInsn && ((addInsn->getOperation().getID() == e_add) ||
        ...
        for (std::set<RegisterAST::Ptr>::const_iterator curReg =
                 boundRegs.begin();
             curReg != boundRegs.end(); ++curReg) {
        ...
        if (imm.defined) {
          ...
          parsing_printf(
              "\tsetting thunkInsn.addrFromInsn to 0x%lx (0x%lx + 0x%lx)\n",
              thunkInsn.addrFromInsn + thunkDiff, thunkInsn.addrFromInsn,
              thunkDiff);
          ...
          parsing_printf(
              "\tadd insn %s found following thunk at 0x%lx, couldn't bind "
              "operands!\n",
...
bool IA_x86Details::handleAdd(IA_IAPI& block) {
  ...
  // could find in the table insn, we have a "table address"; otherwise, check
  // adds
...
void IA_x86Details::findThunkInBlock(Block* curBlock) {
  ...
  InstructionDecoder dec(
      buf, curBlock->size() + InstructionDecoder::maxInstructionLength,
  ...
  while (block.getAddr() < curBlock->end()) {
    ...
    // Look for an AMD64 IP-relative LEA.  If we find one, it should point to
    // the start of a
    {  // relative jump table.
      parsing_printf("\tchecking instruction %s at 0x%lx for IP-relative LEA\n",
                     block.getInstruction()->format().c_str(), block.getAddr());
      Expression::Ptr IPRelAddr =
          block.getInstruction()->getOperand(1).getValue();
      IPRelAddr->bind(currentBlock->thePC[currentBlock->_isrc->getArch()].get(),
                      Result(s64, block.getNextAddr()));
      Result iprel = IPRelAddr->eval();
      if (iprel.defined) {
...
void processPredecessor(ParseAPI::Edge* e, std::set<Block*>& visited,
...
void IA_x86Details::findThunkAndOffset(Block* start) {
  ...
  while (!worklist.empty()) {
    ...
    // indirect jump in control flow (we think) is after it in the address
    // space,
    // it may be a jump table but it's not one our heuristics should expect to
    // handle well.  Better to bail out than try to parse something that will be
    // garbage.
    ...
    std::for_each(
        boost::make_filter_iterator(epred, curBlock->sources().begin(),
                                    curBlock->sources().end()),
        boost::make_filter_iterator(epred, curBlock->sources().end(),
                                    curBlock->sources().end()),
        boost::bind(processPredecessor, _1, boost::ref(visited),
                    boost::ref(worklist)));
  }
  return;
}

boost::tuple<Instruction::Ptr, Instruction::Ptr, bool>
IA_x86Details::findMaxSwitchInsn(Block* start) {
  std::set<Block*> visited;
  std::vector<Block*> WL;
  Block* curBlk;
  int depth = 0;
  ...
  for (unsigned j = 0; j < WL.size(); j++) {
    ...
    while ((i = dec.decode())) {
      ...
      if (i->getCategory() == c_BranchInsn && i->allowsFallThrough()) {
        ...
        for (; tit != curBlk->targets().end(); ++tit) {
          ...
          if ((t->type() == COND_NOT_TAKEN || t->type() == FALLTHROUGH) &&
              (visited.find(t->trg()) != visited.end())) {
            fallthrough_hit = true;
          }
        }
        parsing_printf(
            "\tfindMaxSwitchInsn: taken_hit: %d, fallthrough_hit: %d\n",
    ...
    if (foundMaxSwitch && foundCondBranch) break;  // done
  ...
  parsing_printf(
      ...
      compareOnTakenBranch, foundMaxSwitch && foundCondBranch);
...
void IA_x86Details::computeTableAddress() {
  ...
  if (tableInsn.insn->getCategory() == c_BranchInsn) {
    ...
    if (tableInsn.insn->getOperation().getID() != e_lea) {
      ...
      if (tmp.empty()) {
        parsing_printf(
            "\ttable insn BAD! (not LEA, second operand not a deref)\n");
        return;
      }
      displacementSrc = tmp[0];
    } else {
  ...
  if (!z.isDefined()) {
    parsing_printf(
        "\ttable insn: %s, displacement %s, bind of all GPRs FAILED\n",
...
// of their module subtracted off before we ever use them.  We need to fix this
// up
// to conform to that standard so that Symtab actually believes that there's
// code
...
bool IA_x86Details::fillTableEntries(
    Address thunkOffset, Address tableBase, unsigned tableSize,
    unsigned tableStride, int offsetMultiplier,
    std::vector<std::pair<Address, EdgeTypeEnum> >& outEdges) {
  ...
  for (unsigned int i = 0; i < tableSize; i++) {
    ...
    if (currentBlock->_isrc->isValidAddress(tableEntry)) {
      if (tableStride == sizeof(Address)) {
        ...
        jumpAddress = *(const Address*)currentBlock->_isrc->getPtrToInstruction(
            tableEntry);
        if (0 == jumpAddress) {
          parsing_printf(
        ...
        jumpAddress =
            *(const int*)currentBlock->_isrc->getPtrToInstruction(tableEntry);
        if (0 == jumpAddress) {
          parsing_printf(
              "%s[%d]: failed to get pointer to "
...
bool IA_x86Details::computeTableBounds(Instruction::Ptr maxSwitchInsn,
  ...
  // Sanity check the bounds; 32k tables would be an oddity, and larger is
  // almost certainly
  // a misparse
  ...
  if (tableStride == 8) {
    ...
    static BinaryFunction::Ptr scaleCheck(
        new BinaryFunction(four, dummy, u64, multiplier));
    for (std::set<Expression::Ptr>::const_iterator curExpr =
             tableInsnReadAddr.begin();
         curExpr != tableInsnReadAddr.end(); ++curExpr) {
