...
typedef enum { PreambleMatching, IdiomMatching } GapParsingType;
...
class CodeObject {
 ...
 public:
  ...
  PARSER_EXPORT CodeObject(CodeSource *cs, CFGFactory *fact = NULL,
  ...
  struct NewEdgeToParse {
    ...
    bool checked;  // true if call_ft edges have already had their callees
                   // checked
    NewEdgeToParse(Block *a, Address b, EdgeTypeEnum c)
        : source(a), target(b), edge_type(c), checked(false){};
    NewEdgeToParse(Block *a, Address b, bool c, EdgeTypeEnum d)
        : source(a), target(b), edge_type(d), checked(c) {}
  };
  ...
  PARSER_EXPORT int findFuncs(CodeRegion *cr, Address addr,
                              std::set<Function *> &funcs);
  // Find functions overlapping the range [start,end)
  PARSER_EXPORT int findFuncs(CodeRegion *cr, Address start, Address end,
                              std::set<Function *> &funcs);
  PARSER_EXPORT const funclist &funcs() { return flist; }

  // blocks
  PARSER_EXPORT Block *findBlockByEntry(CodeRegion *cr, Address entry);
  PARSER_EXPORT int findBlocks(CodeRegion *cr, Address addr,
                               std::set<Block *> &blocks);
  // finds blocks without parsing.
  PARSER_EXPORT int findCurrentBlocks(CodeRegion *cr, Address addr,
                                      std::set<Block *> &blocks);
