...
class flist_iter {
 ...
 public:
  flist_iter(elem *cur) : cur_(cur) {}

  flist_iter(allocatable *cur) : cur_(cur) {}
...
class fact_list {
 public:
  ...
  void add_tail(elem &new_elem) { head.prepend(new_elem); }
  void clear() {
    while (head.alloc_next() != &head) head.alloc_next()->remove();
  }
...
class PARSER_EXPORT CFGFactory {
 public:
   ...
   */
  Function *_mkfunc(Address addr, FuncSource src, std::string name,
 ...
 protected:
  virtual Function *mkfunc(Address addr, FuncSource src, std::string name,
                           CodeObject *obj, CodeRegion *region,
                           Dyninst::InstructionSource *isrc);
  virtual Block *mkblock(Function *f, CodeRegion *r, Address addr);
