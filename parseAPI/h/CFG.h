...
class PARSER_EXPORT allocatable {
 public:
  ...
  void remove() {
    if (anext_ != (allocatable *)FLIST_BADNEXT) anext_->aprev_ = aprev_;
    if (aprev_ != (allocatable *)FLIST_BADPREV) aprev_->anext_ = anext_;
  ...
  void append(allocatable &new_elem) {
    if (anext_ == (allocatable *)FLIST_BADNEXT ||
        aprev_ == (allocatable *)FLIST_BADPREV) {
      fprintf(stderr, "allocatable::append called on element not on list: %p\n",
              this);
      return;
    }
...
class PARSER_EXPORT SingleContext : public EdgePredicate {
 ...
 public:
  SingleContext(Function *f, bool forward, bool backward)
      : _context(f), _forward(forward), _backward(backward) {}
  ~SingleContext() {}
  bool pred_impl(Edge *) const;
};
    ...
    bool operator()(Block *const &b1, Block *const &b2) const {
      ...
      if (b1 != b2)
        fprintf(stderr,
                ...
                "conflict",
                b1, b1->start(), b1->end(), b2, b2->start(), b2->end());
...
inline void Block::addSource(Edge *e) { _srclist.push_back(e); }

inline void Block::addTarget(Edge *e) { _trglist.push_back(e); }
...
class PARSER_EXPORT Function : public allocatable, public AnnotatableSparse {
 ...
 public:
  ...
  Address _ret_addr;  // return address of a function stored in stack at
                      // function entry
 ...
 private:
  ...
  /* Loop details*/
  mutable bool _loop_analyzed;  // true if loops in the function have been found
                                // and stored in _loops
  mutable std::set<Loop *> _loops;
  mutable LoopTreeNode
      *_loop_root;  // NULL if the tree structure has not be calculated
  void getLoopsByNestingLevel(std::vector<Loop *> &lbb,
...
class PARSER_EXPORT FuncExtent
    : public Dyninst::SimpleInterval<Address, Function *> {
 private:
  Function *_func;
  Address _start;
  Address _end;
