...
class CFGModifier {
 public:
  ...
  PARSER_EXPORT static Block *split(Block *, Address, bool trust = false,
                                    Address newlast = -1);
  ...
  PARSER_EXPORT static InsertedRegion *insert(CodeObject *obj, Address base,
...
class InsertedRegion : public CodeRegion {
 public:
  PARSER_EXPORT InsertedRegion(Address base, void *data, unsigned size,
                               Architecture arch);
  ...
  PARSER_EXPORT void *getPtrToData(const Address) const { return NULL; };
  PARSER_EXPORT bool isCode(const Address a) const {
  ...
  PARSER_EXPORT unsigned int getAddressWidth() const {
    if (arch_ == Arch_ppc64 || arch_ == Arch_x86_64 || arch_ == Arch_aarch64)
      return 8;
    else
      return 4;
  };
