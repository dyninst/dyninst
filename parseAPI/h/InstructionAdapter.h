...
enum InstrumentableLevel { NORMAL, HAS_BR_INDIR, UNINSTRUMENTABLE };
...
class InstructionAdapter {
 public:
  ...
  void reset(Address start, ParseAPI::CodeObject *o, ParseAPI::CodeRegion *r,
  ...
  virtual bool isGarbageInsn()
      const = 0;  // true for insns indicative of bad parse, for defensive mode
  virtual void getNewEdges(
      std::vector<std::pair<Address, ParseAPI::EdgeTypeEnum> > &outEdges,
      ParseAPI::Function *context, ParseAPI::Block *currBlk,
  ...
  virtual InstrumentableLevel getInstLevel(ParseAPI::Function *context,
                                           unsigned int num_insns) const;
  virtual ParseAPI::FuncReturnStatus getReturnStatus(
      ParseAPI::Function *context, unsigned int num_insns) const;
  virtual bool hasUnresolvedControlFlow(ParseAPI::Function *context,
                                        unsigned int num_insns) const;
  ...
  virtual bool isReturnAddrSave(Address &ret_addr) const = 0;  // ret_addr holds
                                                               // the return
                                                               // address pushed
                                                               // in the stack
                                                               // using mflr at
                                                               // function entry
  virtual bool isTailCall(ParseAPI::Function *, ParseAPI::EdgeTypeEnum type,
                          unsigned int num_insns,
                          const std::set<Address> &) const = 0;

 protected:
  // Uses pattern heuristics or backward slicing to determine if a blr
  // instruction is a return or jump table
  virtual bool isReturn(Dyninst::ParseAPI::Function *context,
                        Dyninst::ParseAPI::Block *currBlk) const = 0;
  ...
  // Block associated with the instruction adapter. This is required for powerpc
  // slicing to determine the return address of a function
