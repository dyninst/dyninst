...
{
public:
    InstructionAdapter(Address start, ParseAPI::CodeObject* o, ParseAPI::CodeRegion* r,
    ...
    virtual ParseAPI::FuncReturnStatus getReturnStatus(ParseAPI::Function* context,
                                                       unsigned int num_insns) const;
    ...
    virtual bool                     isReturnAddrSave(
                            Address& ret_addr) const = 0;  // ret_addr holds the return address pushed in the
                                       // stack using mflr at function entry
    virtual bool isTailCall(const ParseAPI::Function*, ParseAPI::EdgeTypeEnum type,
                            unsigned int num_insns, const std::set<Address>&) const = 0;

protected:
    // Uses pattern heuristics or backward slicing to determine if a blr instruction is a
