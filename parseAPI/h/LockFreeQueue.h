...
public:
    LockFreeQueueItem(T __value)
    : _next(0)
    , _value(__value)
    {
        LFQ_DEBUG(validate = this);
    }
    ~LockFreeQueueItem() { LFQ_DEBUG(validate = 0); }
...
public:
    LockFreeQueueIterator(item_type* _item)
    : item(_item)
    {}
...
public:
    LockFreeQueue(item_type* _head = 0)
    : head(_head)
    {}
...
public:
    ...
    {
        if(q.peek())
        {  // only empty q if it is non-empty
