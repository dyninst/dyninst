...
void hc_write_nl(int id) {
    ...
    if (hc_first_active_lane()) {
        hc_acquire();
        mailbox.arg = __builtin_amdgcn_readfirstlane(v);
        hc_call_and_wait(HC_OP_WRITE_ID);           // reuse WRITE_ID op: log the computed value
        hc_release();
    }
}
...
void hc_write_bid(int /*site (unused): we log blockIdx.x instead)*/) {
    ...
    if (hc_first_active_lane()) {
        hc_acquire();
        mailbox.arg = __builtin_amdgcn_readfirstlane(bx);
        hc_call_and_wait(HC_OP_WRITE_ID);           // reuse WRITE_ID op: log blockIdx.x
        hc_release();
    }
}
...
void hc_write_bdim(int /*site (unused): we log blockDim.x instead)*/) {
    ...
    if (hc_first_active_lane()) {
        hc_acquire();
        mailbox.arg = __builtin_amdgcn_readfirstlane(bd);
        hc_call_and_wait(HC_OP_WRITE_ID);           // reuse WRITE_ID op: log blockDim.x
        hc_release();
    }
}
...
void hc_write_tid(int /*site (unused): we log threadIdx.x instead)*/) {
    ...
    if (hc_first_active_lane()) {
        hc_acquire();
        mailbox.arg = __builtin_amdgcn_readfirstlane(tx);
        hc_call_and_wait(HC_OP_WRITE_ID);           // reuse WRITE_ID op: log threadIdx.x
        hc_release();
    }
}
...
void hc_write_ftid(int /*site (unused)*/) {
    ...
    if (hc_first_active_lane()) {
        hc_acquire();
        mailbox.arg = __builtin_amdgcn_readfirstlane((int)wid);
        hc_call_and_wait(HC_OP_WRITE_ID);           // reuse WRITE_ID op: log flattened wave id
        hc_release();
    }
}
