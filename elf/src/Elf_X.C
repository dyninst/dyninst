...
unsigned short Elf_X::e_endian() const { return isBigEndian; }
...
// Read Interface
Elf *Elf_X::e_elfp() const { return elf; }
...
unsigned short Elf_X::e_phnum() const {
  return (!is64 ? static_cast<unsigned short>(ehdr32->e_phnum)
...
unsigned short Elf_X::e_shnum() const {
  return (!is64 ? static_cast<unsigned short>(ehdr32->e_shnum)
                : static_cast<unsigned short>(ehdr64->e_shnum));
}

unsigned short Elf_X::e_shstrndx() const {
  return (!is64 ? static_cast<unsigned short>(ehdr32->e_shstrndx)
...
// Data Interface
bool Elf_X::isValid() const { return (ehdr32 || ehdr64); }

int Elf_X::wordSize() const { return (!is64 ? 4 : 8); }
...
Elf_X_Phdr::Elf_X_Phdr() : phdr32(NULL), phdr64(NULL), is64(false) {}
...
bool Elf_X_Phdr::isValid() const { return (phdr32 || phdr64); }
...
bool Elf_X_Shdr::isFromDebugFile() const { return fromDebugFile; }
...
void Elf_X_Shdr::setDebugFile(bool b) { fromDebugFile = b; }

// Section Data Interface
Elf_X_Data Elf_X_Shdr::get_data() const { return Elf_X_Data(is64, data); }

// For Sections with Multiple Data Sections
void Elf_X_Shdr::first_data() { data = elf_getdata(scn, NULL); }
...
bool Elf_X_Shdr::isValid() const { return (shdr32 || shdr64); }

unsigned Elf_X_Shdr::wordSize() const { return is64 ? 8 : 4; }

Elf_Scn *Elf_X_Shdr::getScn() const { return scn; }

Elf_X_Nhdr Elf_X_Shdr::get_note() const {
  if (sh_type() != SHT_NOTE) return Elf_X_Nhdr();
  return Elf_X_Nhdr(data, 0);
}

// ------------------------------------------------------------------------
// Class Elf_X_Data simulates the Elf_Data structure.
Elf_X_Data::Elf_X_Data() : data(NULL), is64(false) {}

Elf_X_Data::Elf_X_Data(bool is64_, Elf_Data *input)
    : data(input), is64(is64_) {}

// Read Interface
void *Elf_X_Data::d_buf() const { return data->d_buf; }

Elf_Type Elf_X_Data::d_type() const { return data->d_type; }

unsigned int Elf_X_Data::d_version() const { return data->d_version; }

size_t Elf_X_Data::d_size() const { return data->d_size; }

off_t Elf_X_Data::d_off() const { return (off_t)data->d_off; }

size_t Elf_X_Data::d_align() const { return data->d_align; }

// Write Interface
void Elf_X_Data::d_buf(void *input) { data->d_buf = input; }

void Elf_X_Data::d_type(Elf_Type input) { data->d_type = input; }

void Elf_X_Data::d_version(unsigned int input) { data->d_version = input; }

void Elf_X_Data::d_size(unsigned int input) { data->d_size = input; }

void Elf_X_Data::d_off(signed int input) { data->d_off = input; }

void Elf_X_Data::d_align(unsigned int input) { data->d_align = input; }
...
// Data Interface
const char *Elf_X_Data::get_string() const { return (const char *)data->d_buf; }

Elf_X_Dyn Elf_X_Data::get_dyn() { return Elf_X_Dyn(is64, data); }

Elf_X_Versym Elf_X_Data::get_versyms() { return Elf_X_Versym(is64, data); }
...
Elf_X_Rel Elf_X_Data::get_rel() { return Elf_X_Rel(is64, data); }

Elf_X_Rela Elf_X_Data::get_rela() { return Elf_X_Rela(is64, data); }

Elf_X_Sym Elf_X_Data::get_sym() { return Elf_X_Sym(is64, data); }

bool Elf_X_Data::isValid() const { return data; }
...
bool Elf_X_Versym::isValid() const { return (versym32 || versym64); }
...
Elf_X_Verdaux *Elf_X_Verdaux::get_next() const {
  if (vda_next() == 0) return NULL;
  return new Elf_X_Verdaux(is64, (char *)data + vda_next());
}

// Meta-Info Interface
bool Elf_X_Verdaux::isValid() const { return (verdaux32 || verdaux64); }
...
Elf_X_Vernaux *Elf_X_Vernaux::get_next() const {
  if (vna_next() == 0) return NULL;
  return new Elf_X_Vernaux(is64, (char *)data + vna_next());
}

// Meta-Info Interface
bool Elf_X_Vernaux::isValid() const { return (vernaux32 || vernaux64); }
...
bool Elf_X_Sym::isValid() const { return sym32 || sym64; }

// ------------------------------------------------------------------------
// Class Elf_X_Rel simulates the Elf(32|64)_Rel structure.
Elf_X_Rel::Elf_X_Rel() : data(NULL), rel32(NULL), rel64(NULL), is64(false) {}
...
bool Elf_X_Rel::isValid() const { return (rel32 || rel64); }
...
bool Elf_X_Rela::isValid() const { return (rela32 || rela64); }

// ------------------------------------------------------------------------
// Class Elf_X_Dyn simulates the Elf(32|64)_Dyn structure.
Elf_X_Dyn::Elf_X_Dyn() : data(NULL), dyn32(NULL), dyn64(NULL), is64(false) {}
...
bool Elf_X_Dyn::isValid() const { return (dyn32 || dyn64); }

static bool loadDebugFileFromDisk(string name, char *&output_buffer,
  ...
  char *buffer =
      (char *)mmap(NULL, fileStat.st_size, PROT_READ, MAP_SHARED, fd, 0);
  close(fd);
...
bool Elf_X::findDebugFile(std::string origfilename, string &output_name,
                          char *&output_buffer,
                          unsigned long &output_buffer_size) {
  if (cached_debug) {
  ...
  if (!shnames_hdr.isValid()) return false;
  ...
  for (int i = 0; i < e_shnum(); i++) {
    ...
    if (strcmp(name, DEBUGLINK_NAME) == 0) {
      ...
      for (Elf_X_Nhdr note = scn.get_note(); note.isValid();
           note = note.next()) {
...
Elf_X_Nhdr::Elf_X_Nhdr() : data(NULL), nhdr(NULL) {}
...
Elf_X_Nhdr::Elf_X_Nhdr(Elf_Data *data_, size_t offset)
  ...
  if (data && offset < data->d_size) {
    ...
    if (sizeof(*nhdr) <= size) {
      ...
      if (n_namesz() > size || n_descsz() > size - n_namesz()) nhdr = NULL;
    }
  }
  if (!nhdr) data = NULL;
}
...
bool Elf_X_Nhdr::isValid() const { return (data && nhdr); }
