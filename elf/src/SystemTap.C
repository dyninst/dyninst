...
struct OperandParser {
  typedef qi::rule<string::const_iterator, ArgTree::ptr(), ascii::space_type>
      ArgTreeRule;
  ...
  ArgTree::ptr newBaseIndexScale(ArgTree::ptr base, ArgTree::ptr index,
                                 int scale);
  ArgTree::ptr newAdd(ArgTree::ptr a, ArgTree::ptr b);
  ArgTree::ptr newRegister(const std::vector<char> &v);
};

struct x86OperandParser : public qi::grammar<std::string::const_iterator,
                                             ArgTree::ptr(), ascii::space_type>,
...
struct ppcOperandParser : public qi::grammar<std::string::const_iterator,
                                             ArgTree::ptr(), ascii::space_type>,
                          public OperandParser {
...
struct aarch64OperandParser
    : public qi::grammar<std::string::const_iterator, ArgTree::ptr(),
                         ascii::space_type>,
      public OperandParser {
  ArgTreeRule operand, shex, num, reg;
...
SystemTapEntries *SystemTapEntries::createSystemTapEntries(Elf_X *file_) {
  ...
  if (i != all_entries.end()) return i->second;
...
SystemTapEntries::SystemTapEntries(Elf_X *file_) : file(file_) {}
...
bool SystemTapEntries::readAddr(const unsigned char *buffer, size_t bsize,
                                unsigned &offset, Dyninst::Address &result,
                                unsigned int read_size) {
  if (!read_size) {
    read_size = word_size;
  }
  if (offset + read_size > bsize) return false;
...
bool SystemTapEntries::readString(const unsigned char *buffer, size_t bsize,
                                  unsigned &offset, std::string &result) {
  unsigned int start = offset;
  unsigned int end = start;

  if (start >= bsize) return false;
...
bool SystemTapEntries::parseOperand_x86(std::string op, Arg &arg) {
  ...
  if (!parser) parser = new x86OperandParser(arch);
...
bool SystemTapEntries::parseOperand_ppc(std::string op, Arg &arg) {
  ppcOperandParser *&parser =
      (arch == Arch_ppc32) ? ppc32_parser : ppc64_parser;
  if (!parser) parser = new ppcOperandParser(arch);
...
ArgTree::ptr OperandParser::getReg(std::string name) {
  std::map<std::string, Dyninst::MachRegister>::iterator i =
      register_names.find(name);
...
ArgTree::ptr OperandParser::identity(ArgTree::ptr p) { return p; }
...
ArgTree::ptr OperandParser::newBaseIndexScale(ArgTree::ptr base,
                                              ArgTree::ptr index, int scale) {
  return ArgTree::createAdd(
      base, ArgTree::createMultiply(index, ArgTree::createConstant(scale)));
}
...
x86OperandParser::x86OperandParser(Dyninst::Architecture arch)
  ...
  shex =
      lit("0x") >> hex[qi::_val = phoenix::bind(&OperandParser::newConstant,
                                                this, qi::_1)] |
      lit("-0x") >> hex[qi::_val = phoenix::bind(&OperandParser::newConstant,
                                                 this, -1 * qi::_1)] |
      int_[qi::_val = phoenix::bind(&OperandParser::newConstant, this, qi::_1)];

  reg = '%' >> (+alnum)[qi::_val = phoenix::bind(&OperandParser::newRegister,
                                                 this, qi::_1)];

  mem_modrm_nobase =
      ('(' >> reg >>
       ')')[qi::_val = phoenix::bind(&OperandParser::identity, this, qi::_1)] |
      ('(' >> reg >> ',' >> reg >> ',' >> uint_ >>
       ')')[qi::_val = phoenix::bind(&OperandParser::newBaseIndexScale, this,
                                     qi::_1, qi::_2, qi::_3)];

  mem_modrm =
      (shex >>
       mem_modrm_nobase)[qi::_val = phoenix::bind(&OperandParser::newDerefAdd,
                                                  this, qi::_1, qi::_2)] |
      mem_modrm_nobase[qi::_val = phoenix::bind(&OperandParser::newDeref, this,
                                                qi::_1)] |
  ...
  modrm =
      ...
      mem_modrm[qi::_val =
                    phoenix::bind(&OperandParser::identity, this, qi::_1)] |
      '$' >> shex[qi::_val =
                      phoenix::bind(&OperandParser::identity, this, qi::_1)];

  operand =
      modrm[qi::_val = phoenix::bind(&OperandParser::identity, this, qi::_1)] |
      (reg >> ':' >> modrm)[qi::_val = phoenix::bind(&OperandParser::newSegment,
                                                     this, qi::_1, qi::_2)];
}

void x86OperandParser::createRegisterNames(Dyninst::Architecture arch) {
  Dyninst::MachRegister::NameMap::iterator i =
      Dyninst::MachRegister::names()->begin();
  for (; i != Dyninst::MachRegister::names()->end(); i++) {
    ...
    unsigned int gpr_code =
        (arch == Dyninst::Arch_x86 ? Dyninst::x86::GPR : Dyninst::x86_64::GPR);
    unsigned int seg_code =
        (arch == Dyninst::Arch_x86 ? Dyninst::x86::SEG : Dyninst::x86_64::SEG);
    if (reg.regClass() != gpr_code && reg.regClass() != seg_code &&
        !reg.isPC()) {
      continue;
    }
...
ppcOperandParser::ppcOperandParser(Dyninst::Architecture arch)
  ...
  shex = lit("0x") >> hex[qi::_val = phoenix::bind(&OperandParser::newConstant,
                                                   this, qi::_1)] |
         lit("-0x") >> hex[qi::_val = phoenix::bind(&OperandParser::newConstant,
                                                    this, -1 * qi::_1)];
  ...
  reg = 'r' >> (+alnum)[qi::_val = phoenix::bind(&OperandParser::newRegister,
                                                 this, qi::_1)];
  ;
  ...
  operand =
      reg[qi::_val = phoenix::bind(&OperandParser::identity, this, qi::_1)] |
      (num >> lit("(") >> reg >>
       lit(")"))[qi::_val = phoenix::bind(&OperandParser::newDerefAdd, this,
                                          qi::_1, qi::_2)] |
...
void ppcOperandParser::createRegisterNames(Dyninst::Architecture arch) {
  ...
  for (; i != Dyninst::MachRegister::names()->end(); i++) {
    ...
    unsigned int gpr_code = (arch == Dyninst::Arch_ppc32 ? Dyninst::ppc32::GPR
                                                         : Dyninst::ppc64::GPR);
