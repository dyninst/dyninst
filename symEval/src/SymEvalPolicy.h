...
class SymEvalPolicy {
 public:
  ...
  ~SymEvalPolicy() {};
    ...
    Handle<32> readIP() { 
      return Handle<32>(wrap(Absloc::makePC()));
    }
    
    void writeIP(Handle<32> value) {
      std::map<Absloc, Assignment::Ptr>::iterator i = aaMap.find(Absloc::makePC());
      if (i != aaMap.end()) {
	res[i->second] = value.var();
      }
      // Otherwise we don't care. Annoying that we 
    ...
    void writeMemory(X86SegmentRegister,
      ...
      std::map<Absloc, Assignment::Ptr>::iterator i = aaMap.find(Absloc(Absloc::Heap, 0));
      if (i != aaMap.end()) {
