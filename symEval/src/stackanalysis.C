...
{
  symeval_init_debug();
  sym_stackanalysis_printf("Beginning stack analysis for function %s\n",
		       func->name().c_str());

    sym_stackanalysis_printf("\tSummarizing block effects\n");
    summarizeBlocks();
    
    sym_stackanalysis_printf("\tPerforming fixpoint analysis\n");
    sp_fixpoint();

    sym_stackanalysis_printf("\tCreating SP interval tree\n");
    sp_createIntervals();

    sym_stackanalysis_printf("\tCreating FP interval tree\n");
    fp_fixpoint();
    ...
    sym_stackanalysis_printf("Finished stack analysis for function %s\n",
			 func->name().c_str());
...
void StackAnalysis::summarizeBlocks() {
  ...
  for( ; bit != bs.end(); ++bit) {
    ...
    sym_stackanalysis_printf("\t Block starting at 0x%lx: %s\n", 
			 block->start(),
			 bFunc.format().c_str());
	...
	fp_changePoints[block].push_back(std::make_pair<fp_State, Offset>(fpCopied, off + insn->size()));
      ...
      sym_stackanalysis_printf("\t\t\t At 0x%lx:  %s\n",
			   off,
			   bFunc.format().c_str());
    }
    sym_stackanalysis_printf("\t Block summary for 0x%lx: %s\n", block->start(), bFunc.format().c_str());
  }
}
...
void StackAnalysis::sp_fixpoint() {
  ...
  while (!worklist.empty()) {
    ...
    sym_stackanalysis_printf("\t Fixpoint analysis: visiting block at 0x%lx\n", block->start());
	...
	sym_stackanalysis_printf("\t\t Inserting 0x%lx: %s\n", 
			     edge->src()->start(),
    ...
    sym_stackanalysis_printf("\t New in meet:  %s\n",
    ...
    if (newInEffect == sp_inBlockEffects[block]) {
      // No new work here
      sym_stackanalysis_printf("\t ... equal to current, skipping block\n");
      continue;
    }
    
    sym_stackanalysis_printf("\t ... inequal to current %s, analyzing block\n",
    ...
    sym_stackanalysis_printf("\t ... output from block: %s\n",
...
void StackAnalysis::sp_createIntervals() {
  ...
  for( ; bit != bs.end(); ++bit) {
    ...
    sym_stackanalysis_printf("\t Interval creation (H): visiting block at 0x%lx\n", block->start());
    
    Offset curLB = block->start();
    Offset curUB = 0;
    BlockTransferFunc curHeight = sp_inBlockEffects[block];
    
    sym_stackanalysis_printf("\t\t Block starting state: 0x%lx, %s\n", 
    ...
    for (InsnFuncs::iterator iter = sp_blockToInsnFuncs[block].begin();
	 iter != sp_blockToInsnFuncs[block].end(); iter++) {
      ...
      sym_stackanalysis_printf("\t\t Block continuing state: 0x%lx, %s\n", 
			   curLB, curHeight.format().c_str());
    }
  ...
  for( ; bit != bs.end(); ++bit) {
    ...
    for (FPChangePoints::iterator iter2 = fp_changePoints[block].begin();
	 iter2 != fp_changePoints[block].end(); ++iter2) {
      ...
      sym_stackanalysis_printf("\t\t\t Inserting interval: 0x%lx, 0x%lx, %s\n",
...
void StackAnalysis::computeInsnEffects(Block *block,
				       ...
				       fp_State &fpState) 
{
    sym_stackanalysis_printf("\t\tInsn at 0x%lx\n", off);
    ...
    if (insn->isWritten(theFramePtr)) {
      sym_stackanalysis_printf("\t\t\t FP written\n");
      if (what == e_mov &&
          (insn->isRead(theStackPtr))) {
	fpState = fp_created;
	sym_stackanalysis_printf("\t\t\t Frame created\n");
      }
      else {
	...
	// Thunks have no effect on stack height
	// call-next-PCs do
	// and it's impossible to distinguish without evaluating the CFT. 

	sym_stackanalysis_printf("\t\t\t getPC call: %s\n", iFunc.format().c_str());
	return;
      }
        ...
        for( ; eit != outs.end(); ++eit) {
	        ...
	        if (cur_edge->type() == DIRECT) {
	            ...
	            iFunc.delta() = -1*word_size;
	            sym_stackanalysis_printf("\t\t\t Stack height changed by simulate-jump call\n");
	            return;
	        }
	    ...
	    Function *target_func = target_bbl->obj()->findFuncByEntry(target_bbl->region(), target_bbl->start());
            ...
            sym_stackanalysis_printf("\t\t\t Stack height changed by self-cleaning function: %s\n", iFunc.format().c_str());
            return;
        }
        sym_stackanalysis_printf("\t\t\t Stack height assumed unchanged by call\n");
        return;
    }
	...
	if (operands[i].isRead(operands[i].getValue())) {
	  ...
	  if (res.defined) {
	    ...
	    iFunc.delta() = res.convert<long>();
            sym_stackanalysis_printf("\t\t\t Stack height changed by ref off FP %s: %s\n", insn->format().c_str(), iFunc.format().c_str());
	  }
	  else {
	    iFunc.range() = Range(Range::infinite, 0, off);
            sym_stackanalysis_printf("\t\t\t Stack height changed by unevalled ref off FP %s: %s\n", insn->format().c_str(), iFunc.format().c_str());
	  }
	}
      }
    }
    ...
    case e_pop: {
        ...
        if (arg.getValue()->eval().defined) {
            ...
            sym_stackanalysis_printf("\t\t\t Stack height changed by evaluated push/pop: %s\n", iFunc.format().c_str());
            return;
        }
        iFunc.delta() = sign * arg.getValue()->size();
        sym_stackanalysis_printf("\t\t\t Stack height changed by unevalled push/pop: %s\n", iFunc.format().c_str());
        return;
    }
    case e_ret_near:
    case e_ret_far: {
	...
	if (imm.defined) {
      ...
      sym_stackanalysis_printf("\t\t\t Stack height changed by return: %s\n", iFunc.format().c_str());
    ...
    case e_add:
	  ...
	  sym_stackanalysis_printf("\t\t\t Stack height changed by evalled add/sub: %s\n", iFunc.format().c_str());
	  return;
        }
        iFunc.range() = Range(Range::infinite, 0, off);
        sym_stackanalysis_printf("\t\t\t Stack height changed by unevalled add/sub: %s\n", iFunc.format().c_str());
        return;
    }
    ...
    case e_leave:
        iFunc.abs() = true;
        iFunc.delta() = -1*word_size;
        sym_stackanalysis_printf("\t\t\t Stack height reset by leave: %s\n", iFunc.format().c_str());
        return;
    case power_op_si:
        sign = -1;
    case power_op_addi:
	...
	sym_stackanalysis_printf("\t\t\t Stack height changed by mov ebp, esp: %s\n", iFunc.format().c_str());
	return;
      }
    ...
    default:
        iFunc.range() = Range(Range::infinite, 0, off);
        sym_stackanalysis_printf("\t\t\t Stack height changed by unhandled insn \"%s\": %s\n", 
			     insn->format().c_str(), iFunc.format().c_str());
        return;
    }
    assert(0);
...
StackAnalysis::Region::Ptr StackAnalysis::RangeTree::find(Ranges &str) {
    ...
    for (unsigned i = 0; i < str.size(); i++) {
        ...
        if (iter == cur->children.end()) {
            sym_stackanalysis_printf("\t Creating new node for range %s\n", 
				 str[i].format().c_str());
	  ...
	  sym_stackanalysis_printf("\t Found existing node for range %s\n",
			       str[i].format().c_str());
            cur = iter->second;
        }
    }
    sym_stackanalysis_printf("\t Returning region %s\n", cur->region->format().c_str());
    return cur->region;
}
