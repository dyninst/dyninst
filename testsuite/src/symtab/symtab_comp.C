...
test_results_t ArchiveComponent::group_setup(RunGroup *group, ParameterDict &params)
{
   archive = NULL;
   if (group->mutatee && group->state != SELFSTART)
   {
      bool result = Archive::openArchive(archive, std::string(group->mutatee));
      if (!result || !archive)
         return FAILED;
      archiveFile = group->mutatee;

      archive_ptr.setPtr(archive);
      archiveFile_ptr.setPtr(const_cast<void *>(reinterpret_cast<const void *>(archiveFile)));
   }
   else
   {
      archive_ptr.setPtr(NULL);
   }
   params["Archive"] = &archive_ptr;
   params["ArchiveFile"] = &archiveFile_ptr;
   return PASSED;
}

test_results_t ArchiveComponent::group_teardown(RunGroup *group, ParameterDict &params)
{
   archive = NULL;
   archiveFile = "";
   return PASSED;
...
test_results_t ArchiveMutator::setup(ParameterDict &param)
{
   archive = (Archive *) param["Archive"]->getPtr();
   archiveFile = (char *) param["ArchiveFile"]->getPtr();
   return PASSED;
}

ArchiveMutator::ArchiveMutator() :
   archive(NULL), archiveFile("")
{
}
