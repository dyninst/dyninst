...
static void check()
{
  if (!doomEAcc && validateEA(eaExpCC, eaListCC, accessExpCC)) {
    logerror("Passed test_mem_7 (conditional effective address snippet)\n");
    passed = 1;
  } else {
    logerror("**Failed** test_mem_7 (conditional effective address snippet)\n");
    logerror("    address sequences are different\n");
  }
}

int test_mem_7_mutatee() {
  loadCnt = 0;
  storeCnt = 0;
  prefeCnt = 0;
  accessCnt = 0;
