...
static void check()
{
  if (!doomBCcc && validateBC(bcExpCC, bcListCC, accessExpCC)) {
    logerror("Passed test_mem_8 (conditional byte count snippet)\n");
    passed = 1;
  } else {
    logerror("**Failed** test_mem_8 (conditional byte count snippet)\n");
    logerror("    count sequences are different\n");
  }
}

int test_mem_8_mutatee() {
  loadCnt = 0;
  storeCnt = 0;
  prefeCnt = 0;
  accessCnt = 0;
