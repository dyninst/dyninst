...
static void check() {
  if (prefeCnt == prefeExp) {
    logerror("Passed test_mem_3 (prefetch instrumentation)\n");
    passed = 1;
  } else {
    logerror("**Failed** test_mem_3 (prefetch instrumentation)\n");
    logerror("    prefetch counter seems wrong.\n");
  }
}

/* skeleton test doesn't do anything besides say that it passed */
int test_mem_3_mutatee() {
  loadCnt = 0;
  storeCnt = 0;
  prefeCnt = 0;
  accessCnt = 0;
