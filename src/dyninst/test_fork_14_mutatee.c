...
void test_fork_14_func1();

/* Global variables accessed by the mutator.  These must have globally unique
 * names.
 */

int test_fork_14_global1 = 21;
...
int test_fork_14_mutatee() {
  ...
  if (pid == 0) {   /* child */
    ...
    test_fork_14_func1();
    dprintf("Child: my pid = %d\n", getpid());
    dprintf("Child: done with tests, exiting\n");
  } else if(pid > 0) {
    dprintf("Parent: starting tests\n");
    test_fork_14_func1();
    child = wait(NULL);
    dprintf("Parent: fork pid = %d, wait pid = %d\n", pid, child);
    if (child != pid) {
       test_fork_14_global1 = -1;
    }
    dprintf("Parent: done with tests, exiting\n");
  } else if(pid < 0) {
    dprintf("Parent: fork returned %d, error %d (%s)\n",
  ...
  return 0; /* The return code of this mutatee is not checked */
}
