...
strlist_alloc()
{
    ...
    if(result)
        memset(result, 0, sizeof(strlist));
    return result;
}

bool
strlist_insert(strlist* list, const char* data)
{
    int          result = -1;
...
bool
strlist_push_front(strlist* list, const char* data)
{
...
bool
strlist_pop_front(strlist* list)
{
...
bool
strlist_push_back(strlist* list, const char* data)
{
    strlist_elm* elem = (strlist_elm*) malloc(sizeof(strlist_elm));
    if(!elem)
        return false;
...
char*
strlist_get(strlist* list, unsigned i)
{
    strlist_elm* curr = list->head;
    if(i >= list->count)
        return NULL;
...
#include <stdio.h>
bool
strlist_cmp(strlist* a, strlist* b)
{
    if(a->count != b->count)
        return false;
