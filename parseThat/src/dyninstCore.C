...
void
stateBasedPrint(message* msgData, statusID prevStat, statusID currStat, int* tabDepth,
                logLevel priority)
{
    switch(currStat)
    {
            ...
            if(prevStat == ID_TEST)
            {
                switch(getMsgID(msgData->id_data))
                {
                    case ID_INIT_CREATE_PROCESS:
                        dlog(priority, "Mutatee launched as process %d.\n",
                             msgData->int_data);
                        break;
                    case ID_PARSE_MODULE:
                        dlog(priority, "Found %d module(s).\n", msgData->int_data);
                        break;
                    case ID_PARSE_FUNC:
                        dlog(priority, "Found %d function(s).\n", msgData->int_data);
                        break;
                    case ID_INST_GET_FUNCS:
                        dlog(priority, "Retrieved %d function(s).\n", msgData->int_data);
                        break;
                    default:
                        dlog(priority, "done.\n");
                        break;
    ...
    {
        case ID_FAIL:
            dlog(priority, "*** Failure: %s\n", msgData->str_data);
            break;
        case ID_PASS:
            dlog(priority, "done.\n");
            break;
        case ID_WARN:
            dlog(priority, "Warning: %s\n", msgData->str_data);
            break;
        case ID_INFO:
            switch(getMsgID(msgData->id_data))
            {
                case ID_EXIT_CODE:
                    dlog(priority, "Mutatee exited normally and returned %d\n",
                         msgData->int_data);
                    break;
                case ID_EXIT_SIGNAL:
                    dlog(priority, "Mutatee exited via signal %d\n", msgData->int_data);
                    break;
                case ID_DATA_STRING:
                    dlog(priority, "%s\n", msgData->str_data);
                    break;
                case ID_TRACE_POINT:
                    dlog(priority, "TRACE_POINT: %s\n", msgData->str_data);
                    break;
                case ID_CRASH_HUNT_NUM_ACTIONS:
                    dlog(priority, "Crash hunting checked %d points\n",
                         msgData->int_data);
                    break;
                default:
                    dlog(
                        ERR,
                        "Internal error.  Invalid MessageID used with ID_INFO status.\n");
            }
            break;
        case ID_TEST:
            switch(getMsgID(msgData->id_data))
            {
                case ID_PARSE_FUNC:
                    dlog(priority, "Parsing function data from %s module... ",
                         msgData->str_data);
                    break;
                case ID_PARSE_MODULE_CFG:
                    dlog(priority, "Parsing CFG data from functions in %s module... ",
                         msgData->str_data);
                    break;
                case ID_PARSE_FUNC_CFG:
                    dlog(priority, "Parsing CFG data from %s function... ",
                         msgData->str_data);
                    break;
                case ID_INST_MODULE:
                    dlog(priority, "Instrumenting functions in module %s... ",
                         msgData->str_data);
                    break;
                case ID_INST_FUNC:
                    dlog(priority, "Instrumenting function %s... ", msgData->str_data);
                    break;
                default:
                    dlog(priority, "%s... ", msgStr(getMsgID(msgData->id_data)));
                    break;
            }
            if(priority <= config.verbose)
                ++(*tabDepth);
...
bool
insertSummary(BPatch_function*, BPatch_variableExpr*);
bool
generateInstrumentation(dynHandle*, BPatch_function*, BPatch_snippet*);
BPatch_variableExpr*
allocateIntegerInMutatee(dynHandle*, BPatch_function*);
bool
instrumentFunctionEntry(dynHandle*, BPatch_function*);
bool
instrumentFunctionExit(dynHandle*, BPatch_function*);
bool
instrumentBasicBlocks(dynHandle*, BPatch_function*);
bool
instrumentMemoryReads(dynHandle*, BPatch_function*);
bool
instrumentMemoryWrites(dynHandle*, BPatch_function*);
...
bool
initTraceInMutatee(dynHandle*);
bool
generateTraceSnippet(dynHandle*, BPatch_function*);
void
readTracePipe();
...
launch_mutator()
{
    ...
    if(config.parse_level >= PARSE_MODULE)
    {
        ...
        if(!appModules)
        {
            ...
            sendMsg(config.outfd, ID_PARSE_MODULE, INFO, ID_PASS, appModules->size());
    }
    if(config.parse_level >= PARSE_FUNC)
    {
        for(i = 0; i < appModules->size(); ++i)
        {
            ...
            if(!appFunctions)
            {
                ...
                sendMsg(config.outfd, ID_PARSE_FUNC, INFO, ID_PASS, appFunctions->size());
            ...
            if(config.parse_level >= PARSE_CFG)
            {
                ...
                for(j = 0; j < appFunctions->size(); ++j)
                {
                    ...
                    if(reason)
                        sendMsg(config.outfd, ID_DATA_STRING, INFO, ID_INFO, reason);
                    if(!config.func_rules.isValid(buf))
                        continue;
                ...
                if(cfg_warn_cnt)
                    sendMsg(config.outfd, ID_PARSE_MODULE_CFG, INFO, ID_WARN,
                            sprintf_static("%d warning(s), %d passed.", cfg_warn_cnt,
                                           cfg_pass_cnt));
                else
        ...
        if(config.use_process)
        {
            ...
            if(config.pipefd < 0)
            {
                sendMsg(config.outfd, ID_TRACE_OPEN_READER, VERB1, ID_FAIL,
                        sprintf_static(
                            "Mutator could not open trace pipe (%s) for read: %s\n",
        ...
        for(i = 0; i < appModules->size(); ++i)
        {
            ...
            for(j = 0; j < appFunctions->size(); ++j)
            {
                ...
                if(func_warn_cnt)
                {
                    sendMsg(config.outfd, ID_INST_FUNC, VERB1, ID_WARN,
                            sprintf_static("%d warning(s), %d passed.", func_warn_cnt,
                                           func_pass_cnt));
                    ++mod_warn_cnt;
                }
                else
            ...
            if(mod_warn_cnt)
                sendMsg(config.outfd, ID_INST_MODULE, INFO, ID_WARN,
                        sprintf_static("%d warning(s), %d passed.", mod_warn_cnt,
                                       mod_pass_cnt));
            else
    ...
    if(!config.use_process)
    {
        ...
        if(writeBE != NULL)
        {
            ...
            for(symLib_iter = config.symbol_libraries.begin();
                symLib_iter != config.symbol_libraries.end(); ++symLib_iter)
            {
...
void
printSummary(BPatch_thread* proc, BPatch_exitType exit_type)
{
    unsigned int i, j;
    char         buf[STRING_MAX];
    char*        ptr;
    ...
    switch(exit_type)
    {
        case ExitedNormally:
            ...
            sendMsg(config.outfd, ID_EXIT_CODE, INFO, ID_INFO,
                    proc->getProcess()->getExitCode());
            break;

        case ExitedViaSignal:
            sendMsg(config.outfd, ID_EXIT_SIGNAL, INFO, ID_INFO,
                    proc->getProcess()->getExitSignal());
            if(config.hunt_crashes)
            {
                sendMsg(config.outfd, ID_CRASH_HUNT_NUM_ACTIONS, INFO, ID_INFO,
                        numInstsAllowed);
            }
        ...
        default:
            sendMsg(
                config.outfd, ID_DATA_STRING, INFO, ID_INFO,
...
void
reportNewProcess(BPatch_thread* /*parent*/, BPatch_thread* child)
{
...
//
bool
insertSummary(BPatch_function* func, BPatch_variableExpr* expr)
{
...
BPatch_variableExpr*
allocateIntegerInMutatee(dynHandle* dh, int defaultValue = 0)
{
    ...
    {
        sendMsg(config.outfd, ID_INST_MALLOC_INT, VERB4, ID_FAIL,
                "Failure initializing counter in mutatee "
...
bool
initTraceInMutatee(dynHandle* dh)
{
...
bool
insertTraceSnippet(dynHandle* dh, BPatch_function* func,
                   BPatch_Vector<BPatch_point*>* points)
{
    ...
    for(unsigned int i = 0; i < points->size(); ++i)
    {
        ...
        BPatchSnippetHandle* handle =
            dh->addSpace->insertSnippet(traceSnippet, *point, when, order);
        if(!handle)
        {
            sendMsg(config.outfd, ID_TRACE_INSERT_ONE, VERB4, ID_FAIL,
    ...
    do
    {
        ...
        if(read_len < trace_msglen)
        {
            ...
            else if(read_len > 0)
            {
                ...
                sendMsg(
                    config.outfd, ID_TRACE_READ, DEBUG, ID_FAIL,
                    sprintf_static(
                        "Read partial message from trace pipe.  Discarding message '%s'.",
                        buf));
                break;
            }
            else if(errno)
            {
                ...
                sendMsg(
                    config.outfd, ID_TRACE_READ, DEBUG, ID_FAIL,
                    sprintf_static("Mutator encountered error on trace pipe read(): %s",
                                   strerror(errno)));
                close(config.pipefd);
                config.pipefd = -1;
                break;
        ...
        if(iter == trace_points.end())
        {
            sendMsg(config.outfd, ID_TRACE_READ, DEBUG, ID_FAIL,
                    sprintf_static("Read invalid message from trace pipe.  0x%s does not "
                                   "refer to a valid BPatch_point.",
                                   buf));
            break;
        }
        sendMsg(config.outfd, ID_TRACE_READ, DEBUG, ID_PASS);
        ...
        if(pFunc)
        {
            if(pFunc->getName(buf, sizeof(buf)))
                ...
                pName = sprintf_static("anonymous function at 0x%*p", trace_msglen,
                                       pFunc->getBaseAddr());
        }
        ...
        if(config.pipefd > 0)
        {
            ...
            sendMsg(config.outfd, ID_TRACE_POINT, INFO, ID_INFO,
                    strcat_static(pType, pName));
        }
    } while(errno == 0);
}
...
closeTracePipe()
{
    ...
    if(trace_fd)
        trace_fd->writeValue(&negOne);
}

bool
generateInstrumentation(dynHandle* dh, BPatch_function* func, BPatch_snippet* incSnippet)
{
    if(config.instType == USER_FUNC)
    {
        ...
        BPatch_Vector<BPatch_function*> funcs;
        if(NULL == dh->image->findFunction(instFunction, funcs, false, true, true) ||
    ...
    else
    {
        ...
        BPatch_arithExpr instArithExpr(
            BPatch_assign, *countVar,
            BPatch_arithExpr(BPatch_plus, *countVar, BPatch_constExpr(1)));
...
bool
instrumentFunctionEntry(dynHandle* dh, BPatch_function* func)
{
    BPatch_Vector<BPatch_point*>* points;
    BPatchSnippetHandle*          handle;
    ...
    if(trace_fd)
        insertTraceSnippet(dh, func, points);
...
bool
instrumentFunctionExit(dynHandle* dh, BPatch_function* func)
{
    BPatch_Vector<BPatch_point*>* points;
    BPatchSnippetHandle*          handle;
    BPatch_snippet                incSnippet;
...
bool
instrumentBasicBlocks(dynHandle* dh, BPatch_function* func)
{
...
bool
instrumentMemoryReads(dynHandle* dh, BPatch_function* func)
{
    BPatch_Set<BPatch_basicBlock*>           allBlocks;
    BPatch_snippet                           incSnippet;
...
bool
instrumentMemoryWrites(dynHandle* dh, BPatch_function* func)
{
    BPatch_Set<BPatch_basicBlock*>           allBlocks;
    BPatch_snippet                           incSnippet;
    BPatch_Set<BPatch_opCode>                ops;
