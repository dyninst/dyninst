...
void stateBasedPrint(message *msgData, statusID prevStat, statusID currStat,
                     int *tabDepth, logLevel priority) {
  switch (currStat) {
    case ID_PASS:
    ...
    case ID_INFO:
    case ID_TEST:
      if (prevStat == ID_TEST) dlog(priority, "\n");
      break;
...
int launch_mutator() {
  ...
  if (config.trace_inst) {
    ...
    if (config.use_process) {
      config.pipefd =
          open(config.pipe_filename, O_RDONLY | O_RSYNC | O_NONBLOCK);
      if (config.pipefd < 0) {
  ...
  if (config.inst_level >= INST_FUNC_ENTRY) {
    ...
    if (config.transMode == TRANS_PROCESS) dynEndTransaction(dh);
  }
  ...
  if (!config.use_process) {
    BPatch_binaryEdit *writeBE =
        dynamic_cast<BPatch_binaryEdit *>(dh->addSpace);
...
void printSummary(BPatch_thread *proc, BPatch_exitType exit_type) {
  ...
  switch (exit_type) {
    ...
    default:
      sendMsg(config.outfd, ID_DATA_STRING, INFO, ID_INFO,
              "Monitor error.  Possibly linked with wrong version of "
              "DyninstAPI.\n");
  }
  return;
}
...
void reportNewProcess(BPatch_thread * /*parent*/, BPatch_thread *child) {
  sendMsg(config.outfd, ID_POST_FORK, INFO, ID_INFO,
          child->getProcess()->getPid());
}
...
BPatch_variableExpr *allocateIntegerInMutatee(dynHandle *dh,
                                              int defaultValue = 0) {
...
bool insertTraceSnippet(dynHandle *dh, BPatch_function *func,
...
void readTracePipe() {
  ...
  do {
    ...
    if (read_len < trace_msglen) {
      if (read_len == -1 && errno == EAGAIN) {
        ...
        sendMsg(config.outfd, ID_TRACE_READ, DEBUG, ID_FAIL,
                sprintf_static("Read partial message from trace pipe.  "
    ...
    if (iter == trace_points.end()) {
      sendMsg(config.outfd, ID_TRACE_READ, DEBUG, ID_FAIL,
              sprintf_static("Read invalid message from trace pipe.  0x%s does "
                             "not refer to a valid BPatch_point.",
                             buf));
      break;
    }
    sendMsg(config.outfd, ID_TRACE_READ, DEBUG, ID_PASS);
    ...
    if (pFunc) {
      if (pFunc->getName(buf, sizeof(buf)))
        ...
        pName = sprintf_static("anonymous function at 0x%0*lx", sizeof(void *),
                               pFunc->getBaseAddr());
    }
...
bool generateInstrumentation(dynHandle *dh, BPatch_function *func,
  ...
  if (config.instType == USER_FUNC) {
    ...
    BPatch_Vector<BPatch_function *> funcs;
    if (NULL ==
            dh->image->findFunction(instFunction, funcs, false, true, true) ||
...
bool instrumentFunctionEntry(dynHandle *dh, BPatch_function *func) {
  ...
  if (!generateInstrumentation(dh, func, &incSnippet)) goto fail;
...
bool instrumentBasicBlocks(dynHandle *dh, BPatch_function *func) {
  ...
  for (iter = allBlocks.begin(); iter != allBlocks.end(); iter++) {
    ...
    BPatchSnippetHandle *handle =
        dh->addSpace->insertSnippet(incSnippet, *entry);
    if (!handle) {
  ...
  if (bb_warn_cnt)
    sendMsg(
        config.outfd, ID_INST_BB_LIST, VERB3, ID_WARN,
...
bool instrumentMemoryReads(dynHandle *dh, BPatch_function *func) {
  ...
  for (iter = allBlocks.begin(); iter != allBlocks.end(); iter++) {
    ...
    sendMsg(config.outfd, ID_INST_INSERT_CODE, VERB4);
    BPatchSnippetHandle *handle =
        dh->addSpace->insertSnippet(incSnippet, *points);
    if (!handle) {
      sendMsg(config.outfd, ID_INST_INSERT_CODE, VERB4, ID_FAIL,
              "Failure in BPatch_process::insertSnippet()");
      ++bb_warn_cnt;
