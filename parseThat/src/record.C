...
record_t record_init() {
  record_t result = {false, STRLIST_INITIALIZER, STRLIST_INITIALIZER, {0}, NULL,
                     NULL};
  return result;
}
...
bool record_create(record_t *record, const char *prog, int argc, char **argv) {
  ...
  if (strrchr(prog, '/')) prog_bin = strrchr(prog, '/') + 1;
...
bool record_search(record_t *newRecord) {
      ...
      if (strlist_cmp(&prog_line, &newRecord->prog_line)) {
        ...
        while (!found && (buf = fgets_static(index_fd)) && buf[0] == '\t') {
          ...
          if (strlist_cmp(&lib_line, &newRecord->lib_line)) {
            ...
            if (latest) diff_list = libs_diff(&lib_line, &newRecord->lib_line);
            latest = false;
          }
        ...
        if (!found) {
          fprintf(config.outfd,
                  "* Warning: Current library set unknown.  Differences from "
                  "latest run include:\n");
          fprintf(config.outfd, "*\n");
          ...
          fprintf(config.outfd,
                  "* Warning: Current library signature does not match latest "
                  "run.\n");
  ...
  if (!found) {
    ...
    if (strrchr(prog_file, '/')) prog_file = strrchr(prog_file, '/') + 1;

    snprintf(newRecord->filename, sizeof(newRecord->filename), "%s/%s-XXXXXX",
             config.record_dir, prog_file);
  ...
  strncat(raw_filename, ".raw",
          sizeof(raw_filename) - strlen(raw_filename) - 1);
...
void record_update(record_t *newRecord) {
  ...
  if (!found) {
    ...
    if (buf && buf[0] != '\0') fprintf(new_fd, "%s\n", buf);
  }
