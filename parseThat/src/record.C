 ...
 */
bool
get_libs(strlist* list, const char* prog)
{
    FILE* ldd_in;
    char *buf, *start, *end;
...
strlist
libs_diff(strlist* prev_run, strlist* curr_run)
{
    ...
    while(prev_ptr || curr_ptr)
    {
        if(prev_ptr && curr_ptr)
        {
            ...
            if(retval == 0)
            {
                ...
                {
                    snprintf(buf, sizeof(buf), "* %s has changed since last run.",
                             prev_ptr);
        ...
        if(!curr_ptr || retval < 0)
        {
            ...
            snprintf(buf, sizeof(buf), "* Mutatee is no longer dependent on %s.",
                     prev_ptr);
            strlist_push_back(&result, buf);
...
{
    record_t result = {
        false, STRLIST_INITIALIZER, STRLIST_INITIALIZER, { 0 }, NULL, NULL
    };
    return result;
}

void
record_clear(record_t* record)
{
...
bool
record_create(record_t* record, const char* prog, int argc, char** argv)
{
    ...
    for(i = 0; i < lib_list.count; ++i)
    {
        ...
        if(!sha1_file(lib_path, hash_buf))
            continue;
...
bool
record_search(record_t* newRecord)
{
    FILE* index_fd;
    char* index_filename;
    ...
    index_filename =
        sprintf_static("%s/%s", config.record_dir, HISTORY_RECORD_INDEX_FILE);
    errno    = 0;
    ...
    if(errno && errno == ENOENT)
    {
        ...
        if(!index_fd)
        {
            fprintf(config.outfd,
                    "*\n* Error opening history record index %s.  Disabling history "
                    "record.\n*\n",
                    index_filename);
        ...
        while((buf = fgets_static(index_fd)))
        {
            ...
            if(strlist_cmp(&prog_line, &newRecord->prog_line))
            {
                ...
                while(!found && (buf = fgets_static(index_fd)) && buf[0] == '\t')
                {
                    ...
                    strncpy(newRecord->filename, strlist_get(&lib_line, 0),
                            sizeof(newRecord->filename) - 1U);
                ...
                if(!found)
                {
                    fprintf(config.outfd, "* Warning: Current library set unknown.  "
                                          "Differences from latest run include:\n");
                    fprintf(config.outfd, "*\n");
                    for(unsigned i = 0; i < diff_list.count; ++i)
                        fprintf(config.outfd, "%s\n", strlist_get(&diff_list, i));
                }
                else if(!latest)
                    fprintf(config.outfd, "* Warning: Current library signature does not "
                                          "match latest run.\n");
...
void
record_update(record_t* newRecord)
{
    FILE* index_fd;
    FILE* new_fd;
    ...
    strncat(index_filename, HISTORY_RECORD_INDEX_FILE,
            sizeof(index_filename) - strlen(index_filename) - 1);
    errno    = 0;
    index_fd = fopen(index_filename, "r");
    if(!index_fd)
    {
    ...
    strncat(new_index_filename, ".update",
            sizeof(new_index_filename) - strlen(new_index_filename) - 1);
    ...
    while((buf = fgets_static(index_fd)))
    {
        ...
        if(strcmp(strlist_get(&newRecord->prog_line, 0), strlist_get(&prog_line, 0)) < 0)
            break;
...
void
record_close(record_t* newRecord)
{
