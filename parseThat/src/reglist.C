...
bool
reglist::insert(const char* pattern, rule_t ruletype, bool show_reason)
{
...
reglist::getReason(char* s)
{
    ...
    if(only_rules.size() > 0)
    {
        for(i = only_rules.begin(); i != only_rules.end(); ++i)
        {
            ...
            {
                dlog(ERR, "Error while executing regular expression in reglist::check(): "
...
bool
reglist::print_regerror(regex_t* preg, int num)
{
    static int   len;
    ...
    if(maxlen == 0)
    {
        dlog(ERR, "Error while processing regular expression, but regerror() not "
