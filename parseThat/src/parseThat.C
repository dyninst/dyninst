...
bool runParseThat(int &bannerLen) {
  ...
  if (config.pid > 0) {
    ...
    if (config.record_enabled) {
      if (!record_create(&config.curr_rec, config.target, config.argc,
                         config.argv))
    ...
    fprintf(config.outfd, "[ Done processing %s ] %n", config.target,
            &bannerLen);
...
bool runHunt_binaryEdit() {
  ...
  if (pid == 0) {
    ...
    if (config.binary_args) {
      ...
      if (offset == 0) {
        dlog(ERR,
             "\nInvalid format for command line arguments to the binary "
             "--args=<numArgs>:<comma-seperated args list>\n");
        exit(-2);
      }
      ...
      if (i <= numargs) {
        dlog(ERR,
             "\nWrong number of arguments specified for executing binary. "
             "Expected %d but got only %d arguments\n",
             numargs, i - 1);
        exit(-2);
      }
    }
...
bool runHunt() {
  ...
  if (config.hunt_low == -1 || config.hunt_high == -1) {
    ...
    if (hard_high == -1) {
      fprintf(hunt_file,
              "Mutator crash prevent range discovery, fix this first\n");
      fflush(hunt_file);
      goto done;
    }
  ...
  for (;;) {
    ...
    if (mid == hard_low) {
      fprintf(hunt_file, "** Found Crash point at [%d, %d) **\n", hard_low,
              hard_high);
      fflush(hunt_file);
      if (!postponed_crashes.size()) goto done;
...
void parseArgs(int argc, char **argv) {
      ...
      switch (*ptr) {
        ...
        case 'f':
          if (++i < argc) {
            ...
            if (strchr(config.inst_function, ':')) {
              ...
              fprintf(stderr,
                      "-f flag requires an argument of the format "
        ...
        case 'i':
          ...
          if (config.inst_level < 0 || config.inst_level >= INST_MAX) {
            fprintf(
                stderr,
                "Invalid argument to -i flag.  Valid range is 0 through %d\n",
                INST_MAX - 1);
            userError();
          }
          break;
        ...
        case 'p':
          ...
          if (config.parse_level < 0 || config.parse_level >= PARSE_MAX) {
            fprintf(
                stderr,
                "Invalid argument to -p flag.  Valid range is 0 through %d\n",
                PARSE_MAX - 1);
            userError();
          }
          break;
        ...
        case 'T':
          ...
          if (config.use_process) config.trace_inst = true;
          break;
        ...
        case '-':
          ...
          if (strcmp(ptr, "-check") == 0) {
            ...
            if (!arg) {
              fprintf(stderr,
                      "--only-func requires a regular expression argument.\n");
              userError();
            }
            if (!config.func_rules.insert(arg, RULE_ONLY)) userError();
            if (needShift) ++i;

          } else if (strcmp(ptr, "-only-mod") == 0) {
            if (!arg) {
              fprintf(stderr,
                      "--only-mod requires a regular expression argument.\n");
              userError();
            }
            if (!config.mod_rules.insert(arg, RULE_ONLY)) userError();
            if (needShift) ++i;
            ...
            if (!arg) {
              fprintf(stderr,
                      "-args requires an argument of the format "
                      "<number_of_arguments:comma-seperated list of "
                      "arguments>\n");
              userError();
            } else {
              if (!strchr(arg, ':')) {
            ...
            if (!arg) {
              fprintf(stderr,
                      "--skip-func requires a regular expression argument.\n");
              userError();
            }
            if (!config.func_rules.insert(arg, RULE_SKIP)) userError();
            if (needShift) ++i;

          } else if (strcmp(ptr, "-skip-mod") == 0) {
            if (!arg) {
              fprintf(stderr,
                      "--skip-mod requires a regular expression argument.\n");
              userError();
            }
            if (!config.mod_rules.insert(arg, RULE_SKIP)) userError();
            if (needShift) ++i;
            ...
            if (!config.hunt_file) config.hunt_file = stderr;
            ...
            if (!arg) {
              fprintf(stderr,
                      "No argument supplied to --use-transactions.  Using "
                      "per-function as default.\n");
            ...
            if (strcmp(arg, "func") == 0)
              ...
              config.transMode = TRANS_MODULE;
            else if (strcmp(arg, "proc") == 0)
              config.transMode = TRANS_PROCESS;
            else {
              fprintf(stderr,
                      "Invalid argument supplied to --use-transactions.  Valid "
                      "arguments are func, mod, or proc.\n");
              userError();
            }
            ...
            if (arg) {
              info =
                  (BPatch_remoteWtxInfo *)malloc(sizeof(BPatch_remoteWtxInfo));
              ...
              config.remoteHost =
                  (BPatch_remoteHost *)malloc(sizeof(BPatch_remoteHost));
  ...
  if (config.target[0] == '\0') {
    if (config.use_attach)
      fprintf(
          stderr,
          "Attach mode requries an image file in addition to process id.\n");
    else
  ...
  if (config.runMode == BATCH_FILE && config.target[0] != '\0') {
    fprintf(
        stderr,
        "Warning: Batch file specified.  Ignoring command line argument %s.\n",
        config.target);
  }
  ...
  if (config.record_enabled) {
    if (config.record_dir[0] == '\0') {
      fprintf(stderr,
              "*\n* Environment variable HOME not defined.  Disabling history "
              "records.\n*\n");
      ...
      if (stat(config.record_dir, &dir_stat) < 0) {
        ...
        fprintf(
            stderr,
            "%s exists, but is not a directory.  History record disabled.\n",
            config.record_dir);
  ...
  if (config.trace_inst) {
    if (config.pipe_filename[0] == '\0') {
      fprintf(stderr,
              "*\n* Environment variable HOME not defined.  Disabling "
      ...
      for (int j = 0; retval < 0; ++j) {
        ...
        strncpy(config.pipe_filename, tmp_pipename,
                sizeof(config.pipe_filename));
      }
    }
  }
  ...
  if (config.no_fork && config.runMode != SINGLE_BINARY) {
    fprintf(stderr,
            "Single (No-Fork) only compatible with a single binary mutatee.\n");
    userError();
  }
}
...
void usage(const char *pname) {
  ...
  fprintf(
      stderr,
      "Options:\n"
      "  -a, --all, --include-libs  Include shared libraries for "
      "instrumentation\n"
      "  --args=<number_of_arguments:comma-separated list of arguments>\n"
      "                             While using runHunt, --args is used to "
      "specify\n"
      "                               a list of command line arguments to run "
      "with\n"
      ...
      "                             Call the specified function at "
      "instrumentation\n"
      ...
      "  -l <library_name>          Force load specified library: can be "
      "relative\n"
      "                               to standard search path or full path\n"
      "  --memcpu, --cpumem         Print memory and CPU usage of mutator on "
      "exit\n"
      "  -m, --merge-tramp          Merge minitramps into basetramps (more "
      "efficent)\n"
      "  -o <filename>              Send all monitor output to specified file\n"
      "                               (Mutator and mutatee output not "
      "included)\n"
      "  --only-mod=<regex>, --only-func=<regex>\n"
      "                             Only consider modules or functions that "
      "match\n"
      ...
      "  -r                         Descend into subdirectories when "
      "processing\n"
      "                               directories\n"
      ...
      "                               match the specified basic regular "
      "expression\n"
      "  --suppress-ipc             Disable mutator to monitor IPC messages\n"
      "  -t <seconds>               Time limit before monitor kills mutator\n"
      "  -T [count], --trace[=num]  Report mutatee progress at each function "
      "entry\n"
      "                               on exit. If count > 0, only last [num] "
      "trace\n"
