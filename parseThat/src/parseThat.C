...
runHunt_binaryEdit()
{
    ...
    if(pid == 0)
    {
        ...
        std::string filename =
            std::string("./") +
            ((config.use_exe) ? config.exeFilePath : config.writeFilePath);
            ...
            switch(*ptr)
            {
                ...
                case 'f':
                    if(++i < argc)
                    {
                        ...
                        {
                            fprintf(stderr, "-f flag requires an argument of the format "
                ...
                case '-':
                    ...
                    else if(strcmp(ptr, "-all") == 0 || strcmp(ptr, "-include-libs") == 0)
                    {
                        config.include_libs = true;
                    }
                    ...
                    else if(strcmp(ptr, "-memcpu") == 0 || strcmp(ptr, "-cpumem") == 0)
                    {
                        config.memcpu = true;
                    }
                    ...
                    else if(strcmp(ptr, "-args") == 0)
                    {
                        if(!arg)
                        {
                            fprintf(stderr, "-args requires an argument of the format "
                                            "<number_of_arguments:comma-seperated list "
                                            "of arguments>\n");
                            userError();
                        }
                        else
                    ...
                    else if(strcmp(ptr, "-use-transactions") == 0)
                    {
                        if(!arg)
                        {
                            fprintf(stderr, "No argument supplied to --use-transactions. "
                                            " Using per-function as default.\n");
                            config.transMode = TRANS_FUNCTION;
                        }

                        for(unsigned j = 0; arg[j]; ++j)
                            arg[j] = tolower(arg[j]);
                        ...
                        {
                            fprintf(stderr,
                                    "Invalid argument supplied to --use-transactions.  "
                                    "Valid arguments are func, mod, or proc.\n");
                            userError();
                        }
                    }
                    else if(strcmp(ptr, "-wtx-target") == 0)
                    {
                        ...
                        if(arg)
                        {
                            info = (BPatch_remoteWtxInfo*) malloc(
                                sizeof(BPatch_remoteWtxInfo));
        ...
        else
        {
            ...
            for(int j = 0; retval < 0; ++j)
            {
                ...
                strncpy(config.pipe_filename, tmp_pipename,
                        sizeof(config.pipe_filename) - 1U);
            }
        }
    }
