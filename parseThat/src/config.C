...
bool
getNext_SingleBinary();
bool
getNext_BatchDirectory();
bool
getNext_BatchFile();
...
configInit()
{
    ...
    outdir = "/tmp";
    snprintf(config.record_dir, sizeof(config.record_dir), "%s/%s", outdir,
             HISTORY_RECORD_DIR_DEFAULT);
    snprintf(config.pipe_filename, sizeof(config.pipe_filename), "%s/pipe-%d", outdir,
             (int) getpid());
...
getNextTarget()
{
    ...
    {
        case SINGLE_BINARY:
            return getNext_SingleBinary();
        case BATCH_DIRECTORY:
            return getNext_BatchDirectory();
        case BATCH_FILE:
            return getNext_BatchFile();
        default:
...
validate_file(const char* file)
{
    ...
    {
        dlog(ERR,
             "Could not read from target mutatee %s.  Dyninst requires read access to "
             "mutatees.\n",
             file);
        return false;
    }
...
getNext_BatchDirectory()
{
    ...
    if(dir_q == NULL)
    {
        ...
        {
            dlog(ERR,
                 "Could not allocate memory for strlist in getNext_BatchDirectory()\n");
            exit(-2);
        }
    ...
    while(dir_q->count > 0 || dirfd != NULL)
    {
        ...
        while((d_entry = readdir(dirfd)) != NULL)
        {
            ...
            /*
             * Generate target filename in config.target from base_dir and
             * d_entry->d_name.
             */
            {
            ...
            config.argv[0] =
                strrchr(config.target, '/') + 1; /* strrchr() will never return NULL
                                                    because we control base_dir. */

            /*
             * Check if target filename is a directory, or executable via stat().  Ignore
             * if neither.
             */
            ...
            {
                if(validate_file(config.target))
                    return true;
            }
            else
            {
        ...
        while(closedir(dirfd) != 0)
        {
            ...
            {
                dlog(ERR,
                     "Warning: closedir() failed 10 times.  Ignoring open descriptor.\n");
                break;
            }
        }
        dirfd = NULL;
...
getNext_BatchFile()
{
    ...
    while((buf = fgets_static(filefd)))
    {
        ...
        strncpy(config.target, strlist_get(&cmdline, 0), sizeof(config.target) - 1U);

        if(maxArgc < (cmdline.count + 1))
        {
            char** newargv =
                (char**) realloc(config.argv, (cmdline.count + 1) * sizeof(char*));
            if(!newargv)
            {
                dlog(
                    ERR,
                    "Could not allocate memory for argv array in getNext_BatchFile().\n");
                exit(-2);
            }
            config.argv = newargv;
        }
