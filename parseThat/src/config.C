...
bool validate_file(const char *file) {
  ...
  if (errno) {
    dlog(ERR,
         "Could not read from target mutatee %s.  Dyninst requires read access "
         "to mutatees.\n",
         file);
    return false;
  }
...
bool getNext_BatchDirectory() {
  ...
  if (dir_q == NULL) {
    ...
    if (!dir_q) {
      dlog(ERR,
           "Could not allocate memory for strlist in "
  ...
  while (dir_q->count > 0 || dirfd != NULL) {
    ...
    while ((d_entry = readdir(dirfd)) != NULL) {
       ...
       * d_entry->d_name.
       */
      snprintf(config.target, sizeof(config.target), "%s%s", base_dir,
               d_entry->d_name);
      config.argv[0] =
      ...
      /*
       * Check if target filename is a directory, or executable via stat().
       * Ignore if neither.
       */
    ...
    while (closedir(dirfd) != 0) {
      if (++i > 10) {
        dlog(ERR,
             "Warning: closedir() failed 10 times.  Ignoring open "
             "descriptor.\n");
        break;
      }
    }
...
bool getNext_BatchFile() {
  ...
  if (filefd == NULL) {
    ...
    if (++i > 10) {
      dlog(ERR,
           "open() for %s has been interrupted 10 times.  Try again later.\n",
           config.config_file);
      exit(-2);
    }
    config.argv = NULL;
  ...
  while ((buf = fgets_static(filefd))) {
    ...
    if (maxArgc < (cmdline.count + 1)) {
      ...
      if (!newargv) {
        dlog(ERR,
             "Could not allocate memory for argv array in "
             "getNext_BatchFile().\n");
        exit(-2);
      }
      config.argv = newargv;
