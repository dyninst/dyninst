...
void
printSummary(BPatch_thread*, BPatch_exitType);
void
reportNewProcess(BPatch_thread*, BPatch_thread*);
...
mutatorInit(void)
{
    ...
    else if(config.use_process)
    {
        ...
        dh->addSpace = dh->bpatch->processCreate(config.target,
                                                 const_cast<const char**>(config.argv));
        dh->proc     = dynamic_cast<BPatch_process*>(dh->addSpace);
        if(!dh->proc)
        {
...
bool
dynStartTransaction(dynHandle* dh)
{
...
bool
dynEndTransaction(dynHandle* dh)
{
