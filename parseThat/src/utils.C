...
char*
strcat_static(const char* s1, const char* s2)
{
    static int   len = 0;
    static char* buf = NULL;
...
char*
decodeStr(const char* s, char** end_ptr)
{
    static unsigned len = 0;
    static char*    buf = NULL;
    ...
    if(end_ptr)
        *end_ptr = const_cast<char*>(s + i + 1);
    return buf;
}

static struct rusage ru_start;
static void
mark_usage(struct rusage* ru)
{
