...
#    define DEF_AST_LEAF_TYPE(name, type)                                                \
        ...
        public:                                                                          \
            ...
            {                                                                            \
                return ((a->getID() == V_##name) ? boost::static_pointer_cast<name>(a)   \
                                                 : Ptr());                               \
            }                                                                            \
...
#    define DEF_AST_INTERNAL_TYPE(name, type)                                            \
        ...
        public:                                                                          \
            ...
            static Ptr create(type t, AST::Ptr a, AST::Ptr b)                            \
            {                                                                            \
                return Ptr(new name(t, a, b));                                           \
            }                                                                            \
            static Ptr create(type t, AST::Ptr a, AST::Ptr b, AST::Ptr c)                \
            {                                                                            \
                return Ptr(new name(t, a, b, c));                                        \
            }                                                                            \
            ...
            void        setChild(int i, AST::Ptr a) { kids_[i] = a; }                    \
                                                                                         \
        private:                                                                         \
            name(type t, AST::Ptr a)                                                     \
            : t_(t)                                                                      \
            {                                                                            \
                kids_.push_back(a);                                                      \
            }                                                                            \
            name(type t, AST::Ptr a, AST::Ptr b)                                         \
            : t_(t)                                                                      \
            {                                                                            \
                kids_.push_back(a);                                                      \
                kids_.push_back(b);                                                      \
            }                                                                            \
            name(type t, AST::Ptr a, AST::Ptr b, AST::Ptr c)                             \
            : t_(t)                                                                      \
            {                                                                            \
                kids_.push_back(a);                                                      \
                kids_.push_back(b);                                                      \
            ...
            {                                                                            \
                ...
                for(unsigned i = 0; i < kids_.size(); ++i)                               \
                    if(!(kids_[i]->equals(other.kids_[i])))                              \
                        return false;                                                    \
                return true;                                                             \
            }                                                                            \
            const type t_;                                                               \
...
public:
    ...
    virtual void setChild(int, AST::Ptr) { assert(0); }
