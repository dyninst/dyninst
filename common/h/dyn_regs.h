...
COMMON_EXPORT unsigned
                    getArchAddressWidth(Dyninst::Architecture arch);
...
private:
    ...
    void                                            init_names();
    // reg_class is set to a corresponding enum value that can be found in
    // "external/rose/amdgpuInstructionEnum.h" reg_idx is set to the index/id of the
    // register for future lookup offset is set to the byte offset from where the register
    // value starts, that is, sub register access ( or register access in a register
    // vector)
...
public:
    ...
    bool            isValid() const;
    MachRegisterVal getSubRegValue(const MachRegister& subreg,
