...
class AnnotationContainer {
 ...
 public:
  ...
  virtual Serializable *ac_serialize_impl(SerializerBase *, const char *)
...
class SerializerError {
  //  SerializerError:  a small class that is thrown by
  //  serialization/deserialization
  //  routines.  This exists as an attempt to standardize and simplify error
  //  handling
  //  for ser-des routines that are possibly deeply nested.
  //  Here's the rub:  we don't want stray, unhandled exceptions finding their
  //  way into
  //  the larger system...  thus all entry points to
  //  serialization/deserialization need
  //  to catch this exception to render it transparent to the rest of the
  //  system.
 ...
 public:
  COMMON_EXPORT SerializerError(
      const std::string &__file__, const int &__line__, const std::string &msg,
      SerializerErrorType __err__ = ser_err_unspecified)
      : msg__(msg), file__(__file__), line__(__line__), err__(__err__) {}
...
typedef enum { ser_bin, ser_xml } ser_type_t;

class SerContextBase {
  friend class Serializable;
  friend class SerializerBase;
  COMMON_EXPORT static std::vector<
      std::pair<std::string, dyn_hash_map<std::string, short> *> >
      ser_control_map;
  std::string fname;
...
class SerContext : public SerContextBase {
 ...
 public:
  SerContext(T *scope_, std::string fname)
      : SerContextBase(std::string(typeid(T).name()), fname), scope(scope_) {}
  ~SerContext() {}
...
class SerializerBase {
 ...
 private:
  ...
  COMMON_EXPORT static dyn_hash_map<std::string, subsystem_serializers_t>
      all_serializers;
 ...
 public:
  ...
  COMMON_EXPORT SerializerBase(SerContextBase *scb, std::string name_,
                               std::string filename, iomode_t dir,
                               bool verbose);
  ...
  COMMON_EXPORT static SerializerBase *getSerializer(std::string subsystem,
                                                     std::string fname);
  COMMON_EXPORT static bool addSerializer(std::string subsystem,
                                          std::string fname,
                                          SerializerBase *sb);
  ...
  COMMON_EXPORT virtual void annotation_start(AnnotationClassID &a_id,
                                              void *&lparent_id,
  ...
  COMMON_EXPORT void translate_base(const char *&v, int bufsize,
                                    const char *&t);
  ...
  COMMON_EXPORT void serialize_annotations(void *, std::vector<ser_rec_t> &sers,
                                           const char * = NULL);
  COMMON_EXPORT bool serialize_post_annotation(void *parent_id, void *anno,
                                               AnnotationClassBase *acb,
                                               sparse_or_dense_anno_t,
                                               const char * = NULL);
};

SerializerBase *createSerializer(SerContextBase *, std::string, std::string,
                                 ser_type_t, iomode_t, bool = false);

class AnnotatableSparse;
class AnnotatableDense;
class AnnotationContainerBase;

COMMON_EXPORT void serialize_annotatable_id(SerializerBase *sb, void *&id,
                                            const char *tag);
COMMON_EXPORT bool set_sb_annotatable_sparse_map(SerializerBase *,
                                                 AnnotatableSparse *, void *);
COMMON_EXPORT bool set_sb_annotatable_dense_map(SerializerBase *,
                                                AnnotatableDense *, void *);
COMMON_EXPORT unsigned short get_serializer_index(SerializerBase *sb);
COMMON_EXPORT void annotation_start(SerializerBase *, AnnotationClassID &,
                                    void *&, sparse_or_dense_anno_t &,
                                    const char *);
COMMON_EXPORT void annotation_end(SerializerBase *);
COMMON_EXPORT AnnotatableSparse *find_sparse_annotatable(SerializerBase *,
                                                         void *);
COMMON_EXPORT AnnotatableDense *find_dense_annotatable(SerializerBase *,
                                                       void *);
COMMON_EXPORT bool isEOF(SerializerBase *);
COMMON_EXPORT void throw_ser_err(const char *file__, unsigned int line,
                                 const char *msg);
...
COMMON_EXPORT bool deserialize_container_item(AnnotationContainerBase *acb,
                                              SerializerBase *sb);

template <class T>
void enableSerialize(std::string fname, bool val) {
  serialize_printf("%s[%d]:  %s serialize for type %s\n", FILE__, __LINE__,
                   val ? "enabling" : "disabling", typeid(T).name());
...
void enforceDeserialize(std::string fname, bool val) {
  serialize_printf("%s[%d]:  %s enforced deserialize for type %s\n", FILE__,
                   __LINE__, val ? "enabling" : "disabling", typeid(T).name());
  SerContextBase::enforceDeserialize(std::string(typeid(T).name()), fname, val);
}

template <class T>
bool deserializeEnforced(std::string fname) {
  return SerContextBase::deserializeEnforced(std::string(typeid(T).name()),
                                             fname);
}
...
class Serializable {
 ...
 protected:
    ...
    if (active_serializer_index != (unsigned short)-1) {
      ...
      if (sb) {
        ...
        if (scb->isContextType(this)) {
          ...
          fprintf(stderr,
                  "%s[%d]:  TOP LEVEL SERIALIZE DONE:  removing serializer\n",
                  FILE__, __LINE__);
  ...
  COMMON_EXPORT virtual Serializable *serialize_impl(SerializerBase *,
                                                     const char * = NULL)
      THROW_SPEC(SerializerError) = 0;

 public:
  COMMON_EXPORT unsigned short getID() { return active_serializer_index; }

  COMMON_EXPORT bool serialize(std::string filename, SerContextBase *scb,
                               ser_type_t);
  ...
  COMMON_EXPORT bool deserialize(std::string filename, SerContextBase *scb) {
    ...
    SerializerBase *serializer = createSerializer(
        scb, sername, filename, ser_bin, sd_deserialize, /*verbose*/ false);

    if (!serializer) {
      serialize_printf(
          "%s[%d]:  ERROR:  failed to create deserializer for %s\n", FILE__,
      ...
      try {
        ...
        switch (op) {
          case sp_add_anno: {
            ...
            if (!sf) {
              fprintf(stderr,
                      "%s[%d]:  failed to find serialization function\n",
                      FILE__, __LINE__);
              serializer->clearAnnotationMaps();
              clearContainersByID();
              return false;
            ...
            if (sparse == sod) {
              AnnotatableSparse *as =
                  find_sparse_annotatable(serializer, parent_id);
              if (NULL == as) {
                fprintf(stderr,
                        "%s[%d]:  ERROR:  cannot find anno parent id %p for "
                        "anno of type %s\n",
                ...
                if (!as->addAnnotation(my_anno, acb->getID())) {
                  fprintf(stderr,
                          "%s[%d]:  ERROR:  failed to add annotation here\n",
                          FILE__, __LINE__);
                }
              ...
              AnnotatableDense *ad =
                  find_dense_annotatable(serializer, parent_id);
              if (NULL == ad) {
                fprintf(stderr,
                        "%s[%d]:  ERROR:  cannot find anno parent id = %p\n",
              ...
              fprintf(stderr,
                      "%s[%d]:  ERROR:  sparse/dense not set properly\n",
                      FILE__, __LINE__);
              serializer->clearAnnotationMaps();
              clearContainersByID();
              return false;
  ...
  COMMON_EXPORT virtual Serializable *serialize(SerializerBase *sb,
    ...
    //  then serialize all Annotations for which a serialization function has
    //  been provided
    void *id = this;
    ...
    if (as) {
      ...
      if (is_input(sb)) {
        if (NULL != id) {
          if (!set_sb_annotatable_sparse_map(sb, as, id)) {
            fprintf(stderr,
                    "%s[%d]:  failed to set annotatable-anno mapping here\n",
                    FILE__, __LINE__);
          }
        } else
      ...
      if (ad) {
        ...
        serialize_printf(
            "%s[%d]:  %sserializing annotatable id %p, this = %p\n", FILE__,
            __LINE__, is_output(sb) ? "" : "de", id, this);
        ...
        if (is_input(sb)) {
          if (NULL != id) {
            ...
            if (!set_sb_annotatable_dense_map(sb, ad, id)) {
              fprintf(stderr,
                      "%s[%d]:  failed to set annotatable-annotation map\n",
                      FILE__, __LINE__);
            }

            serialize_printf(
                "%s[%d]:  set dense annotatable mapping for id %p\n", FILE__,
                __LINE__, (void *)id);
          } else
    ...
    if (is_input(sb)) was_deserialized = true;
...
class AnnotationContainerBase : public Serializable {
  friend class Serializable;
  friend COMMON_EXPORT bool deserialize_container_item(SerializerBase *,
                                                       void *);
  friend COMMON_EXPORT bool deserialize_container_item(
      AnnotationContainerBase *, SerializerBase *);
 ...
 public:
  ...
  COMMON_EXPORT virtual Serializable *serialize_impl(SerializerBase *sb,
    ...
    //  serialize the id of this container, then call the container serialize
    //  func
    ...
    annotatable_printf(
        "%s[%d]: %s AnnotationContainer(%p) of type %s, id = %p\n", FILE__,
        __LINE__, is_input(sb) ? "deserialized" : "serialized", this,
  ...
  COMMON_EXPORT static AnnotationContainerBase *getContainer(void *id) {
    ...
    if (iter == containers_by_id.end()) {
      fprintf(stderr,
              "%s[%d]:  ERROR:  cannot find id %p in container-id map\n",
              FILE__, __LINE__, id);
      return NULL;
    }
  ...
  COMMON_EXPORT static void setContainerID(AnnotationContainerBase *acb,
                                           void *id) {
...
template <class T>
Serializable *cont_ser_func_wrapper(void *it, SerializerBase *sb,
                                    const char *) {
  T *itt = (T *)it;
  gtranslate(sb, *itt);
  return NULL;
...
class AnnotationContainer : public AnnotationContainerBase {
 ...
 public:
  ...
  ~AnnotationContainer() { /* if (ac) delete ac; */
  }

  //  The routine that actually adds the item and manages whatever data
  //  structure
  ...
  bool addItem(T t) {
    ...
    //  If a serialization has already occurred for the container, serialize
    //  this item too
  ...
  virtual Serializable *ac_serialize_impl(SerializerBase *, const char *tag)
      THROW_SPEC(SerializerError) = 0;
};
...
void translate_vector(SerializerBase *ser, std::vector<T> &vec,
  ...
  if (ser->iomode() == sd_serialize) nelem = (unsigned long)vec.size();
...
void translate_dyn_hash_map(S *ser, dyn_hash_map<K, V> &map,
  ...
  serialize_printf(
      "%s[%d]:  welcome to translate_dyn_hash_map<%s, %s>, size = %ld\n",
  ...
  serialize_printf("%s[%d]:  leaving translate_dyn_hash_map<%s, %s>\n", FILE__,
                   __LINE__, typeid(K).name(), typeid(V).name());
}

template <class S, class K, class V>
void translate_map(S *ser, std::map<K, V> &map, const char *tag = NULL,
                   const char *elem_tag = NULL) {
  unsigned long nelem = 0UL;
  nelem = map.size();
...
void translate_hash_map(S *ser, dyn_hash_map<K, V> &hash,
                        const char *tag = NULL, const char *key_tag = NULL,
                        const char *value_tag = NULL) {
  fprintf(stderr, "%s[%d]:  welcome to translate_hash_map<%s, %s>()\n",
          __FILE__, __LINE__, typeid(K).name(), typeid(V).name());
  ...
  if (ser->iomode() == sd_serialize) {
    ...
    for (unsigned long i = 0; i < nelem; ++i) {
      ...
      if (dyn_detail::boost::is_pointer<K>::value) memset(&k, 0, sizeof(K));
      if (dyn_detail::boost::is_pointer<V>::value) memset(&v, 0, sizeof(V));
      ser->translate_base(k, key_tag);
      ser->translate_base(v, value_tag);
      hash[k] = v;
    }
...
template <
    typename T, typename T2 = typename dyn_detail::boost::is_enum<T>::type,
    typename T3 = typename dyn_detail::boost::is_base_of<Serializable, T>::type>
class trans_adaptor {
 public:
  trans_adaptor() {}

  T *operator()(SerializerBase *ser, T &it, const char *tag = NULL,
...
class trans_adaptor<char *, dyn_detail::boost::false_type,
 ...
 public:
  ...
  char **operator()(SerializerBase *ser, char *&it, const char *tag = NULL,
                    const char * /*tag2*/ = NULL) {
...
template <typename T,
          bool = dyn_detail::boost::is_base_of<
              Serializable,
...
class deref_adaptor {
 public:
  ...
  T *operator()(SerializerBase *sb, T &it, const char *tag = NULL,
                const char *tag2 = NULL) {
    trans_adaptor<T> ta;
    ta(sb, it, tag, tag2);
    return &it;
...
template <typename T,
          typename T2 = typename dyn_detail::boost::is_pointer<T>::type,
          typename T3 = typename dyn_detail::boost::is_base_of<
              Serializable,
              typename dyn_detail::boost::remove_pointer<T>::type>::type>
class ptr_adaptor {
 public:
...
template <typename T>
class ptr_adaptor<T, dyn_detail::boost::true_type,
                  dyn_detail::boost::false_type> {
 ...
 public:
  ...
  T *operator()(SerializerBase *sb, T &it, const char *tag = NULL,
    ...
    if (!isOutput(sb)) {
      if (it)
        fprintf(stderr,
                "%s[%d]:  WARNING:  allocating to non-null pointer: %s\n",
                FILE__, __LINE__, typeid(T).name());
    ...
    if (!isOutput(sb)) {
      ...
      if (res && !res_typed) {
        fprintf(stderr,
                "%s[%d]:  ERROR:  ser func allocated bad type object! not %s\n",
                FILE__, __LINE__, typeid(T).name());
      }
...
template <>
class ptr_adaptor<char *, dyn_detail::boost::true_type,
                  dyn_detail::boost::false_type> {
 public:
  ptr_adaptor() {}
  char **operator()(SerializerBase *sb, char *&it, const char *tag = NULL,
                    const char *tag2 = NULL) {
...
class const_adaptor {
 public:
  ...
  T *operator()(SerializerBase *sb, T &it, const char *tag = 0,
                const char *tag2 = 0) {
    ptr_adaptor<T> ta;
...
template <typename T>
void gtranslate(SerializerBase *ser, T &it, const char *tag = NULL,
                const char *tag2 = NULL) {
...
template <typename T>
void gtranslate(SerializerBase *ser, std::vector<T> &it, const char *tag = NULL,
                const char *tag2 = NULL) {
...
template <typename T>
void gtranslate(SerializerBase *ser, T &it, const char *(*to_str_func)(T),
                const char *tag = NULL, const char * /*tag2*/ = NULL) {
  assert(ser);
  int enum_int = 0;
  enum_int = (int)it;
...
template <typename T>
bool gtranslate_w_err(SerializerBase *ser, T &it, const char *tag = NULL,
                      const char *tag2 = NULL) {
  try {
