...
COMMON_EXPORT int
annotatable_printf(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
...
public:
    ...
    void*       allocate() DYNINST_MALLOC_ANNOTATION { return (void*) new T(); }
    ...
    AnnotatableDense& operator=(const AnnotatableDense& rhs)
    {
        if(this != &rhs)
        {
            ...
            if(rhs.annotations)
            {
                ...
                annotations->max  = size;
                annotations->data = (anno_list_t*) calloc(sizeof(anno_list_t), (size));
                memcpy(annotations->data, rhs.annotations->data,
                       size * sizeof(anno_list_t));
            }
            else
            {
                annotations = NULL;
    ...
    inline bool removeAnnotation(AnnotationClass<T>& a_id)
    {
        if(annotation_debug_flag())
        {
            fprintf(stderr, "%s[%d]:  Dense(%p) remove %s-%d, %s\n", FILE__, __LINE__,
                    (void*) this, a_id.getName().c_str(), a_id.getID(),
                    a_id.getTypeName());
        }
    ...
    void annotationsReport()
    {
        ...
        if(annotations && annotations->data)
        {
            ...
            fprintf(stderr, "%s[%d]:  Dense(%p):  have %lu annotations\n", FILE__,
                    __LINE__, (void*) this, (unsigned long) atypes.size());
...
#    pragma warning(disable : 4251)
    ...
    ~AnnotatableSparse() { ClearAnnotations("dtor"); }
...
private:
    ...
    AN_INLINE bool addAnnotation(const void* a, AnnotationClassID aid)
    {
        ...
        else
        {
            ...
            if(a != iter->second)
            {
                annotatable_printf("%s[%d]:  WEIRD:  already have annotation of this "
                                   "type: %p, replacing with %p\n",
                                   FILE__, __LINE__, (void*) iter->second,
                                   (const void*) a);
...
public:
    bool operator==(AnnotatableSparse& cmp)
    {
        ...
        for(unsigned int i = 0; i < ntypes; ++i)
        {
            ...
            if((cmp_abt_iter == cmp_abt->end()) && (this_abt_iter != this_abt->end()))
            {
                return false;
            }
    ...
    AN_INLINE bool getAnnotation(T*& a, AnnotationClass<T>& a_id) const
    {
        ...
        void*              annos_for_object =
            getAnnosForObject(abt, (void*) this_noconst, false /*no create if none*/);
