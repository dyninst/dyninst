...
class AnnotationClass : public AnnotationClassBase {
 public:
  AnnotationClass(std::string n, anno_cmp_func_t cmp_func_ = NULL,
                  ser_func_t s = NULL)
...
typedef enum { sparse, dense } sparse_or_dense_anno_t;
...
COMMON_EXPORT bool ser_operation(SerializerBase *, ser_post_op_t &,
                                 const char *);
COMMON_EXPORT bool add_annotations(SerializerBase *, AnnotatableDense *,
                                   std::vector<ser_rec_t> &);
COMMON_EXPORT bool add_annotations(SerializerBase *, AnnotatableSparse *,
                                   std::vector<ser_rec_t> &);
#if !defined(SERIALIZATION_DISABLED)
COMMON_EXPORT bool serialize_annotation_list(void *, std::vector<ser_rec_t> &,
                                             SerializerBase *sb, const char *);
COMMON_EXPORT SerializerBase *getExistingOutputSB(unsigned short);
COMMON_EXPORT bool serialize_post_annotation(void *, void *, SerializerBase *,
                                             AnnotationClassBase *acb,
...
**/
 ...
 private:
  ...
  bool addAnnotation(const void *a, AnnotationClassID id) {
    ...
    //  actual annotations data array in case where we have performed
    //  serialization

    if (annotations->data == NULL) {
      annotations->data = (anno_list_t *)calloc(sizeof(anno_list_t), (size));
      annotations->max = size;
      for (unsigned i = 0; i < size; i++) annotations->data[i] = NULL;
    } else if (id >= annotations->max) {
      int old_max = annotations->max;
      size = annotations->max * 2;
      annotations->max = size;
      annotations->data =
          (anno_list_t *)realloc(annotations->data, sizeof(anno_list_t) * size);
      for (unsigned i = old_max; i < size; i++) annotations->data[i] = NULL;
    }
 ...
 public:
  ...
  ~AnnotatableDense() {
    if (annotations) {
      if (annotations->data) free(annotations->data);
      free(annotations);
    }
  }
  ...
  inline bool removeAnnotation(AnnotationClass<T> &a_id) {
    ...
    if (!annotations->data[id]) return false;
  ...
  void serializeAnnotations(SerializerBase *sb, const char *tag) {
      ...
      if (annotations) {
        for (AnnotationClassID id = 0; id < annotations->max; ++id) {
          ...
          if (anno) {
            ...
            if (!acb) {
              fprintf(stderr,
                      "%s[%d]:  FIXME:  no annotation class for id %d\n",
                      FILE__, __LINE__, id);
              continue;
            }
        ...
        serialize_printf("%s[%d]:  %p set serializer index to %d\n", FILE__,
                         __LINE__, this, annotations->serializer_index);
      } else {
    ...
    if (!add_annotations(sb, this, my_sers)) {
      fprintf(stderr,
              "%s[%d]:  failed to update annotation list after deserialize\n",
              FILE__, __LINE__);
    }
  }
#else
  ...
  void annotationsReport() {
    ...
    if (annotations && annotations->data) {
      ...
      fprintf(stderr, "%s[%d]:  Dense(%p):  have %lu annotations\n", FILE__,
              __LINE__, this, (unsigned long)atypes.size());
...
class COMMON_EXPORT AnnotatableSparse {
 ...
 public:
  struct void_ptr_hasher {
    size_t operator()(const void *a) const { return (size_t)a; }
  };
...
// COMMON_EXPORT static annos_t *annos;
  ...
  annos_by_type_t *getAnnosOfType(AnnotationClassID aid,
  ...
  void *getAnnosForObject(annos_by_type_t *abt, void *obj,
                          bool do_create = false) const {
    assert(abt);
    assert(obj);
  ...
  AN_INLINE bool addAnnotation(const void *a, AnnotationClassID aid) {
    if (annotation_debug_flag()) {
      fprintf(
          stderr, "%s[%d]:  Sparse(%p) add %s-%d void\n", FILE__, __LINE__,
          this,
    ...
    if (iter == abt->end()) {
      ...
      if (a != iter->second) {
        annotatable_printf(
            "%s[%d]:  WEIRD:  already have annotation of this type: %p, "
            "replacing with %p\n",
 ...
 public:
  ...
  AN_INLINE bool addAnnotation(const T *a, AnnotationClass<T> &a_id) {
    annotatable_printf("%s[%d]:  Sparse(%p):  Add %s-%d, %s\n", FILE__,
                       __LINE__, this, a_id.getName().c_str(), a_id.getID(),
                       typeid(T).name());
    void *obj = this;
    ...
    if (iter == abt->end()) {
      ...
      if (iter->second != a) {
        // fprintf(stderr, "%s[%d]:  WEIRD:  already have annotation of type %s:
    ...
    if (seriter != ser_ndx_map.end()) {
      if (seriter->second != (unsigned short)-1) {
        ...
        if (sf) {
          ...
          ser_operation(sb, op, "AnnotationAdd");
          void *aa = (void *)const_cast<T *>(a);
          serialize_post_annotation(this, aa, sb, &a_id, sparse,
                                    "PostAnnotation");
        }
      }
    }
  ...
  void serializeAnnotations(SerializerBase *sb, const char *) {
    ...
    if (is_output(sb)) {
      for (AnnotationClassID id = 0; id < l_annos.size(); ++id) {
        ...
        //  we have an annotation of this type for this object, find the
        //  serialization
        ...
        if (!acb) {
          fprintf(stderr,
                  "%s[%d]:  FIXME, cannot find annotation class base for id "
                  "%d, mode = %s\n",
                  FILE__, __LINE__, id,
                  is_input(sb) ? "deserialize" : "serialize");
          continue;
        }
