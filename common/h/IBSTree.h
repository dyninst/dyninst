...
/** Template class for Interval Binary Search Tree. The implementation is
  ...
  *     title = {The IBS-tree: A data structure for finding all intervals that
  *overlap a point},
  *     institution = {},
  *     year = {1990}
  * }
...
class IBSNode {
 ...
 public:
  ...
  IBSNode(interval_type value, IBSNode *n)
      : val_(value), color(IBS::TREE_RED), left(n), right(n), parent(NULL) {}
...
class IBSTree {
 public:
  ...
  void findIntervals(interval_type X, IBSNode<ITYPE> *R,
...
void IBSTree<ITYPE>::rightRotate(IBSNode<ITYPE> *pivot) {
  ...
  if (y == nil) return;
  pivot->left = y->right;
  if (y->right != nil) y->right->parent = pivot;
  ...
  while (it != y->greater.end()) {
    ...
    if (pit == pivot->greater.end()) {
      ...
      if (pit != pivot->equal.end()) pivot->equal.erase(pit);
...
template <class ITYPE>
IBSNode<ITYPE> *IBSTree<ITYPE>::addLeft(ITYPE *I, IBSNode<ITYPE> *R) {
...
template <class ITYPE>
IBSNode<ITYPE> *IBSTree<ITYPE>::addRight(ITYPE *I, IBSNode<ITYPE> *R) {
  IBSNode<ITYPE> *parent = NULL;
  ...
  while (1) {
    ...
    if (rval == ihigh) {
      ...
      if (leftUp(R) >= ilow) R->less.insert(I);
...
template <class ITYPE>
typename ITYPE::type IBSTree<ITYPE>::rightUp(IBSNode<ITYPE> *R) {
  while (NULL != R->parent) {
    if (R->parent->left == R) return R->parent->value();
    R = R->parent;
  }
...
template <class ITYPE>
typename ITYPE::type IBSTree<ITYPE>::leftUp(IBSNode<ITYPE> *R) {
  while (NULL != R->parent) {
    if (R->parent->right == R) return R->parent->value();
    R = R->parent;
  }
...
void IBSTree<ITYPE>::destroy(IBSNode<ITYPE> *n) {
  if (!n || (n == nil)) return;
  if (n->left != nil) destroy(n->left);
  if (n->right != nil) destroy(n->right);
  delete n;
}
...
template <class ITYPE>
void IBSTree<ITYPE>::findIntervals(interval_type X, IBSNode<ITYPE> *R,
                                   std::set<ITYPE *> &S) const {
  while (R != nil) {
    if (X == R->value()) {
...
*/
template <class ITYPE>
void IBSTree<ITYPE>::findIntervals(ITYPE *I, IBSNode<ITYPE> *R,
                                   std::set<ITYPE *> &S) const {
  if (R == nil) return;
...
void IBSTree<ITYPE>::successor(interval_type X, std::set<ITYPE *> &out) const {
  ...
  while (1) {
    ...
    if (X >= n->value()) {
      ...
      if (last != nil) stack.push_back(last);
      last = n;
      n = n->left;
    }
  }
