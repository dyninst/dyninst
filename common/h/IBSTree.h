 ...
 *     title = {The IBS-tree: A data structure for finding all intervals that overlap a
 *point}, institution = {}, year = {1990}
 * }
 **/
...
template <class ITYPE = SimpleInterval<>>
std::ostream&
operator<<(std::ostream& os, std::set<ITYPE*>& s)
{
    for(auto i = s.begin(); i != s.end(); i++)
    {
        std::cerr << "[0x" << std::hex << (*i)->low() << ", 0x" << (*i)->high()
                  << std::dec << ")  ";
    }
    return os;
}
...
public:
    ...
    size_type      size() const { return treeSize.load(); }
...
template <class ITYPE>
void
IBSTree<ITYPE>::rightRotate(IBSNode<ITYPE>* pivot)
{
    if(!pivot || (pivot == nil))
        return;
...
template <class ITYPE>
void
IBSTree<ITYPE>::leftRotate(IBSNode<ITYPE>* pivot)
{
    if(!pivot || (pivot == nil))
        return;
...
template <class ITYPE>
void
IBSTree<ITYPE>::insertFixup(IBSNode<ITYPE>* x)
{
...
template <class ITYPE>
void
IBSTree<ITYPE>::destroy(IBSNode<ITYPE>* n)
{
    if(!n || (n == nil))
        return;
    if(n->left != nil)
...
template <class ITYPE>
void
IBSTree<ITYPE>::findIntervals(interval_type X, IBSNode<ITYPE>* R,
                              std::set<ITYPE*>& S) const
{
    while(R != nil)
    {
        if(X == R->value())
...
template <class ITYPE>
void
IBSTree<ITYPE>::findIntervals(ITYPE* I, IBSNode<ITYPE>* R, std::set<ITYPE*>& S) const
{
    if(R == nil)
        return;
...
template <class ITYPE>
void
IBSTree<ITYPE>::removeInterval(IBSNode<ITYPE>* R, ITYPE* range)
{
    if(R == nil)
        return;
...
template <class ITYPE>
int
IBSTree<ITYPE>::CountMarks(IBSNode<ITYPE>* R) const
{
    if(R == nil)
        return 0;

    return (R->less.size() + R->greater.size() + R->equal.size()) + CountMarks(R->left) +
           CountMarks(R->right);
}
...
template <class ITYPE>
void
IBSTree<ITYPE>::insert(ITYPE* range)
{
...
template <class ITYPE>
void
IBSTree<ITYPE>::remove(ITYPE* range)
{
...
template <class ITYPE>
int
IBSTree<ITYPE>::find(interval_type X, std::set<ITYPE*>& out) const
{
...
template <class ITYPE>
int
IBSTree<ITYPE>::find(ITYPE* I, std::set<ITYPE*>& out) const
{
    unsigned size = out.size();
    {
        dyn_rwlock::shared_lock l(rwlock);
...
template <class ITYPE>
void
IBSTree<ITYPE>::successor(interval_type X, std::set<ITYPE*>& out) const
{
...
template <class ITYPE>
ITYPE*
IBSTree<ITYPE>::successor(interval_type X) const
{
    std::set<ITYPE*> out;
    {
        dyn_rwlock::shared_lock l(rwlock);
...
template <class ITYPE>
int
IBSTree<ITYPE>::height(IBSNode<ITYPE>* n)
{
    if(!n)
        return 0;
...
template <class ITYPE>
void
IBSTree<ITYPE>::PrintPreorder(IBSNode<ITYPE>* n, int indent)
{
    if(n == nil)
        return;

    // print self
    doIndent(indent) << "node: 0x" << std::hex << n->value() << std::dec << " ("
                     << n->value() << ")" << std::endl;
    if(!n->less.empty())
...
template <class ITYPE>
int
IBSTree<ITYPE>::CountMarks() const
{
    dyn_rwlock::shared_lock l(rwlock);
