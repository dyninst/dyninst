...
void AddressTranslateWin::setNoProc(bool b) { no_proc = b; }

AddressTranslate *AddressTranslate::createAddressTranslator(
    PID pid_, ProcessReader *, SymbolReaderFactory *, PROC_HANDLE phandle_,
    std::string, Address) {
  AddressTranslateWin *new_translate = new AddressTranslateWin(pid_, phandle_);
  if (!new_translate) return NULL;
  if (new_translate->creation_error) return NULL;
  return new_translate;
}

AddressTranslate *AddressTranslate::createAddressTranslator(
    ProcessReader *, SymbolReaderFactory *, std::string, Address) {
  // return createAddressTranslator(GetCurrentProcess(), NULL);
  return createAddressTranslator(GetCurrentProcessId(), NULL, NULL,
                                 GetCurrentProcess());
}

bool AddressTranslateWin::init() {
  bool result = refresh();
  if (!result) creation_error = true;
  return true;
}
...
bool AddressTranslateWin::refresh() {
  ...
  result = EnumProcessModules(currentProcess, NULL, 0, &total_space);
  if (!result) {
    return false;
  }
  ...
  result =
      EnumProcessModules(currentProcess, loadedModules,
                         sizeof(HMODULE) * num_modules_needed, &total_space);
  if (!result) {
    printSysError(GetLastError());
    return false;
  }
  ...
  // Add modules to libs
  for (HMODULE *i = loadedModules; i < loadedModules + num_modules_needed;
       i++) {
    MODULEINFO info;
    ...
    LoadedLib *ll =
        new LoadedLib(std::string(filename), (Address)info.lpBaseOfDll);
...
AddressTranslateWin::AddressTranslateWin(PID pid, PROC_HANDLE phandle_)
    : AddressTranslate(pid, phandle_), no_proc(false) {
  init();
}
