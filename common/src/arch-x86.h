...
#define IS_NOP                                                                           \
    (1 << 9) /* Nop, Lea--lea is only sometime a return, be sure to double check */
...
// and am_stackX for stack operands [this kinda' messy since there are actually two
// operands: the stack byte/word/dword and the (E)SP register itself - but is better than
// naught] added: am_reg, am_stack, am_allgprs ADDED: am_ImplImm for implicit immediates
// ADDED: am_RM, am_UM,
enum
{
    am_A = 1,
...
enum
{
    sNONE = 0,  // the instruction does something that cannot be classified as read/write
                // (by me)
...
class ia32_prefixes
{
    friend COMMON_EXPORT bool ia32_decode_prefixes(const unsigned char* addr,
                                                   ia32_instruction& insn, bool mode_64);
...
inline int
apply_rex_bit(int reg, bool rex_bit)
{
    if(rex_bit)
        return reg + 8;
    else
...
struct ia32_operand
{                         // operand as given in Intel book tables
...
struct ia32_entry
{
    ...
    // the lower half gives operand semantics, e.g. s1RW2R, the upper half is a fXXX hack
    // if needed before hating me for this: it takes a LOT less time to add ONE field to
    // ~2000 table lines! The upper 3 bits of this field (bits 29, 30, 31) are specifiers
...
class ia32_instruction
{
    friend unsigned int       ia32_decode_operands(const ia32_prefixes& pref,
                                                   const ia32_entry& gotit, const char* addr,
                                                   ia32_instruction& instruct);
    friend COMMON_EXPORT bool ia32_decode_prefixes(const unsigned char* addr,
                                                   ia32_instruction& insn, bool mode_64);
    friend COMMON_EXPORT ia32_instruction& ia32_decode(unsigned int         capa,
    ...
    friend COMMON_EXPORT int               ia32_decode_opcode(unsigned int         capa,
                                                              const unsigned char* addr,
                                                              ia32_instruction&    instruct,
                                                              ia32_entry** gotit_ret, bool mode_64);
    friend unsigned int                    ia32_decode_operands(const ia32_prefixes& pref,
                                                                const ia32_entry&    gotit,
                                                                const unsigned char* addr,
                                                                ia32_instruction& instruct, ia32_memacc* mac,
                                                                bool mode_64);
...
/* TODO: documentation*/
COMMON_EXPORT bool
ia32_decode_prefixes(const unsigned char* addr, ia32_instruction& insn, bool mode_64);
...
COMMON_EXPORT ia32_instruction&
              ia32_decode(unsigned int capabilities, const unsigned char* addr, ia32_instruction&,
                          bool mode_64);
...
get_instruction(const unsigned char* instr, unsigned& instType,
                const unsigned char** op_ptr, bool mode_64);

/* get the target of a jump or call */
COMMON_EXPORT Address
...
public:
    ...
    bool isXORRegMemRegMem() const
    {
        ...
        return *p == XOR_RM16_R16 ||
               ...
               // XOR_RM16_R16 and XOR_RM32_R32 are macros with the same value so disable
               // this clause as it is unnecessary and produces a compiler warning
               *p == XOR_RM32_R32 ||
#endif
...
int
set_disp(bool setDisp, instruction* insn, int newOffset, bool outOfFunc);
int
...
inline bool
is_disp32(Address a1, Address a2)
{
    return is_disp32(a2 - (a1 + JUMP_REL32_SZ));
}
inline bool
is_addr32(Address addr)
{
...
COMMON_EXPORT const unsigned char*
skip_headers(const unsigned char*, ia32_instruction* = NULL);
...
COMMON_EXPORT bool
insn_hasSIB(unsigned, unsigned&, unsigned&, unsigned&);
COMMON_EXPORT bool
insn_hasDisp8(unsigned ModRM);
COMMON_EXPORT bool
insn_hasDisp32(unsigned ModRM);
