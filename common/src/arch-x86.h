...
#define IS_NOP                                                              \
  (                                                                         \
      1 << 9) /* Nop, Lea--lea is only sometime a return, be sure to double \
                 check */
...
#define VEXGET_VVVV(b) \
  (unsigned char)((~((unsigned char)((b & VEX_VVVV) >> 3))) & 0xF)
...
#define EVEXGET_VVVV(a, b) \
  ((((unsigned char)~(a) >> 3) & 0x0F)) | (((unsigned char)~(b)&0x08) << 1)
...
// and am_stackX for stack operands [this kinda' messy since there are actually
// two operands:
// the stack byte/word/dword and the (E)SP register itself - but is better than
// naught]
...
enum {
  ...
  am_YV,
  am_YW
};  // pusH and poP produce different addresses

// operand types - idem, but I invented quite a few to make implicit operands
// explicit.
// ADDED: op_y
enum {
  op_a = 1,
...
enum { mEAX = 0, mECX, mEDX, mEBX, mESP, mEBP, mESI, mEDI };

enum { mAX = 0, mCX, mDX, mBX, mSP, mBP, mSI, mDI };

// operand semantic - these make explicit all the implicit stuff in the Intel
// tables
// they are needed for memory access, but may be useful for other things:
// dataflow etc.
// Instructions that do not deal with memory are not tested, so caveat emptor...
// Also note that the stack is never specified as an operand in Intel tables, so
// it
// has to be dealt with here.

enum {
  sNONE = 0,  // the instruction does something that cannot be classified as
              // read/write (by me)
...
class ia32_prefixes {
  friend COMMON_EXPORT bool ia32_decode_prefixes(const unsigned char* addr,
...
// VG(6/20/02): To support Paradyn without forcing it to include
// BPatch_memoryAccess, we
//             define this IA-32 "specific" class to encapsulate the same info -
//             yuck
...
enum sizehacks { shREP = 1, shREPECMPS, shREPESCAS, shREPNECMPS, shREPNESCAS };
...
struct ia32_entry {
  ...
  entryID id;
  unsigned int
      otable;  // which opcode table is next; if t_done it is the current one
  unsigned char
      tabidx;     // at what index to look, 0 if it easy to deduce from opcode
  bool hasModRM;  // true if the instruction has a MOD/RM byte
  ia32_operand operands[3];  // operand descriptors
  unsigned int
      legacyType;  // legacy type of the instruction (e.g. (IS_CALL | REL_W))
  // code to decode memory access - this field should be seen as two 16 bit
  // fields
  // the lower half gives operand semantics, e.g. s1RW2R, the upper half is a
  // fXXX hack if needed
  // before hating me for this: it takes a LOT less time to add ONE field to
  // ~2000 table lines!
  unsigned int opsema;
};
...
using std::vector;
struct flagInfo {
  flagInfo(const vector<Dyninst::MachRegister>& rf,
           const vector<Dyninst::MachRegister>& wf)
      : readFlags(rf), writtenFlags(wf) {}
  flagInfo() {}
...
class ia32_instruction {
  ...
  friend COMMON_EXPORT bool ia32_decode_prefixes(const unsigned char* addr,
                                                 ia32_instruction& insn);
  friend COMMON_EXPORT ia32_instruction& ia32_decode(
      unsigned int capa, const unsigned char* addr, ia32_instruction& instruct);
...
#define IA32_FULL_DECODER                                                \
  (IA32_DECODE_PREFIXES | IA32_DECODE_MNEMONICS | IA32_DECODE_OPERANDS | \
   IA32_DECODE_JMPS | IA32_DECODE_MEMACCESS | IA32_DECODE_CONDITION)
#define IA32_SIZE_DECODER 0

/* TODO: documentation*/
COMMON_EXPORT bool ia32_decode_prefixes(const unsigned char* addr,
                                        ia32_instruction& insn);
...
COMMON_EXPORT Address get_target(const unsigned char* instr, unsigned type,
...
class instruction {
 public:
  ...
  instruction(const void* ptr) : type_(0), size_(0), ptr_(NULL), op_ptr_(0) {
  ...
  COMMON_EXPORT static unsigned jumpSize(Address from, Address to,
                                         unsigned addr_width);
  ...
  bool isUncondJump() const { return ((type_ & IS_JUMP) && !(type_ & IS_JCC)); }
  bool isNop() const;
  ...
  bool isXORRegMemRegMem() const {
    const unsigned char* p = op_ptr_ ? op_ptr_ : ptr_;
    return *p == XOR_RM16_R16 || *p == XOR_RM32_R32 || *p == XOR_R8_RM8 ||
           *p == XOR_R16_RM16 || *p == XOR_R32_RM32;
  }
  ...
  void print() {
    for (unsigned i = 0; i < size_; i++) fprintf(stderr, " %02x", *(ptr_ + i));
    fprintf(stderr, "\n");
  }
...
inline bool is_disp8(long disp) { return (disp >= -128 && disp < 127); }

inline bool is_disp16(long disp) { return (disp >= -32768 && disp < 32767); }
...
inline bool is_addr32(Address addr) { return (addr < UI32_MAX); }
