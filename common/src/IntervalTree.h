...
class IntervalTree {
 ...
 public:
  ...
  void insert(K lb, K ub, V v) { tree_[lb] = std::make_pair(ub, v); }

  void remove(K lb) { erase(lb); }

  void erase(K lb) { tree_.erase(lb); }

  bool find(K key, V &value) const {
    K lb = 0;
    K ub = 0;
    V val;
    if (!precessor(key, lb, ub, val)) return false;
  ...
  bool find(K key, K &l, K &u, V &value) const {
    if (!precessor(key, l, u, value)) return false;
  ...
  void elements(std::vector<Entry> &buffer) const {
    buffer.clear();
    for (c_iter iter = tree_.begin(); iter != tree_.end(); ++iter) {
      Entry e;
      lb(e) = iter->first;
      ub(e) = iter->second.first;
      value(e) = iter->second.second;
