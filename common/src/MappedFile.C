...
MappedFile::MappedFile(void *loc, unsigned long size_, const std::string &name,
                       bool &ok)
    : fullpath(name),
      map_addr(NULL),
#if defined(os_windows)
...
#if defined(os_windows)
  hFile = CreateFile(fullpath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
...
bool MappedFile::map_file() {
  ...
  if (MAP_FAILED == map_addr) {
    sprintf(ebuf, "mmap(0, %lu, prot=0x%x, flags=0x%x, %d, 0): %s", file_size,
            mmap_prot, mmap_flags, fd, strerror(errno));
    goto err;
  }
...
std::string MappedFile::pathname() { return fullpath; }

std::string MappedFile::filename() { return extract_pathname_tail(fullpath); }

void MappedFile::setSharing(bool s) { can_share = s; }

bool MappedFile::canBeShared() { return can_share; }
