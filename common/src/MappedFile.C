...
MappedFile*
MappedFile::createMappedFile(std::string fullpath_)
{
    ...
    // __LINE__, fullpath_.c_str(), mapped_files.size());
    return mf;
}
...
MappedFile*
MappedFile::createMappedFile(void* loc, unsigned long size_, const std::string& name)
{
    bool        ok = false;
...
void
MappedFile::closeMappedFile(MappedFile*& mf)
{
    if(!mf)
    {
...
bool
MappedFile::check_path(std::string& filename)
{
    struct stat statbuf;
...
bool
MappedFile::open_file(void* loc, unsigned long size_)
{
#if defined(os_windows)
...
bool
MappedFile::open_file()
{
#if defined(os_windows)
    hFile = CreateFile(fullpath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_ALWAYS,
...
MappedFile::map_file()
{
    ...
    {
        sprintf(ebuf, "mmap(0, %lu, prot=0x%x, flags=0x%x, %d, 0): %s", file_size,
                (unsigned int) mmap_prot, (unsigned int) mmap_flags, fd, strerror(errno));
        goto err;
    }
