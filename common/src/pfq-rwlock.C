...
void dyn_rwlock::lock_shared() {
    ...
    unsigned int ticket = rin.fetch_add(TICKET, dyncompat::memory_order_acquire);
...
void dyn_rwlock::unlock_shared() {
    ...
    unsigned int ticket = rout.fetch_add(TICKET, dyncompat::memory_order_acq_rel);
...
void dyn_rwlock::lock() {
    ...
    unsigned int lr = rin.fetch_xor(PHASE|WRITER, dyncompat::memory_order_acquire);
    last = (lr - TICKET) ^ (PHASE | WRITER);

    // Let the last reader know that they should wake me up.
    // Rel to "release" the previous write to last.
    unsigned int cr = rout.fetch_xor(PHASE|WRITER, dyncompat::memory_order_acq_rel);
...
void dyn_rwlock::unlock() {
    ...
    rout.fetch_xor(WRITER, dyncompat::memory_order_relaxed);

    // Let in any new readers, since we're done with it at the moment.
    unsigned int phase = rin.fetch_xor(WRITER, dyncompat::memory_order_acq_rel);
    phase &= PHASE;
