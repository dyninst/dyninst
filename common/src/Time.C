...
const timeUnit *timeUnit::nsHelp() { return new timeUnit(fraction(1)); }
const timeUnit *timeUnit::usHelp() { return new timeUnit(fraction(1000)); }
const timeUnit *timeUnit::msHelp() { return new timeUnit(fraction(1000000)); }
...
const timeBase *timeBase::bStdHelp() { return new timeBase(0); }
...
const timeBase *timeBase::b1970Help() {
  ...
  return new timeBase(numStdYears * nsPerYear + numLeapYears * nsPerLeapYear);
}
...
const timeStamp *timeStamp::ts1800Help() {
  ...
  *p_ts1970 -=
      53 * timeLength::year() + 17 * timeLength::leapYear();  // 1900 - 1970
  // beginning 2000 (leap year) - beg 2100 (no leap year), 25 4 year spans
  *p_ts1970 -=
      76 * timeLength::year() + 24 * timeLength::leapYear();  // 1800 - 1900
  return p_ts1970;
}
...
const timeStamp *timeStamp::ts2200Help() {
  timeStamp *p_ts1970 = new timeStamp(0, timeUnit::year(), timeBase::b1970());
  *p_ts1970 +=
      23 * timeLength::year() + 7 * timeLength::leapYear();  // 1970 - 2000
  // beginning 2000 (leap year) - beg 2100 (no leap year), 25 4 year spans
...
timeStamp::timeStamp(double dTime, const timeUnit &u, timeBase b)
    : timeParent() {
...
ostream &operator<<(ostream &s, timeLength z) {
  ...
  if (z.get_ns() == 0) {
    ...
    if (years != 0) {
      s << (prev ? cSt : "") << years << " years";
      prev = true;
    }
    if (days != 0) {
      s << (prev ? cSt : "") << days << " days";
      prev = true;
    }
    if (hours != 0) {
      s << (prev ? cSt : "") << hours << " hours";
      prev = true;
    }
    if (mins != 0) {
      s << (prev ? cSt : "") << mins << " mins";
      prev = true;
    }
    if (secs != 0) {
      s << (prev ? cSt : "") << secs << " secs";
      prev = true;
    }
    if (ms != 0) {
      s << (prev ? cSt : "") << ms << " ms";
...
ostream &operator<<(ostream &s, timeStamp z) {
  time_t s1970 =
      static_cast<time_t>(z.getI(timeUnit::sec(), timeBase::b1970()));
...
bool timeParent::isInitialized() const {
  if (get_ns() == uninitializedValue)
    return false;
  else
    return true;
}

timeParent::timeParent() : ns(uninitializedValue) {}

timeParent::timeParent(int64_t _ns) : ns(_ns) {}
...
COMMON_EXPORT relTimeStamp::relTimeStamp(int64_t iTime, const timeUnit &u)
    : timeParent() {
  initI(iTime, u);
}

COMMON_EXPORT relTimeStamp::relTimeStamp(int iTime, const timeUnit &u)
    : timeParent() {
  initI(iTime, u);
}
...
COMMON_EXPORT timeStamp::timeStamp(int64_t ns_) : timeParent(ns_) {}
...
timeUnit::timeUnit(fraction _ns_per_unit)
    : ns_per_unit(_ns_per_unit), units_per_ns(ns_per_unit.reciprocal()) {
  ns_per_unit.reduce();
  units_per_ns.reduce();
}
...
timeBase::timeBase(int64_t ns2stdMark) { ns2StdBaseMark = ns2stdMark; }

int64_t timeBase::get_ns2StdBaseMark() const { return ns2StdBaseMark; }
...
int64_t timeBase::cvtFrom_bStd(int64_t ns) const { return ns + ns2StdBaseMark; }
...
const timeBase &timeBase::bNone() { return bStd(); }

int64_t timeParent::get_ns() const { return ns; }

void timeParent::assign(const int64_t v) { ns = v; }
...
const timeStamp &timeStamp::tsLongAgoTime() { return timeStamp::ts1800(); }

const timeStamp &timeStamp::tsFarOffTime() { return timeStamp::ts2200(); }
