...
void dedemangle(char *demangled, char *result) {
  ...
  if (demangled[0] == '(' && strstr(demangled, "::") != NULL) {
    int depth = 0, i = 0;
    for (;;) {
      if (demangled[i] == '\0') break;
    ...
    if (demangled[i] != '\0') {
      ...
      if (resultBegins[0] == ':' && resultBegins[1] == ':') resultBegins += 2;
      resultEnds = strrchr(resultBegins, ' ');
      if (resultEnds) *resultEnds = '\0';
    }
    ...
    */
    ...
    for (offset = 0; offset < strlen(resultBegins); offset++) {
      if (resultBegins[offset] == '<') {
        ...
        if (strstr(&(resultBegins[lastColon + 1]), "operator ") ==
            resultBegins + lastColon + 1) {
          resultBegins = demangled;
          offset = 0;
        } else {
...
bool prefixed_by(std::string &haystack, std::string &prefix) {
  if (!haystack.c_str()) return false;

  const char *res = strstr(haystack.c_str(), prefix.c_str());

  if (res == haystack.c_str()) return true;
...
bool suffixed_by(std::string &haystack, std::string &suffix) {
  ...
  for (int i = suffix.length() - 1; i >= 0; i--) {
    if (haystack[lastchar - i] != suffix[j]) return false;
    j++;
  }
