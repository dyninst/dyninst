...
template <class r_debug_X>
bool
r_debug_dyn<r_debug_X>::is_valid()
{
    if(!valid)
    {
...
template <class link_map_X>
bool
link_map_dyn<link_map_X>::is_valid()
{
    return valid;
}
...
template <class link_map_X>
Address
link_map_dyn<link_map_X>::map_address()
{
    return mapaddr;
}
...
template <class link_map_X>
uint64_t
link_map_dyn<link_map_X>::l_addr()
{
...
template <class link_map_X>
char*
link_map_dyn<link_map_X>::l_name()
{
    if(loaded_name)
        return link_name;
    ...
    {
        if(!proc->ReadMem((Address)(link_elm.l_name + i), link_name + i, sizeof(char)))
        {
            valid = false;
            return NULL;
        }
...
template <class link_map_X>
bool
link_map_dyn<link_map_X>::is_last()
{
...
template <class link_map_X>
bool
link_map_dyn<link_map_X>::load_next()
{
    if(is_last())
    {
        return false;
...
template <class link_map_X>
bool
link_map_dyn<link_map_X>::load_link(Address addr)
{
...
bool
ProcessReaderSelf::start()
{
    return true;
}

bool
ProcessReaderSelf::done()
{
    return true;
}

bool
ProcessReaderSelf::ReadMem(Address inTraced, void* inSelf, unsigned amount)
{
...
bool
ProcessReaderSelf::GetReg(MachRegister /*reg*/, MachRegisterVal& /*val*/)
{
    assert(0);
    return false;
}

vector<pair<Address, unsigned long>>*
LoadedLib::getMappedRegions()
{
    ...
    {
        pair<Address, unsigned long> p(load_addr + segs[i].mem_addr, segs[i].mem_size);
...
AddressTranslate*
AddressTranslate::createAddressTranslator(int pid_, ProcessReader* reader_,
                                          SymbolReaderFactory* symfactory_, PROC_HANDLE,
                                          std::string exename, Address interp_base)
{
    translate_printf("Creating AddressTranslateSysV\n");
    AddressTranslate* at =
        new AddressTranslateSysV(pid_, reader_, symfactory_, exename, interp_base);
...
AddressTranslate*
AddressTranslate::createAddressTranslator(ProcessReader*       reader_,
...
bool
AddressTranslateSysV::parseDTDebug()
{
    ...
    for(unsigned i = 0; i < numRegs; ++i)
    {
        ...
        translate_printf("Found segment type %d for 0x%lx - 0x%lx, want %d\n", seg.type,
                         seg.mem_addr, seg.mem_addr + seg.mem_size, PT_DYNAMIC);
...
bool
AddressTranslateSysV::parseInterpreter()
{
    bool result;
...
bool
AddressTranslateSysV::init()
{
    translate_printf("Initing AddressTranslateSysV\n");

    // Try to use DT_DEBUG first, falling back to parsing the interpreter binary if
    // possible
    if(!parseDTDebug())
    {
        if(!parseInterpreter())
...
LoadedLib*
AddressTranslateSysV::getLoadedLibByNameAddr(Address addr, std::string name)
{
...
Address
AddressTranslateSysV::getTrapAddrFromRdebug()
{
    Address retVal = 0;
...
AddressTranslateSysV::refresh()
{
    ...
    if(address_size == sizeof(void*))
    {
        ...
        else if(!r_debug_native->is_valid())
        {
            ...
            {
                libs.push_back(
                    getLoadedLibByNameAddr(interpreter_base, interpreter->getFilename()));
            }
            result = true;
            goto done;
        }
    ...
    do
    {
        ...
        if(obj_name == "linux-vdso.so.1" || obj_name == "linux-vdso64.so.1" ||
...
void
FCNode::getSegments(vector<SymSegment>& segs)
{
    parsefile();
...
static const size_t NUM_DBG_BREAK_NAMES =
    sizeof dbg_break_names / sizeof dbg_break_names[0];
...
FCNode*
FileCache::getNode(const string& filename, SymbolReaderFactory* factory)
{
    struct stat buf;
...
Address
AddressTranslateSysV::getLibraryTrapAddrSysV()
{
    if(real_trap_addr == 0)
        plat_getTrapAddr();
...
bool
AddressTranslateSysV::plat_getTrapAddr()
{
...
Address
AddressTranslateSysV::adjustForAddrSpaceWrap(Address base, std::string name)
{
    ...
    while(true)
    {
        ...
        if(type == PT_PHDR || type == PT_NULL || type == PT_DYNAMIC ||
           type == PT_INTERP || type == PT_NOTE || type == PT_SHLIB || type == PT_TLS)
            continue;
    ...
    else
    {
        ...
        translate_printf(
            "\t Address space wrapping detected, returning modified base of 0x%lx\n",
