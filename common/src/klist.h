...
class KList {
 public:
  ...
  void mapUntil(const Type &item,
                int (*map_func)(const Type &i1, const Type &i2)) const;
...
KList<Type> &KList<Type>::operator=(const KList<Type> &from) {
  ...
  for (temp = from.head; temp; temp = temp->next) prepend(temp->data);
...
template <class Type>
KList<Type> KList<Type>::pure_map(Type (*map_f)(const Type &it)) const {
  KListItem<Type> *temp;
  KList<Type> Ret;
...
void KList<Type>::mapUntil(const Type &item,
  ...
  if (!map_func) return;
  for (temp = head; temp; temp = temp->next)
    if (map_func(temp->data, item)) return;
}
