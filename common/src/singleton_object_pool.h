...
class singleton_object_pool
    : private boost::singleton_pool<T, sizeof(T), Alloc> {
  typedef boost::singleton_pool<T, sizeof(T), Alloc> parent_t;

  inline static void free(T* free_me) { parent_t::free(free_me); }

  inline static T* malloc() { return reinterpret_cast<T*>(parent_t::malloc()); }

 public:
  inline static bool is_from(T* t) { return parent_t::is_from(t); }
...
template <typename T>
inline boost::shared_ptr<T> make_shared(T* t) {
  return boost::shared_ptr<T>(
      t, PoolDestructor<T>() /*, typename unlocked_fast_alloc<T>::type()*/);
}
