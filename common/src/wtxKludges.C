...
bool wtxReadMem(const void *inTarget, u_int nbytes, void *inSelf) {
  ...
  UINT32 result = wtxMemRead(wtxh, 0x0, (WTX_TGT_ADDR_T)inTarget, inSelf,
                             nbytes, WTX_MEM_CACHE_BYPASS);
...
bool wtxWriteMem(void *inTarget, u_int nbytes, const void *inSelf) {
  ...
  UINT32 result =
      wtxMemWrite(wtxh, 0x0, const_cast<void *>(inSelf),
                  (WTX_TGT_ADDR_T)inTarget, nbytes, WTX_MEM_CACHE_BYPASS);
  if (result == static_cast<UINT32>(WTX_ERROR)) {
...
bool relocationTarget(const Address addr, Address *target) {
  if (wtxReloc.count(addr) && target) {
    wtxFindSymbol(wtxReloc[addr].name().c_str(), (WTX_SYMBOL_TYPE)0x0, 0x0,
                  *target);
    return true;
  }
  return false;
...
void printEventpoints(void) {
  ...
  for (unsigned int i = 0; i < pts->nEvtpt; ++i) {
    fprintf(stderr,
            "[%d] evtpt:{ event:{ type:%d narg:%d } ctxid:0x%lx action:{ "
            "type:0x%x arg:%d } tool:0x%lx ptnum:%d\n",
            i, pts->pEvtptInfo[i].wtxEvtpt.event.eventType,
            ...
            pts->pEvtptInfo[i].toolId, pts->pEvtptInfo[i].evtptNum);
  }
  fprintf(stderr, "------------------------------------\n");
}
...
void printModuleInfo(WTX_MODULE_INFO *info) {
  ...
  for (i = 0; i < info->nSections; ++i) {
    fprintf(stderr,
            "\tSection: 0x%x Name: %s Type: 0x%x Flags: 0x%x BaseAddr: 0x%x "
            "Partition: 0x%x Len: 0x%x Sum: 0x%x\n",
            (unsigned int)info->section[i].id, info->section[i].name,
