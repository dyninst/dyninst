...
unsigned int
NS_power::swapBytesIfNeeded(unsigned int i)
{
    static int one = 0x1;
...
int
instruction::signExtend(unsigned int i, unsigned int pos)
{
    int ret;
    if(((i >> (--pos)) & 0x1) == 0x1)
    {
...
instructUnion&
instruction::swapBytes(instructUnion& i)
{
...
void
instruction::setBranchOffset(Address newOffset)
{
    if(isUncondBranch())
    {
...
bool
instruction::isUncondBranch() const
{
...
unsigned
instruction::maxInterFunctionJumpSize(unsigned addr_width)
{
    // 4 for 32-bit...
    // move <high>, r0
    // move <low>, r0
...
bool
instruction::getUsedRegs(std::vector<int>&)
{
    return false;
}
