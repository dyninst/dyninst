...
void Graph::exitNodes(NodeIterator &begin, NodeIterator &end) {
  // std::cerr << "exitNodes called: " << exitNodes_.size() << " nodes" <<
  // std::endl;
...
Graph::Ptr Graph::createGraph() { return Graph::Ptr(new Graph()); }
...
void Graph::markAsEntryNode(NodePtr entry) { entryNodes_.insert(entry); }

void Graph::markAsExitNode(NodePtr exit) { exitNodes_.insert(exit); }
...
void Graph::addNode(Node::Ptr node) {
  ...
  // std::cerr << "\t\t After addNode: " << nodes_.size() << " nodes" <<
  // std::endl;
  // std::cerr << "\t\t\t adding " << node << std::endl;
}

void Graph::clearEntryNodes() { entryNodes_.clear(); }

void Graph::clearExitNodes() { exitNodes_.clear(); }
// Fancy-shmancy predicate based search methods...

bool Graph::find(NodePredicate::Ptr pred, NodeIterator &begin,
                 NodeIterator &end) {
  // We could try to be lazy here, but you have a hard time determining if
  // you're
  // done a priori. So instead we pre-look-up and hand that set into a
  // copy-based
  // set iterator.
...
bool Graph::find(NodePredicateFunc pred, void *user_arg, NodeIterator &begin,
                 NodeIterator &end) {
  // We could try to be lazy here, but you have a hard time determining if
  // you're
  // done a priori. So instead we pre-look-up and hand that set into a
  ...
  allNodes(allBegin, allEnd);
  begin = NodeIterator(
      new NodeIteratorPredicateFunc(pred, user_arg, allBegin, allEnd));
  end = NodeIterator(
      new NodeIteratorPredicateFunc(pred, user_arg, allEnd, allEnd));
  return (begin != end);
}
