...
void
Graph::entryNodes(NodeIterator& begin, NodeIterator& end)
{
...
void
Graph::exitNodes(NodeIterator& begin, NodeIterator& end)
{
...
void
Graph::allNodes(NodeIterator& begin, NodeIterator& end)
{
...
bool
Graph::find(Address addr, NodeIterator& begin, NodeIterator& end)
{
    NodeMap::iterator iter = nodesByAddr_.find(addr);
    if(iter == nodesByAddr_.end())
        return false;
...
void
Graph::insertPair(NodePtr source, NodePtr target, EdgePtr e)
{
...
void
Graph::insertEntryNode(NodePtr entry)
{
    addNode(entry);
...
void
Graph::insertExitNode(NodePtr exit)
{
    addNode(exit);
...
void
Graph::markAsEntryNode(NodePtr entry)
{
    entryNodes_.insert(entry);
}

void
Graph::markAsExitNode(NodePtr exit)
{
    exitNodes_.insert(exit);
}

void
Graph::addNode(Node::Ptr node)
{
...
void
Graph::clearEntryNodes()
{
    entryNodes_.clear();
}

void
Graph::clearExitNodes()
{
    exitNodes_.clear();
}
...
bool
Graph::find(NodePredicate::Ptr pred, NodeIterator& begin, NodeIterator& end)
{
...
bool
Graph::find(NodePredicateFunc pred, void* user_arg, NodeIterator& begin,
            NodeIterator& end)
{
    // We could try to be lazy here, but you have a hard time determining if you're
    // done a priori. So instead we pre-look-up and hand that set into a copy-based
...
bool
Graph::isEntryNode(NodePtr node)
{
    return (entryNodes_.find(node) != entryNodes_.end());
}

bool
Graph::isExitNode(NodePtr node)
{
