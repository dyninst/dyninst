...
DO_INLINE_F typename ListBase<DataType, KeyType>::node *
ListBase<DataType, KeyType>::getLastNode() {
  node *lag = NULL;
  node *curNode = head;
  while (1) {
...
template <class DataType, class KeyType>
bool ListBase<DataType, KeyType>::__find_with_val(
    const DataType &dataVal) const {
  node *curr;
