...
// COMMON_EXPORT dyn_hash_map<Address, AnnotatableBase *>
// SerDesBin::annotatable_id_map;

COMMON_EXPORT dyn_hash_map<std::string, SerializerBase::subsystem_serializers_t>
    SerializerBase::all_serializers;

dyn_hash_map<void *, AnnotationContainerBase *>
    AnnotationContainerBase::containers_by_id;
...
bool &serializer_debug_flag() {
  ...
  //  across library boundaries on windows...   there's probably a better way to
  //  do this...
...
std::vector<std::pair<std::string, dyn_hash_map<std::string, short> *> >
    SerContextBase::ser_control_map;
dyn_hash_map<std::string, short> *SerContextBase::getMapForType(
    std::string tname) {
...
void SerContextBase::enableSerialize(std::string tname, std::string filename,
                                     bool val) {
  dyn_hash_map<std::string, short> *map_p = getMapForType(tname);
  if (!map_p) {
...
void SerContextBase::enableDeserialize(std::string tname, std::string filename,
                                       bool val) {
  dyn_hash_map<std::string, short> *map_p = getMapForType(tname);
  if (!map_p) {
    //  This case should not be able to happen, but, anyways....
...
void SerContextBase::enforceDeserialize(std::string tname, std::string filename,
                                        bool val) {
  dyn_hash_map<std::string, short> *map_p = getMapForType(tname);
  if (!map_p) {
    //  This case should not be able to happen, but, anyways....
...
void SerContextBase::enforceDeserialize(bool val) {
  SerContextBase::enforceDeserialize(std::string(getRootTypename()), fname,
                                     val);
}

void SerContextBase::enableSerDes(std::string tname, std::string filename,
                                  bool val) {
  dyn_hash_map<std::string, short> *map_p = getMapForType(tname);
  if (!map_p) {
    //  This case should not be able to happen, but, anyways....
...
bool SerContextBase::serializeEnabled() {
  dyn_hash_map<std::string, short> *map_p =
      getMapForType(std::string(getRootTypename()));
  if (!map_p) {
    //  This case should not be able to happen, but, anyways....
    return false;
  }
...
bool SerContextBase::deserializeEnforced(std::string tname,
                                         std::string filename) {
  dyn_hash_map<std::string, short> *map_p = getMapForType(tname);
  if (!map_p) {
    //  This case should not be able to happen, but, anyways....
...
bool SerContextBase::deserializeEnforced() {
  return SerContextBase::deserializeEnforced(std::string(getRootTypename()),
                                             fname);
}

std::string SerContextBase::getSerFileName() { return serfilename; }

SerializerBase *createSerializer(SerContextBase *scs, std::string ser_name,
                                 std::string file, ser_type_t ser_type,
...

{
  ...
  try {
    if (!scs) {
      fprintf(stderr,
              "%s[%d]:  ERROR context object not given for serialiezr\n",
              FILE__, __LINE__);
      return NULL;
    }
    ...
    if (ser_type == ser_bin) {
      serialize_printf("%s[%d]:  creating bin serializer %s/%s, %p\n", FILE__,
                       __LINE__, ser_name.c_str(), file.c_str(), scs);
...
SerDes *SerFile::getSD() { return sd; }
...
SerFile::SerFile(std::string fname, iomode_t mode, bool verbose)
  ...
  if (strstr(filename.c_str(), "xml")) {
    ...
    serialize_printf("%s[%d]:  opening %s file for %s\n", FILE__, __LINE__,
                     filename.c_str(),
                     mode == sd_serialize ? "output" : "input");
...
iomode_t SerFile::iomode() { return iomode_; }
...
bool Serializable::serialize(std::string file, SerContextBase *scb,
                             ser_type_t mode) {
  std::string sername = file;
  SerializerBase *serializer =
      createSerializer(scb, sername, file, mode, sd_serialize, false);
...
AnnotatableSparse *find_sparse_annotatable(SerializerBase *serializer,
                                           void *id) {
  assert(serializer);
...
bool SerializerBase::isEOF() { return sd->isEOF(); }
...
AnnotatableSparse *SerializerBase::findSparseAnnotatable(void *id) {
  ...
  if (iter == sparse_annotatable_map->end()) {
    serialize_printf(
        "%s[%d]:  ERROR:  cannot find parent to assign annotation to\n",
        __FILE__, __LINE__);
    return NULL;
  }
  return iter->second;
...
bool isOutput(SerializerBase *ser) { return (ser->iomode() == sd_serialize); }

bool isBinary(SerializerBase *ser) { return ser->isBin(); }
...
bool sb_is_output(SerializerBase *sb) { return (sb->iomode() == sd_serialize); }
...
SerializerBase *getExistingOutputSB(unsigned short ndx) {
  if (SerializerBase::active_serializers.size() <= ndx) return NULL;
...
bool SerializerBase::removeSerializer(unsigned short ndx) {
  if (SerializerBase::active_serializers.size() <= ndx) return false;
...
SerializerBin::SerializerBin(SerContextBase *s, std::string name_,
                             std::string filename, iomode_t dir, bool verbose)
    : SerializerBase(s, std::string(name_), std::string(filename), dir,
                     verbose) {
  SerializerBase *sb = this;
  if (sb->serializationDisabled()) {
    fprintf(
        stderr,
        "%s[%d]:  Failing to construct Bin Translator:  global disable set\n",
        FILE__, __LINE__);
...
FILE *SerDesBin::init(std::string filename, iomode_t mode, bool /*verbose*/) {
  ...
  //  NOTE:  fname is path-resolved and turned into "filename" by the SerDes
  //  ctor
  FILE *f = NULL;
  std::string cache_name;
  ...
  if (!SerDesBin::resolveCachePath(filename, cache_name)) {
    ...
    if (mode == sd_deserialize) {
      ...
      snprintf(msg, 1024, "%s[%d]:  no cache file exists for %s/%s\n", FILE__,
               __LINE__, filename.c_str(), cache_name.c_str());
      fprintf(stderr, "%s\n", msg);
      SER_ERR(msg);
    }
  }
  ...
  serialize_printf("%s[%d]:  opening cache file %s for %s\n", FILE__, __LINE__,
                   cache_name.c_str(),
                   (mode == sd_serialize) ? "serialize" : "deserialize");
  ...
  if (!f) {
    ...
    serialize_printf("%s[%d]: fopen(%s, %s): %s\n", FILE__, __LINE__,
                     cache_name.c_str(), (mode == sd_serialize) ? "w+" : "r",
                     strerror(errno));
  ...
  try {
    ...
    fclose(f);
    serialize_printf(
        "%s[%d]:  %sserialize failed init.\n\t%s[%d]: %s\n\trethrowing.\n",
        FILE__, __LINE__, mode == sd_serialize ? "" : "de", err.file().c_str(),
        err.line(), err.what());
    throw(err);
  }
...
bool SerDesBin::isEOF() { return (0 != feof(f)); }
...
//  windows equiv to S_ISDIR??
  ...
  std::string cpath =
      dot_dyninst_dir + std::string("/") + std::string(DEFAULT_CACHE_DIR);
...
bool SerDesBin::resolveCachePath(std::string full_file_path,
                                 std::string &cache_name) {
  std::string path;
  ...
  cache_name = path + std::string("/") + std::string(CACHE_PREFIX) +
               short_name + std::string("_") + std::string(sizestr);
...
void SerDesBin::readHeaderAndVerify(std::string full_file_path,
                                    std::string cache_name, FILE *fptr) {
  struct stat statbuf;
  ...
  if (fptr)
    ...
    if (!f) {
      ...
      snprintf(msg, 1024, "%s[%d]:  failed to open file %s: %s\n", FILE__,
               __LINE__, full_file_path.c_str(), strerror(errno));
      fprintf(stderr, "%s\n", msg);
      SER_ERR(msg);
    }
  }
  ...
  if (1 != rc) {
    char msg[1024];
    snprintf(msg, 1024,
             "%s[%d]:  failed to read header struct for %s: %s, rc = %d\n",
  ...
  if (header.cache_magic != (unsigned)CACHE_MAGIC) {
    ...
    snprintf(
        msg, 1024,
        "%s[%d]:  magic number check failure for %s--%s: got %d, expected %d\n",
  ...
  if (header.source_file_size != source_file_size) {
    ...
    snprintf(msg, 1024, "%s[%d]:  size discrepancy found for %s/%s\n", FILE__,
             __LINE__, full_file_path.c_str(), cache_name.c_str());
    SER_ERR(msg);
  }
...
bool SerDesBin::verifyChecksum(std::string &full_file_path,
  ...
  if (strncmp(comp_checksum, new_checksum, SHA1_DIGEST_LEN * 2)) {
    fprintf(stderr,
            "%s[%d]:  sha1_file(%s): checksum mismatch: \n\told:%s\n\tnew:%s\n",
            FILE__, __LINE__, full_file_path.c_str(), comp_checksum,
            new_checksum);
    return false;
  }
...
void SerDesBin::vector_start(unsigned long &size, const char *)
    DECLTHROW(SerializerError) {
  if (iomode_ == sd_deserialize) size = 0UL;
...
void SerDesBin::multimap_start(unsigned long &size, const char *)
    DECLTHROW(SerializerError) {
...
void SerDesBin::hash_map_start(unsigned long &size, const char *)
    DECLTHROW(SerializerError) {
...
void SerDesBin::annotation_start(AnnotationClassID &a_id, void *&parent_id,
                                 sparse_or_dense_anno_t &sod, const char *,
                                 const char *) {
  ...
  if (iomode() == sd_deserialize) sod = (sparse_or_dense_anno_t)lsod;

  serialize_printf("%s[%d]:  leaving %s annotation_start: aid = %d, id = %p\n",
                   FILE__, __LINE__,
                   (iomode_ == sd_serialize) ? "serialize" : "deserialize",
                   a_id, parent_id);
}
...
void SerDesBin::magic_check(const char *file__, unsigned int line__) {
  ...
  if (iomode_ == sd_deserialize) {
    if (magic != 33) {
      ...
      if (isEOF()) fprintf(stderr, "%s[%d]:  GOT EOF\n", file__, line__);
      abort();
    }
  }
}

void SerDesBin::annotation_list_start(Address &id, unsigned long &nelem,
                                      const char *) {
  if (iomode_ == sd_deserialize) {
    nelem = 0UL;
  }
  ...
  if (nelem) {
    serialize_printf(
        "%s[%d]: enter annotation_list_start id = %p, nelem = %ld\n", FILE__,
        __LINE__, (void *)id, nelem);
  }
  ...
  if (nelem)
    serialize_printf(
        "%s[%d]: leave annotation_list_start id = %p, nelem = %ld\n", FILE__,
        __LINE__, (void *)id, nelem);
}
...
void SerDesBin::translate(bool &param, const char *tag) {
  ...
  if (iomode_ == sd_serialize) {
    ...
    if (1 != rc) SER_ERR("fread");
  }

  if (noisy)
    serialize_printf("%s[%d]:  %sserialize %s=%s\n", FILE__, __LINE__,
                     iomode_ == sd_serialize ? "" : "de", tag ? tag : "no-tag",
                     param ? "true" : "false");
}
...
void SerDesBin::translate(int &param, const char *tag) {
  ...
  if (iomode_ == sd_serialize) {
    ...
    if (1 != rc) {
      // fprintf(stderr, "%s[%d]:  failed to deserialize int-'%s', rc = %d:%s,
      // noisy = %d\n",
      //		  FILE__, __LINE__, tag ? tag : "no_tag", rc,
      //strerror(errno), noisy);
...
void SerDesBin::translate(const char *&param, int bufsize, const char *tag) {
  ...
  if (iomode_ == sd_serialize) {
    ...
    if (len > bufsize) {
      fprintf(stderr,
              "%s[%d]:  insufficient buffer: %d, need %d...  truncation....\n",
              FILE__, __LINE__, bufsize, len);
      len = bufsize;

      // char msg[1024];
      // snprintf(msg, 1024, "not enough space in string buffer, %d needed",
      // len);
      // SER_ERR("msg");
    }
    ...
    if (len != 0) {
      ...
      // __LINE__, tag ? tag : "no_tag_provided");
    }
...
void SerDesBin::translate(std::string &param, const char *tag) {
  ...
  if ((iomode_ == sd_deserialize) || strstr(param.c_str(), "file")) {
    serialize_printf("%s[%d]:  %sserializing string %s--%s, len = %lu\n",
                     FILE__, __LINE__, (iomode_ == sd_serialize) ? "" : "de",
                     tag ? tag : "unnamed", param.c_str(),
                     (unsigned long)param.length());
  }
}

void SerDesBin::translate(std::vector<std::string> &param, const char *tag,
                          const char *elem_tag) {
...
SerializerBase *Serializable::lookupExistingSerializer() {
  // fprintf(stderr, "%s[%d]:  lookupExistingSerializer: index = %d,
  // serializers.size() = %lu\n", FILE__, __LINE__, active_serializer_index,
  // SerializerBase::active_serializers.size());

  if (active_serializer_index == (unsigned short)-1) return NULL;
...
bool set_sb_annotatable_sparse_map(SerializerBase *sb, AnnotatableSparse *as,
                                   void *id) {
  sb->set_annotatable_sparse_map(as, id);
  return true;
}

bool set_sb_annotatable_dense_map(SerializerBase *sb, AnnotatableDense *ad,
                                  void *id) {
  sb->set_annotatable_dense_map(ad, id);
  return true;
}

void SerializerBase::set_annotatable_sparse_map(AnnotatableSparse *as,
                                                void *id) {
...
SerializerBase::SerializerBase(SerContextBase *scb, std::string name_,
                               std::string filename, iomode_t dir, bool verbose)
    : sparse_annotatable_map(NULL), dense_annotatable_map(NULL) {
  if (!scb) {
  ...
  serialize_printf("%s[%d]:  before new SerFile, scb = %p, name = %s/%s\n",
                   FILE__, __LINE__, scb, serializer_name.c_str(),
                   filename.c_str());
...
unsigned short SerializerBase::getIndex() { return ser_index; }

void SerializerBase::globalDisable() { global_disable = true; }
bool SerializerBase::serializationDisabled() { return global_disable; }

void SerializerBase::globalEnable() { global_disable = false; }
...
SerializerBase *SerializerBase::getSerializer(std::string subsystem,
                                              std::string fname) {
...
bool SerializerBase::addSerializer(std::string subsystem, std::string fname,
                                   SerializerBase *sb) {
...
void SerializerBase::serialize_annotations(void *id,
  ...
  if (isOutput()) nelem = sers.size();

  getSD().annotation_list_start(id_add, nelem);

  if (nelem)
    serialize_printf("%s[%d]:  need to %s %lu annos\n", FILE__, __LINE__,
                     isInput() ? "deserialize" : "serialize", nelem);

  if (sers.size())
    serialize_printf(
        "%s[%d]: serialize_annotations:  %s, id = %p, nelem = %lu\n", FILE__,
        __LINE__, isInput() ? "deserialize" : "serialize", (void *)id_add,
        nelem);
  ...
  for (unsigned long i = 0; i < nelem; ++i) {
    ...
    getSD().annotation_start(a_id, lparent_id, lsod,
                             acb ? acb->getName().c_str() : NULL);
    ...
    if (isInput()) {
      ...
      if (!acb) {
        ...
        fprintf(stderr,
                "%s[%d]:  FIXME:  cannot find annotation class for id %d\n",
                FILE__, __LINE__, a_id);
        return;
      } else {
...
void annotation_start(SerializerBase *sb, AnnotationClassID &a_id,
                      void *&lparent_id, sparse_or_dense_anno_t &lsod,
                      const char *) {
...
void annotation_end(SerializerBase *sb) { sb->annotation_end(); }
...
void vector_end(SerializerBase *sb) { sb->vector_end(); }

AnnotationContainerBase *get_container(void *parent_id) {
  AnnotationContainerBase *cont =
      AnnotationContainerBase::getContainer(parent_id);
  if (!cont) {
...
bool deserialize_container_item(AnnotationContainerBase *acb,
                                SerializerBase *sb) {
...
void SerializerBase::annotation_start(AnnotationClassID &a_id,
  ...
  serialize_printf(
      "%s[%d]:  leaving to annotation_start:  id = %d, ser_id = %d\n", FILE__,
      __LINE__, a_id, ser_index);
}

void SerializerBase::annotation_end() { getSD().annotation_end(); }
...
bool SerializerBase::serialize_post_annotation(void *parent_id, void *anno,
                                               ...
                                               const char *tag) {
  serialize_printf(
      "%s[%d]: serialize_post_annotation:  ser_id = %d, parent_id = %p\n",
  ...
  if (isInput()) {
    ...
    //  now lookup the object to which it belonged in the map of annotatable
    //  objects
...
void SerializerBase::vector_end() { getSD().vector_end(); }

void SerializerBase::hash_map_start(unsigned long &size, const char *tag) {
  getSD().hash_map_start(size, tag);
}

void SerializerBase::hash_map_end() { getSD().hash_map_end(); }

void SerializerBase::multimap_start(unsigned long &size, const char *tag) {
  getSD().multimap_start(size, tag);
}

void SerializerBase::multimap_end() { getSD().multimap_end(); }

void SerializerBase::pair_start(const char *tag) { getSD().pair_start(tag); }
...
void SerializerBase::pair_end() { getSD().pair_end(); }
...
void SerializerBase::translate_base(const char *&v, int bufsize,
                                    const char *&t) {
