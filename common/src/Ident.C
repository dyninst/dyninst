...
Ident::Ident(const char *Vstr, const char *expected_suite) {
...
#endif
  int n = sscanf(Vstr, IdentFormat, suite_, fullrelease, component_, revision_,
                 date_, time_, builder_);
  ...
  if (n <= 0) {
    ...
    cerr << "Warning! Parsed only " << n << " of " << IdentFields
         << " fields from $" << suite_ << " identification string: <" << Vstr
         << ">:" << endl;
    ...
    if (strncmp(expected_suite, suite_, es_len) == 0) {
      ...
      if (char *p = strrchr(suite_, ':')) {
        *p = '\0';
      }
      cerr << "Warning! Identifier contained unexpected suite name: " << suite_
           << " <=> " << expected_suite << endl;
    }
...
ostream &operator<<(ostream &os, const Ident &Id) {
  ...
  pdstring buf = "{" + " suite=" + Id.suite() + " release=" + Id.release() +
                 " buildnum=" + Id.buildnum() + " component=" + Id.component() +
                 " revision=" + Id.revision() + " date=" + Id.date() +
                 " time=" + Id.time() + " builder=" + Id.builder() + " }";
#else
  // concise inline version intended for normal use
  char buf[128];
  sprintf(buf, IdentOutFmt, Id.suite_, Id.release_, Id.buildnum_, Id.component_,
          Id.revision_, Id.date_, Id.time_, Id.builder_);
#endif
