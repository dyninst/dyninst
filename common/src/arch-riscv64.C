...
constexpr int32_t BRANCH_COND_EQ  = BEQFunct3;
constexpr int32_t BRANCH_COND_GE  = BGEFunct3;
constexpr int32_t BRANCH_COND_GEU = BGEUFunct3;
constexpr int32_t BRANCH_COND_LT  = BLTFunct3;
constexpr int32_t BRANCH_COND_LTU = BLTUFunct3;
constexpr int32_t BRANCH_COND_NE  = BNEFunct3;
...
bool instruction::getUsedRegs(std::vector<int> &regs) {
    if (isCondBranch()) {
        if (isCompressed()) {
            unsigned int rs2 = CBRANCH_REG1_ADD + ((insn_.raw & CBRANCH_REG1_MASK) >> CBRANCH_REG1_SHIFT);
            regs.push_back(rs2);
        } else {
    ...
    if (isUncondBranch()) {
        ...
        if (isCompressed()) {
            for (unsigned i = 0; i < CBRANCH_REORDER1.size(); i++) {
                offset |= ((insn_.raw >> (CBRANCH_IMM_OFF1 + i)) & 1) << CBRANCH_REORDER1[i];
            }
            for (unsigned i = 0; i < CBRANCH_REORDER2.size(); i++) {
                offset |= ((insn_.raw >> (CBRANCH_IMM_OFF2 + i)) & 1) << CBRANCH_REORDER2[i];
            }
        }
        // beq, bne, blt, bge, bltu, bgeu
        else {
            for (unsigned i = 0; i < BRANCH_REORDER1.size(); i++) {
                offset |= ((insn_.raw >> (BRANCH_IMM_OFF1 + i)) & 1) << BRANCH_REORDER1[i]
            }
            for (unsigned i = 0; i < BRANCH_REORDER2.size(); i++) {
                offset |= ((insn_.raw >> (BRANCH_IMM_OFF2 + i)) & 1) << BRANCH_REORDER2[i];
            }
        }
    }
    return offset;
...
unsigned instruction::getCondBranchReg1() const {
    assert (isCondBranch());
    if (isCompressed()) {
        return CBRANCH_REG1_ADD + ((insn_.raw & CBRANCH_REG1_MASK) >> CBRANCH_REG1_SHIFT);
    } else {
