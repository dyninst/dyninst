...
unsigned stringhash(const std::string &s) {
  ...
  if (!str) return 1;  // 0 is reserved for unhashed key
...
bool pattern_match(const char *p, const char *s, bool checkCase) {
  ...
  while (true) {
    ...
    if (*p == MULTIPLE_WILDCARD_CHAR) {
      ...
      for (; *s != '\0'; ++s)
        if (pattern_match(p, s, checkCase)) return true;
      // Failed
      return false;
    }

    // If at the end of the string (and at this point, not of the pattern), it
    // fails
    if (*s == '\0') return false;
...
bool wildcardEquiv(const std::string &us, const std::string &them,
                   bool checkCase) {
  if (us == them)
    return true;
  else
...
// SymElf code is exclusively linked in each component, but we still want to
// share
