...
COMMON_EXPORT unsigned
addrHash(const Address& iaddr)
{
    return Dyninst::addrHashCommon(iaddr);
}

COMMON_EXPORT unsigned
ptrHash(const void* iaddr)
{
...
COMMON_EXPORT unsigned
addrHash16(const Address& iaddr)
{
...
bool
wildcardEquiv(const std::string& us, const std::string& them, bool checkCase)
{
    if(us == them)
        return true;
    else
...
COMMON_EXPORT map<string, SymElf*>*
              getSymelfCache()
{
