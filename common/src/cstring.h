...
class Cstring {
 public:
  ...
  int invariant() { return (((len >= 0) && str) || (!len && !str)); }
...
void Cstring::set(const Cstring &S) { set(S.str); }
...
int Cstring::operator==(const char *s) const {
  if ((len == strlen(s)) && ((!len) || (!strcmp(str, s))))
    return CS_TRUE;
  else
    return 0;
}
