...
void Node::addInEdge(const EdgePtr in) { ins_.insert(in); }

void Node::addOutEdge(const EdgePtr out) { outs_.insert(out); }
...
void Node::ins(NodeIterator &begin, NodeIterator &end) {
  ...
  begin =
      NodeIterator(new NodeFromEdgeSet(ins_.begin(), NodeFromEdgeSet::source));
  end = NodeIterator(new NodeFromEdgeSet(ins_.end(), NodeFromEdgeSet::source));
}

void Node::outs(NodeIterator &begin, NodeIterator &end) {
  begin =
      NodeIterator(new NodeFromEdgeSet(outs_.begin(), NodeFromEdgeSet::target));
...
bool Node::hasInEdges() { return !ins_.empty(); }

bool Node::hasOutEdges() { return !outs_.empty(); }
...
void Node::forwardClosure(NodeIterator &begin, NodeIterator &end) {
  ...
  if (!hasOutEdges()) {
    ...
    begin = NodeIterator(new NodeSearchIterator(outBegin, outEnd,
                                                NodeSearchIterator::out,
                                                NodeSearchIterator::breadth));
  }
}
...
void Node::backwardClosure(NodeIterator &begin, NodeIterator &end) {
  ...
  if (!hasInEdges()) {
    ...
    ins(inBegin, inEnd);
    begin = NodeIterator(new NodeSearchIterator(
        inBegin, inEnd, NodeSearchIterator::in, NodeSearchIterator::breadth));
  }
}
...
Node::Ptr PhysicalNode::copy() { return Node::Ptr(new PhysicalNode(addr())); }
...
Node::Ptr VirtualNode::createNode() { return Node::Ptr(new VirtualNode()); }

Node::Ptr VirtualNode::createNode(std::string name) {
  return Node::Ptr(new VirtualNode(name));
}

Node::Ptr VirtualNode::copy() { return Node::Ptr(new VirtualNode(name_)); }

std::string VirtualNode::format() const { return name_; }
...
NodeIterator::NodeIterator(NodeIteratorImpl *iter) : iter_(iter) {}

NodeIterator::NodeIterator(const NodeIterator &rhs) : iter_(rhs.copy()) {}
...
void Node::deleteInEdge(EdgeIterator e) { ins_.erase(*e); }

void Node::deleteOutEdge(EdgeIterator e) { outs_.erase(*e); }
