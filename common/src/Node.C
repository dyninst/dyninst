...
void
Node::addInEdge(const EdgePtr in)
{
    ins_.insert(in);
}

void
Node::addOutEdge(const EdgePtr out)
{
    outs_.insert(out);
}

void
Node::ins(NodeIterator& begin, NodeIterator& end)
{
...
void
Node::outs(NodeIterator& begin, NodeIterator& end)
{
...
void
Node::ins(EdgeIterator& begin, EdgeIterator& end)
{
...
void
Node::outs(EdgeIterator& begin, EdgeIterator& end)
{
    // Iteration over a pre-existing set is easy...
...
void
Node::forwardClosure(NodeIterator& begin, NodeIterator& end)
{
    ...
    else
    {
        ...
        begin = NodeIterator(new NodeSearchIterator(
            outBegin, outEnd, NodeSearchIterator::out, NodeSearchIterator::breadth));
    }
}

void
Node::backwardClosure(NodeIterator& begin, NodeIterator& end)
{
    end = NodeIterator(new NodeSearchIterator());
...
Node::Ptr
PhysicalNode::createNode(Address addr)
{
...
Node::Ptr
VirtualNode::createNode()
{
    return Node::Ptr(new VirtualNode());
}

Node::Ptr
VirtualNode::createNode(std::string name)
{
...
bool
NodeIterator::operator!=(const NodeIterator& rhs) const
{
    if(!iter_)
    {
...
Node::forwardSubgraph()
{
    ...
    while(!worklist.empty())
    {
        ...
        if(visited.find(src.first) != visited.end())
            continue;
...
Node::backwardSubgraph()
{
    ...
    while(!worklist.empty())
    {
        ...
        if(visited.find(targ.first) != visited.end())
            continue;
