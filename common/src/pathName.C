...
std::string expand_tilde_pathname(const std::string &dir) { return dir; }
...
std::string concat_pathname_components(const std::string &comp1,
  ...
  if (temp[comp1.length() - 1] == '/') needToAddSlash = false;

  // if comp2 begins with a "/" then no need to add slash
  if (comp2.length() && comp2[0] == '/') needToAddSlash = false;
#if 0
...
bool executableFromArgv0AndPathAndCwd(std::string &result,
  ...
  if (i_argv0.length() == 0) return false;
  ...
  // path and argv0 wasn't a full path.  Last resort: try current directory +
  // argv0
