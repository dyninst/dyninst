...
// DLLEXPORT SerDesXML(xmlTextWriterPtr w, iomode_t mode)  : SerDes(mode),
// writer(w) { }
// DLLEXPORT static xmlTextWriterPtr init(std::string fname, iomode_t mode, bool
// verbose);

#if defined(cap_have_libxml)
xmlTextWriterPtr (*my_xmlNewTextWriterFilename)(const char *, int) = NULL;

int (*my_xmlTextWriterStartDocument)(xmlTextWriterPtr, const char *,
                                     const char *, const char *) = NULL;

int (*my_xmlTextWriterStartElement)(xmlTextWriterPtr, const xmlChar *) = NULL;

int (*my_xmlTextWriterWriteFormatElement)(xmlTextWriterPtr, const xmlChar *,
                                          const char *, ...) = NULL;
...
int (*my_xmlTextWriterWriteFormatAttribute)(xmlTextWriterPtr, const xmlChar *,
                                            const char *, ...) = NULL;

int (*my_xmlTextWriterEndElement)(xmlTextWriterPtr) = NULL;

xmlTextWriterPtr SerDesXML::init(std::string fname, iomode_t /*mode*/,
                                 bool /*verbose*/) {
#if defined(_MSC_VER)
  ...
  my_xmlNewTextWriterFilename = (xmlTextWriterPtr(*)(
      const char *, int))GetProcAddress(hXML, "xmlNewTextWriterFilename");

  my_xmlTextWriterStartDocument =
      (int (*)(xmlTextWriterPtr, const char *, const char *,
               const char *))GetProcAddress(hXML, "xmlTextWriterStartDocument");

  my_xmlTextWriterStartElement =
      (int (*)(xmlTextWriterPtr, const xmlChar *))GetProcAddress(
          hXML, "xmlTextWriterStartElement");

  my_xmlTextWriterWriteFormatElement =
      (int (*)(xmlTextWriterPtr, const xmlChar *, const char *,
               ...))GetProcAddress(hXML, "xmlTextWriterWriteFormatElement");

  my_xmlTextWriterEndDocument = (int (*)(xmlTextWriterPtr))GetProcAddress(
      hXML, "xmlTextWriterEndDocument");
  ...
  my_xmlTextWriterWriteFormatAttribute =
      (int (*)(xmlTextWriterPtr, const xmlChar *, const char *,
               ...))GetProcAddress(hXML, "xmlTextWriterWriteFormatAttribute");

  my_xmlTextWriterEndElement = (int (*)(xmlTextWriterPtr))GetProcAddress(
      hXML, "xmlTextWriterEndElement");
  ...
  my_xmlNewTextWriterFilename = (xmlTextWriterPtr(*)(const char *, int))dlsym(
      hXML, "xmlNewTextWriterFilename");
  my_xmlTextWriterStartDocument =
      (int (*)(xmlTextWriterPtr, const char *, const char *,
               const char *))dlsym(hXML, "xmlTextWriterStartDocument");
  my_xmlTextWriterStartElement =
      (int (*)(xmlTextWriterPtr, const xmlChar *))dlsym(
          hXML, "xmlTextWriterStartElement");
  my_xmlTextWriterWriteFormatElement =
      (int (*)(xmlTextWriterPtr, const xmlChar *, const char *, ...))dlsym(
          hXML, "xmlTextWriterWriteFormatElement");
  my_xmlTextWriterEndDocument =
      (int (*)(xmlTextWriterPtr))dlsym(hXML, "xmlTextWriterEndDocument");
  my_xmlFreeTextWriter =
      (void (*)(xmlTextWriterPtr))dlsym(hXML, "xmlFreeTextWriter");
  my_xmlTextWriterWriteFormatAttribute =
      (int (*)(xmlTextWriterPtr, const xmlChar *, const char *, ...))dlsym(
          hXML, "xmlTextWriterWriteFormatAttribute");
  my_xmlTextWriterEndElement =
      (int (*)(xmlTextWriterPtr))dlsym(hXML, "xmlTextWriterEndElement");
...
bool start_xml_elem(void *writer, const char *tag) {
  int rc =
      my_xmlTextWriterStartElement((xmlTextWriterPtr)writer, XMLCHAR_CAST tag);
  return (rc >= 0);
}
...
bool write_xml_elem(void *writer, const char *tag, const char *fmt, ...) {
  ...
  int rc = my_xmlTextWriterWriteFormatElement((xmlTextWriterPtr)writer,
                                              XMLCHAR_CAST tag, fmt, ap);
...
bool write_xml_elem(void * /*writer*/, const char * /*tag*/,
                    const char * /*fmt*/, ...) {
  fprintf(stderr, "%s[%d]:  xml output is disabled\n", FILE__, __LINE__);
  return false;
}
...
bool end_xml_elem(SerDesXML &s) { return end_xml_elem(s.writer); }
}
...
bool SerializerXML::start_xml_element(SerializerBase *sb, const char *tag) {
  ...
  if (!sxml) {
    fprintf(stderr,
            "%s[%d]:  FIXME:  called xml function with non xml serializer\n",
            FILE__, __LINE__);
    return false;
  }
...
void SerDesXML::vector_start(unsigned long & /*size*/, const char *tag)
...
void SerDesXML::multimap_start(unsigned long & /*size*/, const char *tag)
    DECLTHROW(SerializerError) {
  bool rc = ::start_xml_elem(writer, tag);
...
void SerDesXML::hash_map_start(unsigned long & /*size*/, const char *tag)
    DECLTHROW(SerializerError) {
  bool rc = ::start_xml_elem(writer, tag);
...
void SerDesXML::annotation_start(Dyninst::AnnotationClassID &a_id,
                                 void *&parent_id, sparse_or_dense_anno_t &sod,
  ...
  // translate((const char *&)const_cast<const char *>(sodstr), 12,
  // "SparseOrDense");
}
...
void SerDesXML::annotation_list_start(Address & /*id*/,
                                      unsigned long & /*nelem*/,
                                      const char *tag) {
  bool rc = ::start_xml_elem(writer, tag);
...
void SerDesXML::translate(bool &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%s", param ? "true" : "false");
...
void SerDesXML::translate(char &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%c", param);
...
void SerDesXML::translate(int &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%d", param);
...
void SerDesXML::translate(long &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%l", param);
...
void SerDesXML::translate(short &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%h", param);
...
void SerDesXML::translate(float &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%e", param);
...
void SerDesXML::translate(double &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%g", param);
...
void SerDesXML::translate(Address &param, const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%p", param);
...
void SerDesXML::translate(const char *&param, int /*bufsize*/,
                          const char *tag) {
  bool rc = write_xml_elem(writer, tag, "%s", param);
...
void SerDesXML::translate(std::string &param, const char *tag) {
  ...
  bool rc = write_xml_elem(writer, tag, "%s", param.c_str());
...
void SerDesXML::translate(std::vector<std::string> &param, const char *tag,
  ...
  for (unsigned int i = 0; i < param.size(); ++i) translate(param[i], elem_tag);
