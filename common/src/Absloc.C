...
Absloc Absloc::makePC() {
  return Absloc(Register, r_EIP);
}

Absloc Absloc::makeSP() {
  return Absloc(Register, r_ESP);
}

Absloc Absloc::makeFP() {
  return Absloc(Register, r_EBP);
}
...
bool AbsRegion::overlaps(const AbsRegion &rhs) const {
  ...
  for (std::set<Absloc>::const_iterator iter = rhs.abslocs_.begin();
       iter != rhs.abslocs_.end(); ++iter) {
    if (abslocs_.find(*iter) != abslocs_.end()) return true;
  }
  return false;
}
