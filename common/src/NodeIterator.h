...
class NodeIteratorSet : public NodeIteratorImpl {
 public:
  ...
  virtual NodeIteratorImpl *copy() { return new NodeIteratorSet(internal_); }
  ...
  NodeIteratorSet(
      const std::unordered_set<Node::Ptr, Node::NodePtrHasher>::iterator iter)
      : internal_(iter){};
...
class NodeFromEdgeSet : public NodeIteratorImpl {
 public:
  typedef enum { target, source, unset } iterType;
  ...
  virtual bool equals(NodeIteratorImpl *rhs) {
    ...
    return ((internal_ == tmp->internal_) && (type_ == tmp->type_));
  }
...
class NodeSearchIterator : public NodeIteratorImpl {
 ...
 public:
  ...
  typedef enum { depth, breadth } Type;
  ...
  // (without deep inspection) whether it contains an unvisited node. Deep
  // inspection
  ...
  */
  ...
  virtual bool equals(NodeIteratorImpl *rhs) {
    ...
    return ((current == tmp->current) && (type == tmp->type) &&
            (direction == tmp->direction) && (worklist == tmp->worklist) &&
            (visited == tmp->visited));
  }
  ...
  NodeSearchIterator(Node::Ptr cur, Direction d, Type t)
      : current(cur), direction(d), type(t) {
    updateVisited(current);
    NodeIterator begin, end;
    getNext(begin, end);
    updateWorklist(begin, end);
  ...
  NodeSearchIterator(Node::Ptr cur, Direction d, Type t,
                     std::deque<Node::Ptr> wl, std::set<Node::Ptr> v)
      : current(cur), direction(d), type(t), worklist(wl), visited(v){};

  NodeSearchIterator(NodeIterator &rangeBegin, NodeIterator &rangeEnd,
                     Direction d, Type t)
      : direction(d), type(t) {
...
class NodeIteratorPredicateObj : public NodeIteratorImpl {
 public:
  ...
  virtual bool equals(NodeIteratorImpl *rhs) {
    NodeIteratorPredicateObj *tmp =
        dynamic_cast<NodeIteratorPredicateObj *>(rhs);
    if (tmp == NULL) return false;
    return ((pred == tmp->pred) && (cur == tmp->cur) && (next == tmp->next) &&
            (end == tmp->end));
  }
  ...
  NodeIteratorPredicateObj(Graph::NodePredicate::Ptr p, NodeIterator &c,
                           NodeIterator &n, NodeIterator &e)
      : pred(p), cur(c), next(n), end(e){};
  NodeIteratorPredicateObj(Graph::NodePredicate::Ptr p, NodeIterator &b,
                           NodeIterator &e)
...
class NodeIteratorPredicateFunc : public NodeIteratorImpl {
 public:
  ...
  virtual Node::Ptr get() { return *cur; }
  virtual bool equals(NodeIteratorImpl *rhs) {
    NodeIteratorPredicateFunc *tmp =
        dynamic_cast<NodeIteratorPredicateFunc *>(rhs);
    if (tmp == NULL) return false;
    return ((pred == tmp->pred) && (user_arg == tmp->user_arg) &&
            (cur == tmp->cur) && (next == tmp->next) && (end == tmp->end));
  }
  ...
  NodeIteratorPredicateFunc(Graph::NodePredicateFunc p, void *u,
                            NodeIterator &c, NodeIterator &n, NodeIterator &e)
      : pred(p), user_arg(u), cur(c), next(n), end(e){};
  NodeIteratorPredicateFunc(Graph::NodePredicateFunc p, void *u,
                            NodeIterator &b, NodeIterator &e)
