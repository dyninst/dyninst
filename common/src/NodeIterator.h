...
public:
    ...
    */
    ...
    NodeSearchIterator(Node::Ptr cur, Direction d, Type t, std::deque<Node::Ptr> wl,
                       std::set<Node::Ptr> v)
    : current(cur)
    , direction(d)
    , type(t)
    ...
    NodeSearchIterator(NodeIterator& rangeBegin, NodeIterator& rangeEnd, Direction d,
                       Type t)
    : direction(d)
    , type(t)
    {
...
public:
    ...
    virtual bool      equals(NodeIteratorImpl* rhs)
    {
        ...
        if(tmp == NULL)
            return false;
        return ((pred == tmp->pred) && (user_arg == tmp->user_arg) && (cur == tmp->cur) &&
                (next == tmp->next) && (end == tmp->end));
    }
    ...
    NodeIteratorPredicateFunc(Graph::NodePredicateFunc p, void* u, NodeIterator& c,
                              NodeIterator& n, NodeIterator& e)
    : pred(p)
    , user_arg(u)
    , cur(c)
    ...
    , end(e)
    {}
    NodeIteratorPredicateFunc(Graph::NodePredicateFunc p, void* u, NodeIterator& b,
                              NodeIterator& e)
    : pred(p)
    , user_arg(u)
    , cur(b)
