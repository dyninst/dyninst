...
void
Buffer::initialize(Address a, unsigned s)
{
    assert(buffer_ == NULL);
    start_ = a;
...
void
Buffer::increase_allocation(int size)
{
    if(size <= 0)
        return;
...
unsigned char*
Buffer::cur_ptr() const
{
    return &(buffer_[size_]);
}

Buffer::byte_iterator
Buffer::begin() const
{
...
template <>
void
Buffer::copy(unsigned char* begin, unsigned char* end)
{
...
void
Buffer::copy(void* buf, unsigned size)
{
