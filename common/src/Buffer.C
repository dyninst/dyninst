...
Buffer::Buffer(Address addr, unsigned initial_size)
    : buffer_(NULL), size_(0), max_(0), start_(0) {
...
unsigned Buffer::size() const { return size_; };
unsigned Buffer::max_size() const { return max_; };
...
unsigned char *Buffer::cur_ptr() const { return &(buffer_[size_]); }

Buffer::byte_iterator Buffer::begin() const { return byte_iterator(buffer_); }

Buffer::byte_iterator Buffer::end() const { return byte_iterator(cur_ptr()); }

namespace Dyninst {
template <>
void Buffer::copy(unsigned char *begin, unsigned char *end) {
  unsigned added_size = (long)end - (long)begin;
  if ((size_ + added_size) > max_) increase_allocation(added_size);
