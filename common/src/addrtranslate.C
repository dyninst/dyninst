...
bool AddressTranslate::getLibAtAddress(Address addr, LoadedLib *&lib) {
  ...
  for (int i = -2; i < (int)libs.size(); i++) {
    ...
    if (i == -2) {
      if (lower_lib_index == -1) continue;
      l = libs[lower_lib_index];
    } else if (i == -1) {
      if (higher_lib_index == -1) continue;
...
bool AddressTranslate::getLibs(vector<LoadedLib *> &libs_) {
  libs_.clear();
  for (unsigned i = 0; i < libs.size(); i++) libs_.push_back(libs[i]);
  return true;
}

PID AddressTranslate::getPid() { return pid; }
...
void AddressTranslate::setReadAbort(bool b) { read_abort = b; }
...
LoadedLib *AddressTranslate::getExecutable() { return exec; }

bool AddressTranslate::getArchLibs(std::vector<LoadedLib *> &) { return true; }

string LoadedLib::getName() const { return name; }
...
void LoadedLib::setShouldClean(bool b) { should_clean = b; }

bool LoadedLib::shouldClean() { return should_clean; }
...
void LoadedLib::setDataLoadAddr(Address a) { data_load_addr = a; }

Address LoadedLib::offToAddress(Offset off) { return off + getCodeLoadAddr(); }

Offset LoadedLib::addrToOffset(Address addr) {
  return addr - getCodeLoadAddr();
}

Address LoadedLib::getCodeLoadAddr() const { return load_addr; }

Address LoadedLib::getDataLoadAddr() const { return data_load_addr; }

Address LoadedLib::getDynamicAddr() const { return dynamic_addr; }
...
void *LoadedLib::getUpPtr() { return up_ptr; }

void LoadedLib::setUpPtr(void *v) { up_ptr = v; }

#include <stdarg.h>

//#if !defined(os_linux)
// This definition is for all the non-System V systems
Address AddressTranslate::getLibraryTrapAddrSysV() { return 0x0; }
