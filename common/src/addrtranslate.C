...
bool
AddressTranslate::getLibAtAddress(Address addr, LoadedLib*& lib)
{
    /**
...
#include <iostream>
bool
AddressTranslate::getLibs(vector<LoadedLib*>& libs_)
{
    libs_.clear();
    for(unsigned i = 0; i < libs.size(); i++)
...
PID
AddressTranslate::getPid()
{
    return pid;
}

LoadedLib*
AddressTranslate::getLoadedLib(std::string name)
{
    for(unsigned i = 0; i < libs.size(); i++)
    {
...
void
AddressTranslate::setReadAbort(bool b)
{
    read_abort = b;
}
...
LoadedLib*
AddressTranslate::getExecutable()
{
    return exec;
}

bool
AddressTranslate::getArchLibs(std::vector<LoadedLib*>&)
{
    return true;
}

string
LoadedLib::getName() const
{
    return name;
}

void
LoadedLib::add_mapped_region(Address addr, unsigned long size)
{
    mapped_regions.push_back(pair<Address, unsigned long>(addr, size));
}

void
LoadedLib::setShouldClean(bool b)
{
    should_clean = b;
}

bool
LoadedLib::shouldClean()
{
    return should_clean;
}
...
void
LoadedLib::setDataLoadAddr(Address a)
{
    data_load_addr = a;
}

Address
LoadedLib::offToAddress(Offset off)
{
    return off + getCodeLoadAddr();
}

Offset
LoadedLib::addrToOffset(Address addr)
{
    return addr - getCodeLoadAddr();
}

Address
LoadedLib::getCodeLoadAddr() const
{
    return load_addr;
}

Address
LoadedLib::getDataLoadAddr() const
{
    return data_load_addr;
}

Address
LoadedLib::getDynamicAddr() const
{
    return dynamic_addr;
}

void
LoadedLib::setFactory(SymbolReaderFactory* factory)
{
    symreader_factory = factory;
}

void
LoadedLib::getOutputs(string& filename, Address& code, Address& data)
{
    filename = name;
    code     = load_addr;
    data     = 0;
...
Address
AddressTranslate::getLibraryTrapAddrSysV()
{
    return 0x0;
}
