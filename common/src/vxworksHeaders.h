...
inline int P_execve(const char *FILENAME, char *const ARGV[],
                    char *const ENVP[]) {
...
inline long int P_tkill(pid_t PID, int SIGNUM) {
...
inline const char *P_strchr(const char *P_STRING, int C) {
...
inline const char *P_strrchr(const char *P_STRING, int C) {
...
inline unsigned long int P_strtoul(const char *P_STRING, char **TAILPTR,
                                   int BASE) {
...
inline int P_setsockopt(int s, int level, int optname, void *optval,
                        int optlen) {
...
inline caddr_t P_mmap(caddr_t addr, size_t len, int prot, int flags, int fd,
                      off_t off) {
...
inline int P_rexec(char **ahost, u_short inport, char *user, char *passwd,
                   char *cmd, int *fd2p) {
...
inline int P_unlink(const char *pathname) { return unlink(pathname); }
#endif
