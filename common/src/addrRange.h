...
class addrRange {
 public:
  ...
  virtual std::string get_name() const { return std::string("UNNAMED"); }
  virtual ~addrRange(){};
};
...
class addrRangeTree {
 ...
 public:
  ...
  void remove(Address key) {
    ...
    if (y->color == TREE_BLACK) deleteFixup(x);
    setSize--;
    delete y;
  }
  ...
  virtual bool find(Address start, Address end, std::vector<T*>& ranges) const {
    ...
    if (!result || cur == nil) result = successor_internal(start, cur);
    if (!result || cur == nil) return false;
    assert(cur);

    if (cur->key + cur->value->get_size() < start) cur = treeSuccessor(cur);
   ...
   * elements of the addrRangeTree in ascending order according to comparison
   * function
