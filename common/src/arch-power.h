...
#define IFORM_LI(x) (instruction::signExtend((((x).asInt() & 0x03fffffc) >> 2), 24))
...
#define BFORM_BD(x) (instruction::signExtend((((x).asInt() & 0x0000fffc) >> 2), 14))
...
#define DFORM_D(x) (instruction::signExtend((((x).asInt() & 0x0000ffff)), 16))
#define DFORM_SI(x) (instruction::signExtend((((x).asInt() & 0x0000ffff)), 16))
...
#define DSFORM_DS(x) (instruction::signExtend((((x).asInt() & 0x0000fffc) >> 2), 14))
...
#define RLINMxop                                                                         \
    21 /* rotate left immediate then AND with mask                                       \
        * -- RLWINMxop */
#define RLMIop 22
#define RLNMop 23
...
#define MIN_IMM32                                                                        \
    (-2147483647 - 1)  // In C90, there are no negative
