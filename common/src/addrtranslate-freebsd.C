  ...
  while (true) {
    ...
    if (result == -1 && errno == EINTR) {
      ...
      if (0 != ptrace(PT_CONTINUE, pid, (caddr_t)1, WSTOPSIG(status))) {
        translate_printf(
            "[%s:%u] - Failed to continue process %u: %s\n" __FILE__, __LINE__,
            pid, strerror(errno));
        done();
        return false;
      }
      ...
      translate_printf(
          "[%s:%u] - Unknown error encountered for process %u: %s\n", __FILE__,
          __LINE__, pid, strerror(errno));
      done();
      return false;
    }
...
ProcessReaderPtrace::ProcessReaderPtrace(int pid_) : pid(pid_) {}

ProcessReaderPtrace::~ProcessReaderPtrace() {}

bool ProcessReaderPtrace::ReadMem(Address inTraced, void *inSelf,
                                  unsigned amount) {
