        ...
        if ( getArchitecture() == Arch_amdgpu_vega){
            ...
            switch(category){
                case amdgpu_vega::SGPR_VEC2:
                    ...
                    ret = "SGPR["+std::to_string(base_val)+":"+std::to_string(base_val+1)+"]";
                    break;
                case amdgpu_vega::VGPR_VEC2:
                    base_val -=  (amdgpu_vega::vgpr_vec2_0&0xff);
                    ret = "VGPR["+std::to_string(base_val)+":"+std::to_string(base_val+1)+"]";
                    break;
                case amdgpu_vega::SGPR_VEC4:
                    base_val -=  (amdgpu_vega::sgpr_vec4_0&0xff);
                    ret = "SGPR["+std::to_string(base_val)+":"+std::to_string(base_val+3)+"]";
                    break;
                case amdgpu_vega::VGPR_VEC4:
                    base_val -=  (amdgpu_vega::vgpr_vec4_0&0xff);
                    ret = "VGPR["+std::to_string(base_val)+":"+std::to_string(base_val+3)+"]";
                    break;
                case amdgpu_vega::SGPR_VEC8:
                    base_val -=  (amdgpu_vega::sgpr_vec8_0&0xff);
                    ret = "SGPR["+std::to_string(base_val)+":"+std::to_string(base_val+7)+"]";
                    break;
                case amdgpu_vega::VGPR_VEC8:
                    base_val -=  (amdgpu_vega::vgpr_vec8_0&0xff);
                    ret = "VGPR["+std::to_string(base_val)+":"+std::to_string(base_val+7)+"]";
                    break;
                case amdgpu_vega::SGPR_VEC16:
                    base_val -=  (amdgpu_vega::sgpr_vec16_0&0xff);
                    ret = "SGPR["+std::to_string(base_val)+":"+std::to_string(base_val+15)+"]";
                    break;
                case amdgpu_vega::VGPR_VEC16:
                    base_val -=  (amdgpu_vega::vgpr_vec16_0&0xff);
                    ret = "VGPR["+std::to_string(base_val)+":"+std::to_string(base_val+15)+"]";
                    break;
                default:
                    ret = iter->second;
                    break;
            ...
            return ret;
        }else if(getArchitecture() == Arch_amdgpu_gfx90a || getArchitecture() == Arch_amdgpu_gfx908 || getArchitecture() == Arch_amdgpu_gfx940){
        	return iter->second;
        }else{
            return iter->second;
        }
        ...
        case Arch_amdgpu_gfx90a:{
                                   ...
                                   if ( reg_class == amdgpu_gfx90a::SGPR || reg_class == amdgpu_gfx90a::VGPR){
                                       ...
                                       switch(reg & 0x00007f00){
                                           ...
                                           case amdgpu_gfx90a::BITS_8:
                                               return 1;
                                           case amdgpu_gfx90a::BITS_9:
                                           case amdgpu_gfx90a::BITS_15:
                                           case amdgpu_gfx90a::BITS_16:
                                               return 2;
                                           case amdgpu_gfx90a::BITS_32:
                                               return 4;
                                           case amdgpu_gfx90a::BITS_48:
                                               return 6;
                                           case amdgpu_gfx90a::BITS_64:
                                               return 8; 
                                           case amdgpu_gfx90a::BITS_128:
                                               return 16;
                                           case amdgpu_gfx90a::BITS_256:
                                               return 32;
                                           case amdgpu_gfx90a::BITS_512:
                                               return 64;
                                       }
                                       common_parsing_printf(" unknown reg size %x\n", (unsigned int)reg);
                                       assert(0);
        ...
        case Arch_amdgpu_gfx940:{
                                   ...
                                   if ( reg_class == amdgpu_gfx940::SGPR || reg_class == amdgpu_gfx940::VGPR){
                                       ...
                                       switch(reg & 0x00007f00){
                                           ...
                                           case amdgpu_gfx940::BITS_8:
                                               return 1;
                                           case amdgpu_gfx940::BITS_9:
                                           case amdgpu_gfx940::BITS_15:
                                           case amdgpu_gfx940::BITS_16:
                                               return 2;
                                           case amdgpu_gfx940::BITS_32:
                                               return 4;
                                           case amdgpu_gfx940::BITS_48:
                                               return 6;
                                           case amdgpu_gfx940::BITS_64:
                                               return 8; 
                                           case amdgpu_gfx940::BITS_128:
                                               return 16;
                                           case amdgpu_gfx940::BITS_256:
                                               return 32;
                                           case amdgpu_gfx940::BITS_512:
                                               return 64;
                                       }
                                       common_parsing_printf(" unknown reg size %x\n", (unsigned int)reg);
                                       assert(0);
...
MachRegister MachRegister::getPC(Dyninst::Architecture arch)
{
    switch (arch)
    {
        ...
        case Arch_amdgpu_gfx90a:
            return amdgpu_gfx90a::pc_all;
        case Arch_amdgpu_gfx940:
            return amdgpu_gfx940::pc_all;
        case Arch_none:
            return InvalidReg;
    }
    return InvalidReg;
...
static void getAmdgpuGfx940RoseRegister(int &reg_class, int &reg_idx, int &offset, const int &reg){
    signed int category = (reg & 0x00ff0000);
    signed int baseID =   (reg & 0x000000ff);
