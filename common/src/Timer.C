...
timer::timer(const timer& t)
    : usecs_(t.usecs_),
      ...
      CYCLES_PER_SEC_(t.CYCLES_PER_SEC_),
      MICROSECS_PER_SEC_(t.MICROSECS_PER_SEC_),
...
void timer::start() {
  if (activation_count_ == 0) get_current(cu_, cs_, cw_);
  activation_count_++;
}
...
double timer::usecs() const { return usecs_; }

double timer::ssecs() const { return ssecs_; }

double timer::wsecs() const { return wsecs_; }

bool timer::is_running() const { return (activation_count_ > 0); }
...
void timer::get_current(double& u, double& s, double& w) {
  /*
    u = user time
    s = system time
    w = wall time
  */
  ...
  if (GetProcessTimes(GetCurrentProcess(), &creatT, &exitT, &kernelT, &userT)) {
    ...
    s = ((double)kernelT.dwHighDateTime * ((double)_UI32_MAX + 1.0) +
         (double)kernelT.dwLowDateTime) *
        100.0 / t.NANOSECS_PER_SEC();
    u = ((double)userT.dwHighDateTime * ((double)_UI32_MAX + 1.0) +
         (double)userT.dwLowDateTime) *
        100.0 / t.NANOSECS_PER_SEC();
  } else {
    u = 0;
    s = 0;
