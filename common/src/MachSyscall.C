...
class SyscallInformation {
 public:
  ...
  MachSyscall::SyscallIDPlatform findIDPlatform(
      Platform, MachSyscall::SyscallIDIndependent);

 private:
  SyscallInformation();
  static SyscallInformation* theInstance;

  typedef dyn_hash_map<MachSyscall::SyscallIDIndependent,
                       MachSyscall::SyscallIDPlatform>
      syscallNumbersMap;
  typedef dyn_hash_map<MachSyscall::SyscallIDPlatform, MachSyscall::SyscallName>
      syscallNamesMap;
...
MachSyscall::SyscallName SyscallInformation::findName(
    Platform plat, MachSyscall::SyscallIDPlatform id) {
...
MachSyscall::SyscallIDPlatform SyscallInformation::findIDPlatform(
    Platform plat, MachSyscall::SyscallIDIndependent id) {
...
MachSyscall::SyscallName MachSyscall::nameLookup(Platform plat,
                                                 SyscallIDPlatform id) {
  return SyscallInformation::getInstance()->findName(plat, id);
}

MachSyscall MachSyscall::makeFromPlatform(Platform plat,
                                          SyscallIDIndependent id) {
  // 1. Convert SyscallIDIndependent to SyscallIDPlatform
  SyscallIDPlatform syscallNumber =
      SyscallInformation::getInstance()->findIDPlatform(plat, id);
...
unsigned long MachSyscall::num() const { return _id; }

MachSyscall::SyscallName MachSyscall::name() const { return _name; }
