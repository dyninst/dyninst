...
bool annotation_debug_flag() { return dyn_debug_annotations; }
...
bool void_ptr_cmp_func(void *v1, void *v2) { return v1 == v2; }
}
...
AnnotationClassBase::AnnotationClassBase(std::string n,
  ...
  if (id >= annotation_types->size()) assert(0 && "bad anno id");
}
...
Dyninst::AnnotationClassBase *AnnotationClassBase::findAnnotationClass(
    unsigned int id) {
  if (id > annotation_types->size()) {
    fprintf(
        stderr,
        "%s[%d]:  cannot find annotation class base for id %d, max is %ld\n",
  ...
  if ((*annotation_types)[id]->getID() != id) {
    fprintf(stderr,
            "%s[%d]:  FIXME:  have bad id in annotation class: %d, not %d\n",
...
void AnnotationClassBase::dumpAnnotationClasses() {
  ...
  for (unsigned int i = 0; i < annotation_types->size(); ++i) {
    ...
    fprintf(stderr, "\tid-%d\t%s, type %s\n", acb->getID(),
            acb->getName().c_str(), acb->getTypeName());
  }
}
...
bool is_input(SerializerBase *sb) { return sb->isInput(); }
bool is_output(SerializerBase *sb) { return sb->isOutput(); }
bool serialize_annotation_list(void *id, std::vector<ser_rec_t> &sers,
                               SerializerBase *sb, const char *tag) {
  if (sers.size())
    serialize_printf(
        "%s[%d]:  welcome to serialize_annotation_list, size %lu, id = %p\n",
...
bool serialize_post_annotation(void *parent, void *anno, SerializerBase *sb,
                               AnnotationClassBase *acb,
                               sparse_or_dense_anno_t sod, const char *tag) {
  serialize_printf(
      "%s[%d]:  welcome to serialize_post_annotation_list, id = %p\n", FILE__,
      __LINE__, parent);
  assert(parent);
  assert(anno);
  if (!sb) {
...
bool add_annotations(SerializerBase *sb, AnnotatableSparse *an,
                     std::vector<ser_rec_t> &sers) {
  if (sers.size())
    serialize_printf("%s[%d]:  welcome to addAnnotations: got %lu\n", FILE__,
                     __LINE__, sers.size());
  //  if we are not doing deserialization, there is nothing to do here, just
  //  return true
  //  to keep from triggering error handling.
  if (sb->isOutput()) return true;
  bool err = false;
...
bool add_annotations(SerializerBase *sb, AnnotatableDense *an,
                     std::vector<ser_rec_t> &sers) {
  serialize_printf("%s[%d]:  welcome to addAnnotations: got %lu\n", FILE__,
                   __LINE__, sers.size());
  //  if we are not doing deserialization, there is nothing to do here, just
...
#else
bool is_input(SerializerBase *) { return false; }

bool is_output(SerializerBase *) { return false; }

bool serialize_annotation_list(void *, std::vector<ser_rec_t> &,
                               SerializerBase *, const char *) {
  return false;
}

bool serialize_post_annotation(void *, void *, SerializerBase *,
                               AnnotationClassBase *, sparse_or_dense_anno_t,
                               const char *) {
  return false;
}

bool add_annotations(SerializerBase *, AnnotatableSparse *,
                     std::vector<ser_rec_t> &) {
  return false;
}

bool add_annotations(SerializerBase *, AnnotatableDense *,
                     std::vector<ser_rec_t> &) {
  return false;
}
#endif
