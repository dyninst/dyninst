...
struct Location {
  static Location Function(PatchFunction *f) {
    return Location(f, NULL, 0, InstructionAPI::Instruction::Ptr(), NULL, true,
                    Function_);
  }
  static Location Block(PatchBlock *b) {
    return Location(NULL, b, 0, InstructionAPI::Instruction::Ptr(), NULL, true,
                    Block_);
  }
  static Location BlockInstance(PatchFunction *f, PatchBlock *b,
                                bool trusted = false) {
    return Location(f, b, 0, InstructionAPI::Instruction::Ptr(), NULL, trusted,
                    BlockInstance_);
  }
  ...
  static Location Instruction(PatchBlock *b, Address a) {
    return Location(NULL, b, a, InstructionAPI::Instruction::Ptr(), NULL, false,
                    Instruction_);
  }
  static Location InstructionInstance(PatchFunction *f, InsnLoc_t l,
                                      bool trusted = false) {
    return Location(f, l.block, l.addr, l.insn, NULL, trusted,
                    InstructionInstance_);
  }
  static Location InstructionInstance(PatchFunction *f, PatchBlock *b,
                                      Address a) {
    return Location(f, b, a, InstructionAPI::Instruction::Ptr(), NULL, false,
                    InstructionInstance_);
  }
  static Location InstructionInstance(PatchFunction *f, PatchBlock *b,
                                      Address a,
  ...
  static Location Edge(PatchEdge *e) {
    return Location(NULL, NULL, 0, InstructionAPI::Instruction::Ptr(), e, true,
                    Edge_);
  }
  static Location EdgeInstance(PatchFunction *f, PatchEdge *e) {
    return Location(f, NULL, 0, InstructionAPI::Instruction::Ptr(), e, false,
                    EdgeInstance_);
  }
  static Location EntrySite(EntrySite_t e) {
    return Location(e.func, e.block, 0, InstructionAPI::Instruction::Ptr(),
                    NULL, true, Entry_);
  }
  static Location EntrySite(PatchFunction *f, PatchBlock *b,
                            bool trusted = false) {
    return Location(f, b, 0, InstructionAPI::Instruction::Ptr(), NULL, trusted,
                    Entry_);
  }
  static Location CallSite(CallSite_t c) {
    return Location(c.func, c.block, 0, InstructionAPI::Instruction::Ptr(),
                    NULL, true, Call_);
  }
  static Location CallSite(PatchFunction *f, PatchBlock *b) {
    return Location(f, b, 0, InstructionAPI::Instruction::Ptr(), NULL, false,
                    Call_);
  }
 ...
 private:
  Location(PatchFunction *f, PatchBlock *b, Address a,
           InstructionAPI::Instruction::Ptr i, PatchEdge *e, bool u, type_t t)
...
class PATCHAPI_EXPORT Point {
 ...
 public:
  ...
  enum Type {
    PreInsn = type_val(0),
    PostInsn = type_val(1),
    // InsnTaken = type_val(2), // This should be edge instrumentation? bernat,
    // 25JUN11
    ...
    LoopTypes =
        LoopStart | LoopEnd | LoopIterStart | LoopIterEnd,  // TODO(wenbin)
    CallTypes = PreCall | PostCall
  };

  template <class Scope>
  static Point *create(Address addr, Point::Type type, PatchMgrPtr mgr,
                       Scope *scope) {
  ...
  Point(Point::Type t, PatchMgrPtr mgr, PatchBlock *, Address,
        InstructionAPI::Instruction::Ptr, PatchFunction * = NULL);
...
enum SnippetType { SYSTEM, USER };

enum SnippetState { FAILED, PENDING, INSERTED };

/* A representation of a particular snippet inserted at a
   particular point */
class PATCHAPI_EXPORT Instance
 ...
 public:
  ...
  Instance(Point *point, SnippetPtr snippet)
      : point_(point),
        ...
        guarded_(true) {}
  virtual ~Instance() {}
  static InstancePtr create(Point *, SnippetPtr, SnippetType type = SYSTEM,
