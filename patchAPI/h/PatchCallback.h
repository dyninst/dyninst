...
class PATCHAPI_EXPORT PatchCallback {
 ...
 private:
  ...
  struct BlockMod {
    ...
    edge_type_t type;
    mod_t mod;
    BlockMod(PatchBlock *b, PatchEdge *e, edge_type_t t, mod_t m)
        : block(b), edge(e), type(t), mod(m){};
  };
  ...
  struct FuncMod {
    ...
    PatchBlock *block;
    mod_t mod;
    FuncMod(PatchFunction *f, PatchBlock *b, mod_t m)
        : func(f), block(b), mod(m){};
  };
  ...
  struct PointMod {
    ...
    PointMod(Point *p, PatchBlock *ob, PatchBlock *nb)
        : point(p), old_block(ob), new_block(nb){};
  };
