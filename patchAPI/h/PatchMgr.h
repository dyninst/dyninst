...
struct Scope {
  ...
  Scope(PatchFunction *f, PatchBlock *b)
      : obj(NULL), func(f), block(b), wholeProgram(false){};
  Scope(PatchFunction *f)
      : obj(NULL), func(f), block(NULL), wholeProgram(false){};
};

class PATCHAPI_EXPORT PatchMgr
    : public boost::enable_shared_from_this<PatchMgr> {
  friend class Point;
 ...
 public:
  ...
  bool removeSnippets(Scope scope, Point::Type types) {
    ...
    char *foo = NULL;
    return removeSnippets<IdentityFilterFunc<char *>, char *>(scope, types,
                                                              filter_func, foo);
  }
 ...
 private:
  bool findInsnPointsByType(Location *, Point::Type, PointSet &,
                            bool create = true);
  bool findBlockPointsByType(Location *, Point::Type, PointSet &,
                             bool create = true);
  bool findEdgePointsByType(Location *, Point::Type, PointSet &,
                            bool create = true);
  bool findFuncPointsByType(Location *, Point::Type, PointSet &,
                            bool create = true);
  bool findFuncSitePointsByType(Location *, Point::Type, PointSet &,
                                bool create = true);
  ...
  void getBlockInstanceCandidates(Scope &scope, Point::Type types,
                                  Candidates &ret);
  // bool getEdgeInstanceCandidates(Scope &scope, Point::Type types, Candidates
  // &ret);
  void getInsnInstanceCandidates(Scope &scope, Point::Type types,
                                 Candidates &ret);
...
template <class FilterFunc, class FilterArgument, class OutputIterator>
bool PatchMgr::findPoints(Scope scope, Point::Type types,
                          FilterFunc filter_func, FilterArgument filter_arg,
                          OutputIterator output_iter, bool create) {
  Candidates candidates;
  if (!getCandidates(scope, types, candidates)) return false;
  for (Candidates::iterator c = candidates.begin(); c != candidates.end();
       ++c) {
...
bool PatchMgr::findPoints(Location loc, Point::Type types,
  ...
  for (EnumeratedTypes::iterator iter = tmp.begin(); iter != tmp.end();
       ++iter) {
