...
class PATCHAPI_EXPORT PatchEdge {
 ...
 public:
  ...
  PatchEdge(ParseAPI::Edge *internalEdge, PatchBlock *source,
            PatchBlock *target);
  PatchEdge(const PatchEdge *parent, PatchBlock *child_src,
...
class PATCHAPI_EXPORT PatchBlock {
 ...
 protected:
  typedef enum { backwards, forwards } Direction;
...
class PATCHAPI_EXPORT PatchFunction {
 ...
 public:
  struct compare {
    bool operator()(PatchBlock *const &b1, PatchBlock *const &b2) const {
  ...
  bool verifyExit(PatchBlock *block) {
    return exitBlocks().find(block) != exitBlocks().end();
  }
  bool verifyCall(PatchBlock *block) {
    return callBlocks().find(block) != callBlocks().end();
  }
  ...
  bool verifyExitConst(const PatchBlock *block) const {
    return exit_blocks_.empty() ||
           exit_blocks_.find(const_cast<PatchBlock *>(block)) !=
               exit_blocks_.end();
  }
  bool verifyCallConst(const PatchBlock *block) const {
    return call_blocks_.empty() ||
           call_blocks_.find(const_cast<PatchBlock *>(block)) !=
               call_blocks_.end();
  }
 ...
 protected:
  ...
  /* Loop details*/
  bool _loop_analyzed;  // true if loops in the function have been found and
                        // stored in _loops
  std::set<PatchLoop *> _loops;
  map<ParseAPI::Loop *, PatchLoop *> _loop_map;
  PatchLoopTreeNode
      *_loop_root;  // NULL if the tree structure has not be calculated
...
class PATCHAPI_EXPORT PatchLoopTreeNode {
 public:
  ...
  PatchLoopTreeNode(PatchObject *obj, ParseAPI::LoopTreeNode *l,
                    std::map<ParseAPI::Loop *, PatchLoop *> &);
...
#define CONSIST_FAIL                           \
  {                                            \
    if (ASSERT_CONSISTENCY_FAILURE) assert(0); \
    return false;                              \
  }
};
};
