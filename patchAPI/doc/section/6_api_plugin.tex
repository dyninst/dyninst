...
\begin{apient}
bool write(PatchObject* obj, Address to, Address from, size_t size);
\end{apient}
...
\begin{apient}
Address malloc(PatchObject* obj, size_t size, Address near);
\end{apient}
...
\begin{apient}
bool realloc(PatchObject* obj, Address orig, size_t size);
\end{apient}
...
\begin{apient}
bool free(PatchObject* obj, Address orig);
\end{apient}
...
\begin{apient}
bool loadObject(PatchObject* obj);
\end{apient}
...
\begin{apient}
bool replaceFunction(PatchFunction* oldfunc, PatchFunction* newfunc);
\end{apient}
...
\begin{apient}
bool revertReplacedFunction(PatchFunction* oldfunc);
\end{apient}
...
\begin{apient}
bool wrapFunction(PatchFunction* oldfunc, PatchFunction* newfunc, std::string name);
\end{apient}
...
\begin{apient}
bool revertWrappedFunction(PatchFunction* oldfunc);
\end{apient}
...
\begin{apient}
typedef std::map<PatchBlock*,        // B  : A call block
            std::map<PatchFunction*, // F_c: Function context
...
\begin{apient}
PatchFunction* makeFunction(ParseAPI::Function* func, PatchObject* obj);
PatchFunction* copyFunction(PatchFunction* func, PatchObject* obj);
PatchBlock* makeBlock(ParseAPI::Block* blk, PatchObject* obj);
PatchBlock* copyBlock(PatchBlock* blk, PatchObject* obj);
PatchEdge* makeEdge(ParseAPI::Edge* edge, PatchBlock* src,
                            PatchBlock* trg, PatchObject* obj);
PatchEdge* copyEdge(PatchEdge* edge, PatchObject* obj);
\end{apient}
...
\begin{apient}
Point* mkFuncPoint(Point::Type t, PatchMgrPtr m, PatchFunction* f);
Point* mkFuncSitePoint(Point::Type t, PatchMgrPtr m, PatchFunction* f, PatchBlock* b);
Point* mkBlockPoint(Point::Type t, PatchMgrPtr m, PatchBlock* b, PatchFunction* context);
Point* mkInsnPoint(Point::Type t, PatchMgrPtr m, PatchBlock *, Address a,
                           InstructionAPI::Instruction i, PatchFunction* context);
Point* mkEdgePoint(Point::Type t, PatchMgrPtr m, PatchEdge* e, PatchFunction* context);
\end{apient}
...
\label{sec:default-plugin}

\subsection{PushFrontCommand and PushBackCommand}
\label{sec:push-commands}
...
\begin{apient}
static PushFrontCommand* create(Point* pt, SnippetPtr snip);
static PushBackCommand* create(Point* pt, SnippetPtr snip);
\end{apient}
...
\label{sec:removesnippetcommand}
...
\begin{apient}
static RemoveSnippetCommand* create(InstancePtr instance);
\end{apient}
...
\begin{apient}
static RemoveCallCommand* create(PatchMgrPtr mgr, PatchBlock* call_block,
                                 PatchFunction* context = NULL);
\end{apient}
...
\begin{apient}
static ReplaceCallCommand* create(PatchMgrPtr mgr, PatchBlock* call_block,
...
\begin{apient}
static ReplaceFuncCommand* create(PatchMgrPtr mgr, PatchFunction* old_func,
