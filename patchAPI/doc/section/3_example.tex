...
\subsubsection{Point Finding} \label{sec:example-point-finding}
...
\emph{pts} found in Section~\ref{sec:example-point-finding}. We'll explain the snippet
(Line 1) in the example in Section~\ref{sec:example-snippet-representation}. Each point maintains
...
\subsubsection{Snippet Representation} \label{sec:example-snippet-representation}
\lstset{language=[GNU]C++,basicstyle=\fontfamily{fvm}\selectfont\small}
\lstset{numbers=left}
