...
.. figure:: ./figure/abstraction/img.pdf
   :alt: [fig:abs]Object Ownership
   :width: 85.0%

   [fig:abs]Object Ownership
...
Figure `[fig:abs] <#fig:abs>`__ shows the ownership hierarchy for
PatchAPI’s classes. Ownership is a “contains” relationship. If one class
owns another, then instances of the owner class maintain exactly one or
...
`[fig:abs] <#fig:abs>`__, for example, each PatchMgr instance contains
exactly one instance of a AddrSpace object, while a PatchMgr instance
may contains more than one instances of a Point object.
...
in Section \ `[sec-public-api] <#sec-public-api>`__ and
Section \ `[sec-plugin-api] <#sec-plugin-api>`__.
...
.. figure:: ./figure/command/img.pdf
   :alt: [fig:inh]Inheritance Hierarchy
   :width: 85.0%

   [fig:inh]Inheritance Hierarchy
...
`[fig:inh] <#fig:inh>`__ illustrates the inheritance hierarchy for
related classes. There is a default implementation of instrumentation
