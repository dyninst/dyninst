...
\begin{apient}
PatchEdge* getEdge(ParseAPI::Edge* edge, PatchBlock* src = NULL,
                   PatchBlock* trg = NULL, bool create = true);
\end{apient}
...
\begin{apient}
ParseAPI::Function* function() const;
\end{apient}
...
\begin{apient}
bool getLoops(std::vector<PatchLoop*>& loops);
\end{apient}
\apidesc{Fill \code{loops} with all the loops in the function}

\begin{apient}
bool getOuterLoops(std::vector<PatchLoop*>& loops);
\end{apient}
...
\begin{apient}
void getImmediateDominates(PatchBlock* A, std::set<PatchBlock*>& imm);
\end{apient}
\apidesc{Fill \code{imm} with all the blocks immediate dominated by block \code{A}}

\begin{apient}
void getAllDominates(PatchBlock* A, std::set<PatchBlock*>& dom);
\end{apient}
...
returns InstructionAPI::Instruction().
}
...
\begin{apient}
int getLoopEntries(std::vector<PatchBlock*>& entries);
\end{apient}
...
\begin{apient}
int getBackEdges(std::vector<PatchEdge*>& edges);
\end{apient}
...
\begin{apient}
bool getContainedLoops(std::vector<PatchLoop*>& loops);
\end{apient}
...
\begin{apient}
bool getLoopBasicBlocks(std::vector<PatchBlock*>& blocks);
\end{apient}
\apidesc{Fills \code{blocks} with all basic blocks in the loop}


\begin{apient}
bool getLoopBasicBlocksExclusive(std::vector<PatchBlock*>& blocks);
\end{apient}
...
\begin{apient}
bool getCallees(std::vector<PatchFunction*>& v);
\end{apient}
...
\label{sec:point-snippet-interface}
...
covered in Section~\ref{sec:point}. PatchAPI creates Points on demand, so if
...
\begin{apient}
InstancePtr pushBack(SnippetPtr, SnippetType type = SnippetType::REGULAR);
InstancePtr pushFront(SnippetPtr, SnippetType type = SnippetType::REGULAR);
\end{apient}
...
\begin{apient}
const InstructionAPI::Instruction& insn() const;
\end{apient}
...
\begin{apient}
void destroy_cb(PatchBlock *);
void destroy_cb(PatchEdge *);
void destroy_cb(PatchFunction *);
void destroy_cb(PatchObject *);
\end{apient}
...
\begin{apient}
void create_cb(PatchBlock *);
void create_cb(PatchEdge *);
void create_cb(PatchFunction *);
void create_cb(PatchObject *);
\end{apient}
...
\begin{apient}
void split_block_cb(PatchBlock* first, PatchBlock* second);
\end{apient}
...
\begin{apient}
void remove_edge_cb(PatchBlock *, PatchEdge *, edge_type_t);
void add_edge_cb(PatchBlock *, PatchEdge *, edge_type_t);
\end{apient}
...
\begin{apient}
void remove_block_cb(PatchFunction *, PatchBlock *);
...
\begin{apient}
void create_cb(Point* pt);
void destroy_cb(Point* pt);
\end{apient}
...
\begin{apient}
void change_cb(Point* pt, PatchBlock* first, PatchBlock* second);
\end{apient}
