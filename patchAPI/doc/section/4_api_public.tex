...
\begin{apient}
int getLoopEntries(set<PatchBlock*>& entries);
\end{apient}
...
loops. The outer loops are contained in the vector of \code{children} of the root. 
...
followed by a \code{\_y}, where \code{y} is 1 to m, where m is the number of sub-loops under the outer loop.  
...
no children. \code{loop\_1} has two child PatchLoopTreeNode objects, named \code{loop\_1\_1} and 
\code{loop\_1\_2}, respectively representing the \code{y} and \code{z} loops. 
