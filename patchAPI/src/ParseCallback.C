...
void PatchParseCallback::split_block_cb(ParseAPI::Block *first,
                                        ParseAPI::Block *second) {
  // 1) we create blocks lazily, so do nothing if the first block doesn't exist
  // yet
  // 2) have the object split the block, it will create the second block and add
  // it
  // 3) tell the first block to split off the second block and all points inside
  // of it
  // 4) have the function split the block and re-calculate its entry,exit,etc
  // blocks
...
void PatchParseCallback::destroy_cb(ParseAPI::Edge *edge) {
  ...
  if (pe) {
    if (inSrc)
      srcObj->cb()->destroy(pe, srcObj);
    else
      dstObj->cb()->destroy(pe, dstObj);
  }
}
...
void PatchParseCallback::remove_edge_cb(ParseAPI::Block *block,
                                        ParseAPI::Edge *edge,
                                        edge_type_t type) {
  // We get this callback before the edge is deleted; we need to tell the block
  // that we
  // nuked an edge.
...
void PatchParseCallback::add_edge_cb(ParseAPI::Block *block,
                                     ParseAPI::Edge *edge, edge_type_t type) {
  PatchObject *pbObj = _obj->addrSpace()->findObject(block->obj());
  PatchBlock *pb = pbObj->getBlock(block, false);
  if (!pb)
    return;  // We haven't created the block, so we'll get around to the edge
             // later.

  // If we haven't requested edges then ignore and we'll get it later.
  if (type == source) {
    if (pb->srclist_.empty()) return;
  } else if (pb->trglist_.empty())
    return;
  ...
  assert(pb2);
  PatchEdge *pe = (type == source) ? pb2Obj->getEdge(edge, pb2, pb)
                                   : pbObj->getEdge(edge, pb, pb2);
  if (type == source)
    pb->addSourceEdge(pe, false);
  else
    pb->addTargetEdge(pe, false);
}
...
void PatchParseCallback::remove_block_cb(ParseAPI::Function *func,
                                         ParseAPI::Block *block) {
  PatchBlock *pb = _obj->getBlock(block, false);
  if (!pb) return;
...
/* Adds blocks lazily, basically does nothing unless block and function have
   already
   been created, in which case it adds the block to the function */
void PatchParseCallback::add_block_cb(ParseAPI::Function *func,
                                      ParseAPI::Block *block) {
  PatchBlock *pb = _obj->getBlock(block, false);
  if (!pb) return;
...
bool PatchParseCallback::absAddr(Address absolute, Address &loadAddr,
