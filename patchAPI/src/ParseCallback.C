...
void
PatchParseCallback::split_block_cb(ParseAPI::Block* first, ParseAPI::Block* second)
{
...
void
PatchParseCallback::destroy_cb(ParseAPI::Block* block)
{
...
void
PatchParseCallback::destroy_cb(ParseAPI::Edge* edge)
{
    bool         inSrc  = true;
...
void
PatchParseCallback::destroy_cb(ParseAPI::Function* func)
{
...
void
PatchParseCallback::remove_edge_cb(ParseAPI::Block* block, ParseAPI::Edge* edge,
                                   edge_type_t type)
{
    ...
    else
    {
        ...
        if(numCalls == 1)
        {
            ...
            for(vector<PatchFunction*>::iterator fit = funcs.begin(); fit != funcs.end();
                fit++)
            {
...
void
PatchParseCallback::add_edge_cb(ParseAPI::Block* block, ParseAPI::Edge* edge,
                                edge_type_t type)
{
    PatchObject* pbObj = _obj->addrSpace()->findObject(block->obj());
    if(!pbObj)
        return;  // e.g. sink block
    PatchBlock* pb = pbObj->getBlock(block, false);
    if(!pb)
        return;  // We haven't created the block, so we'll get around to the edge later.
    ...
    assert(pb2);
    PatchEdge* pe =
        (type == source) ? pb2Obj->getEdge(edge, pb2, pb) : pbObj->getEdge(edge, pb, pb2);
    if(type == source)
...
void
PatchParseCallback::modify_edge_cb(ParseAPI::Edge* edge, ParseAPI::Block* block,
                                   edge_type_t type)
{
    PatchEdge* pe = _obj->getEdge(edge, NULL, NULL, false);
    if(!pe)
        return;  // No edge, so we'll get to it later.
...
void
PatchParseCallback::remove_block_cb(ParseAPI::Function* func, ParseAPI::Block* block)
{
    PatchBlock* pb = _obj->getBlock(block, false);
    if(!pb)
        return;
...
void
PatchParseCallback::add_block_cb(ParseAPI::Function* func, ParseAPI::Block* block)
{
    PatchBlock* pb = _obj->getBlock(block, false);
    if(!pb)
        return;
...
bool
PatchParseCallback::absAddr(Address absolute, Address& loadAddr,
