...
void
PatchMgr::version(int& major, int& minor, int& maintenance)
{
...
Point*
PatchMgr::findPoint(Location loc, Point::Type type, bool create)
{
...
bool
PatchMgr::getCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
    // We hand in pre-generated lists to reduce the requirement for iteration
    if(wantFuncs(scope, types))
        getFuncCandidates(scope, types, ret);
    if(wantCallSites(scope, types))
        getCallSiteCandidates(scope, types, ret);
    if(wantExitSites(scope, types))
        getExitSiteCandidates(scope, types, ret);

    if(wantBlocks(scope, types))
        getBlockCandidates(scope, types, ret);
    if(wantEdges(scope, types))
        getEdgeCandidates(scope, types, ret);
    if(wantInsns(scope, types))
        getInsnCandidates(scope, types, ret);
...
bool
PatchMgr::wantFuncs(Scope& scope, Point::Type types)
{
...
bool
PatchMgr::wantCallSites(Scope& scope, Point::Type types)
{
...
bool
PatchMgr::wantExitSites(Scope& scope, Point::Type types)
{
...
bool
PatchMgr::wantBlocks(Scope& scope, Point::Type types)
{
    return (scope.func == NULL && Point::TestType(types, Point::BlockTypes));
}

bool
PatchMgr::wantBlockInstances(Scope& scope, Point::Type types)
{
    return (scope.func != NULL && Point::TestType(types, Point::BlockTypes));
}

bool
PatchMgr::wantEdges(Scope& scope, Point::Type types)
{
    return (scope.func == NULL && Point::TestType(types, Point::EdgeTypes));
}

bool
PatchMgr::wantInsns(Scope& scope, Point::Type types)
{
    return (scope.func == NULL && Point::TestType(types, Point::InsnTypes));
}

bool
PatchMgr::wantInsnInstances(Scope& scope, Point::Type types)
{
    return (scope.func != NULL && Point::TestType(types, Point::InsnTypes));
}

void
PatchMgr::getFuncCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
    ...
    for(Functions::iterator iter = funcs.begin(); iter != funcs.end(); ++iter)
    {
        ...
        if(types & Point::FuncEntry)
            ret.push_back(Candidate(Location::EntrySite(*iter, (*iter)->entry(), true),
                                    Point::FuncEntry));
    }
}

void
PatchMgr::getCallSiteCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
    CallSites sites;
...
void
PatchMgr::getExitSiteCandidates(Scope& scope, Point::Type, Candidates& ret)
{
    ExitSites sites;
...
void
PatchMgr::getBlockCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
    Blocks blocks;
...
void
PatchMgr::getEdgeCandidates(Scope& scope, Point::Type, Candidates& ret)
{
    Edges edges;
...
void
PatchMgr::getInsnCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
    Insns insns;
...
void
PatchMgr::getBlockInstanceCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
...
void
PatchMgr::getInsnInstanceCandidates(Scope& scope, Point::Type types, Candidates& ret)
{
    InsnInstances insns;
    ...
    for(InsnInstances::iterator iter = insns.begin(); iter != insns.end(); ++iter)
    {
        if(types & Point::PreInsn)
            ret.push_back(
                Candidate(Location::InstructionInstance(iter->first, iter->second, true),
                          Point::PreInsn));
        if(types & Point::PostInsn)
            ret.push_back(
                Candidate(Location::InstructionInstance(iter->first, iter->second, true),
...
void
PatchMgr::getFuncs(Scope& scope, Functions& funcs)
{
...
void
PatchMgr::getCallSites(Scope& scope, CallSites& sites)
{
    ...
    for(Functions::iterator iter = funcs.begin(); iter != funcs.end(); ++iter)
    {
        ...
        for(PatchFunction::Blockset::const_iterator iter2 = c.begin(); iter2 != c.end();
            ++iter2)
        {
...
void
PatchMgr::getExitSites(Scope& scope, ExitSites& sites)
{
    // All sites in whatever functions we want
    Functions funcs;
    getFuncs(scope, funcs);
    for(Functions::iterator iter = funcs.begin(); iter != funcs.end(); ++iter)
    {
        ...
        for(PatchFunction::Blockset::const_iterator iter2 = e.begin(); iter2 != e.end();
            ++iter2)
        {
            if(!scope.block || (scope.block == *iter2))
...
void
PatchMgr::getBlocks(Scope& scope, Blocks& blocks)
{
    if(scope.wholeProgram)
    {
        const AddrSpace::ObjMap& objs = as()->objMap();
        for(AddrSpace::ObjMap::const_iterator iter = objs.begin(); iter != objs.end();
            ++iter)
        {
...
void
PatchMgr::getEdges(Scope& scope, Edges& edges)
{
    if(scope.wholeProgram)
    {
        const AddrSpace::ObjMap& objs = as()->objMap();
...
void
PatchMgr::getInsns(Scope& scope, Insns& insns)
{
    Blocks blocks;
    getBlocks(scope, blocks);
...
void
PatchMgr::getBlockInstances(Scope& scope, BlockInstances& blocks)
{
    Functions funcs;
    getFuncs(scope, funcs);
    ...
    for(Functions::iterator iter = funcs.begin(); iter != funcs.end(); ++iter)
    {
        ...
        for(PatchFunction::Blockset::const_iterator iter2 = b.begin(); iter2 != b.end();
            ++iter2)
        {
...
void
PatchMgr::getInsnInstances(Scope& scope, InsnInstances& insns)
{
    Functions funcs;
    getFuncs(scope, funcs);
    ...
    for(Functions::iterator iter = funcs.begin(); iter != funcs.end(); ++iter)
    {
            ...
            ++iter2)
        {
            ...
            {
                insns.push_back(
                    InsnInstance(*iter, InsnLoc_t(*iter2, iter3->first, iter3->second)));
            }
        }
    }
}
...
void
PatchMgr::enumerateTypes(Point::Type types, EnumeratedTypes& out)
{
...
void
PatchMgr::destroy(Point* p)
{
    p->obj()->cb()->destroy(p);
}

bool
PatchMgr::verify(Location& loc)
{
    if(loc.trusted)
        return true;
    ...
    switch(loc.type)
    {
        ...
        case Location::Instruction_:
            if(loc.addr < loc.block->start())
                return false;
            if(loc.addr > loc.block->last())
                return false;
        ...
        case Location::Entry_:
            if(loc.func->entry() != loc.block)
                return false;
            break;
        case Location::Call_:
            // Check to see if the block is in the call blocks
            if(loc.func->callBlocks().find(loc.block) == loc.func->callBlocks().end())
                return false;
            break;
        case Location::Exit_:
            if(loc.func->exitBlocks().find(loc.block) == loc.func->exitBlocks().end())
                return false;
            break;
        default:
            assert(0);
