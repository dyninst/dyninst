...
PatchFunction::blocks()
{
    ...
    {  // recompute other block lists if block list grew
...
{
    for(auto iter = func_->exitBlocks().begin(); iter != func_->exitBlocks().end();
        ++iter)
    {
    ...
    else
    {
        switch(type)
        {
            ...
            case Point::FuncExit: {
                if(!loc.block)
                    return NULL;
                if(create && !loc.trusted && !verifyExit(loc.block))
                    return NULL;
            ...
            case Point::PreCall: {
                if(!loc.block)
                    return NULL;
                if(create && !loc.trusted && !verifyCall(loc.block))
                    return NULL;
                std::map<PatchBlock*, Point*>::iterator iter =
...
bool
PatchFunction::findInsnPoints(Point::Type type, PatchBlock* block,
...
bool
FuncPoints::consistency(const PatchFunction* func) const
{
    if(entry)
    {
...
// set. not NULL.
void
PatchFunction::getLoopsByNestingLevel(vector<PatchLoop*>& lbb, bool outerMostOnly)
{
    if(_loop_analyzed == false)
    {
        createLoops();
    ...
    for(std::set<PatchLoop*>::iterator iter = _loops.begin(); iter != _loops.end();
        ++iter)
    {
        // if we are only getting the outermost loops
        if(outerMostOnly && (*iter)->parent != NULL)
            continue;
...
bool
PatchFunction::getLoops(vector<PatchLoop*>& lbb)
{
...
bool
PatchFunction::getOuterLoops(vector<PatchLoop*>& lbb)
{
    getLoopsByNestingLevel(lbb, true);
    return true;
}
PatchLoop*
PatchFunction::findLoop(const char* name)
{
...
void
PatchFunction::fillDominatorInfo()
{
...
void
PatchFunction::fillPostDominatorInfo()
{
...
bool
PatchFunction::dominates(PatchBlock* A, PatchBlock* B)
{
    if(A == NULL || B == NULL)
        return false;
    if(A == B)
...
PatchBlock*
PatchFunction::getImmediateDominator(PatchBlock* A)
{
    fillDominatorInfo();
...
void
PatchFunction::getImmediateDominates(PatchBlock* A, set<PatchBlock*>& imd)
{
    fillDominatorInfo();
...
void
PatchFunction::getAllDominates(PatchBlock* A, set<PatchBlock*>& d)
{
    fillDominatorInfo();
    d.insert(A);
    if(immediateDominates[A] == NULL)
...
bool
PatchFunction::postDominates(PatchBlock* A, PatchBlock* B)
{
    if(A == NULL || B == NULL)
        return false;
    if(A == B)
...
PatchBlock*
PatchFunction::getImmediatePostDominator(PatchBlock* A)
{
    fillPostDominatorInfo();
...
void
PatchFunction::getImmediatePostDominates(PatchBlock* A, set<PatchBlock*>& imd)
{
    fillPostDominatorInfo();
...
void
PatchFunction::getAllPostDominates(PatchBlock* A, set<PatchBlock*>& d)
{
    fillPostDominatorInfo();
    d.insert(A);
    if(immediatePostDominates[A] == NULL)
