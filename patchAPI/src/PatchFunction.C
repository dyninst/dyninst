...
PatchFunction::PatchFunction(ParseAPI::Function *f, PatchObject *o)
    : func_(f),
      obj_(o),
      addr_((obj_->codeBase() + func_->addr()) & obj_->addrMask()),
...
PatchFunction::PatchFunction(const PatchFunction *parFunc, PatchObject *child)
    : func_(parFunc->func_),
      obj_(child),
      addr_(obj_->codeBase() + func_->addr()),
      _loop_analyzed(false),
      _loop_root(NULL),
      isDominatorInfoReady(false),
      isPostDominatorInfoReady(false) {}

const PatchFunction::Blockset &PatchFunction::blocks() {
  if (all_blocks_.size() == func_->num_blocks()) return all_blocks_;
...
PatchBlock *PatchFunction::entry() {
  assert(obj());
  assert(func_);
...
const PatchFunction::Blockset &PatchFunction::exitBlocks() {
...
const PatchFunction::Blockset &PatchFunction::callBlocks() {
...
Point *PatchFunction::findPoint(Location loc, Point::Type type, bool create) {
  ...
  if ((type & Point::BlockTypes) || (type & Point::InsnTypes)) {
    ...
    std::map<PatchBlock *, BlockPoints>::iterator iter =
        blockPoints_.find(loc.block);
    if (iter == blockPoints_.end()) {
      if (!create) return NULL;
      BlockPoints bp;
      iter = blockPoints_.insert(blockPoints_.begin(),
                                 std::make_pair(loc.block, bp));
    }
    switch (type) {
    ...
    std::map<PatchEdge *, EdgePoints>::iterator iter =
        edgePoints_.find(loc.edge);
    if (iter == edgePoints_.end()) {
      if (!create) return NULL;
      EdgePoints ep;
      iter =
          edgePoints_.insert(edgePoints_.begin(), std::make_pair(loc.edge, ep));
    }
    if (!iter->second.during && create) {
      iter->second.during = maker->createPoint(loc, type);
    }
    ...
    switch (type) {
      ...
      case Point::FuncExit: {
        ...
        std::map<PatchBlock *, Point *>::iterator iter =
            points_.exits.find(loc.block);
      ...
      case Point::PreCall: {
        ...
        std::map<PatchBlock *, Point *>::iterator iter =
            points_.preCalls.find(loc.block);
      ...
      case Point::PostCall: {
        ...
        if (create && !loc.trusted && !verifyCall(loc.block)) return NULL;
        std::map<PatchBlock *, Point *>::iterator iter =
            points_.postCalls.find(loc.block);
...
bool PatchFunction::findInsnPoints(Point::Type type, PatchBlock *block,
...
void PatchFunction::destroyBlockPoints(PatchBlock *block) {
  ...
  if (points_.entry && points_.entry->block() == block) {
    cb->destroy(
        points_.entry);  // KEVINTEST: can you delete a function's entry point?
    points_.entry = NULL;
  }
  ...
  if (!bit->second.postInsn.empty()) {
    for (InsnPoints::iterator iit = bit->second.postInsn.begin();
         iit != bit->second.postInsn.end(); iit++) {
  ...
  if (!bit->second.preInsn.empty()) {
    for (InsnPoints::iterator iit = bit->second.preInsn.begin();
         iit != bit->second.preInsn.end(); iit++) {
      bit->first->remove(iit->second);
      cb->destroy(iit->second);
    }
...
PatchCallback *PatchFunction::cb() const { return obj_->cb(); }
...
bool PatchFunction::consistency() const {
  ...
  if (!all_blocks_.empty()) {
    if (all_blocks_.size() != func_->num_blocks()) {
      cerr << "Error: size mismatch in all_blocks; PatchAPI "
           << all_blocks_.size() << " and ParseAPI " << func_->num_blocks()
           << endl;
      CONSIST_FAIL;
    }
    for (Blockset::const_iterator iter = all_blocks_.begin();
      ...
      bool found = false;
      for (ParseAPI::Function::blocklist::iterator iter2 =
               func_->blocks().begin();
  ...
  if (!exit_blocks_.empty()) {
    for (Blockset::const_iterator iter = exit_blocks_.begin();
         iter != exit_blocks_.end(); ++iter) {
      bool found = false;
  ...
  if (!call_blocks_.empty()) {
    ...
    for (ParseAPI::Function::edgelist::iterator llit =
             func_->callEdges().begin();
  ...
  for (std::map<PatchBlock *, BlockPoints>::const_iterator iter =
           blockPoints_.begin();
  ...
  for (std::map<PatchEdge *, EdgePoints>::const_iterator iter =
           edgePoints_.begin();
       iter != edgePoints_.end(); ++iter) {
    if (!iter->second.consistency(iter->first, this)) CONSIST_FAIL;
  }
...
// set. not NULL.
void PatchFunction::getLoopsByNestingLevel(vector<PatchLoop *> &lbb,
                                           bool outerMostOnly) {
  if (_loop_analyzed == false) {
    createLoops();
    _loop_analyzed = true;
...
void PatchFunction::createLoops() {
  ...
  for (auto lit = loops.begin(); lit != loops.end(); ++lit) {
    ...
    // set parent pointer
    if (l->parentLoop() != NULL) pl->parent = _loop_map[l->parentLoop()];
...
void PatchFunction::getImmediateDominates(PatchBlock *A,
                                          set<PatchBlock *> &imd) {
  fillDominatorInfo();
...
void PatchFunction::getImmediatePostDominates(PatchBlock *A,
                                              set<PatchBlock *> &imd) {
  fillPostDominatorInfo();
