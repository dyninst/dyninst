...
AddrSpace* AddrSpace::create(PatchObject* obj) {
...
bool AddrSpace::write(PatchObject* /*obj*/, Address /*to*/, Address /*from*/,
                      size_t /*size*/) {
  return false;
}
...
bool AddrSpace::free(PatchObject* /*obj*/, Address /*orig*/) { return false; }
...
bool AddrSpace::consistency(const PatchMgr* m) const {
  ...
  for (ObjMap::const_iterator iter = obj_map_.begin(); iter != obj_map_.end();
       ++iter) {
    if (!iter->second->consistency(this)) {
      cerr << "Error: " << iter->second->format() << " failed consistency!"
           << endl;
      return false;
    }
  }
