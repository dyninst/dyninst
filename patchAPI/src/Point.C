...
InstancePtr Instance::create(Point *point, SnippetPtr snippet, SnippetType type,
                             SnippetState state) {
...
PatchFunction *Point::getCallee() {
  ...
  for (; it != b->targets().end(); ++it) {
    if ((*it)->type() == ParseAPI::CALL) {
      ...
      return obj()->getFunc(obj()->co()->findFuncByEntry(
          trg->block()->region(), trg->block()->start()));
    }
  }
  return NULL;
}
...
Point::Point(Point::Type type, PatchMgrPtr mgr, PatchBlock *b, Address a,
             InstructionAPI::Instruction::Ptr i, PatchFunction *f)
    : addr_(a),
      type_(type),
      mgr_(mgr),
      the_block_(b),
...
bool Point::remove(InstancePtr instance) {
  ...
  InstanceList::iterator it =
      std::find(instanceList_.begin(), instanceList_.end(), instance);
...
size_t Point::size() { return instanceList_.size(); }
...
BlockPoints::~BlockPoints() {
  ...
  if (during) delete during;
  if (exit) delete exit;
  for (InsnPoints::iterator iter = preInsn.begin(); iter != preInsn.end();
       ++iter) {
    delete iter->second;
  }
  for (InsnPoints::iterator iter = postInsn.begin(); iter != postInsn.end();
       ++iter) {
    delete iter->second;
  }
}
