...
InstancePtr
Instance::create(Point* point, SnippetPtr snippet, SnippetType type, SnippetState state)
{
...
Point::getCallee()
{
    ...
    for(; it != b->targets().end(); ++it)
    {
        if((*it)->type() == ParseAPI::CALL ||
           ((((*it)->type() == ParseAPI::DIRECT ||
              (*it)->type() == ParseAPI::COND_TAKEN)) &&
            (*it)->interproc()))
        {
...
void
Point::changeBlock(PatchBlock* block)
{
...
FuncPoints::~FuncPoints()
{
    ...
    if(during)
        delete during;
    for(std::map<PatchBlock*, Point*>::iterator iter = exits.begin(); iter != exits.end();
        ++iter)
    {
        delete iter->second;
    }
