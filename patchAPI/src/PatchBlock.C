...
const PatchBlock::edgelist &PatchBlock::sources() {
  if (srclist_.empty()) {
    for (ParseAPI::Block::edgelist::const_iterator iter =
             block_->sources().begin();
...
PatchEdge *PatchBlock::findSource(ParseAPI::EdgeTypeEnum type) {
  sources();
  for (edgelist::iterator iter = srclist_.begin(); iter != srclist_.end();
       ++iter) {
...
PatchEdge *PatchBlock::findTarget(ParseAPI::EdgeTypeEnum type) {
  targets();
  for (edgelist::iterator iter = trglist_.begin(); iter != trglist_.end();
       ++iter) {
    assert(*iter);
    assert((*iter)->edge());
    if ((*iter)->type() == type) return *iter;
...
void PatchBlock::removeSourceEdge(PatchEdge *e) {
  ...
  if ((iter = std::find(srclist_.begin(), srclist_.end(), e)) !=
      srclist_.end()) {
    srclist_.erase(iter);
  } else {
    cerr << "WARNING: failed to remove target edge from block [" << hex
         << start() << " " << end() << ") from " << e->src()->last() << dec
         << endl;
  }
...
void PatchBlock::removeTargetEdge(PatchEdge *e) {
  ...
  std::vector<PatchEdge *>::iterator iter;
  if ((iter = std::find(trglist_.begin(), trglist_.end(), e)) !=
      trglist_.end()) {
    trglist_.erase(iter);
  } else {
    cerr << "WARNING: failed to remove target edge from block [" << hex
         << start() << " " << end() << ") to " << e->src()->start() << dec
         << endl;
  }
  cb()->remove_edge(this, e, PatchCallback::target);
}

bool PatchBlock::isShared() { return containingFuncs() > 1; }
...
Address PatchBlock::size() const { return block_->size(); }

int PatchBlock::containingFuncs() const { return block_->containingFuncs(); }

int PatchBlock::numRetEdges() const {
  int numRets = 0;
...
std::string PatchBlock::disassemble() const {
  ...
  Insns instances;
  getInsns(instances);
  for (Insns::iterator iter = instances.begin(); iter != instances.end();
       ++iter) {
    ret << "\t" << hex << iter->first << ": " << iter->second->format() << dec
        << endl;
  }
  return ret.str();
}
...
std::string PatchBlock::long_format() const {
  stringstream ret;
  ret << format() << endl;
...
PatchFunction *PatchBlock::getFunction(ParseAPI::Function *f) {
  return obj_->getFunc(f);
}

ParseAPI::Block *PatchBlock::block() const { return block_; }

PatchObject *PatchBlock::object() const { return obj_; }

PatchFunction *PatchBlock::getCallee() {
...
PatchCallback *PatchBlock::cb() const { return obj_->cb(); }

void PatchBlock::splitBlock(PatchBlock *succ) {
  // Okay, get our edges right and stuff.
  // We want to modify when possible so that we keep Points on affected edges
  // the same.
  // Therefore:
  // 1) Incoming edges are unchanged.
  // 2) Outgoing edges from p1 are switched to p2 (except the fallthrough from
  // p1 to p2)
  // 3) Fallthrough edge from p1 to p2 added if it wasn't already (depends on
  // the status
  ...
  PatchEdge *patch_ft =
      obj()->getEdge(*block_->targets().begin(), this, succ, false);
...
bool PatchBlock::consistency() const {
  if (!block_) {
    cerr << "Error: block has no associated ParseAPI block, failed consistency"
         << endl;
    CONSIST_FAIL;
  }
  if (!srclist_.empty()) {
  ...
  if (!trglist_.empty()) {
    if (trglist_.size() != block_->targets().size()) {
      cerr << "Error: block at " << hex << block_->start() << dec
           << " has inconsistent targets size; ParseAPI "
           << block_->targets().size() << " and PatchAPI " << trglist_.size()
           << endl;
      CONSIST_FAIL;
    }
    set<PatchBlock *> trgs;
...
bool BlockPoints::consistency(const PatchBlock *b,
                              const PatchFunction *f) const {
  if (entry) {
  ...
  for (InsnPoints::const_iterator iter = preInsn.begin(); iter != preInsn.end();
       ++iter) {
  ...
  for (InsnPoints::const_iterator iter = postInsn.begin();
...
bool PatchBlock::wasUserAdded() const { return block_->wasUserAdded(); }
