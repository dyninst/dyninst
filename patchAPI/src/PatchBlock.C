...
PatchEdge*
PatchBlock::findSource(ParseAPI::EdgeTypeEnum type)
{
    sources();
    for(edgelist::iterator iter = srclist_.begin(); iter != srclist_.end(); ++iter)
    {
        if((*iter)->type() == type)
            return *iter;
    }
...
PatchEdge*
PatchBlock::findTarget(ParseAPI::EdgeTypeEnum type)
{
    targets();
    for(edgelist::iterator iter = trglist_.begin(); iter != trglist_.end(); ++iter)
    {
        assert(*iter);
        assert((*iter)->edge());
        if((*iter)->type() == type)
...
PatchBlock::removeSourceEdge(PatchEdge* e)
{
    ...
    {
        cerr << "WARNING: failed to remove target edge from block [" << hex << start()
             << " " << end() << ") from " << e->src()->last() << dec << endl;
    }
...
PatchBlock::numCallEdges() const
{
    ...
    for(Block::edgelist::const_iterator titer = trgs.begin(); titer != trgs.end();
        titer++)
    {
        if((*titer)->type() == CALL)
        {
...
PatchBlock::disassemble() const
{
    ...
    Insns        instances;
    getInsns(instances);
    for(Insns::iterator iter = instances.begin(); iter != instances.end(); ++iter)
    {
...
Point*
PatchBlock::findPoint(Location loc, Point::Type type, bool create)
{
...
PatchBlock::splitBlock(PatchBlock* succ)
{
    ...
    if(patch_ft)
    {  // patch_ft may have been created by another callback
...
bool
BlockPoints::consistency(const PatchBlock* b, const PatchFunction* f) const
{
    if(entry)
    {
        ...
        if(entry->func() != f)
            CONSIST_FAIL;
        if(entry->type() != Point::BlockEntry)
            CONSIST_FAIL;
    }
    if(during)
    {
        ...
        if(during->func() != f)
            CONSIST_FAIL;
        if(during->type() != Point::BlockDuring)
            CONSIST_FAIL;
    }
    if(exit)
    {
    ...
    for(InsnPoints::const_iterator iter = preInsn.begin(); iter != preInsn.end(); ++iter)
    {
        if(!iter->second->consistency())
            CONSIST_FAIL;
        if(iter->second->block() != b)
            CONSIST_FAIL;
        if(iter->second->func() != f)
            CONSIST_FAIL;
        if(iter->second->addr() != iter->first)
            CONSIST_FAIL;
        if(iter->second->type() != Point::PreInsn)
            CONSIST_FAIL;
        if(!b->getInsn(iter->first).isValid())
            CONSIST_FAIL;
    }
    for(InsnPoints::const_iterator iter = postInsn.begin(); iter != postInsn.end();
        ++iter)
    {
        if(!iter->second->consistency())
            CONSIST_FAIL;
        ...
        if(iter->second->addr() != iter->first)
            CONSIST_FAIL;
        if(iter->second->type() != Point::PostInsn)
            CONSIST_FAIL;
        if(!b->getInsn(iter->first).isValid())
            CONSIST_FAIL;
    }
