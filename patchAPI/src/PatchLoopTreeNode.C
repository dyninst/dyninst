...
PatchLoopTreeNode::PatchLoopTreeNode(
    PatchObject *obj, ParseAPI::LoopTreeNode *tree,
...
const char *PatchLoopTreeNode::getCalleeName(unsigned int i) {
  assert(i < callees.size());
  assert(callees[i] != NULL);
  return callees[i]->name().c_str();
}

const char *PatchLoopTreeNode::name() {
  assert(loop != NULL);
  return hierarchicalName;
}

unsigned int PatchLoopTreeNode::numCallees() { return callees.size(); }
...
PatchLoop *PatchLoopTreeNode::findLoop(const char *name) {
  if (loop) {
