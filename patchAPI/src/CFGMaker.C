...
PatchBlock* CFGMaker::makeBlock(ParseAPI::Block* b, PatchObject* obj) {
  ...
  if (!ret) {
    cerr << "ERROR: Cannot create PatchBlock for 0x" << std::hex << b->start()
         << "\n"
         << std::dec;
    assert(0);
  }
...
PatchEdge* CFGMaker::makeEdge(ParseAPI::Edge* e, PatchBlock* s, PatchBlock* t,
                              PatchObject* o) {
  return (new PatchEdge(e, s ? s : o->getBlock(e->src()),
                        t ? t : o->getBlock(e->trg())));
}

PatchEdge* CFGMaker::copyEdge(PatchEdge* e, PatchObject* o) {
  return (new PatchEdge(e, o->getBlock(e->src()->block()),
