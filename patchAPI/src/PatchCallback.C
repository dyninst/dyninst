...
PatchCallback::batch_end()
{
    ...
    for(std::vector<std::pair<PatchEdge*, PatchObject*>>::iterator iter =
            destroyedEdges_.begin();
...
void
PatchCallback::destroy(PatchBlock* b)
{
    if(batching_)
...
void
PatchCallback::destroy(PatchEdge* e, PatchObject* o)
{
    if(batching_)
    {
...
void
PatchCallback::destroy(PatchFunction* e)
{
    if(batching_)
        destroyedFuncs_.push_back(e);
    else
    {
        destroy_cb(e);
        delete e;
...
void
PatchCallback::destroy(PatchObject* e)
{
    if(batching_)
        destroyedObjects_.push_back(e);
    else
    {
        destroy_cb(e);
        delete e;
...
void
PatchCallback::create(PatchBlock* b)
{
    if(batching_)
        createdBlocks_.push_back(b);
    else
        create_cb(b);
}

void
PatchCallback::create(PatchEdge* b)
{
    if(batching_)
        createdEdges_.push_back(b);
    else
        create_cb(b);
}

void
PatchCallback::create(PatchFunction* b)
{
    if(batching_)
        createdFuncs_.push_back(b);
    else
        create_cb(b);
}

void
PatchCallback::create(PatchObject* b)
{
    if(batching_)
        createdObjects_.push_back(b);
    else
        create_cb(b);
}

void
PatchCallback::split_block(PatchBlock* a, PatchBlock* b)
{
    if(batching_)
        blockSplits_.push_back(std::make_pair(a, b));
    else
        split_block_cb(a, b);
}

void
PatchCallback::remove_edge(PatchBlock* a, PatchEdge* b, edge_type_t c)
{
    if(batching_)
        blockMods_.push_back(BlockMod(a, b, c, removed));
    else
        remove_edge_cb(a, b, c);
}

void
PatchCallback::add_edge(PatchBlock* a, PatchEdge* b, edge_type_t c)
{
    if(batching_)
        blockMods_.push_back(BlockMod(a, b, c, added));
    else
        add_edge_cb(a, b, c);
}

void
PatchCallback::remove_block(PatchFunction* a, PatchBlock* b)
{
    if(batching_)
        funcMods_.push_back(FuncMod(a, b, removed));
    else
        remove_block_cb(a, b);
}

void
PatchCallback::add_block(PatchFunction* a, PatchBlock* b)
{
    if(batching_)
        funcMods_.push_back(FuncMod(a, b, added));
    else
        add_block_cb(a, b);
}

void
PatchCallback::destroy(Point* e)
{
    if(batching_)
        destroyedPoints_.push_back(e);
    else
    {
        destroy_cb(e);
        delete e;
...
void
PatchCallback::create(Point* b)
{
    if(batching_)
        createdPoints_.push_back(b);
    else
        create_cb(b);
}

void
PatchCallback::change(Point* p, PatchBlock* ob, PatchBlock* nb)
{
    if(batching_)
        pointMods_.push_back(PointMod(p, ob, nb));
    else
        change_cb(p, ob, nb);
}
