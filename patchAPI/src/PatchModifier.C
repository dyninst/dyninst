...
PatchModifier::redirect(PatchEdge* edge, PatchBlock* target)
{
    ...
    if(edge->type() == ParseAPI::INDIRECT || edge->type() == ParseAPI::CATCH ||
       edge->type() == ParseAPI::RET)
        return false;
    ...
    if(!ParseAPI::CFGModifier::redirect(edge->edge(), llTrg))
        return false;
    ...
    */
    ...
    if(oldTrg)
    {
        assert(
            oldTrg->start() ==
                numeric_limits<Address>::max() ||  // don't check sink block's consistency
            oldTrg->consistency());
    }

    if(target)
    {  // otherwise we're redirecting to a sink edge and deleted
...
PatchBlock*
PatchModifier::split(PatchBlock* block, Address addr, bool trust, Address newlast)
{
    if(!trust)
    {
        if(!block->getInsn(addr).isValid())
            return NULL;
    ...
    ParseAPI::Block* split_int =
        ParseAPI::CFGModifier::split(block->block(), offset, true, newlast);
    if(!split_int)
        return NULL;
...
InsertedCode::Ptr
PatchModifier::insert(PatchObject* obj, void* start, unsigned size, Address base)
{
    ParseAPI::CodeObject* co = obj->co();

    ParseAPI::InsertedRegion* newRegion =
        ParseAPI::CFGModifier::insert(co, base, start, size);

    if(!newRegion)
        return InsertedCode::Ptr();
    ...
    std::queue<PatchBlock*> worklist;
    worklist.push(entry);
    while(!worklist.empty())
    {
        PatchBlock* cur = worklist.front();
        worklist.pop();
        assert(cur);

        if(ret->blocks_.find(cur) != ret->blocks_.end())
            continue;
...
InsertedCode::Ptr
PatchModifier::insert(PatchObject* obj, void* start, unsigned size)
{
    ParseAPI::CodeObject* co   = obj->co();
...
InsertedCode::Ptr
PatchModifier::insert(PatchObject* obj, SnippetPtr snip, Point* p)
{
    if(!snip)
        return InsertedCode::Ptr();
...
bool
PatchModifier::remove(vector<PatchBlock*>& blocks, bool force)
{
...
bool
PatchModifier::remove(PatchFunction* func)
{
