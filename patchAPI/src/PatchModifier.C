...
bool PatchModifier::redirect(PatchEdge *edge, PatchBlock *target) {
  ...
  */
  ...
  if (oldTrg) {
    assert(oldTrg->start() == numeric_limits<Address>::max() ||  // don't check
                                                                 // sink block's
                                                                 // consistency
           oldTrg->consistency());
  }
...
PatchBlock *PatchModifier::split(PatchBlock *block, Address addr, bool trust,
                                 Address newlast) {
  if (!trust) {
...
InsertedCode::Ptr PatchModifier::insert(PatchObject *obj, void *start,
                                        unsigned size, Address base) {
...
InsertedCode::Ptr PatchModifier::insert(PatchObject *obj, SnippetPtr snip,
                                        Point *p) {
...
bool PatchModifier::remove(vector<PatchBlock *> &blocks, bool force) {
  ...
  //// DEBUG  // this check came too early, we haven't updated other block
  ///points
