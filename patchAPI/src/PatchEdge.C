...
PatchEdge::PatchEdge(ParseAPI::Edge* internalEdge, PatchBlock* source, PatchBlock* target)
: edge_(internalEdge)
, src_(source)
, trg_(target)
{}
...
PatchEdge::consistency() const
{
    if(src_)
    {
        if(src_->block() != edge_->src())
            CONSIST_FAIL;
    }
    if(trg_)
    {
        if(trg_->block() != edge_->trg())
            CONSIST_FAIL;
    }
...
bool
EdgePoints::consistency(const PatchEdge* edge, const PatchFunction* func) const
{
    if(during)
    {
        if(!during->consistency())
            CONSIST_FAIL;
        if(during->type() != Point::EdgeDuring)
            CONSIST_FAIL;
        if(during->edge() != edge)
            CONSIST_FAIL;
        if(during->func() != func)
