...
PatchEdge *PatchEdge::create(ParseAPI::Edge *ie, PatchBlock *src,
                             PatchBlock *trg) {
  PatchObject *obj = (src ? src->object() : trg->object());
  return obj->getEdge(ie, src, trg);
}

PatchEdge::PatchEdge(ParseAPI::Edge *internalEdge, PatchBlock *source,
                     PatchBlock *target)
...
ParseAPI::Edge *PatchEdge::edge() const { return edge_; }

ParseAPI::EdgeTypeEnum PatchEdge::type() const { return edge_->type(); }

bool PatchEdge::sinkEdge() const { return edge_->sinkEdge(); }

bool PatchEdge::interproc() const {
  return edge_->interproc() || (edge_->type() == ParseAPI::CALL) ||
...
PatchCallback *PatchEdge::cb() const { return src_->object()->cb(); }
...
bool EdgePoints::consistency(const PatchEdge *edge,
                             const PatchFunction *func) const {
  if (during) {
