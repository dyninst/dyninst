...
Instrumenter *Instrumenter::create(AddrSpace *as) {
  Instrumenter *ret = new Instrumenter(as);
  return ret;
}

bool Instrumenter::replaceFunction(PatchFunction *oldfunc,
                                   PatchFunction *newfunc) {
  functionReplacements_[oldfunc] = newfunc;
  return true;
}

bool Instrumenter::revertReplacedFunction(PatchFunction *oldfunc) {
  functionReplacements_.erase(oldfunc);
  return true;
}

bool Instrumenter::wrapFunction(PatchFunction *oldfunc, PatchFunction *newfunc,
                                std::string name) {
  functionWraps_[oldfunc] = std::make_pair(newfunc, name);
  return true;
}

bool Instrumenter::revertWrappedFunction(PatchFunction *oldfunc) {
  functionWraps_.erase(oldfunc);
  return true;
}

bool Instrumenter::modifyCall(PatchBlock *callBlock, PatchFunction *newCallee,
                              PatchFunction *context) {
  callModifications_[callBlock][context] = newCallee;
  return true;
}

bool Instrumenter::revertModifiedCall(PatchBlock *callBlock,
                                      PatchFunction *context) {
...
bool Instrumenter::removeCall(PatchBlock *callBlock, PatchFunction *context) {
