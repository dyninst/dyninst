#ifndef DYN_DETAIL_BOOST_TT_IS_ABSTRACT_CLASS_HPP
#define DYN_DETAIL_BOOST_TT_IS_ABSTRACT_CLASS_HPP
...
//              and John Maddock.
//  - Dec 2004: Added new config macro DYN_DETAIL_BOOST_NO_IS_ABSTRACT which causes the template
//              to degrade gracefully, rather than trash the compiler (John Maddock).
//

#include <dyn_detail/boost/type_traits/intrinsics.hpp>
#ifndef DYN_DETAIL_BOOST_IS_ABSTRACT
...
#ifdef DYN_DETAIL_BOOST_NO_IS_ABSTRACT
#include <dyn_detail/boost/type_traits/is_polymorphic.hpp>
#endif
#endif
...
#ifdef DYN_DETAIL_BOOST_IS_ABSTRACT
template <class T>
struct is_abstract_imp
{
   DYN_DETAIL_BOOST_STATIC_CONSTANT(bool, value = DYN_DETAIL_BOOST_IS_ABSTRACT(T));
};
#elif !defined(DYN_DETAIL_BOOST_NO_IS_ABSTRACT)
template<class T>
struct is_abstract_imp2
{
   ...
   //
   DYN_DETAIL_BOOST_STATIC_ASSERT(sizeof(T) != 0);

   // GCC2 won't even parse this template if we embed the computation
   // of s1 in the computation of value.
#ifdef __GNUC__
   DYN_DETAIL_BOOST_STATIC_CONSTANT(unsigned, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if DYN_DETAIL_BOOST_WORKAROUND(_MSC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   DYN_DETAIL_BOOST_STATIC_CONSTANT(unsigned, s1 = sizeof(check_sig<T>(0)));
#if DYN_DETAIL_BOOST_WORKAROUND(_MSC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::dyn_detail::boost::is_class<T>::value> selector;
   typedef typename selector::template rebind<T> binder;
   typedef typename binder::type type;

   DYN_DETAIL_BOOST_STATIC_CONSTANT(bool, value = type::value);
};
...
DYN_DETAIL_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::dyn_detail::boost::detail::is_abstract_imp<T>::value)
#else
DYN_DETAIL_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::dyn_detail::boost::detail::is_polymorphic_imp<T>::value)
#endif
...
#endif //DYN_DETAIL_BOOST_TT_IS_ABSTRACT_CLASS_HPP
