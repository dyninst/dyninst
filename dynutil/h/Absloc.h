...
class Absloc {
 public:
  ...
  static Absloc makePC();
  static Absloc makeSP();
  static Absloc makeFP();
  ...
  Absloc(Type t,
    ...
    region_(0),
    func_() {
    // Stack types must have an attached BPatch_function
    assert(type_ != Stack);
  };
  Absloc(Type t,
	 long v,
  ...
  const long &loc() const { return loc_; };
 ...
 private:
  ...
  // Absolute address for heap locs
  // Offset from "entry of function" for stack locs
  // Register ID for register locs
  long loc_;
  // Stack frames can have multiple non-overlapping
  // regions. These are created when the stack pointer
  // is changed in a non-constant way.
  int region_;
