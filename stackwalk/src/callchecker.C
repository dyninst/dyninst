...
bool CallChecker::isPrevInstrACall(Address addr, Address &target) {
  ...
  if (buffer[max_call_length - 5] == 0xe8) {
    ...
    target = addr + disp;
    sw_printf(
        "[%s:%u] - Found call encoded by %x to %lx (addr = %lx, disp = %lx)\n",
  ...
  for (unsigned i = 0; i < max_call_length - 1; i++) {
    ...
    switch (modrm_mod) {
      case 0:
        if (modrm_rm == 5) size += 4;  // disp32
        if (modrm_rm == 4) size += 1;  // SIB
        break;
      case 1:
        size += 1;                     // disp8
        if (modrm_rm == 4) size += 1;  // SIB
    ...
    if (i + size == max_call_length) {
      sw_printf("[%s:%u] - Found call of size %d encoded by: ", FILE__,
                __LINE__, size);
