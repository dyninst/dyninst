...
ProcDebug *ProcDebug::newProcDebug(PID, std::string) {
  setLastError(err_unsupported,
               "Third party stackwalking not supported on "
               "this platform");
  return NULL;
}
...
int ProcDebug::getNotificationFD() { return -1; }

ThreadState *ThreadState::createThreadState(ProcDebug *parent, Dyninst::THR_ID,
                                            bool) {
...
void int_walkerSet::initProcSet() { procset = NULL; }
