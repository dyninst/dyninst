...
SymbolReaderFactory*
Dyninst::Stackwalker::getDefaultSymbolReader()
{
#if defined(WITH_SYMLITE)
...
static void
registerLibSpotterSelf(ProcSelf* pself);
...
bool
ProcSelf::readMem(void* dest, Address source, size_t size)
{
...
bool
ProcSelf::getThreadIds(std::vector<THR_ID>& threads)
{
    bool   result;
    THR_ID tid;
...
bool
ProcSelf::getDefaultThread(THR_ID& default_tid)
{
    THR_ID tid = P_gettid();
    if(tid <= 0)
    {
...
registerLibSpotterSelf(ProcSelf* pself)
{
    ...
    {
        if(maps[i].start <= lib_trap_addr_self && maps[i].end > lib_trap_addr_self)
        {
            found = true;
            ...
            if(first_page + size < lib_trap_addr_self + MAX_TRAP_LEN)
                size += pgsize;
            int result =
                mprotect((void*) first_page, size, PROT_READ | PROT_WRITE | PROT_EXEC);
            if(result == -1)
            {
                int errnum = errno;
...
void
BottomOfStackStepperImpl::newLibraryNotification(LibAddrPair*, lib_change_t change)
{
...
void
BottomOfStackStepperImpl::initialize()
{
    ...
    if(!aout_init)
    {
        ...
        if(aout)
        {
            ...
            if(aout->isValidSymbol(start_sym))
            {
                ...
                {
                    sw_printf(
                        "[%s:%d] - %s symbol has 0 length, using length heuristics\n",
                    ...
                    else
                    {
                        for(int i = 0; i < heuristic_length; ++i)
                        {
                            ...
                            if(result && byteValue != expectedValue)
                                continue;
                            break;
                        }
                    }
...
unsigned
SigHandlerStepperImpl::getPriority() const
{
...
void
SigHandlerStepperImpl::newLibraryNotification(LibAddrPair*, lib_change_t change)
{
    if(change == library_unload)
        return;
