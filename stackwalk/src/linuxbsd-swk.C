...
void handle_fault(int /*sig*/) {
  if (!reading_memory) {
    ...
    fprintf(stderr,
            "[%s:%u] - Caught segfault that didn't come "
            "from stackwalker memory read!",
            FILE__, __LINE__);
...
static void lib_trap_handler(int /*sig*/) { local_lib_state->notifyOfUpdate(); }

static Address lib_trap_addr_self = 0x0;
static bool lib_trap_addr_self_err = false;
static void registerLibSpotterSelf(ProcSelf *pself) {
  if (lib_trap_addr_self) return;
  if (lib_trap_addr_self_err) return;

  // Get the address to install a trap to
  LibraryState *libs = pself->getLibraryTracker();
  if (!libs) {
    sw_printf(
        "[%s:%u] - Not using lib tracker, don't know how "
  ...
  for (unsigned i = 0; i < maps_size; i++) {
    if (maps[i].start <= lib_trap_addr_self &&
      ...
      if (first_page + size < lib_trap_addr_self + MAX_TRAP_LEN) size += pgsize;
      int result = mprotect((void *)first_page, size,
                            PROT_READ | PROT_WRITE | PROT_EXEC);
      if (result == -1) {
        int errnum = errno;
        sw_printf(
            "[%s:%u] - Error setting premissions for page containing %lx. "
            "Can't install lib tracker: %s\n",
            FILE__, __LINE__, lib_trap_addr_self, strerror(errnum));
        free(maps);
        lib_trap_addr_self_err = true;
        return;
      }
  ...
  if (!found) {
    sw_printf(
        "[%s:%u] - Couldn't find page containing %lx.  Can't install lib "
        "tracker.",
...
void BottomOfStackStepperImpl::initialize() {
  ...
  if (!libs) {
    sw_printf(
        "[%s:%u] - Error initing StackBottom.  No library state for process.\n",
        FILE__, __LINE__);
    return;
  }
    ...
    if (aout) {
      ...
      if (aout->isValidSymbol(start_sym)) {
        Dyninst::Address start =
            aout->getSymbolOffset(start_sym) + aout_addr.second;
  ...
  if (!libthread_init) {
    ...
    if (libthread) {
      ...
      if (libthread->isValidSymbol(clone_sym)) {
        Dyninst::Address start =
            libthread->getSymbolOffset(clone_sym) + libthread_addr.second;
      ...
      if (libthread->isValidSymbol(startthread_sym)) {
        Dyninst::Address start =
            libthread->getSymbolOffset(startthread_sym) + libthread_addr.second;
        Dyninst::Address end =
            libthread->getSymbolSize(startthread_sym) + start;
        sw_printf(
            "[%s:%u] - Bottom stepper taking %lx to %lx for start_thread\n",
            FILE__, __LINE__, start, end);
        ra_stack_tops.push_back(std::pair<Address, Address>(start, end));
      }
    }
...
SigHandlerStepperImpl::SigHandlerStepperImpl(Walker *w,
                                             SigHandlerStepper *parent)
    : FrameStepper(w),
...
void SigHandlerStepperImpl::newLibraryNotification(LibAddrPair *,
                                                   lib_change_t change) {
  if (change == library_unload) return;
