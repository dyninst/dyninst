...
sigframe_offsets frames_32[] = {
    {28, 60, 32, 728}, {28, 60, 32, 736}, {28, 60, 32, 0}};
...
gcframe_ret_t SigHandlerStepperImpl::getCallerFrame(const Frame &in,
  ...
  for (unsigned i = 0; i < frames_size; i++) {
    ...
    if (last_read_sp_addr != sp_loc.val.addr) {
      ...
      if (!result) {
        sw_printf(
            "[%s:%u] Unexpected error reading from stack memory 0x%lx for "
            "signal frame\n",
            FILE__, __LINE__);
        return gcf_error;
      }
    ...
    if (frames[i].frame_size && (sp != in.getSP() + frames[i].frame_size)) {
      sw_printf(
          "[%s:%u] - Signal frame candidate %d does not fit (%lx != %lx). "
          "Trying another.\n",
    ...
    if (!result) {
      sw_printf(
          "[%s:%u] Unexpected error reading from stack memory 0x%lx for signal "
          "frame\n",
          FILE__, __LINE__);
      return gcf_error;
    }
