...
ProcDebug*
ProcDebug::newProcDebug(PID pid, std::string executable)
{
...
ProcDebug*
ProcDebug::newProcDebug(Dyninst::ProcControlAPI::Process::ptr proc)
{
...
bool
ProcDebug::newProcDebugSet(const std::vector<PID>& pids, std::vector<ProcDebug*>& out_set)
{
...
ProcDebug*
ProcDebug::newProcDebug(std::string executable, const std::vector<std::string>& argv)
{
...
#define CHECK_PROC_LIVE_RET(val)                                                         \
    ...
    {                                                                                    \
        ...
        {                                                                                \
            ...
            Stackwalker::setLastError(err_procexit,                                      \
                                      "Process has exited or been detached");            \
            return (val);                                                                \
        }                                                                                \
    } while(0)
...
bool
ProcDebug::getRegValue(MachRegister reg, THR_ID thread, MachRegisterVal& val)
{
    CHECK_PROC_LIVE;
    if(reg == FrameBase)
    {
...
bool
ProcDebug::readMem(void* dest, Address source, size_t size)
{
    CHECK_PROC_LIVE;
...
bool
ProcDebug::getThreadIds(std::vector<THR_ID>& thrds)
{
    CHECK_PROC_LIVE;
...
bool
ProcDebug::getDefaultThread(THR_ID& default_tid)
{
    CHECK_PROC_LIVE;
...
bool
ProcDebug::preStackwalk(THR_ID tid)
{
    CHECK_PROC_LIVE;
    if(tid == NULL_THR_ID)
    ...
    if(active_thread->isRunning())
    {
        ...
        if(!result)
        {
            ...
            Stackwalker::setLastError(err_proccontrol,
                                      "Could not stop thread for stackwalk\n");
            return false;
        }
...
bool
ProcDebug::postStackwalk(THR_ID tid)
{
    CHECK_PROC_LIVE;
    if(tid == NULL_THR_ID)
        getDefaultThread(tid);
...
bool
ProcDebug::handleDebugEvent(bool block)
{
...
bool
PCLibraryState::cacheLibraryRanges(Library::ptr lib)
{
    ...
    for(int i = 0; i < num_segments; i++)
    {
        ...
        loadedLibs.insert(
            ...
            makeCache(LibAddrPair(lib->getName(), lib->getLoadAddress()), lib));
    }
    return true;
}

bool
PCLibraryState::findInCache(Process::ptr proc, Address addr, LibAddrPair& lib)
{
    cache_t tmp;
...
void
PCLibraryState::removeLibFromCache(cache_t element)
{
...
bool
PCLibraryState::checkLibraryContains(Address addr, Library::ptr lib)
{
...
void
PCLibraryState::checkForNewLib(Library::ptr lib)
{
    if(lib->getData())
        return;
    sw_printf("[%s:%d] - Detected new library %s at %lx, notifying\n", FILE__, __LINE__,
              lib->getName().c_str(), lib->getLoadAddress());
...
bool
PCLibraryState::getLibraryAtAddr(Address addr, LibAddrPair& lib)
{
...
bool
PCLibraryState::memoryScan(Process::ptr proc, Address addr, LibAddrPair& lib)
{
...
bool
PCLibraryState::getLibraries(std::vector<LibAddrPair>& libs, bool allow_refresh)
{
    Process::ptr proc = pdebug->getProc();
    CHECK_PROC_LIVE;
...
bool
PCLibraryState::updateLibraries()
{
    Process::ptr proc = pdebug->getProc();
    CHECK_PROC_LIVE;
...
void
PCLibraryState::notifyOfUpdate()
{}

Address
PCLibraryState::getLibTrapAddress()
{
    return 0;
}

bool
PCLibraryState::getAOut(LibAddrPair& ao)
{
    Process::ptr proc = pdebug->getProc();
    CHECK_PROC_LIVE;
...
void
int_walkerSet::addToProcSet(ProcDebug* pd)
{
...
void
int_walkerSet::eraseFromProcSet(ProcDebug* pd)
{
    ProcessSet::ptr& pset = *((ProcessSet::ptr*) procset);
    Process::ptr     proc = pd->getProc();
...
public:
    ...
    virtual bool addStackFrame(Thread::ptr thr, Dyninst::Address ra, Dyninst::Address sp,
                               Dyninst::Address fp);
...
bool
StackCallback::beginStackWalk(Thread::ptr thr)
{
    assert(!cur);
    ...
    {
        sw_printf(
            "[%s:%d] - Error, unknown process state for %d while starting stackwalk\n",
            FILE__, __LINE__, proc->getPid());
        return false;
    }
...
bool
StackCallback::addStackFrame(Thread::ptr thr, Dyninst::Address ra, Dyninst::Address sp,
                             Dyninst::Address fp)
{
    Frame f(cur_walker);
    f.setRA(ra);
...
void
StackCallback::endStackWalk(Thread::ptr thr)
{
...
bool
int_walkerSet::walkStacksProcSet(CallTree& tree, bool& bad_plat, bool walk_initial_only)
{
    ProcessSet::ptr& pset        = *((ProcessSet::ptr*) procset);
