...
class PCLibraryState : public LibraryState {
 private:
  ...
  cache_t makeCache(LibAddrPair a, Library::ptr b) {
...
ProcDebug::~ProcDebug() {
  if (library_tracker) delete library_tracker;
...
bool ProcDebug::preStackwalk(THR_ID tid) {
  CHECK_PROC_LIVE;
  if (tid == NULL_THR_ID) getDefaultThread(tid);
...
bool ProcDebug::isTerminated() { return (!proc || proc->isTerminated()); }
...
bool ProcDebug::isFirstParty() { return false; }

Architecture ProcDebug::getArchitecture() { return proc->getArchitecture(); }

Process::ptr ProcDebug::getProc() { return proc; }

PCLibraryState::PCLibraryState(ProcessState *pd) : LibraryState(pd) {
...
bool PCLibraryState::findInCache(Process::ptr proc, Address addr,
                                 LibAddrPair &lib) {
  cache_t tmp;
...
bool PCLibraryState::memoryScan(Process::ptr proc, Address addr,
                                LibAddrPair &lib) {
  ...
  /**
   * Check if predessor contains our address first--this should be the typical
   *case
   **/
...
bool PCLibraryState::getLibraries(std::vector<LibAddrPair> &libs,
  ...
  LibraryPool::iterator i;
  for (i = proc->libraries().begin(); i != proc->libraries().end(); i++) {
    if (allow_refresh) checkForNewLib(*i);
...
Address PCLibraryState::getLibTrapAddress() { return 0; }
...
class StackCallback : public Dyninst::ProcControlAPI::CallStackCallback {
 ...
 public:
  ...
  virtual bool addStackFrame(Thread::ptr thr, Dyninst::Address ra,
                             Dyninst::Address sp, Dyninst::Address fp);
  virtual void endStackWalk(Thread::ptr thr);
};

StackCallback::StackCallback(CallTree &t)
    : tree(t), cur(NULL), cur_walker(NULL) {
  top_first = true;
}
...
bool StackCallback::beginStackWalk(Thread::ptr thr) {
  ...
  if (!pstate) {
    sw_printf(
        "[%s:%u] - Error, unknown process state for %d while starting "
        "stackwalk\n",
...
bool StackCallback::addStackFrame(Thread::ptr thr, Dyninst::Address ra,
                                  Dyninst::Address sp, Dyninst::Address fp) {
  Frame f(cur_walker);
  f.setRA(ra);
  f.setSP(sp);
