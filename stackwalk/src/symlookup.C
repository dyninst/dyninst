...
SymbolLookup::SymbolLookup(std::string exec_path)
    : walker(NULL), executable_path(exec_path) {
...
SwkSymtab::SwkSymtab(std::string s) : SymbolLookup(s) { assert(0); }

bool SwkSymtab::lookupAtAddr(Dyninst::Address, std::string &, void *&) {
...
SymDefaultLookup::SymDefaultLookup(std::string s) : SymbolLookup(s) {}
