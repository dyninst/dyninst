...
static DwarfFrameParser::Ptr
getAuxDwarfInfo(std::string s)
{
    ...
    DwarfFrameParser::Ptr dresult =
        DwarfFrameParser::create(*dwarf->frame_dbg(), dwarf->origFile()->e_elfp(), arch);
    if(!dresult)
        return NULL;
...
bool
DebugStepperImpl::ReadMem(Address addr, void* buffer, unsigned size)
{
...
location_t
DebugStepperImpl::getLastComputedLocation(unsigned long value)
{
    location_t loc;
...
bool
DebugStepperImpl::GetReg(MachRegister reg, MachRegisterVal& val)
{
...
#    if defined(arch_aarch64)
        ...
        {
            sw_printf(
                "Cast framestepper %p for frame %p to SigHandlerStepper at address %lx\n",
                (void*) prevDepthFrame->getStepper(), (const void*) prevDepthFrame,
                prevDepthFrame->getRA());

            SigHandlerStepper* ss =
                dynamic_cast<SigHandlerStepper*>(prevDepthFrame->getStepper());
            if(ss != NULL)
            {
                sw_printf("[%s:%d] - Not the first frame, cannot find dbg information, "
                          "and previous frame is a signal trampoline frame. Try to get "
                          "x30 from ucontext as RA\n",
                          FILE__, __LINE__);
                static ucontext_t dummy_context;
                static int lr_offset = (char*) &(dummy_context.uc_mcontext.regs[30]) -
                                       (char*) &dummy_context;
                // This assumes that a ucontext_t is at the following offset from the top
                // of the signal handler's stack.
                ...
                {
                    sw_printf("[%s:%d] - the signal trampoline frame does not have a "
                              "parent frame\n",
                              FILE__, __LINE__);
                }
            }
...
gcframe_ret_t
DebugStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    LibAddrPair lib;
    bool        result;
    ...
    {
        result =
            getProcessState()->getLibraryTracker()->getLibraryAtAddr(out.getRA(), lib);
        if(result)
        {
    ...
    if(!result)
    {
        sw_printf("[%s:%d] - Stackwalking with PC at %lx, which is not found in any "
                  "known libraries\n",
                  FILE__, __LINE__, in.getRA());
        return gcf_not_me;
    }
    Address pc = in.getRA() - lib.second;
    sw_printf("[%s:%d] Dwarf-based stackwalking, using local address 0x%lx from 0x%lx - "
              "0x%lx\n",
...
void
DebugStepperImpl::registerStepperGroup(StepperGroup* group)
{
...
unsigned
DebugStepperImpl::getPriority() const
{
...
#if defined(arch_x86) || defined(arch_x86_64)
gcframe_ret_t
DebugStepperImpl::getCallerFrameArch(Address pc, const Frame& in, Frame& out,
                                     DwarfFrameParser::Ptr dinfo, bool isVsyscallPage)
{
    ...
    result =
        dinfo->getRegValueAtFrame(pc, Dyninst::ReturnAddr, ret_value, this, frame_error);
    ...
    if(!result && frame_error == FE_No_Frame_Entry && isVsyscallPage)
    {
        ...
        // old PC.
        pc += 0xffffe000;
        result = dinfo->getRegValueAtFrame(pc, Dyninst::ReturnAddr, ret_value, this,
                                           frame_error);
    }
    if(!result)
    {
        sw_printf("[%s:%d] - Couldn't get return debug info at %lx, error: %d\n", FILE__,
                  __LINE__, in.getRA(), frame_error);
        return gcf_not_me;
    }
    ...
    result = dinfo->getRegValueAtFrame(pc, frame_reg, frame_value, this, frame_error);
    if(!result)
    {
    ...
    result =
        dinfo->getRegValueAtFrame(pc, Dyninst::FrameBase, stack_value, this, frame_error);
    if(!result)
    {
...
#    endif
    ...
    if(ra_loc.val.addr > MAX_ADDR || fp_loc.val.addr > MAX_ADDR ||
       sp_loc.val.addr > MAX_ADDR)
        return gcf_not_me;
                                     ...
                                     DwarfFrameParser::Ptr dinfo, bool isVsyscallPage)
{
    ...
    result = dinfo->getRegValueAtFrame(
        pc, Dyninst::ReturnAddr,
