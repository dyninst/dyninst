...
static DwarfFrameParser::Ptr getAuxDwarfInfo(std::string s) {
  ...
  std::map<std::string, DwarfFrameParser::Ptr>::iterator i =
      dwarf_aux_info.find(s);
  if (i != dwarf_aux_info.end()) return i->second;
  ...
  DwarfFrameParser::Ptr dresult =
      DwarfFrameParser::create(*dwarf->frame_dbg(), arch);
...
bool DebugStepperImpl::ReadMem(Address addr, void *buffer, unsigned size) {
  ...
  if (size != addr_width) return false;
...
gcframe_ret_t DebugStepperImpl::getCallerFrame(const Frame &in, Frame &out) {
  ...
  if (lookupInCache(in, out)) {
    result = getProcessState()->getLibraryTracker()->getLibraryAtAddr(
        out.getRA(), lib);
    if (result) {
  ...
  sw_printf(
      "[%s:%u] Dwarf-based stackwalking, using local address 0x%lx from 0x%lx "
      "- 0x%lx\n",
  ...
  sw_printf("[%s:%u] - Using DWARF debug file info for %s\n", FILE__, __LINE__,
            lib.first.c_str());
  cur_frame = &in;
  gcframe_ret_t gcresult =
      getCallerFrameArch(pc, in, out, dauxinfo, isVsyscallPage);
  cur_frame = NULL;

  result = getProcessState()->getLibraryTracker()->getLibraryAtAddr(out.getRA(),
                                                                    lib);
...
void DebugStepperImpl::registerStepperGroup(StepperGroup *group) {
  ...
  if (addr_width == 4) group->addStepper(parent_stepper, 0, 0xffffffff);
...
unsigned DebugStepperImpl::getPriority() const { return debugstepper_priority; }
...
gcframe_ret_t DebugStepperImpl::getCallerFrameArch(Address pc, const Frame &in,
  ...
  result =
      dinfo->getRegValueAtFrame(pc, frame_reg, frame_value, this, frame_error);
  if (!result) {
  ...
  result = dinfo->getRegValueAtFrame(pc, Dyninst::FrameBase, stack_value, this,
                                     frame_error);
  if (!result) {
