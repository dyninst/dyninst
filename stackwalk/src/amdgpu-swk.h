#ifndef amdgpu_swk_h_
#define amdgpu_swk_h_
...
class amdgpu_LookupFuncStart : public FrameFuncHelper
{
private:
   static std::map<Dyninst::PID, amdgpu_LookupFuncStart*> all_func_starts;
   amdgpu_LookupFuncStart(ProcessState *proc_);
   int ref_count;
...
public:
   static amdgpu_LookupFuncStart *getLookupFuncStart(ProcessState *p);
   void releaseMe();
   virtual FrameFuncHelper::alloc_frame_t allocatesFrame(Address addr);
   ~amdgpu_LookupFuncStart();
