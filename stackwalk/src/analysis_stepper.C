...
AnalysisStepperImpl::~AnalysisStepperImpl() { delete callchecker; }

#if defined(WITH_SYMLITE)
CodeSource*
AnalysisStepperImpl::getCodeSource(std::string name)
{
...
CodeObject*
AnalysisStepperImpl::getCodeObject(string name)
{
...
gcframe_ret_t
AnalysisStepperImpl::getCallerFrameArch(set<height_pair_t> heights, const Frame& in,
                                        Frame& out)
{
    ...
    for(heightIter = heights.begin(); heightIter != heights.end(); ++heightIter)
    {
        ...
        Address in_sp = in.getSP(), in_fp = in.getFP(), out_sp = 0, out_ra = 0,
                out_fp = 0, out_fp_addr = 0;
        ...
        if(!getOutRA(out_sp, out_ra, out_ra_loc, proc))
            continue;

        // If we have multiple potential heights (due to overlapping functions),
        // check if potential stack height is valid (verify that calculated RA follows a
        // call instr)
        if(heights.size() > 1)
        {
            if(!validateRA(out_ra))
                continue;
        }
        ...
        if(out_fp_addr)
        {
            ...
            {
                sw_printf("[%s:%d] - Warning - current FP %lx does not point to next FP "
                          "located at %lx\n",
...
CodeRegion*
AnalysisStepperImpl::getCodeRegion(std::string name, Offset off)
{
...
std::set<AnalysisStepperImpl::height_pair_t>
AnalysisStepperImpl::analyzeFunction(string name, Offset callSite)
{
    ...
    for(auto i = func->blocks().begin(); i != func->blocks().end(); ++i)
    {
    ...
    if(blocks.size() == 0)
    {
        sw_printf("[%s:%d] - Function at entry point %lx did not contain call site %lx\n",
                  FILE__, __LINE__, entry_addr, callSite);
        return err_heights_pair;
    }
    ...
    heights.insert(height_pair_t(analysis.findSP(block, callSite),
                                 analysis.findFP(block, callSite)));

    sw_printf("[%s:%d] - Have %lu possible stack heights in %s at %lx:\n", FILE__,
              __LINE__, heights.size(), name.c_str(), callSite);
    for(set<height_pair_t>::iterator i = heights.begin(); i != heights.end(); i++)
    {
...
gcframe_ret_t
AnalysisStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    ...
    {
        vector<registerState_t> all_defined_heights =
            fullAnalyzeFunction(name, function_offset);
        if(!all_defined_heights.empty())
        {
            ret = getFirstCallerFrameArch(all_defined_heights, in, out);
        }
    }
    // PGCC can confuse our analysis by popping the RA into a GPR and then storing it
    // somewhere completely different. This shows up as a stack height of zero in our
    // analysis. Fixing the infinite loop here.
    if(in.getRA() == out.getRA())
        return gcf_not_me;
...
unsigned
AnalysisStepperImpl::getPriority() const
{
    return analysis_priority;
}

bool
AnalysisStepperImpl::isPrevInstrACall(Address addr, Address& target)
{
    return callchecker->isPrevInstrACall(addr, target);
}

std::vector<AnalysisStepperImpl::registerState_t>
AnalysisStepperImpl::fullAnalyzeFunction(std::string name, Offset callSite)
{
...
gcframe_ret_t
AnalysisStepperImpl::getFirstCallerFrameArch(const std::vector<registerState_t>& heights,
                                             const Frame& in, Frame& out)
{
    ...
    for(heightIter = heights.begin(); heightIter != heights.end(); ++heightIter)
    {
        ...
        if(heightIter->first.type() != Absloc::Register)
            continue;
        ...
        if(heightIter->second.height() == -1 * (long) proc->getAddressWidth())
        {
            ...
            sw_printf("[%s:%d] - Found candidate FP %s, height 0x%lx\n", __FILE__,
                      __LINE__, heightIter->first.format().c_str(),
                      (unsigned long) heightIter->second.height());
        }
...
bool
AnalysisStepperImpl::validateRA(Address candidateRA)
{
...
gcframe_ret_t
AnalysisStepperImpl::checkResult(bool result)
{
    if(result)
    {
...
bool
AnalysisStepperImpl::getOutRA(Address out_sp, Address& out_ra, location_t& out_ra_loc,
                              ProcessState* proc)
{
    // Since we know the outgoing SP,
    // the outgoing RA must be located just below it
