...
gcframe_ret_t AnalysisStepperImpl::getCallerFrameArch(
    set<height_pair_t> heights, const Frame &in, Frame &out) {
  ProcessState *proc = getProcessState();

  bool result = false;

  set<height_pair_t>::iterator heightIter;
  for (heightIter = heights.begin(); heightIter != heights.end();
       ++heightIter) {
    ...
    // check if potential stack height is valid (verify that calculated RA
    // follows a call instr)
    if (heights.size() > 1) {
    ...
    if (out_fp_addr) {
      if (out_fp_addr != in_fp) {
        sw_printf(
            "[%s:%u] - Warning - current FP %lx does not point to next FP "
            "located at %lx\n",
            FILE__, __LINE__, in_fp, out_fp_addr);
      }
      bool resultMem =
          proc->readMem(&out_fp, out_fp_addr, proc->getAddressWidth());
      if (resultMem) {
...
AnalysisStepperImpl::analyzeFunction(string name, Offset callSite) {
  ...
  if (blocks.size() == 0) {
    sw_printf(
        "[%s:%u] - Function at entry point %lx did not contain call site %lx\n",
  ...
  for (set<height_pair_t>::iterator i = heights.begin(); i != heights.end();
       i++) {
...
gcframe_ret_t AnalysisStepperImpl::getCallerFrame(const Frame &in, Frame &out) {
  ...
  // PGCC can confuse our analysis by popping the RA into a GPR and then storing
  // it somewhere completely
...
unsigned AnalysisStepperImpl::getPriority() const { return analysis_priority; }
...
AnalysisStepperImpl::fullAnalyzeFunction(std::string name, Offset callSite) {
  ...
  for (set<ParseAPI::Function *>::iterator i = funcs.begin(); i != funcs.end();
       i++) {
...
gcframe_ret_t AnalysisStepperImpl::getFirstCallerFrameArch(
    const std::vector<registerState_t> &heights, const Frame &in, Frame &out) {
  ProcessState *proc = getProcessState();
...
bool AnalysisStepperImpl::validateRA(Address candidateRA) {
  ...
  if (!isPrevInstrACall(candidateRA, target)) {
    sw_printf(
        "[%s:%u] - Return location %lx does not follow a call instruction\n",
...
bool AnalysisStepperImpl::getOutRA(Address out_sp, Address &out_ra,
                                   location_t &out_ra_loc, ProcessState *proc) {
  // Since we know the outgoing SP,
  // the outgoing RA must be located just below it
