...
, helper(helper_)
{
    helper = helper_ ? helper_
                     : aarch64_LookupFuncStart::getLookupFuncStart(getProcessState());
}
...
FrameFuncStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    ...
    if(alloc_frame.first != FrameFuncHelper::standard_frame)
    {
        sw_printf("[%s:%d] - alloc_frame.first!=standard_frame (== %x)\n", FILE__,
                  __LINE__, (unsigned int) alloc_frame.first);
    ...
    {
        result =
            getProcessState()->readMem(&this_frame_pair, in_fp, sizeof(this_frame_pair));
    }
    else
    {
        assert(0);
    ...
    if(FrameFuncHelper::unset_frame == alloc_frame.second)
    {
        ...
        else
        {
            ...
            assert(0);
            // result = getProcessState()->getRegValue(aarch32::lr, in.getThread(),
            // out_ra);
        }
        if(!result)
        {
...
bool
WandererHelper::isPrevInstrACall(Address, Address&)
{
...
gcframe_ret_t
DyninstInstrStepperImpl::getCallerFrameArch(const Frame& /*in*/, Frame& /*out*/,
                                              ...
                                              bool)
{
    ...
    result = getProcessState()->readMem(&ra_fp_pair, out_fp_loc, sizeof(ra_fp_pair));
    if(!result)
    {
...
aarch64_LookupFuncStart::getLookupFuncStart(ProcessState* p)
{
    Dyninst::PID                                               pid = p->getProcessId();
    std::map<Dyninst::PID, aarch64_LookupFuncStart*>::iterator i =
        all_func_starts.find(pid);
...
void
aarch64_LookupFuncStart::releaseMe()
{
    ref_count--;
    if(!ref_count)
        delete this;
...
aarch64_LookupFuncStart::allocatesFrame(Address addr)
{
    LibAddrPair   lib;
...
void
aarch64_LookupFuncStart::updateCache(Address addr, alloc_frame_t result)
{
    cache.insert(addr, result);
}

bool
aarch64_LookupFuncStart::checkCache(Address addr, alloc_frame_t& result)
{
