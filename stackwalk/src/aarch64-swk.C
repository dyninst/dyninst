...
Dyninst::Architecture ProcSelf::getArchitecture() { return Arch_aarch64; }
...
gcframe_ret_t FrameFuncStepperImpl::getCallerFrame(const Frame &in,
  ...
  if (FrameFuncHelper::unset_frame == alloc_frame.second) {
    ...
    if (sizeof(uint64_t) == addrWidth) {
      result =
          getProcessState()->getRegValue(aarch64::x30, in.getThread(), out_ra);
    } else {
...
FrameFuncStepperImpl::~FrameFuncStepperImpl() {
  aarch64_LookupFuncStart *lookup =
      dynamic_cast<aarch64_LookupFuncStart *>(helper);
  if (lookup)
    lookup->releaseMe();
  else if (helper)
    delete helper;
...
gcframe_ret_t DyninstDynamicStepperImpl::getCallerFrameArch(
  ...
  if (sizeof(uint64_t) == addrWidth) {
    result = getProcessState()->readMem(&ra_fp_pair, in_fp, sizeof(ra_fp_pair));
  } else {
    // aarch32 is not supported now
    assert(0);
  }
...
std::map<Dyninst::PID, aarch64_LookupFuncStart *>
    aarch64_LookupFuncStart::all_func_starts;
...
aarch64_LookupFuncStart *aarch64_LookupFuncStart::getLookupFuncStart(
    ProcessState *p) {
...
FrameFuncHelper::alloc_frame_t aarch64_LookupFuncStart::allocatesFrame(
    Address addr) {
  LibAddrPair lib;
