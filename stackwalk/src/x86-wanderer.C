...
unsigned StepperWandererImpl::getPriority() const { return wanderer_priority; }
    ...
    if (whelper->isPrevInstrACall(word, target)) {
      ...
      switch (pcs) {
        ...
        case WandererHelper::unknown_s:
          sw_printf(
              "[%s:%u] - Wanderer added word 0x%lx at 0x%lx as candidate "
              "return "
              " address\n",
              FILE__, __LINE__, word, current_stack);
        ...
        case WandererHelper::in_func:
          sw_printf(
              "[%s:%u] - Wanderer thinks word 0x%lx at 0x%lx  is return "
              " address\n",
              FILE__, __LINE__, word, current_stack);
...
WandererHelper::pc_state WandererHelper::isPCInFunc(Address func_entry,
                                                    Address pc) {
  if (!func_entry || !pc) return unknown_s;
  if (func_entry == pc) return in_func;
...
#define MAX_PLT32_IDIOM_SIZE 6
      ...
      if (buffer[0] == 0xff && buffer[1] == 0xa3) {
        ...
        sw_printf("[%s:%u] - Unrecognized PLT idiom at %lx: ", FILE__, __LINE__,
                  func_entry);
...
#define MAX_PLT64_IDIOM_SIZE 6
    ...
    if (got_abs) {
      ...
      sw_printf(
          "[%s:%u] - Computed PLT to be going through GOT abs %lx to "
          "real target %lx\n",
