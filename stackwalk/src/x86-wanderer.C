...
StepperWandererImpl::StepperWandererImpl(Walker* walker_, StepperWanderer* parent_,
...
unsigned
StepperWandererImpl::getPriority() const
{
    return wanderer_priority;
}

void
StepperWandererImpl::registerStepperGroup(StepperGroup* group)
{
    FrameStepper::registerStepperGroup(group);
}

gcframe_ret_t
StepperWandererImpl::getCallerFrame(const Frame& in, Frame& out)
{
        ...
        if(whelper->isPrevInstrACall(word, target))
        {
            ...
            switch(pcs)
            {
                case WandererHelper::outside_func:
                    // TODO re-enable this heuristic for Dyninst when the Analysis Stepper
                    // is working
                ...
                case WandererHelper::unknown_s:
                    sw_printf("[%s:%d] - Wanderer added word 0x%lx at 0x%lx as candidate "
                              "return "
                              " address\n",
                              FILE__, __LINE__, word, current_stack);
...
bool
StepperWandererImpl::getWord(Address& word_out, Address start)
{
    const unsigned addr_width = getProcessState()->getAddressWidth();
    if(start < 1024)
    {
...
bool
WandererHelper::isPrevInstrACall(Address addr, Address& target)
{
    return callchecker->isPrevInstrACall(addr, target);
}

WandererHelper::pc_state
WandererHelper::isPCInFunc(Address func_entry, Address pc)
{
    if(!func_entry || !pc)
        return unknown_s;
    if(func_entry == pc)
...
#define MAX_PLT32_IDIOM_SIZE 6
            ...
            {
                sw_printf("[%s:%d] - Unrecognized PLT idiom at %lx: ", FILE__, __LINE__,
                          func_entry);
...
#define MAX_PLT64_IDIOM_SIZE 6
        ...
        if(got_offset != -1)
        {
            ...
            {
                got_abs =
                    got_offset + reader->getSectionAddress(got_section) + func_lib.second;
            }
        }
    ...
    sw_printf("[%s:%d] - Decided func at offset %lx and pc-func at offset %lx\n", FILE__,
              __LINE__, func_offset, pc_offset);
...
WandererHelper::~WandererHelper() { delete callchecker; }
