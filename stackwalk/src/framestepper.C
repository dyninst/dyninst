...
FrameStepper::FrameStepper(Walker *w) : walker(w) {
  sw_printf("[%s:%u] - Creating FrameStepper %p with walker %p\n", FILE__,
            __LINE__, this, walker);
  assert(walker);
}
...
void FrameStepper::registerStepperGroup(StepperGroup *group) {
  unsigned addr_width =
      group->getWalker()->getProcessState()->getAddressWidth();
  if (addr_width == 4) group->addStepper(this, 0, 0xffffffff);
...
FrameFuncHelper::FrameFuncHelper(ProcessState *proc_) : proc(proc_) {}

FrameFuncHelper::~FrameFuncHelper() {}

std::map<SymReader *, bool> DyninstInstrStepperImpl::isRewritten;

DyninstInstrStepperImpl::DyninstInstrStepperImpl(Walker *w,
                                                 DyninstInstrStepper *p)
    : FrameStepper(w), parent(p) {}

gcframe_ret_t DyninstInstrStepperImpl::getCallerFrame(const Frame &in,
                                                      Frame &out) {
  ...
  result =
      getProcessState()->getLibraryTracker()->getLibraryAtAddr(in.getRA(), lib);
  if (!result) {
  ...
  if (!is_rewritten_binary) {
    sw_printf(
        "[%s:u] - Decided that current binary is not rewritten, "
  ...
  if (!has_stack_frame) {
    sw_printf(
        "[%s:%u] - Don't know how to walk through instrumentation without a "
        "stack frame\n",
        FILE__, __LINE__);
    return gcf_not_me;
  }
...
void DyninstInstrStepperImpl::registerStepperGroup(StepperGroup *group) {
  unsigned addr_width =
      group->getWalker()->getProcessState()->getAddressWidth();
  if (addr_width == 4) group->addStepper(parent, 0, 0xffffffff);
#if defined(arch_64bit)
  else if (addr_width == 8)
...
BottomOfStackStepperImpl::BottomOfStackStepperImpl(Walker *w,
                                                   BottomOfStackStepper *p)
    : FrameStepper(w),
      parent(p),
      libc_init(false),
      aout_init(false),
...
gcframe_ret_t BottomOfStackStepperImpl::getCallerFrame(const Frame &in,
                                                       Frame & /*out*/) {
  /**
...
DyninstDynamicStepperImpl::DyninstDynamicStepperImpl(Walker *w,
                                                     DyninstDynamicStepper *p,
                                                     DyninstDynamicHelper *h)
    : FrameStepper(w), parent(p), helper(h), prevEntryExit(false) {}

gcframe_ret_t DyninstDynamicStepperImpl::getCallerFrame(const Frame &in,
                                                        Frame &out) {
  unsigned stack_height = 0;
  Address orig_ra = 0x0;
  ...
  if (helper) {
    bool instResult = helper->isInstrumentation(
        in.getRA(), &orig_ra, &stack_height, &aligned, &entryExit);
    ...
    if (pEntryExit || instResult) {
      out.setNonCall();
      return getCallerFrameArch(in, out, 0, 0, 0, stack_height, aligned,
                                orig_ra, pEntryExit);
    }
  }
