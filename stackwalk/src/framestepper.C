...
void
FrameStepper::newLibraryNotification(LibAddrPair*, lib_change_t)
{}
...
gcframe_ret_t
DyninstInstrStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    ...
    if(!has_stack_frame)
    {
        sw_printf("[%s:%d] - Don't know how to walk through instrumentation without a "
                  "stack frame\n",
                  FILE__, __LINE__);
        return gcf_not_me;
    }
...
unsigned
DyninstInstrStepperImpl::getPriority() const
{
    return dyninstr_priority;
}

void
DyninstInstrStepperImpl::registerStepperGroup(StepperGroup* group)
{
    unsigned addr_width = group->getWalker()->getProcessState()->getAddressWidth();
    if(addr_width == 4)
...
gcframe_ret_t
BottomOfStackStepperImpl::getCallerFrame(const Frame& in, Frame& /*out*/)
{
    /**
...
unsigned
BottomOfStackStepperImpl::getPriority() const
{
...
void
BottomOfStackStepperImpl::registerStepperGroup(StepperGroup* group)
{
    unsigned addr_width = group->getWalker()->getProcessState()->getAddressWidth();
    if(addr_width == 4)
        group->addStepper(parent, 0, 0xffffffff);
...
gcframe_ret_t
DyninstDynamicStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    unsigned stack_height = 0;
    Address  orig_ra      = 0x0;
    ...
    if(helper)
    {
        bool instResult = helper->isInstrumentation(in.getRA(), &orig_ra, &stack_height,
                                                    &aligned, &entryExit);
        ...
        {
            out.setNonCall();
            return getCallerFrameArch(in, out, 0, 0, 0, stack_height, aligned, orig_ra,
                                      pEntryExit);
        }
    }
...
unsigned
DyninstDynamicStepperImpl::getPriority() const
{
    return dyninstr_priority;
}

void
DyninstDynamicStepperImpl::registerStepperGroup(StepperGroup* group)
{
    unsigned addr_width = group->getWalker()->getProcessState()->getAddressWidth();
    if(addr_width == 4)
        group->addStepper(parent, 0, 0xffffffff);
...
DyninstInstFrameStepperImpl::DyninstInstFrameStepperImpl(Walker*                  w,
                                                         DyninstInstFrameStepper* p)
: FrameStepper(w)
, parent(p)
{}

gcframe_ret_t
DyninstInstFrameStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    /**
        This wralker is designed to "jump over" instrimentation frames when stack walker
       is used in first party mode. The return from this walker will be the next valid
       frame in the program.
        ...
        mov 36b6e340,%rax
        push %rax           // Return Address used by Stackwalker Third Party for
       unwidning push %rbp           // Previous Frame FP, end of dyninst inst frame

        How this function generates a stack frame:

        1. Compares the current FP to the SP to determine if the FP holds an address that
       is potentially on the stack. The reason for this check is to prevent reading
       uninitialized memory. Right now it checks if the FP is within 500 stack positions
       of the SP.

        2. Check for the magic word by reading the memory loaction current FP + addr_width
       * 2.

        3. If the magic word is detected, rthe frame is generated by the following:

            Frame SP: reading the value at FP + addr_width * 3 then adding addr_width to
       this value. Frame FP: reading the value at FP (should be the stored previous FP)
    ...
    */
    ...
    // isn't an instrimentation frame. This check is required to prevent a deref of an
    // invalid FP.
    if(diff >= (addr_width * 500))
    {
        sw_printf("[%s:%d] - I am Rejecting frame because (FP - stackPtr) > 500 stack "
                  "positions - FP: %lx , SP: %lx, DIFF: %lu, Check: %u\n",
...
bool
DyninstInstFrameStepperImpl::getWord(Address& word_out, Address start)
{
    const unsigned addr_width = getProcessState()->getAddressWidth();
    if(start < 1024)
    {
    ...
    {
        sw_printf(
            "[%s:%d] - DyninstInstFrameStepperImpl couldn't read from stack at 0x%lx\n",
            FILE__, __LINE__, start);
        return false;
    }
...
unsigned
DyninstInstFrameStepperImpl::getPriority() const
{
    return dyninstr_priority;
}

void
DyninstInstFrameStepperImpl::registerStepperGroup(StepperGroup* group)
{
    unsigned addr_width = group->getWalker()->getProcessState()->getAddressWidth();
    if(addr_width == 4)
        group->addStepper(parent, 0, 0xffffffff);
...
//#if (defined(os_linux) || defined(os_freebsd)) && (defined(arch_x86) ||
// defined(arch_x86_64))
#if(defined(os_linux) || defined(os_freebsd)) &&                                         \
    (defined(arch_x86) || defined(arch_x86_64) || defined(arch_aarch64))
