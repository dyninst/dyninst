...
CallChecker::CallChecker(ProcessState* proc_)
: proc(proc_)
{}
CallChecker::~CallChecker() {}

bool
CallChecker::isPrevInstrACall(Address addr, Address& target)
{
    ...
    {
        start = addr - size;
        if(proc->readMem(buffer, start, max_call_length))
            break;
    }
    ...
    for(unsigned size = addr - start; size > 0; size--)
    {
        ...
        if((aligned == size) && (prevInsn.getOperation().getID() == e_call))
        {
            int disp = *((int*) (bufferPtr + (size - prevInsn.size() + 2)));
            target   = addr + disp;
            sw_printf(
                "[%s:%d] - Found call encoded by %d to %lx (addr = %lx, disp = %x)\n",
                FILE__, __LINE__, buffer[0], target, addr, (unsigned int) disp);
            return true;
        }
        else
        {
