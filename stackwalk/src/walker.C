...
void
Walker::version(int& major, int& minor, int& maintenance)
{
    major       = SW_MAJOR;
    minor       = SW_MINOR;
    maintenance = SW_BETA;
...
Walker::Walker(ProcessState* p, StepperGroup* grp, SymbolLookup* sym,
...
, call_count(0)
{
    ...
    sw_printf("[%s:%d] - Creating new Walker with proc=%p, sym=%p, step = %d\n", FILE__,
              __LINE__, (void*) proc, (void*) sym, (int) default_steppers);
...
Walker*
Walker::newWalker(std::string exec_name)
{
...
Walker*
Walker::newWalker(Dyninst::PID pid, std::string executable)
{
    sw_printf("[%s:%d] - Creating new stackwalker for process %d on %s\n", FILE__,
              __LINE__, (int) pid, executable.c_str());
...
Walker*
Walker::newWalker(std::string exec_name, const std::vector<std::string>& argv)
{
...
Walker*
Walker::newWalker(ProcessState* proc, StepperGroup* grp, SymbolLookup* lookup,
...
Walker*
Walker::newWalker(Dyninst::ProcControlAPI::Process::ptr proc)
{
    sw_printf("[%s:%d] - Creating new stackwalker for ProcControl process %d\n", FILE__,
              __LINE__, (int) proc->getPid());
...
bool
Walker::newWalker(const std::vector<Dyninst::PID>& pids,
...
void
Walker::setSymbolReader(SymbolReaderFactory* srf)
{
    symrfact = srf;
...
bool
Walker::walkStack(std::vector<Frame>& stackwalk, THR_ID thread)
{
    bool  result;
...
bool
Walker::walkStackFromFrame(std::vector<Frame>& stackwalk, const Frame& frame)
{
    bool result;
    ...
    for(std::vector<Frame>::iterator swi = stackwalk.begin(); swi != stackwalk.end();
        ++swi)
    {
        swi->prev_frame = NULL;
    }
...
bool
Walker::walkSingleFrame(const Frame& in, Frame& out)
{
    ...
    for(;;)
    {
        ...
        bool          res =
            group->findStepperForAddr(last_frame.getRA(), cur_stepper, last_stepper);
        if(!res)
        {
        ...
        if(gcf_result == gcf_success)
        {
            ...
            sw_printf("[%s:%d] - Returning frame with RA %lx, SP %lx, FP %lx\n", FILE__,
                      __LINE__, out.getRA(), out.getSP(), out.getFP());
        ...
        else if(gcf_result == gcf_error)
        {
            sw_printf("[%s:%d] - A stepper reported error %p on frame at %lx\n", FILE__,
                      __LINE__, (void*) cur_stepper, in.getRA());
            result = false;
            goto done;
        }
    }
...
bool
Walker::getInitialFrame(Frame& frame, THR_ID thread)
{
    bool result;
    frame.walker = this;
    result       = callPreStackwalk(thread);
...
bool
Walker::getAvailableThreads(std::vector<THR_ID>& threads) const
{
    threads.clear();
...
ProcessState*
Walker::createDefaultProcess(std::string exec_name)
{
...
ProcessState*
Walker::createDefaultProcess(PID pid, std::string executable)
{
...
ProcessState*
Walker::createDefaultProcess(Dyninst::ProcControlAPI::Process::ptr proc)
{
...
bool
Walker::createDefaultProcess(const vector<Dyninst::PID>& pids, vector<ProcDebug*>& pds)
{
    return ProcDebug::newProcDebugSet(pids, pds);
}

ProcessState*
Walker::createDefaultProcess(std::string exec_name, const std::vector<std::string>& argv)
{
...
bool
Walker::addStepper(FrameStepper* s)
{
    assert(group);
    sw_printf("[%s:%d] - Registering stepper %s with group %p\n", FILE__, __LINE__,
              s->getName(), (void*) group);
    group->registerStepper(s);
    return true;
}

bool
Walker::callPreStackwalk(Dyninst::THR_ID tid)
{
    call_count++;
    if(call_count != 1)
        return true;
...
bool
Walker::callPostStackwalk(Dyninst::THR_ID tid)
{
    call_count--;
...
StepperGroup*
Walker::createDefaultStepperGroup()
{
...
int_walkerSet::~int_walkerSet() { clearProcSet(); }

pair<set<Walker*>::iterator, bool>
int_walkerSet::insert(Walker* w)
{
...
void
int_walkerSet::erase(set<Walker*>::iterator i)
{
...
WalkerSet::~WalkerSet() { delete iwalkerset; }
...
WalkerSet::iterator
WalkerSet::find(Walker* w)
{
    return iwalkerset->walkers.find(w);
}

WalkerSet::const_iterator
...
WalkerSet::const_iterator
WalkerSet::find(Walker* w) const
{
    return ((const int_walkerSet*) iwalkerset)->walkers.find(w);
}

pair<WalkerSet::iterator, bool>
    ...
    for(const_iterator i = begin(); i != end(); i++)
    {
        ...
        if(!result)
        {
            sw_printf("[%s:%d] - Error getting threads for process %d\n", FILE__,
                      __LINE__, walker->getProcessState()->getProcessId());
            had_error = true;
            continue;
        }
