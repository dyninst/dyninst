...
Walker::Walker(ProcessState *p, StepperGroup *grp, SymbolLookup *sym,
    ...
    : proc(NULL), lookup(NULL), creation_error(false), call_count(0) {
  bool result;
...
Walker *Walker::newWalker(Dyninst::PID pid, std::string executable) {
...
Walker *Walker::newWalker(ProcessState *proc, StepperGroup *grp,
                          SymbolLookup *lookup, bool default_steppers) {
  if (!proc) {
    sw_printf("[%s:%u] - Error proc parameter to newWalker must not be NULL\n",
              FILE__, __LINE__);
    setLastError(err_badparam,
                 "Tried to create a walker with a NULL "
                 "ProcessState param");
    return NULL;
  }
  sw_printf(
      "[%s:%u] - Creating custom Walker with proc = %p"
      "lookup = %p\n",
...
Walker *Walker::newWalker(Dyninst::PID pid) { return newWalker(pid, ""); }
...
bool Walker::newWalker(const std::vector<Dyninst::PID> &pids,
  ...
  for (i = new_dbs.begin(); i != new_dbs.end(); i++) {
    ...
    Walker *newwalker =
        new Walker((ProcessState *)pd, NULL, NULL, true, executable);
    if (!newwalker || newwalker->creation_error) {
      sw_printf("[%s:%u] - Error creating new Walker object %p\n", FILE__,
                __LINE__, newwalker);
      walkers_out.push_back(NULL);
    ...
    sw_printf("[%s:%u] - Successfully created walker for %d\n", FILE__,
              __LINE__, pd->getProcessId());
    walkers_out.push_back(newwalker);
  }

  if (num_errors == pids.size()) return false;
  return true;
}
  ...
  for (;;) {
    ...
    if (gcf_result == gcf_success) {
      sw_printf("[%s:%u] - Success using stepper %s on 0x%lx\n", FILE__,
                __LINE__, cur_stepper->getName(), in.getRA());
...
ProcessState *Walker::getProcessState() const { return proc; }

SymbolLookup *Walker::getSymbolLookup() const { return lookup; }
...
StepperGroup *Walker::getStepperGroup() const { return group; }

int_walkerSet::int_walkerSet() : non_pd_walkers(0) { initProcSet(); }
...
WalkerSet *WalkerSet::newWalkerSet() { return new WalkerSet(); }

WalkerSet::WalkerSet() : iwalkerset(new int_walkerSet()) {}

WalkerSet::~WalkerSet() { delete iwalkerset; }

WalkerSet::iterator WalkerSet::begin() { return iwalkerset->walkers.begin(); }

WalkerSet::iterator WalkerSet::end() { return iwalkerset->walkers.end(); }
...
void WalkerSet::erase(WalkerSet::iterator i) { iwalkerset->erase(i); }

bool WalkerSet::empty() const { return iwalkerset->walkers.empty(); }

size_t WalkerSet::size() const { return iwalkerset->walkers.size(); }
...
bool WalkerSet::walkStacks(CallTree &tree, bool walk_initial_only) const {
  ...
  if (!iwalkerset->non_pd_walkers) {
    bool bad_plat = false;
    bool result =
        iwalkerset->walkStacksProcSet(tree, bad_plat, walk_initial_only);
    if (result) {
      // Success
      return true;
    }
  ...
  for (const_iterator i = begin(); i != end(); i++) {
    ...
    for (vector<THR_ID>::iterator j = threads.begin(); j != threads.end();
         j++) {
