...
public:
    FrameFuncStepperImpl(Walker* w, FrameStepper* parent_, FrameFuncHelper* helper_);
...
public:
    ...
    gcframe_ret_t         getCallerFrameArch(const Frame& in, Frame& out, Address base,
                                             Address lib_base, unsigned size,
                                             unsigned stack_height, bool aligned, Address orig_ra,
                                             bool pEntryExit);
    virtual unsigned      getPriority() const;
    virtual void          registerStepperGroup(StepperGroup* group);
    virtual const char*   getName() const;
...
private:
    ...
    set<Walker*> walkers;
    void* procset;  // Opaque pointer, will refer to a ProcControl::ProcessSet in some
                    // situations
};
