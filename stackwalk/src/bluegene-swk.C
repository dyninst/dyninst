...
void ProcSelf::initialize() { proc_self_unsupported(FILE__, __LINE__); }
...
bool ProcDebug::newProcDebugSet(const vector<Dyninst::PID> &,
                                vector<ProcDebug *> &) {
  setLastError(
      err_unsupported,
      "ERROR: newProcDebugSet not implemented for BlueGene Stackwalker!");
  return false;
}
...
DebugEvent ProcDebug::debug_get_event(bool block) {
  ...
  if (returnCode > 0) {
    // Print out a message if we get a return code we don't expect.  Consult the
    // debugger header
    // for the meanings of these.
    sw_printf(
        "[%s:%u] - WARNING: return code for %s on pid %d, tid %d was non-zero: "
        "%d\n",
        FILE__, __LINE__,
        BG_Debugger_Msg::getMessageName(msg.header.messageType), pid, tid,
  ...
  if (t == procbg->threads.end()) {
    BGThreadState *initial_thread =
        dynamic_cast<BGThreadState *>(procbg->initial_thread);

    if (tid != 0) {
      // Check to see if the initial thread is valid.  If its id is zero, it's a
      // stub
      // from creation time and we got a signal from the real initial thread.
      // If it's
      // not zero, then we're seeing a strange thread id.
      if (initial_thread->getTid() != 0) {
        const char *msg =
            "Saw unknown thread id in ProcDebug::debug_get_evet()!";
      ...
      // if we see a threadid we don't know about, it's because we got a
      // SIGNAL_ENCOUNTERED
      // from the main thread.  We default the initial thread id to zero, but
      // this is only because
      // we need to start somewhere to attach.  This sets the main thread id to
      // the real thread id
      // for the process.  This should happen relatively early when we stop the
      // thread during attach.
      ...
      // if the thread id we don't know about is zero, we got an ack for
      // something we
      // did before we discovered what the initial thread id was.  So just point
      // the event
...
int ProcDebug::getNotificationFD() { return BG_DEBUGGER_READ_PIPE; }
  ...
  switch (msg.header.messageType) {
    case PROGRAM_EXITED: {
      ...
      if (exit_type == 0) {
        ...
        ev.dbg = dbg_err;
        sw_printf(
            "[%s:%u] - WARNING: Unknown exit type (%d) on process %d. "
            "May be using outdated BG Debugger Interface!\n",
            FILE__, __LINE__, msg.dataArea.PROGRAM_EXITED.type, pid);
      }
    } break;
    ...
    case GET_MEM_ACK:
      ...
      if (!ev.data.pdata) {
        ev.dbg = dbg_err;
        sw_printf(
            "[%s:%u] - FATAL: Couldn't allocate enough space for memory read "
            "on pid %d\n",
            FILE__, __LINE__, pid);
      } else {
    ...
    case SET_MEM_ACK:
      ...
      sw_printf("[%s:%u] - Memory write ACK on pid %d ($d bytes at %x).  \n",
                FILE__, __LINE__, msg.dataArea.SET_MEM_ACK.len,
                msg.dataArea.SET_MEM_ACK.addr);
      break;

    case SINGLE_STEP_ACK:
      // single step ack is just an ack (like KILL_ACK). We ignore this event
      // and
...
bool ProcDebugBG::version_msg() {
  ...
  if (state() == ps_exited) {
    setLastError(err_procexit,
                 "Process exited unexpectedly during version_msg");
    return false;
  }
  if (!result) {
...
bool ProcDebugBG::debug_continue_with(ThreadState *ts, long sig) {
  ...
  if (!ts->isStopped()) {
    sw_printf(
        "[%s:%u] - Error in debug_continue_with(): pid %d, thread %d not "
        "stopped.\n",
        FILE__, __LINE__, pid, tid);
    return false;
  }
...
bool ProcDebugBG::getRegValue(MachRegister reg, THR_ID tid,
                              MachRegisterVal &val) {
...
bool ProcDebugBG::setRegValue(MachRegister reg, THR_ID tid,
                              MachRegisterVal val) {
  assert(threads.count(tid));
  BGThreadState *thr = dynamic_cast<BGThreadState *>(threads[tid]);
...
bool ProcDebugBG::readMem(void *dest, Address source, size_t size) {
  ...
  for (;;) {
    ...
    sw_printf("[%s:%u] - Caching memory from 0x%lx to 0x%lx\n", FILE__,
              __LINE__, read_cache_start, read_cache_start + read_cache_size);
...
void BottomOfStackStepperImpl::newLibraryNotification(LibAddrPair *,
                                                      lib_change_t change) {
  if (change == library_unload) return;
...
SymbolReaderFactory *getDefaultSymbolReader() {
  ...
  if (!Walker::symrfact) Walker::symrfact = (SymbolReaderFactory *)&symelffact;
