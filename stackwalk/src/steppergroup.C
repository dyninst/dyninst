...
void
StepperGroup::newLibraryNotification(LibAddrPair* libaddr, lib_change_t change)
{
...
bool
AddrRangeGroup::findStepperForAddr(Address addr, FrameStepper*& out,
...
bool
AddrRangeGroup::addStepper(FrameStepper* stepper, Address start, Address end)
{
    ...
    if(!stepper || end <= start)
    {
        sw_printf("[%s:%d] - addStepper called with bad params: %s, %lx, %lx\n", FILE__,
                  __LINE__, stepper->getName(), start, end);
...
void
StepperGroup::getSteppers(std::set<FrameStepper*>& steps)
{
    steps = steppers;
}
