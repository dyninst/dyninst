...
ProcDebugBGP::ProcDebugBGP(PID pid, string exe)
    : ProcDebugBG(pid, exe), lib_load_trap(0) {}
...
bool ProcDebugBGP::debug_post_attach(ThreadState *) {
  ...
  if (!library_tracker) setDefaultLibraryTracker();
  // This should really be checking for exceptions, since the above are
  // constructors.
  if (!library_tracker) {
...
bool ProcDebugBGP::isLibraryTrap(Dyninst::THR_ID thrd) {
  ...
  if (cur_pc == lib_trap_addr || cur_pc - 1 == lib_trap_addr) return true;
  return false;
}
...
bool ProcDebugBGP::cleanOnDetach() {
  ...
  if (!lib_load_trap) {
    sw_printf(
        "[%s:%u] - Couldn't get trap addr, couldn't set up "
  ...
  while (!write_ack) {
    ...
    if (result) {
      sw_printf(
          "[%s:%u] - Unable to get ack for setmem in "
          "registerLibSpotter()\n",
          FILE__, __LINE__);
      break;
    }
...
void ProcDebugBGP::registerLibSpotter() {
  ...
  // construct a bg debugger msg to install a trap instruction at the loader
  // trap address.
  BG_Debugger_Msg write_trap_msg(SET_MEM, pid, 0, 0, 0);
  ...
  while (!write_ack) {
    ...
    if (result) {
      sw_printf(
          "[%s:%u] - Unable to get ack for setmem in registerLibSpotter()\n",
          FILE__, __LINE__);
      break;
    }
  }
...
bool ProcDebugBGP::pollForNewThreads() {
  ...
  get_info_msg.header.dataLength =
      sizeof(get_info_msg.dataArea.GET_THREAD_INFO);
  ...
  if (!debug_waitfor(dbg_thread_info)) {
    sw_printf(
        "[%s:%u] - Unable to get ack for GET_THREAD_INFO in "
        "pollForNewThreads()\n",
        FILE__, __LINE__);
  }
...
bool ProcDebugBGP::debug_handle_event(DebugEvent ev) {
  ...
  switch (ev.dbg) {
    case dbg_libraryload: {
      ...
      if (!debug_continue(ts)) {
        sw_printf("[%s:%u] - Debug continue failed on %d/%d with %d\n", FILE__,
                  __LINE__, pid, tid, ev.data.idata);
        setLastError(err_internal,
                     "debug_continue() failed after library load.");
        return false;
      }
      return true;
    } break;
    ...
    case dbg_thread_info: {
      // Create threads for new ids now that we're actually handling thread_info
      // message.
