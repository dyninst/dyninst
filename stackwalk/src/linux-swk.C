...
vsys_info*
Dyninst::Stackwalker::getVsysInfo(ProcessState* ps)
{
    /*
...
static const char*  vsys_sigreturns[] = { "sigreturn", "_sigreturn", "__sigreturn",
                                         "__kernel_sigreturn", "__kernel_rt_sigreturn" };
static const size_t NUM_VSYS_SIGRETURNS =
    sizeof vsys_sigreturns / sizeof vsys_sigreturns[0];

void
SigHandlerStepperImpl::registerStepperGroup(StepperGroup* group)
{
    ...
    sw_printf(
        "[%s:%d] - Got lib tracker and sym reader OK, checking for __restore_rt...\n",
        FILE__, __LINE__);
    ...
    if(!init_libc)
    {
        ...
        if(!init_libc)
        {
            if(result)
            {
                ...
                {
                    sw_printf(
                        "[%s:%d] - Unable to open libc, not registering restore_rt\n",
                        FILE__, __LINE__);
                }
            }
            if(result && libc)
    ...
    if(!init_libthread)
    {
        ...
        if(libpthread)
        {
            ...
            else
            {
                ...
                sw_printf(
                    "[%s:%d] - Registering libpthread restore_rt as at %lx to %lx\n",
                    FILE__, __LINE__, start, end);
                group->addStepper(parent_stepper, start, end);
            }
        }
