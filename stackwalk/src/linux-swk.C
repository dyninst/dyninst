...
vsys_info *Dyninst::Stackwalker::getVsysInfo(ProcessState *ps) {
  ...
  if (ps->getAddressWidth() == 8) return NULL;
#endif
  ...
  if (i != vsysmap.end()) return i->second;

  AuxvParser *parser =
      AuxvParser::createAuxvParser(ps->getProcessId(), ps->getAddressWidth());
  if (!parser) {
...
bool LibraryState::updateLibsArch(
    std::vector<std::pair<LibAddrPair, unsigned int> > &alibs) {
  if (arch_libs.size()) {
    alibs = arch_libs;
    return true;
  }
...
static const char *vsys_sigreturns[] = {"_sigreturn", "__kernel_sigreturn",
...
void SigHandlerStepperImpl::registerStepperGroup(StepperGroup *group) {
  ...
  if (!libs) {
    sw_printf(
        "[%s:%u] - Custom library tracker.  Don't know how to"
        " to get libc\n",
        FILE__, __LINE__);
    return;
  }
  ...
  sw_printf(
      "[%s:%u] - Got lib tracker and sym reader OK, checking for "
      "__restore_rt...\n",
      FILE__, __LINE__);
  ...
  if (!init_libc) {
    ...
    if (!result) {
      sw_printf(
          "[%s:%u] - Unable to find libc, not registering restore_rt"
          "tracker.\n",
          FILE__, __LINE__);
    }
    if (!init_libc) {
      if (result) {
        ...
        if (!libc) {
          sw_printf(
              "[%s:%u] - Unable to open libc, not registering restore_rt\n",
              FILE__, __LINE__);
        }
      }
      if (result && libc) {
        ...
        if (!libc->isValidSymbol(libc_restore)) {
          ...
          Dyninst::Address start =
              libc->getSymbolOffset(libc_restore) + libc_addr.second;
          Dyninst::Address end = libc->getSymbolSize(libc_restore) + start;
          if (start == end) end = start + 16;  // Estimate--annoying
  ...
  if (!init_libthread) {
    ...
    if (!result) {
      sw_printf(
          "[%s:%u] - Unable to find libpthread, not registering restore_rt"
          "pthread tracker.\n",
          FILE__, __LINE__);
    }
    if (result) {
    ...
    if (libpthread) {
      ...
      if (!libpthread->isValidSymbol(libpthread_restore)) {
        ...
        Dyninst::Address start =
            libpthread->getSymbolOffset(libpthread_restore) +
            libpthread_addr.second;
        Dyninst::Address end =
            libpthread->getSymbolSize(libpthread_restore) + start;
        if (start == end) end = start + 16;  // Estimate--annoying
        sw_printf(
            "[%s:%u] - Registering libpthread restore_rt as at %lx to %lx\n",
            FILE__, __LINE__, start, end);
        group->addStepper(parent_stepper, start, end);
      }
    }
  ...
  if (!vsyscall) {
#if !defined(arch_x86_64)
    sw_printf(
        "[%s:%u] - Odd.  Couldn't find vsyscall page. Signal handler"
    ...
    if (!vsys_syms) {
      ...
      for (unsigned i = 0; i < NUM_VSYS_SIGRETURNS; i++) {
        ...
        if (!vsys_syms->isValidSymbol(sym)) continue;
        ...
        if (!size) size = ps->getAddressWidth();
