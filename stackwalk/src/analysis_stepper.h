...
class AnalysisStepperImpl : public FrameStepper {
 ...
 protected:
  ...
  std::vector<registerState_t> fullAnalyzeFunction(std::string name,
                                                   Offset off);

  virtual bool isPrevInstrACall(Address addr, Address &target);
  virtual gcframe_ret_t getCallerFrameArch(std::set<height_pair_t> height,
                                           const Frame &in, Frame &out);
  gcframe_ret_t getFirstCallerFrameArch(
  ...
  bool getOutRA(Address out_sp, Address &out_ra, location_t &out_ra_loc,
                ProcessState *proc);
};
}
}
