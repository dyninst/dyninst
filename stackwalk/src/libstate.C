...
, procreader(parent, executable_)
{
    ...
    {
        translate = AddressTranslate::createAddressTranslator(
            pid, &procreader, symfactory, INVALID_HANDLE_VALUE, executable_);
    }
    if(!translate)
    {
...
void
TrackLibState::getCurList(std::set<LibAddrPair>& list)
{
...
TrackLibState::updateLibs()
{
    ...
    set_difference(pre.begin(), pre.end(), post.begin(), post.end(),
    ...
    set_difference(post.begin(), post.end(), pre.begin(), pre.end(),
                   inserter(diff, diff.begin()));
    for(set<LibAddrPair>::iterator i = diff.begin(); i != diff.end(); i++)
    {
        LibAddrPair la = *i;
...
bool
TrackLibState::getLibraryAtAddr(Address addr, LibAddrPair& olib)
{
    bool result = refresh();
    if(!result)
    {
...
bool
TrackLibState::getLibraries(std::vector<LibAddrPair>& olibs, bool allow_refresh)
{
    bool result;
    if(allow_refresh)
    {
...
Address
TrackLibState::getLibTrapAddress()
{
    return translate->getLibraryTrapAddrSysV();
}

TrackLibState::~TrackLibState() { delete translate; }

void
TrackLibState::notifyOfUpdate()
{
...
static bool
libNameMatch(const char* s, const char* libname)
{
...
bool
LibraryState::getLibc(LibAddrPair& addr_pair)
{
...
bool
LibraryState::getLibthread(LibAddrPair& addr_pair)
{
    std::vector<LibAddrPair> libs;
    getLibraries(libs, false);
    if(libs.size() == 1)
    ...
    for(std::vector<LibAddrPair>::iterator i = libs.begin(); i != libs.end(); i++)
    {
        if(libNameMatch("pthread", i->first.c_str()) ||
           libNameMatch("thr", i->first.c_str()))
        {
            addr_pair = *i;
            return true;
        }
...
bool
TrackLibState::getAOut(LibAddrPair& addr_pair)
{
    LoadedLib* aout;
    updateLibs();
...
bool
swkProcessReader::ReadMem(Address inTraced, void* inSelf, unsigned amount)
{
...
bool
swkProcessReader::start()
{
    return true;
}

bool
swkProcessReader::done()
{
    return true;
}
...
SymReader*
LibraryWrapper::getLibrary(std::string filename)
{
...
void
LibraryWrapper::registerLibrary(SymReader* reader, std::string filename)
{
    libs.file_map[filename] = reader;
}

SymReader*
LibraryWrapper::testLibrary(std::string filename)
{
    std::map<std::string, SymReader*>::iterator i = libs.file_map.find(filename);
    if(i != libs.file_map.end())
    {
...
bool
StaticBinaryLibState::getLibraryAtAddr(Address /*addr*/, LibAddrPair& olib)
{
    olib = the_exe;
    return true;
}
...
bool
StaticBinaryLibState::getLibraries(std::vector<LibAddrPair>& olibs,
                                   bool /*allow_refresh*/)
{
...
bool
StaticBinaryLibState::getLibc(LibAddrPair& lc)
{
    lc = the_exe;
    return true;
}
...
bool
StaticBinaryLibState::getLibthread(LibAddrPair& lt)
{
    lt = the_exe;
    return true;
}
...
bool
StaticBinaryLibState::getAOut(LibAddrPair& ao)
{
    ao = the_exe;
    return true;
}
...
void
StaticBinaryLibState::notifyOfUpdate()
{}

Address
StaticBinaryLibState::getLibTrapAddress()
{
    return 0x0;
}
