...
TrackLibState::TrackLibState(ProcessState *parent, std::string executable_)
  ...
  sw_printf("[%s:%u] - Creating a TrackLibState on pid %d\n", FILE__, __LINE__,
            pid);
  if (!symfactory) symfactory = Walker::getSymbolReader();

  if (procstate->isFirstParty()) {
    translate = AddressTranslate::createAddressTranslator(
        &procreader, symfactory, executable_);
  } else {
    translate = AddressTranslate::createAddressTranslator(
  ...
  if (!translate) {
    sw_printf(
        "[%s:%u] - Creation of AddressTranslate failed "
        "on pid %d!\n",
        FILE__, __LINE__, pid);
  }
  assert(translate);
...
bool TrackLibState::updateLibs() {
  ...
  if (!result) {
    sw_printf(
        "[%s:%u] - Could not get load addresses out of SymtabAPI for %d."
...
static bool libNameMatch(const char *s, const char *libname) {
  ...
  if (!filestart) filestart = libname;
  const char *lib_start = strstr(filestart, "lib");
  if (lib_start != filestart + 1) return false;
...
bool LibraryState::getLibc(LibAddrPair &addr_pair) {
  ...
  for (std::vector<LibAddrPair>::iterator i = libs.begin(); i != libs.end();
       i++) {
...
bool swkProcessReader::ReadMem(Address inTraced, void *inSelf,
                               unsigned amount) {
...
bool swkProcessReader::start() { return true; }

bool swkProcessReader::done() { return true; }
...
StaticBinaryLibState::StaticBinaryLibState(ProcessState *parent,
                                           std::string executable)
    : LibraryState(parent) {
...
bool StaticBinaryLibState::getLibraryAtAddr(Address /*addr*/,
                                            LibAddrPair &olib) {
  olib = the_exe;
  return true;
}
...
Address StaticBinaryLibState::getLibTrapAddress() { return 0x0; }
