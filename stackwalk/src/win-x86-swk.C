...
ProcSelf::ProcSelf(std::string exe_path) : ProcessState(P_getpid(), exe_path) {}

void ProcSelf::initialize() { setDefaultLibraryTracker(); }
...
void BottomOfStackStepperImpl::initialize() {
  ...
  /*   ProcessState *proc = walker->getProcessState();
     ...
     if (!libs) {
        sw_printf("[%s:%u] - Error initing StackBottom.  No library state for
     process.\n",
                  FILE__, __LINE__);
        return;
     }
     ...
     aout->getSymbolOffset(start_sym)+aout_addr.second;
              ...
              if (start == end) {
                 sw_printf("[%s:%u] - %s symbol has 0 length, using length of
     %lu\n",
                         FILE__, __LINE__, START_FUNC_NAME,
     START_HEURISTIC_LENGTH);
                 end = start + START_HEURISTIC_LENGTH;
              }
              sw_printf("[%s:%u] - Bottom stepper taking %lx to %lx for
     start\n",
     ...
     if (!libthread_init)
     {
        ...
        if (libthread) {
           ...
           if (libthread->isValidSymbol(clone_sym)) {
              ...
              Dyninst::Address end = libthread->getSymbolSize(clone_sym) +
     start;
              sw_printf("[%s:%u] - Bottom stepper taking %lx to %lx for
     clone\n",
                        FILE__, __LINE__, start, end);
           ...
           if (libthread->isValidSymbol(startthread_sym)) {
              Dyninst::Address start =
     libthread->getSymbolOffset(startthread_sym) +
                 libthread_addr.second;
              Dyninst::Address end = libthread->getSymbolSize(startthread_sym) +
     start;
              sw_printf("[%s:%u] - Bottom stepper taking %lx to %lx for
     start_thread\n",
                        FILE__, __LINE__, start, end);
