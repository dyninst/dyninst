...
bool Frame::operator==(const Frame &F) const {
  return ((ra == F.ra) && (fp == F.fp) && (sp == F.sp) &&
          (ra_loc == F.ra_loc) && (fp_loc == F.fp_loc) &&
          (sp_loc == F.sp_loc) && (sym_name == F.sym_name) &&
          (frame_complete == F.frame_complete) && (stepper == F.stepper) &&
          (walker == F.walker) && (originating_thread == F.originating_thread));
}
...
Dyninst::MachRegisterVal Frame::getRA() const { return ra; }

Dyninst::MachRegisterVal Frame::getSP() const { return sp; }

Dyninst::MachRegisterVal Frame::getFP() const { return fp; }

location_t Frame::getRALocation() const { return ra_loc; }

location_t Frame::getSPLocation() const { return sp_loc; }

location_t Frame::getFPLocation() const { return fp_loc; }
...
void Frame::setNameValue() const {
  if (name_val_set == nv_set || name_val_set == nv_err) return;

  if (!walker) {
    setLastError(err_nosymlookup,
                 "No Walker object was associated with this frame");
  ...
  if (!lookup) {
    setLastError(err_nosymlookup,
                 "No SymbolLookup object was associated with the Walker");
...
bool Frame::isTopFrame() const { return top_frame; }

bool Frame::isBottomFrame() const { return bottom_frame; }

const Frame *Frame::getPrevFrame() const { return prev_frame; }

FrameStepper *Frame::getStepper() const { return stepper; }

FrameStepper *Frame::getNextStepper() const { return next_stepper; }

Walker *Frame::getWalker() const { return walker; }

bool Frame::isFrameComplete() const { return frame_complete; }
...
THR_ID Frame::getThread() const { return originating_thread; }

void Frame::setThread(THR_ID t) { originating_thread = t; }

void Frame::setNonCall() { non_call_frame = true; }

bool Frame::nonCall() const { return non_call_frame; }
...
inline bool frame_cmp_wrapper::operator()(const FrameNode *a,
                                          const FrameNode *b) const {
  if (a->frame_type == FrameNode::FTThread &&
      b->frame_type == FrameNode::FTThread) {
    ...
    return true;
           ...
           b->frame_type == FrameNode::FTString)
...
frame_cmp_t CallTree::getComparator() { return cmp_wrapper.f; }

frame_cmp_wrapper CallTree::getCompareWrapper() { return cmp_wrapper; }
...
FrameNode *CallTree::addFrame(const Frame &f, FrameNode *parent) {
  ...
  pair<frame_set_t::iterator, frame_set_t::iterator> is =
      parent->children.equal_range(&search_node);
...
void CallTree::addCallStack(const vector<Frame> &stk, THR_ID thrd,
                            Walker *walker, bool err_stack) {
  FrameNode *cur = head;
  for (vector<Frame>::const_reverse_iterator i = stk.rbegin(); i != stk.rend();
       i++) {
    cur = addFrame(*i, cur);
  }
...
bool Dyninst::Stackwalker::frame_lib_offset_cmp(const Frame &a,
                                                const Frame &b) {
  string a_lib, b_lib;
