...
Frame*
Frame::newFrame(Dyninst::MachRegisterVal pc, Dyninst::MachRegisterVal sp,
                Dyninst::MachRegisterVal fp, Walker* walker)
{
    sw_printf("[%s:%d] - Manually creating frame with %lx, %lx, %lx, %p\n", FILE__,
              __LINE__, pc, sp, fp, (void*) walker);
    if(!walker)
    {
...
{
    return ((ra == F.ra) && (fp == F.fp) && (sp == F.sp) && (ra_loc == F.ra_loc) &&
            (fp_loc == F.fp_loc) && (sp_loc == F.sp_loc) && (sym_name == F.sym_name) &&
...
void
Frame::setRA(Dyninst::MachRegisterVal newval)
{
...
void
Frame::setFP(Dyninst::MachRegisterVal newval)
{
...
void
Frame::setSP(Dyninst::MachRegisterVal newval)
{
...
THR_ID
Frame::getThread() const { return originating_thread; }

void
Frame::setThread(THR_ID t)
{
...
inline bool
frame_cmp_wrapper::operator()(const FrameNode* a, const FrameNode* b) const
{
...
frame_cmp_t
CallTree::getComparator()
{
    return cmp_wrapper.f;
}

frame_cmp_wrapper
CallTree::getCompareWrapper()
{
    return cmp_wrapper;
}

static void
deleteTree(FrameNode* node)
{
...
FrameNode*
CallTree::addFrame(const Frame& f, FrameNode* parent)
{
...
FrameNode*
CallTree::addThread(THR_ID thrd, FrameNode* parent, Walker* walker, bool err_stack)
{
    FrameNode* new_node = new FrameNode(cmp_wrapper);
    assert(walker);
...
void
CallTree::addCallStack(const vector<Frame>& stk, THR_ID thrd, Walker* walker,
                       bool err_stack)
{
    FrameNode* cur = head;
...
bool
Dyninst::Stackwalker::frame_addr_cmp(const Frame& a, const Frame& b)
{
    return a.getRA() < b.getRA();
}

bool
Dyninst::Stackwalker::frame_lib_offset_cmp(const Frame& a, const Frame& b)
{
    string a_lib, b_lib;
...
bool
Dyninst::Stackwalker::frame_symname_cmp(const Frame& a, const Frame& b)
{
...
bool
Dyninst::Stackwalker::frame_lineno_cmp(const Frame&, const Frame&)
{
