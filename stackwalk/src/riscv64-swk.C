...
#include "stackwalk/src/riscv64-swk.h"
...
FrameFuncStepperImpl::FrameFuncStepperImpl(Walker *w, FrameStepper *parent_,
   ...
   helper(helper_)
{
   helper = helper_ ? helper_ : riscv64_LookupFuncStart::getLookupFuncStart(getProcessState());
}
...
gcframe_ret_t FrameFuncStepperImpl::getCallerFrame(const Frame &in, Frame &out)
{
  ...
  //on RISCV, the ra of the bottom stack is not NULL
...
{
   riscv64_LookupFuncStart *lookup = dynamic_cast<riscv64_LookupFuncStart*>(helper);
   if (lookup)
      lookup->releaseMe();
   else if (helper)
      delete helper;
                                                            ...
                                                            Address /*orig_ra*/, bool)
{
  ...
  // in dyninstAPI/src/inst-riscv64.C. This function may need to
...
std::map<Dyninst::PID, riscv64_LookupFuncStart*> riscv64_LookupFuncStart::all_func_starts;
...
riscv64_LookupFuncStart::riscv64_LookupFuncStart(ProcessState *proc_) :
...
riscv64_LookupFuncStart::~riscv64_LookupFuncStart()
{
   Dyninst::PID pid = proc->getProcessId();
   all_func_starts.erase(pid);
}

riscv64_LookupFuncStart *riscv64_LookupFuncStart::getLookupFuncStart(ProcessState *p)
{
   Dyninst::PID pid = p->getProcessId();
   std::map<Dyninst::PID, riscv64_LookupFuncStart*>::iterator i = all_func_starts.find(pid);
   if (i == all_func_starts.end()) {
      return new riscv64_LookupFuncStart(p);
   }
   (*i).second->ref_count++;
   return (*i).second;
}

void riscv64_LookupFuncStart::releaseMe()
{
   ref_count--;
   if (!ref_count)
      delete this;
...
FrameFuncHelper::alloc_frame_t riscv64_LookupFuncStart::allocatesFrame(Address addr)
{
   ...
   // on RISCVv8, if the target process is not broken by BP,
...
void riscv64_LookupFuncStart::updateCache(Address addr, alloc_frame_t result)
{
   cache.insert(addr, result);
}

bool riscv64_LookupFuncStart::checkCache(Address addr, alloc_frame_t &result)
{
