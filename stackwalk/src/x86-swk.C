...
bool
ProcSelf::getRegValue(Dyninst::MachRegister reg, THR_ID, Dyninst::MachRegisterVal& val)
{
...
#    ifdef _WIN64
    assert(0);  // FIXME WIN64-PORTING: Figure out a solution without inline asm, perhaps
                // StackWalk64()?
#    else
    __asm
    {
...
static gcframe_ret_t
HandleStandardFrame(const Frame& in, Frame& out, ProcessState* proc)
{
    ...
    {
        result            = proc->readMem(&ra_fp_pair32, in_fp, sizeof(ra_fp_pair32));
...
bool
Walker::checkValidFrame(const Frame& in, const Frame& out)
{
...
gcframe_ret_t
FrameFuncStepperImpl::getCallerFrame(const Frame& in, Frame& out)
{
    if(!in.getFP())
        return gcf_not_me;
...
LookupFuncStart*
LookupFuncStart::getLookupFuncStart(ProcessState* p)
{
...
unsigned
FrameFuncStepperImpl::getPriority() const
{
...
static unsigned char mov_esp_ebp[2][2] = { { 0x89, 0xe5 }, { 0x8b, 0xec } };
static unsigned char rex_mov_esp_ebp   = 0x48;

FrameFuncHelper::alloc_frame_t
LookupFuncStart::allocatesFrame(Address addr)
{
    LibAddrPair   lib;
    ...
    sw_printf("[%s:%d] - Function containing %lx has frame type %d/%d\n", FILE__,
              __LINE__, addr, (int) res.first, (int) res.second);
    updateCache(addr, res);
    return res;
}

void
LookupFuncStart::updateCache(Address addr, alloc_frame_t result)
{
    cache.insert(addr, result);
}

bool
LookupFuncStart::checkCache(Address addr, alloc_frame_t& result)
{
    return cache.lookup(addr, result);
}

void
LookupFuncStart::clear_func_mapping(Dyninst::PID pid)
{
...
gcframe_ret_t
DyninstInstrStepperImpl::getCallerFrameArch(const Frame& in, Frame& out, Address /*base*/,
...
gcframe_ret_t
DyninstDynamicStepperImpl::getCallerFrameArch(const Frame& in, Frame&     out,
                                              Address /*base*/, Address   lib_base,
                                              unsigned /*size*/, unsigned stack_height,
                                              bool aligned, Address orig_ra,
                                              bool pEntryExit)
{
    bool           result     = false;
    ...
    sw_printf("[%s:%d] - DyninstDynamicStepper with lib_base 0x%lx, stack-height %u, "
              "orig_ra 0x%lx, aligned %d %s\n",
              FILE__, __LINE__, lib_base, stack_height, orig_ra, aligned,
              pEntryExit ? "<entry/exit>" : "<normal>");
    sw_printf("[%s:%d] - incoming frame has RA 0x%lx, SP 0x%lx, FP 0x%lx\n", FILE__,
              __LINE__, in.getRA(), in.getSP(), in.getFP());
    ...
    if(0x0 != orig_ra)
    {
        ...
        out.setRA(orig_ra);
        out.setFP(in.getFP());
        out.setSP(in.getSP());  // Not really correct, but difficult to compute and
                                // unlikely to matter
    ...
    if(pEntryExit)
    {
        ...
        sw_printf(
            "[%s:%d] - DyninstDynamicStepper handled post entry/exit instrumentation\n",
            FILE__, __LINE__);
        return gcf_success;
    }
...
{
void
getTrapInstruction(char* buffer, unsigned buf_size, unsigned& actual_len,
                   bool include_return)
{
    if(include_return)
    {
