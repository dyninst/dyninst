...
Dyninst::Architecture ProcSelf::getArchitecture() {
  if (sizeof(void *) == 8) return Arch_x86_64;
  return Arch_x86;
}

static gcframe_ret_t HandleStandardFrame(const Frame &in, Frame &out,
                                         ProcessState *proc) {
...
bool Walker::checkValidFrame(const Frame &in, const Frame &out) {
  if (out.getSP() <= in.getSP() &&
      out.getSPLocation().location != loc_unknown) {
    sw_printf("[%s:%u] - Stackwalk went backwards, %lx to %lx\n", FILE__,
              __LINE__, in.getSP(), out.getSP());
    return false;
  }
  return true;
}
...
static int hash_address(Address a) { return (int)a; }

LookupFuncStart::LookupFuncStart(ProcessState *proc_)
    : FrameFuncHelper(proc_), cache(cache_size, hash_address) {
...
FrameFuncStepperImpl::FrameFuncStepperImpl(Walker *w, FrameStepper *parent_,
  ...
  helper = helper_ ? helper_
                   : LookupFuncStart::getLookupFuncStart(getProcessState());
}
...
FrameFuncHelper::alloc_frame_t LookupFuncStart::allocatesFrame(Address addr) {
  ...
  if (result) {
    sw_printf("[%s:%u] - Cached value for %lx is %d/%d\n", FILE__, __LINE__,
              addr, (int)res.first, (int)res.second);
    return res;
  }
  ...
  for (; cur < FUNCTION_PROLOG_TOCHECK; cur++) {
    if (proc->getAddressWidth() == 8) {
      if (mem[cur] != rex_mov_esp_ebp) continue;
      cur++;
    }

    if (cur + 1 >= FUNCTION_PROLOG_TOCHECK) break;
...
void LookupFuncStart::clear_func_mapping(Dyninst::PID pid) {
  ...
  if (i == all_func_starts.end()) return;
...
gcframe_ret_t DyninstDynamicStepperImpl::getCallerFrameArch(
    const Frame &in, Frame &out, Address /*base*/, Address lib_base,
    unsigned /*size*/, unsigned stack_height, bool aligned, Address orig_ra,
    bool pEntryExit) {
  bool result = false;
  ...
  sw_printf(
      "[%s:%u] - DyninstDynamicStepper with lib_base 0x%lx, stack-height %d, "
  ...
  if (0x0 != orig_ra) {
    ...
    sw_printf(
        "[%s:%u] - DyninstDynamicStepper handled frameless instrumentation\n",
        FILE__, __LINE__);
    return gcf_success;
  }
  ...
  if (pEntryExit) {
    ...
    sw_printf(
        "[%s:%u] - DyninstDynamicStepper handled post entry/exit "
        "instrumentation\n",
        FILE__, __LINE__);
    return gcf_success;
  }
  ...
  if (stack_height) {
    ...
    sw_printf(
        "[%s:%u] - Read SP %p from addr %p, using stack height of 0x%lx\n",
