...
class ProcDebugBG : public ProcDebug {
 protected:
  ...
  ///
  /// This implements basic BG debugger event -> stackwalker event translation.
  /// It's
  /// called by the BG implementation of ProcDebug::debug_get_event().
  ///
  /// Subclasses (future BG architecture implementations) should override to
  /// handle new
  ...
  /// BG currently has no asynchronous thread control, so the threads are
  /// stopped and
  /// continued all together.  This applies operations to all threads.
  template <class Functor>
  void for_all_threads(Functor f) {
 ...
 public:
  ...
  virtual bool setRegValue(Dyninst::MachRegister reg, Dyninst::THR_ID thread,
                           Dyninst::MachRegisterVal val);
 ...
 protected:
  ...
  DebuggerInterface::BG_Debugger_Msg::DataArea
      *mem_data;  // data from memory reads.
