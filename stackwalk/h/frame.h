...
class SW_EXPORT Frame : public AnnotatableDense {
 ...
 public:
  ...
  bool getLibOffset(std::string &lib, Dyninst::Offset &offset,
                    void *&symtab) const;
...
class SW_EXPORT FrameNode {
 ...
 private:
  ...
  FrameNode *parent;
  enum { FTFrame, FTThread, FTString, FTHead } frame_type;
  Frame frame;
  THR_ID thrd;
  Walker *walker;
  bool had_error;
 ...
 public:
  ...
  const Frame *getFrame() const {
    return (frame_type == FTFrame) ? &frame : NULL;
  }
  Frame *getFrame() { return (frame_type == FTFrame) ? &frame : NULL; }
  THR_ID getThread() const {
    return (frame_type == FTThread) ? thrd : NULL_LWP;
  }
