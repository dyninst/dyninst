...
private:
    ...
    Walker(ProcessState* p, StepperGroup* grp, SymbolLookup* sym, bool default_steppers,
    ...
    Dyninst::THR_ID getActualThread(Dyninst::THR_ID tid, bool& error);
...
public:
    ...
    static bool    newWalker(const std::vector<Dyninst::PID>& pids,
                             std::vector<Walker*>& walkers_out, std::string executable);
    static bool    newWalker(const std::vector<Dyninst::PID>& pids,
                             std::vector<Walker*>&            walkers_out);

    // Create an object that creates the specified process
    static Walker* newWalker(std::string exec_name, const std::vector<std::string>& argv);

    // Create an object with custom backend classes
    static Walker* newWalker(ProcessState* proc, StepperGroup* grp = NULL,
                             SymbolLookup* lookup = NULL, bool default_steppers = true);
