...
class SW_EXPORT Walker {
 private:
  ...
  Walker(ProcessState *p, StepperGroup *grp, SymbolLookup *sym,
         bool default_steppers, std::string exec_name);
  ...
  static ProcessState *createDefaultProcess(
      std::string exec_name = std::string(""));
  ...
  static ProcessState *createDefaultProcess(
      std::string exec_name, const std::vector<std::string> &argv);
