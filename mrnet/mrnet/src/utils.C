                  ...
                  const std::vector<std::string> &arg_list)
{
    ...
    if( (hostName == "") ||
        (hostName == "localhost") ||
        (in_nname == local_nname) ||
        (in_naddr == local_naddr) ){
...
int connectHost(int *sock_in, const std::string &hostname, unsigned short port)
{
    int sock=*sock_in;
                   ...
                   const std::string& in_hostname)
{
    ...
    memcpy( (void*)(&in.s_addr), (void*)(hp->h_addr_list[0]), hp->h_length);
    ...
    if(idx == -1){
        //no "." found in network_name
        mrn_printf(1, MCFL, stderr, "networkname is not fully qualified (%s);"
                   "); using IP address!\n", network_name.c_str() );
        network_name = ip_address;
        return -1;
    }
...
int getNetworkAddr(std::string &ipaddr,
                   const std::string hostname)
{
