...
int BuildNetwork( std::string cfgfile, std::string inode_exe,
                    std::string backend_exe );
int DoRoundtripLatencyExp( MC_Stream* stream,
                            unsigned int nIters,
                            unsigned int nBackends );
int DoReductionThroughputExp( MC_Stream* stream,
                            unsigned int nIters,
                            unsigned int nBackends );
...
main( int argc, char* argv[] )
{
    ...
    if( argc != 6 )
    {
        std::cerr << "Usage: " << argv[0]
                ...
                << " <topology file> <commnode exe> <backend exe>"
                << std::endl;
        return -1;
    }
    ...
    std::string commnode_exe = argv[4];
    std::string backend_exe = argv[5];
    
    // instantiate the network
    BuildNetwork( topology_file, commnode_exe, backend_exe );

    // get a broadcast communicator
    MC_Communicator* bcComm = MC_Communicator::get_BroadcastCommunicator();
    MC_Stream* stream = MC_Stream::new_Stream( bcComm, AGGR_INT_SUM_ID );
    assert( bcComm != NULL );
    assert( stream != NULL );
...
int
BuildNetwork( std::string cfgfile,
                std::string inode_exe,
                std::string backend_exe )
{
    mb_time startTime;
    mb_time endTime;
    ...
    startTime.set_time();
    if( MC_Network::new_Network( cfgfile.c_str(),
                                    inode_exe.c_str(),
                                    backend_exe.c_str() ) == -1 )
    {
        std::cerr << "FE: network initialization failed" << std::endl;
        MC_Network::error_str("FE:");
        exit(-1);
    }
    endTime.set_time();
...
int
DoRoundtripLatencyExp( MC_Stream* stream,
                            unsigned int nIters,
                            unsigned int nBackends )
{
    mb_time startTime;
...
int
DoReductionThroughputExp( MC_Stream* stream,
                            unsigned int nIters,
                            unsigned int nBackends )
{
    mb_time startTime;
