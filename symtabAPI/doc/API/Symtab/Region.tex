...
\end{apient}
\apidesc{
	These region types are drawn from the ELF standard and are not platform independent. 
}
...
\end{apient}
\apidesc{
Set the raw data pointer of the region to newPtr. rawsize represents the size of the raw data buffer. 
Returns true if success or false when unable to set/change the raw data of the region. Implicitly changes the disk and memory sizes of the region.
}
