...
\begin{apient}
static Symtab* findOpenSymtab(const std::string& name);
\end{apient}
...
\begin{apient}
bool findSymbol(std::vector <Symbol*>& ret,
                const std::string& name,
...
\begin{apient}
std::vector<Symbol*> findSymbolByOffset(Offset offset);
\end{apient}
...
\begin{apient}
bool getAllSymbolsByType(std::vector<Symbol*>& ret,
...
\begin{apient}
bool getMappedRegions(std::vector<Region*>& ret) const;
\end{apient}
\apidesc{
...
\begin{apient}
bool emit(const std::string& file, unsigned flag = 0);
\end{apient}
\apidesc{
...
\begin{apient}
bool addLibraryPrereq(const std::string& lib);
\end{apient}
\apidesc{
...
\end{apient}
\apidesc{
\code{AddressRange} is a \code{std::pair<Offset, Offset>} giving the low and
high bounds of a range.
...
\begin{apient}
bool findLocalVariable(std::vector<localVar*>& vars,
                       const std::string& name);
\end{apient}
\apidesc{
