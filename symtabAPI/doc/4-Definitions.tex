...
Otherwise the result of the demangler is returned with any function clone suffixes appended to the result.
...
The pretty name without clone suffixes can be obtained by running the command line tool \code{c++filt} as \code{c++filt -i -p \emph{name}}, or using the libiberty library function \code{cplus\_demangle(name, DMGL\_AUTO | DMGL\_ANSI)}.
...
Otherwise the result of the demangler is returned.
...
The typed name can be obtained by running the command line tool \code{c++filt} as \code{c++filt -i \emph{name}}, or using the libiberty library function \code{cplus\_demangle(name, DMGL\_AUTO | DMGL\_ANSI | DMGL\_PARAMS)}.
