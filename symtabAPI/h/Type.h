...
class SYMTAB_EXPORT Field : public Serializable,
                            public FIELD_ANNOTATABLE_CLASS {
 ...
 public:
  ...
  Serializable *serialize_impl(SerializerBase *sb, const char *tag = "Field")
...
class SYMTAB_EXPORT Type : public Serializable, public TYPE_ANNOTATABLE_CLASS {
 ...
 public:
  ...
  Serializable *serialize_impl(SerializerBase *, const char * = "Type")
      THROW_SPEC(SerializerError);
...
class SYMTAB_EXPORT fieldListType : public Type, public fieldListInterface {
 ...
 public:
  ...
  void addField(std::string fieldname, Type *type, int offsetVal = -1,
                visibility_t vis = visUnknown);
  void addField(unsigned num, std::string fieldname, Type *type,
  ...
  void serialize_fieldlist(Dyninst::SerializerBase *,
                           const char * = "fieldListType")
      THROW_SPEC(SerializerError);
  // void addField(const std::string &fieldname,  dataClass typeDes,
  //               Type *type, int offset, int size, visibility_t vis =
  //               visUnknown);
};
...
class SYMTAB_EXPORT rangedType : public Type, public rangedInterface {
 ...
 protected:
  // rangedType(const std::string &name, typeId_t ID, dataClass typeDes, int
  // size, const char *low, const char *hi);
  ...
  rangedType(std::string &name, dataClass typeDes, int size, unsigned long low,
             unsigned long hi);
  void serialize_ranged(SerializerBase *, const char * = "rangedType")
      THROW_SPEC(SerializerError);
...
class SYMTAB_EXPORT derivedType : public Type, public derivedInterface {
 ...
 public:
  ...
  void serialize_derived(SerializerBase *, const char * = "derivedType")
      THROW_SPEC(SerializerError);
};
...
class SYMTAB_EXPORT typeCommon : public fieldListType {
 ...
 protected:
  ...
  // void postFieldInsert(int offset, int nsize) { if ((unsigned int) (offset +
  // nsize) > size_) size_ = offset + nsize; }
  void fixupUnknowns(Module *);
...
class SYMTAB_EXPORT CBlock : public Serializable, public AnnotatableSparse {
 ...
 public:
  ...
  Serializable *serialize_impl(SerializerBase *, const char *tag = "CBlock")
      THROW_SPEC(SerializerError);
};
...
class SYMTAB_EXPORT typeStruct : public fieldListType {
 ...
 public:
  ...
  static typeStruct *create(std::string &name,
...
class SYMTAB_EXPORT typeUnion : public fieldListType {
 ...
 public:
  ...
  static typeUnion *create(
      std::string &name,
      std::vector<std::pair<std::string, Type *> *> &fieldNames,
      Symtab *obj = NULL);
...
class SYMTAB_EXPORT typeTypedef : public derivedType {
 ...
 public:
  typeTypedef();
  typeTypedef(typeId_t ID, Type *base, std::string name,
...
class SYMTAB_EXPORT typeSubrange : public rangedType {
 private:
  // typeSubrange(int ID, int size, const char *low, const char *hi, const char
  // *name);
 public:
  typeSubrange();
...
class SYMTAB_EXPORT typeArray : public rangedType {
 ...
 public:
  typeArray();
  typeArray(typeId_t ID, Type *base, long low, long hi, std::string name,
            unsigned int sizeHint = 0);
  typeArray(Type *base, long low, long hi, std::string name,
            unsigned int sizeHint = 0);
  static typeArray *create(std::string &name, Type *typ, long low, long hi,
                           Symtab *obj = NULL);
