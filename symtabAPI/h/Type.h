...
public:
    ...
    boost::shared_ptr<Type> reshare()
    {
        ...
        if(!r)
            self_ = (r = boost::shared_ptr<Type>(this));
        return r;
    }
...
public:
    ...
    public:
        ...
        : ptr(p)
        {}
        operator boost::shared_ptr<Type>() { return ptr->reshare(); }
...
class SYMTAB_EXPORT fieldListType
: public Type
...
{
    return dynamic_cast<fieldListType&>(*this);
}
bool
Type::isFieldListType()
{
    return dynamic_cast<fieldListType*>(this) != NULL;
}

class SYMTAB_EXPORT rangedType
: public Type
, public rangedInterface
{
protected:
    unsigned long low_;
    unsigned long hi_;
...
protected:
    // rangedType(const std::string &name, typeId_t ID, dataClass typeDes, int size, const
    // char *low, const char *hi);
    rangedType(std::string& name, typeId_t ID, dataClass typeDes, int size,
               unsigned long low, unsigned long hi);
    rangedType(std::string& name, dataClass typeDes, int size, unsigned long low,
               unsigned long hi);
...
Type::asRangedType()
{
    return dynamic_cast<rangedType&>(*this);
}
bool
Type::isRangedType()
{
    return dynamic_cast<rangedType*>(this) != NULL;
}

class SYMTAB_EXPORT derivedType
: public Type
...
Type::isDerivedType()
{
    return dynamic_cast<derivedType*>(this) != NULL;
}
...
public:
    ...
    static typeEnum* create(std::string&                                name,
                            dyn_c_vector<std::pair<std::string, int>*>& elements,
                            Symtab*                                     obj = NULL);
    static typeEnum* create(std::string& name, dyn_c_vector<std::string>& constNames,
                            Symtab* obj);
...
Type::isEnumType()
{
    return dynamic_cast<typeEnum*>(this) != NULL;
}
...
public:
    ...
    static typeFunction* create(std::string& name, boost::shared_ptr<Type> retType,
                                dyn_c_vector<boost::shared_ptr<Type>>& paramTypes,
                                Symtab*                                obj = NULL);
    static typeFunction* create(std::string& n, Type* rt, dyn_c_vector<Type*>& p,
                                Symtab* o = NULL)
    {
...
public:
    ...
    typeScalar(typeId_t ID, unsigned int size, std::string name = "",
               bool isSigned = false)
    : Type(name, ID, dataScalar)
    {
...
protected:
    ...
    // void postFieldInsert(int offset, int nsize) { if ((unsigned int) (offset + nsize) >
    // size_) size_ = offset + nsize; }
    void fixupUnknowns(Module*);
...
Type::isCommonType()
{
    return dynamic_cast<typeCommon*>(this) != NULL;
}
...
public:
    ...
    static typeStruct* create(
        std::string&                                                    name,
        dyn_c_vector<std::pair<std::string, boost::shared_ptr<Type>>*>& flds,
        Symtab*                                                         obj = NULL);
    static typeStruct* create(std::string&                                  n,
                              dyn_c_vector<std::pair<std::string, Type*>*>& f,
                              Symtab*                                       o = NULL)
    {
...
Type::isStructType()
{
    return dynamic_cast<typeStruct*>(this) != NULL;
}
...
public:
    ...
    static typeUnion* create(
        std::string&                                                    name,
        dyn_c_vector<std::pair<std::string, boost::shared_ptr<Type>>*>& fieldNames,
        Symtab*                                                         obj = NULL);
    static typeUnion* create(std::string&                                  n,
                             dyn_c_vector<std::pair<std::string, Type*>*>& f,
                             Symtab*                                       o = NULL)
    {
        dyn_c_vector<std::pair<std::string, boost::shared_ptr<Type>>*> fp(f.size());
...
public:
    ...
    {}
    static typePointer* create(std::string& name, boost::shared_ptr<Type> ptr,
                               Symtab* obj = NULL);
    ...
    static typePointer* create(std::string& name, boost::shared_ptr<Type> ptr, int size,
                               Symtab* obj = NULL);
    static typePointer* create(std::string& n, Type* p, int s, Symtab* o = NULL)
    {
...
public:
    ...
    static typeSubrange* create(std::string& name, int size, long low, long hi,
                                Symtab* obj = NULL);
    bool isCompatible(boost::shared_ptr<Type> x) { return isCompatible(x.get()); }
    bool isCompatible(Type* otype);
};
...
public:
    typeArray();
    typeArray(typeId_t ID, boost::shared_ptr<Type> base, long low, long hi,
              std::string name, unsigned int sizeHint = 0);
    typeArray(typeId_t i, Type* b, long l, long h, std::string n, unsigned int s = 0)
    : typeArray(i, b->reshare(), l, h, n, s)
    {}
    typeArray(boost::shared_ptr<Type> base, long low, long hi, std::string name,
              unsigned int sizeHint = 0);
    typeArray(Type* b, long l, long h, std::string n, unsigned int s = 0)
    : typeArray(b->reshare(), l, h, n, s)
    {}
    static typeArray* create(std::string& name, boost::shared_ptr<Type> typ, long low,
                             long hi, Symtab* obj = NULL);
...
Type::isArrayType()
{
    return dynamic_cast<typeArray*>(this) != NULL;
}
