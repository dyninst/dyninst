...
#define EMIT_STATIC_ASSERT                                                               \
    "This function is currently unimplemented on this architecture."
...
bool
emitElfStatic::archSpecificRelocation(Symtab*, Symtab*, char*, relocationEntry&, Offset,
...
Offset
emitElfStatic::layoutTLSImage(Offset, Region*, Region*, LinkMap&)
{
    assert(!EMIT_STATIC_ASSERT);
    return 0;
}
...
void
emitElfStatic::cleanupTLSRegionOffsets(map<Region*, LinkMap::AllocPair>&, Region*,
                                       Region*)
{
    assert(!EMIT_STATIC_ASSERT);
}
...
bool
emitElfStatic::isGOTRelocation(unsigned long)
{
    assert(!EMIT_STATIC_ASSERT);
    return false;
}
...
#ifdef OLD_VERSION
Offset
emitElfStatic::getGOTSize(LinkMap&)
{
    assert(!EMIT_STATIC_ASSERT);
    return 0;
}
...
#ifdef OLD_VERSION
void
emitElfStatic::buildGOT(LinkMap&)
{
    assert(!EMIT_STATIC_ASSERT);
}
#endif
...
bool
emitElfStatic::isConstructorRegion(Region*)
{
    assert(!EMIT_STATIC_ASSERT);
    return false;
}
...
Offset
emitElfStatic::layoutNewCtorRegion(LinkMap&)
{
    assert(!EMIT_STATIC_ASSERT);
    return 0;
}
...
bool
emitElfStatic::isDestructorRegion(Region*)
{
    assert(!EMIT_STATIC_ASSERT);
    return false;
}
...
Offset
emitElfStatic::layoutNewDtorRegion(LinkMap&)
{
    assert(!EMIT_STATIC_ASSERT);
    return 0;
}
...
// steve: added
Offset
emitElfStatic::allocStubRegions(LinkMap&, Offset)
{
    assert(!EMIT_STATIC_ASSERT);
    return 0;
}
