...
void setVersion() {
  ...
  for (unsigned i = 0; i < nEntries; i++) {
    if (!strstr(maps[i].path, "libelf")) continue;
  ...
  if (libelfso0Flag && libelfso1Flag) {
    fprintf(stderr,
            "WARNING: SymtabAPI is linked with libelf.so.0 and "
  ...
  if (libelfso1Flag) {
    fprintf(stderr,
            "WARNING: SymtabAPI on FreeBSD is known to function "
...
emitElf<ElfTypes>::emitElf(Elf_X *oldElfHandle_, bool isStripped_, Object *obj_,
  ...
  //    (createNewPhdrRegion)
  //    (c) library_adjust - create room for a new program header in a
  //    position-indepdent library
  //                         by increasing all virtual addresses for the library
  // 2) Use existing Phdr (used in bleugene - will be handled in function
  // fixPhdrs)
...
bool emitElf<ElfTypes>::hasPHdrSectionBug() {
  if (movePHdrsFirst) return false;
  if (!libelfso1Flag) return false;
...
template <class ElfTypes>
bool emitElf<ElfTypes>::createElfSymbol(Symbol *symbol, unsigned strIndex,
  ...
  if (elfSymType(symbol) == STT_TLS) {
    offset_adjust =
        0;  // offset relative to TLS section, any new entries go at end
  } else {
  ...
  sym->st_other =
      ELF64_ST_VISIBILITY(elfSymVisibility(symbol->getVisibility()));
  sym->st_info = (unsigned char)ELF64_ST_INFO(elfSymBind(symbol->getLinkage()),
                                              elfSymType(symbol));
  ...
  if (dynSymFlag) {
    ...
    if (!symbol->getVersionFileName(fileName)) {
      ...
      if (!symbol->getVersions(vers)) {
        ...
        if (vers->size() > 0) {
          // new verdef entry
          mpos += sprintf(mpos, "verdef: symbol=%s  version=%s ",
                          symbol->getMangledName().c_str(), (*vers)[0].c_str());
      ...
      if (!symbol->getVersions(vers) || (vers && vers->size() != 1)) {
        ...
        if (fileName != "") {
          ...
          if (!symbol->getReferringSymbol()->getSymtab()->isExec()) {
            if (find(unversionedNeededEntries.begin(),
              ...
              mpos +=
                  sprintf(mpos, "  new unversioned: %s\n", fileName.c_str());
        ...
        if (vers) {
          ...
          if (verneedEntries.find(fileName) != verneedEntries.end()) {
            if (verneedEntries[fileName].find((*vers)[0]) !=
                verneedEntries[fileName].end()) {
              mpos += sprintf(mpos, "  vernum: %d\n",
                              verneedEntries[fileName][(*vers)[0]]);
              versionSymTable.push_back(
                  (unsigned short)verneedEntries[fileName][(*vers)[0]]);
            } else {
              mpos += sprintf(mpos, "  new entry #%d: %s [%s]\n", curVersionNum,
                              (*vers)[0].c_str(), fileName.c_str());
...
template <class ElfTypes>
void emitElf<ElfTypes>::renameSection(const std::string &oldStr,
...
bool emitElf<ElfTypes>::driver(std::string fName) {
  ...
  for (auto eb = exceptions.begin(); eb != exceptions.end(); ++eb) {
    // cerr << **eb << endl;
  }
  ...
  for (scncount = 0; (scn = elf_nextscn(oldElf, scn)); scncount++) {
    ...
    if (foundSec->isDirty()) {
      ...
      memcpy(newdata->d_buf, foundSec->getPtrToRawData(),
             foundSec->getDiskSize());
    ...
    if (BSSExpandFlag) {
      // Add the expanded SHT_NOBITS section size if the section comes after
      // those sections
      if (scncount > NOBITSstartPoint) newshdr->sh_offset += NOBITStotalsize;

      // Expand the NOBITS sections in file & and change the type from
      // SHT_NOBITS to SHT_PROGBITS
      if (shdr->sh_type == SHT_NOBITS) {
        ...
        if (NOBITSstartPoint == oldEhdr->e_shnum) NOBITSstartPoint = scncount;
        NOBITStotalsize += shdr->sh_size;
      }
    }

    vector<vector<unsigned long> > moveSecAddrRange =
        obj->getObject()->getMoveSecAddrRange();

    for (unsigned i = 0; i != moveSecAddrRange.size(); i++) {
      if ((moveSecAddrRange[i][0] == shdr->sh_addr) ||
          (shdr->sh_addr >= moveSecAddrRange[i][0] &&
           shdr->sh_addr < moveSecAddrRange[i][1])) {
        newshdr->sh_type = SHT_PROGBITS;
    ...
    if (newshdr->sh_offset > 0) newshdr->sh_offset += dirtySecsChange;
    ...
    if (BSSExpandFlag && newshdr->sh_addr) {
      unsigned newOff = newshdr->sh_offset -
                        (newshdr->sh_offset & (pgSize - 1)) +
                        (newshdr->sh_addr & (pgSize - 1));
      if (newOff < newshdr->sh_offset) newOff += pgSize;
  ...
  if (!createNonLoadableSections(newshdr)) return false;
  ...
  for (scncount = 0; (scn = elf_nextscn(newElf, scn)); scncount++) {
    ...
    if (changeMapping[scncount + 1] == 0 && secInfoMapping[scncount + 1] != 0) {
      // For REL and RELA section, info field is a section index - hence of the
      // index changes, the info field must change.
  ...
  if (newEhdr->e_shoff % 8) newEhdr->e_shoff += 8 - (newEhdr->e_shoff % 8);

  // copy program headers
  oldPhdr = ElfTypes::elf_getphdr(oldElf);
...
template <class ElfTypes>
void emitElf<ElfTypes>::createNewPhdrRegion(
  ...
  if (createNewPhdr) phdr_size += oldEhdr->e_phentsize;

  unsigned align = 0;
  if (currEndOffset % 8) align = 8 - (currEndOffset % 8);
  ...
  for (unsigned i = 0; i < oldEhdr->e_phnum; i++) {
    ...
    if (createNewPhdr && !added_new_sec && firstNewLoadSec) {
      if (i + 1 == oldEhdr->e_phnum) {
        ...
        insert_phdr = newPhdr + 1;
      } else if (i == 0 && old->p_type == PT_LOAD &&
                 newSegmentStart < old->p_vaddr) {
    ...
    if (insert_phdr) {
      ...
      newSeg.p_filesz =
          loadSecTotalSize - (newSegmentStart - firstNewLoadSec->sh_addr);
      ...
      added_new_sec = true;
      rewrite_printf(
          "Added New program header : offset 0x%lx,addr 0x%lx file Size 0x%lx "
          "memsize 0x%lx \n",
    ...
    if (old->p_type == PT_DYNAMIC) {
      ...
      newPhdr->p_filesz =
          loadSecTotalSize - (newSegmentStart - firstNewLoadSec->sh_addr);
      newPhdr->p_memsz = (currEndAddress - firstNewLoadSec->sh_addr) -
                         (newSegmentStart - firstNewLoadSec->sh_addr);
    ...
    /* For BlueGeneQ statically linked binary, we cannot create a new LOAD
       section,
        hence we extend existing LOAD section to include instrmentation.
           A new LOAD section must be aligned to 1MB in BlueGene.
         But if we create a LOAD segment after 1MB, the TOC pointer will no
       longer be able to reach the new segment,
         as we have only 4 byte offset from TOC */

    if (0 && isBlueGeneQ && isStaticBinary && last_load_segment) {
      // add new load to this segment
      newPhdr->p_filesz = (newSeg.p_offset - newPhdr->p_offset) +
                          loadSecTotalSize -
                          (newSegmentStart - firstNewLoadSec->sh_addr);
      newPhdr->p_memsz = (newSegmentStart - newPhdr->p_vaddr) +
                         (currEndAddress - firstNewLoadSec->sh_addr) -
                         (newSegmentStart - firstNewLoadSec->sh_addr);
      newPhdr->p_flags = PF_R + PF_W + PF_X;
      ...
      if (insert_phdr) newPhdr++;
    }

    rewrite_printf(
        "Existing program header: type %u (%s), offset 0x%lx, addr 0x%lx\n",
        newPhdr->p_type, phdrTypeStr(newPhdr->p_type).c_str(),
...
// This method updates the .dynamic section to reflect the changes to the
// relocation section
template <class ElfTypes>
 ...
 */
template <class ElfTypes>
void emitElf<ElfTypes>::updateSymbols(Elf_Data *symtabData, Elf_Data *strData,
  ...
  if (symtabData && strData && loadSecsSize) {
    ...
    for (unsigned int i = 0; i < symtabData->d_size / (sizeof(Elf_Sym));
         i++, symPtr++) {
      if (!(strcmp("_end", (char *)strData->d_buf + symPtr->st_name))) {
        if (newSegmentStart >= symPtr->st_value) {
          symPtr->st_value +=
              ((newSegmentStart - symPtr->st_value) + loadSecsSize);
...
template <class ElfTypes>
bool emitElf<ElfTypes>::createLoadableSections(
  ...
  for (unsigned i = 0; i < newSecs.size(); i++) {
    ...
    if (shdr->sh_type == SHT_NOBITS) {
      ...
      newshdr->sh_offset =
          firstNewLoadSec->sh_offset + library_adjust +
        ...
        newSecs[i]->getRegionType() == Region::RT_PLTREL)  // Relocation section
    {
      ...
      if (newSecs[i]->getRegionType() == Region::RT_REL)
        ...
        updateDynamic(DT_JMPREL, newshdr->sh_addr);
               ...
               newSecs[i]->getRegionType() ==
                   Region::RT_PLTRELA)  // Relocation section
    {
      ...
      if (newSecs[i]->getRegionType() == Region::RT_RELA)
        ...
        updateDynamic(DT_JMPREL, newshdr->sh_addr);
    } else if (newSecs[i]->getRegionType() ==
               Region::RT_STRTAB)  // String table Section
    {
      ...
      newdata->d_align = 4;
      dynsymData = newdata;
      newshdr->sh_link = secNames.size();  //.symtab section should have sh_link
                                           //= index of .strtab for .dynsym
      newshdr->sh_flags = SHF_ALLOC;
    ...
    // Check to make sure the (vaddr for the start of the new segment - the
    // offset) is page aligned
    if (!firstNewLoadSec) {
      Offset newoff = newshdr->sh_offset - (newshdr->sh_offset & (pgSize - 1)) +
                      (newshdr->sh_addr & (pgSize - 1));
      if (newoff < newshdr->sh_offset) newoff += pgSize;
      extraAlignSize += newoff - newshdr->sh_offset;
      newshdr->sh_offset = newoff;
      ...
      // Bluegene compute nodes have a 1MB alignment restructions on PT_LOAD
      // section
      // When we are replaceing PT_NOTE with PT_LOAD, we need to make sure the
      // new PT_LOAD is 1MB aligned
      if (!createNewPhdr && replaceNOTE) {
        Offset newaddr = newshdr->sh_addr - (newshdr->sh_addr & (0x100000 - 1));
        if (newaddr < newshdr->sh_addr) newaddr += 0x100000;
    ...
    memcpy(newdata->d_buf, newSecs[i]->getPtrToRawData(),
           newSecs[i]->getDiskSize());
    newdata->d_off = 0;
    newdata->d_size = newSecs[i]->getDiskSize();
    if (!newdata->d_align) newdata->d_align = newshdr->sh_addralign;
    ...
    shdr = newshdr;
    if (!firstNewLoadSec) firstNewLoadSec = shdr;
...
bool emitElf<ElfTypes>::createNonLoadableSections(Elf_Shdr *&shdr) {
  ...
  for (unsigned i = 0; i < nonLoadableSecs.size(); i++) {
    ...
    if (nonLoadableSecs[i]->getRegionType() == Region::RT_TEXT)  // Text Section
    {
      ...
      newshdr->sh_entsize = 1;
      newdata->d_type = ELF_T_BYTE;
    } else if (nonLoadableSecs[i]->getRegionType() ==
               Region::RT_DATA)  // Data Section
    {
      newshdr->sh_type = SHT_PROGBITS;
      ...
      newdata->d_type = ELF_T_BYTE;
               ...
               Region::RT_REL)  // Relocations section
    {
      newshdr->sh_type = SHT_REL;
      newshdr->sh_flags = SHF_WRITE;
      newshdr->sh_entsize = sizeof(Elf_Rel);
      newdata->d_type = ELF_T_BYTE;
               ...
               Region::RT_RELA)  // Relocations section
    {
      newshdr->sh_type = SHT_RELA;
      newshdr->sh_flags = SHF_WRITE;
      newshdr->sh_entsize = sizeof(Elf_Rela);
      ...
      newshdr->sh_link =
          secNames
              .size();  //.symtab section should have sh_link = index of .strtab
      newshdr->sh_flags = 0;
    } else if (nonLoadableSecs[i]->getRegionType() ==
               Region::RT_STRTAB)  // String table Section
    {
  ...
  newshdr->sh_entsize = sizeof(Elf_Sym);
  newdata->d_type = ELF_T_SYM;
  //newshdr->sh_link = newSecSize+i+1;   //.symtab section should have sh_link =
  index of .strtab
  newshdr->sh_flags=  SHF_ALLOC | SHF_WRITE;
      }*/
 ...
 * Method - For every symbol call createElfSymbol to get a Elf_Sym
 * corresposnding
 *          to a Symbol object. Accumulate all and their names to form the
 * sections
...
bool emitElf<ElfTypes>::createSymbolTables(vector<Symbol *> &allSymbols) {
  ...
  dyn_hash_map<int, Region *> secTagRegionMapping =
      obj->getObject()->getTagRegionMapping();

  Region *sec;
  auto foundRegion = secTagRegionMapping.find(DT_STRTAB);
  if ((foundRegion != secTagRegionMapping.end()) &&
      (foundRegion->second != NULL)) {
    // .dynstr
  ...
  for (auto iter = elibs.begin(); iter != elibs.end(); ++iter) {
    addDTNeeded(*iter);
  }
  ...
  // We should increment this here, but for reasons I don't understand we create
  // it with a size of
  // 1.
  // symbolNamesLength++;
  sym->st_name = 0;
  ...
  if (obj->isStaticBinary()) {
    ...
    if (newRegs.size()) {
      ...
      if (!emitElfUtils::updateHeapVariables(obj,
                                             lastRegionAddr + lastRegionSize)) {
        return false;
      }
    }
  }
  ...
  /* We regenerate symtab and symstr section. We do not
     ...
     new index.
     On the other hand, we do not regenerate dynsym and dynstr section. We copy
     over
     old symbols and string in the original order as it was in the
     original binary. We preserve sh_index of Elf symbols (from Symbol's
     strIndex). We append
     new symbols and string that we create for the new binary (targ*, versions
     etc).
  */
  ...
  for (i = 0; i < allDynSymbols.size(); i++) {
    createElfSymbol(allDynSymbols[i], allDynSymbols[i]->getStrIndex(),
                    dynsymbols, true);
    dynSymNameMapping[allDynSymbols[i]->getMangledName().c_str()] =
        allDynSymbols[i]->getIndex();
    dynsymVector.push_back(allDynSymbols[i]);
  }

  // reconstruct .symtab section
  Elf_Sym *syms = (Elf_Sym *)malloc(symbols.size() * sizeof(Elf_Sym));
  for (i = 0; i < symbols.size(); i++) syms[i] = *(symbols[i]);
  ...
  if (!isStripped) {
    ...
    if (obj->findRegion(sec, ".symtab"))
      ...
      obj->addRegion(0, syms, symbols.size() * sizeof(Elf_Sym), ".symtab",
                     Region::RT_SYMTAB);
  } else
    obj->addRegion(0, syms, symbols.size() * sizeof(Elf_Sym), ".symtab",
                   Region::RT_SYMTAB);
  ...
  if (dynsymbols.size() == 1) return true;

  if (!obj->isStaticBinary()) {
    // reconstruct .dynsym section
    Elf_Sym *dynsyms = (Elf_Sym *)malloc(dynsymbols.size() * sizeof(Elf_Sym));
    for (i = 0; i < dynsymbols.size(); i++) dynsyms[i] = *(dynsymbols[i]);

    Elf_Half *symVers;
    char *verneedSecData, *verdefSecData;
    unsigned verneedSecSize = 0, verdefSecSize = 0;

    createSymbolVersions(symVers, verneedSecData, verneedSecSize, verdefSecData,
                         verdefSecSize, dynsymbolNamesLength, dynsymbolStrs);
    ...
    if (hashsecSize) {
      ...
      if (secTagRegionMapping.find(DT_HASH) != secTagRegionMapping.end()) {
        ...
        obj->addRegion(0, hashsecData, hashsecSize * sizeof(Elf_Word), name,
                       Region::RT_HASH, true);
      } else if (secTagRegionMapping.find(0x6ffffef5) !=
                 secTagRegionMapping.end()) {
      ...
      /*
              if( !object->hasReldyn() && !object->hasReladyn() ) {
                  if( object->getRelType() == Region::RT_REL ) {
                      ...
                      assert(!"Relocation type not set to known RT_REL or
         RT_RELA.");
                  }
              }
      */
      createDynamicSection(sec->getPtrToRawData(), sec->getDiskSize(),
                           dynsecData, dynsecSize, dynsymbolNamesLength,
                           dynsymbolStrs);
    }

    if (!dynsymbolNamesLength) return true;
    ...
    obj->addRegion(0, dynsyms, dynsymbols.size() * sizeof(Elf_Sym), name,
                   Region::RT_SYMTAB, true);
    ...
    obj->addRegion(0, dynstr, dynsymbolNamesLength, name, Region::RT_STRTAB,
                   true);
    ...
    obj->addRegion(0, symVers, versionSymTable.size() * sizeof(Elf_Half), name,
                   Region::RT_SYMVERSIONS, true);
    ...
    if (verneedSecSize) {
      ...
      obj->addRegion(0, verneedSecData, verneedSecSize, name,
                     Region::RT_SYMVERNEEDED, true);
    }

    if (verdefSecSize) {
      obj->addRegion(0, verdefSecData, verdefSecSize, ".gnu.version_d",
                     Region::RT_SYMVERDEF, true);
    }
    ...
    if (!has_plt) {
      ...
      createRelocationSections(object->getPLTRelocs(), false,
                               dynSymNameMapping);
      createRelocationSections(object->getDynRelocs(), true, dynSymNameMapping);
    ...
    // add .dynamic section
    if (dynsecSize)
      obj->addRegion(0, dynsecData, dynsecSize * sizeof(Elf_Dyn), ".dynamic",
                     Region::RT_DYNAMIC, true);
  }
...
template <class ElfTypes>
void emitElf<ElfTypes>::createRelocationSections(
  ...
  if (isDynRelocs && newSecs.size()) {
    ...
    for (i = newSecs.begin(); i != newSecs.end(); i++) {
      std::copy((*i)->getRelocations().begin(), (*i)->getRelocations().end(),
  ...
  Elf_Rel *rels = (Elf_Rel *)malloc(sizeof(Elf_Rel) *
                                    (relocation_table.size() + newRels.size()));
  Elf_Rela *relas = (Elf_Rela *)malloc(
      sizeof(Elf_Rela) * (relocation_table.size() + newRels.size()));
  j = 0;
  k = 0;
  l = 0;
  m = 0;
  ...
  for (i = 0; i < relocation_table.size(); i++) {
    ...
    if ((object->getRelType() == Region::RT_REL) &&
        (relocation_table[i].regionType() == Region::RT_REL)) {
      ...
      if (!sym_name.empty()) {
        dyn_hash_map<string, unsigned long>::iterator j =
            dynSymNameMapping.find(sym_name);
        if (j != dynSymNameMapping.end())
          sym_offset = j->second;
        else {
          Symbol *sym = relocation_table[i].getDynSym();
          if (sym) sym_offset = sym->getIndex();
        }
      }

      if (sym_offset) {
        rels[j].r_info = ElfTypes::makeRelocInfo(
            sym_offset, relocation_table[i].getRelType());
      } else {
        rels[j].r_info = ElfTypes::makeRelocInfo(
            (unsigned long)STN_UNDEF, relocation_table[i].getRelType());
      }
      j++;
    } else if ((object->getRelType() == Region::RT_RELA) &&
               (relocation_table[i].regionType() == Region::RT_RELA)) {
      ...
      if (!sym_name.empty()) {
        ...
        if (j != dynSymNameMapping.end()) {
          ...
          if (sym) {
            ...
            if (j != dynSymNameMapping.end()) sym_offset = j->second;
          }
        }
      }
      if (sym_offset) {
        relas[k].r_info = ElfTypes::makeRelocInfo(
            sym_offset, relocation_table[i].getRelType());
      } else {
        relas[k].r_info = ElfTypes::makeRelocInfo(
            (unsigned long)STN_UNDEF, relocation_table[i].getRelType());
  ...
  for (i = 0; i < newRels.size(); i++) {
    if ((object->getRelType() == Region::RT_REL) &&
        (newRels[i].regionType() == Region::RT_REL)) {
      rels[j].r_offset = newRels[i].rel_addr() + library_adjust;
      if (dynSymNameMapping.find(newRels[i].name()) !=
          dynSymNameMapping.end()) {
        rels[j].r_info = ElfTypes::makeRelocInfo(
            dynSymNameMapping[newRels[i].name()], newRels[i].getRelType());
      } else {
      ...
      l++;
               ...
               (newRels[i].regionType() == Region::RT_RELA)) {
  ...
  if (isDynRelocs && object->getRelType() == Region::RT_REL) {
    new_name = ".rel.dyn";
    dtype = DT_REL;
    rtype = Region::RT_REL;
    dsize_type = DT_RELSZ;
  ...
  if (dynamicSecData.find(dsize_type) != dynamicSecData.end())
    ...
    old_reloc_size = 0;
  dynamic_reloc_size =
      old_reloc_size + l * sizeof(Elf64_Rel) + m * sizeof(Elf64_Rela);
  string name;
...
template <class ElfTypes>
void emitElf<ElfTypes>::createSymbolVersions(
    Elf_Half *&symVers, char *&verneedSecData, unsigned &verneedSecSize,
    char *&verdefSecData, unsigned &verdefSecSize,
  ...
  for (; it != verneedEntries.end(); it++)
    verneedSecSize +=
        sizeof(Elf_Verneed) + sizeof(Elf_Vernaux) * it->second.size();

  verneedSecData = (char *)malloc(verneedSecSize);
  unsigned curpos = 0;
  verneednum = 0;
  std::vector<std::string>::iterator dit;
  for (dit = unversionedNeededEntries.begin();
       dit != unversionedNeededEntries.end(); dit++) {
  ...
  for (it = verneedEntries.begin(); it != verneedEntries.end(); it++) {
    Elf_Verneed *verneed =
        reinterpret_cast<Elf_Verneed *>(verneedSecData + curpos);
    ...
    verneed->vn_next =
        sizeof(Elf_Verneed) + it->second.size() * sizeof(Elf_Vernaux);
    if (curpos + verneed->vn_next == verneedSecSize) verneed->vn_next = 0;
    verneednum++;
    int i = 0;
  ...
  for (iter = verdefEntries.begin(); iter != verdefEntries.end(); iter++)
    verdefSecSize += sizeof(Elf_Verdef) +
                     sizeof(Elf_Verdaux) * verdauxEntries[iter->second].size();
  ...
  for (iter = verdefEntries.begin(); iter != verdefEntries.end(); iter++) {
    ...
    verdef->vd_next = sizeof(Elf_Verdef) +
                      verdauxEntries[iter->second].size() * sizeof(Elf_Verdaux);
    if (curpos + verdef->vd_next == verdefSecSize) verdef->vd_next = 0;
    verdefnum++;
...
template <class ElfTypes>
void emitElf<ElfTypes>::createHashSection(Elf_Word *&hashsecData,
  ...
  for (iter = dynSymbols.begin(); iter != dynSymbols.end(); iter++, i++) {
    ...
    if ((find(originalHashEntries.begin(), originalHashEntries.end(), index) ==
         originalHashEntries.end()) &&
...
template <class ElfTypes>
void emitElf<ElfTypes>::createDynamicSection(
    void *dynData, unsigned size, Elf_Dyn *&dynsecData, unsigned &dynsecSize,
    unsigned &dynSymbolNamesLength, std::vector<std::string> &dynStrs) {
  ...
  dynsecSize =
      2 * (count + DT_NEEDEDEntries.size() + new_dynamic_entries.size());
  ...
  // There may be multiple HASH (ELF, GNU etc) sections in the original binary.
  // We consolidate all of them into one.
  ...
  for (unsigned i = 0; i < count; i++) {
    switch (dyns[i].d_tag) {
      ...
      case DT_NEEDED:
        ...
        if (find(DT_NEEDEDEntries.begin(), DT_NEEDEDEntries.end(), rpathstr) !=
            DT_NEEDEDEntries.end()) {
          break;
        }
      ...
      case DT_FINI_ARRAY:
         ...
         * we're dealing with a library without a fixed load address.  We'll be
         *shifting
...
template <class ElfTypes>
void emitElf<ElfTypes>::log_elferror(void (*err_func)(const char *),
                                     const char *msg) {
...
void emitElf<ElfTypes>::addDTNeeded(string s) {
  if (find(DT_NEEDEDEntries.begin(), DT_NEEDEDEntries.end(), s) !=
      DT_NEEDEDEntries.end())
    return;
  vector<string> &libs_rmd = object->libsRMd();
  if (find(libs_rmd.begin(), libs_rmd.end(), s) != libs_rmd.end()) return;
