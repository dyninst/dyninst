...
bool emitElf::driver(Symtab *obj, string fName){
    ...
    {
        //Add a new page before the start of the page and copy elf header and phdr
        memset(newEhdr+sizeof(Elf32_Ehdr), '\0', pgSize - sizeof(Elf32_Ehdr));
	Elf32_Ehdr * ptr = newEhdr + pgSize; 
	ptr = (Elf32_Ehdr *)malloc(sizeof(Elf32_Ehdr));
	memcpy(ptr, oldEhdr, sizeof(Elf32_Ehdr));
    }*/	
	...
	if(shdr->sh_addr+shdr->sh_size == dataSegEnd){
	    insertPoint = scncount;
	    	...
	    	return false;
	    if(addNewSegmentFlag) {
	        newEhdr->e_phoff = firstNewLoadSec->sh_offset+ loadSecTotalSize;
    	        if(firstNewLoadSec)
	    	    loadSecTotalSize += (oldEhdr->e_phnum + 1)* oldEhdr->e_phentsize;
	    }	    
        }
