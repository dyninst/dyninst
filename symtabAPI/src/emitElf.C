...
bool
emitElf<ElfTypes>::cannotRelocatePhdrs()
{
...
static int
elfSymBind(Symbol::SymbolLinkage sLinkage)
{
    switch(sLinkage)
    {
...
static int
elfSymVisibility(Symbol::SymbolVisibility sVisibility)
{
    switch(sVisibility)
    {
...
std::string
phdrTypeStr(Elf64_Word phdr_type)
{
    switch(phdr_type)
    {
        case PT_NULL:
...
template <class ElfTypes>
bool
emitElf<ElfTypes>::createElfSymbol(Symbol* symbol, unsigned strIndex,
                                   vector<Elf_Sym*>& symbols, bool dynSymFlag)
{
...
template <class ElfTypes>
void
emitElf<ElfTypes>::findSegmentEnds()
{
...
template <class ElfTypes>
void
emitElf<ElfTypes>::renameSection(const std::string& oldStr, const std::string& newStr,
                                 bool renameAll)
{
...
template <class ElfTypes>
bool
emitElf<ElfTypes>::driver(std::string fName)
{
  ...
  if((newfd = (open(fName.c_str(), O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IXUSR|S_IRGRP|S_IXGRP)))==-1){
    ...
    for(scncount = 0; (scn = elf_nextscn(oldElf, scn)); scncount++)
    {
        ...
        if(BSSExpandFlag)
        {
            // Add the expanded SHT_NOBITS section size if the section comes after those
            // sections
            if(scncount > NOBITSstartPoint)
                newshdr->sh_offset += NOBITStotalsize;

            // Expand the NOBITS sections in file & and change the type from SHT_NOBITS to
            // SHT_PROGBITS
        ...
        if(library_adjust > 0 &&
           (strcmp(name, ".init_array") == 0 || strcmp(name, ".fini_array") == 0 ||
            strcmp(name, "__libc_subfreeres") == 0 ||
            strcmp(name, "__libc_atexit") == 0 ||
            strcmp(name, "__libc_thread_subfreeres") == 0 ||
            strcmp(name, "__libc_IO_vtables") == 0))
        {
        ...
        {
            unsigned newOff = newshdr->sh_offset - (newshdr->sh_offset & (pgSize - 1)) +
        ...
        rewrite_printf("section %s addr = %lx off = %lx size = %lx\n", name,
                       (long unsigned int) newshdr->sh_addr,
                       (long unsigned int) newshdr->sh_offset,
                       (long unsigned int) newshdr->sh_size);
        rewrite_printf(" %02u Link(%u) Info(%u) change(%u)\n", sectionNumber,
                       secLinkMapping[sectionNumber], secInfoMapping[sectionNumber],
...
template <class ElfTypes>
void
emitElf<ElfTypes>::createNewPhdrRegion(
...
template <class ElfTypes>
void
emitElf<ElfTypes>::fixPhdrs(unsigned& extraAlignSize)
{
    ...
    for(unsigned i = 0; i < oldEhdr->e_phnum; i++)
    {
        ...
        else if(replaceNOTE && old->p_type == PT_NOTE && !replaced)
        {
            ...
            segments[i].p_memsz = (currEndAddress - firstNewLoadSec->sh_addr) -
    ...
    if(createNewPhdr && firstNewLoadSec)
    {
        ...
        for(unsigned i = 0; i < segments.size(); i++)
        {
            ...
            else if(i == 0 && segments[i].p_type == PT_LOAD &&
                    newSegmentStart < segments[i].p_vaddr)
            {
    ...
    for(unsigned i = 0; i < segments.size(); i++)
    {
        ...
        rewrite_printf("Updated program header: type %u (%s), offset 0x%lx, addr 0x%lx\n",
                       ...
                       (long unsigned int) newPhdr->p_offset,
                       (long unsigned int) newPhdr->p_vaddr);
        ++newPhdr;
    }
...
template <class ElfTypes>
void
emitElf<ElfTypes>::updateSymbols(Elf_Data* symtabData, Elf_Data* strData,
...
template <class ElfTypes>
bool
emitElf<ElfTypes>::createLoadableSections(
    ...
    unsigned&                                  sectionNumber)
{
    ...
    for(unsigned i = 0; i < newSecs.size(); i++)
    {
        ...
        // Check to make sure the (vaddr for the start of the new segment - the offset) is
        // page aligned
        if(!firstNewLoadSec)
        {
        ...
        rewrite_printf(
            "new section %s addr = %lx off = %lx size = %lx\n",
            newSecs[i]->getRegionName().c_str(), (long unsigned int) newshdr->sh_addr,
            (long unsigned int) newshdr->sh_offset, (long unsigned int) newshdr->sh_size);
...
template <class ElfTypes>
bool
emitElf<ElfTypes>::addSectionHeaderTable(Elf_Shdr* shdr)
{
    Elf_Scn*  newscn;
    Elf_Data* newdata = NULL;
    Elf_Shdr* newshdr;
...
template <class ElfTypes>
bool
emitElf<ElfTypes>::createNonLoadableSections(Elf_Shdr*& shdr)
{
    Elf_Scn*  newscn;
    Elf_Data* newdata = NULL;
    Elf_Shdr* newshdr;
    ...
    for(unsigned i = 0; i < nonLoadableSecs.size(); i++)
    {
        ...
        else if(nonLoadableSecs[i]->getRegionType() == Region::RT_SYMTAB)
        {
            ...
            newdata->d_type     = ELF_T_SYM;
            newshdr->sh_link =
                secNames.size();  //.symtab section should have sh_link = index of .strtab
            newshdr->sh_flags = 0;
        }
        else if(nonLoadableSecs[i]->getRegionType() ==
      ...
      newshdr->sh_entsize = sizeof(Elf_Sym);
      newdata->d_type = ELF_T_SYM;
      //newshdr->sh_link = newSecSize+i+1;   //.symtab section should have sh_link = index
      of .strtab newshdr->sh_flags=  SHF_ALLOC | SHF_WRITE;
          }*/
...
template <class ElfTypes>
bool
emitElf<ElfTypes>::createSymbolTables(set<Symbol*>& allSymbols)
{
    ...
    // We should increment this here, but for reasons I don't understand we create it with
    // a size of 1.
    // symbolNamesLength++;
    sym->st_name  = 0;
    sym->st_value = 0;
    ...
    if(obj->isStaticBinary())
    {
        ...
        if(newRegs.size())
        {
            ...
            for(newRegIter = newRegs.begin(); newRegIter != newRegs.end(); ++newRegIter)
            {
    ...
    /* We regenerate symtab and symstr section. We do not
       ...
       original binary. We preserve sh_index of Elf symbols (from Symbol's strIndex). We
       append new symbols and string that we create for the new binary (targ*, versions
       etc).
    */
    ...
    for(i = 0; i < allDynSymbols.size(); i++)
    {
        createElfSymbol(allDynSymbols[i], allDynSymbols[i]->getStrIndex(), dynsymbols,
                        true);
        dynSymNameMapping[allDynSymbols[i]->getMangledName().c_str()] =
            i + nTmp;  // allDynSymbols[i]->getIndex();
    ...
    if(!obj->isStaticBinary())
    {
        ...
        if(obj->findRegion(sec, ".dynamic"))
        {
            ...
            */
            createDynamicSection(sec->getPtrToRawData(), sec->getDiskSize(), dynsecData,
                                 dynsecSize, dynsymbolNamesLength, dynsymbolStrs);
        }
...
template <class ElfTypes>
void
emitElf<ElfTypes>::createRelocationSections(
    ...
    std::unordered_map<std::string, unsigned long>& dynSymNameMapping)
{
    ...
    Elf_Rel* rels =
        (Elf_Rel*) malloc(sizeof(Elf_Rel) * (relocation_table.size() + newRels.size()));
    Elf_Rela* relas =
        (Elf_Rela*) malloc(sizeof(Elf_Rela) * (relocation_table.size() + newRels.size()));
    j = 0;
    k = 0;
    l = 0;
    m = 0;
    ...
    for(i = 0; i < relocation_table.size(); i++)
    {
           ...
           (relocation_table[i].regionType() == Region::RT_REL))
        {
            ...
            if(!sym_name.empty())
            {
                std::unordered_map<string, unsigned long>::iterator it =
            ...
            {
                rels[j].r_info =
                    ElfTypes::makeRelocInfo(sym_offset, relocation_table[i].getRelType());
            }
            else
            {
...
template <class ElfTypes>
void
emitElf<ElfTypes>::createSymbolVersions(Elf_Half*& symVers, char*& verneedSecData,
                                        ...
                                        std::vector<std::string>& dynStrs)
{
    ...
    for(dit = unversionedNeededEntries.begin(); dit != unversionedNeededEntries.end();
        dit++)
    {
...
template <class ElfTypes>
void
emitElf<ElfTypes>::createHashSection(Elf_Word*& hashsecData, unsigned& hashsecSize,
...
template <class ElfTypes>
void
emitElf<ElfTypes>::createDynamicSection(void* dynData_, unsigned size,
                                        Elf_Dyn*& dynsecData, unsigned& dynsecSize,
                                        unsigned&                 dynSymbolNamesLength,
                                        std::vector<std::string>& dynStrs)
{
    ...
    // There may be multiple HASH (ELF, GNU etc) sections in the original binary. We
    // consolidate all of them into one.
    ...
    for(unsigned i = 0; i < count; i++)
    {
        switch(dyns[i].d_tag)
        {
            ...
            case DT_PPC64_GLINK:
                 ...
                 *shifting the addresses of that library by a page.
                 **/
...
template <class ElfTypes>
void
emitElf<ElfTypes>::log_elferror(void (*err_func)(const char*), const char* msg)
{
...
template <class ElfTypes>
void
emitElf<ElfTypes>::addDTNeeded(string s)
{
...
char*
emitElf<ElfType>::allocate_buffer(size_t size)
{
