...
bool
Aggregate::removeSymbolInt(Symbol* sym)
{
...
bool
Aggregate::getSymbols(std::vector<Symbol*>& syms) const
{
    syms = symbols_;
    return true;
}
...
Symbol*
Aggregate::getFirstSymbol() const
{
    return firstSymbol;
}
...
SYMTAB_EXPORT bool
Aggregate::addMangledName(string name, bool isPrimary, bool isDebug)
{
...
SYMTAB_EXPORT bool
Aggregate::addPrettyName(string name, bool isPrimary, bool isDebug)
{
    // Check to see if we're duplicating
    {
        boost::unique_lock<dyn_mutex> l(lock_);
...
SYMTAB_EXPORT bool
Aggregate::addTypedName(string name, bool isPrimary, bool isDebug)
{
    // Check to see if we're duplicating
    {
        boost::unique_lock<dyn_mutex> l(lock_);
...
bool
Aggregate::changeSymbolOffset(Symbol* sym)
{
    Offset       oldOffset = getOffset();
    unsigned int old_count = symbols_.size();

    removeSymbolInt(sym);
    if(old_count == symbols_.size())
        return true;
