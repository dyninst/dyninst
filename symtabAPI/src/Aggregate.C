...
Aggregate::Aggregate() : module_(NULL), firstSymbol(NULL), offset_(0L) {}
...
Aggregate::Aggregate(Module *mod) : firstSymbol(NULL), offset_(0) {
  module_ = mod;
}
...
bool Aggregate::addSymbol(Symbol *sym) {
  // We keep a "primary" module, which is defined as "anything not
  // DEFAULT_MODULE".
  if (module_ == NULL) {
    module_ = sym->getModule();
...
Symbol *Aggregate::getFirstSymbol() const { return firstSymbol; }

bool Aggregate::addMangledNameInternal(std::string name, bool /*isPrimary*/,
                                       bool /*demangle*/) {
  // Check to see if we're duplicating
  for (auto i = mangled_names_begin(); i != mangled_names_end(); ++i) {
    if (i->find(name) != string::npos) return false;
  }
  return true;
}
...
SYMTAB_EXPORT bool Aggregate::addMangledName(string name, bool isPrimary,
                                             bool isDebug) {
  if (!addMangledNameInternal(name, isPrimary, false)) return false;
...
SYMTAB_EXPORT bool Aggregate::addPrettyName(string name, bool isPrimary,
                                            bool isDebug) {
  // Check to see if we're duplicating
  for (auto i = pretty_names_begin(); i != pretty_names_end(); i++) {
    if (i->find(name) != string::npos) return false;
  }
...
SYMTAB_EXPORT bool Aggregate::addTypedName(string name, bool isPrimary,
                                           bool isDebug) {
  // Check to see if we're duplicating
  for (auto i = typed_names_begin(); i != typed_names_end(); i++) {
    if (i->find(name) != string::npos) return false;
  }
  return addMangledName(name, isPrimary, isDebug);
}
...
void Aggregate::restore_type_by_id(SerializerBase *, Type *&, unsigned)
    THROW_SPEC(SerializerError) {}

void Aggregate::restore_module_by_name(SerializerBase *, std::string &)
    THROW_SPEC(SerializerError) {}

extern Symbol *getSymForID(SerializerBase *sb, Address id);

void Aggregate::rebuild_symbol_vector(SerializerBase *, std::vector<Address> &)
    THROW_SPEC(SerializerError) {}

std::ostream &operator<<(std::ostream &os,
                         const Dyninst::SymtabAPI::Aggregate &a) {
  std::string modname =
      a.module_ ? a.module_->fullName() : std::string("no_mod");
  os << "Aggregate{"
...
void Aggregate::serialize_aggregate(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {}
