...
void LinkMap::print(Offset globalOffset) { printAll(std::cout, globalOffset); }
...
void LinkMap::printAll(ostream &os, Offset globalOffset) {
  ...
  if (codeRegions.size() > 0) {
    os << "New CODE Region: Offset: 0x" << hex
       << (globalOffset + codeRegionOffset) << dec << " Size: 0x" << hex
       << codeSize << dec << " Alignment: 0x" << hex << codeRegionAlign << dec
       << endl;
  ...
  if (dataRegions.size() > 0) {
    os << "New DATA Region: Offset: 0x" << hex
       << (globalOffset + dataRegionOffset) << dec << " Size: 0x" << hex
       << dataSize << dec << " Alignment: 0x" << hex << dataRegionAlign << dec
       << endl;
  ...
  if (tlsRegions.size() > 0) {
    os << "New TLS Region: Offset: 0x" << hex
       << (globalOffset + tlsRegionOffset) << dec << " Size: 0x" << hex
       << tlsSize << dec << " Alignment: 0x" << hex << tlsRegionAlign << dec
       << endl;
    ...
    for (off_it = off2Sym.begin(); off_it != off2Sym.end(); ++off_it) {
      os << "\tSymbol: " << off_it->second->getMangledName() << " Offset: 0x"
         << hex << off_it->first << dec << endl;
    }
  ...
  if (gotSize > 0) {
    os << "New GOT Region: Offset: 0x" << hex
       << (globalOffset + gotRegionOffset) << dec << " Size: 0x" << hex
       << gotSize << dec << " Alignment: 0x" << hex << gotRegionAlign << dec
       << endl;
    ...
    for (off_it = gotByEntry.begin(); off_it != gotByEntry.end(); ++off_it) {
      os << "\tGOT Offset: 0x" << hex << off_it->first << dec
         << " Symbol: " << off_it->second->getMangledName() << " Offset: 0x"
         << hex << off_it->second->getOffset() << dec << endl;
    }
  ...
  if (newCtorRegions.size() > 0) {
    os << "New .ctors region: Offset: 0x" << hex
       << (globalOffset + ctorRegionOffset) << dec << " Size: 0x" << hex
       << ctorSize << dec << " Alignment: 0x" << hex << ctorRegionAlign << dec
       << endl;
  ...
  if (newDtorRegions.size() > 0) {
    os << "New .dtors region: Offset: 0x" << hex
       << (globalOffset + dtorRegionOffset) << dec << " Size: 0x" << hex
       << dtorSize << dec << " Alignment: 0x" << hex << dtorRegionAlign << dec
       << endl;
  ...
  if (bssRegions.size() > 0) {
    os << "New BSS Region: Offset: 0x" << hex
       << (globalOffset + bssRegionOffset) << dec << " Size: 0x" << hex
       << bssSize << dec << " Alignment: 0x" << hex << bssRegionAlign << dec
       << endl;
...
void LinkMap::printBySymtab(ostream &os, vector<Symtab *> &symtabs,
                            Offset globalOffset) {
...
void LinkMap::printRegions(ostream &os, deque<Region *> &regions,
                           Offset globalOffset) {
...
void LinkMap::printRegionFromInfo(ostream &os, Region *region,
                                  Offset regionOffset, Offset padding) {
  os << "\tRegion " << region->getRegionName() << " Padding: 0x" << hex
     << padding << dec << " Offset: 0x" << hex << regionOffset << dec << " - 0x"
     << hex << (regionOffset + region->getMemSize() - 1) << dec << " Size: 0x"
     << hex << region->getMemSize() << dec << " Alignment: 0x" << hex
