 ...
 */
std::vector<localVar *> *localVarCollection::getAllVars() { return &localVars; }

#if !defined(SERIALIZATION_DISABLED)
Serializable *localVarCollection::ac_serialize_impl(SerializerBase *s,
                                                    const char *tag)
    THROW_SPEC(SerializerError) {
  unsigned short lvmagic = 72;
  serialize_printf(
      "%s[%d]:  welcome to localVarCollection: ac_serialize_impl\n", FILE__,
      __LINE__);
  ...
  serialize_printf(
      "%s[%d]:  localVarCollection: ac_serialize_impl, translate done\n",
      FILE__, __LINE__);
...
#endif
dyn_hash_map<int, std::vector<std::pair<dataClass, Type **> > *>
    *deferred_lookups_p = NULL;

void typeCollection::addDeferredLookup(int tid, dataClass tdc, Type **th) {
  if (!deferred_lookups_p)
    deferred_lookups_p =
        new dyn_hash_map<int, std::vector<std::pair<dataClass, Type **> > *>();
  dyn_hash_map<int, std::vector<std::pair<dataClass, Type **> > *>
      &deferred_lookups = *deferred_lookups_p;
  dyn_hash_map<int, std::vector<std::pair<dataClass, Type **> > *>::iterator
      iter;
...
bool typeCollection::doDeferredLookups(typeCollection *primary_tc) {
  ...
  dyn_hash_map<int, std::vector<std::pair<dataClass, Type **> > *>::iterator
      iter;
  for (iter = deferred_lookups.begin(); iter != deferred_lookups.end();
       iter++) {
    ...
    for (unsigned int i = 0; i < to_assign->size(); ++i) {
      ...
      if (!t) {
        ...
        for (tciter = fileToTypesMap.begin(); tciter != fileToTypesMap.end();
          ...
          if (localt) {
            if (localt->getDataClass() != ldc) continue;
            nfound++;
            t = localt;
          }
          // if (t) break;
 ...
 */
typeCollection::typeCollection()
    : typesByName(), globalVarsByName(), typesByID(), dwarfParsed_(false) {
...
void typeCollection::clearNumberedTypes() {
  for (dyn_hash_map<int, Type *>::iterator it = typesByID.begin();
       it != typesByID.end(); it++) {
    if (it->second) it->second->decrRefCount();
  }
...
#if !defined(SERIALIZATION_DISABLED)
Serializable *typeCollection::serialize_impl(SerializerBase *sb,
                                             const char *tag)
    THROW_SPEC(SerializerError) {
...
#else
Serializable *typeCollection::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}
#endif
 ...
 * builtInTypeCollection::~builtInTypeCollection
 *
 * Destructor for builtInTypeCollection.  Deletes all builtInType objects that
 * have
 * been inserted into the collection.
 */
builtInTypeCollection::~builtInTypeCollection() {
  ...
  for (; bit != builtInTypesByName.end(); bit++) bit->second->decrRefCount();
