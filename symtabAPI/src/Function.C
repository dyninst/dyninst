...
bool
FunctionBase::setReturnType(boost::shared_ptr<Type> newType)
{
    retType_ = newType;
    return true;
}
...
bool
FunctionBase::getParams(std::vector<localVar*>& params_)
{
    getModule()->exec()->parseTypesNow();
    if(!params)
        return false;
...
bool
FunctionBase::addLocalVar(localVar* locVar)
{
    if(!locals)
    {
...
bool
FunctionBase::addParam(localVar* param)
{
    if(!params)
    {
...
bool
FunctionBase::setFramePtr(vector<VariableLocation>* locs)
{
    frameBase_.clear();
...
std::pair<std::string, Dyninst::Offset>
...
void
FunctionBase::expandLocation(const VariableLocation&        loc,
                             std::vector<VariableLocation>& ret)
{
    ...
    DwarfFrameParser::Ptr frameParser = DwarfFrameParser::create(
        *obj->dwarf->frame_dbg(), obj->dwarf->origFile()->e_elfp(), obj->getArch());

    std::vector<VariableLocation>        FDEs;
    Dyninst::DwarfDyninst::FrameErrors_t err;
    if(!frameParser)
        return;
    frameParser->getRegsForFunction(std::make_pair(loc.lowPC, loc.hiPC), Dyninst::CFA,
                                    FDEs, err);
...
int
Function::getFramePtrRegnum() const
{
    return 0;
}

bool
Function::setFramePtrRegnum(int)
{
    return false;
}
...
bool
Function::removeSymbol(Symbol* sym)
{
    removeSymbolInt(sym);
    if(symbols_.empty())
    {
...
std::ostream&
operator<<(std::ostream& os, const Dyninst::VariableLocation& l)
{
    const char* stc  = storageClass2Str(l.stClass);
    const char* strc = storageRefClass2Str(l.refClass);
    os << "{"
       << "storageClass=" << stc << " storageRefClass=" << strc
       << " reg=" << l.mr_reg.name() << " frameOffset=" << l.frameOffset
       << " lowPC=" << l.lowPC << " hiPC=" << l.hiPC << "}";
    return os;
}

std::ostream&
operator<<(std::ostream& os, const Dyninst::SymtabAPI::Function& f)
{
    boost::shared_ptr<Type> retType =
        (const_cast<Function&>(f)).getReturnType(Type::share);
    ...
    os << "Function{"
       << " type=" << tname << " framePtrRegNum_=" << f.getFramePtrRegnum()
...
bool
InlinedFunction::removeSymbol(Symbol*)
{
    return false;
}

bool
InlinedFunction::addMangledName(std::string name, bool /*isPrimary*/, bool /*isDebug*/)
{
    name_ = name;
    return true;
}
...
bool
InlinedFunction::addPrettyName(std::string name, bool /*isPrimary*/, bool /*isDebug*/)
{
    name_ = name;
    return true;
}
...
std::string
InlinedFunction::getName() const
{
    return name_;
}

Offset
InlinedFunction::getOffset() const
{
    return offset_;
}

unsigned
InlinedFunction::getSize() const
{
    return functionSize_;  // inline_parent->getSize();
}

void
InlinedFunction::setFile(string filename)
{
    StringTablePtr                strs = module_->getStrings();
    boost::unique_lock<dyn_mutex> l(strs->lock);
    // This looks gross, but here's what it does:
    // Get index 1 (unique by name). Insert the filename on that index (which defaults to
    // push_back if empty). Returns an <iterator, bool>; get the iterator (we don't care
    // if it's new). Project to random access (index 0). Difference from begin == array
    // index in string table.
    callsite_file_number =
        strs->project<0>(strs->get<1>().insert(StringTableEntry(filename, "")).first) -
        strs->begin();
}

Module*
Function::getModule() const
{
    return module_;
}
