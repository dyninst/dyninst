...
const FuncRangeCollection &FunctionBase::getRanges() { return ranges; }
...
std::vector<Dyninst::VariableLocation> &FunctionBase::getFramePtrRefForInit() {
  if (inline_parent) return inline_parent->getFramePtr();
...
std::vector<Dyninst::VariableLocation> &FunctionBase::getFramePtr() {
  ...
  if (frameBaseExpanded_) return frameBase_;
...
std::pair<std::string, Dyninst::Offset> InlinedFunction::getCallsite() {
  ...
  if (callsite_file_number > 0 &&
      callsite_file_number < module_->getStrings()->size()) {
...
void FunctionBase::expandLocation(const VariableLocation &loc,
  ...
  Dyninst::Dwarf::DwarfFrameParser::Ptr frameParser =
      Dyninst::Dwarf::DwarfFrameParser::create(
          *getModule()->exec()->getObject()->dwarf->frame_dbg(),
...
void *FunctionBase::getData() { return data; }

void FunctionBase::setData(void *d) { data = d; }

Function::Function(Symbol *sym) : FunctionBase(), Aggregate(sym) {}

Function::Function() : FunctionBase() {}

int Function::getFramePtrRegnum() const { return 0; }

bool Function::setFramePtrRegnum(int) { return false; }
...
std::ostream &operator<<(std::ostream &os,
                         const Dyninst::SymtabAPI::Function &f) {
...
bool FunctionBase::operator==(const FunctionBase &f) {
  if (retType_ && !f.retType_) return false;
  if (!retType_ && f.retType_) return false;
  if (retType_)
...
bool InlinedFunction::removeSymbol(Symbol *) { return false; }
...
std::string InlinedFunction::getName() const { return name_; }

Offset InlinedFunction::getOffset() const { return offset_; }
...
void InlinedFunction::setFile(string filename) {
  ...
  // Get index 1 (unique by name). Insert the filename on that index (which
  // defaults to push_back if empty).
  // Returns an <iterator, bool>; get the iterator (we don't care if it's new).
  // Project to random access (index 0).
  // Difference from begin == array index in string table.
  callsite_file_number =
      strs->project<0>(strs->get<1>().insert(filename).first) - strs->begin();
}

Module *Function::getModule() const { return module_; }