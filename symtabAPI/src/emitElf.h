...
struct ElfTypes32 {
  ...
  Elf_Phdr *elf_newphdr(Elf *elf, size_t num) {
    return elf32_newphdr(elf, num);
  }
  ...
  Elf32_Word makeRelocInfo(Elf32_Word sym, Elf32_Word type) {
...
struct ElfTypes64 {
  ...
  Elf64_Xword makeRelocInfo(Elf64_Word sym, Elf64_Word type) {
    return ELF64_R_INFO(sym, type);
  }
};

template <class ElfTypes = ElfTypes64>
class emitElf : public ElfTypes {
 public:
  emitElf(Elf_X *pX, bool i, Object *pObject, void (*pFunction)(const char *),
          Symtab *pSymtab);
 ...
 private:
  ...
  bool createElfSymbol(Symbol *symbol, unsigned strIndex,
                       vector<Elf_Sym *> &symbols, bool dynSymFlag = false);
  void findSegmentEnds();
  void renameSection(const std::string &oldStr, const std::string &newStr,
                     bool renameAll = true);
  void fixPhdrs(unsigned &);
  void createNewPhdrRegion(
  ...
  void createSymbolVersions(Elf_Half *&symVers, char *&verneedSecData,
                            unsigned &verneedSecSize, char *&verdefSecData,
                            unsigned &verdefSecSize,
                            unsigned &dynSymbolNamesLength,
                            std::vector<std::string> &dynStrs);

  void createHashSection(Elf_Word *&hashsecData, unsigned &hashsecSize,
                         std::vector<Symbol *> &dynSymbols);

  void createDynamicSection(void *dynData, unsigned size, Elf_Dyn *&dynsecData,
                            unsigned &dynsecSize,
                            unsigned &dynSymbolNamesLength,
                            std::vector<std::string> &dynStrs);
