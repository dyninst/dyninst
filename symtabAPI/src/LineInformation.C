...
LineInformation::LineInformation()
    : strings_(new StringTable),
      wasted_compares(0),
      num_queries(0) {} /* end LineInformation constructor */

bool LineInformation::addLine(unsigned int lineSource, unsigned int lineNo,
...
bool LineInformation::addLine(std::string lineSource, unsigned int lineNo,
                              unsigned int lineOffset, Offset lowInclusiveAddr,
                              Offset highExclusiveAddr) {
  auto index = strings_->get<1>().insert(lineSource).first;

  return addLine(index->str, lineNo, lineOffset, lowInclusiveAddr,
                 highExclusiveAddr);
}
...
bool LineInformation::addAddressRange(Offset lowInclusiveAddr,
  ...
  return addLine(lineSource, lineNo, lineOffset, lowInclusiveAddr,
                 highExclusiveAddr);
} /* end setAddressRangeToLineMapping() */

std::string print(const Dyninst::SymtabAPI::Statement& stmt) {
  std::stringstream stream;
  stream << std::hex << "Statement: < [" << stmt.startAddr() << ", "
         << stmt.endAddr() << "): " << std::dec << stmt.getFile() << ":"
         << stmt.getLine() << " >";
  return stream.str();
}
...
LineInformation::const_iterator LineInformation::find(
    Offset addressInRange) const {
  ++num_queries;
  const_iterator start_addr_valid = project<Statement::addr_range>(
      get<Statement::upper_bound>().lower_bound(addressInRange));
...
unsigned LineInformation::getSize() const { return impl_t::size(); }

LineInformation::~LineInformation() {
  //    std::cerr << "Line information with " << getSize() << " entries queried
  //    " << num_queries << " times";
  //    if(num_queries)
  //    {
  //        std::cerr << " with " << wasted_compares << " extra compares (" <<
  //        (float)(wasted_compares) / (num_queries) << " per query)";
  //    }
  //    std::cerr << std::endl;
}

LineInformation::const_line_info_iterator LineInformation::begin_by_source()
    const {
  const traits::line_info_index& i = impl_t::get<Statement::line_info>();
  return i.begin();
}

LineInformation::const_line_info_iterator LineInformation::end_by_source()
    const {
  const traits::line_info_index& i = impl_t::get<Statement::line_info>();
  return i.end();
}
...
LineInformation::equal_range(std::string file,
  ...
  while ((*bounds.second)->getLine() > lineNo &&
  ...
  return bounds;
  //    return
  //    get<traits::line_info>().equal_range(boost::cref<std::tuple<unsigned
  //    int, unsigned int> >(search_info));
}
...
LineInformation::equal_range(std::string file) const {
  ...
  //    const traits::line_info_index& by_line_info =
  //    impl_t::get<traits::line_info>();
  //    return by_line_info.equal_range(file);
}

StringTablePtr LineInformation::getStrings() { return strings_; }
...
LineInformation::const_iterator LineInformation::find(
    Offset addressInRange, const_iterator hint) const {
  while (hint != end()) {
