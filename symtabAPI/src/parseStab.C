...
{
std::string
parseStabString(Module* mod, int linenum, char* stabstr, int framePtr,
...
static int
parseSymDesc(char* stabstr, int& cnt);
static boost::shared_ptr<Type>
parseConstantUse(Module*, char* stabstr, int& cnt);
static char*
parseTypeDef(Module*, char* stabstr, const char* name, int ID, unsigned int sizeHint = 0);
static int
parseTypeUse(Module*, char*& stabstr, int& cnt, const char* name);
static inline bool
isSymId(char ch);
static std::string
getIdentifier(char* stabstr, int& cnt, bool stopOnSpace = false);

static std::string currentRawSymbolName;

std::string
convertCharToString(const char* ptr)
{
    if(ptr)
        return ptr;
    else
...
void
vectorNameMatchKLUDGE(char* demangled_sym, std::vector<Function*>& bpfv,
...
Function*
mangledNameMatchKLUDGE(const char* pretty, const char* mangled, Module* mod)
{
...
std::string
Dyninst::SymtabAPI::parseStabString(Module* mod, int linenum, char* stabstr, int framePtr,
                                    typeCommon* commonBlock)
{
    ...
    types_printf("parseStabString, mod %p/%s, linenum %d, stabstr %s\n", (void*) mod,
                 (mod != NULL) ? mod->fileName().c_str() : "NULL", linenum, stabstr);
    ...
    else if(stabstr[cnt])
    {
        ...
        switch(stabstr[cnt])
        {
                ...
                if(!scopeName.length())
                {
                    ...
                    */
                    ...
                    else
                    {
                        if(bpfv.size() > 1)
                        {
                            // warn if we find more than one function with
                            // current_func_name
                            char msg[1024];
                            sprintf(msg,
                                    "%s[%d]:  found %d functions with name %s, using the "
                                    "first",
            ...
            case 'F': /* Global Function */
            {
                ...
                // if (!mod->findSymbol(fpv, symt_current_mangled_func_name,
                // Symbol::ST_FUNCTION, true))
                {
                    std::string modName = mod->fileName();
                    ...
                    {
                        // bpwarn("%s L%d - Cannot find global function with mangled name
                        // '%s' or pretty name '%s' with return type '%s' in module '%s',
                        // possibly extern\n",
                        //       __FILE__, __LINE__,
                        //       symt_current_mangled_func_name.c_str(),
                        //       current_func_name.c_str(),
                        //       ((ptrType->getMangledName() == NULL) ? "" :
                        //       ptrType->getMangledName()), modName);
                        // char prefix[5];
            ...
            case 'E':  /* Extern'd Global ??? - undocumented type for Sun Compilers - jkh
                          6/6/03 */
...

                {
                    ...
                    if(BPtype)
                    {
                        ...
                        if(!result)
                        {
                            ...
                            if(mod->exec()->getDefaultModule()->findVariablesByName(ret,
                                                                                    name))
            ...
            case 'c': /* constant */
            {
                ...
                {
                    std::vector<Function*> fpv;
                    if(mod->exec()->findFunctionsByName(fpv,
            ...
            case 'S': /* Global Static Variable */
            {
                ...
                if(name.find(".") < name.length())
                {
                    ...
                    nameTrailer      = name.substr(name.find(".") + 1,
                                              name.length() - name.find(".") - 1);
                ...
                if(BPtype)
                {
                    Symtab*              img = mod->exec();
                    std::vector<Symbol*> syms;
                    if(img->findSymbol(syms, nameTrailer, Symbol::ST_OBJECT,
                                       mangledName) ||
                       img->findSymbol(syms, nameTrailer, Symbol::ST_OBJECT, mangledName,
                                       true))
                    {
            ...
            case 't':  // Type Name
                ...
                if(stabstr[cnt] == '=')
                {
                    ...
                    stabstr =
                        parseTypeDef(mod, (&stabstr[cnt + 1]), name.c_str(), symdescID);
                    cnt = 0;
                }
                else
                {
                        ...
                        if(fields)
                        {
                            for(unsigned int i = 0; i < fields->size(); i++)
                            {
                                   ...
                                   ((start1 >= start2) && (start1 < end2)))
                                {
                                    ...
                                    //  current_func_name.c_str(), name.c_str(),
                                    //  (*fields)[i]->getName());
...
//
std::string
getIdentifier(char* stabstr, int& cnt, bool stopOnSpace)
{
    int  i      = 0;
    int  brCnt  = 0;
    bool idChar = true;
    ...
    while(idChar)
    {
        switch(stabstr[cnt + i])
        {
            ...
            case ';':
                ...
                //				else if( brCnt ) {
                //					//bperr( "Failed to find identifier in stabstring
                //'%s;\n", stabstr ); 					idChar = false;
                //					}
                else
                {
                    idChar = false;
...
char*
getFieldName(char* stabstr, int& cnt)
{
    int  i      = 0;
    bool idChar = true;
...
//
static int
parseTypeUse(Module* mod, char*& stabstr, int& cnt, const char* name)
{
...
//
static char*
parseCrossRef(typeCollection* moduleTypes, const char* /*name*/, int ID, char* stabstr,
              int& cnt)
{
    std::string             temp;
    ...
    {  // enum
        ...
        boost::shared_ptr<Type> ptrType =
            moduleTypes->findType(temp.c_str(), Type::share);
        if(!ptrType)
        {
            ...
            {
                newType =
                    moduleTypes->addOrUpdateType(Type::make_shared<typeEnum>(ID, temp));
            }
            else if(xreftype == 'u')
            {
                newType =
                    moduleTypes->addOrUpdateType(Type::make_shared<typeUnion>(ID, temp));
            }
            else
            {
                newType =
                    moduleTypes->addOrUpdateType(Type::make_shared<typeStruct>(ID, temp));
            }
            assert(newType);
        }
    }
...
//
static boost::shared_ptr<Type>
parseArrayDef(Module* mod, const char* name, int ID, char*& stabstr, int& cnt,
              unsigned int sizeHint)
{
    ...
    //  //bperr("Symbol Desriptor: %s Descriptor ID: %d Type: %d, Low Bound: %d, Hi Bound:
    //  %d,\n", symdesc, symdescID, elementType, lowbound, hibound);
    (void) symdesc;
    (void) symdescID;  // otherwise unused symbols from above
    ...
    if(ptrType)
    {
        ...
        auto newAType =
            Type::make_shared<typeArray>(ID, ptrType, lowbound, hibound, tName, sizeHint);
        // Add to Collection
...
int
guessSize(const char* low, const char* hi)
{
    long long l, h;
...
//
static char*
parseRangeType(Module* mod, const char* name, int ID, char* stabstr,
               unsigned int sizeHint = 0)
{
    ...
    else
    {
        ...
        if(baseType == NULL)
            newType = Type::make_shared<typeSubrange>(
                ID, sizeHint ? sizeHint / 8 : guessSize(low, hi), low_conv, hi_conv,
                tName);
        else
            newType = Type::make_shared<typeSubrange>(
                ID, sizeHint ? sizeHint / 8 : baseType->getSize(), low_conv, hi_conv,
                tName);
        // Add to Collection
               ...
               unsigned int sizeHint = 0)
{
    ...
    else if(j > 0)
    {
        ...
        if(j == 0)
        {
            ...
            // Add to Collection
            tc->addOrUpdateType(
                Type::make_shared<typeScalar>(ID, size, convertCharToString(name)));
        }
        else
        {
            /* range */
            ...
            if(baseType == NULL)
                newType = Type::make_shared<typeSubrange>(
                    ID, sizeHint ? sizeHint / 8 : sizeof(long), low_conv, hi_conv, tname);
            else
                newType = Type::make_shared<typeSubrange>(
                    ID, sizeHint ? sizeHint / 8 : baseType->getSize(), low_conv, hi_conv,
                    tname);
...
//
static char*
parseAttrType(Module* mod, const char* name, int ID, char* stabstr, int& cnt)
{
    assert(stabstr[cnt] == '@');
    cnt++;  // skip the @
...
//
static char*
parseRefType(Module* mod, const char* name, int ID, char* stabstr, int& cnt)
{
...
//
void
addBaseClassToClass(Module* mod, int baseID, boost::shared_ptr<Type> newType,
                    int /*offset*/)
{
    typeCollection* tc = typeCollection::getModTypeCollection(mod);
    ...
    newType->addField(field->getName(), field->getTypeDesc(), field->getType(),
    field->getOffset()+offset, field->getVisibility());
    }
    */
}
...
//
static char*
parseFieldList(Module* mod, boost::shared_ptr<Type> newType, char* stabstr,
               bool sunCPlusPlus)
{
    int             cnt  = 0;
    int             size = 0;
    ...
    while(stabstr[cnt] && (stabstr[cnt] != ';'))
    {
        ...
        if((stabstr[cnt] == 'u') && (stabstr[cnt + 1] == ':') &&
           (!isdigit(stabstr[cnt + 2])))
        {
            cnt += 2;
        }
            ...
            while(stabstr[cnt] == ':')
            {
                ...
                if(typedescr == dataFunction)
                {
                    ...
                    if(stabstr[cnt] == '*')
                    {
                        ...
                        cnt++;  // Skip '*'
                        while(stabstr[cnt] != ';')
                            cnt++;  // Skip vtable index
                        cnt++;      // Skip ';'
                        if(stabstr[cnt] != ';')
                        {
                            parseTypeUse(mod, stabstr, cnt,
                                         "");  // Skip type number to the base class
                        }
                        cnt++;  // Skip ';'
                        if(isSymId(stabstr[cnt]))
                        {
                    ...
                    else if((stabstr[cnt] == '.') || (stabstr[cnt] == '?'))
                    {
                        cnt++;  // Skip '.' or '?'
                        if(isSymId(stabstr[cnt]))
                        {
    ...
    if(hasVirtuals && stabstr[cnt] == ';' && stabstr[cnt + 1] == '~' &&
       stabstr[cnt + 2] == '%')
    {
        cnt += 3;
        while(stabstr[cnt] != ';')
...
//
static char*
parseCPlusPlusInfo(Module* mod, char* stabstr, const char* mangledName, int ID)
{
    typeCollection* tc = typeCollection::getModTypeCollection(mod);
    int             cnt;
    char*           name;
    ...
    if(stabstr[0])
    {
        ...
        while(stabstr[cnt] && (stabstr[cnt] != ';'))
        {
            ...
            if(*funcName == '-')
                funcName++;  // it's a pure vitual

            while(isdigit(*funcName))
                funcName++;  // skip virtual function index
            funcName++;
            ...
            std::string methodName =
                std::string(className) + std::string(funcName) + std::string("_");
            ...
            newType->asFieldListType().addField(
                fName, Type::make_shared<typeFunction>(ID, fieldType, fName));
...
//
static char*
parseTypeDef(Module* mod, char* stabstr, const char* name, int ID, unsigned int sizeHint)
{
    typeCollection*         tc = typeCollection::getModTypeCollection(mod);
    boost::shared_ptr<Type> newType;
    ...
    if(isSymId(stabstr[0]))
    {
        ...
        else if(stabstr[cnt] == '=')
        {
            ...
            if(!oldType)
                oldType = Symtab::type_Untyped();
            std::string tName = convertCharToString(name);
            tc->addOrUpdateType(
                Type::make_shared<typeTypedef>(ID, oldType, tName, sizeHint));
        }
        else
        {
            boost::shared_ptr<Type> oldType;
    ...
    else
    {
        switch(stabstr[0])
        {
            ...
            case '*': {
                ...
                // Add to typeCollection
                newType =
                    tc->addOrUpdateType(Type::make_shared<typePointer>(ID, ptrType));
                return (&(stabstr[cnt]));
                break;
            }
            case 'a':
            ...
            case 'g': {
                ...
                {
                    std::string tName       = convertCharToString(name);
                    auto        newFunction = tc->addOrUpdateType(
                        Type::make_shared<typeFunction>(ID, ptrType, tName));
                    if(!newFunction)
                    {
                    ...
                    while((stabstr[cnt] != '#') && (stabstr[cnt]))
                    {
                        ...
                        // newFunction2->addField(buffer, newType->getDataClass(),
                        // newType, curOffset, newType->getSize());
                    }
                }
            ...
            case 'M': {
                ...
                else
                {
                    ...
                    newType = tc->addOrUpdateType(
                        Type::make_shared<typeArray>(ID, ptrType, 1, size, tName));
                }
                break;
            }
            case 'R': {
                ...
                newType =
                    tc->addOrUpdateType(Type::make_shared<typeScalar>(ID, bytes, name));

                if(stabstr[cnt] == ';')
                    cnt++;  // skip the final ';'

                // gcc 3.0 adds an extra field that is always 0 (no indication in the code
                // why)
                if(stabstr[cnt] == '0')
                    cnt += 2;  // skip the final '0;'
            ...
            case 'b': {
                ...
                // skip to width
                while(!isdigit(stabstr[cnt + i]) && (i < limit))
                    i++;
                if(i >= limit)
                    return (&(stabstr[cnt]));
            ...
            case 'e':  // Enumerated type
            {
                ...
                // Add type to collection
                auto newEnumType =
                    tc->addOrUpdateType(Type::make_shared<typeEnum>(ID, tName));
            ...
            case 'T':  // Fortran TYPE
            {
                ...
                {
                    newFieldType =
                        tc->addOrUpdateType(Type::make_shared<typeStruct>(ID, tName));
                }
                else
                {
                    newFieldType =
                        tc->addOrUpdateType(Type::make_shared<typeUnion>(ID, tName));
                }
                // add to type collection
...
//
static boost::shared_ptr<Type>
parseConstantUse(Module* mod, char* stabstr, int& cnt)
{
    typeCollection* tc = typeCollection::getModTypeCollection(mod);
    // skip =
    cnt++;
