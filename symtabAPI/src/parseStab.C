...
static char *parseTypeDef(Module *, char *stabstr, const char *name, int ID,
...
static std::string getIdentifier(char *stabstr, int &cnt,
...
void vectorNameMatchKLUDGE(Module *mod, char *demangled_sym,
                           std::vector<Function *> &bpfv,
                           std::vector<int> &matches) {
  // iterate through all matches and demangle names with extra parameters,
  // compare
  for (unsigned int i = 0; i < bpfv.size(); ++i) {
    ...
    if (syms.size()) {
      ...
      char *l_demangled_raw =
          P_cplus_demangle(l_mangled.c_str(), mod->exec()->isNativeCompiler());
...
Function *mangledNameMatchKLUDGE(const char *pretty, const char *mangled,
  ...
  if (!mod->exec()->findFunctionsByName(bpfv, pretty)) {
    // cerr << __FILE__ << __LINE__ << ":  KLUDGE Cannot find " << pretty <<
    // endl;
  ...
  char *demangled_sym =
      P_cplus_demangle(mangled, mod->exec()->isNativeCompiler(), true);
...
std::string Dyninst::SymtabAPI::parseStabString(Module *mod, int linenum,
                                                char *stabstr, int framePtr,
  ...
  types_printf("parseStabString, mod %p/%s, linenum %d, stabstr %s\n", mod,
               (mod != NULL) ? mod->fileName().c_str() : "NULL", linenum,
               stabstr);
  ...
  char *demangled =
      P_cplus_demangle(mangledname.c_str(), mod->exec()->isNativeCompiler());
  std::string name;
  ...
  if (isSymId(stabstr[cnt])) {
    ...
    switch (stabstr[cnt]) {
          ...
          */
          ...
          if (!(mod->exec()->findFunctionsByName(bpfv, name))) {
            ...
            if (bpfv.size() > 1) {
              ...
              sprintf(
                  msg,
                  "%s[%d]:  found %d functions with name %s, using the first",
      ...
      case 'E': /* Extern'd Global ??? - undocumented type for Sun Compilers -
                   jkh 6/6/03 */
      ...
      case 'V': /* Local Static Variable (common block vars too) */
        ...
        if (commonBlock) {
          ...
          if (symt_current_func) {
            locVar =
                new localVar(name, BPtype, fName, linenum, symt_current_func);
            VariableLocation loc;
...
//                 A<arrayDef>
//
static Type *parseArrayDef(Module *mod, const char *name, int ID,
                           char *&stabstr, int &cnt, unsigned int sizeHint) {
  typeCollection *tc = typeCollection::getModTypeCollection(mod);
  char *symdesc;
  int symdescID;
  int elementType;
  ...
  //  //bperr("Symbol Desriptor: %s Descriptor ID: %d Type: %d, Low Bound: %d,
  ...
  if (ptrType) {
    ...
    typeArray *newAType =
        new typeArray(ID, ptrType, lowbound, hibound, tName, sizeHint);
    // Add to Collection
...
static char *parseRangeType(Module *mod, const char *name, int ID,
  ...
  if (!strncpy(low, &(stabstr[cnt]), i)) /* Error copying size/range*/
    exit(1);
  low[i] = '\0';
  ...
  if (!strncpy(hi, &(stabstr[cnt]), i)) /* Error copying upper range */
    exit(1);
  hi[i] = '\0';
  ...
  if (j == 0) {
    ...
    if (baseType == NULL)
      newType =
          new typeSubrange(ID, sizeHint ? sizeHint / 8 : guessSize(low, hi),
                           low_conv, hi_conv, tName);
    else
      newType =
          new typeSubrange(ID, sizeHint ? sizeHint / 8 : baseType->getSize(),
                           low_conv, hi_conv, tName);
    // Add to Collection
...
static char *parseRangeType(Module *mod, const char *name, int ID,
  ...
  if (!strncpy(temp, &(stabstr[cnt]), i)) /* Error copying size/range*/
    exit(1);
  temp[i] = '\0';
  int j = atol(temp);
...
//
static char *parseAttrType(Module *mod, const char *name, int ID, char *stabstr,
                           int &cnt) {
  assert(stabstr[cnt] == '@');
  cnt++;  // skip the @
...
//  <refType> = &<typeUse>
//
static char *parseRefType(Module *mod, const char *name, int ID, char *stabstr,
                          int &cnt) {
...
//
void addBaseClassToClass(Module *mod, int baseID, fieldListType *newType,
                         int /*offset*/) {
  typeCollection *tc = typeCollection::getModTypeCollection(mod);
  ...
  if (!baseCl) {
    ...
    fieldListType *baseCl2 =
        dynamic_cast<typeStruct *>(tc->addOrUpdateType((typeStruct *)baseCl));
  ...
  for (unsigned int fieldNum=0; fieldNum < baseClFields->size(); fieldNum++) {
      ...
      newType->addField(field->getName(), field->getTypeDesc(),
  field->getType(), field->getOffset()+offset, field->getVisibility());
  }
  */
}
...
//
static char *parseFieldList(Module *mod, fieldListType *newType, char *stabstr,
                            bool sunCPlusPlus) {
  int cnt = 0;
  int size = 0;
  char *compname;
  ...
  while (stabstr[cnt] && (stabstr[cnt] != ';')) {
    ...
    if (stabstr[cnt] ==
        ';')  // jaw 03/15/02-- major kludge here for DPCL compat
...
//
static char *parseCPlusPlusInfo(Module *mod, char *stabstr,
                                const char *mangledName, int ID) {
  typeCollection *tc = typeCollection::getModTypeCollection(mod);
  int cnt;
  char *name;
  int structsize;
  ...
  switch (typdescr) {
    ...
    case dataStructure:
      ...
      newType2 = dynamic_cast<fieldListType *>(
          tc->addOrUpdateType((typeStruct *)newType));
      break;
    case dataUnion:
      newType = new typeUnion(ID, tName);
      newType2 = dynamic_cast<fieldListType *>(
          tc->addOrUpdateType((typeUnion *)newType));
      break;
    default:
      assert(0);
  }
  ...
  if (newType2 != newType) newType->decrRefCount();
  ...
  if (stabstr[0]) {
    ...
    while (stabstr[cnt] && (stabstr[cnt] != ';')) {
      ...
      char *name =
          P_cplus_demangle(methodName.c_str(), mod->exec()->isNativeCompiler());
      if (name != NULL) {
...
//
static char *parseTypeDef(Module *mod, char *stabstr, const char *name, int ID,
    ...
    switch (stabstr[0]) {
      ...
      case 'g': {
...

        {
          ...
          if (newFunction) {
            newFunction2 =
                dynamic_cast<typeFunction *>(tc->addOrUpdateType(newFunction));
            if (newFunction2 != newFunction) newFunction->decrRefCount();
          }
          if (!newFunction2) {
          ...
          while ((stabstr[cnt] != '#') && (stabstr[cnt])) {
            ...
            // newFunction2->addField(buffer, newType->getDataClass(), newType,
      ...
      case 'R': {
        ...
        // gcc 3.0 adds an extra field that is always 0 (no indication in the
        // code why)
      ...
      case 'e':  // Enumerated type
      {
        ...
        // Add type to collection
        newEnumType =
            dynamic_cast<typeEnum *>(tc->addOrUpdateType(newEnumType));
...
//		__FILE__, __LINE__);
      ...
      case 'T':  // Fortran TYPE
      {
        ...
        if (typdescr == dataStructure) {
          ...
          newFieldType2 = dynamic_cast<fieldListType *>(
              tc->addOrUpdateType((typeStruct *)newFieldType));
        } else {
          newFieldType = new typeUnion(ID, tName);
          newFieldType2 = dynamic_cast<fieldListType *>(
              tc->addOrUpdateType((typeUnion *)newFieldType));
        }
        // add to type collection
        ...
        if (newFieldType2 != newFieldType) newFieldType->decrRefCount();
