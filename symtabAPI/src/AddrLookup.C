...
AddressLookup*
AddressLookup::createAddressLookup(PID pid, ProcessReader* reader)
{
    AddressTranslate* trans =
        AddressTranslate::createAddressTranslator(pid, reader, getSymtabReaderFactory());
...
AddressLookup*
AddressLookup::createAddressLookup(ProcessReader* reader)
{
    AddressTranslate* trans =
        AddressTranslate::createAddressTranslator(reader, getSymtabReaderFactory());
    if(!trans)
    {
        return NULL;
    }
...
AddressLookup*
AddressLookup::createAddressLookup(const std::vector<LoadedLibrary>& /*name_addrs*/)
{
...
bool
AddressLookup::getAddress(Symtab* tab, Offset off, Address& addr)
{
...
bool
AddressLookup::getAddress(Symtab* tab, Symbol* sym, Address& addr)
{
    LoadedLib* ll = getLoadedLib(tab);
    if(!ll)
        return false;
...
bool
sort_by_addr(const Symbol* a, const Symbol* b)
{
    return a->getOffset() < b->getOffset();
}

vector<Symbol*>*
AddressLookup::getSymsVector(LoadedLib* lib)
{
...
bool
AddressLookup::getOffset(Address addr, Symtab*& tab, Offset& off)
{
    LoadedLib* lib;
    bool       result;
...
bool
AddressLookup::getOffset(Address addr, LoadedLibrary& ll, Offset& off)
{
    LoadedLib* lib;
    bool       result;
...
bool
AddressLookup::getSymbol(Address addr, Symbol*& sym, Symtab*& tab, bool close)
{
    LoadedLib* lib;
    bool       result;
...
bool
AddressLookup::getAllSymtabs(std::vector<Symtab*>& tabs)
{
...
bool
AddressLookup::getLoadAddress(Symtab* sym, Address& load_addr)
{
...
bool
AddressLookup::getDataLoadAddress(Symtab* sym, Address& load_addr)
{
    LoadedLib* ll = getLoadedLib(sym);
    if(!ll)
        return false;
...
bool
AddressLookup::getLoadAddresses(std::vector<LoadedLibrary>& name_addrs)
{
    vector<LoadedLib*> libs;
    bool               result = translator->getLibs(libs);
    if(!result)
...
Address
AddressLookup::getLibraryTrapAddrSysV()
{
...
bool
AddressLookup::refresh()
{
    return translator->refresh();
}

bool
AddressLookup::getExecutable(LoadedLibrary& lib)
{
...
Dyninst::Address
AddressLookup::symToAddress(LoadedLib* ll, Symbol* sym)
{
    return ll->getCodeLoadAddr() + sym->getOffset();
}

LoadedLib*
AddressLookup::getLoadedLib(Symtab* sym)
{
    ...
    for(auto ll : libs)
    {
        if(sym->file() == ll->getName() || sym->name() == ll->getName())
        {
            ll_to_sym[ll]  = sym;
            sym_to_ll[sym] = ll;
            return ll;
...
Symtab*
AddressLookup::getSymtab(LoadedLib* ll)
{
