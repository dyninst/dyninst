...
AddressLookup *AddressLookup::createAddressLookup(PID pid,
  ...
  AddressTranslate *trans = AddressTranslate::createAddressTranslator(
      pid, reader, getSymtabReaderFactory());
...
AddressLookup *AddressLookup::createAddressLookup(
    const std::vector<LoadedLibrary> & /*name_addrs*/) {
...
bool AddressLookup::getSymbol(Address addr, Symbol *&sym, Symtab *&tab,
                              bool close) {
  LoadedLib *lib;
  bool result;
...
AddressLookup::AddressLookup(AddressTranslate *at) : translator(at) {}

AddressLookup::~AddressLookup() {}

bool AddressLookup::refresh() { return translator->refresh(); }

bool AddressLookup::getExecutable(LoadedLibrary &lib) {
  LoadedLib *llib = translator->getExecutable();
  if (!llib) return false;
