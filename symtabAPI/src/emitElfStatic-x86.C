 ...
 */
static bool
computeCtorDtorAddress(relocationEntry& rel, Offset globalOffset, LinkMap& lmap, string&,
                       Offset& symbolOffset)
{
    /*
      if( rel.name() ==  SYMTAB_CTOR_LIST_REL ) {
          ...
          if( lmap.newCtorRegions.size() > 0 ) {
              ...
              errMsg = "Failed to locate original .ctors Region -- cannot apply
      relocation"; return false;
          ...
          if( lmap.newDtorRegions.size() > 0 ) {
              ...
              errMsg = "Failed to locate original .dtors Region -- cannot apply
      relocation"; return false;
          }
      } else*/
                                      ...
                                      Offset globalOffset, LinkMap& lmap, string& errMsg)
{
    ...
    if(X86_WIDTH == addressWidth_)
    {
        ...
        switch(rel.getRelType())
        {
            case R_386_32:
                ...
                break;
            case R_386_PLT32:  // this works because the address of the symbol is known at
                               // link time
            case R_386_PC32:
            ...
            case R_386_GOTOFF:
                relocation =
                    symbolOffset + addend - (lmap.gotRegionOffset + globalOffset);
                break;
            case R_386_GOTPC:
    ...
    else if(X86_64_WIDTH == addressWidth_)
    {
        ...
        rewrite_printf(
            "relocation for '%s': TYPE = %s(%lu) S = %lx A = %lx P = %lx Z = %x\n",
            rel.name().c_str(),
            relocationEntry::relType2Str(rel.getRelType(), addressWidth_),
            rel.getRelType(), symbolOffset, addend, relOffset, symbolSize);
        ...
        switch(rel.getRelType())
        {
            ...
            case R_X86_64_GOT32:  // The address is computed when the GOT is built
                ...
                break;
            case R_X86_64_TPOFF32:  // The necessary value is set during storage
                                    // allocation
                fieldSize  = sizeof(Elf64_Word);
                relocation = symbolOffset;
                break;
...
bool
emitElfStatic::checkSpecialCaseSymbols(Symtab* member, Symbol* checkSym)
{
    /*
     * Special Case 1
     *
...
void
emitElfStatic::getExcludedSymbolNames(set<string>& symNames)
{
    /*
...
Offset
emitElfStatic::getGOTSize(Symtab*, LinkMap& lmap, Offset& layoutStart)
{
    Offset size = 0;
    layoutStart = 0;
...
void
emitElfStatic::buildGOT(Symtab*, LinkMap& lmap)
{
...
inline static bool
adjustValInRegion(Region* reg, Offset offInReg, Offset addressWidth, int adjust)
{
    Offset         newValue;
