  ...
  if (X86_WIDTH == addressWidth_) {
    ...
    switch (rel.getRelType()) {
      case R_386_32:
        ...
        break;
      case R_386_PLT32:  // this works because the address of the symbol is
                         // known at link time
      case R_386_PC32:
        relocation = symbolOffset + addend - relOffset;
        break;
      case R_386_TLS_LE:  // The necessary value is set during storage
                          // allocation
      case R_386_GLOB_DAT:
      case R_386_JMP_SLOT:
    ...
    rewrite_printf(
        "relocation for '%s': TYPE = %s(%lu) S = %llx A = %llx P = %llx Z = "
        "%lx\n",
        rel.name().c_str(),
        relocationEntry::relType2Str(rel.getRelType(), addressWidth_),
...
bool emitElfStatic::checkSpecialCaseSymbols(Symtab *member, Symbol *checkSym) {
  ...
  if (isStripped_) {
    if (LIBC_ATEXIT_INTERNAL == checkSym->getPrettyName() ||
      ...
      fprintf(stderr,
              "WARNING: code in %s(%s) references the libc function %s.\n",
              member->getParentArchive()->name().c_str(),
              member->memberName().c_str(), checkSym->getPrettyName().c_str());
      fprintf(stderr,
              "         Also, the binary to be rewritten is stripped.\n");
      fprintf(stderr,
...
void emitElfStatic::buildGOT(Symtab *, LinkMap &lmap) {
  ...
  for (sym_it = lmap.gotSymbols.begin(); sym_it != lmap.gotSymbols.end();
       ++sym_it) {
...
bool emitElfStatic::createNewCtorRegion(LinkMap &) { return true; }
...
bool emitElfStatic::createNewDtorRegion(LinkMap &) { return true; }
