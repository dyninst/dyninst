...
static int
findIntrensicType(std::string& name);
...
typeId_t
Type::getUniqueTypeId()
{
    return ::getUniqueTypeId();
}

void
Type::updateUniqueTypeId(typeId_t id)
{
...
Type::createFake(std::string name)
{
...
Type::unique_ptr_Type
Type::createPlaceholder(typeId_t ID, std::string name)
{
...
const char*
Dyninst::SymtabAPI::dataClass2Str(dataClass dc)
{
    switch(dc)
    {
...
bool
Type::operator==(const Type& otype) const
{
...
bool
Type::setSize(unsigned int size)
{
    size_ = size;
    return true;
}
...
dataClass
Type::getDataClass() const
{
    return type_;
}

void
Type::fixupUnknowns(Module*)
{}
...
{
    if(getEnumType())
        return std::string("typeEnum");
    if(getPointerType())
        return std::string("typePointer");
    if(getFunctionType())
        return std::string("typeFunction");
    if(getSubrangeType())
        return std::string("typeSubrange");
    if(getArrayType())
        return std::string("typeArray");
    if(getStructType())
        return std::string("typeStruct");
    if(getUnionType())
        return std::string("typeUnion");
    if(getScalarType())
        return std::string("typeScalar");
    if(getCommonType())
        return std::string("typeCommon");
    if(getTypedefType())
        return std::string("typeTypedef");
    if(getRefType())
        return std::string("typeRef");
    return std::string("badType");
}
bool
Type::isCompatible(Type* /*oType*/)
{
    return true;
}
...
typeEnum*
typeEnum::create(std::string& name, dyn_c_vector<std::pair<std::string, int>*>& constants,
                 Symtab* obj)
{
...
typeEnum*
typeEnum::create(std::string& name, dyn_c_vector<std::string>& constNames, Symtab* obj)
{
    typeEnum* typ = new typeEnum(name);
...
dyn_c_vector<std::pair<std::string, int>>&
...
bool
typeEnum::addConstant(const std::string& constName, int value)
{
...
bool
typeEnum::isCompatible(Type* otype)
{
    ...
    if((name_ != "") && (oEnumtype->name_ != "") && (name_ == oEnumtype->name_) &&
       (ID_ == oEnumtype->ID_))
        return true;
...
typePointer*
typePointer::create(std::string& name, boost::shared_ptr<Type> ptr, Symtab* obj)
{
    if(!ptr)
        return NULL;
...
typePointer*
typePointer::create(std::string& name, boost::shared_ptr<Type> ptr, int size, Symtab* obj)
{
    if(!ptr)
        return NULL;
    typePointer* typ = new typePointer(ptr, name);
...
bool
typePointer::setPtr(boost::shared_ptr<Type> ptr)
{
    assert(ptr);
    baseType_ = ptr;
...
bool
typePointer::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typePointer::fixupUnknowns(Module* module)
{
...
typeFunction*
typeFunction::create(std::string& name, boost::shared_ptr<Type> retType,
                     dyn_c_vector<boost::shared_ptr<Type>>& paramTypes, Symtab* obj)
{
...
bool
typeFunction::setRetType(boost::shared_ptr<Type> rtype)
{
    retType_ = rtype;
    return true;
}
...
bool
typeFunction::addParam(boost::shared_ptr<Type> paramType)
{
    params_.push_back(paramType);
    return true;
}

dyn_c_vector<boost::shared_ptr<Type>>&
typeFunction::getParams()
{
    return params_;
}

bool
typeFunction::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typeFunction::fixupUnknowns(Module* module)
{
    typeCollection* tc = typeCollection::getModTypeCollection(module);
    assert(tc);
...
// typeSubRange::typeSubRange(int ID, int size, const char *_low, const char *_hi, const
// char *_name)
...
typeSubrange*
typeSubrange::create(std::string& name, int size, long low, long hi, Symtab* obj)
{
...
bool
typeSubrange::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
typeArray::typeArray(typeId_t ID, boost::shared_ptr<Type> base, long low, long hi,
...
typeArray::typeArray(boost::shared_ptr<Type> base, long low, long hi, std::string name,
...
typeArray*
typeArray::create(std::string& name, boost::shared_ptr<Type> type, long low, long hi,
                  Symtab* obj)
{
...
bool
typeArray::operator==(const Type& otype) const
{
    try
    {
        const typeArray& oArraytype = dynamic_cast<const typeArray&>(otype);
        if(sizeHint_ != oArraytype.sizeHint_)
            return false;
        if(arrayElem && !oArraytype.arrayElem)
            return false;
        if(!arrayElem && oArraytype.arrayElem)
            return false;
        if(arrayElem)
        {
            if(arrayElem->getID() != oArraytype.arrayElem->getID())
                return false;
        }
...
bool
typeArray::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typeArray::fixupUnknowns(Module* module)
{
    if(arrayElem->getDataClass() == dataUnknownType)
    {
        typeCollection* tc = typeCollection::getModTypeCollection(module);
...
typeStruct*
typeStruct::create(std::string&                                                    name,
                   dyn_c_vector<std::pair<std::string, boost::shared_ptr<Type>>*>& flds,
                   Symtab*                                                         obj)
{
    int         offset = 0;
...
typeStruct*
typeStruct::create(std::string& name, dyn_c_vector<Field*>& flds, Symtab* obj)
{
    typeStruct* typ = new typeStruct(name);
    for(unsigned i = 0; i < flds.size(); i++)
        typ->addField(flds[i]);
...
void
typeStruct::postFieldInsert(int)
{}

bool
typeStruct::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typeStruct::fixupUnknowns(Module* module)
{
    for(unsigned int i = 0; i < fieldList.size(); i++)
...
typeUnion*
typeUnion::create(std::string&                                                    name,
                  dyn_c_vector<std::pair<std::string, boost::shared_ptr<Type>>*>& flds,
                  Symtab*                                                         obj)
{
...
typeUnion*
typeUnion::create(std::string& name, dyn_c_vector<Field*>& flds, Symtab* obj)
{
    typeUnion* typ = new typeUnion(name);
    for(unsigned i = 0; i < flds.size(); i++)
        typ->addField(flds[i]);
...
void
typeUnion::postFieldInsert(int nsize)
{
    if((unsigned int) nsize > size_)
        size_ = nsize;
}

bool
typeUnion::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typeUnion::fixupUnknowns(Module* module)
{
    for(unsigned int i = 0; i < fieldList.size(); i++)
        fieldList[i]->fixupUnknown(module);
}
 ...
 */
typeScalar*
typeScalar::create(std::string& name, int size, Symtab* obj)
{
...
bool
typeScalar::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typeCommon::beginCommonBlock()
{
    fieldList.clear();
}

void
typeCommon::endCommonBlock(Symbol* func, void* baseAddr)
{
    unsigned int i, j;
    ...
    {
        localVar* locVar;
        locVar = new localVar(fieldList[j]->getName(), fieldList[j]->getType(Type::share),
                              "", 0, (Function*) func);
#if 0
...
void
typeCommon::fixupUnknowns(Module* module)
{
...
typeTypedef::typeTypedef(typeId_t ID, boost::shared_ptr<Type> base, std::string name,
                         unsigned int sizeHint)
...
typeTypedef::typeTypedef(boost::shared_ptr<Type> base, std::string name,
                         unsigned int sizeHint)
...
typeTypedef*
typeTypedef::create(std::string& name, boost::shared_ptr<Type> baseType, Symtab* obj)
{
    if(!baseType)
        return NULL;
...
bool
typeTypedef::operator==(const Type& otype) const
{
    try
    {
...
bool
typeTypedef::isCompatible(Type* otype)
{
    return baseType_->isCompatible(otype);
}

void
typeTypedef::updateSize()
{
    if(updatingSize)
    {
        size_ = 0;
...
void
typeTypedef::fixupUnknowns(Module* module)
{
    if(baseType_->getDataClass() == dataUnknownType)
    {
        typeCollection* tc = typeCollection::getModTypeCollection(module);
...
typeRef*
typeRef::create(std::string& name, boost::shared_ptr<Type> ref, Symtab* obj)
{
...
bool
typeRef::operator==(const Type& otype) const
{
    try
    {
...
bool
typeRef::isCompatible(Type* otype)
{
    if((otype->getDataClass() == dataUnknownType) ||
       (otype->getDataClass() == dataNullType))
        return true;
...
void
typeRef::fixupUnknowns(Module* module)
{
    if(baseType_->getDataClass() == dataUnknownType)
    {
        typeCollection* tc = typeCollection::getModTypeCollection(module);
...
bool
fieldListType::operator==(const Type& otype) const
{
    try
    {
...
dyn_c_vector<Field*>*
fieldListType::getComponents() const
{
...
fieldListType::fixupComponents()
{
    ...
    for(unsigned int i = 0; i < fieldList.size(); i++)
    {
        ...
        if(currentField->getName() == "{superclass}")
        {
            ...
            auto& superclass =
                dynamic_cast<fieldListInterface&>(*currentField->getType(Type::share));
            ...
            /* FIXME: visibility can also be described on a per-name basis in the
               subclass.  We have now way to convey this information currently, but I'm
               not sure that it matters for our purposes... */
 ...
 */
void
fieldListType::addField(std::string fieldname, boost::shared_ptr<Type> type,
...
void
fieldListType::addField(Field* fld)
{
...
void
fieldListType::addField(unsigned num, std::string fieldname, boost::shared_ptr<Type> type,
                        int offsetVal, visibility_t vis)
{
    Field* newField;
    newField = new Field(fieldname, type, offsetVal, vis);
...
void
fieldListType::addField(unsigned num, Field* fld)
{
    Field* newField = new Field(*fld);
    // Add field to list of struct/union fields
    if(num > fieldList.size() + 1)
...
bool
derivedType::operator==(const Type& otype) const
{
    try
    {
...
rangedType::rangedType(std::string& name, typeId_t ID, dataClass typeDes, int size,
...
rangedType::rangedType(std::string& name, dataClass typeDes, int size, unsigned long low,
                       unsigned long hi)
: Type(name, ::getUniqueTypeId(), typeDes)
, low_(low)
, hi_(hi)
...
/*
rangedType::rangedType(const char *_name, int _ID, dataClass _class, int _size, const char
*_low, const char *_hi) : Type(_name, _ID, _class) {
...
bool
rangedType::operator==(const Type& otype) const
{
    try
    {
 ...
 * type = offset = size = 0;
 */
Field::Field(std::string name, boost::shared_ptr<Type> typ, int offsetVal,
             visibility_t vis)
: FIELD_ANNOTATABLE_CLASS()
, fieldName_(name)
, type_(typ)
...
boost::shared_ptr<Type> Field::getType(Type::do_share_t) { return type_; }
...
void
Field::fixupUnknown(Module* module)
{
...
Field::operator==(const Field& f) const
{
    ...
    if(type_)
    {
        if(type_->getID() != f.type_->getID())
            return false;
    }
    if(fieldName_ != f.fieldName_)
        return false;
    if(vis_ != f.vis_)
        return false;
    if(offset_ != f.offset_)
        return false;
    return true;
}
...
void
CBlock::fixupUnknowns(Module* module)
{
    for(unsigned int i = 0; i < fieldList.size(); i++)
    {
        fieldList[i]->fixupUnknown(module);
