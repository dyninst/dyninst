...
bool Type::operator==(const Type &otype) const {
  return (ID_ == otype.ID_ && name_ == otype.name_ && size_ == otype.size_ &&
          type_ == otype.type_);
}

unsigned int Type::getSize() {
  if (!size_) const_cast<Type *>(this)->updateSize();
  return size_;
}
...
void Type::incrRefCount() { ++refCount; }

void Type::decrRefCount() {
  if (refCount > 0) refCount--;
}

std::string &Type::getName() { return name_; }
...
typeId_t Type::getID() const { return ID_; }

dataClass Type::getDataClass() const { return type_; }

void Type::fixupUnknowns(Module *) {}

typeEnum *Type::getEnumType() { return dynamic_cast<typeEnum *>(this); }
...
typeArray *Type::getArrayType() { return dynamic_cast<typeArray *>(this); }

typeStruct *Type::getStructType() { return dynamic_cast<typeStruct *>(this); }

typeUnion *Type::getUnionType() { return dynamic_cast<typeUnion *>(this); }

typeScalar *Type::getScalarType() { return dynamic_cast<typeScalar *>(this); }

typeCommon *Type::getCommonType() { return dynamic_cast<typeCommon *>(this); }

typeTypedef *Type::getTypedefType() {
  return dynamic_cast<typeTypedef *>(this);
}

typeRef *Type::getRefType() { return dynamic_cast<typeRef *>(this); }
...
bool Type::isCompatible(Type * /*oType*/) { return true; }

/*
 * ENUM
 */
typeEnum::typeEnum(int ID, std::string name) : Type(name, ID, dataEnum) {
  size_ = sizeof(int);
}

typeEnum::typeEnum(std::string name) : Type(name, USER_TYPE_ID--, dataEnum) {
  size_ = sizeof(int);
}

typeEnum *typeEnum::create(
    std::string &name, std::vector<std::pair<std::string, int> *> &constants,
    Symtab *obj) {
  typeEnum *typ = new typeEnum(name);
  for (unsigned i = 0; i < constants.size(); i++)
    typ->addConstant(constants[i]->first, constants[i]->second);

  if (obj) obj->addType(typ);
  // obj->addType(typ); TODO: declare a static container if obj is NULL and add
  // to it.
  // Symtab::noObjTypes->push_back(typ); ??
  return typ;
}

typeEnum *typeEnum::create(std::string &name,
                           std::vector<std::string> &constNames, Symtab *obj) {
  typeEnum *typ = new typeEnum(name);
...
bool typeEnum::isCompatible(Type *otype) {
  ...
  if (oEnumtype == NULL) return false;

  if ((name_ != "") && (oEnumtype->name_ != "") &&
      (name_ == oEnumtype->name_) && (ID_ == oEnumtype->ID_))
    return true;

  const std::vector<std::pair<std::string, int> > &fields1 =
      this->getConstants();
  const std::vector<std::pair<std::string, int> > &fields2 =
      oEnumtype->getConstants();
  ...
  for (unsigned int i = 0; i < fields1.size(); i++) {
    ...
    if ((field1.second != field2.second) || (field1.first != field2.first)) {
...
typePointer *typePointer::create(std::string &name, Type *ptr, int size,
                                 Symtab *obj) {
  if (!ptr) return NULL;
  typePointer *typ = new typePointer(ptr, name);
  typ->setSize(size);
...
typeFunction::typeFunction(typeId_t ID, Type *retType, std::string name)
  ...
  size_ = sizeof(void *);
  if (retType) retType->incrRefCount();
}
...
typeFunction *typeFunction::create(std::string &name, Type *retType,
  ...
  if (obj) obj->addType(type);
  // obj->addType(type); TODO: declare a static container if obj is NULL and add
  // to it.
  // Symtab::noObjTypes->push_back(type); ??
  return type;
}

Type *typeFunction::getReturnType() const { return retType_; }

bool typeFunction::setRetType(Type *rtype) {
  if (retType_) retType_->decrRefCount();
  retType_ = rtype;
...
std::vector<Type *> &typeFunction::getParams() { return params_; }
...
bool typeFunction::isCompatible(Type *otype) {
  ...
  if (oFunctiontype == NULL) return false;

  if (retType_ != oFunctiontype->retType_) return false;

  std::vector<Type *> fields1 = this->getParams();
  std::vector<Type *> fields2 = oFunctiontype->getParams();
  // const std::vector<Field *> * fields2 = (std::vector<Field *> *)
  // &(otype->fieldList);
...
typeFunction::~typeFunction() { retType_->decrRefCount(); }

/*
 * RANGE
 */

// typeSubRange::typeSubRange(int ID, int size, const char *_low, const char
// *_hi, const char *_name)
//   : rangedType(_name, _ID, BPatchSymTypeRange, _size, _low, _hi)
//{
//}

typeSubrange::typeSubrange(typeId_t ID, int size, long low, long hi,
                           std::string name)
...
typeSubrange *typeSubrange::create(std::string &name, int size, long low,
                                   long hi, Symtab *obj) {
...
bool typeSubrange::isCompatible(Type *otype) {
  ...
  if (oRangetype == NULL) return false;
...
typeArray::typeArray(typeId_t ID, Type *base, long low, long hi,
                     std::string name, unsigned int sizeHint)
...
typeArray::typeArray(Type *base, long low, long hi, std::string name,
...
Type *typeArray::getBaseType() const { return arrayElem; }
...
void typeArray::updateSize() {
  ...
  if (arrayElem->getDataClass() == dataUnknownType) size_ = 0;
...
typeStruct *typeStruct::create(
    std::string &name, std::vector<std::pair<std::string, Type *> *> &flds,
    Symtab *obj) {
  int offset = 0;
...
typeStruct *typeStruct::create(std::string &name, std::vector<Field *> &flds,
                               Symtab *obj) {
  typeStruct *typ = new typeStruct(name);
  for (unsigned i = 0; i < flds.size(); i++) typ->addField(flds[i]);
  if (obj) obj->addType(typ);
  // obj->addType(typ); TODO: declare a static container if obj is NULL and add
  // to it.
  // Symtab::noObjTypes->push_back(typ); ??
...
void typeStruct::merge(Type *other) {
  ...
  if (otherstruct->name_ != "") name_ = std::string(otherstruct->name_);
...
bool typeStruct::isCompatible(Type *otype) {
  ...
  if (oStructtype == NULL) return false;
...
typeUnion *typeUnion::create(std::string &name, std::vector<Field *> &flds,
                             Symtab *obj) {
  typeUnion *typ = new typeUnion(name);
  for (unsigned i = 0; i < flds.size(); i++) typ->addField(flds[i]);
  if (obj) obj->addType(typ);
...
void typeUnion::merge(Type *other) {
  ...
  if (!fieldList.size()) return;

  if (otherunion->name_ != "") name_ = std::string(otherunion->name_);
...
void typeUnion::updateSize() {
  ...
  size_ = 0;
  for (unsigned int i = 0; i < fieldList.size(); ++i) {
    if (fieldList[i]->getSize() > size_) size_ = fieldList[i]->getSize();
...
bool typeUnion::isCompatible(Type *otype) {
  ...
  if (oUniontype == NULL) return false;
...
typeScalar::typeScalar(typeId_t ID, unsigned int size, std::string name,
                       bool isSigned)
...
bool typeScalar::isSigned() { return isSigned_; }
...
void typeCommon::endCommonBlock(Symbol *func, void *baseAddr) {
  ...
  for (j = 0; j < fieldList.size(); j++) {
    localVar *locVar;
    locVar = new localVar(fieldList[j]->getName(), fieldList[j]->getType(), "",
                          0, (Function *)func);
#if 0
...
typeTypedef::typeTypedef(typeId_t ID, Type *base, std::string name,
                         unsigned int sizeHint)
...
typeTypedef *typeTypedef::create(std::string &name, Type *baseType,
                                 Symtab *obj) {
...
void typeTypedef::updateSize() {
  ...
  if (baseType_->getDataClass() == dataUnknownType) size_ = 0;
...
typeRef::typeRef(int ID, Type *refType, std::string name)
  ...
  baseType_ = refType;
  if (refType) refType->incrRefCount();
}
...
bool typeRef::isCompatible(Type *otype) {
  ...
  return baseType_->isCompatible(
      const_cast<Type *>(oReftype->getConstituentType()));
}
...
fieldListType::~fieldListType() { fieldList.clear(); }

bool fieldListType::operator==(const Type &otype) const {
  try {
    const fieldListType &oFieldtype =
        dynamic_cast<const fieldListType &>(otype);
    if (fieldList.size() != oFieldtype.fieldList.size()) return false;
    for (unsigned int i = 0; i < fieldList.size(); i++) {
...
void fieldListType::fixupComponents() {
  ...
  for (unsigned int i = 0; i < fieldList.size(); i++) {
    ...
    if (currentField->getName() == "{superclass}") {
      ...
      // );
      fieldListInterface *superclass =
          dynamic_cast<fieldListInterface *>(currentField->getType());
      assert(superclass != NULL);
      const std::vector<Field *> *superClassFields =
          superclass->getComponents();
      // bperr( "Superclass has %d components.\n", superClassFields->size() );
      /* FIXME: do we also need to consider the visibility of the superclass
       * itself? */
      /* FIXME: visibility can also be described on a per-name basis in the
         subclass.  We have now way to convey this information currently, but
         I'm not
 ...
 *     STRUCTS OR UNIONS
 */
void fieldListType::addField(std::string fieldname, Type *type, int offsetVal,
                             visibility_t vis) {
  Field *newField;
...
void fieldListType::addField(unsigned num, std::string fieldname, Type *type,
                             int offsetVal, visibility_t vis) {
  Field *newField;
  newField = new Field(fieldname, type, offsetVal, vis);

  if (num > fieldList.size() + 1) num = fieldList.size();
  // Add field to list of struct/union fields
...
derivedType::derivedType(std::string &name, typeId_t id, int size,
                         dataClass typeDes)
...
Type *derivedType::getConstituentType() const { return baseType_; }

bool derivedType::operator==(const Type &otype) const {
  try {
    // const derivedType &oderivedtype = dynamic_cast<const derivedType
    // &>(otype);
    return Type::operator==(otype);
  } catch (...) {
    return false;
...
derivedType::~derivedType() {
  if (baseType_) baseType_->decrRefCount();
}
...
rangedType::rangedType(std::string &name, typeId_t ID, dataClass typeDes,
                       int size, unsigned long low, unsigned long hi)
    : Type(name, ID, typeDes), low_(low), hi_(hi) {
  size_ = size;
}

rangedType::rangedType(std::string &name, dataClass typeDes, int size,
...
/*
rangedType::rangedType(const char *_name, int _ID, dataClass _class, int _size,
...
struct intrensicTypes_ intrensicTypes[] = {
    {"int", 1}, {"integer*4", 1}, {"INTEGER*4", 1}, {NULL, 0},
};
...
Field::Field()
    : FIELD_ANNOTATABLE_CLASS(), type_(NULL), vis_(visUnknown), offset_(-1) {}
/*
 * Field::Field
 *
...
Field::Field(std::string name, Type *typ, int offsetVal, visibility_t vis)
    : FIELD_ANNOTATABLE_CLASS(),
      ...
      vis_(vis),
      offset_(offsetVal) {
  if (typ) typ->incrRefCount();
}

std::string &Field::getName() { return fieldName_; }

Type *Field::getType() { return type_; }

visibility_t Field::getVisibility() { return vis_; }

int Field::getOffset() { return offset_; }

unsigned int Field::getSize() { return type_ ? type_->getSize() : 0; }

Field::Field(Field &oField) : Serializable(), FIELD_ANNOTATABLE_CLASS() {
  type_ = oField.type_;
  offset_ = oField.offset_;
  fieldName_ = std::string(oField.fieldName_);
  vis_ = oField.vis_;

  if (type_ != NULL) type_->incrRefCount();
}

Field::~Field() {
  if (type_ != NULL) type_->decrRefCount();
}
...
std::vector<Field *> *CBlock::getComponents() { return &fieldList; }

std::vector<Symbol *> *CBlock::getFunctions() { return &functions; }

Type::Type()
    : ID_(0),
      name_(std::string("unnamedType")),
      size_(0),
      type_(dataUnknownType),
      updatingSize(false),
      refCount(1) {}
...
Serializable *Type::serialize_impl(SerializerBase *s, const char *tag)
...
void typeEnum::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typePointer::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
  derivedType *dt = this;
...
void typeFunction::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
  ...
  for (unsigned int i = 0; i < params_.size(); ++i)
    ptypes.push_back(
        std::pair<int, int>(params_[i]->getID(), params_[i]->getDataClass()));
  ...
  if (sb->isInput()) {
    ...
    for (unsigned int i = 0; i < ptypes.size(); ++i) {
      ...
      typeCollection::addDeferredLookup(
          ptypes[i].first, (dataClass)ptypes[i].second, &params_[i]);
    }
  }
}

void typeSubrange::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typeArray::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typeStruct::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typeUnion::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typeScalar::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typeCommon::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
...
void typeTypedef::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
  derivedType *dt = this;
...
void typeRef::serialize_specific(SerializerBase *sb)
    THROW_SPEC(SerializerError) {
  derivedType *dt = this;
...
void fieldListType::serialize_fieldlist(SerializerBase *sb, const char *tag)
    THROW_SPEC(SerializerError) {
  ...
  if (have_derived_field_list) {
    ...
    if (sb->isInput()) derivedFieldList = new std::vector<Field *>();
    gtranslate(sb, *derivedFieldList, "derivedFieldList");
  }
  ifxml_end_element(sb, tag);
}

void derivedType::serialize_derived(SerializerBase *sb, const char *tag)
    THROW_SPEC(SerializerError) {
...
void rangedType::serialize_ranged(SerializerBase *sb, const char *tag)
    THROW_SPEC(SerializerError) {
  ifxml_start_element(sb, tag);
...
Serializable *Field::serialize_impl(SerializerBase *sb, const char *tag)
    THROW_SPEC(SerializerError) {
...
Serializable *CBlock::serialize_impl(SerializerBase *sb, const char *tag)
    THROW_SPEC(SerializerError) {
...
Serializable *Type::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}

void typeEnum::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typePointer::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeFunction::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeSubrange::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeArray::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeStruct::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeUnion::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeScalar::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeCommon::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeTypedef::serialize_specific(SerializerBase *)
    THROW_SPEC(SerializerError) {}

void typeRef::serialize_specific(SerializerBase *) THROW_SPEC(SerializerError) {
}

void fieldListType::serialize_fieldlist(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {}

void derivedType::serialize_derived(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {}

void rangedType::serialize_ranged(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {}

Serializable *Field::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}

Serializable *CBlock::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}
