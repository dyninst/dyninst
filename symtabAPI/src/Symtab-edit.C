...
static Symbol deletedSymbol(std::string("DeletedSymbol"), Symbol::ST_DELETED,
                            Symbol::SL_UNKNOWN, Symbol::SV_UNKNOWN, 0);
/*
 * We're changing the type of a symbol. Therefore we need to rip it out of the
 * indices
...
bool Symtab::deleteFunction(Function *func) {
  ...
  everyFunction.erase(
      std::remove(everyFunction.begin(), everyFunction.end(), func),
      everyFunction.end());
...
bool Symtab::deleteVariable(Variable *var) {
  // First, remove the function
  everyVariable.erase(
      std::remove(everyVariable.begin(), everyVariable.end(), var),
      everyVariable.end());
...
bool Symtab::deleteAggregate(Aggregate *agg) {
  ...
  for (unsigned i = 0; i < syms.size(); i++) {
    if (!deleteSymbolFromIndices(syms[i])) ret = false;
  }
  return ret;
}
...
bool Symtab::changeSymbolOffset(Symbol *sym, Offset newOffset) {
  ...
  while (found != defindex.end() && (*found)->getOffset() == sym->offset_) {
    if (*found == sym) {
...
bool Symtab::changeAggregateOffset(Aggregate *agg, Offset oldOffset,
                                   Offset newOffset) {
...
bool Symtab::addSymbol(Symbol *newSym, Symbol *referringSymbol) {
  ...
  if (!referringSymbol->getSymtab()->isStaticBinary()) {
    ...
    if (referringSymbol->getVersions(vers) && vers != NULL &&
        vers->size() > 0) {
...
Function *Symtab::createFunction(std::string name, Offset offset, size_t sz,
                                 Module *mod) {
  Region *reg = NULL;
  ...
  Symbol *statSym =
      new Symbol(name, Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
...
Variable *Symtab::createVariable(std::string name, Offset offset, size_t sz,
                                 Module *mod) {
  Region *reg = NULL;
  // Let's get the module hammered out.
  if (mod == NULL) {
  ...
  Symbol *statSym =
      new Symbol(name, Symbol::ST_OBJECT, Symbol::SL_GLOBAL, Symbol::SV_DEFAULT,
                 offset, mod, reg, sz, false, false);
  Symbol *dynSym =
      new Symbol(name, Symbol::ST_OBJECT, Symbol::SL_GLOBAL, Symbol::SV_DEFAULT,
                 offset, mod, reg, sz, true, false);
...
SYMTAB_EXPORT bool Symtab::updateRelocations(Address start, Address end,
