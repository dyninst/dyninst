...
struct CtorComp
{
    bool operator()(T lhs, T rhs) const
    {
        ...
        if(lhs->symtab()->name() == first_ctor_name)
            return true;
        if(rhs->symtab()->name() == first_ctor_name)
            return false;
        if(lhs->symtab()->name() == last_ctor_name)
            return false;
        if(rhs->symtab()->name() == last_ctor_name)
            return true;
        return true;
    }
};
...
public:
    ...
    void printRegionFromInfo(ostream& os, Region* region, Offset regionOffset,
                             Offset padding);
