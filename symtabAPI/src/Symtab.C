...
SYMTABEXPORT unsigned Symtab::getAddressWidth() const 
{
   return address_width_;
}
 
SYMTABEXPORT bool Symtab::isNativeCompiler() const 
{
    return nativeCompiler; 
}
 

SYMTABEXPORT Symtab::Symtab(MappedFile *mf_) :
   AnnotatableSparse(),
   mf(mf_), 
   mfForDebugInfo(mf_),
   obj_private(NULL)
...
SYMTABEXPORT bool Symtab::isExec() const 
{
    return is_a_out; 
}

SYMTABEXPORT bool Symtab::isStripped() 
{
...
SYMTABEXPORT Offset Symtab::imageOffset() const 
{
    return imageOffset_;
}

SYMTABEXPORT Offset Symtab::dataOffset() const 
{ 
    return dataOffset_;
}

SYMTABEXPORT Offset Symtab::dataLength() const 
{
    return dataLen_;
} 

SYMTABEXPORT Offset Symtab::imageLength() const 
{
    return imageLen_;
}
/*
SYMTABEXPORT char* Symtab::image_ptr ()  const 
{
   return code_ptr_;
}

SYMTABEXPORT char* Symtab::data_ptr ()  const 
{ 
   return data_ptr_;
}
*/
SYMTABEXPORT const char*  Symtab::getInterpreterName() const 
{
...
SYMTABEXPORT Offset Symtab::getEntryOffset() const 
{ 
   return entry_address_;
}

SYMTABEXPORT Offset Symtab::getBaseOffset() const 
{
   return base_address_;
}

SYMTABEXPORT Offset Symtab::getLoadOffset() const 
{ 
   return load_address_;
}

SYMTABEXPORT Offset Symtab::getTOCoffset() const 
{
   return toc_offset_;
}

SYMTABEXPORT string Symtab::getDefaultNamespacePrefix() const
{
...
SYMTABEXPORT std::vector<std::string> &Symtab::getDependencies(){
    return deps_;
}
...
SYMTABEXPORT bool Symtab::getAddressRanges(std::vector<pair<Offset, Offset> >&ranges,
...
SYMTABEXPORT bool Symtab::getSourceLines(std::vector<LineNoTuple> &lines, Offset addressInRange)
{
...
SYMTABEXPORT bool Symtab::addLine(std::string lineSource, unsigned int lineNo,
...
SYMTABEXPORT bool Symtab::addAddressRange( Offset lowInclusiveAddr, Offset highExclusiveAddr,
      std::string lineSource, unsigned int lineNo,
...
SYMTABEXPORT vector<Type *> *Symtab::getAllstdTypes()
{
    setupStdTypes();
    return stdTypes->getAllTypes(); 	
}

SYMTABEXPORT vector<Type *> *Symtab::getAllbuiltInTypes()
{
    setupStdTypes();
    return builtInTypes->getAllBuiltInTypes();
}

SYMTABEXPORT bool Symtab::findType(Type *&type, std::string name)
{
    parseTypesNow();
    if(!_mods.size())
    	return false;
...
SYMTABEXPORT bool Symtab::findVariableType(Type *&type, std::string name)
{
	parseTypesNow();
    if(!_mods.size())
    	return false;
...
SYMTABEXPORT bool Symtab::findLocalVariable(std::vector<localVar *>&vars, std::string name)
{
    parseTypesNow();
...
SYMTABEXPORT bool Symtab::hasRel() const
{
    return hasRel_;
}

SYMTABEXPORT bool Symtab::hasRela() const
{
    return hasRela_;
}
...
SYMTABEXPORT bool Symtab::emitSymbols(Object *linkedFile,std::string filename, unsigned flag)
{
...
SYMTABEXPORT bool Symtab::emit(std::string filename, unsigned flag)
{
...
SYMTABEXPORT void Symtab::addDynLibSubstitution(std::string oldName, std::string newName)
{
    dynLibSubs[oldName] = newName;
}

SYMTABEXPORT std::string Symtab::getDynLibSubstitution(std::string name)
{
#ifdef BINEDIT_DEBUG
...
SYMTABEXPORT bool Symtab::getSegments(vector<Segment> &segs) const
{
   segs = segments_;
...
SYMTABEXPORT bool Symtab::getMappedRegions(std::vector<Region *> &mappedRegs) const
{
...
SYMTABEXPORT bool Symtab::updateCode(void *buffer, unsigned size)
{
    Region *sec;
...
SYMTABEXPORT bool Symtab::updateData(void *buffer, unsigned size)
{
    Region *sec;
...
SYMTABEXPORT Offset Symtab::getFreeOffset(unsigned size) 
{
...
SYMTABEXPORT ObjectType Symtab::getObjectType() const 
{
   return object_type_;
}

SYMTABEXPORT char *Symtab::mem_image() const 
{
   return (char *)mf->base_addr();
}

SYMTABEXPORT std::string Symtab::file() const 
{
  assert(mf);
   return mf->pathname();
}

SYMTABEXPORT std::string Symtab::name() const 
{
   return mf->filename();
}

SYMTABEXPORT unsigned Symtab::getNumberofRegions() const 
{
    return no_of_sections; 
}

SYMTABEXPORT unsigned Symtab::getNumberofSymbols() const 
{
...
SYMTABEXPORT LookupInterface::LookupInterface() 
{
}
...
SYMTABEXPORT ExceptionBlock::ExceptionBlock(Offset tStart, 
      unsigned tSize, 
      Offset cStart) 
: tryStart_(tStart), trySize_(tSize), catchStart_(cStart), hasTry_(true) 
{
   ...
   SYMTABEXPORT ExceptionBlock::ExceptionBlock(Offset cStart) 
: tryStart_(0), trySize_(0), catchStart_(cStart), hasTry_(false) 
{
}

SYMTABEXPORT ExceptionBlock::ExceptionBlock(const ExceptionBlock &eb) :
   Serializable(),
...
SYMTABEXPORT bool ExceptionBlock::hasTry() const
{ 
    return hasTry_; 
}

SYMTABEXPORT Offset ExceptionBlock::tryStart() const
{ 
    return tryStart_; 
}

SYMTABEXPORT Offset ExceptionBlock::tryEnd() const
{ 
    return tryStart_ + trySize_; 
}

SYMTABEXPORT Offset ExceptionBlock::trySize() const
{
    return trySize_; 
}

SYMTABEXPORT bool ExceptionBlock::contains(Offset a) const
{ 
...
SYMTABEXPORT relocationEntry::relocationEntry()
   :target_addr_(0), rel_addr_(0), addend_(0), rtype_(Region::RT_REL), name_(""), dynref_(NULL), relType_(0)
{
}   

SYMTABEXPORT relocationEntry::relocationEntry(Offset ta, Offset ra, std::string n, Symbol *dynref, unsigned long relType)
   : target_addr_(ta), rel_addr_(ra), addend_(0), rtype_(Region::RT_REL), name_(n), dynref_(dynref), relType_(relType)
{
}   

SYMTABEXPORT relocationEntry::relocationEntry(Offset ta, Offset ra, Offset add, std::string n, Symbol *dynref, unsigned long relType)
   : target_addr_(ta), rel_addr_(ra), addend_(add), rtype_(Region::RT_REL), name_(n), dynref_(dynref), relType_(relType)
{
}

SYMTABEXPORT relocationEntry::relocationEntry(Offset ra, std::string n, Symbol *dynref, unsigned long relType, Region::RegionType rtype)
   : target_addr_(0), rel_addr_(ra), addend_(0), rtype_(rtype), name_(n), dynref_(dynref), relType_(relType)
{
}   

SYMTABEXPORT const relocationEntry& relocationEntry::operator=(const relocationEntry &ra) 
{
...
SYMTABEXPORT void relocationEntry::setAddend(const Offset value) {
    addend_ = value;
}

SYMTABEXPORT Offset relocationEntry::addend() const {
    return addend_;
}

SYMTABEXPORT void relocationEntry::setRegionType(const Region::RegionType value) {
    rtype_ = value;
}

SYMTABEXPORT Region::RegionType relocationEntry::regionType() const {
    return rtype_;
}
