...
void
Symtab::version(int& major, int& minor, int& maintenance)
{
...
void
symtab_log_perror(const char* msg)
{
...
void
Symtab::setSymtabError(SymtabError new_err)
{
    serr = new_err;
}

std::string
Symtab::printError(SymtabError e)
{
    switch(e)
    {
        case Obj_Parsing:
...
boost::shared_ptr<builtInTypeCollection>&
Symtab::builtInTypes()
{
...
boost::shared_ptr<typeCollection>&
Symtab::stdTypes()
{
...
Symtab::setupBuiltinTypes()
{
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-4, sizeof(long), "long", true));
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-6, 1, "signed char", true));
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-10, sizeof(unsigned long), "unsigned long"));
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-12, sizeof(float), "float", true));
    // -13 double, IEEE double precision
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-13, sizeof(double), "double", true));
    // -14 long double, IEEE double precision, size may increase in future
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-14, sizeof(long double), "long double", true));
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-16, sizeof(bool), "boolean"));
    // -17 short real, IEEE single precision
    //  XXX-size may not be correct jdd 4/22/99
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-17, sizeof(float), "short real", true));
    // -18 real, IEEE double precision XXX-size may not be correct jdd 4/22/99
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-18, sizeof(double), "real", true));
    // -19 stringptr XXX- size of void * -- jdd 4/22/99
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-19, sizeof(void*), "stringptr"));
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-25, sizeof(float) * 2, "complex", true));
    // -26 complex, consists of 2 IEEE double-precision floating point values
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-26, sizeof(double) * 2, "complex*16", true));
    // -27 integer*1, 8 bit signed integral type
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-27, 1, "integer*1", true));
    // -28 integer*2, 16 bit signed integral type
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-28, 2, "integer*2", true));
    ...
    */
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-29, 4, "integer*4", true));
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-31, sizeof(LONGLONG), "long long", true));
    // -32 unsigned long long, 64 bit unsigned integral type
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-32, sizeof(ULONGLONG), "unsigned long long"));
#else
    // -31 long long, 64 bit signed integral type
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-31, sizeof(long long), "long long", true));
    // -32 unsigned long long, 64 bit unsigned integral type
    builtInTypes->addBuiltInType(Type::make_shared<typeScalar>(
        -32, sizeof(unsigned long long), "unsigned long long"));
#endif
    ...
    builtInTypes->addBuiltInType(
        Type::make_shared<typeScalar>(-34, 8, "integer*8", true));

    return builtInTypes;
}

boost::shared_ptr<typeCollection>
Symtab::setupStdTypes()
{
...
SYMTAB_EXPORT unsigned
Symtab::getAddressWidth() const
{
    return address_width_;
}

SYMTAB_EXPORT bool
Symtab::getABIVersion(int& major, int& minor) const
{
    return obj_private->getABIVersion(major, minor);
}

SYMTAB_EXPORT bool
Symtab::isBigEndianDataEncoding() const
{
    return obj_private->isBigEndianDataEncoding();
}

SYMTAB_EXPORT
Symtab::Symtab(MappedFile* mf_)
: AnnotatableSparse()
, member_offset_(0)
, parentArchive_(NULL)
, mf(mf_)
...
SYMTAB_EXPORT bool
Symtab::isExecutable() const
{
    return obj_private->isExecutable();
}

SYMTAB_EXPORT bool
Symtab::isSharedLibrary() const
{
...
SYMTAB_EXPORT Offset
Symtab::preferedBase() const
{
    return preferedBase_;
}

SYMTAB_EXPORT Offset
Symtab::imageOffset() const
{
    return imageOffset_;
}

SYMTAB_EXPORT Offset
Symtab::dataOffset() const
{
    return dataOffset_;
}

SYMTAB_EXPORT Offset
Symtab::dataLength() const
{
    return dataLen_;
}

SYMTAB_EXPORT Offset
Symtab::imageLength() const
{
    return imageLen_;
}

SYMTAB_EXPORT void
Symtab::fixup_code_and_data(Offset newImageOffset, Offset newImageLength,
...
Symtab::getInterpreterName() const
{
...
SYMTAB_EXPORT Offset
Symtab::getEntryOffset() const
{
    return entry_address_;
}

SYMTAB_EXPORT Offset
Symtab::getBaseOffset() const
{
    return base_address_;
}

SYMTAB_EXPORT Offset
Symtab::getLoadOffset() const
{
...
SYMTAB_EXPORT Offset
Symtab::getTOCoffset(Offset off) const
{
    return obj_private->getTOCoffset(off);
}

void
Symtab::setTOCOffset(Offset off)
{
    obj_private->setTOCoffset(off);
    return;
}

SYMTAB_EXPORT string
Symtab::getDefaultNamespacePrefix() const
{
...
bool
Symtab::indexed_symbols::insert(Symbol* s)
{
...
void
Symtab::indexed_symbols::erase(Symbol* s)
{
    if(master.erase(s))
    {
        {
...
bool
Symtab::extractSymbolsFromFile(Object* linkedFile, std::vector<Symbol*>& raw_syms)
{
...
bool
Symtab::fixSymRegion(Symbol* sym)
{
    if(!sym->getRegion())
        return true;

    if(sym->getType() != Symbol::ST_FUNCTION && sym->getType() != Symbol::ST_OBJECT)
        return true;

    if(sym->getRegion()->getMemOffset() <= sym->getOffset() &&
       (sym->getRegion()->getMemOffset() + sym->getRegion()->getMemSize()) >
           sym->getOffset())
        return true;
...
bool
Symtab::fixSymModules(std::vector<Symbol*>& raw_syms)
{
...
bool
Symtab::createIndices(std::vector<Symbol*>& raw_syms, bool undefined)
{
...
bool
Symtab::fixSymModule(Symbol*& sym)
{
    Module* mod = NULL;
...
bool
Symtab::addSymbolToIndices(Symbol*& sym, bool undefined)
{
    assert(sym);
    if(!undefined)
    {
...
bool
Symtab::addSymbolToAggregates(const Symbol* sym_tmp)
{
        ...
        case Symbol::ST_OBJECT: {
            ...
            if(found)
            {
                ...
                if(obj_RelocatableFile == getObjectType() &&
                   (var->getRegion() != sym->getRegion() || NULL == sym->getRegion()))
                {
                    var = new Variable(sym);
                    boost::unique_lock<dyn_rwlock> l(symbols_rwlock);
...
// as functions.
bool
Symtab::doNotAggregate(const Symbol* sym)
{
...
bool
Symtab::updateIndices(Symbol* /*sym*/, std::string /*newName*/, NameType /*nameType*/)
{
#if 0
...
void
Symtab::setModuleLanguages(dyn_hash_map<std::string, supportedLanguages>* mod_langs)
{
    if(!mod_langs->size())
        return;  // cannot do anything here
    //  this case will arise on non-stabs platforms until language parsing can be
...
Module*
Symtab::getOrCreateModule(const std::string& modName, const Offset modAddr)
{
...
Module*
Symtab::newModule(const std::string& name, const Offset addr, supportedLanguages lang)
{
    Module* ret = NULL;
    ...
    create_printf("%s[%d]: In %p: Creating new module '%s'/'%s'\n", FILE__, __LINE__,
                  (void*) this, fileNm.c_str(), fullNm.c_str());
...
, _ref_cnt(1)
{
    ...
    obj_private = new Object(mf, defensive_bin, symtab_log_perror, true, this);
...
Symtab::Symtab(unsigned char* mem_image, size_t image_size, const std::string& name,
...
bool
sort_reg_by_addr(const Region* a, const Region* b)
{
...
static bool
ExceptionBlockCmp(ExceptionBlock* a, ExceptionBlock* b)
{
    return a->catchStart() < b->catchStart();
}

bool
Symtab::extractInfo(Object* linkedFile)
{
    ...
    if(0 == imageLen_ || 0 == linkedFile->code_ptr())
    {
        ...
        {
            if(object_type_ != obj_RelocatableFile || linkedFile->code_ptr() == 0)
            {
...
, _ref_cnt(1)
{
    create_printf("%s[%d]: Creating symtab 0x%p from symtab 0x%p\n", FILE__, __LINE__,
                  (const void*) this, (const void*) &obj);
...
// in the data segment
bool
Symtab::isValidOffset(const Offset where) const
{
 ...
 */
bool
Symtab::isCode(const Offset where) const
{
 ...
 * be kept in sorted order */
bool
Symtab::isData(const Offset where) const
{
...
SYMTAB_EXPORT bool
Symtab::getFuncBindingTable(std::vector<relocationEntry>& fbt) const
{
...
SYMTAB_EXPORT bool
Symtab::updateFuncBindingTable(Offset stub_addr, Offset plt_addr)
{
...
SYMTAB_EXPORT std::vector<std::string>&
              Symtab::getDependencies()
{
    return deps_;
}

SYMTAB_EXPORT Archive*
...
bool Symtab::exportXML(string) { return false; }

bool Symtab::exportBin(string) { return false; }

Symtab* Symtab::importBin(std::string) { return NULL; }

bool
Symtab::openFile(Symtab*& obj, void* mem_image, size_t size, std::string name,
                 def_t def_bin)
{
    bool err = false;
#if defined(TIMED_PARSE)
    ...
    cout << __FILE__ << ":" << __LINE__ << ": openFile " << filename << " took "
         << dursecs << " msecs" << endl;
#endif
    if(!err)
    {
...
Symtab*
Symtab::findOpenSymtab(std::string filename)
{
    unsigned numSymtabs = allSymtabs.size();
    for(unsigned u = 0; u < numSymtabs; u++)
    {
        assert(allSymtabs[u]);
        if(filename == allSymtabs[u]->file() && allSymtabs[u]->mf->canBeShared())
        {
...
bool
Symtab::openFile(Symtab*& obj, std::string filename, def_t def_binary)
{
    bool err = false;
#if defined(TIMED_PARSE)
    struct timeval starttime;
...
bool
Symtab::addRegion(Offset vaddr, void* data, unsigned int dataSize, std::string name,
                  Region::RegionType rType_, bool loadable, unsigned long memAlign,
                  bool tls)
{
    Region*  sec;
    unsigned i;
    if(loadable)
    {
        sec = new Region(newSectionInsertPoint, name, vaddr, dataSize, vaddr, dataSize,
                         (char*) data, Region::RP_R, rType_, true, tls, memAlign);
        sec->setSymtab(this);
    ...
    {
        sec = new Region(regions_.size() + 1, name, vaddr, dataSize, 0, 0, (char*) data,
                         Region::RP_R, rType_, loadable, tls, memAlign);
        sec->setSymtab(this);
...
bool
Symtab::addUserRegion(Region* reg)
{
    std::vector<Region*>* user_regions = NULL;
...
SYMTAB_EXPORT bool
Symtab::getAddressRanges(std::vector<AddressRange>& ranges, std::string lineSource,
                         unsigned int lineNo)
{
...
SYMTAB_EXPORT bool
Symtab::getSourceLines(std::vector<Statement::Ptr>& lines, Offset addressInRange)
{
    unsigned int      originalSize = lines.size();
    std::set<Module*> mods_for_offset;
    findModuleByOffset(mods_for_offset, addressInRange);
    for(auto i = mods_for_offset.begin(); i != mods_for_offset.end(); ++i)
    {
...
SYMTAB_EXPORT bool
Symtab::getSourceLines(std::vector<LineNoTuple>& lines, Offset addressInRange)
{
...
SYMTAB_EXPORT bool
Symtab::addLine(std::string lineSource, unsigned int lineNo, unsigned int lineOffset,
                Offset lowInclAddr, Offset highExclAddr)
{
    ...
    return (lineInfo->addLine(lineSource.c_str(), lineNo, lineOffset, lowInclAddr,
                              highExclAddr));
}

SYMTAB_EXPORT bool
Symtab::addAddressRange(Offset lowInclusiveAddr, Offset highExclusiveAddr,
...
SYMTAB_EXPORT void
Symtab::getAllstdTypes(vector<boost::shared_ptr<Type>>& v)
{
    return stdTypes()->getAllTypes(v);
}

SYMTAB_EXPORT void
Symtab::getAllbuiltInTypes(vector<boost::shared_ptr<Type>>& v)
{
    return builtInTypes()->getAllBuiltInTypes(v);
}

SYMTAB_EXPORT bool
Symtab::findType(boost::shared_ptr<Type>& type, std::string name)
{
    parseTypesNow();
...
SYMTAB_EXPORT boost::shared_ptr<Type>
...
SYMTAB_EXPORT bool
Symtab::findVariableType(boost::shared_ptr<Type>& type, std::string name)
{
    parseTypesNow();
    type = NULL;
    for(auto i = indexed_modules.begin(); i != indexed_modules.end(); ++i)
...
SYMTAB_EXPORT bool
Symtab::findLocalVariable(std::vector<localVar*>& vars, std::string name)
{
    parseTypesNow();
...
SYMTAB_EXPORT bool
Symtab::hasRela() const
{
    return hasRela_;
}

SYMTAB_EXPORT bool
Symtab::hasReldyn() const
{
    return hasReldyn_;
}

SYMTAB_EXPORT bool
Symtab::hasReladyn() const
{
    return hasReladyn_;
}

SYMTAB_EXPORT bool
Symtab::hasRelplt() const
{
    return hasRelplt_;
}

SYMTAB_EXPORT bool
Symtab::hasRelaplt() const
{
    return hasRelaplt_;
}

SYMTAB_EXPORT bool
Symtab::isStaticBinary() const
{
    return isStaticBinary_;
}

bool
Symtab::setDefaultNamespacePrefix(string& str)
{
...
SYMTAB_EXPORT bool
Symtab::emitSymbols(Object* linkedFile, std::string filename, unsigned flag)
{
...
SYMTAB_EXPORT bool
Symtab::emit(std::string filename, unsigned flag)
{
    Object* obj = getObject();
    if(!obj)
    {
...
SYMTAB_EXPORT void
Symtab::addDynLibSubstitution(std::string oldName, std::string newName)
{
    dynLibSubs[oldName] = newName;
}

SYMTAB_EXPORT std::string
              Symtab::getDynLibSubstitution(std::string name)
{
#ifdef BINEDIT_DEBUG
...
SYMTAB_EXPORT bool
Symtab::getSegments(vector<Segment>& segs) const
{
    segs = segments_;
...
SYMTAB_EXPORT bool
Symtab::getMappedRegions(std::vector<Region*>& mappedRegs) const
{
...
SYMTAB_EXPORT bool
Symtab::fixup_RegionAddr(const char* name, Offset memOffset, long memSize)
{
    Region* sec;
...
SYMTAB_EXPORT bool
Symtab::fixup_SymbolAddr(const char* name, Offset newOffset)
{
    Symbol* sym;
    {
        // Find the symbol.
...
SYMTAB_EXPORT bool
Symtab::updateRegion(const char* name, void* buffer, unsigned size)
{
    Region* sec;
...
SYMTAB_EXPORT bool
Symtab::updateData(void* buffer, unsigned size)
{
    return updateRegion(".data", buffer, size);
}

SYMTAB_EXPORT Offset
Symtab::getFreeOffset(unsigned size)
{
    ...
    for(unsigned i = 0; i < regions_.size(); i++)
    {
        ...
        {
            secoffset =
                (char*) (regions_[i]->getPtrToRawData()) - linkedFile->mem_image();
            secoffset += regions_[i]->getMemSize();
        }
...
SYMTAB_EXPORT ObjectType
Symtab::getObjectType() const
{
    return object_type_;
}

SYMTAB_EXPORT Dyninst::Architecture
              Symtab::getArchitecture() const
{
    return getObject()->getArch();
}

SYMTAB_EXPORT char*
Symtab::mem_image() const
{
...
SYMTAB_EXPORT unsigned
Symtab::getNumberOfRegions() const
{
    return no_of_sections;
}

SYMTAB_EXPORT unsigned
Symtab::getNumberOfSymbols() const
{
...
SYMTAB_EXPORT
ExceptionBlock::ExceptionBlock(Offset tStart, unsigned tSize, Offset cStart)
: tryStart_(tStart)
, trySize_(tSize)
, catchStart_(cStart)
, hasTry_(true)
...
SYMTAB_EXPORT
ExceptionBlock::ExceptionBlock(Offset cStart)
: tryStart_(0)
, trySize_(0)
, catchStart_(cStart)
, hasTry_(false)
...
SYMTAB_EXPORT bool
ExceptionBlock::hasTry() const
{
    return hasTry_;
}

SYMTAB_EXPORT Offset
ExceptionBlock::tryStart() const
{
    return tryStart_;
}

SYMTAB_EXPORT Offset
ExceptionBlock::tryEnd() const
{
    return tryStart_ + trySize_;
}

SYMTAB_EXPORT Offset
ExceptionBlock::trySize() const
{
    return trySize_;
}

SYMTAB_EXPORT bool
ExceptionBlock::contains(Offset a) const
{
...
SYMTAB_EXPORT
relocationEntry::relocationEntry(Offset ta, Offset ra, std::string n, Symbol* dynref,
...
SYMTAB_EXPORT
relocationEntry::relocationEntry(Offset ta, Offset ra, Offset add, std::string n,
...
SYMTAB_EXPORT
relocationEntry::relocationEntry(Offset ra, std::string n, Symbol* dynref,
...
SYMTAB_EXPORT Offset
relocationEntry::target_addr() const
{
    return target_addr_;
}

SYMTAB_EXPORT void
relocationEntry::setTargetAddr(const Offset off)
{
    target_addr_ = off;
}

SYMTAB_EXPORT Offset
relocationEntry::rel_addr() const
{
    return rel_addr_;
}

SYMTAB_EXPORT void
relocationEntry::setRelAddr(const Offset value)
{
    rel_addr_ = value;
}

SYMTAB_EXPORT const string&
                    relocationEntry::name() const
{
    return name_;
}

SYMTAB_EXPORT Symbol*
              relocationEntry::getDynSym() const
{
    return dynref_;
}

SYMTAB_EXPORT bool
relocationEntry::addDynSym(Symbol* dynref)
{
    dynref_ = dynref;
    return true;
}

SYMTAB_EXPORT Region::RegionType
              relocationEntry::regionType() const
{
    return rtype_;
}

SYMTAB_EXPORT unsigned long
relocationEntry::getRelType() const
{
    return relType_;
}

SYMTAB_EXPORT Offset
relocationEntry::addend() const
{
    return addend_;
}

SYMTAB_EXPORT void
relocationEntry::setAddend(const Offset value)
{
    addend_ = value;
}

SYMTAB_EXPORT void
relocationEntry::setRegionType(const Region::RegionType value)
{
    rtype_ = value;
}

SYMTAB_EXPORT void
relocationEntry::setName(const std::string& newName)
{
    name_ = newName;
}

bool
relocationEntry::operator==(const relocationEntry& r) const
{
    if(target_addr_ != r.target_addr_)
        return false;
    if(rel_addr_ != r.rel_addr_)
        return false;
    if(addend_ != r.addend_)
        return false;
    if(rtype_ != r.rtype_)
        return false;
    if(name_ != r.name_)
        return false;
    if(relType_ != r.relType_)
        return false;
    if(dynref_ && !r.dynref_)
        return false;
    if(!dynref_ && r.dynref_)
        return false;
    if(dynref_)
    {
        if(dynref_->getMangledName() != r.dynref_->getMangledName())
            return false;
        if(dynref_->getOffset() != r.dynref_->getOffset())
            return false;
    }
...
ostream&
Dyninst::SymtabAPI::operator<<(ostream& os, const relocationEntry& r)
{
    if(r.getDynSym() != NULL)
    {
    ...
    os << " Offset: " << std::hex << std::setfill('0') << setw(8) << r.rel_addr()
       << std::dec << std::setfill(' ') << " Offset: " << std::hex << std::setfill('0')
       << setw(8) << r.target_addr() << std::dec << std::setfill(' ')
       << " Addend: " << r.addend()
       << " Region: " << Region::regionType2Str(r.regionType()) << " Type: " << setw(15)
       << relocationEntry::relType2Str(r.getRelType()) << "(" << r.getRelType() << ")";
    if(r.getDynSym() != NULL)
    {
        os << " Symbol Offset: " << std::hex << std::setfill('0') << setw(8)
           << r.getDynSym()->getOffset();
...
const char*
Symbol::symbolType2Str(SymbolType t)
{
    switch(t)
    {
...
const char*
Symbol::symbolLinkage2Str(SymbolLinkage t)
{
    switch(t)
    {
...
const char*
Symbol::symbolTag2Str(SymbolTag t)
{
    switch(t)
    {
...
const char*
Symbol::symbolVisibility2Str(SymbolVisibility t)
{
    switch(t)
    {
...
bool
Symtab::getRegValueAtFrame(Address pc, Dyninst::MachRegister reg,
...
SYMTAB_EXPORT bool
Symtab::addLibraryPrereq(std::string name)
{
    Object* obj = getObject();
    if(!obj)
    {
    ...
    symtab->getObject()->addReference((*fit)->getOffset(), name, funcName);
...
SYMTAB_EXPORT bool
Symtab::addSysVDynamic(long name, long value)
{
#if defined(os_linux) || defined(os_freebsd)
    Object* obj = getObject();
    if(!obj)
...
SYMTAB_EXPORT bool
Symtab::addExternalSymbolReference(Symbol* externalSym, Region* localRegion,
                                   relocationEntry localRel)
{
    ...
    Symbol* symRef =
        new Symbol(externalSym->getMangledName(), externalSym->getType(),
                   Symbol::SL_GLOBAL, Symbol::SV_DEFAULT, (Address) 0, getDefaultModule(),
                   NULL,  // localRegion,
                   externalSym->getSize(), true, false);

    if(!addSymbol(symRef, externalSym))
        return false;
...
SYMTAB_EXPORT bool
Symtab::addTrapHeader_win(Address ptr)
{
#if defined(os_windows)
...
bool
Symtab::getExplicitSymtabRefs(std::set<Symtab*>& refs)
{
...
void
Symtab::getSegmentsSymReader(std::vector<SymSegment>& segs)
{
#if !defined(os_windows)
