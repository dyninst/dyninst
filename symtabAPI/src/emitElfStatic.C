 ...
 */
char*
emitElfStatic::linkStatic(Symtab* target, StaticLinkError& err, string& errMsg)
{
    /*
     * Basic algorithm is:
    ...
    {
        if(lmap.allocatedData)
            delete[] lmap.allocatedData;
        return NULL;
    }
    ...
    for(symOff_it = lmap.origSymbols.begin(); symOff_it != lmap.origSymbols.end();
        ++symOff_it)
    {
    ...
    for(relSym_it = lmap.origRels.begin(); relSym_it != lmap.origRels.end(); ++relSym_it)
    {
 ...
 */
bool
emitElfStatic::resolveSymbols(Symtab* target, vector<Symtab*>& relocatableObjects,
                              LinkMap& lmap, StaticLinkError& err, string& errMsg)
{
    ...
    for(expObj_it = explicitRefs.begin(); expObj_it != explicitRefs.end(); ++expObj_it)
    {
    ...
    while(curObjFilePtr != workSet.end())
    {
            ...
            ++undefSym_it)
        {
            ...
            if(excludeSymNames.count(curUndefSym->getPrettyName()))
                continue;
            ...
            if(extSymbol == NULL)
            {
                    ...
                    ++iter)
                {
                    ...
                    if(syms.empty())
                    {
                        ...
                        err    = Symbol_Resolution_Failure;
                        errMsg = "inconsistency found between archive's symbol table and "
 ...
 */
bool
emitElfStatic::createLinkMap(Symtab* target, vector<Symtab*>& relocatableObjects,
                             ...
                             string& errMsg)
{
                ...
                if(target->isStaticBinary() && (regionName.compare(".toc") == 0))
                {
                    ...
                    for(Offset entry = 0; entry < numSymbols; entry++)
                    {
                        ...
                        if(!relExist)
                        {
                            ...
                            // Offset and name is all we care - offset should be value of
                            // the symbol
                            Offset soffset =
                                *((unsigned long*) (rawRegionData + entry * 8));
                            Symbol* newsym = new Symbol(
                                "dyntoc_entry", Symbol::ST_UNKNOWN, Symbol::SL_UNKNOWN,
                                Symbol::SV_UNKNOWN, soffset);
    ...
    if(maxAlign < lmap.codeRegionAlign)
        maxAlign = lmap.codeRegionAlign;
    ...
    lmap.relGotRegionOffset +=
        computePadding(globalOffset + lmap.relGotRegionOffset, lmap.relGotRegionAlign);
    currentOffset =
        allocateRelGOTSection(lmap.pltEntries, lmap.relGotRegionOffset, lmap.relGotSize);
    ...
    if(lmap.gotSize > 0)
    {
        ...
        calculateTOCs(target, lmap.gotRegions, lmap.gotRegionOffset, gotLayoutOffset,
                      globalOffset);
    }
    ...
    lmap.stubRegionOffset +=
        computePadding(globalOffset + lmap.stubRegionOffset, lmap.codeRegionAlign);
    ...
    lmap.codeRegionOffset +=
        computePadding(globalOffset + lmap.codeRegionOffset, lmap.codeRegionAlign);
    ...
    lmap.dataRegionOffset +=
        computePadding(globalOffset + lmap.dataRegionOffset, lmap.dataRegionAlign);
    ...
    if(lmap.tlsRegions.size() > 0)
    {
        ...
        lmap.tlsRegionOffset +=
            computePadding(lmap.tlsRegionOffset + globalOffset, lmap.tlsRegionAlign);
    ...
    if(lmap.newCtorRegions.size() > 0)
    {
        ...
        lmap.ctorRegionOffset +=
            computePadding(globalOffset + lmap.ctorRegionOffset, lmap.ctorRegionAlign);
    ...
    if(lmap.newDtorRegions.size() > 0)
    {
        ...
        lmap.dtorRegionOffset +=
            computePadding(globalOffset + lmap.dtorRegionOffset, lmap.dtorRegionAlign);
    ...
    if(commonAlignments.size() > 0)
    {
        ...
        for(align_it = commonAlignments.begin(); align_it != commonAlignments.end();
            ++align_it)
        {
 ...
 */
Offset
emitElfStatic::layoutRegions(deque<Region*>&                   regions,
 ...
 */
bool
emitElfStatic::addNewRegions(Symtab* target, Offset globalOffset, LinkMap& lmap)
{
    ...
    if(lmap.gotSize > 0)
    {
        ...
        target->addRegion(globalOffset + lmap.gotRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.gotSize), GOT_NAME,
                          Region::RT_DATA, true, lmap.gotRegionAlign);
    }
#endif
    if(lmap.pltSize > 0)
    {
        target->addRegion(globalOffset + lmap.pltRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.pltSize), PLT_NAME,
                          Region::RT_TEXTDATA, true, lmap.pltRegionAlign);
    }
    ...
    if(lmap.relGotSize > 0)
    {
        target->addRegion(
            ...
            static_cast<unsigned int>(lmap.relGotSize), REL_GOT_NAME, Region::RT_TEXTDATA,
            true, lmap.relGotRegionAlign);
    }
    ...
    if(lmap.dataSize > 0)
    {
        target->addRegion(globalOffset + lmap.dataRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.dataSize), DATA_NAME,
                          Region::RT_DATA, true, lmap.dataRegionAlign);
    }
    ...
    if(lmap.tlsSize > 0)
    {
        target->addRegion(globalOffset + lmap.tlsRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.tlsSize), TLS_DATA_NAME,
                          Region::RT_DATA, true, lmap.tlsRegionAlign, true);
    }
#endif
    if(lmap.newCtorRegions.size() > 0)
    {
        ...
        target->addRegion(globalOffset + lmap.ctorRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.ctorSize), CTOR_NAME,
                          Region::RT_DATA, true, lmap.ctorRegionAlign);
    }
    ...
    if(lmap.newDtorRegions.size() > 0)
    {
        ...
        target->addRegion(globalOffset + lmap.dtorRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.dtorSize), DTOR_NAME,
                          Region::RT_DATA, true, lmap.dtorRegionAlign);
    }
    ...
    if(lmap.bssSize > 0)
    {
        target->addRegion(globalOffset + lmap.bssRegionOffset,
                          ...
                          static_cast<unsigned int>(lmap.bssSize), BSS_NAME,
                          Region::RT_BSS, true, lmap.bssRegionAlign);
    }
 ...
 */
void
emitElfStatic::copyRegions(LinkMap& lmap)
{
    char* targetData = lmap.allocatedData;

    map<Region*, LinkMap::AllocPair>::iterator regOff_it;
    for(regOff_it = lmap.regionAllocs.begin(); regOff_it != lmap.regionAllocs.end();
        ++regOff_it)
    {
 ...
 */
inline Offset
emitElfStatic::computePadding(Offset candidateOffset, Offset alignment)
{
    Offset padding = 0;
 ...
 */
bool
emitElfStatic::applyRelocations(Symtab* target, vector<Symtab*>& relocatableObjects,
                                Offset globalOffset, LinkMap& lmap, StaticLinkError& err,
                                string& errMsg)
{
    // Iterate over all relocations in all relocatable files
    vector<Symtab*>::iterator depObj_it;
    for(depObj_it = relocatableObjects.begin(); depObj_it != relocatableObjects.end();
        ++depObj_it)
    {
        vector<Region*> allRegions;
        ...
        for(region_it = allRegions.begin(); region_it != allRegions.end(); ++region_it)
        {
            ...
            if(regionName.compare(0, 4, ".toc") == 0)
                isTOC = true;
            ...
            if(result != lmap.regionAllocs.end())
            {
                ...
                for(rel_it = region_rels.begin(); rel_it != region_rels.end(); ++rel_it)
                {
                    ...
                    rewrite_printf("Computing relocations to apply to region: %s (%s) @ "
                                   "0x%lx reloffset 0x%lx dest 0x%lx  \n\n",
                                   (*region_it)->getRegionName().c_str(),
                                   (*region_it)->symtab()->file().c_str(), regionOffset,
                                   relOffset, dest);
                    rewrite_printf("\t RelOffset computed as region 0x%lx + rel_addr "
                                   "0x%lx + globalOffset 0x%lx\n",
                    ...
                    /*
                       ...
                       region: %s @ 0x%lx reloffset %d 0x%lx dest %d 0x%lx  \n\n",
                                            (*region_it)->getRegionName().c_str(),
                       regOffset, relOffset,relOffset, dest,dest); if(
                       !archSpecificRelocation(target, *depObj_it, targetData, *rel_it,
 ...
 */
string
emitElfStatic::printStaticLinkError(StaticLinkError err)
{
    switch(err)
    {
 ...
 */
bool
emitElfStatic::hasRewrittenTLS() const
{
...
/* See architecture specific functions that call these for descriptions of function
 * interface */

Offset
emitElfStatic::tlsLayoutVariant1(Offset globalOffset, Region* dataTLS, Region* bssTLS,
                                 LinkMap& lmap)
{
    ...
    deque<Region*>::iterator copyReg_it;
    for(copyReg_it = lmap.tlsRegions.begin(); copyReg_it != lmap.tlsRegions.end();
        ++copyReg_it)
    {
...
Offset
emitElfStatic::tlsLayoutVariant2(Offset globalOffset, Region* dataTLS, Region* bssTLS,
                                 LinkMap& lmap)
{
    // The original init. image needs to remain in the image 1 slot because
    // the TLS data references are relative to that position
    ...
    if(dataTLS != NULL)
        lmap.tlsRegions.push_back(dataTLS);
    if(bssTLS != NULL)
        tlsBssSize = bssTLS->getMemSize();
...
Offset
emitElfStatic::tlsAdjustVariant2(Offset curOffset, Offset tlsSize)
{
...
void
emitElfStatic::tlsCleanupVariant1(map<Region*, LinkMap::AllocPair>& regionAllocs, Region*,
                                  Region*                           bssTLS)
{
    // Existing BSS TLS region is not copied to the new target data
    if(bssTLS != NULL)
        regionAllocs.erase(bssTLS);
}

void
emitElfStatic::tlsCleanupVariant2(map<Region*, LinkMap::AllocPair>& regionAllocs, Region*,
                                  Region*                           bssTLS)
{
    // Existing BSS TLS region is not copied to the new target data
    if(bssTLS != NULL)
...
bool
emitElfUtils::updateHeapVariables(Symtab* obj, unsigned long newSecsEndAddress)
{
    ...
    for(heapSymsIter = heapSyms.begin(); heapSymsIter != heapSyms.end(); ++heapSymsIter)
    {
        Region* symRegion = (*heapSymsIter)->getRegion();
        Offset  symOffset = (*heapSymsIter)->getOffset();
        if(symOffset < symRegion->getDiskOffset())
            continue;
...
bool
emitElfStatic::calculateTOCs(Symtab* target, deque<Region*>& regions, Offset GOTstart,
                             Offset newStart, Offset globalOffset)
{
    rewrite_printf(
        "Calculating TOCs for merged GOT sections, base is 0x%lx, new regions at 0x%lx\n",
    ...
    for(deque<Region*>::iterator iter = regions.begin(); iter != regions.end(); ++iter)
    {
        ...
        // We'd like the TOC to be GOTbase + 0x8000, so long as that can address the end
        // of the current region.
        if((currentTOC + 0x7ff0) < end)
        {
            // Crud...
            // OTOH, we can't reference anything outside the current GOT, so just rebase
            // it
...
Offset
emitElfStatic::allocatePLTEntries(
    std::map<Symbol*, std::pair<Offset, Offset>>& pltEntries, Offset pltOffset,
    Offset& size)
{
    // For each Symtab
...
bool
emitElfStatic::addIndirectSymbol(Symbol* sym, LinkMap& lmap)
{
...
bool
emitElfStatic::buildPLT(Symtab* target, Offset globalOffset, LinkMap& lmap,
...
Offset
emitElfStatic::allocateRelocationSection(
...
Offset
emitElfStatic::allocateRelGOTSection(
    const std::map<Symbol*, std::pair<Offset, Offset>>& entries, Offset relocOffset,
    Offset& size)
{
#if defined(arch_x86_64)
...
bool
emitElfStatic::buildRela(Symtab* target, Offset globalOffset, LinkMap& lmap,
                         StaticLinkError& err, string& errMsg)
{
    (void) err;
    (void) errMsg;  // unused
