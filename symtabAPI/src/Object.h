...
class AObject {
 public:
  ...
  SYMTAB_EXPORT bool get_symbols(std::string &name,
  ...
  SYMTAB_EXPORT bool getAllExceptions(
      std::vector<ExceptionBlock *> &excpBlocks) const;
  SYMTAB_EXPORT std::vector<Region *> getAllRegions() const;

  SYMTAB_EXPORT supportedLanguages
  pickLanguage(std::string &working_module, char *working_options,
  ...
  SYMTAB_EXPORT virtual bool get_func_binding_table(
      std::vector<relocationEntry> &) const;
  SYMTAB_EXPORT virtual bool get_func_binding_table_ptr(
  ...
  SYMTAB_EXPORT dyn_hash_map<std::string, std::vector<Symbol *> >
      *getAllSymbols();

  SYMTAB_EXPORT virtual bool hasFrameDebugInfo() { return false; }
  SYMTAB_EXPORT virtual bool getRegValueAtFrame(
      Address /*pc*/, Dyninst::MachRegister /*reg*/,
  ...
  SYMTAB_EXPORT virtual bool getABIVersion(int & /*major*/,
                                           int & /*minor*/) const {
    return false;
  }
 ...
 protected:
  ...
  std::vector<ExceptionBlock>
      catch_addrs_;  // Addresses of C++ try/catch blocks;
