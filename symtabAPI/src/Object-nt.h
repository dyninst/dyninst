...
class Object : public AObject {
 ...
 public:
  class intSymbol {
   ...
   public:
    intSymbol(std::string _name, DWORD64 _addr, DWORD _type, DWORD _linkage,
              DWORD _size, Region *_region)
        : name(_name),
          addr(_addr),
          type(_type),
          linkage(_linkage),
  ...
  class File {
   ...
   public:
    ...
    void AddSymbol(intSymbol *pSym) { syms.push_back(pSym); }
 ...
 public:
  ...
  SYMTAB_EXPORT bool getCatchBlock(ExceptionBlock &b, Offset addr,
  ...
  SYMTAB_EXPORT void getModuleLanguageInfo(
      dyn_hash_map<std::string, supportedLanguages> *mod_langs);
  SYMTAB_EXPORT bool emitDriver(std::string fName,
  ...
  SYMTAB_EXPORT std::map<std::string, std::map<Offset, std::string> >
      &getRefs() {
    return ref;
  }

  std::vector<std::pair<std::string, IMAGE_IMPORT_DESCRIPTOR> >
 ...
 private:
  ...
  SYMTAB_EXPORT void parseLineInfoForAddr(Offset) { parseFileLineInfo(); }
  ...
  Offset preferedBase;  // Virtual address at which the binary is prefered to be
                        // loaded
  Offset imageBase;     // Virtual Address at which the binary is loaded in its
                        // address space
  ...
  std::vector<Offset> possible_mains;  // Addresses of functions that may be
                                       // main
};
