...
bool addSymID(SerializerBase *, Symbol *, Address) { return false; }

Symbol *getSymForID(SerializerBase *, Address) { return NULL; }

Symbol *Symbol::magicEmitElfSymbol() {
  // I have no idea why this is the way it is,
  // but emitElf needs it...
  return new Symbol("", ST_NOTYPE, SL_LOCAL, SV_DEFAULT, 0, NULL, NULL, 0,
                    false, false);
}

SYMTAB_EXPORT string Symbol::getMangledName() const { return mangledName_; }
...
#endif
  // Assume not native (ie GNU) if we don't have an associated Symtab for some
  // reason
...
SYMTAB_EXPORT Function *Symbol::getFunction() const {
  if (aggregate_ == NULL) return NULL;
...
SYMTAB_EXPORT Symbol::SymbolTag Symbol::tag() const { return tag_; }
...
SYMTAB_EXPORT bool Symbol::setSymbolType(SymbolType sType) {
  ...
  type_ = sType;
  if (module_ && module_->exec()) module_->exec()->changeType(this, oldType);
...
Serializable *Symbol::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}

void Symbol::restore_module_and_region(SerializerBase *, std::string &, Offset)
    THROW_SPEC(SerializerError) {}

std::ostream &Dyninst::SymtabAPI::operator<<(ostream &os, const Symbol &s) {
  return os << "{"
            << " mangled=" << s.getMangledName()
            << " pretty=" << s.getPrettyName() << " module=" << s.module_
            //<< " type="    << (unsigned) s.type_
            << " type=" << s.symbolType2Str(s.type_)
            //<< " linkage=" << (unsigned) s.linkage_
            << " linkage=" << s.symbolLinkage2Str(s.linkage_) << " offset=0x"
            << hex << s.offset_ << dec << " size=0x" << hex << s.size_ << dec
            << " ptr_offset=0x" << hex << s.ptr_offset_ << dec << " localTOC=0x"
            << hex << s.localTOC_ << dec
            //<< " tag="     << (unsigned) s.tag_
...
bool Symbol::operator==(const Symbol &s) const {
  ...
  if (region_ && !s.region_) return false;
  if (region_) {
    if (region_->getDiskOffset() != s.region_->getDiskOffset()) return false;
  }
  ...
  if (module_ && !s.module_) return false;
  if (module_) {
    if (module_->fullName() != s.module_->fullName()) return false;
  }

  return (
      (type_ == s.type_) && (linkage_ == s.linkage_) &&
      (offset_ == s.offset_) && (size_ == s.size_) &&
      (isDynamic_ == s.isDynamic_) && (isAbsolute_ == s.isAbsolute_) &&
      (isDebug_ == s.isDebug_) && (isCommonStorage_ == s.isCommonStorage_) &&
      (versionHidden_ == s.versionHidden_) && (mangledName_ == s.mangledName_));
  //			&& (prettyName_ == s.prettyName_)
  //	&& (typedName_ == s.typedName_));
}

Symtab *Symbol::getSymtab() const { return module_ ? module_->exec() : NULL; }
...
Symbol::Symbol(const std::string &name, SymbolType t, SymbolLinkage l,
               SymbolVisibility v, Offset o, Module *module, Region *r,
               unsigned s, bool d, bool a, int index, int strindex, bool cs)
    : module_(module),
      type_(t),
      internal_type_(0),
      linkage_(l),
...
Symbol *Symbol::getReferringSymbol() { return referring_; }
