...
static inline void
ompc_leftmost(Module*& out, Module*& in)
{
    out = out == NULL ? out : in;
}
#pragma omp declare \
...
DwarfWalker::parse()
{
    ...
    for(Dwarf_Off cu_off = 0;
        dwarf_next_unit(dbg(), cu_off, &next_cu_header, &cu_header_length, NULL,
                        &abbrev_offset, &addr_size, &offset_size, &type_signaturep,
                        NULL) == 0;
    ...
    for(Dwarf_Off cu_off = 0;
        dwarf_nextcu(dbg(), cu_off, &next_cu_header, &cu_header_length, &abbrev_offset,
                     &addr_size, &offset_size) == 0;
        cu_off = next_cu_header)
    {
...
#pragma omp parallel
        {
            DwarfWalker w(symtab(), dbg());
#pragma omp for reduction(leftmost : fixUnknownMod) schedule(dynamic) nowait
            for(unsigned int i = 0; i < module_dies.size(); i++)
            {
                w.push();
                w.parseModule(module_dies[i], fixUnknownMod);
    ...
    for(; variableIter != moduleTypes->globalVarsByName.end(); variableIter++)
    {
        ...
        {
            variableIter->second =
                moduleTypes->findType(variableIter->second->getID(), Type::share);
...
bool
DwarfWalker::parseModule(Dwarf_Die moduleDIE, Module*& fixUnknownMod)
{
    /* Make sure we've got the right one. */
    Dwarf_Half moduleTag;
    moduleTag = dwarf_tag(&moduleDIE);

    if(moduleTag != DW_TAG_compile_unit && moduleTag != DW_TAG_partial_unit &&
       moduleTag != DW_TAG_type_unit)
        return false;

    /* Extract the name of this module. */
    std::string moduleName;
    if(!findDieName(moduleDIE, moduleName))
        return false;
...
void
DwarfParseActions::setModuleFromName(std::string moduleName)
{
...
bool
DwarfWalker::buildSrcFiles(::Dwarf* /*dbg*/, Dwarf_Die entry, StringTablePtr srcFiles)
{
    size_t       cnt = 0;
    Dwarf_Files* df;
...
DwarfWalker::parse_int(Dwarf_Die e, bool parseSib, bool dissociate_context)
{
    ...
    while(1)
    {
        ...
        switch(dwarf_tag(&e))
        {
            ...
            default:
                dwarf_printf(
                    "(0x%lx) Warning: unparsed entry with tag 0x%x, dwarf_tag(): 0x%x\n",
...
bool
DwarfWalker::setFunctionFromRange(inline_t func_type)
{
    ...
    if(!set_lowest)
    {
        ...
        dwarf_printf(
            "(0x%lx) setFunctionFromRange has ranges, but no lowest, returning false\n",
            id());
        return false;
    }
...
void
DwarfWalker::setFuncFromLowest(Address lowest)
{
    Function* f      = NULL;
...
DwarfWalker::createInlineFunc()
{
    ...
    else
    {
        ...
        dwarf_printf(
            "(0x%lx) setFunctionFromRange found inline without parent, returning false\n",
            id());
        return false;
    }
}
...
void
DwarfParseActions::addMangledFuncName(std::string name)
{
    curFunc()->addMangledName(name, true, true);
}
void
DwarfParseActions::addPrettyFuncName(std::string name)
{
    curFunc()->addPrettyName(name, true, true);
}

bool
DwarfWalker::parseSubprogram(DwarfWalker::inline_t func_type)
{
    bool name_result;
    ...
    //    if(func) cout << hex << "Begin parseSubprogram for (" << id() << ") " <<
    //    func->getName() << " at " << func->getOffset() << dec << endl;
    ...
    {
        dwarf_printf("(0x%lx) parseSubprogram not parsing children b/c curFunc() not in "
                     "parsedFuncs\n",
                     id());
        if(name_result)
        {
    ...
    if(name_result && !curName().empty())
    {
        ...
        // Only keep pretty names around for inlines, which probably don't have mangled
        // names
        else
        {
    ...
    if(func_type == InlinedFunc)
    {
        //       cout << "Parsing callsite for (0x" << hex << id() << ") " << curName() <<
        //       " at " << func->getOffset() << dec << endl;
        parseCallsite();
    }
...
void
DwarfWalker::setRanges(FunctionBase* func)
{
...
pair<AddressRange, bool>
DwarfWalker::parseHighPCLowPC(Dwarf* /*dbg*/, Dwarf_Die entry)
{
    Dwarf_Addr low, high;
    int        low_result  = dwarf_lowpc(&entry, &low);
    int        high_result = dwarf_highpc(&entry, &high);
    bool       ok = (low_result == 0) && (high_result == 0) && (low != 0) && (high != 0);
...
bool
DwarfWalker::parseRangeTypes(Dwarf* dbg, Dwarf_Die die)
{
    ...
    for(auto r = newRanges.begin(); r != newRanges.end(); ++r)
    {
        setRange(*r);
    }
...
vector<AddressRange>
DwarfWalker::getDieRanges(Dwarf* /*dbg*/, Dwarf_Die die, Offset /*range_base*/)
{
    ...
    while(1)
    {
        ...
        if(offset < 0)
            return std::vector<AddressRange>();
        if(offset == 0)
            break;
        dwarf_printf("Lexical block from 0x%lx to 0x%lx\n", start, end);
...
Symbol*
DwarfWalker::findSymbolForCommonBlock(const string& commonBlockName)
{
    return findSymbolByName(commonBlockName, Symbol::ST_OBJECT);
}

boost::shared_ptr<Type>
DwarfWalker::getCommonBlockType(string& commonBlockName)
{
...
bool
DwarfWalker::addStaticClassVariable(const vector<VariableLocation>& locs,
                                    boost::shared_ptr<Type>         type)
{
...
void
DwarfWalker::createGlobalVariable(const vector<VariableLocation>& locs,
                                  boost::shared_ptr<Type>         type)
{
...
void
DwarfWalker::createLocalVariable(const vector<VariableLocation>& locs,
                                 boost::shared_ptr<Type> type, Dwarf_Word variableLineNo,
                                 const string& fileName)
{
    localVar* newVariable =
        new localVar(curName(), type, fileName, (int) variableLineNo, curFunc());
    dwarf_printf("(0x%lx) Created localVariable '%s' (%p), currentFunction %p\n", id(),
                 curName().c_str(), (void*) newVariable, (void*) curFunc());

    for(unsigned int i = 0; i < locs.size(); ++i)
    {
        dwarf_printf(
            "(0x%lx) (%s) Adding location %u of %d: (0x%lx - 0x%lx): %s, %s, %s, %ld\n",
            id(), newVariable->getName().c_str(), i + 1, (int) locs.size(), locs[i].lowPC,
            locs[i].hiPC, storageClass2Str(locs[i].stClass),
            storageRefClass2Str(locs[i].refClass), locs[i].mr_reg.name().c_str(),
            locs[i].frameOffset);
...
DwarfWalker::parseFormalParam()
{
    ...
    bool        hasLineNumber = false;
    std::string fileName;
    if(!getLineInformation(lineNo, hasLineNumber, fileName))
        return false;
...
void
DwarfWalker::createParameter(const vector<VariableLocation>& locs,
                             boost::shared_ptr<Type> paramType, Dwarf_Word lineNo,
                             const string& fileName)
{
    localVar* newParameter =
        new localVar(curName(), paramType, fileName, (int) lineNo, curFunc());
    dwarf_printf(
        "(0x%lx) Creating new formal parameter %s/%p (%s) (%p)\n", id(),
        curName().c_str(), (void*) paramType.get(), paramType->getName().c_str(),
    ...
    /* This is just brutally ugly.  Why don't we take care of this invariant
     * automatically? */
...
DwarfWalker::parseBaseType()
{
    ...
    const bool is_floating_point =
        is_float || is_complex_float || is_imaginary_float || is_decimal_float;
    const bool is_string = is_UTF;

    typeScalar::properties_t p = { is_integral,      is_floating_point,
                                   is_string,        is_address,
                                   is_boolean,       is_complex_float,
                                   is_float,         is_imaginary_float,
                                   is_decimal_float, is_signed,
                                   is_signed_char,   is_unsigned,
                                   is_unsigned_char, is_UTF };
    ...
    dwarf_printf(
        "(0x%lx) Created type %p / %s (pre add %p / %s) for id %d, size %u, in TC %p\n",
        id(), (void*) baseTy.get(), baseTy->getName().c_str(), (void*) debug,
        debug->getName().c_str(), (int) offset(), size, (void*) tc());

    return true;
}

bool
DwarfWalker::parseTypedef()
{
    dwarf_printf("(0x%lx) parseTypedef entry\n", id());

    boost::shared_ptr<Type> referencedType;
    if(!findType(referencedType, true))
        return false;
    ...
    if(!nameDefined())
    {
        if(!fixName(curName(), referencedType))
            return false;
    }

    auto typeDef = tc()->addOrUpdateType(
        Type::make_shared<typeTypedef>(type_id(), referencedType, curName()));
    dwarf_printf("(0x%lx) Created type %p / %s for type_id %d, offset 0x%lx, size %u, in "
                 "TC %p, mod:%s\n",
                 id(), (void*) typeDef.get(), typeDef->getName().c_str(), type_id(),
...
DwarfWalker::parseArray()
{
    ...
    */
    ...
    boost::shared_ptr<Type> baseType =
        parseMultiDimensionalArray(&firstRange, elementType);
    ...
    dwarf_printf("(0x%lx) Creating array with base type %s, low bound %lu, high bound "
                 "%lu, named %s\n",
                 id(), baseTypeName.c_str(), baseArrayType.getLow(),
                 baseArrayType.getHigh(), curName().c_str());
    tc()->addOrUpdateType(Type::make_shared<typeArray>(
        type_id(), baseArrayType.getBaseType(Type::share), baseArrayType.getLow(),
        baseArrayType.getHigh(), baseTypeName + "[]"));
...
DwarfWalker::parseEnum()
{
    ...
    if(!findName(curName()))
        return false;
    // setEnum(tc()->addOrUpdateType( Type::make_shared<typeEnum>( type_id(), "enum " +
    // curName())));
...
DwarfWalker::parseInheritance()
{
    ...
    dwarf_printf("(0x%lx) Added type %p as %s to %p\n", id(), (void*) superClass.get(),
                 fName.c_str(), (void*) curEnclosure().get());
    return true;
}

bool
DwarfWalker::parseStructUnionClass()
{
    if(!tc())
        return false;
    ...
    {
        dwarf_printf(
            "WARNING: parseStructUnionClass called on non-aggregate tagged entry\n");
        return false;
    }
    ...
    if(!nameDefined())
    {
        ...
        bool        hasLineNumber = false;
        std::string fileName;
        if(!getLineInformation(lineNumber, hasLineNumber, fileName))
            return false;
        stringstream ss;
        ss << "{anonymous ";
        switch(tag())
    ...
    switch(tag())
    {
        ...
        case DW_TAG_class_type: {
            ...
            dwarf_printf("(0x%lx) Created type %p / %s for type_id %d, offset 0x%lx, "
                         "size %u, in TC %p, mod:%s\n",
                         id(), (void*) containingType.get(),
                         containingType->getName().c_str(), type_id(), offset(),
                         containingType->getSize(), (void*) tc(),
                         mod()->fullName().c_str());
            break;
        }
        case DW_TAG_union_type: {
        ...
        default: {
            Dwarf_Die e = entry();
            dwarf_printf(
                "(0x%lx) Warning: type not created tag 0x%x, dwarf_tag(): 0x%x\n", id(),
                tag(), (unsigned int) dwarf_tag(&e));
        }
    }
    setEnclosure(containingType);
    dwarf_printf("(0x%lx) Started class, union, or struct: %p\n", id(),
                 (void*) containingType.get());
    return true;
}

bool
DwarfWalker::parseEnumEntry()
{
...
DwarfWalker::parseMember()
{
    ...
    if(!decodeLocationList(DW_AT_data_member_location, &initialStackValue, locs))
        return false;
    if(locs.empty())
    {
        if(curEnclosure()->getUnionType())
        {
            ...
            if(!constructConstantVariableLocation((Address) 0, locs))
                return false;
        }
        else
        {
    ...
    if(!fixBitFields(locs, memberSize))
        return false;
    ...
    {
        curEnclosure()->asFieldListType().addField("[anonymous union]", memberType,
                                                   offset_to_use);
    }
    return true;
}
...
DwarfWalker::parseTypeReferences()
{
    ...
    {
        case DW_TAG_subroutine_type:
            indirectType = tc()->addOrUpdateType(
                Type::make_shared<typeFunction>(type_id(), typePointedTo, curName()));
            break;
        case DW_TAG_ptr_to_member_type:
        case DW_TAG_pointer_type:
            indirectType = tc()->addOrUpdateType(
                Type::make_shared<typePointer>(type_id(), typePointedTo, curName()));
            dwarf_printf("(0x%lx) Created type %p / %s for type_id %d, offset 0x%lx, "
                         "size %u, in TC %p, mod:%s\n",
                         id(), (void*) indirectType.get(),
                         indirectType->getName().c_str(), type_id(), offset(),
                         indirectType->getSize(), (void*) tc(),
                         mod()->fullName().c_str());
            break;
        case DW_TAG_reference_type:
            indirectType = tc()->addOrUpdateType(
                Type::make_shared<typeRef>(type_id(), typePointedTo, curName()));
            dwarf_printf("(0x%lx) Created type %p / %s for type_id %d, offset 0x%lx, "
                         "size %u, in TC %p\n",
                         id(), (void*) indirectType.get(),
                         indirectType->getName().c_str(), type_id(), offset(),
        ...
        default:
            dwarf_printf(
                "(0x%lx) Warning: nothing done for tag 0x%x, dwarf_tag(): 0x%x\n", id(),
                tag(), (unsigned int) dwarf_tag(&e));
            return false;
    }
...
bool
DwarfWalker::hasDeclaration(bool& isDecl)
{
    Dwarf_Die e = entry();
...
bool
DwarfWalker::handleAbstractOrigin(bool& isAbstract)
{
    Dwarf_Die absE, e;
    e = entry();
...
bool
DwarfWalker::handleSpecification(bool& hasSpec)
{
    Dwarf_Die specE, e;
    e = entry();
...
bool
DwarfWalker::findDieName(Dwarf_Die die, std::string& name)
{
...
bool
DwarfWalker::findName(std::string& name)
{
    if(!findDieName(specEntry(), name))
        return false;
...
std::vector<VariableLocation>&
DwarfParseActions::getFramePtrRefForInit()
{
...
bool
DwarfWalker::getReturnType(bool hasSpecification, boost::shared_ptr<Type>& returnType)
{
...
bool
DwarfWalker::addFuncToContainer(boost::shared_ptr<Type> returnType)
{
    ...
    curEnclosure()->asFieldListType().addField(
        demangledName,
        Type::make_shared<typeFunction>(type_id(), returnType, demangledName));

    return true;
}

bool
DwarfWalker::isStaticStructMember(std::vector<VariableLocation>& locs, bool& isStatic)
{
    isStatic = false;
...
bool
DwarfWalker::findType(boost::shared_ptr<Type>& type, bool defaultToVoid)
{
    if(!tc())
        return false;
...
bool
DwarfWalker::findDieOffset(Dwarf_Attribute attr, Dwarf_Off& offset)
{
...
bool
DwarfWalker::findAnyType(Dwarf_Attribute typeAttribute, bool is_info,
                         boost::shared_ptr<Type>& type)
{
    ...
    if(!findDieOffset(typeAttribute, typeOffset))
        return false;
    ...
    dwarf_printf("(0x%lx) type offset 0x%lx, is_supplemental=%s\n", id(),
                 (unsigned long) typeOffset, is_sup ? "true" : "false");
    ...
    dwarf_printf("(0x%lx) type pointer %p / name:%s, type_id %d, tc():%p, mod: %s, "
                 "specificType:%s\n",
                 id(), (void*) type.get(), type->getName().c_str(), type_id, (void*) tc(),
                 mod()->fullName().c_str(), type->specificType().c_str());

    return true;
}

bool
DwarfWalker::getLineInformation(Dwarf_Word& variableLineNo, bool& hasLineNumber,
                                std::string& fileName)
{
...
bool
DwarfWalker::decodeLocationList(Dwarf_Half attr, Address* initialStackValue,
                                std::vector<VariableLocation>& locs)
{
    Dwarf_Die e = entry();
    ...
    else
    {
        ...
        do
        {
            offset = dwarf_getlocations(&locationAttribute, offset, &basep, &start, &end,
                                        &exprs, &exprlen);
            if(offset == -1)
            {
        ...
        if(!decodeLocationListForStaticOffsetOrAddress(locDescs, locDescs.size(), locs,
                                                       initialStackValue))
        {
            return false;
        }
...
bool
DwarfWalker::checkForConstantOrExpr(Dwarf_Half /*attr*/,
                                    Dwarf_Attribute& locationAttribute, bool& constant,
                                    bool& expr, Dwarf_Half& form)
{
    constant = false;
...
bool
DwarfWalker::findString(Dwarf_Half attr, string& str)
{
    Dwarf_Half      form;
    ...
    if(attr == DW_AT_call_file || attr == DW_AT_decl_file)
    {
        ...
        {
            dwarf_printf(
                "Dwarf error reading line index %lu from srcFiles(%p) of size %lu\n",
                line_index, (void*) strs.get(), strs->size());
            return false;
        }
        //       cout << "findString found " << (*srcFiles())[line_index].str << " at
        //       srcFiles[" << line_index << "] for " << mod()->fileName() << endl;
...
bool
DwarfWalker::findConstant(Dwarf_Half attr, Address& value, Dwarf_Die* entry,
                          Dwarf* /*dbg*/)
{
...
bool
DwarfWalker::findConstantWithForm(Dwarf_Attribute& locationAttribute, Dwarf_Half form,
                                  Address& value)
{
    value = 0;
...
bool
DwarfWalker::decodeConstantLocation(Dwarf_Attribute& attr, Dwarf_Half form,
                                    std::vector<VariableLocation>& locs)
{
    Address value;
    if(!findConstantWithForm(attr, form, value))
        return false;
    if(!constructConstantVariableLocation(value, locs))
        return false;
    return true;
}

bool
DwarfWalker::constructConstantVariableLocation(Address                        value,
                                               std::vector<VariableLocation>& locs)
{
...
bool
DwarfWalker::findSize(unsigned& size)
{
    bool      hasSize;
    Dwarf_Die e = entry();
    if(!e.addr)
...
bool
DwarfWalker::findVisibility(visibility_t& visibility)
{
...
bool
DwarfWalker::findValue(long& value, bool& valid)
{
...
bool
DwarfWalker::fixBitFields(std::vector<VariableLocation>& locs, long& size)
{
...
bool
DwarfWalker::fixName(std::string& name, boost::shared_ptr<Type> type)
{
    switch(tag())
    {
        case DW_TAG_const_type:
...
void
DwarfWalker::removeFortranUnderscore(std::string& name)
{
...
bool
DwarfWalker::parseSubrangeAUX(Dwarf_Die entry, std::string& loBound, std::string& hiBound)
{
    ...
    if(status != 0)
    {
        if(!decipherBound(lowerBoundAttribute, is_info, loBound))
            return false;
    ...
    if(status != 0)
    {
        if(!decipherBound(upperBoundAttribute, is_info, hiBound))
            return false;
    ...
    dwarf_printf("(0x%lx) Adding subrange type: id %d, low %lu, high %lu, named %s\n",
                 id(), type_id, low_conv, hi_conv, curName().c_str());
                                        ...
                                        boost::shared_ptr<Type> elementType)
{
    ...
    if(status == 1)
    {
        ...
        auto        innermostType = tc()->addOrUpdateType(Type::make_shared<typeArray>(
            elementType, atoi(loBound.c_str()), atoi(hiBound.c_str()), aName));
        /* dwarf_printf("\t(0x%lx)parseMultiDimentionalArray status 1, typ %p,
         * innermosttype %p, lower bound %d, upper bound %d\n", id(), typ, innermostType,
         * innermostType->asArrayType().getLow(), innermostType->asArrayType().getHigh());
         */
        return innermostType;
    } /* end base-case of recursion. */

    /* If it does, build this array type out of the array type returned from the next
     * recusion. */
    boost::shared_ptr<Type> innerType =
        parseMultiDimensionalArray(&nextSibling, elementType);
    if(!innerType)
    {
        dwarf_printf(
            "\tparseMultiDimensionalArray return Null because innerType == NULL\n");
        return NULL;
    }
    // same here - type id ignored    jmo
    std::string aName     = buf;
    auto        outerType = tc()->addOrUpdateType(Type::make_shared<typeArray>(
        innerType, atoi(loBound.c_str()), atoi(hiBound.c_str()), aName));
    dwarf_printf("\t(0x%lx)parseMultiDimentionalArray status 0, lower bound %lu, upper "
                 "bound %lu\n",
                 id(), outerType->asArrayType().getLow(),
                 outerType->asArrayType().getHigh());
    return outerType;
} /* end parseMultiDimensionalArray() */

bool
DwarfWalker::decipherBound(Dwarf_Attribute boundAttribute, bool /*is_info*/,
                           std::string&    boundString)
{
    ...
    switch(boundForm)
    {
        ...
        case DW_FORM_ref_udata: {
            ...
            /* Does it have a name? */
            if(findDieName(boundEntry, boundString) && !boundString.empty())
                return true;

            /* Does it describe a nameless constant? */
            Dwarf_Attribute constBoundAttribute;
            auto            status =
                dwarf_attr(&boundEntry, DW_AT_const_value, &constBoundAttribute);
        ...
        case DW_FORM_block:
        case DW_FORM_block1: {
            /* PGI extends DWARF to allow some bounds to be location lists.  Since we
               can't do anything sane with them, ignore them. */
...
bool
DwarfWalker::decodeExpression(Dwarf_Attribute&               locationAttribute,
                              std::vector<VariableLocation>& locs)
{
    Dwarf_Op* exprs   = NULL;
    size_t    exprlen = 0;
...
bool
DwarfWalker::decodeLocationListForStaticOffsetOrAddress(
    std::vector<LocDesc>& locationList, Dwarf_Sword listLength,
    std::vector<VariableLocation>& locs, Address* initialStackValue)
{
    locs.clear();
     ...
     * function as the frame pointer is readily available on all platforms(except for
     * IA64) May be we would need to parse the location list for IA64 functions to store
     * the register numbers and offsets and use it based on the pc value.
     */
    ...
    for(unsigned locIndex = 0; locIndex < listLength; locIndex++)
    {
        ...
        //
        dwarf_printf("(0x%lx) Decoding entry %u of %d over range 0x%lx - 0x%lx, mod "
                     "0x%lx - 0x%lx, base 0x%lx\n",
                     id(), locIndex + 1, (int) listLength, location->ld_lopc,
                     location->ld_hipc, modLow, modHigh, base);
        ...
        if(location->ld_lopc == 0 && location->ld_hipc == (Dwarf_Addr) ~0)
        {
...
void
DwarfWalker::deallocateLocationList(Dwarf_Op** /*locationList*/, Dwarf_Sword listLength)
{
...
void
DwarfWalker::setEntry(Dwarf_Die entry)
{
...
void
DwarfParseActions::push(bool dissociate_context)
{
...
void
DwarfParseActions::setFunc(FunctionBase* f)
{
...
typeId_t
DwarfWalker::get_type_id(Dwarf_Off offset, bool is_info, bool is_sup)
{
    auto&    type_ids = is_info ? info_type_ids_ : types_type_ids_;
    typeId_t type_id  = 0;
    {
        type_map::const_accessor a;
        if(type_ids.find(a, { offset, is_sup, mod() }))
            type_id = a->second;
    }
    if(type_id)
        return type_id;
    ...
    unsigned int val = getNextTypeId();
    {
        ...
        dwarf_printf("(0x%lx) type_id %u, key created {0x%lx,%s,mod: %s}\n", id(), val,
                     offset, is_sup ? "sup" : "not sup", mod()->fullName().c_str());
    }
...
bool
DwarfWalker::parseModuleSig8(bool is_info)
{
    /* Obtain the type DIE. */
...
bool
DwarfWalker::findSig8Type(Dwarf_Sig8* s, boost::shared_ptr<Type>& returnType)
{
    uint64_t sig8    = *reinterpret_cast<uint64_t*>(s);
    typeId_t type_id = 0;
    {
        dyn_c_hash_map<uint64_t, typeId_t>::const_accessor a;
        if(sig8_type_ids_.find(a, sig8))
            type_id = a->second;
    }
...
Object*
DwarfParseActions::obj() const
{
    return symtab()->getObject();
}

Offset
DwarfParseActions::convertDebugOffset(Offset from)
{
    Offset to;
