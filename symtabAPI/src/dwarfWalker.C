...
DwarfWalker::DwarfWalker(Symtab *symtab, ::Dwarf *dbg, std::shared_ptr<ParsedFuncs> pf, std::shared_ptr<DieOffsetToCalled> d) :
...
bool DwarfWalker::parse() {
    ...
    if (dwarf_getalt(dbg()) != NULL) {
        DwarfWalker w(symtab(), dbg(), parsedFuncs, dieOffsetToCalled);
