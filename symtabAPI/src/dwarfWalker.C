...
bool DwarfWalker::parse() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    while (dwarf_next_cu_header_c(dbg(), is_info, &cu_header_length, &version,
  ...
  if (!fixUnknownMod) return true;

  dwarf_printf("Fixing types for final module %s\n",
               fixUnknownMod->fileName().c_str());

  /* Fix type list. */
  typeCollection *moduleTypes =
      typeCollection::getModTypeCollection(fixUnknownMod);
  assert(moduleTypes);
  ...
  dyn_hash_map<std::string, Type *>::iterator variableIter =
      moduleTypes->globalVarsByName.begin();
...
bool DwarfWalker::parseModule(Dwarf_Bool is_info, Module *&fixUnknownMod) {
  ...
  DWARF_ERROR_RET(
      dwarf_attr(moduleDIE, DW_AT_language, &languageAttribute, NULL));
  ...
  if (!fixUnknownMod) fixUnknownMod = mod();
...
bool DwarfWalker::buildSrcFiles(Dwarf_Debug dbg, Dwarf_Die entry,
                                StringTablePtr srcFiles) {
  Dwarf_Signed cnt = 0;
  char **srcFileList;
...
bool DwarfWalker::parse_int(Dwarf_Die e, bool p) {
  ...
  while (1) {
    ...
    dwarf_printf(
        "(0x%lx) Parsing entry %p with context size %d, func %p, encl %p\n",
        id(), e, stack_size(), curFunc(),
        //                   (curFunc() &&
        //                   !curFunc()->getAllMangledNames().empty()) ?
...
// BLUEGENE BUG HACK
#if defined(os_bg)
    if (tag() == DW_TAG_base_type || tag() == DW_TAG_const_type ||
        tag() == DW_TAG_pointer_type) {
      // XLC compilers nest a bunch of stuff under an invented function;
      // however,
    ...
    dwarf_printf(
        "Finished parsing 0x%lx, ret %d, parseChild %d, parseSibling %d\n",
    ...
    int status =
        dwarf_siblingof_b(dbg(), entry(), is_info, &siblingDwarf, NULL);
...
bool DwarfWalker::setFunctionFromRange(inline_t func_type) {
  ...
  if (!hasRanges()) {
    dwarf_printf(
        "(0x%lx) setFunctionFromRange has no ranges, returning false\n", id());
  ...
  for (range_set_t::iterator i = ranges_begin(); i != ranges_end(); i++) {
    ...
    if (lowest > i->first) lowest = i->first;
  }
  if (!set_lowest) {
    // No ranges.  Don't panic, this is probably an abstract origin or
    // specification
    // we'll get to it latter if it's really used.
    dwarf_printf(
        "(0x%lx) setFunctionFromRange has ranges, but no lowest, returning "
        "false\n",
        id());
    return false;
  }
...
bool DwarfWalker::createInlineFunc() {
  ...
  if (parent) {
    ...
    setFunc(ifunc);
    //         cout << "Created new inline, parent is " << parent->getName() <<
    //         endl;
    return true;
  } else {
    // InlinedSubroutine without containing subprogram.  Weird.
    dwarf_printf(
        "(0x%lx) setFunctionFromRange found inline without parent, returning "
        "false\n",
        id());
    return false;
  }
...
void restore(int old) { common_debug_dwarf = old; }
bool DwarfWalker::parseSubprogram(DwarfWalker::inline_t func_type) {
  ...
  if (curEnclosure() && !func) {
    ...
    dwarf_printf(
        "(0x%lx) parseSubprogram not parsing member function's children\n",
        id());
  ...
  if (!func) {
    dwarf_printf(
        "(0x%lx) parseSubprogram not parsing children b/c curFunc() NULL\n",
        id());
    setParseChild(false);
    return true;
  }
  ...
  if (parsedFuncs.find(func) != parsedFuncs.end()) {
    dwarf_printf(
        "(0x%lx) parseSubprogram not parsing children b/c curFunc() not in "
        "parsedFuncs\n",
        id());
    if (name_result) {
  ...
  if (name_result && !curName().empty()) {
    ...
    // Only keep pretty names around for inlines, which probably don't have
    // mangled names
    else {
      //          printf("(0x%lx) Adding %s as pretty name to inline at
  ...
  if (func_type == InlinedFunc) {
    //       cout << "Parsing callsite for (0x" << hex << id() << ") " <<
    //       curName() << " at " << func->getOffset() << dec << endl;
    parseCallsite();
  }
  ...
  // Dwarf outlines some function information. You have the base entry, which
  // contains
  ...
  if (func_type == InlinedFunc) {
    //        cout << "End parseSubprogram for inlined func " << curName() << "
    //        at " << func->getOffset() << endl;
  }
...
void DwarfWalker::setRanges(FunctionBase *func) {
  if (func->ranges.empty()) {
    ...
    for (auto i = ranges_begin(); i != ranges_end(); i++) {
      ...
      if (last_low == low && last_high == high) continue;
      last_low = low;
      last_high = high;
...
pair<AddressRange, bool> DwarfWalker::parseHighPCLowPC(Dwarf_Debug dbg,
                                                       Dwarf_Die entry) {
  Dwarf_Attribute hasLow;

  Dwarf_Attribute hasHigh;
  std::pair<AddressRange, bool> result = make_pair(AddressRange(0, 0), false);
  if (dwarf_attr(entry, DW_AT_low_pc, &hasLow, NULL) != DW_DLV_OK)
    return result;
  if (dwarf_attr(entry, DW_AT_high_pc, &hasHigh, NULL) != DW_DLV_OK)
    return result;
...
vector<AddressRange> DwarfWalker::getDieRanges(Dwarf_Debug dbg, Dwarf_Die die,
                                               Offset range_base) {
  ...
  if (findConstant(DW_AT_ranges, range_offset, die, dbg)) {
    ...
    int status = (dwarf_get_ranges_a(dbg, (Dwarf_Off)range_offset, die, &ranges,
                                     &ranges_length, NULL, NULL));
...
typeCommon *DwarfWalker::getCommonBlockType(string &commonBlockName) {
  ...
  commonBlockType =
      dynamic_cast<typeCommon *>(tc()->findVariableType(commonBlockName));
...
bool DwarfWalker::addStaticClassVariable(const vector<VariableLocation> &locs,
                                         Type *type) {
...
void DwarfWalker::createGlobalVariable(const vector<VariableLocation> &locs,
                                       Type *type) {
  ...
  Offset addr = 0;
  if (locs.size() && locs[0].stClass == storageAddr) addr = locs[0].frameOffset;
  Variable *var;
...
void DwarfWalker::createLocalVariable(const vector<VariableLocation> &locs,
  ...
  dwarf_printf("(0x%lx) localVariable '%s' (%p), currentFunction %p\n", id(),
               curName().c_str(), newVariable, curFunc());

  for (unsigned int i = 0; i < locs.size(); ++i) {
    dwarf_printf(
        "(0x%lx) (%s) Adding location %d of %d: (0x%lx - 0x%lx): %s, %s, %s, "
        "%ld\n",
        id(), newVariable->getName().c_str(), i + 1, (int)locs.size(),
        locs[i].lowPC, locs[i].hiPC, storageClass2Str(locs[i].stClass),
...
void DwarfWalker::createParameter(const vector<VariableLocation> &locs,
  ...
  dwarf_printf("(0x%lx) Creating new formal parameter %s/%p (%s) (%p)\n", id(),
               curName().c_str(), paramType, paramType->getName().c_str(),
               //                ((curFunc() &&
               //                !curFunc()->getAllMangledNames().empty()) ?
...
bool DwarfWalker::parseBaseType() {
  ...
  typeScalar *baseType =
      new typeScalar(type_id(), (unsigned int)size, curName());
  ...
  dwarf_printf(
      "(0x%lx) Created type %p / %s (pre add %p / %s) for id %d, size %d, in "
      "TC %p\n",
      id(), baseType, baseType->getName().c_str(), debug,
      debug->getName().c_str(), (int)offset(), size, tc());
...
bool DwarfWalker::parseTypedef() {
  ...
  if (tc()) {
    typeTypedef *typedefType =
        new typeTypedef(type_id(), referencedType, curName());
...
bool DwarfWalker::parseArray() {
  ...
  /* Two words about pgf90 arrays.
     ...
     Secundus: we ignore DW_AT_PGI_lbase, DW_AT_PGI_loffset, and
     DW_AT_PGI_lstride,
     even though libdwarf recognizes them, because our type modelling doesn't
     allow
     us to make use of this information.  Similarly, in virtually every place
     where
     the Portland Group extends DWARF to allow _form_block attributes encoding
     location
     lists, we ignore them.  We should, however, recognize these cases and
     ignore them
  ...
  */
  ...
  dwarf_printf(
      "(0x%lx) Creating array with base type %s, low bound %ld, high bound "
      "%ld, named %s\n",
      id(), baseArrayType->getBaseType()->getName().c_str(),
      baseArrayType->getLow(), baseArrayType->getHigh(), curName().c_str());
  typeArray *arrayType = new typeArray(type_id(), baseArrayType->getBaseType(),
                                       baseArrayType->getLow(),
                                       baseArrayType->getHigh(), nameToUse);
...
bool DwarfWalker::parseEnum() {
  ...
  enumerationType =
      dynamic_cast<typeEnum *>(tc()->addOrUpdateType(enumerationType));
...
bool DwarfWalker::parseStructUnionClass() {
  ...
  assert(tag() == DW_TAG_structure_type || tag() == DW_TAG_union_type ||
...
bool DwarfWalker::hasDeclaration(bool &isDecl) {
  Dwarf_Bool tmp;
  DWARF_FAIL_RET(dwarf_hasattr(entry(), DW_AT_declaration, &tmp, NULL));
  isDecl = tmp;
  return true;
}
...
bool DwarfWalker::handleAbstractOrigin(bool &isAbstract) {
  ...
  DWARF_FAIL_RET(
      dwarf_hasattr(entry(), DW_AT_abstract_origin, &isAbstractOrigin, NULL));
  ...
  DWARF_FAIL_RET(
      dwarf_attr(entry(), DW_AT_abstract_origin, &abstractAttribute, NULL));
...
bool DwarfWalker::handleSpecification(bool &hasSpec) {
  ...
  DWARF_FAIL_RET(
      dwarf_hasattr(entry(), DW_AT_specification, &hasSpecification, NULL));
  ...
  DWARF_FAIL_RET(
      dwarf_attr(entry(), DW_AT_specification, &specAttribute, NULL));
...
bool DwarfWalker::findDieName(Dwarf_Debug dbg, Dwarf_Die die,
                              std::string &name) {
  char *cname = NULL;
...
bool DwarfWalker::findFuncName() {
  ...
  int status =
      dwarf_attr(entry(), DW_AT_MIPS_linkage_name, &linkageNameAttr, NULL);
  if (status != DW_DLV_OK)
...
bool DwarfWalker::addFuncToContainer(Type *returnType) {
  ...
  if (!demangledName) {
    ...
    // Strip everything left of the rightmost ':' off to get rid of the class
    // names
    toUse = demangledName;
...
bool DwarfWalker::findAnyType(Dwarf_Attribute typeAttribute, Dwarf_Bool is_info,
                              Type *&type) {
...
bool DwarfWalker::getLineInformation(Dwarf_Unsigned &variableLineNo,
  ...
  int status =
      dwarf_attr(specEntry(), DW_AT_decl_file, &fileDeclAttribute, NULL);
  DWARF_CHECK_RET(status == DW_DLV_ERROR);
  ...
  if (status == DW_DLV_NO_ENTRY) {
    ...
    if (fileNameDeclVal > srcFiles()->size() || fileNameDeclVal <= 0) {
      dwarf_printf(
          "Dwarf error reading line index %d from srcFiles of size %lu\n",
...
bool DwarfWalker::decodeLocationList(Dwarf_Half attr,
  ...
  Dwarf_Half form;
  if (!checkForConstantOrExpr(attr, locationAttribute, isConstant, isExpr,
                              form))
    return false;
  ...
  if (isConstant) {
    ...
    int status =
        dwarf_loclist_n(locationAttribute, &locationList, &listLength, NULL);
    ...
    if (!decodeLocationListForStaticOffsetOrAddress(locationList, listLength,
...
bool DwarfWalker::checkForConstantOrExpr(Dwarf_Half attr,
  ...
  dwarf_printf(
      "(0x%lx) Checking for constant, formtype is 0x%x looking for 0x%x\n",
...
bool DwarfWalker::findString(Dwarf_Half attr, string &str) {
  Dwarf_Half form;
  ...
  if (attr == DW_AT_call_file || attr == DW_AT_decl_file) {
    ...
    //       cout << "findString found " << (*srcFiles())[line_index].str << "
    //       at srcFiles[" << line_index << "] for " << mod()->fileName() <<
    //       endl;
  ...
  switch (form) {
    case DW_FORM_string: {
      ...
      //          cout << "findString found " << s << " in DW_FORM_string" <<
      //          endl;
      str = s;
      result = true;
      break;
    ...
    case DW_FORM_block4: {
      ...
      //          cout << "findString found " << str << " in DW_FORM_block" <<
      //          endl;
      result = !str.empty();
      break;
    }
...
bool DwarfWalker::findConstant(Dwarf_Half attr, Address &value, Dwarf_Die entry,
                               Dwarf_Debug dbg) {
...
bool DwarfWalker::constructConstantVariableLocation(
    Address value, std::vector<VariableLocation> &locs) {
...
bool DwarfWalker::fixBitFields(std::vector<VariableLocation> &locs,
  ...
  if (status == DW_DLV_OK && locs.size()) {
    ...
    if (locs.size()) locs[0].frameOffset *= 8;
    size *= 8;
...
bool DwarfWalker::parseSubrangeAUX(Dwarf_Die entry, std::string &loBound,
                                   std::string &hiBound) {
  ...
  dwarf_printf(
      "(0x%lx) Adding subrange type: id %d, low %ld, high %ld, named %s\n",
      id(), type_id, low_conv, hi_conv, curName().c_str());
  typeSubrange *rangeType =
      new typeSubrange(type_id, 0, low_conv, hi_conv, curName());
...
typeArray *DwarfWalker::parseMultiDimensionalArray(Dwarf_Die range,
  ...
  if (status == DW_DLV_NO_ENTRY) {
    /* Terminate the recursion by building an array type out of the elemental
       type.
       Use the negative dieOffset to avoid conflicts with the range type created
       by parseSubRangeDIE(). */
    // N.B.  I'm going to ignore the type id, and just create an anonymous type
    // here
    std::string aName = buf;
    typeArray *innermostType = new typeArray(elementType, atoi(loBound.c_str()),
                                             atoi(hiBound.c_str()), aName);
  ...
  /* If it does, build this array type out of the array type returned from the
   * next recusion. */
  typeArray *innerType = parseMultiDimensionalArray(nextSibling, elementType);
  assert(innerType != NULL);
  // same here - type id ignored    jmo
  std::string aName = buf;
  typeArray *outerType = new typeArray(innerType, atoi(loBound.c_str()),
                                       atoi(hiBound.c_str()), aName);
...
bool DwarfWalker::decipherBound(Dwarf_Attribute boundAttribute,
                                Dwarf_Bool is_info, std::string &boundString) {
  ...
  switch (boundForm) {
    ...
    case DW_FORM_ref_udata: {
      ...
      DWARF_FAIL_RET(
          dwarf_offdie_b(dbg(), boundOffset, is_info, &boundEntry, NULL));

      /* Does it have a name? */
      if (findDieName(dbg(), boundEntry, boundString) && !boundString.empty())
        return true;

      /* Does it describe a nameless constant? */
      Dwarf_Attribute constBoundAttribute;
      int status =
          dwarf_attr(boundEntry, DW_AT_const_value, &constBoundAttribute, NULL);
      DWARF_CHECK_RET(status == DW_DLV_ERROR);

      if (status == DW_DLV_OK) {
        Dwarf_Unsigned constBoundValue;
        DWARF_FAIL_RET(
            dwarf_formudata(constBoundAttribute, &constBoundValue, NULL));
    ...
    case DW_FORM_block1: {
      ...
      // Dwarf_Locdesc * locationList;
      // Dwarf_Signed listLength;
      // status = dwarf_loclist( boundAttribute, & locationList, & listLength,
      // NULL );
...
bool DwarfWalker::decodeExpression(Dwarf_Attribute &attr,
  ...
  // expr_ptr is a pointer to a bytestream. Try to turn it into a Dwarf_Locdesc
  // so
  ...
  bool ret =
      decodeLocationListForStaticOffsetOrAddress(&descs, cnt, locs, NULL);
...
bool DwarfWalker::decodeLocationListForStaticOffsetOrAddress(
  ...
  when we encounter a function with a DW_AT_frame_base (effectively all of
  them),
  we do NOT use this decoder; we decode the location into an AST, which we
  will use to calculate the frame pointer when asked to do frame-relative
  operations.
  ...
  */
  ...
  /* We now parse the complete location list for variables and parameters within
   * a
   * function. We still ignore the location list defined for DW_AT_frame_base of
   * the
   * function as the frame pointer is readily available on all platforms(except
   * for IA64)
   * May be we would need to parse the location list for IA64 functions to store
   * the
  ...
  for (unsigned locIndex = 0; locIndex < listLength; locIndex++) {
    ...
    //
    dwarf_printf(
        "(0x%lx) Decoding entry %d of %d over range 0x%lx - 0x%lx, mod 0x%lx - "
        "0x%lx, base 0x%lx\n",
        id(), locIndex + 1, (int)listLength, (long)location->ld_lopc,
        (long)location->ld_hipc, modLow, modHigh, base);

    if (location->ld_lopc == max_addr) {
      // This is a base address selection entry, which changes the base address
      // of
      // subsequent entries
    ...
    bool result =
        decodeDwarfExpression(location, tmp, loc, symtab()->getArchitecture());
    if (!result) {
...
bool DwarfWalker::parseModuleSig8(Dwarf_Bool is_info) {
  ...
  if (typeTag != DW_TAG_type_unit) return false;
...
Object *DwarfParseActions::obj() const { return symtab()->getObject(); }
...
void DwarfWalker::setFuncReturnType() {
  ...
  if (!curFunc()->getReturnType()) {
    ...
    if (returnType) curFunc()->setReturnType(returnType);
  }
}
