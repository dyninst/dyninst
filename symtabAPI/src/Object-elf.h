...
namespace SymtabAPI {
/*
 * The standard symbol table in an elf file is the .symtab section. This section
 * does
 * not have information to find the module to which a global symbol belongs, so
 * we must
...
struct stab32 {
  ...
  unsigned short desc;
  unsigned int
      val;  // value of this symbol -- usually zero. The real value must
...
class Object : public AObject {
 ...
 public:
  Object(MappedFile *, bool, void (*)(const char *) = log_msg,
         bool alloc_syms = true, Symtab *st = NULL);
  virtual ~Object();

  bool emitDriver(std::string fName, std::vector<Symbol *> &allSymbols,
                  unsigned flag);

  const char *elf_vaddr_to_ptr(Offset vaddr) const;
  bool hasStabInfo() const {
    return !(!stab_off_ || !stab_size_ || !stabstr_off_);
  }
  ...
  bool get_func_binding_table_ptr(
  ...
  bool isText(Offset addr) const {
    if (addr >= text_addr_ && addr <= text_addr_ + text_size_) return true;
    return false;
  }
  ...
  std::vector<std::vector<Offset> > getMoveSecAddrRange() const {
    return moveSecAddrRange;
  };
  dyn_hash_map<int, Region *> getTagRegionMapping() const {
    return secTagRegionMapping;
  }
  ...
  SYMTAB_EXPORT virtual void getSegmentsSymReader(
      std::vector<SymSegment> &segs);
 ...
 private:
  ...
  bool loaded_elf(Offset &, Offset &, Elf_X_Shdr *&, Elf_X_Shdr *&,
                  Elf_X_Shdr *&, Elf_X_Shdr *&, Elf_X_Shdr *&, Elf_X_Shdr *&,
                  Elf_X_Shdr *&, Elf_X_Shdr *&rel_plt_scnp,
                  Elf_X_Shdr *&plt_scnp, Elf_X_Shdr *&got_scnp,
                  Elf_X_Shdr *&dynsym_scnp, Elf_X_Shdr *&dynstr_scnp,
                  Elf_X_Shdr *&dynamic_scnp, Elf_X_Shdr *&eh_frame,
                  Elf_X_Shdr *&gcc_except, Elf_X_Shdr *&interp_scnp,
                  Elf_X_Shdr *&opd_scnp, bool a_out = false);
 ...
 private:
  ...
  bool get_relocationDyn_entries(unsigned rel_index, Elf_X_Shdr *&dynsym_scnp,
                                 Elf_X_Shdr *&dynstr_scnp);

  // Parses sections with relocations and links these relocations to
  // existing symbols
  bool parse_all_relocations(Elf_X &, Elf_X_Shdr *, Elf_X_Shdr *, Elf_X_Shdr *,
                             Elf_X_Shdr *);
