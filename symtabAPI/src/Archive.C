...
std::string
Archive::printError(SymtabError err)
{
    switch(err)
    {
        case Obj_Parsing:
...
bool
Archive::openArchive(Archive*& img, std::string filename)
{
    bool err = false;
...
bool
Archive::openArchive(Archive*& img, char* mem_image, size_t size)
{
    bool err;
    // Has the archive already been opened?
    std::vector<Archive*>::iterator ar_it;
...
bool
Archive::getMember(Symtab*& img, string& member_name)
{
...
bool
Archive::getMemberByOffset(Symtab*& img, Offset memberOffset)
{
...
bool
Archive::getMemberByGlobalSymbol(Symtab*& img, string& symbol_name)
{
...
bool
Archive::getAllMembers(vector<Symtab*>& members)
{
    dyn_hash_map<string, ArchiveMember*>::iterator mem_it;
...
{
    if(membersByName.count(member_name))
        return true;
    return false;
}
