...
SymtabReader::SymtabReader(std::string file_)
    : symtab(NULL), ref_count(1), ownsSymtab(true) {
  // We'd throw, but...
...
SymtabReader::SymtabReader(Symtab *s)
    : symtab(s), ref_count(1), ownsSymtab(false) {}

SymtabReader::~SymtabReader() {
  if (symtab && ownsSymtab) Symtab::closeSymtab(symtab);
  symtab = NULL;
}
...
Dyninst::Offset SymtabReader::imageOffset() { return symtab->imageOffset(); }

Dyninst::Offset SymtabReader::dataOffset() { return symtab->dataOffset(); }
...
bool SymtabReader::isValidSection(Section_t sec) { return (sec.v1 != NULL); }
...
SymbolReaderFactory *Dyninst::SymtabAPI::getSymtabReaderFactory() {
  ...
  if (!fact) fact = new SymtabReaderFactory();
