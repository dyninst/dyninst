...
, ownsSymtab(true)
{
    ...
    Symtab::openFile(symtab, const_cast<char*>(buffer), size, memName.str());
}
...
#define DEFN_SYMBOL_T(name)                                                              \
    Symbol_t name;                                                                       \
    name.v1 = name.v2 = NULL;                                                            \
    name.i1 = name.i2 = 0
...
bool
SymtabReader::getABIVersion(int& major, int& minor) const
{
    assert(symtab);
...
bool
SymtabReader::isBigEndianDataEncoding() const
{
    assert(symtab);
...
Architecture
SymtabReader::getArchitecture() const
{
...
bool
SymtabReader::getSegment(unsigned num, SymSegment& seg)
{
    buildSegments();
...
Dyninst::Offset
SymtabReader::getSymbolOffset(const Symbol_t& sym)
{
    assert(sym.v2);
...
Dyninst::Offset
SymtabReader::getSymbolTOC(const Symbol_t& sym)
{
    assert(sym.v2);
    Symbol* symbol = (Symbol*) sym.v2;
...
std::string
SymtabReader::getSymbolName(const Symbol_t& sym)
{
    assert(sym.v2);
    Symbol* symbol = (Symbol*) sym.v2;
...
std::string
SymtabReader::getDemangledName(const Symbol_t& sym)
{
    assert(sym.v2);
    Symbol* symbol = (Symbol*) sym.v2;
...
unsigned long
SymtabReader::getSymbolSize(const Symbol_t& sym)
{
    assert(sym.v2);
    Symbol* symbol = (Symbol*) sym.v2;
...
bool
SymtabReader::isValidSymbol(const Symbol_t& sym)
{
...
Dyninst::Address
SymtabReader::getSectionAddress(Section_t sec)
{
...
std::string
SymtabReader::getSectionName(Section_t sec)
{
    Region* region = (Region*) sec.v1;
    assert(region);
...
bool
SymtabReader::isValidSection(Section_t sec)
{
    return (sec.v1 != NULL);
}
...
SymReader*
SymtabReaderFactory::openSymbolReader(const char* buffer, unsigned long size)
{
...
bool
SymtabReaderFactory::closeSymbolReader(SymReader* sr)
{
...
SymbolReaderFactory*
Dyninst::SymtabAPI::getSymtabReaderFactory()
{
