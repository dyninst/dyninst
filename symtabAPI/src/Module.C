...
std::string Statement::getFile() const {
  if (strings_) {
    if (file_index_ < strings_->size()) {
      // can't be ->[] on shared pointer to multi_index container or compiler
      // gets confused
...
bool Module::findSymbol(std::vector<Symbol *> &found, const std::string &name,
  ...
  for (unsigned i = 0; i < obj_syms.size(); i++) {
    if (obj_syms[i]->getModule() == this) found.push_back(obj_syms[i]);
  }

  if (found.size() > orig_size) return true;
...
const std::string &Module::fileName() const { return fileName_; }

const std::string &Module::fullName() const { return fullName_; }

Symtab *Module::exec() const { return exec_; }

supportedLanguages Module::language() const { return language_; }
...
bool Module::getSourceLines(std::vector<Statement::Ptr> &lines,
                            Offset addressInRange) {
  unsigned int originalSize = lines.size();

  LineInformation *lineInformation = parseLineInformation();
  if (lineInformation) lineInformation->getSourceLines(addressInRange, lines);
...
LineInformation *Module::parseLineInformation() {
  ...
  if (!info_.empty()) {
    for (auto cu = info_.begin(); cu != info_.end(); ++cu) {
...
bool Module::getStatements(
    std::vector<LineInformation::Statement_t> &statements) {
...
typeCollection *Module::getModuleTypesPrivate() { return typeInfo_; }
...
LineInformation *Module::getLineInformation() { return lineInfo_; }
...
Module::Module(supportedLanguages lang, Offset adr, std::string fullNm,
               Symtab *img)
    : lineInfo_(NULL),
      typeInfo_(NULL),
      fullName_(fullNm),
...
bool Module::getAllSymbolsByType(std::vector<Symbol *> &found,
                                 Symbol::SymbolType sType) {
  unsigned orig_size = found.size();
  std::vector<Symbol *> obj_syms;

  if (!exec()->getAllSymbolsByType(obj_syms, sType)) return false;
...
void Module::setLanguage(supportedLanguages lang) { language_ = lang; }

Offset Module::addr() const { return addr_; }
...
void Module::addRange(Dyninst::Address low, Dyninst::Address high) {
  ...
  //    fileName().c_str());
  std::set<AddressRange>::iterator lb =
      ranges.lower_bound(AddressRange(low, high));
...
StringTablePtr &Module::getStrings() { return strings_; }
