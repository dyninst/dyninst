...
/* Lookup functions defined in class Symtab. Separated to reduce file size and
 * classify. */
...
bool regexEquiv(const std::string &str, const std::string &them,
                bool checkCase);
...
bool Symtab::findSymbol(std::vector<Symbol *> &ret, const std::string &name,
  ...
  if (!isRegex) {
    ...
    if (nameType & mangledName) {
      ...
      //           symsByMangledName[name].begin(),
      //           symsByMangledName[name].end());
    ...
    if (nameType & prettyName) {
      ...
      // candidates.insert(candidates.end(), symsByPrettyName[name].begin(),
      // symsByPrettyName[name].end());
      // if (includeUndefined) candidates.insert(candidates.end(),
    ...
    if (nameType & typedName) {
      ...
      // candidates.insert(candidates.end(), symsByTypedName[name].begin(),
      // symsByTypedName[name].end());
      // if (includeUndefined) candidates.insert(candidates.end(),
...
bool Symtab::getAllSymbols(std::vector<Symbol *> &ret) {
  std::copy(everyDefinedSymbol.begin(), everyDefinedSymbol.end(),
            back_inserter(ret));
...
bool Symtab::getAllSymbolsByType(std::vector<Symbol *> &ret,
  ...
  if (sType == Symbol::ST_UNKNOWN) return getAllSymbols(ret);
...
bool sort_by_func_ptr(const Function *a, const Function *b) { return a < b; }
...
bool Symtab::findFunctionsByName(std::vector<Function *> &ret,
  ...
  if (!findSymbol(funcSyms, name, Symbol::ST_FUNCTION, nameType, isRegex,
                  checkCase)) {
    return false;
  }
...
bool Symtab::findVariablesByName(std::vector<Variable *> &ret,
  ...
  if (!findSymbol(varSyms, name, Symbol::ST_OBJECT, nameType, isRegex,
                  checkCase))
    return false;
...
bool Symtab::findRegion(Region *&ret, const Offset addr,
...
// Use POSIX regular expression pattern matching to check if std::string s
// matches
...
bool Symtab::addFunctionRange(FunctionBase *func, Dyninst::Offset next_start) {
  ...
  FuncRangeCollection &ranges =
      const_cast<FuncRangeCollection &>(func->getRanges());
  for (FuncRangeCollection::iterator i = ranges.begin(); i != ranges.end();
       i++) {
    FuncRange &range = *i;
  ...
  for (InlineCollection::const_iterator i = inlines.begin(); i != inlines.end();
       i++) {
...
bool Symtab::parseFunctionRanges() {
  ...
  for (vector<Function *>::iterator i = everyFunction.begin();
       i != everyFunction.end(); i++) {
...
bool Symtab::getContainingInlinedFunction(Offset offset, FunctionBase *&func) {
  if (!func_lookup) parseFunctionRanges();
  assert(func_lookup);
  ...
  func = (*ranges.begin())->container;
  for (set<FuncRange *>::iterator i = ++ranges.begin(); i != ranges.end();
       i++) {
...
unsigned Function::getSize() const {
  if (functionSize_) return functionSize_;
  ...
  for (;;) {
    ...
    if (cur == offset) {
      if (mid + 1 >= symtab->everyFunction.size()) return 0;
...
Dyninst::Offset Symtab::fileToDiskOffset(Dyninst::Offset fileOffset) const {
  for (unsigned j = 0; j < regions_.size(); ++j) {
    if (regions_[j]->getFileOffset() <= fileOffset &&
        ((regions_[j]->getFileOffset() + regions_[j]->getDiskSize()) >
         fileOffset)) {
