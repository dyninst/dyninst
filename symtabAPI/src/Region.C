...
Region*
Region::createRegion(Offset diskOff, perm_t perms, RegionType regType,
                     unsigned long diskSize, Offset memOff, unsigned long memSize,
                     std::string name, char* rawDataPtr, bool isLoadable, bool isTLS,
                     unsigned long memAlign)
{
    Region* newreg = new Region(0, name, diskOff, diskSize, memOff, memSize, rawDataPtr,
                                perms, regType, isLoadable, isTLS, memAlign);
    return newreg;
}
...
Region::Region(unsigned regnum, std::string name, Offset diskOff, unsigned long diskSize,
               Offset memOff, unsigned long memSize, char* rawDataPtr, perm_t perms,
               RegionType regType, bool isLoadable, bool isThreadLocal,
               unsigned long memAlignment)
: regNum_(regnum)
, name_(name)
, diskOff_(diskOff)
, diskSize_(diskSize)
...
, symtab_(NULL)
{
    ...
    // (disk), "; cerr << "[" << memOff << ", " << memOff+memSize << ") (memory)" << endl;
    if(memOff)
        isLoadable_ = true;
}
...
const char*
Region::permissions2Str(perm_t p)
{
    switch(p)
    {
...
const char*
Region::regionType2Str(RegionType rt)
{
    switch(rt)
    {
...
bool
Region::setRegionNumber(unsigned regnumber)
{
    regNum_ = regnumber;
    return true;
}
...
std::string
Region::getRegionName() const
{
    return name_;
}

Offset
Region::getDiskOffset() const
{
    return diskOff_;
}

unsigned long
Region::getDiskSize() const
{
    return diskSize_;
}
...
Offset
Region::getMemOffset() const
{
    return memOff_;
}

unsigned long
Region::getMemSize() const
{
    return memSize_;
}
...
void
Region::setMemOffset(Offset newoff)
{
    memOff_ = newoff;
}

void
Region::setFileOffset(Offset newoff)
{
    fileOff_ = newoff;
}

void
Region::setMemSize(unsigned long newsize)
{
    memSize_ = newsize;
}

void
Region::setDiskSize(unsigned long newsize)
{
    diskSize_ = newsize;
}

void*
Region::getPtrToRawData() const
{
    return rawDataPtr_;
}

bool
Region::setPtrToRawData(void* buf, unsigned long newsize)
{
    rawDataPtr_ = buf;
    diskSize_   = newsize;
    isDirty_    = true;
...
bool
Region::isOffsetInRegion(const Offset& offset) const
{
    return (offset >= diskOff_ && offset < (diskOff_ + diskSize_));
}

bool
Region::isLoadable() const
{
    if(isLoadable_)
        return true;
    return (memOff_ != 0);
...
bool
Region::isDirty() const
{
    return isDirty_;
}

std::vector<relocationEntry>&
Region::getRelocations()
{
    return rels_;
}

bool
Region::patchData(Offset off, void* buf, unsigned size)
{
...
bool
Region::addRelocationEntry(Offset ra, Symbol* dynref, unsigned long relType,
...
bool
Region::addRelocationEntry(const relocationEntry& rel)
{
    rels_.push_back(rel);
    return true;
}
...
Region::perm_t
Region::getRegionPermissions() const
{
    return permissions_;
}

bool
Region::setRegionPermissions(Region::perm_t newPerms)
{
...
bool
Region::updateRelocations(Address start, Address end, Symbol* oldsym, Symbol* newsym)
{
