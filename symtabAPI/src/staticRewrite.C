 ...
 * under the undefined set. We iterate over all the undefined symbols and try to
 * find
 * the member in the archive which has the definition and add it to the list of
 * .o's.
 * All the undefined symbols in this .o are added to undefined set and defined
 * symbols
 * are added to the defined set. This process goes on until we do not have any
 * undefined symbols.
 * Otherwise we return an error
 */
bool findDotOs(Symtab *obj, std::vector<Archive *> arfs,
               vector<Symtab *> &members) {
  vector<Symbol *> undefSyms;

  // Initialize undefSyms with all the undefined members in the static
  // executable
  ...
  while (undefSyms.size() != 0) {
    ...
    if (obj->findSymbolByType(foundsyms, sym->getName(), Symbol::ST_UNKNOWN,
                              true))
      continue;
    ...
    // reached if there are undefined symbols that are not defined in any of the
    // archives
    return false;
  }
  return true;
