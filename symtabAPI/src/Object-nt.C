...
//---------------------------------------------------------------------------
BOOL CALLBACK SymEnumSymbolsCallback(PSYMBOL_INFO pSymInfo, ULONG symSize,
                                     PVOID userContext);
/*
 * stripAtSuffix
 *
...
char *cplus_demangle(char *c, int, bool includeTypes) {
  ...
  if (c[0] == '_') {
    ...
    if (includeTypes) {
      if (UnDecorateSymbolName(c, buf, 1000, UNDNAME_COMPLETE |
      ...
      // else if (UnDecorateSymbolName(c, buf, 1000,
...
struct CompareSymAddresses
    : public binary_function<const Object::intSymbol *,
                             const Object::intSymbol *, bool> {
...
Object::Module::Module(std::string _name, DWORD64 _baseAddr, DWORD64 _extent)
    : name(_name), baseAddr(_baseAddr), extent(_extent), isDll(false) {
  defFile = new Object::File();
  files.push_back(defFile);
}

Object::File *Object::Module::FindFile(std::string name) {
  File *ret = NULL;
  for (std::vector<File *>::iterator iter = files.begin(); iter != files.end();
       iter++) {
...
void Object::File::DefineSymbols(
    dyn_hash_map<std::string, std::vector<Symbol *> > &allSyms,
    map<Symbol *, std::string> &symsToMods, const std::string &modName) const {
...
void Object::intSymbol::DefineSymbol(
    dyn_hash_map<std::string, std::vector<Symbol *> > &allSyms,
    map<Symbol *, std::string> &symsToMods, const std::string &modName) const {
  Symbol *sym =
      new Symbol(GetName(), (Symbol::SymbolType)GetType(),
...
void Object::Module::DefineSymbols(
    const Object *obj, dyn_hash_map<std::string, std::vector<Symbol *> > &syms,
...
void Object::Module::PatchSymbolSizes(
    const Object *obj, const std::vector<Object::intSymbol *> &allSyms) const {
...
void Object::Module::BuildSymbolMap(const Object *obj) const {
  ...
  for (; iter != files.end(); iter++) {
    ...
    for (std::vector<Object::intSymbol *>::const_iterator symIter =
             curSyms.begin();
...
static BOOL isGlobalSymbol(PSYMBOL_INFO pSymInfo) {
  return ((pSymInfo->Flags & SYMFLAG_EXPORT) ||
          ...
          ((!pSymInfo->Flags) && ((pSymInfo->Tag == SymTagFunction) ||
...
void Object::ParseGlobalSymbol(PSYMBOL_INFO pSymInfo) {
  ...
  if (SymGetLineFromAddr64(hProc, pSymInfo->Address, &dwDisplacement,
                           &lineInfo)) {
  ...
  if (!isForwarded(((Offset)pSymInfo->Address) - baseAddr)) {
    pFile->AddSymbol(new Object::intSymbol(
        pSymInfo->Name, pSymInfo->Address - get_base_addr(), symType,
        symLinkage, pSymInfo->Size,
...
void Object::ParseSymbolInfo(bool alloc_syms) {
  ...
  if (alloc_syms) curModule->DefineSymbols(this, symbols_, symsToModules_);
...
void Object::AddTLSFunctions() {
  // ensure that the optional header has a TLS directory entry
  if (!peHdr ||
      peHdr->OptionalHeader.NumberOfRvaAndSizes <= IMAGE_DIRECTORY_ENTRY_TLS) {
    return;
  }
  ...
  Offset tlsMemOff =
      peHdr->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS]
          .VirtualAddress;
  ...
  unsigned maxCBs =
      (secn->getDiskSize() - cbOffSec) / sizeof(PIMAGE_TLS_CALLBACK);
  ...
  for (unsigned tidx = 0; tidx < maxCBs && tlsCBs[tidx] != NULL; tidx++) {
    ...
    pFile->AddSymbol(new Object::intSymbol(
        funcName, funcOff, Symbol::ST_FUNCTION, Symbol::SL_GLOBAL,
        0,  // unknown size
        secn));
  }
}
...
std::vector<std::pair<string, IMAGE_IMPORT_DESCRIPTOR> >
  ...
  while (import_d->Name != NULL && import_d->FirstThunk != NULL) {
    ...
    string str(
        (char *)(((char *)mf->base_addr()) + RVA2Offset(import_d->Name)));
...
void Object::FindInterestingSections(bool alloc_syms, bool defensive) {
  ...
  getImportDescriptorTable();  // save the binary's original table, we may
                               // change it later
  ...
  if (!is_aout_ &&
      peHdr->OptionalHeader.NumberOfRvaAndSizes >
    ...
    if (eT2) {
      ...
      DWORD *funcAddrs =
          (DWORD *)::ImageRvaToVa(ImageNtHeader(mapAddr), mapAddr,
                                  ULONG(eT2->AddressOfFunctions), NULL);
      WORD *funcAddrNameMap =
          (WORD *)::ImageRvaToVa(ImageNtHeader(mapAddr), mapAddr,
                                 ULONG(eT2->AddressOfNameOrdinals), NULL);
  ...
  for (unsigned int i = 0; i < nSections; i++) {
    ...
    if (alloc_syms)
      regions_.push_back(new Region(i + 1, (const char *)pScnHdr->Name,
                                    diskOffset, pScnHdr->SizeOfRawData,
...
static BOOL CALLBACK add_line_info(SRCCODEINFO *srcinfo, void *param) {
  ...
  info_for_all_files_t::iterator iter =
      all_info->find(std::string(srcinfo->FileName));
  ...
  finfo->insert(
      line_info_tmp_t((unsigned long)srcinfo->Address, srcinfo->LineNumber));
  return true;
}

static bool store_line_info(Symtab *st, info_for_all_files_t *baseInfo) {
  for (info_for_all_files_t::iterator i = baseInfo->begin();
       i != baseInfo->end(); i++) {
    ...
    for (info_for_file_t::iterator j = (*i).second->begin();
         j != (*i).second->end(); j++) {
...
BOOL CALLBACK enumLocalSymbols(PSYMBOL_INFO pSymInfo, unsigned long symSize,
  ...
  std::string fName =
      convertCharToString(func->getModule()->fileName().c_str());
...
__LINE__);
  ...
  if (func->getSize()) {
    ...
    frame.InstructionOffset =
        locals->base + func->getOffset() + func->getSize();
...
static dataClass getDataClass(HANDLE p, Offset base, int typeIndex) {
  ...
  if (!result) return dataUnknownType;
  switch (wintype) {
    case SymTagFunction:
...
static Type *getEnumType(HANDLE p, Offset base, int typeIndex, Module *mod) {
  ...
  //
  // Get the number of entries in this enum, and store them in the entries
  // structure
  //
  result =
      SymGetTypeInfo(p, base, typeIndex, TI_GET_CHILDRENCOUNT, &numEntries);
  if (!result) numEntries = 0;
  if (numEntries) {
    entriesSize =
        sizeof(TI_FINDCHILDREN_PARAMS) + (numEntries + 1) * sizeof(ULONG);
  ...
  for (i = 0; i < numEntries; i++) {
    ...
    result =
        SymGetTypeInfo(p, base, entries->ChildId[i], TI_GET_VALUE, &entryValue);
    if (!result) continue;
...
static Type *getUDTType(HANDLE p, Offset base, int typeIndex, Module *mod) {
  ...
  //
  result =
      SymGetTypeInfo(p, base, typeIndex, TI_GET_CHILDRENCOUNT, &numChildren);
  ...
  if (numChildren) {
    childrenSize =
        sizeof(TI_FINDCHILDREN_PARAMS) + (numChildren + 1) * sizeof(ULONG);
  ...
  for (i = 0; i < numChildren; i++) {
    ...
    childName = NULL;
    result =
        SymGetTypeInfo(p, base, children->ChildId[i], TI_GET_SYMTAG, &symtag);
    if (result && symtag == SymTagBaseClass) {
      childName = P_strdup("{superclass}");
    }
    if (!childName) childName = getTypeName(p, base, children->ChildId[i]);
    if (!childName) childName = P_strdup(child_type->getName().c_str());

    // Find the offset of this member in the structure
    result = SymGetTypeInfo(p, base, children->ChildId[i], TI_GET_OFFSET,
                            &child_offset);
    if (!result) {
    ...
    if (childName) free((void *)childName);
    childName = NULL;
  }
...
static Type *getFunctionType(HANDLE p, Offset base, int typeIndex,
                             Module *mod) {
...
static Type *getBaseType(HANDLE p, Offset base, int typeIndex, Module *mod) {
  ...
  if (newType) addTypeToCollection(newType, mod);
  return newType;
}
...
static Type *getType(HANDLE p, Offset base, int typeIndex, Module *mod) {
  ...
  //
  // Types on Windows are stored as part of a special type of symbol.
  // TI_GET_SYMTAG
  ...
  switch (symtag) {
    ...
    case SymTagBaseClass:
      ...
      if (foundType) typeIndex = foundType->getID();
      break;
    case SymTagThunk:
      foundType = NULL;
      break;
...
static void findLocalVars(Function *func, proc_mod_pair base) {
  ...
  // at specific points, which makes it hard to enumerate all locals (as we
  // want).
...
BOOL CALLBACK add_type_info(PSYMBOL_INFO pSymInfo, ULONG SymbolSize,
                            void *info) {
  HANDLE p;
  Offset obj_base;
  proc_mod_pair *pair;
  ...
  if (obj->isExec()) {
    // When parsing the a.out, sort the type information into specific modules.
    // This doesn't matter
    // for libraries, because there is a 1:1 mapping between modules and
    // objects.
    //
    // A module is a collection of functions, but doesn't include global data
...
void Object::setTrapHeader(Offset addr) { trapHeaderPtr_ = addr; }
Offset Object::trapHeader() { return trapHeaderPtr_; }
...
Dyninst::Architecture Object::getArch() { return Dyninst::Arch_x86; }
...
/*
        for(it=ref.begin(); it!=ref.end(); it++){
                ...
                //look through the old import table to check if the library has
   been there
                bool isExisting = false;
                for(unsigned int i=0; i<oldImp.size(); i++){

                        //if already been there, use the same of RVA of name
                        if(strcmp(oldImp[i].name, (*it).second.first.c_str()) ==
   0){
                                isExisting = true;
                ...
                if(!isExisting){
                        ...
                        ptrLib = (char*) GlobalAlloc(GMEM_FIXED|GMEM_ZEROINIT,
   strLen+1);
                        memcpy(ptrLib,(*it).second.first.c_str(), strLen);
                        info.push_back(std::pair<char*,unsigned long> (ptrLib,
   strLen+1));
                        strOff+=(strLen+1);
                }
...
void Object::applyRelocs(Region *relocs, Offset delta) {
  ...
  while (section_pointer < section_end) {
    PIMAGE_BASE_RELOCATION curRelocPage =
        (PIMAGE_BASE_RELOCATION)(section_pointer);
    section_pointer += sizeof(IMAGE_BASE_RELOCATION);
    Offset pageBase = curRelocPage->VirtualAddress;
    if (pageBase == 0) break;
    int numRelocs =
        (curRelocPage->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) /
        sizeof(WORD);
