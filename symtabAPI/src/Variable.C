...
{}
void
Variable::setType(boost::shared_ptr<Type> type)
{
    type_ = type;
}
...
bool
Variable::removeSymbol(Symbol* sym)
{
    removeSymbolInt(sym);
    if(symbols_.empty())
    {
...
bool
localVar::addLocation(const VariableLocation& location)
{
    if(!locsExpanded_)
    {
...
void
localVar::expandLocation(const VariableLocation& loc, std::vector<VariableLocation>& ret)
{
...
#ifdef DEBUG
    cerr << "Expanding location for variable " << name_ << " / " << hex << this << dec
         << endl;
#endif
...
void
localVar::fixupUnknown(Module* module)
{
...
boost::shared_ptr<Type> localVar::getType(Type::do_share_t) { return type_; }

bool
localVar::setType(boost::shared_ptr<Type> newType)
{
    type_ = newType;
    return true;
}
...
std::string&
localVar::getFileName()
{
    return fileName_;
}

std::vector<Dyninst::VariableLocation>&
localVar::getLocationLists()
{
    if(!locsExpanded_)
    {
        // Here we get clever
...
bool
localVar::operator==(const localVar& l)
{
    if(type_ && !l.type_)
        return false;
    if(!type_ && l.type_)
    ...
    if(name_ != l.name_)
        return false;
    if(fileName_ != l.fileName_)
        return false;
    if(lineNum_ != l.lineNum_)
        return false;
