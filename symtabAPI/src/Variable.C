...
Variable::Variable(Symbol *sym) : Aggregate(sym), type_(NULL) {}

Variable::Variable() : Aggregate(), type_(NULL) {}
void Variable::setType(Type *type) { type_ = type; }
...
Serializable *Variable::serialize_impl(SerializerBase *sb, const char *tag)
  ...
  //  magic numbers stink, but we use both positive and negative numbers for
  //  type ids
  ...
  try {
    ...
    serialize_printf(
        "%s[%d]:  %sSERIALIZED VARIABLE %s, %lu syms\n", FILE__, __LINE__,
        sb->isInput() ? "DE" : "",
...
#else
Serializable *Variable::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}
#endif
...
localVar::localVar(localVar &lvar) : Serializable() {
  name_ = lvar.name_;
  type_ = lvar.type_;
  ...
  std::copy(lvar.locs_.begin(), lvar.locs_.end(), std::back_inserter(locs_));
...
//#define DEBUG
...
#ifdef DEBUG
  cerr << "Expanding location for variable " << name_ << " / " << hex << this
       << dec << endl;
#endif
  ...
  if (func_fp.empty()) {
     cerr << "Error: function " << hex << func_
          << " / " << func_->getAllMangledNames()[0] << " has no frame pointer!"
  << endl;
  }
  */
  if (func_fp.empty()) return;
...
#ifdef DEBUG
    cerr << "var range: " << hex << varlowPC << ".." << varhiPC << endl;
    cerr << "frame range: " << hex << fplowPC << ".." << fphiPC << dec << endl;
#endif
    if (fplowPC > varhiPC) {
...
#ifdef DEBUG
    cerr << "Created variable location [" << hex << newloc.lowPC << ".."
         << newloc.hiPC << "], reg " << newloc.mr_reg.name() << " /w/ offset "
         << newloc.frameOffset << " = (" << loc.frameOffset << "+"
         << i->frameOffset << ")"
...
std::string &localVar::getName() { return name_; }

Type *localVar::getType() { return type_; }
...
int localVar::getLineNum() { return lineNum_; }

std::string &localVar::getFileName() { return fileName_; }
...
bool localVar::operator==(const localVar &l) {
  ...
  if (type_) {
    if (type_->getID() != l.type_->getID()) return false;
  }
...
#if !defined(SERIALIZATION_DISABLED)
Serializable *localVar::serialize_impl(SerializerBase *sb, const char *tag)
    THROW_SPEC(SerializerError) {
  ...
  serialize_printf("%s[%d]:  %sserialized localVar %s, done\n", FILE__,
                   __LINE__, sb->isInput() ? "de" : "", name_.c_str());
  return NULL;
}
#else
Serializable *localVar::serialize_impl(SerializerBase *, const char *)
    THROW_SPEC(SerializerError) {
  return NULL;
}
#endif
