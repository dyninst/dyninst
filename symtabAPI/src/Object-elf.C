...
static bool pdelf_check_ehdr(Elf_X &elf) {
  ...
  // or a relocatable file (e.g .o) and it either has section headers or program
  // headers
...
const char *pdelf_get_shnames(Elf_X *elf) {
  ...
  if (shstrscn.isValid()) {
    ...
    if (shstrdata.isValid()) result = shstrdata.get_string();
  }
  return result;
}
...
Region::RegionType getSegmentType(unsigned long type, unsigned long flags) {
  if (type == PT_DYNAMIC) return Region::RT_DYNAMIC;
  if (flags == 7) return Region::RT_TEXTDATA;
  if (flags == 5) return Region::RT_TEXT;
  if (flags == 6) return Region::RT_DATA;
...
Elf_X_Shdr *Object::getRegionHdrByAddr(Offset addr) {
  ...
  if (allRegionHdrs[start]->sh_addr() == addr) return allRegionHdrs[start];
  return NULL;
}
...
int Object::getRegionHdrIndexByAddr(Offset addr) {
  ...
  if (allRegionHdrs[start]->sh_addr() == addr) return start;
  return -1;
}

Elf_X_Shdr *Object::getRegionHdrByIndex(unsigned index) {
  if (index >= allRegionHdrs.size()) return NULL;
  return allRegionHdrs[index];
}

/* Check if there is a section which belongs to the segment and update
 * permissions of that section.
 * Return value indicates whether the segment has to be added to the list of
 * regions*/
...
set<string> debugInfoSections =
    list_of(string(SYMTAB_NAME))(string(STRTAB_NAME));

// loaded_elf(): populate elf section pointers
// for EEL rewritten code, also populate "code_*_" members
bool Object::loaded_elf(Offset &txtaddr, Offset &dataddr, Elf_X_Shdr *&bssscnp,
                        ...
                        Elf_X_Shdr *&interp_scnp, Elf_X_Shdr *&opd_scnp, bool) {
  ...
  for (unsigned int i = 0; i < elfHdr_numSections + elfHdrDbg_numSections;
       i++) {
    const char *name;
    ...
    // If the section appears in the memory image of a process address is given
    // by
    // sh_addr()
    ...
    if (!scn.isFromDebugFile()) {
      ...
      if (scn.sh_flags() & SHF_ALLOC) {
        ...
        Region *reg = new Region(
            i, name, scn.sh_addr(), scn.sh_size(), 0, 0, ((char *)data.d_buf()),
    ...
    if (P_strcmp(name, EDITED_TEXT_NAME) == 0) {
      ...
      EEL = true;
      if (txtaddr == 0) txtaddr = scn.sh_addr();
...
fini_addr_ = scn.sh_addr();
...
#else
    else if ((secAddrTagMapping.find(scn.sh_addr()) !=
   ...
   if the binary is built for BGP compute nodes */
      ...
      while (buf && (index < size)) {
        ...
        index += comment.size();
        if (comment.size() ==
            0) {  // Skip NULL characters in the comment section
          index++;
        }
      }
    }
 ...
 **/
  ...
  // if (addressWidth_nbytes == 8) bperr( ">>> 64-bit loaded_elf()
  // successful\n");
  return true;
}
...
bool Object::get_relocationDyn_entries(unsigned rel_scnp_index,
  ...
  if (!symdata.isValid() || !strdata.isValid()) return false;
  ...
  while (num_rel_entries_found != num_rel_entries) {
    ...
    if (sym.isValid() && (rel.isValid() || rela.isValid()) && strs) {
      ...
      for (u_int i = 0; i < (reldata.d_size() / rel_entry_size_); ++i) {
        ...
        relocationEntry re(offset, string(&strs[sym.st_name(index)]), NULL,
                           type);
        re.setAddend(addend);
        ...
        if (symbols_.find(&strs[sym.st_name(index)]) != symbols_.end()) {
          ...
          for (vector<Symbol *>::iterator i = syms.begin(); i != syms.end();
               i++) {
            if (!(*i)->isInDynSymtab()) continue;
            re.addDynSym(*i);
            break;
          }
        }
  ...
  if (rel_plt_size_ && rel_plt_addr_) {
    ...
    if (reldata.isValid() && symdata.isValid() && strdata.isValid()) {
      ...
      if (getArch() == Dyninst::Arch_x86 || getArch() == Dyninst::Arch_x86_64) {
            ...
            for (unsigned int i = 0; i < count; ++i) {
              ...
              if (dyn[i].d_tag == DT_LOPROC) {
                ...
                if (got != NULL) {
                  ...
                  glink_addr = *(
                      unsigned int *)(data + (g_o_t - got->getMemOffset() + 4));
                  break;
                }
              }
            }
        ...
        //  and PLT_ENTRY_SIZE (12).  PLT_SLOT_SIZE seems to be what we want,
        //  even though we also
        ...
        for (iter = symbols_.begin(); iter != symbols_.end(); ++iter) {
          ...
          if (name.length() > 8) {
            ...
            if (pltrel_iter == plt_rel_map.end()) continue;

            Symbol *stub_sym = iter->second[0];
            relocationEntry re(stub_sym->getOffset(), pltrel_iter->second, name,
                               targ_sym);
            fbt_.push_back(re);
          }
        }
        ...
        next_plt_entry_addr +=
            4 * (plt_entry_size_);  // 1st 4 entries are special
      }
      ...
      if (sym.isValid() && (rel.isValid() || rela.isValid()) && strs) {
        ...
        for (u_int i = 0; i < (rel_plt_size_ / rel_plt_entry_size_); ++i) {
          ...
          if (fbt_iter == -1) {  // Create new relocation entry.
            ...
            re.setAddend(addend);
            re.setRegionType(rtype);
            if (dynsym_list.size() > 0) re.addDynSym(dynsym_list[0]);
            fbt_.push_back(re);
...
void Object::load_object(bool alloc_syms) {
  ...
  {  // binding contour (for "goto cleanup")
    ...
    if (!loaded_elf(txtaddr, dataddr, bssscnp, symscnp, strscnp, stabscnp,
                    stabstrscnp, stabs_indxcnp, stabstrs_indxcnp, rel_plt_scnp,
                    plt_scnp, got_scnp, dynsym_scnp, dynstr_scnp, dynamic_scnp,
                    eh_frame_scnp, gcc_except, interp_scnp, opd_scnp, true)) {
      goto cleanup;
    }
    ...
    if (getArch() == Dyninst::Arch_x86 || getArch() == Dyninst::Arch_x86_64) {
      if (eh_frame_scnp != 0 && gcc_except != 0) {
        find_catch_blocks(eh_frame_scnp, gcc_except, txtaddr, dataddr,
                          catch_addrs_);
      }
    }
#endif
...
// if it exists
// If yes set dynamic for the symbol ( How to distinguish between symbols only
// in symtab,
...
// allsymbols = merge(allsymbols, alldynSymbols);
      ...
      unsigned long lstarttime =
          starttime.tv_sec * 1000 * 1000 + starttime.tv_usec;
      unsigned long lendtime = endtime.tv_sec * 1000 * 1000 + endtime.tv_usec;
      unsigned long difftime = lendtime - lstarttime;
      double dursecs = difftime / (1000);
      cout << "parsing/fixing/overriding elf took " << dursecs << " msecs"
           << endl;
#endif
      ...
      parse_all_relocations(*elfHdr, dynsym_scnp, dynstr_scnp, symscnp,
                            strscnp);
...
void Object::load_shared_object(bool alloc_syms) {
  ...
  {  // binding contour (for "goto cleanup2")
    ...
    if (!loaded_elf(txtaddr, dataddr, bssscnp, symscnp, strscnp, stabscnp,
                    stabstrscnp, stabs_indxcnp, stabstrs_indxcnp, rel_plt_scnp,
                    plt_scnp, got_scnp, dynsym_scnp, dynstr_scnp, dynamic_scnp,
                    eh_frame_scnp, gcc_except, interp_scnp, opd_scnp))
      goto cleanup2;
...
// cout << "parsing/fixing/overriding/insertion elf took "<<dursecs <<" msecs"
// << endl;
#endif
      if (dynamic_addr_ && dynsym_scnp && dynstr_scnp) {
        parseDynamic(dynamic_scnp, dynsym_scnp, dynstr_scnp);
...
void Object::parse_opd(Elf_X_Shdr *opd_hdr) {
  ...
  // In some cases, the OPD is a set of 3-tuples: <func offset, TOC, environment
  // ptr>.
...
Symbol *Object::handle_opd_symbol(Region *opd, Symbol *sym) {
  ...
  opd_entry +=
      (soffset - opd->getDiskOffset()) / sizeof(Offset);  // table of offsets;
...
bool Object::parse_symbols(Elf_X_Data &symdata, Elf_X_Data &strdata,
                           Elf_X_Shdr *bssscnp, Elf_X_Shdr *symscnp,
...
*/
      ...
      if (i == 0 && sname == "" && soffset == (Offset)0) continue;
      ...
      if (stype == Symbol::ST_UNKNOWN) newsym->setInternalType(etype);
...
void Object::parse_dynamicSymbols(Elf_X_Shdr *&dyn_scnp, Elf_X_Data &symdata,
                                  Elf_X_Data &strdata, bool /*shared*/,
                                  std::string smodule) {
#if defined(TIMED_PARSE)
  struct timeval starttime;
  gettimeofday(&starttime, NULL);
  ...
  if (versymSec) symVersions = versymSec->get_data().get_versyms();
  if (verdefSec) symVersionDefs = verdefSec->get_data().get_verDefSym();
  if (verneedSec) symVersionNeeds = verneedSec->get_data().get_verNeedSym();
  ...
  if (syms.isValid()) {
    for (unsigned i = 0; i < syms.count(); i++) {
      ...
      // discard "dummy" symbol at beginning of file
      if (i == 0 && sname == "" && soffset == 0) continue;
      ...
      if (versymSec) {
        ...
        if (versionFileNameMapping.find(index) !=
            versionFileNameMapping.end()) {
...
bool Object::dwarf_parse_aranges(Dwarf_Debug dbg,
  ...
  for (int i = 0; i < num_ranges; i++) {
    ...
    status = dwarf_get_arange_info_b(ranges[i], &segment, &segment_size, &start,
                                     &len, &cu_die_off, NULL);
...
bool Object::fix_global_symbol_modules_static_dwarf() {
  ...
  while (dwarf_next_cu_header_c(dbg, Dwarf_Bool(true), NULL, NULL, NULL, NULL,
                                NULL, NULL, NULL, NULL, &cu_die_off,
                                NULL) == DW_DLV_OK) {
...
bool Object::fix_global_symbol_modules_static_dwarf() { return false; }
  ...
  for (unsigned i = 0; i < stabptr->count(); i++) {
    switch (stabptr->type(i)) {
      ...
      case N_SO: /* compilation source or file name */
        if ((stabptr->desc(i) == N_SO_FORTRAN) ||
...
Object::Object(MappedFile *mf_, bool, void (*err_func)(const char *),
  ...
  if (mf->getFD() != -1) {
    ...
    elfHdr =
        Elf_X::newElf_X((char *)mf->base_addr(), mf->size(), mf_->pathname());
  }
...
bool Object::get_func_binding_table_ptr(
    const std::vector<relocationEntry> *&fbt) const {
  if (!plt_addr_ || (!fbt_.size())) return false;
  fbt = &fbt_;
  return true;
}

void Object::getDependencies(std::vector<std::string> &deps) { deps = deps_; }
  ...
  for (unsigned i = 0; i < elf.e_shnum(); ++i) {
    ...
    if (shdr.sh_flags() & SHF_ALLOC) {  // This section is in memory
      if (code_off_ <= shdr.sh_addr() &&
        ...
        if (shdr.sh_addr() < code_vldS_) code_vldS_ = shdr.sh_addr();
        ...
        if (shdr.sh_addr() < data_vldS_) data_vldS_ = shdr.sh_addr();
...
bool parseCompilerType(Object *objPtr) {
  ...
  if (syms->find("pgCC_compiled.") != syms->end()) return true;
  return false;
}
#else
bool parseCompilerType(Object *objPtr) {
...
static unsigned long read_uleb128(const unsigned char *data,
                                  unsigned *bytes_read) {
  ...
  *bytes_read = 0;
  while (1) {
    ...
    if ((data[(*bytes_read)++] & 0x80) == 0) break;
    shift += 7;
  }
  return result;
}

static signed long read_sleb128(const unsigned char *data,
                                unsigned *bytes_read) {
  unsigned long result = 0;
  unsigned shift = 0;
  *bytes_read = 0;
...
static int read_val_of_type(int type, unsigned long *value,
                            const unsigned char *addr,
                            const mach_relative_info &mi) {
  unsigned size = 0;
  if (type == DW_EH_PE_omit) return 0;
...
static int read_except_table_gcc3(Dwarf_Fde *fde_data, Dwarf_Signed fde_count,
                                  mach_relative_info &mi, Elf_X_Shdr *eh_frame,
  ...
  for (int i = 0; i < fde_count; i++) {
    ...
    status = dwarf_get_fde_range(fde, &low_pc, NULL, (void **)&fde_bytes, NULL,
                                 &cie_offset, NULL, &fde_offset, &err);
    if (status != DW_DLV_OK) {
      pd_dwarf_handler(err, NULL);
      return false;
    }
    ...
    status = dwarf_get_cie_info(cie, &bytes_in_cie, NULL, &augmentor, NULL,
    ...
    status = dwarf_get_cie_augmentation_data(cie, &cie_augdata,
                                             &cie_augdata_len, &err);
    if (status != DW_DLV_OK) {
      pd_dwarf_handler(err, NULL);
      return false;
    ...
    // Where-as the CIE augmentation tends to contain things like bytes
    // describing
    // pointer encodings, the FDE contains the actual pointers.
    status = dwarf_get_fde_augmentation_data(fde, &fde_augdata,
                                             &fde_augdata_len, &err);
    if (status != DW_DLV_OK) {
      pd_dwarf_handler(err, NULL);
      return false;
    ...
    for (j = 0; j < augmentor_len; j++) {
      ...
      } else if (augmentor[j] == 'P' || augmentor[j] == 'z' ||
                 augmentor[j] == 'R') {
    ...
    //  result = read_val_of_type(table_format, &table_end, datap + except_off,
    //  mi);
    //  This read should always be a ULEB128 read
    result =
        read_val_of_type(DW_EH_PE_uleb128, &table_end, datap + except_off, mi);
    if (result == -1) {
      continue;
    }
    except_off += result;
...
static bool read_except_table_gcc2(Elf_X_Shdr *except_table,
  ...
  while (i < except_size) {
    ...
    if (try_start != (Offset)-1 && try_end != (Offset)-1) {
      ExceptionBlock eb(try_start, (unsigned)(try_end - try_start),
                        catch_start);
      addresses.push_back(eb);
    }
  }
...
struct exception_compare
    : public binary_function<const ExceptionBlock &, const ExceptionBlock &,
                             bool> {
  bool operator()(const ExceptionBlock &e1, const ExceptionBlock &e2) {
    if (e1.tryStart() < e2.tryStart()) return true;
    return false;
  }
};
 ...
 **/
bool Object::find_catch_blocks(Elf_X_Shdr *eh_frame, Elf_X_Shdr *except_scn,
  ...
  status = dwarf_get_fde_list_eh(dbg, &cie_data, &cie_count, &fde_data,
                                 &fde_count, &err);
  if (status != DW_DLV_OK) {
  ...
  for (i = 0; i < cie_count; i++) {
    status = dwarf_get_cie_info(cie_data[i], &bytes_in_cie, NULL, &augmentor,
  ...
  if (gcc_ver == 2) {
    ...
    result = read_except_table_gcc3(fde_data, fde_count, mi, eh_frame,
                                    except_scn, catch_addrs);
  }
...
cleanup:
  ...
  for (i = 0; i < cie_count; i++) dwarf_dealloc(dbg, cie_data[i], DW_DLA_CIE);
  for (i = 0; i < fde_count; i++) dwarf_dealloc(dbg, fde_data[i], DW_DLA_FDE);
...
ObjectType Object::objType() const { return obj_type_; }

void Object::getModuleLanguageInfo(
    dyn_hash_map<string, supportedLanguages> *mod_langs) {
  ...
  // ends with "backtrack mode" and reiterates through all chosen languages,
  // changing
  // lang_Fortran to lang_Fortran_with_pretty_debug.
  //
  // This may be ugly, but it is set up this way since the information that is
  // used
  ...
  //  A final module<->language decision is not made until we have arrived at
  //  the
  ...
  for (unsigned int i = 0; i < stabptr->count(); i++) {
    if (stabptr->type(i) == N_UNDF) { /* start of object file */
      ...
      //  We can use the compiler option string (in a pinch) to guess at the
      //  source file language
      //  There is possibly more useful information encoded somewhere around
      //  here, but I lack
      //  an immediate reference....
      if (working_name) working_options = const_cast<char *>(stabptr->name(i));
    } else if ((stabptr->type(i) == N_SO) ||
               (stabptr->type(i) ==
                N_ENDM)) { /* compilation source or file name */
      // We have arrived at the next source file, finish up with the last one
      // and reset state
      // before starting next

      //   XXXXXXXXXXX  This block is mirrored near the end of routine, if you
      //   edit it,
      ...
      if ((mod_langs->find(working_module) != mod_langs->end()) &&
        ...
        // cerr << __FILE__ << __LINE__ << ":  Module: " <<working_module<< "
        // has language "<< stabptr->desc(i) << endl;
  ...
  //  Need to make sure we finish up with the module we were last collecting
  //  information
  //  about
  ...
  if (working_name) {
    ...
    if (working_lang == lang_Fortran_with_pretty_debug) fortran_kludge_flag = 1;
    (*mod_langs)[working_module] = working_lang;
  }
  //   XXXXXXXXXXX
  ...
  if (fortran_kludge_flag) {
    ...
    // go through map and change all lang_Fortran to
    // lang_Fortran_with_pretty_symtab
    dyn_hash_map<string, supportedLanguages>::iterator iter =
        (*mod_langs).begin();
    string aname;
  ...
  unsigned long difftime = lendtime - lstarttime;
  double dursecs = difftime / (1000);
  cout << __FILE__ << ":" << __LINE__ << ": getModuleLanguageInfo took "
       << dursecs << " msecs" << endl;
#endif
  delete stabptr;
  ...
  if (hasDwarfInfo()) {
    ...
    while (!done &&
           dwarf_next_cu_header_c(
               dbg, is_info, NULL, NULL, NULL,  // len, stamp, abbrev
    ...
    cleanup_dwarf:
      if (languageAttribute) dwarf_dealloc(dbg, languageAttribute, DW_DLA_ATTR);
      if (moduleName) dwarf_dealloc(dbg, moduleName, DW_DLA_STRING);
      if (moduleDIE) dwarf_dealloc(dbg, moduleDIE, DW_DLA_DIE);
...
bool Object::emitDriver(string fName, std::vector<Symbol *> &allSymbols,
                        unsigned) {
#ifdef BINEDIT_DEBUG
  printf("emitting...\n");
  // print_symbol_map(&symbols_);
...
const char *Object::interpreter_name() const { return interpreter_name_; }
  ...
  for (unsigned int i = 0; i < stabEntry->count(); i++) {
    switch (stabEntry->type(i)) {
      ...
      case N_SLINE: {
        ...
        //  Addresses specified in SLINEs are relative to the beginning of the
        //  fn
        Offset newLineAddress =
            stabEntry->val(i) + currentFunction->getOffset();
...
void Object::parseLineInfoForCU(Dwarf_Die cuDIE,
                                LineInformation *li_for_module) {
  ...
  // so we ensure that we're adding a block of unknown, 1...n to the string
  // table
  ...
  for (int i = 0; i < lineCount; i++) {
    ...
    if (dwarf->debugLinkFile()) {
      ...
      if (result) lineAddr = new_lineAddr;
    }
    //        if(!associated_symtab->isCode(lineAddr))
    //        {
    //            cout << hex << "WARNING: Discarding line info for non-text
    //            addr " << hex << lineAddr << endl;
    //            continue;
    //        }
   ...
   (The end of sequence marker marks discontinuities in the ranges.) */
      if ((lineNo == previousLineNo) && (lineSource == previousLineSource) &&
          !isEndOfSequence) {
        /* Don't update the prev* values; just keep going until we hit the end
   of
   a sequence or  new sourcefile. */
      ...
      if (startAddrToUse && endAddrToUse) {
        if (startAddrToUse == (Dyninst::Offset)(-1) ||
            endAddrToUse == (Dyninst::Offset)(-1)) {
          cout << "Suspicious line info range: [" << hex << startAddrToUse
               << ", " << endAddrToUse << ")\n";
        }
        if (startAddrToUse == (Dyninst::Offset)(1) ||
            endAddrToUse == (Dyninst::Offset)(1)) {
        ...
        if (startAddrToUse != endAddrToUse) {
          ...
          li_for_module->addLine((unsigned int)(previousLineSource + offset),
                                 ...
                                 startAddrToUse, endAddrToUse);
        }
        //                else
        //                {
        ////                    cout << dec << "Skipping entry for " <<
        ///(*strings)[previousLineSource+offset]
        ////                         << ":" << previousLineNo
        ////                         << " at " << hex << startAddrToUse << dec
        ///<< endl;
        //                }

        /* The line 'canonicalLineSource:previousLineNo' has an address range of
         * [previousLineAddr, lineAddr). */
      }
    } /* end if the previous* variables are valid */

    /* If the current line ends the sequence, invalidate previous; otherwise,
     * update. */
    // We've seen a pattern where there are multiple lines for a given address
    // and
    ...
    if (isEndOfSequence) {
      ...
      if (isStatement) {
        isPreviousValid = true;  //((previousLineNo != lineNo) &&
...
/* bperr("Resetting CURRENT FUNCTION NAME FOR NEXT OBJECT FILE\n");*/
        ...
        src_dur += (t2.tv_sec - t1.tv_sec) * 1000.0 +
    ...
    if (parseActive || !is_aout()) {
      ...
      switch (stabptr->type(i)) {
        case N_FUN:
          ...
          // that we have
          // properly set the var currentFunctionName for the later case of
          // (parseActive)
          symt_current_func = NULL;
          int currentEntry = i;
          ...
          if (!ptr || !(colonPtr = strchr(ptr, ':')))
            ...
            if (!associated_symtab->findSymbol(syms, *currentFunctionName,
              ...
              if (!associated_symtab->findSymbol(
                ...
                if (associated_symtab->findSymbol(
                        syms, fortranName, Symbol::ST_FUNCTION, mangledName)) {
                  delete currentFunctionName;
      ...
      switch (stabptr->type(i)) {
        ...
        case 0xc8:  // position-independant external typedefs -- N_ESYM
          ...
          if (parseCompilerType(this))
            temp = parseStabString(mod, mostRecentLinenum, (char *)ptr,
                                   stabptr->val(i), commonBlock);
          else
          ...
          pss_dur += (t2.tv_sec - t1.tv_sec) * 1000.0 +
                     (t2.tv_usec - t1.tv_usec) / 1000.0;
//      pss_dur += (t2.tv_sec/1000 + t2.tv_usec*1000) - (t1.tv_sec/1000 +
//      t1.tv_usec*1000);
#endif
          break;
        default:
  ...
  cout << __FILE__ << ":" << __LINE__ << ": parseTypes(" << mod->fileName()
       << ") took " << dursecs << " msecs" << endl;
  cout << "Breakdown:" << endl;
  cout << "     Functions: " << fun_count << " took " << fun_dur << "msec"
       << endl;
  cout << "     Sources: " << src_count << " took " << src_dur << "msec"
       << endl;
...
std::vector<std::string> &Object::libsRMd() { return rmd_deps; }
...
bool Object::parse_all_relocations(Elf_X &elf, Elf_X_Shdr *dynsym_scnp,
  ...
  for (reg_it = regions_.begin(); reg_it != regions_.end(); ++reg_it) {
    ...
    result = shToRegion.insert(
        std::make_pair((*reg_it)->getRegionNumber(), (*reg_it)));
  }
...
bool Region::isStandardCode() {
  return (
      (getRegionPermissions() == RP_RX || getRegionPermissions() == RP_RWX) &&
      ((name_ == std::string(".text")) || (name_ == std::string(".init")) ||
       (name_ == std::string(".fini"))));
}

void Object::setTruncateLinePaths(bool value) { truncateLineFilenames = value; }

bool Object::getTruncateLinePaths() { return truncateLineFilenames; }
