...
//        }
        ...
        Address low = 0, high = (Dwarf_Addr)(-1);
        if(dwarf_attr(moduleDIE, DW_AT_low_pc, &hasLow, NULL) == DW_DLV_OK &&
            dwarf_attr(moduleDIE, DW_AT_high_pc, &hasHigh, NULL) == DW_DLV_OK)
        {
            Address tempLow, tempHigh;
            if (DwarfWalker::findConstant(DW_AT_low_pc, tempLow, moduleDIE, dbg)
                && DwarfWalker::findConstant(DW_AT_high_pc, tempHigh, moduleDIE, dbg))
            {
                convertDebugOffset(tempLow, low);
                convertDebugOffset(tempHigh, high);
                Dwarf_Half form;
                if(dwarf_whatform(hasHigh, &form, NULL) != DW_DLV_OK) continue;

                if(form != DW_FORM_addr)
                {
                    high += low;
                }
                module_ranges.insert(low, high, moduleName);
            }
        ...
        modules_.push_back(std::pair<std::string, Offset>(moduleName, low));
        dwarf_dealloc(dbg, dwarfModuleName, DW_DLA_STRING);
