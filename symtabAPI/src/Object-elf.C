...
pdelf_check_ehdr(Elf_X& elf)
{
    ...
    return (
        (elf.e_type() == ET_EXEC || elf.e_type() == ET_DYN || elf.e_type() == ET_REL) &&
...
Region::perm_t
getSegmentPerms(unsigned long flags)
{
    if(flags == 7)
...
Region::RegionType
getSegmentType(unsigned long type, unsigned long flags)
{
    if(type == PT_DYNAMIC)
...
Elf_X_Shdr*
Object::getRegionHdrByAddr(Offset addr)
{
...
int
Object::getRegionHdrIndexByAddr(Offset addr)
{
...
Elf_X_Shdr*
Object::getRegionHdrByIndex(unsigned index)
{
...
/* Check if there is a section which belongs to the segment and update permissions of that
 * section. Return value indicates whether the segment has to be added to the list of
 * regions*/
bool
Object::isRegionPresent(Offset segmentStart, Offset segmentSize, unsigned segPerms)
{
    bool present = false;
    for(unsigned i = 0; i < regions_.size(); i++)
    {
        if((regions_[i]->getDiskOffset() >= segmentStart) &&
           ((regions_[i]->getDiskOffset() + regions_[i]->getDiskSize()) <=
...
Region::perm_t
getRegionPerms(unsigned long flags)
{
...
Region::RegionType
getRegionType(unsigned long type, unsigned long flags, const char* reg_name)
{
    switch(type)
    {
        case SHT_DYNSYM:
...
getRelTypeByElfMachine(Elf_X* localHdr)
{
...
bool
Object::loaded_elf(Offset& txtaddr, Offset& dataddr, Elf_X_Shdr*& bssscnp,
                   Elf_X_Shdr*& symscnp, Elf_X_Shdr*& strscnp, Elf_X_Shdr*& stabscnp,
                   Elf_X_Shdr*& stabstrscnp, Elf_X_Shdr*& stabs_indxcnp,
                   Elf_X_Shdr*& stabstrs_indxcnp, Elf_X_Shdr*& rel_plt_scnp,
                   Elf_X_Shdr*& plt_scnp, Elf_X_Shdr*& got_scnp, Elf_X_Shdr*& dynsym_scnp,
                   ...
                   Elf_X_Shdr*& symtab_shndx_scnp, bool)
{
    ...
    for(auto i = 0UL; i < elfHdr->e_shnum(); ++i)
    {
        ...
        if((dynamic_offset_ != 0) && (scn.sh_offset() == dynamic_offset_))
        {
            ...
            {
                // If there are two or more sections with the same offset as the
                // dynamic_offset, use other fields to chose which one the the dynamic
                // section
                if(dynamic_section_size == dynamic_size_ &&
                   dynamic_section_type == SHT_DYNAMIC)
                {
                    // We already have the right section
                }
                else if((scn.sh_size() == dynamic_size_ &&
                         scn.sh_type() == SHT_DYNAMIC) ||
    ...
    for(unsigned int i = 0; i < elfHdr_numSections + elfHdrDbg_numSections; i++)
    {
        ...
        Elf_X_Shdr& scn             = (!isFromDebugFile)
                              ? elfHdr->get_shdr(i)
        ...
        if(!scn.isFromDebugFile())
        {
            ...
            if(scn.sh_flags() & SHF_ALLOC)
            {
                ...
                unsigned long diskSize =
                    (scn.sh_type() == SHT_NOBITS) ? 0 : scn.sh_size();

                Region* reg = new Region(
                    i, name, scn.sh_addr(), diskSize, scn.sh_addr(), scn.sh_size(),
                    ((char*) data.d_buf()), getRegionPerms(scn.sh_flags()),
                    getRegionType(scn.sh_type(), scn.sh_flags(), name), true,
                    ((scn.sh_flags() & SHF_TLS) != 0), scn.sh_addralign());
            ...
            {
                Region* reg =
                    new Region(i, name, scn.sh_addr(), scn.sh_size(), 0, 0,
                               ((char*) data.d_buf()), getRegionPerms(scn.sh_flags()),
                               getRegionType(scn.sh_type(), scn.sh_flags(), name), false,
                               ((scn.sh_flags() & SHF_TLS) != 0), scn.sh_addralign());
  ...
  fini_addr_ = scn.sh_addr();
        ...
        else if(strcmp(name, PLT_NAME) == 0)
        {
            ...
            if(getArch() == Dyninst::Arch_x86 || getArch() == Dyninst::Arch_x86_64)
            {
                ...
                //
                // plt_entry_size_ = plt_size_ / ((rel_plt_size_ / rel_plt_entry_size_) +
                // 1);
                plt_entry_size_ = 16;
            }
            else
                    ...
                    if(scn.sh_flags() & SHF_EXECINSTR)
                    {
                        ...
                        else
                        {
                            if(plt_entry_size_ != 8)
                                create_printf(
                                    "%s[%d]:  weird plt_entry_size_ is %u, not 8\n",
        ...
        else if(strcmp(name, COMMENT_NAME) == 0)
        {
            ...
            while(buf && (index < size))
            {
                ...
                index += comment.size();
                if(comment.size() == 0)
                {  // Skip NULL characters in the comment section
                    index++;
                }
            }
        }
    ...
    {
        if(secAddrTagMapping.find(regions_[j]->getDiskOffset()) !=
           secAddrTagMapping.end())
        {
            secTagRegionMapping[secAddrTagMapping[regions_[j]->getDiskOffset()]] =
                regions_[j];
        }
    }
...
bool
Object::is_offset_in_plt(Offset offset) const
{
    return (offset > plt_addr_ && offset < plt_addr_ + plt_size_);
}

void
Object::parseDynamic(Elf_X_Shdr*& dyn_scnp, Elf_X_Shdr*& dynsym_scnp,
 ...
 */
bool
Object::get_relocationDyn_entries(unsigned rel_scnp_index, Elf_X_Shdr*& dynsym_scnp,
                                  Elf_X_Shdr*& dynstr_scnp)
{
...
bool
Object::get_relocation_entries(Elf_X_Shdr*& rel_plt_scnp, Elf_X_Shdr*& dynsym_scnp,
                               Elf_X_Shdr*& dynstr_scnp)
{
        ...
        if(reldata.isValid() && symdata.isValid() && strdata.isValid())
        {
            ...
            else if(getArch() == Dyninst::Arch_ppc32)
            {
                ...
                if(plt_entry_size_ == 8)
                {
                    ...
                    next_plt_entry_addr +=
                        9 * plt_entry_size_;  // 1st 9 PLT entries are special
                }
                else if(plt_entry_size_ == 16)
                {
                    ...
                    unsigned int*  insn =
                        (unsigned int*) (sec_data + (stub_addr - glink->getMemOffset()));
                    ...
                    while(sec_data < (unsigned char*) insn)
                    {
                        ...
                        if(((back[0] & 0xffff0000) == LWZ_11_30 && back[1] == MTCTR_11 &&
                            back[2] == BCTR)
                    ...
                    if(extraStubs == true)
                    {
                        ...
                        {
                            {
                                dyn_c_hash_map<Offset,
                                               std::vector<Symbol*>>::const_accessor ca;
                ...
                //  actually this is just fudged to make the offset value 72, which is
                //  what binutils uses Note that binutils makes the distinction between
                //  PLT_SLOT_SIZE (8), and PLT_ENTRY_SIZE (12).  PLT_SLOT_SIZE seems to be
                //  what we want, even though we also have PLT_INITIAL_ENTRY_SIZE (72) see
                //  binutils/bfd/elf32-ppc.c/h if more info is needed
                ...
                for(iter = symbols_.begin(); iter != symbols_.end(); ++iter)
                {
                    ...
                    if(name.length() > 8)
                    {
                        ...
                        if(iter->second.size() != 1)
                            continue;
                        dyn_hash_map<string, Offset>::iterator pltrel_iter =
                            plt_rel_map.find(name);
                        if(pltrel_iter == plt_rel_map.end())
                            continue;

                        Symbol*         stub_sym = iter->second[0];
                        relocationEntry re(stub_sym->getOffset(), pltrel_iter->second,
                                           name, targ_sym);
                        fbt_.push_back(re);
                    }
                }
            ...
            {
                next_plt_entry_addr += 4 * (plt_entry_size_);  // 1st 4 entries are
                                                               // special
            }
            ...
            if(sym.isValid() && (rel.isValid() || rela.isValid()) && strs)
            {
                ...
                for(u_int i = 0; i < (rel_plt_size_ / rel_plt_entry_size_); ++i)
                {
                    ...
                    {
                        dyn_c_hash_map<std::string, std::vector<Symbol*>>::const_accessor
                            ca;
                    ...
                    {  // Create new relocation entry.
                        relocationEntry re(next_plt_entry_addr, offset, targ_name, NULL,
                                           type);
                        if(type == R_X86_64_IRELATIVE)
                        {
                            vector<Symbol*> funcs;
                            dyn_c_hash_map<std::string, std::vector<Symbol*>>::iterator
                                iter;
                    ...
                    {  // Update existing relocation entry.
...
void
Object::load_object(bool alloc_syms)
{
    ...
    {  // binding contour (for "goto cleanup")
        ...
        if(!loaded_elf(txtaddr, dataddr, bssscnp, symscnp, strscnp, stabscnp, stabstrscnp,
                       stabs_indxcnp, stabstrs_indxcnp, rel_plt_scnp, plt_scnp, got_scnp,
                       dynsym_scnp, dynstr_scnp, dynamic_scnp, eh_frame_scnp, gcc_except,
                       interp_scnp, opd_scnp, symtab_shndx_scnp, true))
        {
            goto cleanup;
        }
...
//        }
        ...
        if(alloc_syms)
        {
            ...
            if(symscnp && strscnp)
            {
                ...
                parse_symbols(symdata, strdata, bssscnp, symscnp, symtab_shndx_scnp,
                              false, module);
            }
            ...
            // TODO
            // Have a hash on the symbol table. Iterate over dynamic symbol table to check
            // if it exists If yes set dynamic for the symbol ( How to distinguish between
            // symbols only in symtab,
...
static Symbol::SymbolLinkage
pdelf_linkage(int elf_binding)
{
    switch(elf_binding)
    {
        case STB_LOCAL:
...
static Symbol::SymbolVisibility
pdelf_visibility(int elf_visibility)
{
...
// linear search
bool
lookUpSymbol(std::vector<Symbol*>& allsymbols, Offset& addr)
{
...
bool
lookUpAddress(std::vector<Offset>& jumpTargets, Offset& addr)
{
...
void
printSyms(std::vector<Symbol*>& allsymbols)
{
    for(unsigned i = 0; i < allsymbols.size(); i++)
    {
...
void
Object::parse_opd(Elf_X_Shdr* opd_hdr)
{
    ...
    for(sym_it = opdsymbols_.begin(); sym_it != opdsymbols_.end(); ++sym_it)
    {
        for(rel_it = region_rels.begin(); rel_it != region_rels.end(); ++rel_it)
        {
            if((*sym_it)->getPtrOffset() == (*rel_it).rel_addr())
            {
                ...
                while(i < regions_.size())
                {
                    if(regions_[i]->getRegionName().compare(
                           (*rel_it).getDynSym()->getMangledName()) == 0)
                    {
                        Region* targetRegion = regions_[i];
                        Offset  regionOffset =
                            targetRegion->getDiskOffset() + (*rel_it).addend();
                        (*sym_it)->setRegion(targetRegion);
                        (*sym_it)->setOffset(
                            regionOffset);  // Store code address for the function.
                        break;
                    }
                    ++i;
                }
...
Symbol*
Object::handle_opd_symbol(Region* opd, Symbol* sym)
{
    if(!sym)
        return NULL;
...
bool
Object::parse_symbols(Elf_X_Data& symdata, Elf_X_Data& strdata, Elf_X_Shdr* bssscnp,
                      Elf_X_Shdr* symscnp, Elf_X_Shdr* symtab_shndx_scnp, bool /*shared*/,
                      string smodule)
{
#if defined(TIMED_PARSE)
    struct timeval starttime;
...
#pragma omp for schedule(dynamic)
            for(unsigned i = 0; i < syms.count(); i++)
            {
                ...
                if(secNumber == SHN_XINDEX && symtab_shndx_scnp != nullptr)
                {
                    GElf_Sym   symmem;
                    Elf32_Word xndx;
                    gelf_getsymshndx(symdata.elf_data(),
                                     symtab_shndx_scnp->get_data().elf_data(), i, &symmem,
                                     &xndx);
                    secNumber = xndx;
                }
...
#pragma omp critical
          ...
          */
                ...
                Symbol* newsym = new Symbol(
                    sname, stype, slinkage, svisibility, soffset, NULL, sec, ssize, false,
...
void
Object::parse_dynamicSymbols(Elf_X_Shdr*& dyn_scnp, Elf_X_Data& symdata,
                             Elf_X_Data& strdata, bool /*shared*/, std::string smodule)
{
#if defined(TIMED_PARSE)
    struct timeval starttime;
    gettimeofday(&starttime, NULL);
    ...
    for(unsigned i = 0; i < verdefnum; i++)
    {
        ...
        {
            if(strs)
                versionMapping[symVersionDefs->vd_ndx()].push_back(
                    &strs[aux->vda_name()]);
    ...
    for(unsigned i = 0; i < verneednum; i++)
    {
        ...
        for(unsigned j = 0; j < symVersionNeeds->vn_cnt(); j++)
        {
            ...
            if(strs)
                versionFileNameMapping[aux->vna_other()] =
                    &strs[symVersionNeeds->vn_file()];
    ...
    if(syms.isValid())
    {
        for(unsigned i = 0; i < syms.count(); i++)
        {
            ...
            Symbol* newsym = new Symbol(
                sname, stype, slinkage, svisibility, soffset, NULL, sec, ssize,
                true,  // is dynamic
                (secNumber == SHN_ABS), ind, strindex, (secNumber == SHN_COMMON));
...
string
Object::find_symbol(string name)
{
    string name2;
...
bool
Object::dwarf_parse_aranges(Dwarf* dbg, std::set<Dwarf_Off>& /*dies_seen*/)
{
    ...
    for(size_t i = 0; i < num_ranges; i++)
    {
        ...
        cerr << "File in module " << modname << ", DIE CU " << hex << off_die << dec
             << endl;
...
Object::fix_global_symbol_modules_static_dwarf()
{
    ...
    for(size_t i = 0; i < dies.size(); i++)
    {
        ...
        // cerr << "Processing CU DIE for " << modname << " offset: " << next_cu_off <<
        // endl;
        Address tempModLow;
        Address modLow = 0;
...
#    pragma omp critical
        ...
        std::vector<AddressRange> mod_ranges =
            DwarfWalker::getDieRanges(dbg, cu_die, modLow);
        Module* m;
#    pragma omp critical
        ...
        if(!m->hasRanges())
        {
            //            cout << "No ranges for module " << modname << ", need to extract
            //            from statements\n";
            Dwarf_Lines* lines;
            size_t       num_lines;
...
bool
Object::fix_global_symbol_modules_static_stab(Elf_X_Shdr* stabscnp,
                                              Elf_X_Shdr* stabstrscnp)
{
    ...
    if(!stabdata.isValid() || !stabstrdata.isValid())
        return false;

    switch(addressWidth_nbytes)
    {
        case 4:
            stabptr = new stab_entry_32(stabdata.d_buf(), stabstrdata.get_string(),
                                        stabscnp->sh_size() / sizeof(stab32));
            break;

        case 8:
            stabptr = new stab_entry_64(stabdata.d_buf(), stabstrdata.get_string(),
...
void
Object::find_code_and_data(Elf_X& elf, Offset txtaddr, Offset dataddr)
{
    /* Note:
    ...
    */
    ...
    // if (addressWidth_nbytes == 8) bperr( ">>> 64-bit find_code_and_data()
    // successful\n");
}

const char*
Object::elf_vaddr_to_ptr(Offset vaddr) const
{
    const char* ret        = NULL;
    ...
    if(stab_off_ && stab_size_ && stabstr_off_)
    {
        switch(addressWidth_nbytes)
        {
            case 4:  // 32-bit object
                return new stab_entry_32(file_ptr + stab_off_, file_ptr + stabstr_off_,
                                         stab_size_ / sizeof(stab32));
                break;
            case 8:  // 64-bit object
                return new stab_entry_64(file_ptr + stab_off_, file_ptr + stabstr_off_,
...
Object::Object(MappedFile* mf_, bool, void (*err_func)(const char*), bool alloc_syms,
               Symtab* st)
...
void
Object::log_elferror(void (*err_func)(const char*), const char* msg)
{
...
bool
Object::get_func_binding_table(std::vector<relocationEntry>& fbt) const
{
...
bool
Object::get_func_binding_table_ptr(const std::vector<relocationEntry>*& fbt) const
{
    if(!plt_addr_ || (!fbt_.size()))
        return false;
    fbt = &fbt_;
...
void
Object::getDependencies(std::vector<std::string>& deps)
{
    deps = deps_;
}

bool
Object::addRelocationEntry(relocationEntry& re)
{
    relocation_table_.push_back(re);
    return true;
}
...
const ostream&
Object::dump_state_info(ostream& s)
{
...
Offset
Object::getPltSlot(string funcName) const
{
    relocationEntry re;
    Offset          offset = 0;
...
void
Object::get_valid_memory_areas(Elf_X& elf)
{
...
read_uleb128(const unsigned char* data, unsigned* bytes_read)
{
...
static signed long
read_sleb128(const unsigned char* data, unsigned* bytes_read)
{
    unsigned long result = 0;
    unsigned      shift  = 0;
    *bytes_read          = 0;
...
static uint16_t
endian_16bit(const uint16_t value, bool big)
{
    if(big)
...
static uint32_t
endian_32bit(const uint32_t value, bool big)
{
    if(big)
...
static uint64_t
endian_64bit(const uint64_t value, bool big)
{
    if(big)
...
static int
read_val_of_type(int type, unsigned long* value, const unsigned char* addr,
                 const mach_relative_info& mi)
{
    ...
    switch(type & 0x0f)
    {
        case DW_EH_PE_absptr:
            ...
            {
                *value =
                    (unsigned long) endian_32bit(*((const uint32_t*) addr), mi.big_input);
                size = 4;
            }
            else if(mi.word_size == 8)
            {
                *value =
                    (unsigned long) endian_64bit(*((const uint64_t*) addr), mi.big_input);
                size = 8;
            }
            break;
...
static int
read_except_table_gcc3(const unsigned char e_ident[], mach_relative_info& mi,
                       ...
                       std::vector<ExceptionBlock>& addresses)
{
    ...
    do
    {
        ...
        Dwarf_Off       unused;
        dwarf_next_cfi(e_ident, eh_frame_data, true, entry.fde.CIE_pointer, &unused,
                       &thisCIE);
        ...
        for(j = 0; j < augmentor_len; j++)
        {
            ...
            else if(augmentor[j] == 'P')
            {
                ...
                cur_augdata += read_val_of_type(personality_encoding, &personality_val,
                                                cur_augdata, mi);
            }
        ...
        int pc_begin_size =
            read_val_of_type(range_encoding, &pc_begin_val, pc_begin_start, mi);
        ...
        int pc_range_size =
            read_val_of_type(range_encoding, &pc_range_val, pc_range_start, mi);
        ...
        auto aug_length_size =
            read_val_of_type(DW_EH_PE_uleb128, &aug_length_value, aug_length_start, mi);
        ...
        {
            except_off += read_val_of_type(DW_EH_PE_uleb128, &landingpad_base,
                                           datap + except_off, mi);
        }
        else
        {
        ...
        if(ttype_format != DW_EH_PE_omit)
            except_off +=
                read_val_of_type(DW_EH_PE_uleb128, &value, datap + except_off, mi);
        ...
        while(except_off < table_end && except_off < except_size)
        {
            ...
            except_off +=
                read_val_of_type(table_format, &region_start, datap + except_off, mi);
            mi.pc  = except_scn->sh_addr() + except_off;
            tryEnd = except_off;
            // cerr << "Reading tryEnd at " << except_off;
            except_off +=
                read_val_of_type(table_format, &region_size, datap + except_off, mi);
            mi.pc      = except_scn->sh_addr() + except_off;
            catchStart = except_off;
            // cerr << "Reading catchStart at " <<  except_off;
            except_off +=
                read_val_of_type(table_format, &catch_block, datap + except_off, mi);
            // cerr << "Reading action (uleb128) at " << except_off;
            except_off +=
                read_val_of_type(DW_EH_PE_uleb128, &action, datap + except_off, mi);
...
: public binary_function<const ExceptionBlock&, const ExceptionBlock&, bool>
{
 ...
 **/
bool
Object::find_catch_blocks(Elf_X_Shdr* eh_frame, Elf_X_Shdr* except_scn, Address txtaddr,
                          Address dataaddr, std::vector<ExceptionBlock>& catch_addrs)
{
    bool result = false;
    ...
    Elf_X* elfHdrDbg = dwarf->debugLinkFile();
    auto   e_ident =
        (!eh_frame->isFromDebugFile()) ? elfHdr->e_ident() : elfHdrDbg->e_ident();
...
ObjectType
Object::objType() const
{
    return obj_type_;
}

void
Object::getModuleLanguageInfo(dyn_hash_map<string, supportedLanguages>* mod_langs)
{
    ...
    for(unsigned int i = 0; i < stabptr->count(); i++)
    {
        ...
        {
            //  We can use the compiler option string (in a pinch) to guess at the source
            //  file language There is possibly more useful information encoded somewhere
            //  around here, but I lack an immediate reference....
            if(working_name)
        ...
        { /* compilation source or file name */
            // We have arrived at the next source file, finish up with the last one and
            // reset state before starting next

            //   XXXXXXXXXXX  This block is mirrored near the end of routine, if you edit
            //   it,
            //   XXXXXXXXXXX  change it there too.
            if(working_name)
            {
                working_lang =
                    pickLanguage(working_module, working_options, working_lang);
            ...
            if((mod_langs->find(working_module) != mod_langs->end()) &&
               (*mod_langs)[working_module] != lang_Unknown)
            {
                //  we already have a module with this name in the map.  If it has been
                //  given a language assignment (not lang_Unknown), we can just skip ahead
                working_name    = NULL;
                working_options = NULL;
                continue;
            }
            ...
            {
                // cerr << __FILE__ << __LINE__ << ":  Module: " <<working_module<< " has
                // language "<< stabptr->desc(i) << endl;
                switch(stabptr->desc(i))
                {
                    case N_SO_FORTRAN:
                        ...
                        break;
                    case N_SO_F90:
                        working_lang = lang_Fortran;  // not sure if this should be
                                                      // different from N_SO_FORTRAN
                        break;
                    case N_SO_AS:
                    ...
                    case N_SO_CC:
                        ...
                        break;
                    default:
                        //  currently uncovered options are lang_CMFortran, and
                        //  lang_GnuCPlusPlus do we need to make this kind of distinction
                        //  here?
                        working_lang = lang_Unknown;
                        break;
                }
    ...
    unsigned long difftime   = lendtime - lstarttime;
    double        dursecs    = difftime / (1000);
    cout << __FILE__ << ":" << __LINE__ << ": getModuleLanguageInfo took " << dursecs
         << " msecs" << endl;
#endif
    delete stabptr;
    ...
    if(hasDwarfInfo())
    {
        ...
        size_t cu_header_size;
        for(Dwarf_Off cu_off = 0, next_cu_off;
            dwarf_nextcu(dbg, cu_off, &next_cu_off, &cu_header_size, NULL, NULL, NULL) ==
            0;
            cu_off = next_cu_off)
        {
            Dwarf_Off  cu_die_off = cu_off + cu_header_size;
...
bool
AObject::getSegments(vector<Segment>& segs) const
{
    unsigned i;
    for(i = 0; i < regions_.size(); i++)
    {
        if((regions_[i]->getRegionName() == ".text") ||
           (regions_[i]->getRegionName() == ".init") ||
           (regions_[i]->getRegionName() == ".fini") ||
           (regions_[i]->getRegionName() == ".rodata") ||
           (regions_[i]->getRegionName() == ".plt") ||
...
bool
Object::emitDriver(string fName, std::set<Symbol*>& allSymbols, unsigned)
{
#ifdef BINEDIT_DEBUG
    printf("emitting...\n");
    // print_symbol_map(&symbols_);
    ...
    if(elfHdr->e_ident()[EI_CLASS] == ELFCLASS32)
    {
        Dyninst::SymtabAPI::emitElf<Dyninst::SymtabAPI::ElfTypes32>* em =
            new Dyninst::SymtabAPI::emitElf<Dyninst::SymtabAPI::ElfTypes32>(
                elfHdr, isStripped, this, err_func_, associated_symtab);
    ...
    else if(elfHdr->e_ident()[EI_CLASS] == ELFCLASS64)
    {
        Dyninst::SymtabAPI::emitElf<Dyninst::SymtabAPI::ElfTypes64>* em =
            new Dyninst::SymtabAPI::emitElf<Dyninst::SymtabAPI::ElfTypes64>(
                elfHdr, isStripped, this, err_func_, associated_symtab);
        bool ok = em->createSymbolTables(allSymbols);
        if(ok)
        {
    ...
    for(unsigned int i = 0; i < stabEntry->count(); i++)
    {
        switch(stabEntry->type(i))
        {
            ...
            case N_FUN: /* a function */
            {
                ...
                if(!associated_symtab->findFunctionsByName(funcs,
            ...
            case N_SLINE: {
                    ...
                    if(functionLineToPossiblyAdd < newLineSpec)
                    {
                        if(li_for_module)
                            li_for_module->addLine(currentSourceFile,
                                                   functionLineToPossiblyAdd, current_col,
                ...
                if(li_for_module)
                    li_for_module->addLine(currentSourceFile, newLineSpec, current_col,
                                           currentAddress, newLineAddress);
...
public:
    ...
    bool       uninitialized() { return start_addr == noAddress(); }
    void       reset()
    {
    ...
    bool sameFileLineColumn(const open_statement& rhs)
    {
        return ((string_table_index == rhs.string_table_index) &&
                (line_number == rhs.line_number) && (column_number == rhs.column_number));
    }
    ...
    {
        os << hex << st.start_addr << " " << st.end_addr << " line:" << dec
           << st.line_number << " file:" << st.string_table_index
           << " col:" << st.column_number << std::endl;
        return os;
    }
...
void
Object::parseLineInfoForCU(Dwarf_Die cuDIE, LineInformation* li_for_module)
{
        ...
        else
        {
            ...
            if(!current_line.sameFileLineColumn(current_statement) || isEndOfSequence)
            {
                auto success = li_for_module->addLine(
                    (unsigned int) (current_line.string_table_index),
                    (unsigned int) (current_line.line_number),
                    (unsigned int) (current_line.column_number), current_line.start_addr,
                    current_line.end_addr);
                lineinfo_printf("[%lu, %lu) %s:%d %s\n", current_line.start_addr,
                                current_line.end_addr,
                                ((*strings)[current_line.string_table_index]).str.c_str(),
...
LineInformation*
Object::parseLineInfoForObject(StringTablePtr strings_)
{
    if(li_for_object)
    {
    ...
    while((status = dwarf_next_lines(dbg, off = next_off, &next_off, &cu, &files,
                                     &fileCount, &lineBuffer, &lineCount)) == 0)
    {
        ...
        for(size_t i = 0; i < lineCount; i++)
        {
            ...
            if(dwarf->debugLinkFile())
            {
                Offset new_lineAddr;
                bool   result =
                    convertDebugOffset(current_statement.start_addr, new_lineAddr);
                if(result)
                    current_statement.start_addr = new_lineAddr;
            }
...
void
Object::parseLineInfoForAddr(Offset addr_to_find)
{
    ...
    for(auto mod = mod_for_offset.begin(); mod != mod_for_offset.end(); ++mod)
    {
...
Object::parseTypeInfo()
{
    ...
    unsigned long difftime   = lendtime - lstarttime;
    double        dursecs    = difftime / (1000);
    cout << __FILE__ << ":" << __LINE__ << ": parseTypes(" << obj->file() << ") took "
         << dursecs << " msecs" << endl;
#endif
}

void
Object::parseStabTypes()
{
    ...
    currentFuncInfo = NULL;
      ...
      */
            ...
            case N_SO: /* compilation source or file name */
                ...
                // src_dur += (t2.tv_sec/1000 + t2.tv_usec*1000) - (t1.tv_sec/1000 +
                // t1.tv_usec*1000) ;
#endif
                break;
            case N_SLINE:
        ...
        if(parseActive || !is_aout())
        {
            ...
            switch(stabptr->type(i))
            {
                case N_FUN:
                    ...
                    gettimeofday(&t1, NULL);
#endif
                    // all we have to do with function stabs at this point is to assure
                    // that we have properly set the var currentFunctionName for the later
                    // case of (parseActive)
                    symt_current_func = NULL;
                    int currentEntry  = i;
                    ...
                    else
                    {
                        ...
                        if(!associated_symtab->findSymbol(syms, *currentFunctionName,
                                                          Symbol::ST_FUNCTION,
                                                          mangledName))
                        {
                            if(!associated_symtab->findSymbol(
                                   syms, "_" + *currentFunctionName, Symbol::ST_FUNCTION,
                                   mangledName))
                            {
                                string fortranName = *currentFunctionName + string("_");
                                if(associated_symtab->findSymbol(syms, fortranName,
                                                                 Symbol::ST_FUNCTION,
                                                                 mangledName))
                                {
                                    delete currentFunctionName;
...
#ifdef TIMED_PARSE
                    gettimeofday(&t2, NULL);
                    fun_dur += (t2.tv_sec - t1.tv_sec) * 1000.0 +
                               (t2.tv_usec - t1.tv_usec) / 1000.0;
                    // fun_dur += (t2.tv_sec/1000 + t2.tv_usec*1000) - (t1.tv_sec/1000 +
                    // t1.tv_usec*1000);
#endif
                    break;
            }
            ...
            switch(stabptr->type(i))
            {
                case N_BCOMM: {
                    ...
                    if(!associated_symtab->findSymbol(vars, *commonBlockName,
                                                      Symbol::ST_OBJECT, mangledName))
                    {
                        if(!associated_symtab->findSymbol(vars, *commonBlockName,
                                                          Symbol::ST_OBJECT, mangledName,
                                                          true))
                ...
                case N_ECOMM: {
                    ...
                    if(!currentFunctionName)
                        break;
                    if(!associated_symtab->findSymbol(bpfv, *currentFunctionName,
                                                      Symbol::ST_FUNCTION, mangledName))
                    {
                        if(!associated_symtab->findSymbol(bpfv, *currentFunctionName,
                                                          Symbol::ST_FUNCTION,
                                                          mangledName, true))
                        ...
                        {
                            Symbol* func = bpfv[0];
                            commonBlock->asCommonType().endCommonBlock(
                                func, (void*) commonBlockVar->getOffset());
                        }
                    }
                    else
                    {
                        if(bpfv.size() > 1)
                        {
                            ...
                            // //bperr("%s[%d]:  WARNING: found %d funcs matching name %s,
                            // using the first\n",
                ...
                case 0xc8:  // position-independant external typedefs -- N_ESYM
                    ...
                    temp = parseStabString(mod, stabptr->desc(i), (char*) ptr,
                                           stabptr->val(i), &commonBlock->asCommonType());
                    if(temp.length())
                    {
...
#ifdef TIMED_PARSE
                    gettimeofday(&t2, NULL);
                    pss_dur += (t2.tv_sec - t1.tv_sec) * 1000.0 +
                               (t2.tv_usec - t1.tv_usec) / 1000.0;
                    //      pss_dur += (t2.tv_sec/1000 + t2.tv_usec*1000) -
    ...
    unsigned long difftime   = lendtime - lstarttime;
    double        dursecs    = difftime / (1000);
    cout << __FILE__ << ":" << __LINE__ << ": parseTypes(" << mod->fileName() << ") took "
         << dursecs << " msecs" << endl;
    cout << "Breakdown:" << endl;
    cout << "     Functions: " << fun_count << " took " << fun_dur << "msec" << endl;
    cout << "     Sources: " << src_count << " took " << src_dur << "msec" << endl;
    cout << "     parseStabString: " << pss_count << " took " << pss_dur << "msec"
         << endl;
    cout << "     Total: " << pss_dur + fun_dur + src_dur << " msec" << endl;
#endif
}

bool
sort_dbg_map(const Object::DbgAddrConversion_t& a, const Object::DbgAddrConversion_t& b)
{
    return (a.dbg_offset < b.dbg_offset);
}

bool
Object::convertDebugOffset(Offset off, Offset& new_off)
{
...
void
Object::insertPrereqLibrary(std::string libname)
{
    prereq_libs.insert(libname);
}

bool
Object::removePrereqLibrary(std::string libname)
{
...
void
Object::insertDynamicEntry(long name, long value)
{
...
// existing symbols
bool
Object::parse_all_relocations(Elf_X_Shdr* dynsym_scnp, Elf_X_Shdr* dynstr_scnp,
                              Elf_X_Shdr* symtab_scnp, Elf_X_Shdr* strtab_scnp)
{
    // fprintf(stderr, "enter parse_all_relocations for object %s\n",
    // getFileName().c_str() ); const char *shnames = pdelf_get_shnames(*elfHdr);
    ...
    for(symVec_it = symbols_.begin(); symVec_it != symbols_.end(); ++symVec_it)
    {
        ...
        for(sym_it = symVec_it->second.begin(); sym_it != symVec_it->second.end();
            ++sym_it)
        {
            ...
            if((*sym_it)->isInDynSymtab())
            {
                result = dynsymByIndex.insert(
                    std::make_pair((*sym_it)->getIndex(), (*sym_it)));
            }
            else
            {
                result = symtabByIndex.insert(
                    std::make_pair((*sym_it)->getIndex(), (*sym_it)));
            }
    ...
    for(reg_it = regions_.begin(); reg_it != regions_.end(); ++reg_it)
    {
        ...
        result =
            shToRegion.insert(std::make_pair((*reg_it)->getRegionNumber(), (*reg_it)));
    }
        ...
        for(unsigned j = 0; j < (shdr->sh_size() / shdr->sh_entsize()); ++j)
        {
            ...
            switch(shdr->sh_type())
            {
                ...
                case SHT_RELA:
                    ...
                    // fprintf(stderr, "Handle RELA symbol_index %d, type is
            ...
            if(dynstr && curSymHdr && curSymHdr->sh_offset() == dynsym_offset)
            {
                ...
                //		fprintf(stderr, "find DYN relocation for %s, rel offset %lx,
            ...
            else if(strtab && curSymHdr && curSymHdr->sh_offset() == symtab_offset)
            {
                ...
                //		fprintf(stderr, "find relocation for %s, rel offset %lx, addend
                //%lx\n", name.c_str(), relOff, addend);
                dyn_hash_map<int, Symbol*>::iterator sym_it;
...
Region::isStandardCode()
{
    return ((getRegionPermissions() == RP_RX || getRegionPermissions() == RP_RWX) &&
            ((name_ == std::string(".text")) || (name_ == std::string(".init")) ||
             (name_ == std::string(".fini"))));
}

void
Object::setTruncateLinePaths(bool value)
{
    truncateLineFilenames = value;
}

bool
Object::getTruncateLinePaths()
{
    return truncateLineFilenames;
}

Dyninst::Architecture
Object::getArch() const
{
    return elfHdr->getArch();
}

bool
Object::getABIVersion(int& major, int& minor) const
{
...
bool
Object::isBigEndianDataEncoding() const
{
    return (elfHdr->e_endian() != 0);
}

Offset
Object::getTOCoffset(Offset off) const
{
    if(TOC_table_.empty())
        return 0;
...
void
Object::setTOCoffset(Offset off)
{
    TOC_table_.clear();
    TOC_table_[0] = off;
}
...
void
Object::getSegmentsSymReader(vector<SymSegment>& segs)
{
...
//   elfclassify.c
bool
Object::isLoadable() const
{
    return (objType() == obj_Executable || objType() == obj_SharedLib) &&
           hasProgramLoad() && (no_of_sections() == 0 || hasBitsAlloc());
}
...
//   elfclassify.c
bool
Object::isOnlySharedLibrary() const
{
    if(!isLoadable())
    {
        return false;
...
bool
Object::isLinuxKernelModule() const
{
