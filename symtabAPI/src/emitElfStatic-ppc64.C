...
unsigned int
setBits(unsigned int target, unsigned int pos, unsigned int len, unsigned int value)
{
...
#define UNKNOWN_ADDRESS_WIDTH_ASSERT                                                     \
    "An unknown address width was encountered, can't continue"
...
static bool
computeCtorDtorAddress(relocationEntry& rel, Offset globalOffset, LinkMap& lmap,
                       string& errMsg, Offset& symbolOffset)
{
    if(rel.name() == SYMTAB_CTOR_LIST_REL)
    {
        ...
        else
        {
            ...
            rewrite_printf(
                "Failed to locate original .ctors Region -- cannot apply relocation\n");
            return false;
        }
    }
    else if(rel.name() == SYMTAB_DTOR_LIST_REL)
    {
        ...
        else
        {
            ...
            rewrite_printf(
                "Failed to locate original .dtors Region -- cannot apply relocation\n");
            return false;
        }
    }
    return true;
...
bool
emitElfStatic::archSpecificRelocation(Symtab* targetSymtab, Symtab* srcSymtab,
                                      char* targetData, relocationEntry& rel, Offset dest,
                                      Offset relOffset, Offset globalOffset,
                                      LinkMap& lmap, string& errMsg)
{
    if(PPC64_WIDTH == addressWidth_)
    {
        ...
        rewrite_printf(" archSpecificRelocation %s\ndynsym %s, reloc offset 0x%lx\n new "
                       "TOC 0x%lx, cur TOC 0x%lx\n dest 0x%lx \n",
                       rel.name().c_str(), dynsym->getMangledName().c_str(), relOffset,
        ...
        if(rel.getRelType() == R_PPC64_JMP_SLOT)
        {
            ...
            if(!archSpecificRelocation(targetSymtab, srcSymtab, targetData, func, dest,
            ...
            if(!archSpecificRelocation(targetSymtab, srcSymtab, targetData, toc, dest + 8,
                                       relOffset + 8, globalOffset, lmap, errMsg))
            {
                return false;
            }
            return true;
        ...
        if(rel.getRelType() == R_PPC64_REL24 && newTOCoffset != curTOCoffset &&
           handleInterModuleSpecialCase(targetSymtab, srcSymtab, lmap, targetData, rel,
                                        newTOCoffset, curTOCoffset, dest, relOffset,
                                        globalOffset))
        {
            return true;
        }
                ...
                ++region_it)
            {
                if((*region_it)->getRegionName().compare(".toc") == 0)
                {
                    map<Region*, LinkMap::AllocPair>::iterator result =
                        lmap.regionAllocs.find(*region_it);
        ...
        rewrite_printf(
            "\trelocation for '%s': TYPE = %s(%lu) S = %lx A = %x P = %lx Total 0x%lx \n",
            rel.name().c_str(),
        ...
        switch(rel.getRelType())
        {
            ...
            case R_PPC64_GOT_TPREL16_DS:
                ...
                // rewrite_printf(" R_PPC64_REL24 S = 0x%lx A = %d relOffset = 0x%lx
                // relocation without shift 0x%lx %ld \n", symbolOffset, addend,
                // relOffset, symbolOffset + addend - relOffset, symbolOffset + addend -
                // relOffset);
                break;
        ...
        rewrite_printf(
            "\tbefore: relocation = 0x%lx @ 0x%lx target data %lx %lx %lx %lx %lx %lx \n",
            (unsigned long) relocation, relOffset, targetData[dest - 2],
            targetData[dest - 1], targetData[dest], targetData[dest + 1],
            targetData[dest + 2], targetData[dest + 3]);
        ...
        rewrite_printf(
            "\tafter: relocation = 0x%lx @ 0x%lx target data %lx %lx %lx %lx %lx %lx \n",
            (unsigned long) relocation, relOffset, targetData[dest - 2],
            targetData[dest - 1], targetData[dest], targetData[dest + 1],
            targetData[dest + 2], targetData[dest + 3]);
    ...
    else if(PPC32_WIDTH == addressWidth_)
    {
        ...
        switch(rel.getRelType())
        {
            ...
            case R_PPC_ADDR16_HA: /*       6        adjusted high 16bit */
                ...
                relocation        = symbolOffset + addend;
                relocation =
                    (((relocation >> 16) + ((relocation & 0x8000) ? 1 : 0)) & 0xffff);
                break;
            ...
            case R_PPC_SECTOFF_HA: /*      36*/
                ...
                errMsg = tmp.str();
                rewrite_printf(
                    " Relocation type %s  currently unimplemented \n",
                    relocationEntry::relType2Str(rel.getRelType(), addressWidth_));
                return false;
        ...
        rewrite_printf(" relocation = 0x%lx @ 0x%lx target data 0x%lx %lx %lx %lx \n",
                       relocation, relOffset, targetData[dest], targetData[dest + 1],
                       targetData[dest + 2], targetData[dest + 3]);
...
bool
emitElfStatic::checkSpecialCaseSymbols(Symtab*, Symbol*)
{
    return true;
}
...
Offset
emitElfStatic::layoutTLSImage(Offset globalOffset, Region* dataTLS, Region* bssTLS,
                              LinkMap& lmap)
{
...
Offset
emitElfStatic::adjustTLSOffset(Offset curOffset, Offset /*tlsSize*/)
{
    return curOffset;
}
...
void
emitElfStatic::cleanupTLSRegionOffsets(map<Region*, LinkMap::AllocPair>& regionAllocs,
...
Offset
emitElfStatic::getGOTSize(Symtab* /*target*/, LinkMap& lmap, Offset& /*layoutStart*/)
{
    Offset size = 0;
    ...
    {
        rewrite_printf("Determining new GOT size: %lu symtab entries, %lu reserved "
                       "slots, %u slot size\n",
...
Offset
emitElfStatic::getGOTAlign(LinkMap&)
{
    if(PPC32_WIDTH == addressWidth_)
    {
        return 0;
...
void
emitElfStatic::buildGOT(Symtab* /*target*/, LinkMap& lmap)
{
    if(PPC32_WIDTH == addressWidth_)
    {
        return;
    ...
    for(sym_it = lmap.gotSymbolTable.begin(); sym_it != lmap.gotSymbolTable.end();
        ++sym_it)
    {
        ...
        rewrite_printf(" Building GOT at 0x%lx is 0x%lx name %s Offset 0x%lx addend "
                       "0x%lx addr 0x%lx \n",
                       curOffset, value, sym_it->first->getPrettyName().c_str(),
                       sym_it->first->getOffset(), sym_it->second,
                       lmap.gotRegionOffset + curOffset);
        curOffset += slotSize;
    }
...
Offset
emitElfStatic::layoutNewCtorRegion(LinkMap& lmap)
{
    /*
    ...
    for(reg_it = lmap.newCtorRegions.begin(); reg_it != lmap.newCtorRegions.end();
        ++reg_it)
    {
    ...
    if(lmap.originalCtorRegion != NULL)
    {
        ...
        retOffset +=
            lmap.originalCtorRegion->getDiskSize() - addressWidth_ - addressWidth_;
    }
    retOffset += addressWidth_;
...
bool
emitElfStatic::createNewCtorRegion(LinkMap&)
{
    return true;
}

Offset
emitElfStatic::layoutNewDtorRegion(LinkMap& lmap)
{
    /*
    ...
    if(lmap.originalDtorRegion != NULL)
    {
        ...
        retOffset +=
            lmap.originalDtorRegion->getDiskSize() - addressWidth_ - addressWidth_;
    }

    set<Region*, CtorComp<Region*>>::iterator reg_it;
    for(reg_it = lmap.newDtorRegions.begin(); reg_it != lmap.newDtorRegions.end();
        ++reg_it)
    {
        result = lmap.regionAllocs.insert(make_pair(*reg_it, make_pair(0, retOffset)));
...
bool
emitElfStatic::createNewDtorRegion(LinkMap&)
{
    return true;
}

bool
emitElfStatic::isConstructorRegion(Region* reg)
{
    return (CTOR_NAME.compare(reg->getRegionName()) == 0);
}

bool
emitElfStatic::isDestructorRegion(Region* reg)
{
    return (DTOR_NAME.compare(reg->getRegionName()) == 0);
}

bool
emitElfStatic::isGOTRegion(Region* reg)
{
    if(PPC32_WIDTH == addressWidth_)
    {
        return false;
...
bool
emitElfStatic::isGOTRelocation(unsigned long relType)
{
    if(PPC32_WIDTH == addressWidth_)
    {
        switch(relType)
...
void
emitElfStatic::getExcludedSymbolNames(set<string>&)
{}

bool
emitElfStatic::updateTOC(Symtab* file, LinkMap& lmap, Offset globalOffset)
{
    ...
    Address newTOC =
        GOTbase + 0x8000 + globalOffset;  // Convention is to use +8k to split the
    ...
    rewrite_printf("Set TOC to 0x%lx = 0x%lx + 0x%lx + 0x%lx; old 0x%lx\n", newTOC,
                   GOTbase, 0x8000UL, globalOffset, oldTOC);
...
Offset
emitElfStatic::allocStubRegions(LinkMap& lmap, Offset globalOffset)
{
    ...
    // So. Relocation time.
    // We can't just count relocations because there are TOC setting relocs and we'll
    // massively overapproximate. Instead, count of type R_PPC64_REL24. Also, unique.

    Offset relStubOffset = lmap.stubRegionOffset;
    rewrite_printf("Allocating stub regions for inter-module calls\n");

    for(deque<Region*>::iterator reg_iter = lmap.codeRegions.begin();
        reg_iter != lmap.codeRegions.end(); ++reg_iter)
    {
        vector<relocationEntry> region_rels = (*reg_iter)->getRelocations();
        rewrite_printf(
            "\t For region %s in %s, %lu relocations, %lu unique symbols so far\n",
            (*reg_iter)->getRegionName().c_str(), (*reg_iter)->symtab()->name().c_str(),
            region_rels.size(), lmap.stubMap.size());
        for(vector<relocationEntry>::iterator rel_iter = region_rels.begin();
            rel_iter != region_rels.end(); ++rel_iter)
        {
    ...
    rewrite_printf(
        "\t After regions, returning offset 0x%lx = current 0x%lx + stub area 0x%lx\n",
        lmap.stubRegionOffset + (16 * lmap.stubMap.size()), lmap.stubRegionOffset,
        16 * lmap.stubMap.size());
    return relStubOffset;
}

bool
emitElfStatic::handleInterModuleSpecialCase(Symtab* /*target*/, Symtab* /*src*/,
                                            LinkMap& lmap, char* data,
                                            relocationEntry rel, Offset newTOC,
                                            Offset oldTOC, Offset dest, Offset relOffset,
                                            Offset globalOffset)
{
    // This isn't that bad :)
    //
    ...
    Offset stubOffset =
        findOrCreateStub(rel.getDynSym(), lmap, newTOC, oldTOC, data, globalOffset);
    Offset branchOffset = stubOffset - relOffset;
    rewrite_printf("Handling TOC set stub, using stub offset 0x%lx, curr addr 0x%lx so "
                   "branch is 0x%lx \n",
...
Offset
emitElfStatic::findOrCreateStub(Symbol* sym, LinkMap& lmap, Offset newTOC, Offset oldTOC,
                                char* data, Offset globalOffset)
{
    // Easy case
    std::map<Symbol*, TOCstub>::iterator iter = stubMap.find(sym);
    if(iter != stubMap.end() && getNewTOC(iter->second) == newTOC &&
...
void
emitElfStatic::createStub(unsigned* stub, Offset stubOffset, Offset newTOC, Offset oldTOC,
                          Offset dest)
{
    ...
    rewrite_printf(
        ...
        "0x%x (from 0x%lx - 0x%lx),\n\tbranch offset 0x%x (from 0x%lx - 0x%lx)\n",
        deltaTOChi, newTOC, oldTOC, deltaTOClo, newTOC, oldTOC, deltaAddr, dest,
        stubOffset);
...
bool
emitElfUtils::updateRelocation(Symtab* obj, relocationEntry& rel, int library_adjust)
{
    ...
    // because it must initialize itself without relying on other programs to relocate its
    // memory image."

    // In order to implement this, would have determine the final address of a new
    // .dynamic section before outputting the patched GOT data -- this will require some
    // refactoring.

    // rewrite_printf("WARNING: updateRelocation is not implemented on this
