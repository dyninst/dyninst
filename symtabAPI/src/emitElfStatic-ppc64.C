...
/*
 * holds architecture specific functions for x86 and x86_64 architecture needed
 * for the
...
unsigned int setBits(unsigned int target, unsigned int pos, unsigned int len,
                     unsigned int value) {
...
unsigned long setBits64(unsigned long target, unsigned int pos,
                        unsigned int len, unsigned long value) {
  unsigned long mask;
  mask = ~(~0 << len);
  mask = (mask << pos);
  target = target & mask;
...
/**
 *
 * Given a relocation, determines if the relocation corresponds to a .ctors or
 * .dtors
 * table that requires special consideration. Modifies the passed symbol offset
 * to
 * point to the right table, if applicable.
 *
 * rel          The relocation entry to examine
 * globalOffset The offset of the linked code (used for symbol offset
 * calculation)
...
static bool computeCtorDtorAddress(relocationEntry &rel, Offset globalOffset,
  ...
  if (rel.name() == SYMTAB_CTOR_LIST_REL) {
    ...
    if (lmap.newCtorRegions.size() > 0) {
      ...
      rewrite_printf(
          "original CTOR computeCtorDtorAddress symbolOffset 0x%lx \n",
          symbolOffset);
    } else {
      errMsg =
          "Failed to locate original .ctors Region -- cannot apply relocation";
      rewrite_printf(
          "Failed to locate original .ctors Region -- cannot apply "
          "relocation\n");
      return false;
    }
    ...
    if (lmap.newDtorRegions.size() > 0) {
      ...
      rewrite_printf(
          "Failed to locate original .dtors Region -- cannot apply "
          "relocation\n");
      return false;
    }
  }
...
bool emitElfStatic::archSpecificRelocation(Symtab *targetSymtab,
                                           Symtab *srcSymtab, char *targetData,
                                           relocationEntry &rel, Offset dest,
                                           Offset relOffset,
  ...
  if (PPC64_WIDTH == addressWidth_) {
    ...
    if (rel.getRelType() == R_PPC64_JMP_SLOT) {
      ...
      if (!archSpecificRelocation(targetSymtab, srcSymtab, targetData, toc,
                                  dest + 8, relOffset + 8, globalOffset, lmap,
                                  errMsg)) {
        return false;
      }
      return true;
    ...
    if (!computeCtorDtorAddress(rel, globalOffset, lmap, errMsg,
                                symbolOffset)) {
      return false;
    }
    ...
    if (rel.getRelType() == R_PPC64_REL24 && newTOCoffset != curTOCoffset &&
        handleInterModuleSpecialCase(targetSymtab, srcSymtab, lmap, targetData,
                                     rel, newTOCoffset, curTOCoffset, dest,
                                     relOffset, globalOffset)) {
      return true;
    }

    // If symbol is .toc, we must return the got offset
    if (rel.getRelType() == R_PPC64_TOC16_DS ||
        rel.getRelType() == R_PPC64_TOC16) {
    ...
    rewrite_printf(
        "\trelocation for '%s': TYPE = %s(%lu) S = %lx A = %lx P = %lx Total "
        "0x%lx \n",
        rel.name().c_str(),
    ...
    switch (rel.getRelType()) {
      ...
      case R_PPC64_GOT_TPREL16_DS:
        ...
        // symbolOffset, addend, relOffset, symbolOffset + addend - relOffset,
        // symbolOffset + addend - relOffset);
        break;
      case R_PPC64_REL32:
    ...
    rewrite_printf(
        "\tbefore: relocation = 0x%lx @ 0x%lx target data %lx %lx %lx %lx %lx "
        "%lx \n",
        relocation, relOffset, targetData[dest - 2], targetData[dest - 1],
        targetData[dest], targetData[dest + 1], targetData[dest + 2],
        targetData[dest + 3]);
    ...
    rewrite_printf(
        "\tafter: relocation = 0x%lx @ 0x%lx target data %lx %lx %lx %lx %lx "
        "%lx \n",
        relocation, relOffset, targetData[dest - 2], targetData[dest - 1],
        targetData[dest], targetData[dest + 1], targetData[dest + 2],
        targetData[dest + 3]);
...
case R_PPC64_NUM            :/*107 */
    ...
    rewrite_printf(
        "relocation for '%s': TYPE = %s(%lu) S = %lx A = %lx P = %lx\n",
        rel.name().c_str(),
        relocationEntry::relType2Str(rel.getRelType(), addressWidth_),
        rel.getRelType(), symbolOffset, addend, relOffset);
    ...
    rewrite_printf(
        " relocation = 0x%lx @ 0x%lx target data 0x%lx %lx %lx %lx \n",
...
bool emitElfStatic::checkSpecialCaseSymbols(Symtab *, Symbol *) { return true; }

/* The TLS implementation on ppc is Variant 1 */

Offset emitElfStatic::layoutTLSImage(Offset globalOffset, Region *dataTLS,
...
void emitElfStatic::cleanupTLSRegionOffsets(
    map<Region *, LinkMap::AllocPair> &regionAllocs, Region *dataTLS,
    Region *bssTLS) {
...
Offset emitElfStatic::getGOTSize(Symtab * /*target*/, LinkMap &lmap,
  ...
  if (lmap.gotSymbolTable.size() > 0) {
    rewrite_printf(
        "Determining new GOT size: %d symtab entries, %d reserved slots, %d "
        "slot size\n",
...
void emitElfStatic::buildGOT(Symtab * /*target*/, LinkMap &lmap) {
  ...
  for (sym_it = lmap.gotSymbolTable.begin();
       sym_it != lmap.gotSymbolTable.end(); ++sym_it) {
    ...
    rewrite_printf(
        " Building GOT at 0x%lx is 0x%lx name %s Offset 0x%lx addend 0x%lx "
        "addr 0x%lx \n",
...
Offset emitElfStatic::layoutNewCtorRegion(LinkMap &lmap) {
  ...
  for (reg_it = lmap.newCtorRegions.begin();
       reg_it != lmap.newCtorRegions.end(); ++reg_it) {
    result =
        lmap.regionAllocs.insert(make_pair(*reg_it, make_pair(0, retOffset)));
...
bool emitElfStatic::createNewCtorRegion(LinkMap &lmap) {
  ...
  if (PPC32_WIDTH == addressWidth_) {
    ...
    trailerOffset =
        lmap.ctorRegionOffset + lmap.ctorSize - sizeof(X86_64_TRAILER);
...
Offset emitElfStatic::layoutNewDtorRegion(LinkMap &lmap) {
  ...
  set<Region *, CtorComp<Region *> >::iterator reg_it;
  for (reg_it = lmap.newDtorRegions.begin();
       reg_it != lmap.newDtorRegions.end(); ++reg_it) {
    result =
        lmap.regionAllocs.insert(make_pair(*reg_it, make_pair(0, retOffset)));
...
bool emitElfStatic::createNewDtorRegion(LinkMap &lmap) {
  ...
  if (PPC32_WIDTH == addressWidth_) {
    ...
    memcpy(&targetData[headerOffset], &X86_64_HEADER, sizeof(X86_64_HEADER));
    trailerOffset =
        lmap.dtorRegionOffset + lmap.dtorSize - sizeof(X86_64_TRAILER);
    memcpy(&targetData[trailerOffset], &X86_64_TRAILER, sizeof(X86_64_TRAILER));
    headerSize = sizeof(X86_64_HEADER);
    trailerSize = sizeof(X86_64_TRAILER);
  }
  ...
  if (lmap.originalDtorRegion != NULL) {
    ...
    char *rawRegionData =
        reinterpret_cast<char *>(lmap.originalDtorRegion->getPtrToRawData());
    memcpy(&targetData[originalOffset], &rawRegionData[headerSize],
...
bool emitElfStatic::updateTOC(Symtab *file, LinkMap &lmap,
                              Offset globalOffset) {
  // I don't think we need this now that we're using multiple TOCs. Instead
  // we'll
  // split the GOTs.
  // DISABLED
  return true;
  ...
  // reachable distance. TODO: calculate this from the original GOT and TOC
  // value.
  ...
  // We're doing this _before_ the emitElf64::driver call, so we can make
  // changes
  // to original sections. That's good, because we need to update the OPD
  // section
  ...
  for (unsigned i = 1; i < (OPD->getDiskSize() / sizeof(unsigned long));
       i += 3) {
...
Offset emitElfStatic::allocStubRegions(LinkMap &lmap, Offset globalOffset) {
  ...
  // So. Relocation time.
  // We can't just count relocations because there are TOC setting relocs and
  // we'll massively overapproximate.
  ...
  for (deque<Region *>::iterator reg_iter = lmap.codeRegions.begin();
    ...
    rewrite_printf(
        "\t For region %s in %s, %d relocations, %d unique symbols so far\n",
        (*reg_iter)->getRegionName().c_str(),
        (*reg_iter)->symtab()->name().c_str(), region_rels.size(),
        lmap.stubMap.size());
    for (vector<relocationEntry>::iterator rel_iter = region_rels.begin();
      ...
      if (lmap.stubMap.find(rel.getDynSym()) == lmap.stubMap.end()) {
        rewrite_printf("\t\tSetting stub offset for %s to 0x%lx\n",
                       rel.getDynSym()->getMangledName().c_str(),
                       relStubOffset);
  ...
  rewrite_printf(
      "\t After regions, returning offset 0x%lx = current 0x%lx + stub area "
      "0x%lx\n",
...
bool emitElfStatic::handleInterModuleSpecialCase(
    Symtab * /*target*/, Symtab * /*src*/, LinkMap &lmap, char *data,
  ...
  Offset stubOffset = findOrCreateStub(rel.getDynSym(), lmap, newTOC, oldTOC,
                                       data, globalOffset);
  Offset branchOffset = stubOffset - relOffset;
  rewrite_printf(
      "Handling TOC set stub, using stub offset 0x%lx, curr addr 0x%lx so "
      "branch is 0x%lx \n",
...
Offset emitElfStatic::findOrCreateStub(Symbol *sym, LinkMap &lmap,
                                       Offset newTOC, Offset oldTOC, char *data,
                                       Offset globalOffset) {
  // Easy case
...
void emitElfStatic::createStub(unsigned *stub, Offset stubOffset, Offset newTOC,
  ...
  rewrite_printf(
      ...
      "TOC bits 0x%x (from 0x%lx - 0x%lx),\n\tbranch offset 0x%x (from 0x%lx - "
      "0x%lx)\n",
