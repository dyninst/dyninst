...
Address
memEntry::getAddress() const
{
    return addr;
}

char*
memEntry::getBuffer() const
{
    return buffer;
}

unsigned long
memEntry::getSize() const
{
    return size;
}

bool
memEntry::isRead() const
{
    return is_read;
}

bool
memEntry::isWrite() const
{
    return is_write;
}

bool
memEntry::isToken() const
{
    return token_type != token_none;
}

bool
memEntry::operator==(const memEntry& b) const
{
    if(is_read != b.is_read || is_write != b.is_write || token_type != b.token_type ||
       addr != b.addr || size != b.size)
    {
        return false;
    }
...
void
onContinueMemCache(int_thread* thr)
{
...
void
memCache::getPendingAsyncs(set<response::ptr>& resps)
{
...
void
memCache::setSyncHandling(bool b)
{
    sync_handle = b;
}

void
memCache::markToken(token_t tk)
{
...
void
memCache::updateReadCacheWithWrite(Address dest, char* src, unsigned long size)
{
...
async_ret_t
memCache::doOperation(memEntry* me, int_thread* op_thread)
{
    bool          result = false;
    response::ptr resp;
    ...
    else if(me->isWrite())
    {
        ...
        if(sync_handle)
            me->res_resp->markSyncHandled();
        result = proc->writeMem(buffer, me->getAddress(), size, me->res_resp, op_thread);
        resp   = me->res_resp;
    }
    else
    {
...
async_ret_t
memCache::getExistingOperation(mcache_t::iterator i, memEntry* orig)
{
    memEntry*     me = *i;
    response::ptr resp;
...
async_ret_t
memCache::lookupAsync(memEntry* me, int_thread* op_thread)
{
...
async_ret_t
memCache::readMemoryAsync(void* dest, Address src, unsigned long size,
                          set<mem_response::ptr>& resps, int_thread* reading_thread)
{
    pending_async                        = false;
...
async_ret_t
memCache::readMemorySync(void* buffer, Address addr, unsigned long size,
                         int_thread* reading_thread)
{
    ...
    {
        mem_response::ptr memresult =
            mem_response::createMemResponse((char*) &word_cache, sizeof(word_cache));
...
async_ret_t
memCache::writeMemoryAsync(Dyninst::Address dest, void* src, unsigned long size,
...
async_ret_t
memCache::writeMemorySync(Dyninst::Address dest, void* src, unsigned long size,
...
async_ret_t
memCache::readMemory(void* dest, Address src, unsigned long size,
...
async_ret_t
memCache::writeMemory(Dyninst::Address dest, void* src, unsigned long size,
...
async_ret_t
memCache::getRegisters(int_thread* thr, int_registerPool& pool)
{
