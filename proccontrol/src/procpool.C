...
CondVar<> *ProcessPool::condvar() { return &var; }

void ProcessPool::addThread(int_process * /*proc*/, int_thread *thr) {
  if (!LWPIDsAreUnique()) return;
...
void ProcessPool::addDeadThread(Dyninst::LWP lwp) { deadThreads.insert(lwp); }
...
unsigned ProcessPool::numProcs() { return (unsigned)procs.size(); }
