...
public:
    ...
    virtual bool plat_getResidentUsage(unsigned long stacku, unsigned long heapu,
                                       unsigned long sharedu, MemUsageResp_t* resp);

protected:
    int computeAddrWidth();
};

class linux_x86_process
: public linux_process
, public x86_process
{
public:
...
public:
    ...
    virtual bool plat_convertToSystemRegs(const int_registerPool& pool,
                                          unsigned char* regs, bool gprs_only = false);
...
class linux_x86_thread
: virtual public linux_thread
...
public:
    ...
    bool ptrace_write(Dyninst::Address inTrace, unsigned size_, const void* inSelf,
                      int pid_);
...
long
do_ptrace(pt_req request_, pid_t pid_, void* addr_, void* data_);
