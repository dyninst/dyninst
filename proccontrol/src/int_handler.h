...
class HandlerPool {
 public:
  ...
  void notifyOfPendingAsyncs(const std::set<response::ptr> &asyncs,
                             Event::ptr ev);
...
class HandleCallbacks : public Handler {
 ...
 private:
  typedef std::map<EventType, std::set<Process::cb_func_t>, eventtype_cmp>
      cbfuncs_t;
  cbfuncs_t cbfuncs;
 ...
 public:
  ...
  Handler::handler_ret_t deliverCallback(
      Event::ptr ev, const std::set<Process::cb_func_t> &cbset);
