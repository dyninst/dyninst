...
Dyninst::Address DecoderWindows::adjustTrapAddr(Dyninst::Address addr,
...
void DecoderWindows::dumpSurroundingMemory(unsigned problemArea,
                                           int_process* proc) {
...
EventLibrary::ptr DecoderWindows::decodeLibraryEvent(DEBUG_EVENT details,
                                                     int_process* p) {
  ...
  if (details.dwDebugEventCode == LOAD_DLL_DEBUG_EVENT) {
    ...
    if (details.u.LoadDll.lpImageName) {
      read =
          p->plat_readMem(NULL, &libnameaddr,
                          (Dyninst::Address)(details.u.LoadDll.lpImageName), 4);
    }
    // cerr << "imageName @ " << hex << details.u.LoadDll.lpImageName << " and
    // read " << read << dec << endl;
    if (details.u.LoadDll.lpImageName &&  // NULL lpImageName = done
        read && libnameaddr)              // NULL libnameaddr = done
    {
    ...
    // cerr << "\t ... " << result << endl;
    int_library* lib = new int_library(
        result, true, (Dyninst::Address)(details.u.LoadDll.lpBaseOfDll),
    ...
    for (foundLib = proc->memory()->libs.begin();
         foundLib != proc->memory()->libs.end(); ++foundLib) {
      if ((*foundLib)->getAddr() ==
          (Dyninst::Address)(details.u.UnloadDll.lpBaseOfDll)) {
...
Event::ptr DecoderWindows::decodeSingleStepEvent(DEBUG_EVENT e,
                                                 int_process* proc,
                                                 int_thread* thread) {
  Event::ptr evt;
  // This handles user-level breakpoints; if we can't associate this with a
  // thread we know about, it's not
  // something the user installed (or we're in an inconsistent state). Forward
  // to the mutatee.
  ...
  ::GetThreadContext(((windows_thread*)thread)->plat_getHandle(),
                     &verification);

  if (verification.EFlags & TF_BIT) {
    pthrd_printf(
        "BUG ENCOUNTERED: OS handled us a thread with TF set, clearing "
        "manually\n");
    verification.EFlags &= (~TF_BIT);
    ::SetThreadContext(((windows_thread*)thread)->plat_getHandle(),
                       &verification);
  }
...
Event::ptr DecoderWindows::decodeBreakpointEvent(DEBUG_EVENT e,
                                                 int_process* proc,
                                                 int_thread* thread) {
  Event::ptr evt;
  // This handles user-level breakpoints; if we can't associate this with a
  ...
  Dyninst::Address adjusted_addr =
      (Dyninst::Address)(e.u.Exception.ExceptionRecord.ExceptionAddress);
  ...
  if (ibp) {
    ...
    EventBreakpoint::ptr event_bp = EventBreakpoint::ptr(new EventBreakpoint(
...
bool DecoderWindows::decode(ArchEvent* ae, std::vector<Event::ptr>& events) {
  ...
  if (!windows_proc) {
    perr_printf(
        "DecoderWindows::decode() called on nonexistent/deleted process "
        "%d/%d\n",
  ...
  switch (e.dwDebugEventCode) {
    ...
    case EXCEPTION_DEBUG_EVENT:
      ...
      switch (e.u.Exception.ExceptionRecord.ExceptionCode) {
        case EXCEPTION_SINGLE_STEP:
          ...
          break;
        // fprintf(stderr, "Decoded Single-step event at 0x%lx, PID: %d, TID:
        // %d\n", e.u.Exception.ExceptionRecord.ExceptionAddress, e.dwProcessId,
        // e.dwThreadId);
        case EXCEPTION_BREAKPOINT:
          ...
          if (newEvt) {
            pthrd_printf(
                "Decoded Breakpoint event at 0x%lx, PID: %d, TID: %d\n",
                e.u.Exception.ExceptionRecord.ExceptionAddress, e.dwProcessId,
                e.dwThreadId);
            // fprintf(stderr, "Decoded Breakpoint event at 0x%lx, PID: %d, TID:
            // %d\n", e.u.Exception.ExceptionRecord.ExceptionAddress,
            // e.dwProcessId, e.dwThreadId);
            break;
          } else {
            if (proc->getState() != int_process::neonatal &&
              ...
              if (windows_proc->pendingDebugBreak()) {
                ...
                pthrd_printf(
                    "Decoded unhandled exception (breakpoint) event, PID: %d, "
                    "TID: %d\n",
                    e.dwProcessId, e.dwThreadId);
                // Case 3: breakpoint that's not from us, while running. Pass on
                // exception.
                GeneratorWindows* winGen = static_cast<GeneratorWindows*>(
                    GeneratorWindows::getDefaultGenerator());
                newEvt = EventSignal::ptr(new EventSignal(
                    e.u.Exception.ExceptionRecord.ExceptionCode));
              }
            } else {
              // Case 4: breakpoint in ntdll.dll due to startup. This should be
              // skipped and we should bootstrap.
              pthrd_printf("Breakpoint due to startup, ignoring\n");
              newEvt = Event::ptr(new EventBootstrap());
              ntdll_ignore_breakpoint_address =
                  (Address)e.u.Exception.ExceptionRecord.ExceptionAddress;
              if (!thread) {
        ...
        case EXCEPTION_ILLEGAL_INSTRUCTION: {
          ...
          newEvt = EventSignal::ptr(
              new EventSignal(e.u.Exception.ExceptionRecord.ExceptionCode));
          assert(0);
        } break;
        case EXCEPTION_ACCESS_VIOLATION: {
          ...
          if (e.u.Exception.dwFirstChance != 0) {
            ...
            Address addr =
                e.u.Exception.ExceptionRecord.ExceptionInformation[1];
            EventSignal* evSig = nullptr;
            switch (cause) {
              case 0:
                evSig = new EventSignal(sig, addr, EventSignal::ReadViolation,
                                        true);
                break;
              case 1:
                evSig = new EventSignal(sig, addr, EventSignal::WriteViolation,
                                        true);
                break;
              case 8:
                evSig = new EventSignal(sig, addr,
                                        EventSignal::ExecuteViolation, true);
                break;
              default:
                evSig = new EventSignal(sig, addr, EventSignal::Unknown, true);
                break;
            }
            ...
            pthrd_printf("segfault in mutatee, thread %d/%d\n", e.dwProcessId,
                         e.dwThreadId);
            unsigned problemArea =
                (unsigned int)(e.u.Exception.ExceptionRecord.ExceptionAddress);
            ...
            newEvt = EventSignal::ptr(
                new EventSignal(e.u.Exception.ExceptionRecord.ExceptionCode));
            cerr << "Signal is " << e.u.Exception.ExceptionRecord.ExceptionCode
                 << endl;
          }
        } break;
        ...
        default: {
          pthrd_printf(
              "Decoded unhandled exception event, PID: %d, TID: %d, Exception "
              "code = 0x%lx, Exception addr = 0x%lx\n",
              e.dwProcessId, e.dwThreadId,
              e.u.Exception.ExceptionRecord.ExceptionCode,
              e.u.Exception.ExceptionRecord.ExceptionAddress);
          GeneratorWindows* winGen = static_cast<GeneratorWindows*>(
    ...
    case EXIT_PROCESS_DEBUG_EVENT: {
      ...
      if (proc->wasForcedTerminated()) {
        newEvt = EventForceTerminate::ptr(
            new EventForceTerminate(e.u.ExitProcess.dwExitCode));
      } else {
        newEvt = EventExit::ptr(
            new EventExit(EventType::Pre, e.u.ExitProcess.dwExitCode));
      }
      ...
      // Since we're doing thread exit/proc exit, and this means the thread will
      // go away first,
      // we don't set the thread here
      if (thread) newEvt->setThread(thread->thread());
      // We do this here because the generator thread will exit before
      // updateSyncState otherwise
    ...
    case EXIT_THREAD_DEBUG_EVENT:
      ...
      if (thread) {
        ...
        if (e.dwThreadId == windows_proc->getStopThread()) {
          newEvt = EventWinStopThreadDestroy::ptr(
              new EventWinStopThreadDestroy(EventType::Pre));
          windows_proc->clearStopThread();
        } else {
          newEvt = EventLWPDestroy::ptr(new EventLWPDestroy(EventType::Pre));
        }
      } else {
        // If the thread is NULL, we can't give the user an event with a valid
        // thread object anymore.
        // So fail the decode.
        return false;
      }
      break;
    ...
    case OUTPUT_DEBUG_STRING_EVENT: {
      ...
      BOOL result = ::ReadProcessMemory(
          winProc->plat_getHandle(), e.u.DebugString.lpDebugStringData, buf,
          e.u.DebugString.nDebugStringLength, &bytes_read);
      if (result) {
        pthrd_printf("Decoded DebugString event, string: %s\n", buf);
      } else {
        pthrd_printf(
            "Decoded DebugString event, but string was not readable!\n");
      }
    ...
    case RIP_EVENT:
      pthrd_printf("Decoded RIP event, PID: %d, TID: %d, error = 0x%lx\n",
                   e.dwProcessId, e.dwThreadId, e.u.RipInfo.dwError);
...
bool DecoderWindows::checkForFullString(DEBUG_EVENT& details, int chunkSize,
                                        wchar_t* libName, bool gotString,
                                        char* asciiLibName) {
...
bool DecoderWindows::decodeCreateThread(DEBUG_EVENT& e, Event::ptr& newEvt,
                                        int_process*& proc,
  ...
  if (e.dwDebugEventCode == CREATE_PROCESS_DEBUG_EVENT) {
    newEvt = WinEventThreadInfo::ptr(new WinEventThreadInfo(
        (Dyninst::LWP)(e.dwThreadId), e.u.CreateProcessInfo.hThread,
        e.u.CreateProcessInfo.lpStartAddress,
        e.u.CreateProcessInfo.lpThreadLocalBase));
    newEvt->setThread(proc->threadPool()->initialThread()->thread());
    wproc->plat_setHandles(
        e.u.CreateProcessInfo.hProcess, e.u.CreateProcessInfo.hFile,
        (Dyninst::Address)e.u.CreateProcessInfo.lpBaseOfImage);
  } else {
    newEvt = WinEventNewThread::ptr(new WinEventNewThread(
        (Dyninst::LWP)(e.dwThreadId), e.u.CreateThread.hThread,
...
std::string DecoderWindows::readLibNameFromProc(Address libnameaddr,
                                                DEBUG_EVENT details,
                                                int_process* p) {
  ...
  while (!doneReading) {
    ...
    if (p->plat_readMem(NULL, libName, (Dyninst::Address)libnameaddr,
                        chunkSize)) {
      // the read succeeded -
      // did we get the full string?
      gotString = checkForFullString(details, chunkSize, libName, gotString,
                                     asciiLibName);
      if (gotString) {
        doneReading = true;
      } else {
...
std::string DecoderWindows::HACKreadFromFile(DEBUG_EVENT details,
                                             int_process* p) {
  std::string ret;
  ...
  // (using the handle), then ask the OS what file we have mapped at that
  // location.
  // I'm sad now.

  void* pmap = NULL;
  HANDLE fmap = CreateFileMapping(details.u.LoadDll.hFile, NULL, PAGE_READONLY,
                                  0, 1, NULL);
  if (fmap) {
    ...
    if (pmap) {
      ...
      if (result) ret = std::string(filename);
