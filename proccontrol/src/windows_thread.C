...
windows_thread::windows_thread(int_process *p, Dyninst::THR_ID t,
                               Dyninst::LWP l)
    : int_thread(p, t, l),
...
windows_thread::~windows_thread() {
  // Do NOT close the handle here. That's handled when ContinueDebugEvent is
  // called on the thread exit event.
}

void windows_thread::setHandle(HANDLE h) { hthread = h; }
...
bool windows_thread::plat_stop() {
  ...
  if (wproc->pendingDebugBreak() ||
    ...
    // If there's a debug break pending or we're generator stopped, then all we
    // need to do is set state.
    // Only make the debug break call if we don't know that we're going to
    // become stopped at some future point.
    pthrd_printf(
        "Pending debug break (%s) or generator stopped (%s), erasing pending "
        "stop and returning true\n",
    ...
    if (result == -1) {
      ...
      // Don't force this thread to run. We want to force the *actual* stop
      // thread to run.
...
bool windows_thread::plat_suspend() {
  ...
  if (result == -1 && (::GetLastError() == ERROR_ACCESS_DENIED)) {
    // FIXME
    // This happens if the thread is in a system call and thus cannot be
    // modified.
...
bool windows_thread::plat_getRegister(Dyninst::MachRegister reg,
...
bool windows_thread::plat_setAllRegisters(int_registerPool &regpool) {
  if (isRPCpreCreate()) {
    pthrd_printf(
        "setAllRegisters called on dummy RPC thread, grabbing PC for future "
        "use: 0x%lx\n",
  ...
  // std::cerr << "plat_setAllRegisters, EIP = " << std::hex <<
  // regpool.regs[x86::eip] << std::dec << std::endl;
  ...
  ::FlushInstructionCache(
      dynamic_cast<windows_process *>(proc_)->plat_getHandle(), 0, 0);
  // fprintf(stderr, "Set regs, CS:EIP = 0x%x:0x%x\n", c.SegCs, c.Eip);
  // fprintf(stderr, "          TF = %s\n", (c.EFlags & TF_BIT) ? "true" :
  // "false" );
...
bool windows_thread::plat_setRegister(Dyninst::MachRegister reg,
                                      Dyninst::MachRegisterVal val) {
  assert(!"Not implemented");
  return false;
}

bool windows_thread::plat_convertToSystemRegs(const int_registerPool &regpool,
...
bool windows_thread::plat_needsPCSaveBeforeSingleStep() { return true; }

bool windows_thread::plat_needsEmulatedSingleStep(
    std::vector<Dyninst::Address> &result) {
  // NOTE: this modifies the vector of addresses with locations that need
  // emulation.
  // true/false is an error return, not a "do we need emulation" return!
  return true;
}
bool windows_thread::haveUserThreadInfo() { return m_StartAddr != 0; }
...
Address windows_thread::getThreadInfoBlockAddr() {
  ...
  if (!GetThreadSelectorEntry(hthread, fs, &segDesc)) {
    // fprintf(stderr, "%s[%d] Failed to read segment register FS for thread
    // 0x%x with FS index of 0x%x\n",
...
bool windows_thread::getStartFuncAddress(Dyninst::Address &start_addr) {
  ...
  // If the stack query failed, we'll re-query every time. Which is sub-optimal,
...
bool windows_thread::getStackBase(Dyninst::Address &stack_base) {
  ...
  proc_->plat_readMem(this, &stackBase, stack_base_addr,
                      sizeof(Dyninst::Address));
  stack_base = stackBase;
  return true;
}
...
bool windows_thread::getStackSize(unsigned long &stack_size) {
  ...
  proc_->plat_readMem(this, &stack_bottom, stack_bottom_addr,
                      sizeof(Dyninst::Address));
...
void windows_thread::setTLSAddress(Dyninst::Address addr) { m_TLSAddr = addr; }
...
bool windows_thread::notAvailableForRPC() {
  ...
  pthrd_printf(
      "Thread at 0x%lx does not need trap for RPC, prevInsn[1] = 0x%x\n",
...
bool windows_thread::isUser() const { return isUser_; }
...
Address windows_thread::getDummyRPCStart() const { return dummyRpcPC_; }
...
void windows_thread::plat_terminate() { ::TerminateThread(hthread, 0); }
...
bool windows_thread::isRPCThread() const { return (dummyRPC_ != notRPCThread); }
...
void windows_thread::plat_setSuspendCount(int count) {
  ...
  pthrd_printf(
      "Setting suspend count of %d/%d to %d, error code %d, last reported "
      "value %d\n",
      llproc()->getPid(), tid, count, ((result == -1) ? ::GetLastError() : 0),
      current);
}
