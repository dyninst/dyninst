...
Event::ptr Handler::convertEventForCB(Event::ptr /*orig*/) { return Event::ptr(); }
...
void
HandlerPool::addHandlerInt(EventType etype, Handler* handler)
{
...
void
HandlerPool::addHandler(Handler* handler)
{
...
void
HandlerPool::clearNopAsCurEvent()
{
    nop_cur_event = false;
    cur_event     = Event::ptr();
}
...
void
HandlerPool::addLateEvent(Event::ptr ev)
{
    late_events.insert(ev);
}

bool
HandlerPool::hasLateEvents() const
{
    return !late_events.empty();
}

void
HandlerPool::collectLateEvents(Event::ptr parent_ev)
{
...
Event::ptr
HandlerPool::getRealParent(Event::ptr ev) const
{
...
void
HandlerPool::notifyOfPendingAsyncs(const std::set<response::ptr>& asyncs, Event::ptr ev)
{
...
void
HandlerPool::notifyOfPendingAsyncs(response::ptr async, Event::ptr ev)
{
    async->setEvent(getRealParent(ev));
}

bool
HandlerPool::isEventAsyncPostponed(Event::ptr ev) const
{
    return (pending_async_events.find(ev) != pending_async_events.end());
}

bool
HandlerPool::hasAsyncEvent() const
{
    return !pending_async_events.empty();
}

void
HandlerPool::markEventAsyncPending(Event::ptr ev)
{
...
void
HandlerPool::clearEventAsync(Event::ptr ev)
{
...
bool
HandlerPool::insertAsyncPendingEvent(Event::ptr ev)
{
...
bool
HandlerPool::removeAsyncPendingEvent(Event::ptr ev)
{
    set<Event::ptr>::iterator i = pending_async_events.find(ev);
    if(i == pending_async_events.end())
    {
...
void
HandlerPool::addEventToSet(Event::ptr ev, set<Event::ptr>& ev_set) const
{
    ev_set.insert(ev);
...
bool
HandlerPool::handleEvent(Event::ptr orig_ev)
{
...
void
HandlerPool::markProcAsyncPending(HandlerPool* p)
{
...
void
HandlerPool::clearProcAsyncPending(HandlerPool* p)
{
    asyncPendingLock.lock();
...
bool
HandlerPool::hasProcAsyncPending()
{
    asyncPendingLock.lock();
...
void
HandlePreBootstrap::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
void
HandleBootstrap::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Bootstrap));
}

Handler::handler_ret_t
HandleBootstrap::handleEvent(Event::ptr ev)
{
...
void
HandleSignal::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Signal));
}

Handler::handler_ret_t
HandleSignal::handleEvent(Event::ptr ev)
{
    int_thread*  thrd = ev->getThread()->llthrd();
    int_process* proc = ev->getProcess()->llproc();
...
void
HandlePostExit::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::Exit));
}

Handler::handler_ret_t
HandlePostExit::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
    assert(proc);
    if(ev->getThread())
...
void
HandlePostExitCleanup::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::Exit));
...
Handler::handler_ret_t
HandlePostExitCleanup::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
...
int
HandlePostExitCleanup::getPriority() const
{
...
void
HandleCrash::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Crash));
}

Handler::handler_ret_t
HandleCrash::handleEvent(Event::ptr ev)
{
...
void
HandleForceTerminate::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::ForceTerminate));
}

Handler::handler_ret_t
HandleForceTerminate::handleEvent(Event::ptr ev)
{
    int_process*      proc = ev->getProcess()->llproc();
    ...
    pthrd_printf("Handling force terminate for process %d on thread %d\n", proc->getPid(),
                 thrd ? thrd->getLWP() : (Dyninst::LWP)(-1));
...
int
HandleForceTerminate::getPriority() const
{
...
void
HandlePreExit::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Pre, EventType::Exit));
}

Handler::handler_ret_t
HandlePreExit::handleEvent(Event::ptr ev)
{
    int_process* proc   = ev->getProcess()->llproc();
...
void
HandleThreadCreate::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
HandleThreadCreate::handleEvent(Event::ptr ev)
{
    int_process*      proc    = ev->getProcess()->llproc();
    ...
    pthrd_printf("Handle thread create for %d/%d with new thread %d\n", proc->getPid(),
                 thrd ? thrd->getLWP() : (Dyninst::LWP)(-1), threadev->getLWP());
    ...
    int_thread* newthr =
        int_thread::createThread(proc, NULL_THR_ID, threadev->getLWP(), false, astatus);
...
void
HandleThreadDestroy::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
int
HandleThreadDestroy::getPriority() const
{
    return Handler::PostPlatformPriority + 2;  // After thread_db destroy handler
}

Handler::handler_ret_t
HandleThreadDestroy::handleEvent(Event::ptr ev)
{
    int_thread* thrd = ev->getThread()->llthrd();
...
void
HandleThreadCleanup::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Any, EventType::UserThreadDestroy));
    etypes.push_back(EventType(EventType::Any, EventType::LWPDestroy));
    etypes.push_back(EventType(EventType::Any, EventType::WinStopThreadDestroy));
...
int
HandleThreadCleanup::getPriority() const
{
    return Handler::PostCallbackPriority + 1;
}

Handler::handler_ret_t
HandleThreadCleanup::handleEvent(Event::ptr ev)
{
    /**
    ...
    {
        pthrd_printf(
            "Process for thread cleanup event is NULL. We have no work we can do.\n");
        return ret_success;
    }
    if((ev->getEventType().code() == EventType::UserThreadDestroy) &&
       (proc->plat_supportLWPPreDestroy() || proc->plat_supportLWPPostDestroy()))
    {
        // Have a user thread destroy, but platform supports LWP destroys.  Will clean
        // with a latter event.
    ...
    {
        // Have a pre-lwp destroy, but platform supports post-lwp destroy.  Will clean
        // with a latter event.
        pthrd_printf("Nothing to do in HandleThreadCleanup\n");
        return ret_success;
    }
    ...
    if((ev->getEventType().code() == EventType::UserThreadDestroy &&
        !proc->plat_supportLWPPreDestroy() && !proc->plat_supportLWPPostDestroy()))
    {
        // Have a user thread destroy, and platform doesn't support LWP destroys.  Will
        // partially clean,
    ...
    {
        pthrd_printf(
            "Thread for thread cleanup event is NULL. We have no work we can do.\n");
        return ret_success;
    }
...
int
HandleThreadStop::getPriority() const
{
    return DefaultPriority;
}

void
HandleThreadStop::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Stop));
}

Handler::handler_ret_t
HandleThreadStop::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
...
void
HandlePostFork::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::Fork));
}

Handler::handler_ret_t
HandlePostFork::handleEvent(Event::ptr ev)
{
...
void
HandlePostForkCont::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::Fork));
}

Handler::handler_ret_t
HandlePostForkCont::handleEvent(Event::ptr ev)
{
    EventFork*   efork       = static_cast<EventFork*>(ev.get());
    Dyninst::PID child_pid   = efork->getPID();
    int_process* child_proc  = ProcPool()->findProcByPid(child_pid);
...
int
HandlePostForkCont::getPriority() const
{
    return PostCallbackPriority;
}
...
void
HandlePostExec::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::Exec));
}

Handler::handler_ret_t
HandlePostExec::handleEvent(Event::ptr ev)
{
...
void
HandleSingleStep::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::SingleStep));
}

Handler::handler_ret_t
HandleSingleStep::handleEvent(Event::ptr ev)
{
...
void
HandlePreSyscall::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Pre, EventType::PreSyscall));
}

Handler::handler_ret_t
HandlePreSyscall::handleEvent(Event::ptr ev)
{
...
void
HandlePostSyscall::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::PostSyscall));
}

Handler::handler_ret_t
HandlePostSyscall::handleEvent(Event::ptr ev)
{
...
void
HandleBreakpoint::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Breakpoint));
}

Handler::handler_ret_t
HandleBreakpoint::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
    int_thread*  thrd = ev->getThread()->llthrd();
...
void
HandleBreakpointContinue::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Breakpoint));
}

int
HandleBreakpointContinue::getPriority() const
{
    return PostCallbackPriority;
}

Handler::handler_ret_t
HandleBreakpointContinue::handleEvent(Event::ptr ev)
{
    /**
...
void
HandleBreakpointClear::getEventTypesHandled(vector<EventType>& etypes)
{
 ...
 **/
Handler::handler_ret_t
HandleBreakpointClear::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
    int_thread*  thrd = ev->getThread()->llthrd();
    ...
    {
        pthrd_printf("HandleBreakpointClear on thread without breakpoint.  BP must have "
                     "been deleted\n");
    ...
    {
        pthrd_printf(
            "Breakpoint clear is pending in HandleBreakpointClear.  Returning async\n");
    ...
    if(restore_bp)
    {
        ...
        pthrd_printf("Making stop/continue decisions for handleBreakpointClear when "
                     "stepping over BP\n");
    ...
    {
        pthrd_printf(
            "HandleBreakpointClear will not restore BP.  Restoring process state.\n");
    }
...
void
HandleBreakpointRestore::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::BreakpointRestore));
}

Handler::handler_ret_t
HandleBreakpointRestore::handleEvent(Event::ptr ev)
{
    int_process*                proc    = ev->getProcess()->llproc();
    int_thread*                 thrd    = ev->getThread()->llthrd();
    ...
    // Time to restore this bp (maybe, the BP will actually resume when it's last thread
    // is done restoring)
...
void
HandleEmulatedSingleStep::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::Breakpoint));
}

Handler::handler_ret_t
HandleEmulatedSingleStep::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
    int_thread*  thrd = ev->getThread()->llthrd();
...
int
HandleEmulatedSingleStep::getPriority() const
{
...
Handler::handler_ret_t
HandleLibrary::handleEvent(Event::ptr ev)
{
...
void
HandleLibrary::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
HandleDetach::handleEvent(Event::ptr ev)
{
    int_process*             proc            = ev->getProcess()->llproc();
...
void
HandleDetach::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
HandleAsync::handleEvent(Event::ptr ev)
{
...
void
HandleAsync::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
HandleAsyncIO::handleEvent(Event::ptr ev)
{
...
void
HandleAsyncIO::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
HandleAsyncFileRead::handleEvent(Event::ptr ev)
{
...
void
HandleAsyncFileRead::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
void
HandleNop::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
int
HandleCallbacks::getPriority() const
{
    return CallbackPriority;
}

void
HandleCallbacks::getEventTypesHandled(std::vector<EventType>& /*etypes*/)
{
    // Callbacks are special cased, they respond to all event types.
}

bool
HandleCallbacks::hasCBs(Event::const_ptr ev)
{
    return cbfuncs.find(ev->getEventType()) != cbfuncs.end();
}

bool
HandleCallbacks::hasCBs(EventType et)
{
    return cbfuncs.find(et) != cbfuncs.end();
}

bool
HandleCallbacks::requiresCB(Event::const_ptr ev)
{
    return hasCBs(ev) && !ev->suppressCB();
}

Handler::handler_ret_t
HandleCallbacks::handleEvent(Event::ptr ev)
{
    int_thread* thr = NULL;
    if(ev->getThread())
        thr = ev->getThread()->llthrd();
    int_process* proc = ev->getProcess()->llproc();
    ...
    {
        pthrd_printf(
            "Suppressing callback for event %s due to process being in silent mode\n",
            ev->name().c_str());
...
static const char*
action_str(Process::cb_action_t action)
{
    switch(action)
    {
...
bool
HandleCallbacks::handleCBReturn(Process::const_ptr proc, Thread::const_ptr thrd,
...
Handler::handler_ret_t
HandleCallbacks::deliverCallback(Event::ptr ev, const set<Process::cb_func_t>& cbset)
{
    ...
    {
        pthrd_printf(
            "Process is in forced termination, overriding result to cbThreadContinue\n");
...
void
HandleCallbacks::getRealEvents(EventType ev, std::vector<EventType>& out_evs)
{
    switch(ev.code())
    {
...
bool
HandleCallbacks::registerCallback_int(EventType ev, Process::cb_func_t func)
{
...
bool
HandleCallbacks::registerCallback(EventType oev, Process::cb_func_t func)
{
...
bool
HandleCallbacks::removeCallback_int(EventType et, Process::cb_func_t func)
{
...
bool
HandleCallbacks::removeCallback(EventType oet, Process::cb_func_t func)
{
...
bool
HandleCallbacks::removeCallback_int(EventType et)
{
    cbfuncs_t::iterator i = cbfuncs.find(et);
    if(i == cbfuncs.end())
    {
...
bool
HandleCallbacks::removeCallback(EventType et)
{
    bool result = false;
    switch(et.time())
    {
...
bool
HandleCallbacks::removeCallback(Process::cb_func_t func)
{
...
void
HandlePostponedSyscall::getEventTypesHandled(vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::PostponedSyscall));
}

Handler::handler_ret_t
HandlePostponedSyscall::handleEvent(Event::ptr ev)
{
    int_thread* thrd = ev->getThread()->llthrd();
