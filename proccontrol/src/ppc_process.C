...
unsigned
ppc_process::plat_breakpointSize()
{
    return 4;
}

void
ppc_process::plat_breakpointBytes(unsigned char* buffer)
{
...
bool
ppc_process::plat_breakpointAdvancesPC() const
{
    return false;
}

static bool
atomicLoad(const instruction& insn)
{
...
static void
clear_ss_state_cb(int_thread* thr)
{
...
void
ppc_process::cleanupSSOnContinue(int_thread* thr)
{
...
async_ret_t
ppc_process::readPCForSS(int_thread* thr, Address& pc)
{
    ...
    if(!plat_needsAsyncIO())
    {
        ...
        {
            pthrd_printf(
                "Error reading PC address to check for emulated single step condition\n");
            return aret_error;
        }
...
async_ret_t
ppc_process::readInsnForSS(Address pc, int_thread*, unsigned int& rawInsn)
{
    if(!plat_needsAsyncIO())
    {
    ...
    for(i = mem_for_ss.begin(); i != mem_for_ss.end(); i++)
    {
        if(pc >= i->first && pc + 4 <= i->first + i->second->getSize())
        {
            if(!i->second->isReady())
            {
                pthrd_printf("Returning async form memory read while doing emulated "
                             "single step test\n");
                return aret_async;
            }
            if(i->second->hasError())
            {
                pthrd_printf("Error during async read of memory during emulated single "
                             "step test\n");
                return aret_error;
            }
...
async_ret_t
ppc_process::plat_needsEmulatedSingleStep(int_thread* thr, vector<Address>& addrResult)
{
    ...
    do
    {
        ...
        // For control flow instructions, assume target is outside atomic instruction
        // sequence and place breakpoint there as well
...
void
ppc_process::plat_getEmulatedSingleStepAsyncs(int_thread*, std::set<response::ptr> resps)
{
...
bool
ppc_process::plat_convertToBreakpointAddress(Address&, int_thread*)
{
    return true;
}

bool
ppc_process::plat_needsPCSaveBeforeSingleStep()
{
    return true;
}
...
bool
ppc_thread::rmHWBreakpoint(hw_breakpoint*, bool, std::set<response::ptr>&, bool&)
{
    return false;
}

bool
ppc_thread::addHWBreakpoint(hw_breakpoint*, bool, std::set<response::ptr>&, bool&)
{
    return false;
}

unsigned
ppc_thread::hwBPAvail(unsigned)
{
    return 0;
}

EventBreakpoint::ptr
ppc_thread::decodeHWBreakpoint(response::ptr&, bool, Dyninst::MachRegisterVal)
{
    return EventBreakpoint::ptr();
}

bool
ppc_thread::bpNeedsClear(hw_breakpoint*)
{
    return false;
}

void
ppc_thread::setCachedPC(Address pc)
{
    cached_pc      = pc;
    have_cached_pc = true;
}
...
void
ppc_thread::clearCachedPC()
{
    have_cached_pc = false;
}

bool
ppc_thread::haveCachedPC(Address& pc)
{
    if(!have_cached_pc)
        return false;
    pc = cached_pc;
