...
ppc_process::ppc_process(Dyninst::PID p, std::string e,
...
unsigned ppc_process::plat_breakpointSize() { return 4; }

void ppc_process::plat_breakpointBytes(unsigned char *buffer) {
  uint32_t *p = (uint32_t *)buffer;

  *p = 0x7d821008;  // MJMTODO = Assumes host and target architecture match (and
                    // algnment)
}

bool ppc_process::plat_breakpointAdvancesPC() const { return false; }

static bool atomicLoad(const instruction &insn) {
  return ((XFORM_OP(insn) == LXop) && (XFORM_XO(insn) == LWARXxop));
}
...
void ppc_process::registerSSClearCB() {
  ...
  if (registered_ss_clear_cb) return;
...
async_ret_t ppc_process::readPCForSS(int_thread *thr, Address &pc) {
  ...
  assert(ppc_thr);
  if (ppc_thr->haveCachedPC(pc)) return aret_success;

  if (!plat_needsAsyncIO()) {
    // Fast-track for linu/ppc
    reg_response::ptr pcResponse = reg_response::createRegResponse();
    bool result =
        thr->getRegister(MachRegister::getPC(getTargetArch()), pcResponse);
    if (!result || pcResponse->hasError()) {
      pthrd_printf(
          "Error reading PC address to check for emulated single step "
          "condition\n");
      return aret_error;
    }
  ...
  if (!resp->isReady()) {
    pthrd_printf(
        "Async return while getting PC for emulated single step test\n");
    return aret_async;
  }
...
async_ret_t ppc_process::readInsnForSS(Address pc, int_thread *,
                                       unsigned int &rawInsn) {
  if (!plat_needsAsyncIO()) {
    // Fast-track for linux/ppc
    mem_response::ptr new_resp =
        mem_response::createMemResponse((char *)&rawInsn, sizeof(unsigned int));
  ...
  for (i = mem_for_ss.begin(); i != mem_for_ss.end(); i++) {
    if (pc >= i->first && pc + 4 <= i->first + i->second->getSize()) {
      if (!i->second->isReady()) {
        pthrd_printf(
            "Returning async form memory read while doing emulated single step "
            "test\n");
        return aret_async;
      }
      if (i->second->hasError()) {
        pthrd_printf(
            "Error during async read of memory during emulated single step "
            "test\n");
        return aret_error;
      }
  ...
  mem_response::ptr new_resp =
      mem_response::createMemResponse(buffer, read_size);
  bool result = readMem(pc, new_resp);
  mem_for_ss[pc] = new_resp;
  if (!result || new_resp->hasError()) {
    pthrd_printf(
        "Error during async read of memory during emulated single step test\n");
    return aret_error;
  }
  if (!new_resp->isReady()) {
...
async_ret_t ppc_process::plat_needsEmulatedSingleStep(
    int_thread *thr, vector<Address> &addrResult) {
  ...
  if (aresult == aret_error || aresult == aret_async) return aresult;
  ...
  do {
    ...
    if (atomicLoad(insn)) {
      ...
      pthrd_printf(
          "Found the start of an atomic instruction sequence at 0x%lx\n", pc);
    } else {
    ...
    // For control flow instructions, assume target is outside atomic
...
void ppc_process::plat_getEmulatedSingleStepAsyncs(
    int_thread *, std::set<response::ptr> resps) {
...
bool ppc_process::plat_needsPCSaveBeforeSingleStep() { return true; }
...
bool ppc_thread::rmHWBreakpoint(hw_breakpoint *, bool,
                                std::set<response::ptr> &, bool &) {
  return false;
}

bool ppc_thread::addHWBreakpoint(hw_breakpoint *, bool,
                                 std::set<response::ptr> &, bool &) {
  return false;
}

unsigned ppc_thread::hwBPAvail(unsigned) { return 0; }

EventBreakpoint::ptr ppc_thread::decodeHWBreakpoint(response::ptr &, bool,
                                                    Dyninst::MachRegisterVal) {
  return EventBreakpoint::ptr();
}

bool ppc_thread::bpNeedsClear(hw_breakpoint *) { return false; }
...
void ppc_thread::clearCachedPC() { have_cached_pc = false; }

bool ppc_thread::haveCachedPC(Address &pc) {
  if (!have_cached_pc) return false;
  pc = cached_pc;
  return true;
}
