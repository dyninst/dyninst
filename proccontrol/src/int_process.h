...
typedef std::map<Dyninst::MachRegister, std::pair<unsigned int, unsigned int> >
    dynreg_to_user_t;
...
/**
 * Counters are usually* used to determine when to return from
 *waitAndHandleEvents.
 ...
 * when procStopper returns false.  ProcStopEventManager will handle much of
 *this,
...
class int_process {
 ...
 protected:
  ...
  bool attachThreads();
  virtual async_ret_t post_attach(bool wasDetached,
                                  std::set<response::ptr> &aresps);
 ...
 public:
  typedef enum { ct_fork, ct_launch, ct_attach } creationMode_t;
 ...
 public:
  ...
  static bool waitAndHandleForProc(bool block, int_process *proc,
                                   bool &proc_exited);
  ...
  virtual bool plat_individualRegRead(Dyninst::MachRegister reg,
                                      int_thread *thr);
  ...
  bool removeBreakpoint(Dyninst::Address addr, int_breakpoint *bp,
  ...
  virtual bool plat_createAllocationSnippet(Dyninst::Address addr,
                                            bool use_addr, unsigned long size,
                                            void *&buffer,
                                            unsigned long &buffer_size,
                                            unsigned long &start_offset) = 0;
  virtual bool plat_collectAllocationResult(int_thread *thr,
                                            reg_response::ptr resp) = 0;
  ...
  virtual Dyninst::Address direct_infMalloc(unsigned long size,
                                            bool use_addr = false,
                                            Dyninst::Address addr = 0x0);
  ...
  static bool infMalloc(unsigned long size, int_addressSet *aset,
                        bool use_addr);
  static bool infFree(int_addressSet *aset);

  static std::string plat_canonicalizeFileName(std::string s);

  enum bp_write_t { not_bp, bp_install, bp_clear };

  bool readMem(Dyninst::Address remote, mem_response::ptr result,
               int_thread *thr = NULL);
  bool writeMem(const void *local, Dyninst::Address remote, size_t size,
                result_response::ptr result, int_thread *thr = NULL,
                bp_write_t bp_write = not_bp);
  ...
  virtual bool plat_writeMem(int_thread *thr, const void *local,
                             ...
                             bp_write_t bp_write) = 0;

  virtual async_ret_t plat_calcTLSAddress(int_thread *thread, int_library *lib,
                                          Offset off, Address &outaddr,
                                          std::set<response::ptr> &resps);
  ...
  virtual bool plat_getOSRunningStates(
      std::map<Dyninst::LWP, bool> &runningStates) = 0;
  ...
  virtual async_ret_t plat_needsEmulatedSingleStep(
      int_thread *thr, std::vector<Dyninst::Address> &result);
  virtual bool plat_convertToBreakpointAddress(Address &, int_thread *) {
    return true;
  }
  virtual void plat_getEmulatedSingleStepAsyncs(int_thread *thr,
                                                std::set<response::ptr> resps);
 ...
 * If a new platform follows one of these, then inherit them from the
 *appropriate
 * class.
 *
 * indep_lwp_control_process - Each thread/lwp stops and continues independent
 *from
 *  each other one.  (Linux)
 * unified_lwp_control_process - There is no thread-specific control, every
 *thread
...
class indep_lwp_control_process : virtual public int_process {
 ...
 public:
  indep_lwp_control_process(Dyninst::PID p, std::string e,
...
class unified_lwp_control_process : virtual public int_process {
 ...
 public:
  unified_lwp_control_process(Dyninst::PID p, std::string e,
                              std::vector<std::string> a,
                              std::vector<std::string> envp,
                              std::map<int, int> f);
...
class hybrid_lwp_control_process : virtual public int_process {
 ...
 public:
  hybrid_lwp_control_process(Dyninst::PID p, std::string e,
                             std::vector<std::string> a,
                             std::vector<std::string> envp,
                             std::map<int, int> f);
...
/**
 * When a thread/process exits we delete the int_process/int_thread objects.
 *But,
 * we leave the UI handles Process/Thread around until the user gets rid of
 *their
 * last pointer.  There are only a few operations that are legal on an exited
 *process
 * (such as getting the pid or exitcode).  thread_exitstate and proc_exitstate
 *hold that
...
class proc_exitstate {
 public:
  ...
  void setLastError(err_t e_, const char *m) {
    last_error = e_;
    last_error_msg = m;
  }
};
...
/**
 * int_thread repesents a thread/lwp (PC assumes an M:N model of 1:1).  See the
 *comment
 * above int_process, most of which applies here.
 *
 * An int_process also holds the stopped/running state of a process.  See the
 *StateTracker
...
class int_thread {
 ...
 protected:
  ...
  static int_thread *createThreadPlat(int_process *proc, Dyninst::THR_ID thr_id,
                                      Dyninst::LWP lwp_id, bool initial_thrd);

 public:
  enum attach_status_t {
    as_unknown =
        0,  // Threads found by getThreadLWPs come in as_needs_attach, others
    as_created_attached,  // come int as_created_attached.  Up to platforms to
                          // interpret this
    as_needs_attach       // however they want.
  };

  static int_thread *createThread(int_process *proc, Dyninst::THR_ID thr_id,
                                  Dyninst::LWP lwp_id, bool initial_thrd,
...
#define RUNNING_STATE(S) \
  (S == int_thread::running || S == int_thread::neonatal_intermediate)
  typedef enum {
    none = 0,
    neonatal = 1,
  ...
  static const int NumTargetStateIDs =
      (NumStateIDs - 2);  // Handler and Generator states aren't target states
   ...
   *   variable indicating whether a thread should run.  These are the
   *StateTrackers.
   * - When we acutually decide whether a thread should stop/run, we use a
   *priority-based
  ...
  virtual bool notAvailableForRPC() { return false; }
  ...
  virtual bool addHWBreakpoint(hw_breakpoint *bp, bool resume,
                               std::set<response::ptr> &resps, bool &done);
  virtual EventBreakpoint::ptr decodeHWBreakpoint(
      response::ptr &resp, bool have_reg = false,
...
class int_threadPool {
 ...
 private:
  ...
  mutable int_thread
      *initial_thread;  // may be updated by side effect on Windows
  int_process *proc_;
  ThreadPool *up_pool;
...
/**
 * Represents a Dynamic Shared Object (aka DSO, aka .dll/.so) loaded by the
 *process.
 * Each DSO has a library name and load address.
 *
 * int_library doesn't hang directly from a process, but from its mem_state
 *object.
 **/
class int_library {
 ...
 public:
  int_library(std::string n, bool shared_lib, Dyninst::Address load_addr,
...
class int_breakpoint {
 ...
 public:
  ...
  int_breakpoint(unsigned int hw_prems_, unsigned int hw_size_,
                 Breakpoint::ptr up);
  ~int_breakpoint();
...
class bp_instance {
 ...
 public:
  ...
  virtual bool rmBreakpoint(int_process *proc, int_breakpoint *bp, bool &empty,
                            std::set<response::ptr> &resps);
  virtual async_ret_t uninstall(int_process *proc,
                                std::set<response::ptr> &resps) = 0;
  ...
  virtual async_ret_t suspend(int_process *proc,
                              std::set<response::ptr> &resps) = 0;
  virtual async_ret_t resume(int_process *proc,
                             std::set<response::ptr> &resps) = 0;
...
// Long breakpoints can be used to artifically increase the size of the BP
// write,
...
class sw_breakpoint : public bp_instance {
 ...
 public:
  ...
  static sw_breakpoint *create(int_process *proc, int_breakpoint *bp,
...
class hw_breakpoint : public bp_instance {
 ...
 private:
  ...
  hw_breakpoint(int_thread *thr, unsigned mode, unsigned size, bool pwide,
                Dyninst::Address addr);
 ...
 public:
  ...
  static hw_breakpoint *create(int_process *proc, int_breakpoint *bp,
                               Dyninst::Address addr_);
  ~hw_breakpoint();
...
struct clearError {
  void operator()(Process::ptr p) { p->clearLastError(); }
...
struct setError {
 ...
 public:
  setError(err_t e, const char *s) {
    err = e;
    err_str = s;
  }
  void operator()(Process::ptr p) { p->setLastError(err, err_str); }
...
class int_notify {
  ...
  class windows_details {
   ...
   public:
    ...
    void noteEvent() { ::ReleaseSemaphore(evt, 1, NULL); }
    void clearEvent() {
    ...
    bool internalsValid() { return evt != INVALID_HANDLE_VALUE; }
    wait_object_t getWaitObject() { return evt; }
  };
...
class MTManager {
 ...
 public:
  static const Process::thread_mode_t default_thread_mode =
      Process::HandlerThreading;
  MTManager();
  ~MTManager();
...
inline MTManager *mt() { return MTManager::mt_; }
...
class MTLock {
 ...
 public:
  ...
  MTLock() {
    assert(!isGeneratorThread());
    should_unlock = true;
    if (mt()->handlerThreading()) mt()->startWork();
  }

  ~MTLock() {
    if (should_unlock && mt()->handlerThreading()) mt()->endWork();
  }
};
