...
public:
    ...
    virtual Dyninst::Address direct_infMalloc(unsigned long size, bool use_addr = false,
    ...
    virtual bool plat_readMem(int_thread* thr, void* local, Dyninst::Address remote,
                              size_t size)          = 0;
...
#    define RUNNING_STATE(S)                                                             \
     ...
     *priority-based projection to reduce the multiple StateTrackers from each subsystem
     *into a single target state (this happens in int_process::syncRunState).
     *
...
public:
    int_library(std::string n, bool shared_lib, Dyninst::Address load_addr,
                Dyninst::Address dynamic_load_addr, Dyninst::Address data_load_addr = 0,
...
extern void
setGeneratorThread(long t);
void
setHandlerThread(long t);
bool
isGeneratorThread();
bool
isHandlerThread();
...
isUserThread();
HandlerPool*
createDefaultHandlerPool(int_process* p);
HandlerPool*
