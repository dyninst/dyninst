...
static void clear_ss_state_cb(int_thread *thr) {
   amdgpu_process *proc = dynamic_cast<amdgpu_process *>(thr->llproc());
...
{
   amdgpu_thread *amdgpu_thr = dynamic_cast<amdgpu_thread *>(thr);
   assert(amdgpu_thr);
   if (amdgpu_thr->haveCachedPC(pc))
      return aret_success;
   ...
   if (!plat_needsAsyncIO())
   {
      ...
      bool result = amdgpu_thr->getRegister(MachRegister::getPC(getTargetArch()), pcResponse);
