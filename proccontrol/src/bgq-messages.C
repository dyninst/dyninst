...
#define CMD_RET_SIZE(X) \
  case X:               \
    return sizeof(X##Cmd)
#define CMD_RET_VAR_SIZE(X, F) \
  case X:                      \
    return sizeof(X##Cmd) + static_cast<const X##Cmd &>(cmd).F
uint32_t bgq_process::getCommandLength(uint16_t cmd_type,
                                       const ToolCommand &cmd) {
  switch (cmd_type) {
...
#define CMD_RET_ACK_SIZE(X) \
  case X:                   \
    return sizeof(X##AckCmd)
#define CMD_RET_VAR_ACK_SIZE(X, F) \
  case X:                          \
    return sizeof(X##AckCmd) + static_cast<const X##Cmd &>(cmd).F
uint32_t bgq_process::getCommandAckLength(uint16_t cmd_type,
                                          const ToolCommand &cmd) {
  switch (cmd_type) {
...
#define MESSAGE_RET_SIZE(X) \
  case X:                   \
    return sizeof(X##Message)
