 ...
 public:
  ...
  iRPCAllocation()
      : addr(0),
        ...
        start_offset(0),
        orig_data(NULL),
        // HACK: affirmatively set that we do need a data save. If we've just
...
class int_iRPC : public boost::enable_shared_from_this<int_iRPC> {
 ...
 public:
  ...
  int_iRPC(void *binary_blob_, unsigned long binary_size_, bool async_,
           bool alreadyAllocated = false, Dyninst::Address addr = 0);
...
class iRPCMgr {
 ...
 public:
  ...
  iRPCAllocation::ptr findAllocationForRPC(int_thread *thread,
                                           int_iRPC::ptr rpc);
  ...
  int_iRPC::ptr createInfMallocRPC(int_process *proc, unsigned long size,
                                   bool use_addr, Dyninst::Address addr);
  int_iRPC::ptr createInfFreeRPC(int_process *proc, unsigned long size,
                                 Dyninst::Address addr);
...
class rpc_wrapper {
 public:
  rpc_wrapper(int_iRPC::ptr rpc_) : rpc(rpc_) {}

  rpc_wrapper(rpc_wrapper *w) : rpc(w->rpc) {}
