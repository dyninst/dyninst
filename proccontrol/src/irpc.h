...
public:
    ...
    , orig_data(NULL)
    ,
    // HACK: affirmatively set that we do need a data save. If we've just allocated space,
    // why save the data?
...
public:
    ...
    int_iRPC::ptr createInfMallocRPC(int_process* proc, unsigned long size, bool use_addr,
                                     Dyninst::Address addr);
