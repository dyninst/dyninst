...
void MailboxMT::lock_queue() { message_cond.lock(); }

void MailboxMT::unlock_queue() { message_cond.unlock(); }

void MailboxMT::enqueue(Event::ptr ev, bool priority) {
  enqueue_worker(ev, priority, false);
}

void MailboxMT::enqueue_user(Event::ptr ev) { enqueue_worker(ev, false, true); }
...
void MailboxMT::enqueue_worker(Event::ptr ev, bool priority, bool user) {
  ...
  pthrd_printf(
      "Added event %s to mailbox, size = %lu + %lu + %lu = %lu\n",
