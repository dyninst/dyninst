...
void
MailboxMT::lock_queue()
{
    message_cond.lock();
}

void
MailboxMT::unlock_queue()
{
    message_cond.unlock();
}

void
MailboxMT::enqueue(Event::ptr ev, bool priority)
{
    enqueue_worker(ev, priority, false);
}

void
MailboxMT::enqueue_user(Event::ptr ev)
{
    enqueue_worker(ev, false, true);
}

void
MailboxMT::enqueue_worker(Event::ptr ev, bool priority, bool user)
{
    message_cond.lock();
    ...
    pthrd_printf("Added event %s to mailbox, size = %lu + %lu + %lu = %lu\n",
                 ev->name().c_str(), (unsigned long) message_queue.size(),
...
MailboxMT::peek()
{
    message_cond.lock();
    queue<Event::ptr>& q =
        !priority_message_queue.empty() ? priority_message_queue : message_queue;
    if(q.empty())
    {
        message_cond.unlock();
        return Event::ptr();
...
MailboxMT::dequeue(bool block)
{
    ...
    while(priority_message_queue.empty() && message_queue.empty() &&
          (!user_thread || user_message_queue.empty()))
    {
        if(!block)
        {
...
MailboxMT::size()
{
    message_cond.lock();
    unsigned int result =
        (unsigned int) (message_queue.size() + priority_message_queue.size() +
                        user_message_queue.size());
    message_cond.unlock();
    return result;
}
