...
/**
 * AddressSet implementation follows.  This is essentially a std::multimap of Address ->
 *Process::ptr plus some additional features:
...
AddressSet::ptr
AddressSet::newAddressSet(ProcessSet::const_ptr ps, Address addr)
{
    AddressSet::ptr newset = AddressSet::ptr(new AddressSet);
    newset->iaddrs         = new int_addressSet();
...
AddressSet::ptr
AddressSet::newAddressSet(ProcessSet::const_ptr ps, string library_name, Offset off)
{
...
AddressSet::ptr
AddressSet::newAddressSet(Process::const_ptr p, Address addr)
{
    AddressSet::ptr newset = AddressSet::ptr(new AddressSet);
    newset->iaddrs         = new int_addressSet();
...
AddressSet::ptr
AddressSet::newAddressSet(ProcessSet::ptr ps, Address addr)
{
    AddressSet::ptr newset = AddressSet::ptr(new AddressSet);
    newset->iaddrs         = new int_addressSet();
...
AddressSet::ptr
AddressSet::newAddressSet(ProcessSet::ptr ps, string library_name, Offset off)
{
    MTLock lock_this_func;
...
AddressSet::ptr
AddressSet::newAddressSet(Process::ptr p, Address addr)
{
    AddressSet::ptr newset = AddressSet::ptr(new AddressSet);
    newset->iaddrs         = new int_addressSet();
    newset->iaddrs->insert(value_type(addr, p->llproc()->proc()));
...
AddressSet::iterator
AddressSet::find(Address a)
{
    return iaddrs->find(a);
}

AddressSet::iterator
AddressSet::find(Address a, Process::const_ptr p)
{
...
AddressSet::const_iterator
AddressSet::find(Address a) const
{
    return iaddrs->find(a);
}

AddressSet::const_iterator
AddressSet::find(Address a, Process::const_ptr p) const
{
...
size_t
AddressSet::count(Address a) const
{
...
pair<AddressSet::iterator, bool>
AddressSet::insert(Address a, Process::const_ptr p)
{
    Process::ptr         ncp = pc_const_cast<Process>(p);
    pair<iterator, bool> result;
    for(result.first = iaddrs->find(a);
        result.first != iaddrs->end() && result.first->first == a; result.first++)
    {
...
size_t
AddressSet::insert(Address a, ProcessSet::const_ptr ps)
{
...
pair<AddressSet::iterator, bool>
AddressSet::insert(Address a, Process::ptr p)
{
    Process::ptr         ncp = pc_const_cast<Process>(p);
    pair<iterator, bool> result;
    for(result.first = iaddrs->find(a);
...
size_t
AddressSet::insert(Address a, ProcessSet::ptr ps)
{
    size_t count_added = 0;
    for(ProcessSet::iterator i = ps->begin(); i != ps->end(); i++)
    {
...
size_t
AddressSet::erase(Process::const_ptr p)
{
    size_t       num_erased = 0;
    Process::ptr ncp        = pc_const_cast<Process>(p);
...
size_t
AddressSet::erase(Address a, Process::const_ptr p)
{
    Process::ptr ncp = pc_const_cast<Process>(p);
    iterator     i   = find(a, p);
    if(i == end())
...
AddressSet::iterator
AddressSet::lower_bound(Address a)
{
    return iaddrs->lower_bound(a);
}

AddressSet::iterator
AddressSet::upper_bound(Address a)
{
    return iaddrs->upper_bound(a);
}

pair<AddressSet::iterator, AddressSet::iterator>
AddressSet::equal_range(Address a)
{
    return iaddrs->equal_range(a);
}

AddressSet::const_iterator
AddressSet::lower_bound(Address a) const
{
    return iaddrs->lower_bound(a);
}

AddressSet::const_iterator
AddressSet::upper_bound(Address a) const
{
    return iaddrs->upper_bound(a);
}
...
AddressSet::set_union(AddressSet::const_ptr pp) const
{
    ...
    std::set_union(iaddrs->begin(), iaddrs->end(), pp->iaddrs->begin(), pp->iaddrs->end(),
...
static Process::const_ptr
get_proc(const int_addressSet::iterator& i, err_t*)
{
    return i->second;
}

static int_addressSet::iterator
get_begin(AddressSet::ptr as)
{
    return as->get_iaddrs()->begin();
}

static int_addressSet::iterator
get_end(AddressSet::ptr as)
{
    return as->get_iaddrs()->end();
}

static void
thread_err_check(int_thread* ithr, err_t* thread_error)
{
    if(!ithr)
    {
...
static Process::const_ptr
get_proc(const map<Thread::const_ptr, MachRegisterVal>::const_iterator& i,
         err_t*                                                         thread_error)
{
    if(thread_error)
...
static Process::const_ptr
get_proc(const map<Thread::const_ptr, RegisterPool>::const_iterator& i,
         err_t*                                                      thread_error)
{
    if(thread_error)
        thread_err_check(i->first->llthrd(), thread_error);
...
static Process::const_ptr
get_proc(const multimap<Thread::const_ptr, IRPC::ptr>::const_iterator& i,
...
static Process::const_ptr
get_proc(const int_processSet::iterator& i, err_t*)
{
    return *i;
}

static Process::const_ptr
get_proc(const int_threadSet::iterator& i, err_t* thread_error = NULL)
{
    if(thread_error)
...
ProcessSet::ptr
ProcessSet::newProcessSet(Process::const_ptr pp)
{
...
ProcessSet::ptr
ProcessSet::newProcessSet(ProcessSet::const_ptr pp)
{
    return newProcessSet(*pp->procset);
}

ProcessSet::ptr
ProcessSet::newProcessSet(const set<Process::ptr>& procs)
{
    ProcessSet::ptr newps = newProcessSet();
...
ProcessSet::ptr
ProcessSet::newProcessSet(Process::ptr p)
{
    ProcessSet::ptr newps = newProcessSet();
    newps->insert(p);
    return newps;
...
ProcessSet::ptr
ProcessSet::newProcessSet(ProcessSet::ptr pp)
{
    return newProcessSet(*pp->procset);
}
...
ProcessSet::ptr
ProcessSet::newProcessSet(const set<Process::const_ptr>& procs)
{
    ProcessSet::ptr newps  = newProcessSet();
...
ProcessSet::ptr
ProcessSet::newProcessSet(AddressSet::const_iterator begin,
...
ProcessSet::ptr
ProcessSet::createProcessSet(vector<CreateInfo>& cinfo)
{
...
ProcessSet::ptr
ProcessSet::attachProcessSet(vector<AttachInfo>& ainfo)
{
    MTLock lock_this_func(MTLock::allow_init, MTLock::deliver_callbacks);
...
ProcessSet::iterator
ProcessSet::find(Process::const_ptr p)
{
    return iterator(procset->find(pc_const_cast<Process>(p)));
}

ProcessSet::iterator
ProcessSet::find(PID p)
{
    ProcPool()->condvar()->lock();
...
ProcessSet::const_iterator
ProcessSet::find(Process::const_ptr p) const
{
    return const_iterator(procset->find(pc_const_cast<Process>(p)));
}

ProcessSet::const_iterator
ProcessSet::find(PID p) const
{
    ProcPool()->condvar()->lock();
    int_process* llproc = ProcPool()->findProcByPid(p);
    ProcPool()->condvar()->unlock();
...
pair<ProcessSet::iterator, bool>
ProcessSet::insert(Process::const_ptr p)
{
...
size_t
ProcessSet::erase(Process::const_ptr p)
{
...
const MemoryUsageSet*
ProcessSet::getMemoryUsage() const
{
...
void
ProcessSet::getErrorSubsets(map<ProcControlAPI::err_t, ProcessSet::ptr>& err_sets) const
{
    MTLock lock_this_func;
...
static bool
all_match(iter b, iter e, pred p)
{
    bool result = true;
    for(iter i = b; i != e; i++)
    {
...
template <class iter, class pred>
static bool
any_match(iter b, iter e, pred p)
{
    bool result = false;
    for(iter i = b; i != e; i++)
    {
...
template <class iter, class pred>
static ProcessSet::ptr
create_subset(iter b, iter e, pred p)
{
...
template <class iter, class pred>
static ThreadSet::ptr
create_thrsubset(iter b, iter e, pred p)
{
...
struct test_thr
{
    ...
    test_thr(test_t t_, int_thread::State ts_)
    : test(t_)
    , ts(ts_)
    {}
...
static bool
do_detach(int_processSet* procset, bool temporary, bool leaveStopped)
{
    MTLock lock_this_func(MTLock::deliver_callbacks);
    bool   had_error   = false;
    bool   had_success = false;
...
bool
ProcessSet::freeMemory(AddressSet::ptr addrset) const
{
    bool   had_error = false;
    MTLock lock_this_func(MTLock::deliver_callbacks);
    if(int_process::isInCB())
...
bool
ProcessSet::readMemory(AddressSet::ptr addrset, multimap<Process::ptr, void*>& mem_result,
                       size_t size) const
{
...
bool
ProcessSet::readMemory(AddressSet::ptr addrset, map<void*, ProcessSet::ptr>& mem_result,
...
bool
ProcessSet::readMemory(multimap<Process::const_ptr, read_t>& addrs) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ProcessSet::writeMemory(AddressSet::ptr addrset, const void* buffer, size_t size) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ProcessSet::writeMemory(multimap<Process::const_ptr, write_t>& addrs) const
{
    MTLock lock_this_func;
    bool   had_error = false;
    for_each(procset->begin(), procset->end(), clearError());
...
static bool
addBreakpointWorker(set<pair<int_process*, bp_install_state*>>& bp_installs)
{
    bool had_error = false;
    bool result;
...
bool
ProcessSet::addBreakpoint(AddressSet::ptr addrset, Breakpoint::ptr bp) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ProcessSet::rmBreakpoint(AddressSet::ptr addrset, Breakpoint::ptr bp) const
{
    MTLock lock_this_func;
    bool   had_error = false;
    for_each(procset->begin(), procset->end(), clearError());
...
bool
ProcessSet::postIRPC(const multimap<Process::const_ptr, IRPC::ptr>& rpcs) const
{
    MTLock lock_this_func;
    bool   had_error = false;

    rpcmap_iter iter("Post RPC", had_error, ERR_CHCK_NORM);
    for(multimap<Process::const_ptr, IRPC::ptr>::const_iterator i = iter.begin(&rpcs);
...
bool
ProcessSet::postIRPC(IRPC::ptr irpc, multimap<Process::ptr, IRPC::ptr>* result) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ProcessSet::postIRPC(IRPC::ptr irpc, AddressSet::ptr addrset,
...
bool
ProcessSet::iterator::operator==(const ProcessSet::iterator& i) const
{
...
bool
ProcessSet::const_iterator::operator==(const ProcessSet::const_iterator& i) const
{
    return int_iter == i.int_iter;
}
...
{
    Thread::ptr operator()(Thread::const_ptr t) const { return pc_const_cast<Thread>(t); }
};
...
ThreadSet::ptr
ThreadSet::newThreadSet(const ThreadPool& threadp)
{
...
ThreadSet::ptr
ThreadSet::newThreadSet(const set<Thread::const_ptr>& threads)
{
    ThreadSet::ptr  newts  = ThreadSet::ptr(new ThreadSet());
    int_threadSet*& newset = newts->ithrset;
...
ThreadSet::ptr
ThreadSet::newThreadSet(Thread::ptr thr)
{
    ThreadSet::ptr newts = ThreadSet::ptr(new ThreadSet());
...
ThreadSet::ptr
ThreadSet::newThreadSet(ProcessSet::ptr ps, bool initial_only)
{
    MTLock lock_this_func;
    bool   had_error = false;
...
ThreadSet::iterator
ThreadSet::find(Thread::const_ptr p)
{
    return iterator(ithrset->find(pc_const_cast<Thread>(p)));
}

ThreadSet::const_iterator
...
ThreadSet::const_iterator
ThreadSet::find(Thread::const_ptr p) const
{
...
pair<ThreadSet::iterator, bool>
ThreadSet::insert(Thread::const_ptr p)
{
...
void
ThreadSet::erase(iterator pos)
{
    ithrset->erase(pos.int_iter);
}

size_t
ThreadSet::erase(Thread::const_ptr t)
{
...
void
ThreadSet::getErrorSubsets(map<ProcControlAPI::err_t, ThreadSet::ptr>& err_sets) const
{
    MTLock lock_this_func;
...
static bool
getRegisterWorker(Dyninst::MachRegister reg, int_threadSet* ithrset,
...
bool
ThreadSet::getRegister(Dyninst::MachRegister                       reg,
...
bool
ThreadSet::setRegister(Dyninst::MachRegister                                   reg,
...
bool
ThreadSet::getAllRegisters(map<Thread::ptr, RegisterPool>& results) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ThreadSet::setAllRegisters(const map<Thread::const_ptr, RegisterPool>& reg_vals) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ThreadSet::postIRPC(const multimap<Thread::const_ptr, IRPC::ptr>& rpcs) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ThreadSet::postIRPC(IRPC::ptr irpc, multimap<Thread::ptr, IRPC::ptr>* results) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
ThreadSet::iterator::operator==(const iterator& i)
{
    return int_iter == i.int_iter;
}
...
ThreadSet::const_iterator::const_iterator(int_threadSet::iterator i) { int_iter = i; }
...
bool
ThreadSet::const_iterator::operator==(const const_iterator& i)
{
    return int_iter == i.int_iter;
}
...
LWPTrackingSet::refreshLWPs() const
{
    ...
    for(set<int_process*>::iterator i = change_procs.begin(); i != change_procs.end();
        i++)
    {
        int_process* proc = *i;
...
bool
FollowForkSet::setFollowFork(FollowFork::follow_t f) const
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
RemoteIOSet::getFileNames(FileSet* fset)
{
    MTLock lock_this_func;
    bool   had_error = false;
...
RemoteIOSet::getFileStatData(FileSet* fset)
{
    ...
    for(FileSet::iterator i = fset->begin(); i != fset->end(); i++)
    {
        ...
        bool result =
            proc->plat_getFileStatData(info->filename, &info->stat_results, all_resps);
        if(!result)
        {
...
bool
RemoteIOSet::readFileContents(const FileSet* fset)
{
    MTLock lock_this_func;
    bool   had_error = false;
...
bool
MemoryUsageSet::usedX(std::map<Process::const_ptr, unsigned long>& used,
                      MemoryUsageSet::mem_usage_t                  mu) const
{
    ...
    map<int_memUsage*, MemUsageResp_t*> shared_results, stack_results, heap_results,
        resident_results;
    ...
    if(mu == mus_resident)
    {
            ...
            i                          = iter.inc())
        {
            ...
            {
                perr_printf("Failed to read shared stack or heap on process %d for "
                            "resident memory\n",
                            proc->getPid());
                continue;
            }
            ...
            if(!result)
            {
                perr_printf("Error calculating resident usage from stack, heap and "
                            "shared on %d\n",
                            proc->getPid());
                delete resp;
                continue;
...
bool
MemoryUsageSet::sharedUsed(std::map<Process::const_ptr, unsigned long>& used) const
{
    return usedX(used, mus_shared);
}

bool
MemoryUsageSet::heapUsed(std::map<Process::const_ptr, unsigned long>& used) const
{
    return usedX(used, mus_heap);
}

bool
MemoryUsageSet::stackUsed(std::map<Process::const_ptr, unsigned long>& used) const
{
    return usedX(used, mus_stack);
}

bool
MemoryUsageSet::resident(std::map<Process::const_ptr, unsigned long>& res) const
{
