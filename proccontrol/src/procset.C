...
TSetFeatures::TSetFeatures() : stkset(NULL) {}
...
/**
 * AddressSet implementation follows.  This is essentially a std::multimap of
 *Address -> Process::ptr
 * plus some additional features:
 *  - Ability to create addresses based on ProcControlAPI objects, such as
 *libraries.
...
AddressSet::AddressSet() : iaddrs(NULL) {}
...
AddressSet::ptr AddressSet::newAddressSet(ProcessSet::const_ptr ps,
                                          Address addr) {
  AddressSet::ptr newset = AddressSet::ptr(new AddressSet);
  newset->iaddrs = new int_addressSet();
...
AddressSet::ptr AddressSet::newAddressSet(ProcessSet::const_ptr ps,
                                          string library_name, Offset off) {
...
AddressSet::ptr AddressSet::newAddressSet(ProcessSet::ptr ps,
                                          string library_name, Offset off) {
  MTLock lock_this_func;
...
AddressSet::iterator AddressSet::begin() { return iaddrs->begin(); }

AddressSet::iterator AddressSet::end() { return iaddrs->end(); }

AddressSet::iterator AddressSet::find(Address a) { return iaddrs->find(a); }

AddressSet::iterator AddressSet::find(Address a, Process::const_ptr p) {
  pair<iterator, iterator> range = equal_range(a);
  for (iterator i = range.first; i != range.second; i++) {
    if (i->second == p) return i;
  }
  return end();
}

AddressSet::const_iterator AddressSet::begin() const { return iaddrs->begin(); }

AddressSet::const_iterator AddressSet::end() const { return iaddrs->end(); }

AddressSet::const_iterator AddressSet::find(Address a) const {
  return iaddrs->find(a);
}

AddressSet::const_iterator AddressSet::find(Address a,
                                            Process::const_ptr p) const {
...
size_t AddressSet::count(Address a) const { return iaddrs->count(a); }

size_t AddressSet::size() const { return iaddrs->size(); }

bool AddressSet::empty() const { return iaddrs->empty(); }

pair<AddressSet::iterator, bool> AddressSet::insert(Address a,
                                                    Process::const_ptr p) {
  Process::ptr ncp = pc_const_cast<Process>(p);
  pair<iterator, bool> result;
  for (result.first = iaddrs->find(a);
       result.first != iaddrs->end() && result.first->first == a;
       result.first++) {
...
void AddressSet::erase(AddressSet::iterator pos) { iaddrs->erase(pos); }
...
void AddressSet::clear() { iaddrs->clear(); }
...
pair<AddressSet::iterator, AddressSet::iterator> AddressSet::equal_range(
    Address a) {
...
pair<AddressSet::const_iterator, AddressSet::const_iterator>
AddressSet::equal_range(Address a) const {
  return iaddrs->equal_range(a);
}
...
AddressSet::ptr AddressSet::set_union(AddressSet::const_ptr pp) const {
  ...
  std::set_union(iaddrs->begin(), iaddrs->end(), pp->iaddrs->begin(),
                 pp->iaddrs->end(),
...
AddressSet::ptr AddressSet::set_intersection(AddressSet::const_ptr pp) const {
  ...
  std::set_intersection(iaddrs->begin(), iaddrs->end(), pp->iaddrs->begin(),
                        pp->iaddrs->end(),
                        inserter(*newset->iaddrs, newset->iaddrs->end()));
...
AddressSet::ptr AddressSet::set_difference(AddressSet::const_ptr pp) const {
  ...
  std::set_difference(iaddrs->begin(), iaddrs->end(), pp->iaddrs->begin(),
                      pp->iaddrs->end(),
                      inserter(*newset->iaddrs, newset->iaddrs->end()));
 ...
 * real work. These classes/templates are an attempt to bring all that error
 *checking
 * and iteration into a common place.
 *
 * Things are complicated by the fact that we use many different types of
 *collections
 * We might be operating over sets of Process::ptrs, or multimaps from
 *Thread::ptr to
 * register values, etc.  The common iteration and error handling code is in the
 *iter_t
 * template, which takes the type of collection it's iterating over as template
 *parameters.
 *
 * The three big operations iter_t needs to do is extract a process from an
 *iterator and
 * get the begin/end iterators from a collection.  These operations are done by
 *a set
 * of overloaded functions: get_proc, get_begin, and get_end.  We have an
 *instance of these
...
static Process::const_ptr get_proc(
    const map<Thread::const_ptr, MachRegisterVal>::const_iterator &i,
    err_t *thread_error) {
  if (thread_error) thread_err_check(i->first->llthrd(), thread_error);
  return i->first->getProcess();
}

static Process::const_ptr get_proc(
    const map<Thread::const_ptr, RegisterPool>::const_iterator &i,
    err_t *thread_error) {
  if (thread_error) thread_err_check(i->first->llthrd(), thread_error);
  return i->first->getProcess();
}
...
static Process::const_ptr get_proc(
    const multimap<Thread::const_ptr, IRPC::ptr>::const_iterator &i,
    err_t *thread_error = NULL) {
  if (thread_error) thread_err_check(i->first->llthrd(), thread_error);
  return i->first->getProcess();
}
...
static Process::const_ptr get_proc(const int_threadSet::iterator &i,
                                   err_t *thread_error = NULL) {
  if (thread_error) thread_err_check((*i)->llthrd(), thread_error);
  return (*i)->getProcess();
}
 ...
 private:
  ...
  bool proc_check(Process::const_ptr p, err_t thr_error) {
    ...
    if (!proc) {
      ...
      if (p)
        p->setLastError(err_exited, "Operation attempted on exited process");
      had_error = true;
      return false;
    }
    ...
    if ((flags & ERR_CHCK_THRD_STOPPED) && thr_error == err_notrunning) {
      perr_printf("%s attempted on running thread in process %d\n", msg,
                  p->getPid());
      p->setLastError(err_notrunning,
                      "Group operation attempted on running thread");
      had_error = true;
      return false;
    }
    if ((flags & ERR_CHCK_EXITED) && !proc) {
      perr_printf("%s attempted on exited process %d\n", msg, p->getPid());
      p->setLastError(err_exited,
                      "Group operation attempted on exited process");
      had_error = true;
      return false;
    }
    if ((flags & ERR_CHCK_DETACHED) && p->isDetached()) {
      perr_printf("%s attempted on detached process %d\n", msg, proc->getPid());
      p->setLastError(err_detached,
                      "Group operation attempted on detached process");
      had_error = true;
      return false;
    }
    if ((flags & ERR_CHCK_STOPPED) && p->hasRunningThread()) {
      perr_printf("%s attempted on running process %d\n", msg, proc->getPid());
      p->setLastError(err_notstopped,
                      "Group operation attempted on running process");
      had_error = true;
      return false;
    }
    return true;
 ...
 public:
  ...
  iterator_t begin(cont_t c) {
    ...
    if (!finished_clear && (flags & THREAD_CLEAR_ERRS)) {
      ...
      if (did_begin) finished_clear = true;
    }
    did_begin = true;
    ...
    if (!proc_check(proc, thr_error)) return inc();
    return iter;
  }

  iterator_t end() { return get_end(container); }
...
typedef iter_t<multimap<Process::const_ptr, ProcessSet::read_t> *,
               multimap<Process::const_ptr, ProcessSet::read_t>::iterator>
    readmap_iter;
typedef iter_t<multimap<Process::const_ptr, ProcessSet::write_t> *,
               multimap<Process::const_ptr, ProcessSet::write_t>::iterator>
    writemap_iter;
typedef iter_t<const multimap<Process::const_ptr, IRPC::ptr> *,
               multimap<Process::const_ptr, IRPC::ptr>::const_iterator>
    rpcmap_iter;
typedef iter_t<const multimap<Thread::const_ptr, IRPC::ptr> *,
               multimap<Thread::const_ptr, IRPC::ptr>::const_iterator>
    rpcmap_thr_iter;
typedef iter_t<int_threadSet *, int_threadSet::iterator> thrset_iter;
typedef iter_t<const map<Thread::const_ptr, Dyninst::MachRegisterVal> *,
               map<Thread::const_ptr, Dyninst::MachRegisterVal>::const_iterator>
    setreg_iter;
typedef iter_t<const map<Thread::const_ptr, RegisterPool> *,
               map<Thread::const_ptr, RegisterPool>::const_iterator>
    setallreg_iter;

ProcessSet::ProcessSet() : features(NULL) { procset = new int_processSet; }
...
ProcessSet::ptr ProcessSet::newProcessSet(const set<Process::ptr> &procs) {
  ProcessSet::ptr newps = newProcessSet();
  copy(procs.begin(), procs.end(),
       inserter(*newps->procset, newps->procset->end()));
  return newps;
}
...
ProcessSet::ptr ProcessSet::newProcessSet(
    const set<Process::const_ptr> &procs) {
  ProcessSet::ptr newps = newProcessSet();
  int_processSet &newset = *newps->procset;
  transform(procs.begin(), procs.end(), inserter(newset, newset.end()),
            proc_strip_const());
  return newps;
}

ProcessSet::ptr ProcessSet::newProcessSet(AddressSet::const_iterator begin,
...
ProcessSet::ptr ProcessSet::createProcessSet(vector<CreateInfo> &cinfo) {
  ...
  if (int_process::isInCB()) {
    ...
    for (vector<CreateInfo>::iterator i = cinfo.begin(); i != cinfo.end();
         i++) {
  ...
  for (vector<CreateInfo>::iterator i = cinfo.begin(); i != cinfo.end(); i++) {
    ...
    int_process *llproc =
        int_process::createProcess(i->executable, i->argv, i->envp, i->fds);
  ...
  for (ProcessSet::iterator i = newps->begin(); i != newps->end();) {
    ...
    map<int_process *, vector<CreateInfo>::iterator>::iterator j =
        info_map.find(proc);
...
ProcessSet::ptr ProcessSet::attachProcessSet(vector<AttachInfo> &ainfo) {
  ...
  if (int_process::isInCB()) {
    perr_printf("User attempted call on process create while in CB, erroring.");
    for (vector<AttachInfo>::iterator i = ainfo.begin(); i != ainfo.end();
         i++) {
      i->error_ret = err_incallback;
    }
    return ProcessSet::ptr();
  ...
  for (ProcessSet::iterator i = newps->begin(); i != newps->end();) {
    int_process *proc = (*i)->llproc();
    map<int_process *, vector<AttachInfo>::iterator>::iterator j =
        info_map.find(proc);
    assert(j != info_map.end());
...
ProcessSet::ptr ProcessSet::set_union(ProcessSet::ptr pp) const {
  ...
  std::set_union(me->begin(), me->end(), you->begin(), you->end(),
                 inserter(*them, them->end()));
  return newps;
}
...
ProcessSet::ptr ProcessSet::set_intersection(ProcessSet::ptr pp) const {
  ...
  std::set_intersection(me->begin(), me->end(), you->begin(), you->end(),
                        inserter(*them, them->end()));
  return newps;
}
...
ProcessSet::ptr ProcessSet::set_difference(ProcessSet::ptr pp) const {
  ...
  std::set_difference(me->begin(), me->end(), you->begin(), you->end(),
                      inserter(*them, them->end()));
  return newps;
}

ProcessSet::iterator ProcessSet::begin() { return iterator(procset->begin()); }

ProcessSet::iterator ProcessSet::end() { return iterator(procset->end()); }
...
bool ProcessSet::empty() const { return procset->empty(); }

size_t ProcessSet::size() const { return procset->size(); }

pair<ProcessSet::iterator, bool> ProcessSet::insert(Process::const_ptr p) {
  pair<int_processSet::iterator, bool> result =
      procset->insert(pc_const_cast<Process>(p));
  return pair<ProcessSet::iterator, bool>(ProcessSet::iterator(result.first),
                                          result.second);
}
...
void ProcessSet::clear() { procset->clear(); }

LibraryTrackingSet *ProcessSet::getLibraryTracking() {
  if (features && features->libset) return features->libset;

  MTLock lock_this_func;
  if (!procset) return NULL;
  if (!features) {
    features = new PSetFeatures();
  }
  for (int_processSet::iterator i = procset->begin(); i != procset->end();
       i++) {
    Process::ptr p = *i;
...
ThreadTrackingSet *ProcessSet::getThreadTracking() {
  if (features && features->thrdset) return features->thrdset;
...
LWPTrackingSet *ProcessSet::getLWPTracking() {
  if (features && features->lwpset) return features->lwpset;
...
FollowForkSet *ProcessSet::getFollowFork() {
  if (features && features->forkset) return features->forkset;
...
RemoteIOSet *ProcessSet::getRemoteIO() {
  if (features && features->ioset) return features->ioset;
...
MemoryUsageSet *ProcessSet::getMemoryUsage() {
  if (features && features->memset) return features->memset;
...
void ProcessSet::getErrorSubsets(
    map<ProcControlAPI::err_t, ProcessSet::ptr> &err_sets) const {
  MTLock lock_this_func;
...
struct test_exited {
  bool operator()(Process::ptr p) {
    p->clearLastError();
    int_process *llproc = p->llproc();
    if (!llproc) return p->exitstate()->exited;
    return false;
  }
};
...
struct test_crashed {
  bool operator()(Process::ptr p) {
    p->clearLastError();
    int_process *llproc = p->llproc();
    if (!llproc) return p->exitstate()->crashed;
    return false;
  }
};
...
struct test_thr {
  ...
  bool operator()(Process::ptr p) {
    ...
    for (int_threadPool::iterator i = tp->begin(); i != tp->end(); i++) {
      ...
      if (match && test == any) return true;
      if (!match && test == all) return false;
    }
    return (test == all);
  }
...
bool ProcessSet::anyThreadStopped() const {
  MTLock lock_this_func;
  return any_match(procset->begin(), procset->end(),
                   test_thr(test_thr::any, int_thread::stopped));
}

bool ProcessSet::allThreadsStopped() const {
  MTLock lock_this_func;
  return all_match(procset->begin(), procset->end(),
                   test_thr(test_thr::all, int_thread::stopped));
}

ProcessSet::ptr ProcessSet::getAllThreadStoppedSubset() const {
  MTLock lock_this_func;
  return create_subset(procset->begin(), procset->end(),
                       test_thr(test_thr::all, int_thread::stopped));
}
...
bool ProcessSet::anyThreadRunning() const {
  MTLock lock_this_func;
  return any_match(procset->begin(), procset->end(),
                   test_thr(test_thr::any, int_thread::running));
}

bool ProcessSet::allThreadsRunning() const {
  MTLock lock_this_func;
  return all_match(procset->begin(), procset->end(),
                   test_thr(test_thr::all, int_thread::running));
}
...
bool ProcessSet::continueProcs() const {
  ...
  if (int_process::isInCB()) {
    ...
    for_each(procset->begin(), procset->end(),
             setError(err_incallback, "Cannot continueProc from callback\n"));
    return false;
  }

  procset_iter iter("continueProc", had_error, ERR_CHCK_NORM);
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
       i = iter.inc()) {
    Process::ptr p = *i;
...
bool ProcessSet::stopProcs() const {
  ...
  if (!had_success) return false;

  bool result = int_process::waitAndHandleEvents(false);
  if (!result) {
    perr_printf("Internal error calling waitAndHandleEvents\n");
    for_each(
        procset->begin(), procset->end(),
        setError(
            err_internal,
            "Error while calling waitAndHandleForProc from process stop\n"));
    return false;
  }
...
static bool do_detach(int_processSet *procset, bool temporary,
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (temporary && !proc->plat_supportDOTF()) {
      ...
      p->setLastError(err_unsupported,
                      "Temporary detach not supported on this platform\n");
      had_error = true;
      continue;
    }
    ...
    if (has_rpc) {
      ...
      p->setLastError(err_pendingirpcs,
                      "Process has pending iRPCs, cannot detach\n");
      had_error = true;
      continue;
    }
    ...
    if (!result) {
      ...
      for_each(
          ...
          setError(err_internal,
                   "Error while calling waitAndHandleForProc from detach\n"));
      return false;
    }
  }
...
bool ProcessSet::reAttach() const {
  ...
  if (int_process::isInCB()) {
    perr_printf("User attempted call on process while in CB, erroring.");
    for_each(procset->begin(), procset->end(),
             setError(err_incallback, "Cannot reAttach from callback\n"));
    return false;
  }
...
bool ProcessSet::terminate() const {
  ...
  if (int_process::isInCB()) {
    perr_printf("User attempted call on process while in CB, erroring.");
    for_each(procset->begin(), procset->end(),
             setError(err_incallback, "Cannot terminate from callback\n"));
    return false;
  }
  ...
  if (run_sync) {
    ...
    if (!result) {
      ...
      for_each(
          procset->begin(), procset->end(),
          setError(err_internal,
                   "Error while calling waitAndHandleEvents from terminate\n"));
      had_error = true;
    }
  }
...
AddressSet::ptr ProcessSet::mallocMemory(size_t size) const {
  ...
  if (int_process::isInCB()) {
    perr_printf("User attempted call on process while in CB, erroring.");
    for_each(procset->begin(), procset->end(),
             setError(err_incallback, "Cannot mallocMemory from callback\n"));
...
bool ProcessSet::mallocMemory(size_t size, AddressSet::ptr location) const {
  ...
  for (int_addressSet::iterator i = location->iaddrs->begin();
       i != location->iaddrs->end(); i++)
    ;
...
bool ProcessSet::freeMemory(AddressSet::ptr addrset) const {
  ...
  if (int_process::isInCB()) {
    perr_printf("User attempted call on process while in CB, erroring.");
    for_each(procset->begin(), procset->end(),
             setError(err_incallback, "Cannot freeMemory from callback\n"));
    return false;
  }

  addrset_iter iter("free memory", had_error, ERR_CHCK_NORM);
  for (int_addressSet::iterator i = iter.begin(addrset); i != iter.end();
       i = iter.inc())
    ;
...
bool ProcessSet::readMemory(AddressSet::ptr addrset,
                            multimap<Process::ptr, void *> &mem_result,
                            size_t size) const {
  // Use the read_t form of readMemory below
  multimap<Process::const_ptr, read_t> all_reads;
  for (int_addressSet::iterator i = addrset->iaddrs->begin();
       i != addrset->iaddrs->end(); i++) {
  ...
  bool had_error = false;
  for (multimap<Process::const_ptr, read_t>::iterator i = all_reads.begin();
       i != all_reads.end(); i++) {
...
struct bufferCompare {
  ...
  bool operator<(const bufferCompare &bc) const {
    if (use_checksum) return checksum < bc.checksum;
    if (checksum < bc.checksum) return true;
    if (checksum > bc.checksum) return false;
...
bool ProcessSet::readMemory(AddressSet::ptr addrset,
                            map<void *, ProcessSet::ptr> &mem_result,
                            size_t size, bool use_checksum) const {
  ...
  for (multimap<Process::ptr, void *>::iterator i = initial_result.begin();
       i != initial_result.end(); i++) {
  ...
  for (map<bufferCompare, ProcessSet::ptr>::iterator i = unique_results.begin();
       i != unique_results.end(); i++) {
...
bool ProcessSet::readMemory(multimap<Process::const_ptr, read_t> &addrs) const {
  ...
  map<response::ptr, multimap<Process::const_ptr, read_t>::const_iterator>
      resps_to_procs;

  readmap_iter iter("read memory", had_error, ERR_CHCK_ALL);
  for (readmap_iter::i_t i = iter.begin(&addrs); i != iter.end();
       i = iter.inc()) {
    Process::const_ptr p = i->first;
    int_process *proc = p->llproc();
    const read_t &r = i->second;
    ...
    size_t size = r.size;
    pthrd_printf(
        "User wants to read memory from 0x%lx of size %lu in process %d\n",
  ...
  map<response::ptr,
      multimap<Process::const_ptr, read_t>::const_iterator>::iterator i;
...
bool ProcessSet::writeMemory(AddressSet::ptr addrset, const void *buffer,
                             size_t size) const {
  MTLock lock_this_func;
  bool had_error = false;
  ...
  for (set<response::ptr>::iterator i = all_responses.begin();
       i != all_responses.end(); i++) {
    response::ptr resp = *i;
    if (resp->hasError()) had_error = true;
  }

  return !had_error;
}

bool ProcessSet::writeMemory(
    multimap<Process::const_ptr, write_t> &addrs) const {
  MTLock lock_this_func;
  bool had_error = false;
  for_each(procset->begin(), procset->end(), clearError());

  set<response::ptr> all_responses;
  map<response::ptr, multimap<Process::const_ptr, write_t>::const_iterator>
      resps_to_procs;

  writemap_iter iter("read memory", had_error, ERR_CHCK_ALL);
  for (writemap_iter::i_t i = iter.begin(&addrs); i != iter.end();
       i = iter.inc()) {
    Process::const_ptr p = i->first;
    int_process *proc = p->llproc();
    ...
    if (!result) {
      perr_printf("Failed to write memory to %d at %lx", proc->getPid(),
                  w.addr);
      (void)resp->isReady();
      had_error = true;
      continue;
  ...
  map<response::ptr,
      multimap<Process::const_ptr, write_t>::const_iterator>::iterator i;
  for (i = resps_to_procs.begin(); i != resps_to_procs.end(); i++) {
...
static bool addBreakpointWorker(
    set<pair<int_process *, bp_install_state *> > &bp_installs) {
  bool had_error = false;
  bool result;

  set<response::ptr> all_responses;
  for (set<pair<int_process *, bp_install_state *> >::iterator i =
           bp_installs.begin();
       i != bp_installs.end();) {
...
bool ProcessSet::addBreakpoint(AddressSet::ptr addrset,
...
bool ProcessSet::rmBreakpoint(AddressSet::ptr addrset,
                              Breakpoint::ptr bp) const {
  MTLock lock_this_func;
  bool had_error = false;
  for_each(procset->begin(), procset->end(), clearError());
...
bool ProcessSet::postIRPC(
    const multimap<Process::const_ptr, IRPC::ptr> &rpcs) const {
  MTLock lock_this_func;
  bool had_error = false;

  rpcmap_iter iter("Post RPC", had_error, ERR_CHCK_NORM);
  for (multimap<Process::const_ptr, IRPC::ptr>::const_iterator i =
           iter.begin(&rpcs);
       i != iter.end(); i = iter.inc()) {
    Process::const_ptr p = i->first;
    int_process *proc = p->llproc();
...
bool ProcessSet::postIRPC(IRPC::ptr irpc,
                          multimap<Process::ptr, IRPC::ptr> *result) const {
  MTLock lock_this_func;
  bool had_error = false;
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (result) result->insert(make_pair(p, local_rpc));
  }
  return !had_error;
}

bool ProcessSet::postIRPC(IRPC::ptr irpc, AddressSet::ptr addrset,
                          multimap<Process::ptr, IRPC::ptr> *result) const {
  MTLock lock_this_func;
  bool had_error = false;
...
ProcessSet::iterator::iterator(int_processSet::iterator i) { int_iter = i; }

ProcessSet::iterator::iterator() {}

ProcessSet::iterator::~iterator() {}

Process::ptr ProcessSet::iterator::operator*() const { return *int_iter; }
...
Process::ptr ProcessSet::const_iterator::operator*() const { return *int_iter; }

bool ProcessSet::const_iterator::operator==(
    const ProcessSet::const_iterator &i) const {
  return int_iter == i.int_iter;
}
...
int_processSet *ProcessSet::getIntProcessSet() { return procset; }
...
ThreadSet::ThreadSet() : features(NULL) { ithrset = new int_threadSet; }
...
ThreadSet::ptr ThreadSet::newThreadSet(const ThreadPool &threadp) {
  ...
  transform(threadp.begin(), threadp.end(), inserter(*newset, newset->end()),
            thread_strip_const());
  return newts;
}
...
ThreadSet::ptr ThreadSet::newThreadSet(const set<Thread::const_ptr> &threads) {
  ThreadSet::ptr newts = ThreadSet::ptr(new ThreadSet());
  int_threadSet *&newset = newts->ithrset;
  transform(threads.begin(), threads.end(), inserter(*newset, newset->end()),
            thread_strip_const());
  return newts;
}
...
ThreadSet::ptr ThreadSet::newThreadSet(ProcessSet::ptr ps, bool initial_only) {
  ...
  for (procset_iter::i_t i = iter.begin(ps->procset); i != iter.end();
       i = iter.inc()) {
    if (had_error) {
...
ThreadSet::iterator ThreadSet::begin() { return iterator(ithrset->begin()); }

ThreadSet::iterator ThreadSet::end() { return iterator(ithrset->end()); }
...
bool ThreadSet::empty() const { return ithrset->empty(); }

size_t ThreadSet::size() const { return ithrset->size(); }

int_threadSet *ThreadSet::getIntThreadSet() const { return ithrset; }

pair<ThreadSet::iterator, bool> ThreadSet::insert(Thread::const_ptr p) {
  pair<int_threadSet::iterator, bool> result =
      ithrset->insert(pc_const_cast<Thread>(p));
  return pair<ThreadSet::iterator, bool>(ThreadSet::iterator(result.first),
                                         result.second);
}

void ThreadSet::erase(iterator pos) { ithrset->erase(pos.int_iter); }
...
void ThreadSet::clear() { ithrset->clear(); }
...
void ThreadSet::getErrorSubsets(
    map<ProcControlAPI::err_t, ThreadSet::ptr> &err_sets) const {
  MTLock lock_this_func;
...
bool ThreadSet::anyStopped() const {
  MTLock lock_this_func;
  return any_match(ithrset->begin(), ithrset->end(),
                   test_thr(test_thr::any, int_thread::stopped));
}

bool ThreadSet::allStopped() const {
  MTLock lock_this_func;
  return all_match(ithrset->begin(), ithrset->end(),
                   test_thr(test_thr::all, int_thread::stopped));
}

ThreadSet::ptr ThreadSet::getStoppedSubset() const {
  MTLock lock_this_func;
  return create_thrsubset(ithrset->begin(), ithrset->end(),
                          test_thr(test_thr::any, int_thread::stopped));
}
...
bool ThreadSet::getStartFunctions(AddressSet::ptr result) const {
  ...
  for (thrset_iter::i_t i = iter.begin(ithrset); i != iter.end();
       i = iter.inc()) {
    Thread::ptr t = *i;
    Process::ptr p = t->getProcess();
    Address addr;
    bool bresult = t->llthrd()->getStartFuncAddress(addr);
    if (bresult) result->insert(addr, p);
  }
  return !had_error;
}
...
bool ThreadSet::stopThreads() const {
  ...
  for (set<int_process *>::iterator i = all_procs.begin(); i != all_procs.end();
       i++) {
    int_process *proc = *i;
    proc->throwNopEvent();
  }
...
bool ThreadSet::setSingleStepMode(bool v) const {
  ...
  thrset_iter iter("set single step", had_error,
                   ERR_CHCK_THRD | ERR_CHCK_THRD_STOPPED);
  for (thrset_iter::i_t i = iter.begin(ithrset); i != iter.end();
       i = iter.inc()) {
    Thread::ptr thr = *i;
...
static bool getRegisterWorker(
    Dyninst::MachRegister reg, int_threadSet *ithrset,
    set<pair<Thread::ptr, reg_response::ptr> > &thr_to_response) {
  bool had_error = false;

  set<response::ptr> all_responses;
  thrset_iter iter("getRegister", had_error,
                   ERR_CHCK_THRD | ERR_CHCK_THRD_STOPPED);
  for (thrset_iter::i_t i = iter.begin(ithrset); i != iter.end();
       i = iter.inc()) {
    Thread::ptr t = *i;
  ...
  for (set<pair<Thread::ptr, reg_response::ptr> >::iterator i =
           thr_to_response.begin();
       i != thr_to_response.end();) {
    Thread::ptr thr = i->first;
    reg_response::ptr resp = i->second;

    if (resp->hasError()) {
      thr->getProcess()->setLastError(resp->errorCode(),
                                      thr->getProcess()->getLastErrorMsg());
      pthrd_printf("Error in response from %d/%d\n",
                   thr->llthrd()->llproc()->getPid(), thr->llthrd()->getLWP());
      had_error = true;
...
bool ThreadSet::getRegister(
    Dyninst::MachRegister reg,
    map<Thread::ptr, Dyninst::MachRegisterVal> &results) const {
  MTLock lock_this_func;
  bool had_error = false;
...
bool ThreadSet::getRegister(
    Dyninst::MachRegister reg,
    map<Dyninst::MachRegisterVal, ThreadSet::ptr> &results) const {
  MTLock lock_this_func;
  bool had_error = false;
...
bool ThreadSet::setRegister(
    Dyninst::MachRegister reg,
    const map<Thread::const_ptr, Dyninst::MachRegisterVal> &vals) const {
  MTLock lock_this_func;
  bool had_error = false;
  ...
  setreg_iter iter("setRegister", had_error,
                   ERR_CHCK_THRD | ERR_CHCK_THRD_STOPPED);
  for (setreg_iter::i_t i = iter.begin(&vals); i != iter.end();
       i = iter.inc()) {
  ...
  for (set<pair<Thread::ptr, result_response::ptr> >::iterator i =
           thr_to_response.begin();
       i != thr_to_response.end(); i++) {
    Thread::ptr thr = i->first;
...
bool ThreadSet::setRegister(Dyninst::MachRegister reg,
                            Dyninst::MachRegisterVal val) const {
  MTLock lock_this_func;
  bool had_error = false;
  map<Thread::const_ptr, Dyninst::MachRegisterVal> vals;

  thrset_iter iter("setRegister", had_error,
                   ERR_CHCK_THRD | ERR_CHCK_THRD_STOPPED);
  for (thrset_iter::i_t i = iter.begin(ithrset); i != iter.end();
       i = iter.inc()) {
...
bool ThreadSet::getAllRegisters(map<Thread::ptr, RegisterPool> &results) const {
  ...
  thrset_iter iter("getAllRegisters", had_error,
                   ERR_CHCK_THRD | ERR_CHCK_THRD_STOPPED);
  for (thrset_iter::i_t i = iter.begin(ithrset); i != iter.end();
       i = iter.inc()) {
    Thread::ptr t = *i;
    ...
    allreg_response::ptr response =
        allreg_response::createAllRegResponse(newpool);
  ...
  if (!result) {
    pthrd_printf("Error waiting for async events to complete\n");
    for (set<pair<Thread::ptr, allreg_response::ptr> >::iterator i =
             thr_to_response.begin();
         i != thr_to_response.end(); i++) {
...
bool ThreadSet::setAllRegisters(
    const map<Thread::const_ptr, RegisterPool> &reg_vals) const {
  MTLock lock_this_func;
  bool had_error = false;
  ...
  setallreg_iter iter("setAllRegisters", had_error,
                      ERR_CHCK_THRD | ERR_CHCK_THRD_STOPPED);
  for (setallreg_iter::i_t i = iter.begin(&reg_vals); i != iter.end();
       i = iter.inc()) {
    Thread::const_ptr t = i->first;
...
bool ThreadSet::postIRPC(
    const multimap<Thread::const_ptr, IRPC::ptr> &rpcs) const {
  MTLock lock_this_func;
  bool had_error = false;

  rpcmap_thr_iter iter("Post RPC", had_error, ERR_CHCK_NORM);
  for (rpcmap_thr_iter::i_t i = iter.begin(&rpcs); i != iter.end();
       i = iter.inc()) {
    IRPC::ptr rpc = i->second;
    Thread::const_ptr t = i->first;
...
bool ThreadSet::postIRPC(IRPC::ptr irpc,
                         multimap<Thread::ptr, IRPC::ptr> *results) const {
  MTLock lock_this_func;
  bool had_error = false;
  ...
  for (thrset_iter::i_t i = iter.begin(ithrset); i != iter.end();
    ...
    if (results) results->insert(make_pair(t, local_rpc));
  }
  return !had_error;
}

CallStackUnwindingSet *ThreadSet::getCallStackUnwinding() {
  if (features && features->stkset) return features->stkset;

  MTLock lock_this_func;
  if (!features) {
    features = new TSetFeatures();
  }
  if (!ithrset) return NULL;
...
ThreadSet::iterator::iterator(int_threadSet::iterator i) { int_iter = i; }

ThreadSet::iterator::iterator() {}

ThreadSet::iterator::~iterator() {}

Thread::ptr ThreadSet::iterator::operator*() { return *int_iter; }
...
Thread::ptr ThreadSet::const_iterator::operator*() { return *int_iter; }
...
LibraryTrackingSet::LibraryTrackingSet(ProcessSet::ptr ps_) : wps(ps_) {}

LibraryTrackingSet::~LibraryTrackingSet() { wps = ProcessSet::weak_ptr(); }
...
bool LibraryTrackingSet::setTrackLibraries(bool b) const {
  ...
  if (!ps) {
    ...
    globalSetLastError(
        err_badparam,
        "setTrackLibraries attempted on deleted ProcessSet object");
    return false;
  }
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (!proc) {
      perr_printf("Library tracking not supported on process %d\n",
                  p->getPid());
      p->setLastError(err_unsupported,
                      "No library tracking on this platform\n");
      had_error = true;
      continue;
    }
  ...
  if (bps_to_install.empty()) return !had_error;
...
bool LibraryTrackingSet::refreshLibraries() const {
  ...
  for (int_processSet::iterator i = iter.begin(ps->getIntProcessSet());
       i != iter.end(); i = iter.inc()) {
...
ThreadTrackingSet::ThreadTrackingSet(ProcessSet::ptr ps_) : wps(ps_) {}
...
bool ThreadTrackingSet::setTrackThreads(bool b) const {
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (add_bp) {
      for (set<pair<int_breakpoint *, Address> >::iterator j = bps.begin();
           j != bps.end(); j++) {
        bp_install_state *is = new bp_install_state();
        is->addr = j->second;
        is->bp = j->first;
...
bool ThreadTrackingSet::refreshThreads() const {
  ...
  if (!ps) {
    ...
    globalSetLastError(err_badparam,
                       "refreshThreads attempted on deleted ProcessSet object");
    return false;
  }
  int_processSet *procset = ps->getIntProcessSet();
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (!proc) {
      perr_printf("Thread tracking not supported on process %d\n",
                  proc->getPid());
      proc->setLastError(err_unsupported,
                         "No thread tracking on this platform\n");
      had_error = true;
      continue;
    }
...
LWPTrackingSet::LWPTrackingSet(ProcessSet::ptr ps_) : wps(ps_) {}
...
bool LWPTrackingSet::setTrackLWPs(bool b) const {
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (!lwpt) {
      p->setLastError(err_unsupported,
                      "LWP Tracking not supported on this process");
      had_error = true;
      continue;
    }
...
bool LWPTrackingSet::refreshLWPs() const {
  ...
  if (change_procs.empty()) return !had_error;
  ...
  for (set<int_process *>::iterator i = change_procs.begin();
       i != change_procs.end(); i++) {
    int_process *proc = *i;
...
FollowForkSet::FollowForkSet(ProcessSet::ptr ps_) : wps(ps_) {}

FollowForkSet::~FollowForkSet() { wps = ProcessSet::weak_ptr(); }
...
bool FollowForkSet::setFollowFork(FollowFork::follow_t f) const {
  ...
  if (!ps) {
    ...
    globalSetLastError(err_badparam,
                       "setFollowFork attempted on deleted ProcessSet object");
    return false;
  }
  int_processSet *procset = ps->getIntProcessSet();
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (!proc) {
      ...
      proc->setLastError(err_unsupported,
                         "No follow fork control on this platform\n");
      had_error = true;
      continue;
    }
...
CallStackUnwindingSet::CallStackUnwindingSet(ThreadSet::ptr ts) : wts(ts) {}

CallStackUnwindingSet::~CallStackUnwindingSet() { wts = ThreadSet::weak_ptr(); }
...
bool CallStackUnwindingSet::walkStack(CallStackCallback *stk_cb) {
  ...
  while (!all_responses.empty()) {
    ...
    if (!did_something) int_process::waitForAsyncEvent(a_resp);
  }
...
bool RemoteIOSet::getFileNames(FileSet *fset) {
  ...
  for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
    ...
    if (!proc) {
      perr_printf("getFileNames attempted on non RemoteIO process %d\n",
                  proc->getPid());
      proc->setLastError(err_unsupported,
                         "getFileNames not supported on this platform");
      had_error = true;
      continue;
    }
  ...
  for (set<FileSetResp_t *>::iterator i = all_resps.begin();
       i != all_resps.end(); i++) {
...
bool RemoteIOSet::getFileStatData(FileSet *fset) {
  ...
  for (FileSet::iterator i = fset->begin(); i != fset->end(); i++) {
    ...
    if (!proc) {
      perr_printf("getFileStatData attempted on non RemoteIO process %d\n",
                  proc->getPid());
      proc->setLastError(err_unsupported,
                         "getFileStatData not supported on this platform");
      had_error = true;
      continue;
    }
    ...
    if (info->filename.empty()) {
      ...
      proc->setLastError(err_badparam,
                         "Empty filename specified in stat operation");
      had_error = true;
      continue;
    }

    bool result = proc->plat_getFileStatData(info->filename,
                                             &info->stat_results, all_resps);
    if (!result) {
  ...
  for (set<StatResp_t *>::iterator i = all_resps.begin(); i != all_resps.end();
       i++) {
    StatResp_t *resp = *i;
    resp->getProc()->waitForEvent(resp);
    delete resp;
...
MemoryUsageSet::MemoryUsageSet(ProcessSet::ptr ps_) : wps(ps_) {
  pthrd_printf("Constructed MemoryUsageSet %p on procset of size %lu\n", this,
               (unsigned long)ps_->size());
}

MemoryUsageSet::~MemoryUsageSet() { wps = ProcessSet::weak_ptr(); }

bool MemoryUsageSet::usedX(std::map<Process::const_ptr, unsigned long> &used,
                           MemoryUsageSet::mem_usage_t mu) const {
  MTLock lock_this_func;
  bool had_error = false;
  ...
  if (!ps) {
    ...
    globalSetLastError(
        err_badparam,
        "memory usage query attempted on deleted ProcessSet object");
    return false;
  }
  set<MemUsageResp_t *> resps;
  const unsigned int max_operations =
      (mu == mus_resident ? ps->size() * 4 : ps->size());
  unsigned long *result_sizes = new unsigned long[max_operations];
  pthrd_printf("Performing set operation getting %s (may use %u ops)\n", mu_str,
               max_operations);

  map<int_memUsage *, MemUsageResp_t *> shared_results, stack_results,
      heap_results, resident_results;
  ...
  if (mu == mus_resident) {
    for (int_processSet::iterator i = iter.begin(procset); i != iter.end();
      ...
      int_memUsage *proc = (*i)->llproc()->getMemUsage();
      if (!proc) continue;
      if (!proc->plat_residentNeedsMemVals()) continue;
      ...
      if (sh == shared_results.end() || st == stack_results.end() ||
          he == heap_results.end() || sh->second->hadError() ||
          st->second->hadError() || he->second->hadError()) {
        perr_printf(
            "Failed to read shared stack or heap on process %d for resident "
            "memory\n",
            proc->getPid());
        continue;
      }
      ...
      assert(cur < max_operations);
      MemUsageResp_t *resp = new MemUsageResp_t(result_sizes + cur++, proc);
      bool result = proc->plat_getResidentUsage(
          *st->second->get(), *he->second->get(), *sh->second->get(), resp);
      if (!result) {
        perr_printf(
            "Error calculating resident usage from stack, heap and shared on "
            "%d\n",
            proc->getPid());
        delete resp;
        continue;
    ...
    for (set<MemUsageResp_t *>::iterator i = resps.begin(); i != resps.end();
         i++)
      (*i)->getProc()->waitForEvent(*i);
  }
  ...
  for (i = stack_results.begin(); i != stack_results.end(); i++)
    delete i->second;
  for (i = heap_results.begin(); i != heap_results.end(); i++) delete i->second;
...
bool MemoryUsageSet::sharedUsed(
    std::map<Process::const_ptr, unsigned long> &used) const {
  return usedX(used, mus_shared);
}

bool MemoryUsageSet::heapUsed(
    std::map<Process::const_ptr, unsigned long> &used) const {
  return usedX(used, mus_heap);
}

bool MemoryUsageSet::stackUsed(
    std::map<Process::const_ptr, unsigned long> &used) const {
  return usedX(used, mus_stack);
}

bool MemoryUsageSet::resident(
    std::map<Process::const_ptr, unsigned long> &res) const {
