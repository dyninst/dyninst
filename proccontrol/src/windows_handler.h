...
class PC_EXPORT WinEventNewThread : public EventNewLWP {
  ...
  friend void boost::checked_delete<const WinEventNewThread>(
      const WinEventNewThread *);
...
class PC_EXPORT WinEventThreadInfo : public Event {
  ...
  friend void boost::checked_delete<const WinEventThreadInfo>(
      const WinEventThreadInfo *);
 ...
 public:
  ...
  WinEventThreadInfo(Dyninst::LWP l, HANDLE ht, LPTHREAD_START_ROUTINE ts,
                     LPVOID base)
      : Event(EventType(EventType::None, EventType::ThreadInfo)),
        hthread(ht),
        thread_start(ts),
        tls_base(base),
        lwp(l) {}
