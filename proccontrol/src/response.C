...
response::~response() { assert(error || state != ready || checked_ready); }

bool
response::isReady() const
{
    checked_ready = true;
    return (state == ready);
}
...
bool
response::testReady() const
{
    return (state == ready);
}

Event::ptr
response::getEvent() const
{
    return event;
}

bool
response::hasError() const
{
    return error;
}

int
response::errorCode() const
{
    assert(error);
    return errorcode;
}
...
void
response::setEvent(Event::ptr ev)
{
...
void
response::markSyncHandled()
{
    isSyncHandled = true;
}

unsigned int
response::getID() const
{
    return id;
}

int_eventAsyncIO*
response::getAsyncIOEvent()
{
    return aio;
}

void
response::setAsyncIOEvent(int_eventAsyncIO* aio_)
{
    aio = aio_;
}
...
unsigned int
response::markAsMultiResponse(int num_resps)
{
    assert(num_resps);
    assert(state == unset);
    id_lock.lock();
...
bool
response::isMultiResponse()
{
    return multi_resp_size != 0;
}

unsigned int
response::multiResponseSize()
{
    return multi_resp_size;
}

bool
response::isMultiResponseComplete()
{
    return (multi_resp_size == multi_resp_recvd);
}

void
response::setDecoderEvent(ArchEvent* ae)
{
    decoder_event = ae;
}

ArchEvent*
response::getDecoderEvent()
{
    return decoder_event;
}

int_process*
response::getProcess() const
{
    return proc;
}

void
response::setProcess(int_process* p)
{
    proc = p;
}
...
response::ptr
responses_pending::rmResponse(unsigned int id)
{
...
bool
responses_pending::waitFor(response::ptr resp)
{
    cvar.lock();
...
bool
responses_pending::hasAsyncPending(bool ev_only)
{
    bool ret = false;
    cvar.lock();
    if(!ev_only)
...
response::ptr
responses_pending::getResponse(unsigned int id)
{
    cvar.lock();
...
void
responses_pending::lock()
{
    cvar.lock();
}

void
responses_pending::unlock()
{
    cvar.unlock();
}

void
responses_pending::signal()
{
    cvar.broadcast();
}

CondVar<>&
responses_pending::condvar()
{
    return cvar;
}

void
responses_pending::addResponse(response::ptr r, int_process* proc)
{
...
void
responses_pending::noteResponse()
{
...
result_response::ptr
result_response::createResultResponse()
{
...
void
result_response::setResponse(bool b_)
{
    postResponse(b_);
    markReady();
}
...
void
result_response::postResponse(bool b_)
{
    if(!isMultiResponse())
    {
        b = b_;
...
bool
result_response::getResult() const
{
    return b;
}

reg_response::ptr
reg_response::createRegResponse()
{
...
void
reg_response::setRegThread(Dyninst::MachRegister r, int_thread* t)
{
    reg = r;
    thr = t;
}
...
void
reg_response::setResponse(Dyninst::MachRegisterVal v)
{
    postResponse(v);
    markReady();
}
...
void
reg_response::postResponse(Dyninst::MachRegisterVal v)
{
    assert(reg && thr);
...
reg_response::getResult() const
{
    return val;
}

allreg_response::ptr
allreg_response::createAllRegResponse()
{
    return allreg_response::ptr(new allreg_response());
}

allreg_response::ptr
allreg_response::createAllRegResponse(int_registerPool* regpool)
{
...
void
allreg_response::setThread(int_thread* t)
{
    thr = t;
}

void
allreg_response::setRegPool(int_registerPool* p)
{
    regpool = p;
}

void
allreg_response::setResponse()
{
    postResponse();
    markReady();
}
...
void
allreg_response::setIndividualRegAccess(reg_response::ptr     iacc,
                                        Dyninst::MachRegister ireg)
{
    assert(!indiv_access);
    indiv_access = iacc;
    indiv_reg    = ireg;
...
allreg_response::getIndividualReg()
{
    return indiv_reg;
}

reg_response::ptr
allreg_response::getIndividualAcc()
{
    return indiv_access;
}

int_registerPool*
allreg_response::getRegPool() const
{
    return regpool;
}

mem_response::ptr
mem_response::createMemResponse()
{
    return mem_response::ptr(new mem_response());
}

mem_response::ptr
mem_response::createMemResponse(char* targ, unsigned targ_size)
{
...
void
mem_response::setBuffer(char* targ, unsigned targ_size)
{
    assert(!buffer_set);
    buffer     = targ;
    size       = targ_size;
...
void
mem_response::setResponse(char* src, unsigned src_size)
{
...
void
mem_response::postResponse(char* src, unsigned src_size, Address src_addr)
{
    assert(buffer_set);
...
void
mem_response::setLastBase(Address a)
{
    last_base = a;
}

Address
mem_response::lastBase()
{
    return last_base;
}

void
mem_response::postResponse()
{}

char*
mem_response::getBuffer() const
{
    return buffer;
}

unsigned
mem_response::getSize() const
{
    return size;
}

stack_response::ptr
stack_response::createStackResponse(int_thread* t)
{
...
void*
stack_response::getData()
{
    return data;
}

int_thread*
stack_response::getThread()
{
    return thr;
}

void
stack_response::postResponse(void* d)
{
    data = d;
}
...
data_response::ptr
data_response::createDataResponse()
{
...
void
data_response::postResponse(void* d)
{
    data = d;
    markReady();
}
...
void
ResponseSet::addID(unsigned id, unsigned index)
{
    ids.insert(make_pair(index, id));
}

unsigned
ResponseSet::getIDByIndex(unsigned int index, bool& found) const
{
...
unsigned int
ResponseSet::getID() const
{
    return myid;
}

ResponseSet*
ResponseSet::getResponseSetByID(unsigned int id)
{
