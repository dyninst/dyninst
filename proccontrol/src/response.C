...
bool response::testReady() const { return (state == ready); }

Event::ptr response::getEvent() const { return event; }

bool response::hasError() const { return error; }
...
bool response::isPosted() const { return state != unset; }

void response::markReady() {
  if (multi_resp_size && multi_resp_size < multi_resp_recvd) return;
...
void response::markSyncHandled() { isSyncHandled = true; }

unsigned int response::getID() const { return id; }

int_eventAsyncIO *response::getAsyncIOEvent() { return aio; }

void response::setAsyncIOEvent(int_eventAsyncIO *aio_) { aio = aio_; }
...
bool response::isMultiResponse() { return multi_resp_size != 0; }

unsigned int response::multiResponseSize() { return multi_resp_size; }

bool response::isMultiResponseComplete() {
  return (multi_resp_size == multi_resp_recvd);
}

void response::setDecoderEvent(ArchEvent *ae) { decoder_event = ae; }

ArchEvent *response::getDecoderEvent() { return decoder_event; }

int_process *response::getProcess() const { return proc; }

void response::setProcess(int_process *p) { proc = p; }
...
response::ptr responses_pending::rmResponse(unsigned int id) {
  ...
  if (i == pending.end()) {
    ...
    pthrd_printf(
        "Unknown response.  Recieved duplicate ACK message on BlueGene?\n");
    return response::ptr();
  }
...
void responses_pending::lock() { cvar.lock(); }

void responses_pending::unlock() { cvar.unlock(); }

void responses_pending::signal() { cvar.broadcast(); }

CondVar<> &responses_pending::condvar() { return cvar; }
...
void responses_pending::addResponse(response::ptr r, int_process *proc) {
  ...
  if (r->isSyncHandled) ev = Event::ptr();
...
result_response::result_response() : b(false) { resp_type = rt_result; }
...
bool result_response::getResult() const { return b; }

reg_response::ptr reg_response::createRegResponse() {
  return reg_response::ptr(new reg_response());
}

reg_response::reg_response() : val(0), thr(NULL) { resp_type = rt_reg; }
...
Dyninst::MachRegisterVal reg_response::getResult() const { return val; }
...
allreg_response::allreg_response() : regpool(NULL), thr(NULL) {
  resp_type = rt_allreg;
}

allreg_response::~allreg_response() {}

void allreg_response::setThread(int_thread *t) { thr = t; }

void allreg_response::setRegPool(int_registerPool *p) { regpool = p; }
...
void allreg_response::setIndividualRegAccess(reg_response::ptr iacc,
...
Dyninst::MachRegister allreg_response::getIndividualReg() { return indiv_reg; }

reg_response::ptr allreg_response::getIndividualAcc() { return indiv_access; }

int_registerPool *allreg_response::getRegPool() const { return regpool; }

mem_response::ptr mem_response::createMemResponse() {
  return mem_response::ptr(new mem_response());
}

mem_response::ptr mem_response::createMemResponse(char *targ,
                                                  unsigned targ_size) {
  return mem_response::ptr(new mem_response(targ, targ_size));
}

mem_response::mem_response()
    : buffer(NULL), size(0), buffer_set(false), last_base(0) {
  resp_type = rt_mem;
}

mem_response::mem_response(char *targ, unsigned targ_size)
    : buffer(targ), size(targ_size), buffer_set(true), last_base(0) {
  resp_type = rt_mem;
}
...
void mem_response::setLastBase(Address a) { last_base = a; }

Address mem_response::lastBase() { return last_base; }

void mem_response::postResponse() {}

char *mem_response::getBuffer() const { return buffer; }

unsigned mem_response::getSize() const { return size; }

stack_response::ptr stack_response::createStackResponse(int_thread *t) {
  return stack_response::ptr(new stack_response(t));
}

stack_response::stack_response(int_thread *t) : data(NULL), thr(t) {
  resp_type = rt_stack;
}

stack_response::~stack_response() {}

void *stack_response::getData() { return data; }

int_thread *stack_response::getThread() { return thr; }

void stack_response::postResponse(void *d) { data = d; }

data_response::data_response() : data(NULL) { resp_type = rt_data; }
...
void *data_response::getData() { return data; }
...
unsigned int ResponseSet::getID() const { return myid; }
