...
#if !defined(riscv_process_h_)
#define riscv_process_h_
...
{
 public:
  riscv_process(Dyninst::PID p, std::string e, std::vector<std::string> a,
              std::vector<std::string> envp, std::map<int, int> f);
  riscv_process(Dyninst::PID pid_, int_process *p) ;
  virtual ~riscv_process();
...
class riscv_thread : virtual public int_thread
{
  protected:
    bool have_cached_pc;
    Address cached_pc;
