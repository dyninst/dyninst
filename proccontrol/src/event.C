...
void
Event::setLastError(err_t ec, const char* es)
{
    if(proc)
    {
...
void
EventExec::setExecPath(std::string e)
{
    execpath = e;
}
...
void
EventBreakpoint::getBreakpoints(std::vector<Breakpoint::const_ptr>& bps) const
{
    if(!int_bp)
        return;
...
void
EventBreakpoint::getBreakpoints(std::vector<Breakpoint::ptr>& bps)
{
    if(!int_bp)
        return;
    bp_instance*                        ibp = int_bp->lookupInstalledBreakpoint();
...
EventBreakpoint::suppressCB() const
{
    ...
    {
        pthrd_printf(
            "Suppressing CB on EventBreakpoint: checking for unsuppressed HL BP: %s\n",
            (*iter)->suppressCallbacks() ? "<suppress>" : "<no suppress>");
        if(!(*iter)->suppressCallbacks())
            return false;
    }
...
long
EventSyscall::getSyscallNumber() const
{
    MachRegisterVal    syscallNumber;
    Process::const_ptr p    = getProcess();
    Thread::const_ptr  thrd = getThread();
    thrd->getRegister(MachRegister::getSyscallNumberOReg(p->getArchitecture()),
                      syscallNumber);
    return syscallNumber;
}

MachSyscall
EventSyscall::getSyscall() const
{
    return makeFromEvent(this);
}

long
EventPostSyscall::getReturnValue() const
{
    MachRegisterVal    syscallReturnValue;
    Process::const_ptr p    = getProcess();
    Thread::const_ptr  thrd = getThread();
    thrd->getRegister(MachRegister::getSyscallReturnValueReg(p->getArchitecture()),
                      syscallReturnValue);
...
void
EventLibrary::setLibs(const std::set<Library::ptr>& added_libs_,
                      const std::set<Library::ptr>& rmd_libs_)
{
    added_libs = added_libs_;
    rmd_libs   = rmd_libs_;
...
void
EventIntBootstrap::setData(void* d)
{
    data = d;
}
...
bp_instance*
int_eventBreakpoint::lookupInstalledBreakpoint()
{
    if(hwbp)
...
void
int_eventRPC::getPendingAsyncs(std::set<response::ptr>& pending)
{
...
set<response::ptr>&
int_eventAsync::getResponses()
{
    return resp;
}

void
int_eventAsync::addResp(response::ptr r)
{
    resp.insert(r);
}
...
bool
int_eventAsyncFileRead::isComplete()
{
    if(!whole_file)
        return true;
