...
int_iRPC::int_iRPC(void *binary_blob_, unsigned long binary_size_, bool async_,
                   bool alreadyAllocated, Dyninst::Address addr)
    : state(Unassigned),
      type(User),
...
void int_iRPC::setType(int_iRPC::Type t) { type = t; }

void int_iRPC::setBinaryBlob(void *b) { binary_blob = b; }

void int_iRPC::setBinarySize(unsigned long s) { binary_size = s; }
...
void int_iRPC::setStartOffset(unsigned long s) { start_offset = s; }

void int_iRPC::setAsync(bool a) { async = a; }

void int_iRPC::setThread(int_thread *t) { thrd = t; }

void int_iRPC::setIRPC(IRPC::weak_ptr o) { hl_irpc = o; }

void int_iRPC::setAllocSize(unsigned long s) { cur_allocation->size = s; }
...
void int_iRPC::setShouldSaveData(bool b) { cur_allocation->needs_datasave = b; }
...
static unsigned long roundUpPageSize(int_process *proc, unsigned long val) {
  ...
  if (aligned_addr < val) aligned_addr += pgsize;
...
bool int_iRPC::fillInAllocation() {
  ...
  if (type == Allocation) {
    ...
    unsigned long aligned_size =
        roundUpPageSize(thread()->llproc(), target_allocation->size);
    ...
    pthrd_printf(
        "Setting up allocation snippet %lu to allocate memory of "
        "size %lu\n",
        id(), target_allocation->size);
    result = thrd->llproc()->plat_createAllocationSnippet(
        (Dyninst::Address)0, false, target_allocation->size, binary_blob,
        binary_size, start_offset);
  } else if (type == Deallocation) {
    // Set up a snippet with a call to something like munmap for the
    // address and size.
    pthrd_printf(
        "Setting up deallocation snippet %lu to clear memory %lx of "
        "size %lu\n",
        id(), target_allocation->addr, target_allocation->size);
    result = thrd->llproc()->plat_createDeallocationSnippet(
        target_allocation->addr, target_allocation->size, binary_blob,
        binary_size, start_offset);
  }
  assert(result);

  /**
   * We have a chicken and egg problem.  We'd like to run a snippet to allocate
   *or
   * deallocate memory, but we don't have any memory to run it out of.
   *
   * We'll ask the platform specifics for a piece of executable memory to run
   *the
   * initial allocation snippet out of.  As an example platform, on linux we'll
   * go searching through /proc/PID/maps for a big enough piece of executable
   *memory
...
bool int_iRPC::countedSync() { return counted_sync; }
...
iRPCAllocation::ptr iRPCMgr::findAllocationForRPC(int_thread *thread,
                                                  int_iRPC::ptr rpc) {
  if (rpc->allocation()) return rpc->allocation();
  ...
  for (rpc_list_t::iterator i = posted->begin(); i != posted->end(); i++) {
    int_iRPC::ptr cur = *i;
    if (cur->getType() == int_iRPC::User &&
        cur->allocSize() >= rpc->binarySize() && !cur->userAllocated()) {
      return cur->allocation();
    }
  }
  int_iRPC::ptr running = thread->runningRPC();
  if (running && running->getType() == int_iRPC::User &&
      running->allocSize() >= rpc->binarySize() && !running->userAllocated()) {
...
bool iRPCMgr::postRPCToThread(int_thread *thread, int_iRPC::ptr rpc) {
  ...
  if (thread->notAvailableForRPC()) {
    cerr << "Skipping thread that is marked as system - in thread-specific RPC"
         << endl;
    return false;
  }
   ...
   * allocation (e.g, call mmap) and deallocation (e.g, call munmap) iRPC around
   *it.
  ...
  if (allocation) {
    ...
    if (rpc->binarySize() > rpc->allocSize()) {
      pthrd_printf(
          "Resized existing allocation from %lu to %lu to fit iRPC %lu\n",
  ...
  pthrd_printf(
      "Created new allocation and deallocation of size %lu to fit iRPC\n",
      rpc->binarySize());
  ...
  if (dyninst_debug_proccontrol) {
    ...
    for (rpc_list_t::iterator i = cur_list->begin(); i != cur_list->end();
         i++) {
...
Dyninst::Address int_iRPC::infMallocResult() { return malloc_result; }

void int_iRPC::setMallocResult(Dyninst::Address addr) { malloc_result = addr; }

Address int_iRPC::getInfFreeTarget() { return inffree_target; }
...
unsigned long int_iRPC::id() const { return my_id; }

int_iRPC::State int_iRPC::getState() const { return state; }

int_iRPC::Type int_iRPC::getType() const { return type; }

void *int_iRPC::binaryBlob() const { return binary_blob; }

unsigned long int_iRPC::binarySize() const { return binary_size; }

unsigned long int_iRPC::startOffset() const { return start_offset; }

bool int_iRPC::isAsync() const { return async; }

int_thread *int_iRPC::thread() const { return thrd; }

IRPC::weak_ptr int_iRPC::getIRPC() const { return hl_irpc; }

iRPCAllocation::ptr int_iRPC::allocation() const { return cur_allocation; }
...
bool int_iRPC::userAllocated() const {
  ...
  return cur_allocation->creation_irpc.expired() &&
         cur_allocation->deletion_irpc.expired();
}
...
bool int_iRPC::saveRPCState() {
  ...
  if (shouldSaveData() && !allocation()->orig_data) {
    ...
    pthrd_printf(
        "Saving original %lu bytes of memory at %lx from application "
        "for %d/%d\n",
        (unsigned long)allocSize(), addr(), thrd->llproc()->getPid(),
        thrd->getLWP());
    allocation()->orig_data = (char *)malloc(allocSize());
    memsave_result = mem_response::createMemResponse(
        (char *)allocation()->orig_data, allocSize());
    bool result = thrd->llproc()->readMem(
        addr(), memsave_result, (thrd->isRPCEphemeral() ? thrd : NULL));
    assert(result);
    if (!result) return false;
  }
...
bool int_iRPC::checkRPCFinishedSave() {
  ...
  if (memsave_result && !memsave_result->isReady()) return false;
  if (regsave_result && !regsave_result->isReady()) return false;
...
bool int_iRPC::writeToProc() {
  ...
  if (!rpcwrite_result) {
    ...
    bool result = thr->llproc()->writeMem(binaryBlob(), addr(), binarySize(),
                                          rpcwrite_result,
                                          (thr->isRPCEphemeral() ? thr : NULL));
    if (!result) {
  ...
  if (!pcset_result) {
    ...
    Dyninst::MachRegister pc =
        Dyninst::MachRegister::getPC(thr->llproc()->getTargetArch());
...
bool int_iRPC::checkRPCFinishedWrite() {
  ...
  if (!rpcwrite_result->isReady() || rpcwrite_result->hasError()) return false;
  if (!pcset_result->isReady() || pcset_result->hasError()) return false;
...
bool int_iRPC::runIRPC() {
  ...
  if (thrd->isRPCEphemeral()) {
    ...
    // And the threadpool needs to know about us so that we will
    // ContinueDebugEvent. Ugh.
...
bool int_iRPC::needsToRestoreInternal() const { return restore_internal; }

void int_iRPC::setRestoreInternal(bool b) { restore_internal = b; }

int_thread::State int_iRPC::getRestoreToState() const { return restore_at_end; }

void int_iRPC::setRestoreToState(int_thread::State s) { restore_at_end = s; }
...
bool iRPCMgr::isRPCTrap(int_thread *thr, Dyninst::Address addr) {
  ...
  if (addr >= start && addr < start + size) {
    pthrd_printf(
        "%d/%d trap at %lx lies between %lx and %lx, is iRPC %lu trap\n",
  ...
  pthrd_printf("%d/%d trap at %lx outside %lx and %lx, not iRPC %lu trap\n",
               thr->llproc()->getPid(), thr->getLWP(), addr, start, end,
               rpc->id());
  result = false;
...
int_iRPC::ptr iRPCMgr::createInfMallocRPC(int_process *proc, unsigned long size,
  ...
  bool result = proc->plat_createAllocationSnippet(
      addr, use_addr, size, binary_blob, binary_size, start_offset);
...
int_iRPC::ptr iRPCMgr::createInfFreeRPC(int_process *proc, unsigned long size,
  ...
  void *binary_blob;
  unsigned long binary_size, start_offset;
  bool result = proc->plat_createDeallocationSnippet(addr, size, binary_blob,
                                                     binary_size, start_offset);
  irpc->setBinaryBlob(binary_blob);
  irpc->setBinarySize(binary_size);
  irpc->setStartOffset(start_offset);
...
iRPCHandler::iRPCHandler() : Handler(std::string("RPC Handler")) {}
...
int iRPCHandler::getPriority() const {
  // This *must* be after callbacks, so that the user can read any return result
  // they need...
...
Handler::handler_ret_t iRPCHandler::handleEvent(Event::ptr ev) {
  ...
  if ((rpc->getType() == int_iRPC::InfMalloc ||
       rpc->getType() == int_iRPC::Allocation) &&
    ...
    bool result =
        proc->plat_collectAllocationResult(thr, ievent->alloc_regresult);
  ...
  if (rpc->shouldSaveData() && !ievent->memrestore_response) {
    ...
    pthrd_printf("Restoring memory to %lx from %p of size %lu\n", rpc->addr(),
                 rpc->allocation()->orig_data, rpc->allocSize());
    assert(rpc->allocation()->orig_data);
    ievent->memrestore_response = result_response::createResultResponse();
    bool result = proc->writeMem(rpc->allocation()->orig_data, rpc->addr(),
                                 rpc->allocSize(), ievent->memrestore_response);
    assert(result);
    if (!result) return ret_error;
  }
  if (rpc->directFree()) {
...
// CLEANUP on aisle 1
      ...
      // don't do an extra desync here, it's handled by
      // throwEventsBeforeContinue()
    }
    ...
    bool result =
        thr->restoreRegsForRPC(isLastRPC, ievent->regrestore_response);
    assert(result);
    if (!result) return ret_error;
  }
...
Handler::handler_ret_t iRPCPreCallbackHandler::handleEvent(Event::ptr ev) {
  ...
  if (newstate == int_thread::none) return ret_success;
...
void iRPCPreCallbackHandler::getEventTypesHandled(
    std::vector<EventType> &etypes) {
  etypes.push_back(EventType(EventType::None, EventType::RPC));
}

iRPCLaunchHandler::iRPCLaunchHandler() : Handler("iRPC Launch Handler") {}
...
Handler::handler_ret_t iRPCLaunchHandler::handleEvent(Event::ptr ev) {
  ...
  if (!posted_rpc || thr->runningRPC()) return Handler::ret_success;

  assert(posted_rpc->getState() != int_iRPC::Posted);

  pthrd_printf(
      "Handling next posted irpc %lu on %d/%d of type %s in state %s\n",
      posted_rpc->id(), proc->getPid(), thr->getLWP(), posted_rpc->getStrType(),
      posted_rpc->getStrState());
...
IRPC::ptr IRPC::createIRPC(void *binary_blob, unsigned size,
                           bool non_blocking) {
  int_iRPC::ptr irpc =
      int_iRPC::ptr(new int_iRPC(binary_blob, size, non_blocking));
...
IRPC::ptr IRPC::createIRPC(void *binary_blob, unsigned size,
  ...
  int_iRPC::ptr irpc =
      int_iRPC::ptr(new int_iRPC(binary_blob, size, non_blocking, true, addr));
  rpc_wrapper *wrapper = new rpc_wrapper(irpc);
  IRPC::ptr rpc = IRPC::ptr(new IRPC(wrapper));
  irpc->setIRPC(rpc);
  irpc->copyBinaryBlob(binary_blob, size);
...
IRPC::ptr IRPC::createIRPC(IRPC::ptr o) {
  ...
  if (orig->cur_allocation)
    irpc = int_iRPC::ptr(new int_iRPC(orig->binary_blob, orig->binary_size,
                                      orig->async, true, orig->addr()));
  else
    irpc = int_iRPC::ptr(new int_iRPC(orig->binary_blob, orig->binary_size,
                                      orig->async, false, 0));
...
IRPC::IRPC(rpc_wrapper *wrapper_) : wrapper(wrapper_) {}
...
rpc_wrapper *IRPC::llrpc() const { return wrapper; }

void *IRPC::getData() const { return wrapper->rpc->user_data; }

void IRPC::setData(void *p) const { wrapper->rpc->user_data = p; }

Dyninst::Address IRPC::getAddress() const { return wrapper->rpc->addr(); }

void *IRPC::getBinaryCode() const { return wrapper->rpc->binaryBlob(); }

unsigned IRPC::getBinaryCodeSize() const { return wrapper->rpc->binarySize(); }

unsigned long IRPC::getID() const { return wrapper->rpc->id(); }

void IRPC::setStartOffset(unsigned long o) { wrapper->rpc->setStartOffset(o); }

unsigned long IRPC::getStartOffset() const {
  return wrapper->rpc->startOffset();
}

bool IRPC::isBlocking() const { return !wrapper->rpc->isAsync(); }
...
bool IRPC::continueStoppedIRPC() {
  ...
  if (cur_state != Running && cur_state != Posted) {
    perr_printf(
        "Tried to continueStoppedIRPC on RPC %lu with invalid state %d\n",
        rpc->id(), cur_state);
    if (thrd)
  ...
  if (!result) {
    pthrd_printf("Failed to continue thread %d/%d\n", thrd->llproc()->getPid(),
                 thrd->getLWP());
    thrd->setLastError(err_internal,
                       "Could not continue thread associated with iRPC\n");
    return false;
  }
