...
void
int_iRPC::setState(int_iRPC::State s)
{
...
void
int_iRPC::setType(int_iRPC::Type t)
{
    type = t;
}

void
int_iRPC::setBinaryBlob(void* b)
{
    binary_blob = b;
}

void
int_iRPC::setBinarySize(unsigned long s)
{
    binary_size = s;
}
...
void
int_iRPC::setStartOffset(unsigned long s)
{
    start_offset = s;
}

void
int_iRPC::setAsync(bool a)
{
    async = a;
}

void
int_iRPC::setThread(int_thread* t)
{
    thrd = t;
}

void
int_iRPC::setIRPC(IRPC::weak_ptr o)
{
    hl_irpc = o;
}

void
int_iRPC::setAllocSize(unsigned long s)
{
    cur_allocation->size = s;
}

void
int_iRPC::setAllocation(iRPCAllocation::ptr a)
{
    cur_allocation = a;
    a->ref_count++;
}
...
void
int_iRPC::setShouldSaveData(bool b)
{
    cur_allocation->needs_datasave = b;
}

void
int_iRPC::setTargetAllocation(iRPCAllocation::ptr a)
{
    target_allocation = a;
}

static unsigned long
roundUpPageSize(int_process* proc, unsigned long val)
{
...
int_iRPC::fillInAllocation()
{
    ...
    else if(type == Deallocation)
    {
        ...
        result = thrd->llproc()->plat_createDeallocationSnippet(
            target_allocation->addr, target_allocation->size, binary_blob, binary_size,
            start_offset);
    }
    assert(result);
...
iRPCAllocation::ptr
iRPCMgr::findAllocationForRPC(int_thread* thread, int_iRPC::ptr rpc)
{
    if(rpc->allocation())
...
bool
iRPCMgr::postRPCToProc(int_process* proc, int_iRPC::ptr rpc)
{
...
bool
iRPCMgr::postRPCToThread(int_thread* thread, int_iRPC::ptr rpc)
{
...
Dyninst::Address
int_iRPC::infMallocResult()
{
    return malloc_result;
}

void
int_iRPC::setMallocResult(Dyninst::Address addr)
{
    malloc_result = addr;
}

Address
int_iRPC::getInfFreeTarget()
{
    return inffree_target;
}

rpc_wrapper*
int_iRPC::getWrapperForDecode()
{
...
int_iRPC::State
int_iRPC::getState() const
{
    return state;
}

int_iRPC::Type
int_iRPC::getType() const
{
    return type;
}

void*
int_iRPC::binaryBlob() const
{
    return binary_blob;
}

unsigned long
int_iRPC::binarySize() const
{
    return binary_size;
}

unsigned long
int_iRPC::startOffset() const
{
    return start_offset;
}

bool
int_iRPC::isAsync() const
{
    return async;
}

int_thread*
int_iRPC::thread() const
{
    return thrd;
}

IRPC::weak_ptr
int_iRPC::getIRPC() const
{
    return hl_irpc;
}

iRPCAllocation::ptr
int_iRPC::allocation() const
{
    return cur_allocation;
}

iRPCAllocation::ptr
int_iRPC::targetAllocation() const
{
    return target_allocation;
}

bool
int_iRPC::isProcStopRPC() const
{
    return isInternalRPC() || isMemManagementRPC();
}

bool
int_iRPC::isInternalRPC() const
{
    return type == InfMalloc || type == InfFree;
}

unsigned long
int_iRPC::allocSize() const
{
...
bool
int_iRPC::isMemManagementRPC() const
{
    return (type == Allocation || type == Deallocation);
}

int_iRPC::ptr
int_iRPC::allocationRPC() const
{
    assert(cur_allocation);
...
unsigned
iRPCMgr::numActiveRPCs(int_thread* thr)
{
    int rpc_count = 0;
...
int_iRPC::saveRPCState()
{
    ...
    if(shouldSaveData() && !allocation()->orig_data)
    {
        ...
        memsave_result =
            mem_response::createMemResponse((char*) allocation()->orig_data, allocSize());
        bool result = thrd->llproc()->readMem(addr(), memsave_result,
                                              (thrd->isRPCEphemeral() ? thrd : NULL));
        assert(result);
        if(!result)
            return false;
    }
...
int_iRPC::writeToProc()
{
    ...
    if(!rpcwrite_result)
    {
        ...
        bool result =
            thr->llproc()->writeMem(binaryBlob(), addr(), binarySize(), rpcwrite_result,
...
bool
int_iRPC::checkRPCFinishedWrite()
{
...
int_iRPC::runIRPC()
{
    ...
    if(thrd->isRPCEphemeral())
    {
        ...
        // And the threadpool needs to know about us so that we will ContinueDebugEvent.
        // Ugh.
...
void
int_iRPC::getPendingResponses(std::set<response::ptr>& resps)
{
    if(memsave_result)
    {
...
void
int_iRPC::syncAsyncResponses(bool is_sync)
{
...
bool
int_iRPC::needsToRestoreInternal() const
{
    return restore_internal;
}

void
int_iRPC::setRestoreInternal(bool b)
{
    restore_internal = b;
}

int_thread::State
int_iRPC::getRestoreToState() const
{
    return restore_at_end;
}

void
int_iRPC::setRestoreToState(int_thread::State s)
{
    restore_at_end = s;
}

bool
iRPCMgr::isRPCTrap(int_thread* thr, Dyninst::Address addr)
{
    bool             result;
    int_iRPC::ptr    rpc;
...
int_iRPC::ptr
iRPCMgr::createInfMallocRPC(int_process* proc, unsigned long size, bool use_addr,
...
int_iRPC::ptr
iRPCMgr::createInfFreeRPC(int_process* proc, unsigned long size, Dyninst::Address addr)
{
    int_iRPC::ptr irpc = int_iRPC::ptr(new int_iRPC(NULL, 0, true, true, 0));
...
void
iRPCHandler::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::RPC));
}

int
iRPCHandler::getPriority() const
{
    // This *must* be after callbacks, so that the user can read any return result they
    // need...
    return PostCallbackPriority;
}

Handler::handler_ret_t
iRPCHandler::handleEvent(Event::ptr ev)
{
...
Handler::handler_ret_t
iRPCPreCallbackHandler::handleEvent(Event::ptr ev)
{
    EventRPC*     event = static_cast<EventRPC*>(ev.get());
    int_iRPC::ptr rpc   = event->getllRPC()->rpc;
...
void
iRPCPreCallbackHandler::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::RPC));
}
...
void
iRPCLaunchHandler::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::None, EventType::RPCLaunch));
}

Handler::handler_ret_t
iRPCLaunchHandler::handleEvent(Event::ptr ev)
{
    int_process* proc = ev->getProcess()->llproc();
    int_thread*  thr  = ev->getThread()->llthrd();
...
IRPC::ptr
IRPC::createIRPC(void* binary_blob, unsigned size, bool non_blocking)
{
...
IRPC::ptr
IRPC::createIRPC(void* binary_blob, unsigned size, Dyninst::Address addr,
                 bool non_blocking)
{
    int_iRPC::ptr irpc =
...
IRPC::createIRPC(IRPC::ptr o)
{
    ...
    if(orig->cur_allocation)
        ...
        irpc = int_iRPC::ptr(
            new int_iRPC(orig->binary_blob, orig->binary_size, orig->async, false, 0));
...
IRPC::ptr
IRPC::createIRPC(IRPC::ptr o, Address addr)
{
    int_iRPC::ptr orig = o->llrpc()->rpc;
    int_iRPC::ptr irpc;
    irpc = int_iRPC::ptr(
        new int_iRPC(orig->binary_blob, orig->binary_size, orig->async, true, addr));
    rpc_wrapper* wrapper = new rpc_wrapper(irpc);
    IRPC::ptr    rpc     = IRPC::ptr(new IRPC(wrapper));
    irpc->setIRPC(rpc);
    irpc->copyBinaryBlob(orig->binary_blob, orig->binary_size);
...
void
IRPC::setStartOffset(unsigned long o)
{
...
#if !defined(os_windows)
int_thread*
iRPCMgr::createThreadForRPC(int_process*, int_thread* candidate)
{
    return candidate;
}
#endif
