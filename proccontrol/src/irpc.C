...
bool iRPCMgr::postRPCToProc(int_process *proc, int_iRPC::ptr rpc)
{
   ...
   std::set<Dyninst::Address> traps_needed;
   ...
   if(!traps_needed.empty())
   {
	   for(std::set<Dyninst::Address>::iterator it = traps_needed.begin();
		   it != traps_needed.end();
		   ++it)
	   {
		   Breakpoint::ptr transfer = Breakpoint::newTransferBreakpoint(rpc->addr());
		   proc->proc()->addBreakpoint(*it, transfer);
		   pthrd_printf("Inserting transfer breakpoint from 0x%x to 0x%x\n", *it, transfer->getToAddress());
		   transfer_breakpoints.insert(std::make_pair(transfer->getToAddress(), rpc));
	   }
	   return true;
   }
