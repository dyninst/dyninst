...
class bgq_process : public sysv_process,
 ...
 private:
  static bool sendMessage(const ToolMessage &msg, uint16_t msg_type,
                          uint32_t rank, response::ptr resp);

  bool sendCommand(const ToolCommand &cmd, uint16_t cmd_type,
                   response::ptr resp = response::ptr(),
                   unsigned int resp_mod = 0);
 ...
 public:
  ...
  static uint32_t getCommandAckLength(uint16_t cmd_type,
  ...
  virtual bool plat_handleStackInfo(stack_response::ptr stk_resp,
                                    CallStackCallback *cbs);

  virtual bool plat_waitAndHandleForProc();
  virtual bool plat_readMem(int_thread *thr, void *local, Dyninst::Address addr,
                            size_t size);
  ...
  bool internal_writeMem(int_thread *stop_thr, const void *local,
                         Dyninst::Address addr, size_t size,
                         result_response::ptr result, int_thread *thr,
                         bp_write_t bp_write);
  ...
  uint32_t getRank();
  void fillInToolMessage(ToolMessage &toolmsg, uint16_t msg_type,
                         response::ptr resp = response::ptr());
  ...
  virtual void bgq_getProcCoordinates(unsigned &a, unsigned &b, unsigned &c,
                                      unsigned &d, unsigned &e,
                                      unsigned &t) const;
  virtual unsigned int bgq_getComputeNodeID() const;
  virtual void bgq_getSharedMemRange(Dyninst::Address &start,
                                     Dyninst::Address &end) const;
  virtual void bgq_getPersistantMemRange(Dyninst::Address &start,
                                         Dyninst::Address &end) const;
  virtual void bgq_getHeapMemRange(Dyninst::Address &start,
                                   Dyninst::Address &end) const;

  virtual bool plat_getFileNames(FileSetResp_t *resp);
  virtual bool plat_getFileStatData(
 ...
 private:
  ...
  EventControlAuthority::ptr
      pending_control_authority;  // Used for releasing control authority
  int_eventControlAuthority
      *stopwait_on_control_authority;  // Used for gaining control authority
  std::string tooltag;
  uint8_t priority;
...
class bgq_thread : public thread_db_thread, public ppc_thread {
 ...
 public:
  ...
  static void regBufferToPool(BG_Reg_t *gprs, BG_Special_Regs *sregs,
                              int_registerPool &reg);
  static void regPoolToBuffer(int_registerPool &reg, BG_Reg_t *gprs,
...
struct buffer_t {
  ...
  buffer_t(void *b, size_t s, bool h)
      : buffer(b), size(s), is_heap_allocated(h), is_timeout(false) {}
  buffer_t()
      : buffer(NULL), size(0), is_heap_allocated(false), is_timeout(false) {}
};
...
class ComputeNode {
 ...
 public:
  ...
  int getFD() const;
  int getID() const;  // The BG/Q doc calls this the 'pid', which is unfortunate
                      // naming cause
  ...
  bool writeToolAttachMessage(bgq_process *proc, ToolMessage *msg,
                              bool heap_alloced);
...
// Handles the releasing of CA, triggers on the continue after a
// HandlePreControlAuthority event
...
class DecoderBlueGeneQ : public Decoder {
 private:
  Event::ptr decodeCompletedResponse(
      response::ptr resp, int_process *proc, int_thread *thrd,
      map<Event::ptr, EventAsync::ptr> &async_evs);

  bool decodeStartupEvent(ArchEventBGQ *ae, bgq_process *proc, void *data,
  ...
  bool decodeSignal(ArchEventBGQ *archevent, bgq_process *proc,
                    vector<Event::ptr> &events);
  bool decodeBreakpoint(ArchEventBGQ *archevent, bgq_process *proc,
                        int_thread *thr, Address addr,
                        vector<Event::ptr> &events,
                        bool allow_signal_decode = true);
  bool decodeGenericSignal(ArchEventBGQ *archevent, bgq_process *proc,
  ...
  bool decodeExit(ArchEventBGQ *archevent, bgq_process *proc,
                  std::vector<Event::ptr> &events);
  bool decodeControlNotify(ArchEventBGQ *archevent, bgq_process *proc,
                           std::vector<Event::ptr> &events);
  bool decodeDetachAck(ArchEventBGQ *archevent, bgq_process *proc,
                       std::vector<Event::ptr> &events);
  bool decodeReleaseControlAck(ArchEventBGQ *archevent, bgq_process *proc,
                               int err_code, std::vector<Event::ptr> &events);
  bool decodeControlAck(ArchEventBGQ *ev, bgq_process *qproc,
                        vector<Event::ptr> &events);
  bool decodeLWPRefresh(ArchEventBGQ *ev, bgq_process *proc, ToolCommand *cmd,
                        std::vector<Event::ptr> &events);
  bool decodeTimeout(vector<Event::ptr> &events);
  bool decodeFileStat(ToolCommand *cmd, bgq_process *proc, unsigned int resp_id,
                      int rc);
  bool decodeFileContents(ArchEventBGQ *ev, bgq_process *proc, ToolCommand *cmd,
                          int rc, unsigned int resp_id, bool owns_msg,
                          std::vector<Event::ptr> &events);
  bool decodeGetFilenames(ArchEventBGQ *ev, bgq_process *proc, ToolCommand *cmd,
                          int rc, int id);
  bool decodeMemoryQuery(ArchEventBGQ *ev, bgq_process *proc, ToolCommand *cmd,
