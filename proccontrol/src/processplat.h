...
class int_libraryTracking : virtual public int_process {
 public:
  ...
  virtual bool setTrackLibraries(bool b, int_breakpoint *&bp, Address &addr,
                                 bool &add_bp) = 0;
...
class int_multiToolControl : virtual public int_process {
 ...
 public:
  ...
  int_multiToolControl(Dyninst::PID p, std::string e,
                       std::vector<std::string> a,
                       std::vector<std::string> envp, std::map<int, int> f);
