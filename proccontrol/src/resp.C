...
void resp_process::rmResponse(Resp::ptr resp, bool lock_held) {
  if (resp->isCleaned) return;
  resp->isCleaned = true;

  if (!lock_held) active_resps_lock.lock();
  ...
  if (!lock_held) active_resps_lock.unlock();
}
...
Resp::Resp(resp_process *proc_)
    : id_start(newResponseID()), id_end(id_start + 1), proc(proc_) {
  init();
}

Resp::Resp(unsigned int multi_size, resp_process *proc_)
    : id_start(newResponseID()), id_end(id_start + multi_size), proc(proc_) {
  init();
}
...
unsigned int Resp::getID() { return id_start; }

unsigned int Resp::getIDEnd() { return id_end; }

resp_process *Resp::getProc() { return proc; }

void Resp::done() { proc->markRespDone(this); }

void Resp::post() { proc->markRespPosted(this); }

bool Resp::hadError() const { return (state == Error); }
