...
void
resp_process::addResp(Resp::ptr resp, unsigned id_start, unsigned id_end)
{
...
Resp::ptr
resp_process::recvResp(unsigned int id, bool& is_complete)
{
    Resp::ptr resp;
...
void
resp_process::markRespDone(Resp::ptr resp)
{
    active_resps_lock.lock();
...
void
resp_process::markRespPosted(Resp::ptr resp)
{
    resp->state = Resp::Posted;
}

void
resp_process::waitForEvent(Resp::ptr resp)
{
...
void
resp_process::rmResponse(Resp::ptr resp, bool lock_held)
{
    if(resp->isCleaned)
        return;
    resp->isCleaned = true;
