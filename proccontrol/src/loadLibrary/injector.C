...
bool
Injector::inject(std::string libname)
{
    ...
    int_iRPC::ptr irpc =
        int_iRPC::ptr(new int_iRPC(codegen.buffer().start_ptr(), codegen.buffer().size(),
                                   false, true, codegen.buffer().startAddr()));
...
bool
Injector::checkIfExists(std::string name)
{
...
bool
Injector::libraryLoaded(std::string name)
{
