...
bool Codegen::generate() {
  ...
  if (objSymReader) objSymReader->getABIVersion(abimajversion_, abiminversion_);
...
Address Codegen::findSymbolAddr(const std::string name, bool saveTOC) {
  ...
  for (auto li = libs.begin(); li != libs.end(); li++) {
    ...
    Address addr =
        (*li)->getLoadAddress() + objSymReader->getSymbolOffset(lookupSym);

    if (saveTOC) {
      toc_[addr] =
          (*li)->getLoadAddress() + objSymReader->getSymbolTOC(lookupSym);
    }
    return addr;
  }
  return 0;
