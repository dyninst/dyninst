...
Codegen::generate()
{
    ...
    SymReader* objSymReader =
        proc_->llproc()->getSymReader()->openSymbolReader(exe->getName());
    if(objSymReader)
...
unsigned
Codegen::startOffset() const
{
    return codeStart_ - buffer_.startAddr();
}

unsigned
Codegen::estimateSize()
{
...
Address
Codegen::findSymbolAddr(const std::string name, bool saveTOC)
{
    ...
    for(auto li = libs.begin(); li != libs.end(); li++)
    {
        ...
        SymReader* objSymReader =
            proc_->llproc()->getSymReader()->openSymbolReader((*li)->getName());
        if(!objSymReader)
            continue;
...
Address
Codegen::copyString(std::string name)
{
...
Address
Codegen::copyBuf(void* buf, unsigned size)
{
    Address ret = buffer_.curAddr();
...
Address
Codegen::copyByte(unsigned char c)
{
    Address ret = buffer_.curAddr();
    buffer_.push_back(c);
    return ret;
...
Address
Codegen::copyInt(unsigned int i)
{
    Address ret = buffer_.curAddr();
    buffer_.push_back(i);
    return ret;
...
Address
Codegen::copyLong(unsigned long l)
{
    Address ret = buffer_.curAddr();
    buffer_.push_back(l);
    return ret;
...
bool
Codegen::generateCall(Address addr, const std::vector<Address>& args)
{
