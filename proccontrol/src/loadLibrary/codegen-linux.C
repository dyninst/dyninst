...
Codegen::generateInt()
{
    ...
    {
        if(!generateStackUnprotect(var_addr, mprotect_addr))
            return false;
    }

    if(!generateCall(dlopen_addr, arguments))
        return false;
...
Address
Codegen::buildLinuxArgStruct(Address libbase, unsigned mode)
{
    ...
    if(proc_->getArchitecture() == Arch_x86 || proc_->getArchitecture() == Arch_ppc32)
    {
...
bool
Codegen::generateStackUnprotect(Address var_addr, Address mprotect_addr)
{
