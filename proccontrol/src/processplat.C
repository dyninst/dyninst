...
LibraryTracking::LibraryTracking(Process::ptr proc_) : proc(proc_) {}

LibraryTracking::~LibraryTracking() { proc = Process::weak_ptr(); }
...
ThreadTracking::ThreadTracking(Process::ptr proc_) : proc(proc_) {}

ThreadTracking::~ThreadTracking() { proc = Process::weak_ptr(); }
...
LWPTracking::LWPTracking(Process::ptr proc_) : proc(proc_) {}

LWPTracking::~LWPTracking() { proc = Process::weak_ptr(); }
...
FollowFork::FollowFork(Process::ptr proc_) : proc(proc_) {}

FollowFork::~FollowFork() { proc = Process::weak_ptr(); }

FollowFork::follow_t FollowFork::default_should_follow_fork =
    FollowFork::Follow;
...
CallStackUnwinding::CallStackUnwinding(Thread::ptr t) : wt(t) {}
...
CallStackCallback::CallStackCallback() : top_first(top_first_default_value) {}
...
MultiToolControl::MultiToolControl(Process::ptr p) : proc(p) {}

MultiToolControl::~MultiToolControl() { proc = Process::weak_ptr(); }
...
MemoryUsage::MemoryUsage(Process::ptr proc_) : proc(proc_) {}

MemoryUsage::~MemoryUsage() { proc = Process::weak_ptr(); }
...
bool MemoryUsage::resident(unsigned long &resident) const {
  ...
  MemUsageResp_t st_resp(&st, llproc), he_resp(&he, llproc),
      sh_resp(&sh, llproc), re_resp(&re, llproc);
...
SignalMask::SignalMask(Process::ptr proc_) : proc(proc_) {}
...
BGQData::BGQData(Process::ptr proc_) : proc(proc_) {}
...
bool BGQData::getProcCoordinates(unsigned &a, unsigned &b, unsigned &c,
                                 unsigned &d, unsigned &e, unsigned &t) const {
  MTLock lock_this_func;
  Process::ptr p = proc.lock();
...
bool BGQData::getSharedMemRange(Dyninst::Address &start,
...
bool BGQData::getPersistantMemRange(Dyninst::Address &start,
                                    Dyninst::Address &end) const {
  MTLock lock_this_func;
  Process::ptr p = proc.lock();
...
bool BGQData::getHeapMemRange(Dyninst::Address &start,
                              Dyninst::Address &end) const {
  MTLock lock_this_func;
  Process::ptr p = proc.lock();
...
FileInfo::FileInfo(const FileInfo &fi) { info = fi.info; }
...
int_fileInfo_ptr FileInfo::getInfo() const {
  if (!info) info = int_fileInfo_ptr(new int_fileInfo());
  return info;
}

RemoteIO::RemoteIO(Process::ptr proc_) : proc(proc_) {}
...
FileSet *RemoteIO::getFileSet(string filename) const {
  ...
  new_fs->insert(
      make_pair(static_cast<Process::const_ptr>(p), FileInfo(filename)));
  return new_fs;
}
...
FileSet *RemoteIO::getFileSet(const set<string> &filenames) const {
  ...
  for (set<string>::const_iterator i = filenames.begin(); i != filenames.end();
       i++) {
...
RemoteIOSet::RemoteIOSet(ProcessSet::ptr procs_) { pset = procs_; }
...
FileSet *RemoteIOSet::getFileSet(string filename) {
  ...
  if (!procs || procs->empty()) {
    globalSetLastError(err_badparam,
                       "Cannot create fileset from empty process set");
    return NULL;
  }
...
FileSet *RemoteIOSet::getFileSet(const set<string> &filenames) {
  ...
  if (filenames.empty()) {
    globalSetLastError(err_badparam,
                       "Cannot create a fileset from empty list of names");
    return NULL;
  }

  FileSet *new_fs = new FileSet();
  for (ProcessSet::iterator i = procs->begin(); i != procs->end(); i++) {
    for (set<string>::iterator j = filenames.begin(); j != filenames.end();
         j++) {
...
int_libraryTracking::int_libraryTracking(Dyninst::PID p, string e,
                                         vector<string> a, vector<string> envp,
                                         map<int, int> f)
    : int_process(p, e, a, envp, f), up_ptr(NULL) {}

int_libraryTracking::int_libraryTracking(Dyninst::PID pid_, int_process *p)
    : int_process(pid_, p), up_ptr(NULL) {}
...
bool int_LWPTracking::lwp_setTracking(bool b) {
  ...
  if (b == lwp_tracking) return true;
  lwp_tracking = b;
  return plat_lwpChangeTracking(b);
}

bool int_LWPTracking::plat_lwpChangeTracking(bool) { return true; }

bool int_LWPTracking::lwp_getTracking() { return lwp_tracking; }
...
bool int_LWPTracking::plat_lwpRefresh(result_response::ptr) { return false; }
...
bool int_LWPTracking::lwp_refreshCheck(bool &change) {
  ...
  if (lwps.size() - new_lwps_found != pool->size()) {
    for (int_threadPool::iterator i = pool->begin(); i != pool->end(); i++) {
      int_thread *thr = *i;
      bool found = false;
      for (vector<Dyninst::LWP>::iterator j = lwps.begin(); j != lwps.end();
           j++) {
        if (thr->getLWP() == *j) {
          found = true;
          break;
...
int_threadTracking::int_threadTracking(Dyninst::PID p, string e,
                                       vector<string> a, vector<string> envp,
                                       map<int, int> f)
    : int_process(p, e, a, envp, f), up_ptr(NULL) {}
...
int_multiToolControl::int_multiToolControl(Dyninst::PID p, string e,
                                           vector<string> a,
                                           vector<string> envp, map<int, int> f)
    : int_process(p, e, a, envp, f), up_ptr(NULL) {}
...
int_callStackUnwinding::int_callStackUnwinding(Dyninst::PID p, string e,
                                               vector<string> a,
                                               vector<string> envp,
                                               map<int, int> f)
    : int_process(p, e, a, envp, f) {}

int_callStackUnwinding::int_callStackUnwinding(Dyninst::PID pid_,
                                               int_process *p)
    : int_process(pid_, p) {}

int_callStackUnwinding::~int_callStackUnwinding() {}

int_memUsage::int_memUsage(Dyninst::PID p, std::string e,
...
int_memUsage::int_memUsage(Dyninst::PID pid_, int_process *p)
    : int_process(pid_, p), resp_process(pid_, p), up_ptr(NULL) {}
...
unsigned int int_BGQData::startup_timeout_sec =
    BGQData::startup_timeout_sec_default;
bool int_BGQData::block_for_ca = BGQData::block_for_ca_default;

int_fileInfo::int_fileInfo() : stat_results(NULL), cur_pos(0) {}
...
int_remoteIO::int_remoteIO(Dyninst::PID p, std::string e,
                           std::vector<std::string> a,
                           std::vector<std::string> envp, std::map<int, int> f)
    : int_process(p, e, a, envp, f),
      resp_process(p, e, a, envp, f),
...
bool int_remoteIO::getFileStatData(FileSet &files) {
  ...
  for (FileSet::iterator i = files.begin(); i != files.end(); i++) {
    ...
    bool result =
        plat_getFileStatData(info->filename, &info->stat_results, resps);
    if (!result) {
