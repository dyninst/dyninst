...
unsigned
arm_process::plat_breakpointSize()
{
...
void
arm_process::plat_breakpointBytes(unsigned char* buffer)
{
    // memory oppucation:
    // high---low addr
    //[3] [2] [1] [0]
...
bool
arm_process::plat_breakpointAdvancesPC() const
{
...
bool
arm_process::plat_convertToBreakpointAddress(Address&, int_thread*)
{
    return true;
}
...
void
arm_process::cleanupSSOnContinue(int_thread* thr)
{
...
async_ret_t
arm_process::readPCForSS(int_thread* thr, Address& pc)
{
...
async_ret_t
arm_process::readInsnForSS(Address pc, int_thread*, unsigned int& rawInsn)
{
    if(!plat_needsAsyncIO())
    {
        // Fast-track for linux/arm
...
async_ret_t
arm_process::plat_needsEmulatedSingleStep(int_thread*           thr,
                                          std::vector<Address>& addrResult)
{
    ...
    do
    {
        ...
        // sequence and place breakpoint there as well First check if is branch reg
        // instruction.
        Address cfTarget;
        if(insn.isBranchReg())
        {
            //#warning "Test cases don't cover this type of instruction. Potential bugs
            // here.\n"
            pthrd_printf("ARM-DEBUG: find Branch Reg instruction, going to retrieve the "
                         "target address.\n");
            ...
            bool result = thr->getRegister(MachRegister::getArchReg(regNum, Arch_aarch64),
                                           Response);
            if(!result || Response->hasError())
            {
                pthrd_printf("Error reading PC address to check for emulated single step "
                             "condition\n");
                return aret_error;
            }
        ...
        {  // return target address by calculating the offset and pc
...
void
arm_process::plat_getEmulatedSingleStepAsyncs(int_thread*, std::set<response::ptr> resps)
{
...
bool
arm_thread::rmHWBreakpoint(hw_breakpoint*, bool, std::set<response::ptr>&, bool&)
{
    return false;
}

bool
arm_thread::addHWBreakpoint(hw_breakpoint*, bool, std::set<response::ptr>&, bool&)
{
    return false;
}

unsigned
arm_thread::hwBPAvail(unsigned)
{
    return 0;
}

EventBreakpoint::ptr
arm_thread::decodeHWBreakpoint(response::ptr&, bool, Dyninst::MachRegisterVal)
{
    return EventBreakpoint::ptr();
}

bool
arm_thread::bpNeedsClear(hw_breakpoint*)
{
...
void
arm_thread::setCachedPC(Address pc)
{
    // what is cached PC?
    cached_pc      = pc;
    have_cached_pc = true;
...
void
arm_thread::clearCachedPC()
{
    have_cached_pc = false;
}

bool
arm_thread::haveCachedPC(Address& pc)
{
    if(!have_cached_pc)
        return false;
    pc = cached_pc;
