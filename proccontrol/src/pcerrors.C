...
unsigned long gettod() {
  ...
  unsigned long long t =
      (unsigned long long)((val.tv_sec * 1000) + (val.tv_usec / 1000));
  if (!start_set) {
    start_set = true;
    start = t;
  }
...
void setGeneratorThread(long t) { gen_thrd_id = t; }

void setHandlerThread(long t) { handler_thrd_id = t; }

void setXThread(long t) { x_thrd_id = t; }

void setRThread(long t) { r_thrd_id = t; }

void setWThread(long t) { w_thrd_id = t; }

bool isGeneratorThread() { return DThread::self() == gen_thrd_id; }

bool isHandlerThread() { return DThread::self() == handler_thrd_id; }

bool isUserThread() { return !isGeneratorThread() && !isHandlerThread(); }
err_t Dyninst::ProcControlAPI::getLastError() { return last_error; }

void Dyninst::ProcControlAPI::clearLastError() { last_error = 0; }
...
void Dyninst::ProcControlAPI::setDebugChannel(FILE *f) { pctrl_err_out = f; }
