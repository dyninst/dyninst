...
bool
GeneratorLinux::canFastHandle()
{
    return false;
}

ArchEvent*
GeneratorLinux::getEvent(bool block)
{
    int status, options;
...
void
GeneratorLinux::evictFromWaitpid()
{
    if(!generator_lwp)
        return;
...
unsigned
DecoderLinux::getPriority() const
{
    return Decoder::default_priority;
}

Dyninst::Address
DecoderLinux::adjustTrapAddr(Dyninst::Address addr, Dyninst::Architecture arch)
{
...
bool
DecoderLinux::decode(ArchEvent* ae, std::vector<Event::ptr>& events)
{
    bool            result;
            ...
            case(SIGTRAP | 0x80):  // PTRACE_O_TRACESYSGOOD
                ...
                if(lthread->hasPostponedSyscallEvent())
                {
                       ...
                       lthread->addr_fakeSyscallExitBp == addr)
                    {
                        ...
                        {
                            perr_printf(
                                "ARM-error: Failed to remove inserted BP, addr %p.\n",
                                (void*) addr);
                        }
                    ...
                    switch(ext)
                    {
                        ...
                        case PTRACE_EVENT_CLONE:
                            pthrd_printf(
                                "Resuming %s event after syscall exit on %d/%d\n",
                ...
                // through the rest of the event handling
                ...
                if(ext)
                {
                    ...
                    switch(ext)
                    {
                        case PTRACE_EVENT_EXIT:
                            ...
                            if(thread->getLWP() == proc->getPid())
                            {
                                ...
                                if(r == -1)
                                {
                                    int error = errno;
                                    perr_printf(
                                        "Error getting event message from exit\n");
                                    if(error == ESRCH)
                                        proc->setLastError(
                                            err_exited,
                                ...
                                if(WIFSIGNALED(exitcode))
                                {
                                    ...
                                    pthrd_printf("Decoded event to pre-exit due to "
                                                 "crash/signal %d\n",
                                                 termsig);
                                ...
                                pthrd_printf("Decoded event to pre-exit of process %d/%d "
                                             "with code %i\n",
                                             proc->getPid(), thread->getLWP(), exitcode);
                                event =
                                    Event::ptr(new EventExit(EventType::Pre, exitcode));

                                for(int_threadPool::iterator j =
                                        proc->threadPool()->begin();
                                    j != proc->threadPool()->end(); ++j)
                                {
                                    dynamic_cast<linux_thread*>(*j)
                                        ->setGeneratorExiting();
                                }
                            }
                            else
                            {
                                EventLWPDestroy::ptr lwp_ev = EventLWPDestroy::ptr(
                                    new EventLWPDestroy(EventType::Pre));
                                event = lwp_ev;
                        ...
                        case PTRACE_EVENT_CLONE: {
                            ...
                            int           r      = do_ptrace((pt_req) PTRACE_GETEVENTMSG,
                                              (pid_t) thread->getLWP(), NULL, &cpid_l);
                            if(r == -1)
                            {
                                int error = errno;
                                perr_printf(
                                    "Error getting event message from fork/clone\n");
                                if(error == ESRCH)
            ...
            printf("0x%8x\n", *((unsigned int*)buffer_inst) );
                ...
                if(ibp && ibp != clearingbp)
                {
                    ...
                    if(adjusted_addr == lproc->getLibBreakpointAddr())
                    {
                        ...
                        EventLibrary::ptr lib_event =
                            EventLibrary::ptr(new EventLibrary());
	       ...
	       fprintf(stderr,"0x%8x\n", *((unsigned int*)buffer_inst) );
               ...
               //while (1) sleep(1);
                ...
                result = thread->plat_getRegister(
                    MachRegister::getPC(proc->getTargetArch()), addr);
                event = Event::ptr(
                    new EventSignal(stopsig, addr, EventSignal::Unknown, false));
        }
...
int_process*
int_process::createProcess(Dyninst::PID p, std::string e)
{
...
int_process*
int_process::createProcess(std::string e, std::vector<std::string> a,
...
int_process*
int_process::createProcess(Dyninst::PID pid_, int_process* p)
{
...
int
linux_process::computeAddrWidth()
{
    /**
...
bool
linux_process::plat_getOSRunningStates(std::map<Dyninst::LWP, bool>& runningStates)
{
...
// Ubuntu 10.10 and other hardened systems do not allow arbitrary ptrace_attaching;
// instead you may only attach to a child process
...
warn_user_ptrace_restrictions()
{
    ...
    if(ptrace_scope.is_open())
    {
        ...
        if(val == 1)
        {
            cerr << "Warning: your Linux system provides limited ptrace functionality as "
                    "a security"
                 << endl
                 << "measure. This measure prevents ProcControl and Dyninst from "
                    "attaching to binaries."
                 << endl
                 << "To temporarily disable this measure (until a reboot), execute the "
                    "following command:"
                 << endl
...
bool
linux_process::plat_attach(bool, bool&)
{
    ...
    if(!attachWillTriggerStop)
    {
        ...
        pthrd_printf(
            "Attach will not trigger stop, calling PTRACE_CONT to flush out stop\n");
...
bool
linux_process::plat_attachThreadsSync()
{
    while(true)
    {
...
bool
linux_process::plat_attachWillTriggerStop()
{
    char  procName[64];
    char  cmd[256];
    pid_t tmpPid;
    ...
    if(!sfile)
    {
        perr_printf("Failed to determine whether attach would trigger stop -- assuming "
                    "it will\n");
        return true;
    }

    if(fscanf(sfile.get(), "%d %255s %c %d %d %d", &tmpPid, cmd, &state_char, &tmpPid,
              &tmpPid, &ttyNumber) < 0)
    {
        perr_printf("Failed to determine whether attach would trigger stop -- assuming "
                    "it will\n");
...
bool
linux_process::plat_forked()
{
    return true;
}

bool
linux_process::plat_readMem(int_thread* thr, void* local, Dyninst::Address remote,
                            size_t size)
{
    char file[128];
...
bool
linux_process::plat_writeMem(int_thread* thr, const void* local, Dyninst::Address remote,
                             size_t size, bp_write_t)
{
    char file[128];
    snprintf(file, 64, "/proc/%d/mem", getPid());
    int     fd  = open(file, O_RDWR);
...
linux_x86_process::getTargetArch()
{
...
bool
linux_x86_process::plat_supportHWBreakpoint()
{
    return true;
}
...
Dyninst::Architecture
linux_ppc_process::getTargetArch()
{
    if(arch != Dyninst::Arch_none)
    {
        return arch;
...
Dyninst::Architecture
linux_arm_process::getTargetArch()
{
    if(arch != Dyninst::Arch_none)
    {
        return arch;
...
void
linux_thread::fake_async_main(void*)
{
    for(;;)
    {
...
bool
linux_process::plat_needsAsyncIO() const
{
...
bool
linux_process::plat_readMemAsync(int_thread* thr, Dyninst::Address addr,
...
bool
linux_process::plat_writeMemAsync(int_thread* thr, const void* local,
...
bool
linux_process::needIndividualThreadAttach()
{
    return true;
}

bool
linux_process::getThreadLWPs(std::vector<Dyninst::LWP>& lwps)
{
    return findProcLWPs(pid, lwps);
}

bool
linux_process::plat_supportLWPCreate()
{
    return true;
}

bool
linux_process::plat_supportLWPPreDestroy()
{
    return true;
}

bool
linux_process::plat_supportLWPPostDestroy()
{
    return true;
}

void
linux_thread::postponeSyscallEvent(ArchEventLinux* event)
{
...
bool
linux_thread::hasPostponedSyscallEvent()
{
    return postponed_syscall_event != NULL;
}

ArchEventLinux*
linux_thread::getPostponedSyscallEvent()
{
...
SymbolReaderFactory*
linux_process::plat_defaultSymReader()
{
    return getElfReader();
}
...
int_thread*
int_thread::createThreadPlat(int_process* proc, Dyninst::THR_ID thr_id,
...
bool
linux_process::plat_individualRegAccess()
{
    return true;
}

bool
linux_process::plat_detach(result_response::ptr, bool leave_stopped)
{
...
bool
linux_process::plat_terminate(bool& needs_sync)
{
...
linux_process::preTerminate()
{
    ...
    for(i = threadPool()->begin(); i != threadPool()->end(); i++)
    {
        ...
        {
            perr_printf(
                "Failed to unset options for thread %d/%d in pre-terminate handling\n",
                getPid(), thr->getLWP());
            return false;
        }
    }
...
OSType
linux_process::getOS() const
{
    return Dyninst::Linux;
}

Dyninst::Address
linux_process::plat_mallocExecMemory(Dyninst::Address min, unsigned size)
{
...
bool
linux_process::fork_setTracking(FollowFork::follow_t f)
{
    int_threadPool::iterator i;
    for(i = threadPool()->begin(); i != threadPool()->end(); i++)
    {
...
FollowFork::follow_t
linux_process::fork_isTracking()
{
    return fork_tracking;
}

bool
linux_process::plat_lwpChangeTracking(bool)
{
    int_threadPool* pool = threadPool();
    if(!pool->allStopped(int_thread::UserStateID))
    {
        perr_printf("Attempted to change lwpTracking, but not all threads stopped in %d",
                    getPid());
        setLastError(err_notstopped,
...
bool
linux_process::allowSignal(int signal_no)
{
...
bool
linux_process::readStatM(unsigned long& stk, unsigned long& heap, unsigned long& shrd)
{
    char path[64];
    ...
    if(fscanf(f.get(), "%lu %lu %lu %lu %lu %lu %lu", &size, &resident, &shared, &text,
              &lib, &data, &dt) < 0)
    {
...
bool
linux_process::plat_getStackUsage(MemUsageResp_t* resp)
{
...
bool
linux_process::plat_getHeapUsage(MemUsageResp_t* resp)
{
    unsigned long stk, heap, shrd;
    bool          result = readStatM(stk, heap, shrd);
    if(!result)
...
bool
linux_process::plat_getSharedUsage(MemUsageResp_t* resp)
{
    unsigned long stk, heap, shrd;
    bool          result = readStatM(stk, heap, shrd);
    if(!result)
...
bool
linux_process::plat_residentNeedsMemVals()
{
    return false;
}

bool
linux_process::plat_getResidentUsage(unsigned long, unsigned long, unsigned long,
                                     MemUsageResp_t*)
{
    return false;
}
...
#endif
bool
linux_thread::plat_getAllRegisters(int_registerPool& regpool)
{
...
#    define MY_PTRACE_GETREGS 0
    ...
    if(sentinel1 != 0xfeedface || sentinel2 != 0xfeedface)
        return false;
...
bool
linux_thread::plat_getRegister(Dyninst::MachRegister reg, Dyninst::MachRegisterVal& val)
{
#if defined(bug_registers_after_exit)
    /* On some kernels, attempting to read registers from a thread in a pre-Exit
     * state causes an oops
...
#else
    result =
        do_ptrace((pt_req) PTRACE_PEEKUSER, lwp, (void*) (unsigned long) offset, NULL);
#endif
    // unsigned long result = do_ptrace((pt_req) PTRACE_PEEKUSER, lwp, (void *) (unsigned
    // long) offset, NULL);
#if defined(arch_aarch64)
    if(ret != 0)
    {
...
bool
linux_thread::plat_setAllRegisters(int_registerPool& regpool)
{
...
#    define MY_PTRACE_SETREGS 0
    ...
    if(have_setregs)
    {
        ...
        if(!plat_convertToSystemRegs(regpool, user_area))
            return false;
    ...
    if(!have_setregs)
    {
        ...
        // set regs
        for(int_registerPool::iterator i = regpool.regs.begin(); i != regpool.regs.end();
            i++)
        {
...
bool
linux_thread::plat_convertToSystemRegs(const int_registerPool& regpool,
                                       unsigned char* user_area, bool gprs_only)
{
    init_dynreg_to_user();
...
bool
linux_thread::plat_setRegister(Dyninst::MachRegister reg, Dyninst::MachRegisterVal val)
{
#if defined(bug_registers_after_exit)
    /* On some kernels, attempting to read registers from a thread in a pre-Exit
     * state causes an oops
    ...
    init_dynreg_to_user();
    dynreg_to_user_t::iterator i = dynreg_to_user.find(reg);
    if(reg.getArchitecture() != llproc()->getTargetArch() || i == dynreg_to_user.end())
    {
    ...
    result = do_ptrace((pt_req) PTRACE_POKEUSER, lwp, (void*) (uintptr_t) offset,
                       (void*) value);
#endif
    // result = do_ptrace((pt_req) PTRACE_POKEUSER, lwp, (void *) (uintptr_t)offset, (void
    // *) value);
...
bool
linux_thread::plat_getAllRegistersAsync(allreg_response::ptr result)
{
...
bool
linux_thread::plat_getRegisterAsync(Dyninst::MachRegister reg, reg_response::ptr result)
{
...
bool
linux_thread::plat_setAllRegistersAsync(int_registerPool&    pool,
...
bool
linux_thread::plat_setRegisterAsync(Dyninst::MachRegister    reg,
...
bool
linux_thread::plat_handle_ghost_thread()
{
    std::string loc =
        "/proc/" + std::to_string(proc()->getPid()) + "/task/" + std::to_string(getLWP());
    struct stat dummy;
    ...
    if(res != -1)
    {
        // NB: We got here because ptrace returned ESRCH (no such process) in
        // LinuxPtrace::ptrace_int,
        //     yet the process is alive. This is a valid state for ptrace, but ptrace_int
        //     doesn't check for this. The ptrace man-page indicates that this can happen
        //     when a process received a STOP signal, but it hasn't transitioned to the
        //     new state yet.
        return false;
    }
...
bool
linux_thread::thrdb_getThreadArea(int val, Dyninst::Address& addr)
{
...
bool
linux_thread::getSegmentBase(Dyninst::MachRegister reg, Dyninst::MachRegisterVal& val)
{
    switch(llproc()->getTargetArch())
    {
        case Arch_x86_64:
...
bool
linux_thread::suppressSanityChecks()
{
...
bool
ArchEventLinux::findPairedEvent(ArchEventLinux*& parent, ArchEventLinux*& child)
{
    bool is_parent;
...
void
ArchEventLinux::postponePairedEvent()
{
...
Handler::handler_ret_t
LinuxHandleNewThr::handleEvent(Event::ptr ev)
{
...
int
LinuxHandleNewThr::getPriority() const
{
    return PostPlatformPriority;
}

void
LinuxHandleNewThr::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
LinuxHandleLWPDestroy::handleEvent(Event::ptr ev)
{
...
int
LinuxHandleLWPDestroy::getPriority() const
{
    return PostPlatformPriority;
}

void
LinuxHandleLWPDestroy::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
LinuxHandleForceTerminate::handleEvent(Event::ptr ev)
{
...
int
LinuxHandleForceTerminate::getPriority() const
{
    return PostPlatformPriority;
}

void
LinuxHandleForceTerminate::getEventTypesHandled(std::vector<EventType>& etypes)
{
    etypes.push_back(EventType(EventType::Post, EventType::ForceTerminate));
}

HandlerPool*
linux_createDefaultHandlerPool(HandlerPool* hpool)
{
...
HandlerPool*
plat_createDefaultHandlerPool(HandlerPool* hpool)
{
    return linux_createDefaultHandlerPool(hpool);
}

bool
ProcessPool::LWPIDsAreUnique()
{
    return true;
}
...
long
do_ptrace(pt_req request, pid_t pid, void* addr, void* data)
{
    return LinuxPtrace::getPtracer()->ptrace_int(request, pid, addr, data);
}

LinuxPtrace*
LinuxPtrace::getPtracer()
{
    if(!linuxptrace)
    {
...
long
LinuxPtrace::ptrace_int(pt_req request_, pid_t pid_, void* addr_, void* data_)
{
    start_request();
...
bool
LinuxPtrace::plat_create(linux_process* p)
{
    start_request();
...
bool
LinuxPtrace::ptrace_read(Dyninst::Address inTrace, unsigned size_, void* inSelf, int pid_)
{
    start_request();
...
bool
LinuxPtrace::ptrace_write(Dyninst::Address inTrace, unsigned size_, const void* inSelf,
                          int pid_)
{
    start_request();
...
void
linux_process::plat_adjustSyncType(Event::ptr ev, bool gen_)
{
    if(gen_)
        return;
    ...
    pthrd_printf(
        "plat_adjustSyncType: thread %d raced with exit, setting event to async\n",
        thrd->getLWP());
