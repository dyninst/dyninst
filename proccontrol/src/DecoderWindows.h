...
class DecoderWindows : public Decoder {
 ...
 private:
  Event::ptr decodeBreakpointEvent(DEBUG_EVENT e, int_process* proc,
                                   int_thread* thread);
  Event::ptr decodeSingleStepEvent(DEBUG_EVENT e, int_process* proc,
                                   int_thread* thread);
  bool checkForFullString(DEBUG_EVENT& details, int chunkSize, wchar_t* libName,
                          bool gotString, char* asciiLibName);
  ...
  bool decodeCreateThread(DEBUG_EVENT& e, Event::ptr& newEvt,
                          int_process*& proc, std::vector<Event::ptr>& events);

  std::string readLibNameFromProc(Address libnameaddr, DEBUG_EVENT details,
                                  int_process* p);
