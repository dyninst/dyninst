...
void
int_notify::unix_details::writeToPipe()
{
    if(pipe_out == -1)
        return;
    ...
    if(result == -1)
    {
        ...
        ProcControlAPI::globalSetLastError(err_internal,
                                           "Could not write to notification pipe\n");
...
void
int_notify::unix_details::readFromPipe()
{
    if(pipe_out == -1)
        return;
    ...
    if(result == -1)
    {
        ...
        ProcControlAPI::globalSetLastError(err_internal,
                                           "Could not read from notification pipe\n");
...
void
int_notify::unix_details::clearEvent()
{
    readFromPipe();
}
...
bool
int_notify::unix_details::internalsValid()
{
    return (pipe_in != -1 && pipe_out != -1);
}

void
int_notify::unix_details::noteEvent()
{
    writeToPipe();
}

bool
int_notify::unix_details::createInternals()
{
