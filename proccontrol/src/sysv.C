...
sysv_process::sysv_process(Dyninst::PID pid_, int_process *p)
    : int_process(pid_, p), int_libraryTracking(pid_, p) {
  ...
  procreader = NULL;
  if (sp->procreader) procreader = new PCProcReader(this);
  translator_ = NULL;
  if (sp->translator_) {
...
AddressTranslate *sysv_process::constructTranslator(Dyninst::PID pid_) {
  ...
  if (result) {
    return AddressTranslate::createAddressTranslator(
        pid_, procreader, getSymReader(), INVALID_HANDLE_VALUE, std::string(""),
        base);
  } else {
...
PCProcReader::PCProcReader(sysv_process *proc_) : proc(proc_) {}

PCProcReader::~PCProcReader() { proc->getMemCache()->clear(); }

bool PCProcReader::start() { return true; }

bool PCProcReader::done() { return true; }
...
bool PCProcReader::getNewAsyncs(set<response::ptr> &resps) {
  for (set<mem_response::ptr>::iterator i = memresults.begin();
       i != memresults.end(); i++)
    resps.insert(*i);
  memresults.clear();
  return true;
...
bool sysv_process::initLibraryMechanism() {
  ...
  if (!procreader) procreader = new PCProcReader(this);
  assert(procreader);
...
bool sysv_process::refresh_libraries(set<int_library *> &added_libs,
  ...
  for (vector<LoadedLib *>::iterator i = ll_libs.begin(); i != ll_libs.end();
       i++) {
    LoadedLib *ll = *i;
    ...
    if (!lib) {
      pthrd_printf("Creating new library object for %s\n",
                   ll->getName().c_str());
      // Note: we set them all to "I'm a shared library"; the a.out is
      // overridden below.
...
bool sysv_process::plat_isStaticBinary() { return (breakpoint_addr == 0); }

int_library *sysv_process::plat_getExecutable() { return aout; }
...
bool sysv_process::plat_getInterpreterBase(Address &) { return false; }

bool sysv_process::setTrackLibraries(bool b, int_breakpoint *&bp, Address &addr,
                                     bool &add_bp) {
...
bool sysv_process::isTrackingLibraries() { return track_libraries; }
...
void sysv_process::createAddrTranslator() { translator_state = Ready; }
