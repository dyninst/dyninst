...
AddressTranslate*
sysv_process::constructTranslator(Dyninst::PID pid_)
{
    Address base;
    bool    result = plat_getInterpreterBase(base);
    if(result)
    {
        return AddressTranslate::createAddressTranslator(pid_, procreader, getSymReader(),
...
PCProcReader::~PCProcReader() { proc->getMemCache()->clear(); }
...
bool
PCProcReader::ReadMem(Address addr, void* buffer, unsigned size)
{
...
bool
PCProcReader::GetReg(MachRegister /*reg*/, MachRegisterVal& /*val*/)
{
    assert(0);  // Not needed
    return true;
}
...
bool
PCProcReader::hasPendingAsync()
{
    return proc->getMemCache()->hasPendingAsync();
}

bool
PCProcReader::getNewAsyncs(set<response::ptr>& resps)
{
    for(set<mem_response::ptr>::iterator i = memresults.begin(); i != memresults.end();
        i++)
        resps.insert(*i);
    memresults.clear();
    return true;
...
bool
sysv_process::initLibraryMechanism()
{
    if(lib_initialized)
    {
        if(translator() == NULL)
...
bool
sysv_process::refresh_libraries(set<int_library*>& added_libs,
                                set<int_library*>& rmd_libs, bool& waiting_for_async,
                                set<response::ptr>& async_responses)
{
    ...
    for(vector<LoadedLib*>::iterator i = ll_libs.begin(); i != ll_libs.end(); i++)
    {
        ...
        if(!lib)
        {
            ...
            // Note: we set them all to "I'm a shared library"; the a.out is overridden
            // below.

            lib = new int_library(ll->getName(), true, ll->getCodeLoadAddr(),
                                  ll->getDynamicAddr());
...
Dyninst::Address
sysv_process::getLibBreakpointAddr() const
{
    return breakpoint_addr;
}

bool
sysv_process::plat_execed()
{
...
bool
sysv_process::plat_isStaticBinary()
{
    return (breakpoint_addr == 0);
}

int_library*
sysv_process::plat_getExecutable()
{
    return aout;
}

bool
sysv_process::addSysVHandlers(HandlerPool*)
{
...
bool
sysv_process::plat_getInterpreterBase(Address&)
{
    return false;
}

bool
sysv_process::setTrackLibraries(bool b, int_breakpoint*& bp, Address& addr, bool& add_bp)
{
...
bool
sysv_process::isTrackingLibraries()
{
    return track_libraries;
}

AddressTranslate*
sysv_process::translator()
{
...
void
sysv_process::createAddrTranslator()
{
    translator_state = Ready;
}

void
sysv_process::deleteAddrTranslator()
{
    translator_state = NotReady;
    if(translator_)
        delete translator_;
