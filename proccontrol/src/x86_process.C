...
x86_process::x86_process(Dyninst::PID p, std::string e, std::vector<std::string> a,
...
unsigned
x86_process::plat_breakpointSize()
{
    return 1;
}

void
x86_process::plat_breakpointBytes(unsigned char* buffer)
{
    buffer[0] = 0xcc;
}

bool
x86_process::plat_breakpointAdvancesPC() const
{
    return true;
}
...
#define PERM_R 1 << 2
unsigned int
x86_thread::spaceNeeded(unsigned int perms)
{
    unsigned int needed = 0;
    if(perms & PERM_X)
        needed++;
    if((perms & PERM_R) || (perms & PERM_W))
        needed++;
    return needed;
}

unsigned int
x86_thread::spaceAvail()
{
...
int
x86_thread::getSlotForHBP(hw_breakpoint* hwbp, int last_slot)
{
...
bool
x86_thread::rmHWBreakpoint(hw_breakpoint* bp, bool suspend,
...
bool
x86_thread::addHWBreakpoint(hw_breakpoint* bp, bool resume,
                            std::set<response::ptr>& resps, bool& done)
{
...
EventBreakpoint::ptr
x86_thread::decodeHWBreakpoint(response::ptr&, bool have_reg,
...
unsigned
x86_thread::hwBPAvail(unsigned mode)
{
    return spaceAvail() / spaceNeeded(mode);
}

bool
x86_thread::bpNeedsClear(hw_breakpoint* hwbp)
{
