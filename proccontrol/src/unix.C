...
unix_process::unix_process(Dyninst::PID p, std::string e,
...
void unix_process::plat_execv() {
  ...
  for (std::map<int, int>::iterator fdit = fds.begin(), fdend = fds.end();
       fdit != fdend; ++fdit) {
    ...
    if (result == -1) {
      ...
      setLastError(err_internal,
                   "Unable to close file descriptor for redirection");
      exit(-1);
    }
  ...
  if (errnum == ENOENT) setLastError(err_nofile, "No such file");
...
bool unix_process::plat_findAllocatedRegionAround(
...
// I'm not sure that unix_process is the proper place for this--it's really
// based on whether
// /proc/PID/maps exists.  Currently, that matches our platforms that use
// unix_process, so
// I'll leave it be for now.
Dyninst::Address unix_process::plat_mallocExecMemory(Dyninst::Address min,
                                                     unsigned size) {
...
bool unix_process::plat_supportFork() { return true; }

bool unix_process::plat_supportExec() { return true; }
