...
unix_process::plat_execv()
{
    ...
    for(std::map<int, int>::iterator fdit = fds.begin(), fdend = fds.end(); fdit != fdend;
        ++fdit)
    {
...
unsigned
unix_process::getTargetPageSize()
{
...
bool
unix_process::plat_decodeMemoryRights(Process::mem_perm& perm, unsigned long rights)
{
    switch(rights)
    {
        default:
...
bool
unix_process::plat_encodeMemoryRights(Process::mem_perm perm, unsigned long& rights)
{
    if(perm.isNone())
    {
        rights = PROT_NONE;
...
bool
unix_process::plat_getMemoryAccessRights(Dyninst::Address addr, Process::mem_perm& perm)
{
    (void) addr;
    (void) perm;
...
bool
unix_process::plat_setMemoryAccessRights(Dyninst::Address addr, size_t size,
...
bool
unix_process::plat_findAllocatedRegionAround(Dyninst::Address       addr,
                                             Process::MemoryRegion& memRegion)
{
    ...
    setLastError(err_unsupported,
                 "Find Allocated Memory Region not supported on this platform\n");
    return false;
}
// I'm not sure that unix_process is the proper place for this--it's really based on
// whether
// /proc/PID/maps exists.  Currently, that matches our platforms that use unix_process, so
// I'll leave it be for now.
Dyninst::Address
unix_process::plat_mallocExecMemory(Dyninst::Address min, unsigned size)
{
...
bool
unix_process::plat_supportFork()
{
    return true;
}

bool
unix_process::plat_supportExec()
{
    return true;
}
