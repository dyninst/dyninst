...
#if !defined(amdgpu_process_h_)
#define amdgpu_process_h_
...
class amdgpu_process : virtual public int_process
{
 public:
  amdgpu_process(Dyninst::PID p, std::string e, std::vector<std::string> a,
              std::vector<std::string> envp, std::map<int, int> f);
  amdgpu_process(Dyninst::PID pid_, int_process *p) ;
  virtual ~amdgpu_process();
...
class amdgpu_thread : virtual public int_thread
{
  protected:
    bool have_cached_pc;
    Address cached_pc;
  public:
    amdgpu_thread(int_process *p, Dyninst::THR_ID t, Dyninst::LWP l);
    virtual ~amdgpu_thread();
