...
MachSyscall makeFromEvent(const EventSyscall* ev) {
  ...
  MachSyscall::SyscallName syscallName =
      MachSyscall::nameLookup(plat, syscallNumber);
#else
...
MachSyscall makeFromID(Process::ptr proc,
                       MachSyscall::SyscallIDIndependent id) {
  Architecture arch = proc->getArchitecture();
  OSType os = proc->getOS();
  Platform plat(arch, os);
