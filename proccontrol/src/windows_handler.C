...
WinHandleSingleStep::WinHandleSingleStep() : Handler("Windows Single Step") {}
...
int WinHandleSingleStep::getPriority() const { return PostPlatformPriority; }
...
ArchEventWindows::ArchEventWindows(DEBUG_EVENT e) : evt(e) {}

ArchEventWindows::~ArchEventWindows() {}

std::vector<ArchEventWindows *> ArchEventWindows::pending_events;

bool ArchEventWindows::findPairedEvent(ArchEventWindows *&parent,
                                       ArchEventWindows *&child) {
  assert(!"Not implemented");
  return false;
}

void ArchEventWindows::postponePairedEvent() { pending_events.push_back(this); }

WindowsHandleNewThr::WindowsHandleNewThr() : Handler("Windows New Thread") {}
...
Handler::handler_ret_t WindowsHandleNewThr::handleEvent(Event::ptr ev) {
  ...
  WinEventNewThread::ptr we =
      boost::dynamic_pointer_cast<WinEventNewThread>(ev);
  if (we) {
    pthrd_printf(
        "WinHandleCreateThread handling thread creation for thread %d, handle "
        "%x\n",
        lwp, we->getHandle());
  ...
  // Check to see if our start address (if we have one...) is in a system
  // library (if we know
  ...
  if (thr->getStartFuncAddress(start_addr)) {
    if (thr->llproc()->addrInSystemLib(start_addr)) {
      ...
      if (thr->llproc()
        ...
        // If the parent thread had a pending stop and this is a system thread,
        // we may have strong confidence that
...
int WindowsHandleNewThr::getPriority() const { return PostPlatformPriority; }
...
void WindowsHandleLWPDestroy::getEventTypesHandled(
...
WindowsHandleProcessExit::WindowsHandleProcessExit() : Handler("Windows Exit") {
...
void WindowsHandleProcessExit::getEventTypesHandled(
    std::vector<EventType> &etypes) {
...
int WinHandleBootstrap::getPriority() const { return PostPlatformPriority; }
...
Handler::handler_ret_t WindowsHandleSetThreadInfo::handleEvent(Event::ptr ev) {
  windows_thread *thr =
      static_cast<windows_thread *>(ev->getThread()->llthrd());

  WinEventThreadInfo::ptr we =
      boost::dynamic_pointer_cast<WinEventThreadInfo>(ev);
  if (we) {
    ProcPool()->rmThread(thr);
    pthrd_printf(
        "WinHandleSetThreadInfo handling thread info update for initial thread "
        "%d, handle %x\n",
...
int WindowsHandleSetThreadInfo::getPriority() const { return DefaultPriority; }

void WindowsHandleSetThreadInfo::getEventTypesHandled(
    std::vector<EventType> &etypes) {
...
void WindowsHandleThreadStop::getEventTypesHandled(
    std::vector<EventType> &etypes) {
