...
ps_err_e ps_pread(struct ps_prochandle *handle, psaddr_t remote, void *local,
                  size_t size) {
...
ps_err_e ps_pdread(struct ps_prochandle *handle, psaddr_t remote, void *local,
                   size_t size) {
  return ps_pread(handle, remote, local, size);
}

ps_err_e ps_ptread(struct ps_prochandle *handle, psaddr_t remote, void *local,
                   size_t size) {
  return ps_pread(handle, remote, local, size);
}

ps_err_e ps_pwrite(struct ps_prochandle *handle, psaddr_t remote,
                   const void *local, size_t size) {
  ...
  async_ret_t result = proc->getMemCache()->writeMemory(
      (Address)remote, const_cast<void *>(local), size, proc->res_resps,
      proc->triggerThread());
  switch (result) {
    case aret_success:
...
ps_err_e ps_pdwrite(struct ps_prochandle *handle, psaddr_t remote,
                    const void *local, size_t size) {
  return ps_pwrite(handle, remote, local, size);
}

ps_err_e ps_ptwrite(struct ps_prochandle *handle, psaddr_t remote,
                    const void *local, size_t size) {
  return ps_pwrite(handle, remote, local, size);
}
...
ps_err_e ps_lgetregs(struct ps_prochandle *handle, lwpid_t lwp,
                     prgregset_t regs) {
  thread_db_process *proc = handle->thread_db_proc;
  int_threadPool *tp = proc->threadPool();
  assert(tp);
  ...
  bool bresult =
      thr->plat_convertToSystemRegs(pool, (unsigned char *)regs, true);
  if (!bresult) {
...
ps_err_e ps_get_thread_area(const struct ps_prochandle *phandle, lwpid_t lwp,
                            int val, psaddr_t *addr) {
  thread_db_process *tdb_proc = phandle->thread_db_proc;
  thread_db_thread *tdb_thread = dynamic_cast<thread_db_thread *>(
      tdb_proc->threadPool()->findThreadByLWP(lwp));

  Dyninst::Address daddr = 0;
  bool result = tdb_thread->thrdb_getThreadArea(val, daddr);
  if (addr && result) *addr = (psaddr_t)daddr;
...
thread_db_process::td_thr_event_enable_t
    thread_db_process::p_td_thr_event_enable;
thread_db_process::td_thr_set_event_t thread_db_process::p_td_thr_set_event;
thread_db_process::td_thr_event_getmsg_t
    thread_db_process::p_td_thr_event_getmsg;
...
thread_db_process::td_thr_tls_get_addr_t
    thread_db_process::p_td_thr_tls_get_addr;
...
bool thread_db_process::loadedThreadDBLibrary() {
  if (tdb_loaded) return tdb_loaded_result;
  tdb_loaded = true;
...
Event::ptr thread_db_process::decodeThreadEvent(td_event_msg_t *eventMsg,
  ...
  switch (eventMsg->event) {
    ...
    case TD_DEATH: {
      ...
      EventUserThreadDestroy::ptr new_ev = EventUserThreadDestroy::ptr(
          new EventUserThreadDestroy(EventType::Pre));
      new_ev->setProcess(proc());
...
thread_db_process::thread_db_process(Dyninst::PID p, std::string e,
  ...
  if (!loadedThreadDBLibrary()) return;
...
thread_db_process::~thread_db_process() {
  ...
  for (brkptIter = addr2Event.begin(); brkptIter != addr2Event.end();
       ++brkptIter) {
...
async_ret_t thread_db_process::initThreadWithHandle(td_thrhandle_t *thr,
                                                    td_thrinfo_t *info,
                                                    LWP lwp) {
  ...
  if (!info) {
    ...
    if (result == aret_error) {
      pthrd_printf(
          "Error calling ll_fetchThreadInfo from initThreadWithHandle\n");
      return aret_error;
    }
    if (result == aret_async) {
  ...
  if (!tdb_thread) {
    perr_printf(
        "Error.  Thread_db reports thread %d/%d, but couldn't find existing "
        "LWP\n",
        getPid(), lwp);
    return aret_error;
  }
  ...
  if (info->ti_tid) tdb_thread->tinfo_initialized = true;
...
async_ret_t thread_db_process::handleThreadAttach(td_thrhandle_t *thr,
                                                  Dyninst::LWP lwp) {
...
async_ret_t thread_db_process::initThreadDB() {
  ...
  if (!createdThreadAgent) {
    ...
    switch (errVal) {
      ...
      case TD_NOLIBTHREAD:
        pthrd_printf(
            "Debuggee isn't multithreaded at this point, libthread_db not "
            "enabled\n");
        return aret_success;
      case TD_ERR:
...
#error td_event_fillset is not a macro on this platform
  ...
  td_event_e allEvents[] = {
      TD_CATCHSIG, TD_CONCURRENCY, TD_CREATE,      TD_DEATH,  TD_IDLE,
      TD_LOCK_TRY, TD_PREEMPT,     TD_PRI_INHERIT, TD_READY,  TD_REAP,
      TD_SLEEP,    TD_SWITCHFROM,  TD_SWITCHTO,    TD_TIMEOUT};
  ...
  for (unsigned i = 0; i < (sizeof(allEvents) / sizeof(td_event_e)); ++i) {
    ...
    switch (allEvents[i]) {
      ...
      default:
        pthrd_printf(
            "Unimplemented libthread_db event encountered. Skipping for "
            "now.\n");
        continue;
    }
    ...
    pthrd_printf(
        "Received address of 0x%lx for breakpoint, checking platform "
        "conversion\n",
        addr);
    ...
    pair<map<Dyninst::Address, pair<int_breakpoint *, EventType> >::iterator,
         bool>
        insertIter;
    insertIter =
...
const char *thread_db_process::getThreadLibName(const char *) { return ""; }
...
bool thread_db_process::decodeTdbLWPExit(EventLWPDestroy::ptr lwp_ev) {
  ...
  if (db_thread->destroyed || !db_thread->thread_initialized) return false;

  pthrd_printf(
      "Decoded LWP exit without thread exit on %d/%d.  Faking thread exit "
      "event\n",
...
async_ret_t thread_db_process::decodeTdbBreakpoint(EventBreakpoint::ptr bp) {
  ...
  if (addrIter == addr2Event.end()) return aret_error;
  ...
  switch (ecode) {
    ...
    default:
      pthrd_printf(
          "Failed to decode any thread events due to the breakpoint\n");
      return aret_error;
  }

  for (vector<Event::ptr>::iterator i = threadEvents.begin();
       i != threadEvents.end(); i++) {
    Event::ptr ev = *i;
    if (!ev->getThread()) ev->setThread(bp->getThread());
    if (!ev->getProcess()) ev->setProcess(proc());
    if (ev->getSyncType() == Event::unset) ev->setSyncType(Event::sync_process);
...
td_thragent_t *thread_db_process::getThreadDBAgent() { return threadAgent; }

static string stripLibraryName(const char *libname) {
  const char *filename_c = strrchr(libname, '/');
  if (!filename_c) filename_c = strrchr(libname, '\\');
  if (!filename_c)
    filename_c = libname;
  else
    filename_c++;
  ...
  if (dot_ext) lesser_ext = dot_ext;
  const char *dash_ext = strchr(filename_c, '-');
  if (dash_ext && (!lesser_ext || dash_ext < lesser_ext)) lesser_ext = dash_ext;
...
ps_err_e thread_db_process::getSymbolAddr(const char *objName,
                                          const char *symName,
                                          psaddr_t *symbolAddr) {
...
async_ret_t thread_db_process::post_create(
  ...
  return aret_success;  // Swallow these errors, thread_db failure does not
                        // bring down rest of startup
}

async_ret_t thread_db_process::post_attach(bool wasDetached,
...
int_thread *thread_db_process::triggerThread() const { return trigger_thread; }

async_ret_t thread_db_process::ll_fetchThreadInfo(td_thrhandle_t *th,
                                                  td_thrinfo_t *info) {
  ...
  pthrd_printf(
      "Successful ll_fetchThreadInfo for handle %p - tid = %lu, lid = %lu\n",
...
Handler::handler_ret_t ThreadDBDispatchHandler::handleEvent(Event::ptr ev) {
  ...
  if (main_thread->tinfo_initialized) proc->initialThreadEventCreated = true;
  ...
  if (!proc->initialThreadEventCreated) {
    ...
    int td_result = thread_db_process::p_td_ta_map_lwp2thr(
        proc->getThreadDBAgent(), main_thread->getLWP(),
        main_thread->threadHandle);
    if (td_result == TD_ERR && proc->getMemCache()->hasPendingAsync()) {
      pthrd_printf(
          "async return from td_ta_map_lwp2thr while creating event for main "
          "thread\n");
      ...
      perr_printf(
          "Error return from td_ta_map_lwp2thr while creating event for main "
          "thread\n");
      proc->dispatch_event = EventThreadDB::ptr();
      return ret_error;
    }
    ...
    async_ret_t result =
        proc->ll_fetchThreadInfo(main_thread->threadHandle, &tinfo);
    if (result == aret_async) {
  ...
  for (set<Event::ptr>::iterator i = int_ev->new_evs.begin();
       i != int_ev->new_evs.end(); i++) {
...
void ThreadDBDispatchHandler::getEventTypesHandled(
...
Handler::handler_ret_t ThreadDBLibHandler::handleEvent(Event::ptr ev) {
  ...
  for (set<Library::ptr>::const_iterator i = rmLibs.begin(); i != rmLibs.end();
       i++) {
    int_library *ll_lib = (*i)->debug();
    if (cached_libs.find(ll_lib) == cached_libs.end()) continue;
    ...
    for (int_threadPool::iterator j = proc->threadPool()->begin();
      ...
      if (!thrd) continue;
      map<int_library *, Address>::iterator k =
          thrd->cached_tls_areas.find(ll_lib);
      if (k == thrd->cached_tls_areas.end()) continue;
  ...
  for (libIter = addLibs.begin(); libIter != addLibs.end(); ++libIter) {
    if (!proc->isSupportedThreadLib((*libIter)->getName())) continue;
...
int ThreadDBLibHandler::getPriority() const { return PostPlatformPriority; }
...
Handler::handler_ret_t ThreadDBCreateHandler::handleEvent(Event::ptr ev) {
  ...
  if (threadEv->getInternalEvent()->needs_update) {
    pthrd_printf(
        "Updating user thread data for %d/%d in thread_db create handler\n",
        tdb_proc->getPid(), tdb_thread->getLWP());
    assert(tdb_proc);
    new_thread_data_t *thrdata =
        (new_thread_data_t *)threadEv->getInternalEvent()->raw_data;

    async_ret_t result = tdb_proc->initThreadWithHandle(
        thrdata->thr_handle, &thrdata->thr_info, NULL_LWP);
    if (result == aret_error) {
...
Handler::handler_ret_t ThreadDBDestroyHandler::handleEvent(Event::ptr ev) {
  ...
  thread_db_process *proc =
      dynamic_cast<thread_db_process *>(ev->getProcess()->llproc());
  thread_db_thread *thrd =
      dynamic_cast<thread_db_thread *>(ev->getThread()->llthrd());
...
thread_db_thread::thread_db_thread(int_process *p, Dyninst::THR_ID t,
                                   Dyninst::LWP l)
    : int_thread(p, t, l),
      threadHandle(NULL),
      destroyed(false),
...
thread_db_thread::~thread_db_thread() {
  if (threadHandle_alloced) delete threadHandle;
}
...
bool thread_db_thread::initThreadHandle() {
  ...
  td_err_e errVal = thread_db_process::p_td_ta_map_lwp2thr(
      lproc->getThreadDBAgent(), lwp, threadHandle);
  if (TD_OK != errVal) {
    perr_printf("Failed to map LWP %d to thread_db thread: %s(%d)\n", lwp,
                tdErr2Str(errVal), errVal);
    setLastError(err_internal, "Failed to get thread_db thread handle");
...
async_ret_t thread_db_process::getEventForThread(int_eventThreadDB *iev) {
  ...
  for (int i = iev->msgs.size() - 1; i >= 0; i--) {
    ...
    if (newEvent) iev->new_evs.insert(newEvent);
    iev->msgs.pop_back();
...
bool thread_db_process::setTrackThreads(
    bool b, std::set<std::pair<int_breakpoint *, Address> > &bps,
    bool &add_bp) {
  if (b == track_threads) {
    pthrd_printf(
        "User wants to %s thread_db on %d, which is already done.  Leaving in "
        "same state\n",
...
bool thread_db_process::isTrackingThreads() { return track_threads; }
...
int thread_db_process::threaddb_getPid() { return getPid(); }

async_ret_t thread_db_process::plat_calcTLSAddress(int_thread *thread,
                                                   int_library *lib, Offset off,
                                                   Address &outaddr,
  ...
  if (!thrd || !thrd->initThreadHandle()) {
    ...
    setLastError(err_unsupported,
                 "TLS Operations not supported on this thread\n");
    return aret_error;
  }
  ...
  if ((!is_staticbinary && !p_td_thr_tls_get_addr) ||
    ...
    setLastError(err_unsupported,
                 "TLS Operations not supported on this system\n");
    return aret_error;
  }
...
bool thread_db_thread::fetchThreadInfo() {
  ...
  if (!thread_initialized) {
    ...
    setLastError(err_nouserthrd,
                 "Attempted to read user thread info, but user thread has not "
                 "been created.");
    return false;
  }
...
void thread_db_thread::markDestroyed() { destroyed = true; }

bool thread_db_thread::isDestroyed() { return destroyed; }
...
// Empty place holder functions in-case we're built on a machine without
// libthread_db.so
...
bool thread_db_thread::haveUserThreadInfo() { return false; }

bool thread_db_thread::getTID(Dyninst::THR_ID &) {
  perr_printf("Error. thread_db not installed on this platform.\n");
  setLastError(err_unsupported,
               "Cannot perform thread operations without thread_db\n");
  return false;
}
...
bool thread_db_process::plat_getLWPInfo(lwpid_t, void *) { return false; }
...
bool thread_db_process::plat_supportThreadEvents() { return false; }
