...
void
ps_plog(const char* format, ...) DYNINST_PRINTF_ANNOTATION(1, 2);
...
ps_err_e
ps_pglobal_lookup(struct ps_prochandle* handle, const char* objName, const char* symName,
                  psaddr_t* symbolAddr)
{
...
ps_err_e
ps_pread(struct ps_prochandle* handle, psaddr_t remote, void* local, size_t size)
{
    thread_db_process* llproc = handle->thread_db_proc;
    pthrd_printf("thread_db reading from %#lx to %#lx, size = %d on %d\n",
                 (unsigned long) remote, (unsigned long) local, (int) size,
                 llproc->getPid());

    llproc->resps.clear();
    async_ret_t result = llproc->getMemCache()->readMemory(
        local, (Address) remote, size, llproc->resps, llproc->triggerThread());
...
ps_err_e
ps_pdread(struct ps_prochandle* handle, psaddr_t remote, void* local, size_t size)
{
    return ps_pread(handle, remote, local, size);
}

ps_err_e
ps_ptread(struct ps_prochandle* handle, psaddr_t remote, void* local, size_t size)
{
    return ps_pread(handle, remote, local, size);
}

ps_err_e
ps_pwrite(struct ps_prochandle* handle, psaddr_t remote, const void* local, size_t size)
{
    pthrd_printf("thread_db writing to %#lx from %#lx, size = %d on %d\n",
                 (unsigned long) remote, (unsigned long) local, (int) size,
                 handle->thread_db_proc->getPid());

    thread_db_process* proc = handle->thread_db_proc;

    async_ret_t result =
        proc->getMemCache()->writeMemory((Address) remote, const_cast<void*>(local), size,
                                         proc->res_resps, proc->triggerThread());
    switch(result)
    {
        case aret_success:
...
ps_err_e
ps_pdwrite(struct ps_prochandle* handle, psaddr_t remote, const void* local, size_t size)
{
    return ps_pwrite(handle, remote, local, size);
}

ps_err_e
ps_ptwrite(struct ps_prochandle* handle, psaddr_t remote, const void* local, size_t size)
{
    return ps_pwrite(handle, remote, local, size);
}

ps_err_e
ps_linfo(struct ps_prochandle* handle, lwpid_t lwp, void* lwpInfo)
{
...
ps_err_e
ps_lstop(struct ps_prochandle* handle, lwpid_t lwp)
{
...
ps_err_e
ps_lcontinue(struct ps_prochandle* handle, lwpid_t lwp)
{
    int_process*    proc = handle->thread_db_proc;
    int_threadPool* tp   = proc->threadPool();
    assert(tp);
...
ps_err_e
ps_lgetregs(struct ps_prochandle* handle, lwpid_t lwp, prgregset_t regs)
{
    thread_db_process* proc = handle->thread_db_proc;
    int_threadPool*    tp   = proc->threadPool();
    assert(tp);
...
ps_err_e
ps_pdmodel(struct ps_prochandle*, int*)
{
    assert(!NA_IMPLEMENTED);
    return PS_ERR;
}
...
ps_err_e
ps_get_thread_area(const struct ps_prochandle* phandle, lwpid_t lwp, int val,
                   psaddr_t* addr)
{
    thread_db_process* tdb_proc = phandle->thread_db_proc;
    thread_db_thread*  tdb_thread =
        dynamic_cast<thread_db_thread*>(tdb_proc->threadPool()->findThreadByLWP(lwp));
...
#        define TDB_BIND(SYM) p_##SYM = SYM
#    else
#        define TDB_BIND(SYM)                                                            \
            do                                                                           \
            {                                                                            \
...
bool
thread_db_process::loadedThreadDBLibrary()
{
    if(tdb_loaded)
...
Event::ptr
thread_db_process::decodeThreadEvent(td_event_msg_t* eventMsg, bool& async)
{
    td_thrinfo_t info;
    async = false;
...
#    endif
    ...
    int_thread*  thr = threadPool()->findThreadByLWP(
        lwp);  // thr may be NULL if OS doesn't support LWP events (BG/P)
    switch(eventMsg->event)
    {
        case TD_CREATE: {
            pthrd_printf("Decoded to user thread create of %d/%d\n", getPid(), lwp);

            EventNewUserThread::ptr new_ev =
                EventNewUserThread::ptr(new EventNewUserThread());
            ...
            new_thread_data_t* thrdata =
                (new_thread_data_t*) malloc(sizeof(new_thread_data_t));
        ...
        case TD_DEATH: {
            ...
            EventUserThreadDestroy::ptr new_ev =
                EventUserThreadDestroy::ptr(new EventUserThreadDestroy(EventType::Pre));
            new_ev->setProcess(proc());
        ...
        default: {
            pthrd_printf(
                "Unimplemented libthread_db event encountered. Skipping for now.\n");
            break;
        }
    }
...
thread_db_process::thread_db_process(Dyninst::PID p, std::string e,
                                     std::vector<std::string> envp,
                                     std::vector<std::string> a, std::map<int, int> f)
...
async_ret_t
thread_db_process::initThreadWithHandle(td_thrhandle_t* thr, td_thrinfo_t* info, LWP lwp)
{
    ...
    thread_db_thread* tdb_thread =
        dynamic_cast<thread_db_thread*>(threadPool()->findThreadByLWP(lwp));
    if(!tdb_thread)
    {
        perr_printf(
            "Error.  Thread_db reports thread %d/%d, but couldn't find existing LWP\n",
            getPid(), lwp);
        return aret_error;
    }
...
async_ret_t
thread_db_process::handleThreadAttach(td_thrhandle_t* thr, Dyninst::LWP lwp)
{
    return initThreadWithHandle(thr, NULL, lwp);
}

async_ret_t
thread_db_process::initThreadDB()
{
    ...
    if(!createdThreadAgent)
    {
        ...
        switch(errVal)
        {
            case TD_OK:
                ...
                break;
            case TD_NOLIBTHREAD:
                pthrd_printf("Debuggee isn't multithreaded at this point, libthread_db "
                             "not enabled\n");
                return aret_success;
            case TD_ERR:
    ...
    for(int_threadPool::iterator i = threadPool()->begin(); i != threadPool()->end(); i++)
    {
        ...
        {
            all_handles.insert(pair<td_thrhandle_t*, LWP>(tdb_thread->threadHandle,
                                                          tdb_thread->getLWP()));
            continue;
        }
        ...
        errVal = p_td_ta_map_lwp2thr(getThreadDBAgent(), tdb_thread->getLWP(),
                                     tdb_thread->threadHandle);
        if(errVal != TD_OK)
        {
            if(getMemCache()->hasPendingAsync())
            {
        ...
        all_handles.insert(
            pair<td_thrhandle_t*, LWP>(tdb_thread->threadHandle, tdb_thread->getLWP()));
    }
    if(hasAsync)
    {
    ...
    for(set<pair<td_thrhandle_t*, LWP>>::iterator i = all_handles.begin();
        i != all_handles.end(); i++)
    {
...
#    endif
    ...
    td_event_e allEvents[] = { TD_CATCHSIG, TD_CONCURRENCY, TD_CREATE,  TD_DEATH,
                               TD_IDLE,     TD_LOCK_TRY,    TD_PREEMPT, TD_PRI_INHERIT,
                               TD_READY,    TD_REAP,        TD_SLEEP,   TD_SWITCHFROM,
                               TD_SWITCHTO, TD_TIMEOUT };
    ...
    for(unsigned i = 0; i < (sizeof(allEvents) / sizeof(td_event_e)); ++i)
    {
        ...
        pthrd_printf(
            "Received address of 0x%lx for breakpoint, checking platform conversion\n",
            addr);
        if(!plat_convertToBreakpointAddress(addr, triggerThread()))
        {
            ...
            setLastError(err_internal,
                         "Failed to install new thread_db event breakpoint");
            thread_db_proc_initialized = true;
            return aret_error;
        }
...
#    endif
        ...
        pair<map<Dyninst::Address, pair<int_breakpoint*, EventType>>::iterator, bool>
            insertIter;
        insertIter =
            addr2Event.insert(make_pair(addr, make_pair(newEventBrkpt, newEvent)));
...
void
thread_db_process::freeThreadDBAgent()
{
...
const char*
thread_db_process::getThreadLibName(const char*)
{
    return "";
}

bool
thread_db_process::decodeTdbLWPExit(EventLWPDestroy::ptr lwp_ev)
{
    thread_db_thread* db_thread =
        dynamic_cast<thread_db_thread*>(lwp_ev->getThread()->llthrd());
    assert(db_thread);
    ...
    pthrd_printf(
        "Decoded LWP exit without thread exit on %d/%d.  Faking thread exit event\n",
        db_thread->llproc()->getPid(), db_thread->getLWP());

    EventUserThreadDestroy::ptr new_ev =
        EventUserThreadDestroy::ptr(new EventUserThreadDestroy(EventType::Post));
...
async_ret_t
thread_db_process::decodeTdbBreakpoint(EventBreakpoint::ptr bp)
{
    ...
    pthrd_printf("Address 0x%lx corresponds to a thread %s breakpoint.\n", addr,
                 ecode == EventType::ThreadCreate ? "create" : "destroy");
    switch(ecode)
    {
    ...
    for(vector<Event::ptr>::iterator i = threadEvents.begin(); i != threadEvents.end();
        i++)
    {
        Event::ptr ev = *i;
        if(!ev->getThread())
...
td_thragent_t*
thread_db_process::getThreadDBAgent()
{
    return threadAgent;
}

static string
stripLibraryName(const char* libname)
{
...
ps_err_e
thread_db_process::getSymbolAddr(const char* objName, const char* symName,
                                 psaddr_t* symbolAddr)
{
    ...
    else
    {
        ...
        for(set<int_library*>::iterator i = memory()->libs.begin();
            i != memory()->libs.end(); i++)
        {
            int_library* l = *i;
...
async_ret_t
thread_db_process::post_create(std::set<response::ptr>& async_responses)
{
    async_ret_t result;
    if(!completed_post)
    {
    ...
    return aret_success;  // Swallow these errors, thread_db failure does not bring down
                          // rest of startup
}

async_ret_t
thread_db_process::post_attach(bool wasDetached, set<response::ptr>& aresps)
{
    async_ret_t result;
    if(!completed_post)
    {
...
bool
thread_db_process::isSupportedThreadLib(string libName)
{
    return (libName.find("libpthread") != string::npos);
}

void
thread_db_process::addThreadDBHandlers(HandlerPool* hpool)
{
...
bool
thread_db_process::plat_getLWPInfo(lwpid_t, void*)
{
...
bool
thread_db_process::plat_supportThreadEvents()
{
    if(!loadedThreadDBLibrary())
    {
        return false;
...
bool
thread_db_thread::plat_convertToSystemRegs(const int_registerPool&, unsigned char*, bool)
{
    return true;
}

int_thread*
thread_db_process::triggerThread() const
{
    return trigger_thread;
}

async_ret_t
thread_db_process::ll_fetchThreadInfo(td_thrhandle_t* th, td_thrinfo_t* info)
{
...
int
ThreadDBDispatchHandler::getPriority() const
{
    return Handler::PostPlatformPriority;
}

Handler::handler_ret_t
ThreadDBDispatchHandler::handleEvent(Event::ptr ev)
{
    /**
    ...
    thread_db_process* proc =
        dynamic_cast<thread_db_process*>(etdb->getProcess()->llproc());
    assert(proc);
    ...
    thread_db_thread* main_thread =
        dynamic_cast<thread_db_thread*>(proc->threadPool()->initialThread());
    ...
    if(!proc->initialThreadEventCreated)
    {
        ...
        int td_result = thread_db_process::p_td_ta_map_lwp2thr(
            proc->getThreadDBAgent(), main_thread->getLWP(), main_thread->threadHandle);
        if(td_result == TD_ERR && proc->getMemCache()->hasPendingAsync())
        {
            pthrd_printf("async return from td_ta_map_lwp2thr while creating event for "
                         "main thread\n");
        ...
        {
            perr_printf("Error return from td_ta_map_lwp2thr while creating event for "
                        "main thread\n");
            proc->dispatch_event = EventThreadDB::ptr();
            return ret_error;
        }
    ...
    for(set<Event::ptr>::iterator i = int_ev->new_evs.begin(); i != int_ev->new_evs.end();
        i++)
    {
...
void
ThreadDBDispatchHandler::getEventTypesHandled(std::vector<EventType>& etypes)
{
...
Handler::handler_ret_t
ThreadDBLibHandler::handleEvent(Event::ptr ev)
{
    ...
    thread_db_process*      proc =
        dynamic_cast<thread_db_process*>(ev->getProcess()->llproc());
...
int
ThreadDBLibHandler::getPriority() const
{
    return PostPlatformPriority;
}

void
ThreadDBLibHandler::getEventTypesHandled(vector<EventType>& etypes)
{
...
int
ThreadDBCreateHandler::getPriority() const
{
...
Handler::handler_ret_t
ThreadDBCreateHandler::handleEvent(Event::ptr ev)
{
    ...
    thread_db_process*      tdb_proc =
        dynamic_cast<thread_db_process*>(threadEv->getProcess()->llproc());
    thread_db_thread* tdb_thread =
        dynamic_cast<thread_db_thread*>(threadEv->getNewThread()->llthrd());
    ...
    if(threadEv->getInternalEvent()->needs_update)
    {
        ...
        assert(tdb_proc);
        new_thread_data_t* thrdata =
            (new_thread_data_t*) threadEv->getInternalEvent()->raw_data;

        async_ret_t result = tdb_proc->initThreadWithHandle(thrdata->thr_handle,
                                                            &thrdata->thr_info, NULL_LWP);
        if(result == aret_error)
        {
        ...
        if(thrdata->threadHandle_alloced)
            tdb_thread->threadHandle_alloced = true;
    }
...
void
ThreadDBCreateHandler::getEventTypesHandled(vector<EventType>& etypes)
{
...
int
ThreadDBDestroyHandler::getPriority() const
{
    // After dispatch handler, which runs at PostPlatformPriority
    return Handler::PostPlatformPriority + 1;
}
...
Handler::handler_ret_t
ThreadDBDestroyHandler::handleEvent(Event::ptr ev)
{
    if(!thread_db_process::loadedThreadDBLibrary())
    {
        pthrd_printf("Failed to load thread_db.  Not running handlers\n");
...
void
ThreadDBDestroyHandler::getEventTypesHandled(vector<EventType>& etypes)
{
...
async_ret_t
thread_db_process::getEventForThread(int_eventThreadDB* iev)
{
    ...
    if(!iev->completed_getmsgs)
    {
        ...
        for(;;)
        {
            ...
            if(msgErr != TD_OK)
            {
                ...
                {
                    pthrd_printf(
                        "Async return in getEventForThread from td_ta_event_getmsg\n");
                    return aret_async;
                }
...
bool
thread_db_process::setTrackThreads(bool                                           b,
                                   std::set<std::pair<int_breakpoint*, Address>>& bps,
                                   bool&                                          add_bp)
{
    if(b == track_threads)
    {
        pthrd_printf("User wants to %s thread_db on %d, which is already done.  Leaving "
                     "in same state\n",
...
bool
thread_db_process::isTrackingThreads()
{
    return track_threads;
}

bool
thread_db_process::refreshThreads()
{
...
int
thread_db_process::threaddb_getPid()
{
    return getPid();
}

async_ret_t
thread_db_process::plat_calcTLSAddress(int_thread* thread, int_library* lib, Offset off,
                                       Address& outaddr, set<response::ptr>& resps_)
{
    ...
    if(!is_staticbinary)
        err = p_td_thr_tls_get_addr(thrd->threadHandle, (void*) lib->mapAddress(), 0,
                                    &tls_base);
    else
...
async_ret_t
thread_db_thread::setEventReporting(bool on)
{
    if(!initThreadHandle())
        return aret_error;
    if(enabled_event_reporting == on)
        return aret_success;

    pthrd_printf("Enabled thread_db events for LWP %d\n", lwp);
    td_err_e errVal =
        thread_db_process::p_td_thr_event_enable(threadHandle, (on ? 1 : 0));
...
bool
thread_db_thread::fetchThreadInfo()
{
    if(!thread_db_process::loadedThreadDBLibrary())
    {
        perr_printf("Failed to load thread_db.  Not fetching thread data.");
        setLastError(err_unsupported,
                     "thread_db.so not loaded.  User-level thread data unavailable.");
        return false;
    }
    if(!thread_initialized)
    {
        perr_printf(
            "Attempt to read user thread info of %d/%d before user thread create\n",
            llproc()->getPid(), getLWP());
        setLastError(
            err_nouserthrd,
            "Attempted to read user thread info, but user thread has not been created.");
        return false;
    }
    ...
    while(result == aret_async)
    {
        ...
        {
            pthrd_printf(
                "Returning error in fetchThreadInfo due to ll_fetchThreadInfo\n");
            return false;
        }
    }
...
void
thread_db_thread::markDestroyed()
{
    destroyed = true;
}

bool
thread_db_thread::isDestroyed()
{
    return destroyed;
}

bool
thread_db_thread::thrdb_getThreadArea(int, Dyninst::Address&)
{
...
bool
thread_db_thread::haveUserThreadInfo()
{
    pthrd_printf("haveUserThreadInfo (%d/%d): %d\n", (llproc() ? llproc()->getPid() : 0),
                 lwp, thread_initialized);
    return thread_initialized;
}

bool
thread_db_thread::getTID(Dyninst::THR_ID& tid_)
{
    if(!fetchThreadInfo())
    {
        return false;
...
bool
thread_db_thread::getStartFuncAddress(Dyninst::Address& addr)
{
    if(!fetchThreadInfo())
    {
        return false;
...
bool
thread_db_thread::getStackBase(Dyninst::Address& addr)
{
    if(!fetchThreadInfo())
    {
        return false;
...
bool
thread_db_thread::getStackSize(unsigned long& size)
{
    if(!fetchThreadInfo())
    {
        return false;
...
bool
thread_db_thread::getTLSPtr(Dyninst::Address& addr)
{
    if(!fetchThreadInfo())
    {
        return false;
...
bool thread_db_process::decodeTdbLWPExit(EventLWPDestroy::ptr) { return false; }
...
void
thread_db_process::addThreadDBHandlers(HandlerPool*)
{}
...
bool
thread_db_thread::getTID(Dyninst::THR_ID&)
{
...
bool
thread_db_thread::getStartFuncAddress(Dyninst::Address&)
{
    perr_printf("Error. thread_db not installed on this platform.\n");
    setLastError(err_unsupported, "Cannot perform thread operations without thread_db\n");
    return false;
...
bool
thread_db_thread::getStackBase(Dyninst::Address&)
{
    perr_printf("Error. thread_db not installed on this platform.\n");
    setLastError(err_unsupported, "Cannot perform thread operations without thread_db\n");
    return false;
...
bool
thread_db_thread::getStackSize(unsigned long&)
{
    perr_printf("Error. thread_db not installed on this platform.\n");
    setLastError(err_unsupported, "Cannot perform thread operations without thread_db\n");
    return false;
...
bool
thread_db_thread::getTLSPtr(Dyninst::Address&)
{
    perr_printf("Error. thread_db not installed on this platform.\n");
    setLastError(err_unsupported, "Cannot perform thread operations without thread_db\n");
    return false;
...
async_ret_t
thread_db_process::post_attach(bool b, set<response::ptr>& s)
{
...
async_ret_t
thread_db_process::getEventForThread(int_eventThreadDB*)
{
    return aret_error;
}

bool thread_db_process::isSupportedThreadLib(string) { return false; }
...
bool
thread_db_process::setTrackThreads(bool, std::set<std::pair<int_breakpoint*, Address>>&,
                                   bool&)
{
    perr_printf("Error. thread_db not installed on this platform.\n");
    setLastError(err_unsupported, "Cannot perform thread operations without thread_db\n");
...
ThreadTracking*
thread_db_process::threaddb_getThreadTracking()
{
    perr_printf("Error. thread_db not installed on this platform.\n");
    setLastError(err_unsupported, "Cannot perform thread operations without thread_db\n");
    return NULL;
