...
bool int_process::create(int_processSet *ps) {
  ...
  transform(ps->begin(), ps->end(), inserter(procs, procs.end()),
            ProcToIntProc());
...
bool int_process::attachThreads() {
  if (!needIndividualThreadAttach()) return true;
  ...
  do {
    ...
    for (vector<Dyninst::LWP>::iterator i = lwps.begin(); i != lwps.end();
         ++i) {
...
bool int_process::attach(int_processSet *ps, bool reattach) {
  ...
  for (set<int_process *>::iterator i = procs.begin(); i != procs.end();) {
    ...
    if (reattach && proc->getState() != int_process::detached) {
      perr_printf("Attempted to reattach to attached process %d\n",
                  proc->getPid());
      proc->setLastError(err_attached,
                         "Cannot reAttach to attached process.\n");
      procs.erase(i++);
      had_error = true;
      continue;
    }
    ...
    // Keep track of the initial running states for each thread.  We'll fill
    // them in latter
    // after we create the int_thread objects.
    bool allStopped = true;
    for (map<Dyninst::LWP, bool>::iterator j = temp_runningStates.begin();
         j != temp_runningStates.end(); ++j) {
      if (j->second) {
        allStopped = false;
      }
    ...
    if (local_should_sync) should_sync = true;
    i++;
  }
  ...
  if (!reattach) {
    for (set<int_process *>::iterator i = procs.begin(); i != procs.end();
      ...
      int_process *proc = *i;
      ProcPool()->addProcess(proc);
      int_thread::createThread(
          proc, NULL_THR_ID, NULL_LWP, true,
    ...
    if (reattach) {
      ...
      for (int_threadPool::iterator j = tp->begin(); j != tp->end(); j++) {
        ...
        if (thr->getDetachState().getState() != int_thread::detached) {
          ...
          // those for the entire process.  Temporarily set this thread to
          // detached
        ...
        map<pair<int_process *, LWP>, bool>::iterator findIter =
            runningStates.find(key);
        ...
        if (findIter == runningStates.end() || !thr->attach()) {
          ...
          if (proc->plat_supportLWPPostDestroy()) {
            ...
            perr_printf(
                "Platform does not support any thread destroy events.  Now "
                "what?\n");
            assert(0);
          }
  ...
  // Some OSs need to do their attachThreads here.  Since the operation is
  // supposed to be
  // idempotent after success, then just do it again.
  for (set<int_process *>::iterator i = procs.begin(); i != procs.end();) {
    int_process *proc = *i;
    if (proc->getState() == errorstate) continue;
    bool result = proc->attachThreads();
    if (!result) {
    ...
    for (int_threadPool::iterator j = tp->begin(); j != tp->end(); j++) {
      ...
      thr->setRunningWhenAttached(
          findIter == runningStates.end() ? true : findIter->second);
    }
  ...
  while (!pa_procs.empty()) {
    ...
    bool ret_async = false;
    for (set<int_process *>::iterator i = pa_procs.begin();
         i != pa_procs.end();) {
      int_process *proc = *i;
  ...
  if (!reattach) return !had_error;

  async_responses.clear();
  for (set<int_process *>::iterator i = procs.begin(); i != procs.end(); i++) {
    // Resume all breakpoints
    int_process *proc = *i;
    for (std::map<Dyninst::Address, sw_breakpoint *>::iterator j =
             proc->mem->breakpoints.begin();
  ...
  for (vector<Event::ptr>::iterator i = observedEvents.begin();
    ...
    pthrd_printf("Queuing event %s for thread %d/%d\n",
                 (*i)->getEventType().name().c_str(),
                 (*i)->getProcess()->getPid(), (*i)->getThread()->getLWP());
...
bool int_process::reattach(int_processSet *pset) { return attach(pset, true); }
...
bool int_process::execed() {
  ...
  if (should_clean) delete mem;
  mem = new mem_state(this);

  arch = Dyninst::Arch_none;
  exec_mem_cache.clear();

  int_thread::State user_initial_thrd_state =
      threadpool->initialThread()->getUserState().getState();
  int_thread::State gen_initial_thrd_state =
      threadpool->initialThread()->getGeneratorState().getState();
  int_thread::State handler_initial_thrd_state =
      threadpool->initialThread()->getHandlerState().getState();
  ...
  int_thread *initial_thread = int_thread::createThread(
      this, NULL_THR_ID, NULL_LWP, true, int_thread::as_created_attached);
...
bool int_process::plat_execed() { return true; }
...
async_ret_t int_process::initializeAddressSpace(
  ...
  if (!result && have_asyncs) {
    pthrd_printf(
        "Postponing initializeAddressSpace of %d for async operations\n",
        getPid());
    return aret_async;
  }
  if (!result) {
...
async_ret_t int_process::post_attach(bool,
                                     std::set<response::ptr> &async_responses) {
...
bool int_process::plat_processGroupContinues() { return false; }

bool int_process::getThreadLWPs(std::vector<Dyninst::LWP> &) { return false; }
...
void int_process::setState(int_process::State s) {
  ...
  switch (s) {
    case neonatal:
      new_thr_state = int_thread::neonatal;
      break;
    case neonatal_intermediate:
      new_thr_state = int_thread::neonatal_intermediate;
      break;
    case running:
      new_thr_state = threadpool->initialThread()->getHandlerState().getState();
      break;
    case exited:
      new_thr_state = int_thread::exited;
      break;
    case detached:
      new_thr_state = int_thread::detached;
      break;
    case errorstate:
      new_thr_state = int_thread::errorstate;
      break;
  }
...
int_process::State int_process::getState() const { return state; }

int_process::creationMode_t int_process::getCreationMode() const {
  return creation_mode;
}

void int_process::setContSignal(int sig) { continueSig = sig; }

int int_process::getContSignal() const { return continueSig; }
...
Dyninst::PID int_process::getPid() const { return pid; }

int_threadPool *int_process::threadPool() const { return threadpool; }

Process::ptr int_process::proc() const { return up_proc; }
...
bool int_process::syncRunState() {
   ...
   * events and throw it.
   * 3), Check if any proc stopper events are ready, and if so then re-throw
   *them
  ...
  for (int_threadPool::iterator i = tp->begin(); i != tp->end(); i++) {
    int_thread *thr = *i;
    if (!RUNNING_STATE(thr->getTargetState())) continue;
  ...
  if (dyninst_debug_proccontrol) {
    ...
    for (int_threadPool::iterator i = tp->begin(); i != tp->end(); i++) {
      ...
      for (int i = 0; i < int_thread::NumStateIDs; i++) {
        state_code[i] =
            int_thread::stateLetter(thr->getStateByID(i).getState());
      }
      ...
      pthrd_printf("%d/%d hand: %s, gen: %s, %s: %s, code: %s\n", getPid(),
                   ...
                   int_thread::stateStr(target.getState()), state_code);
    }
    if (tp->empty()) {
...
bool int_process::waitForAsyncEvent(response::ptr resp) {
  if (resp) {
    ...
    if (proc) proc->plat_preAsyncWait();
  }
...
Counter &int_process::asyncEventCount() { return async_event_count; }
...
bool int_process::plat_waitAndHandleForProc() { return true; }

int_process *int_process::in_waitHandleProc = NULL;

bool int_process::waitAndHandleForProc(bool block, int_process *proc,
                                       bool &proc_exited) {
...
#define checkAsyncPending \
  (hasAsyncPending = (int)HandlerPool::hasProcAsyncPending())
#define checkRunningThread \
  (hasRunningThread = (int)Counter::global(Counter::HandlerRunningThreads))
#define checkClearingBP \
  (hasClearingBP = (int)Counter::global(Counter::ClearingBPs))
#define checkStopPending \
  (hasStopPending = (int)Counter::global(Counter::PendingStops))
#define checkSyncRPCRunningThrd \
  (hasSyncRPCRunningThrd = (int)Counter::global(Counter::SyncRPCRunningThreads))
#define checkProcStopRPC \
  (hasProcStopRPC = (int)Counter::global(Counter::ProcStopRPCs))
#define checkStartupTeardownProcs \
  (hasStartupTeardownProc =       \
       (int)Counter::global(Counter::StartupTeardownProcesses))
#define checkNeonatalThreads \
  (hasNeonatalThreads = (int)Counter::global(Counter::NeonatalThreads))
#define checkAsyncEvents \
  (hasAsyncEvents = (int)Counter::global(Counter::AsyncEvents))
...
bool int_process::waitAndHandleEvents(bool block) {
  ...
  for (;;) {
     ...
     *
     * The weirdness in the #defines and variables here is to record the result
     *of
     * each check, which is used if debug printing is turned on, while also
     *letting the
     * C++ partial evaluator not check parts this expression (as the checks
     *involve an
     * expensive locking operation).
     *
     * For most people, just ignore the #defines.  The important bit is that
     *we're really
     * calling the Counter::global(...) function to determine whether we should
     *block.
     **/
    int hasHandlerThread = UNSET_CHECK, hasAsyncPending = UNSET_CHECK,
        hasRunningThread = UNSET_CHECK;
    int hasClearingBP = UNSET_CHECK, hasStopPending = UNSET_CHECK,
        hasSyncRPCRunningThrd = UNSET_CHECK;
    int hasProcStopRPC = UNSET_CHECK, hasBlock = UNSET_CHECK,
        hasGotEvent = UNSET_CHECK;
    int hasStartupTeardownProc = UNSET_CHECK, hasNeonatalThreads = UNSET_CHECK,
        hasAsyncEvents = UNSET_CHECK;

    bool should_block =
        (!checkHandlerThread &&
         ((checkBlock && !checkGotEvent && checkRunningThread) ||
          (checkSyncRPCRunningThrd) || (checkStopPending) ||
          (checkClearingBP) || (checkProcStopRPC) || (checkAsyncPending) ||
          (checkStartupTeardownProcs) || (checkNeonatalThreads) ||
          (checkAsyncEvents)));
    // Entry for this print match the above tests in order and one-for-one.
    pthrd_printf(
        "%s for events = !%c && ((%c && !%c && %c) || %c || %c || %c || %c || "
        "%c || %c || %c || %c)\n",
        should_block ? "Blocking" : "Polling", printCheck(hasHandlerThread),
        printCheck(hasBlock), printCheck(hasGotEvent),
        printCheck(hasRunningThread), printCheck(hasSyncRPCRunningThrd),
        printCheck(hasStopPending), printCheck(hasClearingBP),
        printCheck(hasProcStopRPC), printCheck(hasAsyncPending),
        printCheck(hasStartupTeardownProc), printCheck(hasNeonatalThreads),
        printCheck(hasAsyncEvents));

    // TODO: If/When we move to per-process locks, then we'll need a smarter
    // should_block check
    //      We don't want the should_block changing between the above
    //      measurement
    //      and the below dequeue.  Perhaps dequeue should alway poll, and the
    //      user thread loops
    //      over it while (should_block == true), with a condition variable
    //      signaling when the
    //      should_block would go to false (so we don't just spin).

    if (should_block && Counter::globalCount(Counter::ForceGeneratorBlock)) {
      // Entirely possible we didn't continue anything, but we want the
      // generator to
      // wake up anyway
      ProcPool()->condvar()->broadcast();
    }
    ...
    if (ev == Event::ptr()) {
      ...
      if (should_block) {
        ...
        ProcControlAPI::globalSetLastError(
            err_internal, "Blocking wait returned without events");
        error = true;
        goto done;
      }
      if (hasHandlerThread) {
      ...
      if (!hasRunningThread) {
        ...
        ProcControlAPI::globalSetLastError(
            err_notrunning, "No threads are running to produce events\n");
        error = true;
        goto done;
      }

      ProcControlAPI::globalSetLastError(err_noevents,
                                         "Poll failed to find events");
    ...
    if (llproc) {
      ...
      if (!result) {
        pthrd_printf(
            "syncRunState failed.  Returning error from waitAndHandleEvents\n");
        error = true;
        goto done;
      }
      ...
      pthrd_printf(
          "Process is gone, skipping syncRunState and deleting handler pool\n");
      delete hpool;
    }
  }
done:
  pthrd_printf(
      "Leaving WaitAndHandleEvents with return %s, 'cause we're done\n",
      !error ? "true" : "false");
  recurse = false;
  return !error;
}
...
void int_process::throwDetachEvent(bool temporary, bool leaveStopped) {
  ...
  threadPool()->initialThread()->getDetachState().desyncStateProc(
      int_thread::stopped);

  mbox()->enqueue(detach_ev);
}

bool int_process::plat_detachDone() { return true; }
...
#if defined(os_windows)
  // If we're on windows, we want to force the generator thread into waiting for
  // a debug
  // event _if the process is stopped_. If the process is running then we're
  // already
  // waiting (or will wait) for a debug event, and forcing the generator to
  // block may
  // lead to doubling up.

  // The following code is similar to GeneratorWindows::hasLiveProc. This is not
  // a coincidence.
  bool all_stopped = true;
  for (int_threadPool::iterator iter = threadpool->begin();
       iter != threadpool->end(); ++iter) {
...
int_LWPTracking *int_process::getLWPTracking() {
  ...
  if (!pLWPTracking) return NULL;
  if (!pLWPTracking->up_ptr) pLWPTracking->up_ptr = new LWPTracking(proc());
  return pLWPTracking;
}

int_threadTracking *int_process::getThreadTracking() {
  if (ThreadTracking_set) return pThreadTracking;
  ThreadTracking_set = true;
  pThreadTracking = dynamic_cast<int_threadTracking *>(this);
  if (!pThreadTracking) return NULL;
...
int_followFork *int_process::getFollowFork() {
  if (FollowFork_set) return pFollowFork;
  FollowFork_set = true;
  pFollowFork = dynamic_cast<int_followFork *>(this);
  if (!pFollowFork) return NULL;
  if (!pFollowFork->up_ptr) pFollowFork->up_ptr = new FollowFork(proc());
  return pFollowFork;
}

int_callStackUnwinding *int_process::getCallStackUnwinding() {
  if (CallStackUnwinding_set) return pCallStackUnwinding;
...
int_memUsage *int_process::getMemUsage() {
  if (MemUsage_set) return pMemUsage;
  MemUsage_set = true;
  pMemUsage = dynamic_cast<int_memUsage *>(this);
  if (!pMemUsage) return NULL;
  if (!pMemUsage->up_ptr) pMemUsage->up_ptr = new MemoryUsage(proc());
  return pMemUsage;
}

int_signalMask *int_process::getSignalMask() {
  if (SignalMask_set) return pSignalMask;
  SignalMask_set = true;
  pSignalMask = dynamic_cast<int_signalMask *>(this);
  if (!pSignalMask) return NULL;
  if (!pSignalMask->up_ptr) pSignalMask->up_ptr = new SignalMask(proc());
  return pSignalMask;
}

int_BGQData *int_process::getBGQData() {
  if (BGQData_set) return pBGQData;
  BGQData_set = true;
  pBGQData = dynamic_cast<int_BGQData *>(this);
  if (!pBGQData) return NULL;
  if (!pBGQData->up_ptr) pBGQData->up_ptr = new BGQData(proc());
  return pBGQData;
}

int_remoteIO *int_process::getRemoteIO() {
  if (remoteIO_set) return pRemoteIO;
  remoteIO_set = true;
  pRemoteIO = dynamic_cast<int_remoteIO *>(this);
  if (!pRemoteIO) return NULL;
  if (!pRemoteIO->up_ptr) pRemoteIO->up_ptr = new RemoteIO(proc());
  return pRemoteIO;
}
...
bool int_process::readMem(Dyninst::Address remote, mem_response::ptr result,
  ...
  if (!thr && plat_needsThreadForMemOps()) {
    ...
    if (!thr) {
      setLastError(err_notstopped,
                   "A thread must be stopped to read from memory");
  ...
  if (!plat_needsAsyncIO()) {
    ...
    if (iev) {
      pthrd_printf(
          "Enqueueing new EventAsyncRead into mailbox on synchronous "
          "platform\n");
    ...
    pthrd_printf(
        "Async read from remote memory %lx to %p, size = %lu on %d/%d\n",
        remote, result->getBuffer(), (unsigned long)result->getSize(), getPid(),
        thr ? thr->getLWP() : (Dyninst::LWP)(-1));
...
bool int_process::writeMem(const void *local, Dyninst::Address remote,
  ...
  if (!thr && plat_needsThreadForMemOps()) {
    ...
    if (!thr) {
      setLastError(err_notstopped,
                   "A thread must be stopped to write to memory");
  ...
  if (!plat_needsAsyncIO()) {
    pthrd_printf("Writing to remote memory %lx from %p, size = %lu on %d/%d\n",
                 remote, local, (unsigned long)size, getPid(),
                 thr ? thr->getLWP() : (Dyninst::LWP)(-1));
    ...
    if (iev) {
      pthrd_printf(
          "Enqueueing new EventAsyncWrite into mailbox on synchronous "
          "platform\n");
    ...
    pthrd_printf(
        "Async writing to remote memory %lx from %p, size = %lu on %d/%d\n",
        remote, local, (unsigned long)size, getPid(),
        thr ? thr->getLWP() : (Dyninst::LWP)(-1));
...
Dyninst::Address int_process::mallocExecMemory(unsigned size) {
  ...
  for (i = exec_mem_cache.begin(); i != exec_mem_cache.end(); ++i) {
    if (i->first + i->second > max) max = i->first + i->second;
  }
...
Dyninst::Address int_process::direct_infMalloc(unsigned long, bool,
                                               Dyninst::Address) {
  assert(0);
  return 0;
}
...
Address int_process::infMalloc(unsigned long size, bool use_addr,
                               Address addr) {
  int_addressSet as;
...
bool int_process::infMalloc(unsigned long size, int_addressSet *aset,
                            bool use_addr) {
  bool had_error = false;
  ...
  for (int_addressSet::iterator i = aset->begin(); i != aset->end(); i++) {
    ...
    if (proc->getState() == int_process::detached) continue;
    pthrd_printf("Process %d is allocating memory of size %lu at 0x%lx\n",
                 proc->getPid(), size, addr);

    if (proc->plat_supportDirectAllocation()) {
      // Platforms like Windows don't need IRPCs for allocation.  Just do it and
      // put the results
      // in direct_results, which will be unioned with aset latter.
      Dyninst::Address addr_result =
          proc->direct_infMalloc(size, use_addr, addr);
      if (addr_result == 0) {
        had_error = true;
        continue;
      }
    ...
    int_iRPC::ptr rpc =
        rpcMgr()->createInfMallocRPC(proc, size, use_addr, addr);
    assert(rpc);
  ...
  if (!active_mallocs.empty()) {
    ...
    if (!result) {
      ...
      for_each(
          ...
          setError(
              err_internal,
              "Error while calling waitAndHandleForProc from infMalloc\n"));
      return false;
    }
  }

  if (!use_addr) aset->clear();
  if (!direct_results.empty())
    aset->insert(direct_results.begin(), direct_results.end());

  for (set<pair<int_process *, int_iRPC::ptr> >::iterator i =
           active_mallocs.begin();
    ...
    if (aresult == (unsigned long)-1) {
      ...
      proc->setLastError(err_procread,
                         "Unable to allocate memory at given address");
      had_error = true;
      continue;
    }
    proc->memory()->inf_malloced_memory.insert(make_pair(aresult, size));
    if (use_addr) continue;
...
bool int_process::infFree(int_addressSet *aset) {
  ...
  for (int_addressSet::iterator i = aset->begin(); i != aset->end(); i++) {
    ...
    std::map<Dyninst::Address, unsigned long>::iterator j =
        proc->mem->inf_malloced_memory.find(addr);
    ...
    assert(rpc);
    pthrd_printf(
        "Process %d is freeing memory of size %lu at 0x%lx with rpc %lu\n",
        proc->getPid(), size, addr, rpc->id());
    bool result = rpcMgr()->postRPCToProc(proc, rpc);
    if (!result) {
    ...
    if (!result) {
      ...
      for_each(
          ...
          setError(err_internal,
                   "Error while calling waitAndHandleForProc from infFree\n"));
      return false;
    }
  }

  for (set<pair<Process::ptr, int_iRPC::ptr> >::iterator i =
           active_frees.begin();
       i != active_frees.end(); i++) {
    ...
    map<Dyninst::Address, unsigned long>::iterator j =
        proc->mem->inf_malloced_memory.find(addr);
...
bool int_process::plat_decodeMemoryRights(Process::mem_perm &rights_internal,
  ...
  setLastError(err_unsupported,
               "Decode Mem Permission not supported on this platform\n");
  return false;
}
...
bool int_process::plat_encodeMemoryRights(Process::mem_perm rights_internal,
  ...
  setLastError(err_unsupported,
               "Encode Memory Permission not supported on this platform\n");
  return false;
}

bool int_process::getMemoryAccessRights(Dyninst::Address addr,
...
bool int_process::plat_getMemoryAccessRights(Dyninst::Address addr,
                                             Process::mem_perm &rights) {
  (void)addr;
  (void)rights;
  perr_printf("Called getMemoryAccessRights on unspported platform\n");
  setLastError(err_unsupported,
               "Get Memory Permission not supported on this platform\n");
  return false;
}
...
bool int_process::setMemoryAccessRights(Dyninst::Address addr, size_t size,
  ...
  if (!plat_setMemoryAccessRights(addr, size, rights, oldRights)) {
    pthrd_printf(
        "ERROR: set rights to %s from memory %lx on target process %d\n",
...
bool int_process::plat_setMemoryAccessRights(Dyninst::Address addr, size_t size,
  ...
  setLastError(err_unsupported,
               "Set Memory Permission not supported on this platform\n");
  return false;
}
...
bool int_process::findAllocatedRegionAround(Dyninst::Address addr,
  ...
  if (!plat_findAllocatedRegionAround(addr, memRegion)) {
    pthrd_printf(
        "Error when find allocated memory region"
...
bool int_process::plat_findAllocatedRegionAround(
...
void int_process::setForceGeneratorBlock(bool b) {
  if (b) {
    ...
    if (force_generator_block_count.local()) force_generator_block_count.dec();
  }
...
HandlerPool *int_process::handlerPool() const { return handlerpool; }
...
bool int_process::addBreakpoint(Dyninst::Address addr, int_breakpoint *bp) {
  if (getState() != running) {
    perr_printf("Attempted to add breakpoint at %lx to exited process %d\n",
                addr, getPid());
    setLastError(err_exited,
                 "Attempted to insert breakpoint into exited process\n");
    return false;
  }
...
bool int_process::removeAllBreakpoints() {
  ...
  for (std::map<Dyninst::Address, sw_breakpoint *>::iterator iter =
...
bool int_process::removeBreakpoint(Dyninst::Address addr, int_breakpoint *bp,
  ...
  if (bps_to_remove.empty()) {
    ...
    setLastError(err_notfound,
                 "Tried to uninstall breakpoint that isn't installed.\n");
    return false;
  }

  for (set<bp_instance *>::iterator i = bps_to_remove.begin();
       i != bps_to_remove.end(); i++) {
    bool empty;
    bp_instance *ibp = *i;
...
sw_breakpoint *int_process::getBreakpoint(Dyninst::Address addr) {
  std::map<Dyninst::Address, sw_breakpoint *>::iterator i =
      mem->breakpoints.find(addr);
  if (i == mem->breakpoints.end()) return NULL;
  return i->second;
}
...
int_library *int_process::getLibraryByName(std::string s) const {
  ...
  for (set<int_library *>::iterator i = mem->libs.begin(); i != mem->libs.end();
       ++i) {
...
unsigned int int_process::plat_getCapabilities() {
  return (Process::pc_read | Process::pc_write | Process::pc_irpc |
          Process::pc_control);
}
...
bool int_process::plat_threadOpsNeedProcStop() { return false; }

size_t int_process::numLibs() const { return mem->libs.size(); }

std::string int_process::getExecutable() const {
  return executable;  // The name of the exec passed to PC
}

bool int_process::isInCallback() { return in_callback; }

mem_state::ptr int_process::memory() const { return mem; }

err_t int_process::getLastError() { return last_error; }

const char *int_process::getLastErrorMsg() { return last_error_string; }
...
bool int_process::wasForcedTerminated() const { return forcedTermination; }

bool int_process::plat_individualRegRead(Dyninst::MachRegister, int_thread *) {
  return plat_individualRegAccess();
}

bool int_process::plat_individualRegSet() { return plat_individualRegAccess(); }

bool int_process::isInCB() { return in_callback; }
...
async_ret_t int_process::plat_calcTLSAddress(int_thread *, int_library *,
                                             Offset, Address &,
...
bool int_process::plat_needsAsyncIO() const { return false; }
...
bool int_process::plat_writeMemAsync(int_thread *, const void *,
                                     Dyninst::Address, size_t,
                                     result_response::ptr, bp_write_t) {
  assert(0);
  return false;
}

memCache *int_process::getMemCache() { return &mem_cache; }

void int_process::updateSyncState(Event::ptr ev, bool gen) {
  // This works around a Linux bug where a continue races with a whole-process
  // exit
  ...
  switch (ev->getSyncType()) {
    ...
    case Event::sync_thread: {
      ...
      int_thread::StateTracker &st =
          gen ? thrd->getGeneratorState() : thrd->getHandlerState();
      if (!thrd) {
        pthrd_printf(
            "No thread for sync thread event, assuming thread exited\n");
        return;
      }
      ...
      if (old_state == int_thread::exited) {
        ...
        pthrd_printf(
            "Recieved events for exited thread, not changing thread state\n");
        break;
      }
      pthrd_printf(
          "Event %s is thread synchronous, marking thread %d %s stopped\n",
      ...
      if (old_state == int_thread::errorstate) break;
      st.setState(int_thread::stopped);
      break;
    }
    case Event::sync_process: {
      pthrd_printf(
          "Event %s is process synchronous, marking process %d %s stopped\n",
      ...
      for (int_threadPool::iterator i = tp->begin(); i != tp->end(); ++i) {
        ...
        int_thread::StateTracker &st =
            gen ? thrd->getGeneratorState() : thrd->getHandlerState();
        int_thread::State old_state = st.getState();
        if (!RUNNING_STATE(old_state)) continue;
        st.setState(int_thread::stopped);
      }
      break;
    }
...
bool int_process::plat_supportThreadEvents() { return false; }

bool int_process::plat_supportLWPCreate() { return false; }

bool int_process::plat_supportLWPPreDestroy() { return false; }

bool int_process::plat_supportLWPPostDestroy() { return false; }

bool int_process::plat_supportHWBreakpoint() { return false; }

bool int_process::plat_supportFork() { return false; }

bool int_process::plat_supportExec() { return false; }

async_ret_t int_process::plat_needsEmulatedSingleStep(int_thread *,
                                                      std::vector<Address> &) {
  return aret_success;
}

void int_process::plat_getEmulatedSingleStepAsyncs(int_thread *,
                                                   std::set<response::ptr>) {
  assert(0);
}

bool int_process::plat_needsPCSaveBeforeSingleStep() { return false; }

map<int, int> &int_process::getProcDesyncdStates() {
  return proc_desyncd_states;
}

bool int_process::isRunningSilent() { return silent_mode; }

void int_process::setRunningSilent(bool b) { silent_mode = b; }

bool int_process::plat_supportDOTF() { return true; }

void int_process::noteNewDequeuedEvent(Event::ptr) {}

bool int_process::plat_preHandleEvent() { return true; }

bool int_process::plat_postHandleEvent() { return true; }

bool int_process::plat_preAsyncWait() { return true; }
...
indep_lwp_control_process::indep_lwp_control_process(
    Dyninst::PID p, std::string e, std::vector<std::string> a,
    std::vector<std::string> envp, std::map<int, int> f)
    : int_process(p, e, a, envp, f) {}

indep_lwp_control_process::indep_lwp_control_process(Dyninst::PID pid_,
                                                     int_process *p)
    : int_process(pid_, p) {}
...
bool indep_lwp_control_process::plat_syncRunState() {
  for (int_threadPool::iterator i = threadPool()->begin();
    ...
    if (handler_state == target_state) {
      ...
      result = thr->intCont();
    } else if (RUNNING_STATE(handler_state) &&
               target_state == int_thread::stopped) {
      result = thr->intStop();
    }
...
unified_lwp_control_process::unified_lwp_control_process(Dyninst::PID pid_,
                                                         int_process *p)
    : int_process(pid_, p) {}
...
bool unified_lwp_control_process::plat_syncRunState() {
  ...
  if (getState() == detached || getState() == exited ||
      getState() == errorstate) {
    pthrd_printf(
        "Process %d is in state %s, doing nothing in plat_syncRunState\n",
  ...
  for (int_threadPool::iterator i = threadPool()->begin();
    ...
    if (!RUNNING_STATE(handler_state) && RUNNING_STATE(target_state) &&
        thr->singleStep()) {
  ...
  pthrd_printf(
      "In plat_syncRunState: want_all_stopped = %s, want_all_running = %s, "
      "want_some_stopped = %s, want_some_running = %s, is_all_stopped = %s, "
      "is_all_running = %s, want_ss_running = %s\n",
      want_all_stopped ? "t" : "f", want_all_running ? "t" : "f",
      want_some_stopped ? "t" : "f", want_some_running ? "t" : "f",
      is_all_stopped ? "t" : "f", is_all_running ? "t" : "f",
      want_ss_running ? "t" : "f");
  ...
  if (want_ss_running) {
    ...
    for (int_threadPool::iterator i = threadPool()->begin();
      ...
      int_thread *thr = *i;
      if (thr->singleStep() &&
          thr->getHandlerState().getState() == int_thread::stopped &&
    ...
    pthrd_printf(
        "Process %d is partially stopped, needs to stop.  Stopping process\n",
        getPid());
    result = threadPool()->initialThread()->intStop();
  } else if (want_some_running && !is_all_running) {
    pthrd_printf(
        "Process %d is partially running, needs to run.  Continuing process\n",
        getPid());
    result = threadPool()->initialThread()->intCont();
  } else {
...
bool unified_lwp_control_process::plat_processGroupContinues() { return true; }

hybrid_lwp_control_process::hybrid_lwp_control_process(
    Dyninst::PID p, std::string e, std::vector<std::string> a,
    std::vector<std::string> envp, std::map<int, int> f)
    : int_process(p, e, a, envp, f), debugger_stopped(false) {}

hybrid_lwp_control_process::hybrid_lwp_control_process(Dyninst::PID pid_,
                                                       int_process *p)
    : int_process(pid_, p), debugger_stopped(false) {
  // Clone debugger_stopped from parent
  hybrid_lwp_control_process *par =
      dynamic_cast<hybrid_lwp_control_process *>(p);
  ...
  pthrd_printf("Set debugger stopped to %s on %d, matching parent %d\n",
               (debugger_stopped ? "true" : "false"), pid_, p->getPid());
}

hybrid_lwp_control_process::~hybrid_lwp_control_process() {}

bool hybrid_lwp_control_process::suspendThread(int_thread *thr) {
  if (thr->isSuspended()) return true;
...
bool hybrid_lwp_control_process::plat_processGroupContinues() { return true; }
...
bool hybrid_lwp_control_process::plat_syncRunState() {
  ...
  for (i = tp->begin(); i != tp->end(); i++) {
    ...
    pthrd_printf("Checking %d/%d: state %s\n", getPid(), thr->getLWP(),
                 int_thread::stateStr(thr->getUserState().getState()));
  ...
  if (!any_target_running && !any_running) {
    pthrd_printf(
        "Target process state %d is stopped and process is stopped, leaving\n",
        getPid());
    return true;
  }
  if (!any_target_stopped && !any_stopped) {
    pthrd_printf(
        "Target process state %d is running and process is running, leaving\n",
        getPid());
    return true;
  }
  if (!plat_debuggerSuspended()) {
    pthrd_printf(
        "Process %d is not debugger suspended, but have changes.  Stopping "
        "process.\n",
        getPid());
    return a_running_thread->intStop();
  }

  // If we're here, we must be debuggerSuspended, and thus no threads are
  // running (!any_running)
  // since we didn't trigger the above if statement, we must have some thread we
  // want to run
  // (any_target_running)
  ...
  for (i = tp->begin(); i != tp->end(); i++) {
    ...
    if (thr->getDetachState().getState() == int_thread::detached) continue;
...
async_ret_t int_thread::handleSingleStepContinue() {
  ...
  for (set<int_thread *>::iterator i = thrds.begin(); i != thrds.end(); i++) {
    ...
    if (!new_es) new_es = new emulated_singlestep(thr);
    for (vector<Address>::iterator j = addrs.begin(); j != addrs.end(); j++) {
      Address addr = *j;
      pthrd_printf(
          "Installing emulated single-step breakpoint for %d/%d at %lx\n",
          llproc()->getPid(), thr->getLWP(), addr);
      aresult = new_es->add(addr);
      if (aresult == aret_async) {
        pthrd_printf(
            "Async return while installing breakpoint for "
            "emulated_singlestep\n");
        ret = aret_async;
        goto done;
      }
      if (aresult == aret_error) {
        pthrd_printf(
            "Error return while installing breakpoint for "
            "emulated_singlestep\n");
        ret = aret_error;
        goto done;
      }
...
bool int_thread::hasPendingStop() const { return pending_stop.local(); }

bool int_thread::wasRunningWhenAttached() const {
  return running_when_attached;
}

void int_thread::setRunningWhenAttached(bool b) { running_when_attached = b; }

Process::ptr int_thread::proc() const { return proc_->proc(); }

int_process *int_thread::llproc() const { return proc_; }

Dyninst::LWP int_thread::getLWP() const { return lwp; }

void int_thread::addContinueCB(continue_cb_t cb) { continue_cbs.insert(cb); }

void int_thread::triggerContinueCBs() {
  bool sync_conts = llproc()->plat_processGroupContinues();
  for (set<continue_cb_t>::iterator i = continue_cbs.begin();
       i != continue_cbs.end(); i++) {
    if (!sync_conts) {
...
int_thread::StateTracker &int_thread::getIRPCState() { return irpc_state; }
...
int_thread::StateTracker &int_thread::getAsyncState() { return async_state; }

int_thread::StateTracker &int_thread::getInternalState() {
  return internal_state;
}

int_thread::StateTracker &int_thread::getDetachState() { return detach_state; }
...
int_thread::StateTracker &int_thread::getUserState() { return user_state; }
...
int_thread::State int_thread::getTargetState() const { return target_state; }
...
int_thread::StateTracker &int_thread::getStateByID(int id) {
  switch (id) {
    ...
    case AsyncStateID:
      return async_state;
    case CallbackStateID:
      return callback_state;
    case PendingStopStateID:
      return pending_stop_state;
    case IRPCStateID:
      return irpc_state;
    case IRPCSetupStateID:
      return irpc_setup_state;
    case IRPCWaitStateID:
      return irpc_wait_state;
    case BreakpointStateID:
      return breakpoint_state;
    case BreakpointHoldStateID:
      return breakpoint_hold_state;
    case BreakpointResumeStateID:
      return breakpoint_resume_state;
    case InternalStateID:
      return internal_state;
    case DetachStateID:
      return detach_state;
    case UserRPCStateID:
      return user_irpc_state;
    case ControlAuthorityStateID:
      return control_authority_state;
    case UserStateID:
      return user_state;
    case HandlerStateID:
      return handler_state;
    case GeneratorStateID:
      return generator_state;
  }
  assert(0);
  return exiting_state;
...
Counter &int_thread::syncRPCCount() { return sync_rpc_count; }

Counter &int_thread::runningSyncRPCThreadCount() {
  return sync_rpc_running_thr_count;
}

Counter &int_thread::pendingStopsCount() { return pending_stop; }

Counter &int_thread::clearingBPCount() { return clearing_bp_count; }

Counter &int_thread::procStopRPCCount() { return proc_stop_rpc_count; }

Counter &int_thread::getGeneratorNonExitedThreadCount() {
  return generator_nonexited_thrd_count;
}

Counter &int_thread::neonatalThreadCount() { return neonatal_threads; }

Counter &int_thread::pendingStackwalkCount() { return pending_stackwalk_count; }

void int_thread::setContSignal(int sig) { continueSig_ = sig; }

int int_thread::getContSignal() { return continueSig_; }

int_thread *int_thread::createThread(int_process *proc, Dyninst::THR_ID thr_id,
  ...
  pthrd_printf("Creating %s thread %d/%d, thr_id = 0x%lx\n",
               initial_thrd ? "initial" : "new", proc->getPid(),
               newthr->getLWP(), thr_id);
...
void int_thread::throwEventsBeforeContinue() {
  ...
  if (rpc && !runningRPC() && rpc->getState() == int_iRPC::Posted) {
    ...
    if (bpi->swBP()) {
      ...
      pthrd_printf(
          "Found thread %d/%d to be stopped on a software BP, not continuing\n",
          llproc()->getPid(), getLWP());
      getBreakpointState().desyncStateProc(int_thread::stopped);
      EventBreakpointClear::ptr evclear =
          EventBreakpointClear::ptr(new EventBreakpointClear());
      evclear->getInternal()->stopped_proc = true;
      new_ev = evclear;
    } else {
      // No process stop needed for a hardware breakpoint
      pthrd_printf(
          "Found thread %d/%d to be stopped on a hardware BP, not continuing\n",
          llproc()->getPid(), getLWP());
...
bool int_thread::suppressSanityChecks() { return false; }

bool int_thread::isExiting() const { return handler_exiting_state; }

void int_thread::setExiting(bool b) { handler_exiting_state = b; }

bool int_thread::isExitingInGenerator() const {
  return generator_exiting_state;
}

void int_thread::setExitingInGenerator(bool b) { generator_exiting_state = b; }
...
void int_thread::cleanFromHandler(int_thread *thrd, bool should_delete) {
  ...
  if (should_delete) {
    ...
    if (ProcPool()->findThread(thrd->getLWP()) != NULL) {
      ...
      pthrd_printf(
          "%d/%d already gone from top level ProcPool(), not removing\n",
...
Thread::ptr int_thread::thread() { return up_thread; }
...
bool int_thread::getAllRegisters(allreg_response::ptr response) {
  ...
  if (!llproc()->plat_needsAsyncIO()) {
    ...
    *(response->getRegPool()) = cached_regpool;
    ...
    if (iev) {
      pthrd_printf(
          "Enqueueing new EventAsyncReadAllRegs into mailbox on synchronous "
          "platform\n");
      EventAsyncReadAllRegs::ptr ev =
          EventAsyncReadAllRegs::ptr(new EventAsyncReadAllRegs(iev));
      ev->setProcess(proc());
      ev->setThread(thread());
      ev->setSyncType(Event::async);
      mbox()->enqueue(ev);
...
bool int_thread::setAllRegisters(int_registerPool &pool,
  ...
  if (!llproc()->plat_needsAsyncIO()) {
    ...
    if (iev) {
      pthrd_printf(
          "Enqueueing new EventAsyncSetAllRegs into mailbox on synchronous "
          "platform\n");
      EventAsyncSetAllRegs::ptr ev =
          EventAsyncSetAllRegs::ptr(new EventAsyncSetAllRegs(iev));
      ev->setProcess(proc());
      ev->setThread(thread());
      ev->setSyncType(Event::async);
      mbox()->enqueue(ev);
...
bool int_thread::getRegister(Dyninst::MachRegister reg,
  ...
  if (!llproc()->plat_individualRegRead(reg, this)) {
    ...
    pthrd_printf(
        "Platform does not support individual register access, "
        "getting everything\n");
    if (!llproc()->plat_needsAsyncIO()) {
      int_registerPool pool;
      allreg_response::ptr allreg_resp =
          allreg_response::createAllRegResponse(&pool);
      ...
      allreg_response::ptr allreg_resp =
          allreg_response::createAllRegResponse(&cached_regpool);
...
bool int_thread::setRegister(Dyninst::MachRegister reg,
...
rpc_list_t *int_thread::getPostedRPCs() { return &posted_rpcs; }

bool int_thread::hasPostedRPCs() { return (posted_rpcs.size() != 0); }
...
void int_thread::setRunningRPC(int_iRPC::ptr rpc_) {
  ...
  if (stopped_on_breakpoint_addr) {
    pthrd_printf(
        "Thread %d/%d is stopped on BP at 0x%lx and about to run RPC.  "
        "Postponing BP clear.\n",
...
int_iRPC::ptr int_thread::runningRPC() const { return running_rpc; }

void int_thread::clearRunningRPC() {
  if (postponed_stopped_on_breakpoint_addr) {
    pthrd_printf(
        "Thread %d/%d is moving back to stopped breakpoint at %lx after IRPC "
        "completion\n",
...
bool int_thread::hasSavedRPCRegs() { return rpc_regs.full; }
...
bool int_thread::hasSyncRPC() { return sync_rpc_count.local(); }

int_iRPC::ptr int_thread::nextPostedIRPC() const {
  if (!posted_rpcs.size()) return int_iRPC::ptr();
  return posted_rpcs.front();
}

bool int_thread::singleStepMode() const { return single_step; }

void int_thread::setSingleStepMode(bool s) { single_step = s; }

bool int_thread::singleStepUserMode() const { return user_single_step; }

void int_thread::setSingleStepUserMode(bool s) { user_single_step = s; }

bool int_thread::singleStep() const { return single_step || user_single_step; }

void int_thread::setSyscallUserMode(bool s) { user_syscall = s; }

bool int_thread::syscallUserMode() const { return user_syscall; }

bool int_thread::syscallMode() const { return user_syscall; }
...
bool int_thread::preSyscall() {
  ...
  if (!plat_getRegister(
          MachRegister::getSyscallReturnValueReg(llproc()->getTargetArch()),
          syscallReturnValue)) {
    ret = true;
  }
...
bp_instance *int_thread::isStoppedOnBP() {
  ...
  if (swbp) return static_cast<bp_instance *>(swbp);
  hw_breakpoint *hwbp = getHWBreakpoint(stopped_on_breakpoint_addr);
  return static_cast<bp_instance *>(hwbp);
}

void int_thread::setTID(Dyninst::THR_ID tid_) { tid = tid_; }

bp_instance *int_thread::isClearingBreakpoint() { return clearing_breakpoint; }

bool int_thread::haveUserThreadInfo() { return false; }
...
bool int_thread::getStartFuncAddress(Dyninst::Address &) {
  perr_printf("Unsupported attempt to get start func address on %d/%d\n",
              llproc()->getPid(), getLWP());
  setLastError(err_unsupported,
               "getStartFuncAddress not supported on this platform\n");
  return false;
}
...
bool int_thread::getStackBase(Dyninst::Address &) {
  perr_printf("Unsupported attempt to get stack base on %d/%d\n",
              llproc()->getPid(), getLWP());
  setLastError(err_unsupported,
               "getStackBase not supported on this platform\n");
  return false;
}
...
bool int_thread::getStackSize(unsigned long &) {
  perr_printf("Unsupported attempt to get stack size on %d/%d\n",
              llproc()->getPid(), getLWP());
  setLastError(err_unsupported,
               "getStackSize not supported on this platform\n");
  return false;
}
...
Dyninst::Address int_thread::getThreadInfoBlockAddr() {
  perr_printf("Unsupported attempt to get ThreadInfoBlock Address on %d/%d\n",
              llproc()->getPid(), getLWP());
  setLastError(err_unsupported,
               "getThreadInfoBlockAddr not supported on this platform\n");
  return 0;
}

unsigned int_thread::hwBPAvail(unsigned) { return 0; }

EventBreakpoint::ptr int_thread::decodeHWBreakpoint(response::ptr &, bool,
                                                    Dyninst::MachRegisterVal) {
  return EventBreakpoint::ptr();
}

bool int_thread::addHWBreakpoint(hw_breakpoint *, bool,
                                 std::set<response::ptr> &, bool &) {
  perr_printf("Tried to use hardware breakpoint on unsupported system\n");
  setLastError(err_unsupported,
               "Hardware breakpoints not supported on this platform\n");
  return false;
}

bool int_thread::rmHWBreakpoint(hw_breakpoint *, bool,
                                std::set<response::ptr> &, bool &) {
  perr_printf("Tried to use hardware breakpoint on unsupported system\n");
  setLastError(err_unsupported,
               "Hardware breakpoints not supported on this platform\n");
...
bool int_thread::bpNeedsClear(hw_breakpoint *) { return false; }
...
int_thread::StateTracker::StateTracker(int_thread *t, int id_,
                                       int_thread::State initial)
    : state(int_thread::none), id(id_), sync_level(0), up_thr(t) {
...
void int_thread::StateTracker::desyncState(State ns) {
  ...
  pthrd_printf("Desyncing %d/%d %s state to level %d\n",
               up_thr->llproc()->getPid(), up_thr->getLWP(), getName().c_str(),
               sync_level);
  assert(id != int_thread::HandlerStateID &&
         id != int_thread::GeneratorStateID && id != int_thread::UserStateID);
...
int_thread::State int_thread::StateTracker::getState() const { return state; }

bool int_thread::StateTracker::isDesynced() const { return (sync_level != 0); }
...
bool int_thread::StateTracker::setState(State to) {
  ...
  if (state == errorstate) {
    perr_printf(
        "Attempted %s state reversion for %d/%d from errorstate to %s\n",
  ...
  if (id == int_thread::HandlerStateID) {
    if ((state == int_thread::neonatal ||
         state == int_thread::neonatal_intermediate) &&
        (to != int_thread::neonatal &&
         to != int_thread::neonatal_intermediate)) {
  ...
  pthrd_printf("Changing %s state for %d/%d from %s to %s\n", s.c_str(), pid,
               lwp, stateStr(state), stateStr(to));
  state = to;
  ...
  if (up_thr->up_thread && !up_thr->suppressSanityChecks()) {
    ...
    if (handler_state == stopped)
      assert(generator_state == stopped || generator_state == exited ||
...
int int_thread::StateTracker::getID() const { return id; }

void int_thread::setSuspended(bool b) { suspended = b; }

bool int_thread::isSuspended() const { return suspended; }

void int_thread::setLastError(err_t ec, const char *es) {
  if (proc_) proc_->setLastError(ec, es);
}
...
int_thread *int_threadPool::findThreadByLWP(Dyninst::LWP lwp) {
  ...
  if (i == thrds_by_lwp.end()) return NULL;
  return i->second;
}
...
bool int_threadPool::allHandlerStopped() {
  for (iterator i = begin(); i != end(); i++) {
    if ((*i)->getHandlerState().getState() != int_thread::stopped) return false;
  }
  return true;
}
...
bool int_threadPool::allStopped(int state_id) {
  for (iterator i = begin(); i != end(); i++) {
    if (!(*i)->isStopped(state_id)) return false;
  }
  return true;
}

bool int_threadPool::hadMultipleThreads() const { return had_multiple_threads; }
...
// process exit. So we have to handle the case where the initial thread goes
// away.
// In particular this means we need to have a valid initial thread in the
// threadpool after
// we finish removing this one, so that we have an arbitrary thread available
// for continue calls.

// FIXME: should probably kill most of the data duplication here. Log search for
// thread by LWP is
// I guess legitimate, but there's AFAICT no reason for the initial thread to be
// anything other than
// pinned to threads[0]. And the data duplication involved in threads by LWP is
// not a good thing
// for consistency...
...
int_process *int_threadPool::proc() const { return proc_; }

unsigned int_threadPool::size() const { return threads.size(); }

ThreadPool *int_threadPool::pool() const { return up_pool; }
...
void int_threadPool::restoreUserState() {
  for (iterator i = begin(); i != end(); i++) {
    int_thread *thr = *i;
    if (thr->saved_user_state == int_thread::none) continue;
...
int_threadPool::int_threadPool(int_process *p)
    : initial_thread(NULL), proc_(p), had_multiple_threads(false) {
...
int_threadPool::~int_threadPool() {
  ...
  for (vector<int_thread *>::iterator i = threads.begin(); i != threads.end();
       ++i) {
...
int_breakpoint::int_breakpoint(unsigned int hw_prems_, unsigned int hw_size_,
                               Breakpoint::ptr up)
    : up_bp(up),
      to(0x0),
      isCtrlTransfer_(false),
...
bool int_breakpoint::isCtrlTransfer() const { return isCtrlTransfer_; }

Address int_breakpoint::toAddr() const { return to; }

void *int_breakpoint::getData() const { return data; }

void int_breakpoint::setData(void *v) { data = v; }

Breakpoint::weak_ptr int_breakpoint::upBreakpoint() const { return up_bp; }

void int_breakpoint::setThreadSpecific(Thread::const_ptr p) {
  thread_specific.insert(p);
}

void int_breakpoint::setOneTimeBreakpoint(bool b) { onetime_bp = b; }
...
bool int_breakpoint::isOneTimeBreakpoint() const { return onetime_bp; }

bool int_breakpoint::isOneTimeBreakpointHit() const { return onetime_bp_hit; }
...
void int_breakpoint::setProcessStopper(bool b) { procstopper = b; }

bool int_breakpoint::isProcessStopper() const { return procstopper; }

bool int_breakpoint::isHW() const { return hw; }

unsigned int_breakpoint::getHWSize() const { return hw_size; }

unsigned int_breakpoint::getHWPerms() const { return hw_perms; }

void int_breakpoint::setSuppressCallbacks(bool b) { suppress_callbacks = b; }

bool int_breakpoint::suppressCallbacks() const { return suppress_callbacks; }

bool int_breakpoint::isOffsetTransfer() const { return offset_transfer; }

bp_instance::bp_instance(Address addr_)
    : addr(addr_), installed(false), suspend_count(0), swbp(NULL), hwbp(NULL) {}
...
bool bp_instance::checkBreakpoint(int_breakpoint *bp, int_process *p) {
  if (bp->isCtrlTransfer()) {
    for (set<int_breakpoint *>::iterator i = bps.begin(); i != bps.end(); i++) {
      if ((*i)->isCtrlTransfer()) {
        perr_printf(
            "Error.  Attempted to add two control transfer breakpoints "
            "at same place\n");
        for (set<int_process *>::iterator i = p->memory()->procs.begin();
             i != p->memory()->procs.end(); i++) {
          (*i)->setLastError(err_badparam,
                             "Attempted two control transfer breakpoints at "
                             "same location\n");
        }
        return false;
      }
...
bool bp_instance::rmBreakpoint(int_process *proc, int_breakpoint *bp,
                               bool &empty, set<response::ptr> &resps) {
  empty = false;
  set<int_breakpoint *>::iterator i = bps.find(bp);
  if (i == bps.end()) {
    perr_printf("Attempted to remove a non-installed breakpoint\n");
    proc->setLastError(err_badparam,
                       "Breakpoint was not installed in process\n");
    return false;
  }
  bps.erase(i);
...
Address bp_instance::getAddr() const { return addr; }

bp_instance::iterator bp_instance::begin() { return bps.begin(); }

bp_instance::iterator bp_instance::end() { return bps.end(); }
...
int_breakpoint *bp_instance::getCtrlTransferBP(int_thread *thrd) {
  for (iterator i = begin(); i != end(); i++) {
    int_breakpoint *bp = *i;
    if (!bp->isCtrlTransfer()) continue;
    if (thrd && bp->isThreadSpecific() &&
        !bp->isThreadSpecificTo(thrd->thread()))
      continue;
    return bp;
  }
  return NULL;
...
bool bp_instance::isInstalled() const { return installed; }

sw_breakpoint *bp_instance::swBP() { return swbp; }

hw_breakpoint *bp_instance::hwBP() { return hwbp; }
...
sw_breakpoint::sw_breakpoint(mem_state::ptr memory_, const sw_breakpoint *ip)
    : bp_instance(ip),
      memory(memory_),
...
sw_breakpoint *sw_breakpoint::create(int_process *proc, int_breakpoint *bp,
...
bool sw_breakpoint::writeBreakpoint(int_process *proc,
  ...
  return proc->writeMem(bp_insn, addr, buffer_size, write_response, NULL,
                        int_process::bp_install);
}

bool sw_breakpoint::saveBreakpointData(int_process *proc,
...
bool sw_breakpoint::restoreBreakpointData(int_process *proc,
                                          result_response::ptr res_resp) {
  assert(buffer_size != 0);

  pthrd_printf("Restoring original code over breakpoint at %lx\n", addr);
  return proc->writeMem(buffer, addr, buffer_size, res_resp, NULL,
                        int_process::bp_clear);
}

async_ret_t sw_breakpoint::uninstall(int_process *proc,
                                     set<response::ptr> &resps) {
  assert(installed);
  ...
  if (i == memory->breakpoints.end()) {
    ...
    proc->setLastError(err_notfound,
                       "Tried to uninstall breakpoint that isn't installed.\n");
    return aret_error;
  }
...
async_ret_t sw_breakpoint::suspend(int_process *proc,
                                   set<response::ptr> &resps) {
  if (suspend_common()) return aret_success;
...
async_ret_t sw_breakpoint::resume(int_process *proc,
                                  set<response::ptr> &resps) {
  if (resume_common()) return aret_success;
  result_response::ptr result_resp = result_response::createResultResponse();
...
unsigned sw_breakpoint::getNumIntBreakpoints() const { return bps.size(); }
...
bool sw_breakpoint::prepBreakpoint(int_process *proc,
...
bool sw_breakpoint::insertBreakpoint(int_process *proc,
                                     result_response::ptr res_resp) {
  assert(prepped);
  assert(!installed);
...
bool sw_breakpoint::needsClear() { return true; }
...
hw_breakpoint *hw_breakpoint::create(int_process *proc, int_breakpoint *bp,
                                     Dyninst::Address addr) {
  ...
  if (!proc_wide) {
    for (set<Thread::const_ptr>::iterator i = bp->thread_specific.begin();
         i != bp->thread_specific.end(); i++) {
      int_thread *llthrd = (*i)->llthrd();
      thrds.insert(llthrd);
      pthrd_printf(
          "Adding thread-specific hardware breakpoint to %d/%d at %lx\n",
  ...
  for (set<int_thread *>::iterator i = thrds.begin(); i != thrds.end(); i++) {
    ...
    if (!thr->hwBPAvail(mode)) {
      perr_printf(
          "Do not have space for new hardware breakpoint of mode 0%u on thread "
          "%d/%d\n",
  ...
  if (no_space) {
    proc->setLastError(err_bpfull,
                       "Not enough space for a new hardware breakpoint\n");
    return NULL;
  }
  ...
  for (set<int_thread *>::iterator i = thrds.begin(); i != thrds.end(); i++) {
    ...
    assert(hwbp);
    if (!first_bp) first_bp = hwbp;
    hw_bps.insert(hwbp);
    hwbp->installed = true;
    if (!hwbp->checkBreakpoint(bp, proc)) return NULL;
  }

  bool had_error = false;
  while (!hw_bps.empty()) {
    set<response::ptr> resps;
    for (set<hw_breakpoint *>::iterator i = hw_bps.begin();
         i != hw_bps.end();) {
      hw_breakpoint *bp = *i;
      bool done = false;
...
async_ret_t hw_breakpoint::uninstall(int_process *proc,
                                     set<response::ptr> &resps) {
  bool done = false;
  ...
  if (ibp == thr->hw_breakpoints.end()) {
    perr_printf(
        "Tried to uninstall non-installed hardware breakpoint on %d/%d\n",
        thr->llproc()->getPid(), thr->getLWP());
    proc->setLastError(
        err_badparam, "Tried to uninstall non-installed hardware breakpoint\n");
    return aret_error;
  }
...
unsigned int hw_breakpoint::getPerms() const { return hw_perms; }

unsigned int hw_breakpoint::getSize() const { return hw_size; }

int_thread *hw_breakpoint::getThread() const { return thr; }

bool hw_breakpoint::needsClear() { return thr->bpNeedsClear(this); }
...
std::string int_library::getName() { return name; }

std::string int_library::getAbsName() {
  if (!abs_name.empty()) return abs_name;
  abs_name = int_process::plat_canonicalizeFileName(name);
  return abs_name;
}

Dyninst::Address int_library::getAddr() { return load_address; }

Dyninst::Address int_library::getDataAddr() { return data_load_address; }

bool int_library::hasDataAddr() { return has_data_load; }

Dyninst::Address int_library::getDynamicAddr() { return dynamic_address; }

void int_library::setLoadAddress(Address addr) { load_address = addr; }

void int_library::setDynamicAddress(Address addr) { dynamic_address = addr; }

Address int_library::mapAddress() { return sysv_map_address; }

void int_library::setMapAddress(Address a) { sysv_map_address = a; }

bool int_library::inProcess(int_process *p) { return (p->memory() == memory); }

bool int_library::isSharedLib() const { return is_shared_lib; }

void int_library::setMark(bool b) { marked = b; }

bool int_library::isMarked() const { return marked; }

void int_library::setUserData(void *d) { user_data = d; }

void *int_library::getUserData() { return user_data; }

Library::ptr int_library::getUpPtr() const { return up_lib; }
...
mem_state::mem_state(int_process *proc) { procs.insert(proc); }
...
void mem_state::addProc(int_process *p) {
  pthrd_printf(
      "Adding process %d as sharing a memory state with existing proc\n",
      p->getPid());
  procs.insert(p);
}
...
void mem_state::rmLibrary(int_library *lib) {
  ...
  if (i == libs.end()) return;
  libs.erase(i);
  lib->memory = NULL;
}

int_notify *int_notify::the_notify = NULL;
int_notify::int_notify() : events_noted(0) {
  the_notify = this;
  up_notify.llnotify = this;
}

int_notify *notify() {
  if (int_notify::the_notify) return int_notify::the_notify;
...
void int_notify::noteEvent() {
  // MATT TODO lock around event pipe write/read when/if we move to process
  // locks
  assert(isHandlerThread());
  if (events_noted == 0) my_internals.noteEvent();
  events_noted++;
...
static void notifyNewEvent() { notify()->noteEvent(); }

void int_notify::clearEvent() {
  assert(!isHandlerThread());
  events_noted--;
  pthrd_printf("clearEvent - %d\n", events_noted);
  if (events_noted == 0) my_internals.clearEvent();
}

bool int_notify::hasEvents() { return (events_noted > 0); }

void int_notify::registerCB(EventNotify::notify_cb_t cb) { cbs.insert(cb); }
...
RegisterPool::RegisterPool() { llregpool = new int_registerPool(); }
...
RegisterPool::~RegisterPool() { delete llregpool; }
...
size_t RegisterPool::size() const { return llregpool->regs.size(); }

Thread::ptr RegisterPool::getThread() { return llregpool->thread->thread(); }
...
RegisterPool::iterator::iterator(int_iter i_) : i(i_) {}

RegisterPool::iterator::~iterator() {}

std::pair<Dyninst::MachRegister, Dyninst::MachRegisterVal>
...
RegisterPool::const_iterator::const_iterator(int_iter i_) : i(i_) {}
...
RegisterPool::const_iterator RegisterPool::const_iterator::
operator++()  // prefix
{
  i++;
  return *this;
...
int_registerPool::int_registerPool() : full(false), thread(NULL) {
...
int_registerPool::int_registerPool(const int_registerPool &c)
    : regs(c.regs), full(c.full), thread(c.thread) {}
...
bool Library::isSharedLib() const { return lib->isSharedLib(); }

void *Library::getData() const { return lib->getUserData(); }

void Library::setData(void *p) const { lib->setUserData(p); }

LibraryPool::LibraryPool() : proc(NULL) {}
...
Library::const_ptr LibraryPool::getLibraryByName(std::string s) const {
  ...
  int_library *int_lib = proc->getLibraryByName(s);
  if (!int_lib) return Library::const_ptr();
  return int_lib->up_lib;
}
...
bool LibraryPool::const_iterator::operator==(
    const LibraryPool::const_iterator &i) {
  return int_iter == i.int_iter;
}
...
bool Process::handleEvents(bool block) {
  ...
  if (int_process::isInCB()) {
    ...
    ProcControlAPI::globalSetLastError(err_incallback,
                                       "Cannot handleEvents from callback\n");
    return false;
  }
  ...
  if (!result) {
    if (!block && ProcControlAPI::getLastError() == err_noevents)
      pthrd_printf(
          "Polling Process::handleEvents returning false due to no events\n");
    else
...
Process::ptr Process::createProcess(std::string executable,
  ...
  if (int_process::isInCB()) {
    ...
    ProcControlAPI::globalSetLastError(err_incallback,
                                       "Cannot createProcess from callback\n");
    return Process::ptr();
  }
...
Process::ptr Process::attachProcess(Dyninst::PID pid, std::string executable) {
  ...
  if (int_process::isInCB()) {
    perr_printf(
        "User attempted call on process attach while in CB, erroring.\n");
    ProcControlAPI::globalSetLastError(err_incallback,
                                       "Cannot attachProcess from callback\n");
    return Process::ptr();
  }
...
Process::Process() : llproc_(NULL), exitstate_(NULL) {}
...
bool Process::addLibrary(std::string library) {
  ...
  if (hasRunningThread()) {
    ...
    setLastError(err_notstopped,
                 "Attempted to addLibrary on running process\n");
    return false;
  }
...
bool Process::isCrashed() const {
  ...
  return (llproc_->getState() == int_process::exited &&
          llproc_->getCrashSignal(crashSignal));
}
...
int Process::getCrashSignal() const {
  ...
  int crashSignal = 0;
  if (!(llproc_->getState() == int_process::exited &&
        llproc_->getCrashSignal(crashSignal)))
    return 0;
  return crashSignal;
}
...
bool Process::hasStoppedThread() const {
  ...
  int_threadPool::iterator i;
  for (i = llproc_->threadPool()->begin(); i != llproc_->threadPool()->end();
       i++) {
    if ((*i)->getUserState().getState() == int_thread::stopped) return true;
  }
  return false;
}
...
bool Process::hasRunningThread() const {
  ...
  for (i = llproc_->threadPool()->begin(); i != llproc_->threadPool()->end();
    ...
    if ((*i)->getUserState().getState() == int_thread::running) return true;
  }
  return false;
}
...
bool Process::allThreadsStopped() const {
  ...
  for (i = llproc_->threadPool()->begin(); i != llproc_->threadPool()->end();
    ...
    if ((*i)->getUserState().getState() == int_thread::running ||
        (*i)->getUserState().getState() == int_thread::detached)
      return false;
  }
  return true;
}
...
bool Process::runIRPCSync(IRPC::ptr irpc) {
  ...
  while (irpc->state() != IRPC::Done) {
    ...
    if (thr && thr->isStopped(int_thread::UserStateID)) {
      pthrd_printf(
          "RPC thread %d/%d was stopped during runIRPCSync, returning "
          "notrunning error\n",
          proc->getPid(), thr->getLWP());
      setLastError(err_notrunning,
                   "No threads are running to produce events\n");
      return false;
    }
...
bool Process::getPostedIRPCs(std::vector<IRPC::ptr> &rpcs) const {
  ...
  for (int_threadPool::iterator i = tp->begin(); i != tp->end(); ++i) {
    ...
    for (rpc_list_t::iterator j = rpc_list->begin(); j != rpc_list->end();
         ++j) {
      IRPC::ptr up_rpc = (*j)->getIRPC().lock();
      if (up_rpc == IRPC::ptr()) continue;
...
bool Process::supportsLWPEvents() const {
  ...
  // Intentionally not testing plat_supportLWP*Destroy, which is complicated on
  // BG
...
bool Process::writeMemory(Dyninst::Address addr, const void *buffer,
                          size_t size) const {
  MTLock lock_this_func;
  PROC_EXIT_DETACH_TEST("writeMemory", false);

  pthrd_printf(
      "User wants to write memory to remote addr 0x%lx from buffer 0x%p of "
      "size %lu\n",
...
bool Process::readMemory(void *buffer, Dyninst::Address addr,
                         size_t size) const {
  MTLock lock_this_func;
...
bool Process::writeMemoryAsync(Dyninst::Address addr, const void *buffer,
                               size_t size, void *opaque_val) const {
  MTLock lock_this_func;
  PROC_EXIT_DETACH_TEST("writeMemoryAsync", false);
  pthrd_printf(
      "User wants to async write memory to remote addr 0x%lx from buffer 0x%p "
      "of size %lu\n",
      addr, buffer, (unsigned long)size);
  result_response::ptr resp = result_response::createResultResponse();
  int_eventAsyncIO *iev =
      new int_eventAsyncIO(resp, int_eventAsyncIO::memwrite);
...
bool Process::readMemoryAsync(void *buffer, Dyninst::Address addr, size_t size,
                              void *opaque_val) const {
  MTLock lock_this_func;
  PROC_EXIT_DETACH_TEST("readMemoryAsync", false);

  pthrd_printf(
      "User wants to async read memory from 0x%lx to 0x%p of size %lu\n", addr,
      buffer, (unsigned long)size);

  mem_response::ptr memresult =
      mem_response::createMemResponse((char *)buffer, size);
  int_eventAsyncIO *iev =
      new int_eventAsyncIO(memresult, int_eventAsyncIO::memread);
...
bool Process::findAllocatedRegionAround(Dyninst::Address addr,
  ...
  if (!llproc_->findAllocatedRegionAround(addr, memRegion)) {
    pthrd_printf(
        "Error to find Allocated Region contains %lx on target process %d\n",
        addr, llproc_->getPid());
    return false;
  }
...
bool Process::addBreakpoint(Address addr, Breakpoint::ptr bp) const {
  ...
  if (hasRunningThread()) {
    ...
    setLastError(err_notstopped,
                 "Attempted to insert breakpoint into running process\n");
    return false;
  }

  if (llproc_->getState() == int_process::detached) {
    perr_printf("User attempted to add breakpoint to detached process\n");
    setLastError(err_detached,
                 "Attempted to insert breakpoint into detached process\n");
    return false;
  }
...
bool Process::rmBreakpoint(Dyninst::Address addr, Breakpoint::ptr bp) const {
  ...
  if (hasRunningThread()) {
    ...
    setLastError(err_notstopped,
                 "Attempted to remove breakpoint on running process\n");
    return false;
  }
...
Thread::Thread() : llthread_(NULL), exitstate_(NULL) {}
...
void *Thread::getData() const {
  MTLock lock_this_func;
  if (exitstate_) return exitstate_->user_data;
...
int_thread *Thread::llthrd() const { return llthread_; }
...
bool Thread::stopThread() {
  ...
  if (!result) {
    perr_printf(
        "Thread %d/%d was not in a stoppable state, error return from "
        "setState\n",
        proc->getPid(), thrd->getLWP());
    setLastError(err_internal,
                 "Could not set user state while stopping thread\n");
    return false;
  }
  proc->throwNopEvent();
  ...
  if (proc_exited) {
    perr_printf(
        "Process exited while waiting for user thread stop, erroring\n");
  ...
  if (!result) {
    perr_printf("Internal error calling waitAndHandleForProc on %d\n",
                proc->getPid());
    setLastError(err_internal,
                 "Error while calling waitAndHandleForProc from thread stop\n");
    return false;
  }
  return true;
}
...
bool Thread::continueThread() {
  ...
  if (!result) {
    perr_printf(
        "Thread %d/%d was not in a continuable state, error return from "
        "setState\n",
        proc->getPid(), thrd->getLWP());
    setLastError(err_internal,
                 "Could not set user state while continuing thread\n");
    return false;
  }
  proc->throwNopEvent();
  return true;
...
bool Thread::getAllRegisters(RegisterPool &pool) const {
  ...
  allreg_response::ptr response =
      allreg_response::createAllRegResponse(pool.llregpool);
...
bool Thread::getRegister(Dyninst::MachRegister reg,
...
bool Thread::setRegister(Dyninst::MachRegister reg,
                         Dyninst::MachRegisterVal val) const {
  MTLock lock_this_func;
...
bool Thread::getAllRegistersAsync(RegisterPool &pool, void *opaque_val) const {
  ...
  allreg_response::ptr response =
      allreg_response::createAllRegResponse(pool.llregpool);
  int_eventAsyncIO *iev =
      new int_eventAsyncIO(response, int_eventAsyncIO::regallread);
  iev->opaque_value = opaque_val;
  iev->rpool = &pool;
...
bool Thread::setAllRegistersAsync(RegisterPool &pool, void *opaque_val) const {
  ...
  result_response::ptr response = result_response::createResultResponse();
  int_eventAsyncIO *iev =
      new int_eventAsyncIO(response, int_eventAsyncIO::regallwrite);
  iev->opaque_value = opaque_val;
  response->setAsyncIOEvent(iev);
  bool result = llthread_->setAllRegisters(*pool.llregpool, response);
  if (!result) {
...
bool Thread::readThreadLocalMemory(void *buffer, Library::const_ptr lib,
  ...
  if (!intlib || !intlib->inProcess(llproc)) {
    ...
    setLastError(err_badparam,
                 "Library object is not loaded in specified process\n");
    return false;
  }

  pthrd_printf(
      "User wants to read TLS memory on thread %d/%d from library %s at offset "
      "%lu of size %lu\n",
  ...
  mem_response::ptr memresp =
      mem_response::createMemResponse((char *)buffer, size);
  bool result = llproc->readMem(var_address, memresp, llthrd);
  if (result) llproc->waitForAsyncEvent(memresp);
...
bool Thread::writeThreadLocalMemory(Library::const_ptr lib,
                                    Dyninst::Offset tls_symbol_offset,
  ...
  pthrd_printf(
      "User wants to write to TLS memory on thread %d/%d in library %s at "
      "offset %lu of size %lu\n",
      llproc->getPid(), llthrd->getLWP(), lib->getName().c_str(),
      (unsigned long)tls_symbol_offset, (unsigned long)size);
  ...
  if (result) llproc->waitForAsyncEvent(resp);
...
bool Thread::getThreadLocalAddress(Library::const_ptr lib,
                                   Dyninst::Offset tls_symbol_offset,
  ...
  pthrd_printf(
      "User wants to get TLS address on thread %d/%d in library %s at offset "
      "%lu\n",
      llproc->getPid(), llthrd->getLWP(), lib->getName().c_str(),
...
IRPC::const_ptr Thread::getRunningIRPC() const {
  ...
  if (running == int_iRPC::ptr()) return IRPC::const_ptr();
...
Thread::ptr ThreadPool::iterator::operator*() const {
  ...
  // !curh->llthrd()) ); //Likely dereferenced bad thread iterator
  return curh;
}
...
int EventNotify::getFD() { return (int)(llnotify->getWaitable()); }

void EventNotify::registerCB(notify_cb_t cb) {
  return llnotify->registerCB(cb);
}

void EventNotify::removeCB(notify_cb_t cb) { return llnotify->removeCB(cb); }
...
int_breakpoint *Breakpoint::llbp() const { return llbreakpoint_; }
...
void *Breakpoint::getData() const { return llbreakpoint_->getData(); }

void Breakpoint::setData(void *d) const { llbreakpoint_->setData(d); }
...
Counter::Counter(CounterType ct_) : local_count(0), ct(ct_) {
...
void Counter::inc() { adjust(1); }

void Counter::dec() { adjust(-1); }

bool Counter::local() const { return localCount() != 0; }

int Counter::localCount() const { return local_count; }

bool Counter::global(CounterType ct) { return globalCount(ct) != 0; }
...
Process::thread_mode_t MTManager::getThreadMode() { return threadMode; }
...
bool MTManager::setThreadMode(Process::thread_mode_t tm, bool init) {
  if (ProcPool()->numProcs()) {
    ...
    ProcControlAPI::globalSetLastError(
        err_noproc, "Can't setThreadMode while processes are running\n");
    return false;
  }
  ...
  switch (tm) {
    ...
    default:
      ...
      ProcControlAPI::globalSetLastError(
          err_badparam, "Invalid parameter to setThreadMode\n");
      return false;
  }

  if ((tm == Process::NoThreads || tm == Process::GeneratorThreading) &&
      (!init) && (threadMode == Process::HandlerThreading ||
                  threadMode == Process::CallbackThreading)) {
    pthrd_printf(
        "New state does not use handler threading, stopping handler thread\n");
    stop();
  }
  if ((tm == Process::HandlerThreading || tm == Process::CallbackThreading) &&
      (init || (threadMode == Process::NoThreads ||
                threadMode == Process::GeneratorThreading))) {
  ...
  if (tm == Process::GeneratorThreading) {
    pthrd_printf(
        "Moving to generator threading, registering notification cb\n");
    Generator::registerNewEventCB(notifyNewEvent);
  }
  if (!init && threadMode == Process::GeneratorThreading) {
    pthrd_printf(
        "Moving away from generator threading, removing notification cb\n");
...
void MTManager::eventqueue_cb_wrapper() { mt()->eventqueue_cb(); }
...
void MTManager::startWork() { work_lock.lock(); }
...
ProcStopEventManager::ProcStopEventManager(int_process *p) : proc(p) {}
...
void ProcStopEventManager::checkEvents() {
  for (set<Event::ptr>::iterator i = held_pstop_events.begin();
       i != held_pstop_events.end();) {
    Event::ptr ev = *i;
    if (ev->procStopper()) {
      i++;
      continue;
...
emulated_singlestep::emulated_singlestep(int_thread *thr_) : thr(thr_) {
...
async_ret_t emulated_singlestep::add(Address addr) {
  if (addrs.find(addr) != addrs.end()) return aret_success;
...
async_ret_t emulated_singlestep::clear() {
  ...
  for (set<response::ptr>::iterator i = clear_resps.begin();
       i != clear_resps.end();) {
    response::ptr resp = *i;
    if (resp->isReady()) {
      clear_resps.erase(i++);
...
void int_thread::terminate() { plat_terminate(); }

void int_thread::plat_terminate() { assert(0 && "Unimplemented!"); }
