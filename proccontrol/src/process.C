...
void
Process::version(int& major, int& minor, int& maintenance)
{
...
bool
int_process::create(int_processSet* ps)
{
...
void
int_process::plat_threadAttachDone()
{}

bool
int_process::attachThreads(bool& found_new_threads)
{
...
bool
int_process::plat_attachThreadsSync()
{
...
bool
int_process::attach(int_processSet* ps, bool reattach)
{
    ...
    for(set<int_process*>::iterator i = procs.begin(); i != procs.end();)
    {
        ...
        // Keep track of the initial running states for each thread.  We'll fill them in
        // latter
        ...
        if(reattach)
        {
            ...
            for(int_threadPool::iterator j = tp->begin(); j != tp->end(); j++)
            {
                ...
                if(thr->getDetachState().getState() != int_thread::detached)
                {
                    ...
                    // detached so that it gets properly restored with the rest.
                ...
                if(findIter == runningStates.end() || !thr->attach())
                {
                    ...
                    {
                        destroyEv =
                            Event::ptr(new EventUserThreadDestroy(EventType::Post));
                    }
                    else
                    {
                        perr_printf("Platform does not support any thread destroy "
                                    "events.  Now what?\n");
                        assert(0);
                    }
    ...
    // Some OSs need to do their attachThreads here.  Since the operation is supposed to
    // be idempotent after success, then just do it again.
    for(set<int_process*>::iterator i = procs.begin(); i != procs.end();)
    {
        int_process* proc = *i;
        if(proc->getState() == errorstate)
...
bool
int_process::reattach(int_processSet* pset)
{
...
async_ret_t
int_process::initializeAddressSpace(std::set<response::ptr>& async_responses)
{
...
async_ret_t
int_process::post_attach(bool, std::set<response::ptr>& async_responses)
{
...
async_ret_t
int_process::post_create(std::set<response::ptr>& async_responses)
{
...
bool
int_process::plat_processGroupContinues()
{
    return false;
}

bool
int_process::getThreadLWPs(std::vector<Dyninst::LWP>&)
{
    return false;
}

const char*
int_process::stateName(int_process::State s)
{
    switch(s)
    {
        case neonatal:
...
void
int_process::setState(int_process::State s)
{
...
int_process::State
int_process::getState() const
{
    return state;
}

int_process::creationMode_t
int_process::getCreationMode() const
{
    return creation_mode;
}

void
int_process::setContSignal(int sig)
{
    continueSig = sig;
}

int
int_process::getContSignal() const
{
    return continueSig;
}

void
int_process::setPid(Dyninst::PID p)
{
...
Dyninst::PID
int_process::getPid() const
{
    return pid;
}

int_threadPool*
int_process::threadPool() const
{
    return threadpool;
}
...
bool
int_process::waitForAsyncEvent(response::ptr resp)
{
    if(resp)
    {
...
bool
int_process::waitForAsyncEvent(std::set<response::ptr> resp)
{
...
Counter&
int_process::asyncEventCount()
{
    return async_event_count;
}

Counter&
int_process::getForceGeneratorBlockCount()
{
    return force_generator_block_count;
}

Counter&
int_process::getStartupTeardownProcs()
{
    return startupteardown_procs;
}

bool
int_process::plat_waitAndHandleForProc()
{
    return true;
}
...
bool
int_process::waitAndHandleForProc(bool block, int_process* proc, bool& proc_exited)
{
...
#define checkStartupTeardownProcs                                                        \
    (hasStartupTeardownProc = (int) Counter::global(Counter::StartupTeardownProcesses))
...
int_process::waitAndHandleEvents(bool block)
{
    ...
    for(;;)
    {
         ...
         * each check, which is used if debug printing is turned on, while also letting
         *the C++ partial evaluator not check parts this expression (as the checks involve
         *an expensive locking operation).
         *
         * For most people, just ignore the #defines.  The important bit is that we're
         *really calling the Counter::global(...) function to determine whether we should
         *block.
         **/
        ...
        bool should_block =
            (!checkHandlerThread &&
             ((checkBlock && !checkGotEvent && checkRunningThread) ||
              (checkSyncRPCRunningThrd) || (checkStopPending) || (checkClearingBP) ||
              (checkProcStopRPC) || (checkAsyncPending) || (checkStartupTeardownProcs) ||
              (checkNeonatalThreads) || (checkAsyncEvents)));
        // Entry for this print match the above tests in order and one-for-one.
        pthrd_printf("%s for events = !%c && ((%c && !%c && %c) || %c || %c || %c || %c "
                     "|| %c || %c || %c || %c)\n",
        ...
        //      and the below dequeue.  Perhaps dequeue should alway poll, and the user
        //      thread loops over it while (should_block == true), with a condition
        //      variable signaling when the should_block would go to false (so we don't
        //      just spin).
        ...
        if(terminating)
        {
            if(!exitEvent || !llproc)
            {
                ...
                pthrd_printf("Received event %s on terminated process, ignoring\n",
                             ev->name().c_str());
                if(!isHandlerThread() && ev->noted_event)
                    notify()->clearEvent();
                continue;
            }
        }
...
void
int_process::throwDetachEvent(bool temporary, bool leaveStopped)
{
...
bool
int_process::plat_detachDone()
{
    return true;
}

bool
int_process::terminate(bool& needs_sync)
{
    ...
    // The following code is similar to GeneratorWindows::hasLiveProc. This is not a
    // coincidence.
    bool all_stopped = true;
    for(int_threadPool::iterator iter = threadpool->begin(); iter != threadpool->end();
        ++iter)
    {
...
bool
int_process::preTerminate()
{
    return true;
}

int_libraryTracking*
int_process::getLibraryTracking()
{
...
int_callStackUnwinding*
int_process::getCallStackUnwinding()
{
...
int_multiToolControl*
int_process::getMultiToolControl()
{
...
void
int_process::initializeProcess(Process::ptr p)
{
    assert(!p->llproc_);
    p->llproc_   = this;
    up_proc      = p;
...
bool
int_process::readMem(Dyninst::Address remote, mem_response::ptr result, int_thread* thr)
{
    ...
    if(!plat_needsAsyncIO())
    {
        pthrd_printf("Reading from remote memory %lx to %p, size = %lu on %d/%d\n",
                     remote, (void*) result->getBuffer(),
                     (unsigned long) result->getSize(), getPid(),
        ...
        {
            pthrd_printf(
                "Enqueueing new EventAsyncRead into mailbox on synchronous platform\n");
...
bool
int_process::writeMem(const void* local, Dyninst::Address remote, size_t size,
                      result_response::ptr result, int_thread* thr, bp_write_t bp_write)
{
    if(getAddressWidth() == 4)
    {
        Address old = remote;
    ...
    if(!plat_needsAsyncIO())
    {
        ...
        {
            pthrd_printf(
                "Enqueueing new EventAsyncWrite into mailbox on synchronous platform\n");
...
unsigned
int_process::plat_getRecommendedReadSize()
{
    return getTargetPageSize();
}

Dyninst::Address
int_process::mallocExecMemory(unsigned size)
{
...
void
int_process::freeExecMemory(Dyninst::Address addr)
{
    std::map<Dyninst::Address, unsigned>::iterator i;
...
Dyninst::Address
int_process::direct_infMalloc(unsigned long, bool, Dyninst::Address)
{
    assert(0);
    return 0;
}
...
Address
int_process::infMalloc(unsigned long size, bool use_addr, Address addr)
{
    int_addressSet as;
...
bool
int_process::infFree(Address addr)
{
    int_addressSet as;
    as.insert(make_pair(addr, proc()));
...
bool
int_process::infMalloc(unsigned long size, int_addressSet* aset, bool use_addr)
{
    bool                                   had_error = false;
    ...
    for(int_addressSet::iterator i = aset->begin(); i != aset->end(); i++)
    {
        ...
        {
            // Platforms like Windows don't need IRPCs for allocation.  Just do it and put
            // the results
...
bool
int_process::infFree(int_addressSet* aset)
{
    bool                                   had_error = false;
    ...
    for(set<pair<Process::ptr, int_iRPC::ptr>>::iterator i = active_frees.begin();
...
bool
int_process::plat_decodeMemoryRights(Process::mem_perm& rights_internal,
...
bool
int_process::plat_encodeMemoryRights(Process::mem_perm rights_internal,
                                     unsigned long&    rights)
{
    (void) rights_internal;
    (void) rights;
...
bool
int_process::getMemoryAccessRights(Dyninst::Address addr, Process::mem_perm& rights)
{
...
bool
int_process::plat_getMemoryAccessRights(Dyninst::Address addr, Process::mem_perm& rights)
{
    (void) addr;
    (void) rights;
...
bool
int_process::setMemoryAccessRights(Dyninst::Address addr, size_t size,
...
bool
int_process::plat_setMemoryAccessRights(Dyninst::Address addr, size_t size,
...
bool
int_process::findAllocatedRegionAround(Dyninst::Address       addr,
...
bool
int_process::plat_findAllocatedRegionAround(Dyninst::Address       addr,
                                            Process::MemoryRegion& memRegion)
{
    (void) addr;
    memRegion.first  = 0;
...
void
int_process::setSymReader(SymbolReaderFactory* fact)
{
    symbol_reader = fact;
}

SymbolReaderFactory*
int_process::plat_defaultSymReader()
{
...
void
int_process::setForceGeneratorBlock(bool b)
{
    if(b)
    {
...
HandlerPool*
int_process::handlerPool() const
{
    return handlerpool;
}

bool
int_process::addBreakpoint_phase1(bp_install_state* is)
{
    is->ibp                                  = NULL;
...
bool
int_process::addBreakpoint_phase2(bp_install_state* is)
{
    if(!is->do_install)
        return true;
...
bool
int_process::addBreakpoint_phase3(bp_install_state* is)
{
    if(!is->do_install)
        return true;
...
bool
int_process::addBreakpoint(Dyninst::Address addr, int_breakpoint* bp)
{
    if(getState() != running)
    {
...
bool
int_process::removeBreakpoint(Dyninst::Address addr, int_breakpoint* bp,
...
sw_breakpoint*
int_process::getBreakpoint(Dyninst::Address addr)
{
...
int_library*
int_process::getLibraryByName(std::string s) const
{
    ...
    for(set<int_library*>::iterator i = mem->libs.begin(); i != mem->libs.end(); ++i)
    {
...
unsigned int
int_process::plat_getCapabilities()
{
...
Event::ptr
int_process::plat_throwEventsBeforeContinue(int_thread*)
{
    return Event::ptr();
}

bool
int_process::plat_threadOpsNeedProcStop()
{
    return false;
}

size_t
int_process::numLibs() const
{
    return mem->libs.size();
}

std::string
int_process::getExecutable() const
{
    return executable;  // The name of the exec passed to PC
}

bool
int_process::isInCallback()
{
    return in_callback;
}

mem_state::ptr
int_process::memory() const
{
    return mem;
}

err_t
int_process::getLastError()
{
    return last_error;
}

const char*
int_process::getLastErrorMsg()
{
    return last_error_string;
}

void
int_process::clearLastError()
{
    last_error        = err_none;
...
void
int_process::setLastError(err_t err, const char* str)
{
    last_error        = err;
...
void
int_process::setCrashSignal(int s)
{
    assert(!hasExitCode);
    hasCrashSignal = true;
    crashSignal    = s;
...
bool
int_process::getCrashSignal(int& s)
{
    s = crashSignal;
...
bool
int_process::wasForcedTerminated() const
{
    return forcedTermination;
}

bool
int_process::plat_individualRegRead(Dyninst::MachRegister, int_thread*)
{
    return plat_individualRegAccess();
}

bool
int_process::plat_individualRegSet()
{
    return plat_individualRegAccess();
}
...
async_ret_t
int_process::plat_calcTLSAddress(int_thread*, int_library*, Offset, Address&,
...
bool
int_process::plat_needsAsyncIO() const
{
    return false;
}

bool
int_process::plat_readMemAsync(int_thread*, Dyninst::Address, mem_response::ptr)
{
    assert(0);
    return false;
}
...
bool
int_process::plat_writeMemAsync(int_thread*, const void*, Dyninst::Address, size_t,
...
memCache*
int_process::getMemCache()
{
    return &mem_cache;
}

void
int_process::updateSyncState(Event::ptr ev, bool gen)
{
    ...
    switch(ev->getSyncType())
    {
        ...
        case Event::sync_thread: {
             ...
             * 	 I think it is possible for the generator to receive one sync event one a
             thread,
             * 	 and before the handler handles it and unset its status, the generator
             * 	 receives another sync event on the same thread.
             *
             * 	 This is because that even thought we attempt to stop the thread for a
             sync event,
             * 	 there is a gap between the sync event and the stop. There could be
             another sync
             * 	 event happening.
             *
                     assert(RUNNING_STATE(old_state) ||
                            thrd->llproc()->wasForcedTerminated() ||
                            (old_state == int_thread::stopped && (thrd->isExiting() ||
             thrd->isExitingInGenerator())));
            */
...
int_process::getProcStopManager()
{
    return proc_stop_manager;
}

bool
int_process::plat_supportThreadEvents()
{
    return false;
}

bool
int_process::plat_supportLWPCreate()
{
    return false;
}

bool
int_process::plat_supportLWPPreDestroy()
{
    return false;
}

bool
int_process::plat_supportLWPPostDestroy()
{
    return false;
}

bool
int_process::plat_supportHWBreakpoint()
{
    return false;
}

bool
int_process::plat_supportFork()
{
    return false;
}

bool
int_process::plat_supportExec()
{
    return false;
}

async_ret_t
int_process::plat_needsEmulatedSingleStep(int_thread*, std::vector<Address>&)
{
    return aret_success;
}

void
int_process::plat_getEmulatedSingleStepAsyncs(int_thread*, std::set<response::ptr>)
{
    assert(0);
}

bool
int_process::plat_needsPCSaveBeforeSingleStep()
{
    return false;
}

map<int, int>&
int_process::getProcDesyncdStates()
{
    return proc_desyncd_states;
}

bool
int_process::isRunningSilent()
{
    return silent_mode;
}

void
int_process::setRunningSilent(bool b)
{
    silent_mode = b;
}

bool
int_process::plat_supportDOTF()
{
    return true;
}
...
bool
int_process::plat_preHandleEvent()
{
    return true;
}

bool
int_process::plat_postHandleEvent()
{
    return true;
}

bool
int_process::plat_preAsyncWait()
{
    return true;
}
...
int_process::~int_process()
{
    ...
    if(ProcPool()->findProcByPid(getPid()))
        ProcPool()->rmProcess(this);
}

indep_lwp_control_process::indep_lwp_control_process(Dyninst::PID p, std::string e,
...
bool
indep_lwp_control_process::plat_syncRunState()
{
    for(int_threadPool::iterator i = threadPool()->begin(); i != threadPool()->end(); i++)
    {
        int_thread*       thr           = *i;
...
bool
unified_lwp_control_process::plat_syncRunState()
{
    ...
    pthrd_printf("In plat_syncRunState: want_all_stopped = %s, want_all_running = %s, "
...
bool
unified_lwp_control_process::plat_processGroupContinues()
{
    return true;
}
...
bool
hybrid_lwp_control_process::suspendThread(int_thread* thr)
{
    if(thr->isSuspended())
        return true;
...
bool
hybrid_lwp_control_process::resumeThread(int_thread* thr)
{
    if(!thr->isSuspended())
        return true;
...
bool
hybrid_lwp_control_process::plat_processGroupContinues()
{
    return true;
}

void
hybrid_lwp_control_process::noteNewDequeuedEvent(Event::ptr ev)
{
...
bool
hybrid_lwp_control_process::plat_debuggerSuspended()
{
    pthrd_printf("Querying, debugger stopped: %d (%p) (%p) (%d)\n", debugger_stopped,
                 (void*) &debugger_stopped, (void*) this, getPid());
    return debugger_stopped;
}

bool
hybrid_lwp_control_process::plat_syncRunState()
{
    ...
    {
        pthrd_printf("Process %d is not debugger suspended, but have changes.  Stopping "
                     "process.\n",
                     getPid());
        return a_running_thread->intStop();
    }

    // If we're here, we must be debuggerSuspended, and thus no threads are running
    // (!any_running)
    // since we didn't trigger the above if statement, we must have some thread we want to
    // run (any_target_running)
...
async_ret_t
int_thread::handleSingleStepContinue()
{
    async_ret_t      ret;
    ...
    for(set<int_thread*>::iterator i = thrds.begin(); i != thrds.end(); i++)
    {
        ...
        for(vector<Address>::iterator j = addrs.begin(); j != addrs.end(); j++)
        {
            ...
            {
                pthrd_printf(
                    "Async return while installing breakpoint for emulated_singlestep\n");
                ret = aret_async;
                goto done;
            }
            ...
            {
                pthrd_printf(
                    "Error return while installing breakpoint for emulated_singlestep\n");
                ret = aret_error;
                goto done;
            }
        }
...
bool
int_thread::isStopped(int state_id)
{
...
void
int_thread::setPendingStop(bool b)
{
...
bool
int_thread::hasPendingStop() const
{
    return pending_stop.local();
}

bool
int_thread::wasRunningWhenAttached() const
{
    return running_when_attached;
}

void
int_thread::setRunningWhenAttached(bool b)
{
    running_when_attached = b;
}

Process::ptr
int_thread::proc() const
{
    return proc_->proc();
}

int_process*
int_thread::llproc() const
{
    return proc_;
}

Dyninst::LWP
int_thread::getLWP() const
{
    return lwp;
}

void
int_thread::addContinueCB(continue_cb_t cb)
{
    continue_cbs.insert(cb);
}

void
int_thread::triggerContinueCBs()
{
    bool sync_conts = llproc()->plat_processGroupContinues();
    for(set<continue_cb_t>::iterator i = continue_cbs.begin(); i != continue_cbs.end();
        i++)
    {
        if(!sync_conts)
        {
...
int_thread::StateTracker&
int_thread::getPostponedSyscallState()
{
    return postponed_syscall_state;
}

int_thread::StateTracker&
int_thread::getBreakpointState()
{
    return breakpoint_state;
}

int_thread::StateTracker&
int_thread::getBreakpointHoldState()
{
    return breakpoint_hold_state;
}

int_thread::StateTracker&
int_thread::getBreakpointResumeState()
{
    return breakpoint_resume_state;
}

int_thread::StateTracker&
int_thread::getIRPCState()
{
    return irpc_state;
}

int_thread::StateTracker&
int_thread::getIRPCSetupState()
{
    return irpc_setup_state;
}

int_thread::StateTracker&
int_thread::getIRPCWaitState()
{
    return irpc_wait_state;
}

int_thread::StateTracker&
int_thread::getAsyncState()
{
    return async_state;
}

int_thread::StateTracker&
int_thread::getInternalState()
{
    return internal_state;
}

int_thread::StateTracker&
int_thread::getDetachState()
{
    return detach_state;
}

int_thread::StateTracker&
int_thread::getUserRPCState()
{
    return user_irpc_state;
}

int_thread::StateTracker&
int_thread::getControlAuthorityState()
{
    return control_authority_state;
}

int_thread::StateTracker&
int_thread::getUserState()
{
    return user_state;
}

int_thread::StateTracker&
int_thread::getHandlerState()
{
    return handler_state;
}

int_thread::StateTracker&
int_thread::getGeneratorState()
{
    return generator_state;
}

int_thread::StateTracker&
int_thread::getExitingState()
{
    return exiting_state;
}

int_thread::StateTracker&
int_thread::getStartupState()
{
    return startup_state;
}

int_thread::StateTracker&
int_thread::getCallbackState()
{
    return callback_state;
}

int_thread::StateTracker&
int_thread::getPendingStopState()
{
    return pending_stop_state;
}

int_thread::State
int_thread::getTargetState() const
{
    return target_state;
}

void
int_thread::setTargetState(State s)
{
...
int_thread::StateTracker&
int_thread::getStateByID(int id)
{
    switch(id)
    {
...
std::string
int_thread::stateIDToName(int id)
{
    switch(id)
    {
        case PostponedSyscallStateID:
...
const char*
int_thread::stateStr(int_thread::State s)
{
    switch(s)
    {
        case none:
...
char
int_thread::stateLetter(int_thread::State s)
{
    switch(s)
    {
        case none:
...
Counter&
int_thread::handlerRunningThreadsCount()
{
    return handler_running_thrd_count;
}

Counter&
int_thread::generatorRunningThreadsCount()
{
    return generator_running_thrd_count;
}

Counter&
int_thread::syncRPCCount()
{
    return sync_rpc_count;
}

Counter&
int_thread::runningSyncRPCThreadCount()
{
    return sync_rpc_running_thr_count;
}

Counter&
int_thread::pendingStopsCount()
{
    return pending_stop;
}

Counter&
int_thread::clearingBPCount()
{
    return clearing_bp_count;
}

Counter&
int_thread::procStopRPCCount()
{
    return proc_stop_rpc_count;
}

Counter&
int_thread::getGeneratorNonExitedThreadCount()
{
    return generator_nonexited_thrd_count;
}

Counter&
int_thread::neonatalThreadCount()
{
    return neonatal_threads;
}

Counter&
int_thread::pendingStackwalkCount()
{
    return pending_stackwalk_count;
}

void
int_thread::setContSignal(int sig)
{
    continueSig_ = sig;
}

int
int_thread::getContSignal()
{
    return continueSig_;
}

int_thread*
int_thread::createThread(int_process* proc, Dyninst::THR_ID thr_id, Dyninst::LWP lwp_id,
                         bool initial_thrd, attach_status_t astatus)
{
    ...
    pthrd_printf("Creating %s thread %d/%d, thr_id = 0x%lx\n",
                 initial_thrd ? "initial" : "new", proc->getPid(), newthr->getLWP(),
...
void
int_thread::changeLWP(Dyninst::LWP new_lwp)
{
...
bool
int_thread::suppressSanityChecks()
{
    return false;
}

bool
int_thread::isExiting() const
{
    return handler_exiting_state;
}

void
int_thread::setExiting(bool b)
{
    handler_exiting_state = b;
}

bool
int_thread::isExitingInGenerator() const
{
    return generator_exiting_state;
}

void
int_thread::setExitingInGenerator(bool b)
{
    generator_exiting_state = b;
}

void
int_thread::cleanFromHandler(int_thread* thrd, bool should_delete)
{
    ProcPool()->condvar()->lock();
...
bool
int_thread::getAllRegisters(allreg_response::ptr response)
{
    ...
    if(!llproc()->plat_needsAsyncIO())
    {
        ...
        *(response->getRegPool())      = cached_regpool;
        ...
        if(iev)
        {
            pthrd_printf("Enqueueing new EventAsyncReadAllRegs into mailbox on "
...
bool
int_thread::setAllRegisters(int_registerPool& pool, result_response::ptr response)
{
    ...
    if(!llproc()->plat_needsAsyncIO())
    {
        ...
        if(iev)
        {
            pthrd_printf("Enqueueing new EventAsyncSetAllRegs into mailbox on "
                         "synchronous platform\n");
...
bool
int_thread::getRegister(Dyninst::MachRegister reg, reg_response::ptr response)
{
...
bool
int_thread::setRegister(Dyninst::MachRegister reg, Dyninst::MachRegisterVal val,
...
void
int_thread::updateRegCache(int_registerPool& pool)
{
    regpool_lock.lock();
    pool.full      = true;
    cached_regpool = pool;
...
void
int_thread::updateRegCache(Dyninst::MachRegister reg, Dyninst::MachRegisterVal val)
{
    regpool_lock.lock();
...
bool int_thread::plat_getRegisterAsync(Dyninst::MachRegister, reg_response::ptr)
{
    assert(0);
    return false;
}
...
bool
int_thread::plat_setAllRegistersAsync(int_registerPool&, result_response::ptr)
{
    assert(0);
    return false;
}

bool int_thread::plat_setRegisterAsync(Dyninst::MachRegister, Dyninst::MachRegisterVal,
                                       result_response::ptr)
{
    assert(0);
    return false;
...
bool
int_thread::plat_handle_ghost_thread()
{
    return true;
}

void
int_thread::addPostedRPC(int_iRPC::ptr rpc_)
{
    assert(rpc_);
...
rpc_list_t*
int_thread::getPostedRPCs()
{
    return &posted_rpcs;
}

bool
int_thread::hasPostedRPCs()
{
    return (posted_rpcs.size() != 0);
}

void
int_thread::setRunningRPC(int_iRPC::ptr rpc_)
{
    assert(!running_rpc);
    running_rpc = rpc_;
    ...
    {
        pthrd_printf("Thread %d/%d is stopped on BP at 0x%lx and about to run RPC.  "
                     "Postponing BP clear.\n",
...
int_iRPC::ptr
int_thread::runningRPC() const
{
    return running_rpc;
}

void
int_thread::clearRunningRPC()
{
    if(postponed_stopped_on_breakpoint_addr)
    {
        pthrd_printf("Thread %d/%d is moving back to stopped breakpoint at %lx after "
                     "IRPC completion\n",
...
bool
int_thread::saveRegsForRPC(allreg_response::ptr response)
{
...
bool
int_thread::restoreRegsForRPC(bool clear, result_response::ptr response)
{
    assert(rpc_regs.full);
...
bool
int_thread::hasSavedRPCRegs()
{
    return rpc_regs.full;
}

void
int_thread::incSyncRPCCount()
{
...
bool
int_thread::hasSyncRPC()
{
    return sync_rpc_count.local();
}

int_iRPC::ptr
int_thread::nextPostedIRPC() const
{
    if(!posted_rpcs.size())
        return int_iRPC::ptr();
...
bool
int_thread::singleStepMode() const
{
    return single_step;
}

void
int_thread::setSingleStepMode(bool s)
{
    single_step = s;
}

bool
int_thread::singleStepUserMode() const
{
    return user_single_step;
}

void
int_thread::setSingleStepUserMode(bool s)
{
    user_single_step = s;
}

bool
int_thread::singleStep() const
{
    return single_step || user_single_step;
}

void
int_thread::setSyscallUserMode(bool s)
{
    user_syscall = s;
}

bool
int_thread::syscallUserMode() const
{
    return user_syscall;
}

bool
int_thread::syscallMode() const
{
    return user_syscall;
}

bool
int_thread::preSyscall()
{
...
void
int_thread::markStoppedOnBP(bp_instance* bp)
{
    stopped_on_breakpoint_addr = bp ? bp->getAddr() : 0x0;
}

bp_instance*
int_thread::isStoppedOnBP()
{
    if(!stopped_on_breakpoint_addr)
    {
        return NULL;
...
void
int_thread::setTID(Dyninst::THR_ID tid_)
{
    tid = tid_;
}

bp_instance*
int_thread::isClearingBreakpoint()
{
    return clearing_breakpoint;
}

bool
int_thread::haveUserThreadInfo()
{
    return false;
}

bool
int_thread::getTID(Dyninst::THR_ID&)
{
...
bool
int_thread::getStartFuncAddress(Dyninst::Address&)
{
...
bool
int_thread::getStackBase(Dyninst::Address&)
{
...
bool
int_thread::getStackSize(unsigned long&)
{
...
bool
int_thread::getTLSPtr(Dyninst::Address&)
{
...
Dyninst::Address
int_thread::getThreadInfoBlockAddr()
{
...
unsigned
int_thread::hwBPAvail(unsigned)
{
    return 0;
}

EventBreakpoint::ptr
int_thread::decodeHWBreakpoint(response::ptr&, bool, Dyninst::MachRegisterVal)
{
    return EventBreakpoint::ptr();
}

bool
int_thread::addHWBreakpoint(hw_breakpoint*, bool, std::set<response::ptr>&, bool&)
{
...
bool
int_thread::rmHWBreakpoint(hw_breakpoint*, bool, std::set<response::ptr>&, bool&)
{
    perr_printf("Tried to use hardware breakpoint on unsupported system\n");
    setLastError(err_unsupported,
                 "Hardware breakpoints not supported on this platform\n");
...
bool
int_thread::bpNeedsClear(hw_breakpoint*)
{
    return false;
}

void
int_thread::addEmulatedSingleStep(emulated_singlestep* es)
{
...
void
int_thread::rmEmulatedSingleStep(emulated_singlestep* es)
{
    (void) es;
...
emulated_singlestep*
int_thread::getEmulatedSingleStep()
{
    return em_singlestep;
}

void
int_thread::clearRegCache()
{
    regpool_lock.lock();
...
void
int_thread::StateTracker::desyncState(State ns)
{
    sync_level++;
    pthrd_printf("Desyncing %d/%d %s state to level %d\n", up_thr->llproc()->getPid(),
                 up_thr->getLWP(), getName().c_str(), sync_level);
    assert(id != int_thread::HandlerStateID && id != int_thread::GeneratorStateID &&
           id != int_thread::UserStateID);
...
void
int_thread::StateTracker::desyncStateProc(State ns)
{
...
void
int_thread::StateTracker::restoreState()
{
    sync_level--;
...
void
int_thread::StateTracker::restoreStateProc()
{
    int_threadPool* pool = up_thr->llproc()->threadPool();
    for(int_threadPool::iterator i = pool->begin(); i != pool->end(); i++)
    {
...
bool
int_thread::StateTracker::isDesynced() const
{
    return (sync_level != 0);
}

bool
int_thread::StateTracker::setState(State to)
{
     ...
     *
     ...
     generator_state == detached ); if (generator_state == running) assert(handler_state
     == running);
       }
    */
...
bool
int_thread::StateTracker::setStateProc(State ns)
{
    bool            had_error = false;
    int_threadPool* pool      = up_thr->llproc()->threadPool();
    for(int_threadPool::iterator i = pool->begin(); i != pool->end(); i++)
...
std::string
int_thread::StateTracker::getName() const
{
    return int_thread::stateIDToName(id);
}

int
int_thread::StateTracker::getID() const
{
    return id;
}

void
int_thread::setSuspended(bool b)
{
    suspended = b;
}

bool
int_thread::isSuspended() const
{
    return suspended;
}

void
int_thread::setLastError(err_t ec, const char* es)
{
    if(proc_)
...
hw_breakpoint*
int_thread::getHWBreakpoint(Address a)
{
...
int_thread*
int_threadPool::findThreadByLWP(Dyninst::LWP lwp)
{
...
int_thread*
int_threadPool::initialThread() const
{
    // if(!initial_thread) {
...
bool
int_threadPool::allHandlerStopped()
{
...
bool
int_threadPool::allStopped(int state_id)
{
    for(iterator i = begin(); i != end(); i++)
    {
...
bool
int_threadPool::hadMultipleThreads() const
{
    return had_multiple_threads;
}

void
int_threadPool::addThread(int_thread* thrd)
{
...
void
int_threadPool::rmThread(int_thread* thrd)
{
    ...
    // In particular this means we need to have a valid initial thread in the threadpool
    // after we finish removing this one, so that we have an arbitrary thread available
    // for continue calls.

    // FIXME: should probably kill most of the data duplication here. Log search for
    // thread by LWP is I guess legitimate, but there's AFAICT no reason for the initial
    // thread to be anything other than pinned to threads[0]. And the data duplication
    // involved in threads by LWP is not a good thing for consistency...
...
void
int_threadPool::noteUpdatedLWP(int_thread* thrd)
{
...
void
int_threadPool::setInitialThread(int_thread* thrd)
{
    initial_thread = thrd;
}

int_process*
int_threadPool::proc() const
{
    return proc_;
}

unsigned
int_threadPool::size() const
{
    return threads.size();
}

ThreadPool*
int_threadPool::pool() const
{
    return up_pool;
}

void
int_threadPool::saveUserState(Event::ptr ev)
{
...
void
int_threadPool::restoreUserState()
{
    for(iterator i = begin(); i != end(); i++)
    {
        int_thread* thr = *i;
...
bool
int_breakpoint::isCtrlTransfer() const
{
    return isCtrlTransfer_;
}

Address
int_breakpoint::toAddr() const
{
    return to;
}

void*
int_breakpoint::getData() const
{
    return data;
}

void
int_breakpoint::setData(void* v)
{
    data = v;
}

Breakpoint::weak_ptr
int_breakpoint::upBreakpoint() const
{
    return up_bp;
}

void
int_breakpoint::setThreadSpecific(Thread::const_ptr p)
{
    thread_specific.insert(p);
}

void
int_breakpoint::setOneTimeBreakpoint(bool b)
{
    onetime_bp = b;
}

void
int_breakpoint::markOneTimeHit()
{
    assert(onetime_bp);
    onetime_bp_hit = true;
}
...
bool
int_breakpoint::isOneTimeBreakpoint() const
{
    return onetime_bp;
}

bool
int_breakpoint::isOneTimeBreakpointHit() const
{
    return onetime_bp_hit;
}

bool
int_breakpoint::isThreadSpecific() const
{
    return !thread_specific.empty();
}

bool
int_breakpoint::isThreadSpecificTo(Thread::const_ptr p) const
{
    return thread_specific.find(p) != thread_specific.end();
}

void
int_breakpoint::setProcessStopper(bool b)
{
    procstopper = b;
}

bool
int_breakpoint::isProcessStopper() const
{
    return procstopper;
}

bool
int_breakpoint::isHW() const
{
    return hw;
}

unsigned
int_breakpoint::getHWSize() const
{
    return hw_size;
}

unsigned
int_breakpoint::getHWPerms() const
{
    return hw_perms;
}

void
int_breakpoint::setSuppressCallbacks(bool b)
{
    suppress_callbacks = b;
}

bool
int_breakpoint::suppressCallbacks() const
{
    return suppress_callbacks;
}

bool
int_breakpoint::isOffsetTransfer() const
{
...
bool
bp_instance::checkBreakpoint(int_breakpoint* bp, int_process* p)
{
...
bool
bp_instance::rmBreakpoint(int_process* proc, int_breakpoint* bp, bool& empty,
                          set<response::ptr>& resps)
{
    empty                            = false;
...
Address
bp_instance::getAddr() const
{
    return addr;
}

bp_instance::iterator
bp_instance::begin()
{
    return bps.begin();
}
...
bool
bp_instance::containsIntBreakpoint(int_breakpoint* bp)
{
    return (bps.count(bp) > 0);
}

int_breakpoint*
bp_instance::getCtrlTransferBP(int_thread* thrd)
{
    for(iterator i = begin(); i != end(); i++)
    {
        int_breakpoint* bp = *i;
...
sw_breakpoint*
sw_breakpoint::create(int_process* proc, int_breakpoint* bp, Dyninst::Address addr)
{
    bool             result;
    bp_install_state is;
    is.addr = addr;
...
bool
sw_breakpoint::writeBreakpoint(int_process* proc, result_response::ptr write_response_)
{
    ...
    return proc->writeMem(bp_insn, addr, buffer_size, write_response_, NULL,
                          int_process::bp_install);
}

bool
sw_breakpoint::saveBreakpointData(int_process* proc, mem_response::ptr read_response_)
{
    if(buffer_size != 0)
    {
        return true;
...
bool
sw_breakpoint::restoreBreakpointData(int_process* proc, result_response::ptr res_resp)
{
    assert(buffer_size != 0);
...
async_ret_t
sw_breakpoint::uninstall(int_process* proc, set<response::ptr>& resps)
{
    assert(installed);
...
async_ret_t
sw_breakpoint::suspend(int_process* proc, set<response::ptr>& resps)
{
    if(suspend_common())
        return aret_success;
...
async_ret_t
sw_breakpoint::resume(int_process* proc, set<response::ptr>& resps)
{
    if(resume_common())
        return aret_success;
    result_response::ptr result_resp = result_response::createResultResponse();
...
unsigned
sw_breakpoint::getNumIntBreakpoints() const
{
    return bps.size();
}

bool
sw_breakpoint::addToIntBreakpoint(int_breakpoint* bp, int_process*)
{
...
bool
sw_breakpoint::prepBreakpoint(int_process* proc, mem_response::ptr mem_resp)
{
    assert(!prepped);
    assert(!installed);
...
bool
sw_breakpoint::insertBreakpoint(int_process* proc, result_response::ptr res_resp)
{
    assert(prepped);
    assert(!installed);
...
bool
sw_breakpoint::needsClear()
{
    return true;
}

hw_breakpoint::hw_breakpoint(int_thread* thread, unsigned mode, unsigned size, bool pwide,
...
bool
hw_breakpoint::install(bool& done, set<response::ptr>& resps)
{
...
hw_breakpoint*
hw_breakpoint::create(int_process* proc, int_breakpoint* bp, Dyninst::Address addr)
{
    ...
    for(set<int_thread*>::iterator i = thrds.begin(); i != thrds.end(); i++)
    {
        ...
        {
            perr_printf("Do not have space for new hardware breakpoint of mode 0%u on "
                        "thread %d/%d\n",
...
async_ret_t
hw_breakpoint::suspend(int_process*, set<response::ptr>& resps)
{
    if(suspend_common())
        return aret_success;
...
async_ret_t
hw_breakpoint::resume(int_process*, set<response::ptr>& resps)
{
    if(resume_common())
        return aret_success;
...
async_ret_t
hw_breakpoint::uninstall(int_process* proc, set<response::ptr>& resps)
{
    bool done = false;
    ...
    if(ibp == thr->hw_breakpoints.end())
    {
        ...
        proc->setLastError(err_badparam,
                           "Tried to uninstall non-installed hardware breakpoint\n");
        return aret_error;
    }
...
unsigned int
hw_breakpoint::getPerms() const
{
    return hw_perms;
}

unsigned int
hw_breakpoint::getSize() const
{
    return hw_size;
}

int_thread*
hw_breakpoint::getThread() const
{
    return thr;
}

bool
hw_breakpoint::needsClear()
{
    return thr->bpNeedsClear(this);
}

int_library::int_library(std::string n, bool shared_lib, Dyninst::Address load_addr,
                         Dyninst::Address dynamic_load_addr,
                         Dyninst::Address data_load_addr, bool has_data_load_addr)
: name(n)
...
Dyninst::Address
int_library::getDataAddr()
{
    return data_load_address;
}

bool
int_library::hasDataAddr()
{
    return has_data_load;
}

Dyninst::Address
int_library::getDynamicAddr()
{
    return dynamic_address;
}

void
int_library::setLoadAddress(Address addr)
{
    load_address = addr;
}

void
int_library::setDynamicAddress(Address addr)
{
    dynamic_address = addr;
}

Address
int_library::mapAddress()
{
    return sysv_map_address;
}

void
int_library::setMapAddress(Address a)
{
    sysv_map_address = a;
}

bool
int_library::inProcess(int_process* p)
{
    return (p->memory() == memory);
}

bool
int_library::isSharedLib() const
{
    return is_shared_lib;
}

void
int_library::setMark(bool b)
{
    marked = b;
}

bool
int_library::isMarked() const
{
    return marked;
}

void
int_library::setUserData(void* d)
{
    user_data = d;
}

void*
int_library::getUserData()
{
    return user_data;
}

Library::ptr
int_library::getUpPtr() const
{
    return up_lib;
}

void
int_library::markAsCleanable()
{
...
void
mem_state::addProc(int_process* p)
{
...
void
mem_state::rmProc(int_process* p, bool& should_clean)
{
...
void
mem_state::addLibrary(int_library* lib)
{
    libs.insert(lib);
    lib->memory = this;
}
...
void
mem_state::rmLibrary(int_library* lib)
{
...
bool
int_notify::hasEvents()
{
    return (events_noted > 0);
}

void
int_notify::registerCB(EventNotify::notify_cb_t cb)
{
    cbs.insert(cb);
}

void
int_notify::removeCB(EventNotify::notify_cb_t cb)
{
...
int_notify::details_t::wait_object_t
int_notify::getWaitable()
{
...
RegisterPool::iterator
RegisterPool::find(MachRegister r)
{
...
RegisterPool::const_iterator
RegisterPool::find(MachRegister r) const
{
...
MachRegisterVal& RegisterPool::operator[](MachRegister r) { return llregpool->regs[r]; }
...
RegisterPool::iterator
RegisterPool::iterator::operator++()  // prefix
{
    i++;
    return *this;
}
...
RegisterPool::iterator
RegisterPool::iterator::operator++(int)  // postfix
{
...
RegisterPool::const_iterator
RegisterPool::const_iterator::operator++()  // prefix
{
    i++;
    return *this;
}
...
RegisterPool::const_iterator
RegisterPool::const_iterator::operator++(int)  // postfix
{
...
void
regpoolClearOnCont(int_thread* thr)
{
...
Library::ptr LibraryPool::iterator::operator*() const { return (*int_iter)->up_lib; }

LibraryPool::iterator
LibraryPool::iterator::operator++()  // prefix
{
    ++int_iter;
    return *this;
}
...
LibraryPool::iterator
LibraryPool::iterator::operator++(int)  // postfix
{
...
LibraryPool::const_iterator
LibraryPool::const_iterator::operator++()  // prefix
{
    ++int_iter;
    return *this;
}
...
LibraryPool::const_iterator
LibraryPool::const_iterator::operator++(int)  // postfix
{
...
bool
Process::registerEventCallback(EventType evt, Process::cb_func_t cbfunc)
{
...
bool
Process::removeEventCallback(EventType evt, cb_func_t cbfunc)
{
    MTLock           lock_this_func(MTLock::allow_init);
    HandleCallbacks* cbhandler = HandleCallbacks::getCB();
...
bool
Process::removeEventCallback(EventType evt)
{
    MTLock           lock_this_func(MTLock::allow_init);
    HandleCallbacks* cbhandler = HandleCallbacks::getCB();
...
bool
Process::removeEventCallback(cb_func_t cbfunc)
{
    MTLock           lock_this_func(MTLock::allow_init);
    HandleCallbacks* cbhandler = HandleCallbacks::getCB();
...
bool
Process::handleEvents(bool block)
{
...
bool
Process::setThreadingMode(thread_mode_t tm)
{
    MTLock lock_this_func(MTLock::allow_init);
...
Process::ptr
Process::createProcess(std::string executable, const std::vector<std::string>& argv,
...
Process::ptr
Process::attachProcess(Dyninst::PID pid, std::string executable)
{
    MTLock lock_this_func(MTLock::allow_init, MTLock::deliver_callbacks);
...
bool
Process::addLibrary(std::string library)
{
    MTLock lock_this_func;
    if(!llproc_)
    {
...
bool
Process::runIRPCAsync(IRPC::ptr irpc)
{
    MTLock lock_this_func;
...
bool
Process::runIRPCSync(IRPC::ptr irpc)
{
    MTLock lock_this_func;
    ...
    while(irpc->state() != IRPC::Done)
    {
        ...
        {
            pthrd_printf("RPC thread %d/%d was stopped during runIRPCSync, returning "
                         "notrunning error\n",
                         proc->getPid(), thr->getLWP());
...
bool
Thread::runIRPCAsync(IRPC::ptr irpc)
{
    int_iRPC::ptr rpc = irpc->llrpc()->rpc;
...
bool
Thread::runIRPCSync(IRPC::ptr irpc)
{
    int_iRPC::ptr rpc = irpc->llrpc()->rpc;
    rpc->setThread(llthrd());
...
int
Process::mem_perm::permVal() const
{
    int tmp = 0;
...
bool
Process::mem_perm::operator<(const mem_perm& p) const
{
...
bool
Process::writeMemory(Dyninst::Address addr, const void* buffer, size_t size) const
{
    MTLock lock_this_func;
    PROC_EXIT_DETACH_TEST("writeMemory", false);

    pthrd_printf(
        "User wants to write memory to remote addr 0x%lx from buffer 0x%p of size %lu\n",
...
bool
Process::writeMemoryAsync(Dyninst::Address addr, const void* buffer, size_t size,
                          void* opaque_val) const
{
    MTLock lock_this_func;
    PROC_EXIT_DETACH_TEST("writeMemoryAsync", false);
    pthrd_printf("User wants to async write memory to remote addr 0x%lx from buffer 0x%p "
                 "of size %lu\n",
                 addr, buffer, (unsigned long) size);
    result_response::ptr resp = result_response::createResultResponse();
...
bool
Process::readMemoryAsync(void* buffer, Dyninst::Address addr, size_t size,
                         void* opaque_val) const
{
    MTLock lock_this_func;
...
bool
Process::getMemoryAccessRights(Dyninst::Address addr, mem_perm& rights)
{
    if(!llproc_)
    {
...
bool
Process::setMemoryAccessRights(Dyninst::Address addr, size_t size, mem_perm rights,
                               mem_perm& oldrights)
{
    MTLock lock_this_func;
    if(!llproc_)
...
bool
Process::findAllocatedRegionAround(Dyninst::Address addr, MemoryRegion& memRegion)
{
    if(!llproc_)
    {
...
bool
Process::addBreakpoint(Address addr, Breakpoint::ptr bp) const
{
    MTLock lock_this_func;
...
unsigned
Process::numHardwareBreakpointsAvail(unsigned mode)
{
    MTLock lock_this_func;
...
void
Process::setDefaultSymbolReader(SymbolReaderFactory* f)
{
...
void
Process::setSymbolReader(SymbolReaderFactory* f) const
{
    MTLock lock_this_func;
    if(!llproc_)
    {
...
void
Process::setLastError(err_t ec, const char* es) const
{
    MTLock lock_this_func;
    if(!llproc_)
    {
...
Thread::stopThread()
{
    ...
    {
        perr_printf(
            "Thread %d/%d was not in a stoppable state, error return from setState\n",
            proc->getPid(), thrd->getLWP());
...
Thread::continueThread()
{
    ...
    {
        perr_printf(
            "Thread %d/%d was not in a continuable state, error return from setState\n",
            proc->getPid(), thrd->getLWP());
...
bool
Thread::readThreadLocalMemory(void* buffer, Library::const_ptr lib,
                              Dyninst::Offset tls_symbol_offset, size_t size) const
{
    MTLock lock_this_func;
    ...
    pthrd_printf("User wants to read TLS memory on thread %d/%d from library %s at "
    ...
    {
        set<response::ptr> resps;
        ret = llproc->plat_calcTLSAddress(llthrd, intlib, tls_symbol_offset, var_address,
                                          resps);
        if(ret == aret_error)
        {
...
bool
Thread::writeThreadLocalMemory(Library::const_ptr lib, Dyninst::Offset tls_symbol_offset,
                               const void* buffer, size_t size) const
{
    ...
    pthrd_printf("User wants to write to TLS memory on thread %d/%d in library %s at "
                 "offset %lu of size %lu\n",
                 llproc->getPid(), llthrd->getLWP(), lib->getName().c_str(),
                 (unsigned long) tls_symbol_offset, (unsigned long) size);
...
bool
Thread::getThreadLocalAddress(Library::const_ptr lib, Dyninst::Offset tls_symbol_offset,
                              Dyninst::Address& result_addr) const
{
    ...
    pthrd_printf(
        "User wants to get TLS address on thread %d/%d in library %s at offset %lu\n",
        llproc->getPid(), llthrd->getLWP(), lib->getName().c_str(),
...
bool
Thread::setSyscallMode(bool s) const
{
    MTLock lock_this_func;
...
void
Thread::setLastError(err_t ec, const char* es) const
{
    if(!llthread_)
    {
        assert(exitstate_);
...
Thread::ptr ThreadPool::iterator::operator*() const
{
    MTLock lock_this_func;
    assert(curp);
    // assert(curi >= 0 && ((curi < (signed) curp->hl_threads.size()) || !curh->llthrd())
    // ); //Likely dereferenced bad thread iterator
    return curh;
}
...
ThreadPool::const_iterator
ThreadPool::const_iterator::operator++(int)  // postfix
{
    MTLock                     lock_this_func;
...
ThreadPool::begin() const
{
    ...
    if(!threadpool->hl_threads.empty())
    {
        while(i.curi < (int) threadpool->hl_threads.size() &&
              i.curp->hl_threads[i.curi] && !i.curp->hl_threads[i.curi]->isUser())
        {
            i.curi++;
        }
        i.curh = i.curp->hl_threads[i.curi];
...
void
EventNotify::registerCB(notify_cb_t cb)
{
    return llnotify->registerCB(cb);
}

void
EventNotify::removeCB(notify_cb_t cb)
{
    return llnotify->removeCB(cb);
}

EventNotify*
Dyninst::ProcControlAPI::evNotify()
{
...
Breakpoint::ptr
Breakpoint::newTransferBreakpoint(Dyninst::Address to)
{
    Breakpoint::ptr newbp = Breakpoint::ptr(new Breakpoint());
...
Breakpoint::ptr
Breakpoint::newTransferOffsetBreakpoint(signed long shift)
{
    Breakpoint::ptr newbp = Breakpoint::ptr(new Breakpoint());
...
Breakpoint::ptr
Breakpoint::newHardwareBreakpoint(unsigned int mode, unsigned int size)
{
    Breakpoint::ptr newbp = Breakpoint::ptr(new Breakpoint());
...
void
Breakpoint::setData(void* d) const
{
...
void
Breakpoint::setSuppressCallbacks(bool b)
{
...
int
Counter::localCount() const
{
    return local_count;
}

bool
Counter::global(CounterType ct)
{
    return globalCount(ct) != 0;
}

int
Counter::processCount(CounterType ct, int_process* p)
{
    int sum = 0;
    switch(ct)
    {
...
int
Counter::globalCount(CounterType ct)
{
...
#endif
const char*
Counter::getNameForCounter(int counter_type)
{
    switch(counter_type)
    {
...
bool
MTManager::handlerThreading()
{
    return (threadMode == Process::HandlerThreading ||
            threadMode == Process::CallbackThreading);
}

Process::thread_mode_t
MTManager::getThreadMode()
{
    return threadMode;
}

bool
MTManager::setThreadMode(Process::thread_mode_t tm, bool init)
{
    ...
    switch(tm)
    {
        ...
        default:
            ...
            ProcControlAPI::globalSetLastError(err_badparam,
                                               "Invalid parameter to setThreadMode\n");
            return false;
    }

    if((tm == Process::NoThreads || tm == Process::GeneratorThreading) && (!init) &&
       (threadMode == Process::HandlerThreading ||
        threadMode == Process::CallbackThreading))
    {
        pthrd_printf(
    ...
    if((tm == Process::HandlerThreading || tm == Process::CallbackThreading) &&
       (init ||
        (threadMode == Process::NoThreads || threadMode == Process::GeneratorThreading)))
    {
...
#if defined(os_windows)
unsigned long
MTManager::evhandler_main_wrapper(void*)
#else
void
MTManager::evhandler_main_wrapper(void*)
#endif
...
bool
ProcStopEventManager::prepEvent(Event::ptr ev)
{
    if(!ev->procStopper())
    {
...
void
ProcStopEventManager::checkEvents()
{
...
bool
ProcStopEventManager::processStoppedTo(int state_id)
{
    return proc->threadPool()->allStopped(state_id);
}

bool
ProcStopEventManager::threadStoppedTo(int_thread* thr, int state_id)
{
...
bool
emulated_singlestep::containsBreakpoint(Address addr) const
{
    return addrs.find(addr) != addrs.end();
}

async_ret_t
emulated_singlestep::add(Address addr)
{
...
void
emulated_singlestep::restoreSSMode()
{
...
void
int_thread::terminate()
{
    plat_terminate();
}

void
int_thread::plat_terminate()
{
