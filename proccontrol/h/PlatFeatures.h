...
class PC_EXPORT FollowFork {
 ...
 public:
  typedef enum {
    ...
    DisableBreakpointsDetach,  // Remove inherited breakpoints from children,
                               // then detach.
...
class PC_EXPORT CallStackCallback {
 ...
 public:
  ...
  virtual bool addStackFrame(Thread::ptr thr, Dyninst::Address ra,
                             Dyninst::Address sp, Dyninst::Address fp) = 0;
...
class PC_EXPORT MemoryUsageSet {
 ...
 protected:
  ...
  bool usedX(std::map<Process::const_ptr, unsigned long> &used,
             mem_usage_t mu) const;
...
class PC_EXPORT BGQData {
 ...
 public:
  ...
  bool getProcCoordinates(unsigned &a, unsigned &b, unsigned &c, unsigned &d,
                          unsigned &e, unsigned &t) const;
  ...
  bool getPersistantMemRange(Dyninst::Address &start,
