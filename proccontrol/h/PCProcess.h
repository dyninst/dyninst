...
class PC_EXPORT Breakpoint {
 ...
 public:
  ...
  static Breakpoint::ptr newHardwareBreakpoint(unsigned int mode,
                                               unsigned int size);
...
class PC_EXPORT Process : public boost::enable_shared_from_this<Process> {
 ...
 public:
  ...
  static Process::ptr createProcess(
      std::string executable, const std::vector<std::string> &argv,
      const std::vector<std::string> &envp = emptyEnvp,
      const std::map<int, int> &fds = emptyFDs);
  static Process::ptr attachProcess(Dyninst::PID pid,
  ...
  // cb_func_t really takes an 'Event::const_ptr' as parameter, but this
  // declaration
  // defines the shared_ptr declaration due to Event::const_ptr not being
  // defined yet.
  ...
  bool writeMemory(Dyninst::Address addr, const void *buffer,
                   size_t size) const;
  bool readMemory(void *buffer, Dyninst::Address addr, size_t size) const;

  bool writeMemoryAsync(Dyninst::Address addr, const void *buffer, size_t size,
                        void *opaque_val = NULL) const;
  bool readMemoryAsync(void *buffer, Dyninst::Address addr, size_t size,
                       void *opaque_val = NULL) const;

  /**
   * Currently Windows-only, needed for the test infrastructure but possibly
   *useful elsewhere
   **/
  ...
  bool findAllocatedRegionAround(Dyninst::Address addr,
...
class PC_EXPORT Thread : public boost::enable_shared_from_this<Thread> {
 ...
 public:
  ...
  bool readThreadLocalMemory(void *buffer, Library::const_ptr lib,
                             Dyninst::Offset tls_symbol_offset,
                             size_t size) const;
  bool writeThreadLocalMemory(Library::const_ptr lib,
                              Dyninst::Offset tls_symbol_offset,
                              const void *buffer, size_t size) const;
  bool getThreadLocalAddress(Library::const_ptr lib, Dyninst::Offset tls_offset,
...
class PC_EXPORT RegisterPool {
 ...
 public:
  ...
  class PC_EXPORT iterator {
   ...
   private:
    typedef std::map<Dyninst::MachRegister, Dyninst::MachRegisterVal>::iterator
        int_iter;
    int_iter i;
  ...
  class PC_EXPORT const_iterator {
   ...
   private:
    typedef std::map<Dyninst::MachRegister,
                     Dyninst::MachRegisterVal>::const_iterator int_iter;
    int_iter i;
