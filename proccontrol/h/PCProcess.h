...
public:
    ...
    static IRPC::ptr createIRPC(void* binary_blob, unsigned size, Dyninst::Address addr,
                                bool non_blocking = false);
...
public:
    ...
    /**
     * Currently Windows-only, needed for the test infrastructure but possibly useful
     *elsewhere
     **/
    Dyninst::Address findFreeMemory(size_t size);

    bool getMemoryAccessRights(Dyninst::Address addr, mem_perm& rights);
    bool setMemoryAccessRights(Dyninst::Address addr, size_t size, mem_perm rights,
                               mem_perm& oldRights);
...
public:
    ...
    bool writeThreadLocalMemory(Library::const_ptr lib, Dyninst::Offset tls_symbol_offset,
...
public:
    ...
    private:
        typedef std::map<Dyninst::MachRegister, Dyninst::MachRegisterVal>::const_iterator
                 int_iter;
        int_iter i;
