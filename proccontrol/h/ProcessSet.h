...
public:
    ...
    static AddressSet::ptr newAddressSet(ProcessSet_ptr ps, std::string library_name,
                                         Dyninst::Offset off = 0);
     ...
     *myset.equal_range(0x1000); for (AddressSet::iterator i = range.first; i !=
     *range.second; i++) {
...
public:
    ...
    // These non-const factories may seem redundant, but gcc 4.1 gets confused without
    // them
    ...
    /**
     * Read/Write memory
     * - Use the AddressSet forms to read/write from the same memory location in each
     *process
     * - Use the read_t/write_t forms to read/write from different memory locations/sizes
     *in each process
     * - The AddressSet forms of readMemory need to have their memory free'd by the user.
     * - The readMemory form that outputs 'std::map<void *, ProcessSet::ptr> &result'
     *groups processes based on having the same memory contents.
     **/
    struct write_t
    {
        void*            buffer;
    ...
    bool readMemory(AddressSet::ptr addr, std::multimap<Process::ptr, void*>& result,
                    size_t size) const;
    bool readMemory(AddressSet::ptr addr, std::map<void*, ProcessSet::ptr>& result,
                    size_t size, bool use_checksum = true) const;
     ...
     *processes. The user may pass a single example IRPC, copies of which are posted to
     *each process and returned. The user may pass a single example IRPC and an
     *AddressSet. Copies of that IRPC are posted to each process at the addresses
     *provided.
     **/
...
/**
 * Return a unique ProcessSet representing every running process.  It is an error to try
 *to manually modify this ProcessSet.  Modifications are done automatically by
 *ProcControlAPI as processes are created and destroyed.
 **/
...
public:
     ...
     *ThreadSet::ptr>' groups processes based on similar return values of registers.
     **/
     ...
     * The user may pass a single example IRPC, copies of which are posted to each process
     *and returned.
     **/
