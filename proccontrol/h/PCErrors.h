...
#define perr_printf(format, ...)                                        \
  do {                                                                  \
    if (dyninst_debug_proccontrol) {                                    \
      PC_PRINT_LOCK;                                                    \
      fprintf(pctrl_err_out, "[%s:%u-%s@%lu] - Error: " format, FILE__, \
...
#define pthrd_printf(format, ...)                                      \
  do {                                                                 \
    if (dyninst_debug_proccontrol) {                                   \
      PC_PRINT_LOCK;                                                   \
      fprintf(pctrl_err_out, "[%s:%u-%s] - " format, FILE__, __LINE__, \
...
#define perr_printf(format, ...)                                              \
  do {                                                                        \
    if (dyninst_debug_proccontrol) {                                          \
      PC_PRINT_LOCK;                                                          \
      fprintf(pctrl_err_out, "[%s:%u-%s] - Error: " format, FILE__, __LINE__, \
              thrdName(), ##__VA_ARGS__);                                     \
      PC_PRINT_UNLOCK;                                                        \
    }                                                                         \
  } while (0)
...
#define parmerr_printf(format, ...)                                     \
  do {                                                                  \
    if (dyninst_debug_proccontrol) {                                    \
      PC_PRINT_LOCK;                                                    \
      fprintf(pctrl_err_out, "[%s:%u-%s] - Error-ARM: " format, FILE__, \
              __LINE__, thrdName(), ##__VA_ARGS__);                     \
      PC_PRINT_UNLOCK;                                                  \
    }                                                                   \
  } while (0)
